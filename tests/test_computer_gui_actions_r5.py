...
def changed(payload, *, binding=True, change=True):
    return {"status": "verified", "injected": True, "released": True,
            "postcondition": {"type": "visual_change", "method": "raster_digest_after_release",
                "target_application_matches": binding,
                "actual": {"before_sha256": "a" * 64,
                           "after_sha256": ("b" if change else "a") * 64},
...
async def test_web_operator_owner_read_cross_channel_not_model_authority(tmp_path):
    async with setup(tmp_path) as (controller, backend, ctx, inp):
        operator = RequestContext(ctx.owner_id, "web-session", "web-operator", ctx.host_id,
                                  surface="webui")
