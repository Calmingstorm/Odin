...
def test_injector_independent_dual_ledger_releases_on_controller_eof(monkeypatch):
    ...
    stream = InjectorStream(
        b'{"display_name":":fake","mode":"independent",'
        b'"expected_device_identity":[71,72]}\n',
...
def _execute_environment(monkeypatch, native, *, mode="independent", expected=(71, 72)):
    ...
    connection = SimpleNamespace(
        ...
        named_sources=lambda _topology, _names: [selected], _display=object(), close=Mock())
    scope = SimpleNamespace(assert_snapshot=Mock())
    config = {"display_name": ":fake", "xauthority": "", "monitor_names": ["screen"]}
    modules = {
        "x11_attached": SimpleNamespace(attachment_configuration=lambda *_args: config,
        ...
        "x11_owned_device": SimpleNamespace(open_input=lambda *_args, **_kwargs: native,
    ...
    request = {**config, "selected": selected,
               "scope": {"rect": [0, 0, 100, 100], "focus_window": 91},
               "input_mode": mode, "expected_device_identity": expected,
...
def test_execute_rejects_unadmitted_mode_or_identity_before_injection(monkeypatch, field, value, reason):
    ...
    request[field] = value
    with pytest.raises(guardian.GuardianFailure, match=reason):
        guardian.execute(request, controller_fd=None)
...
def _worker_request():
    return {"display_name": ":fake", "xauthority": "", "monitor_names": ["screen"],
            "operation": "sources"}
...
def test_watch_topology_emits_only_state_changes_then_closes(monkeypatch, capsys):
    ...
    assert [item["event"] for item in replies] == [
        "topology_ready", "topology_changed", "topology_changed"]
