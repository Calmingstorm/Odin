...
class TestReexecVetoInMain:
    ...
    def test_main_exits_instead_of_execing_when_vetoed(self, entry_point):  # noqa: F811
        ...
        # Real loop, private config and fake service/gateway boundaries. The
        # old stub passed only when an earlier test made main exit prematurely.
        with patch("os.execve") as execve, patch.object(
            restart, "reexec", side_effect=AssertionError("must not exec")
        ):
...
from tests.test_main_exit_codes import entry_point  # noqa: E402, F401
