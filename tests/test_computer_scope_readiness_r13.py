...
async def test_start_selects_eligible_monitor_and_reports_real_readiness(monkeypatch, eligible):
    backend = X11AttachedBackend(enabled=True, display_name=":177", xauthority="/dev/null",
                                 monitor_names=["left", "right"], input_enabled=True)
    monitors = [{"name": name, "index": i, "width": 800, "height": 600}
...
@pytest.mark.parametrize("state,age,expected", [
    ("active", 0, "target_available"), ("active", 6, "observation_required"),
    ("paused", 0, "inactive"), ("cancelled", 0, "inactive"),
])
def test_controller_readiness_never_reuses_expired_or_inactive_authority(state, age, expected):
    ...
    live = SimpleNamespace(backend=SimpleNamespace(input_readiness="target_available",
...
@pytest.mark.parametrize("blocker,visible", [("application_uid_mismatch", True),
                                          ("private process details", False)])
async def test_http_retains_only_known_scope_failure_reasons(tmp_path, blocker, visible):
    async with harness(tmp_path) as h:
        ...
        async def status(**actor):
            return {**await original(**actor), "backend": {
                "platform": "x11", "environment": "existing_session", "input_supported": False,
                "readiness": "no_input_target", "input_blocker": blocker}}
...
def test_worker_readiness_discovers_targets_without_capture_or_input(monkeypatch):
    ...
    checker = Mock()
    checker.inspect.side_effect = [(None, "focused_application_outside_source"),
                                   ({"private_native_id": 123}, None)]
    monkeypatch.setattr(scope, "AppScope", lambda *_: checker)
    reply = worker.run({"display_name": ":177", "xauthority": "/dev/null",
                        "monitor_names": ["left", "right"], "operation": "scope_readiness"},
                       capture)
