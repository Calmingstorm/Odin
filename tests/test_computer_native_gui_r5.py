...
@pytest.mark.parametrize("kind,fields", [
    ...
    ("click", {"x": True}), ("click", {"x": 25.0}),
    ("click", {"button": "right"}), ("click", {"expected_modal": "invented"}),
])
...
@pytest.mark.parametrize("policy,token,admitted", [
    ("safe_application", "correct", True), ("safe_application", "stale", False),
    ("unknown", "correct", False), ("denied", "correct", False),
    (None, "correct", False), ("safe_application", None, False),
])
...
@pytest.mark.parametrize("change,kind", [
    ("none", "safe_application"), ("foreign", "unknown"),
    ("terminal", "denied"), ("password", "denied"), ("buttons", "unknown"),
])
def test_drawing_onboarding_classification_is_bounded(change, kind):
    ...
    nodes = [{"role": "alert", "name": "Information"},
...
def test_pointer_native_tree_proof_rejects_sibling(monkeypatch, path, expected):
    ...
    closed = []
    monkeypatch.setattr(display, "Display", lambda target: SimpleNamespace(
...
@pytest.mark.asyncio
@pytest.mark.parametrize("kind,fields", [
    ("type", {"text": "test"}), ("key", {"chord": "Return"}),
...
async def test_native_through_adapter_visual_proof(kind, fields):
    ...
    receipt = await backend.act(dict(type=kind, source_id=frame.source.source_id,
