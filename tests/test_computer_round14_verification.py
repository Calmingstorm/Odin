...
@pytest.mark.parametrize("measurement,transition,status", [
...
async def test_pointer_postcondition_does_not_cancel_acknowledged_click(
    ...
    async with fixture(tmp_path, monkeypatch) as (controller, ctx, action, state, calls):
        ...
        assert await controller.act(ctx, action) == result and len(calls) == 1
...
def test_guardian_queries_pointer_only_after_fence_and_release(monkeypatch, outcome):
    ...
    monkeypatch.setattr(sys.modules["src.computer.runtime.x11_attached"],
                        "same_application_scope", same_application_scope, raising=False)
    scope.snapshot = Mock()
    binding = {"process": {"pid": 123, "start_ticks": 10}, "window": 90,
               "focus_window": 91, "rect": [0, 0, 100, 100], "focused": True,
               "topology": "one", "source_rect": [0, 0, 100, 100],
               "source_origin": [0, 0], "modal_kind": None}
    ...
    if outcome == "unavailable":
        ...
        assert result["pointer_observation"] == {
            "x": 10, "y": 20, "target_window_matches": outcome == "same"}
...
async def test_fresh_crop_survives_model_planning_and_maps_click_exactly(tmp_path, monkeypatch):
    async with fixture(tmp_path, monkeypatch) as (controller, ctx, action, state, calls):
        ...
        async def capture(operation, **kwargs):
            ...
            frame = render_frame(pixels, source, mode="RGB", observation_id="worker",
                session_id="worker", generation=1, captured_monotonic_ns=1,
                crop=FrameCrop(**kwargs["crop"]) if kwargs.get("crop") else None)
            reply.update(width=frame.metadata.width, height=frame.metadata.height,
                         resize_scale=frame.metadata.resize_scale,
                         delivered_to_source=frame.metadata.delivered_to_source.public(),
                         crop=(list(vars(frame.metadata.crop).values())
                               if frame.metadata.crop else None),
                         image=base64.b64encode(frame.png).decode())
            return reply
        monkeypatch.setattr(backend, "_read_worker", capture)
        crop = {"x": 10, "y": 4, "width": 16, "height": 8}
        observed = await controller.observe(ctx, {"session_id": action["session_id"],
                                                 "generation": 1, "crop": crop})
        ...
        plan = plan_model_frames([{"role": "user", "content": [full["__image_block__"]]},
                                  {"role": "user", "content": [image["__image_block__"]]}])
        assert plan.frame_count == 1 and plan.protected_message_indices == (1,)
        assert (plan.messages[1]["content"][0]["source"]["data"]
        ...
        images = [block for item in wire for block in item.get("content", [])
                  if block.get("type") == "input_image"]
        assert len(images) == 1
        assert base64.b64decode(images[0]["image_url"].split(",", 1)[1]) == observed["image_bytes"]
        action.update(observation_id=obs.observation_id, source_revision=obs.source.source_revision,
                      operation="click", x=0, y=0)
        result = await controller.act(ctx, action)
...
@pytest.mark.parametrize("target_state,focused,status", [
    ("destroyed", False, "verified"), ("unmapped", False, "verified"),
    ("destroyed", True, "verified"), ("present", True, "not_satisfied"),
    ("unavailable", True, "not_satisfied"), ("replaced", True, "not_satisfied"),
])
...
async def test_native_disappearance_not_just_focus_change_satisfies_close(
    ...
    async with fixture(tmp_path, monkeypatch) as (controller, ctx, action, state, calls):
        ...
        action.update(operation="key", key="alt+F4")
        result = await controller.act(ctx, action)
        assert result["status"] == status
        assert result["execution"] == {"injected": True, "released": True}
        assert result["verification"].get("target_disappeared", False) == (status == "verified")
        if status == "verified":
            assert result["verification"]["type"] == "target_disappeared"
            assert result["verification"]["scope"] == "native_window_presence_only"
        assert controller.store.get_session(action["session_id"]).state == "active"
        assert await controller.act(ctx, action) == result and len(calls) == 1
...
@pytest.mark.parametrize("mode", ["present", "unmapped", "destroyed", "replaced",
                                  "topology", "error"])
def test_prior_target_native_query_does_not_confuse_unfocused_with_gone(app, monkeypatch, mode):
    ...
    if mode == "unmapped":
        ...
        class Gone(error.BadWindow):
            def __init__(self):
                pass
        display.target.get_attributes = Mock(side_effect=Gone() if mode == "destroyed"
                                              else OSError("native failure"))
...
def test_capture_worker_reports_native_target_state_not_focus_absence(native, monkeypatch, failed):
    ...
    verify = {"window": 20}
    query = Mock(side_effect=OSError("query failed") if failed else None,
                 return_value="destroyed")
    monkeypatch.setattr(x11_app_scope.AppScope, "target_state", query)
    result = worker.run(request("capture", selected=selected, input_enabled=True,
                                verify_scope=verify))
...
async def test_pointer_post_capture_unavailable_is_not_unknown_input(tmp_path, monkeypatch):
    async with fixture(tmp_path, monkeypatch) as (controller, ctx, action, state, calls):
        ...
        async def measured(request):
            ...
            return {**receipt, "pointer_observation": {
                "x": 105, "y": 205, "target_window_matches": True}}
        monkeypatch.setattr(backend, "_input_worker", measured)
        action.update(operation="click", x=2, y=2, expect={"type": "pointer_at", "x": 2, "y": 2})
        result = await controller.act(ctx, action)
