...
IDENTITY_SCOPE = {**SCOPE, "wm_class": "ordinary-app", "application": {
    "pid": 456, "uid": 1000, "start_ticks": 123, "exe": "/home/operator/.local/bin/app",
    "exe_identity": [1, 2], "cmdline_digest": "b" * 64, "trusted_executable": False,
    "script_identity": None}}


@pytest.mark.parametrize("size,crop,dimensions,scale", [
    ((3840, 2160), None, (1600, 900), Fraction(5, 12)),
    ((3840, 2160), {"x": 123, "y": 59, "width": 3200, "height": 1801},
     (1600, 901), Fraction(1, 2)),
    ((5120, 1440), {"x": 1900, "y": 101, "width": 3200, "height": 1001},
     (1600, 501), Fraction(1, 2)),
    ((5120, 1440), {"x": 2500, "y": 200, "width": 801, "height": 601},
     (801, 601), Fraction(1)),
])
async def test_controller_delivers_exact_crop_transform_and_dispatches(
    ...
    async def capture(portal, node_id):
        return {"source_metadata": dict(portal.source), "image": output.getvalue(),
                "width": size[0], "height": size[1], "captured_at": time.monotonic(),
                "clock_verified": True, "generation": portal.current_generation}
    ...
    try:
        ...
        adapter._scope_provider.scope = {**IDENTITY_SCOPE, "bounds": {
        ...
        action = {**args, "operation": "click", "x": 17, "y": 23, "action_id": "click",
                  "observation_id": obs.observation_id, "source_id": obs.source.source_id,
                  "source_revision": obs.source.source_revision, "consent_generation": 1,
        ...
        x, y = obs.source.input_point(metadata.delivered_to_source, 17, 23,
...
@pytest.mark.parametrize("modal,expected", [(False, None), (True, "a" * 64),
                                            (True, None), (True, "wrong")])
async def test_controller_unicode_preflight_and_ordinary_modal(
        tmp_path, adapter, modal, expected):
    store = ComputerStore(tmp_path / "db", tmp_path / "evidence")
    controller = ComputerController(store, lambda _: adapter, lambda _: True, enabled=True)
    context = RequestContext("operator", "channel", "turn", "host")
    try:
        ...
        if modal:
            adapter._scope_provider.scope = {**IDENTITY_SCOPE, "modal": True,
                "modal_title_digest": "a" * 64, "modal_kind": "safe_application"}
        calls = []
        async def reject(command):
            calls.append(command)
            error = WaylandGuardianError("unsupported_character")
            error.details = {"event": "action_rejected", "input_was_sent": False,
        ...
        action = {**args, "operation": "type", "text": "😀", "action_id": "unicode",
                  "observation_id": obs.observation_id, "source_id": obs.source.source_id,
                  "source_revision": obs.source.source_revision, "consent_generation": 1,
                  "expect": {"type": "visual_change"}}
