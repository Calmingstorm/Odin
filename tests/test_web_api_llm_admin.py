...
# --------------------------------------------------------------------------- #
class TestMaskedPlaceholderIsRefused:
    """Status reads report secrets as ••••••••.

    A client that renders one as an editable control sends it straight back.
    Before this fence these routes returned 200, installed eight bullets as the
    live API key, and persisted it — destroying the credential through the
    ordinary save path. The generic /api/config route already refused it; every
    route accepting a secret has to, or the fence covers one door of several.
    """

    MASK = "•" * 8

    @pytest.mark.asyncio
    @pytest.mark.parametrize("section", ["ollama", "kimi"])
    async def test_a_masked_api_key_is_refused_and_the_real_one_survives(self, section):
        app, bot = _app(register_provider_config)
        _gw(bot)
        setattr(getattr(bot.config, section), "api_key", "REAL-KEY")

        async with TestClient(TestServer(app)) as c:
            resp = await c.put(f"/api/llm/{section}/config", json={"api_key": self.MASK})
            body = await resp.json()

        assert resp.status == 400
        assert "redacted placeholder" in body["error"]
        assert getattr(bot.config, section).api_key == "REAL-KEY"

    @pytest.mark.asyncio
    async def test_the_mask_is_refused_anywhere_in_the_body(self):
        app, bot = _app(register_provider_config)
        _gw(bot)
        bot.llm_gateway.codex_client = object()
        async with TestClient(TestServer(app)) as c:
            resp = await c.put("/api/llm/codex/config", json={"model": self.MASK})
        assert resp.status == 400

    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        ("registrar_name", "method", "route"),
        [
            ("register_provider_config", "put", "/api/llm/codex/config"),
            ("register_provider_config", "put", "/api/llm/auxiliary/config"),
            ("register_provider_config", "put", "/api/llm/ollama/config"),
            ("register_provider_config", "put", "/api/llm/kimi/config"),
            ("register_llm_provider", "post", "/api/llm/switch"),
            ("register_ollama_admin", "post", "/api/ollama/model"),
            ("register_kimi_admin", "post", "/api/kimi/model"),
        ],
    )
    async def test_every_body_accepting_route_refuses_the_mask(
        self, registrar_name, method, route
    ):
        """Uniformity is the point. A fence on some doors is the shape of the
        original bug: the generic save refused it while two dedicated routes
        installed it as the live key."""
        import src.web.api.llm_admin as mod

        registrar = getattr(mod, registrar_name, None)
        if registrar is None:
            pytest.skip(f"{registrar_name} not present")
        app, bot = _app(registrar)
        _gw(bot)
        bot.llm_gateway.codex_client = object()
        async with TestClient(TestServer(app)) as c:
            resp = await getattr(c, method)(route, json={"model": self.MASK})
        assert resp.status == 400, f"{route} accepted the redaction mask"

    @pytest.mark.asyncio
    async def test_a_real_secret_still_goes_through(self):
        """The fence must reject the mask only — replacing a secret has to
        keep working, since no dedicated set/clear route exists yet."""
        app, bot = _app(register_provider_config)
        _gw(bot)
        bot.config.kimi.api_key = "OLD"
        async with TestClient(TestServer(app)) as c:
            resp = await c.put("/api/llm/kimi/config", json={"api_key": "NEW-REAL-KEY"})
        assert resp.status == 200
        assert bot.config.kimi.api_key == "NEW-REAL-KEY"
