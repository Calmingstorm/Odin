...
@pytest.mark.parametrize('field,value', [
    ('display', 'localhost:0'), ('xauthority', 'relative'),
    ('xauthority', '/tmp/bad\x00'), ('monitor_names', ['DP-1', 'DP-1']),
    ('monitor_names', 'DP-1'), ('storage_dir', 'relative'),
])
...
def test_explicit_local_desktop_bindings_roundtrip():
    config = ComputerUseConfig(display=':20', xauthority='/tmp/isolated-auth',
                              monitor_names=['DP-1'], storage_dir=' /tmp/private ')
...
async def test_generic_config_refuses_computer_without_any_publication(computer):
    ...
    assert body['error'] == 'Computer settings are read-only on this route'
...
def test_missing_previously_required_frame_retires_all_computer_evidence(monkeypatch):
    ...
    monkeypatch.setattr('src.computer.vision.plan_model_frames',
                        lambda value: SimpleNamespace(messages=value))
