"""Startup approval cannot spread to changed pixels, processes or unsafe labels."""
import pytest
...
@pytest.mark.parametrize('change,expected', [
    ('missing_buttons', 'safe_application'), ('pixels', 'unknown'),
    ('window', 'unknown'), ('process', 'unknown'), ('foreign', 'unknown'),
    ('new_label', 'unknown'), ('password', 'denied'), ('terminal', 'denied'),
    ('security_text', 'denied'), ('empty', 'unknown'), ('unapproved', 'unknown'),
])
def test_same_startup_missing_descendants_require_exact_prior_proof(change, expected):
    ...
    full = [{'role': 'alert', 'name': 'Information'},
