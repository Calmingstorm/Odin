...
## Round 44 — Context auto-compression with prompt caching (static prefix caching)
**Focus**: Implement context auto-compression for multi-iteration tool loops — compress older tool iterations when context exceeds a character budget, and track static prefix stability for LLM provider-side KV cache reuse.
**Baseline pytest**: 4274 passed, 0 failed
**Post-round pytest**: 4372 passed, 0 failed (+98 new tests)

### Prior round review
- Round 43 notes said "Round 44 (context auto-compression with prompt caching) has no dependencies on Round 43." Confirmed — no issues to fix from prior rounds.
- Checked rounds 41–43 "watch for" items: hedging exemption precedence (round 41), stuck loop detector not yet wired into runtime (round 42), duplicated `_truncate_smart` across tools/discord (round 43). None required fixes for this round's scope.

### Design overview

During a tool loop, messages accumulate with each iteration:
```
[system prompt] + [history…] + [tool_use₁ + tool_result₁] + [tool_use₂ + tool_result₂] + …
```

The system prompt + initial history messages form the **static prefix** — identical across all iterations within a single tool loop. When this prefix stays unchanged, LLM providers (OpenAI, etc.) can cache the KV computation for the prefix, reducing latency and cost on subsequent iterations.

The **dynamic suffix** (tool iterations) grows with each tool call. When it exceeds a character budget, older iterations are compressed into a compact summary, keeping the most recent N iterations intact. This reduces context size while preserving the static prefix for cache reuse.

**Two components:**
1. **PrefixTracker**: Hashes system prompt + prefix messages on each call. Reports cache hits (prefix stable) and misses (prefix changed). Detects anti-patterns like regenerating datetime in the system prompt on every iteration.
2. **compress_tool_context**: Splits messages into prefix + iterations, compresses older iterations into `[Earlier tool calls: run_command→OK; read_file→OK]` summaries. The prefix is **never modified** — this is the key design constraint for prompt caching.

### Work done

#### 1. New module: `src/llm/context_compressor.py` (239 lines)

**`CompressionStats`** (line 28, `@dataclass`):
- Counters: `compressions`, `iterations_compressed`, `chars_saved`, `prefix_hits`, `prefix_misses`, `total_checks`
- `as_dict()` method returns all counters + computed `prefix_hit_rate`

**`PrefixTracker`** (line 56, class with `__slots__`):
- `__init__(stats=None)`: Accepts optional shared `CompressionStats`
- `check(system, prefix_messages) -> bool`: Hashes system prompt + prefix messages via SHA-256 (truncated to 16 hex chars). Returns `True` on cache hit (prefix matches previous call). First call always returns `False`.
- `reset()`: Clears stored hash (call between separate tool loops)
- `stats` property: Access the underlying `CompressionStats`

**`_hash_prefix(system, messages) -> str`** (line 94): Deterministic SHA-256 hash of system prompt + message roles + contents. List-type content (tool blocks) serialized via `json.dumps(sort_keys=True)`. Returns 16-char hex string.

**Helper functions** (lines 108–139):
- `_is_tool_message(msg)`: True if content contains `tool_use` or `tool_result` blocks
- `_is_tool_use_message(msg)`: True if content contains `tool_use` blocks
- `_is_tool_result_message(msg)`: True if content contains `tool_result` blocks

**`split_prefix_and_iterations(messages)`** (line 147): Splits message list into:
- `prefix`: All messages before the first tool_use/tool_result block (history, summary pair, user query)
- `iterations`: List of message groups, each starting with a tool_use message and containing subsequent messages until the next tool_use (handles both sequential and parallel tool calls)

**`estimate_message_chars(messages) -> int`** (line 176): Estimates total character payload across a message list. Handles string content, list content blocks (tool_use/tool_result), nested dict inputs, and role strings.

**`summarize_iteration(iteration) -> str`** (line 198): Produces compact `tool_name→OK/ERR` summaries for one iteration. Detects errors via 7 known error prefixes. Handles parallel tools (multiple tool_use blocks per message), list-type result content, and non-string results. Truncates at `COMPRESSED_ITERATION_MAX_CHARS` (120 chars).

**`compress_tool_context(messages, *, max_context_chars, keep_recent, stats)`** (line 237): Main entry point. When `estimate_message_chars(messages) > max_context_chars`:
1. Splits into prefix + iterations via `split_prefix_and_iterations`
2. If `len(iterations) <= keep_recent`, returns unchanged (nothing to compress)
3. Replaces older iterations with a single summary message: `[Earlier tool calls: cmd→OK; read_file→ERR]`
4. Returns `(compressed_messages, iterations_compressed)`

**Constants**:
- `DEFAULT_MAX_CONTEXT_CHARS = 48_000` (~12k tokens at 4 chars/token)
- `DEFAULT_KEEP_RECENT = 3` iterations preserved intact
- `COMPRESSED_ITERATION_MAX_CHARS = 120` per-iteration summary cap

#### 2. Config: `src/config/schema.py` (line 87)

**`ContextCompressionConfig(BaseModel)`**:
- `enabled: bool = True`
- `max_context_chars: int = 48_000`
- `keep_recent_iterations: int = 3`

Added as `context_compression` field on `OpenAICodexConfig` (line 149).

#### 3. REST API: `src/web/api.py` (line 2202)

**`GET /api/compression/stats`**: Returns `CompressionStats.as_dict()` — live counters for compression events and prefix cache efficiency.

REST API endpoint count is now **125** (was 124).

### Tests: `tests/test_context_compressor.py` — 98 tests across 16 test classes

**TestIsToolMessage** (8): Text not tool, tool_use detected, tool_result detected, empty list, non-list content, mixed blocks, text-only list, missing content key.

**TestToolUseResultClassification** (6): tool_use true, false on result, tool_result true, false on use, false on text (both).

**TestHashPrefix** (8): Same input same hash, different system, different messages, different role, empty messages, list content, deterministic, hash length.

**TestSplitPrefixAndIterations** (8): No tool messages, simple split, multiple iterations, prefix with summary, empty messages, only tool messages, partial iteration, tool_result grouped with use.

**TestEstimateMessageChars** (7): String content, tool_use, tool_result, empty, multiple messages, nested dict, list result content.

**TestSummarizeIteration** (11): Single tool OK, single error, multiple tools, truncation, empty, text-only, all error prefixes, Command failed, list result content, non-string result, missing outcome.

**TestCompressToolContextBasic** (7): Under budget, no iterations, too few iterations, compresses old, prefix preserved, recent preserved, summary inserted.

**TestCompressToolContextStats** (3): Stats updated, not updated when no compression, cumulative.

**TestCompressToolContextEdgeCases** (7): Empty messages, keep_recent == iterations, exceeds iterations, original not modified, all-but-keep compressed, default parameters, context actually shrinks.

**TestPrefixTracker** (10): First miss, second hit, different system miss, different messages miss, reset, stats tracking, stats property, consecutive hits, realistic tool loop, changing system prompt.

**TestCompressionStats** (6): Initial zeros, as_dict keys, zero hit rate, calculated hit rate, 100% hit rate, cumulative updates.

**TestContextCompressionConfig** (4): Default config, custom config, in codex config, full config parse.

**TestConstants** (4): Default max chars, default keep recent, compressed max chars, error prefixes tuple.

**TestImports** (2): All public symbols, _hash_prefix importable.

**TestRealisticScenarios** (4): Deploy scenario (8 tools, errors), mixed tools, small conversation, tracker + compression together.

**TestMultiToolIterations** (2): Parallel tools one iteration, compress parallel tools.

**TestAPIEndpoint** (1): Stats dict JSON-serializable.

### Files changed
- `src/llm/context_compressor.py` (new, 239 lines): `CompressionStats`, `PrefixTracker`, `compress_tool_context`, `split_prefix_and_iterations`, `estimate_message_chars`, `summarize_iteration`.
- `src/config/schema.py` (lines 87–91, 149): `ContextCompressionConfig` class, added to `OpenAICodexConfig`.
- `src/web/api.py` (lines 2202–2210): `GET /api/compression/stats` endpoint.
- `tests/test_context_compressor.py` (new, 98 tests): 16 test classes.

### Design decisions

1. **Local compression, no LLM call**: Compression uses deterministic `tool_name→OK/ERR` summaries, not an LLM summarization call. This avoids adding latency to the tool loop and keeps the compressor free of external dependencies. The LLM-based session compaction (in `sessions/manager.py`) handles inter-session summarization; this module handles intra-loop compression.

2. **Prefix never modified**: The key constraint for prompt caching. All compression operates on the dynamic suffix (tool iterations) only. The prefix (history + user query) is returned verbatim. This means providers that automatically cache common prefixes (OpenAI, Anthropic) get maximum reuse.

3. **Character budget, not token budget**: The module uses character counts rather than token estimates for the budget threshold. This is simpler and avoids importing the token estimator into a module that should be lightweight. At ~4 chars/token, 48,000 chars ≈ 12,000 tokens — well within the useful range for tool loops.

4. **Summary as user message**: The compressed summary is inserted as a `role: user` message (not `assistant`). This follows the convention used by session summaries in `sessions/manager.py` — summaries are context injections, not LLM-generated responses.

5. **Iteration grouping heuristic**: Each "iteration" starts at a `tool_use` message and extends to the next `tool_use`. This naturally groups `[tool_use, tool_result]` pairs and handles parallel tool calls (multiple tool_use blocks in one message) as a single iteration.

6. **Error detection via prefix matching**: The `summarize_iteration` function detects errors by checking if tool results start with known error prefixes (`Error`, `Command failed`, `Timeout`, etc.). This matches the error detection patterns used in `result_validator.py` and `background_task.py`.

7. **PrefixTracker as separate class**: The tracker is decoupled from compression — callers can use either or both. The tracker provides observability into whether the system prompt is being regenerated unnecessarily (e.g., new timestamps on every iteration).

8. **NOT wired into tool loop runtime**: Following the pattern of prior rounds (Round 39 auxiliary client, Round 42 stuck loop detector), the module is implemented as a standalone library with full tests. Wiring into the tool loop requires: (a) create `CompressionStats` on bot init, (b) create `PrefixTracker` at tool loop start, (c) call `compress_tool_context()` before each `chat_with_tools()` call, (d) freeze the system prompt for the duration of a tool loop (build once, reuse across iterations).

### Next round watch for
- Round 45 (smart model routing) has no dependencies on Round 44.
- REST API endpoint count is now 125 (was 124). One new endpoint: `/api/compression/stats`.
- The `compress_tool_context` function is NOT yet called anywhere in runtime code. Wiring into the tool loop requires:
  - (a) Freeze system prompt at loop start (avoid regenerating datetime on each iteration).
  - (b) Create a `PrefixTracker` per tool loop invocation.
  - (c) Call `compress_tool_context(messages, max_context_chars=cfg.context_compression.max_context_chars, keep_recent=cfg.context_compression.keep_recent_iterations)` before each `chat_with_tools()` call.
  - (d) Pass `stats=bot.compression_stats` for observability.
- The `PrefixTracker` is designed to detect when the system prompt changes between iterations. Currently, `_format_datetime()` in `system_prompt.py` returns a new timestamp every second, so naively rebuilding the system prompt on each tool iteration would cause 0% prefix cache hits. The fix is to call `build_system_prompt()` once at the start of the tool loop and reuse the result.
