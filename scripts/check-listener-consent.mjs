...
console.warn = warn;
const calls = [];
const consentRequest = api.consentListener.bind(api);
let settle;
api.consentListener = (token) => {
  calls.push({ token });
...
await state.saveListenerConsent();
assert.deepEqual(calls, [{ token: 'one-shot-admin' }]);
...
api.consentListener = async () => { throw new Error('authenticated admin access required'); };
await state.saveListenerConsent();
...
assert.deepEqual(await consentRequest('fresh-admin-credential'), { persisted: true });
...
assert.deepEqual(JSON.parse(requests[0].options.body), { expose_beyond_loopback: true });
