...
def typed_event(event, *, key_event=False, button_event=False):
    ...
    return dict(key=scalar('get_keyval', int, True) if key_event else None,
                button=scalar('get_button', int, True) if button_event else None,
                x=float(x) if ok else None, y=float(y) if ok else None,
...
def main():
    ...
    def log(kind, **values):
        print(json.dumps(dict(kind=kind, monotonic=time.monotonic(), pid=os.getpid(),
                              **values)), flush=True)
    ...
    def event(_win, ev):
        try:
            ...
            if ev.type == Gdk.EventType.KEY_PRESS: pressed_keys.add(data['key'])
            if ev.type == Gdk.EventType.KEY_RELEASE: pressed_keys.discard(data['key'])
            if ev.type == Gdk.EventType.BUTTON_PRESS: pressed_buttons.add(data['button'])
            if ev.type == Gdk.EventType.BUTTON_RELEASE: pressed_buttons.discard(data['button'])
            device = ev.get_device()
            log('event', type=str(ev.type), **data,
                keys=sorted(pressed_keys), buttons=sorted(pressed_buttons),
    ...
    def sample():
        ...
        log('sample', pointer_x=int(result[-3]), pointer_y=int(result[-2]), state=int(result[-1]),
            state_source='Gdk.Window.get_device_position(default_seat.pointer)',
            keys=sorted(pressed_keys), buttons=sorted(pressed_buttons),
            text=entry.get_text(), focused=entry.has_focus(),
            active=win.is_active(), toplevel_focus=win.has_toplevel_focus())

    win.show_all()
    entry.grab_focus()
    log('receiver_start', backend=type(Gdk.Display.get_default()).__name__)
    GLib.timeout_add(100, lambda: recurring(sample, lambda reason:
        log('telemetry_error', where='sample', reason=reason)))
    Gtk.main()
