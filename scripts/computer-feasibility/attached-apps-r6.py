...
def outer(args):
    ...
    for name in ('x11-run.py','x11_records.py','x11-passwd','x11-group',
                 ...
                 'x11-crossuid-bootstrap.sh','x11-owned-bus.conf','r6_reaper.py'):
    ...
    subprocess.run(['sudo','-n','install','-d','-m','755','-o','0','-g','0',str(workspace)],check=True,timeout=5)
    ...
    runner = runner.replace("'MemoryMax=1G'", "'MemoryMax=2G'").replace("'RuntimeMaxSec=120'", "'RuntimeMaxSec=240'")
    runner = runner.replace("'--ro-bind', '/etc/fonts', '/etc/fonts',", "'--ro-bind', '/etc/fonts', '/etc/fonts', '--ro-bind', '/etc/libreoffice', '/etc/libreoffice',")
    runner = runner.replace("'--size', '268435456', '--tmpfs', '/workspace',", "'--bind', '/r6-output', '/workspace',")
    runner = runner.replace("f'BindReadOnlyPaths={root}:/xi2-source']", "f'BindReadOnlyPaths={root}:/xi2-source', 'BindPaths="+str(workspace)+":/r6-output']")
    runner = runner.replace("'XI2_PRIVATE_SANDBOX': '1'", "'XI2_PRIVATE_SANDBOX': '1', 'SAL_USE_VCLPLUGIN': 'gtk3'")
    (harness/'x11-run.py').write_text(runner)
    result = 1
    try:
        with (out/'runner.log').open('w') as log:
            p = subprocess.Popen(['/usr/bin/python3',str(harness/'x11-run.py'),
...
async def exercise(task, app, d):
    ...
    class EvidenceBackend(X11AttachedBackend):
        async def act(self,payload):
            try:
                ...
                raise
    backend = EvidenceBackend(enabled=True,display_name=':177',monitor_names=['screen'],
                                app_profile=profile,input_enabled=True,runtime_sudo=True)
    ...
    def census():
        ...
        for xid in prop.value if prop else []:
            ...
            rows.append({'xid':int(xid),'title':bytes(t.value).decode('utf-8') if t else w.get_wm_name(),
                         'wm_class':w.get_wm_class(),'transient':parent.id if parent else None,
    ...
    async def observe():
        ...
        r = await ctl.observe(ctx,{'session_id':current.session_id,'generation':current.generation})
    ...
    async def act(operation,**fields):
        for attempt in range(5):
            try:
                ...
                if str(exc) not in {'visual_target_changed','stale_source_binding'} or attempt == 4:
                    raise
    ...
    try:
        ...
        if task == 'writer':
            ...
            bold_styles = {s.get('{'+ns['style']+'}name') for s in document.findall('.//style:style',ns)
                           if s.find('style:text-properties',ns) is not None
                           and s.find('style:text-properties',ns).get('{'+ns['fo']+'}font-weight') == 'bold'}
            spans = paragraphs[1].findall('.//text:span',ns)
            assert (paragraphs[1].get('{'+ns['text']+'}style-name') in bold_styles
...
def inner():
    ...
    def spawn(name,argv):
        ...
        p = subprocess.Popen(['setpriv','--reuid=65534','--regid=65534','--clear-groups',
                              '--bounding-set=-all','--no-new-privs',*argv],stdout=log,stderr=log)
    ...
    try:
        ...
        if task == 'inkscape':
            ...
            settings.write_text('<inkscape version="1"><group id="options"><group id="boot" enabled="0"/></group></inkscape>')
        ...
        if task != 'inkscape':
            ...
            settings.write_text('<?xml version="1.0" encoding="UTF-8"?><oor:items xmlns:oor="http://openoffice.org/2001/registry"><item oor:path="/org.openoffice.Office.Common/Misc"><prop oor:name="ShowTipOfTheDay" oor:op="fuse"><value>false</value></prop></item></oor:items>')
            for path in (profile.parent,profile,settings):
                os.chown(path,65534,65534)
        Path('/tmp/.X11-unix').mkdir(mode=0o1777)
        os.chmod('/tmp/.X11-unix',0o1777)
        spawn('xvfb',['Xvfb',':177','-screen','0','1280x900x24','-nolisten','tcp','-noreset','-extension','GLX'])
        wait(lambda:subprocess.run(['xdpyinfo','-display',':177'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL).returncode == 0)
        ...
        app_args = ['inkscape'] if task == 'inkscape' else [
            'soffice','-env:UserInstallation=file:///workspace/home/profile',
            '--norestore','--nofirststartwizard','--'+('writer' if task == 'census' else task)]
        app = spawn('app',['dbus-run-session','--config-file=/harness/x11-owned-bus.conf','--',*app_args])
        def find():
            prop = d.screen().root.get_full_property(d.intern_atom('_NET_CLIENT_LIST'),0)
            for xid in prop.value if prop else []:
                w = d.create_resource_object('window',xid)
                if any(p in str(w.get_wm_class()).lower() for p in ('libreoffice','inkscape')) and w.get_attributes().map_state == X.IsViewable:
                    return w
        target = wait(find)
        subprocess.run(['xdotool','windowsize',str(target.id),'1200','840'],check=True,timeout=3)
        ...
        Path('/workspace/inner.json').write_text(json.dumps({'passed':passed,'cleanup':cleanup})+'\n')
