...
def metadata(pid):
    ...
    return {'pid': pid, 'start_ticks': int(fields[19]),
            'uid': directory.stat().st_uid, 'ppid': int(fields[1]),
...
def supervise(argv, deadline, grace, report_path):
    ...
    result = {'schema': 1, 'supervisor_pid': os.getpid(), 'primary_pid': None,
              'primary_returncode': None, 'deadline_exceeded': False,
              'interrupted_signal': None, 'signals': [], 'reaped': [],
              'residuals': [], 'census_complete': True,
              'cleanup_ok': False, 'completed': False}
    primary = None
    previous = {}
    ...
    try:
        ...
        if primary is not None:
            ...
            while time.monotonic() - cleanup_start < grace + 3:
                ...
                if complete and not descendants and primary.returncode is not None:
                    break
                number = (signal.SIGTERM if time.monotonic() - cleanup_start < grace
                          else signal.SIGKILL)
                for row in descendants.values():
        ...
        result['cleanup_ok'] = (not result.get('failure') and result['census_complete']
            and not result['residuals'] and (primary is None or primary.returncode is not None))
        result['completed'] = True
        result['seconds'] = round(time.monotonic() - started, 3)
        write_report(report_path, result)
        for number, handler in previous.items():
            signal.signal(number, handler)
    print(json.dumps({'owned_test_report': str(report_path),
                      'primary_returncode': result['primary_returncode'],
                      'cleanup_ok': result['cleanup_ok']}), file=sys.stderr, flush=True)
    if result['interrupted_signal'] is not None:
