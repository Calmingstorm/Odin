...
from gi.repository import Gdk, GLib, Gtk, GdkX11  # noqa: F401,E402
...
def snapshot(kind, **values):
    print(json.dumps(dict(kind=kind, t=time.monotonic(),
        texts={r: e.get_text() for r, e in entries.items()},
        ink_pixels={r: len(points) for r, points in ink.items()},
        held={name: {k: sorted(v) for k, v in state.items()}
              for name, state in held.items()}, **values)), flush=True)
...
def receipt(widget, event):
    ...
    if kind in (Gdk.EventType.KEY_PRESS, Gdk.EventType.KEY_RELEASE):
        ...
        (state['buttons'].add if kind == Gdk.EventType.BUTTON_PRESS else state['buttons'].discard)(button)
    elif kind != Gdk.EventType.MOTION_NOTIFY:
        return False
    device = event.get_device()
    snapshot('event', widget=widget.get_name(), event=int(kind), keycode=key, button=button,
             x=float(event.get_coords()[1]), y=float(event.get_coords()[2]),
             state=int(event.get_state()[1]), synthetic=bool(event.send_event),
...
def scribble(widget, event, role):
    ...
    if paint:
        ...
        for dx in range(-4, 5):
            for dy in range(-4, 5):
                if 0 <= x+dx < widget.get_allocated_width() and 0 <= y+dy < widget.get_allocated_height():
...
def ready():
    ...
    for role, window in windows.items():
        ...
        for name, widget in [('entry', entries[role]), ('canvas', canvases[role])]:
            ...
            data[role][name] = dict(xid=widget.get_window().get_xid(),
