...
async def worker(root, mode):
    ...
    executor = ToolExecutor(config.tools, memory_path=str(state / 'memory.json'),
                            app_config=config)
    ...
    async def measure(kind, count):
        ...
        if kind == 'dispatch':
            ...
            operation = {'cached_catalog': catalog.merged_definitions,
                         'uncached_catalog': lambda: (
                             catalog.invalidate(), catalog.merged_definitions()),
                         'scoped': scoped, 'preprocess': preprocess}[kind]
            for _ in range(count):
                operation()
        return time.perf_counter_ns() - start

    print(json.dumps({'ready': True, 'catalog_sha256': hashlib.sha256(encoded).hexdigest(),
                      'ordinary_count': len(ordinary), 'total_count': len(definitions),
                      'lifecycle': lifecycle_evidence}), flush=True)
    for line in sys.stdin:
        ...
        if command['kind'] == 'close':
            ...
            print(json.dumps({'closed': True, 'remaining_tasks': len(asyncio.all_tasks()) - 1}),
                  flush=True)
            return
...
def main():
    ...
    if subprocess.check_output(['git', 'diff', 'HEAD', '--', 'src'], cwd=repo):
        raise SystemExit('Refusing: source has uncommitted differences; '
                         'archive would misrepresent it')
    output = Path(tempfile.mkdtemp(prefix='odin-r5-overhead-'))
    refs = {'base': 'd5fc7eb', 'disabled': 'HEAD', 'idle': 'HEAD'}
    processes = {}
    evidence = {'root': str(output), 'seed': 350, 'rounds': 30,
                'python': sys.version, 'source': {}, 'ready': {}, 'samples': [], 'close': {}}
    ...
    try:
        for mode, ref in refs.items():
            ...
            evidence['source'][mode] = subprocess.check_output(
                ['git', 'rev-parse', ref], cwd=repo, text=True).strip()
            ...
            env = {'PATH': os.environ.get('PATH', ''), 'HOME': str(root),
                   'PYTHONHASHSEED': '350', 'LANG': 'C.UTF-8'}
        ...
        catalogs = [(output / mode / 'benchmark-state/ordinary-catalog.json').read_bytes()
                    for mode in refs]
        ...
        counts = {'cached_catalog': 100000, 'uncached_catalog': 1000, 'scoped': 10000,
                  'preprocess': 10000, 'dispatch': 1000, 'workload': 1000}
        ...
        for iteration in range(30):
            ...
            for mode, kind in jobs:
                ...
                evidence['samples'].append({'round': iteration, 'mode': mode, 'kind': kind,
                                            'count': counts[kind], 'ns': result['ns']})
        evidence['summary'] = {}
        for kind in counts:
            modes = {}
            for mode in refs:
                values = sorted(s['ns'] / s['count'] for s in evidence['samples']
                                if s['mode'] == mode and s['kind'] == kind)
                median = statistics.median(values)
                modes[mode] = {'median_ns_per_op': median,
                               'p95_ns_per_op': values[math.ceil(.95 * len(values)) - 1],
                               'min_ns_per_op': min(values), 'max_ns_per_op': max(values),
                               'mad_percent': statistics.median(
                                   abs(v - median) for v in values) / median * 100}
            modes['disabled_delta_percent'] = (
                modes['disabled']['median_ns_per_op'] / modes['base']['median_ns_per_op'] - 1) * 100
