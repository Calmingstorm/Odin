...
def state(kind, **values):
    record(kind, texts={k: e.get_text() for k, e in entries.items()},
           focus={k: w.get_focus().get_name() if w.get_focus() else None
                  for k, w in windows.items()}, **values)

def event(widget, ev):
    if ev.type not in (Gdk.EventType.KEY_PRESS, Gdk.EventType.KEY_RELEASE,
                       Gdk.EventType.BUTTON_PRESS, Gdk.EventType.BUTTON_RELEASE,
                       Gdk.EventType.FOCUS_CHANGE, Gdk.EventType.MOTION_NOTIFY):
        return False
    dev, source = ev.get_device(), ev.get_source_device()
    state('event', widget=widget.get_name(), event=str(ev.type),
          device=dev.get_name() if dev else None, source=source.get_name() if source else None,
          keyval=ev.keyval if ev.type in (Gdk.EventType.KEY_PRESS, Gdk.EventType.KEY_RELEASE) else None,
          modifiers=int(ev.get_state()[1]))
    return False

record('version', gtk=f'{Gtk.get_major_version()}.{Gtk.get_minor_version()}.{Gtk.get_micro_version()}')
for role, x in [('human', 20), ('robot', 420)]:
    ...
    win.connect('set-focus', lambda w, e: state('set-focus', window=w.get_name(),
                target=e.get_name() if e else None))
...
def ready():
    for role, win in windows.items():
        ...
        record('ready', role=role, xid=win.get_window().get_xid(), position=list(win.get_position()))
    state('ready-state')
    return False
