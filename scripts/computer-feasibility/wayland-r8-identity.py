...
def identify(pid, backend):
    ...
    selected = [p for p in paths if any(s in p for s in ('libmutter-', '/gnome-shell', 'libeis-', 'libei.'))]
    mapped = []
    for name in selected:
        path = proc / 'root' / name.lstrip('/')
        info = path.stat()
        mapped.append(dict(path=name, sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
    ...
    return dict(schema=1, pid=pid, start_ticks=before, uid=proc.stat().st_uid,
                argv=argv, backend=backend, mapped_files=mapped,
                boot_id=Path('/proc/sys/kernel/random/boot_id').read_text().strip(),
                packages=subprocess.check_output(['dpkg-query', '-W', 'gnome-shell', 'libmutter-16-0',
                    'libei1', 'libeis1', 'xdg-desktop-portal', 'xdg-desktop-portal-gnome'], text=True),
                physical_input=False, no_libinput=backend == 'native-headless')
