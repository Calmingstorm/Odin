...
def audit(root):
    ...
    for event_file in sorted(root.glob('case-*/events.jsonl')):
        ...
        if 'exact_utf8' in row:
            ...
            closed = subprocess.run(['tesseract', str(directory/'closed-tab.png'), 'stdout'],
            ...
            checks.append(dict(task=row['task'], exact_bytes=len(blob),
                               sha256=hashlib.sha256(blob).hexdigest(), closed_ocr=closed))
    for start in starts:
        unit = start['unit']
        result = subprocess.run(['systemctl', 'show', unit, '-p', 'MainPID', '-p', 'ActiveState'],
    ...
    report = dict(root=str(root), fixtures=len(starts), cleanup='pass',
                  independent_artifacts=checks)
    (root/'independent-audit.json').write_text(json.dumps(report, indent=2))
    print(json.dumps(dict(root=str(root), exact_cleanup_fixtures=len(starts),
