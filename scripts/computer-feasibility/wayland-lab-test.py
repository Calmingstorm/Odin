...
class LabTest(unittest.TestCase):
    def setUp(self):
        ...
        self.evidence = Path('/home/odin/tmp/wayland-stub-' + uuid.uuid4().hex)
        ...
        python.chmod(0o700)
        self.env = {**os.environ, 'PATH': str(self.tmp) + ':' + os.environ['PATH'],
    ...
    def command(self):
        return ['bash', str(self.script), 'experiment-lifecycle',
                '--parent-authorized-after-contract-correction', str(self.evidence)]

    def run_mode(self, mode, code):
        result = subprocess.run(self.command(), env={**self.env, 'STUB_MODE':mode},
    ...
    def assert_clean(self):
        ...
        for call in calls:
            if call[0] in ('stop','rm'): self.assertEqual(call[-1], 'abcdef')
    ...
    def test_term_reaps_attachment(self):
        p = subprocess.Popen(self.command(), env={**self.env, 'STUB_MODE':'term'},
                             stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        try:
            end = time.monotonic() + 5
            while not (self.evidence/'attach-client.pid').exists():
                if time.monotonic() > end: self.fail('attachment never started')
                time.sleep(.02)
...
class ProcessLedgerTest(unittest.TestCase):
    def setUp(self):
        spec = importlib.util.spec_from_file_location('wayland_ledger', Path(__file__).with_name('wayland-process-ledger.py'))
    ...
    def test_permission_error_marks_scan_incomplete(self):
        ...
        with patch.object(Path, 'glob', return_value=[fake]), patch.object(Path, 'read_text', side_effect=PermissionError):
    ...
    def test_vanished_process_race_only_if_directory_absent(self):
        ...
        for remains in (False, True):
            with patch.object(Path, 'glob', return_value=[fake]), patch.object(Path, 'read_text', side_effect=FileNotFoundError), patch.object(Path, 'exists', return_value=remains):
                _, errors = self.ledger.processes()
            self.assertEqual(bool(errors), remains)

    def test_malformed_and_catatonit(self):
        with patch.object(Path, 'glob', return_value=[Path('/proc/999/stat')]), patch.object(Path, 'read_text', return_value='malformed'):
            _, errors = self.ledger.processes()
