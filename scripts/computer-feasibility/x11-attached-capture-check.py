...
server = subprocess.Popen(["Xvfb", ":177", "-screen", "0", "900x500x24",
                           "-nolisten", "tcp", "-noreset", "-extension", "GLX"],
                          stdout=subprocess.DEVNULL)
app = None
try:
    for _ in range(50):
        if subprocess.run(["xdpyinfo", "-display", ":177"], stdout=subprocess.DEVNULL,
                          stderr=subprocess.DEVNULL).returncode == 0:
            break
        time.sleep(.05)
    app = subprocess.Popen(["/usr/bin/python3", "/harness/x11-same-target.py"],
    ...
    async def check():
        ...
        try:
            ...
            print("SOURCES_ONLY_DIAGNOSTIC", diagnostic.returncode,
                  diagnostic.stdout[:1000], diagnostic.stderr[:1000], flush=True)
            raise
        ...
        print(json.dumps({"kind": "actual-capture-adapter", "passed": True,
                          "dimensions": image.size, "bytes": len(frame.image_bytes),
                          "capture_only": True, "receipt": receipt}), flush=True)
    asyncio.run(check())
    ...
    print(json.dumps({"kind": "reaped", "xvfb": server.pid, "app": app.pid if app else None,
                      "server_exit": server.returncode, "app_exit": app.returncode if app else None}),
          flush=True)
