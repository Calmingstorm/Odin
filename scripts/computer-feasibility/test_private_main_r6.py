...
spec = importlib.util.spec_from_file_location('main_smoke',
    Path(__file__).with_name('main-session-app-smoke-r6.py'))
    ...
    def test_main_qualification_gate_is_not_private_override(self):
        with patch.object(harness, 'PRIVATE_TASK_QUALIFIED', False):
            with self.assertRaisesRegex(RuntimeError, 'not_privately_qualified'):
                harness.validate_args(SimpleNamespace(confirm_scratch_only=True,
                                                      private_qualification=False))

    def test_svg_requires_real_nonempty_rectangle_and_ellipse(self):
        valid = b'<svg xmlns="http://www.w3.org/2000/svg"><rect width="10" height="20"/><ellipse rx="3" ry="4"/></svg>'
        self.assertEqual(harness.verify_shapes(valid)['rectangles'], 1)
        for blob in (valid.replace(b'width="10"', b'width="0"'),
