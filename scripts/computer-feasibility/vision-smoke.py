...
async def main():
    ...
    palette = {"red": (255, 0, 0), "green": (0, 160, 0), "blue": (0, 0, 255),
    ...
    metadata = FrameMetadata(
        observation_id="synthetic-live-smoke", session_id="synthetic-live-smoke",
        generation=1, captured_monotonic_ns=time.monotonic_ns(), width=480, height=320,
        source_id="synthetic", source_revision=1, consent_generation=1,
        source_width=480, source_height=320)
    ...
    async def sent(_session, _context, params):
        ...
        images = [block for item in body["input"] for block in item.get("content", [])
                  if block.get("type") == "input_image"]
        valid = (len(images) == 1
                 and images[0].get("image_url") == "data:image/png;base64," + encoded)
        if not valid:
        ...
        wire.append({"model": body["model"], "native_images": 1,
                     "png_sha256": hashlib.sha256(decoded).hexdigest(),
                     "serialized_bytes": len(params.chunk), "store": body["store"]})

    trace.on_request_chunk_sent.append(sent)
    client = CodexChatClient(ReadOnlyAuth(args.credentials), model=args.model,
    ...
    async with aiohttp.ClientSession(trace_configs=[trace], auto_decompress=False,
        ...
        try:
            response = await asyncio.wait_for(client.chat_with_tools(
                [{"role": "user", "content": [{"type": "text", "text":
                  "This generated test image has six colored tiles, three columns and two rows. "
                  "Return only a JSON array naming the six colors in row-major order, "
                  "left to right, top row first. Names: red, green, blue, yellow, magenta, "
                  "cyan, black, white. No tools, no explanation."}, native["__image_block__"]]}],
                "Interpret the actual supplied pixels. Do not guess from metadata.", []), 100)
            try:
                actual = json.loads(response.text)
            except (ValueError, TypeError):
                actual = None
            passed = actual == selected and len(wire) == 1
            print(json.dumps({"passed": passed, "synthetic_only": True,
                  "provider_image_response_received": True,
                  "visual_answer_correct": actual == selected,
                  "elapsed_seconds": round(time.monotonic() - started, 3),
                  "wire": wire, "credential_writes": False}, sort_keys=True), flush=True)
            ...
            print(json.dumps({"passed": False, "error_type": type(error).__name__,
                              "wire": wire, "credential_writes": False}), flush=True)
            return 1
        finally:
            await client.close()
