...
try:
    ...
    app = spawn("gtk", ["/usr/bin/python3", "/harness/x11-same-target.py"],
    ...
    for cls in (protocol.event.KeyPress, protocol.event.KeyRelease):
        event = cls(time=X.CurrentTime, root=root, window=window, child=X.NONE,
                    root_x=470, root_y=80, event_x=50, event_y=60,
                    state=0, detail=keycode, same_screen=1)
    ...
    record("send_event_result", text_receipts=delivered, app_alive=app.poll() is None,
