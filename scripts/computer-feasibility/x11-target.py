...
def event(widget, ev):
    ...
    record('event', widget=widget.get_name(), event=str(ev.type),
           device=dev.get_name() if dev else None, source=source.get_name() if source else None,
           state=int(ev.get_state()[1]), xy=list(ev.get_coords()))
    return False
...
def show_menu(widget):
    record('menu-open')
    menu.popup_at_widget(widget, Gdk.Gravity.SOUTH_WEST, Gdk.Gravity.NORTH_WEST, Gtk.get_current_event())
mb.connect('clicked', show_menu)
modal = Gtk.Button(label='Modal')
modal.set_size_request(140, 40)
fixed.put(modal, 10, 300)
def show_modal(*_):
    dialog = Gtk.MessageDialog(transient_for=win, modal=True, buttons=Gtk.ButtonsType.OK,
                               text='Harmless modal receipt')
    record('modal-open')
    dialog.connect('response', lambda d, response: (record('modal-response', response=response), d.destroy()))
    dialog.show_all()
