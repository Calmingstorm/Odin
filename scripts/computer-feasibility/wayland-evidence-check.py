...
spec = importlib.util.spec_from_file_location('ledger', Path(__file__).with_name('wayland-process-ledger.py'))
ledger = importlib.util.module_from_spec(spec)
spec.loader.exec_module(ledger)

SUFFIXES = ('guardian1', 'causal-baseline', 'causal-fixed1', 'guardian-fixed1',
            'guardian-fixed2', 'guardian-fixed3')
current, errors = ledger.processes()
assert not errors, errors
report = []
for suffix in SUFFIXES:
    root = Path('/home/odin/tmp/wayland-r5-' + suffix + '-20260907')
    ...
    survivors = [p for p in owned if p['pid'] in current and p['start'] == current[p['pid']]['start']]
    assert not survivors, survivors
    assert json.loads((root / 'census-errors.json').read_text()) == []
    assert (root / 'artifact-inventory.json').is_file()
    report.append(dict(suffix=suffix, cgroup_absent=True, identities_absent=len(owned),
