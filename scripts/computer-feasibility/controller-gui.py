...
async def main():
    ...
    controller = ComputerController(store, lambda _: backend, lambda ctx: ctx == context,
                                    enabled=True)
    grant = None
    ...
    async def observe():
        ...
        result = await controller.observe(context, {"session_id": current.session_id,
    ...
    try:
        ...
        record({"event": "started", "grant": grant, "unit": backend._unit,
                "supervisor_pid": backend._process.pid, "evidence": str(out)})
        obs = await observe()
        ...
        while True:
            ...
            try:
                if command.get("operation") in {"pause", "resume"}:
                    current = store.get_session(grant["session_id"])
                    result = await controller.session(context, {"operation": command["operation"],
                ...
                if command.get("operation") == "export":
                    current = store.get_session(grant["session_id"])
                    result = await controller.session(context, {"operation": "export",
                        "session_id": current.session_id, "generation": current.generation,
                        "name": command["name"]})
                    blob, metadata = store.read_evidence(context, result["artifact_id"])
                    (out / command["name"]).write_bytes(blob)
                    record({"event": "export", "metadata": metadata,
                ...
                payload = {"session_id": obs.session_id, "generation": obs.generation,
                    ...
                    "expect": {"type": "visual_change"}, **command}
                ...
                record({"event": "receipt", "operation": command["operation"],
        ...
        record({"event": "cleanup", "seconds": time.monotonic() - start,
                "unit": backend._unit, "supervisor_returncode": (
