...
def fixture():
    ...
    windows = []
    for n in range(2):
        w = d.screen().root.create_window(150 + n * 380, 180 + n * 170, 280, 180, 0,
...
def inner(backend):
    ...
    env = {'PATH': '/usr/bin:/bin', 'LANG': 'C.UTF-8', 'HOME': str(home),
           'XAUTHORITY': str(authority), 'DISPLAY': ':172',
           'XDG_CONFIG_HOME': str(home / 'config'), 'XDG_CACHE_HOME': str(home / 'cache'),
           'XDG_DATA_HOME': str(home / 'data'), 'XDG_RUNTIME_DIR': str(home),
    ...
    subprocess.run(['/usr/bin/xauth', '-f', str(authority)],
        ...
        text=True, env=env, timeout=5, check=True, stdout=subprocess.DEVNULL)
    children = Children()
    result = {'backend': backend, 'namespace_isolated': True, 'native_app_evidence': False,
              'hardware_dpms_evidence': False, 'success': False}
    d = None

    def ready(name):
        return subprocess.run(['/usr/bin/xdpyinfo'], env={**env, 'DISPLAY': name}, timeout=2,
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode == 0

    try:
        children.spawn('xvfb', ['/usr/bin/Xvfb', ':171', '-screen', '0', '1600x1100x24',
            '-auth', str(authority), '-nolisten', 'tcp', '-noreset', '-extension', 'GLX'], env)
        wait(lambda: ready(':171'))
        children.spawn('xephyr', ['/usr/bin/Xephyr', ':172', '-screen', '1280x900',
            '-resizeable', '-auth', str(authority), '-nolisten', 'tcp', '-noreset',
            '-extension', 'GLX'], {**env, 'DISPLAY': ':171'})
        wait(lambda: ready(':172'))
        rejected = {}
        for name in (':171', ':172'):
            rejected[name] = subprocess.run(['/usr/bin/xdpyinfo'],
                env={**env, 'DISPLAY': name, 'XAUTHORITY': '/nonexistent'}, timeout=2,
                stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode != 0
        ...
        time.sleep(1)
        app = children.spawn('fixture',
            ['/usr/bin/python3', '/code/private-recovery-r7.py', '--fixture'], env)
        ...
        try:
            ...
            with Path('/workspace/window-tree.txt').open('w') as stream:
                subprocess.run(['/usr/bin/xwininfo', '-root', '-tree'], env=env, timeout=3,
        ...
        wait(lambda: '_NET_WM_STATE_HIDDEN' in
             windows.snapshot(d)['windows'][hidden]['state_names'])
        time.sleep(.3)
        before = windows.snapshot(d)
        windows.validate(before)
        write(Path('/workspace/windows-before.json'), before)
        result['fixture_baseline'] = {'owner': before['windows'][hidden]['identity'],
                                     'wm_pid': wm.pid, 'app_pid': app.pid}
        if baseline is not None:
            output = next(o for o in baseline['outputs'] if o['crtc'])
            target = next(m for m in baseline['modes'] if m['id'] in output['modes']
                          and m['width'] == 800 and m['height'] == 600)
            changed = subprocess.run(['/usr/bin/xrandr', '--output', output['name'],
                '--mode', hex(target['id'])], env=env, capture_output=True, text=True, timeout=5)
            result['topology_change_command'] = {'returncode': changed.returncode,
        ...
        for wid in (visible, hidden):
            ...
            windows._send(d, wid, '_NET_MOVERESIZE_WINDOW',
                          [10 | (3 << 8) | (2 << 12), x + 40, y + 35, 0, 0])
        wait(lambda: windows.snapshot(d)['windows'][hidden]['geometry'][:2]
        ...
        assert all(windows._same(after_windows['windows'][w], before['windows'][w], before)
                   for w in (visible, hidden))
        assert app.poll() is None and wm.poll() is None
        assert all(after_windows['windows'][w]['identity'] == before['windows'][w]['identity']
                   for w in (visible, hidden))
        result['fixture_survivors'] = {'both_same_xid_pid_start_ticks': True,
            'hidden_exact_record_restored': True, 'visible_geometry_state_restored': True,
...
def outer(backend):
    ...
    argv = ['/usr/bin/bwrap', '--unshare-all', '--die-with-parent', '--new-session',
            ...
            '--ro-bind', '/etc/fonts', '/etc/fonts', '--ro-bind', '/etc/X11', '/etc/X11',
            '--ro-bind', str(code), '/code', '--bind', str(workspace), '/workspace',
            '--chdir', '/workspace', '--clearenv', '--setenv', 'PATH', '/usr/bin:/bin',
            '--setenv', 'LANG', 'C.UTF-8', '--setenv', 'PYTHONDONTWRITEBYTECODE', '1',
            '/usr/bin/python3', '/code/private-recovery-r7.py', '--inner', '--backend', backend]
    with (out / 'namespace.log').open('w') as log:
        p = subprocess.Popen(argv, env={'PATH': '/usr/bin:/bin', 'LANG': 'C.UTF-8'},
        ...
        try:
            ...
            os.close(fd)
    report = {'artifact_directory': str(out), 'namespace_returncode': status,
              'source_sha256': hashes, 'backend': backend}
...
if __name__ == '__main__':
    ...
    raise SystemExit(fixture() if args.fixture else
                     inner(args.backend) if args.inner else outer(args.backend))
