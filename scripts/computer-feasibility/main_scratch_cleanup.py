...
def cleanup_status(stages, *, start, baseline_validated):
    ...
    rows = stages[start:]
    exact = next((row['ok'] for row in reversed(rows)
                  if row['stage'] == 'compare_exact_session'), False)
    return {'cleanup_complete': all(row['ok'] for row in rows)
            and (exact or not baseline_validated),
...
def manual_actions(stages):
    ...
    return [{'stage': row['stage'], 'action': actions.get(row['stage'],
             'inspect_stage_and_supervisor_json_clean_only_verified_owned_resources')}
            for row in stages if not row['ok']]
...
async def close_controller_verified(controller, context):
    ...
    required = ('complete', 'stopped', 'released', 'applications_preserved',
                'input_revoked', 'capture_revoked')
    device_states = {'removed', 'retained_inactive', 'not_created'}
    if (controller._live or (grant and (grant.state not in {'closed', 'cancelled'}
