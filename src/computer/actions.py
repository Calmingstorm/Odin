...
_REQUIRED = {"session_id", "generation", "consent_generation", "source_id",
             "source_revision", "action_id", "observation_id", "operation", "expect"}
...
def click_payload(inp, observation):
    ...
    if (inp["source_id"] != source.source_id
    ...
    try:
        point = source.input_point(observation.delivered_to_source, inp["x"], inp["y"],
    ...
    payload = {"type": "click", "source_id": source.source_id,
               "source_revision": source.source_revision,
               "consent_generation": source.consent_generation,
               "x": inp["x"], "y": inp["y"], "expected": dict(inp["expect"])}
...
def click_receipt(raw, observation, target):
    ...
    if (type(actual) is not dict or set(actual) != {"x", "y"}
            or type(actual["x"]) is not int or type(actual["y"]) is not int
    ...
    result["verification"] = {
        "status": "satisfied" if satisfied else "not_satisfied", "type": "pointer_at",
        "scope": "pointer_location_only", "method": "pointer_query_after_release",
        "actual": dict(actual), "source_id": source.source_id,
