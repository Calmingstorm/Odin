...
def compositor_argv(backend):
    argv = ["/usr/bin/kwin_wayland", "--socket", "wayland-probe",
...
def setup(trial, identity, dbus):
    trial.wait(lambda: dbus("NameHasOwner", "org.kde.KWin"),
               "probe_private_kwin_unavailable", 15)
...
def connect(trial, glib):
    ...
    try:
        result, fds = trial.call("/org/kde/KWin/EIS/RemoteDesktop",
                                "org.kde.KWin.EIS.RemoteDesktop", "connectToEIS",
                                glib.Variant("(i)", (3,)), fd=True)
    ...
    if (len(values) != 2 or type(values[0]) is not int
            or type(values[1]) is not int or values[1] <= 0):
        import os
