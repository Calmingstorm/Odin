...
def _canary() -> dict:
    ...
    png = (b"\x89PNG\r\n\x1a\n"
           ...
           + chunk(b"IEND", b""))
    return observation_image(png, FrameMetadata(
        observation_id="transport-canary", session_id="transport-canary",
        generation=1, captured_monotonic_ns=1, width=2, height=1,
...
def native_transport_evidence(serving) -> NativeTransportEvidence:
    ...
    if (getattr(serving, "provider", None) != "codex"
            ...
            or type(model) is not str or not model.strip() or model != model.strip()):
        raise PermissionError("Computer use requires verified native image transport.")
    canary = _canary()
    encoded = canary["__image_block__"]["source"]["data"]
    messages = [
        {"role": "assistant", "content": [{"type": "tool_use", "id": "canary-call",
          "name": "computer_observe", "input": {}}]},
        {"role": "user", "content": [{"type": "tool_result", "tool_use_id": "canary-call",
    ...
    try:
        ...
        if (len(images) != 1 or images[0].get("image_url") != f"data:image/png;base64,{encoded}"
                or len(calls) != 1 or len(results) != 1
