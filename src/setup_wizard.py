"""
Interactive CLI setup wizard for Odin.
...
class SetupWizard:
    ...
    def step_hosts(self) -> None:
        ...
        self._print("Odin can manage remote servers via SSH.")
    ...
    def step_start_service(self) -> None:
        ...
        if not detect_systemd():
            self._print(f"\n  {color_dim('systemd not detected — start Odin manually.')}")
            return

        self._print(f"\n{color_header('=== Step 7: Start Odin ===')}\n")
        action = "restart" if self.reconfigure else "start"
        if self._prompt_yes_no(f"{action.title()} Odin service now?"):
            try:
                subprocess.run(
                    ["sudo", "systemctl", action, "odin"],
                    check=True,
                    capture_output=True,
                    text=True,
                    timeout=30,
                )
                self._print(color_success(f"  Odin service {action}ed!"))
            except subprocess.CalledProcessError as exc:
                self._print(color_error(f"  Failed to {action} service: {exc.stderr}"))
            except FileNotFoundError:
                self._print(color_warn(f"  'sudo' not found — {action} manually: systemctl {action} odin"))
    ...
    def run(self) -> bool:
        ...
        """
        banner = "  Odin Setup Wizard"
        if self.reconfigure:
            banner = "  Odin Reconfigure Wizard"
