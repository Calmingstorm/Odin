"""
Validate Odin packaging artifacts (systemd unit, shell scripts, nfpm config).
...
REQUIRED_CONTENT_DESTINATIONS = [
    "/opt/odin/src",
    "/opt/odin/ui",
    "/opt/odin/pyproject.toml",
    "/opt/odin/config.yml",
    "/opt/odin/.env.example",
    "/usr/lib/systemd/system/odin.service",
]
...
def validate_nfpm_config(config: dict[str, Any]) -> list[str]:
    ...
    # Package name
    if config.get("name") != "odin":
        errors.append(f"Package name must be 'odin', got '{config.get('name')}'")
...
def validate_nfpm_contents_consistency(config: dict[str, Any]) -> list[str]:
    ...
    # Application code must go under /opt/odin/
    app_dsts = [d for d in destinations if d.startswith("/opt/odin/")]
    if not app_dsts:
        errors.append("No application files under /opt/odin/")

    # Check that source code directory is included
    if not any(d.startswith("/opt/odin/src") for d in destinations):
        errors.append("Source code directory /opt/odin/src/ not in contents")
