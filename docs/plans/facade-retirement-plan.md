...
**Status:** PLAN OF RECORD — Odin LGTM 2026-07-05 conditional on the two R1 amendments, which are applied below
...
| `src/health/server.py` | 1 | `bot.audit` only (`src/health/checker.py` additionally reads public discord surface — `is_ready`, `guilds` — untouched by the underscore gate) |
...
- discord.py inherited surface + lifecycle hooks (`setup_hook`, `on_ready`, `on_message`, `on_voice_state_update`, `close`) and their private helpers that do real work (`_backfill_archives`, `_on_voice_transcription`, `_resolve_prefix`, `_log_startup_config`). Scheduler and infra-watcher **callbacks wire directly to `scheduled_events` methods** (R1: infra_watcher construction reorders after `build_components` in P2 so the callback target exists); the bot-side callback delegates die in P7 with the rest.
- `config` (live root), `start_time` (was `_start_time`).
- `services: BotServices` + the existing **flat subsystem handles** (sessions, scheduler, tool_executor, skill_manager, loop_manager, audit, agent_manager, reflector, channel_config, channel_logger, context_loader, infra_watcher, voice_manager, browser_manager, permissions, host_access_manager, api_token_manager, cost_tracker, subsystem_guard, audit_signer, diff_tracker, model_router, context_compressor, prefix_tracker, auxiliary_llm_client, outbound_webhook_dispatcher, trajectory_saver, agent_trajectory_saver, loop_reflection_gate*, knowledge_store*, embedder*, vector_store*, fts_index*, stuck_loop_tracker_cls, classify_command_risk, classify_tool_risk). Starred = renamed public from `_`-private. Rationale: these are plain composition handles, zero logic — the god-object problem was delegates + hidden state + logic, not the existence of handles. Deleting them buys nothing and breaks thousands of legitimate spellings.
...
- Late-bound (absent at construction, unchanged semantics): `startup_report`, `health_server`, `process_registry`, `mcp_manager`, `compression_stats`, `_codex_auth_pool`. (`_web_channel_locks` leaves this list — becomes app state.)
...
| `_execute_scheduled_tool`, `_run_scheduled_workflow`, `_on_scheduled_task`, `_on_schedule_failure`, `_on_scheduled_digest`, `_format_digest_raw`, `_resolve_mentions`, `_on_monitor_alert` | `bot.scheduled_events.<method>` — `on_ready`'s `scheduler.start(...)` and `InfraWatcher(alert_callback=...)` receive `scheduled_events` methods directly (R1); no temporary bot delegate survives P7 |
...
- **Live state is always a provider callable**: `get_config`, `get_knowledge_store` (already conventions). Gateway/clients: inject the *gateway* (stable object owning swappable clients) — never a captured client. The P7 grep gate covers **both** capture forms (R1): direct assignments (`self.config = ` etc.) *and* Deps-dataclass fields typed as live objects (`config:`, `codex_client:`, `ollama_client:`, `kimi_client:` are banned field names — spell `get_config: Callable` or inject the gateway).
...
|---|---|---|
| **P1** | Seam-carve `tool_loop.py` (§3.4). Move `ensure_failure_visible`, `_scrub_tool_input_for_storage`, `_EMPTY_RESPONSE_FALLBACK`, webhook-allowlist global+init to `tool_loop_helpers.py` (compat re-exports stay until P7; `background_task.py` import migrated now). Kills the client late-imports. | characterization suite byte-green; no method >160 ln; lint zero-new; **R1 carve gate**: AST-normalization check — each carved phase body statement-identical to its pre-carve block modulo the mechanical closure-var→turn-state renames (checker script + block map attached to the PR); plus unit assertions pinning mutation semantics the closures relied on: `messages` list identity across phases, tool-result ordering = gather order, op-details append order, retry-flag one-shot semantics, cancel-check placement |
...
| **P6** | Web/slash/main migration: api.py + chat.py + health/server.py + slash_commands.py spellings per §3.2 table; `_web_channel_locks` → aiohttp app state; api.py prompt-rebuild sites → `prompt_builder.rebuild_default()`; the 13 cache-null sites → `invalidate()`. | full web/endpoint test files green; grep: **zero `bot._<underscore-attr>` in src/web + src/health** (public handles — config, audit, guilds, is_ready — explicitly allowed); UI dist untouched; R1: focused test that two concurrent web chats for the same channel serialize through the same app-state lock (no per-request lock creation) |
