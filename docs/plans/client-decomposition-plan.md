...
| | |
|---|---|
| **Status** | **CODE-COMPLETE** (2026-07-05) — P0–P10 + P5c implemented and Odin-reviewed on `refactor/client-decomposition`. §10 metrics reported in R6. Next: local soak-test deployment with all live data retained (R2), then the project owner's master-merge/pipeline decision |
| **Author** | Claude (on behalf of the project owner) |
| **Reviewer** | Odin |
| **Date** | 2026-07-04 |
...
| Delegation & agents | 4149–4697 | 550 | `_handle_delegate_task`, task list/cancel, loop start/stop/list, `_handle_spawn_agent` (119), agent send/list/kill/results/wait |
...
| Late-bound (NOT set in `__init__`) | — | `startup_report` (setup_hook), `_web_channel_locks` (created by `web/chat.py`!), `mcp_manager`, `_codex_auth_pool`, `compression_stats`, `health_server` (web layer) |
...
**Execution model (R2, per the project owner 2026-07-04 — supersedes the per-phase release mapping below).** The campaign runs on a long-lived integration branch **`refactor/client-decomposition`** cut from master `081e231`. Each phase is a PR from a phase branch (`refactor/client-p<N>-<slug>`) **into the campaign branch**; Odin reviews every phase PR before it merges (hard rule unchanged). **Master is not touched and the release pipeline is not run during the campaign.** After P10, the completed branch is deployed to a local test instance retaining all live data (Codex auth, API tokens, `memory.json`, `learned.json`, knowledge store — standard deploy discipline) and soak-tested extensively with Odin running under the new code; only after that soak does the project owner decide on the final master merge + release pipeline. To prevent drift, master hotfixes landed mid-campaign are merged master → campaign branch as they occur.
...
- **Approval gates.** This RFC needs Odin's review (this request) and the project owner's direction approval. Then **each phase separately** goes through the normal per-file-change-list approval with the project owner before any code is written, followed by branch → PR (into the campaign branch, per R2) → Odin review → merge. RFC approval is not approval to code.
...
**Late-bound (must remain absent until set):** `startup_report`, `_web_channel_locks`, `mcp_manager`, `_codex_auth_pool`, `compression_stats`, `health_server`.
...
- **R2 (2026-07-04)** — Execution model set by the project owner (direction approval): long-lived campaign branch `refactor/client-decomposition`; phase PRs merge into it with Odin review each; **no master merge, no release pipeline, no live deploy during the campaign**; post-P10 local soak test with all live data retained, then the pipeline decision. §7 execution-model paragraph updated; original per-phase release mapping retained inactive.
- **R6 (2026-07-05)** — **Campaign code-complete.** P10 delivered `scheduled_events.py`, `turn_recorder.py`, `slash_commands.py` + final sweep. §10 metrics vs targets: **met** — one dispatch table (zero `elif tool_name ==` outside the registry), duplicate loop deleted, facade contract green throughout, external consumers (web/, health/, `__main__`, cogs) have ZERO diffs since baseline `081e231`, tests 6,040 ≥ baseline 5,905 (+127 characterization), suite runtime ~62s (≤ baseline), every phase Odin-reviewed and independently revertable. **Missed, deliberately** — `client.py` = 1,272 lines vs ≤~900 target (~450 lines are the one-line delegate/shim facade that bought zero external breakage; the true adapter is ~820) and `tool_loop.py` = 1,365 vs ≤~900 (it deliberately holds BOTH pipelines + LoopPolicy; re-splitting would recreate the dual-file sync hazard the campaign existed to kill). Long literals in moved scheduled/slash code carry per-line `noqa: E501` (verbatim-move disclosure) rather than wraps. Facade-delegate slimming, if ever desired, requires migrating `web/api.py` off private names first — explicitly out of scope (§3).
- **R5 (2026-07-04)** — P7 delivered as a **move-only** extraction (reviewer-approved deviation): `_process_with_tools` lands verbatim in `ToolLoopRunner.run` (host-based, per the P5a pattern) and the promised decomposition into ~10 named methods **rides with P8**, where LoopPolicy parameterization forces exactly those seams — one restructuring pass over the loop's lines instead of two. CompletionClassifier extracted per plan (provider-backed llm client; facade delegates + class-attr alias retained).
