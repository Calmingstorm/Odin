...
|---|---|---|
| Lexical launch alias | Load and migrate the canonical target, then register the separate lexical launch path with existing workspace protection. Preserve symlink/parent-traversal semantics and original restart arguments. | Real `main()` / `load_config()` probes for file and directory symlinks preserve both protected roots and reject alias workspaces. A symlink-plus-`..` negative control caught lexical collapse selecting the wrong file; the corrected startup loads the original target. |
