...
- `src/` — all source code (Python, asyncio-based)
  ...
  - `src/llm/model_router.py` — Smart model routing: heuristic intent classification (chat/query/task/complex) with optional LLM fallback; routes cheap vs strong model
  ...
  - `src/monitoring/resource_usage.py` — resource usage stats (sessions, knowledge, trajectories, storage)
  ...
  - `src/health/startup.py` — startup diagnostics: boot-time checks for Codex auth, SSH hosts, DB, knowledge store, config consistency, with helpful error messages
  - `src/health/subsystem_guard.py` — graceful degradation: SubsystemGuard tracks subsystem availability (AVAILABLE/DEGRADED/UNAVAILABLE), auto-transitions on failure thresholds, auto-recovers on success
...
- `ui/` — web management UI (Vue 3 + Tailwind CSS, CDN-based, no build step)
  ...
  - `ui/js/pages/` — 18 page components (dashboard, sessions, tools, skills, knowledge, schedules, loops, processes, audit, config, logs, memory, chat, agents, usage, traces, health, resources)
...
Web management UI:
  ...
  /api/*               → REST API (128 endpoints, Bearer token auth)
...
### Web Management UI
- **Backend**: aiohttp REST API (128 endpoints) + WebSocket, mounted on the health server (port 3939)
...
- **Pages**: dashboard, sessions, tools, skills, knowledge, schedules, loops, processes, audit, config, logs, memory, chat, agents, usage, traces, health, resources
