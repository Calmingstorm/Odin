var km=Object.defineProperty;var wm=(e,t,s)=>t in e?km(e,t,{enumerable:!0,configurable:!0,writable:!0,value:s}):e[t]=s;var Xe=(e,t,s)=>wm(e,typeof t!="symbol"?t+"":t,s);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function s(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(a){if(a.ep)return;a.ep=!0;const i=s(a);fetch(a.href,i)}})();class Sm{constructor(){this._persist=localStorage.getItem("odin_persist")==="1",this._token=this._persist?localStorage.getItem("odin_token")||"":sessionStorage.getItem("odin_token")||"";const t=this._persist?localStorage:sessionStorage;this._sessionTimeout=parseInt(t.getItem("odin_session_timeout")||"0",10),this._lastActivity=Date.now(),this._activityTimer=null,this.onSessionExpired=null,this._token&&this._sessionTimeout>0&&this._startActivityMonitor()}get token(){return this._token}get sessionTimeout(){return this._sessionTimeout}setToken(t,s=0){if(this._token=t,this._sessionTimeout=s,this._lastActivity=Date.now(),t){const n=this._persist?localStorage:sessionStorage;n.setItem("odin_token",t),this._persist&&localStorage.setItem("odin_persist","1"),s>0?n.setItem("odin_session_timeout",String(s)):n.removeItem("odin_session_timeout"),this._startActivityMonitor()}else sessionStorage.removeItem("odin_token"),sessionStorage.removeItem("odin_session_timeout"),localStorage.removeItem("odin_token"),localStorage.removeItem("odin_persist"),localStorage.removeItem("odin_session_timeout"),this._stopActivityMonitor()}setPersist(t){this._persist=t}_startActivityMonitor(){this._stopActivityMonitor(),!(this._sessionTimeout<=0)&&(this._activityTimer=setInterval(()=>{(Date.now()-this._lastActivity)/1e3>=this._sessionTimeout&&(this._stopActivityMonitor(),this.onSessionExpired&&this.onSessionExpired())},1e4))}_stopActivityMonitor(){this._activityTimer&&(clearInterval(this._activityTimer),this._activityTimer=null)}_headers(t={}){const s={"Content-Type":"application/json",...t};return this._token&&(s.Authorization=`Bearer ${this._token}`),s}async _request(t,s,n=null){this._lastActivity=Date.now();const a={method:t,headers:this._headers()};n!==null&&(a.body=JSON.stringify(n));const i=await fetch(s,a);if(i.status===401)throw new mr("Unauthorized");const l=await i.json().catch(()=>null);if(!i.ok){const r=(l==null?void 0:l.error)||`HTTP ${i.status}`;throw new Tm(r,i.status,l)}return l}get(t){return this._request("GET",t)}post(t,s){return this._request("POST",t,s)}put(t,s){return this._request("PUT",t,s)}del(t){return this._request("DELETE",t)}async login(t){const s=await fetch("/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:t})}),n=await s.json().catch(()=>null);if(!s.ok)throw new mr((n==null?void 0:n.error)||"Login failed");return this.setToken(n.session_id,n.timeout_seconds||0),n}async logout(){try{await this.post("/api/auth/logout",{})}catch{}this.setToken("")}async check(){try{return await this.get("/api/status"),{ok:!0,needsAuth:!1}}catch(t){return t instanceof mr?{ok:!1,needsAuth:!0}:{ok:!1,needsAuth:!1,error:t.message}}}}class mr extends Error{constructor(t){super(t),this.name="AuthError"}}class Tm extends Error{constructor(t,s,n){super(t),this.name="ApiError",this.status=s,this.data=n}}class Cm{constructor(t){this._api=t,this._ws=null,this._handlers={logs:[],events:[],chat:[]},this._reconnectDelay=1e3,this._maxReconnectDelay=3e4,this._shouldConnect=!1,this._subscriptions=new Set,this._reconnectAttempt=0,this._lastPongTime=0,this._pingInterval=null,this._latency=-1,this._chatPending=!1,this._state="disconnected",this.onStatusChange=null,this.onStateChange=null}get connected(){var t;return((t=this._ws)==null?void 0:t.readyState)===WebSocket.OPEN}get state(){return this._state}get reconnectAttempt(){return this._reconnectAttempt}get latency(){return this._latency}connect(){this._shouldConnect=!0,this._setState("connecting"),this._open()}disconnect(){this._shouldConnect=!1,this._reconnectAttempt=0,this._latency=-1,this._stopPing(),this._ws&&(this._ws.close(),this._ws=null),this._setState("disconnected")}_setState(t){this._state!==t&&(this._state=t,this.onStateChange&&this.onStateChange(t,{attempt:this._reconnectAttempt,latency:this._latency}))}_startPing(){this._stopPing(),this._pingInterval=setInterval(()=>{if(this.connected)try{this._ws.send(JSON.stringify({type:"ping",ts:Date.now()}))}catch{}},15e3)}_stopPing(){this._pingInterval&&(clearInterval(this._pingInterval),this._pingInterval=null)}subscribe(t,s){this._handlers[t]||(this._handlers[t]=[]),this._handlers[t].push(s),t!=="chat"&&(this._subscriptions.add(t),this.connected&&this._ws.send(JSON.stringify({subscribe:t})))}unsubscribe(t,s){const n=this._handlers[t];if(n){const a=n.indexOf(s);a>=0&&n.splice(a,1),n.length===0&&t!=="chat"&&(this._subscriptions.delete(t),this.connected&&this._ws.send(JSON.stringify({unsubscribe:t})))}}on(t,s){return this.subscribe(t,s)}off(t,s){return this.unsubscribe(t,s)}sendChat(t,{channelId:s,userId:n,username:a}={}){return this.connected?(this._ws.send(JSON.stringify({type:"chat",content:t,channel_id:s||"web-default",user_id:n||void 0,username:a||void 0})),this._chatPending=!0,!0):!1}_open(){if(this._ws)return;let s=`${location.protocol==="https:"?"wss:":"ws:"}//${location.host}/api/ws`;this._api.token&&(s+=`?token=${encodeURIComponent(this._api.token)}`),this._ws=new WebSocket(s),this._ws.onopen=()=>{this._reconnectDelay=1e3,this._reconnectAttempt=0;for(const n of this._subscriptions)this._ws.send(JSON.stringify({subscribe:n}));this._startPing(),this._setState("connected"),this.onStatusChange&&this.onStatusChange(!0)},this._ws.onmessage=n=>{let a;try{a=JSON.parse(n.data)}catch{return}const i=a.type;if(i==="pong"){a.ts&&(this._latency=Date.now()-a.ts,this._lastPongTime=Date.now());return}if(i==="log")for(const l of this._handlers.logs||[])l(a);else if(i==="event")for(const l of this._handlers.events||[])l(a);else if(i==="chat_response"||i==="chat_error"){this._chatPending=!1;for(const l of this._handlers.chat||[])l(a)}},this._ws.onclose=()=>{if(this._ws=null,this._stopPing(),this._latency=-1,this._chatPending){this._chatPending=!1;const n={type:"chat_error",error:"Connection lost — the response may still complete; check session history."};for(const a of this._handlers.chat||[])a(n)}this.onStatusChange&&this.onStatusChange(!1),this._shouldConnect?(this._reconnectAttempt++,this._setState("reconnecting"),setTimeout(()=>this._open(),this._reconnectDelay),this._reconnectDelay=Math.min(this._reconnectDelay*2,this._maxReconnectDelay)):this._setState("disconnected")},this._ws.onerror=()=>{}}}const q=new Sm,We=new Cm(q);/**
* @vue/shared v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/function rs(e){const t=Object.create(null);for(const s of e.split(","))t[s]=1;return s=>s in t}const Fe={},ua=[],Lt=()=>{},oa=()=>!1,zn=e=>e.charCodeAt(0)===111&&e.charCodeAt(1)===110&&(e.charCodeAt(2)>122||e.charCodeAt(2)<97),Fl=e=>e.startsWith("onUpdate:"),Pe=Object.assign,To=(e,t)=>{const s=e.indexOf(t);s>-1&&e.splice(s,1)},Em=Object.prototype.hasOwnProperty,ze=(e,t)=>Em.call(e,t),he=Array.isArray,da=e=>Na(e)==="[object Map]",Kn=e=>Na(e)==="[object Set]",Zc=e=>Na(e)==="[object Date]",Am=e=>Na(e)==="[object RegExp]",we=e=>typeof e=="function",Ee=e=>typeof e=="string",Bt=e=>typeof e=="symbol",je=e=>e!==null&&typeof e=="object",Co=e=>(je(e)||we(e))&&we(e.then)&&we(e.catch),Gd=Object.prototype.toString,Na=e=>Gd.call(e),Rm=e=>Na(e).slice(8,-1),$l=e=>Na(e)==="[object Object]",Ul=e=>Ee(e)&&e!=="NaN"&&e[0]!=="-"&&""+parseInt(e,10)===e,Zs=rs(",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted"),Im=rs("bind,cloak,else-if,else,for,html,if,model,on,once,pre,show,slot,text,memo"),Bl=e=>{const t=Object.create(null);return(s=>t[s]||(t[s]=e(s)))},Nm=/-\w/g,Qe=Bl(e=>e.replace(Nm,t=>t.slice(1).toUpperCase())),Om=/\B([A-Z])/g,Xt=Bl(e=>e.replace(Om,"-$1").toLowerCase()),qn=Bl(e=>e.charAt(0).toUpperCase()+e.slice(1)),fa=Bl(e=>e?`on${qn(e)}`:""),Ct=(e,t)=>!Object.is(e,t),pa=(e,...t)=>{for(let s=0;s<e.length;s++)e[s](...t)},Wd=(e,t,s,n=!1)=>{Object.defineProperty(e,t,{configurable:!0,enumerable:!1,writable:n,value:s})},Hl=e=>{const t=parseFloat(e);return isNaN(t)?e:t},rl=e=>{const t=Ee(e)?Number(e):NaN;return isNaN(t)?e:t};let Jc;const Vl=()=>Jc||(Jc=typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{});function Lm(e,t){return e+JSON.stringify(t,(s,n)=>typeof n=="function"?n.toString():n)}const Dm="Infinity,undefined,NaN,isFinite,isNaN,parseFloat,parseInt,decodeURI,decodeURIComponent,encodeURI,encodeURIComponent,Math,Number,Date,Array,Object,Boolean,String,RegExp,Map,Set,JSON,Intl,BigInt,console,Error,Symbol",Mm=rs(Dm);function Si(e){if(he(e)){const t={};for(let s=0;s<e.length;s++){const n=e[s],a=Ee(n)?Zd(n):Si(n);if(a)for(const i in a)t[i]=a[i]}return t}else if(Ee(e)||je(e))return e}const Pm=/;(?![^(]*\))/g,Fm=/:([^]+)/,$m=/\/\*[^]*?\*\//g;function Zd(e){const t={};return e.replace($m,"").split(Pm).forEach(s=>{if(s){const n=s.split(Fm);n.length>1&&(t[n[0].trim()]=n[1].trim())}}),t}function Ti(e){let t="";if(Ee(e))t=e;else if(he(e))for(let s=0;s<e.length;s++){const n=Ti(e[s]);n&&(t+=n+" ")}else if(je(e))for(const s in e)e[s]&&(t+=s+" ");return t.trim()}function Um(e){if(!e)return null;let{class:t,style:s}=e;return t&&!Ee(t)&&(e.class=Ti(t)),s&&(e.style=Si(s)),e}const Bm="html,body,base,head,link,meta,style,title,address,article,aside,footer,header,hgroup,h1,h2,h3,h4,h5,h6,nav,section,div,dd,dl,dt,figcaption,figure,picture,hr,img,li,main,ol,p,pre,ul,a,b,abbr,bdi,bdo,br,cite,code,data,dfn,em,i,kbd,mark,q,rp,rt,ruby,s,samp,small,span,strong,sub,sup,time,u,var,wbr,area,audio,map,track,video,embed,object,param,source,canvas,script,noscript,del,ins,caption,col,colgroup,table,thead,tbody,td,th,tr,button,datalist,fieldset,form,input,label,legend,meter,optgroup,option,output,progress,select,textarea,details,dialog,menu,summary,template,blockquote,iframe,tfoot",Hm="svg,animate,animateMotion,animateTransform,circle,clipPath,color-profile,defs,desc,discard,ellipse,feBlend,feColorMatrix,feComponentTransfer,feComposite,feConvolveMatrix,feDiffuseLighting,feDisplacementMap,feDistantLight,feDropShadow,feFlood,feFuncA,feFuncB,feFuncG,feFuncR,feGaussianBlur,feImage,feMerge,feMergeNode,feMorphology,feOffset,fePointLight,feSpecularLighting,feSpotLight,feTile,feTurbulence,filter,foreignObject,g,hatch,hatchpath,image,line,linearGradient,marker,mask,mesh,meshgradient,meshpatch,meshrow,metadata,mpath,path,pattern,polygon,polyline,radialGradient,rect,set,solidcolor,stop,switch,symbol,text,textPath,title,tspan,unknown,use,view",Vm="annotation,annotation-xml,maction,maligngroup,malignmark,math,menclose,merror,mfenced,mfrac,mfraction,mglyph,mi,mlabeledtr,mlongdiv,mmultiscripts,mn,mo,mover,mpadded,mphantom,mprescripts,mroot,mrow,ms,mscarries,mscarry,msgroup,msline,mspace,msqrt,msrow,mstack,mstyle,msub,msubsup,msup,mtable,mtd,mtext,mtr,munder,munderover,none,semantics",jm="area,base,br,col,embed,hr,img,input,link,meta,param,source,track,wbr",zm=rs(Bm),Km=rs(Hm),qm=rs(Vm),Gm=rs(jm),Wm="itemscope,allowfullscreen,formnovalidate,ismap,nomodule,novalidate,readonly",Zm=rs(Wm);function Jd(e){return!!e||e===""}function Jm(e,t){if(e.length!==t.length)return!1;let s=!0;for(let n=0;s&&n<e.length;n++)s=Xs(e[n],t[n]);return s}function Xs(e,t){if(e===t)return!0;let s=Zc(e),n=Zc(t);if(s||n)return s&&n?e.getTime()===t.getTime():!1;if(s=Bt(e),n=Bt(t),s||n)return e===t;if(s=he(e),n=he(t),s||n)return s&&n?Jm(e,t):!1;if(s=je(e),n=je(t),s||n){if(!s||!n)return!1;const a=Object.keys(e).length,i=Object.keys(t).length;if(a!==i)return!1;for(const l in e){const r=e.hasOwnProperty(l),o=t.hasOwnProperty(l);if(r&&!o||!r&&o||!Xs(e[l],t[l]))return!1}}return String(e)===String(t)}function jl(e,t){return e.findIndex(s=>Xs(s,t))}const Yd=e=>!!(e&&e.__v_isRef===!0),Qd=e=>Ee(e)?e:e==null?"":he(e)||je(e)&&(e.toString===Gd||!we(e.toString))?Yd(e)?Qd(e.value):JSON.stringify(e,Xd,2):String(e),Xd=(e,t)=>Yd(t)?Xd(e,t.value):da(t)?{[`Map(${t.size})`]:[...t.entries()].reduce((s,[n,a],i)=>(s[gr(n,i)+" =>"]=a,s),{})}:Kn(t)?{[`Set(${t.size})`]:[...t.values()].map(s=>gr(s))}:Bt(t)?gr(t):je(t)&&!he(t)&&!$l(t)?String(t):t,gr=(e,t="")=>{var s;return Bt(e)?`Symbol(${(s=e.description)!=null?s:t})`:e};function Ym(e){return e==null?"initial":typeof e=="string"?e===""?" ":e:String(e)}/**
* @vue/reactivity v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/let wt;class Eo{constructor(t=!1){this.detached=t,this._active=!0,this._on=0,this.effects=[],this.cleanups=[],this._isPaused=!1,this._warnOnRun=!0,this.__v_skip=!0,!t&&wt&&(wt.active?(this.parent=wt,this.index=(wt.scopes||(wt.scopes=[])).push(this)-1):(this._active=!1,this._warnOnRun=!1))}get active(){return this._active}pause(){if(this._active){this._isPaused=!0;let t,s;if(this.scopes)for(t=0,s=this.scopes.length;t<s;t++)this.scopes[t].pause();for(t=0,s=this.effects.length;t<s;t++)this.effects[t].pause()}}resume(){if(this._active&&this._isPaused){this._isPaused=!1;let t,s;if(this.scopes)for(t=0,s=this.scopes.length;t<s;t++)this.scopes[t].resume();for(t=0,s=this.effects.length;t<s;t++)this.effects[t].resume()}}run(t){if(this._active){const s=wt;try{return wt=this,t()}finally{wt=s}}}on(){++this._on===1&&(this.prevScope=wt,wt=this)}off(){if(this._on>0&&--this._on===0){if(wt===this)wt=this.prevScope;else{let t=wt;for(;t;){if(t.prevScope===this){t.prevScope=this.prevScope;break}t=t.prevScope}}this.prevScope=void 0}}stop(t){if(this._active){this._active=!1;let s,n;for(s=0,n=this.effects.length;s<n;s++)this.effects[s].stop();for(this.effects.length=0,s=0,n=this.cleanups.length;s<n;s++)this.cleanups[s]();if(this.cleanups.length=0,this.scopes){for(s=0,n=this.scopes.length;s<n;s++)this.scopes[s].stop(!0);this.scopes.length=0}if(!this.detached&&this.parent&&!t){const a=this.parent.scopes.pop();a&&a!==this&&(this.parent.scopes[this.index]=a,a.index=this.index)}this.parent=void 0}}}function Qm(e){return new Eo(e)}function ef(){return wt}function Xm(e,t=!1){wt&&wt.cleanups.push(e)}let st;const vr=new WeakSet;class li{constructor(t){this.fn=t,this.deps=void 0,this.depsTail=void 0,this.flags=5,this.next=void 0,this.cleanup=void 0,this.scheduler=void 0,wt&&(wt.active?wt.effects.push(this):this.flags&=-2)}pause(){this.flags|=64}resume(){this.flags&64&&(this.flags&=-65,vr.has(this)&&(vr.delete(this),this.trigger()))}notify(){this.flags&2&&!(this.flags&32)||this.flags&8||sf(this)}run(){if(!(this.flags&1))return this.fn();this.flags|=2,Yc(this),nf(this);const t=st,s=ys;st=this,ys=!0;try{return this.fn()}finally{af(this),st=t,ys=s,this.flags&=-3}}stop(){if(this.flags&1){for(let t=this.deps;t;t=t.nextDep)Io(t);this.deps=this.depsTail=void 0,Yc(this),this.onStop&&this.onStop(),this.flags&=-2}}trigger(){this.flags&64?vr.add(this):this.scheduler?this.scheduler():this.runIfDirty()}runIfDirty(){jr(this)&&this.run()}get dirty(){return jr(this)}}let tf=0,Ja,Ya;function sf(e,t=!1){if(e.flags|=8,t){e.next=Ya,Ya=e;return}e.next=Ja,Ja=e}function Ao(){tf++}function Ro(){if(--tf>0)return;if(Ya){let t=Ya;for(Ya=void 0;t;){const s=t.next;t.next=void 0,t.flags&=-9,t=s}}let e;for(;Ja;){let t=Ja;for(Ja=void 0;t;){const s=t.next;if(t.next=void 0,t.flags&=-9,t.flags&1)try{t.trigger()}catch(n){e||(e=n)}t=s}}if(e)throw e}function nf(e){for(let t=e.deps;t;t=t.nextDep)t.version=-1,t.prevActiveLink=t.dep.activeLink,t.dep.activeLink=t}function af(e){let t,s=e.depsTail,n=s;for(;n;){const a=n.prevDep;n.version===-1?(n===s&&(s=a),Io(n),eg(n)):t=n,n.dep.activeLink=n.prevActiveLink,n.prevActiveLink=void 0,n=a}e.deps=t,e.depsTail=s}function jr(e){for(let t=e.deps;t;t=t.nextDep)if(t.dep.version!==t.version||t.dep.computed&&(lf(t.dep.computed)||t.dep.version!==t.version))return!0;return!!e._dirty}function lf(e){if(e.flags&4&&!(e.flags&16)||(e.flags&=-17,e.globalVersion===ri)||(e.globalVersion=ri,!e.isSSR&&e.flags&128&&(!e.deps&&!e._dirty||!jr(e))))return;e.flags|=2;const t=e.dep,s=st,n=ys;st=e,ys=!0;try{nf(e);const a=e.fn(e._value);(t.version===0||Ct(a,e._value))&&(e.flags|=128,e._value=a,t.version++)}catch(a){throw t.version++,a}finally{st=s,ys=n,af(e),e.flags&=-3}}function Io(e,t=!1){const{dep:s,prevSub:n,nextSub:a}=e;if(n&&(n.nextSub=a,e.prevSub=void 0),a&&(a.prevSub=n,e.nextSub=void 0),s.subs===e&&(s.subs=n,!n&&s.computed)){s.computed.flags&=-5;for(let i=s.computed.deps;i;i=i.nextDep)Io(i,!0)}!t&&!--s.sc&&s.map&&s.map.delete(s.key)}function eg(e){const{prevDep:t,nextDep:s}=e;t&&(t.nextDep=s,e.prevDep=void 0),s&&(s.prevDep=t,e.nextDep=void 0)}function tg(e,t){e.effect instanceof li&&(e=e.effect.fn);const s=new li(e);t&&Pe(s,t);try{s.run()}catch(a){throw s.stop(),a}const n=s.run.bind(s);return n.effect=s,n}function sg(e){e.effect.stop()}let ys=!0;const rf=[];function en(){rf.push(ys),ys=!1}function tn(){const e=rf.pop();ys=e===void 0?!0:e}function Yc(e){const{cleanup:t}=e;if(e.cleanup=void 0,t){const s=st;st=void 0;try{t()}finally{st=s}}}let ri=0;class ng{constructor(t,s){this.sub=t,this.dep=s,this.version=s.version,this.nextDep=this.prevDep=this.nextSub=this.prevSub=this.prevActiveLink=void 0}}class zl{constructor(t){this.computed=t,this.version=0,this.activeLink=void 0,this.subs=void 0,this.map=void 0,this.key=void 0,this.sc=0,this.__v_skip=!0}track(t){if(!st||!ys||st===this.computed)return;let s=this.activeLink;if(s===void 0||s.sub!==st)s=this.activeLink=new ng(st,this),st.deps?(s.prevDep=st.depsTail,st.depsTail.nextDep=s,st.depsTail=s):st.deps=st.depsTail=s,of(s);else if(s.version===-1&&(s.version=this.version,s.nextDep)){const n=s.nextDep;n.prevDep=s.prevDep,s.prevDep&&(s.prevDep.nextDep=n),s.prevDep=st.depsTail,s.nextDep=void 0,st.depsTail.nextDep=s,st.depsTail=s,st.deps===s&&(st.deps=n)}return s}trigger(t){this.version++,ri++,this.notify(t)}notify(t){Ao();try{for(let s=this.subs;s;s=s.prevSub)s.sub.notify()&&s.sub.dep.notify()}finally{Ro()}}}function of(e){if(e.dep.sc++,e.sub.flags&4){const t=e.dep.computed;if(t&&!e.dep.subs){t.flags|=20;for(let n=t.deps;n;n=n.nextDep)of(n)}const s=e.dep.subs;s!==e&&(e.prevSub=s,s&&(s.nextSub=e)),e.dep.subs=e}}const ol=new WeakMap,Dn=Symbol(""),zr=Symbol(""),oi=Symbol("");function Ft(e,t,s){if(ys&&st){let n=ol.get(e);n||ol.set(e,n=new Map);let a=n.get(s);a||(n.set(s,a=new zl),a.map=n,a.key=s),a.track()}}function zs(e,t,s,n,a,i){const l=ol.get(e);if(!l){ri++;return}const r=o=>{o&&o.trigger()};if(Ao(),t==="clear")l.forEach(r);else{const o=he(e),c=o&&Ul(s);if(o&&s==="length"){const u=Number(n);l.forEach((d,f)=>{(f==="length"||f===oi||!Bt(f)&&f>=u)&&r(d)})}else switch((s!==void 0||l.has(void 0))&&r(l.get(s)),c&&r(l.get(oi)),t){case"add":o?c&&r(l.get("length")):(r(l.get(Dn)),da(e)&&r(l.get(zr)));break;case"delete":o||(r(l.get(Dn)),da(e)&&r(l.get(zr)));break;case"set":da(e)&&r(l.get(Dn));break}}Ro()}function ag(e,t){const s=ol.get(e);return s&&s.get(t)}function Xn(e){const t=Ue(e);return t===e?t:(Ft(t,"iterate",oi),ts(e)?t:t.map(_s))}function Kl(e){return Ft(e=Ue(e),"iterate",oi),e}function Is(e,t){return Os(e)?ya(Js(e)?_s(t):t):_s(t)}const ig={__proto__:null,[Symbol.iterator](){return br(this,Symbol.iterator,e=>Is(this,e))},concat(...e){return Xn(this).concat(...e.map(t=>he(t)?Xn(t):t))},entries(){return br(this,"entries",e=>(e[1]=Is(this,e[1]),e))},every(e,t){return Ps(this,"every",e,t,void 0,arguments)},filter(e,t){return Ps(this,"filter",e,t,s=>s.map(n=>Is(this,n)),arguments)},find(e,t){return Ps(this,"find",e,t,s=>Is(this,s),arguments)},findIndex(e,t){return Ps(this,"findIndex",e,t,void 0,arguments)},findLast(e,t){return Ps(this,"findLast",e,t,s=>Is(this,s),arguments)},findLastIndex(e,t){return Ps(this,"findLastIndex",e,t,void 0,arguments)},forEach(e,t){return Ps(this,"forEach",e,t,void 0,arguments)},includes(...e){return yr(this,"includes",e)},indexOf(...e){return yr(this,"indexOf",e)},join(e){return Xn(this).join(e)},lastIndexOf(...e){return yr(this,"lastIndexOf",e)},map(e,t){return Ps(this,"map",e,t,void 0,arguments)},pop(){return Pa(this,"pop")},push(...e){return Pa(this,"push",e)},reduce(e,...t){return Qc(this,"reduce",e,t)},reduceRight(e,...t){return Qc(this,"reduceRight",e,t)},shift(){return Pa(this,"shift")},some(e,t){return Ps(this,"some",e,t,void 0,arguments)},splice(...e){return Pa(this,"splice",e)},toReversed(){return Xn(this).toReversed()},toSorted(e){return Xn(this).toSorted(e)},toSpliced(...e){return Xn(this).toSpliced(...e)},unshift(...e){return Pa(this,"unshift",e)},values(){return br(this,"values",e=>Is(this,e))}};function br(e,t,s){const n=Kl(e),a=n[t]();return n!==e&&!ts(e)&&(a._next=a.next,a.next=()=>{const i=a._next();return i.done||(i.value=s(i.value)),i}),a}const lg=Array.prototype;function Ps(e,t,s,n,a,i){const l=Kl(e),r=l!==e&&!ts(e),o=l[t];if(o!==lg[t]){const d=o.apply(e,i);return r?_s(d):d}let c=s;l!==e&&(r?c=function(d,f){return s.call(this,Is(e,d),f,e)}:s.length>2&&(c=function(d,f){return s.call(this,d,f,e)}));const u=o.call(l,c,n);return r&&a?a(u):u}function Qc(e,t,s,n){const a=Kl(e),i=a!==e&&!ts(e);let l=s,r=!1;a!==e&&(i?(r=n.length===0,l=function(c,u,d){return r&&(r=!1,c=Is(e,c)),s.call(this,c,Is(e,u),d,e)}):s.length>3&&(l=function(c,u,d){return s.call(this,c,u,d,e)}));const o=a[t](l,...n);return r?Is(e,o):o}function yr(e,t,s){const n=Ue(e);Ft(n,"iterate",oi);const a=n[t](...s);return(a===-1||a===!1)&&Ci(s[0])?(s[0]=Ue(s[0]),n[t](...s)):a}function Pa(e,t,s=[]){en(),Ao();const n=Ue(e)[t].apply(e,s);return Ro(),tn(),n}const rg=rs("__proto__,__v_isRef,__isVue"),cf=new Set(Object.getOwnPropertyNames(Symbol).filter(e=>e!=="arguments"&&e!=="caller").map(e=>Symbol[e]).filter(Bt));function og(e){Bt(e)||(e=String(e));const t=Ue(this);return Ft(t,"has",e),t.hasOwnProperty(e)}class uf{constructor(t=!1,s=!1){this._isReadonly=t,this._isShallow=s}get(t,s,n){if(s==="__v_skip")return t.__v_skip;const a=this._isReadonly,i=this._isShallow;if(s==="__v_isReactive")return!a;if(s==="__v_isReadonly")return a;if(s==="__v_isShallow")return i;if(s==="__v_raw")return n===(a?i?gf:mf:i?hf:pf).get(t)||Object.getPrototypeOf(t)===Object.getPrototypeOf(n)?t:void 0;const l=he(t);if(!a){let o;if(l&&(o=ig[s]))return o;if(s==="hasOwnProperty")return og}const r=Reflect.get(t,s,bt(t)?t:n);if((Bt(s)?cf.has(s):rg(s))||(a||Ft(t,"get",s),i))return r;if(bt(r)){const o=l&&Ul(s)?r:r.value;return a&&je(o)?cl(o):o}return je(r)?a?cl(r):_n(r):r}}class df extends uf{constructor(t=!1){super(!1,t)}set(t,s,n,a){let i=t[s];const l=he(t)&&Ul(s);if(!this._isShallow){const c=Os(i);if(!ts(n)&&!Os(n)&&(i=Ue(i),n=Ue(n)),!l&&bt(i)&&!bt(n))return c||(i.value=n),!0}const r=l?Number(s)<t.length:ze(t,s),o=Reflect.set(t,s,n,bt(t)?t:a);return t===Ue(a)&&(r?Ct(n,i)&&zs(t,"set",s,n):zs(t,"add",s,n)),o}deleteProperty(t,s){const n=ze(t,s);t[s];const a=Reflect.deleteProperty(t,s);return a&&n&&zs(t,"delete",s,void 0),a}has(t,s){const n=Reflect.has(t,s);return(!Bt(s)||!cf.has(s))&&Ft(t,"has",s),n}ownKeys(t){return Ft(t,"iterate",he(t)?"length":Dn),Reflect.ownKeys(t)}}class ff extends uf{constructor(t=!1){super(!0,t)}set(t,s){return!0}deleteProperty(t,s){return!0}}const cg=new df,ug=new ff,dg=new df(!0),fg=new ff(!0),Kr=e=>e,$i=e=>Reflect.getPrototypeOf(e);function pg(e,t,s){return function(...n){const a=this.__v_raw,i=Ue(a),l=da(i),r=e==="entries"||e===Symbol.iterator&&l,o=e==="keys"&&l,c=a[e](...n),u=s?Kr:t?ya:_s;return!t&&Ft(i,"iterate",o?zr:Dn),Pe(Object.create(c),{next(){const{value:d,done:f}=c.next();return f?{value:d,done:f}:{value:r?[u(d[0]),u(d[1])]:u(d),done:f}}})}}function Ui(e){return function(...t){return e==="delete"?!1:e==="clear"?void 0:this}}function hg(e,t){const s={get(a){const i=this.__v_raw,l=Ue(i),r=Ue(a);e||(Ct(a,r)&&Ft(l,"get",a),Ft(l,"get",r));const{has:o}=$i(l),c=t?Kr:e?ya:_s;if(o.call(l,a))return c(i.get(a));if(o.call(l,r))return c(i.get(r));i!==l&&i.get(a)},get size(){const a=this.__v_raw;return!e&&Ft(Ue(a),"iterate",Dn),a.size},has(a){const i=this.__v_raw,l=Ue(i),r=Ue(a);return e||(Ct(a,r)&&Ft(l,"has",a),Ft(l,"has",r)),a===r?i.has(a):i.has(a)||i.has(r)},forEach(a,i){const l=this,r=l.__v_raw,o=Ue(r),c=t?Kr:e?ya:_s;return!e&&Ft(o,"iterate",Dn),r.forEach((u,d)=>a.call(i,c(u),c(d),l))}};return Pe(s,e?{add:Ui("add"),set:Ui("set"),delete:Ui("delete"),clear:Ui("clear")}:{add(a){const i=Ue(this),l=$i(i),r=Ue(a),o=!t&&!ts(a)&&!Os(a)?r:a;return l.has.call(i,o)||Ct(a,o)&&l.has.call(i,a)||Ct(r,o)&&l.has.call(i,r)||(i.add(o),zs(i,"add",o,o)),this},set(a,i){!t&&!ts(i)&&!Os(i)&&(i=Ue(i));const l=Ue(this),{has:r,get:o}=$i(l);let c=r.call(l,a);c||(a=Ue(a),c=r.call(l,a));const u=o.call(l,a);return l.set(a,i),c?Ct(i,u)&&zs(l,"set",a,i):zs(l,"add",a,i),this},delete(a){const i=Ue(this),{has:l,get:r}=$i(i);let o=l.call(i,a);o||(a=Ue(a),o=l.call(i,a)),r&&r.call(i,a);const c=i.delete(a);return o&&zs(i,"delete",a,void 0),c},clear(){const a=Ue(this),i=a.size!==0,l=a.clear();return i&&zs(a,"clear",void 0,void 0),l}}),["keys","values","entries",Symbol.iterator].forEach(a=>{s[a]=pg(a,e,t)}),s}function ql(e,t){const s=hg(e,t);return(n,a,i)=>a==="__v_isReactive"?!e:a==="__v_isReadonly"?e:a==="__v_raw"?n:Reflect.get(ze(s,a)&&a in n?s:n,a,i)}const mg={get:ql(!1,!1)},gg={get:ql(!1,!0)},vg={get:ql(!0,!1)},bg={get:ql(!0,!0)},pf=new WeakMap,hf=new WeakMap,mf=new WeakMap,gf=new WeakMap;function yg(e){switch(e){case"Object":case"Array":return 1;case"Map":case"Set":case"WeakMap":case"WeakSet":return 2;default:return 0}}function _n(e){return Os(e)?e:Gl(e,!1,cg,mg,pf)}function No(e){return Gl(e,!1,dg,gg,hf)}function cl(e){return Gl(e,!0,ug,vg,mf)}function xg(e){return Gl(e,!0,fg,bg,gf)}function Gl(e,t,s,n,a){if(!je(e)||e.__v_raw&&!(t&&e.__v_isReactive)||e.__v_skip||!Object.isExtensible(e))return e;const i=a.get(e);if(i)return i;const l=yg(Rm(e));if(l===0)return e;const r=new Proxy(e,l===2?n:s);return a.set(e,r),r}function Js(e){return Os(e)?Js(e.__v_raw):!!(e&&e.__v_isReactive)}function Os(e){return!!(e&&e.__v_isReadonly)}function ts(e){return!!(e&&e.__v_isShallow)}function Ci(e){return e?!!e.__v_raw:!1}function Ue(e){const t=e&&e.__v_raw;return t?Ue(t):e}function vf(e){return!ze(e,"__v_skip")&&Object.isExtensible(e)&&Wd(e,"__v_skip",!0),e}const _s=e=>je(e)?_n(e):e,ya=e=>je(e)?cl(e):e;function bt(e){return e?e.__v_isRef===!0:!1}function h(e){return bf(e,!1)}function Oo(e){return bf(e,!0)}function bf(e,t){return bt(e)?e:new _g(e,t)}class _g{constructor(t,s){this.dep=new zl,this.__v_isRef=!0,this.__v_isShallow=!1,this._rawValue=s?t:Ue(t),this._value=s?t:_s(t),this.__v_isShallow=s}get value(){return this.dep.track(),this._value}set value(t){const s=this._rawValue,n=this.__v_isShallow||ts(t)||Os(t);t=n?t:Ue(t),Ct(t,s)&&(this._rawValue=t,this._value=n?t:_s(t),this.dep.trigger())}}function kg(e){e.dep&&e.dep.trigger()}function Ns(e){return bt(e)?e.value:e}function wg(e){return we(e)?e():Ns(e)}const Sg={get:(e,t,s)=>t==="__v_raw"?e:Ns(Reflect.get(e,t,s)),set:(e,t,s,n)=>{const a=e[t];return bt(a)&&!bt(s)?(a.value=s,!0):Reflect.set(e,t,s,n)}};function Lo(e){return Js(e)?e:new Proxy(e,Sg)}class Tg{constructor(t){this.__v_isRef=!0,this._value=void 0;const s=this.dep=new zl,{get:n,set:a}=t(s.track.bind(s),s.trigger.bind(s));this._get=n,this._set=a}get value(){return this._value=this._get()}set value(t){this._set(t)}}function yf(e){return new Tg(e)}function Cg(e){const t=he(e)?new Array(e.length):{};for(const s in e)t[s]=xf(e,s);return t}class Eg{constructor(t,s,n){this._object=t,this._defaultValue=n,this.__v_isRef=!0,this._value=void 0,this._key=Bt(s)?s:String(s),this._raw=Ue(t);let a=!0,i=t;if(!he(t)||Bt(this._key)||!Ul(this._key))do a=!Ci(i)||ts(i);while(a&&(i=i.__v_raw));this._shallow=a}get value(){let t=this._object[this._key];return this._shallow&&(t=Ns(t)),this._value=t===void 0?this._defaultValue:t}set value(t){if(this._shallow&&bt(this._raw[this._key])){const s=this._object[this._key];if(bt(s)){s.value=t;return}}this._object[this._key]=t}get dep(){return ag(this._raw,this._key)}}class Ag{constructor(t){this._getter=t,this.__v_isRef=!0,this.__v_isReadonly=!0,this._value=void 0}get value(){return this._value=this._getter()}}function Rg(e,t,s){return bt(e)?e:we(e)?new Ag(e):je(e)&&arguments.length>1?xf(e,t,s):h(e)}function xf(e,t,s){return new Eg(e,t,s)}class Ig{constructor(t,s,n){this.fn=t,this.setter=s,this._value=void 0,this.dep=new zl(this),this.__v_isRef=!0,this.deps=void 0,this.depsTail=void 0,this.flags=16,this.globalVersion=ri-1,this.next=void 0,this.effect=this,this.__v_isReadonly=!s,this.isSSR=n}notify(){if(this.flags|=16,!(this.flags&8)&&st!==this)return sf(this,!0),!0}get value(){const t=this.dep.track();return lf(this),t&&(t.version=this.dep.version),this._value}set value(t){this.setter&&this.setter(t)}}function Ng(e,t,s=!1){let n,a;return we(e)?n=e:(n=e.get,a=e.set),new Ig(n,a,s)}const Og={GET:"get",HAS:"has",ITERATE:"iterate"},Lg={SET:"set",ADD:"add",DELETE:"delete",CLEAR:"clear"},Bi={},ul=new WeakMap;let hn;function Dg(){return hn}function _f(e,t=!1,s=hn){if(s){let n=ul.get(s);n||ul.set(s,n=[]),n.push(e)}}function Mg(e,t,s=Fe){const{immediate:n,deep:a,once:i,scheduler:l,augmentJob:r,call:o}=s,c=x=>a?x:ts(x)||a===!1||a===0?Ks(x,1):Ks(x);let u,d,f,p,b=!1,m=!1;if(bt(e)?(d=()=>e.value,b=ts(e)):Js(e)?(d=()=>c(e),b=!0):he(e)?(m=!0,b=e.some(x=>Js(x)||ts(x)),d=()=>e.map(x=>{if(bt(x))return x.value;if(Js(x))return c(x);if(we(x))return o?o(x,2):x()})):we(e)?t?d=o?()=>o(e,2):e:d=()=>{if(f){en();try{f()}finally{tn()}}const x=hn;hn=u;try{return o?o(e,3,[p]):e(p)}finally{hn=x}}:d=Lt,t&&a){const x=d,T=a===!0?1/0:a;d=()=>Ks(x(),T)}const k=ef(),E=()=>{u.stop(),k&&k.active&&To(k.effects,u)};if(i&&t){const x=t;t=(...T)=>{const N=x(...T);return E(),N}}let v=m?new Array(e.length).fill(Bi):Bi;const y=x=>{if(!(!(u.flags&1)||!u.dirty&&!x))if(t){const T=u.run();if(x||a||b||(m?T.some((N,O)=>Ct(N,v[O])):Ct(T,v))){f&&f();const N=hn;hn=u;try{const O=[T,v===Bi?void 0:m&&v[0]===Bi?[]:v,p];v=T,o?o(t,3,O):t(...O)}finally{hn=N}}}else u.run()};return r&&r(y),u=new li(d),u.scheduler=l?()=>l(y,!1):y,p=x=>_f(x,!1,u),f=u.onStop=()=>{const x=ul.get(u);if(x){if(o)o(x,4);else for(const T of x)T();ul.delete(u)}},t?n?y(!0):v=u.run():l?l(y.bind(null,!0),!0):u.run(),E.pause=u.pause.bind(u),E.resume=u.resume.bind(u),E.stop=E,E}function Ks(e,t=1/0,s){if(t<=0||!je(e)||e.__v_skip||(s=s||new Map,(s.get(e)||0)>=t))return e;if(s.set(e,t),t--,bt(e))Ks(e.value,t,s);else if(he(e))for(let n=0;n<e.length;n++)Ks(e[n],t,s);else if(Kn(e)||da(e))e.forEach(n=>{Ks(n,t,s)});else if($l(e)){for(const n in e)Ks(e[n],t,s);for(const n of Object.getOwnPropertySymbols(e))Object.prototype.propertyIsEnumerable.call(e,n)&&Ks(e[n],t,s)}return e}/**
* @vue/runtime-core v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const kf=[];function Pg(e){kf.push(e)}function Fg(){kf.pop()}function $g(e,t){}const Ug={SETUP_FUNCTION:0,0:"SETUP_FUNCTION",RENDER_FUNCTION:1,1:"RENDER_FUNCTION",NATIVE_EVENT_HANDLER:5,5:"NATIVE_EVENT_HANDLER",COMPONENT_EVENT_HANDLER:6,6:"COMPONENT_EVENT_HANDLER",VNODE_HOOK:7,7:"VNODE_HOOK",DIRECTIVE_HOOK:8,8:"DIRECTIVE_HOOK",TRANSITION_HOOK:9,9:"TRANSITION_HOOK",APP_ERROR_HANDLER:10,10:"APP_ERROR_HANDLER",APP_WARN_HANDLER:11,11:"APP_WARN_HANDLER",FUNCTION_REF:12,12:"FUNCTION_REF",ASYNC_COMPONENT_LOADER:13,13:"ASYNC_COMPONENT_LOADER",SCHEDULER:14,14:"SCHEDULER",COMPONENT_UPDATE:15,15:"COMPONENT_UPDATE",APP_UNMOUNT_CLEANUP:16,16:"APP_UNMOUNT_CLEANUP"},Bg={sp:"serverPrefetch hook",bc:"beforeCreate hook",c:"created hook",bm:"beforeMount hook",m:"mounted hook",bu:"beforeUpdate hook",u:"updated",bum:"beforeUnmount hook",um:"unmounted hook",a:"activated hook",da:"deactivated hook",ec:"errorCaptured hook",rtc:"renderTracked hook",rtg:"renderTriggered hook",0:"setup function",1:"render function",2:"watcher getter",3:"watcher callback",4:"watcher cleanup function",5:"native event handler",6:"component event handler",7:"vnode hook",8:"directive hook",9:"transition hook",10:"app errorHandler",11:"app warnHandler",12:"ref function",13:"async component loader",14:"scheduler flush",15:"component update",16:"app unmount cleanup function"};function Oa(e,t,s,n){try{return n?e(...n):e()}catch(a){Gn(a,t,s)}}function is(e,t,s,n){if(we(e)){const a=Oa(e,t,s,n);return a&&Co(a)&&a.catch(i=>{Gn(i,t,s)}),a}if(he(e)){const a=[];for(let i=0;i<e.length;i++)a.push(is(e[i],t,s,n));return a}}function Gn(e,t,s,n=!0){const a=t?t.vnode:null,{errorHandler:i,throwUnhandledErrorInProduction:l}=t&&t.appContext.config||Fe;if(t){let r=t.parent;const o=t.proxy,c=`https://vuejs.org/error-reference/#runtime-${s}`;for(;r;){const u=r.ec;if(u){for(let d=0;d<u.length;d++)if(u[d](e,o,c)===!1)return}r=r.parent}if(i){en(),Oa(i,null,10,[e,o,c]),tn();return}}Hg(e,s,a,n,l)}function Hg(e,t,s,n=!0,a=!1){if(a)throw e;console.error(e)}const zt=[];let As=-1;const ha=[];let mn=null,aa=0;const wf=Promise.resolve();let dl=null;function Et(e){const t=dl||wf;return e?t.then(this?e.bind(this):e):t}function Vg(e){let t=As+1,s=zt.length;for(;t<s;){const n=t+s>>>1,a=zt[n],i=ui(a);i<e||i===e&&a.flags&2?t=n+1:s=n}return t}function Do(e){if(!(e.flags&1)){const t=ui(e),s=zt[zt.length-1];!s||!(e.flags&2)&&t>=ui(s)?zt.push(e):zt.splice(Vg(t),0,e),e.flags|=1,Sf()}}function Sf(){dl||(dl=wf.then(Tf))}function ci(e){he(e)?ha.push(...e):mn&&e.id===-1?mn.splice(aa+1,0,e):e.flags&1||(ha.push(e),e.flags|=1),Sf()}function Xc(e,t,s=As+1){for(;s<zt.length;s++){const n=zt[s];if(n&&n.flags&2){if(e&&n.id!==e.uid)continue;zt.splice(s,1),s--,n.flags&4&&(n.flags&=-2),n(),n.flags&4||(n.flags&=-2)}}}function fl(e){if(ha.length){const t=[...new Set(ha)].sort((s,n)=>ui(s)-ui(n));if(ha.length=0,mn){mn.push(...t);return}for(mn=t,aa=0;aa<mn.length;aa++){const s=mn[aa];s.flags&4&&(s.flags&=-2),s.flags&8||s(),s.flags&=-2}mn=null,aa=0}}const ui=e=>e.id==null?e.flags&2?-1:1/0:e.id;function Tf(e){try{for(As=0;As<zt.length;As++){const t=zt[As];t&&!(t.flags&8)&&(t.flags&4&&(t.flags&=-2),Oa(t,t.i,t.i?15:14),t.flags&4||(t.flags&=-2))}}finally{for(;As<zt.length;As++){const t=zt[As];t&&(t.flags&=-2)}As=-1,zt.length=0,fl(),dl=null,(zt.length||ha.length)&&Tf()}}let ia,Hi=[];function Cf(e,t){var s,n;ia=e,ia?(ia.enabled=!0,Hi.forEach(({event:a,args:i})=>ia.emit(a,...i)),Hi=[]):typeof window<"u"&&window.HTMLElement&&!((n=(s=window.navigator)==null?void 0:s.userAgent)!=null&&n.includes("jsdom"))?((t.__VUE_DEVTOOLS_HOOK_REPLAY__=t.__VUE_DEVTOOLS_HOOK_REPLAY__||[]).push(i=>{Cf(i,t)}),setTimeout(()=>{ia||(t.__VUE_DEVTOOLS_HOOK_REPLAY__=null,Hi=[])},3e3)):Hi=[]}let Ot=null,Wl=null;function di(e){const t=Ot;return Ot=e,Wl=e&&e.type.__scopeId||null,t}function jg(e){Wl=e}function zg(){Wl=null}const Kg=e=>Mo;function Mo(e,t=Ot,s){if(!t||e._n)return e;const n=(...a)=>{n._d&&mi(-1);const i=di(t);let l;try{l=e(...a)}finally{di(i),n._d&&mi(1)}return l};return n._n=!0,n._c=!0,n._d=!0,n}function qg(e,t){if(Ot===null)return e;const s=Ii(Ot),n=e.dirs||(e.dirs=[]);for(let a=0;a<t.length;a++){let[i,l,r,o=Fe]=t[a];i&&(we(i)&&(i={mounted:i,updated:i}),i.deep&&Ks(l),n.push({dir:i,instance:s,value:l,oldValue:void 0,arg:r,modifiers:o}))}return e}function Rs(e,t,s,n){const a=e.dirs,i=t&&t.dirs;for(let l=0;l<a.length;l++){const r=a[l];i&&(r.oldValue=i[l].value);let o=r.dir[n];o&&(en(),is(o,s,8,[e.el,r,e,t]),tn())}}function Qa(e,t){if(Nt){let s=Nt.provides;const n=Nt.parent&&Nt.parent.provides;n===s&&(s=Nt.provides=Object.create(n)),s[e]=t}}function ps(e,t,s=!1){const n=qt();if(n||Mn){let a=Mn?Mn._context.provides:n?n.parent==null||n.ce?n.vnode.appContext&&n.vnode.appContext.provides:n.parent.provides:void 0;if(a&&e in a)return a[e];if(arguments.length>1)return s&&we(t)?t.call(n&&n.proxy):t}}function Gg(){return!!(qt()||Mn)}const Ef=Symbol.for("v-scx"),Af=()=>ps(Ef);function Wg(e,t){return Ei(e,null,t)}function Zg(e,t){return Ei(e,null,{flush:"post"})}function Rf(e,t){return Ei(e,null,{flush:"sync"})}function hs(e,t,s){return Ei(e,t,s)}function Ei(e,t,s=Fe){const{immediate:n,deep:a,flush:i,once:l}=s,r=Pe({},s),o=t&&n||!t&&i!=="post";let c;if(Hn){if(i==="sync"){const p=Af();c=p.__watcherHandles||(p.__watcherHandles=[])}else if(!o){const p=()=>{};return p.stop=Lt,p.resume=Lt,p.pause=Lt,p}}const u=Nt;r.call=(p,b,m)=>is(p,u,b,m);let d=!1;i==="post"?r.scheduler=p=>{gt(p,u&&u.suspense)}:i!=="sync"&&(d=!0,r.scheduler=(p,b)=>{b?p():Do(p)}),r.augmentJob=p=>{t&&(p.flags|=4),d&&(p.flags|=2,u&&(p.id=u.uid,p.i=u))};const f=Mg(e,t,r);return Hn&&(c?c.push(f):o&&f()),f}function Jg(e,t,s){const n=this.proxy,a=Ee(e)?e.includes(".")?If(n,e):()=>n[e]:e.bind(n,n);let i;we(t)?i=t:(i=t.handler,s=t);const l=La(this),r=Ei(a,i.bind(n),s);return l(),r}function If(e,t){const s=t.split(".");return()=>{let n=e;for(let a=0;a<s.length&&n;a++)n=n[s[a]];return n}}const fn=new WeakMap,Nf=Symbol("_vte"),Of=e=>e.__isTeleport,In=e=>e&&(e.disabled||e.disabled===""),Yg=e=>e&&(e.defer||e.defer===""),eu=e=>typeof SVGElement<"u"&&e instanceof SVGElement,tu=e=>typeof MathMLElement=="function"&&e instanceof MathMLElement,qr=(e,t)=>{const s=e&&e.to;return Ee(s)?t?t(s):null:s},Qg={name:"Teleport",__isTeleport:!0,process(e,t,s,n,a,i,l,r,o,c){const{mc:u,pc:d,pbc:f,o:{insert:p,querySelector:b,createText:m,createComment:k,parentNode:E}}=c,v=In(t.props);let{dynamicChildren:y}=t;const x=(O,w,R)=>{O.shapeFlag&16&&u(O.children,w,R,a,i,l,r,o)},T=(O=t)=>{const w=In(O.props),R=O.target=qr(O.props,b),L=Gr(R,O,m,p);R&&(l!=="svg"&&eu(R)?l="svg":l!=="mathml"&&tu(R)&&(l="mathml"),a&&a.isCE&&(a.ce._teleportTargets||(a.ce._teleportTargets=new Set)).add(R),w||(x(O,R,L),Ka(O,!1)))},N=O=>{const w=()=>{if(fn.get(O)===w){if(fn.delete(O),In(O.props)){const R=E(O.el)||s;x(O,R,O.anchor),Ka(O,!0)}T(O)}};fn.set(O,w),gt(w,i)};if(e==null){const O=t.el=m(""),w=t.anchor=m("");if(p(O,s,n),p(w,s,n),Yg(t.props)||i&&i.pendingBranch){N(t);return}v&&(x(t,s,w),Ka(t,!0)),T()}else{t.el=e.el;const O=t.anchor=e.anchor,w=fn.get(e);if(w){w.flags|=8,fn.delete(e),N(t);return}t.targetStart=e.targetStart;const R=t.target=e.target,L=t.targetAnchor=e.targetAnchor,U=In(e.props),P=U?s:R,S=U?O:L;if(l==="svg"||eu(R)?l="svg":(l==="mathml"||tu(R))&&(l="mathml"),y?(f(e.dynamicChildren,y,P,a,i,l,r),Wo(e,t,!0)):o||d(e,t,P,S,a,i,l,r,!1),v)U?t.props&&e.props&&t.props.to!==e.props.to&&(t.props.to=e.props.to):Vi(t,s,O,c,1);else if((t.props&&t.props.to)!==(e.props&&e.props.to)){const $=t.target=qr(t.props,b);$&&Vi(t,$,null,c,0)}else U&&Vi(t,R,L,c,1);Ka(t,v)}},remove(e,t,s,{um:n,o:{remove:a}},i){const{shapeFlag:l,children:r,anchor:o,targetStart:c,targetAnchor:u,target:d,props:f}=e,p=i||!In(f),b=fn.get(e);if(b&&(b.flags|=8,fn.delete(e)),d&&(a(c),a(u)),i&&a(o),!b&&l&16)for(let m=0;m<r.length;m++){const k=r[m];n(k,t,s,p,!!k.dynamicChildren)}},move:Vi,hydrate:Xg};function Vi(e,t,s,{o:{insert:n},m:a},i=2){i===0&&n(e.targetAnchor,t,s);const{el:l,anchor:r,shapeFlag:o,children:c,props:u}=e,d=i===2;if(d&&n(l,t,s),!fn.has(e)&&(!d||In(u))&&o&16)for(let f=0;f<c.length;f++)a(c[f],t,s,2);d&&n(r,t,s)}function Xg(e,t,s,n,a,i,{o:{nextSibling:l,parentNode:r,querySelector:o,insert:c,createText:u}},d){function f(k,E){let v=E;for(;v;){if(v&&v.nodeType===8){if(v.data==="teleport start anchor")t.targetStart=v;else if(v.data==="teleport anchor"){t.targetAnchor=v,k._lpa=t.targetAnchor&&l(t.targetAnchor);break}}v=l(v)}}function p(k,E){E.anchor=d(l(k),E,r(k),s,n,a,i)}const b=t.target=qr(t.props,o),m=In(t.props);if(b){const k=b._lpa||b.firstChild;t.shapeFlag&16&&(m?(p(e,t),f(b,k),t.targetAnchor||Gr(b,t,u,c,r(e)===b?e:null)):(t.anchor=l(e),f(b,k),t.targetAnchor||Gr(b,t,u,c),d(k&&l(k),t,b,s,n,a,i))),Ka(t,m)}else m&&t.shapeFlag&16&&(p(e,t),t.targetStart=e,t.targetAnchor=l(e));return t.anchor&&l(t.anchor)}const ev=Qg;function Ka(e,t){const s=e.ctx;if(s&&s.ut){let n,a;for(t?(n=e.el,a=e.anchor):(n=e.targetStart,a=e.targetAnchor);n&&n!==a;)n.nodeType===1&&n.setAttribute("data-v-owner",s.uid),n=n.nextSibling;s.ut()}}function Gr(e,t,s,n,a=null){const i=t.targetStart=s(""),l=t.targetAnchor=s("");return i[Nf]=l,e&&(n(i,e,a),n(l,e,a)),l}const us=Symbol("_leaveCb"),Fa=Symbol("_enterCb");function Po(){const e={isMounted:!1,isLeaving:!1,isUnmounting:!1,leavingVNodes:new Map};return Be(()=>{e.isMounted=!0}),Ql(()=>{e.isUnmounting=!0}),e}const cs=[Function,Array],Fo={mode:String,appear:Boolean,persisted:Boolean,onBeforeEnter:cs,onEnter:cs,onAfterEnter:cs,onEnterCancelled:cs,onBeforeLeave:cs,onLeave:cs,onAfterLeave:cs,onLeaveCancelled:cs,onBeforeAppear:cs,onAppear:cs,onAfterAppear:cs,onAppearCancelled:cs},Lf=e=>{const t=e.subTree;return t.component?Lf(t.component):t},tv={name:"BaseTransition",props:Fo,setup(e,{slots:t}){const s=qt(),n=Po();return()=>{const a=t.default&&Zl(t.default(),!0),i=a&&a.length?Df(a):s.subTree?gp():void 0;if(!i)return;const l=Ue(e),{mode:r}=l;if(n.isLeaving)return xr(i);const o=su(i);if(!o)return xr(i);let c=xa(o,l,n,s,d=>c=d);o.type!==pt&&sn(o,c);let u=s.subTree&&su(s.subTree);if(u&&u.type!==pt&&!bs(u,o)&&Lf(s).type!==pt){let d=xa(u,l,n,s);if(sn(u,d),r==="out-in"&&o.type!==pt)return n.isLeaving=!0,d.afterLeave=()=>{n.isLeaving=!1,s.job.flags&8||s.update(),delete d.afterLeave,u=void 0},xr(i);r==="in-out"&&o.type!==pt?d.delayLeave=(f,p,b)=>{const m=Pf(n,u);m[String(u.key)]=u,f[us]=()=>{p(),f[us]=void 0,delete c.delayedLeave,u=void 0},c.delayedLeave=()=>{b(),delete c.delayedLeave,u=void 0}}:u=void 0}else u&&(u=void 0);return i}}};function Df(e){let t=e[0];if(e.length>1){for(const s of e)if(s.type!==pt){t=s;break}}return t}const Mf=tv;function Pf(e,t){const{leavingVNodes:s}=e;let n=s.get(t.type);return n||(n=Object.create(null),s.set(t.type,n)),n}function xa(e,t,s,n,a){const{appear:i,mode:l,persisted:r=!1,onBeforeEnter:o,onEnter:c,onAfterEnter:u,onEnterCancelled:d,onBeforeLeave:f,onLeave:p,onAfterLeave:b,onLeaveCancelled:m,onBeforeAppear:k,onAppear:E,onAfterAppear:v,onAppearCancelled:y}=t,x=String(e.key),T=Pf(s,e),N=(R,L)=>{R&&is(R,n,9,L)},O=(R,L)=>{const U=L[1];N(R,L),he(R)?R.every(P=>P.length<=1)&&U():R.length<=1&&U()},w={mode:l,persisted:r,beforeEnter(R){let L=o;if(!s.isMounted)if(i)L=k||o;else return;R[us]&&R[us](!0);const U=T[x];U&&bs(e,U)&&U.el[us]&&U.el[us](),N(L,[R])},enter(R){if(T[x]===e)return;let L=c,U=u,P=d;if(!s.isMounted)if(i)L=E||c,U=v||u,P=y||d;else return;let S=!1;R[Fa]=W=>{S||(S=!0,W?N(P,[R]):N(U,[R]),w.delayedLeave&&w.delayedLeave(),R[Fa]=void 0)};const $=R[Fa].bind(null,!1);L?O(L,[R,$]):$()},leave(R,L){const U=String(e.key);if(R[Fa]&&R[Fa](!0),s.isUnmounting)return L();N(f,[R]);let P=!1;R[us]=$=>{P||(P=!0,L(),$?N(m,[R]):N(b,[R]),R[us]=void 0,T[U]===e&&delete T[U])};const S=R[us].bind(null,!1);T[U]=e,p?O(p,[R,S]):S()},clone(R){const L=xa(R,t,s,n,a);return a&&a(L),L}};return w}function xr(e){if(Ri(e))return e=Ls(e),e.children=null,e}function su(e){if(!Ri(e))return Of(e.type)&&e.children?Df(e.children):e;if(e.component)return e.component.subTree;const{shapeFlag:t,children:s}=e;if(s){if(t&16)return s[0];if(t&32&&we(s.default))return s.default()}}function sn(e,t){e.shapeFlag&6&&e.component?(e.transition=t,sn(e.component.subTree,t)):e.shapeFlag&128?(e.ssContent.transition=t.clone(e.ssContent),e.ssFallback.transition=t.clone(e.ssFallback)):e.transition=t}function Zl(e,t=!1,s){let n=[],a=0;for(let i=0;i<e.length;i++){let l=e[i];const r=s==null?l.key:String(s)+String(l.key!=null?l.key:i);l.type===At?(l.patchFlag&128&&a++,n=n.concat(Zl(l.children,t,r))):(t||l.type!==pt)&&n.push(r!=null?Ls(l,{key:r}):l)}if(a>1)for(let i=0;i<n.length;i++)n[i].patchFlag=-2;return n}function Ai(e,t){return we(e)?Pe({name:e.name},t,{setup:e}):e}function sv(){const e=qt();return e?(e.appContext.config.idPrefix||"v")+"-"+e.ids[0]+e.ids[1]++:""}function $o(e){e.ids=[e.ids[0]+e.ids[2]+++"-",0,0]}function nv(e){const t=qt(),s=Oo(null);if(t){const a=t.refs===Fe?t.refs={}:t.refs;Object.defineProperty(a,e,{enumerable:!0,get:()=>s.value,set:i=>s.value=i})}return s}function nu(e,t){let s;return!!((s=Object.getOwnPropertyDescriptor(e,t))&&!s.configurable)}const pl=new WeakMap;function ma(e,t,s,n,a=!1){if(he(e)){e.forEach((m,k)=>ma(m,t&&(he(t)?t[k]:t),s,n,a));return}if(Ys(n)&&!a){n.shapeFlag&512&&n.type.__asyncResolved&&n.component.subTree.component&&ma(e,t,s,n.component.subTree);return}const i=n.shapeFlag&4?Ii(n.component):n.el,l=a?null:i,{i:r,r:o}=e,c=t&&t.r,u=r.refs===Fe?r.refs={}:r.refs,d=r.setupState,f=Ue(d),p=d===Fe?oa:m=>nu(u,m)?!1:ze(f,m),b=(m,k)=>!(k&&nu(u,k));if(c!=null&&c!==o){if(au(t),Ee(c))u[c]=null,p(c)&&(d[c]=null);else if(bt(c)){const m=t;b(c,m.k)&&(c.value=null),m.k&&(u[m.k]=null)}}if(we(o))Oa(o,r,12,[l,u]);else{const m=Ee(o),k=bt(o);if(m||k){const E=()=>{if(e.f){const v=m?p(o)?d[o]:u[o]:b()||!e.k?o.value:u[e.k];if(a)he(v)&&To(v,i);else if(he(v))v.includes(i)||v.push(i);else if(m)u[o]=[i],p(o)&&(d[o]=u[o]);else{const y=[i];b(o,e.k)&&(o.value=y),e.k&&(u[e.k]=y)}}else m?(u[o]=l,p(o)&&(d[o]=l)):k&&(b(o,e.k)&&(o.value=l),e.k&&(u[e.k]=l))};if(l){const v=()=>{E(),pl.delete(e)};v.id=-1,pl.set(e,v),gt(v,s)}else au(e),E()}}}function au(e){const t=pl.get(e);t&&(t.flags|=8,pl.delete(e))}let iu=!1;const ea=()=>{iu||(console.error("Hydration completed but contains mismatches."),iu=!0)},av=e=>e.namespaceURI.includes("svg")&&e.tagName!=="foreignObject",iv=e=>e.namespaceURI.includes("MathML"),ji=e=>{if(e.nodeType===1){if(av(e))return"svg";if(iv(e))return"mathml"}},ca=e=>e.nodeType===8;function lv(e){const{mt:t,p:s,o:{patchProp:n,createText:a,nextSibling:i,parentNode:l,remove:r,insert:o,createComment:c}}=e,u=(y,x)=>{if(!x.hasChildNodes()){s(null,y,x),fl(),x._vnode=y;return}d(x.firstChild,y,null,null,null),fl(),x._vnode=y},d=(y,x,T,N,O,w=!1)=>{w=w||!!x.dynamicChildren;const R=ca(y)&&y.data==="[",L=()=>m(y,x,T,N,O,R),{type:U,ref:P,shapeFlag:S,patchFlag:$}=x;let W=y.nodeType;x.el=y,$===-2&&(w=!1,x.dynamicChildren=null);let G=null;switch(U){case bn:W!==3?x.children===""?(o(x.el=a(""),l(y),y),G=y):G=L():(y.data!==x.children&&(ea(),y.data=x.children),G=i(y));break;case pt:v(y)?(G=i(y),E(x.el=y.content.firstChild,y,T)):W!==8||R?G=L():G=i(y);break;case Pn:if(R&&(y=i(y),W=y.nodeType),W===1||W===3){G=y;const D=!x.children.length;for(let I=0;I<x.staticCount;I++)D&&(x.children+=G.nodeType===1?G.outerHTML:G.data),I===x.staticCount-1&&(x.anchor=G),G=i(G);return R?i(G):G}else L();break;case At:R?G=b(y,x,T,N,O,w):G=L();break;default:if(S&1)(W!==1||x.type.toLowerCase()!==y.tagName.toLowerCase())&&!v(y)?G=L():G=f(y,x,T,N,O,w);else if(S&6){x.slotScopeIds=O;const D=l(y);if(R?G=k(y):ca(y)&&y.data==="teleport start"?G=k(y,y.data,"teleport end"):G=i(y),t(x,D,null,T,N,ji(D),w),Ys(x)&&!x.type.__asyncResolved){let I;R?(I=lt(At),I.anchor=G?G.previousSibling:D.lastChild):I=y.nodeType===3?Jo(""):lt("div"),I.el=y,x.component.subTree=I}}else S&64?W!==8?G=L():G=x.type.hydrate(y,x,T,N,O,w,e,p):S&128&&(G=x.type.hydrate(y,x,T,N,ji(l(y)),O,w,e,d))}return P!=null&&ma(P,null,N,x),G},f=(y,x,T,N,O,w)=>{w=w||!!x.dynamicChildren;const{type:R,props:L,patchFlag:U,shapeFlag:P,dirs:S,transition:$}=x,W=R==="input"||R==="option";if(W||U!==-1){S&&Rs(x,null,T,"created");let G=!1;if(v(y)){G=rp(null,$)&&T&&T.vnode.props&&T.vnode.props.appear;const I=y.content.firstChild;if(G){const z=I.getAttribute("class");z&&(I.$cls=z),$.beforeEnter(I)}E(I,y,T),x.el=y=I}if(P&16&&!(L&&(L.innerHTML||L.textContent))){let I=p(y.firstChild,x,y,T,N,O,w);for(I&&!zi(y,1)&&ea();I;){const z=I;I=I.nextSibling,r(z)}}else if(P&8){let I=x.children;I[0]===`
`&&(y.tagName==="PRE"||y.tagName==="TEXTAREA")&&(I=I.slice(1));const{textContent:z}=y;z!==I&&z!==I.replace(/\r\n|\r/g,`
`)&&(zi(y,0)||ea(),y.textContent=x.children)}if(L){if(W||!w||U&48){const I=y.tagName.includes("-");for(const z in L)(W&&(z.endsWith("value")||z==="indeterminate")||zn(z)&&!Zs(z)||z[0]==="."||I&&!Zs(z))&&n(y,z,null,L[z],void 0,T)}else if(L.onClick)n(y,"onClick",null,L.onClick,void 0,T);else if(U&4&&Js(L.style))for(const I in L.style)L.style[I]}let D;(D=L&&L.onVnodeBeforeMount)&&Jt(D,T,x),S&&Rs(x,null,T,"beforeMount"),((D=L&&L.onVnodeMounted)||S||G)&&dp(()=>{D&&Jt(D,T,x),G&&$.enter(y),S&&Rs(x,null,T,"mounted")},N)}return y.nextSibling},p=(y,x,T,N,O,w,R)=>{R=R||!!x.dynamicChildren;const L=x.children,U=L.length;let P=!1;for(let S=0;S<U;S++){const $=R?L[S]:L[S]=Qt(L[S]),W=$.type===bn;y?(W&&!R&&S+1<U&&Qt(L[S+1]).type===bn&&(o(a(y.data.slice($.children.length)),T,i(y)),y.data=$.children),y=d(y,$,N,O,w,R)):W&&!$.children?o($.el=a(""),T):(P||(P=!0,zi(T,1)||ea()),s(null,$,T,null,N,O,ji(T),w))}return y},b=(y,x,T,N,O,w)=>{const{slotScopeIds:R}=x;R&&(O=O?O.concat(R):R);const L=l(y),U=p(i(y),x,L,T,N,O,w);return U&&ca(U)&&U.data==="]"?i(x.anchor=U):(ea(),o(x.anchor=c("]"),L,U),U)},m=(y,x,T,N,O,w)=>{if(zi(y.parentElement,1)||ea(),x.el=null,w){const U=k(y);for(;;){const P=i(y);if(P&&P!==U)r(P);else break}}const R=i(y),L=l(y);return r(y),s(null,x,L,R,T,N,ji(L),O),T&&(T.vnode.el=x.el,er(T,x.el)),R},k=(y,x="[",T="]")=>{let N=0;for(;y;)if(y=i(y),y&&ca(y)&&(y.data===x&&N++,y.data===T)){if(N===0)return i(y);N--}return y},E=(y,x,T)=>{const N=x.parentNode;N&&N.replaceChild(y,x);let O=T;for(;O;)O.vnode.el===x&&(O.vnode.el=O.subTree.el=y),O=O.parent},v=y=>y.nodeType===1&&y.tagName==="TEMPLATE";return[u,d]}const lu="data-allow-mismatch",rv={0:"text",1:"children",2:"class",3:"style",4:"attribute"};function zi(e,t){if(t===0||t===1)for(;e&&!e.hasAttribute(lu);)e=e.parentElement;const s=e&&e.getAttribute(lu);if(s==null)return!1;if(s==="")return!0;{const n=s.split(",");return t===0&&n.includes("children")?!0:n.includes(rv[t])}}const ov=Vl().requestIdleCallback||(e=>setTimeout(e,1)),cv=Vl().cancelIdleCallback||(e=>clearTimeout(e)),uv=(e=1e4)=>t=>{const s=ov(t,{timeout:e});return()=>cv(s)};function dv(e){const{top:t,left:s,bottom:n,right:a}=e.getBoundingClientRect(),{innerHeight:i,innerWidth:l}=window;return(t>0&&t<i||n>0&&n<i)&&(s>0&&s<l||a>0&&a<l)}const fv=e=>(t,s)=>{const n=new IntersectionObserver(a=>{for(const i of a)if(i.isIntersecting){n.disconnect(),t();break}},e);return s(a=>{if(a instanceof Element){if(dv(a))return t(),n.disconnect(),!1;n.observe(a)}}),()=>n.disconnect()},pv=e=>t=>{if(e){const s=matchMedia(e);if(s.matches)t();else return s.addEventListener("change",t,{once:!0}),()=>s.removeEventListener("change",t)}},hv=(e=[])=>(t,s)=>{Ee(e)&&(e=[e]);let n=!1;const a=l=>{n||(n=!0,i(),t(),l.target.dispatchEvent(new l.constructor(l.type,l)))},i=()=>{s(l=>{for(const r of e)l.removeEventListener(r,a)})};return s(l=>{for(const r of e)l.addEventListener(r,a,{once:!0})}),i};function mv(e,t){if(ca(e)&&e.data==="["){let s=1,n=e.nextSibling;for(;n;){if(n.nodeType===1){if(t(n)===!1)break}else if(ca(n))if(n.data==="]"){if(--s===0)break}else n.data==="["&&s++;n=n.nextSibling}}else t(e)}const Ys=e=>!!e.type.__asyncLoader;function gv(e){we(e)&&(e={loader:e});const{loader:t,loadingComponent:s,errorComponent:n,delay:a=200,hydrate:i,timeout:l,suspensible:r=!0,onError:o}=e;let c=null,u,d=0;const f=()=>(d++,c=null,p()),p=()=>{let b;return c||(b=c=t().catch(m=>{if(m=m instanceof Error?m:new Error(String(m)),o)return new Promise((k,E)=>{o(m,()=>k(f()),()=>E(m),d+1)});throw m}).then(m=>b!==c&&c?c:(m&&(m.__esModule||m[Symbol.toStringTag]==="Module")&&(m=m.default),u=m,m)))};return Ai({name:"AsyncComponentWrapper",__asyncLoader:p,__asyncHydrate(b,m,k){let E=!1;(m.bu||(m.bu=[])).push(()=>E=!0);const v=()=>{E||k()},y=i?()=>{const x=i(v,T=>mv(b,T));x&&(m.bum||(m.bum=[])).push(x)}:v;u?y():p().then(()=>!m.isUnmounted&&y())},get __asyncResolved(){return u},setup(){const b=Nt;if($o(b),u)return()=>Ki(u,b);const m=T=>{c=null,Gn(T,b,13,!n)};if(r&&b.suspense||Hn)return p().then(T=>()=>Ki(T,b)).catch(T=>(m(T),()=>n?lt(n,{error:T}):null));const k=h(!1),E=h(),v=h(!!a);let y,x;return ht(()=>{y!=null&&clearTimeout(y),x!=null&&clearTimeout(x)}),a&&(x=setTimeout(()=>{b.isUnmounted||(v.value=!1)},a)),l!=null&&(y=setTimeout(()=>{if(!b.isUnmounted&&!k.value&&!E.value){const T=new Error(`Async component timed out after ${l}ms.`);m(T),E.value=T}},l)),p().then(()=>{b.isUnmounted||(k.value=!0,b.parent&&Ri(b.parent.vnode)&&b.parent.update())}).catch(T=>{if(b.isUnmounted){c=null;return}m(T),E.value=T}),()=>{if(k.value&&u)return Ki(u,b);if(E.value&&n)return lt(n,{error:E.value});if(s&&!v.value)return Ki(s,b)}}})}function Ki(e,t){const{ref:s,props:n,children:a,ce:i}=t.vnode,l=lt(e,n,a);return l.ref=s,l.ce=i,delete t.vnode.ce,l}const Ri=e=>e.type.__isKeepAlive,vv={name:"KeepAlive",__isKeepAlive:!0,props:{include:[String,RegExp,Array],exclude:[String,RegExp,Array],max:[String,Number]},setup(e,{slots:t}){const s=qt(),n=s.ctx;if(!n.renderer)return()=>{const v=t.default&&t.default();return v&&v.length===1?v[0]:v};const a=new Map,i=new Set;let l=null;const r=s.suspense,{renderer:{p:o,m:c,um:u,o:{createElement:d}}}=n,f=d("div");n.activate=(v,y,x,T,N)=>{const O=v.component;c(v,y,x,0,r),o(O.vnode,v,y,x,O,r,T,v.slotScopeIds,N),gt(()=>{O.isDeactivated=!1,O.a&&pa(O.a);const w=v.props&&v.props.onVnodeMounted;w&&Jt(w,O.parent,v)},r)},n.deactivate=v=>{const y=v.component;ml(y.m),ml(y.a),c(v,f,null,1,r),gt(()=>{y.da&&pa(y.da);const x=v.props&&v.props.onVnodeUnmounted;x&&Jt(x,y.parent,v),y.isDeactivated=!0},r)};function p(v){_r(v),u(v,s,r,!0)}function b(v){a.forEach((y,x)=>{const T=so(Ys(y)?y.type.__asyncResolved||{}:y.type);T&&!v(T)&&m(x)})}function m(v){const y=a.get(v);y&&(!l||!bs(y,l))?p(y):l&&_r(l),a.delete(v),i.delete(v)}hs(()=>[e.include,e.exclude],([v,y])=>{v&&b(x=>qa(v,x)),y&&b(x=>!qa(y,x))},{flush:"post",deep:!0});let k=null;const E=()=>{k!=null&&(gl(s.subTree.type)?gt(()=>{a.set(k,qi(s.subTree))},s.subTree.suspense):a.set(k,qi(s.subTree)))};return Be(E),Yl(E),Ql(()=>{a.forEach(v=>{const{subTree:y,suspense:x}=s,T=qi(y);if(v.type===T.type&&v.key===T.key){_r(T);const N=T.component.da;N&&gt(N,x);return}p(v)})}),()=>{if(k=null,!t.default)return l=null;const v=t.default(),y=v[0];if(v.length>1)return l=null,v;if(!nn(y)||!(y.shapeFlag&4)&&!(y.shapeFlag&128))return l=null,y;let x=qi(y);if(x.type===pt)return l=null,x;const T=x.type,N=so(Ys(x)?x.type.__asyncResolved||{}:T),{include:O,exclude:w,max:R}=e;if(O&&(!N||!qa(O,N))||w&&N&&qa(w,N))return x.shapeFlag&=-257,l=x,y;const L=x.key==null?T:x.key,U=a.get(L);return x.el&&(x=Ls(x),y.shapeFlag&128&&(y.ssContent=x)),k=L,U?(x.el=U.el,x.component=U.component,x.transition&&sn(x,x.transition),x.shapeFlag|=512,i.delete(L),i.add(L)):(i.add(L),R&&i.size>parseInt(R,10)&&m(i.values().next().value)),x.shapeFlag|=256,l=x,gl(y.type)?y:x}}},bv=vv;function qa(e,t){return he(e)?e.some(s=>qa(s,t)):Ee(e)?e.split(",").includes(t):Am(e)?(e.lastIndex=0,e.test(t)):!1}function Uo(e,t){Ff(e,"a",t)}function Bo(e,t){Ff(e,"da",t)}function Ff(e,t,s=Nt){const n=e.__wdc||(e.__wdc=()=>{let a=s;for(;a;){if(a.isDeactivated)return;a=a.parent}return e()});if(Jl(t,n,s),s){let a=s.parent;for(;a&&a.parent;)Ri(a.parent.vnode)&&yv(n,t,s,a),a=a.parent}}function yv(e,t,s,n){const a=Jl(t,e,n,!0);ht(()=>{To(n[t],a)},s)}function _r(e){e.shapeFlag&=-257,e.shapeFlag&=-513}function qi(e){return e.shapeFlag&128?e.ssContent:e}function Jl(e,t,s=Nt,n=!1){if(s){const a=s[e]||(s[e]=[]),i=t.__weh||(t.__weh=(...l)=>{en();const r=La(s),o=is(t,s,e,l);return r(),tn(),o});return n?a.unshift(i):a.push(i),i}}const an=e=>(t,s=Nt)=>{(!Hn||e==="sp")&&Jl(e,(...n)=>t(...n),s)},$f=an("bm"),Be=an("m"),Ho=an("bu"),Yl=an("u"),Ql=an("bum"),ht=an("um"),Uf=an("sp"),Bf=an("rtg"),Hf=an("rtc");function Vf(e,t=Nt){Jl("ec",e,t)}const Vo="components",xv="directives";function _v(e,t){return jo(Vo,e,!0,t)||e}const jf=Symbol.for("v-ndc");function kv(e){return Ee(e)?jo(Vo,e,!1)||e:e||jf}function wv(e){return jo(xv,e)}function jo(e,t,s=!0,n=!1){const a=Ot||Nt;if(a){const i=a.type;if(e===Vo){const r=so(i,!1);if(r&&(r===t||r===Qe(t)||r===qn(Qe(t))))return i}const l=ru(a[e]||i[e],t)||ru(a.appContext[e],t);return!l&&n?i:l}}function ru(e,t){return e&&(e[t]||e[Qe(t)]||e[qn(Qe(t))])}function Sv(e,t,s,n){let a;const i=s&&s[n],l=he(e);if(l||Ee(e)){const r=l&&Js(e);let o=!1,c=!1;r&&(o=!ts(e),c=Os(e),e=Kl(e)),a=new Array(e.length);for(let u=0,d=e.length;u<d;u++)a[u]=t(o?c?ya(_s(e[u])):_s(e[u]):e[u],u,void 0,i&&i[u])}else if(typeof e=="number"){a=new Array(e);for(let r=0;r<e;r++)a[r]=t(r+1,r,void 0,i&&i[r])}else if(je(e))if(e[Symbol.iterator])a=Array.from(e,(r,o)=>t(r,o,void 0,i&&i[o]));else{const r=Object.keys(e);a=new Array(r.length);for(let o=0,c=r.length;o<c;o++){const u=r[o];a[o]=t(e[u],u,o,i&&i[o])}}else a=[];return s&&(s[n]=a),a}function Tv(e,t){for(let s=0;s<t.length;s++){const n=t[s];if(he(n))for(let a=0;a<n.length;a++)e[n[a].name]=n[a].fn;else n&&(e[n.name]=n.key?(...a)=>{const i=n.fn(...a);return i&&(i.key=n.key),i}:n.fn)}return e}function Cv(e,t,s={},n,a){if(Ot.ce||Ot.parent&&Ys(Ot.parent)&&Ot.parent.ce){const c=Object.keys(s).length>0;return t!=="default"&&(s.name=t),hi(),vl(At,null,[lt("slot",s,n&&n())],c?-2:64)}let i=e[t];i&&i._c&&(i._d=!1),hi();const l=i&&zo(i(s)),r=s.key||l&&l.key,o=vl(At,{key:(r&&!Bt(r)?r:`_${t}`)+(!l&&n?"_fb":"")},l||(n?n():[]),l&&e._===1?64:-2);return!a&&o.scopeId&&(o.slotScopeIds=[o.scopeId+"-s"]),i&&i._c&&(i._d=!0),o}function zo(e){return e.some(t=>nn(t)?!(t.type===pt||t.type===At&&!zo(t.children)):!0)?e:null}function Ev(e,t){const s={};for(const n in e)s[t&&/[A-Z]/.test(n)?`on:${n}`:fa(n)]=e[n];return s}const Wr=e=>e?yp(e)?Ii(e):Wr(e.parent):null,Xa=Pe(Object.create(null),{$:e=>e,$el:e=>e.vnode.el,$data:e=>e.data,$props:e=>e.props,$attrs:e=>e.attrs,$slots:e=>e.slots,$refs:e=>e.refs,$parent:e=>Wr(e.parent),$root:e=>Wr(e.root),$host:e=>e.ce,$emit:e=>e.emit,$options:e=>Ko(e),$forceUpdate:e=>e.f||(e.f=()=>{Do(e.update)}),$nextTick:e=>e.n||(e.n=Et.bind(e.proxy)),$watch:e=>Jg.bind(e)}),kr=(e,t)=>e!==Fe&&!e.__isScriptSetup&&ze(e,t),Zr={get({_:e},t){if(t==="__v_skip")return!0;const{ctx:s,setupState:n,data:a,props:i,accessCache:l,type:r,appContext:o}=e;if(t[0]!=="$"){const f=l[t];if(f!==void 0)switch(f){case 1:return n[t];case 2:return a[t];case 4:return s[t];case 3:return i[t]}else{if(kr(n,t))return l[t]=1,n[t];if(a!==Fe&&ze(a,t))return l[t]=2,a[t];if(ze(i,t))return l[t]=3,i[t];if(s!==Fe&&ze(s,t))return l[t]=4,s[t];Jr&&(l[t]=0)}}const c=Xa[t];let u,d;if(c)return t==="$attrs"&&Ft(e.attrs,"get",""),c(e);if((u=r.__cssModules)&&(u=u[t]))return u;if(s!==Fe&&ze(s,t))return l[t]=4,s[t];if(d=o.config.globalProperties,ze(d,t))return d[t]},set({_:e},t,s){const{data:n,setupState:a,ctx:i}=e;return kr(a,t)?(a[t]=s,!0):n!==Fe&&ze(n,t)?(n[t]=s,!0):ze(e.props,t)||t[0]==="$"&&t.slice(1)in e?!1:(i[t]=s,!0)},has({_:{data:e,setupState:t,accessCache:s,ctx:n,appContext:a,props:i,type:l}},r){let o;return!!(s[r]||e!==Fe&&r[0]!=="$"&&ze(e,r)||kr(t,r)||ze(i,r)||ze(n,r)||ze(Xa,r)||ze(a.config.globalProperties,r)||(o=l.__cssModules)&&o[r])},defineProperty(e,t,s){return s.get!=null?e._.accessCache[t]=0:ze(s,"value")&&this.set(e,t,s.value,null),Reflect.defineProperty(e,t,s)}},Av=Pe({},Zr,{get(e,t){if(t!==Symbol.unscopables)return Zr.get(e,t,e)},has(e,t){return t[0]!=="_"&&!Mm(t)}});function Rv(){return null}function Iv(){return null}function Nv(e){}function Ov(e){}function Lv(){return null}function Dv(){}function Mv(e,t){return null}function Pv(){return zf().slots}function Fv(){return zf().attrs}function zf(e){const t=qt();return t.setupContext||(t.setupContext=wp(t))}function fi(e){return he(e)?e.reduce((t,s)=>(t[s]=null,t),{}):e}function $v(e,t){const s=fi(e);for(const n in t){if(n.startsWith("__skip"))continue;let a=s[n];a?he(a)||we(a)?a=s[n]={type:a,default:t[n]}:a.default=t[n]:a===null&&(a=s[n]={default:t[n]}),a&&t[`__skip_${n}`]&&(a.skipFactory=!0)}return s}function Uv(e,t){return!e||!t?e||t:he(e)&&he(t)?e.concat(t):Pe({},fi(e),fi(t))}function Bv(e,t){const s={};for(const n in e)t.includes(n)||Object.defineProperty(s,n,{enumerable:!0,get:()=>e[n]});return s}function Hv(e){const t=qt(),s=Hn;let n=e();gi(),s&&va(!1);const a=()=>{La(t),s&&va(!0)},i=()=>{qt()!==t&&t.scope.off(),gi(),s&&va(!1)};return Co(n)&&(n=n.catch(l=>{throw a(),Promise.resolve().then(()=>Promise.resolve().then(i)),l})),[n,()=>{a(),Promise.resolve().then(i)}]}let Jr=!0;function Vv(e){const t=Ko(e),s=e.proxy,n=e.ctx;Jr=!1,t.beforeCreate&&ou(t.beforeCreate,e,"bc");const{data:a,computed:i,methods:l,watch:r,provide:o,inject:c,created:u,beforeMount:d,mounted:f,beforeUpdate:p,updated:b,activated:m,deactivated:k,beforeDestroy:E,beforeUnmount:v,destroyed:y,unmounted:x,render:T,renderTracked:N,renderTriggered:O,errorCaptured:w,serverPrefetch:R,expose:L,inheritAttrs:U,components:P,directives:S,filters:$}=t;if(c&&jv(c,n,null),l)for(const D in l){const I=l[D];we(I)&&(n[D]=I.bind(s))}if(a){const D=a.call(s,s);je(D)&&(e.data=_n(D))}if(Jr=!0,i)for(const D in i){const I=i[D],z=we(I)?I.bind(s,s):we(I.get)?I.get.bind(s,s):Lt,ce=!we(I)&&we(I.set)?I.set.bind(s):Lt,de=Y({get:z,set:ce});Object.defineProperty(n,D,{enumerable:!0,configurable:!0,get:()=>de.value,set:se=>de.value=se})}if(r)for(const D in r)Kf(r[D],n,s,D);if(o){const D=we(o)?o.call(s):o;Reflect.ownKeys(D).forEach(I=>{Qa(I,D[I])})}u&&ou(u,e,"c");function G(D,I){he(I)?I.forEach(z=>D(z.bind(s))):I&&D(I.bind(s))}if(G($f,d),G(Be,f),G(Ho,p),G(Yl,b),G(Uo,m),G(Bo,k),G(Vf,w),G(Hf,N),G(Bf,O),G(Ql,v),G(ht,x),G(Uf,R),he(L))if(L.length){const D=e.exposed||(e.exposed={});L.forEach(I=>{Object.defineProperty(D,I,{get:()=>s[I],set:z=>s[I]=z,enumerable:!0})})}else e.exposed||(e.exposed={});T&&e.render===Lt&&(e.render=T),U!=null&&(e.inheritAttrs=U),P&&(e.components=P),S&&(e.directives=S),R&&$o(e)}function jv(e,t,s=Lt){he(e)&&(e=Yr(e));for(const n in e){const a=e[n];let i;je(a)?"default"in a?i=ps(a.from||n,a.default,!0):i=ps(a.from||n):i=ps(a),bt(i)?Object.defineProperty(t,n,{enumerable:!0,configurable:!0,get:()=>i.value,set:l=>i.value=l}):t[n]=i}}function ou(e,t,s){is(he(e)?e.map(n=>n.bind(t.proxy)):e.bind(t.proxy),t,s)}function Kf(e,t,s,n){let a=n.includes(".")?If(s,n):()=>s[n];if(Ee(e)){const i=t[e];we(i)&&hs(a,i)}else if(we(e))hs(a,e.bind(s));else if(je(e))if(he(e))e.forEach(i=>Kf(i,t,s,n));else{const i=we(e.handler)?e.handler.bind(s):t[e.handler];we(i)&&hs(a,i,e)}}function Ko(e){const t=e.type,{mixins:s,extends:n}=t,{mixins:a,optionsCache:i,config:{optionMergeStrategies:l}}=e.appContext,r=i.get(t);let o;return r?o=r:!a.length&&!s&&!n?o=t:(o={},a.length&&a.forEach(c=>hl(o,c,l,!0)),hl(o,t,l)),je(t)&&i.set(t,o),o}function hl(e,t,s,n=!1){const{mixins:a,extends:i}=t;i&&hl(e,i,s,!0),a&&a.forEach(l=>hl(e,l,s,!0));for(const l in t)if(!(n&&l==="expose")){const r=zv[l]||s&&s[l];e[l]=r?r(e[l],t[l]):t[l]}return e}const zv={data:cu,props:uu,emits:uu,methods:Ga,computed:Ga,beforeCreate:Ht,created:Ht,beforeMount:Ht,mounted:Ht,beforeUpdate:Ht,updated:Ht,beforeDestroy:Ht,beforeUnmount:Ht,destroyed:Ht,unmounted:Ht,activated:Ht,deactivated:Ht,errorCaptured:Ht,serverPrefetch:Ht,components:Ga,directives:Ga,watch:qv,provide:cu,inject:Kv};function cu(e,t){return t?e?function(){return Pe(we(e)?e.call(this,this):e,we(t)?t.call(this,this):t)}:t:e}function Kv(e,t){return Ga(Yr(e),Yr(t))}function Yr(e){if(he(e)){const t={};for(let s=0;s<e.length;s++)t[e[s]]=e[s];return t}return e}function Ht(e,t){return e?[...new Set([].concat(e,t))]:t}function Ga(e,t){return e?Pe(Object.create(null),e,t):t}function uu(e,t){return e?he(e)&&he(t)?[...new Set([...e,...t])]:Pe(Object.create(null),fi(e),fi(t??{})):t}function qv(e,t){if(!e)return t;if(!t)return e;const s=Pe(Object.create(null),e);for(const n in t)s[n]=Ht(e[n],t[n]);return s}function qf(){return{app:null,config:{isNativeTag:oa,performance:!1,globalProperties:{},optionMergeStrategies:{},errorHandler:void 0,warnHandler:void 0,compilerOptions:{}},mixins:[],components:{},directives:{},provides:Object.create(null),optionsCache:new WeakMap,propsCache:new WeakMap,emitsCache:new WeakMap}}let Gv=0;function Wv(e,t){return function(n,a=null){we(n)||(n=Pe({},n)),a!=null&&!je(a)&&(a=null);const i=qf(),l=new WeakSet,r=[];let o=!1;const c=i.app={_uid:Gv++,_component:n,_props:a,_container:null,_context:i,_instance:null,version:Tp,get config(){return i.config},set config(u){},use(u,...d){return l.has(u)||(u&&we(u.install)?(l.add(u),u.install(c,...d)):we(u)&&(l.add(u),u(c,...d))),c},mixin(u){return i.mixins.includes(u)||i.mixins.push(u),c},component(u,d){return d?(i.components[u]=d,c):i.components[u]},directive(u,d){return d?(i.directives[u]=d,c):i.directives[u]},mount(u,d,f){if(!o){const p=c._ceVNode||lt(n,a);return p.appContext=i,f===!0?f="svg":f===!1&&(f=void 0),d&&t?t(p,u):e(p,u,f),o=!0,c._container=u,u.__vue_app__=c,Ii(p.component)}},onUnmount(u){r.push(u)},unmount(){o&&(is(r,c._instance,16),e(null,c._container),delete c._container.__vue_app__)},provide(u,d){return i.provides[u]=d,c},runWithContext(u){const d=Mn;Mn=c;try{return u()}finally{Mn=d}}};return c}}let Mn=null;function Zv(e,t,s=Fe){const n=qt(),a=Qe(t),i=Xt(t),l=Gf(e,a),r=yf((o,c)=>{let u,d=Fe,f;return Rf(()=>{const p=e[a];Ct(u,p)&&(u=p,c())}),{get(){return o(),s.get?s.get(u):u},set(p){const b=s.set?s.set(p):p;if(!Ct(b,u)&&!(d!==Fe&&Ct(p,d)))return;const m=n.vnode.props,k=!!(m&&(t in m||a in m||i in m)&&(`onUpdate:${t}`in m||`onUpdate:${a}`in m||`onUpdate:${i}`in m));k||(u=p,c()),n.emit(`update:${t}`,b),Ct(p,d)&&(Ct(p,b)&&!Ct(b,f)||k&&d!==Fe&&!Ct(b,u))&&c(),d=p,f=b}}});return r[Symbol.iterator]=()=>{let o=0;return{next(){return o<2?{value:o++?l||Fe:r,done:!1}:{done:!0}}}},r}const Gf=(e,t)=>t==="modelValue"||t==="model-value"?e.modelModifiers:e[`${t}Modifiers`]||e[`${Qe(t)}Modifiers`]||e[`${Xt(t)}Modifiers`];function Jv(e,t,...s){if(e.isUnmounted)return;const n=e.vnode.props||Fe;let a=s;const i=t.startsWith("update:"),l=i&&Gf(n,t.slice(7));l&&(l.trim&&(a=s.map(u=>Ee(u)?u.trim():u)),l.number&&(a=s.map(Hl)));let r,o=n[r=fa(t)]||n[r=fa(Qe(t))];!o&&i&&(o=n[r=fa(Xt(t))]),o&&is(o,e,6,a);const c=n[r+"Once"];if(c){if(!e.emitted)e.emitted={};else if(e.emitted[r])return;e.emitted[r]=!0,is(c,e,6,a)}}const Yv=new WeakMap;function Wf(e,t,s=!1){const n=s?Yv:t.emitsCache,a=n.get(e);if(a!==void 0)return a;const i=e.emits;let l={},r=!1;if(!we(e)){const o=c=>{const u=Wf(c,t,!0);u&&(r=!0,Pe(l,u))};!s&&t.mixins.length&&t.mixins.forEach(o),e.extends&&o(e.extends),e.mixins&&e.mixins.forEach(o)}return!i&&!r?(je(e)&&n.set(e,null),null):(he(i)?i.forEach(o=>l[o]=null):Pe(l,i),je(e)&&n.set(e,l),l)}function Xl(e,t){return!e||!zn(t)?!1:(t=t.slice(2).replace(/Once$/,""),ze(e,t[0].toLowerCase()+t.slice(1))||ze(e,Xt(t))||ze(e,t))}function el(e){const{type:t,vnode:s,proxy:n,withProxy:a,propsOptions:[i],slots:l,attrs:r,emit:o,render:c,renderCache:u,props:d,data:f,setupState:p,ctx:b,inheritAttrs:m}=e,k=di(e);let E,v;try{if(s.shapeFlag&4){const x=a||n,T=x;E=Qt(c.call(T,x,u,d,p,f,b)),v=r}else{const x=t;E=Qt(x.length>1?x(d,{attrs:r,slots:l,emit:o}):x(d,null)),v=t.props?r:Xv(r)}}catch(x){ei.length=0,Gn(x,e,1),E=lt(pt)}let y=E;if(v&&m!==!1){const x=Object.keys(v),{shapeFlag:T}=y;x.length&&T&7&&(i&&x.some(Fl)&&(v=eb(v,i)),y=Ls(y,v,!1,!0))}return s.dirs&&(y=Ls(y,null,!1,!0),y.dirs=y.dirs?y.dirs.concat(s.dirs):s.dirs),s.transition&&sn(y,s.transition),E=y,di(k),E}function Qv(e,t=!0){let s;for(let n=0;n<e.length;n++){const a=e[n];if(nn(a)){if(a.type!==pt||a.children==="v-if"){if(s)return;s=a}}else return}return s}const Xv=e=>{let t;for(const s in e)(s==="class"||s==="style"||zn(s))&&((t||(t={}))[s]=e[s]);return t},eb=(e,t)=>{const s={};for(const n in e)(!Fl(n)||!(n.slice(9)in t))&&(s[n]=e[n]);return s};function tb(e,t,s){const{props:n,children:a,component:i}=e,{props:l,children:r,patchFlag:o}=t,c=i.emitsOptions;if(t.dirs||t.transition)return!0;if(s&&o>=0){if(o&1024)return!0;if(o&16)return n?du(n,l,c):!!l;if(o&8){const u=t.dynamicProps;for(let d=0;d<u.length;d++){const f=u[d];if(Zf(l,n,f)&&!Xl(c,f))return!0}}}else return(a||r)&&(!r||!r.$stable)?!0:n===l?!1:n?l?du(n,l,c):!0:!!l;return!1}function du(e,t,s){const n=Object.keys(t);if(n.length!==Object.keys(e).length)return!0;for(let a=0;a<n.length;a++){const i=n[a];if(Zf(t,e,i)&&!Xl(s,i))return!0}return!1}function Zf(e,t,s){const n=e[s],a=t[s];return s==="style"&&je(n)&&je(a)?!Xs(n,a):n!==a}function er({vnode:e,parent:t,suspense:s},n){for(;t;){const a=t.subTree;if(a.suspense&&a.suspense.activeBranch===e&&(a.suspense.vnode.el=a.el=n,e=a),a===e)(e=t.vnode).el=n,t=t.parent;else break}s&&s.activeBranch===e&&(s.vnode.el=n)}const Jf={},Yf=()=>Object.create(Jf),Qf=e=>Object.getPrototypeOf(e)===Jf;function sb(e,t,s,n=!1){const a={},i=Yf();e.propsDefaults=Object.create(null),Xf(e,t,a,i);for(const l in e.propsOptions[0])l in a||(a[l]=void 0);s?e.props=n?a:No(a):e.type.props?e.props=a:e.props=i,e.attrs=i}function nb(e,t,s,n){const{props:a,attrs:i,vnode:{patchFlag:l}}=e,r=Ue(a),[o]=e.propsOptions;let c=!1;if((n||l>0)&&!(l&16)){if(l&8){const u=e.vnode.dynamicProps;for(let d=0;d<u.length;d++){let f=u[d];if(Xl(e.emitsOptions,f))continue;const p=t[f];if(o)if(ze(i,f))p!==i[f]&&(i[f]=p,c=!0);else{const b=Qe(f);a[b]=Qr(o,r,b,p,e,!1)}else p!==i[f]&&(i[f]=p,c=!0)}}}else{Xf(e,t,a,i)&&(c=!0);let u;for(const d in r)(!t||!ze(t,d)&&((u=Xt(d))===d||!ze(t,u)))&&(o?s&&(s[d]!==void 0||s[u]!==void 0)&&(a[d]=Qr(o,r,d,void 0,e,!0)):delete a[d]);if(i!==r)for(const d in i)(!t||!ze(t,d))&&(delete i[d],c=!0)}c&&zs(e.attrs,"set","")}function Xf(e,t,s,n){const[a,i]=e.propsOptions;let l=!1,r;if(t)for(let o in t){if(Zs(o))continue;const c=t[o];let u;a&&ze(a,u=Qe(o))?!i||!i.includes(u)?s[u]=c:(r||(r={}))[u]=c:Xl(e.emitsOptions,o)||(!(o in n)||c!==n[o])&&(n[o]=c,l=!0)}if(i){const o=Ue(s),c=r||Fe;for(let u=0;u<i.length;u++){const d=i[u];s[d]=Qr(a,o,d,c[d],e,!ze(c,d))}}return l}function Qr(e,t,s,n,a,i){const l=e[s];if(l!=null){const r=ze(l,"default");if(r&&n===void 0){const o=l.default;if(l.type!==Function&&!l.skipFactory&&we(o)){const{propsDefaults:c}=a;if(s in c)n=c[s];else{const u=La(a);n=c[s]=o.call(null,t),u()}}else n=o;a.ce&&a.ce._setProp(s,n)}l[0]&&(i&&!r?n=!1:l[1]&&(n===""||n===Xt(s))&&(n=!0))}return n}const ab=new WeakMap;function ep(e,t,s=!1){const n=s?ab:t.propsCache,a=n.get(e);if(a)return a;const i=e.props,l={},r=[];let o=!1;if(!we(e)){const u=d=>{o=!0;const[f,p]=ep(d,t,!0);Pe(l,f),p&&r.push(...p)};!s&&t.mixins.length&&t.mixins.forEach(u),e.extends&&u(e.extends),e.mixins&&e.mixins.forEach(u)}if(!i&&!o)return je(e)&&n.set(e,ua),ua;if(he(i))for(let u=0;u<i.length;u++){const d=Qe(i[u]);fu(d)&&(l[d]=Fe)}else if(i)for(const u in i){const d=Qe(u);if(fu(d)){const f=i[u],p=l[d]=he(f)||we(f)?{type:f}:Pe({},f),b=p.type;let m=!1,k=!0;if(he(b))for(let E=0;E<b.length;++E){const v=b[E],y=we(v)&&v.name;if(y==="Boolean"){m=!0;break}else y==="String"&&(k=!1)}else m=we(b)&&b.name==="Boolean";p[0]=m,p[1]=k,(m||ze(p,"default"))&&r.push(d)}}const c=[l,r];return je(e)&&n.set(e,c),c}function fu(e){return e[0]!=="$"&&!Zs(e)}const qo=e=>e==="_"||e==="_ctx"||e==="$stable",Go=e=>he(e)?e.map(Qt):[Qt(e)],ib=(e,t,s)=>{if(t._n)return t;const n=Mo((...a)=>Go(t(...a)),s);return n._c=!1,n},tp=(e,t,s)=>{const n=e._ctx;for(const a in e){if(qo(a))continue;const i=e[a];if(we(i))t[a]=ib(a,i,n);else if(i!=null){const l=Go(i);t[a]=()=>l}}},sp=(e,t)=>{const s=Go(t);e.slots.default=()=>s},np=(e,t,s)=>{for(const n in t)(s||!qo(n))&&(e[n]=t[n])},lb=(e,t,s)=>{const n=e.slots=Yf();if(e.vnode.shapeFlag&32){const a=t._;a?(np(n,t,s),s&&Wd(n,"_",a,!0)):tp(t,n)}else t&&sp(e,t)},rb=(e,t,s)=>{const{vnode:n,slots:a}=e;let i=!0,l=Fe;if(n.shapeFlag&32){const r=t._;r?s&&r===1?i=!1:np(a,t,s):(i=!t.$stable,tp(t,a)),l=t}else t&&(sp(e,t),l={default:1});if(i)for(const r in a)!qo(r)&&l[r]==null&&delete a[r]},gt=dp;function ap(e){return lp(e)}function ip(e){return lp(e,lv)}function lp(e,t){const s=Vl();s.__VUE__=!0;const{insert:n,remove:a,patchProp:i,createElement:l,createText:r,createComment:o,setText:c,setElementText:u,parentNode:d,nextSibling:f,setScopeId:p=Lt,insertStaticContent:b}=e,m=(g,C,M,Z=null,A=null,F=null,J=void 0,X=null,te=!!C.dynamicChildren)=>{if(g===C)return;g&&!bs(g,C)&&(Z=j(g),se(g,A,F,!0),g=null),C.patchFlag===-2&&(te=!1,C.dynamicChildren=null);const{type:Q,ref:me,shapeFlag:le}=C;switch(Q){case bn:k(g,C,M,Z);break;case pt:E(g,C,M,Z);break;case Pn:g==null&&v(C,M,Z,J);break;case At:P(g,C,M,Z,A,F,J,X,te);break;default:le&1?T(g,C,M,Z,A,F,J,X,te):le&6?S(g,C,M,Z,A,F,J,X,te):(le&64||le&128)&&Q.process(g,C,M,Z,A,F,J,X,te,ge)}me!=null&&A?ma(me,g&&g.ref,F,C||g,!C):me==null&&g&&g.ref!=null&&ma(g.ref,null,F,g,!0)},k=(g,C,M,Z)=>{if(g==null)n(C.el=r(C.children),M,Z);else{const A=C.el=g.el;C.children!==g.children&&c(A,C.children)}},E=(g,C,M,Z)=>{g==null?n(C.el=o(C.children||""),M,Z):C.el=g.el},v=(g,C,M,Z)=>{[g.el,g.anchor]=b(g.children,C,M,Z,g.el,g.anchor)},y=({el:g,anchor:C},M,Z)=>{let A;for(;g&&g!==C;)A=f(g),n(g,M,Z),g=A;n(C,M,Z)},x=({el:g,anchor:C})=>{let M;for(;g&&g!==C;)M=f(g),a(g),g=M;a(C)},T=(g,C,M,Z,A,F,J,X,te)=>{if(C.type==="svg"?J="svg":C.type==="math"&&(J="mathml"),g==null)N(C,M,Z,A,F,J,X,te);else{const Q=g.el&&g.el._isVueCE?g.el:null;try{Q&&Q._beginPatch(),R(g,C,A,F,J,X,te)}finally{Q&&Q._endPatch()}}},N=(g,C,M,Z,A,F,J,X)=>{let te,Q;const{props:me,shapeFlag:le,transition:fe,dirs:xe}=g;if(te=g.el=l(g.type,F,me&&me.is,me),le&8?u(te,g.children):le&16&&w(g.children,te,null,Z,A,wr(g,F),J,X),xe&&Rs(g,null,Z,"created"),O(te,g,g.scopeId,J,Z),me){for(const Ae in me)Ae!=="value"&&!Zs(Ae)&&i(te,Ae,null,me[Ae],F,Z);"value"in me&&i(te,"value",null,me.value,F),(Q=me.onVnodeBeforeMount)&&Jt(Q,Z,g)}xe&&Rs(g,null,Z,"beforeMount");const Se=rp(A,fe);Se&&fe.beforeEnter(te),n(te,C,M),((Q=me&&me.onVnodeMounted)||Se||xe)&&gt(()=>{try{Q&&Jt(Q,Z,g),Se&&fe.enter(te),xe&&Rs(g,null,Z,"mounted")}finally{}},A)},O=(g,C,M,Z,A)=>{if(M&&p(g,M),Z)for(let F=0;F<Z.length;F++)p(g,Z[F]);if(A){let F=A.subTree;if(C===F||gl(F.type)&&(F.ssContent===C||F.ssFallback===C)){const J=A.vnode;O(g,J,J.scopeId,J.slotScopeIds,A.parent)}}},w=(g,C,M,Z,A,F,J,X,te=0)=>{for(let Q=te;Q<g.length;Q++){const me=g[Q]=X?Vs(g[Q]):Qt(g[Q]);m(null,me,C,M,Z,A,F,J,X)}},R=(g,C,M,Z,A,F,J)=>{const X=C.el=g.el;let{patchFlag:te,dynamicChildren:Q,dirs:me}=C;te|=g.patchFlag&16;const le=g.props||Fe,fe=C.props||Fe;let xe;if(M&&Cn(M,!1),(xe=fe.onVnodeBeforeUpdate)&&Jt(xe,M,C,g),me&&Rs(C,g,M,"beforeUpdate"),M&&Cn(M,!0),(le.innerHTML&&fe.innerHTML==null||le.textContent&&fe.textContent==null)&&u(X,""),Q?L(g.dynamicChildren,Q,X,M,Z,wr(C,A),F):J||I(g,C,X,null,M,Z,wr(C,A),F,!1),te>0){if(te&16)U(X,le,fe,M,A);else if(te&2&&le.class!==fe.class&&i(X,"class",null,fe.class,A),te&4&&i(X,"style",le.style,fe.style,A),te&8){const Se=C.dynamicProps;for(let Ae=0;Ae<Se.length;Ae++){const B=Se[Ae],re=le[B],ye=fe[B];(ye!==re||B==="value")&&i(X,B,re,ye,A,M)}}te&1&&g.children!==C.children&&u(X,C.children)}else!J&&Q==null&&U(X,le,fe,M,A);((xe=fe.onVnodeUpdated)||me)&&gt(()=>{xe&&Jt(xe,M,C,g),me&&Rs(C,g,M,"updated")},Z)},L=(g,C,M,Z,A,F,J)=>{for(let X=0;X<C.length;X++){const te=g[X],Q=C[X],me=te.el&&(te.type===At||!bs(te,Q)||te.shapeFlag&198)?d(te.el):M;m(te,Q,me,null,Z,A,F,J,!0)}},U=(g,C,M,Z,A)=>{if(C!==M){if(C!==Fe)for(const F in C)!Zs(F)&&!(F in M)&&i(g,F,C[F],null,A,Z);for(const F in M){if(Zs(F))continue;const J=M[F],X=C[F];J!==X&&F!=="value"&&i(g,F,X,J,A,Z)}"value"in M&&i(g,"value",C.value,M.value,A)}},P=(g,C,M,Z,A,F,J,X,te)=>{const Q=C.el=g?g.el:r(""),me=C.anchor=g?g.anchor:r("");let{patchFlag:le,dynamicChildren:fe,slotScopeIds:xe}=C;xe&&(X=X?X.concat(xe):xe),g==null?(n(Q,M,Z),n(me,M,Z),w(C.children||[],M,me,A,F,J,X,te)):le>0&&le&64&&fe&&g.dynamicChildren&&g.dynamicChildren.length===fe.length?(L(g.dynamicChildren,fe,M,A,F,J,X),(C.key!=null||A&&C===A.subTree)&&Wo(g,C,!0)):I(g,C,M,me,A,F,J,X,te)},S=(g,C,M,Z,A,F,J,X,te)=>{C.slotScopeIds=X,g==null?C.shapeFlag&512?A.ctx.activate(C,M,Z,J,te):$(C,M,Z,A,F,J,te):W(g,C,te)},$=(g,C,M,Z,A,F,J)=>{const X=g.component=bp(g,Z,A);if(Ri(g)&&(X.ctx.renderer=ge),xp(X,!1,J),X.asyncDep){if(A&&A.registerDep(X,G,J),!g.el){const te=X.subTree=lt(pt);E(null,te,C,M),g.placeholder=te.el}}else G(X,g,C,M,A,F,J)},W=(g,C,M)=>{const Z=C.component=g.component;if(tb(g,C,M))if(Z.asyncDep&&!Z.asyncResolved){D(Z,C,M);return}else Z.next=C,Z.update();else C.el=g.el,Z.vnode=C},G=(g,C,M,Z,A,F,J)=>{const X=()=>{if(g.isMounted){let{next:le,bu:fe,u:xe,parent:Se,vnode:Ae}=g;{const He=op(g);if(He){le&&(le.el=Ae.el,D(g,le,J)),He.asyncDep.then(()=>{gt(()=>{g.isUnmounted||Q()},A)});return}}let B=le,re;Cn(g,!1),le?(le.el=Ae.el,D(g,le,J)):le=Ae,fe&&pa(fe),(re=le.props&&le.props.onVnodeBeforeUpdate)&&Jt(re,Se,le,Ae),Cn(g,!0);const ye=el(g),Le=g.subTree;g.subTree=ye,m(Le,ye,d(Le.el),j(Le),g,A,F),le.el=ye.el,B===null&&er(g,ye.el),xe&&gt(xe,A),(re=le.props&&le.props.onVnodeUpdated)&&gt(()=>Jt(re,Se,le,Ae),A)}else{let le;const{el:fe,props:xe}=C,{bm:Se,m:Ae,parent:B,root:re,type:ye}=g,Le=Ys(C);if(Cn(g,!1),Se&&pa(Se),!Le&&(le=xe&&xe.onVnodeBeforeMount)&&Jt(le,B,C),Cn(g,!0),fe&&Oe){const He=()=>{g.subTree=el(g),Oe(fe,g.subTree,g,A,null)};Le&&ye.__asyncHydrate?ye.__asyncHydrate(fe,g,He):He()}else{re.ce&&re.ce._hasShadowRoot()&&re.ce._injectChildStyle(ye,g.parent?g.parent.type:void 0);const He=g.subTree=el(g);m(null,He,M,Z,g,A,F),C.el=He.el}if(Ae&&gt(Ae,A),!Le&&(le=xe&&xe.onVnodeMounted)){const He=C;gt(()=>Jt(le,B,He),A)}(C.shapeFlag&256||B&&Ys(B.vnode)&&B.vnode.shapeFlag&256)&&g.a&&gt(g.a,A),g.isMounted=!0,C=M=Z=null}};g.scope.on();const te=g.effect=new li(X);g.scope.off();const Q=g.update=te.run.bind(te),me=g.job=te.runIfDirty.bind(te);me.i=g,me.id=g.uid,te.scheduler=()=>Do(me),Cn(g,!0),Q()},D=(g,C,M)=>{C.component=g;const Z=g.vnode.props;g.vnode=C,g.next=null,nb(g,C.props,Z,M),rb(g,C.children,M),en(),Xc(g),tn()},I=(g,C,M,Z,A,F,J,X,te=!1)=>{const Q=g&&g.children,me=g?g.shapeFlag:0,le=C.children,{patchFlag:fe,shapeFlag:xe}=C;if(fe>0){if(fe&128){ce(Q,le,M,Z,A,F,J,X,te);return}else if(fe&256){z(Q,le,M,Z,A,F,J,X,te);return}}xe&8?(me&16&&Ie(Q,A,F),le!==Q&&u(M,le)):me&16?xe&16?ce(Q,le,M,Z,A,F,J,X,te):Ie(Q,A,F,!0):(me&8&&u(M,""),xe&16&&w(le,M,Z,A,F,J,X,te))},z=(g,C,M,Z,A,F,J,X,te)=>{g=g||ua,C=C||ua;const Q=g.length,me=C.length,le=Math.min(Q,me);let fe;for(fe=0;fe<le;fe++){const xe=C[fe]=te?Vs(C[fe]):Qt(C[fe]);m(g[fe],xe,M,null,A,F,J,X,te)}Q>me?Ie(g,A,F,!0,!1,le):w(C,M,Z,A,F,J,X,te,le)},ce=(g,C,M,Z,A,F,J,X,te)=>{let Q=0;const me=C.length;let le=g.length-1,fe=me-1;for(;Q<=le&&Q<=fe;){const xe=g[Q],Se=C[Q]=te?Vs(C[Q]):Qt(C[Q]);if(bs(xe,Se))m(xe,Se,M,null,A,F,J,X,te);else break;Q++}for(;Q<=le&&Q<=fe;){const xe=g[le],Se=C[fe]=te?Vs(C[fe]):Qt(C[fe]);if(bs(xe,Se))m(xe,Se,M,null,A,F,J,X,te);else break;le--,fe--}if(Q>le){if(Q<=fe){const xe=fe+1,Se=xe<me?C[xe].el:Z;for(;Q<=fe;)m(null,C[Q]=te?Vs(C[Q]):Qt(C[Q]),M,Se,A,F,J,X,te),Q++}}else if(Q>fe)for(;Q<=le;)se(g[Q],A,F,!0),Q++;else{const xe=Q,Se=Q,Ae=new Map;for(Q=Se;Q<=fe;Q++){const et=C[Q]=te?Vs(C[Q]):Qt(C[Q]);et.key!=null&&Ae.set(et.key,Q)}let B,re=0;const ye=fe-Se+1;let Le=!1,He=0;const Je=new Array(ye);for(Q=0;Q<ye;Q++)Je[Q]=0;for(Q=xe;Q<=le;Q++){const et=g[Q];if(re>=ye){se(et,A,F,!0);continue}let Ye;if(et.key!=null)Ye=Ae.get(et.key);else for(B=Se;B<=fe;B++)if(Je[B-Se]===0&&bs(et,C[B])){Ye=B;break}Ye===void 0?se(et,A,F,!0):(Je[Ye-Se]=Q+1,Ye>=He?He=Ye:Le=!0,m(et,C[Ye],M,null,A,F,J,X,te),re++)}const xt=Le?ob(Je):ua;for(B=xt.length-1,Q=ye-1;Q>=0;Q--){const et=Se+Q,Ye=C[et],ws=C[et+1],Ds=et+1<me?ws.el||cp(ws):Z;Je[Q]===0?m(null,Ye,M,Ds,A,F,J,X,te):Le&&(B<0||Q!==xt[B]?de(Ye,M,Ds,2):B--)}}},de=(g,C,M,Z,A=null)=>{const{el:F,type:J,transition:X,children:te,shapeFlag:Q}=g;if(Q&6){de(g.component.subTree,C,M,Z);return}if(Q&128){g.suspense.move(C,M,Z);return}if(Q&64){J.move(g,C,M,ge);return}if(J===At){n(F,C,M);for(let le=0;le<te.length;le++)de(te[le],C,M,Z);n(g.anchor,C,M);return}if(J===Pn){y(g,C,M);return}if(Z!==2&&Q&1&&X)if(Z===0)X.persisted&&!F[us]?n(F,C,M):(X.beforeEnter(F),n(F,C,M),gt(()=>X.enter(F),A));else{const{leave:le,delayLeave:fe,afterLeave:xe}=X,Se=()=>{g.ctx.isUnmounted?a(F):n(F,C,M)},Ae=()=>{const B=F._isLeaving||!!F[us];F._isLeaving&&F[us](!0),X.persisted&&!B?Se():le(F,()=>{Se(),xe&&xe()})};fe?fe(F,Se,Ae):Ae()}else n(F,C,M)},se=(g,C,M,Z=!1,A=!1)=>{const{type:F,props:J,ref:X,children:te,dynamicChildren:Q,shapeFlag:me,patchFlag:le,dirs:fe,cacheIndex:xe,memo:Se}=g;if(le===-2&&(A=!1),X!=null&&(en(),ma(X,null,M,g,!0),tn()),xe!=null&&(C.renderCache[xe]=void 0),me&256){C.ctx.deactivate(g);return}const Ae=me&1&&fe,B=!Ys(g);let re;if(B&&(re=J&&J.onVnodeBeforeUnmount)&&Jt(re,C,g),me&6)ue(g.component,M,Z);else{if(me&128){g.suspense.unmount(M,Z);return}Ae&&Rs(g,null,C,"beforeUnmount"),me&64?g.type.remove(g,C,M,ge,Z):Q&&!Q.hasOnce&&(F!==At||le>0&&le&64)?Ie(Q,C,M,!1,!0):(F===At&&le&384||!A&&me&16)&&Ie(te,C,M),Z&&pe(g)}const ye=Se!=null&&xe==null;(B&&(re=J&&J.onVnodeUnmounted)||Ae||ye)&&gt(()=>{re&&Jt(re,C,g),Ae&&Rs(g,null,C,"unmounted"),ye&&(g.el=null)},M)},pe=g=>{const{type:C,el:M,anchor:Z,transition:A}=g;if(C===At){ee(M,Z);return}if(C===Pn){x(g);return}const F=()=>{a(M),A&&!A.persisted&&A.afterLeave&&A.afterLeave()};if(g.shapeFlag&1&&A&&!A.persisted){const{leave:J,delayLeave:X}=A,te=()=>J(M,F);X?X(g.el,F,te):te()}else F()},ee=(g,C)=>{let M;for(;g!==C;)M=f(g),a(g),g=M;a(C)},ue=(g,C,M)=>{const{bum:Z,scope:A,job:F,subTree:J,um:X,m:te,a:Q}=g;ml(te),ml(Q),Z&&pa(Z),A.stop(),F&&(F.flags|=8,se(J,g,C,M)),X&&gt(X,C),gt(()=>{g.isUnmounted=!0},C)},Ie=(g,C,M,Z=!1,A=!1,F=0)=>{for(let J=F;J<g.length;J++)se(g[J],C,M,Z,A)},j=g=>{if(g.shapeFlag&6)return j(g.component.subTree);if(g.shapeFlag&128)return g.suspense.next();const C=f(g.anchor||g.el),M=C&&C[Nf];return M?f(M):C};let oe=!1;const ie=(g,C,M)=>{let Z;g==null?C._vnode&&(se(C._vnode,null,null,!0),Z=C._vnode.component):m(C._vnode||null,g,C,null,null,null,M),C._vnode=g,oe||(oe=!0,Xc(Z),fl(),oe=!1)},ge={p:m,um:se,m:de,r:pe,mt:$,mc:w,pc:I,pbc:L,n:j,o:e};let be,Oe;return t&&([be,Oe]=t(ge)),{render:ie,hydrate:be,createApp:Wv(ie,be)}}function wr({type:e,props:t},s){return s==="svg"&&e==="foreignObject"||s==="mathml"&&e==="annotation-xml"&&t&&t.encoding&&t.encoding.includes("html")?void 0:s}function Cn({effect:e,job:t},s){s?(e.flags|=32,t.flags|=4):(e.flags&=-33,t.flags&=-5)}function rp(e,t){return(!e||e&&!e.pendingBranch)&&t&&!t.persisted}function Wo(e,t,s=!1){const n=e.children,a=t.children;if(he(n)&&he(a))for(let i=0;i<n.length;i++){const l=n[i];let r=a[i];r.shapeFlag&1&&!r.dynamicChildren&&((r.patchFlag<=0||r.patchFlag===32)&&(r=a[i]=Vs(a[i]),r.el=l.el),!s&&r.patchFlag!==-2&&Wo(l,r)),r.type===bn&&(r.patchFlag===-1&&(r=a[i]=Vs(r)),r.el=l.el),r.type===pt&&!r.el&&(r.el=l.el)}}function ob(e){const t=e.slice(),s=[0];let n,a,i,l,r;const o=e.length;for(n=0;n<o;n++){const c=e[n];if(c!==0){if(a=s[s.length-1],e[a]<c){t[n]=a,s.push(n);continue}for(i=0,l=s.length-1;i<l;)r=i+l>>1,e[s[r]]<c?i=r+1:l=r;c<e[s[i]]&&(i>0&&(t[n]=s[i-1]),s[i]=n)}}for(i=s.length,l=s[i-1];i-- >0;)s[i]=l,l=t[l];return s}function op(e){const t=e.subTree.component;if(t)return t.asyncDep&&!t.asyncResolved?t:op(t)}function ml(e){if(e)for(let t=0;t<e.length;t++)e[t].flags|=8}function cp(e){if(e.placeholder)return e.placeholder;const t=e.component;return t?cp(t.subTree):null}const gl=e=>e.__isSuspense;let Xr=0;const cb={name:"Suspense",__isSuspense:!0,process(e,t,s,n,a,i,l,r,o,c){if(e==null)db(t,s,n,a,i,l,r,o,c);else{if(i&&i.deps>0&&!e.suspense.isInFallback){t.suspense=e.suspense,t.suspense.vnode=t,t.el=e.el;return}fb(e,t,s,n,a,l,r,o,c)}},hydrate:pb,normalize:hb},ub=cb;function pi(e,t){const s=e.props&&e.props[t];we(s)&&s()}function db(e,t,s,n,a,i,l,r,o){const{p:c,o:{createElement:u}}=o,d=u("div"),f=e.suspense=up(e,a,n,t,d,s,i,l,r,o);c(null,f.pendingBranch=e.ssContent,d,null,n,f,i,l),f.deps>0?(pi(e,"onPending"),pi(e,"onFallback"),c(null,e.ssFallback,t,s,n,null,i,l),ga(f,e.ssFallback)):f.resolve(!1,!0)}function fb(e,t,s,n,a,i,l,r,{p:o,um:c,o:{createElement:u}}){const d=t.suspense=e.suspense;d.vnode=t,t.el=e.el;const f=t.ssContent,p=t.ssFallback,{activeBranch:b,pendingBranch:m,isInFallback:k,isHydrating:E}=d;if(m)d.pendingBranch=f,bs(m,f)?(o(m,f,d.hiddenContainer,null,a,d,i,l,r),d.deps<=0?d.resolve():k&&(E||(o(b,p,s,n,a,null,i,l,r),ga(d,p)))):(d.pendingId=Xr++,E?(d.isHydrating=!1,d.activeBranch=m):c(m,a,d),d.deps=0,d.effects.length=0,d.hiddenContainer=u("div"),k?(o(null,f,d.hiddenContainer,null,a,d,i,l,r),d.deps<=0?d.resolve():(o(b,p,s,n,a,null,i,l,r),ga(d,p))):b&&bs(b,f)?(o(b,f,s,n,a,d,i,l,r),d.resolve(!0)):(o(null,f,d.hiddenContainer,null,a,d,i,l,r),d.deps<=0&&d.resolve()));else if(b&&bs(b,f))o(b,f,s,n,a,d,i,l,r),ga(d,f);else if(pi(t,"onPending"),d.pendingBranch=f,f.shapeFlag&512?d.pendingId=f.component.suspenseId:d.pendingId=Xr++,o(null,f,d.hiddenContainer,null,a,d,i,l,r),d.deps<=0)d.resolve();else{const{timeout:v,pendingId:y}=d;v>0?setTimeout(()=>{d.pendingId===y&&d.fallback(p)},v):v===0&&d.fallback(p)}}function up(e,t,s,n,a,i,l,r,o,c,u=!1){const{p:d,m:f,um:p,n:b,o:{parentNode:m,remove:k}}=c;let E;const v=mb(e);v&&t&&t.pendingBranch&&(E=t.pendingId,t.deps++);const y=e.props?rl(e.props.timeout):void 0,x=i,T={vnode:e,parent:t,parentComponent:s,namespace:l,container:n,hiddenContainer:a,deps:0,pendingId:Xr++,timeout:typeof y=="number"?y:-1,activeBranch:null,isFallbackMountPending:!1,pendingBranch:null,isInFallback:!u,isHydrating:u,isUnmounted:!1,effects:[],resolve(N=!1,O=!1){const{vnode:w,activeBranch:R,pendingBranch:L,pendingId:U,effects:P,parentComponent:S,container:$,isInFallback:W}=T;let G=!1;if(T.isHydrating)T.isHydrating=!1;else if(!N){G=R&&L.transition&&L.transition.mode==="out-in";let z=!1;G&&(R.transition.afterLeave=()=>{U===T.pendingId&&(f(L,$,i===x&&!z?b(R):i,0),ci(P),W&&w.ssFallback&&(w.ssFallback.el=null))}),R&&!T.isFallbackMountPending&&(m(R.el)===$&&(i=b(R),z=!0),p(R,S,T,!0),!G&&W&&w.ssFallback&&gt(()=>w.ssFallback.el=null,T)),G||f(L,$,i,0)}T.isFallbackMountPending=!1,ga(T,L),T.pendingBranch=null,T.isInFallback=!1;let D=T.parent,I=!1;for(;D;){if(D.pendingBranch){D.effects.push(...P),I=!0;break}D=D.parent}!I&&!G&&ci(P),T.effects=[],v&&t&&t.pendingBranch&&E===t.pendingId&&(t.deps--,t.deps===0&&!O&&t.resolve()),pi(w,"onResolve")},fallback(N){if(!T.pendingBranch)return;const{vnode:O,activeBranch:w,parentComponent:R,container:L,namespace:U}=T;pi(O,"onFallback");const P=b(w),S=()=>{T.isFallbackMountPending=!1,T.isInFallback&&(d(null,N,L,P,R,null,U,r,o),ga(T,N))},$=N.transition&&N.transition.mode==="out-in";$&&(T.isFallbackMountPending=!0,w.transition.afterLeave=S),T.isInFallback=!0,p(w,R,null,!0),$||S()},move(N,O,w){T.activeBranch&&f(T.activeBranch,N,O,w),T.container=N},next(){return T.activeBranch&&b(T.activeBranch)},registerDep(N,O,w){const R=!!T.pendingBranch;R&&T.deps++;const L=N.vnode.el;N.asyncDep.catch(U=>{Gn(U,N,0)}).then(U=>{if(N.isUnmounted||T.isUnmounted||T.pendingId!==N.suspenseId)return;gi(),N.asyncResolved=!0;const{vnode:P}=N;eo(N,U,!1),L&&(P.el=L);const S=!L&&N.subTree.el;O(N,P,m(L||N.subTree.el),L?null:b(N.subTree),T,l,w),S&&(P.placeholder=null,k(S)),er(N,P.el),R&&--T.deps===0&&T.resolve()})},unmount(N,O){T.isUnmounted=!0,T.activeBranch&&p(T.activeBranch,s,N,O),T.pendingBranch&&p(T.pendingBranch,s,N,O)}};return T}function pb(e,t,s,n,a,i,l,r,o){const c=t.suspense=up(t,n,s,e.parentNode,document.createElement("div"),null,a,i,l,r,!0),u=o(e,c.pendingBranch=t.ssContent,s,c,i,l);return c.deps===0&&c.resolve(!1,!0),u}function hb(e){const{shapeFlag:t,children:s}=e,n=t&32;e.ssContent=pu(n?s.default:s),e.ssFallback=n?pu(s.fallback):lt(pt)}function pu(e){let t;if(we(e)){const s=Bn&&e._c;s&&(e._d=!1,hi()),e=e(),s&&(e._d=!0,t=$t,fp())}return he(e)&&(e=Qv(e)),e=Qt(e),t&&!e.dynamicChildren&&(e.dynamicChildren=t.filter(s=>s!==e)),e}function dp(e,t){t&&t.pendingBranch?he(e)?t.effects.push(...e):t.effects.push(e):ci(e)}function ga(e,t){e.activeBranch=t;const{vnode:s,parentComponent:n}=e;let a=t.el;for(;!a&&t.component;)t=t.component.subTree,a=t.el;s.el=a,n&&n.subTree===s&&(n.vnode.el=a,er(n,a))}function mb(e){const t=e.props&&e.props.suspensible;return t!=null&&t!==!1}const At=Symbol.for("v-fgt"),bn=Symbol.for("v-txt"),pt=Symbol.for("v-cmt"),Pn=Symbol.for("v-stc"),ei=[];let $t=null;function hi(e=!1){ei.push($t=e?null:[])}function fp(){ei.pop(),$t=ei[ei.length-1]||null}let Bn=1;function mi(e,t=!1){Bn+=e,e<0&&$t&&t&&($t.hasOnce=!0)}function pp(e){return e.dynamicChildren=Bn>0?$t||ua:null,fp(),Bn>0&&$t&&$t.push(e),e}function gb(e,t,s,n,a,i){return pp(Zo(e,t,s,n,a,i,!0))}function vl(e,t,s,n,a){return pp(lt(e,t,s,n,a,!0))}function nn(e){return e?e.__v_isVNode===!0:!1}function bs(e,t){return e.type===t.type&&e.key===t.key}function vb(e){}const hp=({key:e})=>e??null,tl=({ref:e,ref_key:t,ref_for:s})=>(typeof e=="number"&&(e=""+e),e!=null?Ee(e)||bt(e)||we(e)?{i:Ot,r:e,k:t,f:!!s}:e:null);function Zo(e,t=null,s=null,n=0,a=null,i=e===At?0:1,l=!1,r=!1){const o={__v_isVNode:!0,__v_skip:!0,type:e,props:t,key:t&&hp(t),ref:t&&tl(t),scopeId:Wl,slotScopeIds:null,children:s,component:null,suspense:null,ssContent:null,ssFallback:null,dirs:null,transition:null,el:null,anchor:null,target:null,targetStart:null,targetAnchor:null,staticCount:0,shapeFlag:i,patchFlag:n,dynamicProps:a,dynamicChildren:null,appContext:null,ctx:Ot};return r?(Yo(o,s),i&128&&e.normalize(o)):s&&(o.shapeFlag|=Ee(s)?8:16),Bn>0&&!l&&$t&&(o.patchFlag>0||i&6)&&o.patchFlag!==32&&$t.push(o),o}const lt=bb;function bb(e,t=null,s=null,n=0,a=null,i=!1){if((!e||e===jf)&&(e=pt),nn(e)){const r=Ls(e,t,!0);return s&&Yo(r,s),Bn>0&&!i&&$t&&(r.shapeFlag&6?$t[$t.indexOf(e)]=r:$t.push(r)),r.patchFlag=-2,r}if(Tb(e)&&(e=e.__vccOpts),t){t=mp(t);let{class:r,style:o}=t;r&&!Ee(r)&&(t.class=Ti(r)),je(o)&&(Ci(o)&&!he(o)&&(o=Pe({},o)),t.style=Si(o))}const l=Ee(e)?1:gl(e)?128:Of(e)?64:je(e)?4:we(e)?2:0;return Zo(e,t,s,n,a,l,i,!0)}function mp(e){return e?Ci(e)||Qf(e)?Pe({},e):e:null}function Ls(e,t,s=!1,n=!1){const{props:a,ref:i,patchFlag:l,children:r,transition:o}=e,c=t?vp(a||{},t):a,u={__v_isVNode:!0,__v_skip:!0,type:e.type,props:c,key:c&&hp(c),ref:t&&t.ref?s&&i?he(i)?i.concat(tl(t)):[i,tl(t)]:tl(t):i,scopeId:e.scopeId,slotScopeIds:e.slotScopeIds,children:r,target:e.target,targetStart:e.targetStart,targetAnchor:e.targetAnchor,staticCount:e.staticCount,shapeFlag:e.shapeFlag,patchFlag:t&&e.type!==At?l===-1?16:l|16:l,dynamicProps:e.dynamicProps,dynamicChildren:e.dynamicChildren,appContext:e.appContext,dirs:e.dirs,transition:o,component:e.component,suspense:e.suspense,ssContent:e.ssContent&&Ls(e.ssContent),ssFallback:e.ssFallback&&Ls(e.ssFallback),placeholder:e.placeholder,el:e.el,anchor:e.anchor,ctx:e.ctx,ce:e.ce};return o&&n&&sn(u,o.clone(u)),u}function Jo(e=" ",t=0){return lt(bn,null,e,t)}function yb(e,t){const s=lt(Pn,null,e);return s.staticCount=t,s}function gp(e="",t=!1){return t?(hi(),vl(pt,null,e)):lt(pt,null,e)}function Qt(e){return e==null||typeof e=="boolean"?lt(pt):he(e)?lt(At,null,e.slice()):nn(e)?Vs(e):lt(bn,null,String(e))}function Vs(e){return e.el===null&&e.patchFlag!==-1||e.memo?e:Ls(e)}function Yo(e,t){let s=0;const{shapeFlag:n}=e;if(t==null)t=null;else if(he(t))s=16;else if(typeof t=="object")if(n&65){const a=t.default;a&&(a._c&&(a._d=!1),Yo(e,a()),a._c&&(a._d=!0));return}else{s=32;const a=t._;!a&&!Qf(t)?t._ctx=Ot:a===3&&Ot&&(Ot.slots._===1?t._=1:(t._=2,e.patchFlag|=1024))}else we(t)?(t={default:t,_ctx:Ot},s=32):(t=String(t),n&64?(s=16,t=[Jo(t)]):s=8);e.children=t,e.shapeFlag|=s}function vp(...e){const t={};for(let s=0;s<e.length;s++){const n=e[s];for(const a in n)if(a==="class")t.class!==n.class&&(t.class=Ti([t.class,n.class]));else if(a==="style")t.style=Si([t.style,n.style]);else if(zn(a)){const i=t[a],l=n[a];l&&i!==l&&!(he(i)&&i.includes(l))?t[a]=i?[].concat(i,l):l:l==null&&i==null&&!Fl(a)&&(t[a]=l)}else a!==""&&(t[a]=n[a])}return t}function Jt(e,t,s,n=null){is(e,t,7,[s,n])}const xb=qf();let _b=0;function bp(e,t,s){const n=e.type,a=(t?t.appContext:e.appContext)||xb,i={uid:_b++,vnode:e,type:n,parent:t,appContext:a,root:null,next:null,subTree:null,effect:null,update:null,job:null,scope:new Eo(!0),render:null,proxy:null,exposed:null,exposeProxy:null,withProxy:null,provides:t?t.provides:Object.create(a.provides),ids:t?t.ids:["",0,0],accessCache:null,renderCache:[],components:null,directives:null,propsOptions:ep(n,a),emitsOptions:Wf(n,a),emit:null,emitted:null,propsDefaults:Fe,inheritAttrs:n.inheritAttrs,ctx:Fe,data:Fe,props:Fe,attrs:Fe,slots:Fe,refs:Fe,setupState:Fe,setupContext:null,suspense:s,suspenseId:s?s.pendingId:0,asyncDep:null,asyncResolved:!1,isMounted:!1,isUnmounted:!1,isDeactivated:!1,bc:null,c:null,bm:null,m:null,bu:null,u:null,um:null,bum:null,da:null,a:null,rtg:null,rtc:null,ec:null,sp:null};return i.ctx={_:i},i.root=t?t.root:i,i.emit=Jv.bind(null,i),e.ce&&e.ce(i),i}let Nt=null;const qt=()=>Nt||Ot;let bl,va;{const e=Vl(),t=(s,n)=>{let a;return(a=e[s])||(a=e[s]=[]),a.push(n),i=>{a.length>1?a.forEach(l=>l(i)):a[0](i)}};bl=t("__VUE_INSTANCE_SETTERS__",s=>Nt=s),va=t("__VUE_SSR_SETTERS__",s=>Hn=s)}const La=e=>{const t=Nt;return bl(e),e.scope.on(),()=>{e.scope.off(),bl(t)}},gi=()=>{Nt&&Nt.scope.off(),bl(null)};function yp(e){return e.vnode.shapeFlag&4}let Hn=!1;function xp(e,t=!1,s=!1){t&&va(t);const{props:n,children:a}=e.vnode,i=yp(e);sb(e,n,i,t),lb(e,a,s||t);const l=i?kb(e,t):void 0;return t&&va(!1),l}function kb(e,t){const s=e.type;e.accessCache=Object.create(null),e.proxy=new Proxy(e.ctx,Zr);const{setup:n}=s;if(n){en();const a=e.setupContext=n.length>1?wp(e):null,i=La(e),l=Oa(n,e,0,[e.props,a]),r=Co(l);if(tn(),i(),(r||e.sp)&&!Ys(e)&&$o(e),r){if(l.then(gi,gi),t)return l.then(o=>{eo(e,o,t)}).catch(o=>{Gn(o,e,0)});e.asyncDep=l}else eo(e,l,t)}else kp(e,t)}function eo(e,t,s){we(t)?e.type.__ssrInlineRender?e.ssrRender=t:e.render=t:je(t)&&(e.setupState=Lo(t)),kp(e,s)}let yl,to;function _p(e){yl=e,to=t=>{t.render._rc&&(t.withProxy=new Proxy(t.ctx,Av))}}const wb=()=>!yl;function kp(e,t,s){const n=e.type;if(!e.render){if(!t&&yl&&!n.render){const a=n.template||Ko(e).template;if(a){const{isCustomElement:i,compilerOptions:l}=e.appContext.config,{delimiters:r,compilerOptions:o}=n,c=Pe(Pe({isCustomElement:i,delimiters:r},l),o);n.render=yl(a,c)}}e.render=n.render||Lt,to&&to(e)}{const a=La(e);en();try{Vv(e)}finally{tn(),a()}}}const Sb={get(e,t){return Ft(e,"get",""),e[t]}};function wp(e){const t=s=>{e.exposed=s||{}};return{attrs:new Proxy(e.attrs,Sb),slots:e.slots,emit:e.emit,expose:t}}function Ii(e){return e.exposed?e.exposeProxy||(e.exposeProxy=new Proxy(Lo(vf(e.exposed)),{get(t,s){if(s in t)return t[s];if(s in Xa)return Xa[s](e)},has(t,s){return s in t||s in Xa}})):e.proxy}function so(e,t=!0){return we(e)?e.displayName||e.name:e.name||t&&e.__name}function Tb(e){return we(e)&&"__vccOpts"in e}const Y=(e,t)=>Ng(e,t,Hn);function _a(e,t,s){try{mi(-1);const n=arguments.length;return n===2?je(t)&&!he(t)?nn(t)?lt(e,null,[t]):lt(e,t):lt(e,null,t):(n>3?s=Array.prototype.slice.call(arguments,2):n===3&&nn(s)&&(s=[s]),lt(e,t,s))}finally{mi(1)}}function Cb(){}function Eb(e,t,s,n){const a=s[n];if(a&&Sp(a,e))return a;const i=t();return i.memo=e.slice(),i.cacheIndex=n,s[n]=i}function Sp(e,t){const s=e.memo;if(s.length!=t.length)return!1;for(let n=0;n<s.length;n++)if(Ct(s[n],t[n]))return!1;return Bn>0&&$t&&$t.push(e),!0}const Tp="3.5.38",Ab=Lt,Rb=Bg,Ib=ia,Nb=Cf,Ob={createComponentInstance:bp,setupComponent:xp,renderComponentRoot:el,setCurrentRenderingInstance:di,isVNode:nn,normalizeVNode:Qt,getComponentPublicInstance:Ii,ensureValidVNode:zo,pushWarningContext:Pg,popWarningContext:Fg},Lb=Ob,Db=null,Mb=null,Pb=null;/**
* @vue/runtime-dom v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/let no;const hu=typeof window<"u"&&window.trustedTypes;if(hu)try{no=hu.createPolicy("vue",{createHTML:e=>e})}catch{}const Cp=no?e=>no.createHTML(e):e=>e,Fb="http://www.w3.org/2000/svg",$b="http://www.w3.org/1998/Math/MathML",Hs=typeof document<"u"?document:null,mu=Hs&&Hs.createElement("template"),Ep={insert:(e,t,s)=>{t.insertBefore(e,s||null)},remove:e=>{const t=e.parentNode;t&&t.removeChild(e)},createElement:(e,t,s,n)=>{const a=t==="svg"?Hs.createElementNS(Fb,e):t==="mathml"?Hs.createElementNS($b,e):s?Hs.createElement(e,{is:s}):Hs.createElement(e);return e==="select"&&n&&n.multiple!=null&&a.setAttribute("multiple",n.multiple),a},createText:e=>Hs.createTextNode(e),createComment:e=>Hs.createComment(e),setText:(e,t)=>{e.nodeValue=t},setElementText:(e,t)=>{e.textContent=t},parentNode:e=>e.parentNode,nextSibling:e=>e.nextSibling,querySelector:e=>Hs.querySelector(e),setScopeId(e,t){e.setAttribute(t,"")},insertStaticContent(e,t,s,n,a,i){const l=s?s.previousSibling:t.lastChild;if(a&&(a===i||a.nextSibling))for(;t.insertBefore(a.cloneNode(!0),s),!(a===i||!(a=a.nextSibling)););else{mu.innerHTML=Cp(n==="svg"?`<svg>${e}</svg>`:n==="mathml"?`<math>${e}</math>`:e);const r=mu.content;if(n==="svg"||n==="mathml"){const o=r.firstChild;for(;o.firstChild;)r.appendChild(o.firstChild);r.removeChild(o)}t.insertBefore(r,s)}return[l?l.nextSibling:t.firstChild,s?s.previousSibling:t.lastChild]}},cn="transition",$a="animation",ka=Symbol("_vtc"),Ap={name:String,type:String,css:{type:Boolean,default:!0},duration:[String,Number,Object],enterFromClass:String,enterActiveClass:String,enterToClass:String,appearFromClass:String,appearActiveClass:String,appearToClass:String,leaveFromClass:String,leaveActiveClass:String,leaveToClass:String},Rp=Pe({},Fo,Ap),Ub=e=>(e.displayName="Transition",e.props=Rp,e),Bb=Ub((e,{slots:t})=>_a(Mf,Ip(e),t)),En=(e,t=[])=>{he(e)?e.forEach(s=>s(...t)):e&&e(...t)},gu=e=>e?he(e)?e.some(t=>t.length>1):e.length>1:!1;function Ip(e){const t={};for(const P in e)P in Ap||(t[P]=e[P]);if(e.css===!1)return t;const{name:s="v",type:n,duration:a,enterFromClass:i=`${s}-enter-from`,enterActiveClass:l=`${s}-enter-active`,enterToClass:r=`${s}-enter-to`,appearFromClass:o=i,appearActiveClass:c=l,appearToClass:u=r,leaveFromClass:d=`${s}-leave-from`,leaveActiveClass:f=`${s}-leave-active`,leaveToClass:p=`${s}-leave-to`}=e,b=Hb(a),m=b&&b[0],k=b&&b[1],{onBeforeEnter:E,onEnter:v,onEnterCancelled:y,onLeave:x,onLeaveCancelled:T,onBeforeAppear:N=E,onAppear:O=v,onAppearCancelled:w=y}=t,R=(P,S,$,W)=>{P._enterCancelled=W,pn(P,S?u:r),pn(P,S?c:l),$&&$()},L=(P,S)=>{P._isLeaving=!1,pn(P,d),pn(P,p),pn(P,f),S&&S()},U=P=>(S,$)=>{const W=P?O:v,G=()=>R(S,P,$);En(W,[S,G]),vu(()=>{pn(S,P?o:i),Ts(S,P?u:r),gu(W)||bu(S,n,m,G)})};return Pe(t,{onBeforeEnter(P){En(E,[P]),Ts(P,i),Ts(P,l)},onBeforeAppear(P){En(N,[P]),Ts(P,o),Ts(P,c)},onEnter:U(!1),onAppear:U(!0),onLeave(P,S){P._isLeaving=!0;const $=()=>L(P,S);Ts(P,d),P._enterCancelled?(Ts(P,f),ao(P)):(ao(P),Ts(P,f)),vu(()=>{P._isLeaving&&(pn(P,d),Ts(P,p),gu(x)||bu(P,n,k,$))}),En(x,[P,$])},onEnterCancelled(P){R(P,!1,void 0,!0),En(y,[P])},onAppearCancelled(P){R(P,!0,void 0,!0),En(w,[P])},onLeaveCancelled(P){L(P),En(T,[P])}})}function Hb(e){if(e==null)return null;if(je(e))return[Sr(e.enter),Sr(e.leave)];{const t=Sr(e);return[t,t]}}function Sr(e){return rl(e)}function Ts(e,t){t.split(/\s+/).forEach(s=>s&&e.classList.add(s)),(e[ka]||(e[ka]=new Set)).add(t)}function pn(e,t){t.split(/\s+/).forEach(n=>n&&e.classList.remove(n));const s=e[ka];s&&(s.delete(t),s.size||(e[ka]=void 0))}function vu(e){requestAnimationFrame(()=>{requestAnimationFrame(e)})}let Vb=0;function bu(e,t,s,n){const a=e._endId=++Vb,i=()=>{a===e._endId&&n()};if(s!=null)return setTimeout(i,s);const{type:l,timeout:r,propCount:o}=Np(e,t);if(!l)return n();const c=l+"end";let u=0;const d=()=>{e.removeEventListener(c,f),i()},f=p=>{p.target===e&&++u>=o&&d()};setTimeout(()=>{u<o&&d()},r+1),e.addEventListener(c,f)}function Np(e,t){const s=window.getComputedStyle(e),n=b=>(s[b]||"").split(", "),a=n(`${cn}Delay`),i=n(`${cn}Duration`),l=yu(a,i),r=n(`${$a}Delay`),o=n(`${$a}Duration`),c=yu(r,o);let u=null,d=0,f=0;t===cn?l>0&&(u=cn,d=l,f=i.length):t===$a?c>0&&(u=$a,d=c,f=o.length):(d=Math.max(l,c),u=d>0?l>c?cn:$a:null,f=u?u===cn?i.length:o.length:0);const p=u===cn&&/\b(?:transform|all)(?:,|$)/.test(n(`${cn}Property`).toString());return{type:u,timeout:d,propCount:f,hasTransform:p}}function yu(e,t){for(;e.length<t.length;)e=e.concat(e);return Math.max(...t.map((s,n)=>xu(s)+xu(e[n])))}function xu(e){return e==="auto"?0:Number(e.slice(0,-1).replace(",","."))*1e3}function ao(e){return(e?e.ownerDocument:document).body.offsetHeight}function jb(e,t,s){const n=e[ka];n&&(t=(t?[t,...n]:[...n]).join(" ")),t==null?e.removeAttribute("class"):s?e.setAttribute("class",t):e.className=t}const xl=Symbol("_vod"),Qo=Symbol("_vsh"),Op={name:"show",beforeMount(e,{value:t},{transition:s}){e[xl]=e.style.display==="none"?"":e.style.display,s&&t?s.beforeEnter(e):Ua(e,t)},mounted(e,{value:t},{transition:s}){s&&t&&s.enter(e)},updated(e,{value:t,oldValue:s},{transition:n}){!t!=!s&&(n?t?(n.beforeEnter(e),Ua(e,!0),n.enter(e)):n.leave(e,()=>{Ua(e,!1)}):Ua(e,t))},beforeUnmount(e,{value:t}){Ua(e,t)}};function Ua(e,t){e.style.display=t?e[xl]:"none",e[Qo]=!t}function zb(){Op.getSSRProps=({value:e})=>{if(!e)return{style:{display:"none"}}}}const Lp=Symbol("");function Kb(e){const t=qt();if(!t)return;const s=t.ut=(a=e(t.proxy))=>{Array.from(document.querySelectorAll(`[data-v-owner="${t.uid}"]`)).forEach(i=>_l(i,a))},n=()=>{const a=e(t.proxy);t.ce?_l(t.ce,a):io(t.subTree,a),s(a)};Ho(()=>{ci(n)}),Be(()=>{hs(n,Lt,{flush:"post"});const a=new MutationObserver(n);a.observe(t.subTree.el.parentNode,{childList:!0}),ht(()=>a.disconnect())})}function io(e,t){if(e.shapeFlag&128){const s=e.suspense;e=s.activeBranch,s.pendingBranch&&!s.isHydrating&&s.effects.push(()=>{io(s.activeBranch,t)})}for(;e.component;)e=e.component.subTree;if(e.shapeFlag&1&&e.el)_l(e.el,t);else if(e.type===At)e.children.forEach(s=>io(s,t));else if(e.type===Pn){let{el:s,anchor:n}=e;for(;s&&(_l(s,t),s!==n);)s=s.nextSibling}}function _l(e,t){if(e.nodeType===1){const s=e.style;let n="";for(const a in t){const i=Ym(t[a]);s.setProperty(`--${a}`,i),n+=`--${a}: ${i};`}s[Lp]=n}}const qb=/(?:^|;)\s*display\s*:/;function Gb(e,t,s){const n=e.style,a=Ee(s);let i=!1;if(s&&!a){if(t)if(Ee(t))for(const l of t.split(";")){const r=l.slice(0,l.indexOf(":")).trim();s[r]==null&&Wa(n,r,"")}else for(const l in t)s[l]==null&&Wa(n,l,"");for(const l in s){l==="display"&&(i=!0);const r=s[l];r!=null?Zb(e,l,!Ee(t)&&t?t[l]:void 0,r)||Wa(n,l,r):Wa(n,l,"")}}else if(a){if(t!==s){const l=n[Lp];l&&(s+=";"+l),n.cssText=s,i=qb.test(s)}}else t&&e.removeAttribute("style");xl in e&&(e[xl]=i?n.display:"",e[Qo]&&(n.display="none"))}const _u=/\s*!important$/;function Wa(e,t,s){if(he(s))s.forEach(n=>Wa(e,t,n));else if(s==null&&(s=""),t.startsWith("--"))e.setProperty(t,s);else{const n=Wb(e,t);_u.test(s)?e.setProperty(Xt(n),s.replace(_u,""),"important"):e[n]=s}}const ku=["Webkit","Moz","ms"],Tr={};function Wb(e,t){const s=Tr[t];if(s)return s;let n=Qe(t);if(n!=="filter"&&n in e)return Tr[t]=n;n=qn(n);for(let a=0;a<ku.length;a++){const i=ku[a]+n;if(i in e)return Tr[t]=i}return t}function Zb(e,t,s,n){return e.tagName==="TEXTAREA"&&(t==="width"||t==="height")&&Ee(n)&&s===n}const wu="http://www.w3.org/1999/xlink";function Su(e,t,s,n,a,i=Zm(t)){n&&t.startsWith("xlink:")?s==null?e.removeAttributeNS(wu,t.slice(6,t.length)):e.setAttributeNS(wu,t,s):s==null||i&&!Jd(s)?e.removeAttribute(t):e.setAttribute(t,i?"":Bt(s)?String(s):s)}function Tu(e,t,s,n,a){if(t==="innerHTML"||t==="textContent"){s!=null&&(e[t]=t==="innerHTML"?Cp(s):s);return}const i=e.tagName;if(t==="value"&&i!=="PROGRESS"&&!i.includes("-")){const r=i==="OPTION"?e.getAttribute("value")||"":e.value,o=s==null?e.type==="checkbox"?"on":"":String(s);(r!==o||!("_value"in e))&&(e.value=o),s==null&&e.removeAttribute(t),e._value=s;return}let l=!1;if(s===""||s==null){const r=typeof e[t];r==="boolean"?s=Jd(s):s==null&&r==="string"?(s="",l=!0):r==="number"&&(s=0,l=!0)}try{e[t]=s}catch{}l&&e.removeAttribute(a||t)}function qs(e,t,s,n){e.addEventListener(t,s,n)}function Jb(e,t,s,n){e.removeEventListener(t,s,n)}const Cu=Symbol("_vei");function Yb(e,t,s,n,a=null){const i=e[Cu]||(e[Cu]={}),l=i[t];if(n&&l)l.value=n;else{const[r,o]=Qb(t);if(n){const c=i[t]=ty(n,a);qs(e,r,c,o)}else l&&(Jb(e,r,l,o),i[t]=void 0)}}const Eu=/(?:Once|Passive|Capture)$/;function Qb(e){let t;if(Eu.test(e)){t={};let n;for(;n=e.match(Eu);)e=e.slice(0,e.length-n[0].length),t[n[0].toLowerCase()]=!0}return[e[2]===":"?e.slice(3):Xt(e.slice(2)),t]}let Cr=0;const Xb=Promise.resolve(),ey=()=>Cr||(Xb.then(()=>Cr=0),Cr=Date.now());function ty(e,t){const s=n=>{if(!n._vts)n._vts=Date.now();else if(n._vts<=s.attached)return;const a=s.value;if(he(a)){const i=n.stopImmediatePropagation;n.stopImmediatePropagation=()=>{i.call(n),n._stopped=!0};const l=a.slice(),r=[n];for(let o=0;o<l.length&&!n._stopped;o++){const c=l[o];c&&is(c,t,5,r)}}else is(a,t,5,[n])};return s.value=e,s.attached=ey(),s}const Au=e=>e.charCodeAt(0)===111&&e.charCodeAt(1)===110&&e.charCodeAt(2)>96&&e.charCodeAt(2)<123,Dp=(e,t,s,n,a,i)=>{const l=a==="svg";t==="class"?jb(e,n,l):t==="style"?Gb(e,s,n):zn(t)?Fl(t)||Yb(e,t,s,n,i):(t[0]==="."?(t=t.slice(1),!0):t[0]==="^"?(t=t.slice(1),!1):sy(e,t,n,l))?(Tu(e,t,n),!e.tagName.includes("-")&&(t==="value"||t==="checked"||t==="selected")&&Su(e,t,n,l,i,t!=="value")):e._isVueCE&&(ny(e,t)||e._def.__asyncLoader&&(/[A-Z]/.test(t)||!Ee(n)))?Tu(e,Qe(t),n,i,t):(t==="true-value"?e._trueValue=n:t==="false-value"&&(e._falseValue=n),Su(e,t,n,l))};function sy(e,t,s,n){if(n)return!!(t==="innerHTML"||t==="textContent"||t in e&&Au(t)&&we(s));if(t==="spellcheck"||t==="draggable"||t==="translate"||t==="autocorrect"||t==="sandbox"&&e.tagName==="IFRAME"||t==="form"||t==="list"&&e.tagName==="INPUT"||t==="type"&&e.tagName==="TEXTAREA")return!1;if(t==="width"||t==="height"){const a=e.tagName;if(a==="IMG"||a==="VIDEO"||a==="CANVAS"||a==="SOURCE")return!1}return Au(t)&&Ee(s)?!1:t in e}function ny(e,t){const s=e._def.props;if(!s)return!1;const n=Qe(t);return Array.isArray(s)?s.some(a=>Qe(a)===n):Object.keys(s).some(a=>Qe(a)===n)}const Ru={};function Mp(e,t,s){let n=Ai(e,t);$l(n)&&(n=Pe({},n,t));class a extends tr{constructor(l){super(n,l,s)}}return a.def=n,a}const ay=((e,t)=>Mp(e,t,Wp)),iy=typeof HTMLElement<"u"?HTMLElement:class{};class tr extends iy{constructor(t,s={},n=Sl){super(),this._def=t,this._props=s,this._createApp=n,this._isVueCE=!0,this._instance=null,this._app=null,this._nonce=this._def.nonce,this._connected=!1,this._resolved=!1,this._patching=!1,this._dirty=!1,this._numberProps=null,this._styleChildren=new WeakSet,this._styleAnchors=new WeakMap,this._ob=null,this.shadowRoot&&n!==Sl?this._root=this.shadowRoot:t.shadowRoot!==!1?(this.attachShadow(Pe({},t.shadowRootOptions,{mode:"open"})),this._root=this.shadowRoot):this._root=this}connectedCallback(){if(!this.isConnected)return;!this.shadowRoot&&!this._resolved&&this._parseSlots(),this._connected=!0;let t=this;for(;t=t&&(t.assignedSlot||t.parentNode||t.host);)if(t instanceof tr){this._parent=t;break}this._instance||(this._resolved?this._mount(this._def):t&&t._pendingResolve?this._pendingResolve=t._pendingResolve.then(()=>{this._pendingResolve=void 0,this._resolveDef()}):this._resolveDef())}_setParent(t=this._parent){t&&(this._instance.parent=t._instance,this._inheritParentContext(t))}_inheritParentContext(t=this._parent){t&&this._app&&Object.setPrototypeOf(this._app._context.provides,t._instance.provides)}disconnectedCallback(){this._connected=!1,Et(()=>{this._connected||(this._ob&&(this._ob.disconnect(),this._ob=null),this._app&&this._app.unmount(),this._instance&&(this._instance.ce=void 0),this._app=this._instance=null,this._teleportTargets&&(this._teleportTargets.clear(),this._teleportTargets=void 0))})}_processMutations(t){for(const s of t)this._setAttr(s.attributeName)}_resolveDef(){if(this._pendingResolve)return;for(let n=0;n<this.attributes.length;n++)this._setAttr(this.attributes[n].name);this._ob=new MutationObserver(this._processMutations.bind(this)),this._ob.observe(this,{attributes:!0});const t=(n,a=!1)=>{this._resolved=!0,this._pendingResolve=void 0;const{props:i,styles:l}=n;let r;if(i&&!he(i))for(const o in i){const c=i[o];(c===Number||c&&c.type===Number)&&(o in this._props&&(this._props[o]=rl(this._props[o])),(r||(r=Object.create(null)))[Qe(o)]=!0)}this._numberProps=r,this._resolveProps(n),this.shadowRoot&&this._applyStyles(l),this._mount(n)},s=this._def.__asyncLoader;s?this._pendingResolve=s().then(n=>{n.configureApp=this._def.configureApp,t(this._def=n,!0)}):t(this._def)}_mount(t){this._app=this._createApp(t),this._inheritParentContext(),t.configureApp&&t.configureApp(this._app),this._app._ceVNode=this._createVNode(),this._app.mount(this._root);const s=this._instance&&this._instance.exposed;if(s)for(const n in s)ze(this,n)||Object.defineProperty(this,n,{get:()=>Ns(s[n])})}_resolveProps(t){const{props:s}=t,n=he(s)?s:Object.keys(s||{});for(const a of Object.keys(this))a[0]!=="_"&&n.includes(a)&&this._setProp(a,this[a]);for(const a of n.map(Qe))Object.defineProperty(this,a,{get(){return this._getProp(a)},set(i){this._setProp(a,i,!0,!this._patching)}})}_setAttr(t){if(t.startsWith("data-v-"))return;const s=this.hasAttribute(t);let n=s?this.getAttribute(t):Ru;const a=Qe(t);s&&this._numberProps&&this._numberProps[a]&&(n=rl(n)),this._setProp(a,n,!1,!0)}_getProp(t){return this._props[t]}_setProp(t,s,n=!0,a=!1){if(s!==this._props[t]&&(this._dirty=!0,s===Ru?delete this._props[t]:(this._props[t]=s,t==="key"&&this._app&&(this._app._ceVNode.key=s)),a&&this._instance&&this._update(),n)){const i=this._ob;i&&(this._processMutations(i.takeRecords()),i.disconnect()),s===!0?this.setAttribute(Xt(t),""):typeof s=="string"||typeof s=="number"?this.setAttribute(Xt(t),s+""):s||this.removeAttribute(Xt(t)),i&&i.observe(this,{attributes:!0})}}_update(){const t=this._createVNode();this._app&&(t.appContext=this._app._context),Gp(t,this._root)}_createVNode(){const t={};this.shadowRoot||(t.onVnodeMounted=t.onVnodeUpdated=this._renderSlots.bind(this));const s=lt(this._def,Pe(t,this._props));return this._instance||(s.ce=n=>{this._instance=n,n.ce=this,n.isCE=!0;const a=(i,l)=>{this.dispatchEvent(new CustomEvent(i,$l(l[0])?Pe({detail:l},l[0]):{detail:l}))};n.emit=(i,...l)=>{a(i,l),Xt(i)!==i&&a(Xt(i),l)},this._setParent()}),s}_applyStyles(t,s,n){if(!t)return;if(s){if(s===this._def||this._styleChildren.has(s))return;this._styleChildren.add(s)}const a=this._nonce,i=this.shadowRoot,l=n?this._getStyleAnchor(n)||this._getStyleAnchor(this._def):this._getRootStyleInsertionAnchor(i);let r=null;for(let o=t.length-1;o>=0;o--){const c=document.createElement("style");a&&c.setAttribute("nonce",a),c.textContent=t[o],i.insertBefore(c,r||l),r=c,o===0&&(n||this._styleAnchors.set(this._def,c),s&&this._styleAnchors.set(s,c))}}_getStyleAnchor(t){if(!t)return null;const s=this._styleAnchors.get(t);return s&&s.parentNode===this.shadowRoot?s:(s&&this._styleAnchors.delete(t),null)}_getRootStyleInsertionAnchor(t){for(let s=0;s<t.childNodes.length;s++){const n=t.childNodes[s];if(!(n instanceof HTMLStyleElement))return n}return null}_parseSlots(){const t=this._slots={};let s;for(;s=this.firstChild;){const n=s.nodeType===1&&s.getAttribute("slot")||"default";(t[n]||(t[n]=[])).push(s),this.removeChild(s)}}_renderSlots(){const t=this._getSlots(),s=this._instance.type.__scopeId;for(let n=0;n<t.length;n++){const a=t[n],i=a.getAttribute("name")||"default",l=this._slots[i],r=a.parentNode;if(l)for(const o of l){if(s&&o.nodeType===1){const c=s+"-s",u=document.createTreeWalker(o,1);o.setAttribute(c,"");let d;for(;d=u.nextNode();)d.setAttribute(c,"")}r.insertBefore(o,a)}else for(;a.firstChild;)r.insertBefore(a.firstChild,a);r.removeChild(a)}}_getSlots(){const t=[this];this._teleportTargets&&t.push(...this._teleportTargets);const s=new Set;for(const n of t){const a=n.querySelectorAll("slot");for(let i=0;i<a.length;i++)s.add(a[i])}return Array.from(s)}_injectChildStyle(t,s){this._applyStyles(t.styles,t,s)}_beginPatch(){this._patching=!0,this._dirty=!1}_endPatch(){this._patching=!1,this._dirty&&this._instance&&this._update()}_hasShadowRoot(){return this._def.shadowRoot!==!1}_removeChildStyle(t){}}function Pp(e){const t=qt(),s=t&&t.ce;return s||null}function ly(){const e=Pp();return e&&e.shadowRoot}function ry(e="$style"){{const t=qt();if(!t)return Fe;const s=t.type.__cssModules;if(!s)return Fe;const n=s[e];return n||Fe}}const Fp=new WeakMap,$p=new WeakMap,kl=Symbol("_moveCb"),Iu=Symbol("_enterCb"),oy=e=>(delete e.props.mode,e),cy=oy({name:"TransitionGroup",props:Pe({},Rp,{tag:String,moveClass:String}),setup(e,{slots:t}){const s=qt(),n=Po();let a,i;return Yl(()=>{if(!a.length)return;const l=e.moveClass||`${e.name||"v"}-move`;if(!hy(a[0].el,s.vnode.el,l)){a=[];return}a.forEach(dy),a.forEach(fy);const r=a.filter(py);ao(s.vnode.el),r.forEach(o=>{const c=o.el,u=c.style;Ts(c,l),u.transform=u.webkitTransform=u.transitionDuration="";const d=c[kl]=f=>{f&&f.target!==c||(!f||f.propertyName.endsWith("transform"))&&(c.removeEventListener("transitionend",d),c[kl]=null,pn(c,l))};c.addEventListener("transitionend",d)}),a=[]}),()=>{const l=Ue(e),r=Ip(l);let o=l.tag||At;if(a=[],i)for(let c=0;c<i.length;c++){const u=i[c];u.el&&u.el instanceof Element&&!u.el[Qo]&&(a.push(u),sn(u,xa(u,r,n,s)),Fp.set(u,Up(u.el)))}i=t.default?Zl(t.default()):[];for(let c=0;c<i.length;c++){const u=i[c];u.key!=null&&sn(u,xa(u,r,n,s))}return lt(o,null,i)}}}),uy=cy;function dy(e){const t=e.el;t[kl]&&t[kl](),t[Iu]&&t[Iu]()}function fy(e){$p.set(e,Up(e.el))}function py(e){const t=Fp.get(e),s=$p.get(e),n=t.left-s.left,a=t.top-s.top;if(n||a){const i=e.el,l=i.style,r=i.getBoundingClientRect();let o=1,c=1;return i.offsetWidth&&(o=r.width/i.offsetWidth),i.offsetHeight&&(c=r.height/i.offsetHeight),(!Number.isFinite(o)||o===0)&&(o=1),(!Number.isFinite(c)||c===0)&&(c=1),Math.abs(o-1)<.01&&(o=1),Math.abs(c-1)<.01&&(c=1),l.transform=l.webkitTransform=`translate(${n/o}px,${a/c}px)`,l.transitionDuration="0s",e}}function Up(e){const t=e.getBoundingClientRect();return{left:t.left,top:t.top}}function hy(e,t,s){const n=e.cloneNode(),a=e[ka];a&&a.forEach(r=>{r.split(/\s+/).forEach(o=>o&&n.classList.remove(o))}),s.split(/\s+/).forEach(r=>r&&n.classList.add(r)),n.style.display="none";const i=t.nodeType===1?t:t.parentNode;i.appendChild(n);const{hasTransform:l}=Np(n);return i.removeChild(n),l}const xn=e=>{const t=e.props["onUpdate:modelValue"]||!1;return he(t)?s=>pa(t,s):t};function my(e){e.target.composing=!0}function Nu(e){const t=e.target;t.composing&&(t.composing=!1,t.dispatchEvent(new Event("input")))}const ms=Symbol("_assign");function Ou(e,t,s){return t&&(e=e.trim()),s&&(e=Hl(e)),e}const wl={created(e,{modifiers:{lazy:t,trim:s,number:n}},a){e[ms]=xn(a);const i=n||a.props&&a.props.type==="number";qs(e,t?"change":"input",l=>{l.target.composing||e[ms](Ou(e.value,s,i))}),(s||i)&&qs(e,"change",()=>{e.value=Ou(e.value,s,i)}),t||(qs(e,"compositionstart",my),qs(e,"compositionend",Nu),qs(e,"change",Nu))},mounted(e,{value:t}){e.value=t??""},beforeUpdate(e,{value:t,oldValue:s,modifiers:{lazy:n,trim:a,number:i}},l){if(e[ms]=xn(l),e.composing)return;const r=(i||e.type==="number")&&!/^0\d/.test(e.value)?Hl(e.value):e.value,o=t??"";if(r===o)return;const c=e.getRootNode();(c instanceof Document||c instanceof ShadowRoot)&&c.activeElement===e&&e.type!=="range"&&(n&&t===s||a&&e.value.trim()===o)||(e.value=o)}},Xo={deep:!0,created(e,t,s){e[ms]=xn(s),qs(e,"change",()=>{const n=e._modelValue,a=wa(e),i=e.checked,l=e[ms];if(he(n)){const r=jl(n,a),o=r!==-1;if(i&&!o)l(n.concat(a));else if(!i&&o){const c=[...n];c.splice(r,1),l(c)}}else if(Kn(n)){const r=new Set(n);i?r.add(a):r.delete(a),l(r)}else l(Hp(e,i))})},mounted:Lu,beforeUpdate(e,t,s){e[ms]=xn(s),Lu(e,t,s)}};function Lu(e,{value:t,oldValue:s},n){e._modelValue=t;let a;if(he(t))a=jl(t,n.props.value)>-1;else if(Kn(t))a=t.has(n.props.value);else{if(t===s)return;a=Xs(t,Hp(e,!0))}e.checked!==a&&(e.checked=a)}const ec={created(e,{value:t},s){e.checked=Xs(t,s.props.value),e[ms]=xn(s),qs(e,"change",()=>{e[ms](wa(e))})},beforeUpdate(e,{value:t,oldValue:s},n){e[ms]=xn(n),t!==s&&(e.checked=Xs(t,n.props.value))}},Bp={deep:!0,created(e,{value:t,modifiers:{number:s}},n){const a=Kn(t);qs(e,"change",()=>{const i=Array.prototype.filter.call(e.options,l=>l.selected).map(l=>s?Hl(wa(l)):wa(l));e[ms](e.multiple?a?new Set(i):i:i[0]),e._assigning=!0,Et(()=>{e._assigning=!1})}),e[ms]=xn(n)},mounted(e,{value:t}){Du(e,t)},beforeUpdate(e,t,s){e[ms]=xn(s)},updated(e,{value:t}){e._assigning||Du(e,t)}};function Du(e,t){const s=e.multiple,n=he(t);if(!(s&&!n&&!Kn(t))){for(let a=0,i=e.options.length;a<i;a++){const l=e.options[a],r=wa(l);if(s)if(n){const o=typeof r;o==="string"||o==="number"?l.selected=t.some(c=>String(c)===String(r)):l.selected=jl(t,r)>-1}else l.selected=t.has(r);else if(Xs(wa(l),t)){e.selectedIndex!==a&&(e.selectedIndex=a);return}}!s&&e.selectedIndex!==-1&&(e.selectedIndex=-1)}}function wa(e){return"_value"in e?e._value:e.value}function Hp(e,t){const s=t?"_trueValue":"_falseValue";return s in e?e[s]:t}const Vp={created(e,t,s){Gi(e,t,s,null,"created")},mounted(e,t,s){Gi(e,t,s,null,"mounted")},beforeUpdate(e,t,s,n){Gi(e,t,s,n,"beforeUpdate")},updated(e,t,s,n){Gi(e,t,s,n,"updated")}};function jp(e,t){switch(e){case"SELECT":return Bp;case"TEXTAREA":return wl;default:switch(t){case"checkbox":return Xo;case"radio":return ec;default:return wl}}}function Gi(e,t,s,n,a){const l=jp(e.tagName,s.props&&s.props.type)[a];l&&l(e,t,s,n)}function gy(){wl.getSSRProps=({value:e})=>({value:e}),ec.getSSRProps=({value:e},t)=>{if(t.props&&Xs(t.props.value,e))return{checked:!0}},Xo.getSSRProps=({value:e},t)=>{if(he(e)){if(t.props&&jl(e,t.props.value)>-1)return{checked:!0}}else if(Kn(e)){if(t.props&&e.has(t.props.value))return{checked:!0}}else if(e)return{checked:!0}},Vp.getSSRProps=(e,t)=>{if(typeof t.type!="string")return;const s=jp(t.type.toUpperCase(),t.props&&t.props.type);if(s.getSSRProps)return s.getSSRProps(e,t)}}const vy=["ctrl","shift","alt","meta"],by={stop:e=>e.stopPropagation(),prevent:e=>e.preventDefault(),self:e=>e.target!==e.currentTarget,ctrl:e=>!e.ctrlKey,shift:e=>!e.shiftKey,alt:e=>!e.altKey,meta:e=>!e.metaKey,left:e=>"button"in e&&e.button!==0,middle:e=>"button"in e&&e.button!==1,right:e=>"button"in e&&e.button!==2,exact:(e,t)=>vy.some(s=>e[`${s}Key`]&&!t.includes(s))},yy=(e,t)=>{if(!e)return e;const s=e._withMods||(e._withMods={}),n=t.join(".");return s[n]||(s[n]=((a,...i)=>{for(let l=0;l<t.length;l++){const r=by[t[l]];if(r&&r(a,t))return}return e(a,...i)}))},xy={esc:"escape",space:" ",up:"arrow-up",left:"arrow-left",right:"arrow-right",down:"arrow-down",delete:"backspace"},_y=(e,t)=>{const s=e._withKeys||(e._withKeys={}),n=t.join(".");return s[n]||(s[n]=(a=>{if(!("key"in a))return;const i=Xt(a.key);if(t.some(l=>l===i||xy[l]===i))return e(a)}))},zp=Pe({patchProp:Dp},Ep);let ti,Mu=!1;function Kp(){return ti||(ti=ap(zp))}function qp(){return ti=Mu?ti:ip(zp),Mu=!0,ti}const Gp=((...e)=>{Kp().render(...e)}),ky=((...e)=>{qp().hydrate(...e)}),Sl=((...e)=>{const t=Kp().createApp(...e),{mount:s}=t;return t.mount=n=>{const a=Jp(n);if(!a)return;const i=t._component;!we(i)&&!i.render&&!i.template&&(i.template=a.innerHTML),a.nodeType===1&&(a.textContent="");const l=s(a,!1,Zp(a));return a instanceof Element&&(a.removeAttribute("v-cloak"),a.setAttribute("data-v-app","")),l},t}),Wp=((...e)=>{const t=qp().createApp(...e),{mount:s}=t;return t.mount=n=>{const a=Jp(n);if(a)return s(a,!0,Zp(a))},t});function Zp(e){if(e instanceof SVGElement)return"svg";if(typeof MathMLElement=="function"&&e instanceof MathMLElement)return"mathml"}function Jp(e){return Ee(e)?document.querySelector(e):e}let Pu=!1;const wy=()=>{Pu||(Pu=!0,gy(),zb())},Sy=Object.freeze(Object.defineProperty({__proto__:null,BaseTransition:Mf,BaseTransitionPropsValidators:Fo,Comment:pt,DeprecationTypes:Pb,EffectScope:Eo,ErrorCodes:Ug,ErrorTypeStrings:Rb,Fragment:At,KeepAlive:bv,ReactiveEffect:li,Static:Pn,Suspense:ub,Teleport:ev,Text:bn,TrackOpTypes:Og,Transition:Bb,TransitionGroup:uy,TriggerOpTypes:Lg,VueElement:tr,assertNumber:$g,callWithAsyncErrorHandling:is,callWithErrorHandling:Oa,camelize:Qe,capitalize:qn,cloneVNode:Ls,compatUtils:Mb,computed:Y,createApp:Sl,createBlock:vl,createCommentVNode:gp,createElementBlock:gb,createElementVNode:Zo,createHydrationRenderer:ip,createPropsRestProxy:Bv,createRenderer:ap,createSSRApp:Wp,createSlots:Tv,createStaticVNode:yb,createTextVNode:Jo,createVNode:lt,customRef:yf,defineAsyncComponent:gv,defineComponent:Ai,defineCustomElement:Mp,defineEmits:Iv,defineExpose:Nv,defineModel:Dv,defineOptions:Ov,defineProps:Rv,defineSSRCustomElement:ay,defineSlots:Lv,devtools:Ib,effect:tg,effectScope:Qm,getCurrentInstance:qt,getCurrentScope:ef,getCurrentWatcher:Dg,getTransitionRawChildren:Zl,guardReactiveProps:mp,h:_a,handleError:Gn,hasInjectionContext:Gg,hydrate:ky,hydrateOnIdle:uv,hydrateOnInteraction:hv,hydrateOnMediaQuery:pv,hydrateOnVisible:fv,initCustomFormatter:Cb,initDirectivesForSSR:wy,inject:ps,isMemoSame:Sp,isProxy:Ci,isReactive:Js,isReadonly:Os,isRef:bt,isRuntimeOnly:wb,isShallow:ts,isVNode:nn,markRaw:vf,mergeDefaults:$v,mergeModels:Uv,mergeProps:vp,nextTick:Et,nodeOps:Ep,normalizeClass:Ti,normalizeProps:Um,normalizeStyle:Si,onActivated:Uo,onBeforeMount:$f,onBeforeUnmount:Ql,onBeforeUpdate:Ho,onDeactivated:Bo,onErrorCaptured:Vf,onMounted:Be,onRenderTracked:Hf,onRenderTriggered:Bf,onScopeDispose:Xm,onServerPrefetch:Uf,onUnmounted:ht,onUpdated:Yl,onWatcherCleanup:_f,openBlock:hi,patchProp:Dp,popScopeId:zg,provide:Qa,proxyRefs:Lo,pushScopeId:jg,queuePostFlushCb:ci,reactive:_n,readonly:cl,ref:h,registerRuntimeCompiler:_p,render:Gp,renderList:Sv,renderSlot:Cv,resolveComponent:_v,resolveDirective:wv,resolveDynamicComponent:kv,resolveFilter:Db,resolveTransitionHooks:xa,setBlockTracking:mi,setDevtoolsHook:Nb,setTransitionHooks:sn,shallowReactive:No,shallowReadonly:xg,shallowRef:Oo,ssrContextKey:Ef,ssrUtils:Lb,stop:sg,toDisplayString:Qd,toHandlerKey:fa,toHandlers:Ev,toRaw:Ue,toRef:Rg,toRefs:Cg,toValue:wg,transformVNodeArgs:vb,triggerRef:kg,unref:Ns,useAttrs:Fv,useCssModule:ry,useCssVars:Kb,useHost:Pp,useId:sv,useModel:Zv,useSSRContext:Af,useShadowRoot:ly,useSlots:Pv,useTemplateRef:nv,useTransitionState:Po,vModelCheckbox:Xo,vModelDynamic:Vp,vModelRadio:ec,vModelSelect:Bp,vModelText:wl,vShow:Op,version:Tp,warn:Ab,watch:hs,watchEffect:Wg,watchPostEffect:Zg,watchSyncEffect:Rf,withAsyncContext:Hv,withCtx:Mo,withDefaults:Mv,withDirectives:qg,withKeys:_y,withMemo:Eb,withModifiers:yy,withScopeId:Kg},Symbol.toStringTag,{value:"Module"}));/**
* @vue/compiler-core v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const vi=Symbol(""),si=Symbol(""),tc=Symbol(""),Tl=Symbol(""),Yp=Symbol(""),Vn=Symbol(""),Qp=Symbol(""),Xp=Symbol(""),sc=Symbol(""),nc=Symbol(""),Ni=Symbol(""),ac=Symbol(""),eh=Symbol(""),ic=Symbol(""),lc=Symbol(""),rc=Symbol(""),oc=Symbol(""),cc=Symbol(""),uc=Symbol(""),th=Symbol(""),sh=Symbol(""),sr=Symbol(""),Cl=Symbol(""),dc=Symbol(""),fc=Symbol(""),bi=Symbol(""),Oi=Symbol(""),pc=Symbol(""),lo=Symbol(""),Ty=Symbol(""),ro=Symbol(""),El=Symbol(""),Cy=Symbol(""),Ey=Symbol(""),hc=Symbol(""),Ay=Symbol(""),Ry=Symbol(""),mc=Symbol(""),nh=Symbol(""),Sa={[vi]:"Fragment",[si]:"Teleport",[tc]:"Suspense",[Tl]:"KeepAlive",[Yp]:"BaseTransition",[Vn]:"openBlock",[Qp]:"createBlock",[Xp]:"createElementBlock",[sc]:"createVNode",[nc]:"createElementVNode",[Ni]:"createCommentVNode",[ac]:"createTextVNode",[eh]:"createStaticVNode",[ic]:"resolveComponent",[lc]:"resolveDynamicComponent",[rc]:"resolveDirective",[oc]:"resolveFilter",[cc]:"withDirectives",[uc]:"renderList",[th]:"renderSlot",[sh]:"createSlots",[sr]:"toDisplayString",[Cl]:"mergeProps",[dc]:"normalizeClass",[fc]:"normalizeStyle",[bi]:"normalizeProps",[Oi]:"guardReactiveProps",[pc]:"toHandlers",[lo]:"camelize",[Ty]:"capitalize",[ro]:"toHandlerKey",[El]:"setBlockTracking",[Cy]:"pushScopeId",[Ey]:"popScopeId",[hc]:"withCtx",[Ay]:"unref",[Ry]:"isRef",[mc]:"withMemo",[nh]:"isMemoSame"};function Iy(e){Object.getOwnPropertySymbols(e).forEach(t=>{Sa[t]=e[t]})}const os={start:{line:1,column:1,offset:0},end:{line:1,column:1,offset:0},source:""};function Ny(e,t=""){return{type:0,source:t,children:e,helpers:new Set,components:[],directives:[],hoists:[],imports:[],cached:[],temps:0,codegenNode:void 0,loc:os}}function yi(e,t,s,n,a,i,l,r=!1,o=!1,c=!1,u=os){return e&&(r?(e.helper(Vn),e.helper(Ea(e.inSSR,c))):e.helper(Ca(e.inSSR,c)),l&&e.helper(cc)),{type:13,tag:t,props:s,children:n,patchFlag:a,dynamicProps:i,directives:l,isBlock:r,disableTracking:o,isComponent:c,loc:u}}function Fn(e,t=os){return{type:17,loc:t,elements:e}}function fs(e,t=os){return{type:15,loc:t,properties:e}}function vt(e,t){return{type:16,loc:os,key:Ee(e)?Ne(e,!0):e,value:t}}function Ne(e,t=!1,s=os,n=0){return{type:4,loc:s,content:e,isStatic:t,constType:t?3:n}}function xs(e,t=os){return{type:8,loc:t,children:e}}function St(e,t=[],s=os){return{type:14,loc:s,callee:e,arguments:t}}function Ta(e,t=void 0,s=!1,n=!1,a=os){return{type:18,params:e,returns:t,newline:s,isSlot:n,loc:a}}function oo(e,t,s,n=!0){return{type:19,test:e,consequent:t,alternate:s,newline:n,loc:os}}function Oy(e,t,s=!1,n=!1){return{type:20,index:e,value:t,needPauseTracking:s,inVOnce:n,needArraySpread:!1,loc:os}}function Ly(e){return{type:21,body:e,loc:os}}function Ca(e,t){return e||t?sc:nc}function Ea(e,t){return e||t?Qp:Xp}function gc(e,{helper:t,removeHelper:s,inSSR:n}){e.isBlock||(e.isBlock=!0,s(Ca(n,e.isComponent)),t(Vn),t(Ea(n,e.isComponent)))}const Fu=new Uint8Array([123,123]),$u=new Uint8Array([125,125]);function Uu(e){return e>=97&&e<=122||e>=65&&e<=90}function ns(e){return e===32||e===10||e===9||e===12||e===13}function un(e){return e===47||e===62||ns(e)}function Al(e){const t=new Uint8Array(e.length);for(let s=0;s<e.length;s++)t[s]=e.charCodeAt(s);return t}const Dt={Cdata:new Uint8Array([67,68,65,84,65,91]),CdataEnd:new Uint8Array([93,93,62]),CommentEnd:new Uint8Array([45,45,62]),ScriptEnd:new Uint8Array([60,47,115,99,114,105,112,116]),StyleEnd:new Uint8Array([60,47,115,116,121,108,101]),TitleEnd:new Uint8Array([60,47,116,105,116,108,101]),TextareaEnd:new Uint8Array([60,47,116,101,120,116,97,114,101,97])};class Dy{constructor(t,s){this.stack=t,this.cbs=s,this.state=1,this.buffer="",this.sectionStart=0,this.index=0,this.entityStart=0,this.baseState=1,this.inRCDATA=!1,this.inXML=!1,this.inVPre=!1,this.newlines=[],this.mode=0,this.delimiterOpen=Fu,this.delimiterClose=$u,this.delimiterIndex=-1,this.currentSequence=void 0,this.sequenceIndex=0}get inSFCRoot(){return this.mode===2&&this.stack.length===0}reset(){this.state=1,this.mode=0,this.buffer="",this.sectionStart=0,this.index=0,this.baseState=1,this.inRCDATA=!1,this.currentSequence=void 0,this.newlines.length=0,this.delimiterOpen=Fu,this.delimiterClose=$u}getPos(t){let s=1,n=t+1;const a=this.newlines.length;let i=-1;if(a>100){let l=-1,r=a;for(;l+1<r;){const o=l+r>>>1;this.newlines[o]<t?l=o:r=o}i=l}else for(let l=a-1;l>=0;l--)if(t>this.newlines[l]){i=l;break}return i>=0&&(s=i+2,n=t-this.newlines[i]),{column:n,line:s,offset:t}}peek(){return this.buffer.charCodeAt(this.index+1)}stateText(t){t===60?(this.index>this.sectionStart&&this.cbs.ontext(this.sectionStart,this.index),this.state=5,this.sectionStart=this.index):!this.inVPre&&t===this.delimiterOpen[0]&&(this.state=2,this.delimiterIndex=0,this.stateInterpolationOpen(t))}stateInterpolationOpen(t){if(t===this.delimiterOpen[this.delimiterIndex])if(this.delimiterIndex===this.delimiterOpen.length-1){const s=this.index+1-this.delimiterOpen.length;s>this.sectionStart&&this.cbs.ontext(this.sectionStart,s),this.state=3,this.sectionStart=s}else this.delimiterIndex++;else this.inRCDATA?(this.state=32,this.stateInRCDATA(t)):(this.state=1,this.stateText(t))}stateInterpolation(t){t===this.delimiterClose[0]&&(this.state=4,this.delimiterIndex=0,this.stateInterpolationClose(t))}stateInterpolationClose(t){t===this.delimiterClose[this.delimiterIndex]?this.delimiterIndex===this.delimiterClose.length-1?(this.cbs.oninterpolation(this.sectionStart,this.index+1),this.inRCDATA?this.state=32:this.state=1,this.sectionStart=this.index+1):this.delimiterIndex++:(this.state=3,this.stateInterpolation(t))}stateSpecialStartSequence(t){const s=this.sequenceIndex===this.currentSequence.length;if(!(s?un(t):(t|32)===this.currentSequence[this.sequenceIndex]))this.inRCDATA=!1;else if(!s){this.sequenceIndex++;return}this.sequenceIndex=0,this.state=6,this.stateInTagName(t)}stateInRCDATA(t){if(this.sequenceIndex===this.currentSequence.length){if(t===62||ns(t)){const s=this.index-this.currentSequence.length;if(this.sectionStart<s){const n=this.index;this.index=s,this.cbs.ontext(this.sectionStart,s),this.index=n}this.sectionStart=s+2,this.stateInClosingTagName(t),this.inRCDATA=!1;return}this.sequenceIndex=0}(t|32)===this.currentSequence[this.sequenceIndex]?this.sequenceIndex+=1:this.sequenceIndex===0?this.currentSequence===Dt.TitleEnd||this.currentSequence===Dt.TextareaEnd&&!this.inSFCRoot?!this.inVPre&&t===this.delimiterOpen[0]&&(this.state=2,this.delimiterIndex=0,this.stateInterpolationOpen(t)):this.fastForwardTo(60)&&(this.sequenceIndex=1):this.sequenceIndex=+(t===60)}stateCDATASequence(t){t===Dt.Cdata[this.sequenceIndex]?++this.sequenceIndex===Dt.Cdata.length&&(this.state=28,this.currentSequence=Dt.CdataEnd,this.sequenceIndex=0,this.sectionStart=this.index+1):(this.sequenceIndex=0,this.state=23,this.stateInDeclaration(t))}fastForwardTo(t){for(;++this.index<this.buffer.length;){const s=this.buffer.charCodeAt(this.index);if(s===10&&this.newlines.push(this.index),s===t)return!0}return this.index=this.buffer.length-1,!1}stateInCommentLike(t){t===this.currentSequence[this.sequenceIndex]?++this.sequenceIndex===this.currentSequence.length&&(this.currentSequence===Dt.CdataEnd?this.cbs.oncdata(this.sectionStart,this.index-2):this.cbs.oncomment(this.sectionStart,this.index-2),this.sequenceIndex=0,this.sectionStart=this.index+1,this.state=1):this.sequenceIndex===0?this.fastForwardTo(this.currentSequence[0])&&(this.sequenceIndex=1):t!==this.currentSequence[this.sequenceIndex-1]&&(this.sequenceIndex=0)}startSpecial(t,s){this.enterRCDATA(t,s),this.state=31}enterRCDATA(t,s){this.inRCDATA=!0,this.currentSequence=t,this.sequenceIndex=s}stateBeforeTagName(t){t===33?(this.state=22,this.sectionStart=this.index+1):t===63?(this.state=24,this.sectionStart=this.index+1):Uu(t)?(this.sectionStart=this.index,this.mode===0?this.state=6:this.inSFCRoot?this.state=34:this.inXML?this.state=6:t===116?this.state=30:this.state=t===115?29:6):t===47?this.state=8:(this.state=1,this.stateText(t))}stateInTagName(t){un(t)&&this.handleTagName(t)}stateInSFCRootTagName(t){if(un(t)){const s=this.buffer.slice(this.sectionStart,this.index);s!=="template"&&this.enterRCDATA(Al("</"+s),0),this.handleTagName(t)}}handleTagName(t){this.cbs.onopentagname(this.sectionStart,this.index),this.sectionStart=-1,this.state=11,this.stateBeforeAttrName(t)}stateBeforeClosingTagName(t){ns(t)||(t===62?(this.state=1,this.sectionStart=this.index+1):(this.state=Uu(t)?9:27,this.sectionStart=this.index))}stateInClosingTagName(t){(t===62||ns(t))&&(this.cbs.onclosetag(this.sectionStart,this.index),this.sectionStart=-1,this.state=10,this.stateAfterClosingTagName(t))}stateAfterClosingTagName(t){t===62&&(this.state=1,this.sectionStart=this.index+1)}stateBeforeAttrName(t){t===62?(this.cbs.onopentagend(this.index),this.inRCDATA?this.state=32:this.state=1,this.sectionStart=this.index+1):t===47?this.state=7:t===60&&this.peek()===47?(this.cbs.onopentagend(this.index),this.state=5,this.sectionStart=this.index):ns(t)||this.handleAttrStart(t)}handleAttrStart(t){t===118&&this.peek()===45?(this.state=13,this.sectionStart=this.index):t===46||t===58||t===64||t===35?(this.cbs.ondirname(this.index,this.index+1),this.state=14,this.sectionStart=this.index+1):(this.state=12,this.sectionStart=this.index)}stateInSelfClosingTag(t){t===62?(this.cbs.onselfclosingtag(this.index),this.state=1,this.sectionStart=this.index+1,this.inRCDATA=!1):ns(t)||(this.state=11,this.stateBeforeAttrName(t))}stateInAttrName(t){(t===61||un(t))&&(this.cbs.onattribname(this.sectionStart,this.index),this.handleAttrNameEnd(t))}stateInDirName(t){t===61||un(t)?(this.cbs.ondirname(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===58?(this.cbs.ondirname(this.sectionStart,this.index),this.state=14,this.sectionStart=this.index+1):t===46&&(this.cbs.ondirname(this.sectionStart,this.index),this.state=16,this.sectionStart=this.index+1)}stateInDirArg(t){t===61||un(t)?(this.cbs.ondirarg(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===91?this.state=15:t===46&&(this.cbs.ondirarg(this.sectionStart,this.index),this.state=16,this.sectionStart=this.index+1)}stateInDynamicDirArg(t){t===93?this.state=14:(t===61||un(t))&&(this.cbs.ondirarg(this.sectionStart,this.index+1),this.handleAttrNameEnd(t))}stateInDirModifier(t){t===61||un(t)?(this.cbs.ondirmodifier(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===46&&(this.cbs.ondirmodifier(this.sectionStart,this.index),this.sectionStart=this.index+1)}handleAttrNameEnd(t){this.sectionStart=this.index,this.state=17,this.cbs.onattribnameend(this.index),this.stateAfterAttrName(t)}stateAfterAttrName(t){t===61?this.state=18:t===47||t===62?(this.cbs.onattribend(0,this.sectionStart),this.sectionStart=-1,this.state=11,this.stateBeforeAttrName(t)):ns(t)||(this.cbs.onattribend(0,this.sectionStart),this.handleAttrStart(t))}stateBeforeAttrValue(t){t===34?(this.state=19,this.sectionStart=this.index+1):t===39?(this.state=20,this.sectionStart=this.index+1):ns(t)||(this.sectionStart=this.index,this.state=21,this.stateInAttrValueNoQuotes(t))}handleInAttrValue(t,s){(t===s||this.fastForwardTo(s))&&(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=-1,this.cbs.onattribend(s===34?3:2,this.index+1),this.state=11)}stateInAttrValueDoubleQuotes(t){this.handleInAttrValue(t,34)}stateInAttrValueSingleQuotes(t){this.handleInAttrValue(t,39)}stateInAttrValueNoQuotes(t){ns(t)||t===62?(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=-1,this.cbs.onattribend(1,this.index),this.state=11,this.stateBeforeAttrName(t)):(t===39||t===60||t===61||t===96)&&this.cbs.onerr(18,this.index)}stateBeforeDeclaration(t){t===91?(this.state=26,this.sequenceIndex=0):this.state=t===45?25:23}stateInDeclaration(t){(t===62||this.fastForwardTo(62))&&(this.state=1,this.sectionStart=this.index+1)}stateInProcessingInstruction(t){(t===62||this.fastForwardTo(62))&&(this.cbs.onprocessinginstruction(this.sectionStart,this.index),this.state=1,this.sectionStart=this.index+1)}stateBeforeComment(t){t===45?(this.state=28,this.currentSequence=Dt.CommentEnd,this.sequenceIndex=2,this.sectionStart=this.index+1):this.state=23}stateInSpecialComment(t){(t===62||this.fastForwardTo(62))&&(this.cbs.oncomment(this.sectionStart,this.index),this.state=1,this.sectionStart=this.index+1)}stateBeforeSpecialS(t){t===Dt.ScriptEnd[3]?this.startSpecial(Dt.ScriptEnd,4):t===Dt.StyleEnd[3]?this.startSpecial(Dt.StyleEnd,4):(this.state=6,this.stateInTagName(t))}stateBeforeSpecialT(t){t===Dt.TitleEnd[3]?this.startSpecial(Dt.TitleEnd,4):t===Dt.TextareaEnd[3]?this.startSpecial(Dt.TextareaEnd,4):(this.state=6,this.stateInTagName(t))}startEntity(){}stateInEntity(){}parse(t){for(this.buffer=t;this.index<this.buffer.length;){const s=this.buffer.charCodeAt(this.index);switch(s===10&&this.state!==33&&this.newlines.push(this.index),this.state){case 1:{this.stateText(s);break}case 2:{this.stateInterpolationOpen(s);break}case 3:{this.stateInterpolation(s);break}case 4:{this.stateInterpolationClose(s);break}case 31:{this.stateSpecialStartSequence(s);break}case 32:{this.stateInRCDATA(s);break}case 26:{this.stateCDATASequence(s);break}case 19:{this.stateInAttrValueDoubleQuotes(s);break}case 12:{this.stateInAttrName(s);break}case 13:{this.stateInDirName(s);break}case 14:{this.stateInDirArg(s);break}case 15:{this.stateInDynamicDirArg(s);break}case 16:{this.stateInDirModifier(s);break}case 28:{this.stateInCommentLike(s);break}case 27:{this.stateInSpecialComment(s);break}case 11:{this.stateBeforeAttrName(s);break}case 6:{this.stateInTagName(s);break}case 34:{this.stateInSFCRootTagName(s);break}case 9:{this.stateInClosingTagName(s);break}case 5:{this.stateBeforeTagName(s);break}case 17:{this.stateAfterAttrName(s);break}case 20:{this.stateInAttrValueSingleQuotes(s);break}case 18:{this.stateBeforeAttrValue(s);break}case 8:{this.stateBeforeClosingTagName(s);break}case 10:{this.stateAfterClosingTagName(s);break}case 29:{this.stateBeforeSpecialS(s);break}case 30:{this.stateBeforeSpecialT(s);break}case 21:{this.stateInAttrValueNoQuotes(s);break}case 7:{this.stateInSelfClosingTag(s);break}case 23:{this.stateInDeclaration(s);break}case 22:{this.stateBeforeDeclaration(s);break}case 25:{this.stateBeforeComment(s);break}case 24:{this.stateInProcessingInstruction(s);break}case 33:{this.stateInEntity();break}}this.index++}this.cleanup(),this.finish()}cleanup(){this.sectionStart!==this.index&&(this.state===1||this.state===32&&this.sequenceIndex===0?(this.cbs.ontext(this.sectionStart,this.index),this.sectionStart=this.index):(this.state===19||this.state===20||this.state===21)&&(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=this.index))}finish(){this.handleTrailingData(),this.cbs.onend()}handleTrailingData(){const t=this.buffer.length;this.sectionStart>=t||(this.state===28?this.currentSequence===Dt.CdataEnd?this.cbs.oncdata(this.sectionStart,t):this.cbs.oncomment(this.sectionStart,t):this.state===6||this.state===11||this.state===18||this.state===17||this.state===12||this.state===13||this.state===14||this.state===15||this.state===16||this.state===20||this.state===19||this.state===21||this.state===9||this.cbs.ontext(this.sectionStart,t))}emitCodePoint(t,s){}}function Bu(e,{compatConfig:t}){const s=t&&t[e];return e==="MODE"?s||3:s}function $n(e,t){const s=Bu("MODE",t),n=Bu(e,t);return s===3?n===!0:n!==!1}function xi(e,t,s,...n){return $n(e,t)}function vc(e){throw e}function ah(e){}function at(e,t,s,n){const a=`https://vuejs.org/error-reference/#compiler-${e}`,i=new SyntaxError(String(a));return i.code=e,i.loc=t,i}const es=e=>e.type===4&&e.isStatic;function ih(e){switch(e){case"Teleport":case"teleport":return si;case"Suspense":case"suspense":return tc;case"KeepAlive":case"keep-alive":return Tl;case"BaseTransition":case"base-transition":return Yp}}const My=/^$|^\d|[^\$\w\xA0-\uFFFF]/,bc=e=>!My.test(e),lh=/[A-Za-z_$\xA0-\uFFFF]/,Py=/[\.\?\w$\xA0-\uFFFF]/,Fy=/\s+[.[]\s*|\s*[.[]\s+/g,rh=e=>e.type===4?e.content:e.loc.source,$y=e=>{const t=rh(e).trim().replace(Fy,r=>r.trim());let s=0,n=[],a=0,i=0,l=null;for(let r=0;r<t.length;r++){const o=t.charAt(r);switch(s){case 0:if(o==="[")n.push(s),s=1,a++;else if(o==="(")n.push(s),s=2,i++;else if(!(r===0?lh:Py).test(o))return!1;break;case 1:o==="'"||o==='"'||o==="`"?(n.push(s),s=3,l=o):o==="["?a++:o==="]"&&(--a||(s=n.pop()));break;case 2:if(o==="'"||o==='"'||o==="`")n.push(s),s=3,l=o;else if(o==="(")i++;else if(o===")"){if(r===t.length-1)return!1;--i||(s=n.pop())}break;case 3:o===l&&(s=n.pop(),l=null);break}}return!a&&!i},oh=$y,Uy=/^\s*(?:async\s*)?(?:\([^)]*?\)|[\w$_]+)\s*(?::[^=]+)?=>|^\s*(?:async\s+)?function(?:\s+[\w$]+)?\s*\(/,By=e=>Uy.test(rh(e)),Hy=By;function ds(e,t,s=!1){for(let n=0;n<e.props.length;n++){const a=e.props[n];if(a.type===7&&(s||a.exp)&&(Ee(t)?a.name===t:t.test(a.name)))return a}}function nr(e,t,s=!1,n=!1){for(let a=0;a<e.props.length;a++){const i=e.props[a];if(i.type===6){if(s)continue;if(i.name===t&&(i.value||n))return i}else if(i.name==="bind"&&(i.exp||n)&&Nn(i.arg,t))return i}}function Nn(e,t){return!!(e&&es(e)&&e.content===t)}function Vy(e){return e.props.some(t=>t.type===7&&t.name==="bind"&&(!t.arg||t.arg.type!==4||!t.arg.isStatic))}function Er(e){return e.type===5||e.type===2}function Hu(e){return e.type===7&&e.name==="pre"}function jy(e){return e.type===7&&e.name==="slot"}function Rl(e){return e.type===1&&e.tagType===3}function Il(e){return e.type===1&&e.tagType===2}const zy=new Set([bi,Oi]);function ch(e,t=[]){if(e&&!Ee(e)&&e.type===14){const s=e.callee;if(!Ee(s)&&zy.has(s))return ch(e.arguments[0],t.concat(e))}return[e,t]}function Nl(e,t,s){let n,a=e.type===13?e.props:e.arguments[2],i=[],l;if(a&&!Ee(a)&&a.type===14){const r=ch(a);a=r[0],i=r[1],l=i[i.length-1]}if(a==null||Ee(a))n=fs([t]);else if(a.type===14){const r=a.arguments[0];!Ee(r)&&r.type===15?Vu(t,r)||r.properties.unshift(t):a.callee===pc?n=St(s.helper(Cl),[fs([t]),a]):a.arguments.unshift(fs([t])),!n&&(n=a)}else a.type===15?(Vu(t,a)||a.properties.unshift(t),n=a):(n=St(s.helper(Cl),[fs([t]),a]),l&&l.callee===Oi&&(l=i[i.length-2]));e.type===13?l?l.arguments[0]=n:e.props=n:l?l.arguments[0]=n:e.arguments[2]=n}function Vu(e,t){let s=!1;if(e.key.type===4){const n=e.key.content;s=t.properties.some(a=>a.key.type===4&&a.key.content===n)}return s}function _i(e,t){return`_${t}_${e.replace(/[^\w]/g,(s,n)=>s==="-"?"_":e.charCodeAt(n).toString())}`}function Ky(e){return e.type===14&&e.callee===mc?e.arguments[1].returns:e}const qy=/([\s\S]*?)\s+(?:in|of)\s+(\S[\s\S]*)/;function uh(e){for(let t=0;t<e.length;t++)if(!ns(e.charCodeAt(t)))return!1;return!0}function yc(e){return e.type===2&&uh(e.content)||e.type===12&&yc(e.content)}function dh(e){return e.type===3||yc(e)}const fh={parseMode:"base",ns:0,delimiters:["{{","}}"],getNamespace:()=>0,isVoidTag:oa,isPreTag:oa,isIgnoreNewlineTag:oa,isCustomElement:oa,onError:vc,onWarn:ah,comments:!1,prefixIdentifiers:!1};let Ve=fh,ki=null,Qs="",Pt=null,$e=null,Zt="",Bs=-1,Rn=-1,xc=0,gn=!1,co=null;const nt=[],ut=new Dy(nt,{onerr:Fs,ontext(e,t){Wi(It(e,t),e,t)},ontextentity(e,t,s){Wi(e,t,s)},oninterpolation(e,t){if(gn)return Wi(It(e,t),e,t);let s=e+ut.delimiterOpen.length,n=t-ut.delimiterClose.length;for(;ns(Qs.charCodeAt(s));)s++;for(;ns(Qs.charCodeAt(n-1));)n--;let a=It(s,n);a.includes("&")&&(a=Ve.decodeEntities(a,!1)),uo({type:5,content:nl(a,!1,ft(s,n)),loc:ft(e,t)})},onopentagname(e,t){const s=It(e,t);Pt={type:1,tag:s,ns:Ve.getNamespace(s,nt[0],Ve.ns),tagType:0,props:[],children:[],loc:ft(e-1,t),codegenNode:void 0}},onopentagend(e){zu(e)},onclosetag(e,t){const s=It(e,t);if(!Ve.isVoidTag(s)){let n=!1;for(let a=0;a<nt.length;a++)if(nt[a].tag.toLowerCase()===s.toLowerCase()){n=!0,a>0&&Fs(24,nt[0].loc.start.offset);for(let l=0;l<=a;l++){const r=nt.shift();sl(r,t,l<a)}break}n||Fs(23,ph(e,60))}},onselfclosingtag(e){const t=Pt.tag;Pt.isSelfClosing=!0,zu(e),nt[0]&&nt[0].tag===t&&sl(nt.shift(),e)},onattribname(e,t){$e={type:6,name:It(e,t),nameLoc:ft(e,t),value:void 0,loc:ft(e)}},ondirname(e,t){const s=It(e,t),n=s==="."||s===":"?"bind":s==="@"?"on":s==="#"?"slot":s.slice(2);if(!gn&&n===""&&Fs(26,e),gn||n==="")$e={type:6,name:s,nameLoc:ft(e,t),value:void 0,loc:ft(e)};else if($e={type:7,name:n,rawName:s,exp:void 0,arg:void 0,modifiers:s==="."?[Ne("prop")]:[],loc:ft(e)},n==="pre"){gn=ut.inVPre=!0,co=Pt;const a=Pt.props;for(let i=0;i<a.length;i++)a[i].type===7&&(a[i]=s0(a[i]))}},ondirarg(e,t){if(e===t)return;const s=It(e,t);if(gn&&!Hu($e))$e.name+=s,On($e.nameLoc,t);else{const n=s[0]!=="[";$e.arg=nl(n?s:s.slice(1,-1),n,ft(e,t),n?3:0)}},ondirmodifier(e,t){const s=It(e,t);if(gn&&!Hu($e))$e.name+="."+s,On($e.nameLoc,t);else if($e.name==="slot"){const n=$e.arg;n&&(n.content+="."+s,On(n.loc,t))}else{const n=Ne(s,!0,ft(e,t));$e.modifiers.push(n)}},onattribdata(e,t){Zt+=It(e,t),Bs<0&&(Bs=e),Rn=t},onattribentity(e,t,s){Zt+=e,Bs<0&&(Bs=t),Rn=s},onattribnameend(e){const t=$e.loc.start.offset,s=It(t,e);$e.type===7&&($e.rawName=s),Pt.props.some(n=>(n.type===7?n.rawName:n.name)===s)&&Fs(2,t)},onattribend(e,t){if(Pt&&$e){if(On($e.loc,t),e!==0)if(Zt.includes("&")&&(Zt=Ve.decodeEntities(Zt,!0)),$e.type===6)$e.name==="class"&&(Zt=mh(Zt).trim()),e===1&&!Zt&&Fs(13,t),$e.value={type:2,content:Zt,loc:e===1?ft(Bs,Rn):ft(Bs-1,Rn+1)},ut.inSFCRoot&&Pt.tag==="template"&&$e.name==="lang"&&Zt&&Zt!=="html"&&ut.enterRCDATA(Al("</template"),0);else{let s=0;$e.exp=nl(Zt,!1,ft(Bs,Rn),0,s),$e.name==="for"&&($e.forParseResult=Wy($e.exp));let n=-1;$e.name==="bind"&&(n=$e.modifiers.findIndex(a=>a.content==="sync"))>-1&&xi("COMPILER_V_BIND_SYNC",Ve,$e.loc,$e.arg.loc.source)&&($e.name="model",$e.modifiers.splice(n,1))}($e.type!==7||$e.name!=="pre")&&Pt.props.push($e)}Zt="",Bs=Rn=-1},oncomment(e,t){Ve.comments&&uo({type:3,content:It(e,t),loc:ft(e-4,t+3)})},onend(){const e=Qs.length;for(let t=0;t<nt.length;t++)sl(nt[t],e-1),Fs(24,nt[t].loc.start.offset)},oncdata(e,t){(nt[0]?nt[0].ns:Ve.ns)!==0?Wi(It(e,t),e,t):Fs(1,e-9)},onprocessinginstruction(e){(nt[0]?nt[0].ns:Ve.ns)===0&&Fs(21,e-1)}}),ju=/,([^,\}\]]*)(?:,([^,\}\]]*))?$/,Gy=/^\(|\)$/g;function Wy(e){const t=e.loc,s=e.content,n=s.match(qy);if(!n)return;const[,a,i]=n,l=(d,f,p=!1)=>{const b=t.start.offset+f,m=b+d.length;return nl(d,!1,ft(b,m),0,p?1:0)},r={source:l(i.trim(),s.indexOf(i,a.length)),value:void 0,key:void 0,index:void 0,finalized:!1};let o=a.trim().replace(Gy,"").trim();const c=a.indexOf(o),u=o.match(ju);if(u){o=o.replace(ju,"").trim();const d=u[1].trim();let f;if(d&&(f=s.indexOf(d,c+o.length),r.key=l(d,f,!0)),u[2]){const p=u[2].trim();p&&(r.index=l(p,s.indexOf(p,r.key?f+d.length:c+o.length),!0))}}return o&&(r.value=l(o,c,!0)),r}function It(e,t){return Qs.slice(e,t)}function zu(e){ut.inSFCRoot&&(Pt.innerLoc=ft(e+1,e+1)),uo(Pt);const{tag:t,ns:s}=Pt;s===0&&Ve.isPreTag(t)&&xc++,Ve.isVoidTag(t)?sl(Pt,e):(nt.unshift(Pt),(s===1||s===2)&&(ut.inXML=!0)),Pt=null}function Wi(e,t,s){{const i=nt[0]&&nt[0].tag;i!=="script"&&i!=="style"&&e.includes("&")&&(e=Ve.decodeEntities(e,!1))}const n=nt[0]||ki,a=n.children[n.children.length-1];a&&a.type===2?(a.content+=e,On(a.loc,s)):n.children.push({type:2,content:e,loc:ft(t,s)})}function sl(e,t,s=!1){s?On(e.loc,ph(t,60)):On(e.loc,Zy(t,62)+1),ut.inSFCRoot&&(e.children.length?e.innerLoc.end=Pe({},e.children[e.children.length-1].loc.end):e.innerLoc.end=Pe({},e.innerLoc.start),e.innerLoc.source=It(e.innerLoc.start.offset,e.innerLoc.end.offset));const{tag:n,ns:a,children:i}=e;if(gn||(n==="slot"?e.tagType=2:Ku(e)?e.tagType=3:Yy(e)&&(e.tagType=1)),ut.inRCDATA||(e.children=hh(i)),a===0&&Ve.isIgnoreNewlineTag(n)){const l=i[0];l&&l.type===2&&(l.content=l.content.replace(/^\r?\n/,""))}a===0&&Ve.isPreTag(n)&&xc--,co===e&&(gn=ut.inVPre=!1,co=null),ut.inXML&&(nt[0]?nt[0].ns:Ve.ns)===0&&(ut.inXML=!1);{const l=e.props;if(!ut.inSFCRoot&&$n("COMPILER_NATIVE_TEMPLATE",Ve)&&e.tag==="template"&&!Ku(e)){const o=nt[0]||ki,c=o.children.indexOf(e);o.children.splice(c,1,...e.children)}const r=l.find(o=>o.type===6&&o.name==="inline-template");r&&xi("COMPILER_INLINE_TEMPLATE",Ve,r.loc)&&e.children.length&&(r.value={type:2,content:It(e.children[0].loc.start.offset,e.children[e.children.length-1].loc.end.offset),loc:r.loc})}}function Zy(e,t){let s=e;for(;Qs.charCodeAt(s)!==t&&s<Qs.length-1;)s++;return s}function ph(e,t){let s=e;for(;Qs.charCodeAt(s)!==t&&s>=0;)s--;return s}const Jy=new Set(["if","else","else-if","for","slot"]);function Ku({tag:e,props:t}){if(e==="template"){for(let s=0;s<t.length;s++)if(t[s].type===7&&Jy.has(t[s].name))return!0}return!1}function Yy({tag:e,props:t}){if(Ve.isCustomElement(e))return!1;if(e==="component"||Qy(e.charCodeAt(0))||ih(e)||Ve.isBuiltInComponent&&Ve.isBuiltInComponent(e)||Ve.isNativeTag&&!Ve.isNativeTag(e))return!0;for(let s=0;s<t.length;s++){const n=t[s];if(n.type===6){if(n.name==="is"&&n.value){if(n.value.content.startsWith("vue:"))return!0;if(xi("COMPILER_IS_ON_ELEMENT",Ve,n.loc))return!0}}else if(n.name==="bind"&&Nn(n.arg,"is")&&xi("COMPILER_IS_ON_ELEMENT",Ve,n.loc))return!0}return!1}function Qy(e){return e>64&&e<91}const Xy=/\r\n/g;function hh(e){const t=Ve.whitespace!=="preserve";let s=!1;for(let n=0;n<e.length;n++){const a=e[n];if(a.type===2)if(xc)a.content=a.content.replace(Xy,`
`);else if(uh(a.content)){const i=e[n-1]&&e[n-1].type,l=e[n+1]&&e[n+1].type;!i||!l||t&&(i===3&&(l===3||l===1)||i===1&&(l===3||l===1&&e0(a.content)))?(s=!0,e[n]=null):a.content=" "}else t&&(a.content=mh(a.content))}return s?e.filter(Boolean):e}function e0(e){for(let t=0;t<e.length;t++){const s=e.charCodeAt(t);if(s===10||s===13)return!0}return!1}function mh(e){let t="",s=!1;for(let n=0;n<e.length;n++)ns(e.charCodeAt(n))?s||(t+=" ",s=!0):(t+=e[n],s=!1);return t}function uo(e){(nt[0]||ki).children.push(e)}function ft(e,t){return{start:ut.getPos(e),end:t==null?t:ut.getPos(t),source:t==null?t:It(e,t)}}function t0(e){return ft(e.start.offset,e.end.offset)}function On(e,t){e.end=ut.getPos(t),e.source=It(e.start.offset,t)}function s0(e){const t={type:6,name:e.rawName,nameLoc:ft(e.loc.start.offset,e.loc.start.offset+e.rawName.length),value:void 0,loc:e.loc};if(e.exp){const s=e.exp.loc;s.end.offset<e.loc.end.offset&&(s.start.offset--,s.start.column--,s.end.offset++,s.end.column++),t.value={type:2,content:e.exp.content,loc:s}}return t}function nl(e,t=!1,s,n=0,a=0){return Ne(e,t,s,n)}function Fs(e,t,s){Ve.onError(at(e,ft(t,t)))}function n0(){ut.reset(),Pt=null,$e=null,Zt="",Bs=-1,Rn=-1,nt.length=0}function a0(e,t){if(n0(),Qs=e,Ve=Pe({},fh),t){let a;for(a in t)t[a]!=null&&(Ve[a]=t[a])}ut.mode=Ve.parseMode==="html"?1:Ve.parseMode==="sfc"?2:0,ut.inXML=Ve.ns===1||Ve.ns===2;const s=t&&t.delimiters;s&&(ut.delimiterOpen=Al(s[0]),ut.delimiterClose=Al(s[1]));const n=ki=Ny([],e);return ut.parse(Qs),n.loc=ft(0,e.length),n.children=hh(n.children),ki=null,n}function i0(e,t){al(e,void 0,t,!!gh(e))}function gh(e){const t=e.children.filter(s=>s.type!==3);return t.length===1&&t[0].type===1&&!Il(t[0])?t[0]:null}function al(e,t,s,n=!1,a=!1){const{children:i}=e,l=[];for(let u=0;u<i.length;u++){const d=i[u];if(d.type===1&&d.tagType===0){const f=n?0:as(d,s);if(f>0){if(f>=2){d.codegenNode.patchFlag=-1,l.push(d);continue}}else{const p=d.codegenNode;if(p.type===13){const b=p.patchFlag;if((b===void 0||b===512||b===1)&&bh(d,s)>=2){const m=yh(d);m&&(p.props=s.hoist(m))}p.dynamicProps&&(p.dynamicProps=s.hoist(p.dynamicProps))}}}else if(d.type===12&&(n?0:as(d,s))>=2){d.codegenNode.type===14&&d.codegenNode.arguments.length>0&&d.codegenNode.arguments.push("-1"),l.push(d);continue}if(d.type===1){const f=d.tagType===1;f&&s.scopes.vSlot++,al(d,e,s,!1,a),f&&s.scopes.vSlot--}else if(d.type===11)al(d,e,s,d.children.length===1,!0);else if(d.type===9)for(let f=0;f<d.branches.length;f++)al(d.branches[f],e,s,d.branches[f].children.length===1,a)}let r=!1;if(l.length===i.length&&e.type===1){if(e.tagType===0&&e.codegenNode&&e.codegenNode.type===13&&he(e.codegenNode.children))e.codegenNode.children=o(Fn(e.codegenNode.children)),r=!0;else if(e.tagType===1&&e.codegenNode&&e.codegenNode.type===13&&e.codegenNode.children&&!he(e.codegenNode.children)&&e.codegenNode.children.type===15){const u=c(e.codegenNode,"default");u&&(u.returns=o(Fn(u.returns)),r=!0)}else if(e.tagType===3&&t&&t.type===1&&t.tagType===1&&t.codegenNode&&t.codegenNode.type===13&&t.codegenNode.children&&!he(t.codegenNode.children)&&t.codegenNode.children.type===15){const u=ds(e,"slot",!0),d=u&&u.arg&&c(t.codegenNode,u.arg);d&&(d.returns=o(Fn(d.returns)),r=!0)}}if(!r)for(const u of l)u.codegenNode=s.cache(u.codegenNode);function o(u){const d=s.cache(u);return d.needArraySpread=!0,d}function c(u,d){if(u.children&&!he(u.children)&&u.children.type===15){const f=u.children.properties.find(p=>p.key===d||p.key.content===d);return f&&f.value}}l.length&&s.transformHoist&&s.transformHoist(i,s,e)}function as(e,t){const{constantCache:s}=t;switch(e.type){case 1:if(e.tagType!==0)return 0;const n=s.get(e);if(n!==void 0)return n;const a=e.codegenNode;if(a.type!==13||a.isBlock&&e.tag!=="svg"&&e.tag!=="foreignObject"&&e.tag!=="math")return 0;if(a.patchFlag===void 0){let l=3;const r=bh(e,t);if(r===0)return s.set(e,0),0;r<l&&(l=r);for(let o=0;o<e.children.length;o++){const c=as(e.children[o],t);if(c===0)return s.set(e,0),0;c<l&&(l=c)}if(l>1)for(let o=0;o<e.props.length;o++){const c=e.props[o];if(c.type===7&&c.name==="bind"&&c.exp){const u=as(c.exp,t);if(u===0)return s.set(e,0),0;u<l&&(l=u)}}if(a.isBlock){for(let o=0;o<e.props.length;o++)if(e.props[o].type===7)return s.set(e,0),0;t.removeHelper(Vn),t.removeHelper(Ea(t.inSSR,a.isComponent)),a.isBlock=!1,t.helper(Ca(t.inSSR,a.isComponent))}return s.set(e,l),l}else return s.set(e,0),0;case 2:case 3:return 3;case 9:case 11:case 10:return 0;case 5:case 12:return as(e.content,t);case 4:return e.constType;case 8:let i=3;for(let l=0;l<e.children.length;l++){const r=e.children[l];if(Ee(r)||Bt(r))continue;const o=as(r,t);if(o===0)return 0;o<i&&(i=o)}return i;case 20:return 2;default:return 0}}const l0=new Set([dc,fc,bi,Oi]);function vh(e,t){if(e.type===14&&!Ee(e.callee)&&l0.has(e.callee)){const s=e.arguments[0];if(s.type===4)return as(s,t);if(s.type===14)return vh(s,t)}return 0}function bh(e,t){let s=3;const n=yh(e);if(n&&n.type===15){const{properties:a}=n;for(let i=0;i<a.length;i++){const{key:l,value:r}=a[i],o=as(l,t);if(o===0)return o;o<s&&(s=o);let c;if(r.type===4?c=as(r,t):r.type===14?c=vh(r,t):c=0,c===0)return c;c<s&&(s=c)}}return s}function yh(e){const t=e.codegenNode;if(t.type===13)return t.props}function r0(e,{filename:t="",prefixIdentifiers:s=!1,hoistStatic:n=!1,hmr:a=!1,cacheHandlers:i=!1,nodeTransforms:l=[],directiveTransforms:r={},transformHoist:o=null,isBuiltInComponent:c=Lt,isCustomElement:u=Lt,expressionPlugins:d=[],scopeId:f=null,slotted:p=!0,ssr:b=!1,inSSR:m=!1,ssrCssVars:k="",bindingMetadata:E=Fe,inline:v=!1,isTS:y=!1,onError:x=vc,onWarn:T=ah,compatConfig:N}){const O=t.replace(/\?.*$/,"").match(/([^/\\]+)\.\w+$/),w={filename:t,selfName:O&&qn(Qe(O[1])),prefixIdentifiers:s,hoistStatic:n,hmr:a,cacheHandlers:i,nodeTransforms:l,directiveTransforms:r,transformHoist:o,isBuiltInComponent:c,isCustomElement:u,expressionPlugins:d,scopeId:f,slotted:p,ssr:b,inSSR:m,ssrCssVars:k,bindingMetadata:E,inline:v,isTS:y,onError:x,onWarn:T,compatConfig:N,root:e,helpers:new Map,components:new Set,directives:new Set,hoists:[],imports:[],cached:[],constantCache:new WeakMap,vForMemoKeyedNodes:new WeakSet,temps:0,identifiers:Object.create(null),scopes:{vFor:0,vSlot:0,vPre:0,vOnce:0},parent:null,grandParent:null,currentNode:e,childIndex:0,inVOnce:!1,helper(R){const L=w.helpers.get(R)||0;return w.helpers.set(R,L+1),R},removeHelper(R){const L=w.helpers.get(R);if(L){const U=L-1;U?w.helpers.set(R,U):w.helpers.delete(R)}},helperString(R){return`_${Sa[w.helper(R)]}`},replaceNode(R){w.parent.children[w.childIndex]=w.currentNode=R},removeNode(R){const L=w.parent.children,U=R?L.indexOf(R):w.currentNode?w.childIndex:-1;!R||R===w.currentNode?(w.currentNode=null,w.onNodeRemoved()):w.childIndex>U&&(w.childIndex--,w.onNodeRemoved()),w.parent.children.splice(U,1)},onNodeRemoved:Lt,addIdentifiers(R){},removeIdentifiers(R){},hoist(R){Ee(R)&&(R=Ne(R)),w.hoists.push(R);const L=Ne(`_hoisted_${w.hoists.length}`,!1,R.loc,2);return L.hoisted=R,L},cache(R,L=!1,U=!1){const P=Oy(w.cached.length,R,L,U);return w.cached.push(P),P}};return w.filters=new Set,w}function o0(e,t){const s=r0(e,t);ar(e,s),t.hoistStatic&&i0(e,s),t.ssr||c0(e,s),e.helpers=new Set([...s.helpers.keys()]),e.components=[...s.components],e.directives=[...s.directives],e.imports=s.imports,e.hoists=s.hoists,e.temps=s.temps,e.cached=s.cached,e.transformed=!0,e.filters=[...s.filters]}function c0(e,t){const{helper:s}=t,{children:n}=e;if(n.length===1){const a=gh(e);if(a&&a.codegenNode){const i=a.codegenNode;i.type===13&&gc(i,t),e.codegenNode=i}else e.codegenNode=n[0]}else if(n.length>1){let a=64;e.codegenNode=yi(t,s(vi),void 0,e.children,a,void 0,void 0,!0,void 0,!1)}}function u0(e,t){let s=0;const n=()=>{s--};for(;s<e.children.length;s++){const a=e.children[s];Ee(a)||(t.grandParent=t.parent,t.parent=e,t.childIndex=s,t.onNodeRemoved=n,ar(a,t))}}function ar(e,t){t.currentNode=e;const{nodeTransforms:s}=t,n=[];for(let i=0;i<s.length;i++){const l=s[i](e,t);if(l&&(he(l)?n.push(...l):n.push(l)),t.currentNode)e=t.currentNode;else return}switch(e.type){case 3:t.ssr||t.helper(Ni);break;case 5:t.ssr||t.helper(sr);break;case 9:for(let i=0;i<e.branches.length;i++)ar(e.branches[i],t);break;case 10:case 11:case 1:case 0:u0(e,t);break}t.currentNode=e;let a=n.length;for(;a--;)n[a]()}function xh(e,t){const s=Ee(e)?n=>n===e:n=>e.test(n);return(n,a)=>{if(n.type===1){const{props:i}=n;if(n.tagType===3&&i.some(jy))return;const l=[];for(let r=0;r<i.length;r++){const o=i[r];if(o.type===7&&s(o.name)){i.splice(r,1),r--;const c=t(n,o,a);c&&l.push(c)}}return l}}}const ir="/*@__PURE__*/",_h=e=>`${Sa[e]}: _${Sa[e]}`;function d0(e,{mode:t="function",prefixIdentifiers:s=t==="module",sourceMap:n=!1,filename:a="template.vue.html",scopeId:i=null,optimizeImports:l=!1,runtimeGlobalName:r="Vue",runtimeModuleName:o="vue",ssrRuntimeModuleName:c="vue/server-renderer",ssr:u=!1,isTS:d=!1,inSSR:f=!1}){const p={mode:t,prefixIdentifiers:s,sourceMap:n,filename:a,scopeId:i,optimizeImports:l,runtimeGlobalName:r,runtimeModuleName:o,ssrRuntimeModuleName:c,ssr:u,isTS:d,inSSR:f,source:e.source,code:"",column:1,line:1,offset:0,indentLevel:0,pure:!1,map:void 0,helper(m){return`_${Sa[m]}`},push(m,k=-2,E){p.code+=m},indent(){b(++p.indentLevel)},deindent(m=!1){m?--p.indentLevel:b(--p.indentLevel)},newline(){b(p.indentLevel)}};function b(m){p.push(`
...
`,-1),o())),e.components.length&&(Ar(e.components,"component",s),(e.directives.length||e.temps>0)&&o()),e.directives.length&&(Ar(e.directives,"directive",s),e.temps>0&&o()),e.filters&&e.filters.length&&(o(),Ar(e.filters,"filter",s),o()),e.temps>0){a("let ");for(let v=0;v<e.temps;v++)a(`${v>0?", ":""}_temp${v}`)}return(e.components.length||e.directives.length||e.temps)&&(a(`
...
`,-1)}h0(e.hoists,t),i(),a("return ")}function Ar(e,t,{helper:s,push:n,newline:a,isTS:i}){const l=s(t==="filter"?oc:t==="component"?ic:rc);for(let r=0;r<e.length;r++){let o=e[r];const c=o.endsWith("__self");c&&(o=o.slice(0,-6)),n(`const ${_i(o,t)} = ${l}(${JSON.stringify(o)}${c?", true":""})${i?"!":""}`),r<e.length-1&&a()}}function h0(e,t){if(!e.length)return;t.pure=!0;const{push:s,newline:n}=t;n();for(let a=0;a<e.length;a++){const i=e[a];i&&(s(`const _hoisted_${a+1} = `),Ut(i,t),n())}t.pure=!1}function _c(e,t){const s=e.length>3||!1;t.push("["),s&&t.indent(),Li(e,t,s),s&&t.deindent(),t.push("]")}function Li(e,t,s=!1,n=!0){const{push:a,newline:i}=t;for(let l=0;l<e.length;l++){const r=e[l];Ee(r)?a(r,-3):he(r)?_c(r,t):Ut(r,t),l<e.length-1&&(s?(n&&a(","),i()):n&&a(", "))}}function Ut(e,t){if(Ee(e)){t.push(e,-3);return}if(Bt(e)){t.push(t.helper(e));return}switch(e.type){case 1:case 9:case 11:Ut(e.codegenNode,t);break;case 2:m0(e,t);break;case 4:kh(e,t);break;case 5:g0(e,t);break;case 12:Ut(e.codegenNode,t);break;case 8:wh(e,t);break;case 3:b0(e,t);break;case 13:y0(e,t);break;case 14:_0(e,t);break;case 15:k0(e,t);break;case 17:w0(e,t);break;case 18:S0(e,t);break;case 19:T0(e,t);break;case 20:C0(e,t);break;case 21:Li(e.body,t,!0,!1);break}}function m0(e,t){t.push(JSON.stringify(e.content),-3,e)}function kh(e,t){const{content:s,isStatic:n}=e;t.push(n?JSON.stringify(s):s,-3,e)}function g0(e,t){const{push:s,helper:n,pure:a}=t;a&&s(ir),s(`${n(sr)}(`),Ut(e.content,t),s(")")}function wh(e,t){for(let s=0;s<e.children.length;s++){const n=e.children[s];Ee(n)?t.push(n,-3):Ut(n,t)}}function v0(e,t){const{push:s}=t;if(e.type===8)s("["),wh(e,t),s("]");else if(e.isStatic){const n=bc(e.content)?e.content:JSON.stringify(e.content);s(n,-2,e)}else s(`[${e.content}]`,-3,e)}function b0(e,t){const{push:s,helper:n,pure:a}=t;a&&s(ir),s(`${n(Ni)}(${JSON.stringify(e.content)})`,-3,e)}function y0(e,t){const{push:s,helper:n,pure:a}=t,{tag:i,props:l,children:r,patchFlag:o,dynamicProps:c,directives:u,isBlock:d,disableTracking:f,isComponent:p}=e;let b;o&&(b=String(o)),u&&s(n(cc)+"("),d&&s(`(${n(Vn)}(${f?"true":""}), `),a&&s(ir);const m=d?Ea(t.inSSR,p):Ca(t.inSSR,p);s(n(m)+"(",-2,e),Li(x0([i,l,r,b,c]),t),s(")"),d&&s(")"),u&&(s(", "),Ut(u,t),s(")"))}function x0(e){let t=e.length;for(;t--&&e[t]==null;);return e.slice(0,t+1).map(s=>s||"null")}function _0(e,t){const{push:s,helper:n,pure:a}=t,i=Ee(e.callee)?e.callee:n(e.callee);a&&s(ir),s(i+"(",-2,e),Li(e.arguments,t),s(")")}function k0(e,t){const{push:s,indent:n,deindent:a,newline:i}=t,{properties:l}=e;if(!l.length){s("{}",-2,e);return}const r=l.length>1||!1;s(r?"{":"{ "),r&&n();for(let o=0;o<l.length;o++){const{key:c,value:u}=l[o];v0(c,t),s(": "),Ut(u,t),o<l.length-1&&(s(","),i())}r&&a(),s(r?"}":" }")}function w0(e,t){_c(e.elements,t)}function S0(e,t){const{push:s,indent:n,deindent:a}=t,{params:i,returns:l,body:r,newline:o,isSlot:c}=e;c&&s(`_${Sa[hc]}(`),s("(",-2,e),he(i)?Li(i,t):i&&Ut(i,t),s(") => "),(o||r)&&(s("{"),n()),l?(o&&s("return "),he(l)?_c(l,t):Ut(l,t)):r&&Ut(r,t),(o||r)&&(a(),s("}")),c&&(e.isNonScopedSlot&&s(", undefined, true"),s(")"))}function T0(e,t){const{test:s,consequent:n,alternate:a,newline:i}=e,{push:l,indent:r,deindent:o,newline:c}=t;if(s.type===4){const d=!bc(s.content);d&&l("("),kh(s,t),d&&l(")")}else l("("),Ut(s,t),l(")");i&&r(),t.indentLevel++,i||l(" "),l("? "),Ut(n,t),t.indentLevel--,i&&c(),i||l(" "),l(": ");const u=a.type===19;u||t.indentLevel++,Ut(a,t),u||t.indentLevel--,i&&o(!0)}function C0(e,t){const{push:s,helper:n,indent:a,deindent:i,newline:l}=t,{needPauseTracking:r,needArraySpread:o}=e;o&&s("[...("),s(`_cache[${e.index}] || (`),r&&(a(),s(`${n(El)}(-1`),e.inVOnce&&s(", true"),s("),"),l(),s("(")),s(`_cache[${e.index}] = `),Ut(e.value,t),r&&(s(`).cacheIndex = ${e.index},`),l(),s(`${n(El)}(1),`),l(),s(`_cache[${e.index}]`),i()),s(")"),o&&s(")]")}new RegExp("\\b"+"arguments,await,break,case,catch,class,const,continue,debugger,default,delete,do,else,export,extends,finally,for,function,if,import,let,new,return,super,switch,throw,try,var,void,while,with,yield".split(",").join("\\b|\\b")+"\\b");const E0=xh(/^(?:if|else|else-if)$/,(e,t,s)=>A0(e,t,s,(n,a,i)=>{const l=s.parent.children;let r=l.indexOf(n),o=0;for(;r-->=0;){const c=l[r];c&&c.type===9&&(o+=c.branches.length)}return()=>{if(i)n.codegenNode=Gu(a,o,s);else{const c=R0(n.codegenNode);c.alternate=Gu(a,o+n.branches.length-1,s)}}}));function A0(e,t,s,n){if(t.name!=="else"&&(!t.exp||!t.exp.content.trim())){const a=t.exp?t.exp.loc:e.loc;s.onError(at(28,t.loc)),t.exp=Ne("true",!1,a)}if(t.name==="if"){const a=qu(e,t),i={type:9,loc:t0(e.loc),branches:[a]};if(s.replaceNode(i),n)return n(i,a,!0)}else{const a=s.parent.children;let i=a.indexOf(e);for(;i-->=-1;){const l=a[i];if(l&&dh(l)){s.removeNode(l);continue}if(l&&l.type===9){(t.name==="else-if"||t.name==="else")&&l.branches[l.branches.length-1].condition===void 0&&s.onError(at(30,e.loc)),s.removeNode();const r=qu(e,t);l.branches.push(r);const o=n&&n(l,r,!1);ar(r,s),o&&o(),s.currentNode=null}else s.onError(at(30,e.loc));break}}}function qu(e,t){const s=e.tagType===3;return{type:10,loc:e.loc,condition:t.name==="else"?void 0:t.exp,children:s&&!ds(e,"for")?e.children:[e],userKey:nr(e,"key"),isTemplateIf:s}}function Gu(e,t,s){return e.condition?oo(e.condition,Wu(e,t,s),St(s.helper(Ni),['""',"true"])):Wu(e,t,s)}function Wu(e,t,s){const{helper:n}=s,a=vt("key",Ne(`${t}`,!1,os,2)),{children:i}=e,l=i[0];if(i.length!==1||l.type!==1)if(i.length===1&&l.type===11){const o=l.codegenNode;return Nl(o,a,s),o}else return yi(s,n(vi),fs([a]),i,64,void 0,void 0,!0,!1,!1,e.loc);else{const o=l.codegenNode,c=Ky(o);return c.type===13&&gc(c,s),Nl(c,a,s),o}}function R0(e){for(;;)if(e.type===19)if(e.alternate.type===19)e=e.alternate;else return e;else e.type===20&&(e=e.value)}const I0=xh("for",(e,t,s)=>{const{helper:n,removeHelper:a}=s;return N0(e,t,s,i=>{const l=St(n(uc),[i.source]),r=Rl(e),o=ds(e,"memo"),c=nr(e,"key",!1,!0);c&&c.type;let u=c&&(c.type===6?c.value?Ne(c.value.content,!0):void 0:c.exp);const d=u?vt("key",u):null,f=i.source.type===4&&i.source.constType>0,p=f?64:c?128:256;return i.codegenNode=yi(s,n(vi),void 0,l,p,void 0,void 0,!0,!f,!1,e.loc),()=>{let b;const{children:m}=i,k=m.length!==1||m[0].type!==1,E=Il(e)?e:r&&e.children.length===1&&Il(e.children[0])?e.children[0]:null;if(E?(b=E.codegenNode,r&&d&&Nl(b,d,s)):k?b=yi(s,n(vi),d?fs([d]):void 0,e.children,64,void 0,void 0,!0,void 0,!1):(b=m[0].codegenNode,r&&d&&Nl(b,d,s),b.isBlock!==!f&&(b.isBlock?(a(Vn),a(Ea(s.inSSR,b.isComponent))):a(Ca(s.inSSR,b.isComponent))),b.isBlock=!f,b.isBlock?(n(Vn),n(Ea(s.inSSR,b.isComponent))):n(Ca(s.inSSR,b.isComponent))),o){const v=Ta(fo(i.parseResult,[Ne("_cached")]));v.body=Ly([xs(["const _memo = (",o.exp,")"]),xs(["if (_cached && _cached.el",...u?[" && _cached.key === ",u]:[],` && ${s.helperString(nh)}(_cached, _memo)) return _cached`]),xs(["const _item = ",b]),Ne("_item.memo = _memo"),Ne("return _item")]),l.arguments.push(v,Ne("_cache"),Ne(String(s.cached.length))),s.cached.push(null)}else l.arguments.push(Ta(fo(i.parseResult),b,!0))}})});function N0(e,t,s,n){if(!t.exp){s.onError(at(31,t.loc));return}const a=t.forParseResult;if(!a){s.onError(at(32,t.loc));return}Sh(a);const{addIdentifiers:i,removeIdentifiers:l,scopes:r}=s,{source:o,value:c,key:u,index:d}=a,f={type:11,loc:t.loc,source:o,valueAlias:c,keyAlias:u,objectIndexAlias:d,parseResult:a,children:Rl(e)?e.children:[e]};s.replaceNode(f),r.vFor++;const p=n&&n(f);return()=>{r.vFor--,p&&p()}}function Sh(e,t){e.finalized||(e.finalized=!0)}function fo({value:e,key:t,index:s},n=[]){return O0([e,t,s,...n])}function O0(e){let t=e.length;for(;t--&&!e[t];);return e.slice(0,t+1).map((s,n)=>s||Ne("_".repeat(n+1),!1))}const Zu=Ne("undefined",!1),L0=(e,t)=>{if(e.type===1&&(e.tagType===1||e.tagType===3)){const s=ds(e,"slot");if(s)return s.exp,t.scopes.vSlot++,()=>{t.scopes.vSlot--}}},D0=(e,t,s,n)=>Ta(e,s,!1,!0,s.length?s[0].loc:n);function M0(e,t,s=D0){t.helper(hc);const{children:n,loc:a}=e,i=[],l=[];let r=t.scopes.vSlot>0||t.scopes.vFor>0;const o=ds(e,"slot",!0);if(o){const{arg:k,exp:E}=o;k&&!es(k)&&(r=!0),i.push(vt(k||Ne("default",!0),s(E,void 0,n,a)))}let c=!1,u=!1;const d=[],f=new Set;let p=0;for(let k=0;k<n.length;k++){const E=n[k];let v;if(!Rl(E)||!(v=ds(E,"slot",!0))){E.type!==3&&d.push(E);continue}if(o){t.onError(at(37,v.loc));break}c=!0;const{children:y,loc:x}=E,{arg:T=Ne("default",!0),exp:N,loc:O}=v;let w;es(T)?w=T?T.content:"default":r=!0;const R=ds(E,"for"),L=s(N,R,y,x);let U,P;if(U=ds(E,"if"))r=!0,l.push(oo(U.exp,Zi(T,L,p++),Zu));else if(P=ds(E,/^else(?:-if)?$/,!0)){let S=k,$;for(;S--&&($=n[S],!!dh($)););if($&&Rl($)&&ds($,/^(?:else-)?if$/)){let W=l[l.length-1];for(;W.alternate.type===19;)W=W.alternate;W.alternate=P.exp?oo(P.exp,Zi(T,L,p++),Zu):Zi(T,L,p++)}else t.onError(at(30,P.loc))}else if(R){r=!0;const S=R.forParseResult;S?(Sh(S),l.push(St(t.helper(uc),[S.source,Ta(fo(S),Zi(T,L),!0)]))):t.onError(at(32,R.loc))}else{if(w){if(f.has(w)){t.onError(at(38,O));continue}f.add(w),w==="default"&&(u=!0)}i.push(vt(T,L))}}if(!o){const k=(E,v)=>{const y=s(E,void 0,v,a);return t.compatConfig&&(y.isNonScopedSlot=!0),vt("default",y)};c?d.length&&!d.every(yc)&&(u?t.onError(at(39,d[0].loc)):i.push(k(void 0,d))):i.push(k(void 0,n))}const b=r?2:il(e.children)?3:1;let m=fs(i.concat(vt("_",Ne(b+"",!1))),a);return l.length&&(m=St(t.helper(sh),[m,Fn(l)])),{slots:m,hasDynamicSlots:r}}function Zi(e,t,s){const n=[vt("name",e),vt("fn",t)];return s!=null&&n.push(vt("key",Ne(String(s),!0))),fs(n)}function il(e){for(let t=0;t<e.length;t++){const s=e[t];switch(s.type){case 1:if(s.tagType===2||il(s.children))return!0;break;case 9:if(il(s.branches))return!0;break;case 10:case 11:if(il(s.children))return!0;break}}return!1}const Th=new WeakMap,P0=(e,t)=>function(){if(e=t.currentNode,!(e.type===1&&(e.tagType===0||e.tagType===1)))return;const{tag:n,props:a}=e,i=e.tagType===1;let l=i?F0(e,t):`"${n}"`;const r=je(l)&&l.callee===lc;let o,c,u=0,d,f,p,b=r||l===si||l===tc||!i&&(n==="svg"||n==="foreignObject"||n==="math");if(a.length>0){const m=Ch(e,t,void 0,i,r);o=m.props,u=m.patchFlag,f=m.dynamicPropNames;const k=m.directives;p=k&&k.length?Fn(k.map(E=>U0(E,t))):void 0,m.shouldUseBlock&&(b=!0)}if(e.children.length>0)if(l===Tl&&(b=!0,u|=1024),i&&l!==si&&l!==Tl){const{slots:k,hasDynamicSlots:E}=M0(e,t);c=k,E&&(u|=1024)}else if(e.children.length===1&&l!==si){const k=e.children[0],E=k.type,v=E===5||E===8;v&&as(k,t)===0&&(u|=1),v||E===2?c=k:c=e.children}else c=e.children;f&&f.length&&(d=B0(f)),e.codegenNode=yi(t,l,o,c,u===0?void 0:u,d,p,!!b,!1,i,e.loc)};function F0(e,t,s=!1){let{tag:n}=e;const a=po(n),i=nr(e,"is",!1,!0);if(i)if(a||$n("COMPILER_IS_ON_ELEMENT",t)){let r;if(i.type===6?r=i.value&&Ne(i.value.content,!0):(r=i.exp,r||(r=Ne("is",!1,i.arg.loc))),r)return St(t.helper(lc),[r])}else i.type===6&&i.value.content.startsWith("vue:")&&(n=i.value.content.slice(4));const l=ih(n)||t.isBuiltInComponent(n);return l?(s||t.helper(l),l):(t.helper(ic),t.components.add(n),_i(n,"component"))}function Ch(e,t,s=e.props,n,a,i=!1){const{tag:l,loc:r,children:o}=e;let c=[];const u=[],d=[],f=o.length>0;let p=!1,b=0,m=!1,k=!1,E=!1,v=!1,y=!1,x=!1;const T=[],N=L=>{c.length&&(u.push(fs(Ju(c),r)),c=[]),L&&u.push(L)},O=()=>{t.scopes.vFor>0&&c.push(vt(Ne("ref_for",!0),Ne("true")))},w=({key:L,value:U})=>{if(es(L)){const P=L.content,S=zn(P);if(S&&(!n||a)&&P.toLowerCase()!=="onclick"&&P!=="onUpdate:modelValue"&&!Zs(P)&&(v=!0),S&&Zs(P)&&(x=!0),S&&U.type===14&&(U=U.arguments[0]),U.type===20||(U.type===4||U.type===8)&&as(U,t)>0)return;P==="ref"?m=!0:P==="class"?k=!0:P==="style"?E=!0:P!=="key"&&!T.includes(P)&&T.push(P),n&&(P==="class"||P==="style")&&!T.includes(P)&&T.push(P)}else y=!0};for(let L=0;L<s.length;L++){const U=s[L];if(U.type===6){const{loc:P,name:S,nameLoc:$,value:W}=U;let G=!0;if(S==="ref"&&(m=!0,O()),S==="is"&&(po(l)||W&&W.content.startsWith("vue:")||$n("COMPILER_IS_ON_ELEMENT",t)))continue;c.push(vt(Ne(S,!0,$),Ne(W?W.content:"",G,W?W.loc:P)))}else{const{name:P,arg:S,exp:$,loc:W,modifiers:G}=U,D=P==="bind",I=P==="on";if(P==="slot"){n||t.onError(at(40,W));continue}if(P==="once"||P==="memo"||P==="is"||D&&Nn(S,"is")&&(po(l)||$n("COMPILER_IS_ON_ELEMENT",t))||I&&i)continue;if((D&&Nn(S,"key")||I&&f&&Nn(S,"vue:before-update"))&&(p=!0),D&&Nn(S,"ref")&&O(),!S&&(D||I)){if(y=!0,$)if(D){if(N(),$n("COMPILER_V_BIND_OBJECT_ORDER",t)){u.unshift($);continue}O(),N(),u.push($)}else N({type:14,loc:W,callee:t.helper(pc),arguments:n?[$]:[$,"true"]});else t.onError(at(D?34:35,W));continue}D&&G.some(ce=>ce.content==="prop")&&(b|=32);const z=t.directiveTransforms[P];if(z){const{props:ce,needRuntime:de}=z(U,e,t);!i&&ce.forEach(w),I&&S&&!es(S)?N(fs(ce,r)):c.push(...ce),de&&(d.push(U),Bt(de)&&Th.set(U,de))}else Im(P)||(d.push(U),f&&(p=!0))}}let R;if(u.length?(N(),u.length>1?R=St(t.helper(Cl),u,r):R=u[0]):c.length&&(R=fs(Ju(c),r)),y?b|=16:(k&&!n&&(b|=2),E&&!n&&(b|=4),T.length&&(b|=8),v&&(b|=32)),!p&&(b===0||b===32)&&(m||x||d.length>0)&&(b|=512),!t.inSSR&&R)switch(R.type){case 15:let L=-1,U=-1,P=!1;for(let W=0;W<R.properties.length;W++){const G=R.properties[W].key;es(G)?G.content==="class"?L=W:G.content==="style"&&(U=W):G.isHandlerKey||(P=!0)}const S=R.properties[L],$=R.properties[U];P?R=St(t.helper(bi),[R]):(S&&!es(S.value)&&(S.value=St(t.helper(dc),[S.value])),$&&(E||$.value.type===4&&$.value.content.trim()[0]==="["||$.value.type===17)&&($.value=St(t.helper(fc),[$.value])));break;case 14:break;default:R=St(t.helper(bi),[St(t.helper(Oi),[R])]);break}return{props:R,directives:d,patchFlag:b,dynamicPropNames:T,shouldUseBlock:p}}function Ju(e){const t=new Map,s=[];for(let n=0;n<e.length;n++){const a=e[n];if(a.key.type===8||!a.key.isStatic){s.push(a);continue}const i=a.key.content,l=t.get(i);l?(i==="style"||i==="class"||zn(i))&&$0(l,a):(t.set(i,a),s.push(a))}return s}function $0(e,t){e.value.type===17?e.value.elements.push(t.value):e.value=Fn([e.value,t.value],e.loc)}function U0(e,t){const s=[],n=Th.get(e);n?s.push(t.helperString(n)):(t.helper(rc),t.directives.add(e.name),s.push(_i(e.name,"directive")));const{loc:a}=e;if(e.exp&&s.push(e.exp),e.arg&&(e.exp||s.push("void 0"),s.push(e.arg)),Object.keys(e.modifiers).length){e.arg||(e.exp||s.push("void 0"),s.push("void 0"));const i=Ne("true",!1,a);s.push(fs(e.modifiers.map(l=>vt(l,i)),a))}return Fn(s,e.loc)}function B0(e){let t="[";for(let s=0,n=e.length;s<n;s++)t+=JSON.stringify(e[s]),s<n-1&&(t+=", ");return t+"]"}function po(e){return e==="component"||e==="Component"}const H0=(e,t)=>{if(Il(e)){const{children:s,loc:n}=e,{slotName:a,slotProps:i}=V0(e,t),l=[t.prefixIdentifiers?"_ctx.$slots":"$slots",a,"{}","undefined","true"];let r=2;i&&(l[2]=i,r=3),s.length&&(l[3]=Ta([],s,!1,!1,n),r=4),t.scopeId&&!t.slotted&&(r=5),l.splice(r),e.codegenNode=St(t.helper(th),l,n)}};function V0(e,t){let s='"default"',n;const a=[];for(let i=0;i<e.props.length;i++){const l=e.props[i];if(l.type===6)l.value&&(l.name==="name"?s=JSON.stringify(l.value.content):(l.name=Qe(l.name),a.push(l)));else if(l.name==="bind"&&Nn(l.arg,"name")){if(l.exp)s=l.exp;else if(l.arg&&l.arg.type===4){const r=Qe(l.arg.content);s=l.exp=Ne(r,!1,l.arg.loc)}}else l.name==="bind"&&l.arg&&es(l.arg)&&(l.arg.content=Qe(l.arg.content)),a.push(l)}if(a.length>0){const{props:i,directives:l}=Ch(e,t,a,!1,!1);n=i,l.length&&t.onError(at(36,l[0].loc))}return{slotName:s,slotProps:n}}const Eh=(e,t,s,n)=>{const{loc:a,modifiers:i,arg:l}=e;!e.exp&&!i.length&&s.onError(at(35,a));let r;if(l.type===4)if(l.isStatic){let d=l.content;d.startsWith("vue:")&&(d=`vnode-${d.slice(4)}`);const f=t.tagType!==0||d.startsWith("vnode")||!/[A-Z]/.test(d)?fa(Qe(d)):`on:${d}`;r=Ne(f,!0,l.loc)}else r=xs([`${s.helperString(ro)}(`,l,")"]);else r=l,r.children.unshift(`${s.helperString(ro)}(`),r.children.push(")");let o=e.exp;o&&!o.content.trim()&&(o=void 0);let c=s.cacheHandlers&&!o&&!s.inVOnce;if(o){const d=oh(o),f=!(d||Hy(o)),p=o.content.includes(";");(f||c&&d)&&(o=xs([`${f?"$event":"(...args)"} => ${p?"{":"("}`,o,p?"}":")"]))}let u={props:[vt(r,o||Ne("() => {}",!1,a))]};return n&&(u=n(u)),c&&(u.props[0].value=s.cache(u.props[0].value)),u.props.forEach(d=>d.key.isHandlerKey=!0),u},j0=(e,t,s)=>{const{modifiers:n,loc:a}=e,i=e.arg;let{exp:l}=e;return l&&l.type===4&&!l.content.trim()&&(l=void 0),i.type!==4?(i.children.unshift("("),i.children.push(') || ""')):i.isStatic||(i.content=i.content?`${i.content} || ""`:'""'),n.some(r=>r.content==="camel")&&(i.type===4?i.isStatic?i.content=Qe(i.content):i.content=`${s.helperString(lo)}(${i.content})`:(i.children.unshift(`${s.helperString(lo)}(`),i.children.push(")"))),s.inSSR||(n.some(r=>r.content==="prop")&&Yu(i,"."),n.some(r=>r.content==="attr")&&Yu(i,"^")),{props:[vt(i,l)]}},Yu=(e,t)=>{e.type===4?e.isStatic?e.content=t+e.content:e.content=`\`${t}\${${e.content}}\``:(e.children.unshift(`'${t}' + (`),e.children.push(")"))},z0=(e,t)=>{if(e.type===0||e.type===1||e.type===11||e.type===10)return()=>{const s=e.children;let n,a=!1;for(let i=0;i<s.length;i++){const l=s[i];if(Er(l)){a=!0;for(let r=i+1;r<s.length;r++){const o=s[r];if(Er(o))n||(n=s[i]=xs([l],l.loc)),n.children.push(" + ",o),s.splice(r,1),r--;else{n=void 0;break}}}}if(!(!a||s.length===1&&(e.type===0||e.type===1&&e.tagType===0&&!e.props.find(i=>i.type===7&&!t.directiveTransforms[i.name])&&e.tag!=="template")))for(let i=0;i<s.length;i++){const l=s[i];if(Er(l)||l.type===8){const r=[];(l.type!==2||l.content!==" ")&&r.push(l),!t.ssr&&as(l,t)===0&&r.push("1"),s[i]={type:12,content:l,loc:l.loc,codegenNode:St(t.helper(ac),r)}}}}},Qu=new WeakSet,K0=(e,t)=>{if(e.type===1&&ds(e,"once",!0))return Qu.has(e)||t.inVOnce||t.inSSR?void 0:(Qu.add(e),t.inVOnce=!0,t.helper(El),()=>{t.inVOnce=!1;const s=t.currentNode;s.codegenNode&&(s.codegenNode=t.cache(s.codegenNode,!0,!0))})},Ah=(e,t,s)=>{const{exp:n,arg:a}=e;if(!n)return s.onError(at(41,e.loc)),Ba();const i=n.loc.source.trim(),l=n.type===4?n.content:i,r=s.bindingMetadata[i];if(r==="props"||r==="props-aliased")return s.onError(at(44,n.loc)),Ba();if(r==="literal-const"||r==="setup-const")return s.onError(at(45,n.loc)),Ba();if(!l.trim()||!oh(n))return s.onError(at(42,n.loc)),Ba();const o=a||Ne("modelValue",!0),c=a?es(a)?`onUpdate:${Qe(a.content)}`:xs(['"onUpdate:" + ',a]):"onUpdate:modelValue";let u;const d=s.isTS?"($event: any)":"$event";u=xs([`${d} => ((`,n,") = $event)"]);const f=[vt(o,e.exp),vt(c,u)];if(e.modifiers.length&&t.tagType===1){const p=e.modifiers.map(m=>m.content).map(m=>(bc(m)?m:JSON.stringify(m))+": true").join(", "),b=a?es(a)?`${a.content}Modifiers`:xs([a,' + "Modifiers"']):"modelModifiers";f.push(vt(b,Ne(`{ ${p} }`,!1,e.loc,2)))}return Ba(f)};function Ba(e=[]){return{props:e}}const q0=/[\w).+\-_$\]]/,G0=(e,t)=>{$n("COMPILER_FILTERS",t)&&(e.type===5?Ol(e.content,t):e.type===1&&e.props.forEach(s=>{s.type===7&&s.name!=="for"&&s.exp&&Ol(s.exp,t)}))};function Ol(e,t){if(e.type===4)Xu(e,t);else for(let s=0;s<e.children.length;s++){const n=e.children[s];typeof n=="object"&&(n.type===4?Xu(n,t):n.type===8?Ol(e,t):n.type===5&&Ol(n.content,t))}}function Xu(e,t){const s=e.content;let n=!1,a=!1,i=!1,l=!1,r=0,o=0,c=0,u=0,d,f,p,b,m=[];for(p=0;p<s.length;p++)if(f=d,d=s.charCodeAt(p),n)d===39&&f!==92&&(n=!1);else if(a)d===34&&f!==92&&(a=!1);else if(i)d===96&&f!==92&&(i=!1);else if(l)d===47&&f!==92&&(l=!1);else if(d===124&&s.charCodeAt(p+1)!==124&&s.charCodeAt(p-1)!==124&&!r&&!o&&!c)b===void 0?(u=p+1,b=s.slice(0,p).trim()):k();else{switch(d){case 34:a=!0;break;case 39:n=!0;break;case 96:i=!0;break;case 40:c++;break;case 41:c--;break;case 91:o++;break;case 93:o--;break;case 123:r++;break;case 125:r--;break}if(d===47){let E=p-1,v;for(;E>=0&&(v=s.charAt(E),v===" ");E--);(!v||!q0.test(v))&&(l=!0)}}b===void 0?b=s.slice(0,p).trim():u!==0&&k();function k(){m.push(s.slice(u,p).trim()),u=p+1}if(m.length){for(p=0;p<m.length;p++)b=W0(b,m[p],t);e.content=b,e.ast=void 0}}function W0(e,t,s){s.helper(oc);const n=t.indexOf("(");if(n<0)return s.filters.add(t),`${_i(t,"filter")}(${e})`;{const a=t.slice(0,n),i=t.slice(n+1);return s.filters.add(a),`${_i(a,"filter")}(${e}${i!==")"?","+i:i}`}}const ed=new WeakSet,Z0=(e,t)=>{if(e.type===1){const s=ds(e,"memo");return!s||ed.has(e)||t.inSSR?void 0:(ed.add(e),()=>{const n=e.codegenNode||t.currentNode.codegenNode;n&&n.type===13&&(e.tagType!==1&&gc(n,t),e.codegenNode=St(t.helper(mc),[s.exp,Ta(void 0,n),"_cache",String(t.cached.length)]),t.cached.push(null))})}},J0=(e,t)=>{if(e.type===1){for(const s of e.props)if(s.type===7&&s.name==="bind"&&(!s.exp||s.exp.type===4&&!s.exp.content.trim())&&s.arg){const n=s.arg;if(n.type!==4||!n.isStatic)t.onError(at(53,n.loc)),s.exp=Ne("",!0,n.loc);else{const a=Qe(n.content);(lh.test(a[0])||a[0]==="-")&&(s.exp=Ne(a,!1,n.loc))}}}};function Y0(e){return[[J0,K0,E0,Z0,I0,G0,H0,P0,L0,z0],{on:Eh,bind:j0,model:Ah}]}function Q0(e,t={}){const s=t.onError||vc,n=t.mode==="module";t.prefixIdentifiers===!0?s(at(48)):n&&s(at(49));const a=!1;t.cacheHandlers&&s(at(50)),t.scopeId&&!n&&s(at(51));const i=Pe({},t,{prefixIdentifiers:a}),l=Ee(e)?a0(e,i):e,[r,o]=Y0();return o0(l,Pe({},i,{nodeTransforms:[...r,...t.nodeTransforms||[]],directiveTransforms:Pe({},o,t.directiveTransforms||{})})),f0(l,i)}const X0=()=>({props:[]});/**
* @vue/compiler-dom v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const Rh=Symbol(""),Ih=Symbol(""),Nh=Symbol(""),Oh=Symbol(""),ho=Symbol(""),Lh=Symbol(""),Dh=Symbol(""),Mh=Symbol(""),Ph=Symbol(""),Fh=Symbol("");Iy({[Rh]:"vModelRadio",[Ih]:"vModelCheckbox",[Nh]:"vModelText",[Oh]:"vModelSelect",[ho]:"vModelDynamic",[Lh]:"withModifiers",[Dh]:"withKeys",[Mh]:"vShow",[Ph]:"Transition",[Fh]:"TransitionGroup"});let ta;function ex(e,t=!1){return ta||(ta=document.createElement("div")),t?(ta.innerHTML=`<div foo="${e.replace(/"/g,"&quot;")}">`,ta.children[0].getAttribute("foo")):(ta.innerHTML=e,ta.textContent)}const tx={parseMode:"html",isVoidTag:Gm,isNativeTag:e=>zm(e)||Km(e)||qm(e),isPreTag:e=>e==="pre",isIgnoreNewlineTag:e=>e==="pre"||e==="textarea",decodeEntities:ex,isBuiltInComponent:e=>{if(e==="Transition"||e==="transition")return Ph;if(e==="TransitionGroup"||e==="transition-group")return Fh},getNamespace(e,t,s){let n=t?t.ns:s;if(t&&n===2)if(t.tag==="annotation-xml"){if(e==="svg")return 1;t.props.some(a=>a.type===6&&a.name==="encoding"&&a.value!=null&&(a.value.content==="text/html"||a.value.content==="application/xhtml+xml"))&&(n=0)}else/^m(?:[ions]|text)$/.test(t.tag)&&e!=="mglyph"&&e!=="malignmark"&&(n=0);else t&&n===1&&(t.tag==="foreignObject"||t.tag==="desc"||t.tag==="title")&&(n=0);if(n===0){if(e==="svg")return 1;if(e==="math")return 2}return n}},sx=e=>{e.type===1&&e.props.forEach((t,s)=>{t.type===6&&t.name==="style"&&t.value&&(e.props[s]={type:7,name:"bind",arg:Ne("style",!0,t.loc),exp:nx(t.value.content,t.loc),modifiers:[],loc:t.loc})})},nx=(e,t)=>{const s=Zd(e);return Ne(JSON.stringify(s),!1,t,3)};function yn(e,t){return at(e,t)}const ax=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(yn(54,a)),t.children.length&&(s.onError(yn(55,a)),t.children.length=0),{props:[vt(Ne("innerHTML",!0,a),n||Ne("",!0))]}},ix=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(yn(56,a)),t.children.length&&(s.onError(yn(57,a)),t.children.length=0),{props:[vt(Ne("textContent",!0),n?as(n,s)>0?n:St(s.helperString(sr),[n],a):Ne("",!0))]}},lx=(e,t,s)=>{const n=Ah(e,t,s);if(!n.props.length||t.tagType===1)return n;e.arg&&s.onError(yn(59,e.arg.loc));const{tag:a}=t,i=s.isCustomElement(a);if(a==="input"||a==="textarea"||a==="select"||i){let l=Nh,r=!1;if(a==="input"||i){const o=nr(t,"type");if(o){if(o.type===7)l=ho;else if(o.value)switch(o.value.content){case"radio":l=Rh;break;case"checkbox":l=Ih;break;case"file":r=!0,s.onError(yn(60,e.loc));break}}else Vy(t)&&(l=ho)}else a==="select"&&(l=Oh);r||(n.needRuntime=s.helper(l))}else s.onError(yn(58,e.loc));return n.props=n.props.filter(l=>!(l.key.type===4&&l.key.content==="modelValue")),n},rx=rs("passive,once,capture"),ox=rs("stop,prevent,self,ctrl,shift,alt,meta,exact,middle"),cx=rs("left,right"),$h=rs("onkeyup,onkeydown,onkeypress"),ux=(e,t,s,n)=>{const a=[],i=[],l=[];for(let r=0;r<t.length;r++){const o=t[r].content;o==="native"&&xi("COMPILER_V_ON_NATIVE",s)||rx(o)?l.push(o):cx(o)?es(e)?$h(e.content.toLowerCase())?a.push(o):i.push(o):(a.push(o),i.push(o)):ox(o)?i.push(o):a.push(o)}return{keyModifiers:a,nonKeyModifiers:i,eventOptionModifiers:l}},td=(e,t)=>es(e)&&e.content.toLowerCase()==="onclick"?Ne(t,!0):e.type!==4?xs(["(",e,`) === "onClick" ? "${t}" : (`,e,")"]):e,dx=(e,t,s)=>Eh(e,t,s,n=>{const{modifiers:a}=e;if(!a.length)return n;let{key:i,value:l}=n.props[0];const{keyModifiers:r,nonKeyModifiers:o,eventOptionModifiers:c}=ux(i,a,s,e.loc);if(o.includes("right")&&(i=td(i,"onContextmenu")),o.includes("middle")&&(i=td(i,"onMouseup")),o.length&&(l=St(s.helper(Lh),[l,JSON.stringify(o)])),r.length&&(!es(i)||$h(i.content.toLowerCase()))&&(l=St(s.helper(Dh),[l,JSON.stringify(r)])),c.length){const u=c.map(qn).join("");i=es(i)?Ne(`${i.content}${u}`,!0):xs(["(",i,`) + "${u}"`])}return{props:[vt(i,l)]}}),fx=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(yn(62,a)),{props:[],needRuntime:s.helper(Mh)}},px=(e,t)=>{e.type===1&&e.tagType===0&&(e.tag==="script"||e.tag==="style")&&t.removeNode()},hx=[sx],mx={cloak:X0,html:ax,text:ix,model:lx,on:dx,show:fx};function gx(e,t={}){return Q0(e,Pe({},tx,t,{nodeTransforms:[px,...hx,...t.nodeTransforms||[]],directiveTransforms:Pe({},mx,t.directiveTransforms||{}),transformHoist:null}))}/**
* vue v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const sd=Object.create(null);function vx(e,t){if(!Ee(e))if(e.nodeType)e=e.innerHTML;else return Lt;const s=Lm(e,t),n=sd[s];if(n)return n;if(e[0]==="#"){const r=document.querySelector(e);e=r?r.innerHTML:""}const a=Pe({hoistStatic:!0,onError:void 0,onWarn:Lt},t);!a.isCustomElement&&typeof customElements<"u"&&(a.isCustomElement=r=>!!customElements.get(r));const{code:i}=gx(e,a),l=new Function("Vue",i)(Sy);return l._rc=!0,sd[s]=l}_p(vx);const Ll=_n({items:[]});let bx=1;function lr(e,t="info",s=3e3){const n=bx++;return Ll.items.push({id:n,message:String(e),type:t}),s>0&&setTimeout(()=>kc(n),s),n}function kc(e){const t=Ll.items.findIndex(s=>s.id===e);t>=0&&Ll.items.splice(t,1)}function ke(e,t="info",s=3e3){return lr(e,t,s)}ke.success=(e,t=3e3)=>lr(e,"success",t);ke.error=(e,t=5e3)=>lr(e,"error",t);ke.info=(e,t=3e3)=>lr(e,"info",t);ke.dismiss=kc;const yx={setup(){return{state:Ll,dismiss:kc}},template:`
  ...
  `},js=_n({open:!1,title:"Confirm",message:"",confirmLabel:"Confirm",cancelLabel:"Cancel",danger:!1});let ba=null;function ls({title:e="Confirm",message:t="",confirmLabel:s="Confirm",cancelLabel:n="Cancel",danger:a=!1}={}){return ba&&ba(!1),js.title=e,js.message=t,js.confirmLabel=s,js.cancelLabel=n,js.danger=a,js.open=!0,new Promise(i=>{ba=i})}function nd(e){js.open=!1,ba&&(ba(e),ba=null)}const xx={setup(){function e(t){js.open&&t.key==="Escape"&&(t.stopPropagation(),nd(!1))}return Be(()=>document.addEventListener("keydown",e,!0)),ht(()=>document.removeEventListener("keydown",e,!0)),{state:js,settle:nd}},template:`
 ...
 * @license MIT
 */const la=typeof document<"u";function Uh(e){return typeof e=="object"||"displayName"in e||"props"in e||"__vccOpts"in e}function _x(e){return e.__esModule||e[Symbol.toStringTag]==="Module"||e.default&&Uh(e.default)}const Ge=Object.assign;function Rr(e,t){const s={};for(const n in t){const a=t[n];s[n]=ks(a)?a.map(e):e(a)}return s}const ni=()=>{},ks=Array.isArray;function ad(e,t){const s={};for(const n in e)s[n]=n in t?t[n]:e[n];return s}const Bh=/#/g,kx=/&/g,wx=/\//g,Sx=/=/g,Tx=/\?/g,Hh=/\+/g,Cx=/%5B/g,Ex=/%5D/g,Vh=/%5E/g,Ax=/%60/g,jh=/%7B/g,Rx=/%7C/g,zh=/%7D/g,Ix=/%20/g;function wc(e){return e==null?"":encodeURI(""+e).replace(Rx,"|").replace(Cx,"[").replace(Ex,"]")}function Nx(e){return wc(e).replace(jh,"{").replace(zh,"}").replace(Vh,"^")}function mo(e){return wc(e).replace(Hh,"%2B").replace(Ix,"+").replace(Bh,"%23").replace(kx,"%26").replace(Ax,"`").replace(jh,"{").replace(zh,"}").replace(Vh,"^")}function Ox(e){return mo(e).replace(Sx,"%3D")}function Lx(e){return wc(e).replace(Bh,"%23").replace(Tx,"%3F")}function Dx(e){return Lx(e).replace(wx,"%2F")}function wi(e){if(e==null)return null;try{return decodeURIComponent(""+e)}catch{}return""+e}const Mx=/\/$/,Px=e=>e.replace(Mx,"");function Ir(e,t,s="/"){let n,a={},i="",l="";const r=t.indexOf("#");let o=t.indexOf("?");return o=r>=0&&o>r?-1:o,o>=0&&(n=t.slice(0,o),i=t.slice(o,r>0?r:t.length),a=e(i.slice(1))),r>=0&&(n=n||t.slice(0,r),l=t.slice(r,t.length)),n=Bx(n??t,s),{fullPath:n+i+l,path:n,query:a,hash:wi(l)}}function Fx(e,t){const s=t.query?e(t.query):"";return t.path+(s&&"?")+s+(t.hash||"")}function id(e,t){return!t||!e.toLowerCase().startsWith(t.toLowerCase())?e:e.slice(t.length)||"/"}function $x(e,t,s){const n=t.matched.length-1,a=s.matched.length-1;return n>-1&&n===a&&Aa(t.matched[n],s.matched[a])&&Kh(t.params,s.params)&&e(t.query)===e(s.query)&&t.hash===s.hash}function Aa(e,t){return(e.aliasOf||e)===(t.aliasOf||t)}function Kh(e,t){if(Object.keys(e).length!==Object.keys(t).length)return!1;for(var s in e)if(!Ux(e[s],t[s]))return!1;return!0}function Ux(e,t){return ks(e)?ld(e,t):ks(t)?ld(t,e):(e==null?void 0:e.valueOf())===(t==null?void 0:t.valueOf())}function ld(e,t){return ks(t)?e.length===t.length&&e.every((s,n)=>s===t[n]):e.length===1&&e[0]===t}function Bx(e,t){if(e.startsWith("/"))return e;if(!e)return t;const s=t.split("/"),n=e.split("/"),a=n[n.length-1];(a===".."||a===".")&&n.push("");let i=s.length-1,l,r;for(l=0;l<n.length;l++)if(r=n[l],r!==".")if(r==="..")i>1&&i--;else break;return s.slice(0,i).join("/")+"/"+n.slice(l).join("/")}const dn={path:"/",name:void 0,params:{},query:{},hash:"",fullPath:"/",matched:[],meta:{},redirectedFrom:void 0};let go=(function(e){return e.pop="pop",e.push="push",e})({}),Nr=(function(e){return e.back="back",e.forward="forward",e.unknown="",e})({});function Hx(e){if(!e)if(la){const t=document.querySelector("base");e=t&&t.getAttribute("href")||"/",e=e.replace(/^\w+:\/\/[^\/]+/,"")}else e="/";return e[0]!=="/"&&e[0]!=="#"&&(e="/"+e),Px(e)}const Vx=/^[^#]+#/;function jx(e,t){return e.replace(Vx,"#")+t}function zx(e,t){const s=document.documentElement.getBoundingClientRect(),n=e.getBoundingClientRect();return{behavior:t.behavior,left:n.left-s.left-(t.left||0),top:n.top-s.top-(t.top||0)}}const rr=()=>({left:window.scrollX,top:window.scrollY});function Kx(e){let t;if("el"in e){const s=e.el,n=typeof s=="string"&&s.startsWith("#"),a=typeof s=="string"?n?document.getElementById(s.slice(1)):document.querySelector(s):s;if(!a)return;t=zx(a,e)}else t=e;"scrollBehavior"in document.documentElement.style?window.scrollTo(t):window.scrollTo(t.left!=null?t.left:window.scrollX,t.top!=null?t.top:window.scrollY)}function rd(e,t){return(history.state?history.state.position-t:-1)+e}const vo=new Map;function qx(e,t){vo.set(e,t)}function Gx(e){const t=vo.get(e);return vo.delete(e),t}function Wx(e){return typeof e=="string"||e&&typeof e=="object"}function qh(e){return typeof e=="string"||typeof e=="symbol"}let ct=(function(e){return e[e.MATCHER_NOT_FOUND=1]="MATCHER_NOT_FOUND",e[e.NAVIGATION_GUARD_REDIRECT=2]="NAVIGATION_GUARD_REDIRECT",e[e.NAVIGATION_ABORTED=4]="NAVIGATION_ABORTED",e[e.NAVIGATION_CANCELLED=8]="NAVIGATION_CANCELLED",e[e.NAVIGATION_DUPLICATED=16]="NAVIGATION_DUPLICATED",e})({});const Gh=Symbol("");ct.MATCHER_NOT_FOUND+"",ct.NAVIGATION_GUARD_REDIRECT+"",ct.NAVIGATION_ABORTED+"",ct.NAVIGATION_CANCELLED+"",ct.NAVIGATION_DUPLICATED+"";function Ra(e,t){return Ge(new Error,{type:e,[Gh]:!0},t)}function $s(e,t){return e instanceof Error&&Gh in e&&(t==null||!!(e.type&t))}const Zx=["params","query","hash"];function Jx(e){if(typeof e=="string")return e;if(e.path!=null)return e.path;const t={};for(const s of Zx)s in e&&(t[s]=e[s]);return JSON.stringify(t,null,2)}function Yx(e){const t={};if(e===""||e==="?")return t;const s=(e[0]==="?"?e.slice(1):e).split("&");for(let n=0;n<s.length;++n){const a=s[n].replace(Hh," "),i=a.indexOf("="),l=wi(i<0?a:a.slice(0,i)),r=i<0?null:wi(a.slice(i+1));if(l in t){let o=t[l];ks(o)||(o=t[l]=[o]),o.push(r)}else t[l]=r}return t}function od(e){let t="";for(let s in e){const n=e[s];if(s=Ox(s),n==null){n!==void 0&&(t+=(t.length?"&":"")+s);continue}(ks(n)?n.map(a=>a&&mo(a)):[n&&mo(n)]).forEach(a=>{a!==void 0&&(t+=(t.length?"&":"")+s,a!=null&&(t+="="+a))})}return t}function Qx(e){const t={};for(const s in e){const n=e[s];n!==void 0&&(t[s]=ks(n)?n.map(a=>a==null?null:""+a):n==null?n:""+n)}return t}const Xx=Symbol(""),cd=Symbol(""),or=Symbol(""),Sc=Symbol(""),bo=Symbol("");function Ha(){let e=[];function t(n){return e.push(n),()=>{const a=e.indexOf(n);a>-1&&e.splice(a,1)}}function s(){e=[]}return{add:t,list:()=>e.slice(),reset:s}}function vn(e,t,s,n,a,i=l=>l()){const l=n&&(n.enterCallbacks[a]=n.enterCallbacks[a]||[]);return()=>new Promise((r,o)=>{const c=f=>{f===!1?o(Ra(ct.NAVIGATION_ABORTED,{from:s,to:t})):f instanceof Error?o(f):Wx(f)?o(Ra(ct.NAVIGATION_GUARD_REDIRECT,{from:t,to:f})):(l&&n.enterCallbacks[a]===l&&typeof f=="function"&&l.push(f),r())},u=i(()=>e.call(n&&n.instances[a],t,s,c));let d=Promise.resolve(u);e.length<3&&(d=d.then(c)),d.catch(f=>o(f))})}function Or(e,t,s,n,a=i=>i()){const i=[];for(const l of e)for(const r in l.components){let o=l.components[r];if(!(t!=="beforeRouteEnter"&&!l.instances[r]))if(Uh(o)){const c=(o.__vccOpts||o)[t];c&&i.push(vn(c,s,n,l,r,a))}else{let c=o();i.push(()=>c.then(u=>{if(!u)throw new Error(`Couldn't resolve component "${r}" at "${l.path}"`);const d=_x(u)?u.default:u;l.mods[r]=u,l.components[r]=d;const f=(d.__vccOpts||d)[t];return f&&vn(f,s,n,l,r,a)()}))}}return i}function e_(e,t){const s=[],n=[],a=[],i=Math.max(t.matched.length,e.matched.length);for(let l=0;l<i;l++){const r=t.matched[l];r&&(e.matched.find(c=>Aa(c,r))?n.push(r):s.push(r));const o=e.matched[l];o&&(t.matched.find(c=>Aa(c,o))||a.push(o))}return[s,n,a]}/*!
 * vue-router v4.6.4
 * (c) 2025 Eduardo San Martin Morote
 * @license MIT
 */let t_=()=>location.protocol+"//"+location.host;function Wh(e,t){const{pathname:s,search:n,hash:a}=t,i=e.indexOf("#");if(i>-1){let l=a.includes(e.slice(i))?e.slice(i).length:1,r=a.slice(l);return r[0]!=="/"&&(r="/"+r),id(r,"")}return id(s,e)+n+a}function s_(e,t,s,n){let a=[],i=[],l=null;const r=({state:f})=>{const p=Wh(e,location),b=s.value,m=t.value;let k=0;if(f){if(s.value=p,t.value=f,l&&l===b){l=null;return}k=m?f.position-m.position:0}else n(p);a.forEach(E=>{E(s.value,b,{delta:k,type:go.pop,direction:k?k>0?Nr.forward:Nr.back:Nr.unknown})})};function o(){l=s.value}function c(f){a.push(f);const p=()=>{const b=a.indexOf(f);b>-1&&a.splice(b,1)};return i.push(p),p}function u(){if(document.visibilityState==="hidden"){const{history:f}=window;if(!f.state)return;f.replaceState(Ge({},f.state,{scroll:rr()}),"")}}function d(){for(const f of i)f();i=[],window.removeEventListener("popstate",r),window.removeEventListener("pagehide",u),document.removeEventListener("visibilitychange",u)}return window.addEventListener("popstate",r),window.addEventListener("pagehide",u),document.addEventListener("visibilitychange",u),{pauseListeners:o,listen:c,destroy:d}}function ud(e,t,s,n=!1,a=!1){return{back:e,current:t,forward:s,replaced:n,position:window.history.length,scroll:a?rr():null}}function n_(e){const{history:t,location:s}=window,n={value:Wh(e,s)},a={value:t.state};a.value||i(n.value,{back:null,current:n.value,forward:null,position:t.length-1,replaced:!0,scroll:null},!0);function i(o,c,u){const d=e.indexOf("#"),f=d>-1?(s.host&&document.querySelector("base")?e:e.slice(d))+o:t_()+e+o;try{t[u?"replaceState":"pushState"](c,"",f),a.value=c}catch(p){console.error(p),s[u?"replace":"assign"](f)}}function l(o,c){i(o,Ge({},t.state,ud(a.value.back,o,a.value.forward,!0),c,{position:a.value.position}),!0),n.value=o}function r(o,c){const u=Ge({},a.value,t.state,{forward:o,scroll:rr()});i(u.current,u,!0),i(o,Ge({},ud(n.value,o,null),{position:u.position+1},c),!1),n.value=o}return{location:n,state:a,push:r,replace:l}}function a_(e){e=Hx(e);const t=n_(e),s=s_(e,t.state,t.location,t.replace);function n(i,l=!0){l||s.pauseListeners(),history.go(i)}const a=Ge({location:"",base:e,go:n,createHref:jx.bind(null,e)},t,s);return Object.defineProperty(a,"location",{enumerable:!0,get:()=>t.location.value}),Object.defineProperty(a,"state",{enumerable:!0,get:()=>t.state.value}),a}function i_(e){return e=location.host?e||location.pathname+location.search:"",e.includes("#")||(e+="#"),a_(e)}let Ln=(function(e){return e[e.Static=0]="Static",e[e.Param=1]="Param",e[e.Group=2]="Group",e})({});var kt=(function(e){return e[e.Static=0]="Static",e[e.Param=1]="Param",e[e.ParamRegExp=2]="ParamRegExp",e[e.ParamRegExpEnd=3]="ParamRegExpEnd",e[e.EscapeNext=4]="EscapeNext",e})(kt||{});const l_={type:Ln.Static,value:""},r_=/[a-zA-Z0-9_]/;function o_(e){if(!e)return[[]];if(e==="/")return[[l_]];if(!e.startsWith("/"))throw new Error(`Invalid path "${e}"`);function t(p){throw new Error(`ERR (${s})/"${c}": ${p}`)}let s=kt.Static,n=s;const a=[];let i;function l(){i&&a.push(i),i=[]}let r=0,o,c="",u="";function d(){c&&(s===kt.Static?i.push({type:Ln.Static,value:c}):s===kt.Param||s===kt.ParamRegExp||s===kt.ParamRegExpEnd?(i.length>1&&(o==="*"||o==="+")&&t(`A repeatable param (${c}) must be alone in its segment. eg: '/:ids+.`),i.push({type:Ln.Param,value:c,regexp:u,repeatable:o==="*"||o==="+",optional:o==="*"||o==="?"})):t("Invalid state to consume buffer"),c="")}function f(){c+=o}for(;r<e.length;){if(o=e[r++],o==="\\"&&s!==kt.ParamRegExp){n=s,s=kt.EscapeNext;continue}switch(s){case kt.Static:o==="/"?(c&&d(),l()):o===":"?(d(),s=kt.Param):f();break;case kt.EscapeNext:f(),s=n;break;case kt.Param:o==="("?s=kt.ParamRegExp:r_.test(o)?f():(d(),s=kt.Static,o!=="*"&&o!=="?"&&o!=="+"&&r--);break;case kt.ParamRegExp:o===")"?u[u.length-1]=="\\"?u=u.slice(0,-1)+o:s=kt.ParamRegExpEnd:u+=o;break;case kt.ParamRegExpEnd:d(),s=kt.Static,o!=="*"&&o!=="?"&&o!=="+"&&r--,u="";break;default:t("Unknown state");break}}return s===kt.ParamRegExp&&t(`Unfinished custom RegExp for param "${c}"`),d(),l(),a}const dd="[^/]+?",c_={sensitive:!1,strict:!1,start:!0,end:!0};var jt=(function(e){return e[e._multiplier=10]="_multiplier",e[e.Root=90]="Root",e[e.Segment=40]="Segment",e[e.SubSegment=30]="SubSegment",e[e.Static=40]="Static",e[e.Dynamic=20]="Dynamic",e[e.BonusCustomRegExp=10]="BonusCustomRegExp",e[e.BonusWildcard=-50]="BonusWildcard",e[e.BonusRepeatable=-20]="BonusRepeatable",e[e.BonusOptional=-8]="BonusOptional",e[e.BonusStrict=.7000000000000001]="BonusStrict",e[e.BonusCaseSensitive=.25]="BonusCaseSensitive",e})(jt||{});const u_=/[.+*?^${}()[\]/\\]/g;function d_(e,t){const s=Ge({},c_,t),n=[];let a=s.start?"^":"";const i=[];for(const c of e){const u=c.length?[]:[jt.Root];s.strict&&!c.length&&(a+="/");for(let d=0;d<c.length;d++){const f=c[d];let p=jt.Segment+(s.sensitive?jt.BonusCaseSensitive:0);if(f.type===Ln.Static)d||(a+="/"),a+=f.value.replace(u_,"\\$&"),p+=jt.Static;else if(f.type===Ln.Param){const{value:b,repeatable:m,optional:k,regexp:E}=f;i.push({name:b,repeatable:m,optional:k});const v=E||dd;if(v!==dd){p+=jt.BonusCustomRegExp;try{`${v}`}catch(x){throw new Error(`Invalid custom RegExp for param "${b}" (${v}): `+x.message)}}let y=m?`((?:${v})(?:/(?:${v}))*)`:`(${v})`;d||(y=k&&c.length<2?`(?:/${y})`:"/"+y),k&&(y+="?"),a+=y,p+=jt.Dynamic,k&&(p+=jt.BonusOptional),m&&(p+=jt.BonusRepeatable),v===".*"&&(p+=jt.BonusWildcard)}u.push(p)}n.push(u)}if(s.strict&&s.end){const c=n.length-1;n[c][n[c].length-1]+=jt.BonusStrict}s.strict||(a+="/?"),s.end?a+="$":s.strict&&!a.endsWith("/")&&(a+="(?:/|$)");const l=new RegExp(a,s.sensitive?"":"i");function r(c){const u=c.match(l),d={};if(!u)return null;for(let f=1;f<u.length;f++){const p=u[f]||"",b=i[f-1];d[b.name]=p&&b.repeatable?p.split("/"):p}return d}function o(c){let u="",d=!1;for(const f of e){(!d||!u.endsWith("/"))&&(u+="/"),d=!1;for(const p of f)if(p.type===Ln.Static)u+=p.value;else if(p.type===Ln.Param){const{value:b,repeatable:m,optional:k}=p,E=b in c?c[b]:"";if(ks(E)&&!m)throw new Error(`Provided param "${b}" is an array but it is not repeatable (* or + modifiers)`);const v=ks(E)?E.join("/"):E;if(!v)if(k)f.length<2&&(u.endsWith("/")?u=u.slice(0,-1):d=!0);else throw new Error(`Missing required param "${b}"`);u+=v}}return u||"/"}return{re:l,score:n,keys:i,parse:r,stringify:o}}function f_(e,t){let s=0;for(;s<e.length&&s<t.length;){const n=t[s]-e[s];if(n)return n;s++}return e.length<t.length?e.length===1&&e[0]===jt.Static+jt.Segment?-1:1:e.length>t.length?t.length===1&&t[0]===jt.Static+jt.Segment?1:-1:0}function Zh(e,t){let s=0;const n=e.score,a=t.score;for(;s<n.length&&s<a.length;){const i=f_(n[s],a[s]);if(i)return i;s++}if(Math.abs(a.length-n.length)===1){if(fd(n))return 1;if(fd(a))return-1}return a.length-n.length}function fd(e){const t=e[e.length-1];return e.length>0&&t[t.length-1]<0}const p_={strict:!1,end:!0,sensitive:!1};function h_(e,t,s){const n=d_(o_(e.path),s),a=Ge(n,{record:e,parent:t,children:[],alias:[]});return t&&!a.record.aliasOf==!t.record.aliasOf&&t.children.push(a),a}function m_(e,t){const s=[],n=new Map;t=ad(p_,t);function a(d){return n.get(d)}function i(d,f,p){const b=!p,m=hd(d);m.aliasOf=p&&p.record;const k=ad(t,d),E=[m];if("alias"in d){const x=typeof d.alias=="string"?[d.alias]:d.alias;for(const T of x)E.push(hd(Ge({},m,{components:p?p.record.components:m.components,path:T,aliasOf:p?p.record:m})))}let v,y;for(const x of E){const{path:T}=x;if(f&&T[0]!=="/"){const N=f.record.path,O=N[N.length-1]==="/"?"":"/";x.path=f.record.path+(T&&O+T)}if(v=h_(x,f,k),p?p.alias.push(v):(y=y||v,y!==v&&y.alias.push(v),b&&d.name&&!md(v)&&l(d.name)),Jh(v)&&o(v),m.children){const N=m.children;for(let O=0;O<N.length;O++)i(N[O],v,p&&p.children[O])}p=p||v}return y?()=>{l(y)}:ni}function l(d){if(qh(d)){const f=n.get(d);f&&(n.delete(d),s.splice(s.indexOf(f),1),f.children.forEach(l),f.alias.forEach(l))}else{const f=s.indexOf(d);f>-1&&(s.splice(f,1),d.record.name&&n.delete(d.record.name),d.children.forEach(l),d.alias.forEach(l))}}function r(){return s}function o(d){const f=b_(d,s);s.splice(f,0,d),d.record.name&&!md(d)&&n.set(d.record.name,d)}function c(d,f){let p,b={},m,k;if("name"in d&&d.name){if(p=n.get(d.name),!p)throw Ra(ct.MATCHER_NOT_FOUND,{location:d});k=p.record.name,b=Ge(pd(f.params,p.keys.filter(y=>!y.optional).concat(p.parent?p.parent.keys.filter(y=>y.optional):[]).map(y=>y.name)),d.params&&pd(d.params,p.keys.map(y=>y.name))),m=p.stringify(b)}else if(d.path!=null)m=d.path,p=s.find(y=>y.re.test(m)),p&&(b=p.parse(m),k=p.record.name);else{if(p=f.name?n.get(f.name):s.find(y=>y.re.test(f.path)),!p)throw Ra(ct.MATCHER_NOT_FOUND,{location:d,currentLocation:f});k=p.record.name,b=Ge({},f.params,d.params),m=p.stringify(b)}const E=[];let v=p;for(;v;)E.unshift(v.record),v=v.parent;return{name:k,path:m,params:b,matched:E,meta:v_(E)}}e.forEach(d=>i(d));function u(){s.length=0,n.clear()}return{addRoute:i,resolve:c,removeRoute:l,clearRoutes:u,getRoutes:r,getRecordMatcher:a}}function pd(e,t){const s={};for(const n of t)n in e&&(s[n]=e[n]);return s}function hd(e){const t={path:e.path,redirect:e.redirect,name:e.name,meta:e.meta||{},aliasOf:e.aliasOf,beforeEnter:e.beforeEnter,props:g_(e),children:e.children||[],instances:{},leaveGuards:new Set,updateGuards:new Set,enterCallbacks:{},components:"components"in e?e.components||null:e.component&&{default:e.component}};return Object.defineProperty(t,"mods",{value:{}}),t}function g_(e){const t={},s=e.props||!1;if("component"in e)t.default=s;else for(const n in e.components)t[n]=typeof s=="object"?s[n]:s;return t}function md(e){for(;e;){if(e.record.aliasOf)return!0;e=e.parent}return!1}function v_(e){return e.reduce((t,s)=>Ge(t,s.meta),{})}function b_(e,t){let s=0,n=t.length;for(;s!==n;){const i=s+n>>1;Zh(e,t[i])<0?n=i:s=i+1}const a=y_(e);return a&&(n=t.lastIndexOf(a,n-1)),n}function y_(e){let t=e;for(;t=t.parent;)if(Jh(t)&&Zh(e,t)===0)return t}function Jh({record:e}){return!!(e.name||e.components&&Object.keys(e.components).length||e.redirect)}function gd(e){const t=ps(or),s=ps(Sc),n=Y(()=>{const o=Ns(e.to);return t.resolve(o)}),a=Y(()=>{const{matched:o}=n.value,{length:c}=o,u=o[c-1],d=s.matched;if(!u||!d.length)return-1;const f=d.findIndex(Aa.bind(null,u));if(f>-1)return f;const p=vd(o[c-2]);return c>1&&vd(u)===p&&d[d.length-1].path!==p?d.findIndex(Aa.bind(null,o[c-2])):f}),i=Y(()=>a.value>-1&&S_(s.params,n.value.params)),l=Y(()=>a.value>-1&&a.value===s.matched.length-1&&Kh(s.params,n.value.params));function r(o={}){if(w_(o)){const c=t[Ns(e.replace)?"replace":"push"](Ns(e.to)).catch(ni);return e.viewTransition&&typeof document<"u"&&"startViewTransition"in document&&document.startViewTransition(()=>c),c}return Promise.resolve()}return{route:n,href:Y(()=>n.value.href),isActive:i,isExactActive:l,navigate:r}}function x_(e){return e.length===1?e[0]:e}const __=Ai({name:"RouterLink",compatConfig:{MODE:3},props:{to:{type:[String,Object],required:!0},replace:Boolean,activeClass:String,exactActiveClass:String,custom:Boolean,ariaCurrentValue:{type:String,default:"page"},viewTransition:Boolean},useLink:gd,setup(e,{slots:t}){const s=_n(gd(e)),{options:n}=ps(or),a=Y(()=>({[bd(e.activeClass,n.linkActiveClass,"router-link-active")]:s.isActive,[bd(e.exactActiveClass,n.linkExactActiveClass,"router-link-exact-active")]:s.isExactActive}));return()=>{const i=t.default&&x_(t.default(s));return e.custom?i:_a("a",{"aria-current":s.isExactActive?e.ariaCurrentValue:null,href:s.href,onClick:s.navigate,class:a.value},i)}}}),k_=__;function w_(e){if(!(e.metaKey||e.altKey||e.ctrlKey||e.shiftKey)&&!e.defaultPrevented&&!(e.button!==void 0&&e.button!==0)){if(e.currentTarget&&e.currentTarget.getAttribute){const t=e.currentTarget.getAttribute("target");if(/\b_blank\b/i.test(t))return}return e.preventDefault&&e.preventDefault(),!0}}function S_(e,t){for(const s in t){const n=t[s],a=e[s];if(typeof n=="string"){if(n!==a)return!1}else if(!ks(a)||a.length!==n.length||n.some((i,l)=>i.valueOf()!==a[l].valueOf()))return!1}return!0}function vd(e){return e?e.aliasOf?e.aliasOf.path:e.path:""}const bd=(e,t,s)=>e??t??s,T_=Ai({name:"RouterView",inheritAttrs:!1,props:{name:{type:String,default:"default"},route:Object},compatConfig:{MODE:3},setup(e,{attrs:t,slots:s}){const n=ps(bo),a=Y(()=>e.route||n.value),i=ps(cd,0),l=Y(()=>{let c=Ns(i);const{matched:u}=a.value;let d;for(;(d=u[c])&&!d.components;)c++;return c}),r=Y(()=>a.value.matched[l.value]);Qa(cd,Y(()=>l.value+1)),Qa(Xx,r),Qa(bo,a);const o=h();return hs(()=>[o.value,r.value,e.name],([c,u,d],[f,p,b])=>{u&&(u.instances[d]=c,p&&p!==u&&c&&c===f&&(u.leaveGuards.size||(u.leaveGuards=p.leaveGuards),u.updateGuards.size||(u.updateGuards=p.updateGuards))),c&&u&&(!p||!Aa(u,p)||!f)&&(u.enterCallbacks[d]||[]).forEach(m=>m(c))},{flush:"post"}),()=>{const c=a.value,u=e.name,d=r.value,f=d&&d.components[u];if(!f)return yd(s.default,{Component:f,route:c});const p=d.props[u],b=p?p===!0?c.params:typeof p=="function"?p(c):p:null,k=_a(f,Ge({},b,t,{onVnodeUnmounted:E=>{E.component.isUnmounted&&(d.instances[u]=null)},ref:o}));return yd(s.default,{Component:k,route:c})||k}}});function yd(e,t){if(!e)return null;const s=e(t);return s.length===1?s[0]:s}const C_=T_;function E_(e){const t=m_(e.routes,e),s=e.parseQuery||Yx,n=e.stringifyQuery||od,a=e.history,i=Ha(),l=Ha(),r=Ha(),o=Oo(dn);let c=dn;la&&e.scrollBehavior&&"scrollRestoration"in history&&(history.scrollRestoration="manual");const u=Rr.bind(null,j=>""+j),d=Rr.bind(null,Dx),f=Rr.bind(null,wi);function p(j,oe){let ie,ge;return qh(j)?(ie=t.getRecordMatcher(j),ge=oe):ge=j,t.addRoute(ge,ie)}function b(j){const oe=t.getRecordMatcher(j);oe&&t.removeRoute(oe)}function m(){return t.getRoutes().map(j=>j.record)}function k(j){return!!t.getRecordMatcher(j)}function E(j,oe){if(oe=Ge({},oe||o.value),typeof j=="string"){const C=Ir(s,j,oe.path),M=t.resolve({path:C.path},oe),Z=a.createHref(C.fullPath);return Ge(C,M,{params:f(M.params),hash:wi(C.hash),redirectedFrom:void 0,href:Z})}let ie;if(j.path!=null)ie=Ge({},j,{path:Ir(s,j.path,oe.path).path});else{const C=Ge({},j.params);for(const M in C)C[M]==null&&delete C[M];ie=Ge({},j,{params:d(C)}),oe.params=d(oe.params)}const ge=t.resolve(ie,oe),be=j.hash||"";ge.params=u(f(ge.params));const Oe=Fx(n,Ge({},j,{hash:Nx(be),path:ge.path})),g=a.createHref(Oe);return Ge({fullPath:Oe,hash:be,query:n===od?Qx(j.query):j.query||{}},ge,{redirectedFrom:void 0,href:g})}function v(j){return typeof j=="string"?Ir(s,j,o.value.path):Ge({},j)}function y(j,oe){if(c!==j)return Ra(ct.NAVIGATION_CANCELLED,{from:oe,to:j})}function x(j){return O(j)}function T(j){return x(Ge(v(j),{replace:!0}))}function N(j,oe){const ie=j.matched[j.matched.length-1];if(ie&&ie.redirect){const{redirect:ge}=ie;let be=typeof ge=="function"?ge(j,oe):ge;return typeof be=="string"&&(be=be.includes("?")||be.includes("#")?be=v(be):{path:be},be.params={}),Ge({query:j.query,hash:j.hash,params:be.path!=null?{}:j.params},be)}}function O(j,oe){const ie=c=E(j),ge=o.value,be=j.state,Oe=j.force,g=j.replace===!0,C=N(ie,ge);if(C)return O(Ge(v(C),{state:typeof C=="object"?Ge({},be,C.state):be,force:Oe,replace:g}),oe||ie);const M=ie;M.redirectedFrom=oe;let Z;return!Oe&&$x(n,ge,ie)&&(Z=Ra(ct.NAVIGATION_DUPLICATED,{to:M,from:ge}),de(ge,ge,!0,!1)),(Z?Promise.resolve(Z):L(M,ge)).catch(A=>$s(A)?$s(A,ct.NAVIGATION_GUARD_REDIRECT)?A:ce(A):I(A,M,ge)).then(A=>{if(A){if($s(A,ct.NAVIGATION_GUARD_REDIRECT))return O(Ge({replace:g},v(A.to),{state:typeof A.to=="object"?Ge({},be,A.to.state):be,force:Oe}),oe||M)}else A=P(M,ge,!0,g,be);return U(M,ge,A),A})}function w(j,oe){const ie=y(j,oe);return ie?Promise.reject(ie):Promise.resolve()}function R(j){const oe=ee.values().next().value;return oe&&typeof oe.runWithContext=="function"?oe.runWithContext(j):j()}function L(j,oe){let ie;const[ge,be,Oe]=e_(j,oe);ie=Or(ge.reverse(),"beforeRouteLeave",j,oe);for(const C of ge)C.leaveGuards.forEach(M=>{ie.push(vn(M,j,oe))});const g=w.bind(null,j,oe);return ie.push(g),Ie(ie).then(()=>{ie=[];for(const C of i.list())ie.push(vn(C,j,oe));return ie.push(g),Ie(ie)}).then(()=>{ie=Or(be,"beforeRouteUpdate",j,oe);for(const C of be)C.updateGuards.forEach(M=>{ie.push(vn(M,j,oe))});return ie.push(g),Ie(ie)}).then(()=>{ie=[];for(const C of Oe)if(C.beforeEnter)if(ks(C.beforeEnter))for(const M of C.beforeEnter)ie.push(vn(M,j,oe));else ie.push(vn(C.beforeEnter,j,oe));return ie.push(g),Ie(ie)}).then(()=>(j.matched.forEach(C=>C.enterCallbacks={}),ie=Or(Oe,"beforeRouteEnter",j,oe,R),ie.push(g),Ie(ie))).then(()=>{ie=[];for(const C of l.list())ie.push(vn(C,j,oe));return ie.push(g),Ie(ie)}).catch(C=>$s(C,ct.NAVIGATION_CANCELLED)?C:Promise.reject(C))}function U(j,oe,ie){r.list().forEach(ge=>R(()=>ge(j,oe,ie)))}function P(j,oe,ie,ge,be){const Oe=y(j,oe);if(Oe)return Oe;const g=oe===dn,C=la?history.state:{};ie&&(ge||g?a.replace(j.fullPath,Ge({scroll:g&&C&&C.scroll},be)):a.push(j.fullPath,be)),o.value=j,de(j,oe,ie,g),ce()}let S;function $(){S||(S=a.listen((j,oe,ie)=>{if(!ue.listening)return;const ge=E(j),be=N(ge,ue.currentRoute.value);if(be){O(Ge(be,{replace:!0,force:!0}),ge).catch(ni);return}c=ge;const Oe=o.value;la&&qx(rd(Oe.fullPath,ie.delta),rr()),L(ge,Oe).catch(g=>$s(g,ct.NAVIGATION_ABORTED|ct.NAVIGATION_CANCELLED)?g:$s(g,ct.NAVIGATION_GUARD_REDIRECT)?(O(Ge(v(g.to),{force:!0}),ge).then(C=>{$s(C,ct.NAVIGATION_ABORTED|ct.NAVIGATION_DUPLICATED)&&!ie.delta&&ie.type===go.pop&&a.go(-1,!1)}).catch(ni),Promise.reject()):(ie.delta&&a.go(-ie.delta,!1),I(g,ge,Oe))).then(g=>{g=g||P(ge,Oe,!1),g&&(ie.delta&&!$s(g,ct.NAVIGATION_CANCELLED)?a.go(-ie.delta,!1):ie.type===go.pop&&$s(g,ct.NAVIGATION_ABORTED|ct.NAVIGATION_DUPLICATED)&&a.go(-1,!1)),U(ge,Oe,g)}).catch(ni)}))}let W=Ha(),G=Ha(),D;function I(j,oe,ie){ce(j);const ge=G.list();return ge.length?ge.forEach(be=>be(j,oe,ie)):console.error(j),Promise.reject(j)}function z(){return D&&o.value!==dn?Promise.resolve():new Promise((j,oe)=>{W.add([j,oe])})}function ce(j){return D||(D=!j,$(),W.list().forEach(([oe,ie])=>j?ie(j):oe()),W.reset()),j}function de(j,oe,ie,ge){const{scrollBehavior:be}=e;if(!la||!be)return Promise.resolve();const Oe=!ie&&Gx(rd(j.fullPath,0))||(ge||!ie)&&history.state&&history.state.scroll||null;return Et().then(()=>be(j,oe,Oe)).then(g=>g&&Kx(g)).catch(g=>I(g,j,oe))}const se=j=>a.go(j);let pe;const ee=new Set,ue={currentRoute:o,listening:!0,addRoute:p,removeRoute:b,clearRoutes:t.clearRoutes,hasRoute:k,getRoutes:m,resolve:E,options:e,push:x,replace:T,go:se,back:()=>se(-1),forward:()=>se(1),beforeEach:i.add,beforeResolve:l.add,afterEach:r.add,onError:G.add,isReady:z,install(j){j.component("RouterLink",k_),j.component("RouterView",C_),j.config.globalProperties.$router=ue,Object.defineProperty(j.config.globalProperties,"$route",{enumerable:!0,get:()=>Ns(o)}),la&&!pe&&o.value===dn&&(pe=!0,x(a.location).catch(ge=>{}));const oe={};for(const ge in dn)Object.defineProperty(oe,ge,{get:()=>o.value[ge],enumerable:!0});j.provide(or,ue),j.provide(Sc,No(oe)),j.provide(bo,o);const ie=j.unmount;ee.add(j),j.unmount=function(){ee.delete(j),ee.size<1&&(c=dn,S&&S(),S=null,o.value=dn,pe=!1,D=!1),ie()}}};function Ie(j){return j.reduce((oe,ie)=>oe.then(()=>R(ie)),Promise.resolve())}return ue}function Yh(){return ps(or)}function A_(e){return ps(Sc)}const R_=[{group:"Workspace",label:"Dashboard",icon:"dashboard",to:{path:"/dashboard"}},{group:"Workspace",label:"Chat",icon:"chat",to:{path:"/chat"}},...["Live","Agents","Loops","Processes","Schedules"].map(e=>({group:"Operations",label:e,icon:"operations",to:{path:"/operations",query:{tab:e.toLowerCase()}}})),...["Audit","Sessions","Traces","Usage"].map(e=>({group:"History",label:e,icon:"history",to:{path:"/history",query:{tab:e.toLowerCase()}}})),...["Tools","Skills","Knowledge","Memory","Learned"].map(e=>({group:"Capabilities",label:e,icon:"capabilities",to:{path:"/capabilities",query:{tab:e.toLowerCase()}}})),{group:"Manage",label:"Personality",icon:"personality",to:{path:"/personality"}},...[["Health","health"],["Resources","resources"],["Logs","logs"],["Config","config"],["Discord","discord"],["Host Access","host-access"],["API Tokens","api-tokens"],["LLM Config","llm"],["Internals","internals"],["Update","update"]].map(([e,t])=>({group:"System",label:e,icon:"system",to:{path:"/system",query:{tab:t}}}))],Yt=_n({open:!1,query:"",selected:0});function xd(){Yt.query="",Yt.selected=0,Yt.open=!0}function Lr(){Yt.open=!1}function I_(e,t){const s=e.label.toLowerCase(),n=`${e.group} ${e.label}`.toLowerCase();return t?s.startsWith(t)?100:n.startsWith(t)?80:s.includes(t)?60:n.includes(t)?40:0:1}const N_={setup(){const e=Yh(),t=h(null),s=Y(()=>{const i=Yt.query.trim().toLowerCase();return R_.map(l=>({...l,_score:I_(l,i)})).filter(l=>l._score>0).sort((l,r)=>r._score-l._score)});hs(()=>Yt.open,async i=>{var l;i&&(await Et(),(l=t.value)==null||l.focus())}),hs(()=>Yt.query,()=>{Yt.selected=0});function n(i){Lr(),e.push(i.to)}function a(i){if(i.key==="Escape"){i.preventDefault(),Lr();return}if(i.key==="ArrowDown")i.preventDefault(),Yt.selected=Math.min(Yt.selected+1,s.value.length-1);else if(i.key==="ArrowUp")i.preventDefault(),Yt.selected=Math.max(Yt.selected-1,0);else if(i.key==="Enter"){i.preventDefault();const l=s.value[Yt.selected];l&&n(l)}}return{state:Yt,results:s,inputEl:t,go:n,onKeydown:a,closePalette:Lr}},template:`
    <transition name="modal">
...
... (truncated)`:s}function kd(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function em(e){return e==null||!isFinite(e)?"—":Number(e).toLocaleString()}function M_(e){return e==null||!isFinite(e)?"—":e>=1e3?`${(e/1e3).toFixed(1)}k`:String(e)}const P_={template:`
    ...
    </div>`,setup(){const e=h({}),t=h(!0),s=h(null),n=h([]),a=h(!1),i=h([]),l=h(!1),r=h([]),o=h(0),c=h(null),u=h({reload:!1,clearSessions:!1,stopLoops:!1});let d=0;const f=Y(()=>{const S=e.value.uptime_seconds||0,$=Math.floor(S/86400),W=Math.floor(S%86400/3600),G=Math.floor(S%3600/60),D=[];return $>0&&D.push(`${$}d`),W>0&&D.push(`${W}h`),(D.length===0||$===0&&W===0)&&D.push(`${G}m`),D.join(" ")}),p=Y(()=>{const S=e.value.uptime_seconds||0;return 125.66*(1-Math.min(S/86400,1))}),b=Y(()=>{const S=e.value;return[{label:"Guilds",value:S.guild_count??0,icon:"home",iconColor:"text-blue-400"},{label:"Sessions",value:S.session_count??0,icon:"message",iconColor:"text-yellow-400"},{label:"Tools",value:S.tool_count??0,icon:"wrench",iconColor:"text-purple-400",sub:`${S.skill_count??0} skills`,subColor:"text-gray-500"},{label:"Loops",value:S.loop_count??0,icon:"rotate",iconColor:"text-green-400",color:S.loop_count>0?"text-green-400":"",highlight:S.loop_count>0},{label:"Agents",value:S.agent_running??0,icon:"bot",iconColor:"text-cyan-400",sub:S.agent_count>0?`${S.agent_count} total`:"",subColor:"text-gray-500",highlight:(S.agent_running??0)>0},{label:"Processes",value:S.process_running??0,icon:"sliders",iconColor:"text-orange-400",sub:S.process_count>0?`${S.process_count} total`:"",subColor:"text-gray-500",highlight:(S.process_running??0)>0},{label:"Schedules",value:S.schedule_count??0,icon:"clock",iconColor:"text-amber-400",sub:(S.schedule_failing>0?`${S.schedule_failing} failing`:"")+(S.schedule_failing>0&&S.schedule_paused>0?", ":"")+(S.schedule_paused>0?`${S.schedule_paused} paused`:"")||void 0,subColor:S.schedule_failing>0?"text-red-400":"text-yellow-400",color:S.schedule_failing>0?"text-red-400":"",highlight:S.schedule_failing>0},{label:"Users",value:S.user_count??0,icon:"users",iconColor:"text-indigo-400"},...c.value!==null?[{label:"Knowledge",value:c.value,icon:"book",iconColor:"text-teal-400",sub:"chunks",subColor:"text-gray-500"}]:[]]}),m=Y(()=>{const S=e.value,$=[];return $.push({label:"Bot",status:S.status==="online"?"ok":"warn",detail:S.status==="online"?"Online":"Starting"}),(S.schedule_failing||0)>0?$.push({label:"Schedules",status:"error",detail:`${S.schedule_failing} failing`}):(S.schedule_count||0)>0&&$.push({label:"Schedules",status:"ok",detail:`${S.schedule_count} configured`}),(S.loop_count||0)>0&&$.push({label:"Loops",status:"ok",detail:`${S.loop_count} active`}),(S.agent_running||0)>0&&$.push({label:"Agents",status:"ok",detail:`${S.agent_running} running`}),(S.process_running||0)>0&&$.push({label:"Processes",status:"ok",detail:`${S.process_running} running`}),$});async function k(){try{e.value=await q.get("/api/status"),s.value=null}catch(S){s.value=S.message}finally{t.value=!1}}async function E(){a.value=!0;try{n.value=await q.get("/api/audit?limit=10"),o.value=0}catch{}a.value=!1}async function v(){l.value=!0;try{i.value=await q.get("/api/audit?error_only=1&limit=5")}catch{}l.value=!1}async function y(){try{const S=await q.get("/api/knowledge");c.value=(Array.isArray(S)?S:[]).reduce(($,W)=>$+(W.chunks||0),0)}catch{c.value=null}}async function x(){try{const S=await q.get("/api/agents");r.value=S.filter($=>$.status==="running")}catch{}}async function T(){u.value={...u.value,reload:!0};try{await q.post("/api/reload"),ke.success("Config reloaded")}catch(S){ke.error(S.message)}u.value={...u.value,reload:!1}}async function N(){if(!await ls({title:"Clear all sessions",message:"Clear all conversation sessions? This cannot be undone.",confirmLabel:"Clear All",danger:!0}))return;u.value={...u.value,clearSessions:!0};const $=e.value.session_count;e.value={...e.value,session_count:0};try{const W=await q.post("/api/sessions/clear-all");ke.success(`Cleared ${W.count} session${W.count!==1?"s":""}`),await k()}catch(W){e.value={...e.value,session_count:$},ke.error(W.message)}u.value={...u.value,clearSessions:!1}}async function O(){if(!await ls({title:"Stop all loops",message:"Stop all running loops?",confirmLabel:"Stop Loops",danger:!0}))return;u.value={...u.value,stopLoops:!0};const $=e.value.loop_count;e.value={...e.value,loop_count:0};try{const W=await q.post("/api/loops/stop-all");ke.success(W.result),await k()}catch(W){e.value={...e.value,loop_count:$},ke.error(W.message)}u.value={...u.value,stopLoops:!1}}function w(){t.value=!0,s.value=null,k(),E(),v(),x()}let R=null,L=null,U=null;function P(S){if(S.payload&&S.payload.tool_name){const $={...S.payload,_isNew:!0,_key:++d};n.value.unshift($),n.value.length>10&&n.value.pop(),o.value++,$.error&&(i.value.unshift($),i.value.length>5&&i.value.pop()),setTimeout(()=>{$._isNew=!1},1500),clearTimeout(U),U=setTimeout(()=>{o.value=0},1e4)}}return Be(async()=>{await Promise.all([k(),E(),v(),x(),y()]),R=setInterval(k,15e3),L=setInterval(x,1e4),We.subscribe("events",P)}),ht(()=>{R&&clearInterval(R),L&&clearInterval(L),clearTimeout(U),We.unsubscribe("events",P)}),{status:e,loading:t,error:s,uptime:f,uptimeRingOffset:p,stats:b,healthIndicators:m,activity:n,activityLoading:a,newEventCount:o,errors:i,errorsLoading:l,agents:r,actionLoading:u,fetchActivity:E,fetchStatus:k,formatTime:Cc,formatDuration:Ia,retry:w,reloadConfig:T,clearSessions:N,stopAllLoops:O}}};/*! @license DOMPurify 3.4.9 | (c) Cure53 and other contributors | Released under the Apache license 2.0 and Mozilla Public License 2.0 | github.com/cure53/DOMPurify/blob/3.4.9/LICENSE */function wd(e,t){(t==null||t>e.length)&&(t=e.length);for(var s=0,n=Array(t);s<t;s++)n[s]=e[s];return n}function F_(e){if(Array.isArray(e))return e}function $_(e,t){var s=e==null?null:typeof Symbol<"u"&&e[Symbol.iterator]||e["@@iterator"];if(s!=null){var n,a,i,l,r=[],o=!0,c=!1;try{if(i=(s=s.call(e)).next,t!==0)for(;!(o=(n=i.call(s)).done)&&(r.push(n.value),r.length!==t);o=!0);}catch(u){c=!0,a=u}finally{try{if(!o&&s.return!=null&&(l=s.return(),Object(l)!==l))return}finally{if(c)throw a}}return r}}function U_(){throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`)}function B_(e,t){return F_(e)||$_(e,t)||H_(e,t)||U_()}function H_(e,t){if(e){if(typeof e=="string")return wd(e,t);var s={}.toString.call(e).slice(8,-1);return s==="Object"&&e.constructor&&(s=e.constructor.name),s==="Map"||s==="Set"?Array.from(e):s==="Arguments"||/^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(s)?wd(e,t):void 0}}const tm=Object.entries,Sd=Object.setPrototypeOf,V_=Object.isFrozen,j_=Object.getPrototypeOf,z_=Object.getOwnPropertyDescriptor;let Gt=Object.freeze,gs=Object.seal,ra=Object.create,sm=typeof Reflect<"u"&&Reflect,xo=sm.apply,_o=sm.construct;Gt||(Gt=function(t){return t});gs||(gs=function(t){return t});xo||(xo=function(t,s){for(var n=arguments.length,a=new Array(n>2?n-2:0),i=2;i<n;i++)a[i-2]=arguments[i];return t.apply(s,a)});_o||(_o=function(t){for(var s=arguments.length,n=new Array(s>1?s-1:0),a=1;a<s;a++)n[a-1]=arguments[a];return new t(...n)});const Us=yt(Array.prototype.forEach),K_=yt(Array.prototype.lastIndexOf),Td=yt(Array.prototype.pop),sa=yt(Array.prototype.push),q_=yt(Array.prototype.splice),Vt=Array.isArray,Za=yt(String.prototype.toLowerCase),Dr=yt(String.prototype.toString),Cd=yt(String.prototype.match),na=yt(String.prototype.replace),Ed=yt(String.prototype.indexOf),G_=yt(String.prototype.trim),W_=yt(Number.prototype.toString),Z_=yt(Boolean.prototype.toString),Ad=typeof BigInt>"u"?null:yt(BigInt.prototype.toString),Rd=typeof Symbol>"u"?null:yt(Symbol.prototype.toString),ot=yt(Object.prototype.hasOwnProperty),Va=yt(Object.prototype.toString),Rt=yt(RegExp.prototype.test),An=J_(TypeError);function yt(e){return function(t){t instanceof RegExp&&(t.lastIndex=0);for(var s=arguments.length,n=new Array(s>1?s-1:0),a=1;a<s;a++)n[a-1]=arguments[a];return xo(e,t,n)}}function J_(e){return function(){for(var t=arguments.length,s=new Array(t),n=0;n<t;n++)s[n]=arguments[n];return _o(e,s)}}function De(e,t){let s=arguments.length>2&&arguments[2]!==void 0?arguments[2]:Za;if(Sd&&Sd(e,null),!Vt(t))return e;let n=t.length;for(;n--;){let a=t[n];if(typeof a=="string"){const i=s(a);i!==a&&(V_(t)||(t[n]=i),a=i)}e[a]=!0}return e}function Y_(e){for(let t=0;t<e.length;t++)ot(e,t)||(e[t]=null);return e}function Mt(e){const t=ra(null);for(const n of tm(e)){var s=B_(n,2);const a=s[0],i=s[1];ot(e,a)&&(Vt(i)?t[a]=Y_(i):i&&typeof i=="object"&&i.constructor===Object?t[a]=Mt(i):t[a]=i)}return t}function Q_(e){switch(typeof e){case"string":return e;case"number":return W_(e);case"boolean":return Z_(e);case"bigint":return Ad?Ad(e):"0";case"symbol":return Rd?Rd(e):"Symbol()";case"undefined":return Va(e);case"function":case"object":{if(e===null)return Va(e);const t=e,s=Cs(t,"toString");if(typeof s=="function"){const n=s(t);return typeof n=="string"?n:Va(n)}return Va(e)}default:return Va(e)}}function Cs(e,t){for(;e!==null;){const n=z_(e,t);if(n){if(n.get)return yt(n.get);if(typeof n.value=="function")return yt(n.value)}e=j_(e)}function s(){return null}return s}function X_(e){try{return Rt(e,""),!0}catch{return!1}}const Id=Gt(["a","abbr","acronym","address","area","article","aside","audio","b","bdi","bdo","big","blink","blockquote","body","br","button","canvas","caption","center","cite","code","col","colgroup","content","data","datalist","dd","decorator","del","details","dfn","dialog","dir","div","dl","dt","element","em","fieldset","figcaption","figure","font","footer","form","h1","h2","h3","h4","h5","h6","head","header","hgroup","hr","html","i","img","input","ins","kbd","label","legend","li","main","map","mark","marquee","menu","menuitem","meter","nav","nobr","ol","optgroup","option","output","p","picture","pre","progress","q","rp","rt","ruby","s","samp","search","section","select","shadow","slot","small","source","spacer","span","strike","strong","style","sub","summary","sup","table","tbody","td","template","textarea","tfoot","th","thead","time","tr","track","tt","u","ul","var","video","wbr"]),Mr=Gt(["svg","a","altglyph","altglyphdef","altglyphitem","animatecolor","animatemotion","animatetransform","circle","clippath","defs","desc","ellipse","enterkeyhint","exportparts","filter","font","g","glyph","glyphref","hkern","image","inputmode","line","lineargradient","marker","mask","metadata","mpath","part","path","pattern","polygon","polyline","radialgradient","rect","stop","style","switch","symbol","text","textpath","title","tref","tspan","view","vkern"]),Pr=Gt(["feBlend","feColorMatrix","feComponentTransfer","feComposite","feConvolveMatrix","feDiffuseLighting","feDisplacementMap","feDistantLight","feDropShadow","feFlood","feFuncA","feFuncB","feFuncG","feFuncR","feGaussianBlur","feImage","feMerge","feMergeNode","feMorphology","feOffset","fePointLight","feSpecularLighting","feSpotLight","feTile","feTurbulence"]),ek=Gt(["animate","color-profile","cursor","discard","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","foreignobject","hatch","hatchpath","mesh","meshgradient","meshpatch","meshrow","missing-glyph","script","set","solidcolor","unknown","use"]),Fr=Gt(["math","menclose","merror","mfenced","mfrac","mglyph","mi","mlabeledtr","mmultiscripts","mn","mo","mover","mpadded","mphantom","mroot","mrow","ms","mspace","msqrt","mstyle","msub","msup","msubsup","mtable","mtd","mtext","mtr","munder","munderover","mprescripts"]),tk=Gt(["maction","maligngroup","malignmark","mlongdiv","mscarries","mscarry","msgroup","mstack","msline","msrow","semantics","annotation","annotation-xml","mprescripts","none"]),Nd=Gt(["#text"]),Od=Gt(["accept","action","align","alt","autocapitalize","autocomplete","autopictureinpicture","autoplay","background","bgcolor","border","capture","cellpadding","cellspacing","checked","cite","class","clear","color","cols","colspan","command","commandfor","controls","controlslist","coords","crossorigin","datetime","decoding","default","dir","disabled","disablepictureinpicture","disableremoteplayback","download","draggable","enctype","enterkeyhint","exportparts","face","for","headers","height","hidden","high","href","hreflang","id","inert","inputmode","integrity","ismap","kind","label","lang","list","loading","loop","low","max","maxlength","media","method","min","minlength","multiple","muted","name","nonce","noshade","novalidate","nowrap","open","optimum","part","pattern","placeholder","playsinline","popover","popovertarget","popovertargetaction","poster","preload","pubdate","radiogroup","readonly","rel","required","rev","reversed","role","rows","rowspan","spellcheck","scope","selected","shape","size","sizes","slot","span","srclang","start","src","srcset","step","style","summary","tabindex","title","translate","type","usemap","valign","value","width","wrap","xmlns"]),$r=Gt(["accent-height","accumulate","additive","alignment-baseline","amplitude","ascent","attributename","attributetype","azimuth","basefrequency","baseline-shift","begin","bias","by","class","clip","clippathunits","clip-path","clip-rule","color","color-interpolation","color-interpolation-filters","color-profile","color-rendering","cx","cy","d","dx","dy","diffuseconstant","direction","display","divisor","dur","edgemode","elevation","end","exponent","fill","fill-opacity","fill-rule","filter","filterunits","flood-color","flood-opacity","font-family","font-size","font-size-adjust","font-stretch","font-style","font-variant","font-weight","fx","fy","g1","g2","glyph-name","glyphref","gradientunits","gradienttransform","height","href","id","image-rendering","in","in2","intercept","k","k1","k2","k3","k4","kerning","keypoints","keysplines","keytimes","lang","lengthadjust","letter-spacing","kernelmatrix","kernelunitlength","lighting-color","local","marker-end","marker-mid","marker-start","markerheight","markerunits","markerwidth","maskcontentunits","maskunits","max","mask","mask-type","media","method","mode","min","name","numoctaves","offset","operator","opacity","order","orient","orientation","origin","overflow","paint-order","path","pathlength","patterncontentunits","patterntransform","patternunits","points","preservealpha","preserveaspectratio","primitiveunits","r","rx","ry","radius","refx","refy","repeatcount","repeatdur","restart","result","rotate","scale","seed","shape-rendering","slope","specularconstant","specularexponent","spreadmethod","startoffset","stddeviation","stitchtiles","stop-color","stop-opacity","stroke-dasharray","stroke-dashoffset","stroke-linecap","stroke-linejoin","stroke-miterlimit","stroke-opacity","stroke","stroke-width","style","surfacescale","systemlanguage","tabindex","tablevalues","targetx","targety","transform","transform-origin","text-anchor","text-decoration","text-rendering","textlength","type","u1","u2","unicode","values","viewbox","visibility","version","vert-adv-y","vert-origin-x","vert-origin-y","width","word-spacing","wrap","writing-mode","xchannelselector","ychannelselector","x","x1","x2","xmlns","y","y1","y2","z","zoomandpan"]),Ld=Gt(["accent","accentunder","align","bevelled","close","columnalign","columnlines","columnspacing","columnspan","denomalign","depth","dir","display","displaystyle","encoding","fence","frame","height","href","id","largeop","length","linethickness","lquote","lspace","mathbackground","mathcolor","mathsize","mathvariant","maxsize","minsize","movablelimits","notation","numalign","open","rowalign","rowlines","rowspacing","rowspan","rspace","rquote","scriptlevel","scriptminsize","scriptsizemultiplier","selection","separator","separators","stretchy","subscriptshift","supscriptshift","symmetric","voffset","width","xmlns"]),Ji=Gt(["xlink:href","xml:id","xlink:title","xml:space","xmlns:xlink"]),sk=gs(/{{[\w\W]*|^[\w\W]*}}/g),nk=gs(/<%[\w\W]*|^[\w\W]*%>/g),ak=gs(/\${[\w\W]*/g),ik=gs(/^data-[\-\w.\u00B7-\uFFFF]+$/),lk=gs(/^aria-[\-\w]+$/),Dd=gs(/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|sms|cid|xmpp|matrix):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i),rk=gs(/^(?:\w+script|data):/i),ok=gs(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g),ck=gs(/^html$/i),uk=gs(/^[a-z][.\w]*(-[.\w]+)+$/i),Ss={element:1,attribute:2,text:3,cdataSection:4,entityReference:5,entityNode:6,progressingInstruction:7,comment:8,document:9,documentType:10,documentFragment:11,notation:12},dk=function(){return typeof window>"u"?null:window},fk=function(t,s){if(typeof t!="object"||typeof t.createPolicy!="function")return null;let n=null;const a="data-tt-policy-suffix";s&&s.hasAttribute(a)&&(n=s.getAttribute(a));const i="dompurify"+(n?"#"+n:"");try{return t.createPolicy(i,{createHTML(l){return l},createScriptURL(l){return l}})}catch{return console.warn("TrustedTypes policy "+i+" could not be created."),null}},Md=function(){return{afterSanitizeAttributes:[],afterSanitizeElements:[],afterSanitizeShadowDOM:[],beforeSanitizeAttributes:[],beforeSanitizeElements:[],beforeSanitizeShadowDOM:[],uponSanitizeAttribute:[],uponSanitizeElement:[],uponSanitizeShadowNode:[]}};function nm(){let e=arguments.length>0&&arguments[0]!==void 0?arguments[0]:dk();const t=ve=>nm(ve);if(t.version="3.4.9",t.removed=[],!e||!e.document||e.document.nodeType!==Ss.document||!e.Element)return t.isSupported=!1,t;let s=e.document;const n=s,a=n.currentScript;e.DocumentFragment;const i=e.HTMLTemplateElement,l=e.Node,r=e.Element,o=e.NodeFilter,c=e.NamedNodeMap;c===void 0&&(e.NamedNodeMap||e.MozNamedAttrMap),e.HTMLFormElement;const u=e.DOMParser,d=e.trustedTypes,f=r.prototype,p=Cs(f,"cloneNode"),b=Cs(f,"remove"),m=Cs(f,"nextSibling"),k=Cs(f,"childNodes"),E=Cs(f,"parentNode"),v=Cs(f,"shadowRoot"),y=Cs(f,"attributes"),x=l&&l.prototype?Cs(l.prototype,"nodeType"):null,T=l&&l.prototype?Cs(l.prototype,"nodeName"):null;if(typeof i=="function"){const ve=s.createElement("template");ve.content&&ve.content.ownerDocument&&(s=ve.content.ownerDocument)}let N,O="",w,R=!1,L=0;const U=function(){if(L>0)throw An('A configured TRUSTED_TYPES_POLICY callback (createHTML or createScriptURL) must not call DOMPurify.sanitize, as that causes infinite recursion. Do not pass a policy whose callbacks wrap DOMPurify as TRUSTED_TYPES_POLICY; see the "DOMPurify and Trusted Types" section of the README.')},P=function(_){U(),L++;try{return N.createHTML(_)}finally{L--}},S=function(_){U(),L++;try{return N.createScriptURL(_)}finally{L--}},$=function(){return R||(w=fk(d,a),R=!0),w},W=s,G=W.implementation,D=W.createNodeIterator,I=W.createDocumentFragment,z=W.getElementsByTagName,ce=n.importNode;let de=Md();t.isSupported=typeof tm=="function"&&typeof E=="function"&&G&&G.createHTMLDocument!==void 0;const se=sk,pe=nk,ee=ak,ue=ik,Ie=lk,j=rk,oe=ok,ie=uk;let ge=Dd,be=null;const Oe=De({},[...Id,...Mr,...Pr,...Fr,...Nd]);let g=null;const C=De({},[...Od,...$r,...Ld,...Ji]);let M=Object.seal(ra(null,{tagNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},allowCustomizedBuiltInElements:{writable:!0,configurable:!1,enumerable:!0,value:!1}})),Z=null,A=null;const F=Object.seal(ra(null,{tagCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeCheck:{writable:!0,configurable:!1,enumerable:!0,value:null}}));let J=!0,X=!0,te=!1,Q=!0,me=!1,le=!0,fe=!1,xe=!1,Se=!1,Ae=!1,B=!1,re=!1,ye=!0,Le=!1;const He="user-content-";let Je=!0,xt=!1,et={},Ye=null;const ws=De({},["annotation-xml","audio","colgroup","desc","foreignobject","head","iframe","math","mi","mn","mo","ms","mtext","noembed","noframes","noscript","plaintext","script","selectedcontent","style","svg","template","thead","title","video","xmp"]);let Ds=null;const Zn=De({},["audio","video","img","source","image","track"]);let wn=null;const Jn=De({},["alt","class","for","id","label","name","pattern","placeholder","role","summary","title","value","style","xmlns"]),ln="http://www.w3.org/1998/Math/MathML",rn="http://www.w3.org/2000/svg",V="http://www.w3.org/1999/xhtml";let _e=V,ss=!1,Sn=null;const Mi=De({},[ln,rn,V],Dr);let H=De({},["mi","mo","mn","ms","mtext"]),ne=De({},["annotation-xml"]);const Ce=De({},["title","style","font","a","script"]);let qe=null;const rt=["application/xhtml+xml","text/html"],Wt="text/html";let it=null,Yn=null;const vm=s.createElement("form"),Fc=function(_){return _ instanceof RegExp||_ instanceof Function},fr=function(){let _=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};if(Yn&&Yn===_)return;(!_||typeof _!="object")&&(_={}),_=Mt(_),qe=rt.indexOf(_.PARSER_MEDIA_TYPE)===-1?Wt:_.PARSER_MEDIA_TYPE,it=qe==="application/xhtml+xml"?Dr:Za,be=ot(_,"ALLOWED_TAGS")&&Vt(_.ALLOWED_TAGS)?De({},_.ALLOWED_TAGS,it):Oe,g=ot(_,"ALLOWED_ATTR")&&Vt(_.ALLOWED_ATTR)?De({},_.ALLOWED_ATTR,it):C,Sn=ot(_,"ALLOWED_NAMESPACES")&&Vt(_.ALLOWED_NAMESPACES)?De({},_.ALLOWED_NAMESPACES,Dr):Mi,wn=ot(_,"ADD_URI_SAFE_ATTR")&&Vt(_.ADD_URI_SAFE_ATTR)?De(Mt(Jn),_.ADD_URI_SAFE_ATTR,it):Jn,Ds=ot(_,"ADD_DATA_URI_TAGS")&&Vt(_.ADD_DATA_URI_TAGS)?De(Mt(Zn),_.ADD_DATA_URI_TAGS,it):Zn,Ye=ot(_,"FORBID_CONTENTS")&&Vt(_.FORBID_CONTENTS)?De({},_.FORBID_CONTENTS,it):ws,Z=ot(_,"FORBID_TAGS")&&Vt(_.FORBID_TAGS)?De({},_.FORBID_TAGS,it):Mt({}),A=ot(_,"FORBID_ATTR")&&Vt(_.FORBID_ATTR)?De({},_.FORBID_ATTR,it):Mt({}),et=ot(_,"USE_PROFILES")?_.USE_PROFILES&&typeof _.USE_PROFILES=="object"?Mt(_.USE_PROFILES):_.USE_PROFILES:!1,J=_.ALLOW_ARIA_ATTR!==!1,X=_.ALLOW_DATA_ATTR!==!1,te=_.ALLOW_UNKNOWN_PROTOCOLS||!1,Q=_.ALLOW_SELF_CLOSE_IN_ATTR!==!1,me=_.SAFE_FOR_TEMPLATES||!1,le=_.SAFE_FOR_XML!==!1,fe=_.WHOLE_DOCUMENT||!1,Ae=_.RETURN_DOM||!1,B=_.RETURN_DOM_FRAGMENT||!1,re=_.RETURN_TRUSTED_TYPE||!1,Se=_.FORCE_BODY||!1,ye=_.SANITIZE_DOM!==!1,Le=_.SANITIZE_NAMED_PROPS||!1,Je=_.KEEP_CONTENT!==!1,xt=_.IN_PLACE||!1,ge=X_(_.ALLOWED_URI_REGEXP)?_.ALLOWED_URI_REGEXP:Dd,_e=typeof _.NAMESPACE=="string"?_.NAMESPACE:V,H=ot(_,"MATHML_TEXT_INTEGRATION_POINTS")&&_.MATHML_TEXT_INTEGRATION_POINTS&&typeof _.MATHML_TEXT_INTEGRATION_POINTS=="object"?Mt(_.MATHML_TEXT_INTEGRATION_POINTS):De({},["mi","mo","mn","ms","mtext"]),ne=ot(_,"HTML_INTEGRATION_POINTS")&&_.HTML_INTEGRATION_POINTS&&typeof _.HTML_INTEGRATION_POINTS=="object"?Mt(_.HTML_INTEGRATION_POINTS):De({},["annotation-xml"]);const K=ot(_,"CUSTOM_ELEMENT_HANDLING")&&_.CUSTOM_ELEMENT_HANDLING&&typeof _.CUSTOM_ELEMENT_HANDLING=="object"?Mt(_.CUSTOM_ELEMENT_HANDLING):ra(null);if(M=ra(null),ot(K,"tagNameCheck")&&Fc(K.tagNameCheck)&&(M.tagNameCheck=K.tagNameCheck),ot(K,"attributeNameCheck")&&Fc(K.attributeNameCheck)&&(M.attributeNameCheck=K.attributeNameCheck),ot(K,"allowCustomizedBuiltInElements")&&typeof K.allowCustomizedBuiltInElements=="boolean"&&(M.allowCustomizedBuiltInElements=K.allowCustomizedBuiltInElements),me&&(X=!1),B&&(Ae=!0),et&&(be=De({},Nd),g=ra(null),et.html===!0&&(De(be,Id),De(g,Od)),et.svg===!0&&(De(be,Mr),De(g,$r),De(g,Ji)),et.svgFilters===!0&&(De(be,Pr),De(g,$r),De(g,Ji)),et.mathMl===!0&&(De(be,Fr),De(g,Ld),De(g,Ji))),F.tagCheck=null,F.attributeCheck=null,ot(_,"ADD_TAGS")&&(typeof _.ADD_TAGS=="function"?F.tagCheck=_.ADD_TAGS:Vt(_.ADD_TAGS)&&(be===Oe&&(be=Mt(be)),De(be,_.ADD_TAGS,it))),ot(_,"ADD_ATTR")&&(typeof _.ADD_ATTR=="function"?F.attributeCheck=_.ADD_ATTR:Vt(_.ADD_ATTR)&&(g===C&&(g=Mt(g)),De(g,_.ADD_ATTR,it))),ot(_,"ADD_URI_SAFE_ATTR")&&Vt(_.ADD_URI_SAFE_ATTR)&&De(wn,_.ADD_URI_SAFE_ATTR,it),ot(_,"FORBID_CONTENTS")&&Vt(_.FORBID_CONTENTS)&&(Ye===ws&&(Ye=Mt(Ye)),De(Ye,_.FORBID_CONTENTS,it)),ot(_,"ADD_FORBID_CONTENTS")&&Vt(_.ADD_FORBID_CONTENTS)&&(Ye===ws&&(Ye=Mt(Ye)),De(Ye,_.ADD_FORBID_CONTENTS,it)),Je&&(be["#text"]=!0),fe&&De(be,["html","head","body"]),be.table&&(De(be,["tbody"]),delete Z.tbody),_.TRUSTED_TYPES_POLICY){if(typeof _.TRUSTED_TYPES_POLICY.createHTML!="function")throw An('TRUSTED_TYPES_POLICY configuration option must provide a "createHTML" hook.');if(typeof _.TRUSTED_TYPES_POLICY.createScriptURL!="function")throw An('TRUSTED_TYPES_POLICY configuration option must provide a "createScriptURL" hook.');const ae=N;N=_.TRUSTED_TYPES_POLICY;try{O=P("")}catch(Te){throw N=ae,Te}}else _.TRUSTED_TYPES_POLICY===null?(N=void 0,O=""):(N===void 0&&(N=$()),N&&typeof O=="string"&&(O=P("")));(de.uponSanitizeElement.length>0||de.uponSanitizeAttribute.length>0)&&be===Oe&&(be=Mt(be)),de.uponSanitizeAttribute.length>0&&g===C&&(g=Mt(g)),Gt&&Gt(_),Yn=_},$c=De({},[...Mr,...Pr,...ek]),Uc=De({},[...Fr,...tk]),bm=function(_){let K=E(_);(!K||!K.tagName)&&(K={namespaceURI:_e,tagName:"template"});const ae=Za(_.tagName),Te=Za(K.tagName);return Sn[_.namespaceURI]?_.namespaceURI===rn?K.namespaceURI===V?ae==="svg":K.namespaceURI===ln?ae==="svg"&&(Te==="annotation-xml"||H[Te]):!!$c[ae]:_.namespaceURI===ln?K.namespaceURI===V?ae==="math":K.namespaceURI===rn?ae==="math"&&ne[Te]:!!Uc[ae]:_.namespaceURI===V?K.namespaceURI===rn&&!ne[Te]||K.namespaceURI===ln&&!H[Te]?!1:!Uc[ae]&&(Ce[ae]||!$c[ae]):!!(qe==="application/xhtml+xml"&&Sn[_.namespaceURI]):!1},vs=function(_){sa(t.removed,{element:_});try{E(_).removeChild(_)}catch{if(b(_),!E(_))throw An("a node selected for removal could not be detached from its tree and cannot be safely returned; refusing to sanitize in place")}},Bc=function(_){const K=k?k(_):_.childNodes;if(K){const Te=[];Us(K,Re=>{sa(Te,Re)}),Us(Te,Re=>{try{b(Re)}catch{}})}const ae=y?y(_):null;if(ae)for(let Te=ae.length-1;Te>=0;--Te){const Re=ae[Te],Me=Re&&Re.name;if(typeof Me=="string")try{_.removeAttribute(Me)}catch{}}},Tn=function(_,K){try{sa(t.removed,{attribute:K.getAttributeNode(_),from:K})}catch{sa(t.removed,{attribute:null,from:K})}if(K.removeAttribute(_),_==="is")if(Ae||B)try{vs(K)}catch{}else try{K.setAttribute(_,"")}catch{}},ym=function(_){const K=y?y(_):_.attributes;if(K)for(let ae=K.length-1;ae>=0;--ae){const Te=K[ae],Re=Te&&Te.name;if(!(typeof Re!="string"||g[it(Re)]))try{_.removeAttribute(Re)}catch{}}},xm=function(_){const K=[_];for(;K.length>0;){const ae=K.pop();(x?x(ae):ae.nodeType)===Ss.element&&ym(ae);const Re=k?k(ae):ae.childNodes;if(Re)for(let Me=Re.length-1;Me>=0;--Me)K.push(Re[Me])}},Hc=function(_){let K=null,ae=null;if(Se)_="<remove></remove>"+_;else{const Me=Cd(_,/^[\r\n\t ]+/);ae=Me&&Me[0]}qe==="application/xhtml+xml"&&_e===V&&(_='<html xmlns="http://www.w3.org/1999/xhtml"><head></head><body>'+_+"</body></html>");const Te=N?P(_):_;if(_e===V)try{K=new u().parseFromString(Te,qe)}catch{}if(!K||!K.documentElement){K=G.createDocument(_e,"template",null);try{K.documentElement.innerHTML=ss?O:Te}catch{}}const Re=K.body||K.documentElement;return _&&ae&&Re.insertBefore(s.createTextNode(ae),Re.childNodes[0]||null),_e===V?z.call(K,fe?"html":"body")[0]:fe?K.documentElement:Re},Vc=function(_){return D.call(_.ownerDocument||_,_,o.SHOW_ELEMENT|o.SHOW_COMMENT|o.SHOW_TEXT|o.SHOW_PROCESSING_INSTRUCTION|o.SHOW_CDATA_SECTION,null)},pr=function(_){var K,ae;_.normalize();const Te=D.call(_.ownerDocument||_,_,o.SHOW_TEXT|o.SHOW_COMMENT|o.SHOW_CDATA_SECTION|o.SHOW_PROCESSING_INSTRUCTION,null);let Re=Te.nextNode();for(;Re;){let _t=Re.data;Us([se,pe,ee],tt=>{_t=na(_t,tt," ")}),Re.data=_t,Re=Te.nextNode()}const Me=(K=(ae=_.querySelectorAll)===null||ae===void 0?void 0:ae.call(_,"template"))!==null&&K!==void 0?K:[];Us(Array.from(Me),_t=>{Qn(_t.content)&&pr(_t.content)})},Pi=function(_){const K=T?T(_):null;return typeof K!="string"||it(K)!=="form"?!1:typeof _.nodeName!="string"||typeof _.textContent!="string"||typeof _.removeChild!="function"||_.attributes!==y(_)||typeof _.removeAttribute!="function"||typeof _.setAttribute!="function"||typeof _.namespaceURI!="string"||typeof _.insertBefore!="function"||typeof _.hasChildNodes!="function"||_.nodeType!==x(_)||_.childNodes!==k(_)},Qn=function(_){if(!x||typeof _!="object"||_===null)return!1;try{return x(_)===Ss.documentFragment}catch{return!1}},Ma=function(_){if(!x||typeof _!="object"||_===null)return!1;try{return typeof x(_)=="number"}catch{return!1}};function Ms(ve,_,K){Us(ve,ae=>{ae.call(t,_,K,Yn)})}const jc=function(_){let K=null;if(Ms(de.beforeSanitizeElements,_,null),Pi(_))return vs(_),!0;const ae=it(T?T(_):_.nodeName);if(Ms(de.uponSanitizeElement,_,{tagName:ae,allowedTags:be}),le&&_.hasChildNodes()&&!Ma(_.firstElementChild)&&Rt(/<[/\w!]/g,_.innerHTML)&&Rt(/<[/\w!]/g,_.textContent)||le&&_.namespaceURI===V&&ae==="style"&&Ma(_.firstElementChild)||_.nodeType===Ss.progressingInstruction||le&&_.nodeType===Ss.comment&&Rt(/<[/\w]/g,_.data))return vs(_),!0;if(Z[ae]||!(F.tagCheck instanceof Function&&F.tagCheck(ae))&&!be[ae]){if(!Z[ae]&&Kc(ae)&&(M.tagNameCheck instanceof RegExp&&Rt(M.tagNameCheck,ae)||M.tagNameCheck instanceof Function&&M.tagNameCheck(ae)))return!1;if(Je&&!Ye[ae]){const Re=E(_),Me=k(_);if(Me&&Re){const _t=Me.length;for(let tt=_t-1;tt>=0;--tt){const dt=xt?Me[tt]:p(Me[tt],!0);Re.insertBefore(dt,m(_))}}}return vs(_),!0}return(x?x(_):_.nodeType)===Ss.element&&!bm(_)||(ae==="noscript"||ae==="noembed"||ae==="noframes")&&Rt(/<\/no(script|embed|frames)/i,_.innerHTML)?(vs(_),!0):(me&&_.nodeType===Ss.text&&(K=_.textContent,Us([se,pe,ee],Re=>{K=na(K,Re," ")}),_.textContent!==K&&(sa(t.removed,{element:_.cloneNode()}),_.textContent=K)),Ms(de.afterSanitizeElements,_,null),!1)},zc=function(_,K,ae){if(A[K]||ye&&(K==="id"||K==="name")&&(ae in s||ae in vm))return!1;const Te=g[K]||F.attributeCheck instanceof Function&&F.attributeCheck(K,_);if(!(X&&!A[K]&&Rt(ue,K))){if(!(J&&Rt(Ie,K))){if(!Te||A[K]){if(!(Kc(_)&&(M.tagNameCheck instanceof RegExp&&Rt(M.tagNameCheck,_)||M.tagNameCheck instanceof Function&&M.tagNameCheck(_))&&(M.attributeNameCheck instanceof RegExp&&Rt(M.attributeNameCheck,K)||M.attributeNameCheck instanceof Function&&M.attributeNameCheck(K,_))||K==="is"&&M.allowCustomizedBuiltInElements&&(M.tagNameCheck instanceof RegExp&&Rt(M.tagNameCheck,ae)||M.tagNameCheck instanceof Function&&M.tagNameCheck(ae))))return!1}else if(!wn[K]){if(!Rt(ge,na(ae,oe,""))){if(!((K==="src"||K==="xlink:href"||K==="href")&&_!=="script"&&Ed(ae,"data:")===0&&Ds[_])){if(!(te&&!Rt(j,na(ae,oe,"")))){if(ae)return!1}}}}}}return!0},_m=De({},["annotation-xml","color-profile","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","missing-glyph"]),Kc=function(_){return!_m[Za(_)]&&Rt(ie,_)},qc=function(_){Ms(de.beforeSanitizeAttributes,_,null);const K=_.attributes;if(!K||Pi(_))return;const ae={attrName:"",attrValue:"",keepAttr:!0,allowedAttributes:g,forceKeepAttr:void 0};let Te=K.length;for(;Te--;){const Re=K[Te],Me=Re.name,_t=Re.namespaceURI,tt=Re.value,dt=it(Me),on=tt;let Tt=Me==="value"?on:G_(on);if(ae.attrName=dt,ae.attrValue=Tt,ae.keepAttr=!0,ae.forceKeepAttr=void 0,Ms(de.uponSanitizeAttribute,_,ae),Tt=ae.attrValue,Le&&(dt==="id"||dt==="name")&&Ed(Tt,He)!==0&&(Tn(Me,_),Tt=He+Tt),le&&Rt(/((--!?|])>)|<\/(style|script|title|xmp|textarea|noscript|iframe|noembed|noframes)/i,Tt)){Tn(Me,_);continue}if(dt==="attributename"&&Cd(Tt,"href")){Tn(Me,_);continue}if(ae.forceKeepAttr)continue;if(!ae.keepAttr){Tn(Me,_);continue}if(!Q&&Rt(/\/>/i,Tt)){Tn(Me,_);continue}me&&Us([se,pe,ee],Wc=>{Tt=na(Tt,Wc," ")});const Gc=it(_.nodeName);if(!zc(Gc,dt,Tt)){Tn(Me,_);continue}if(N&&typeof d=="object"&&typeof d.getAttributeType=="function"&&!_t)switch(d.getAttributeType(Gc,dt)){case"TrustedHTML":{Tt=P(Tt);break}case"TrustedScriptURL":{Tt=S(Tt);break}}if(Tt!==on)try{_t?_.setAttributeNS(_t,Me,Tt):_.setAttribute(Me,Tt),Pi(_)?vs(_):Td(t.removed)}catch{Tn(Me,_)}}Ms(de.afterSanitizeAttributes,_,null)},Fi=function(_){let K=null;const ae=Vc(_);for(Ms(de.beforeSanitizeShadowDOM,_,null);K=ae.nextNode();)if(Ms(de.uponSanitizeShadowNode,K,null),jc(K),qc(K),Qn(K.content)&&Fi(K.content),(x?x(K):K.nodeType)===Ss.element){const Re=v?v(K):K.shadowRoot;Qn(Re)&&(hr(Re),Fi(Re))}Ms(de.afterSanitizeShadowDOM,_,null)},hr=function(_){const K=[{node:_,shadow:null}];for(;K.length>0;){const ae=K.pop();if(ae.shadow){Fi(ae.shadow);continue}const Te=ae.node,Me=(x?x(Te):Te.nodeType)===Ss.element,_t=k?k(Te):Te.childNodes;if(_t)for(let tt=_t.length-1;tt>=0;--tt)K.push({node:_t[tt],shadow:null});if(Me){const tt=T?T(Te):null;if(typeof tt=="string"&&it(tt)==="template"){const dt=Te.content;Qn(dt)&&K.push({node:dt,shadow:null})}}if(Me){const tt=v?v(Te):Te.shadowRoot;Qn(tt)&&K.push({node:null,shadow:tt},{node:tt,shadow:null})}}};return t.sanitize=function(ve){let _=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{},K=null,ae=null,Te=null,Re=null;if(ss=!ve,ss&&(ve="<!-->"),typeof ve!="string"&&!Ma(ve)&&(ve=Q_(ve),typeof ve!="string"))throw An("dirty is not a string, aborting");if(!t.isSupported)return ve;xe||fr(_),t.removed=[];const Me=xt&&typeof ve!="string"&&Ma(ve);if(Me){const dt=T?T(ve):ve.nodeName;if(typeof dt=="string"){const on=it(dt);if(!be[on]||Z[on])throw An("root node is forbidden and cannot be sanitized in-place")}if(Pi(ve))throw An("root node is clobbered and cannot be sanitized in-place");try{hr(ve)}catch(on){throw Bc(ve),on}}else if(Ma(ve))K=Hc("<!---->"),ae=K.ownerDocument.importNode(ve,!0),ae.nodeType===Ss.element&&ae.nodeName==="BODY"||ae.nodeName==="HTML"?K=ae:K.appendChild(ae),hr(ae);else{if(!Ae&&!me&&!fe&&ve.indexOf("<")===-1)return N&&re?P(ve):ve;if(K=Hc(ve),!K)return Ae?null:re?O:""}K&&Se&&vs(K.firstChild);const _t=Vc(Me?ve:K);try{for(;Te=_t.nextNode();)jc(Te),qc(Te),Qn(Te.content)&&Fi(Te.content)}catch(dt){throw Me&&Bc(ve),dt}if(Me)return Us(t.removed,dt=>{dt.element&&xm(dt.element)}),me&&pr(ve),ve;if(Ae){if(me&&pr(K),B)for(Re=I.call(K.ownerDocument);K.firstChild;)Re.appendChild(K.firstChild);else Re=K;return(g.shadowroot||g.shadowrootmode)&&(Re=ce.call(n,Re,!0)),Re}let tt=fe?K.outerHTML:K.innerHTML;return fe&&be["!doctype"]&&K.ownerDocument&&K.ownerDocument.doctype&&K.ownerDocument.doctype.name&&Rt(ck,K.ownerDocument.doctype.name)&&(tt="<!DOCTYPE "+K.ownerDocument.doctype.name+`>
`+tt),me&&Us([se,pe,ee],dt=>{tt=na(tt,dt," ")}),N&&re?P(tt):tt},t.setConfig=function(){let ve=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};fr(ve),xe=!0},t.clearConfig=function(){Yn=null,xe=!1,N=w,O=""},t.isValidAttribute=function(ve,_,K){Yn||fr({});const ae=it(ve),Te=it(_);return zc(ae,Te,K)},t.addHook=function(ve,_){typeof _=="function"&&sa(de[ve],_)},t.removeHook=function(ve,_){if(_!==void 0){const K=K_(de[ve],_);return K===-1?void 0:q_(de[ve],K,1)[0]}return Td(de[ve])},t.removeHooks=function(ve){de[ve]=[]},t.removeAllHooks=function(){de=Md()},t}var Pd=nm();function Ac(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}var Wn=Ac();function am(e){Wn=e}var ai={exec:()=>null};function Ze(e,t=""){let s=typeof e=="string"?e:e.source;const n={replace:(a,i)=>{let l=typeof i=="string"?i:i.source;return l=l.replace(Kt.caret,"$1"),s=s.replace(a,l),n},getRegex:()=>new RegExp(s,t)};return n}var Kt={codeRemoveIndent:/^(?: {1,4}| {0,3}\t)/gm,outputLinkReplace:/\\([\[\]])/g,indentCodeCompensation:/^(\s+)(?:```)/,beginningSpace:/^\s+/,endingHash:/#$/,startingSpaceChar:/^ /,endingSpaceChar:/ $/,nonSpaceChar:/[^ ]/,newLineCharGlobal:/\n/g,tabCharGlobal:/\t/g,multipleSpaceGlobal:/\s+/g,blankLine:/^[ \t]*$/,doubleBlankLine:/\n[ \t]*\n[ \t]*$/,blockquoteStart:/^ {0,3}>/,blockquoteSetextReplace:/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,blockquoteSetextReplace2:/^ {0,3}>[ \t]?/gm,listReplaceTabs:/^\t+/,listReplaceNesting:/^ {1,4}(?=( {4})*[^ ])/g,listIsTask:/^\[[ xX]\] /,listReplaceTask:/^\[[ xX]\] +/,anyLine:/\n.*\n/,hrefBrackets:/^<(.*)>$/,tableDelimiter:/[:|]/,tableAlignChars:/^\||\| *$/g,tableRowBlankLine:/\n[ \t]*$/,tableAlignRight:/^ *-+: *$/,tableAlignCenter:/^ *:-+: *$/,tableAlignLeft:/^ *:-+ *$/,startATag:/^<a /i,endATag:/^<\/a>/i,startPreScriptTag:/^<(pre|code|kbd|script)(\s|>)/i,endPreScriptTag:/^<\/(pre|code|kbd|script)(\s|>)/i,startAngleBracket:/^</,endAngleBracket:/>$/,pedanticHrefTitle:/^([^'"]*[^\s])\s+(['"])(.*)\2/,unicodeAlphaNumeric:/[\p{L}\p{N}]/u,escapeTest:/[&<>"']/,escapeReplace:/[&<>"']/g,escapeTestNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,escapeReplaceNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/g,unescapeTest:/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig,caret:/(^|[^\[])\^/g,percentDecode:/%25/g,findPipe:/\|/g,splitPipe:/ \|/,slashPipe:/\\\|/g,carriageReturn:/\r\n|\r/g,spaceLine:/^ +$/gm,notSpaceStart:/^\S*/,endingNewline:/\n$/,listItemRegex:e=>new RegExp(`^( {0,3}${e})((?:[	 ][^\\n]*)?(?:\\n|$))`),nextBulletRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),hrRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),fencesBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:\`\`\`|~~~)`),headingBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}#`),htmlBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}<(?:[a-z].*>|!--)`,"i")},pk=/^(?:[ \t]*(?:\n|$))+/,hk=/^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/,mk=/^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/,Di=/^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,gk=/^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/,Rc=/(?:[*+-]|\d{1,9}[.)])/,im=/^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/,lm=Ze(im).replace(/bull/g,Rc).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/\|table/g,"").getRegex(),vk=Ze(im).replace(/bull/g,Rc).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/table/g,/ {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),Ic=/^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/,bk=/^[^\n]+/,Nc=/(?!\s*\])(?:\\.|[^\[\]\\])+/,yk=Ze(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace("label",Nc).replace("title",/(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(),xk=Ze(/^( {0,3}bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g,Rc).getRegex(),cr="address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul",Oc=/<!--(?:-?>|[\s\S]*?(?:-->|$))/,_k=Ze("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))","i").replace("comment",Oc).replace("tag",cr).replace("attribute",/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),rm=Ze(Ic).replace("hr",Di).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("|table","").replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",cr).getRegex(),kk=Ze(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph",rm).getRegex(),Lc={blockquote:kk,code:hk,def:yk,fences:mk,heading:gk,hr:Di,html:_k,lheading:lm,list:xk,newline:pk,paragraph:rm,table:ai,text:bk},Fd=Ze("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr",Di).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("blockquote"," {0,3}>").replace("code","(?: {4}| {0,3}	)[^\\n]").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",cr).getRegex(),wk={...Lc,lheading:vk,table:Fd,paragraph:Ze(Ic).replace("hr",Di).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("table",Fd).replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",cr).getRegex()},Sk={...Lc,html:Ze(`^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:"[^"]*"|'[^']*'|\\s[^'"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))`).replace("comment",Oc).replace(/tag/g,"(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b").getRegex(),def:/^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +(["(][^\n]+[")]))? *(?:\n+|$)/,heading:/^(#{1,6})(.*)(?:\n+|$)/,fences:ai,lheading:/^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,paragraph:Ze(Ic).replace("hr",Di).replace("heading",` *#{1,6} *[^
]`).replace("lheading",lm).replace("|table","").replace("blockquote"," {0,3}>").replace("|fences","").replace("|list","").replace("|html","").replace("|tag","").getRegex()},Tk=/^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/,Ck=/^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/,om=/^( {2,}|\\)\n(?!\s*$)/,Ek=/^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/,ur=/[\p{P}\p{S}]/u,Dc=/[\s\p{P}\p{S}]/u,cm=/[^\s\p{P}\p{S}]/u,Ak=Ze(/^((?![*_])punctSpace)/,"u").replace(/punctSpace/g,Dc).getRegex(),um=/(?!~)[\p{P}\p{S}]/u,Rk=/(?!~)[\s\p{P}\p{S}]/u,Ik=/(?:[^\s\p{P}\p{S}]|~)/u,Nk=/\[[^[\]]*?\]\((?:\\.|[^\\\(\)]|\((?:\\.|[^\\\(\)])*\))*\)|`[^`]*?`|<[^<>]*?>/g,dm=/^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,Ok=Ze(dm,"u").replace(/punct/g,ur).getRegex(),Lk=Ze(dm,"u").replace(/punct/g,um).getRegex(),fm="^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",Dk=Ze(fm,"gu").replace(/notPunctSpace/g,cm).replace(/punctSpace/g,Dc).replace(/punct/g,ur).getRegex(),Mk=Ze(fm,"gu").replace(/notPunctSpace/g,Ik).replace(/punctSpace/g,Rk).replace(/punct/g,um).getRegex(),Pk=Ze("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)","gu").replace(/notPunctSpace/g,cm).replace(/punctSpace/g,Dc).replace(/punct/g,ur).getRegex(),Fk=Ze(/\\(punct)/,"gu").replace(/punct/g,ur).getRegex(),$k=Ze(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme",/[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email",/[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),Uk=Ze(Oc).replace("(?:-->|$)","-->").getRegex(),Bk=Ze("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment",Uk).replace("attribute",/\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),Dl=/(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/,Hk=Ze(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label",Dl).replace("href",/<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title",/"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),pm=Ze(/^!?\[(label)\]\[(ref)\]/).replace("label",Dl).replace("ref",Nc).getRegex(),hm=Ze(/^!?\[(ref)\](?:\[\])?/).replace("ref",Nc).getRegex(),Vk=Ze("reflink|nolink(?!\\()","g").replace("reflink",pm).replace("nolink",hm).getRegex(),Mc={_backpedal:ai,anyPunctuation:Fk,autolink:$k,blockSkip:Nk,br:om,code:Ck,del:ai,emStrongLDelim:Ok,emStrongRDelimAst:Dk,emStrongRDelimUnd:Pk,escape:Tk,link:Hk,nolink:hm,punctuation:Ak,reflink:pm,reflinkSearch:Vk,tag:Bk,text:Ek,url:ai},jk={...Mc,link:Ze(/^!?\[(label)\]\((.*?)\)/).replace("label",Dl).getRegex(),reflink:Ze(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label",Dl).getRegex()},ko={...Mc,emStrongRDelimAst:Mk,emStrongLDelim:Lk,url:Ze(/^((?:ftp|https?):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/,"i").replace("email",/[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),_backpedal:/(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,del:/^(~~?)(?=[^\s~])((?:\\.|[^\\])*?(?:\\.|[^\s~\\]))\1(?=[^~]|$)/,text:/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|https?:\/\/|ftp:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/},zk={...ko,br:Ze(om).replace("{2,}","*").getRegex(),text:Ze(ko.text).replace("\\b_","\\b_| {2,}\\n").replace(/\{2,\}/g,"*").getRegex()},Yi={normal:Lc,gfm:wk,pedantic:Sk},ja={normal:Mc,gfm:ko,breaks:zk,pedantic:jk},Kk={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},$d=e=>Kk[e];function Es(e,t){if(t){if(Kt.escapeTest.test(e))return e.replace(Kt.escapeReplace,$d)}else if(Kt.escapeTestNoEncode.test(e))return e.replace(Kt.escapeReplaceNoEncode,$d);return e}function Ud(e){try{e=encodeURI(e).replace(Kt.percentDecode,"%")}catch{return null}return e}function Bd(e,t){var i;const s=e.replace(Kt.findPipe,(l,r,o)=>{let c=!1,u=r;for(;--u>=0&&o[u]==="\\";)c=!c;return c?"|":" |"}),n=s.split(Kt.splitPipe);let a=0;if(n[0].trim()||n.shift(),n.length>0&&!((i=n.at(-1))!=null&&i.trim())&&n.pop(),t)if(n.length>t)n.splice(t);else for(;n.length<t;)n.push("");for(;a<n.length;a++)n[a]=n[a].trim().replace(Kt.slashPipe,"|");return n}function za(e,t,s){const n=e.length;if(n===0)return"";let a=0;for(;a<n&&e.charAt(n-a-1)===t;)a++;return e.slice(0,n-a)}function qk(e,t){if(e.indexOf(t[1])===-1)return-1;let s=0;for(let n=0;n<e.length;n++)if(e[n]==="\\")n++;else if(e[n]===t[0])s++;else if(e[n]===t[1]&&(s--,s<0))return n;return s>0?-2:-1}function Hd(e,t,s,n,a){const i=t.href,l=t.title||null,r=e[1].replace(a.other.outputLinkReplace,"$1");n.state.inLink=!0;const o={type:e[0].charAt(0)==="!"?"image":"link",raw:s,href:i,title:l,text:r,tokens:n.inlineTokens(r)};return n.state.inLink=!1,o}function Gk(e,t,s){const n=e.match(s.other.indentCodeCompensation);if(n===null)return t;const a=n[1];return t.split(`
...
`+s.join(`
`),m=this.blockquote(b);i[i.length-1]=m,n=n.substring(0,n.length-p.raw.length)+m.raw,a=a.substring(0,a.length-p.text.length)+m.text;break}else if((f==null?void 0:f.type)==="list"){const p=f,b=p.raw+`
`+s.join(`
`),m=this.list(b);i[i.length-1]=m,n=n.substring(0,n.length-f.raw.length)+m.raw,a=a.substring(0,a.length-p.raw.length)+m.raw,s=b.substring(i.at(-1).raw.length).split(`
...
`+n+"</"+a+`>
`}listitem(e){var s;let t="";if(e.task){const n=this.checkbox({checked:!!e.checked});e.loose?((s=e.tokens[0])==null?void 0:s.type)==="paragraph"?(e.tokens[0].text=n+" "+e.tokens[0].text,e.tokens[0].tokens&&e.tokens[0].tokens.length>0&&e.tokens[0].tokens[0].type==="text"&&(e.tokens[0].tokens[0].text=n+" "+Es(e.tokens[0].tokens[0].text),e.tokens[0].tokens[0].escaped=!0)):e.tokens.unshift({type:"text",raw:n+" ",text:n+" ",escaped:!0}):t+=n+" "}return t+=this.parser.parse(e.tokens,!!e.loose),`<li>${t}</li>
...
Please report this to https://github.com/markedjs/marked.`,e){const n="<p>An error occurred:</p><pre>"+Es(s.message+"",!0)+"</pre>";return t?Promise.resolve(n):n}if(t)return Promise.reject(s);throw s}}},jn=new Wk;function Ke(e,t){return jn.parse(e,t)}Ke.options=Ke.setOptions=function(e){return jn.setOptions(e),Ke.defaults=jn.defaults,am(Ke.defaults),Ke};Ke.getDefaults=Ac;Ke.defaults=Wn;Ke.use=function(...e){return jn.use(...e),Ke.defaults=jn.defaults,am(Ke.defaults),Ke};Ke.walkTokens=function(e,t){return jn.walkTokens(e,t)};Ke.parseInline=jn.parseInline;Ke.Parser=Ws;Ke.parser=Ws.parse;Ke.Renderer=Pl;Ke.TextRenderer=Pc;Ke.Lexer=Gs;Ke.lexer=Gs.lex;Ke.Tokenizer=Ml;Ke.Hooks=ll;Ke.parse=Ke;Ke.options;Ke.setOptions;Ke.use;Ke.walkTokens;Ke.parseInline;Ws.parse;Gs.lex;const Zk={breaks:!0,gfm:!0};function Vd(e){if(!e)return"";try{if(typeof Ke<"u"&&Ke.parse){const t=Ke.parse(e,Zk);return typeof Pd<"u"?Pd.sanitize(t):t}}catch{}return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\n/g,"<br>")}function Jk(e){const t=new Date(e),s=t.getHours().toString().padStart(2,"0"),n=t.getMinutes().toString().padStart(2,"0");return`${s}:${n}`}const Yk={run_command:"terminal",ssh_command:"terminal",run_script:"terminal",read_file:"file",write_file:"edit",list_directory:"folder",search_knowledge:"search",ingest_document:"book",generate_image:"image",analyze_image:"eye",analyze_pdf:"file",browser_screenshot:"globe",manage_process:"sliders"};function Qk(e){return Yk[e]||"wrench"}const Xk=/https?:\/\/\S+\.(?:png|jpg|jpeg|gif|webp|svg)(?:\?\S*)?/gi;function jd(e){if(!e)return[];const t=e.match(Xk);return t?[...new Set(t)]:[]}const ew={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(""),s=h(!1),n=h(null),a=h(null),i=h(0),l=h("");let r=null,o=0;const c=["Check system health","List running services","Show disk usage","What can you do?"],u=Y(()=>t.value.trim().length>0&&!s.value),d=Y(()=>{const S=We.state;return S==="connected"?"Connected":S==="reconnecting"?"Reconnecting…":S==="connecting"?"Connecting…":"REST fallback"}),f=["Watching across all realms...","Processing...","Consulting the bifrost...","Observing..."],p=Y(()=>{const S=Math.floor(i.value/4)%f.length,$=i.value;return $>3?`${f[S]} (${$}s)`:f[0]});function b(){Et(()=>{n.value&&(n.value.scrollTop=n.value.scrollHeight)})}function m(){if(!a.value)return;const S=a.value;S.style.height="auto",S.style.height=Math.min(S.scrollHeight,120)+"px"}function k(S,$,W={}){const G={id:++o,role:S,content:$,timestamp:Date.now(),html:S==="bot"?Vd($):"",tools_used:W.tools_used||[],is_error:W.is_error||!1,images:S==="bot"?jd($):[],files:W.files||[],_showTools:!1};return e.value.push(G),b(),S==="bot"&&Et(()=>E()),G}function E(){if(!n.value)return;n.value.querySelectorAll(".chat-markdown pre:not([data-copy])").forEach($=>{$.setAttribute("data-copy","true"),$.style.position="relative";const W=document.createElement("button");W.className="chat-code-copy",W.textContent="Copy",W.addEventListener("click",()=>{const G=$.querySelector("code"),D=G?G.textContent:$.textContent;navigator.clipboard.writeText(D).then(()=>{W.textContent="Copied!",setTimeout(()=>{W.textContent="Copy"},1500)}).catch(()=>{})}),$.appendChild(W)})}function v(S){if(S===0)return!0;const $=e.value[S-1],W=e.value[S],G=new Date($.timestamp).toDateString(),D=new Date(W.timestamp).toDateString();return G!==D}function y(S){const $=new Date(S),W=new Date;if($.toDateString()===W.toDateString())return"Today";const G=new Date(W);return G.setDate(G.getDate()-1),$.toDateString()===G.toDateString()?"Yesterday":$.toLocaleDateString(void 0,{month:"short",day:"numeric",year:"numeric"})}function x(S){t.value=S,Et(()=>U())}function T(S){window.open(S,"_blank","noopener")}function N(S){S.target.style.display="none"}function O(){i.value=0,r=setInterval(()=>{i.value++},1e3)}function w(){r&&(clearInterval(r),r=null),i.value=0}function R(S){s.value&&(s.value=!1,w(),S.type==="chat_response"?k("bot",S.content,{tools_used:S.tools_used||[],is_error:S.is_error||!1,files:S.files||[]}):S.type==="chat_error"&&k("bot",S.error||"Unknown error",{is_error:!0}),Et(()=>{var $;return($=a.value)==null?void 0:$.focus()}))}async function L(S){try{const $=await q.post("/api/chat",{content:S,channel_id:l.value});k("bot",$.response,{tools_used:$.tools_used||[],is_error:$.is_error||!1,files:$.files||[]})}catch($){k("bot",$.message||"Failed to send message",{is_error:!0})}}async function U(){const S=t.value.trim();if(!S||s.value)return;k("user",S),t.value="",s.value=!0,O(),a.value&&(a.value.style.height="auto"),We.connected&&We.sendChat(S,{channelId:l.value})||(await L(S),s.value=!1,w()),Et(()=>{var W;return(W=a.value)==null?void 0:W.focus()})}async function P(){try{if(!l.value){const $=await q.get("/api/auth/session");l.value=$.channel_id||$.user_id||"web-user"}const S=await q.get("/api/sessions/"+encodeURIComponent(l.value));if(S&&S.messages&&S.messages.length>0){for(const $ of S.messages){const W=$.role==="user"?"user":"bot";let G=$.content||"";if(W==="user"){const I=G.match(/^\[.*?\]:\s*/);I&&(G=G.slice(I[0].length))}if(!G.trim())continue;const D={id:++o,role:W,content:G,timestamp:$.timestamp?$.timestamp*1e3:Date.now(),html:W==="bot"?Vd(G):"",tools_used:[],is_error:!1,images:W==="bot"?jd(G):[],files:[],_showTools:!1};e.value.push(D)}Et(()=>{b(),E()})}}catch{}}return Be(()=>{We.subscribe("chat",R),P(),Et(()=>{var S;return(S=a.value)==null?void 0:S.focus()})}),ht(()=>{We.unsubscribe("chat",R),w()}),{messages:e,input:t,sending:s,messagesEl:n,inputEl:a,canSend:u,wsStatus:d,typingText:p,suggestions:c,send:U,autoResize:m,formatTime:Jk,formatDate:y,showDateSeparator:v,useSuggestion:x,openImage:T,onImageError:N,getToolIcon:Qk}}},dr={props:{tabs:{type:Array,required:!0},defaultTab:{type:String,default:""},groupLabel:{type:String,default:""}},setup(e){const t=A_(),s=Yh(),n=Y({get(){var o;const r=t.query.tab;return r&&e.tabs.some(c=>c.id===r)?r:e.defaultTab||((o=e.tabs[0])==null?void 0:o.id)||""},set(r){s.replace({query:{...t.query,tab:r}})}}),a=Y(()=>{var r;return((r=e.tabs.find(o=>o.id===n.value))==null?void 0:r.component)||null}),i=Y(()=>{var r;return((r=e.tabs.find(o=>o.id===n.value))==null?void 0:r.label)||""});hs(i,r=>{e.groupLabel&&r&&(document.title=`Odin — ${e.groupLabel} › ${r}`)},{immediate:!0});function l(r,o){if(!["ArrowLeft","ArrowRight","Home","End"].includes(r.key))return;r.preventDefault();let c=o;r.key==="ArrowRight"&&(c=(o+1)%e.tabs.length),r.key==="ArrowLeft"&&(c=(o-1+e.tabs.length)%e.tabs.length),r.key==="Home"&&(c=0),r.key==="End"&&(c=e.tabs.length-1),n.value=e.tabs[c].id,requestAnimationFrame(()=>{var u;return(u=document.getElementById("tab-"+e.tabs[c].id))==null?void 0:u.focus()})}return{activeTab:n,activeComponent:a,activeLabel:i,onTabKeydown:l}},template:`
...
`)}return}}let i=null;function l(){const c=Date.now();e.value.forEach(u=>{u.status==="running"&&(u.elapsed=c-u.startTime)})}Be(()=>{We.on("events",a),i=setInterval(l,500)}),ht(()=>{We.off("events",a),i&&clearInterval(i)});function r(c){return c<1e3?`${c}ms`:`${(c/1e3).toFixed(1)}s`}function o(c){return c==="running"?"clock":c==="success"?"success":c==="error"?"error":"info"}return{activeTasks:e,recentHistory:t,streamOutput:s,formatMs:r,statusIcon:o}},template:`
  ...
  `},sw={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(null),a=h(!0),i=h("all");let l=null;const r=Y(()=>e.value.filter(E=>E.status==="running").length),o=Y(()=>e.value.filter(E=>E.status==="completed").length),c=Y(()=>e.value.filter(E=>["failed","timeout","killed"].includes(E.status)).length),u=Y(()=>[{value:"all",label:"All",count:e.value.length},{value:"running",label:"Running",count:r.value},{value:"completed",label:"Completed",count:o.value},{value:"failed",label:"Failed",count:c.value}]),d=Y(()=>i.value==="all"?e.value:i.value==="failed"?e.value.filter(E=>["failed","timeout","killed"].includes(E.status)):e.value.filter(E=>E.status===i.value));function f(E){return Math.min(100,Math.round(E.iteration_count/30*100))}async function p(E=!1){E=E===!0,E||(t.value=!0);try{const v=await q.get("/api/agents");e.value=Array.isArray(v)?v:[],s.value=null}catch(v){E||(s.value=v.message)}E||(t.value=!1)}async function b(E){const v=e.value.find(x=>x.id===E);if(await ls({title:"Kill agent",message:`Kill agent "${(v==null?void 0:v.label)||E}"? Its current work will be lost.`,confirmLabel:"Kill",danger:!0})){n.value=E;try{await q.del(`/api/agents/${encodeURIComponent(E)}`),ke.success("Agent killed"),await p()}catch(x){ke.error(x.message||"Failed to kill agent")}n.value=null}}function m(){k(),a.value&&(l=setInterval(()=>{a.value&&p(!0)},5e3))}function k(){l&&(clearInterval(l),l=null)}return Be(()=>{p(),m()}),ht(()=>{k()}),{agents:e,loading:t,error:s,killing:n,autoRefresh:a,statusFilter:i,runningCount:r,completedCount:o,failedCount:c,statusFilters:u,filteredAgents:d,formatTs:Da,formatDuration:Ia,progressPercent:f,fetchAgents:p,killAgent:b,startAutoRefresh:m,stopAutoRefresh:k}}},nw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(!1),a=h({goal:"",interval_seconds:60,mode:"notify",max_iterations:50,stop_condition:"",channel_id:""}),i=h(!1),l=h(null),r=h(null),o=h(null),c=h({}),u=Y(()=>e.value.reduce((N,O)=>N+(O.iteration_count||0),0)),d=Y(()=>e.value.filter(N=>N.status==="running").length);function f(N){return N==="running"?"loop-status-running":N==="error"?"loop-status-error":"loop-status-stopped"}function p(N){return N==="running"?"badge-success":N==="error"?"badge-danger":N==="completed"?"badge-info":"badge-warning"}function b(N){return N==="act"?"badge-warning":N==="silent"?"badge-info":"badge-success"}function m(N){c.value={...c.value,[N]:!c.value[N]}}async function k(N=!1){N=N===!0,N||(t.value=!0);try{e.value=await q.get("/api/loops"),s.value=null}catch(O){N||(s.value=O.message)}N||(t.value=!1)}async function E(){l.value=null;const N=a.value;if(!N.goal.trim()){l.value="Goal is required";return}if(!N.channel_id.trim()){l.value="Channel ID is required";return}const O={goal:N.goal.trim(),channel_id:N.channel_id.trim(),interval_seconds:N.interval_seconds||60,mode:N.mode,max_iterations:N.max_iterations||50};N.stop_condition.trim()&&(O.stop_condition=N.stop_condition.trim()),i.value=!0;try{const w=await q.post("/api/loops",O);ke.success(`Loop started: ${w.loop_id}`),a.value={goal:"",interval_seconds:60,mode:"notify",max_iterations:50,stop_condition:"",channel_id:""},n.value=!1,await k()}catch(w){l.value=w.message}i.value=!1}async function v(N){if(await ls({title:"Stop loop",message:`Stop loop ${N}? The current iteration will finish before stopping.`,confirmLabel:"Stop Loop",danger:!0})){r.value=N;try{await q.del(`/api/loops/${encodeURIComponent(N)}`),ke.success("Loop stopped"),await k()}catch(w){ke.error(w.message||"Failed to stop loop")}r.value=null}}async function y(N){o.value=N;try{await q.post(`/api/loops/${encodeURIComponent(N)}/restart`),ke.success("Loop restarted"),await k()}catch(O){ke.error(O.message||"Failed to restart loop")}o.value=null}function x(N){N.payload&&(N.payload.loop_id||N.payload.type==="loop")&&k(!0)}let T=null;return Be(()=>{k(),We.subscribe("events",x),T=setInterval(()=>{k(!0)},5e3)}),ht(()=>{We.unsubscribe("events",x),T&&clearInterval(T)}),{loops:e,loading:t,error:s,showCreate:n,form:a,creating:i,createError:l,stoppingId:r,restartingId:o,expandedHistory:c,totalIterations:u,runningCount:d,statusDotClass:f,statusBadge:p,modeBadge:b,formatDuration:Ia,formatAge:Qh,toggleHistory:m,fetchLoops:k,doCreate:E,doStop:v,doRestart:y}}},aw={template:`
    <div class="p-6 page-fade-in">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(!0);let a=null;const i=h(null),l=Y(()=>e.value.filter(m=>m.status==="running").length),r=Y(()=>e.value.filter(m=>m.status!=="running").length);function o(m){return m==="running"?"loop-status-running":m==="failed"||m==="error"?"loop-status-error":"loop-status-stopped"}function c(m){return m==="running"?"badge-success":m==="completed"||m==="exited"?"badge-info":m==="killed"||m==="error"||m==="failed"?"badge-danger":"badge-warning"}async function u(m=!1){m=m===!0,m||(t.value=!0);try{e.value=await q.get("/api/processes"),s.value=null}catch(k){m||(s.value=k.message)}m||(t.value=!1)}function d(){f(),n.value&&(a=setInterval(()=>{t.value||u(!0)},5e3))}function f(){a&&(clearInterval(a),a=null)}hs(n,m=>{m?d():f()});async function p(m){if(await ls({title:"Kill process",message:`Kill process ${m}?`,confirmLabel:"Kill",danger:!0})){i.value=m;try{await q.del(`/api/processes/${m}`),ke.success(`Process ${m} killed`),await u()}catch(E){ke.error(E.message||"Failed to kill process")}i.value=null}}function b(m){m.payload&&(m.payload.pid||m.payload.type==="process")&&u(!0)}return Be(()=>{u(),We.subscribe("events",b),d()}),ht(()=>{We.unsubscribe("events",b),f()}),{processes:e,loading:t,error:s,autoRefresh:n,killingPid:i,runningCount:l,completedCount:r,procStatusDot:o,statusBadge:c,formatDuration:Ia,fetchProcesses:u,doKill:p}}},iw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-start justify-between mb-4 flex-wrap gap-3">
        <div>
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(!1),a=h({description:"",action:"reminder",channel_id:"",cron:"",run_at:"",message:"",tool_name:"",tool_input_str:""}),i=h(!1),l=h(null),r=h(null),o=h(!1),c=[{label:"Every hour",expr:"0 * * * *"},{label:"Every 6h",expr:"0 */6 * * *"},{label:"Daily 9am",expr:"0 9 * * *"},{label:"Weekly Mon",expr:"0 9 * * 1"},{label:"Every 30m",expr:"*/30 * * * *"}],u=h(null),d=h(null),f=h(null),p=h(null),b=h(null),m=h([]),k=h(!1),E=Y(()=>e.value.filter(D=>D.cron&&!D.one_time).length),v=Y(()=>e.value.filter(D=>D.one_time).length),y=Y(()=>e.value.filter(D=>D.trigger).length),x=Y(()=>e.value.filter(D=>D.paused).length),T=Y(()=>e.value.filter(D=>D.consecutive_failures>0).length);function N(D){if(!D)return"-";const I=Date.now(),ce=(new Date(D).getTime()-I)/1e3;if(ce<0)return"overdue";if(ce<60)return"in < 1 min";if(ce<3600)return`in ${Math.floor(ce/60)} min`;if(ce<86400){const se=Math.floor(ce/3600),pe=Math.floor(ce%3600/60);return pe>0?`in ${se}h ${pe}m`:`in ${se}h`}const de=Math.floor(ce/86400);return`in ${de} day${de!==1?"s":""}`}function O(D){return D==null?"-":D<1e3?`${D}ms`:D<6e4?`${(D/1e3).toFixed(1)}s`:Ia(D/1e3)}function w(){r.value=null}async function R(){const D=a.value.cron.trim();if(D){o.value=!0;try{r.value=await q.post("/api/schedules/validate-cron",{expression:D})}catch(I){r.value={valid:!1,error:I.message}}o.value=!1}}async function L(){t.value=!0,s.value=null;try{e.value=await q.get("/api/schedules")}catch(D){s.value=D.message}t.value=!1}async function U(D){if(b.value===D){b.value=null,m.value=[];return}b.value=D,k.value=!0,m.value=[];try{m.value=await q.get(`/api/schedules/${encodeURIComponent(D)}/history?limit=10`)}catch{m.value=[]}k.value=!1}async function P(){l.value=null;const D=a.value;if(!D.description.trim()){l.value="Description is required";return}if(!D.channel_id.trim()){l.value="Channel ID is required";return}if(!D.cron.trim()&&!D.run_at.trim()){l.value="Cron expression or run_at time is required";return}const I={description:D.description.trim(),action:D.action,channel_id:D.channel_id.trim()};if(D.cron.trim()&&(I.cron=D.cron.trim()),D.run_at.trim()&&(I.run_at=D.run_at.trim()),D.action==="reminder"&&D.message.trim()&&(I.message=D.message.trim()),D.action==="check"&&(D.tool_name.trim()&&(I.tool_name=D.tool_name.trim()),D.tool_input_str.trim()))try{I.tool_input=JSON.parse(D.tool_input_str.trim())}catch{l.value="Tool input must be valid JSON";return}i.value=!0;try{await q.post("/api/schedules",I),ke.success("Schedule created"),a.value={description:"",action:"reminder",channel_id:"",cron:"",run_at:"",message:"",tool_name:"",tool_input_str:""},r.value=null,n.value=!1,await L()}catch(z){l.value=z.message}i.value=!1}async function S(D){u.value=D;try{const I=await q.post(`/api/schedules/${encodeURIComponent(D)}/run`);if(I.status==="failure")ke.error(`Execution failed: ${I.error||"unknown error"}`);else{const z=I.warning?`Executed (${I.warning})`:"Executed successfully";ke.success(z)}await L()}catch(I){ke.error(I.message||"Failed to trigger")}u.value=null}async function $(D){f.value=D.id;const I=!D.paused;try{await q.put(`/api/schedules/${encodeURIComponent(D.id)}`,{paused:I}),ke.success(I?"Schedule paused":"Schedule resumed"),await L()}catch(z){ke.error(z.message||"Failed to update schedule")}f.value=null}async function W(D){p.value=D;try{await q.post(`/api/schedules/${encodeURIComponent(D)}/reset-failures`),ke.success("Failure counters reset"),await L()}catch(I){ke.error(I.message||"Failed to reset")}p.value=null}async function G(D){const I=e.value.find(ce=>ce.id===D);if(await ls({title:"Delete schedule",message:`Delete "${(I==null?void 0:I.description)||D}"? This cannot be undone.`,confirmLabel:"Delete",danger:!0})){d.value=D;try{await q.del(`/api/schedules/${encodeURIComponent(D)}`),ke.success("Schedule deleted"),await L()}catch(ce){ke.error(ce.message||"Failed to delete schedule")}d.value=null}}return Be(()=>{L()}),{schedules:e,loading:t,error:s,showCreate:n,form:a,creating:i,createError:l,cronResult:r,validatingCron:o,cronPresets:c,runningId:u,deletingId:d,togglingId:f,resettingId:p,expandedId:b,history:m,historyLoading:k,cronCount:E,oneTimeCount:v,webhookCount:y,pausedCount:x,failingCount:T,formatTs:Da,formatAge:Qh,formatFuture:N,formatMs:O,formatDuration:Ia,onCronInput:w,validateCron:R,toggleExpand:U,fetchSchedules:L,doCreate:P,doRunNow:S,doTogglePause:$,doResetFailures:W,doDelete:G}}},lw={components:{TabbedPage:dr},setup(){return{tabs:[{id:"live",label:"Live",component:tw},{id:"agents",label:"Agents",component:sw},{id:"loops",label:"Loops",component:nw},{id:"processes",label:"Processes",component:aw},{id:"schedules",label:"Schedules",component:iw}]}},template:'<tabbed-page :tabs="tabs" default-tab="live" group-label="Operations" />'},rw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(null),a=h(null),i=h(!1),l=h(null),r=h(!1),o=h(new Set),c=h(!1),u=h("all"),d=h(""),f=h("last_active"),p=h(!1),b=zd,m=ow,k=h([]),E=h(!1),v=h(""),y=h("flat"),x=h(new Set),T=h(""),N=h(""),O=h(""),w=h(null),R=h(!1);function L(){try{const B=localStorage.getItem("odin-session-presets");B&&(k.value=JSON.parse(B))}catch{}}function U(){try{localStorage.setItem("odin-session-presets",JSON.stringify(k.value))}catch{}}const P=Y(()=>d.value.trim()!==""||u.value!=="all"),S=Y(()=>{let B=[...e.value];const re=zd.find(Je=>Je.id===u.value),ye=re?re.filters:{};if(ye.source&&(B=B.filter(Je=>Je.source===ye.source)),ye.minMessages&&(B=B.filter(Je=>Je.message_count>=ye.minMessages)),ye.hasCompaction&&(B=B.filter(Je=>Je.has_summary)),ye.maxAge!=null){const Je=Date.now()/1e3;B=B.filter(xt=>xt.last_active&&Je-xt.last_active<=ye.maxAge)}if(d.value.trim()){const Je=d.value.toLowerCase().trim();B=B.filter(xt=>(xt.channel_id||"").toLowerCase().includes(Je)||(xt.last_user_id||"").toLowerCase().includes(Je)||(xt.source||"").toLowerCase().includes(Je))}const Le=f.value,He=p.value?1:-1;return B.sort((Je,xt)=>{const et=Je[Le]||0,Ye=xt[Le]||0;return(et-Ye)*He}),B}),$=Y(()=>{if(!a.value||!a.value.messages)return[];const B=a.value.messages;if(B.length===0)return[];const re=[];let ye=[];for(const Le of B)Le.role==="user"&&ye.length>0&&(re.push(ye),ye=[]),ye.push(Le);return ye.length>0&&re.push(ye),re}),W=Y(()=>S.value.length>0&&o.value.size===S.value.length);function G(B){const re=B.find(ye=>ye.role==="user");if(re&&re.content){const ye=re.content.slice(0,120);return ye.length<re.content.length?ye+"...":ye}return"(no user message)"}function D(B){const re=new Set(x.value);re.has(B)?re.delete(B):re.add(B),x.value=re}function I(B){u.value=B}function z(B){u.value=B.id,B.filters.searchQuery!=null&&(d.value=B.filters.searchQuery),B.filters.sortBy&&(f.value=B.filters.sortBy)}function ce(){if(!v.value.trim())return;const B={id:"custom-"+Date.now(),name:v.value.trim(),filters:{searchQuery:d.value,sortBy:f.value}};k.value=[...k.value,B],U(),E.value=!1,v.value=""}function de(B){k.value=k.value.filter(re=>re.id!==B),U(),u.value===B&&(u.value="all")}function se(){u.value="all",d.value="",f.value="last_active",p.value=!1}function pe(B){if(!B)return"—";const re=Date.now()/1e3-B;if(re<60)return"just now";if(re<3600){const Le=Math.floor(re/60);return`${Le} minute${Le!==1?"s":""} ago`}if(re<86400){const Le=Math.floor(re/3600);return`${Le} hour${Le!==1?"s":""} ago`}const ye=Math.floor(re/86400);return`${ye} day${ye!==1?"s":""} ago`}function ee(B){if(!B)return"";try{return new Date(B*1e3).toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return""}}function ue(B){if(!B)return"";try{return new Date(B*1e3).toLocaleString()}catch{return""}}function Ie(B){return B==="user"?"bg-gray-900/50 border border-gray-800":B==="assistant"?"bg-indigo-950/30 border border-indigo-900/30":"bg-gray-900/30 border border-gray-800/50"}function j(B){return B==="user"?"sess-msg-user":B==="assistant"?"sess-msg-assistant":"sess-msg-system"}function oe(B){return B==="user"?"badge-info":B==="assistant"?"badge-success":"badge-warning"}function ie(B){return B==="user"?"sess-dot-user":B==="assistant"?"sess-dot-assistant":"sess-dot-system"}function ge(B){return B==="user"?"text-cyan-400":B==="assistant"?"text-indigo-400":"text-gray-500"}function be(B){return B?B.length>2e3?B.slice(0,2e3)+`
... (truncated)`:B:""}async function Oe(){const B=T.value.trim();if(B){R.value=!0;try{let re=`/api/sessions/search?q=${encodeURIComponent(B)}&limit=50`;N.value.trim()&&(re+=`&channel_id=${encodeURIComponent(N.value.trim())}`),O.value.trim()&&(re+=`&user_id=${encodeURIComponent(O.value.trim())}`);const ye=await q.get(re);w.value=ye.results||[]}catch{w.value=[]}R.value=!1}}function g(){T.value="",N.value="",O.value="",w.value=null}function C(B){return B?B.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/&gt;&gt;&gt;/g,'<mark class="fts-highlight">').replace(/&lt;&lt;&lt;/g,"</mark>"):""}function M(B){return B==="user"?"fts-result-user":B==="assistant"?"fts-result-assistant":B==="summary"?"fts-result-summary":B==="fts"?"fts-result-fts":B==="channel"?"fts-result-channel":"fts-result-default"}function Z(B){return B==="user"?"badge-info":B==="assistant"?"badge-success":B==="summary"?"badge-warning":B==="fts"?"badge-success":"badge-info"}async function A(){t.value=!0,s.value=null;try{e.value=await q.get("/api/sessions")}catch(B){s.value=B.message}t.value=!1}function F(){s.value=null,A()}async function J(B){if(n.value===B){n.value=null,a.value=null,x.value=new Set;return}n.value=B,a.value=null,i.value=!0,x.value=new Set;try{a.value=await q.get(`/api/sessions/${encodeURIComponent(B)}`)}catch{a.value={messages:[],summary:""}}i.value=!1}function X(B){const re=new Set(o.value);re.has(B)?re.delete(B):re.add(B),o.value=re}function te(){W.value?o.value=new Set:o.value=new Set(S.value.map(B=>B.channel_id))}function Q(B){l.value=B}async function me(){if(l.value){r.value=!0;try{await q.del(`/api/sessions/${encodeURIComponent(l.value)}`),n.value===l.value&&(n.value=null,a.value=null),o.value.delete(l.value),await A()}catch(B){s.value=B.message||"Failed to clear session"}r.value=!1,l.value=null}}function le(){c.value=!0}async function fe(){if(o.value.size!==0){r.value=!0;try{await q.post("/api/sessions/clear-bulk",{channel_ids:[...o.value]}),o.value.has(n.value)&&(n.value=null,a.value=null),o.value=new Set,await A()}catch(B){s.value=B.message||"Failed to clear sessions"}r.value=!1,c.value=!1}}function xe(B,re){const ye=q._token;let Le=`/api/sessions/${encodeURIComponent(B)}/export?format=${re}`;ye&&(Le+=`&token=${encodeURIComponent(ye)}`);const He=document.createElement("a");He.href=Le,He.download=`session-${B}.${re==="text"?"txt":"json"}`,document.body.appendChild(He),He.click(),document.body.removeChild(He)}let Se=null;function Ae(B){B.payload&&B.payload.channel_id&&(clearTimeout(Se),Se=setTimeout(()=>{A(),n.value&&B.payload.channel_id===n.value&&q.get(`/api/sessions/${encodeURIComponent(n.value)}`).then(re=>{a.value=re}).catch(()=>{})},2e3))}return Be(()=>{L(),A(),We.subscribe("events",Ae)}),ht(()=>{We.unsubscribe("events",Ae),clearTimeout(Se)}),{sessions:e,loading:t,error:s,expandedId:n,detail:a,detailLoading:i,clearTarget:l,clearing:r,selected:o,allSelected:W,bulkClearing:c,activePreset:u,searchQuery:d,sortBy:f,sortAsc:p,filterPresets:b,sortOptions:m,filteredSessions:S,hasActiveFilters:P,customPresets:k,showSavePreset:E,newPresetName:v,threadView:y,threads:$,collapsedThreads:x,ftsQuery:T,ftsChannelId:N,ftsUserId:O,ftsResults:w,ftsSearching:R,formatAge:pe,formatTimestamp:ee,formatFullTimestamp:ue,messageClass:Ie,threadMsgClass:j,roleBadge:oe,roleDotClass:ie,roleLabelClass:ge,truncateContent:be,threadSummary:G,fetchSessions:A,retry:F,toggleSession:J,toggleSelect:X,toggleSelectAll:te,confirmClear:Q,clearSession:me,confirmBulkClear:le,doBulkClear:fe,exportSession:xe,applyPreset:I,applyCustomPreset:z,saveCustomPreset:ce,removeCustomPreset:de,resetFilters:se,toggleThread:D,runFtsSearch:Oe,clearFtsSearch:g,highlightSnippet:C,ftsResultClass:M,ftsTypeBadge:Z}}},uw={props:["trace"],template:`
  ...
  `,setup(){return{formatTokens:M_}}},dw={components:{ContextAssemblyPanel:uw},template:`
    ...
    </div>`,setup(){const e=h([]),t=h([]),s=h(!0),n=h(null),a=h(null),i=h(null),l=h(""),r=h(""),o=h(0),c=h({}),u=h({channel_id:"",user_id:"",tool_name:"",errors_only:!1,limit:50});function d(O){if(!O)return"—";try{const w=new Date(O);return isNaN(w.getTime())?O:w.toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"})}catch{return O}}function f(O){return!O&&O!==0?"—":O<1e3?O+"ms":(O/1e3).toFixed(1)+"s"}function p(O){return!O&&O!==0?"—":O>=1e3?(O/1e3).toFixed(1)+"k":String(O)}function b(O){if(!O)return"";if(typeof O=="string")return O;try{return JSON.stringify(O,null,2)}catch{return String(O)}}function m(O){a.value===O?a.value=null:(a.value=O,c.value={})}function k(O,w){const R=O+"-"+w;c.value={...c.value,[R]:!c.value[R]}}function E(O,w){return!!c.value[O+"-"+w]}function v(){u.value={channel_id:"",user_id:"",tool_name:"",errors_only:!1,limit:50},r.value="",l.value="",i.value=null,T()}async function y(){try{const O=await q.get("/api/trajectories");e.value=O.files||[],o.value=O.count||0}catch{}}let x=0;async function T(){const O=++x;s.value=!0,n.value=null,a.value=null,i.value=null,c.value={};try{if(r.value){const w=await q.get(`/api/trajectories/${encodeURIComponent(r.value)}?limit=${u.value.limit}`);if(O!==x)return;let R=w.entries||[];u.value.tool_name&&(R=R.filter(L=>(L.tools_used||[]).includes(u.value.tool_name))),u.value.errors_only&&(R=R.filter(L=>L.is_error)),u.value.channel_id&&(R=R.filter(L=>L.channel_id===u.value.channel_id)),u.value.user_id&&(R=R.filter(L=>L.user_id===u.value.user_id)),t.value=R}else{const w=new URLSearchParams;u.value.channel_id&&w.set("channel_id",u.value.channel_id),u.value.user_id&&w.set("user_id",u.value.user_id),u.value.tool_name&&w.set("tool_name",u.value.tool_name),u.value.errors_only&&w.set("errors_only","true"),w.set("limit",String(u.value.limit));const R=w.toString(),L=await q.get(`/api/trajectories/search/query?${R}`);if(O!==x)return;t.value=L.results||[]}}catch(w){if(O!==x)return;n.value=w.message}O===x&&(s.value=!1)}async function N(){if(!l.value.trim())return;const O=++x;s.value=!0,n.value=null,c.value={};try{const w=await q.get(`/api/trajectories/message/${encodeURIComponent(l.value.trim())}`);if(O!==x)return;i.value=w.entry||null,i.value||(n.value="No trace found for this message ID")}catch(w){if(O!==x)return;w.status===404?(i.value=null,n.value="No trace found for message ID: "+l.value):n.value=w.message}O===x&&(s.value=!1)}return Be(async()=>{await y(),await T()}),{files:e,entries:t,loading:s,error:n,expandedIdx:a,singleTrace:i,messageIdQuery:l,selectedFile:r,totalSaved:o,filters:u,expandedIterations:c,formatTs:d,formatDuration:f,formatTokens:p,formatJSON:b,truncateBlock:Xh,toggleExpand:m,toggleIteration:k,isIterationExpanded:E,clearFilters:v,fetchFiles:y,fetchTraces:T,lookupMessage:N}}},fw={template:`
  ...
  `,setup(){const e=h(!0),t=h(null),s=h({by_user:{},by_channel:{},by_tool:{},recent:[],pricing:{}}),n=h({requests:0,input_tokens:0,output_tokens:0,total_tokens:0,cost_usd:0}),a=h("user");let i=null;const l=[{key:"user",label:"By User"},{key:"channel",label:"By Channel"},{key:"tool",label:"By Tool"},{key:"recent",label:"Recent"}],r=Y(()=>[...s.value.recent||[]].reverse()),o=async()=>{try{const u=await q.get("/api/usage");s.value=u,n.value=u.totals||n.value,t.value=null}catch(u){t.value=u.message}finally{e.value=!1}},c=()=>{e.value=!0,o()};return Be(()=>{o(),i=setInterval(o,15e3)}),ht(()=>{i&&clearInterval(i)}),{loading:e,error:t,data:s,totals:n,activeTab:a,tabs:l,recentReversed:r,fmtNum:em,formatTime:Cc,retry:c}}},pw={components:{TabbedPage:dr},setup(){return{tabs:[{id:"audit",label:"Audit",component:rw},{id:"sessions",label:"Sessions",component:cw},{id:"traces",label:"Traces",component:dw},{id:"usage",label:"Usage",component:fw}]}},template:'<tabbed-page :tabs="tabs" default-tab="audit" group-label="History" />'},Ur=[{id:"system",label:"System & Commands",icon:"terminal",match:e=>/^(run_command|run_script|read_file|write_file|list_directory|search_files|manage_process|file_|post_file)/.test(e)},{id:"devops",label:"DevOps & Infrastructure",icon:"server",match:e=>/^(git_ops|docker_ops|kubectl|terraform_ops|http_probe)/.test(e)},{id:"agents",label:"Agents & Orchestration",icon:"bot",match:e=>/^(spawn_agent|send_to_agent|wait_for_agents|get_agent_results|kill_agent|list_agents|spawn_loop_agents|collect_loop_agents)/.test(e)},{id:"workflow",label:"Workflows & Tasks",icon:"workflow",match:e=>/^(delegate_task|cancel_task|list_tasks|schedule_|start_loop|stop_loop|list_loops|delete_schedule|list_schedules|update_schedule|parse_time)/.test(e)},{id:"network",label:"Network & Web",icon:"globe",match:e=>/^(web_|browser_|search_web|fetch_url|http_)/.test(e)},{id:"knowledge",label:"Knowledge & Search",icon:"book",match:e=>/^(search_knowledge|ingest_|knowledge_|search_history|search_audit|bulk_ingest|delete_knowledge|list_knowledge)/.test(e)},{id:"discord",label:"Discord & Admin",icon:"message",match:e=>/^(send_|add_reaction|create_poll|purge_|discord_|embed_|read_channel|set_permission)/.test(e)},{id:"skills",label:"Skills",icon:"puzzle",match:e=>/^(create_skill|edit_skill|delete_skill|enable_skill|disable_skill|install_skill|export_skill|skill_status|invoke_skill|list_skills)/.test(e)},{id:"memory",label:"Memory & State",icon:"brain",match:e=>/^(memory_manage|list_manage)/.test(e)},{id:"ai",label:"AI & Generation",icon:"sparkles",match:e=>/^(generate_|analyze_|claude_|vision_|comfyui_)/.test(e)},{id:"integrations",label:"Integrations",icon:"link",match:e=>/^(issue_tracker|slack_|grafana_|mcp_)/.test(e)},{id:"other",label:"Other Tools",icon:"wrench",match:()=>!0}],hw={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(""),a=h({}),i=h({}),l=h("cards"),r=h(null),o=Y(()=>e.value.filter(v=>v.is_core).length),c=Y(()=>e.value.filter(v=>!v.is_core).length),u=Y(()=>Object.values(a.value).reduce((v,y)=>v+y,0));function d(v){for(const y of Ur)if(y.id!=="other"&&y.match(v))return y.id;return"other"}const f=Y(()=>{let v=e.value;if(n.value){const y=n.value.toLowerCase();v=v.filter(x=>x.name.toLowerCase().includes(y)||(x.description||"").toLowerCase().includes(y))}return r.value&&(v=v.filter(y=>d(y.name)===r.value)),v}),p=Y(()=>{const v=new Set;for(const y of e.value)v.add(d(y.name));return Ur.filter(y=>v.has(y.id))}),b=Y(()=>{const v=f.value,y={};for(const T of v){const N=d(T.name);y[N]||(y[N]=[]),y[N].push(T)}const x=[];for(const T of Ur)y[T.id]&&y[T.id].length>0&&x.push({label:T.label,icon:T.icon,tools:y[T.id].sort((N,O)=>N.name.localeCompare(O.name))});return x});function m(v){i.value={...i.value,[v]:!i.value[v]}}async function k(){t.value=!0,s.value=null;try{const[v,y]=await Promise.all([q.get("/api/tools"),q.get("/api/tools/stats").catch(()=>({}))]);e.value=v,a.value=y||{};const x=Object.values(y||{}).filter(T=>T>0).sort((T,N)=>T-N)}catch(v){s.value=v.message}t.value=!1}function E(){k()}return Be(()=>{k()}),{tools:e,loading:t,error:s,search:n,stats:a,expanded:i,viewMode:l,activeCategory:r,coreCount:o,skillCount:c,totalUsage:u,filteredTools:f,groupedTools:b,usedCategories:p,truncate:Ec,toggleExpand:m,refresh:E}}};function mw(e){if(!e)return"";let t=e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");t=t.replace(/("""[\s\S]*?"""|'''[\s\S]*?'''|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g,'<span class="sk-str">$1</span>'),t=t.replace(/(#[^\n]*)/g,'<span class="sk-cmt">$1</span>');const s="\\b(def|class|return|if|elif|else|for|while|import|from|as|try|except|finally|raise|with|async|await|yield|pass|break|continue|and|or|not|in|is|None|True|False|self|lambda)\\b";t=t.replace(new RegExp(s,"g"),'<span class="sk-kw">$1</span>');const n="\\b(print|len|range|str|int|float|list|dict|set|tuple|type|isinstance|hasattr|getattr|setattr|super|property|staticmethod|classmethod|enumerate|zip|map|filter|sorted|reversed|any|all|min|max|sum|abs|round|open|format)\\b";return t=t.replace(new RegExp(n,"g"),'<span class="sk-builtin">$1</span>'),t=t.replace(/(@\w+)/g,'<span class="sk-dec">$1</span>'),t=t.replace(/\b(\d+\.?\d*)\b/g,'<span class="sk-num">$1</span>'),t}function gw(e){if(!e)return"1";const t=e.split(`
...
`)}const vw={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h({}),a=h({}),i=h(null),l=h(""),r=h(null),o=h(!1),c=h("create"),u=h(""),d=h(""),f=h(null),p=h(null),b=h(!1),m=h(null),k=h(null),E=h(!1),v=Y(()=>e.value.length),y=Y(()=>e.value.reduce((ee,ue)=>ee+(ue.execution_count||0),0)),x=Y(()=>e.value.reduce((ee,ue)=>ee+L(ue.code),0)),T=Y(()=>{if(!l.value)return e.value;const ee=l.value.toLowerCase();return e.value.filter(ue=>ue.name.toLowerCase().includes(ee)||(ue.description||"").toLowerCase().includes(ee))}),N=Y(()=>d.value?d.value.split(`
`).length:0),O=Y(()=>{const ee=Math.max(N.value,1);return Array.from({length:ee},(ue,Ie)=>Ie+1).join(`
`)}),w=Y(()=>{const ee=d.value.trim();return ee?ee.includes("SKILL_DEFINITION")?ee.includes("async def execute")?{valid:!0,message:""}:{valid:!1,message:"Missing async def execute function"}:{valid:!1,message:"Missing SKILL_DEFINITION dict"}:null});function R(ee){return mw(ee)}function L(ee){return ee?ee.split(`
`).length:0}function U(ee){return gw(ee)}function P(ee){n.value={...n.value,[ee]:!n.value[ee]}}async function S(ee){try{await navigator.clipboard.writeText(ee);const ue=e.value.find(Ie=>Ie.code===ee);ue&&(r.value=ue.name,setTimeout(()=>{r.value=null},2e3))}catch{}}function $(ee){if(ee.key==="Tab"){ee.preventDefault();const ue=ee.target,Ie=ue.selectionStart,j=ue.selectionEnd;d.value=d.value.substring(0,Ie)+"    "+d.value.substring(j),Et(()=>{ue.selectionStart=ue.selectionEnd=Ie+4})}}function W(ee){const ue=ee.target.previousElementSibling;ue&&(ue.scrollTop=ee.target.scrollTop)}async function G(){t.value=!0,s.value=null;try{e.value=await q.get("/api/skills")}catch(ee){s.value=ee.message}t.value=!1}async function D(ee){i.value=ee,delete a.value[ee],a.value={...a.value};try{const ue=await q.post(`/api/skills/${encodeURIComponent(ee)}/test`);a.value={...a.value,[ee]:ue}}catch(ue){a.value={...a.value,[ee]:{result:ue.message,is_error:!0}}}i.value=null}function I(){o.value=!0,c.value="create",u.value="",d.value="",f.value=null,p.value=null}function z(ee){o.value=!0,c.value="edit",u.value=ee.name,d.value=ee.code||"",f.value=null,p.value=null}function ce(){o.value=!1,f.value=null,p.value=null}async function de(){f.value=null,p.value=null;const ee=u.value.trim(),ue=d.value.trim();if(!ee){f.value="Name is required";return}if(!ue){f.value="Code is required";return}b.value=!0;try{c.value==="create"?(await q.post("/api/skills",{name:ee,code:ue}),p.value="Skill created successfully"):(await q.put(`/api/skills/${encodeURIComponent(ee)}`,{code:ue}),p.value="Skill updated successfully"),await G(),setTimeout(()=>{o.value=!1},800)}catch(Ie){f.value=Ie.message}b.value=!1}function se(ee){k.value=ee}async function pe(){if(k.value){E.value=!0;try{await q.del(`/api/skills/${encodeURIComponent(k.value)}`),await G()}catch{}E.value=!1,k.value=null}}return Be(()=>{G()}),{skills:e,loading:t,error:s,showCode:n,testResults:a,testing:i,search:l,copied:r,editing:o,editMode:c,editName:u,editCode:d,editError:f,editSuccess:p,saving:b,editorRef:m,deleteTarget:k,deleting:E,enabledCount:v,totalExecutions:y,totalLines:x,displayedSkills:T,editLineCount:N,editorLineNums:O,editValidation:w,highlight:R,truncate:Ec,formatTs:Da,countLines:L,getLineNumbers:U,toggleCode:P,copyCode:S,handleEditorKey:$,syncScroll:W,fetchSkills:G,testSkill:D,showCreate:I,editSkill:z,cancelEdit:ce,saveSkill:de,confirmDelete:se,doDelete:pe}}};function bw(e,t){if(!e||!t)return kd(e);const s=kd(e),n=t.trim().split(/\s+/).filter(Boolean);if(!n.length)return s;const a=n.map(i=>i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")).join("|");try{return s.replace(new RegExp(`(${a})`,"gi"),'<mark class="knowledge-highlight">$1</mark>')}catch{return s}}const yw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(""),a=h(null),i=h(!1),l=h(""),r=h(null),o=h(!1),c=h(""),u=h(""),d=h(null),f=h(null),p=h(!1),b=h(null),m=h(null);let k=null;const E=h(null),v=h(!1),y=h({}),x=h({}),T=h(null),N=h(null),O=Y(()=>e.value.reduce((I,z)=>I+(z.chunks||0),0)),w=Y(()=>new Set(e.value.map(z=>z.uploader).filter(Boolean)).size);function R(I,z){const ce=x.value[z];if(!ce||ce.length===0)return 0;const de=Math.max(...ce.map(se=>se.char_count||0));return de===0?0:Math.round(I.char_count/de*100)}async function L(){t.value=!0,s.value=null;try{const I=await q.get("/api/knowledge");e.value=Array.isArray(I)?I:[]}catch(I){s.value=I.message}t.value=!1}async function U(I){if(y.value[I]){y.value[I]=!1,N.value=null;return}if(y.value[I]=!0,!(x.value[I]||T.value===I)){T.value=I;try{const z=await q.get(`/api/knowledge/${encodeURIComponent(I)}/chunks`);x.value[I]=Array.isArray(z)?z:[]}catch(z){x.value[I]=[],ke.error(`Failed to load chunks: ${z.message}`)}T.value=null}}async function P(){const I=n.value.trim();if(I){i.value=!0,r.value=null,l.value=I;try{const z=await q.get(`/api/knowledge/search?q=${encodeURIComponent(I)}`);a.value=Array.isArray(z)?z:[]}catch(z){a.value=[],r.value=z.message||"Search failed"}i.value=!1}}function S(){a.value=null,n.value="",r.value=null}async function $(){d.value=null,f.value=null;const I=c.value.trim(),z=u.value.trim();if(!I){d.value="Source name is required";return}if(!z){d.value="Content is required";return}p.value=!0;try{const ce=await q.post("/api/knowledge",{source:I,content:z});f.value=`Ingested ${ce.chunks||0} chunks from "${I}"`,c.value="",u.value="",x.value={},await L(),setTimeout(()=>{o.value=!1,f.value=null},1500)}catch(ce){d.value=ce.message}p.value=!1}async function W(I){b.value=I,m.value=null,k&&(clearTimeout(k),k=null);try{const z=await q.post(`/api/knowledge/${encodeURIComponent(I)}/reingest`);m.value={source:I,error:!1,message:`Re-ingested ${z.chunks||0} chunks`},delete x.value[I],await L(),k=setTimeout(()=>{m.value=null,k=null},3e3)}catch(z){m.value={source:I,error:!0,message:z.message}}b.value=null}function G(I){E.value=I}async function D(){if(E.value){v.value=!0;try{await q.del(`/api/knowledge/${encodeURIComponent(E.value)}`),delete x.value[E.value],await L()}catch{}v.value=!1,E.value=null}}return Be(()=>{L()}),{sources:e,loading:t,error:s,searchQuery:n,searchResults:a,searching:i,lastQuery:l,searchError:r,showIngest:o,ingestSource:c,ingestContent:u,ingestError:d,ingestSuccess:f,ingesting:p,reingesting:b,reingestResult:m,deleteTarget:E,deleting:v,expanded:y,sourceChunks:x,loadingChunks:T,selectedChunk:N,totalChunks:O,uploaderCount:w,truncate:Ec,formatTs:Da,highlightTerms:bw,chunkBarWidth:R,fetchSources:L,toggleSource:U,doSearch:P,clearSearch:S,doIngest:$,doReingest:W,confirmDelete:G,doDelete:D}}},xw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h({}),s=h(!0),n=h(null),a=h({}),i=h(null),l=h(""),r=h(!1),o=h({scope:"global",key:"",value:""}),c=h(!1),u=h(null),d=h(null),f=h(null),p=h(""),b=h(!1),m=h(null),k=h(null),E=h(new Set),v=h(null),y=h(!1),x=h(!1),T=Y(()=>e.value.reduce((se,pe)=>se+pe.count,0)),N=Y(()=>E.value.size);function O(se){const pe=t.value[se];if(!pe)return[];if(!l.value.trim())return pe;const ee=l.value.trim().toLowerCase();return pe.filter(ue=>ue.key.toLowerCase().includes(ee)||ue.value&&ue.value.toLowerCase().includes(ee))}function w(se,pe){return E.value.has(se+"/"+pe)}function R(se,pe){const ee=se+"/"+pe,ue=new Set(E.value);ue.has(ee)?ue.delete(ee):ue.add(ee),E.value=ue}function L(se){const pe=t.value[se];return!pe||pe.length===0?!1:pe.every(ee=>E.value.has(se+"/"+ee.key))}function U(se,pe){const ee=t.value[se];if(!ee)return;const ue=new Set(E.value);for(const Ie of ee){const j=se+"/"+Ie.key;pe?ue.add(j):ue.delete(j)}E.value=ue}async function P(){s.value=!0,n.value=null;try{const se=await q.get("/api/memory");e.value=Object.entries(se).map(([pe,ee])=>({name:pe,keys:ee.keys||[],count:ee.count||0}))}catch(se){n.value=se.message}s.value=!1}async function S(se){if(a.value[se]){a.value[se]=!1;return}a.value[se]=!0;const pe=e.value.find(ue=>ue.name===se);if(!pe||t.value[se]||i.value===se)return;i.value=se;const ee=await Promise.all(pe.keys.map(async ue=>{try{const Ie=await q.get(`/api/memory/${encodeURIComponent(se)}/${encodeURIComponent(ue)}`);return{key:ue,value:Ie.value||""}}catch{return{key:ue,value:"(error loading)"}}}));t.value[se]=ee,i.value=null}function $(se,pe,ee){f.value=se+"/"+pe,p.value=ee}async function W(se,pe){b.value=!0,m.value=null;try{await q.put(`/api/memory/${encodeURIComponent(se)}/${encodeURIComponent(pe)}`,{value:p.value});const ee=t.value[se];if(ee){const ue=ee.find(Ie=>Ie.key===pe);ue&&(ue.value=p.value)}f.value=null}catch(ee){m.value=`Failed to save: ${ee.message||"unknown error"}`}b.value=!1}async function G(se,pe){try{await navigator.clipboard.writeText(pe.value),k.value=se+"/"+pe.key,setTimeout(()=>{k.value=null},1500)}catch{}}async function D(){u.value=null,d.value=null;const se=o.value.scope.trim(),pe=o.value.key.trim(),ee=o.value.value.trim();if(!se){u.value="Scope is required";return}if(!pe){u.value="Key is required";return}if(!ee){u.value="Value is required";return}c.value=!0;try{await q.put(`/api/memory/${encodeURIComponent(se)}/${encodeURIComponent(pe)}`,{value:ee}),d.value="Entry saved",o.value={scope:"global",key:"",value:""},t.value={},await P(),setTimeout(()=>{r.value=!1,d.value=null},800)}catch(ue){u.value=ue.message}c.value=!1}function I(se,pe){v.value={scope:se,key:pe}}async function z(){if(!v.value)return;y.value=!0,m.value=null;const{scope:se,key:pe}=v.value;try{await q.del(`/api/memory/${encodeURIComponent(se)}/${encodeURIComponent(pe)}`);const ee=t.value[se];ee&&(t.value[se]=ee.filter(j=>j.key!==pe));const ue=e.value.find(j=>j.name===se);ue&&(ue.count--,ue.keys=ue.keys.filter(j=>j!==pe));const Ie=new Set(E.value);Ie.delete(se+"/"+pe),E.value=Ie}catch(ee){m.value=`Failed to delete: ${ee.message||"unknown error"}`}y.value=!1,v.value=null}function ce(){x.value=!0}async function de(){y.value=!0,m.value=null;const se=[];for(const pe of E.value){const ee=pe.indexOf("/");se.push({scope:pe.slice(0,ee),key:pe.slice(ee+1)})}try{await q.post("/api/memory/bulk-delete",{entries:se}),E.value=new Set,t.value={},await P()}catch(pe){m.value=`Bulk delete failed: ${pe.message||"unknown error"}`}y.value=!1,x.value=!1}return Be(()=>{P()}),{scopes:e,scopeEntries:t,loading:s,error:n,expanded:a,loadingScope:i,filterQuery:l,showAdd:r,addForm:o,adding:c,addError:u,addSuccess:d,editingKey:f,editValue:p,saving:b,actionError:m,copied:k,selected:E,selectedCount:N,totalEntries:T,deleteTarget:v,deleting:y,showBulkDelete:x,fetchMemory:P,toggleScope:S,startEdit:$,doEdit:W,copyValue:G,doAdd:D,confirmDelete:I,doDelete:z,confirmBulkDelete:ce,doBulkDelete:de,isSelected:w,toggleSelect:R,isScopeAllSelected:L,toggleSelectAll:U,filteredEntries:O}}},_w={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
        <div>
  ...
  `,setup(){const e=h([]),t=h(null),s=h(!0),n=h(null),a=h(null),i=h(null),l=h(""),r=Y(()=>[...new Set(e.value.map(k=>k.category))].sort()),o=Y(()=>{const m={};return e.value.forEach(k=>{m[k.category]=(m[k.category]||0)+1}),m}),c=Y(()=>a.value?e.value.filter(m=>m.category===a.value):e.value);function u(m){return m==="correction"?"badge-warning":m==="operational"?"badge-info":m==="preference"?"badge-success":"badge-info"}function d(m){i.value=m.key,l.value=m.content}async function f(m){try{await q.put("/api/learned/"+encodeURIComponent(m),{content:l.value}),i.value=null,ke.success("Entry updated"),await b()}catch(k){ke.error(k.message||"Failed to save entry")}}async function p(m){if(await ls({title:"Delete learned entry",message:`Delete "${m}"? Odin will no longer apply this learned context.`,confirmLabel:"Delete",danger:!0}))try{await q.del("/api/learned/"+encodeURIComponent(m)),ke.success("Entry deleted"),await b()}catch(E){ke.error(E.message||"Failed to delete entry")}}async function b(){s.value=!0,n.value=null;try{const m=await q.get("/api/learned");e.value=m.entries||[],t.value={last_reflection:m.last_reflection,count:m.count}}catch(m){n.value=m.message}s.value=!1}return Be(b),{entries:e,meta:t,loading:s,error:n,filterCat:a,editing:i,editContent:l,categories:r,catCounts:o,filtered:c,catBadge:u,formatTs:Da,startEdit:d,saveEdit:f,deleteEntry:p,fetchEntries:b}}},kw={components:{TabbedPage:dr},setup(){return{tabs:[{id:"tools",label:"Tools",component:hw},{id:"skills",label:"Skills",component:vw},{id:"knowledge",label:"Knowledge",component:yw},{id:"memory",label:"Memory",component:xw},{id:"learned",label:"Learned",component:_w}]}},template:'<tabbed-page :tabs="tabs" default-tab="tools" group-label="Capabilities" />'},ww={setup(){const e=h("odin"),t=h(""),s=h(""),n=h(""),a=h({}),i=h([]),l=h([]),r=h(!1),o=h(!1),c=h(null),u=h(!0),d=h(""),f=h(!1),p=h(!1),b=Y(()=>e.value==="custom"),m=Y(()=>[...i.value,...l.value]),k=Y(()=>l.value.includes(e.value)),E=Y(()=>{var w;return b.value?t.value||"Odin":((w=a.value[e.value])==null?void 0:w.name)||e.value}),v=Y(()=>{var w;return b.value?s.value||"(empty — will use Odin default)":((w=a.value[e.value])==null?void 0:w.identity)||""}),y=Y(()=>{var w;return b.value?n.value||"(empty — will use Odin default)":((w=a.value[e.value])==null?void 0:w.voice)||""});async function x(){u.value=!0;try{const w=await q.get("/api/personality");e.value=w.preset||"odin",t.value=w.custom_name||"",s.value=w.custom_identity||"",n.value=w.custom_voice||"",a.value=w.presets||{},i.value=w.builtin_presets||[],l.value=w.user_presets||[]}catch(w){c.value=w.message}finally{u.value=!1}}async function T(){r.value=!0,c.value=null,o.value=!1;try{await q.put("/api/personality",{preset:e.value,custom_name:t.value,custom_identity:s.value,custom_voice:n.value}),o.value=!0,setTimeout(()=>o.value=!1,3e3)}catch(w){c.value=w.message}finally{r.value=!1}}async function N(){const w=d.value.trim();if(w){p.value=!0,c.value=null;try{await q.post("/api/personality/presets",{name:w,display_name:E.value,identity:v.value,voice:y.value}),f.value=!1,d.value="",await x(),e.value=w.toLowerCase().replace(/ /g,"_")}catch(R){c.value=R.message}finally{p.value=!1}}}async function O(){if(await ls({title:"Delete preset",message:`Delete preset "${e.value}"? This cannot be undone.`,confirmLabel:"Delete",danger:!0})){c.value=null;try{await q.del(`/api/personality/presets/${encodeURIComponent(e.value)}`),await x(),e.value="odin"}catch(R){c.value=R.message}}}return Be(x),{preset:e,customName:t,customIdentity:s,customVoice:n,presets:a,presetNames:m,isCustom:b,isUserPreset:k,previewName:E,previewIdentity:v,previewVoice:y,saving:r,saved:o,error:c,loading:u,save:T,showSavePreset:f,newPresetName:d,savingPreset:p,saveAsPreset:N,deletePreset:O,builtinPresets:i,userPresets:l}},template:`
  ...
  `},Sw={ok:"text-green-400",degraded:"text-yellow-400",down:"text-red-400",unconfigured:"text-gray-500"},Tw={ok:"success",degraded:"warning",down:"error",unconfigured:"minus"},Cw={healthy:"text-green-400",degraded:"text-yellow-400",unhealthy:"text-red-400"},Ew={template:`
    ...
    </div>`,setup(){const e=h({}),t=h(!0),s=h(null),n=h(!1),a=Y(()=>e.value.components||[]),i=Y(()=>Cw[e.value.overall]||"text-gray-400"),l=Y(()=>e.value.overall==="healthy"?"success":e.value.overall==="degraded"?"warning":e.value.overall==="unhealthy"?"error":"minus"),r=Y(()=>{const v=e.value.overall;return v==="healthy"?"All Systems Healthy":v==="degraded"?"Some Systems Degraded":v==="unhealthy"?"System Issues Detected":"Unknown"});function o(v){return Sw[v]||"text-gray-400"}function c(v){return Tw[v]||"info"}function u(v){return v==="ok"?"badge-success":v==="degraded"?"badge-warning":v==="down"?"badge-danger":"badge-info"}function d(v){return v==="closed"?"text-green-400":v==="half_open"?"text-yellow-400":v==="open"?"text-red-400":"text-gray-400"}function f(v){return v.replace(/_/g," ").replace(/\b\w/g,y=>y.toUpperCase())}function p(v){if(!v)return"—";try{return new Date(v).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}catch{return v}}function b(v){return v>=1e6?(v/1e6).toFixed(1)+"M":v>=1e3?(v/1e3).toFixed(1)+"K":String(v)}async function m(){n.value=!0;try{e.value=await q.get("/api/health/components"),s.value=null}catch(v){s.value=v.message}finally{t.value=!1,n.value=!1}}function k(){t.value=!0,s.value=null,m()}let E=null;return Be(async()=>{await m(),E=setInterval(m,3e4)}),ht(()=>{E&&clearInterval(E)}),{data:e,loading:t,error:s,refreshing:n,components:a,overallColor:i,overallIcon:l,overallLabel:r,statusColor:o,statusIcon:c,badgeClass:u,circuitColor:d,formatName:f,formatTime:p,formatNumber:b,fetchHealth:m,retry:k}}},Aw={template:`
  ...
  `,setup(){const e=h(!0),t=h(null),s=h(!1),n=h("sessions"),a=h(null);let i=null;const l=[{key:"sessions",label:"Sessions"},{key:"knowledge",label:"Knowledge"},{key:"trajectories",label:"Trajectories"},{key:"storage",label:"Storage"}],r=Y(()=>{if(!a.value||!a.value.collected_at)return"";try{return new Date(a.value.collected_at).toLocaleTimeString()}catch{return""}}),o=Y(()=>{if(!a.value)return[];const f=a.value,p=f.storage_total_bytes||1;return[{label:"Session Persistence",mb:f.sessions.persist_dir.total_mb,bytes:f.sessions.persist_dir.total_bytes,files:f.sessions.persist_dir.file_count,pct:Math.min(100,Math.round(f.sessions.persist_dir.total_bytes/p*100)),color:"res-bar-blue"},{label:"Knowledge Database",mb:f.knowledge.db_file.total_mb,bytes:f.knowledge.db_file.total_bytes,files:f.knowledge.db_file.file_count,pct:Math.min(100,Math.round(f.knowledge.db_file.total_bytes/p*100)),color:"res-bar-purple"},{label:"Message Trajectories",mb:f.trajectories.message_dir.total_mb,bytes:f.trajectories.message_dir.total_bytes,files:f.trajectories.message_dir.file_count,pct:Math.min(100,Math.round(f.trajectories.message_dir.total_bytes/p*100)),color:"res-bar-emerald"},{label:"Agent Trajectories",mb:f.trajectories.agent_dir.total_mb,bytes:f.trajectories.agent_dir.total_bytes,files:f.trajectories.agent_dir.file_count,pct:Math.min(100,Math.round(f.trajectories.agent_dir.total_bytes/p*100)),color:"res-bar-amber"}]});async function c(){try{const f=await q.get("/api/resource-usage");a.value=f,t.value=null}catch(f){t.value=f.message||"Failed to load resource usage"}finally{e.value=!1,s.value=!1}}async function u(){s.value=!0,await c()}function d(){e.value=!0,t.value=null,c()}return Be(()=>{c(),i=setInterval(c,3e4)}),ht(()=>{i&&clearInterval(i)}),{loading:e,error:t,refreshing:s,data:a,activeTab:n,tabs:l,collectedAt:r,storageItems:o,fmtNum:em,refresh:u,retry:d}}},Rw=["INFO","WARNING","ERROR"],Iw=[{id:"all",name:"All Logs",icon:"list",filters:{}},{id:"errors",name:"Errors Only",icon:"error",filters:{level:"ERROR"}},{id:"warnings",name:"Warnings+",icon:"warning",filters:{levels:["WARNING","ERROR"]}},{id:"tools",name:"Tool Activity",icon:"wrench",filters:{hasToolName:!0}},{id:"recent-errors",name:"Recent Errors",icon:"flame",filters:{level:"ERROR",timeRange:"last_1h"}}],Br=[{value:"",label:"All Time"},{value:"last_5m",label:"Last 5 min",seconds:300},{value:"last_15m",label:"Last 15 min",seconds:900},{value:"last_1h",label:"Last 1 hour",seconds:3600},{value:"last_4h",label:"Last 4 hours",seconds:14400},{value:"last_24h",label:"Last 24 hours",seconds:86400}],Nw=[50,100,200,500],Ow={template:`
    ...
    </div>`,setup(){const e=h("live"),t=h([]),s=h(!1),n=h(!0),a=h(""),i=h(""),l=h(!1),r=h(!1),o=h(We.state||"disconnected"),c=Y(()=>{switch(o.value){case"connected":return"Live";case"connecting":return"Connecting…";case"reconnecting":return"Reconnecting…";default:return"Disconnected"}}),u=h(null),d=h(!1),f=h(null),p=2e3,b=Rw,m=Iw,k=Br,E=h("all"),v=h(""),y=h([]),x=h(!1),T=h(""),N=h([]);function O(){try{const H=localStorage.getItem("odin-log-presets");H&&(y.value=JSON.parse(H))}catch{}}function w(){try{localStorage.setItem("odin-log-presets",JSON.stringify(y.value))}catch{}}const R=Y(()=>a.value!==""||i.value.trim()!==""||v.value!==""),L=Y(()=>{const H=Br.find(ne=>ne.value===v.value);return H?H.label:""}),U=Y(()=>{if(!l.value||!i.value)return null;try{return new RegExp(i.value,"i"),null}catch(H){return H.message}}),P=24,S=Y(()=>{if(t.value.length===0)return[];const H=[],ne=new Date,Ce=3600*1e3;for(let qe=P-1;qe>=0;qe--){const rt=new Date(ne.getTime()-(qe+1)*Ce),Wt=new Date(ne.getTime()-qe*Ce);H.push({start:rt,end:Wt,label:D(rt,Wt),shortLabel:Wt.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),total:0,info:0,warnings:0,errors:0})}for(const qe of t.value){if(!qe._time)continue;const rt=qe._time.getTime();for(const Wt of H)if(rt>=Wt.start.getTime()&&rt<Wt.end.getTime()){Wt.total++,qe.level==="ERROR"?Wt.errors++:qe.level==="WARNING"?Wt.warnings++:Wt.info++;break}}return H}),$=Y(()=>{let H=1;for(const ne of S.value)ne.total>H&&(H=ne.total);return H}),W=Y(()=>S.value.length===0?"":"Last 24 hours"),G=Y(()=>Math.ceil(P/8));function D(H,ne){const Ce={hour:"2-digit",minute:"2-digit"};return H.toLocaleTimeString([],Ce)+" - "+ne.toLocaleTimeString([],Ce)}function I(H,ne){return!ne||!H?"0px":Math.max(2,H/ne*100)+"%"}function z(H){const ne=ce.value.findIndex(Ce=>Ce._time&&Ce._time.getTime()>=H.start.getTime()&&Ce._time.getTime()<H.end.getTime());if(ne>=0&&u.value){const Ce=u.value.querySelectorAll(".log-line");Ce[ne]&&(Ce[ne].scrollIntoView({behavior:"smooth",block:"center"}),n.value=!1)}}const ce=Y(()=>{let H=t.value;if(a.value&&(H=H.filter(ne=>(ne.level||"INFO")===a.value)),v.value){const ne=Br.find(Ce=>Ce.value===v.value);if(ne&&ne.seconds){const Ce=new Date(Date.now()-ne.seconds*1e3);H=H.filter(qe=>qe._time&&qe._time>=Ce)}}if(i.value&&!U.value)if(l.value)try{const ne=new RegExp(i.value,"i");H=H.filter(Ce=>{const qe=Ce.text||Ce.raw||"",rt=Ce.tool||"";return ne.test(qe)||ne.test(rt)})}catch{}else{const ne=i.value.toLowerCase();H=H.filter(Ce=>{const qe=(Ce.text||Ce.raw||"").toLowerCase(),rt=(Ce.tool||"").toLowerCase();return qe.includes(ne)||rt.includes(ne)})}return H});function de(H){if(H.type==="log"&&H.line)try{const ne=typeof H.line=="string"?JSON.parse(H.line):H.line,Ce=ne.timestamp?new Date(ne.timestamp):new Date;return{ts:Ce.toLocaleTimeString(),_time:Ce,level:ne.error?"ERROR":"INFO",text:ne.tool_name?`[${ne.tool_name}] ${ne.result_summary||""}`.trim():ne.message||JSON.stringify(ne),tool:ne.tool_name||"",raw:null}}catch{return{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:String(H.line),tool:"",raw:String(H.line)}}if(H.payload){const ne=H.payload,Ce=ne.timestamp?new Date(ne.timestamp):new Date;return{ts:Ce.toLocaleTimeString(),_time:Ce,level:ne.error?"ERROR":"INFO",text:ne.tool_name?`[${ne.tool_name}] ${ne.result_summary||""}`.trim():ne.message||JSON.stringify(ne),tool:ne.tool_name||"",raw:null}}return typeof H=="string"?{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:H,tool:"",raw:H}:{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:JSON.stringify(H),tool:"",raw:null}}function se(H){const ne=de(H);if(s.value){N.value.push(ne);return}pe(ne)}function pe(H){t.value.push(H),t.value.length>p&&(t.value=t.value.slice(-p)),n.value&&Et(()=>ee())}function ee(){const H=u.value;if(H){const ne=H.scrollHeight-H.scrollTop-H.clientHeight;H.scrollTo({top:H.scrollHeight,behavior:ne<500?"smooth":"instant"})}}function ue(){n.value=!0,d.value=!1,Et(()=>ee())}function Ie(){const H=u.value;if(!H)return;const ne=H.scrollHeight-H.scrollTop-H.clientHeight<40;d.value=!ne&&t.value.length>0,!ne&&n.value&&(n.value=!1)}function j(){if(s.value=!s.value,!s.value&&N.value.length>0){for(const H of N.value)pe(H);N.value=[]}}function oe(){t.value=[],N.value=[],d.value=!1}function ie(){let H;e.value==="search"?H=re.value.map(rt=>{const Wt=rt.error?"ERROR":"INFO",it=rt.tool_name?`[${rt.tool_name}] `:"";return`${rt.timestamp||""} ${Wt} ${it}${rt.result_summary||rt.message||""}`}).join(`
`):H=ce.value.map(rt=>`${rt.ts} ${rt.level} ${rt.text}`).join(`
`);const ne=new Blob([H],{type:"text/plain"}),Ce=URL.createObjectURL(ne),qe=document.createElement("a");qe.href=Ce,qe.download=`odin-logs-${new Date().toISOString().slice(0,19).replace(/:/g,"-")}.txt`,qe.click(),URL.revokeObjectURL(Ce)}function ge(H,ne){const Ce=`${H.ts} ${H.level} ${H.text||H.raw||""}`;navigator.clipboard.writeText(Ce).then(()=>{f.value=ne,setTimeout(()=>{f.value=null},1500)}).catch(()=>{})}function be(H){a.value=a.value===H?"":H,E.value="all"}function Oe(H){return H.level==="ERROR"?"log-line-error":H.level==="WARNING"?"log-line-warning":"text-gray-300"}function g(H){return H==="ERROR"?"text-red-500 font-semibold":H==="WARNING"?"text-yellow-500":"text-blue-500"}function C(H){return H==="ERROR"?"log-chip-error":H==="WARNING"?"log-chip-warning":"log-chip-info"}function M(H){E.value=H.id;const ne=H.filters;a.value=ne.level||"",v.value=ne.timeRange||"",i.value=ne.text||"",ne.levels&&(a.value=ne.levels[0]||""),ne.hasToolName&&(i.value="")}function Z(H){E.value=H.id,a.value=H.filters.level||"",v.value=H.filters.timeRange||"",i.value=H.filters.text||""}function A(){if(!T.value.trim())return;const H={id:"custom-"+Date.now(),name:T.value.trim(),filters:{level:a.value,timeRange:v.value,text:i.value}};y.value=[...y.value,H],w(),x.value=!1,T.value=""}function F(H){y.value=y.value.filter(ne=>ne.id!==H),w(),E.value===H&&(E.value="all")}const J=h("all"),X=h(""),te=h(""),Q=h(""),me=h(""),le=h(""),fe=h(100),xe=Nw,Se=h(!1),Ae=h(!1),B=h(""),re=h([]),ye=h(null),Le=h(null);function He(){e.value="search",ye.value||Je()}async function Je(){try{ye.value=await q.get("/api/logs/stats")}catch{}}function xt(){const H=le.value;if(!H){Q.value="",me.value="";return}const Ce={last_5m:300,last_15m:900,last_1h:3600,last_4h:14400,last_24h:86400,last_7d:604800}[H];if(Ce){const qe=new Date(Date.now()-Ce*1e3);Q.value=et(qe),me.value=""}}function et(H){const ne=Ce=>String(Ce).padStart(2,"0");return`${H.getFullYear()}-${ne(H.getMonth()+1)}-${ne(H.getDate())}T${ne(H.getHours())}:${ne(H.getMinutes())}`}function Ye(H){if(!H)return"";const ne=new Date(H);return isNaN(ne.getTime())?"":ne.toISOString()}async function ws(){Se.value=!0,B.value="",Ae.value=!0,Le.value=null;try{const H=new URLSearchParams;J.value&&J.value!=="all"&&H.set("level",J.value),X.value&&H.set("tool",X.value),te.value&&H.set("q",te.value);const ne=Ye(Q.value),Ce=Ye(me.value);ne&&H.set("start",ne),Ce&&H.set("end",Ce),H.set("limit",String(fe.value));const qe=await q.get(`/api/logs/search?${H.toString()}`);re.value=qe.entries||[]}catch(H){B.value=H.message||"Search failed",re.value=[]}finally{Se.value=!1}}function Ds(){J.value="all",X.value="",te.value="",Q.value="",me.value="",le.value="",fe.value=100,re.value=[],Ae.value=!1,B.value="",Le.value=null}function Zn(H){Le.value=Le.value===H?null:H}function wn(H){if(!H.timestamp)return"";try{return new Date(H.timestamp).toLocaleString()}catch{return H.timestamp}}function Jn(H){return H.type==="web_action"?`${H.status||""} (${H.execution_time_ms||0}ms)`:(H.result_summary||"").slice(0,200)}function ln(H){return H.error?"log-line-error":"text-gray-300"}function rn(H){try{return JSON.stringify(H,null,2)}catch{return String(H)}}let V=null,_e=null,ss=!1;function Sn(){ss||(ss=!0,We.subscribe("logs",se),r.value=We.connected,o.value=We.state||"disconnected",V=We.onStateChange,_e=(H,ne)=>{o.value=H,r.value=H==="connected",V&&V(H,ne)},We.onStateChange=_e)}function Mi(){ss&&(ss=!1,We.unsubscribe("logs",se),We.onStateChange===_e&&(We.onStateChange=V),_e=null,V=null)}return Be(O),Uo(Sn),Bo(Mi),ht(Mi),{mode:e,logs:t,paused:s,autoScroll:n,levelFilter:a,textFilter:i,useRegex:l,subscribed:r,wsState:o,wsStateLabel:c,logContainer:u,filteredLogs:ce,pauseBuffer:N,showJumpBottom:d,copiedIndex:f,regexError:U,levels:b,logPresets:m,timeRanges:k,timeRange:v,activeLogPreset:E,customLogPresets:y,showSaveLogPreset:x,newLogPresetName:T,hasActiveLogFilters:R,timeRangeLabel:L,timelineBuckets:S,timelineMax:$,timelineSpanLabel:W,timelineLabelSkip:G,togglePause:j,clearLogs:oe,exportLogs:ie,logLineClass:Oe,levelClass:g,levelChipClass:C,toggleLevel:be,copyLine:ge,jumpToBottom:ue,onScroll:Ie,applyLogPreset:M,applyCustomLogPreset:Z,saveLogCustomPreset:A,removeLogCustomPreset:F,segmentHeight:I,jumpToTimelineBucket:z,searchLevel:J,searchTool:X,searchKeyword:te,searchStart:Q,searchEnd:me,searchTimePreset:le,searchLimit:fe,searchLimits:xe,searching:Se,searchRan:Ae,searchError:B,searchResults:re,searchStats:ye,expandedSearch:Le,switchToSearch:He,runSearch:ws,clearSearchFilters:Ds,toggleSearchExpand:Zn,formatSearchTs:wn,searchEntryText:Jn,searchLogLineClass:ln,formatJson:rn,applySearchTimePreset:xt}}},Lw=new Set(["token","api_token","secret","ssh_key_path","credentials_path","api_key","password"]),Dw={"logging.level":["DEBUG","INFO","WARNING","ERROR","CRITICAL"]},Mw={"discord.allowed_users":{type:"array",itemType:"string",message:"Must be a list of user IDs"},"discord.channels":{type:"array",itemType:"string",message:"Must be a list of channel IDs"},"openai_codex.max_tokens":{type:"number",min:1,max:128e3,message:"Must be 1–128000"},"sessions.max_history":{type:"number",min:1,max:1e4,message:"Must be 1–10000"},"sessions.max_age_hours":{type:"number",min:1,message:"Must be at least 1"},"learning.max_entries":{type:"number",min:1,message:"Must be at least 1"},"learning.consolidation_target":{type:"number",min:1,message:"Must be at least 1"},"browser.default_timeout_ms":{type:"number",min:100,message:"Must be at least 100ms"},"browser.viewport_width":{type:"number",min:100,max:7680,message:"Must be 100–7680"},"browser.viewport_height":{type:"number",min:100,max:4320,message:"Must be 100–4320"},"tools.command_timeout_seconds":{type:"number",min:10,max:3600,message:"Must be 10–3600 seconds"}},Hr=[{key:"core",label:"Core",icon:"sliders",sections:["timezone","discord","logging","permissions","personality","graceful_degradation"]},{key:"llm",label:"LLM & AI",icon:"brain",sections:["llm_provider","openai_codex","ollama","kimi","context","agents"]},{key:"data",label:"Data & Storage",icon:"database",sections:["sessions","learning","search","usage","audit","attachments"]},{key:"services",label:"Services",icon:"link",sections:["webhook","observability","email","browser","comfyui","mcp","slack"]},{key:"infra",label:"Infrastructure",icon:"server",sections:["tools"]},{key:"ui",label:"Web UI",icon:"globe",sections:["web"]},{key:"automation",label:"Automation",icon:"workflow",sections:["message_triggers","reaction_triggers","grafana_alerts","outbound_webhooks"]},{key:"integrations",label:"Integrations",icon:"puzzle",sections:["issue_tracker"]}],mm="••••••••",Pw=50;function Fw(e){return Lw.has(e)}function $w(e){return e===mm}function Qi(e){return JSON.parse(JSON.stringify(e))}function Un(e,t){return JSON.stringify(e)===JSON.stringify(t)}function Uw(e,t){const s={};for(const n of Object.keys(t)){if(!(n in e))continue;const a=e[n],i=t[n];if(!Un(a,i))if(typeof a=="object"&&a!==null&&!Array.isArray(a)&&typeof i=="object"&&i!==null&&!Array.isArray(i)){const l={};for(const r of Object.keys(i))Un(a[r],i[r])||(l[r]=i[r]);Object.keys(l).length>0&&(s[n]=l)}else s[n]=i}return s}function Bw(e,t,s){const n=Mw[e+"."+t];if(!n)return null;if(n.type==="number"){const a=Number(s);if(isNaN(a))return"Must be a number";if(n.min!==void 0&&a<n.min)return n.message||"Value too low";if(n.max!==void 0&&a>n.max)return n.message||"Value too high"}return n.type==="array"&&!Array.isArray(s)?n.message||"Must be an array":null}function Kd(e,t){const s=[];for(const n of Object.keys(t)){if(!(n in e))continue;const a=e[n],i=t[n];if(!Un(a,i))if(typeof a=="object"&&a!==null&&!Array.isArray(a)&&typeof i=="object"&&i!==null&&!Array.isArray(i))for(const l of Object.keys(i))Un(a[l],i[l])||s.push({section:n,key:l,oldVal:a[l],newVal:i[l]});else s.push({section:n,key:null,oldVal:a,newVal:i})}return s}const Hw={template:`
    <div class="p-6 page-fade-in">
      <!-- Header -->
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
    ...
    </div>`,setup(){const e=h(null),t=h(null),s=h(!0),n=h(null),a=h({}),i=h({}),l=h({}),r=h(!1),o=h(!1),c=h(null),u=h(!1),d=h([]),f=h([]),p=Y(()=>d.value.length>0),b=Y(()=>f.value.length>0),m=Y(()=>r.value&&t.value?t.value:e.value),k=Y(()=>!e.value||!t.value?!1:!Un(e.value,t.value)),E=Y(()=>!e.value||!t.value?0:Kd(e.value,t.value).length),v=Y(()=>{if(!r.value||!t.value)return{};const A={};for(const F of Object.keys(t.value)){const J=t.value[F];if(typeof J=="object"&&J!==null&&!Array.isArray(J))for(const X of Object.keys(J)){const te=Bw(F,X,J[X]);te&&(A[F+"."+X]=te)}}return A}),y=Y(()=>Object.keys(v.value).length>0),x=Y(()=>e.value?Object.keys(e.value).length:0),T=Y(()=>O.value.length),N=Y(()=>!e.value||!t.value?[]:Kd(e.value,t.value)),O=Y(()=>e.value?Hr.map(A=>({...A,sections:A.sections.filter(F=>F in e.value)})).filter(A=>A.sections.length>0):[]),w=Y(()=>{if(!e.value)return[];const A=new Set(Hr.flatMap(F=>F.sections));return Object.keys(e.value).filter(F=>!A.has(F))});function R(A){return m.value?m.value[A]:null}function L(A){return!e.value||!t.value?!1:!Un(e.value[A],t.value[A])}function U(A){return A.sections.some(F=>L(F))}function P(A,F){if(!e.value||!t.value)return!1;const J=e.value[A],X=t.value[A];return!J||!X?!1:!Un(J[F],X[F])}function S(A){return t.value?t.value[A]:e.value[A]}function $(A,F){const J=t.value||e.value;return J[A]?J[A][F]:void 0}function W(A,F){const J=r.value&&t.value?t.value:e.value;return J[A]?J[A][F]:!1}function G(A,F){return v.value[A+"."+F]||null}function D(A,F){return Dw[A+"."+F]||null}function I(A,F,J){t.value&&(F===null?t.value[A]=J:(t.value[A]||(t.value[A]={}),t.value[A][F]=J),t.value={...t.value})}function z(A,F,J){if(!t.value)return;const X=Qi(t.value);I(A,F,J),d.value.push(X),d.value.length>Pw&&d.value.shift(),f.value=[]}function ce(A,F,J){try{const X=JSON.parse(J);z(A,F,X)}catch{}}function de(){d.value.length!==0&&(f.value.push(Qi(t.value)),t.value=d.value.pop())}function se(){f.value.length!==0&&(d.value.push(Qi(t.value)),t.value=f.value.pop())}function pe(A,F,J){if(!t.value||!t.value[A])return;const X=[...t.value[A][F]];X.splice(J,1),z(A,F,X)}function ee(A,F){if(!t.value||!t.value[A])return;const J=[...t.value[A][F]||[]],X=prompt("Enter new value:");X!==null&&(J.push(X),z(A,F,J))}function ue(A){a.value={...a.value,[A]:!a.value[A]}}function Ie(A){l.value={...l.value,[A]:!l.value[A]}}function j(A){i.value={...i.value,[A]:!i.value[A]}}function oe(A){try{return JSON.stringify(A,null,2)}catch{return String(A)}}function ie(A){return A==null?"null":typeof A=="object"?JSON.stringify(A,null,2):String(A)}function ge(A,F){c.value={type:A,message:F},setTimeout(()=>{c.value=null},3e3)}function be(){t.value=Qi(e.value),r.value=!0,d.value=[],f.value=[]}function Oe(){r.value=!1,t.value=null,d.value=[],f.value=[]}function g(){u.value=!0}async function C(){if(!(!k.value||y.value)){o.value=!0;try{const A=Uw(e.value,t.value);if(Object.keys(A).length===0){ge("success","No changes to save."),o.value=!1;return}const F=await q.put("/api/config",A);e.value=F,r.value=!1,t.value=null,d.value=[],f.value=[],ge("success","Config saved successfully.")}catch(A){ge("error",A.message||"Failed to save config")}o.value=!1}}async function M(){s.value=!0,n.value=null;try{e.value=await q.get("/api/config");for(const A of Object.keys(e.value))a.value[A]===void 0&&(a.value[A]=!0);for(const A of Hr)l.value[A.key]===void 0&&(l.value[A.key]=!0)}catch(A){n.value=A.message}s.value=!1}function Z(A){if(!r.value)return;const F=A.target;F instanceof HTMLElement&&(F.matches("input, textarea, select")||F.isContentEditable)||((A.ctrlKey||A.metaKey)&&!A.shiftKey&&A.key.toLowerCase()==="z"?(A.preventDefault(),de()):(A.ctrlKey||A.metaKey)&&(A.key==="y"||A.shiftKey&&A.key==="z"||A.shiftKey&&A.key==="Z")&&(A.preventDefault(),se()))}return Be(()=>{M(),document.addEventListener("keydown",Z)}),ht(()=>{document.removeEventListener("keydown",Z)}),{config:e,displayConfig:m,editValues:t,loading:s,error:n,expanded:a,expandedNested:i,expandedGroups:l,editing:r,saving:o,toast:c,hasChanges:k,hasErrors:y,changeCount:E,REDACTED:mm,showDiffModal:u,diffEntries:N,canUndo:p,canRedo:b,sectionCount:x,groupCount:T,visibleGroups:O,ungroupedSections:w,validationErrors:v,isSensitiveKey:Fw,isRedacted:$w,sectionChanged:L,groupChanged:U,fieldChanged:P,getDisplay:R,getEdited:S,getEditedField:$,getDisplayBool:W,pushEdit:z,pushEditJson:ce,getValidationError:G,getEnumOptions:D,removeArrayItem:pe,addArrayItem:ee,toggleSection:ue,toggleGroup:Ie,toggleNested:j,formatJson:oe,formatDiffVal:ie,showToast:ge,showDiff:g,fetchConfig:M,startEdit:be,cancelEdit:Oe,saveConfig:C,undo:de,redo:se}}},Vw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=h([]),t=h(!0),s=h(null),n=h({});function a(p){return p.config&&p.config.enabled!==void 0?p.config.enabled:!0}function i(p){return p.config&&p.config.require_mention!==void 0?p.config.require_mention:!1}function l(p){return p.config&&p.config.respond_to_bots!==void 0?p.config.respond_to_bots:!1}function r(p){return p.config&&Object.keys(p.config).length>0}function o(p){n.value[p]=!n.value[p]}async function c(){t.value=!0,s.value=null;try{e.value=await q.get("/api/discord/guilds")}catch(p){s.value=p.message}t.value=!1}async function u(p,b,m){try{await q.put("/api/discord/guild/"+p+"/config",{[b]:m}),await c()}catch(k){s.value=k.message}}async function d(p,b,m,k){try{await q.put("/api/discord/channel/"+p+"/config",{[m]:k}),await c()}catch(E){s.value=E.message}}async function f(p,b){try{await q.put("/api/discord/channel/"+p+"/config",{clear:!0}),await c()}catch(m){s.value=m.message}}return Be(c),{guilds:e,loading:t,error:s,expanded:n,guildEnabled:a,guildMention:i,guildBots:l,hasOverride:r,toggleGuild:o,fetchGuilds:c,setGuildConfig:u,setChannelConfig:d,clearOverride:f}}},jw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=h(!0),t=h(""),s=h(null),n=h([]),a=h({allowed_hosts:[],default_host:""}),i=h({}),l=h(!1),r=h(""),o=h(!1),c=h(0),u=h([]),d=h(null),f=Y(()=>{const I={};for(const z of u.value)I[z.id]=z;return I});function p(I){return f.value[I]||null}const b=Y(()=>/^\d{15,25}$/.test(r.value.trim())),m=Y(()=>{if(o.value){if(k.value[c.value])return"host-user-option-"+c.value;if(b.value)return"host-user-option-raw"}}),k=Y(()=>{const I=r.value.toLowerCase().trim();return I?u.value.filter(z=>!i.value[z.id]&&(z.display_name.toLowerCase().includes(I)||z.username.toLowerCase().includes(I)||z.id.includes(I))):u.value.filter(z=>!i.value[z.id])});function E(I,z){return I?I.allowed_hosts===null||I.allowed_hosts===void 0?{allowed_hosts:[...z],default_host:I.default_host||"",allow_all:!0}:{allowed_hosts:I.allowed_hosts,default_host:I.default_host||"",allow_all:!1}:{allowed_hosts:[...z],default_host:z[0]||"",allow_all:!0}}async function v(){e.value=!0,t.value="";try{const I=await q.get("/api/host-access");s.value=I,n.value=I.available_hosts||[],a.value=E(I.default_policy,n.value);const z=I.users||{},ce={};for(const[de,se]of Object.entries(z))ce[de]=E(se,n.value);i.value=ce}catch(I){t.value=I.message||"Failed to fetch host access data"}finally{e.value=!1}try{u.value=await q.get("/api/discord/members")||[]}catch{u.value=[]}}async function y(){try{const I=a.value.allow_all?null:a.value.allowed_hosts;await q.put("/api/host-access/default-policy",{allowed_hosts:I,default_host:a.value.default_host}),ke.success("Default policy updated")}catch(I){ke.error(I.message||"Failed to save")}}function x(I,z){a.value.allow_all=!1,z?a.value.allowed_hosts.includes(I)||a.value.allowed_hosts.push(I):(a.value.allowed_hosts=a.value.allowed_hosts.filter(ce=>ce!==I),a.value.default_host===I&&(a.value.default_host=a.value.allowed_hosts[0]||"")),y()}async function T(I){const z=i.value[I];if(z)try{const ce=z.allow_all?null:z.allowed_hosts;await q.put(`/api/host-access/user/${I}`,{allowed_hosts:ce,default_host:z.default_host});const de=p(I);ke.success(`Updated access for ${de?de.display_name:I}`)}catch(ce){ke.error(ce.message||"Failed to save")}}function N(I,z,ce){const de=i.value[I];de&&(de.allow_all=!1,ce?de.allowed_hosts.includes(z)||de.allowed_hosts.push(z):(de.allowed_hosts=de.allowed_hosts.filter(se=>se!==z),de.default_host===z&&(de.default_host=de.allowed_hosts[0]||"")),T(I))}function O(I,z){const ce=i.value[I];ce&&(ce.default_host=z,T(I))}function w(){l.value=!0,r.value="",c.value=0,Et(()=>{d.value&&d.value.focus()})}function R(){o.value=!0,c.value=0}function L(){c.value<k.value.length-1&&c.value++}function U(){c.value>0&&c.value--}function P(){const I=k.value[c.value];if(I){$(I);return}b.value&&S()}function S(){const I=r.value.trim();/^\d{15,25}$/.test(I)&&(i.value[I]={allowed_hosts:[...n.value],default_host:n.value[0]||"",allow_all:!1},T(I),r.value="",o.value=!1,l.value=!1)}function $(I){i.value[I.id]={allowed_hosts:[...n.value],default_host:n.value[0]||"",allow_all:!1},T(I.id),r.value="",o.value=!1,l.value=!1}function W(){o.value=!1}function G(){setTimeout(()=>{o.value=!1},150)}async function D(I){const z=p(I);if(await ls({title:"Remove user override",message:`Remove the host access override for ${z?z.display_name:I}? They will fall back to the default policy.`,confirmLabel:"Remove",danger:!0}))try{await q.del(`/api/host-access/user/${I}`),delete i.value[I],ke.success(`Removed override for ${z?z.display_name:I}`)}catch(de){ke.error(de.message||"Failed to delete")}}return Be(v),{loading:e,error:t,data:s,availableHosts:n,defaultPolicy:a,users:i,showAddUser:l,searchQuery:r,showDropdown:o,highlightIdx:c,members:u,filteredMembers:k,isRawId:b,activeOptionId:m,searchInput:d,fetchData:v,saveDefaultPolicy:y,toggleDefaultHost:x,getMember:p,toggleUserHost:N,setUserDefault:O,openAddUser:w,deleteUser:D,onSearchInput:R,highlightNext:L,highlightPrev:U,selectHighlighted:P,selectMember:$,closeDropdown:W,onBlur:G,addRawId:S}}},zw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=h(!0),t=h(""),s=h(null),n=h([]),a=h(!1),i=h(!1),l=h(null),r=h(null),o=h(!1),c=h({user_id:"",username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""}),u=h({username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""}),d=Y(()=>c.value.host_mode==="select"?c.value.allowed_hosts:c.value.host_mode==="none"?[]:n.value),f=Y(()=>u.value.host_mode==="select"?u.value.allowed_hosts:u.value.host_mode==="none"?[]:n.value);function p(w){return w==="admin"?"text-xs px-1.5 py-0.5 rounded bg-red-900/50 text-red-400":w==="user"?"text-xs px-1.5 py-0.5 rounded bg-blue-900/50 text-blue-400":"text-xs px-1.5 py-0.5 rounded bg-gray-700 text-gray-400"}async function b(){e.value=!0,t.value="";try{const w=await q.get("/api/tokens");s.value=w.tokens||[],n.value=w.available_hosts||[]}catch(w){t.value=w.message||"Failed to load tokens"}finally{e.value=!1}}function m(w){return!w||!w.trim()?[]:w.split(",").map(R=>R.trim()).filter(Boolean)}function k(w,R){const L=c.value.allowed_hosts;if(R&&!L.includes(w)&&L.push(w),!R){const U=L.indexOf(w);U>=0&&L.splice(U,1)}}function E(w,R){const L=u.value.allowed_hosts;if(R&&!L.includes(w)&&L.push(w),!R){const U=L.indexOf(w);U>=0&&L.splice(U,1)}}async function v(){var w;i.value=!0;try{const R=m(c.value.allowed_tools_str),L=c.value.host_mode,U=L==="none"?[]:L==="select"?c.value.allowed_hosts:null,P={user_id:c.value.user_id.trim(),username:c.value.username.trim()||"API",tier:c.value.tier,label:c.value.label.trim(),allowed_tools:R.length?R:[]};U!==null&&(P.allowed_hosts=U),P.default_host=c.value.default_host||"";const S=await q.post("/api/tokens",P);l.value=S.token,c.value={user_id:"",username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""},a.value=!1,ke.success("Token created"),await b()}catch(R){ke.error(((w=R.data)==null?void 0:w.error)||R.message||"Failed to create token")}finally{i.value=!1}}function y(w){r.value=w;const R=w.allowed_hosts;let L="default";R==null?L="default":Array.isArray(R)&&R.length===0?L="none":Array.isArray(R)&&(L="select"),u.value={username:w.username||"",tier:w.tier||"admin",label:w.label||"",host_mode:L,allowed_hosts:Array.isArray(R)?[...R]:[],default_host:w.default_host||"",allowed_tools_str:(w.allowed_tools||[]).join(", ")}}async function x(){var w;if(r.value){o.value=!0;try{const R=m(u.value.allowed_tools_str),L=u.value.host_mode,U={username:u.value.username,tier:u.value.tier,label:u.value.label,allowed_tools:R};L==="none"?U.allowed_hosts=[]:L==="select"?U.allowed_hosts=u.value.allowed_hosts:U.allowed_hosts=null,U.default_host=u.value.default_host||"",await q.put("/api/tokens/"+encodeURIComponent(r.value.user_id),U),r.value=null,ke.success("Token updated"),await b()}catch(R){ke.error(((w=R.data)==null?void 0:w.error)||R.message||"Failed to update")}finally{o.value=!1}}}async function T(w){var L;if(await ls({title:"Regenerate token",message:`Regenerate token for ${w.username||w.user_id}? The old token will stop working immediately.`,confirmLabel:"Regenerate",danger:!0}))try{const U=await q.post("/api/tokens/"+encodeURIComponent(w.user_id)+"/regenerate");l.value=U.token,ke.success("Token regenerated")}catch(U){ke.error(((L=U.data)==null?void 0:L.error)||U.message||"Failed to regenerate")}}async function N(w){var L;if(await ls({title:"Delete token",message:`Delete token for ${w.username||w.user_id}? This cannot be undone.`,confirmLabel:"Delete",danger:!0}))try{await q.del("/api/tokens/"+encodeURIComponent(w.user_id)),ke.success("Token deleted"),await b()}catch(U){ke.error(((L=U.data)==null?void 0:L.error)||U.message||"Failed to delete")}}async function O(){if(l.value)try{await navigator.clipboard.writeText(l.value),ke.success("Copied to clipboard")}catch{ke.error("Copy failed — select and copy manually")}}return Be(b),{loading:e,error:t,tokens:s,availableHosts:n,showCreate:a,creating:i,newToken:l,editing:r,saving:o,createForm:c,editForm:u,createDefaultHostOptions:d,editDefaultHostOptions:f,fetchData:b,tierBadge:p,toggleCreateHost:k,toggleEditHost:E,createToken:v,startEdit:y,saveEdit:x,confirmRegenerate:T,confirmDelete:N,copyToken:O}}};function Xi(e,t=500){let s=null;const n=(...a)=>{s&&clearTimeout(s),s=setTimeout(()=>{s=null,e(...a)},t)};return n.pending=()=>s!==null,n.cancel=()=>{s&&(clearTimeout(s),s=null)},n}const Kw={template:`
    <div class="p-6 page-fade-in">
  ...
  `,setup(){const e=h(!0),t=h(null),s=h("codex"),n=h({enabled:!1,model:"gpt-5.5",max_tokens:4096,reasoning_effort:"medium",agent_reasoning_effort:"",agent_model:""}),a=["gpt-5.6-sol","gpt-5.6-terra","gpt-5.6-luna","gpt-5.5"],i=Y(()=>{const V=n.value.model;return V&&!a.includes(V)?[V,...a]:a}),l=Y(()=>{const V=n.value.agent_model;return V&&V!=="auto"&&!a.includes(V)?[V,...a]:a}),r=["gpt-5.5","gpt-5.4","gpt-5.4-mini"],o=Y(()=>!r.includes(n.value.model)&&!(r.includes(n.value.agent_model)&&n.value.agent_reasoning_effort==="")),c=Y(()=>{const V=n.value.agent_model;return V==="auto"?!0:!r.includes(V||n.value.model)}),u=Y(()=>{const V=n.value.agent_reasoning_effort;return V==="auto"?!1:(V||n.value.reasoning_effort)==="max"}),d=V=>r.includes(V)&&(n.value.reasoning_effort==="max"||n.value.agent_model===""&&u.value),f=V=>r.includes(V)&&u.value,p=h({enabled:!1,model:"gpt-5.6-luna"}),b=h({unavailable_reason:null}),m=Y(()=>{const V=p.value.model;return V&&!a.includes(V)?[V,...a]:a});function k(V){const _e=V.target.value;p.value.enabled=_e!=="",_e!==""&&(p.value.model=_e),ye()}const E=h(!1),v=h({enabled:!1,base_url:"",model:"",api_key:"",max_tokens:4096}),y=h({enabled:!1,api_key:"",model:"",max_tokens:4096}),x=h(!1),T=h(!1),N=h(!1),O=h(!1),w=h(!1),R=h(!1),L=h(!1),U=h({configured:!1}),P=h([]),S=h(""),$=h(!1),W=h(!1),G=h({configured:!1}),D=h([]),I=h(""),z=h(!1),ce=h(!1),de=h(!0),se=h(""),pe=h({configured:!1,accounts:[]}),ee=h(null),ue=h(null),Ie=h(""),j=h(null),oe=h(!1),ie=h(null),ge=h(null),be=h("");let Oe=null;function g(V,_e="success"){ke(V,_e==="error"?"error":"success")}function C(V){if(!V)return"?";const _e=V/(1024*1024*1024);return _e>=1?_e.toFixed(1)+" GB":(V/(1024*1024)).toFixed(0)+" MB"}async function M(){e.value=!0,await Promise.all([Z(),A(),me(),F()]),e.value=!1}async function Z(){try{const V=await q.get("/api/llm/status");t.value=V,s.value=V.active_provider||"codex",V.codex&&!re.pending()&&(n.value.enabled=V.codex.enabled,n.value.model=V.codex.model||"gpt-5.5",n.value.reasoning_effort=V.codex.reasoning_effort||"medium",n.value.agent_reasoning_effort=V.codex.agent_reasoning_effort||"",n.value.agent_model=V.codex.agent_model||"",n.value.max_tokens=V.codex.max_tokens||4096),V.ollama&&!Le.pending()&&(v.value.enabled=V.ollama.enabled,v.value.base_url=V.ollama.base_url||"",v.value.model=V.ollama.model||"",v.value.max_tokens=V.ollama.max_tokens||4096),V.kimi&&!He.pending()&&(y.value.enabled=V.kimi.enabled,y.value.model=V.kimi.model||"",y.value.max_tokens=V.kimi.max_tokens||4096),V.auxiliary&&(b.value=V.auxiliary,ye.pending()||(p.value.enabled=V.auxiliary.enabled,p.value.model=V.auxiliary.model||"gpt-5.6-luna"))}catch{t.value={active_provider:"codex",codex:{configured:!1},ollama:{configured:!1},kimi:{configured:!1}}}}async function A(){try{if(U.value=await q.get("/api/ollama/status"),U.value.model&&(S.value=U.value.model),U.value.configured)try{const V=await q.get("/api/ollama/models");P.value=V.models||[]}catch{P.value=[]}else if(v.value.base_url)try{const V=await q.post("/api/ollama/probe-models",{base_url:v.value.base_url});P.value=V.models||[]}catch{P.value=[]}}catch{U.value={configured:!1}}}async function F(){de.value=!0,se.value="";try{pe.value=await q.get("/api/codex/status")}catch(V){se.value=V.message||"Failed to fetch Codex status"}finally{de.value=!1}}async function J(){const V=t.value?t.value.active_provider:"codex";L.value=!0;try{const _e=await q.post("/api/llm/switch",{provider:s.value});_e.error?(s.value=V,g(_e.error,"error")):(g("Switched to "+s.value+" ("+_e.model+")"),await M())}catch(_e){s.value=V,g(_e.message||"Switch failed","error")}finally{L.value=!1}}async function X(){$.value=!0;try{const V=await q.post("/api/ollama/reload");g(V.configured?"Ollama reloaded":V.reason||"Ollama not configured",V.configured?"success":"error"),await M()}catch(V){g(V.message||"Reload failed","error")}finally{$.value=!1}}async function te(){W.value=!0;try{await q.post("/api/ollama/model",{model:S.value}),g("Model set to "+S.value),await M()}catch(V){g(V.message||"Failed","error")}finally{W.value=!1}}async function Q(){const V=v.value.base_url;if(!V){g("Enter a base URL first","error");return}R.value=!0;try{const _e=await q.post("/api/ollama/probe-models",{base_url:V});P.value=_e.models||[],P.value.length?(g(P.value.length+" model(s) found"),!v.value.model&&P.value.length&&(v.value.model=P.value[0].name)):g("No models found at "+V,"error")}catch(_e){g(_e.message||"Could not reach Ollama","error")}finally{R.value=!1}}async function me(){try{if(G.value=await q.get("/api/kimi/status"),G.value.model&&(I.value=G.value.model),G.value.configured)try{const V=await q.get("/api/kimi/models");D.value=V.models||[]}catch{D.value=[]}}catch{G.value={configured:!1}}}async function le(){z.value=!0;try{const V=await q.post("/api/kimi/reload");g(V.configured?"Kimi reloaded":V.reason||"Kimi not configured",V.configured?"success":"error"),await M()}catch(V){g(V.message||"Reload failed","error")}finally{z.value=!1}}async function fe(){ce.value=!0;try{await q.post("/api/kimi/model",{model:I.value}),g("Model set to "+I.value),await M()}catch(V){g(V.message||"Failed","error")}finally{ce.value=!1}}async function xe(){if(N.value){re();return}N.value=!0;try{await q.put("/api/llm/codex/config",n.value),g("Codex config saved"),await Promise.all([Z(),F()])}catch(V){g(V.message||"Failed","error"),await Promise.all([Z(),F()])}finally{N.value=!1}}async function Se(){if(O.value){Le();return}O.value=!0;try{const V={...v.value},_e=x.value?v.value.api_key:null;_e===null&&delete V.api_key,await q.put("/api/llm/ollama/config",V),g("Ollama config saved"),_e!==null&&v.value.api_key===_e&&(v.value.api_key="",x.value=!1),await Promise.all([Z(),A()])}catch(V){g(V.message||"Failed","error")}finally{O.value=!1}}async function Ae(){if(w.value){He();return}w.value=!0;try{const V={...y.value},_e=T.value?y.value.api_key:null;_e===null&&delete V.api_key,await q.put("/api/llm/kimi/config",V),g("Kimi config saved"),_e!==null&&y.value.api_key===_e&&(y.value.api_key="",T.value=!1),await Promise.all([Z(),me()])}catch(V){g(V.message||"Failed","error")}finally{w.value=!1}}async function B(){if(E.value){ye();return}E.value=!0;try{await q.put("/api/llm/auxiliary/config",p.value),g("Auxiliary config saved"),await Z()}catch(V){g(V.message||"Failed","error"),await Z()}finally{E.value=!1}}const re=Xi(xe),ye=Xi(B),Le=Xi(Se),He=Xi(Ae),Je=()=>(re.cancel(),xe()),xt=()=>(Le.cancel(),Se()),et=()=>(He.cancel(),Ae());async function Ye(V){try{await q.post("/api/codex/account/"+V+"/activate"),g("Active account switched"),await F()}catch(_e){g(_e.message||"Failed","error")}}async function ws(V){ee.value=V;try{await q.post("/api/codex/account/"+V+"/refresh"),g("Token refreshed"),await F()}catch(_e){g(_e.message||"Refresh failed","error")}finally{ee.value=null}}function Ds(V,_e){ue.value=V,Ie.value=_e||""}async function Zn(V){try{await q.put("/api/codex/account/"+V+"/label",{label:Ie.value}),g("Label updated"),ue.value=null,await F()}catch(_e){g(_e.message||"Failed","error")}}async function wn(V,_e){if(await ls({title:"Delete Codex account",message:`Delete ${_e||"account #"+(V+1)}? The pool will reload without it.`,confirmLabel:"Delete",danger:!0}))try{await q.del("/api/codex/account/"+V),g("Deleted. Pool reloaded."),await F()}catch(Sn){g(Sn.message||"Failed","error")}}async function Jn(){oe.value=!0;try{const V=await q.post("/api/codex/device-code");ie.value=V,j.value="pending",ln(V)}catch(V){g(V.message||"Failed","error")}finally{oe.value=!1}}async function ln(V){Oe={cancelled:!1};const _e=Oe;try{const ss=await q.post("/api/codex/device-poll",{device_auth_id:V.device_auth_id,user_code:V.user_code,interval:V.interval});if(_e.cancelled)return;ge.value=ss,j.value="success",await M()}catch(ss){if(_e.cancelled)return;be.value=ss.message||"Device login failed",j.value="error"}}function rn(){Oe&&(Oe.cancelled=!0),j.value=null,ie.value=null}return Be(M),ht(()=>{Oe&&(Oe.cancelled=!0),re.cancel(),ye.cancel(),Le.cancel(),He.cancel()}),{loading:e,llmStatus:t,selectedProvider:s,switching:L,codexForm:n,codexModelOptions:i,codexAgentModelOptions:l,mainMaxAllowed:o,agentMaxAllowed:c,mainModelOptionDisabled:d,agentModelOptionDisabled:f,auxForm:p,auxData:b,auxModelOptions:m,onAuxModelChange:k,savingAux:E,saveAuxConfigDebounced:ye,ollamaForm:v,kimiForm:y,savingCodex:N,savingOllama:O,savingKimi:w,probingOllama:R,ollamaKeyDirty:x,kimiKeyDirty:T,ollamaStatus:U,ollamaModels:P,ollamaSelectedModel:S,reloading:$,settingModel:W,kimiStatus:G,kimiModels:D,kimiSelectedModel:I,reloadingKimi:z,settingKimiModel:ce,codexLoading:de,codexError:se,codexData:pe,refreshing:ee,editingLabel:ue,labelValue:Ie,deviceState:j,deviceLoading:oe,deviceInfo:ie,deviceResult:ge,deviceError:be,fetchAll:M,switchProvider:J,reloadOllama:X,setOllamaModel:te,reloadKimi:le,setKimiModel:fe,probeOllamaModels:Q,saveCodexConfig:xe,saveOllamaConfig:Se,saveKimiConfig:Ae,saveCodexConfigDebounced:re,saveOllamaConfigDebounced:Le,saveKimiConfigDebounced:He,saveCodexConfigNow:Je,saveOllamaConfigNow:xt,saveKimiConfigNow:et,activateAccount:Ye,refreshAccount:ws,startEditLabel:Ds,saveLabel:Zn,deleteAccount:wn,startDeviceLogin:Jn,cancelDeviceLogin:rn,formatSize:C}}},qd={ok:"text-green-400",pass:"text-green-400",degraded:"text-yellow-400",warn:"text-yellow-400",down:"text-red-400",fail:"text-red-400",unconfigured:"text-gray-500",skipped:"text-gray-500"};function qw(e){return qd[e]||qd[(e||"").toLowerCase()]||"text-gray-400"}const Gw={template:`
  ...
  `},Zw={components:{TabbedPage:dr},setup(){return{tabs:[{id:"health",label:"Health",component:Ew},{id:"resources",label:"Resources",component:Aw},{id:"logs",label:"Logs",component:Ow},{id:"config",label:"Config",component:Hw},{id:"discord",label:"Discord",component:Vw},{id:"host-access",label:"Host Access",component:jw},{id:"api-tokens",label:"API Tokens",component:zw},{id:"llm",label:"LLM Config",component:Kw},{id:"internals",label:"Internals",component:Gw},{id:"update",label:"Update",component:Ww}]}},template:'<tabbed-page :tabs="tabs" default-tab="health" group-label="System" />'},mt=(e,t)=>s=>({path:e,query:{...s.query,tab:t}}),gm=[{path:"/",redirect:"/dashboard"},{path:"/dashboard",component:P_,meta:{label:"Dashboard",icon:"dashboard",section:"Workspace",description:"System posture and recent activity"}},{path:"/chat",component:ew,meta:{label:"Chat",icon:"chat",section:"Workspace",description:"Direct operator conversation"}},{path:"/operations",component:lw,meta:{label:"Operations",icon:"operations",section:"Operate",description:"Execution, agents, loops, processes, and schedules"}},{path:"/history",component:pw,meta:{label:"History",icon:"history",section:"Observe",description:"Audit trail, sessions, traces, and usage"}},{path:"/capabilities",component:kw,meta:{label:"Capabilities",icon:"capabilities",section:"Manage",description:"Tools, skills, knowledge, and memory"}},{path:"/personality",component:ww,meta:{label:"Personality",icon:"personality",section:"Manage",description:"Behavior and response profile"}},{path:"/system",component:Zw,meta:{label:"System",icon:"system",section:"Manage",description:"Health, configuration, access, and updates"}},{path:"/execution",redirect:mt("/operations","live")},{path:"/agents",redirect:mt("/operations","agents")},{path:"/loops",redirect:mt("/operations","loops")},{path:"/processes",redirect:mt("/operations","processes")},{path:"/schedules",redirect:mt("/operations","schedules")},{path:"/audit",redirect:mt("/history","audit")},{path:"/sessions",redirect:mt("/history","sessions")},{path:"/traces",redirect:mt("/history","traces")},{path:"/usage",redirect:mt("/history","usage")},{path:"/tools",redirect:mt("/capabilities","tools")},{path:"/skills",redirect:mt("/capabilities","skills")},{path:"/knowledge",redirect:mt("/capabilities","knowledge")},{path:"/memory",redirect:mt("/capabilities","memory")},{path:"/learned",redirect:mt("/capabilities","learned")},{path:"/health",redirect:mt("/system","health")},{path:"/resources",redirect:mt("/system","resources")},{path:"/logs",redirect:mt("/system","logs")},{path:"/config",redirect:mt("/system","config")},{path:"/host-access",redirect:mt("/system","host-access")},{path:"/internals",redirect:mt("/system","internals")}],ii=E_({history:i_(),routes:gm});ii.afterEach(e=>{var s;const t=(s=e.meta)==null?void 0:s.label;document.title=t?`Odin — ${t}`:"Odin — Management"});const Jw={template:`
    ...
    <toast-container />
    <confirm-host />
    <command-palette />`,setup(){const e=h("checking"),t=h(!1),s=h(!1),n=h(!1),a=h(null),i=h(null),l=h(!1);let r=null,o=null;const c=h(!1),u=h("disconnected"),d=h(-1),f=h(null);let p=null;const b=h("starting"),m=h(""),k=gm.filter(D=>D.meta),E=Y(()=>["Workspace","Operate","Observe","Manage"].map(D=>({name:D,routes:k.filter(I=>I.meta.section===D)})).filter(D=>D.routes.length)),v=Y(()=>{var D;return((D=ii.currentRoute.value.meta)==null?void 0:D.label)||"Odin"}),y=Y(()=>{var D;return((D=ii.currentRoute.value.meta)==null?void 0:D.section)||"Management"}),x=Y(()=>{var D;return((D=ii.currentRoute.value.meta)==null?void 0:D.description)||"Management console"});q.onSessionExpired=()=>{t.value=!0,We.disconnect(),q.setToken(""),e.value="login"};function T(D){var I;if((D.ctrlKey||D.metaKey)&&D.key.toLowerCase()==="k"){e.value==="ready"&&(D.preventDefault(),xd());return}if(n.value&&D.key==="Tab"){const z=[...((I=a.value)==null?void 0:I.querySelectorAll('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])'))||[]];if(z.length){const ce=z[0],de=z[z.length-1];if(D.shiftKey&&(document.activeElement===ce||!a.value.contains(document.activeElement))){D.preventDefault(),de.focus();return}if(!D.shiftKey&&(document.activeElement===de||!a.value.contains(document.activeElement))){D.preventDefault(),ce.focus();return}}}if(D.key==="Escape"&&n.value){n.value=!1,D.preventDefault();return}if(D.key==="/"&&!["INPUT","TEXTAREA","SELECT"].includes(D.target.tagName)){D.preventDefault();const z=document.querySelector('.hm-main input[type="text"], .hm-main .hm-input:not(textarea):not(select)');z&&z.focus()}}function N(){l.value=!!(r!=null&&r.matches),l.value||(n.value=!1)}Be(async()=>{document.addEventListener("keydown",T),r=window.matchMedia("(max-width: 900px)"),N(),r.addEventListener("change",N);const D=await q.check();D.ok?(e.value="ready",W()):D.needsAuth?e.value="login":(e.value="ready",W())});function O(){t.value=!1,e.value="ready",W()}async function w(){await q.logout(),We.disconnect(),e.value="login"}function R(){s.value=!s.value}function L(){n.value=!n.value}hs(n,async D=>{var I,z;if(D)o=document.activeElement,await Et(),(z=(I=a.value)==null?void 0:I.querySelector(".nav-item"))==null||z.focus();else if(o!=null&&o.isConnected){const ce=o;o=null,requestAnimationFrame(()=>ce.focus())}});const U=Y(()=>{switch(u.value){case"connected":return"Live";case"connecting":return"Connecting…";case"reconnecting":return"Reconnecting…";default:return"Disconnected"}});function P(D,I="info",z=3e3){f.value={text:D,level:I},clearTimeout(p),p=setTimeout(()=>{f.value=null},z)}let S=null,$=!1;function W(){We.onStatusChange=D=>{c.value=D},We.onStateChange=(D,I)=>{u.value=D,d.value=I.latency??-1,D==="connected"?($&&P("Connection restored","success"),$=!0):D==="reconnecting"&&I.attempt===1&&P("Connection lost — reconnecting…","warn")},We.connect(),G(),S&&clearInterval(S),S=setInterval(G,15e3)}async function G(){try{const D=await q.get("/api/status");b.value=D.status==="online"?"online":"starting";const I=D.uptime_seconds||0,z=Math.floor(I/3600),ce=Math.floor(I%3600/60);m.value=`${z}h ${ce}m uptime`}catch{b.value="offline",m.value=""}}return ht(()=>{S&&clearInterval(S),We.disconnect(),document.removeEventListener("keydown",T),r==null||r.removeEventListener("change",N)}),{authState:e,sessionExpired:t,sidebarCollapsed:s,mobileOpen:n,wsConnected:c,wsState:u,wsLatency:d,wsLabel:U,wsToast:f,botStatus:b,botUptime:m,navRoutes:k,navGroups:E,currentPage:v,currentSection:y,currentDescription:x,sidebarEl:a,mobileMenuButton:i,isMobileViewport:l,onLogin:O,logout:w,toggleSidebar:R,toggleMobileNavigation:L,openPalette:xd}}},kn=Sl(Yw);kn.component("odin-icon",O_);kn.component("login-screen",Jw);kn.component("toast-container",yx);kn.component("confirm-host",xx);kn.component("command-palette",N_);kn.directive("modal-focus",D_);kn.use(ii);kn.mount("#app");
