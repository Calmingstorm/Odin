var kg=Object.defineProperty;var wg=(e,t,s)=>t in e?kg(e,t,{enumerable:!0,configurable:!0,writable:!0,value:s}):e[t]=s;var et=(e,t,s)=>wg(e,typeof t!="symbol"?t+"":t,s);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function s(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(a){if(a.ep)return;a.ep=!0;const i=s(a);fetch(a.href,i)}})();class Sg{constructor(){this._persist=localStorage.getItem("odin_persist")==="1",this._token=this._persist?localStorage.getItem("odin_token")||"":sessionStorage.getItem("odin_token")||"";const t=this._persist?localStorage:sessionStorage;this._sessionTimeout=parseInt(t.getItem("odin_session_timeout")||"0",10),this._lastActivity=Date.now(),this._activityTimer=null,this.onSessionExpired=null,this._token&&this._sessionTimeout>0&&this._startActivityMonitor()}get token(){return this._token}get sessionTimeout(){return this._sessionTimeout}setToken(t,s=0){if(this._token=t,this._sessionTimeout=s,this._lastActivity=Date.now(),t){const n=this._persist?localStorage:sessionStorage;n.setItem("odin_token",t),this._persist&&localStorage.setItem("odin_persist","1"),s>0?n.setItem("odin_session_timeout",String(s)):n.removeItem("odin_session_timeout"),this._startActivityMonitor()}else sessionStorage.removeItem("odin_token"),sessionStorage.removeItem("odin_session_timeout"),localStorage.removeItem("odin_token"),localStorage.removeItem("odin_persist"),localStorage.removeItem("odin_session_timeout"),this._stopActivityMonitor()}setPersist(t){this._persist=t}_startActivityMonitor(){this._stopActivityMonitor(),!(this._sessionTimeout<=0)&&(this._activityTimer=setInterval(()=>{(Date.now()-this._lastActivity)/1e3>=this._sessionTimeout&&(this._stopActivityMonitor(),this.onSessionExpired&&this.onSessionExpired())},1e4))}_stopActivityMonitor(){this._activityTimer&&(clearInterval(this._activityTimer),this._activityTimer=null)}_headers(t={}){const s={"Content-Type":"application/json",...t};return this._token&&(s.Authorization=`Bearer ${this._token}`),s}async _request(t,s,n=null){this._lastActivity=Date.now();const a={method:t,headers:this._headers()};n!==null&&(a.body=JSON.stringify(n));const i=await fetch(s,a);if(i.status===401)throw new xr("Unauthorized");const l=await i.json().catch(()=>null);if(!i.ok){const r=(l==null?void 0:l.error)||`HTTP ${i.status}`;throw new Tg(r,i.status,l)}return l}get(t){return this._request("GET",t)}post(t,s){return this._request("POST",t,s)}put(t,s){return this._request("PUT",t,s)}del(t){return this._request("DELETE",t)}async login(t){const s=await fetch("/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:t})}),n=await s.json().catch(()=>null);if(!s.ok)throw new xr((n==null?void 0:n.error)||"Login failed");return this.setToken(n.session_id,n.timeout_seconds||0),n}async logout(){try{await this.post("/api/auth/logout",{})}catch{}this.setToken("")}async check(){try{return await this.get("/api/status"),{ok:!0,needsAuth:!1}}catch(t){return t instanceof xr?{ok:!1,needsAuth:!0}:{ok:!1,needsAuth:!1,error:t.message}}}}class xr extends Error{constructor(t){super(t),this.name="AuthError"}}class Tg extends Error{constructor(t,s,n){super(t),this.name="ApiError",this.status=s,this.data=n}}class Cg{constructor(t){this._api=t,this._ws=null,this._handlers={logs:[],events:[],chat:[]},this._reconnectDelay=1e3,this._maxReconnectDelay=3e4,this._shouldConnect=!1,this._subscriptions=new Set,this._reconnectAttempt=0,this._lastPongTime=0,this._pingInterval=null,this._latency=-1,this._chatPending=!1,this._state="disconnected",this.onStatusChange=null,this.onStateChange=null}get connected(){var t;return((t=this._ws)==null?void 0:t.readyState)===WebSocket.OPEN}get state(){return this._state}get reconnectAttempt(){return this._reconnectAttempt}get latency(){return this._latency}connect(){this._shouldConnect=!0,this._setState("connecting"),this._open()}disconnect(){this._shouldConnect=!1,this._reconnectAttempt=0,this._latency=-1,this._stopPing(),this._ws&&(this._ws.close(),this._ws=null),this._setState("disconnected")}_setState(t){this._state!==t&&(this._state=t,this.onStateChange&&this.onStateChange(t,{attempt:this._reconnectAttempt,latency:this._latency}))}_startPing(){this._stopPing(),this._pingInterval=setInterval(()=>{if(this.connected)try{this._ws.send(JSON.stringify({type:"ping",ts:Date.now()}))}catch{}},15e3)}_stopPing(){this._pingInterval&&(clearInterval(this._pingInterval),this._pingInterval=null)}subscribe(t,s){this._handlers[t]||(this._handlers[t]=[]),this._handlers[t].push(s),t!=="chat"&&(this._subscriptions.add(t),this.connected&&this._ws.send(JSON.stringify({subscribe:t})))}unsubscribe(t,s){const n=this._handlers[t];if(n){const a=n.indexOf(s);a>=0&&n.splice(a,1),n.length===0&&t!=="chat"&&(this._subscriptions.delete(t),this.connected&&this._ws.send(JSON.stringify({unsubscribe:t})))}}on(t,s){return this.subscribe(t,s)}off(t,s){return this.unsubscribe(t,s)}sendChat(t,{channelId:s,userId:n,username:a}={}){return this.connected?(this._ws.send(JSON.stringify({type:"chat",content:t,channel_id:s||"web-default",user_id:n||void 0,username:a||void 0})),this._chatPending=!0,!0):!1}_open(){if(this._ws)return;let s=`${location.protocol==="https:"?"wss:":"ws:"}//${location.host}/api/ws`;this._api.token&&(s+=`?token=${encodeURIComponent(this._api.token)}`),this._ws=new WebSocket(s),this._ws.onopen=()=>{this._reconnectDelay=1e3,this._reconnectAttempt=0;for(const n of this._subscriptions)this._ws.send(JSON.stringify({subscribe:n}));this._startPing(),this._setState("connected"),this.onStatusChange&&this.onStatusChange(!0)},this._ws.onmessage=n=>{let a;try{a=JSON.parse(n.data)}catch{return}const i=a.type;if(i==="pong"){a.ts&&(this._latency=Date.now()-a.ts,this._lastPongTime=Date.now());return}if(i==="log")for(const l of this._handlers.logs||[])l(a);else if(i==="event")for(const l of this._handlers.events||[])l(a);else if(i==="chat_response"||i==="chat_error"){this._chatPending=!1;for(const l of this._handlers.chat||[])l(a)}},this._ws.onclose=()=>{if(this._ws=null,this._stopPing(),this._latency=-1,this._chatPending){this._chatPending=!1;const n={type:"chat_error",error:"Connection lost — the response may still complete; check session history."};for(const a of this._handlers.chat||[])a(n)}this.onStatusChange&&this.onStatusChange(!1),this._shouldConnect?(this._reconnectAttempt++,this._setState("reconnecting"),setTimeout(()=>this._open(),this._reconnectDelay),this._reconnectDelay=Math.min(this._reconnectDelay*2,this._maxReconnectDelay)):this._setState("disconnected")},this._ws.onerror=()=>{}}}const G=new Sg,We=new Cg(G);/**
* @vue/shared v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/function os(e){const t=Object.create(null);for(const s of e.split(","))t[s]=1;return s=>s in t}const Fe={},pa=[],Ot=()=>{},da=()=>!1,Gn=e=>e.charCodeAt(0)===111&&e.charCodeAt(1)===110&&(e.charCodeAt(2)>122||e.charCodeAt(2)<97),Hl=e=>e.startsWith("onUpdate:"),Pe=Object.assign,Io=(e,t)=>{const s=e.indexOf(t);s>-1&&e.splice(s,1)},Eg=Object.prototype.hasOwnProperty,ze=(e,t)=>Eg.call(e,t),he=Array.isArray,ha=e=>Da(e)==="[object Map]",Wn=e=>Da(e)==="[object Set]",Yc=e=>Da(e)==="[object Date]",Ag=e=>Da(e)==="[object RegExp]",we=e=>typeof e=="function",Ae=e=>typeof e=="string",Bt=e=>typeof e=="symbol",je=e=>e!==null&&typeof e=="object",No=e=>(je(e)||we(e))&&we(e.then)&&we(e.catch),Zd=Object.prototype.toString,Da=e=>Zd.call(e),Rg=e=>Da(e).slice(8,-1),Vl=e=>Da(e)==="[object Object]",jl=e=>Ae(e)&&e!=="NaN"&&e[0]!=="-"&&""+parseInt(e,10)===e,Qs=os(",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted"),Ig=os("bind,cloak,else-if,else,for,html,if,model,on,once,pre,show,slot,text,memo"),zl=e=>{const t=Object.create(null);return(s=>t[s]||(t[s]=e(s)))},Ng=/-\w/g,Qe=zl(e=>e.replace(Ng,t=>t.slice(1).toUpperCase())),Lg=/\B([A-Z])/g,es=zl(e=>e.replace(Lg,"-$1").toLowerCase()),Zn=zl(e=>e.charAt(0).toUpperCase()+e.slice(1)),ga=zl(e=>e?`on${Zn(e)}`:""),Et=(e,t)=>!Object.is(e,t),ma=(e,...t)=>{for(let s=0;s<e.length;s++)e[s](...t)},Jd=(e,t,s,n=!1)=>{Object.defineProperty(e,t,{configurable:!0,enumerable:!1,writable:n,value:s})},ql=e=>{const t=parseFloat(e);return isNaN(t)?e:t},dl=e=>{const t=Ae(e)?Number(e):NaN;return isNaN(t)?e:t};let Qc;const Kl=()=>Qc||(Qc=typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{});function Og(e,t){return e+JSON.stringify(t,(s,n)=>typeof n=="function"?n.toString():n)}const Dg="Infinity,undefined,NaN,isFinite,isNaN,parseFloat,parseInt,decodeURI,decodeURIComponent,encodeURI,encodeURIComponent,Math,Number,Date,Array,Object,Boolean,String,RegExp,Map,Set,JSON,Intl,BigInt,console,Error,Symbol",Mg=os(Dg);function Ri(e){if(he(e)){const t={};for(let s=0;s<e.length;s++){const n=e[s],a=Ae(n)?Yd(n):Ri(n);if(a)for(const i in a)t[i]=a[i]}return t}else if(Ae(e)||je(e))return e}const Pg=/;(?![^(]*\))/g,Fg=/:([^]+)/,$g=/\/\*[^]*?\*\//g;function Yd(e){const t={};return e.replace($g,"").split(Pg).forEach(s=>{if(s){const n=s.split(Fg);n.length>1&&(t[n[0].trim()]=n[1].trim())}}),t}function Ii(e){let t="";if(Ae(e))t=e;else if(he(e))for(let s=0;s<e.length;s++){const n=Ii(e[s]);n&&(t+=n+" ")}else if(je(e))for(const s in e)e[s]&&(t+=s+" ");return t.trim()}function Ug(e){if(!e)return null;let{class:t,style:s}=e;return t&&!Ae(t)&&(e.class=Ii(t)),s&&(e.style=Ri(s)),e}const Bg="html,body,base,head,link,meta,style,title,address,article,aside,footer,header,hgroup,h1,h2,h3,h4,h5,h6,nav,section,div,dd,dl,dt,figcaption,figure,picture,hr,img,li,main,ol,p,pre,ul,a,b,abbr,bdi,bdo,br,cite,code,data,dfn,em,i,kbd,mark,q,rp,rt,ruby,s,samp,small,span,strong,sub,sup,time,u,var,wbr,area,audio,map,track,video,embed,object,param,source,canvas,script,noscript,del,ins,caption,col,colgroup,table,thead,tbody,td,th,tr,button,datalist,fieldset,form,input,label,legend,meter,optgroup,option,output,progress,select,textarea,details,dialog,menu,summary,template,blockquote,iframe,tfoot",Hg="svg,animate,animateMotion,animateTransform,circle,clipPath,color-profile,defs,desc,discard,ellipse,feBlend,feColorMatrix,feComponentTransfer,feComposite,feConvolveMatrix,feDiffuseLighting,feDisplacementMap,feDistantLight,feDropShadow,feFlood,feFuncA,feFuncB,feFuncG,feFuncR,feGaussianBlur,feImage,feMerge,feMergeNode,feMorphology,feOffset,fePointLight,feSpecularLighting,feSpotLight,feTile,feTurbulence,filter,foreignObject,g,hatch,hatchpath,image,line,linearGradient,marker,mask,mesh,meshgradient,meshpatch,meshrow,metadata,mpath,path,pattern,polygon,polyline,radialGradient,rect,set,solidcolor,stop,switch,symbol,text,textPath,title,tspan,unknown,use,view",Vg="annotation,annotation-xml,maction,maligngroup,malignmark,math,menclose,merror,mfenced,mfrac,mfraction,mglyph,mi,mlabeledtr,mlongdiv,mmultiscripts,mn,mo,mover,mpadded,mphantom,mprescripts,mroot,mrow,ms,mscarries,mscarry,msgroup,msline,mspace,msqrt,msrow,mstack,mstyle,msub,msubsup,msup,mtable,mtd,mtext,mtr,munder,munderover,none,semantics",jg="area,base,br,col,embed,hr,img,input,link,meta,param,source,track,wbr",zg=os(Bg),qg=os(Hg),Kg=os(Vg),Gg=os(jg),Wg="itemscope,allowfullscreen,formnovalidate,ismap,nomodule,novalidate,readonly",Zg=os(Wg);function Qd(e){return!!e||e===""}function Jg(e,t){if(e.length!==t.length)return!1;let s=!0;for(let n=0;s&&n<e.length;n++)s=sn(e[n],t[n]);return s}function sn(e,t){if(e===t)return!0;let s=Yc(e),n=Yc(t);if(s||n)return s&&n?e.getTime()===t.getTime():!1;if(s=Bt(e),n=Bt(t),s||n)return e===t;if(s=he(e),n=he(t),s||n)return s&&n?Jg(e,t):!1;if(s=je(e),n=je(t),s||n){if(!s||!n)return!1;const a=Object.keys(e).length,i=Object.keys(t).length;if(a!==i)return!1;for(const l in e){const r=e.hasOwnProperty(l),o=t.hasOwnProperty(l);if(r&&!o||!r&&o||!sn(e[l],t[l]))return!1}}return String(e)===String(t)}function Gl(e,t){return e.findIndex(s=>sn(s,t))}const Xd=e=>!!(e&&e.__v_isRef===!0),ef=e=>Ae(e)?e:e==null?"":he(e)||je(e)&&(e.toString===Zd||!we(e.toString))?Xd(e)?ef(e.value):JSON.stringify(e,tf,2):String(e),tf=(e,t)=>Xd(t)?tf(e,t.value):ha(t)?{[`Map(${t.size})`]:[...t.entries()].reduce((s,[n,a],i)=>(s[_r(n,i)+" =>"]=a,s),{})}:Wn(t)?{[`Set(${t.size})`]:[...t.values()].map(s=>_r(s))}:Bt(t)?_r(t):je(t)&&!he(t)&&!Vl(t)?String(t):t,_r=(e,t="")=>{var s;return Bt(e)?`Symbol(${(s=e.description)!=null?s:t})`:e};function Yg(e){return e==null?"initial":typeof e=="string"?e===""?" ":e:String(e)}/**
* @vue/reactivity v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/let wt;class Lo{constructor(t=!1){this.detached=t,this._active=!0,this._on=0,this.effects=[],this.cleanups=[],this._isPaused=!1,this._warnOnRun=!0,this.__v_skip=!0,!t&&wt&&(wt.active?(this.parent=wt,this.index=(wt.scopes||(wt.scopes=[])).push(this)-1):(this._active=!1,this._warnOnRun=!1))}get active(){return this._active}pause(){if(this._active){this._isPaused=!0;let t,s;if(this.scopes)for(t=0,s=this.scopes.length;t<s;t++)this.scopes[t].pause();for(t=0,s=this.effects.length;t<s;t++)this.effects[t].pause()}}resume(){if(this._active&&this._isPaused){this._isPaused=!1;let t,s;if(this.scopes)for(t=0,s=this.scopes.length;t<s;t++)this.scopes[t].resume();for(t=0,s=this.effects.length;t<s;t++)this.effects[t].resume()}}run(t){if(this._active){const s=wt;try{return wt=this,t()}finally{wt=s}}}on(){++this._on===1&&(this.prevScope=wt,wt=this)}off(){if(this._on>0&&--this._on===0){if(wt===this)wt=this.prevScope;else{let t=wt;for(;t;){if(t.prevScope===this){t.prevScope=this.prevScope;break}t=t.prevScope}}this.prevScope=void 0}}stop(t){if(this._active){this._active=!1;let s,n;for(s=0,n=this.effects.length;s<n;s++)this.effects[s].stop();for(this.effects.length=0,s=0,n=this.cleanups.length;s<n;s++)this.cleanups[s]();if(this.cleanups.length=0,this.scopes){for(s=0,n=this.scopes.length;s<n;s++)this.scopes[s].stop(!0);this.scopes.length=0}if(!this.detached&&this.parent&&!t){const a=this.parent.scopes.pop();a&&a!==this&&(this.parent.scopes[this.index]=a,a.index=this.index)}this.parent=void 0}}}function Qg(e){return new Lo(e)}function sf(){return wt}function Xg(e,t=!1){wt&&wt.cleanups.push(e)}let st;const kr=new WeakSet;class di{constructor(t){this.fn=t,this.deps=void 0,this.depsTail=void 0,this.flags=5,this.next=void 0,this.cleanup=void 0,this.scheduler=void 0,wt&&(wt.active?wt.effects.push(this):this.flags&=-2)}pause(){this.flags|=64}resume(){this.flags&64&&(this.flags&=-65,kr.has(this)&&(kr.delete(this),this.trigger()))}notify(){this.flags&2&&!(this.flags&32)||this.flags&8||af(this)}run(){if(!(this.flags&1))return this.fn();this.flags|=2,Xc(this),lf(this);const t=st,s=_s;st=this,_s=!0;try{return this.fn()}finally{rf(this),st=t,_s=s,this.flags&=-3}}stop(){if(this.flags&1){for(let t=this.deps;t;t=t.nextDep)Mo(t);this.deps=this.depsTail=void 0,Xc(this),this.onStop&&this.onStop(),this.flags&=-2}}trigger(){this.flags&64?kr.add(this):this.scheduler?this.scheduler():this.runIfDirty()}runIfDirty(){Wr(this)&&this.run()}get dirty(){return Wr(this)}}let nf=0,ti,si;function af(e,t=!1){if(e.flags|=8,t){e.next=si,si=e;return}e.next=ti,ti=e}function Oo(){nf++}function Do(){if(--nf>0)return;if(si){let t=si;for(si=void 0;t;){const s=t.next;t.next=void 0,t.flags&=-9,t=s}}let e;for(;ti;){let t=ti;for(ti=void 0;t;){const s=t.next;if(t.next=void 0,t.flags&=-9,t.flags&1)try{t.trigger()}catch(n){e||(e=n)}t=s}}if(e)throw e}function lf(e){for(let t=e.deps;t;t=t.nextDep)t.version=-1,t.prevActiveLink=t.dep.activeLink,t.dep.activeLink=t}function rf(e){let t,s=e.depsTail,n=s;for(;n;){const a=n.prevDep;n.version===-1?(n===s&&(s=a),Mo(n),em(n)):t=n,n.dep.activeLink=n.prevActiveLink,n.prevActiveLink=void 0,n=a}e.deps=t,e.depsTail=s}function Wr(e){for(let t=e.deps;t;t=t.nextDep)if(t.dep.version!==t.version||t.dep.computed&&(of(t.dep.computed)||t.dep.version!==t.version))return!0;return!!e._dirty}function of(e){if(e.flags&4&&!(e.flags&16)||(e.flags&=-17,e.globalVersion===fi)||(e.globalVersion=fi,!e.isSSR&&e.flags&128&&(!e.deps&&!e._dirty||!Wr(e))))return;e.flags|=2;const t=e.dep,s=st,n=_s;st=e,_s=!0;try{lf(e);const a=e.fn(e._value);(t.version===0||Et(a,e._value))&&(e.flags|=128,e._value=a,t.version++)}catch(a){throw t.version++,a}finally{st=s,_s=n,rf(e),e.flags&=-3}}function Mo(e,t=!1){const{dep:s,prevSub:n,nextSub:a}=e;if(n&&(n.nextSub=a,e.prevSub=void 0),a&&(a.prevSub=n,e.nextSub=void 0),s.subs===e&&(s.subs=n,!n&&s.computed)){s.computed.flags&=-5;for(let i=s.computed.deps;i;i=i.nextDep)Mo(i,!0)}!t&&!--s.sc&&s.map&&s.map.delete(s.key)}function em(e){const{prevDep:t,nextDep:s}=e;t&&(t.nextDep=s,e.prevDep=void 0),s&&(s.prevDep=t,e.nextDep=void 0)}function tm(e,t){e.effect instanceof di&&(e=e.effect.fn);const s=new di(e);t&&Pe(s,t);try{s.run()}catch(a){throw s.stop(),a}const n=s.run.bind(s);return n.effect=s,n}function sm(e){e.effect.stop()}let _s=!0;const cf=[];function nn(){cf.push(_s),_s=!1}function an(){const e=cf.pop();_s=e===void 0?!0:e}function Xc(e){const{cleanup:t}=e;if(e.cleanup=void 0,t){const s=st;st=void 0;try{t()}finally{st=s}}}let fi=0;class nm{constructor(t,s){this.sub=t,this.dep=s,this.version=s.version,this.nextDep=this.prevDep=this.nextSub=this.prevSub=this.prevActiveLink=void 0}}class Wl{constructor(t){this.computed=t,this.version=0,this.activeLink=void 0,this.subs=void 0,this.map=void 0,this.key=void 0,this.sc=0,this.__v_skip=!0}track(t){if(!st||!_s||st===this.computed)return;let s=this.activeLink;if(s===void 0||s.sub!==st)s=this.activeLink=new nm(st,this),st.deps?(s.prevDep=st.depsTail,st.depsTail.nextDep=s,st.depsTail=s):st.deps=st.depsTail=s,uf(s);else if(s.version===-1&&(s.version=this.version,s.nextDep)){const n=s.nextDep;n.prevDep=s.prevDep,s.prevDep&&(s.prevDep.nextDep=n),s.prevDep=st.depsTail,s.nextDep=void 0,st.depsTail.nextDep=s,st.depsTail=s,st.deps===s&&(st.deps=n)}return s}trigger(t){this.version++,fi++,this.notify(t)}notify(t){Oo();try{for(let s=this.subs;s;s=s.prevSub)s.sub.notify()&&s.sub.dep.notify()}finally{Do()}}}function uf(e){if(e.dep.sc++,e.sub.flags&4){const t=e.dep.computed;if(t&&!e.dep.subs){t.flags|=20;for(let n=t.deps;n;n=n.nextDep)uf(n)}const s=e.dep.subs;s!==e&&(e.prevSub=s,s&&(s.nextSub=e)),e.dep.subs=e}}const fl=new WeakMap,Fn=Symbol(""),Zr=Symbol(""),pi=Symbol("");function Ft(e,t,s){if(_s&&st){let n=fl.get(e);n||fl.set(e,n=new Map);let a=n.get(s);a||(n.set(s,a=new Wl),a.map=n,a.key=s),a.track()}}function Gs(e,t,s,n,a,i){const l=fl.get(e);if(!l){fi++;return}const r=o=>{o&&o.trigger()};if(Oo(),t==="clear")l.forEach(r);else{const o=he(e),c=o&&jl(s);if(o&&s==="length"){const u=Number(n);l.forEach((d,f)=>{(f==="length"||f===pi||!Bt(f)&&f>=u)&&r(d)})}else switch((s!==void 0||l.has(void 0))&&r(l.get(s)),c&&r(l.get(pi)),t){case"add":o?c&&r(l.get("length")):(r(l.get(Fn)),ha(e)&&r(l.get(Zr)));break;case"delete":o||(r(l.get(Fn)),ha(e)&&r(l.get(Zr)));break;case"set":ha(e)&&r(l.get(Fn));break}}Do()}function am(e,t){const s=fl.get(e);return s&&s.get(t)}function sa(e){const t=Be(e);return t===e?t:(Ft(t,"iterate",pi),ss(e)?t:t.map(ws))}function Zl(e){return Ft(e=Be(e),"iterate",pi),e}function Os(e,t){return Ms(e)?ka(Xs(e)?ws(t):t):ws(t)}const im={__proto__:null,[Symbol.iterator](){return wr(this,Symbol.iterator,e=>Os(this,e))},concat(...e){return sa(this).concat(...e.map(t=>he(t)?sa(t):t))},entries(){return wr(this,"entries",e=>(e[1]=Os(this,e[1]),e))},every(e,t){return Us(this,"every",e,t,void 0,arguments)},filter(e,t){return Us(this,"filter",e,t,s=>s.map(n=>Os(this,n)),arguments)},find(e,t){return Us(this,"find",e,t,s=>Os(this,s),arguments)},findIndex(e,t){return Us(this,"findIndex",e,t,void 0,arguments)},findLast(e,t){return Us(this,"findLast",e,t,s=>Os(this,s),arguments)},findLastIndex(e,t){return Us(this,"findLastIndex",e,t,void 0,arguments)},forEach(e,t){return Us(this,"forEach",e,t,void 0,arguments)},includes(...e){return Sr(this,"includes",e)},indexOf(...e){return Sr(this,"indexOf",e)},join(e){return sa(this).join(e)},lastIndexOf(...e){return Sr(this,"lastIndexOf",e)},map(e,t){return Us(this,"map",e,t,void 0,arguments)},pop(){return Ha(this,"pop")},push(...e){return Ha(this,"push",e)},reduce(e,...t){return eu(this,"reduce",e,t)},reduceRight(e,...t){return eu(this,"reduceRight",e,t)},shift(){return Ha(this,"shift")},some(e,t){return Us(this,"some",e,t,void 0,arguments)},splice(...e){return Ha(this,"splice",e)},toReversed(){return sa(this).toReversed()},toSorted(e){return sa(this).toSorted(e)},toSpliced(...e){return sa(this).toSpliced(...e)},unshift(...e){return Ha(this,"unshift",e)},values(){return wr(this,"values",e=>Os(this,e))}};function wr(e,t,s){const n=Zl(e),a=n[t]();return n!==e&&!ss(e)&&(a._next=a.next,a.next=()=>{const i=a._next();return i.done||(i.value=s(i.value)),i}),a}const lm=Array.prototype;function Us(e,t,s,n,a,i){const l=Zl(e),r=l!==e&&!ss(e),o=l[t];if(o!==lm[t]){const d=o.apply(e,i);return r?ws(d):d}let c=s;l!==e&&(r?c=function(d,f){return s.call(this,Os(e,d),f,e)}:s.length>2&&(c=function(d,f){return s.call(this,d,f,e)}));const u=o.call(l,c,n);return r&&a?a(u):u}function eu(e,t,s,n){const a=Zl(e),i=a!==e&&!ss(e);let l=s,r=!1;a!==e&&(i?(r=n.length===0,l=function(c,u,d){return r&&(r=!1,c=Os(e,c)),s.call(this,c,Os(e,u),d,e)}):s.length>3&&(l=function(c,u,d){return s.call(this,c,u,d,e)}));const o=a[t](l,...n);return r?Os(e,o):o}function Sr(e,t,s){const n=Be(e);Ft(n,"iterate",pi);const a=n[t](...s);return(a===-1||a===!1)&&Ni(s[0])?(s[0]=Be(s[0]),n[t](...s)):a}function Ha(e,t,s=[]){nn(),Oo();const n=Be(e)[t].apply(e,s);return Do(),an(),n}const rm=os("__proto__,__v_isRef,__isVue"),df=new Set(Object.getOwnPropertyNames(Symbol).filter(e=>e!=="arguments"&&e!=="caller").map(e=>Symbol[e]).filter(Bt));function om(e){Bt(e)||(e=String(e));const t=Be(this);return Ft(t,"has",e),t.hasOwnProperty(e)}class ff{constructor(t=!1,s=!1){this._isReadonly=t,this._isShallow=s}get(t,s,n){if(s==="__v_skip")return t.__v_skip;const a=this._isReadonly,i=this._isShallow;if(s==="__v_isReactive")return!a;if(s==="__v_isReadonly")return a;if(s==="__v_isShallow")return i;if(s==="__v_raw")return n===(a?i?bf:vf:i?mf:gf).get(t)||Object.getPrototypeOf(t)===Object.getPrototypeOf(n)?t:void 0;const l=he(t);if(!a){let o;if(l&&(o=im[s]))return o;if(s==="hasOwnProperty")return om}const r=Reflect.get(t,s,yt(t)?t:n);if((Bt(s)?df.has(s):rm(s))||(a||Ft(t,"get",s),i))return r;if(yt(r)){const o=l&&jl(s)?r:r.value;return a&&je(o)?pl(o):o}return je(r)?a?pl(r):Sn(r):r}}class pf extends ff{constructor(t=!1){super(!1,t)}set(t,s,n,a){let i=t[s];const l=he(t)&&jl(s);if(!this._isShallow){const c=Ms(i);if(!ss(n)&&!Ms(n)&&(i=Be(i),n=Be(n)),!l&&yt(i)&&!yt(n))return c||(i.value=n),!0}const r=l?Number(s)<t.length:ze(t,s),o=Reflect.set(t,s,n,yt(t)?t:a);return t===Be(a)&&(r?Et(n,i)&&Gs(t,"set",s,n):Gs(t,"add",s,n)),o}deleteProperty(t,s){const n=ze(t,s);t[s];const a=Reflect.deleteProperty(t,s);return a&&n&&Gs(t,"delete",s,void 0),a}has(t,s){const n=Reflect.has(t,s);return(!Bt(s)||!df.has(s))&&Ft(t,"has",s),n}ownKeys(t){return Ft(t,"iterate",he(t)?"length":Fn),Reflect.ownKeys(t)}}class hf extends ff{constructor(t=!1){super(!0,t)}set(t,s){return!0}deleteProperty(t,s){return!0}}const cm=new pf,um=new hf,dm=new pf(!0),fm=new hf(!0),Jr=e=>e,Vi=e=>Reflect.getPrototypeOf(e);function pm(e,t,s){return function(...n){const a=this.__v_raw,i=Be(a),l=ha(i),r=e==="entries"||e===Symbol.iterator&&l,o=e==="keys"&&l,c=a[e](...n),u=s?Jr:t?ka:ws;return!t&&Ft(i,"iterate",o?Zr:Fn),Pe(Object.create(c),{next(){const{value:d,done:f}=c.next();return f?{value:d,done:f}:{value:r?[u(d[0]),u(d[1])]:u(d),done:f}}})}}function ji(e){return function(...t){return e==="delete"?!1:e==="clear"?void 0:this}}function hm(e,t){const s={get(a){const i=this.__v_raw,l=Be(i),r=Be(a);e||(Et(a,r)&&Ft(l,"get",a),Ft(l,"get",r));const{has:o}=Vi(l),c=t?Jr:e?ka:ws;if(o.call(l,a))return c(i.get(a));if(o.call(l,r))return c(i.get(r));i!==l&&i.get(a)},get size(){const a=this.__v_raw;return!e&&Ft(Be(a),"iterate",Fn),a.size},has(a){const i=this.__v_raw,l=Be(i),r=Be(a);return e||(Et(a,r)&&Ft(l,"has",a),Ft(l,"has",r)),a===r?i.has(a):i.has(a)||i.has(r)},forEach(a,i){const l=this,r=l.__v_raw,o=Be(r),c=t?Jr:e?ka:ws;return!e&&Ft(o,"iterate",Fn),r.forEach((u,d)=>a.call(i,c(u),c(d),l))}};return Pe(s,e?{add:ji("add"),set:ji("set"),delete:ji("delete"),clear:ji("clear")}:{add(a){const i=Be(this),l=Vi(i),r=Be(a),o=!t&&!ss(a)&&!Ms(a)?r:a;return l.has.call(i,o)||Et(a,o)&&l.has.call(i,a)||Et(r,o)&&l.has.call(i,r)||(i.add(o),Gs(i,"add",o,o)),this},set(a,i){!t&&!ss(i)&&!Ms(i)&&(i=Be(i));const l=Be(this),{has:r,get:o}=Vi(l);let c=r.call(l,a);c||(a=Be(a),c=r.call(l,a));const u=o.call(l,a);return l.set(a,i),c?Et(i,u)&&Gs(l,"set",a,i):Gs(l,"add",a,i),this},delete(a){const i=Be(this),{has:l,get:r}=Vi(i);let o=l.call(i,a);o||(a=Be(a),o=l.call(i,a)),r&&r.call(i,a);const c=i.delete(a);return o&&Gs(i,"delete",a,void 0),c},clear(){const a=Be(this),i=a.size!==0,l=a.clear();return i&&Gs(a,"clear",void 0,void 0),l}}),["keys","values","entries",Symbol.iterator].forEach(a=>{s[a]=pm(a,e,t)}),s}function Jl(e,t){const s=hm(e,t);return(n,a,i)=>a==="__v_isReactive"?!e:a==="__v_isReadonly"?e:a==="__v_raw"?n:Reflect.get(ze(s,a)&&a in n?s:n,a,i)}const gm={get:Jl(!1,!1)},mm={get:Jl(!1,!0)},vm={get:Jl(!0,!1)},bm={get:Jl(!0,!0)},gf=new WeakMap,mf=new WeakMap,vf=new WeakMap,bf=new WeakMap;function ym(e){switch(e){case"Object":case"Array":return 1;case"Map":case"Set":case"WeakMap":case"WeakSet":return 2;default:return 0}}function Sn(e){return Ms(e)?e:Yl(e,!1,cm,gm,gf)}function Po(e){return Yl(e,!1,dm,mm,mf)}function pl(e){return Yl(e,!0,um,vm,vf)}function xm(e){return Yl(e,!0,fm,bm,bf)}function Yl(e,t,s,n,a){if(!je(e)||e.__v_raw&&!(t&&e.__v_isReactive)||e.__v_skip||!Object.isExtensible(e))return e;const i=a.get(e);if(i)return i;const l=ym(Rg(e));if(l===0)return e;const r=new Proxy(e,l===2?n:s);return a.set(e,r),r}function Xs(e){return Ms(e)?Xs(e.__v_raw):!!(e&&e.__v_isReactive)}function Ms(e){return!!(e&&e.__v_isReadonly)}function ss(e){return!!(e&&e.__v_isShallow)}function Ni(e){return e?!!e.__v_raw:!1}function Be(e){const t=e&&e.__v_raw;return t?Be(t):e}function yf(e){return!ze(e,"__v_skip")&&Object.isExtensible(e)&&Jd(e,"__v_skip",!0),e}const ws=e=>je(e)?Sn(e):e,ka=e=>je(e)?pl(e):e;function yt(e){return e?e.__v_isRef===!0:!1}function h(e){return xf(e,!1)}function Fo(e){return xf(e,!0)}function xf(e,t){return yt(e)?e:new _m(e,t)}class _m{constructor(t,s){this.dep=new Wl,this.__v_isRef=!0,this.__v_isShallow=!1,this._rawValue=s?t:Be(t),this._value=s?t:ws(t),this.__v_isShallow=s}get value(){return this.dep.track(),this._value}set value(t){const s=this._rawValue,n=this.__v_isShallow||ss(t)||Ms(t);t=n?t:Be(t),Et(t,s)&&(this._rawValue=t,this._value=n?t:ws(t),this.dep.trigger())}}function km(e){e.dep&&e.dep.trigger()}function Ds(e){return yt(e)?e.value:e}function wm(e){return we(e)?e():Ds(e)}const Sm={get:(e,t,s)=>t==="__v_raw"?e:Ds(Reflect.get(e,t,s)),set:(e,t,s,n)=>{const a=e[t];return yt(a)&&!yt(s)?(a.value=s,!0):Reflect.set(e,t,s,n)}};function $o(e){return Xs(e)?e:new Proxy(e,Sm)}class Tm{constructor(t){this.__v_isRef=!0,this._value=void 0;const s=this.dep=new Wl,{get:n,set:a}=t(s.track.bind(s),s.trigger.bind(s));this._get=n,this._set=a}get value(){return this._value=this._get()}set value(t){this._set(t)}}function _f(e){return new Tm(e)}function Cm(e){const t=he(e)?new Array(e.length):{};for(const s in e)t[s]=kf(e,s);return t}class Em{constructor(t,s,n){this._object=t,this._defaultValue=n,this.__v_isRef=!0,this._value=void 0,this._key=Bt(s)?s:String(s),this._raw=Be(t);let a=!0,i=t;if(!he(t)||Bt(this._key)||!jl(this._key))do a=!Ni(i)||ss(i);while(a&&(i=i.__v_raw));this._shallow=a}get value(){let t=this._object[this._key];return this._shallow&&(t=Ds(t)),this._value=t===void 0?this._defaultValue:t}set value(t){if(this._shallow&&yt(this._raw[this._key])){const s=this._object[this._key];if(yt(s)){s.value=t;return}}this._object[this._key]=t}get dep(){return am(this._raw,this._key)}}class Am{constructor(t){this._getter=t,this.__v_isRef=!0,this.__v_isReadonly=!0,this._value=void 0}get value(){return this._value=this._getter()}}function Rm(e,t,s){return yt(e)?e:we(e)?new Am(e):je(e)&&arguments.length>1?kf(e,t,s):h(e)}function kf(e,t,s){return new Em(e,t,s)}class Im{constructor(t,s,n){this.fn=t,this.setter=s,this._value=void 0,this.dep=new Wl(this),this.__v_isRef=!0,this.deps=void 0,this.depsTail=void 0,this.flags=16,this.globalVersion=fi-1,this.next=void 0,this.effect=this,this.__v_isReadonly=!s,this.isSSR=n}notify(){if(this.flags|=16,!(this.flags&8)&&st!==this)return af(this,!0),!0}get value(){const t=this.dep.track();return of(this),t&&(t.version=this.dep.version),this._value}set value(t){this.setter&&this.setter(t)}}function Nm(e,t,s=!1){let n,a;return we(e)?n=e:(n=e.get,a=e.set),new Im(n,a,s)}const Lm={GET:"get",HAS:"has",ITERATE:"iterate"},Om={SET:"set",ADD:"add",DELETE:"delete",CLEAR:"clear"},zi={},hl=new WeakMap;let vn;function Dm(){return vn}function wf(e,t=!1,s=vn){if(s){let n=hl.get(s);n||hl.set(s,n=[]),n.push(e)}}function Mm(e,t,s=Fe){const{immediate:n,deep:a,once:i,scheduler:l,augmentJob:r,call:o}=s,c=x=>a?x:ss(x)||a===!1||a===0?Ws(x,1):Ws(x);let u,d,f,p,b=!1,g=!1;if(yt(e)?(d=()=>e.value,b=ss(e)):Xs(e)?(d=()=>c(e),b=!0):he(e)?(g=!0,b=e.some(x=>Xs(x)||ss(x)),d=()=>e.map(x=>{if(yt(x))return x.value;if(Xs(x))return c(x);if(we(x))return o?o(x,2):x()})):we(e)?t?d=o?()=>o(e,2):e:d=()=>{if(f){nn();try{f()}finally{an()}}const x=vn;vn=u;try{return o?o(e,3,[p]):e(p)}finally{vn=x}}:d=Ot,t&&a){const x=d,w=a===!0?1/0:a;d=()=>Ws(x(),w)}const T=sf(),O=()=>{u.stop(),T&&T.active&&Io(T.effects,u)};if(i&&t){const x=t;t=(...w)=>{const I=x(...w);return O(),I}}let y=g?new Array(e.length).fill(zi):zi;const v=x=>{if(!(!(u.flags&1)||!u.dirty&&!x))if(t){const w=u.run();if(x||a||b||(g?w.some((I,L)=>Et(I,y[L])):Et(w,y))){f&&f();const I=vn;vn=u;try{const L=[w,y===zi?void 0:g&&y[0]===zi?[]:y,p];y=w,o?o(t,3,L):t(...L)}finally{vn=I}}}else u.run()};return r&&r(v),u=new di(d),u.scheduler=l?()=>l(v,!1):v,p=x=>wf(x,!1,u),f=u.onStop=()=>{const x=hl.get(u);if(x){if(o)o(x,4);else for(const w of x)w();hl.delete(u)}},t?n?v(!0):y=u.run():l?l(v.bind(null,!0),!0):u.run(),O.pause=u.pause.bind(u),O.resume=u.resume.bind(u),O.stop=O,O}function Ws(e,t=1/0,s){if(t<=0||!je(e)||e.__v_skip||(s=s||new Map,(s.get(e)||0)>=t))return e;if(s.set(e,t),t--,yt(e))Ws(e.value,t,s);else if(he(e))for(let n=0;n<e.length;n++)Ws(e[n],t,s);else if(Wn(e)||ha(e))e.forEach(n=>{Ws(n,t,s)});else if(Vl(e)){for(const n in e)Ws(e[n],t,s);for(const n of Object.getOwnPropertySymbols(e))Object.prototype.propertyIsEnumerable.call(e,n)&&Ws(e[n],t,s)}return e}/**
* @vue/runtime-core v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const Sf=[];function Pm(e){Sf.push(e)}function Fm(){Sf.pop()}function $m(e,t){}const Um={SETUP_FUNCTION:0,0:"SETUP_FUNCTION",RENDER_FUNCTION:1,1:"RENDER_FUNCTION",NATIVE_EVENT_HANDLER:5,5:"NATIVE_EVENT_HANDLER",COMPONENT_EVENT_HANDLER:6,6:"COMPONENT_EVENT_HANDLER",VNODE_HOOK:7,7:"VNODE_HOOK",DIRECTIVE_HOOK:8,8:"DIRECTIVE_HOOK",TRANSITION_HOOK:9,9:"TRANSITION_HOOK",APP_ERROR_HANDLER:10,10:"APP_ERROR_HANDLER",APP_WARN_HANDLER:11,11:"APP_WARN_HANDLER",FUNCTION_REF:12,12:"FUNCTION_REF",ASYNC_COMPONENT_LOADER:13,13:"ASYNC_COMPONENT_LOADER",SCHEDULER:14,14:"SCHEDULER",COMPONENT_UPDATE:15,15:"COMPONENT_UPDATE",APP_UNMOUNT_CLEANUP:16,16:"APP_UNMOUNT_CLEANUP"},Bm={sp:"serverPrefetch hook",bc:"beforeCreate hook",c:"created hook",bm:"beforeMount hook",m:"mounted hook",bu:"beforeUpdate hook",u:"updated",bum:"beforeUnmount hook",um:"unmounted hook",a:"activated hook",da:"deactivated hook",ec:"errorCaptured hook",rtc:"renderTracked hook",rtg:"renderTriggered hook",0:"setup function",1:"render function",2:"watcher getter",3:"watcher callback",4:"watcher cleanup function",5:"native event handler",6:"component event handler",7:"vnode hook",8:"directive hook",9:"transition hook",10:"app errorHandler",11:"app warnHandler",12:"ref function",13:"async component loader",14:"scheduler flush",15:"component update",16:"app unmount cleanup function"};function Ma(e,t,s,n){try{return n?e(...n):e()}catch(a){Jn(a,t,s)}}function ls(e,t,s,n){if(we(e)){const a=Ma(e,t,s,n);return a&&No(a)&&a.catch(i=>{Jn(i,t,s)}),a}if(he(e)){const a=[];for(let i=0;i<e.length;i++)a.push(ls(e[i],t,s,n));return a}}function Jn(e,t,s,n=!0){const a=t?t.vnode:null,{errorHandler:i,throwUnhandledErrorInProduction:l}=t&&t.appContext.config||Fe;if(t){let r=t.parent;const o=t.proxy,c=`https://vuejs.org/error-reference/#runtime-${s}`;for(;r;){const u=r.ec;if(u){for(let d=0;d<u.length;d++)if(u[d](e,o,c)===!1)return}r=r.parent}if(i){nn(),Ma(i,null,10,[e,o,c]),an();return}}Hm(e,s,a,n,l)}function Hm(e,t,s,n=!0,a=!1){if(a)throw e;console.error(e)}const zt=[];let Ns=-1;const va=[];let bn=null,ra=0;const Tf=Promise.resolve();let gl=null;function St(e){const t=gl||Tf;return e?t.then(this?e.bind(this):e):t}function Vm(e){let t=Ns+1,s=zt.length;for(;t<s;){const n=t+s>>>1,a=zt[n],i=gi(a);i<e||i===e&&a.flags&2?t=n+1:s=n}return t}function Uo(e){if(!(e.flags&1)){const t=gi(e),s=zt[zt.length-1];!s||!(e.flags&2)&&t>=gi(s)?zt.push(e):zt.splice(Vm(t),0,e),e.flags|=1,Cf()}}function Cf(){gl||(gl=Tf.then(Ef))}function hi(e){he(e)?va.push(...e):bn&&e.id===-1?bn.splice(ra+1,0,e):e.flags&1||(va.push(e),e.flags|=1),Cf()}function tu(e,t,s=Ns+1){for(;s<zt.length;s++){const n=zt[s];if(n&&n.flags&2){if(e&&n.id!==e.uid)continue;zt.splice(s,1),s--,n.flags&4&&(n.flags&=-2),n(),n.flags&4||(n.flags&=-2)}}}function ml(e){if(va.length){const t=[...new Set(va)].sort((s,n)=>gi(s)-gi(n));if(va.length=0,bn){bn.push(...t);return}for(bn=t,ra=0;ra<bn.length;ra++){const s=bn[ra];s.flags&4&&(s.flags&=-2),s.flags&8||s(),s.flags&=-2}bn=null,ra=0}}const gi=e=>e.id==null?e.flags&2?-1:1/0:e.id;function Ef(e){try{for(Ns=0;Ns<zt.length;Ns++){const t=zt[Ns];t&&!(t.flags&8)&&(t.flags&4&&(t.flags&=-2),Ma(t,t.i,t.i?15:14),t.flags&4||(t.flags&=-2))}}finally{for(;Ns<zt.length;Ns++){const t=zt[Ns];t&&(t.flags&=-2)}Ns=-1,zt.length=0,ml(),gl=null,(zt.length||va.length)&&Ef()}}let oa,qi=[];function Af(e,t){var s,n;oa=e,oa?(oa.enabled=!0,qi.forEach(({event:a,args:i})=>oa.emit(a,...i)),qi=[]):typeof window<"u"&&window.HTMLElement&&!((n=(s=window.navigator)==null?void 0:s.userAgent)!=null&&n.includes("jsdom"))?((t.__VUE_DEVTOOLS_HOOK_REPLAY__=t.__VUE_DEVTOOLS_HOOK_REPLAY__||[]).push(i=>{Af(i,t)}),setTimeout(()=>{oa||(t.__VUE_DEVTOOLS_HOOK_REPLAY__=null,qi=[])},3e3)):qi=[]}let Lt=null,Ql=null;function mi(e){const t=Lt;return Lt=e,Ql=e&&e.type.__scopeId||null,t}function jm(e){Ql=e}function zm(){Ql=null}const qm=e=>Bo;function Bo(e,t=Lt,s){if(!t||e._n)return e;const n=(...a)=>{n._d&&xi(-1);const i=mi(t);let l;try{l=e(...a)}finally{mi(i),n._d&&xi(1)}return l};return n._n=!0,n._c=!0,n._d=!0,n}function Km(e,t){if(Lt===null)return e;const s=Mi(Lt),n=e.dirs||(e.dirs=[]);for(let a=0;a<t.length;a++){let[i,l,r,o=Fe]=t[a];i&&(we(i)&&(i={mounted:i,updated:i}),i.deep&&Ws(l),n.push({dir:i,instance:s,value:l,oldValue:void 0,arg:r,modifiers:o}))}return e}function Ls(e,t,s,n){const a=e.dirs,i=t&&t.dirs;for(let l=0;l<a.length;l++){const r=a[l];i&&(r.oldValue=i[l].value);let o=r.dir[n];o&&(nn(),ls(o,s,8,[e.el,r,e,t]),an())}}function ni(e,t){if(Nt){let s=Nt.provides;const n=Nt.parent&&Nt.parent.provides;n===s&&(s=Nt.provides=Object.create(n)),s[e]=t}}function hs(e,t,s=!1){const n=Kt();if(n||$n){let a=$n?$n._context.provides:n?n.parent==null||n.ce?n.vnode.appContext&&n.vnode.appContext.provides:n.parent.provides:void 0;if(a&&e in a)return a[e];if(arguments.length>1)return s&&we(t)?t.call(n&&n.proxy):t}}function Gm(){return!!(Kt()||$n)}const Rf=Symbol.for("v-scx"),If=()=>hs(Rf);function Wm(e,t){return Li(e,null,t)}function Zm(e,t){return Li(e,null,{flush:"post"})}function Nf(e,t){return Li(e,null,{flush:"sync"})}function gs(e,t,s){return Li(e,t,s)}function Li(e,t,s=Fe){const{immediate:n,deep:a,flush:i,once:l}=s,r=Pe({},s),o=t&&n||!t&&i!=="post";let c;if(zn){if(i==="sync"){const p=If();c=p.__watcherHandles||(p.__watcherHandles=[])}else if(!o){const p=()=>{};return p.stop=Ot,p.resume=Ot,p.pause=Ot,p}}const u=Nt;r.call=(p,b,g)=>ls(p,u,b,g);let d=!1;i==="post"?r.scheduler=p=>{vt(p,u&&u.suspense)}:i!=="sync"&&(d=!0,r.scheduler=(p,b)=>{b?p():Uo(p)}),r.augmentJob=p=>{t&&(p.flags|=4),d&&(p.flags|=2,u&&(p.id=u.uid,p.i=u))};const f=Mm(e,t,r);return zn&&(c?c.push(f):o&&f()),f}function Jm(e,t,s){const n=this.proxy,a=Ae(e)?e.includes(".")?Lf(n,e):()=>n[e]:e.bind(n,n);let i;we(t)?i=t:(i=t.handler,s=t);const l=Pa(this),r=Li(a,i.bind(n),s);return l(),r}function Lf(e,t){const s=t.split(".");return()=>{let n=e;for(let a=0;a<s.length&&n;a++)n=n[s[a]];return n}}const gn=new WeakMap,Of=Symbol("_vte"),Df=e=>e.__isTeleport,On=e=>e&&(e.disabled||e.disabled===""),Ym=e=>e&&(e.defer||e.defer===""),su=e=>typeof SVGElement<"u"&&e instanceof SVGElement,nu=e=>typeof MathMLElement=="function"&&e instanceof MathMLElement,Yr=(e,t)=>{const s=e&&e.to;return Ae(s)?t?t(s):null:s},Qm={name:"Teleport",__isTeleport:!0,process(e,t,s,n,a,i,l,r,o,c){const{mc:u,pc:d,pbc:f,o:{insert:p,querySelector:b,createText:g,createComment:T,parentNode:O}}=c,y=On(t.props);let{dynamicChildren:v}=t;const x=(L,k,A)=>{L.shapeFlag&16&&u(L.children,k,A,a,i,l,r,o)},w=(L=t)=>{const k=On(L.props),A=L.target=Yr(L.props,b),D=Qr(A,L,g,p);A&&(l!=="svg"&&su(A)?l="svg":l!=="mathml"&&nu(A)&&(l="mathml"),a&&a.isCE&&(a.ce._teleportTargets||(a.ce._teleportTargets=new Set)).add(A),k||(x(L,A,D),Ja(L,!1)))},I=L=>{const k=()=>{if(gn.get(L)===k){if(gn.delete(L),On(L.props)){const A=O(L.el)||s;x(L,A,L.anchor),Ja(L,!0)}w(L)}};gn.set(L,k),vt(k,i)};if(e==null){const L=t.el=g(""),k=t.anchor=g("");if(p(L,s,n),p(k,s,n),Ym(t.props)||i&&i.pendingBranch){I(t);return}y&&(x(t,s,k),Ja(t,!0)),w()}else{t.el=e.el;const L=t.anchor=e.anchor,k=gn.get(e);if(k){k.flags|=8,gn.delete(e),I(t);return}t.targetStart=e.targetStart;const A=t.target=e.target,D=t.targetAnchor=e.targetAnchor,B=On(e.props),M=B?s:A,S=B?L:D;if(l==="svg"||su(A)?l="svg":(l==="mathml"||nu(A))&&(l="mathml"),v?(f(e.dynamicChildren,v,M,a,i,l,r),Xo(e,t,!0)):o||d(e,t,M,S,a,i,l,r,!1),y)B?t.props&&e.props&&t.props.to!==e.props.to&&(t.props.to=e.props.to):Ki(t,s,L,c,1);else if((t.props&&t.props.to)!==(e.props&&e.props.to)){const U=t.target=Yr(t.props,b);U&&Ki(t,U,null,c,0)}else B&&Ki(t,A,D,c,1);Ja(t,y)}},remove(e,t,s,{um:n,o:{remove:a}},i){const{shapeFlag:l,children:r,anchor:o,targetStart:c,targetAnchor:u,target:d,props:f}=e,p=i||!On(f),b=gn.get(e);if(b&&(b.flags|=8,gn.delete(e)),d&&(a(c),a(u)),i&&a(o),!b&&l&16)for(let g=0;g<r.length;g++){const T=r[g];n(T,t,s,p,!!T.dynamicChildren)}},move:Ki,hydrate:Xm};function Ki(e,t,s,{o:{insert:n},m:a},i=2){i===0&&n(e.targetAnchor,t,s);const{el:l,anchor:r,shapeFlag:o,children:c,props:u}=e,d=i===2;if(d&&n(l,t,s),!gn.has(e)&&(!d||On(u))&&o&16)for(let f=0;f<c.length;f++)a(c[f],t,s,2);d&&n(r,t,s)}function Xm(e,t,s,n,a,i,{o:{nextSibling:l,parentNode:r,querySelector:o,insert:c,createText:u}},d){function f(T,O){let y=O;for(;y;){if(y&&y.nodeType===8){if(y.data==="teleport start anchor")t.targetStart=y;else if(y.data==="teleport anchor"){t.targetAnchor=y,T._lpa=t.targetAnchor&&l(t.targetAnchor);break}}y=l(y)}}function p(T,O){O.anchor=d(l(T),O,r(T),s,n,a,i)}const b=t.target=Yr(t.props,o),g=On(t.props);if(b){const T=b._lpa||b.firstChild;t.shapeFlag&16&&(g?(p(e,t),f(b,T),t.targetAnchor||Qr(b,t,u,c,r(e)===b?e:null)):(t.anchor=l(e),f(b,T),t.targetAnchor||Qr(b,t,u,c),d(T&&l(T),t,b,s,n,a,i))),Ja(t,g)}else g&&t.shapeFlag&16&&(p(e,t),t.targetStart=e,t.targetAnchor=l(e));return t.anchor&&l(t.anchor)}const ev=Qm;function Ja(e,t){const s=e.ctx;if(s&&s.ut){let n,a;for(t?(n=e.el,a=e.anchor):(n=e.targetStart,a=e.targetAnchor);n&&n!==a;)n.nodeType===1&&n.setAttribute("data-v-owner",s.uid),n=n.nextSibling;s.ut()}}function Qr(e,t,s,n,a=null){const i=t.targetStart=s(""),l=t.targetAnchor=s("");return i[Of]=l,e&&(n(i,e,a),n(l,e,a)),l}const ds=Symbol("_leaveCb"),Va=Symbol("_enterCb");function Ho(){const e={isMounted:!1,isLeaving:!1,isUnmounting:!1,leavingVNodes:new Map};return He(()=>{e.isMounted=!0}),sr(()=>{e.isUnmounting=!0}),e}const us=[Function,Array],Vo={mode:String,appear:Boolean,persisted:Boolean,onBeforeEnter:us,onEnter:us,onAfterEnter:us,onEnterCancelled:us,onBeforeLeave:us,onLeave:us,onAfterLeave:us,onLeaveCancelled:us,onBeforeAppear:us,onAppear:us,onAfterAppear:us,onAppearCancelled:us},Mf=e=>{const t=e.subTree;return t.component?Mf(t.component):t},tv={name:"BaseTransition",props:Vo,setup(e,{slots:t}){const s=Kt(),n=Ho();return()=>{const a=t.default&&Xl(t.default(),!0),i=a&&a.length?Pf(a):s.subTree?bp():void 0;if(!i)return;const l=Be(e),{mode:r}=l;if(n.isLeaving)return Tr(i);const o=au(i);if(!o)return Tr(i);let c=wa(o,l,n,s,d=>c=d);o.type!==ht&&ln(o,c);let u=s.subTree&&au(s.subTree);if(u&&u.type!==ht&&!xs(u,o)&&Mf(s).type!==ht){let d=wa(u,l,n,s);if(ln(u,d),r==="out-in"&&o.type!==ht)return n.isLeaving=!0,d.afterLeave=()=>{n.isLeaving=!1,s.job.flags&8||s.update(),delete d.afterLeave,u=void 0},Tr(i);r==="in-out"&&o.type!==ht?d.delayLeave=(f,p,b)=>{const g=$f(n,u);g[String(u.key)]=u,f[ds]=()=>{p(),f[ds]=void 0,delete c.delayedLeave,u=void 0},c.delayedLeave=()=>{b(),delete c.delayedLeave,u=void 0}}:u=void 0}else u&&(u=void 0);return i}}};function Pf(e){let t=e[0];if(e.length>1){for(const s of e)if(s.type!==ht){t=s;break}}return t}const Ff=tv;function $f(e,t){const{leavingVNodes:s}=e;let n=s.get(t.type);return n||(n=Object.create(null),s.set(t.type,n)),n}function wa(e,t,s,n,a){const{appear:i,mode:l,persisted:r=!1,onBeforeEnter:o,onEnter:c,onAfterEnter:u,onEnterCancelled:d,onBeforeLeave:f,onLeave:p,onAfterLeave:b,onLeaveCancelled:g,onBeforeAppear:T,onAppear:O,onAfterAppear:y,onAppearCancelled:v}=t,x=String(e.key),w=$f(s,e),I=(A,D)=>{A&&ls(A,n,9,D)},L=(A,D)=>{const B=D[1];I(A,D),he(A)?A.every(M=>M.length<=1)&&B():A.length<=1&&B()},k={mode:l,persisted:r,beforeEnter(A){let D=o;if(!s.isMounted)if(i)D=T||o;else return;A[ds]&&A[ds](!0);const B=w[x];B&&xs(e,B)&&B.el[ds]&&B.el[ds](),I(D,[A])},enter(A){if(w[x]===e)return;let D=c,B=u,M=d;if(!s.isMounted)if(i)D=O||c,B=y||u,M=v||d;else return;let S=!1;A[Va]=W=>{S||(S=!0,W?I(M,[A]):I(B,[A]),k.delayedLeave&&k.delayedLeave(),A[Va]=void 0)};const U=A[Va].bind(null,!1);D?L(D,[A,U]):U()},leave(A,D){const B=String(e.key);if(A[Va]&&A[Va](!0),s.isUnmounting)return D();I(f,[A]);let M=!1;A[ds]=U=>{M||(M=!0,D(),U?I(g,[A]):I(b,[A]),A[ds]=void 0,w[B]===e&&delete w[B])};const S=A[ds].bind(null,!1);w[B]=e,p?L(p,[A,S]):S()},clone(A){const D=wa(A,t,s,n,a);return a&&a(D),D}};return k}function Tr(e){if(Di(e))return e=Ps(e),e.children=null,e}function au(e){if(!Di(e))return Df(e.type)&&e.children?Pf(e.children):e;if(e.component)return e.component.subTree;const{shapeFlag:t,children:s}=e;if(s){if(t&16)return s[0];if(t&32&&we(s.default))return s.default()}}function ln(e,t){e.shapeFlag&6&&e.component?(e.transition=t,ln(e.component.subTree,t)):e.shapeFlag&128?(e.ssContent.transition=t.clone(e.ssContent),e.ssFallback.transition=t.clone(e.ssFallback)):e.transition=t}function Xl(e,t=!1,s){let n=[],a=0;for(let i=0;i<e.length;i++){let l=e[i];const r=s==null?l.key:String(s)+String(l.key!=null?l.key:i);l.type===At?(l.patchFlag&128&&a++,n=n.concat(Xl(l.children,t,r))):(t||l.type!==ht)&&n.push(r!=null?Ps(l,{key:r}):l)}if(a>1)for(let i=0;i<n.length;i++)n[i].patchFlag=-2;return n}function Oi(e,t){return we(e)?Pe({name:e.name},t,{setup:e}):e}function sv(){const e=Kt();return e?(e.appContext.config.idPrefix||"v")+"-"+e.ids[0]+e.ids[1]++:""}function jo(e){e.ids=[e.ids[0]+e.ids[2]+++"-",0,0]}function nv(e){const t=Kt(),s=Fo(null);if(t){const a=t.refs===Fe?t.refs={}:t.refs;Object.defineProperty(a,e,{enumerable:!0,get:()=>s.value,set:i=>s.value=i})}return s}function iu(e,t){let s;return!!((s=Object.getOwnPropertyDescriptor(e,t))&&!s.configurable)}const vl=new WeakMap;function ba(e,t,s,n,a=!1){if(he(e)){e.forEach((g,T)=>ba(g,t&&(he(t)?t[T]:t),s,n,a));return}if(en(n)&&!a){n.shapeFlag&512&&n.type.__asyncResolved&&n.component.subTree.component&&ba(e,t,s,n.component.subTree);return}const i=n.shapeFlag&4?Mi(n.component):n.el,l=a?null:i,{i:r,r:o}=e,c=t&&t.r,u=r.refs===Fe?r.refs={}:r.refs,d=r.setupState,f=Be(d),p=d===Fe?da:g=>iu(u,g)?!1:ze(f,g),b=(g,T)=>!(T&&iu(u,T));if(c!=null&&c!==o){if(lu(t),Ae(c))u[c]=null,p(c)&&(d[c]=null);else if(yt(c)){const g=t;b(c,g.k)&&(c.value=null),g.k&&(u[g.k]=null)}}if(we(o))Ma(o,r,12,[l,u]);else{const g=Ae(o),T=yt(o);if(g||T){const O=()=>{if(e.f){const y=g?p(o)?d[o]:u[o]:b()||!e.k?o.value:u[e.k];if(a)he(y)&&Io(y,i);else if(he(y))y.includes(i)||y.push(i);else if(g)u[o]=[i],p(o)&&(d[o]=u[o]);else{const v=[i];b(o,e.k)&&(o.value=v),e.k&&(u[e.k]=v)}}else g?(u[o]=l,p(o)&&(d[o]=l)):T&&(b(o,e.k)&&(o.value=l),e.k&&(u[e.k]=l))};if(l){const y=()=>{O(),vl.delete(e)};y.id=-1,vl.set(e,y),vt(y,s)}else lu(e),O()}}}function lu(e){const t=vl.get(e);t&&(t.flags|=8,vl.delete(e))}let ru=!1;const na=()=>{ru||(console.error("Hydration completed but contains mismatches."),ru=!0)},av=e=>e.namespaceURI.includes("svg")&&e.tagName!=="foreignObject",iv=e=>e.namespaceURI.includes("MathML"),Gi=e=>{if(e.nodeType===1){if(av(e))return"svg";if(iv(e))return"mathml"}},fa=e=>e.nodeType===8;function lv(e){const{mt:t,p:s,o:{patchProp:n,createText:a,nextSibling:i,parentNode:l,remove:r,insert:o,createComment:c}}=e,u=(v,x)=>{if(!x.hasChildNodes()){s(null,v,x),ml(),x._vnode=v;return}d(x.firstChild,v,null,null,null),ml(),x._vnode=v},d=(v,x,w,I,L,k=!1)=>{k=k||!!x.dynamicChildren;const A=fa(v)&&v.data==="[",D=()=>g(v,x,w,I,L,A),{type:B,ref:M,shapeFlag:S,patchFlag:U}=x;let W=v.nodeType;x.el=v,U===-2&&(k=!1,x.dynamicChildren=null);let F=null;switch(B){case _n:W!==3?x.children===""?(o(x.el=a(""),l(v),v),F=v):F=D():(v.data!==x.children&&(na(),v.data=x.children),F=i(v));break;case ht:y(v)?(F=i(v),O(x.el=v.content.firstChild,v,w)):W!==8||A?F=D():F=i(v);break;case Un:if(A&&(v=i(v),W=v.nodeType),W===1||W===3){F=v;const N=!x.children.length;for(let C=0;C<x.staticCount;C++)N&&(x.children+=F.nodeType===1?F.outerHTML:F.data),C===x.staticCount-1&&(x.anchor=F),F=i(F);return A?i(F):F}else D();break;case At:A?F=b(v,x,w,I,L,k):F=D();break;default:if(S&1)(W!==1||x.type.toLowerCase()!==v.tagName.toLowerCase())&&!y(v)?F=D():F=f(v,x,w,I,L,k);else if(S&6){x.slotScopeIds=L;const N=l(v);if(A?F=T(v):fa(v)&&v.data==="teleport start"?F=T(v,v.data,"teleport end"):F=i(v),t(x,N,null,w,I,Gi(N),k),en(x)&&!x.type.__asyncResolved){let C;A?(C=lt(At),C.anchor=F?F.previousSibling:N.lastChild):C=v.nodeType===3?tc(""):lt("div"),C.el=v,x.component.subTree=C}}else S&64?W!==8?F=D():F=x.type.hydrate(v,x,w,I,L,k,e,p):S&128&&(F=x.type.hydrate(v,x,w,I,Gi(l(v)),L,k,e,d))}return M!=null&&ba(M,null,I,x),F},f=(v,x,w,I,L,k)=>{k=k||!!x.dynamicChildren;const{type:A,props:D,patchFlag:B,shapeFlag:M,dirs:S,transition:U}=x,W=A==="input"||A==="option";if(W||B!==-1){S&&Ls(x,null,w,"created");let F=!1;if(y(v)){F=cp(null,U)&&w&&w.vnode.props&&w.vnode.props.appear;const C=v.content.firstChild;if(F){const j=C.getAttribute("class");j&&(C.$cls=j),U.beforeEnter(C)}O(C,v,w),x.el=v=C}if(M&16&&!(D&&(D.innerHTML||D.textContent))){let C=p(v.firstChild,x,v,w,I,L,k);for(C&&!Wi(v,1)&&na();C;){const j=C;C=C.nextSibling,r(j)}}else if(M&8){let C=x.children;C[0]===`
`&&(v.tagName==="PRE"||v.tagName==="TEXTAREA")&&(C=C.slice(1));const{textContent:j}=v;j!==C&&j!==C.replace(/\r\n|\r/g,`
`)&&(Wi(v,0)||na(),v.textContent=x.children)}if(D){if(W||!k||B&48){const C=v.tagName.includes("-");for(const j in D)(W&&(j.endsWith("value")||j==="indeterminate")||Gn(j)&&!Qs(j)||j[0]==="."||C&&!Qs(j))&&n(v,j,null,D[j],void 0,w)}else if(D.onClick)n(v,"onClick",null,D.onClick,void 0,w);else if(B&4&&Xs(D.style))for(const C in D.style)D.style[C]}let N;(N=D&&D.onVnodeBeforeMount)&&Yt(N,w,x),S&&Ls(x,null,w,"beforeMount"),((N=D&&D.onVnodeMounted)||S||F)&&pp(()=>{N&&Yt(N,w,x),F&&U.enter(v),S&&Ls(x,null,w,"mounted")},I)}return v.nextSibling},p=(v,x,w,I,L,k,A)=>{A=A||!!x.dynamicChildren;const D=x.children,B=D.length;let M=!1;for(let S=0;S<B;S++){const U=A?D[S]:D[S]=Xt(D[S]),W=U.type===_n;v?(W&&!A&&S+1<B&&Xt(D[S+1]).type===_n&&(o(a(v.data.slice(U.children.length)),w,i(v)),v.data=U.children),v=d(v,U,I,L,k,A)):W&&!U.children?o(U.el=a(""),w):(M||(M=!0,Wi(w,1)||na()),s(null,U,w,null,I,L,Gi(w),k))}return v},b=(v,x,w,I,L,k)=>{const{slotScopeIds:A}=x;A&&(L=L?L.concat(A):A);const D=l(v),B=p(i(v),x,D,w,I,L,k);return B&&fa(B)&&B.data==="]"?i(x.anchor=B):(na(),o(x.anchor=c("]"),D,B),B)},g=(v,x,w,I,L,k)=>{if(Wi(v.parentElement,1)||na(),x.el=null,k){const B=T(v);for(;;){const M=i(v);if(M&&M!==B)r(M);else break}}const A=i(v),D=l(v);return r(v),s(null,x,D,A,w,I,Gi(D),L),w&&(w.vnode.el=x.el,ar(w,x.el)),A},T=(v,x="[",w="]")=>{let I=0;for(;v;)if(v=i(v),v&&fa(v)&&(v.data===x&&I++,v.data===w)){if(I===0)return i(v);I--}return v},O=(v,x,w)=>{const I=x.parentNode;I&&I.replaceChild(v,x);let L=w;for(;L;)L.vnode.el===x&&(L.vnode.el=L.subTree.el=v),L=L.parent},y=v=>v.nodeType===1&&v.tagName==="TEMPLATE";return[u,d]}const ou="data-allow-mismatch",rv={0:"text",1:"children",2:"class",3:"style",4:"attribute"};function Wi(e,t){if(t===0||t===1)for(;e&&!e.hasAttribute(ou);)e=e.parentElement;const s=e&&e.getAttribute(ou);if(s==null)return!1;if(s==="")return!0;{const n=s.split(",");return t===0&&n.includes("children")?!0:n.includes(rv[t])}}const ov=Kl().requestIdleCallback||(e=>setTimeout(e,1)),cv=Kl().cancelIdleCallback||(e=>clearTimeout(e)),uv=(e=1e4)=>t=>{const s=ov(t,{timeout:e});return()=>cv(s)};function dv(e){const{top:t,left:s,bottom:n,right:a}=e.getBoundingClientRect(),{innerHeight:i,innerWidth:l}=window;return(t>0&&t<i||n>0&&n<i)&&(s>0&&s<l||a>0&&a<l)}const fv=e=>(t,s)=>{const n=new IntersectionObserver(a=>{for(const i of a)if(i.isIntersecting){n.disconnect(),t();break}},e);return s(a=>{if(a instanceof Element){if(dv(a))return t(),n.disconnect(),!1;n.observe(a)}}),()=>n.disconnect()},pv=e=>t=>{if(e){const s=matchMedia(e);if(s.matches)t();else return s.addEventListener("change",t,{once:!0}),()=>s.removeEventListener("change",t)}},hv=(e=[])=>(t,s)=>{Ae(e)&&(e=[e]);let n=!1;const a=l=>{n||(n=!0,i(),t(),l.target.dispatchEvent(new l.constructor(l.type,l)))},i=()=>{s(l=>{for(const r of e)l.removeEventListener(r,a)})};return s(l=>{for(const r of e)l.addEventListener(r,a,{once:!0})}),i};function gv(e,t){if(fa(e)&&e.data==="["){let s=1,n=e.nextSibling;for(;n;){if(n.nodeType===1){if(t(n)===!1)break}else if(fa(n))if(n.data==="]"){if(--s===0)break}else n.data==="["&&s++;n=n.nextSibling}}else t(e)}const en=e=>!!e.type.__asyncLoader;function mv(e){we(e)&&(e={loader:e});const{loader:t,loadingComponent:s,errorComponent:n,delay:a=200,hydrate:i,timeout:l,suspensible:r=!0,onError:o}=e;let c=null,u,d=0;const f=()=>(d++,c=null,p()),p=()=>{let b;return c||(b=c=t().catch(g=>{if(g=g instanceof Error?g:new Error(String(g)),o)return new Promise((T,O)=>{o(g,()=>T(f()),()=>O(g),d+1)});throw g}).then(g=>b!==c&&c?c:(g&&(g.__esModule||g[Symbol.toStringTag]==="Module")&&(g=g.default),u=g,g)))};return Oi({name:"AsyncComponentWrapper",__asyncLoader:p,__asyncHydrate(b,g,T){let O=!1;(g.bu||(g.bu=[])).push(()=>O=!0);const y=()=>{O||T()},v=i?()=>{const x=i(y,w=>gv(b,w));x&&(g.bum||(g.bum=[])).push(x)}:y;u?v():p().then(()=>!g.isUnmounted&&v())},get __asyncResolved(){return u},setup(){const b=Nt;if(jo(b),u)return()=>Zi(u,b);const g=w=>{c=null,Jn(w,b,13,!n)};if(r&&b.suspense||zn)return p().then(w=>()=>Zi(w,b)).catch(w=>(g(w),()=>n?lt(n,{error:w}):null));const T=h(!1),O=h(),y=h(!!a);let v,x;return gt(()=>{v!=null&&clearTimeout(v),x!=null&&clearTimeout(x)}),a&&(x=setTimeout(()=>{b.isUnmounted||(y.value=!1)},a)),l!=null&&(v=setTimeout(()=>{if(!b.isUnmounted&&!T.value&&!O.value){const w=new Error(`Async component timed out after ${l}ms.`);g(w),O.value=w}},l)),p().then(()=>{b.isUnmounted||(T.value=!0,b.parent&&Di(b.parent.vnode)&&b.parent.update())}).catch(w=>{if(b.isUnmounted){c=null;return}g(w),O.value=w}),()=>{if(T.value&&u)return Zi(u,b);if(O.value&&n)return lt(n,{error:O.value});if(s&&!y.value)return Zi(s,b)}}})}function Zi(e,t){const{ref:s,props:n,children:a,ce:i}=t.vnode,l=lt(e,n,a);return l.ref=s,l.ce=i,delete t.vnode.ce,l}const Di=e=>e.type.__isKeepAlive,vv={name:"KeepAlive",__isKeepAlive:!0,props:{include:[String,RegExp,Array],exclude:[String,RegExp,Array],max:[String,Number]},setup(e,{slots:t}){const s=Kt(),n=s.ctx;if(!n.renderer)return()=>{const y=t.default&&t.default();return y&&y.length===1?y[0]:y};const a=new Map,i=new Set;let l=null;const r=s.suspense,{renderer:{p:o,m:c,um:u,o:{createElement:d}}}=n,f=d("div");n.activate=(y,v,x,w,I)=>{const L=y.component;c(y,v,x,0,r),o(L.vnode,y,v,x,L,r,w,y.slotScopeIds,I),vt(()=>{L.isDeactivated=!1,L.a&&ma(L.a);const k=y.props&&y.props.onVnodeMounted;k&&Yt(k,L.parent,y)},r)},n.deactivate=y=>{const v=y.component;yl(v.m),yl(v.a),c(y,f,null,1,r),vt(()=>{v.da&&ma(v.da);const x=y.props&&y.props.onVnodeUnmounted;x&&Yt(x,v.parent,y),v.isDeactivated=!0},r)};function p(y){Cr(y),u(y,s,r,!0)}function b(y){a.forEach((v,x)=>{const w=ro(en(v)?v.type.__asyncResolved||{}:v.type);w&&!y(w)&&g(x)})}function g(y){const v=a.get(y);v&&(!l||!xs(v,l))?p(v):l&&Cr(l),a.delete(y),i.delete(y)}gs(()=>[e.include,e.exclude],([y,v])=>{y&&b(x=>Ya(y,x)),v&&b(x=>!Ya(v,x))},{flush:"post",deep:!0});let T=null;const O=()=>{T!=null&&(xl(s.subTree.type)?vt(()=>{a.set(T,Ji(s.subTree))},s.subTree.suspense):a.set(T,Ji(s.subTree)))};return He(O),tr(O),sr(()=>{a.forEach(y=>{const{subTree:v,suspense:x}=s,w=Ji(v);if(y.type===w.type&&y.key===w.key){Cr(w);const I=w.component.da;I&&vt(I,x);return}p(y)})}),()=>{if(T=null,!t.default)return l=null;const y=t.default(),v=y[0];if(y.length>1)return l=null,y;if(!rn(v)||!(v.shapeFlag&4)&&!(v.shapeFlag&128))return l=null,v;let x=Ji(v);if(x.type===ht)return l=null,x;const w=x.type,I=ro(en(x)?x.type.__asyncResolved||{}:w),{include:L,exclude:k,max:A}=e;if(L&&(!I||!Ya(L,I))||k&&I&&Ya(k,I))return x.shapeFlag&=-257,l=x,v;const D=x.key==null?w:x.key,B=a.get(D);return x.el&&(x=Ps(x),v.shapeFlag&128&&(v.ssContent=x)),T=D,B?(x.el=B.el,x.component=B.component,x.transition&&ln(x,x.transition),x.shapeFlag|=512,i.delete(D),i.add(D)):(i.add(D),A&&i.size>parseInt(A,10)&&g(i.values().next().value)),x.shapeFlag|=256,l=x,xl(v.type)?v:x}}},bv=vv;function Ya(e,t){return he(e)?e.some(s=>Ya(s,t)):Ae(e)?e.split(",").includes(t):Ag(e)?(e.lastIndex=0,e.test(t)):!1}function zo(e,t){Uf(e,"a",t)}function qo(e,t){Uf(e,"da",t)}function Uf(e,t,s=Nt){const n=e.__wdc||(e.__wdc=()=>{let a=s;for(;a;){if(a.isDeactivated)return;a=a.parent}return e()});if(er(t,n,s),s){let a=s.parent;for(;a&&a.parent;)Di(a.parent.vnode)&&yv(n,t,s,a),a=a.parent}}function yv(e,t,s,n){const a=er(t,e,n,!0);gt(()=>{Io(n[t],a)},s)}function Cr(e){e.shapeFlag&=-257,e.shapeFlag&=-513}function Ji(e){return e.shapeFlag&128?e.ssContent:e}function er(e,t,s=Nt,n=!1){if(s){const a=s[e]||(s[e]=[]),i=t.__weh||(t.__weh=(...l)=>{nn();const r=Pa(s),o=ls(t,s,e,l);return r(),an(),o});return n?a.unshift(i):a.push(i),i}}const on=e=>(t,s=Nt)=>{(!zn||e==="sp")&&er(e,(...n)=>t(...n),s)},Bf=on("bm"),He=on("m"),Ko=on("bu"),tr=on("u"),sr=on("bum"),gt=on("um"),Hf=on("sp"),Vf=on("rtg"),jf=on("rtc");function zf(e,t=Nt){er("ec",e,t)}const Go="components",xv="directives";function _v(e,t){return Wo(Go,e,!0,t)||e}const qf=Symbol.for("v-ndc");function kv(e){return Ae(e)?Wo(Go,e,!1)||e:e||qf}function wv(e){return Wo(xv,e)}function Wo(e,t,s=!0,n=!1){const a=Lt||Nt;if(a){const i=a.type;if(e===Go){const r=ro(i,!1);if(r&&(r===t||r===Qe(t)||r===Zn(Qe(t))))return i}const l=cu(a[e]||i[e],t)||cu(a.appContext[e],t);return!l&&n?i:l}}function cu(e,t){return e&&(e[t]||e[Qe(t)]||e[Zn(Qe(t))])}function Sv(e,t,s,n){let a;const i=s&&s[n],l=he(e);if(l||Ae(e)){const r=l&&Xs(e);let o=!1,c=!1;r&&(o=!ss(e),c=Ms(e),e=Zl(e)),a=new Array(e.length);for(let u=0,d=e.length;u<d;u++)a[u]=t(o?c?ka(ws(e[u])):ws(e[u]):e[u],u,void 0,i&&i[u])}else if(typeof e=="number"){a=new Array(e);for(let r=0;r<e;r++)a[r]=t(r+1,r,void 0,i&&i[r])}else if(je(e))if(e[Symbol.iterator])a=Array.from(e,(r,o)=>t(r,o,void 0,i&&i[o]));else{const r=Object.keys(e);a=new Array(r.length);for(let o=0,c=r.length;o<c;o++){const u=r[o];a[o]=t(e[u],u,o,i&&i[o])}}else a=[];return s&&(s[n]=a),a}function Tv(e,t){for(let s=0;s<t.length;s++){const n=t[s];if(he(n))for(let a=0;a<n.length;a++)e[n[a].name]=n[a].fn;else n&&(e[n.name]=n.key?(...a)=>{const i=n.fn(...a);return i&&(i.key=n.key),i}:n.fn)}return e}function Cv(e,t,s={},n,a){if(Lt.ce||Lt.parent&&en(Lt.parent)&&Lt.parent.ce){const c=Object.keys(s).length>0;return t!=="default"&&(s.name=t),yi(),_l(At,null,[lt("slot",s,n&&n())],c?-2:64)}let i=e[t];i&&i._c&&(i._d=!1),yi();const l=i&&Zo(i(s)),r=s.key||l&&l.key,o=_l(At,{key:(r&&!Bt(r)?r:`_${t}`)+(!l&&n?"_fb":"")},l||(n?n():[]),l&&e._===1?64:-2);return!a&&o.scopeId&&(o.slotScopeIds=[o.scopeId+"-s"]),i&&i._c&&(i._d=!0),o}function Zo(e){return e.some(t=>rn(t)?!(t.type===ht||t.type===At&&!Zo(t.children)):!0)?e:null}function Ev(e,t){const s={};for(const n in e)s[t&&/[A-Z]/.test(n)?`on:${n}`:ga(n)]=e[n];return s}const Xr=e=>e?_p(e)?Mi(e):Xr(e.parent):null,ai=Pe(Object.create(null),{$:e=>e,$el:e=>e.vnode.el,$data:e=>e.data,$props:e=>e.props,$attrs:e=>e.attrs,$slots:e=>e.slots,$refs:e=>e.refs,$parent:e=>Xr(e.parent),$root:e=>Xr(e.root),$host:e=>e.ce,$emit:e=>e.emit,$options:e=>Jo(e),$forceUpdate:e=>e.f||(e.f=()=>{Uo(e.update)}),$nextTick:e=>e.n||(e.n=St.bind(e.proxy)),$watch:e=>Jm.bind(e)}),Er=(e,t)=>e!==Fe&&!e.__isScriptSetup&&ze(e,t),eo={get({_:e},t){if(t==="__v_skip")return!0;const{ctx:s,setupState:n,data:a,props:i,accessCache:l,type:r,appContext:o}=e;if(t[0]!=="$"){const f=l[t];if(f!==void 0)switch(f){case 1:return n[t];case 2:return a[t];case 4:return s[t];case 3:return i[t]}else{if(Er(n,t))return l[t]=1,n[t];if(a!==Fe&&ze(a,t))return l[t]=2,a[t];if(ze(i,t))return l[t]=3,i[t];if(s!==Fe&&ze(s,t))return l[t]=4,s[t];to&&(l[t]=0)}}const c=ai[t];let u,d;if(c)return t==="$attrs"&&Ft(e.attrs,"get",""),c(e);if((u=r.__cssModules)&&(u=u[t]))return u;if(s!==Fe&&ze(s,t))return l[t]=4,s[t];if(d=o.config.globalProperties,ze(d,t))return d[t]},set({_:e},t,s){const{data:n,setupState:a,ctx:i}=e;return Er(a,t)?(a[t]=s,!0):n!==Fe&&ze(n,t)?(n[t]=s,!0):ze(e.props,t)||t[0]==="$"&&t.slice(1)in e?!1:(i[t]=s,!0)},has({_:{data:e,setupState:t,accessCache:s,ctx:n,appContext:a,props:i,type:l}},r){let o;return!!(s[r]||e!==Fe&&r[0]!=="$"&&ze(e,r)||Er(t,r)||ze(i,r)||ze(n,r)||ze(ai,r)||ze(a.config.globalProperties,r)||(o=l.__cssModules)&&o[r])},defineProperty(e,t,s){return s.get!=null?e._.accessCache[t]=0:ze(s,"value")&&this.set(e,t,s.value,null),Reflect.defineProperty(e,t,s)}},Av=Pe({},eo,{get(e,t){if(t!==Symbol.unscopables)return eo.get(e,t,e)},has(e,t){return t[0]!=="_"&&!Mg(t)}});function Rv(){return null}function Iv(){return null}function Nv(e){}function Lv(e){}function Ov(){return null}function Dv(){}function Mv(e,t){return null}function Pv(){return Kf().slots}function Fv(){return Kf().attrs}function Kf(e){const t=Kt();return t.setupContext||(t.setupContext=Tp(t))}function vi(e){return he(e)?e.reduce((t,s)=>(t[s]=null,t),{}):e}function $v(e,t){const s=vi(e);for(const n in t){if(n.startsWith("__skip"))continue;let a=s[n];a?he(a)||we(a)?a=s[n]={type:a,default:t[n]}:a.default=t[n]:a===null&&(a=s[n]={default:t[n]}),a&&t[`__skip_${n}`]&&(a.skipFactory=!0)}return s}function Uv(e,t){return!e||!t?e||t:he(e)&&he(t)?e.concat(t):Pe({},vi(e),vi(t))}function Bv(e,t){const s={};for(const n in e)t.includes(n)||Object.defineProperty(s,n,{enumerable:!0,get:()=>e[n]});return s}function Hv(e){const t=Kt(),s=zn;let n=e();_i(),s&&xa(!1);const a=()=>{Pa(t),s&&xa(!0)},i=()=>{Kt()!==t&&t.scope.off(),_i(),s&&xa(!1)};return No(n)&&(n=n.catch(l=>{throw a(),Promise.resolve().then(()=>Promise.resolve().then(i)),l})),[n,()=>{a(),Promise.resolve().then(i)}]}let to=!0;function Vv(e){const t=Jo(e),s=e.proxy,n=e.ctx;to=!1,t.beforeCreate&&uu(t.beforeCreate,e,"bc");const{data:a,computed:i,methods:l,watch:r,provide:o,inject:c,created:u,beforeMount:d,mounted:f,beforeUpdate:p,updated:b,activated:g,deactivated:T,beforeDestroy:O,beforeUnmount:y,destroyed:v,unmounted:x,render:w,renderTracked:I,renderTriggered:L,errorCaptured:k,serverPrefetch:A,expose:D,inheritAttrs:B,components:M,directives:S,filters:U}=t;if(c&&jv(c,n,null),l)for(const N in l){const C=l[N];we(C)&&(n[N]=C.bind(s))}if(a){const N=a.call(s,s);je(N)&&(e.data=Sn(N))}if(to=!0,i)for(const N in i){const C=i[N],j=we(C)?C.bind(s,s):we(C.get)?C.get.bind(s,s):Ot,ce=!we(C)&&we(C.set)?C.set.bind(s):Ot,de=J({get:j,set:ce});Object.defineProperty(n,N,{enumerable:!0,configurable:!0,get:()=>de.value,set:se=>de.value=se})}if(r)for(const N in r)Gf(r[N],n,s,N);if(o){const N=we(o)?o.call(s):o;Reflect.ownKeys(N).forEach(C=>{ni(C,N[C])})}u&&uu(u,e,"c");function F(N,C){he(C)?C.forEach(j=>N(j.bind(s))):C&&N(C.bind(s))}if(F(Bf,d),F(He,f),F(Ko,p),F(tr,b),F(zo,g),F(qo,T),F(zf,k),F(jf,I),F(Vf,L),F(sr,y),F(gt,x),F(Hf,A),he(D))if(D.length){const N=e.exposed||(e.exposed={});D.forEach(C=>{Object.defineProperty(N,C,{get:()=>s[C],set:j=>s[C]=j,enumerable:!0})})}else e.exposed||(e.exposed={});w&&e.render===Ot&&(e.render=w),B!=null&&(e.inheritAttrs=B),M&&(e.components=M),S&&(e.directives=S),A&&jo(e)}function jv(e,t,s=Ot){he(e)&&(e=so(e));for(const n in e){const a=e[n];let i;je(a)?"default"in a?i=hs(a.from||n,a.default,!0):i=hs(a.from||n):i=hs(a),yt(i)?Object.defineProperty(t,n,{enumerable:!0,configurable:!0,get:()=>i.value,set:l=>i.value=l}):t[n]=i}}function uu(e,t,s){ls(he(e)?e.map(n=>n.bind(t.proxy)):e.bind(t.proxy),t,s)}function Gf(e,t,s,n){let a=n.includes(".")?Lf(s,n):()=>s[n];if(Ae(e)){const i=t[e];we(i)&&gs(a,i)}else if(we(e))gs(a,e.bind(s));else if(je(e))if(he(e))e.forEach(i=>Gf(i,t,s,n));else{const i=we(e.handler)?e.handler.bind(s):t[e.handler];we(i)&&gs(a,i,e)}}function Jo(e){const t=e.type,{mixins:s,extends:n}=t,{mixins:a,optionsCache:i,config:{optionMergeStrategies:l}}=e.appContext,r=i.get(t);let o;return r?o=r:!a.length&&!s&&!n?o=t:(o={},a.length&&a.forEach(c=>bl(o,c,l,!0)),bl(o,t,l)),je(t)&&i.set(t,o),o}function bl(e,t,s,n=!1){const{mixins:a,extends:i}=t;i&&bl(e,i,s,!0),a&&a.forEach(l=>bl(e,l,s,!0));for(const l in t)if(!(n&&l==="expose")){const r=zv[l]||s&&s[l];e[l]=r?r(e[l],t[l]):t[l]}return e}const zv={data:du,props:fu,emits:fu,methods:Qa,computed:Qa,beforeCreate:Ht,created:Ht,beforeMount:Ht,mounted:Ht,beforeUpdate:Ht,updated:Ht,beforeDestroy:Ht,beforeUnmount:Ht,destroyed:Ht,unmounted:Ht,activated:Ht,deactivated:Ht,errorCaptured:Ht,serverPrefetch:Ht,components:Qa,directives:Qa,watch:Kv,provide:du,inject:qv};function du(e,t){return t?e?function(){return Pe(we(e)?e.call(this,this):e,we(t)?t.call(this,this):t)}:t:e}function qv(e,t){return Qa(so(e),so(t))}function so(e){if(he(e)){const t={};for(let s=0;s<e.length;s++)t[e[s]]=e[s];return t}return e}function Ht(e,t){return e?[...new Set([].concat(e,t))]:t}function Qa(e,t){return e?Pe(Object.create(null),e,t):t}function fu(e,t){return e?he(e)&&he(t)?[...new Set([...e,...t])]:Pe(Object.create(null),vi(e),vi(t??{})):t}function Kv(e,t){if(!e)return t;if(!t)return e;const s=Pe(Object.create(null),e);for(const n in t)s[n]=Ht(e[n],t[n]);return s}function Wf(){return{app:null,config:{isNativeTag:da,performance:!1,globalProperties:{},optionMergeStrategies:{},errorHandler:void 0,warnHandler:void 0,compilerOptions:{}},mixins:[],components:{},directives:{},provides:Object.create(null),optionsCache:new WeakMap,propsCache:new WeakMap,emitsCache:new WeakMap}}let Gv=0;function Wv(e,t){return function(n,a=null){we(n)||(n=Pe({},n)),a!=null&&!je(a)&&(a=null);const i=Wf(),l=new WeakSet,r=[];let o=!1;const c=i.app={_uid:Gv++,_component:n,_props:a,_container:null,_context:i,_instance:null,version:Ep,get config(){return i.config},set config(u){},use(u,...d){return l.has(u)||(u&&we(u.install)?(l.add(u),u.install(c,...d)):we(u)&&(l.add(u),u(c,...d))),c},mixin(u){return i.mixins.includes(u)||i.mixins.push(u),c},component(u,d){return d?(i.components[u]=d,c):i.components[u]},directive(u,d){return d?(i.directives[u]=d,c):i.directives[u]},mount(u,d,f){if(!o){const p=c._ceVNode||lt(n,a);return p.appContext=i,f===!0?f="svg":f===!1&&(f=void 0),d&&t?t(p,u):e(p,u,f),o=!0,c._container=u,u.__vue_app__=c,Mi(p.component)}},onUnmount(u){r.push(u)},unmount(){o&&(ls(r,c._instance,16),e(null,c._container),delete c._container.__vue_app__)},provide(u,d){return i.provides[u]=d,c},runWithContext(u){const d=$n;$n=c;try{return u()}finally{$n=d}}};return c}}let $n=null;function Zv(e,t,s=Fe){const n=Kt(),a=Qe(t),i=es(t),l=Zf(e,a),r=_f((o,c)=>{let u,d=Fe,f;return Nf(()=>{const p=e[a];Et(u,p)&&(u=p,c())}),{get(){return o(),s.get?s.get(u):u},set(p){const b=s.set?s.set(p):p;if(!Et(b,u)&&!(d!==Fe&&Et(p,d)))return;const g=n.vnode.props,T=!!(g&&(t in g||a in g||i in g)&&(`onUpdate:${t}`in g||`onUpdate:${a}`in g||`onUpdate:${i}`in g));T||(u=p,c()),n.emit(`update:${t}`,b),Et(p,d)&&(Et(p,b)&&!Et(b,f)||T&&d!==Fe&&!Et(b,u))&&c(),d=p,f=b}}});return r[Symbol.iterator]=()=>{let o=0;return{next(){return o<2?{value:o++?l||Fe:r,done:!1}:{done:!0}}}},r}const Zf=(e,t)=>t==="modelValue"||t==="model-value"?e.modelModifiers:e[`${t}Modifiers`]||e[`${Qe(t)}Modifiers`]||e[`${es(t)}Modifiers`];function Jv(e,t,...s){if(e.isUnmounted)return;const n=e.vnode.props||Fe;let a=s;const i=t.startsWith("update:"),l=i&&Zf(n,t.slice(7));l&&(l.trim&&(a=s.map(u=>Ae(u)?u.trim():u)),l.number&&(a=s.map(ql)));let r,o=n[r=ga(t)]||n[r=ga(Qe(t))];!o&&i&&(o=n[r=ga(es(t))]),o&&ls(o,e,6,a);const c=n[r+"Once"];if(c){if(!e.emitted)e.emitted={};else if(e.emitted[r])return;e.emitted[r]=!0,ls(c,e,6,a)}}const Yv=new WeakMap;function Jf(e,t,s=!1){const n=s?Yv:t.emitsCache,a=n.get(e);if(a!==void 0)return a;const i=e.emits;let l={},r=!1;if(!we(e)){const o=c=>{const u=Jf(c,t,!0);u&&(r=!0,Pe(l,u))};!s&&t.mixins.length&&t.mixins.forEach(o),e.extends&&o(e.extends),e.mixins&&e.mixins.forEach(o)}return!i&&!r?(je(e)&&n.set(e,null),null):(he(i)?i.forEach(o=>l[o]=null):Pe(l,i),je(e)&&n.set(e,l),l)}function nr(e,t){return!e||!Gn(t)?!1:(t=t.slice(2).replace(/Once$/,""),ze(e,t[0].toLowerCase()+t.slice(1))||ze(e,es(t))||ze(e,t))}function al(e){const{type:t,vnode:s,proxy:n,withProxy:a,propsOptions:[i],slots:l,attrs:r,emit:o,render:c,renderCache:u,props:d,data:f,setupState:p,ctx:b,inheritAttrs:g}=e,T=mi(e);let O,y;try{if(s.shapeFlag&4){const x=a||n,w=x;O=Xt(c.call(w,x,u,d,p,f,b)),y=r}else{const x=t;O=Xt(x.length>1?x(d,{attrs:r,slots:l,emit:o}):x(d,null)),y=t.props?r:Xv(r)}}catch(x){ii.length=0,Jn(x,e,1),O=lt(ht)}let v=O;if(y&&g!==!1){const x=Object.keys(y),{shapeFlag:w}=v;x.length&&w&7&&(i&&x.some(Hl)&&(y=eb(y,i)),v=Ps(v,y,!1,!0))}return s.dirs&&(v=Ps(v,null,!1,!0),v.dirs=v.dirs?v.dirs.concat(s.dirs):s.dirs),s.transition&&ln(v,s.transition),O=v,mi(T),O}function Qv(e,t=!0){let s;for(let n=0;n<e.length;n++){const a=e[n];if(rn(a)){if(a.type!==ht||a.children==="v-if"){if(s)return;s=a}}else return}return s}const Xv=e=>{let t;for(const s in e)(s==="class"||s==="style"||Gn(s))&&((t||(t={}))[s]=e[s]);return t},eb=(e,t)=>{const s={};for(const n in e)(!Hl(n)||!(n.slice(9)in t))&&(s[n]=e[n]);return s};function tb(e,t,s){const{props:n,children:a,component:i}=e,{props:l,children:r,patchFlag:o}=t,c=i.emitsOptions;if(t.dirs||t.transition)return!0;if(s&&o>=0){if(o&1024)return!0;if(o&16)return n?pu(n,l,c):!!l;if(o&8){const u=t.dynamicProps;for(let d=0;d<u.length;d++){const f=u[d];if(Yf(l,n,f)&&!nr(c,f))return!0}}}else return(a||r)&&(!r||!r.$stable)?!0:n===l?!1:n?l?pu(n,l,c):!0:!!l;return!1}function pu(e,t,s){const n=Object.keys(t);if(n.length!==Object.keys(e).length)return!0;for(let a=0;a<n.length;a++){const i=n[a];if(Yf(t,e,i)&&!nr(s,i))return!0}return!1}function Yf(e,t,s){const n=e[s],a=t[s];return s==="style"&&je(n)&&je(a)?!sn(n,a):n!==a}function ar({vnode:e,parent:t,suspense:s},n){for(;t;){const a=t.subTree;if(a.suspense&&a.suspense.activeBranch===e&&(a.suspense.vnode.el=a.el=n,e=a),a===e)(e=t.vnode).el=n,t=t.parent;else break}s&&s.activeBranch===e&&(s.vnode.el=n)}const Qf={},Xf=()=>Object.create(Qf),ep=e=>Object.getPrototypeOf(e)===Qf;function sb(e,t,s,n=!1){const a={},i=Xf();e.propsDefaults=Object.create(null),tp(e,t,a,i);for(const l in e.propsOptions[0])l in a||(a[l]=void 0);s?e.props=n?a:Po(a):e.type.props?e.props=a:e.props=i,e.attrs=i}function nb(e,t,s,n){const{props:a,attrs:i,vnode:{patchFlag:l}}=e,r=Be(a),[o]=e.propsOptions;let c=!1;if((n||l>0)&&!(l&16)){if(l&8){const u=e.vnode.dynamicProps;for(let d=0;d<u.length;d++){let f=u[d];if(nr(e.emitsOptions,f))continue;const p=t[f];if(o)if(ze(i,f))p!==i[f]&&(i[f]=p,c=!0);else{const b=Qe(f);a[b]=no(o,r,b,p,e,!1)}else p!==i[f]&&(i[f]=p,c=!0)}}}else{tp(e,t,a,i)&&(c=!0);let u;for(const d in r)(!t||!ze(t,d)&&((u=es(d))===d||!ze(t,u)))&&(o?s&&(s[d]!==void 0||s[u]!==void 0)&&(a[d]=no(o,r,d,void 0,e,!0)):delete a[d]);if(i!==r)for(const d in i)(!t||!ze(t,d))&&(delete i[d],c=!0)}c&&Gs(e.attrs,"set","")}function tp(e,t,s,n){const[a,i]=e.propsOptions;let l=!1,r;if(t)for(let o in t){if(Qs(o))continue;const c=t[o];let u;a&&ze(a,u=Qe(o))?!i||!i.includes(u)?s[u]=c:(r||(r={}))[u]=c:nr(e.emitsOptions,o)||(!(o in n)||c!==n[o])&&(n[o]=c,l=!0)}if(i){const o=Be(s),c=r||Fe;for(let u=0;u<i.length;u++){const d=i[u];s[d]=no(a,o,d,c[d],e,!ze(c,d))}}return l}function no(e,t,s,n,a,i){const l=e[s];if(l!=null){const r=ze(l,"default");if(r&&n===void 0){const o=l.default;if(l.type!==Function&&!l.skipFactory&&we(o)){const{propsDefaults:c}=a;if(s in c)n=c[s];else{const u=Pa(a);n=c[s]=o.call(null,t),u()}}else n=o;a.ce&&a.ce._setProp(s,n)}l[0]&&(i&&!r?n=!1:l[1]&&(n===""||n===es(s))&&(n=!0))}return n}const ab=new WeakMap;function sp(e,t,s=!1){const n=s?ab:t.propsCache,a=n.get(e);if(a)return a;const i=e.props,l={},r=[];let o=!1;if(!we(e)){const u=d=>{o=!0;const[f,p]=sp(d,t,!0);Pe(l,f),p&&r.push(...p)};!s&&t.mixins.length&&t.mixins.forEach(u),e.extends&&u(e.extends),e.mixins&&e.mixins.forEach(u)}if(!i&&!o)return je(e)&&n.set(e,pa),pa;if(he(i))for(let u=0;u<i.length;u++){const d=Qe(i[u]);hu(d)&&(l[d]=Fe)}else if(i)for(const u in i){const d=Qe(u);if(hu(d)){const f=i[u],p=l[d]=he(f)||we(f)?{type:f}:Pe({},f),b=p.type;let g=!1,T=!0;if(he(b))for(let O=0;O<b.length;++O){const y=b[O],v=we(y)&&y.name;if(v==="Boolean"){g=!0;break}else v==="String"&&(T=!1)}else g=we(b)&&b.name==="Boolean";p[0]=g,p[1]=T,(g||ze(p,"default"))&&r.push(d)}}const c=[l,r];return je(e)&&n.set(e,c),c}function hu(e){return e[0]!=="$"&&!Qs(e)}const Yo=e=>e==="_"||e==="_ctx"||e==="$stable",Qo=e=>he(e)?e.map(Xt):[Xt(e)],ib=(e,t,s)=>{if(t._n)return t;const n=Bo((...a)=>Qo(t(...a)),s);return n._c=!1,n},np=(e,t,s)=>{const n=e._ctx;for(const a in e){if(Yo(a))continue;const i=e[a];if(we(i))t[a]=ib(a,i,n);else if(i!=null){const l=Qo(i);t[a]=()=>l}}},ap=(e,t)=>{const s=Qo(t);e.slots.default=()=>s},ip=(e,t,s)=>{for(const n in t)(s||!Yo(n))&&(e[n]=t[n])},lb=(e,t,s)=>{const n=e.slots=Xf();if(e.vnode.shapeFlag&32){const a=t._;a?(ip(n,t,s),s&&Jd(n,"_",a,!0)):np(t,n)}else t&&ap(e,t)},rb=(e,t,s)=>{const{vnode:n,slots:a}=e;let i=!0,l=Fe;if(n.shapeFlag&32){const r=t._;r?s&&r===1?i=!1:ip(a,t,s):(i=!t.$stable,np(t,a)),l=t}else t&&(ap(e,t),l={default:1});if(i)for(const r in a)!Yo(r)&&l[r]==null&&delete a[r]},vt=pp;function lp(e){return op(e)}function rp(e){return op(e,lv)}function op(e,t){const s=Kl();s.__VUE__=!0;const{insert:n,remove:a,patchProp:i,createElement:l,createText:r,createComment:o,setText:c,setElementText:u,parentNode:d,nextSibling:f,setScopeId:p=Ot,insertStaticContent:b}=e,g=(m,E,P,Z=null,R=null,$=null,Y=void 0,ee=null,te=!!E.dynamicChildren)=>{if(m===E)return;m&&!xs(m,E)&&(Z=z(m),se(m,R,$,!0),m=null),E.patchFlag===-2&&(te=!1,E.dynamicChildren=null);const{type:X,ref:be,shapeFlag:le}=E;switch(X){case _n:T(m,E,P,Z);break;case ht:O(m,E,P,Z);break;case Un:m==null&&y(E,P,Z,Y);break;case At:M(m,E,P,Z,R,$,Y,ee,te);break;default:le&1?w(m,E,P,Z,R,$,Y,ee,te):le&6?S(m,E,P,Z,R,$,Y,ee,te):(le&64||le&128)&&X.process(m,E,P,Z,R,$,Y,ee,te,me)}be!=null&&R?ba(be,m&&m.ref,$,E||m,!E):be==null&&m&&m.ref!=null&&ba(m.ref,null,$,m,!0)},T=(m,E,P,Z)=>{if(m==null)n(E.el=r(E.children),P,Z);else{const R=E.el=m.el;E.children!==m.children&&c(R,E.children)}},O=(m,E,P,Z)=>{m==null?n(E.el=o(E.children||""),P,Z):E.el=m.el},y=(m,E,P,Z)=>{[m.el,m.anchor]=b(m.children,E,P,Z,m.el,m.anchor)},v=({el:m,anchor:E},P,Z)=>{let R;for(;m&&m!==E;)R=f(m),n(m,P,Z),m=R;n(E,P,Z)},x=({el:m,anchor:E})=>{let P;for(;m&&m!==E;)P=f(m),a(m),m=P;a(E)},w=(m,E,P,Z,R,$,Y,ee,te)=>{if(E.type==="svg"?Y="svg":E.type==="math"&&(Y="mathml"),m==null)I(E,P,Z,R,$,Y,ee,te);else{const X=m.el&&m.el._isVueCE?m.el:null;try{X&&X._beginPatch(),A(m,E,R,$,Y,ee,te)}finally{X&&X._endPatch()}}},I=(m,E,P,Z,R,$,Y,ee)=>{let te,X;const{props:be,shapeFlag:le,transition:ge,dirs:xe}=m;if(te=m.el=l(m.type,$,be&&be.is,be),le&8?u(te,m.children):le&16&&k(m.children,te,null,Z,R,Ar(m,$),Y,ee),xe&&Ls(m,null,Z,"created"),L(te,m,m.scopeId,Y,Z),be){for(const Ee in be)Ee!=="value"&&!Qs(Ee)&&i(te,Ee,null,be[Ee],$,Z);"value"in be&&i(te,"value",null,be.value,$),(X=be.onVnodeBeforeMount)&&Yt(X,Z,m)}xe&&Ls(m,null,Z,"beforeMount");const ke=cp(R,ge);ke&&ge.beforeEnter(te),n(te,E,P),((X=be&&be.onVnodeMounted)||ke||xe)&&vt(()=>{try{X&&Yt(X,Z,m),ke&&ge.enter(te),xe&&Ls(m,null,Z,"mounted")}finally{}},R)},L=(m,E,P,Z,R)=>{if(P&&p(m,P),Z)for(let $=0;$<Z.length;$++)p(m,Z[$]);if(R){let $=R.subTree;if(E===$||xl($.type)&&($.ssContent===E||$.ssFallback===E)){const Y=R.vnode;L(m,Y,Y.scopeId,Y.slotScopeIds,R.parent)}}},k=(m,E,P,Z,R,$,Y,ee,te=0)=>{for(let X=te;X<m.length;X++){const be=m[X]=ee?qs(m[X]):Xt(m[X]);g(null,be,E,P,Z,R,$,Y,ee)}},A=(m,E,P,Z,R,$,Y)=>{const ee=E.el=m.el;let{patchFlag:te,dynamicChildren:X,dirs:be}=E;te|=m.patchFlag&16;const le=m.props||Fe,ge=E.props||Fe;let xe;if(P&&Rn(P,!1),(xe=ge.onVnodeBeforeUpdate)&&Yt(xe,P,E,m),be&&Ls(E,m,P,"beforeUpdate"),P&&Rn(P,!0),(le.innerHTML&&ge.innerHTML==null||le.textContent&&ge.textContent==null)&&u(ee,""),X?D(m.dynamicChildren,X,ee,P,Z,Ar(E,R),$):Y||C(m,E,ee,null,P,Z,Ar(E,R),$,!1),te>0){if(te&16)B(ee,le,ge,P,R);else if(te&2&&le.class!==ge.class&&i(ee,"class",null,ge.class,R),te&4&&i(ee,"style",le.style,ge.style,R),te&8){const ke=E.dynamicProps;for(let Ee=0;Ee<ke.length;Ee++){const H=ke[Ee],re=le[H],ye=ge[H];(ye!==re||H==="value")&&i(ee,H,re,ye,R,P)}}te&1&&m.children!==E.children&&u(ee,E.children)}else!Y&&X==null&&B(ee,le,ge,P,R);((xe=ge.onVnodeUpdated)||be)&&vt(()=>{xe&&Yt(xe,P,E,m),be&&Ls(E,m,P,"updated")},Z)},D=(m,E,P,Z,R,$,Y)=>{for(let ee=0;ee<E.length;ee++){const te=m[ee],X=E[ee],be=te.el&&(te.type===At||!xs(te,X)||te.shapeFlag&198)?d(te.el):P;g(te,X,be,null,Z,R,$,Y,!0)}},B=(m,E,P,Z,R)=>{if(E!==P){if(E!==Fe)for(const $ in E)!Qs($)&&!($ in P)&&i(m,$,E[$],null,R,Z);for(const $ in P){if(Qs($))continue;const Y=P[$],ee=E[$];Y!==ee&&$!=="value"&&i(m,$,ee,Y,R,Z)}"value"in P&&i(m,"value",E.value,P.value,R)}},M=(m,E,P,Z,R,$,Y,ee,te)=>{const X=E.el=m?m.el:r(""),be=E.anchor=m?m.anchor:r("");let{patchFlag:le,dynamicChildren:ge,slotScopeIds:xe}=E;xe&&(ee=ee?ee.concat(xe):xe),m==null?(n(X,P,Z),n(be,P,Z),k(E.children||[],P,be,R,$,Y,ee,te)):le>0&&le&64&&ge&&m.dynamicChildren&&m.dynamicChildren.length===ge.length?(D(m.dynamicChildren,ge,P,R,$,Y,ee),(E.key!=null||R&&E===R.subTree)&&Xo(m,E,!0)):C(m,E,P,be,R,$,Y,ee,te)},S=(m,E,P,Z,R,$,Y,ee,te)=>{E.slotScopeIds=ee,m==null?E.shapeFlag&512?R.ctx.activate(E,P,Z,Y,te):U(E,P,Z,R,$,Y,te):W(m,E,te)},U=(m,E,P,Z,R,$,Y)=>{const ee=m.component=xp(m,Z,R);if(Di(m)&&(ee.ctx.renderer=me),kp(ee,!1,Y),ee.asyncDep){if(R&&R.registerDep(ee,F,Y),!m.el){const te=ee.subTree=lt(ht);O(null,te,E,P),m.placeholder=te.el}}else F(ee,m,E,P,R,$,Y)},W=(m,E,P)=>{const Z=E.component=m.component;if(tb(m,E,P))if(Z.asyncDep&&!Z.asyncResolved){N(Z,E,P);return}else Z.next=E,Z.update();else E.el=m.el,Z.vnode=E},F=(m,E,P,Z,R,$,Y)=>{const ee=()=>{if(m.isMounted){let{next:le,bu:ge,u:xe,parent:ke,vnode:Ee}=m;{const Ue=up(m);if(Ue){le&&(le.el=Ee.el,N(m,le,Y)),Ue.asyncDep.then(()=>{vt(()=>{m.isUnmounted||X()},R)});return}}let H=le,re;Rn(m,!1),le?(le.el=Ee.el,N(m,le,Y)):le=Ee,ge&&ma(ge),(re=le.props&&le.props.onVnodeBeforeUpdate)&&Yt(re,ke,le,Ee),Rn(m,!0);const ye=al(m),Me=m.subTree;m.subTree=ye,g(Me,ye,d(Me.el),z(Me),m,R,$),le.el=ye.el,H===null&&ar(m,ye.el),xe&&vt(xe,R),(re=le.props&&le.props.onVnodeUpdated)&&vt(()=>Yt(re,ke,le,Ee),R)}else{let le;const{el:ge,props:xe}=E,{bm:ke,m:Ee,parent:H,root:re,type:ye}=m,Me=en(E);if(Rn(m,!1),ke&&ma(ke),!Me&&(le=xe&&xe.onVnodeBeforeMount)&&Yt(le,H,E),Rn(m,!0),ge&&Le){const Ue=()=>{m.subTree=al(m),Le(ge,m.subTree,m,R,null)};Me&&ye.__asyncHydrate?ye.__asyncHydrate(ge,m,Ue):Ue()}else{re.ce&&re.ce._hasShadowRoot()&&re.ce._injectChildStyle(ye,m.parent?m.parent.type:void 0);const Ue=m.subTree=al(m);g(null,Ue,P,Z,m,R,$),E.el=Ue.el}if(Ee&&vt(Ee,R),!Me&&(le=xe&&xe.onVnodeMounted)){const Ue=E;vt(()=>Yt(le,H,Ue),R)}(E.shapeFlag&256||H&&en(H.vnode)&&H.vnode.shapeFlag&256)&&m.a&&vt(m.a,R),m.isMounted=!0,E=P=Z=null}};m.scope.on();const te=m.effect=new di(ee);m.scope.off();const X=m.update=te.run.bind(te),be=m.job=te.runIfDirty.bind(te);be.i=m,be.id=m.uid,te.scheduler=()=>Uo(be),Rn(m,!0),X()},N=(m,E,P)=>{E.component=m;const Z=m.vnode.props;m.vnode=E,m.next=null,nb(m,E.props,Z,P),rb(m,E.children,P),nn(),tu(m),an()},C=(m,E,P,Z,R,$,Y,ee,te=!1)=>{const X=m&&m.children,be=m?m.shapeFlag:0,le=E.children,{patchFlag:ge,shapeFlag:xe}=E;if(ge>0){if(ge&128){ce(X,le,P,Z,R,$,Y,ee,te);return}else if(ge&256){j(X,le,P,Z,R,$,Y,ee,te);return}}xe&8?(be&16&&Ie(X,R,$),le!==X&&u(P,le)):be&16?xe&16?ce(X,le,P,Z,R,$,Y,ee,te):Ie(X,R,$,!0):(be&8&&u(P,""),xe&16&&k(le,P,Z,R,$,Y,ee,te))},j=(m,E,P,Z,R,$,Y,ee,te)=>{m=m||pa,E=E||pa;const X=m.length,be=E.length,le=Math.min(X,be);let ge;for(ge=0;ge<le;ge++){const xe=E[ge]=te?qs(E[ge]):Xt(E[ge]);g(m[ge],xe,P,null,R,$,Y,ee,te)}X>be?Ie(m,R,$,!0,!1,le):k(E,P,Z,R,$,Y,ee,te,le)},ce=(m,E,P,Z,R,$,Y,ee,te)=>{let X=0;const be=E.length;let le=m.length-1,ge=be-1;for(;X<=le&&X<=ge;){const xe=m[X],ke=E[X]=te?qs(E[X]):Xt(E[X]);if(xs(xe,ke))g(xe,ke,P,null,R,$,Y,ee,te);else break;X++}for(;X<=le&&X<=ge;){const xe=m[le],ke=E[ge]=te?qs(E[ge]):Xt(E[ge]);if(xs(xe,ke))g(xe,ke,P,null,R,$,Y,ee,te);else break;le--,ge--}if(X>le){if(X<=ge){const xe=ge+1,ke=xe<be?E[xe].el:Z;for(;X<=ge;)g(null,E[X]=te?qs(E[X]):Xt(E[X]),P,ke,R,$,Y,ee,te),X++}}else if(X>ge)for(;X<=le;)se(m[X],R,$,!0),X++;else{const xe=X,ke=X,Ee=new Map;for(X=ke;X<=ge;X++){const Ye=E[X]=te?qs(E[X]):Xt(E[X]);Ye.key!=null&&Ee.set(Ye.key,X)}let H,re=0;const ye=ge-ke+1;let Me=!1,Ue=0;const Je=new Array(ye);for(X=0;X<ye;X++)Je[X]=0;for(X=xe;X<=le;X++){const Ye=m[X];if(re>=ye){se(Ye,R,$,!0);continue}let Ke;if(Ye.key!=null)Ke=Ee.get(Ye.key);else for(H=ke;H<=ge;H++)if(Je[H-ke]===0&&xs(Ye,E[H])){Ke=H;break}Ke===void 0?se(Ye,R,$,!0):(Je[Ke-ke]=X+1,Ke>=Ue?Ue=Ke:Me=!0,g(Ye,E[Ke],P,null,R,$,Y,ee,te),re++)}const dt=Me?ob(Je):pa;for(H=dt.length-1,X=ye-1;X>=0;X--){const Ye=ke+X,Ke=E[Ye],Wt=E[Ye+1],bs=Ye+1<be?Wt.el||dp(Wt):Z;Je[X]===0?g(null,Ke,P,bs,R,$,Y,ee,te):Me&&(H<0||X!==dt[H]?de(Ke,P,bs,2):H--)}}},de=(m,E,P,Z,R=null)=>{const{el:$,type:Y,transition:ee,children:te,shapeFlag:X}=m;if(X&6){de(m.component.subTree,E,P,Z);return}if(X&128){m.suspense.move(E,P,Z);return}if(X&64){Y.move(m,E,P,me);return}if(Y===At){n($,E,P);for(let le=0;le<te.length;le++)de(te[le],E,P,Z);n(m.anchor,E,P);return}if(Y===Un){v(m,E,P);return}if(Z!==2&&X&1&&ee)if(Z===0)ee.persisted&&!$[ds]?n($,E,P):(ee.beforeEnter($),n($,E,P),vt(()=>ee.enter($),R));else{const{leave:le,delayLeave:ge,afterLeave:xe}=ee,ke=()=>{m.ctx.isUnmounted?a($):n($,E,P)},Ee=()=>{const H=$._isLeaving||!!$[ds];$._isLeaving&&$[ds](!0),ee.persisted&&!H?ke():le($,()=>{ke(),xe&&xe()})};ge?ge($,ke,Ee):Ee()}else n($,E,P)},se=(m,E,P,Z=!1,R=!1)=>{const{type:$,props:Y,ref:ee,children:te,dynamicChildren:X,shapeFlag:be,patchFlag:le,dirs:ge,cacheIndex:xe,memo:ke}=m;if(le===-2&&(R=!1),ee!=null&&(nn(),ba(ee,null,P,m,!0),an()),xe!=null&&(E.renderCache[xe]=void 0),be&256){E.ctx.deactivate(m);return}const Ee=be&1&&ge,H=!en(m);let re;if(H&&(re=Y&&Y.onVnodeBeforeUnmount)&&Yt(re,E,m),be&6)ue(m.component,P,Z);else{if(be&128){m.suspense.unmount(P,Z);return}Ee&&Ls(m,null,E,"beforeUnmount"),be&64?m.type.remove(m,E,P,me,Z):X&&!X.hasOnce&&($!==At||le>0&&le&64)?Ie(X,E,P,!1,!0):($===At&&le&384||!R&&be&16)&&Ie(te,E,P),Z&&fe(m)}const ye=ke!=null&&xe==null;(H&&(re=Y&&Y.onVnodeUnmounted)||Ee||ye)&&vt(()=>{re&&Yt(re,E,m),Ee&&Ls(m,null,E,"unmounted"),ye&&(m.el=null)},P)},fe=m=>{const{type:E,el:P,anchor:Z,transition:R}=m;if(E===At){Q(P,Z);return}if(E===Un){x(m);return}const $=()=>{a(P),R&&!R.persisted&&R.afterLeave&&R.afterLeave()};if(m.shapeFlag&1&&R&&!R.persisted){const{leave:Y,delayLeave:ee}=R,te=()=>Y(P,$);ee?ee(m.el,$,te):te()}else $()},Q=(m,E)=>{let P;for(;m!==E;)P=f(m),a(m),m=P;a(E)},ue=(m,E,P)=>{const{bum:Z,scope:R,job:$,subTree:Y,um:ee,m:te,a:X}=m;yl(te),yl(X),Z&&ma(Z),R.stop(),$&&($.flags|=8,se(Y,m,E,P)),ee&&vt(ee,E),vt(()=>{m.isUnmounted=!0},E)},Ie=(m,E,P,Z=!1,R=!1,$=0)=>{for(let Y=$;Y<m.length;Y++)se(m[Y],E,P,Z,R)},z=m=>{if(m.shapeFlag&6)return z(m.component.subTree);if(m.shapeFlag&128)return m.suspense.next();const E=f(m.anchor||m.el),P=E&&E[Of];return P?f(P):E};let oe=!1;const ie=(m,E,P)=>{let Z;m==null?E._vnode&&(se(E._vnode,null,null,!0),Z=E._vnode.component):g(E._vnode||null,m,E,null,null,null,P),E._vnode=m,oe||(oe=!0,tu(Z),ml(),oe=!1)},me={p:g,um:se,m:de,r:fe,mt:U,mc:k,pc:C,pbc:D,n:z,o:e};let pe,Le;return t&&([pe,Le]=t(me)),{render:ie,hydrate:pe,createApp:Wv(ie,pe)}}function Ar({type:e,props:t},s){return s==="svg"&&e==="foreignObject"||s==="mathml"&&e==="annotation-xml"&&t&&t.encoding&&t.encoding.includes("html")?void 0:s}function Rn({effect:e,job:t},s){s?(e.flags|=32,t.flags|=4):(e.flags&=-33,t.flags&=-5)}function cp(e,t){return(!e||e&&!e.pendingBranch)&&t&&!t.persisted}function Xo(e,t,s=!1){const n=e.children,a=t.children;if(he(n)&&he(a))for(let i=0;i<n.length;i++){const l=n[i];let r=a[i];r.shapeFlag&1&&!r.dynamicChildren&&((r.patchFlag<=0||r.patchFlag===32)&&(r=a[i]=qs(a[i]),r.el=l.el),!s&&r.patchFlag!==-2&&Xo(l,r)),r.type===_n&&(r.patchFlag===-1&&(r=a[i]=qs(r)),r.el=l.el),r.type===ht&&!r.el&&(r.el=l.el)}}function ob(e){const t=e.slice(),s=[0];let n,a,i,l,r;const o=e.length;for(n=0;n<o;n++){const c=e[n];if(c!==0){if(a=s[s.length-1],e[a]<c){t[n]=a,s.push(n);continue}for(i=0,l=s.length-1;i<l;)r=i+l>>1,e[s[r]]<c?i=r+1:l=r;c<e[s[i]]&&(i>0&&(t[n]=s[i-1]),s[i]=n)}}for(i=s.length,l=s[i-1];i-- >0;)s[i]=l,l=t[l];return s}function up(e){const t=e.subTree.component;if(t)return t.asyncDep&&!t.asyncResolved?t:up(t)}function yl(e){if(e)for(let t=0;t<e.length;t++)e[t].flags|=8}function dp(e){if(e.placeholder)return e.placeholder;const t=e.component;return t?dp(t.subTree):null}const xl=e=>e.__isSuspense;let ao=0;const cb={name:"Suspense",__isSuspense:!0,process(e,t,s,n,a,i,l,r,o,c){if(e==null)db(t,s,n,a,i,l,r,o,c);else{if(i&&i.deps>0&&!e.suspense.isInFallback){t.suspense=e.suspense,t.suspense.vnode=t,t.el=e.el;return}fb(e,t,s,n,a,l,r,o,c)}},hydrate:pb,normalize:hb},ub=cb;function bi(e,t){const s=e.props&&e.props[t];we(s)&&s()}function db(e,t,s,n,a,i,l,r,o){const{p:c,o:{createElement:u}}=o,d=u("div"),f=e.suspense=fp(e,a,n,t,d,s,i,l,r,o);c(null,f.pendingBranch=e.ssContent,d,null,n,f,i,l),f.deps>0?(bi(e,"onPending"),bi(e,"onFallback"),c(null,e.ssFallback,t,s,n,null,i,l),ya(f,e.ssFallback)):f.resolve(!1,!0)}function fb(e,t,s,n,a,i,l,r,{p:o,um:c,o:{createElement:u}}){const d=t.suspense=e.suspense;d.vnode=t,t.el=e.el;const f=t.ssContent,p=t.ssFallback,{activeBranch:b,pendingBranch:g,isInFallback:T,isHydrating:O}=d;if(g)d.pendingBranch=f,xs(g,f)?(o(g,f,d.hiddenContainer,null,a,d,i,l,r),d.deps<=0?d.resolve():T&&(O||(o(b,p,s,n,a,null,i,l,r),ya(d,p)))):(d.pendingId=ao++,O?(d.isHydrating=!1,d.activeBranch=g):c(g,a,d),d.deps=0,d.effects.length=0,d.hiddenContainer=u("div"),T?(o(null,f,d.hiddenContainer,null,a,d,i,l,r),d.deps<=0?d.resolve():(o(b,p,s,n,a,null,i,l,r),ya(d,p))):b&&xs(b,f)?(o(b,f,s,n,a,d,i,l,r),d.resolve(!0)):(o(null,f,d.hiddenContainer,null,a,d,i,l,r),d.deps<=0&&d.resolve()));else if(b&&xs(b,f))o(b,f,s,n,a,d,i,l,r),ya(d,f);else if(bi(t,"onPending"),d.pendingBranch=f,f.shapeFlag&512?d.pendingId=f.component.suspenseId:d.pendingId=ao++,o(null,f,d.hiddenContainer,null,a,d,i,l,r),d.deps<=0)d.resolve();else{const{timeout:y,pendingId:v}=d;y>0?setTimeout(()=>{d.pendingId===v&&d.fallback(p)},y):y===0&&d.fallback(p)}}function fp(e,t,s,n,a,i,l,r,o,c,u=!1){const{p:d,m:f,um:p,n:b,o:{parentNode:g,remove:T}}=c;let O;const y=gb(e);y&&t&&t.pendingBranch&&(O=t.pendingId,t.deps++);const v=e.props?dl(e.props.timeout):void 0,x=i,w={vnode:e,parent:t,parentComponent:s,namespace:l,container:n,hiddenContainer:a,deps:0,pendingId:ao++,timeout:typeof v=="number"?v:-1,activeBranch:null,isFallbackMountPending:!1,pendingBranch:null,isInFallback:!u,isHydrating:u,isUnmounted:!1,effects:[],resolve(I=!1,L=!1){const{vnode:k,activeBranch:A,pendingBranch:D,pendingId:B,effects:M,parentComponent:S,container:U,isInFallback:W}=w;let F=!1;if(w.isHydrating)w.isHydrating=!1;else if(!I){F=A&&D.transition&&D.transition.mode==="out-in";let j=!1;F&&(A.transition.afterLeave=()=>{B===w.pendingId&&(f(D,U,i===x&&!j?b(A):i,0),hi(M),W&&k.ssFallback&&(k.ssFallback.el=null))}),A&&!w.isFallbackMountPending&&(g(A.el)===U&&(i=b(A),j=!0),p(A,S,w,!0),!F&&W&&k.ssFallback&&vt(()=>k.ssFallback.el=null,w)),F||f(D,U,i,0)}w.isFallbackMountPending=!1,ya(w,D),w.pendingBranch=null,w.isInFallback=!1;let N=w.parent,C=!1;for(;N;){if(N.pendingBranch){N.effects.push(...M),C=!0;break}N=N.parent}!C&&!F&&hi(M),w.effects=[],y&&t&&t.pendingBranch&&O===t.pendingId&&(t.deps--,t.deps===0&&!L&&t.resolve()),bi(k,"onResolve")},fallback(I){if(!w.pendingBranch)return;const{vnode:L,activeBranch:k,parentComponent:A,container:D,namespace:B}=w;bi(L,"onFallback");const M=b(k),S=()=>{w.isFallbackMountPending=!1,w.isInFallback&&(d(null,I,D,M,A,null,B,r,o),ya(w,I))},U=I.transition&&I.transition.mode==="out-in";U&&(w.isFallbackMountPending=!0,k.transition.afterLeave=S),w.isInFallback=!0,p(k,A,null,!0),U||S()},move(I,L,k){w.activeBranch&&f(w.activeBranch,I,L,k),w.container=I},next(){return w.activeBranch&&b(w.activeBranch)},registerDep(I,L,k){const A=!!w.pendingBranch;A&&w.deps++;const D=I.vnode.el;I.asyncDep.catch(B=>{Jn(B,I,0)}).then(B=>{if(I.isUnmounted||w.isUnmounted||w.pendingId!==I.suspenseId)return;_i(),I.asyncResolved=!0;const{vnode:M}=I;io(I,B,!1),D&&(M.el=D);const S=!D&&I.subTree.el;L(I,M,g(D||I.subTree.el),D?null:b(I.subTree),w,l,k),S&&(M.placeholder=null,T(S)),ar(I,M.el),A&&--w.deps===0&&w.resolve()})},unmount(I,L){w.isUnmounted=!0,w.activeBranch&&p(w.activeBranch,s,I,L),w.pendingBranch&&p(w.pendingBranch,s,I,L)}};return w}function pb(e,t,s,n,a,i,l,r,o){const c=t.suspense=fp(t,n,s,e.parentNode,document.createElement("div"),null,a,i,l,r,!0),u=o(e,c.pendingBranch=t.ssContent,s,c,i,l);return c.deps===0&&c.resolve(!1,!0),u}function hb(e){const{shapeFlag:t,children:s}=e,n=t&32;e.ssContent=gu(n?s.default:s),e.ssFallback=n?gu(s.fallback):lt(ht)}function gu(e){let t;if(we(e)){const s=jn&&e._c;s&&(e._d=!1,yi()),e=e(),s&&(e._d=!0,t=$t,hp())}return he(e)&&(e=Qv(e)),e=Xt(e),t&&!e.dynamicChildren&&(e.dynamicChildren=t.filter(s=>s!==e)),e}function pp(e,t){t&&t.pendingBranch?he(e)?t.effects.push(...e):t.effects.push(e):hi(e)}function ya(e,t){e.activeBranch=t;const{vnode:s,parentComponent:n}=e;let a=t.el;for(;!a&&t.component;)t=t.component.subTree,a=t.el;s.el=a,n&&n.subTree===s&&(n.vnode.el=a,ar(n,a))}function gb(e){const t=e.props&&e.props.suspensible;return t!=null&&t!==!1}const At=Symbol.for("v-fgt"),_n=Symbol.for("v-txt"),ht=Symbol.for("v-cmt"),Un=Symbol.for("v-stc"),ii=[];let $t=null;function yi(e=!1){ii.push($t=e?null:[])}function hp(){ii.pop(),$t=ii[ii.length-1]||null}let jn=1;function xi(e,t=!1){jn+=e,e<0&&$t&&t&&($t.hasOnce=!0)}function gp(e){return e.dynamicChildren=jn>0?$t||pa:null,hp(),jn>0&&$t&&$t.push(e),e}function mb(e,t,s,n,a,i){return gp(ec(e,t,s,n,a,i,!0))}function _l(e,t,s,n,a){return gp(lt(e,t,s,n,a,!0))}function rn(e){return e?e.__v_isVNode===!0:!1}function xs(e,t){return e.type===t.type&&e.key===t.key}function vb(e){}const mp=({key:e})=>e??null,il=({ref:e,ref_key:t,ref_for:s})=>(typeof e=="number"&&(e=""+e),e!=null?Ae(e)||yt(e)||we(e)?{i:Lt,r:e,k:t,f:!!s}:e:null);function ec(e,t=null,s=null,n=0,a=null,i=e===At?0:1,l=!1,r=!1){const o={__v_isVNode:!0,__v_skip:!0,type:e,props:t,key:t&&mp(t),ref:t&&il(t),scopeId:Ql,slotScopeIds:null,children:s,component:null,suspense:null,ssContent:null,ssFallback:null,dirs:null,transition:null,el:null,anchor:null,target:null,targetStart:null,targetAnchor:null,staticCount:0,shapeFlag:i,patchFlag:n,dynamicProps:a,dynamicChildren:null,appContext:null,ctx:Lt};return r?(sc(o,s),i&128&&e.normalize(o)):s&&(o.shapeFlag|=Ae(s)?8:16),jn>0&&!l&&$t&&(o.patchFlag>0||i&6)&&o.patchFlag!==32&&$t.push(o),o}const lt=bb;function bb(e,t=null,s=null,n=0,a=null,i=!1){if((!e||e===qf)&&(e=ht),rn(e)){const r=Ps(e,t,!0);return s&&sc(r,s),jn>0&&!i&&$t&&(r.shapeFlag&6?$t[$t.indexOf(e)]=r:$t.push(r)),r.patchFlag=-2,r}if(Tb(e)&&(e=e.__vccOpts),t){t=vp(t);let{class:r,style:o}=t;r&&!Ae(r)&&(t.class=Ii(r)),je(o)&&(Ni(o)&&!he(o)&&(o=Pe({},o)),t.style=Ri(o))}const l=Ae(e)?1:xl(e)?128:Df(e)?64:je(e)?4:we(e)?2:0;return ec(e,t,s,n,a,l,i,!0)}function vp(e){return e?Ni(e)||ep(e)?Pe({},e):e:null}function Ps(e,t,s=!1,n=!1){const{props:a,ref:i,patchFlag:l,children:r,transition:o}=e,c=t?yp(a||{},t):a,u={__v_isVNode:!0,__v_skip:!0,type:e.type,props:c,key:c&&mp(c),ref:t&&t.ref?s&&i?he(i)?i.concat(il(t)):[i,il(t)]:il(t):i,scopeId:e.scopeId,slotScopeIds:e.slotScopeIds,children:r,target:e.target,targetStart:e.targetStart,targetAnchor:e.targetAnchor,staticCount:e.staticCount,shapeFlag:e.shapeFlag,patchFlag:t&&e.type!==At?l===-1?16:l|16:l,dynamicProps:e.dynamicProps,dynamicChildren:e.dynamicChildren,appContext:e.appContext,dirs:e.dirs,transition:o,component:e.component,suspense:e.suspense,ssContent:e.ssContent&&Ps(e.ssContent),ssFallback:e.ssFallback&&Ps(e.ssFallback),placeholder:e.placeholder,el:e.el,anchor:e.anchor,ctx:e.ctx,ce:e.ce};return o&&n&&ln(u,o.clone(u)),u}function tc(e=" ",t=0){return lt(_n,null,e,t)}function yb(e,t){const s=lt(Un,null,e);return s.staticCount=t,s}function bp(e="",t=!1){return t?(yi(),_l(ht,null,e)):lt(ht,null,e)}function Xt(e){return e==null||typeof e=="boolean"?lt(ht):he(e)?lt(At,null,e.slice()):rn(e)?qs(e):lt(_n,null,String(e))}function qs(e){return e.el===null&&e.patchFlag!==-1||e.memo?e:Ps(e)}function sc(e,t){let s=0;const{shapeFlag:n}=e;if(t==null)t=null;else if(he(t))s=16;else if(typeof t=="object")if(n&65){const a=t.default;a&&(a._c&&(a._d=!1),sc(e,a()),a._c&&(a._d=!0));return}else{s=32;const a=t._;!a&&!ep(t)?t._ctx=Lt:a===3&&Lt&&(Lt.slots._===1?t._=1:(t._=2,e.patchFlag|=1024))}else we(t)?(t={default:t,_ctx:Lt},s=32):(t=String(t),n&64?(s=16,t=[tc(t)]):s=8);e.children=t,e.shapeFlag|=s}function yp(...e){const t={};for(let s=0;s<e.length;s++){const n=e[s];for(const a in n)if(a==="class")t.class!==n.class&&(t.class=Ii([t.class,n.class]));else if(a==="style")t.style=Ri([t.style,n.style]);else if(Gn(a)){const i=t[a],l=n[a];l&&i!==l&&!(he(i)&&i.includes(l))?t[a]=i?[].concat(i,l):l:l==null&&i==null&&!Hl(a)&&(t[a]=l)}else a!==""&&(t[a]=n[a])}return t}function Yt(e,t,s,n=null){ls(e,t,7,[s,n])}const xb=Wf();let _b=0;function xp(e,t,s){const n=e.type,a=(t?t.appContext:e.appContext)||xb,i={uid:_b++,vnode:e,type:n,parent:t,appContext:a,root:null,next:null,subTree:null,effect:null,update:null,job:null,scope:new Lo(!0),render:null,proxy:null,exposed:null,exposeProxy:null,withProxy:null,provides:t?t.provides:Object.create(a.provides),ids:t?t.ids:["",0,0],accessCache:null,renderCache:[],components:null,directives:null,propsOptions:sp(n,a),emitsOptions:Jf(n,a),emit:null,emitted:null,propsDefaults:Fe,inheritAttrs:n.inheritAttrs,ctx:Fe,data:Fe,props:Fe,attrs:Fe,slots:Fe,refs:Fe,setupState:Fe,setupContext:null,suspense:s,suspenseId:s?s.pendingId:0,asyncDep:null,asyncResolved:!1,isMounted:!1,isUnmounted:!1,isDeactivated:!1,bc:null,c:null,bm:null,m:null,bu:null,u:null,um:null,bum:null,da:null,a:null,rtg:null,rtc:null,ec:null,sp:null};return i.ctx={_:i},i.root=t?t.root:i,i.emit=Jv.bind(null,i),e.ce&&e.ce(i),i}let Nt=null;const Kt=()=>Nt||Lt;let kl,xa;{const e=Kl(),t=(s,n)=>{let a;return(a=e[s])||(a=e[s]=[]),a.push(n),i=>{a.length>1?a.forEach(l=>l(i)):a[0](i)}};kl=t("__VUE_INSTANCE_SETTERS__",s=>Nt=s),xa=t("__VUE_SSR_SETTERS__",s=>zn=s)}const Pa=e=>{const t=Nt;return kl(e),e.scope.on(),()=>{e.scope.off(),kl(t)}},_i=()=>{Nt&&Nt.scope.off(),kl(null)};function _p(e){return e.vnode.shapeFlag&4}let zn=!1;function kp(e,t=!1,s=!1){t&&xa(t);const{props:n,children:a}=e.vnode,i=_p(e);sb(e,n,i,t),lb(e,a,s||t);const l=i?kb(e,t):void 0;return t&&xa(!1),l}function kb(e,t){const s=e.type;e.accessCache=Object.create(null),e.proxy=new Proxy(e.ctx,eo);const{setup:n}=s;if(n){nn();const a=e.setupContext=n.length>1?Tp(e):null,i=Pa(e),l=Ma(n,e,0,[e.props,a]),r=No(l);if(an(),i(),(r||e.sp)&&!en(e)&&jo(e),r){if(l.then(_i,_i),t)return l.then(o=>{io(e,o,t)}).catch(o=>{Jn(o,e,0)});e.asyncDep=l}else io(e,l,t)}else Sp(e,t)}function io(e,t,s){we(t)?e.type.__ssrInlineRender?e.ssrRender=t:e.render=t:je(t)&&(e.setupState=$o(t)),Sp(e,s)}let wl,lo;function wp(e){wl=e,lo=t=>{t.render._rc&&(t.withProxy=new Proxy(t.ctx,Av))}}const wb=()=>!wl;function Sp(e,t,s){const n=e.type;if(!e.render){if(!t&&wl&&!n.render){const a=n.template||Jo(e).template;if(a){const{isCustomElement:i,compilerOptions:l}=e.appContext.config,{delimiters:r,compilerOptions:o}=n,c=Pe(Pe({isCustomElement:i,delimiters:r},l),o);n.render=wl(a,c)}}e.render=n.render||Ot,lo&&lo(e)}{const a=Pa(e);nn();try{Vv(e)}finally{an(),a()}}}const Sb={get(e,t){return Ft(e,"get",""),e[t]}};function Tp(e){const t=s=>{e.exposed=s||{}};return{attrs:new Proxy(e.attrs,Sb),slots:e.slots,emit:e.emit,expose:t}}function Mi(e){return e.exposed?e.exposeProxy||(e.exposeProxy=new Proxy($o(yf(e.exposed)),{get(t,s){if(s in t)return t[s];if(s in ai)return ai[s](e)},has(t,s){return s in t||s in ai}})):e.proxy}function ro(e,t=!0){return we(e)?e.displayName||e.name:e.name||t&&e.__name}function Tb(e){return we(e)&&"__vccOpts"in e}const J=(e,t)=>Nm(e,t,zn);function Sa(e,t,s){try{xi(-1);const n=arguments.length;return n===2?je(t)&&!he(t)?rn(t)?lt(e,null,[t]):lt(e,t):lt(e,null,t):(n>3?s=Array.prototype.slice.call(arguments,2):n===3&&rn(s)&&(s=[s]),lt(e,t,s))}finally{xi(1)}}function Cb(){}function Eb(e,t,s,n){const a=s[n];if(a&&Cp(a,e))return a;const i=t();return i.memo=e.slice(),i.cacheIndex=n,s[n]=i}function Cp(e,t){const s=e.memo;if(s.length!=t.length)return!1;for(let n=0;n<s.length;n++)if(Et(s[n],t[n]))return!1;return jn>0&&$t&&$t.push(e),!0}const Ep="3.5.38",Ab=Ot,Rb=Bm,Ib=oa,Nb=Af,Lb={createComponentInstance:xp,setupComponent:kp,renderComponentRoot:al,setCurrentRenderingInstance:mi,isVNode:rn,normalizeVNode:Xt,getComponentPublicInstance:Mi,ensureValidVNode:Zo,pushWarningContext:Pm,popWarningContext:Fm},Ob=Lb,Db=null,Mb=null,Pb=null;/**
* @vue/runtime-dom v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/let oo;const mu=typeof window<"u"&&window.trustedTypes;if(mu)try{oo=mu.createPolicy("vue",{createHTML:e=>e})}catch{}const Ap=oo?e=>oo.createHTML(e):e=>e,Fb="http://www.w3.org/2000/svg",$b="http://www.w3.org/1998/Math/MathML",zs=typeof document<"u"?document:null,vu=zs&&zs.createElement("template"),Rp={insert:(e,t,s)=>{t.insertBefore(e,s||null)},remove:e=>{const t=e.parentNode;t&&t.removeChild(e)},createElement:(e,t,s,n)=>{const a=t==="svg"?zs.createElementNS(Fb,e):t==="mathml"?zs.createElementNS($b,e):s?zs.createElement(e,{is:s}):zs.createElement(e);return e==="select"&&n&&n.multiple!=null&&a.setAttribute("multiple",n.multiple),a},createText:e=>zs.createTextNode(e),createComment:e=>zs.createComment(e),setText:(e,t)=>{e.nodeValue=t},setElementText:(e,t)=>{e.textContent=t},parentNode:e=>e.parentNode,nextSibling:e=>e.nextSibling,querySelector:e=>zs.querySelector(e),setScopeId(e,t){e.setAttribute(t,"")},insertStaticContent(e,t,s,n,a,i){const l=s?s.previousSibling:t.lastChild;if(a&&(a===i||a.nextSibling))for(;t.insertBefore(a.cloneNode(!0),s),!(a===i||!(a=a.nextSibling)););else{vu.innerHTML=Ap(n==="svg"?`<svg>${e}</svg>`:n==="mathml"?`<math>${e}</math>`:e);const r=vu.content;if(n==="svg"||n==="mathml"){const o=r.firstChild;for(;o.firstChild;)r.appendChild(o.firstChild);r.removeChild(o)}t.insertBefore(r,s)}return[l?l.nextSibling:t.firstChild,s?s.previousSibling:t.lastChild]}},fn="transition",ja="animation",Ta=Symbol("_vtc"),Ip={name:String,type:String,css:{type:Boolean,default:!0},duration:[String,Number,Object],enterFromClass:String,enterActiveClass:String,enterToClass:String,appearFromClass:String,appearActiveClass:String,appearToClass:String,leaveFromClass:String,leaveActiveClass:String,leaveToClass:String},Np=Pe({},Vo,Ip),Ub=e=>(e.displayName="Transition",e.props=Np,e),Bb=Ub((e,{slots:t})=>Sa(Ff,Lp(e),t)),In=(e,t=[])=>{he(e)?e.forEach(s=>s(...t)):e&&e(...t)},bu=e=>e?he(e)?e.some(t=>t.length>1):e.length>1:!1;function Lp(e){const t={};for(const M in e)M in Ip||(t[M]=e[M]);if(e.css===!1)return t;const{name:s="v",type:n,duration:a,enterFromClass:i=`${s}-enter-from`,enterActiveClass:l=`${s}-enter-active`,enterToClass:r=`${s}-enter-to`,appearFromClass:o=i,appearActiveClass:c=l,appearToClass:u=r,leaveFromClass:d=`${s}-leave-from`,leaveActiveClass:f=`${s}-leave-active`,leaveToClass:p=`${s}-leave-to`}=e,b=Hb(a),g=b&&b[0],T=b&&b[1],{onBeforeEnter:O,onEnter:y,onEnterCancelled:v,onLeave:x,onLeaveCancelled:w,onBeforeAppear:I=O,onAppear:L=y,onAppearCancelled:k=v}=t,A=(M,S,U,W)=>{M._enterCancelled=W,mn(M,S?u:r),mn(M,S?c:l),U&&U()},D=(M,S)=>{M._isLeaving=!1,mn(M,d),mn(M,p),mn(M,f),S&&S()},B=M=>(S,U)=>{const W=M?L:y,F=()=>A(S,M,U);In(W,[S,F]),yu(()=>{mn(S,M?o:i),As(S,M?u:r),bu(W)||xu(S,n,g,F)})};return Pe(t,{onBeforeEnter(M){In(O,[M]),As(M,i),As(M,l)},onBeforeAppear(M){In(I,[M]),As(M,o),As(M,c)},onEnter:B(!1),onAppear:B(!0),onLeave(M,S){M._isLeaving=!0;const U=()=>D(M,S);As(M,d),M._enterCancelled?(As(M,f),co(M)):(co(M),As(M,f)),yu(()=>{M._isLeaving&&(mn(M,d),As(M,p),bu(x)||xu(M,n,T,U))}),In(x,[M,U])},onEnterCancelled(M){A(M,!1,void 0,!0),In(v,[M])},onAppearCancelled(M){A(M,!0,void 0,!0),In(k,[M])},onLeaveCancelled(M){D(M),In(w,[M])}})}function Hb(e){if(e==null)return null;if(je(e))return[Rr(e.enter),Rr(e.leave)];{const t=Rr(e);return[t,t]}}function Rr(e){return dl(e)}function As(e,t){t.split(/\s+/).forEach(s=>s&&e.classList.add(s)),(e[Ta]||(e[Ta]=new Set)).add(t)}function mn(e,t){t.split(/\s+/).forEach(n=>n&&e.classList.remove(n));const s=e[Ta];s&&(s.delete(t),s.size||(e[Ta]=void 0))}function yu(e){requestAnimationFrame(()=>{requestAnimationFrame(e)})}let Vb=0;function xu(e,t,s,n){const a=e._endId=++Vb,i=()=>{a===e._endId&&n()};if(s!=null)return setTimeout(i,s);const{type:l,timeout:r,propCount:o}=Op(e,t);if(!l)return n();const c=l+"end";let u=0;const d=()=>{e.removeEventListener(c,f),i()},f=p=>{p.target===e&&++u>=o&&d()};setTimeout(()=>{u<o&&d()},r+1),e.addEventListener(c,f)}function Op(e,t){const s=window.getComputedStyle(e),n=b=>(s[b]||"").split(", "),a=n(`${fn}Delay`),i=n(`${fn}Duration`),l=_u(a,i),r=n(`${ja}Delay`),o=n(`${ja}Duration`),c=_u(r,o);let u=null,d=0,f=0;t===fn?l>0&&(u=fn,d=l,f=i.length):t===ja?c>0&&(u=ja,d=c,f=o.length):(d=Math.max(l,c),u=d>0?l>c?fn:ja:null,f=u?u===fn?i.length:o.length:0);const p=u===fn&&/\b(?:transform|all)(?:,|$)/.test(n(`${fn}Property`).toString());return{type:u,timeout:d,propCount:f,hasTransform:p}}function _u(e,t){for(;e.length<t.length;)e=e.concat(e);return Math.max(...t.map((s,n)=>ku(s)+ku(e[n])))}function ku(e){return e==="auto"?0:Number(e.slice(0,-1).replace(",","."))*1e3}function co(e){return(e?e.ownerDocument:document).body.offsetHeight}function jb(e,t,s){const n=e[Ta];n&&(t=(t?[t,...n]:[...n]).join(" ")),t==null?e.removeAttribute("class"):s?e.setAttribute("class",t):e.className=t}const Sl=Symbol("_vod"),nc=Symbol("_vsh"),Dp={name:"show",beforeMount(e,{value:t},{transition:s}){e[Sl]=e.style.display==="none"?"":e.style.display,s&&t?s.beforeEnter(e):za(e,t)},mounted(e,{value:t},{transition:s}){s&&t&&s.enter(e)},updated(e,{value:t,oldValue:s},{transition:n}){!t!=!s&&(n?t?(n.beforeEnter(e),za(e,!0),n.enter(e)):n.leave(e,()=>{za(e,!1)}):za(e,t))},beforeUnmount(e,{value:t}){za(e,t)}};function za(e,t){e.style.display=t?e[Sl]:"none",e[nc]=!t}function zb(){Dp.getSSRProps=({value:e})=>{if(!e)return{style:{display:"none"}}}}const Mp=Symbol("");function qb(e){const t=Kt();if(!t)return;const s=t.ut=(a=e(t.proxy))=>{Array.from(document.querySelectorAll(`[data-v-owner="${t.uid}"]`)).forEach(i=>Tl(i,a))},n=()=>{const a=e(t.proxy);t.ce?Tl(t.ce,a):uo(t.subTree,a),s(a)};Ko(()=>{hi(n)}),He(()=>{gs(n,Ot,{flush:"post"});const a=new MutationObserver(n);a.observe(t.subTree.el.parentNode,{childList:!0}),gt(()=>a.disconnect())})}function uo(e,t){if(e.shapeFlag&128){const s=e.suspense;e=s.activeBranch,s.pendingBranch&&!s.isHydrating&&s.effects.push(()=>{uo(s.activeBranch,t)})}for(;e.component;)e=e.component.subTree;if(e.shapeFlag&1&&e.el)Tl(e.el,t);else if(e.type===At)e.children.forEach(s=>uo(s,t));else if(e.type===Un){let{el:s,anchor:n}=e;for(;s&&(Tl(s,t),s!==n);)s=s.nextSibling}}function Tl(e,t){if(e.nodeType===1){const s=e.style;let n="";for(const a in t){const i=Yg(t[a]);s.setProperty(`--${a}`,i),n+=`--${a}: ${i};`}s[Mp]=n}}const Kb=/(?:^|;)\s*display\s*:/;function Gb(e,t,s){const n=e.style,a=Ae(s);let i=!1;if(s&&!a){if(t)if(Ae(t))for(const l of t.split(";")){const r=l.slice(0,l.indexOf(":")).trim();s[r]==null&&Xa(n,r,"")}else for(const l in t)s[l]==null&&Xa(n,l,"");for(const l in s){l==="display"&&(i=!0);const r=s[l];r!=null?Zb(e,l,!Ae(t)&&t?t[l]:void 0,r)||Xa(n,l,r):Xa(n,l,"")}}else if(a){if(t!==s){const l=n[Mp];l&&(s+=";"+l),n.cssText=s,i=Kb.test(s)}}else t&&e.removeAttribute("style");Sl in e&&(e[Sl]=i?n.display:"",e[nc]&&(n.display="none"))}const wu=/\s*!important$/;function Xa(e,t,s){if(he(s))s.forEach(n=>Xa(e,t,n));else if(s==null&&(s=""),t.startsWith("--"))e.setProperty(t,s);else{const n=Wb(e,t);wu.test(s)?e.setProperty(es(n),s.replace(wu,""),"important"):e[n]=s}}const Su=["Webkit","Moz","ms"],Ir={};function Wb(e,t){const s=Ir[t];if(s)return s;let n=Qe(t);if(n!=="filter"&&n in e)return Ir[t]=n;n=Zn(n);for(let a=0;a<Su.length;a++){const i=Su[a]+n;if(i in e)return Ir[t]=i}return t}function Zb(e,t,s,n){return e.tagName==="TEXTAREA"&&(t==="width"||t==="height")&&Ae(n)&&s===n}const Tu="http://www.w3.org/1999/xlink";function Cu(e,t,s,n,a,i=Zg(t)){n&&t.startsWith("xlink:")?s==null?e.removeAttributeNS(Tu,t.slice(6,t.length)):e.setAttributeNS(Tu,t,s):s==null||i&&!Qd(s)?e.removeAttribute(t):e.setAttribute(t,i?"":Bt(s)?String(s):s)}function Eu(e,t,s,n,a){if(t==="innerHTML"||t==="textContent"){s!=null&&(e[t]=t==="innerHTML"?Ap(s):s);return}const i=e.tagName;if(t==="value"&&i!=="PROGRESS"&&!i.includes("-")){const r=i==="OPTION"?e.getAttribute("value")||"":e.value,o=s==null?e.type==="checkbox"?"on":"":String(s);(r!==o||!("_value"in e))&&(e.value=o),s==null&&e.removeAttribute(t),e._value=s;return}let l=!1;if(s===""||s==null){const r=typeof e[t];r==="boolean"?s=Qd(s):s==null&&r==="string"?(s="",l=!0):r==="number"&&(s=0,l=!0)}try{e[t]=s}catch{}l&&e.removeAttribute(a||t)}function Zs(e,t,s,n){e.addEventListener(t,s,n)}function Jb(e,t,s,n){e.removeEventListener(t,s,n)}const Au=Symbol("_vei");function Yb(e,t,s,n,a=null){const i=e[Au]||(e[Au]={}),l=i[t];if(n&&l)l.value=n;else{const[r,o]=Qb(t);if(n){const c=i[t]=ty(n,a);Zs(e,r,c,o)}else l&&(Jb(e,r,l,o),i[t]=void 0)}}const Ru=/(?:Once|Passive|Capture)$/;function Qb(e){let t;if(Ru.test(e)){t={};let n;for(;n=e.match(Ru);)e=e.slice(0,e.length-n[0].length),t[n[0].toLowerCase()]=!0}return[e[2]===":"?e.slice(3):es(e.slice(2)),t]}let Nr=0;const Xb=Promise.resolve(),ey=()=>Nr||(Xb.then(()=>Nr=0),Nr=Date.now());function ty(e,t){const s=n=>{if(!n._vts)n._vts=Date.now();else if(n._vts<=s.attached)return;const a=s.value;if(he(a)){const i=n.stopImmediatePropagation;n.stopImmediatePropagation=()=>{i.call(n),n._stopped=!0};const l=a.slice(),r=[n];for(let o=0;o<l.length&&!n._stopped;o++){const c=l[o];c&&ls(c,t,5,r)}}else ls(a,t,5,[n])};return s.value=e,s.attached=ey(),s}const Iu=e=>e.charCodeAt(0)===111&&e.charCodeAt(1)===110&&e.charCodeAt(2)>96&&e.charCodeAt(2)<123,Pp=(e,t,s,n,a,i)=>{const l=a==="svg";t==="class"?jb(e,n,l):t==="style"?Gb(e,s,n):Gn(t)?Hl(t)||Yb(e,t,s,n,i):(t[0]==="."?(t=t.slice(1),!0):t[0]==="^"?(t=t.slice(1),!1):sy(e,t,n,l))?(Eu(e,t,n),!e.tagName.includes("-")&&(t==="value"||t==="checked"||t==="selected")&&Cu(e,t,n,l,i,t!=="value")):e._isVueCE&&(ny(e,t)||e._def.__asyncLoader&&(/[A-Z]/.test(t)||!Ae(n)))?Eu(e,Qe(t),n,i,t):(t==="true-value"?e._trueValue=n:t==="false-value"&&(e._falseValue=n),Cu(e,t,n,l))};function sy(e,t,s,n){if(n)return!!(t==="innerHTML"||t==="textContent"||t in e&&Iu(t)&&we(s));if(t==="spellcheck"||t==="draggable"||t==="translate"||t==="autocorrect"||t==="sandbox"&&e.tagName==="IFRAME"||t==="form"||t==="list"&&e.tagName==="INPUT"||t==="type"&&e.tagName==="TEXTAREA")return!1;if(t==="width"||t==="height"){const a=e.tagName;if(a==="IMG"||a==="VIDEO"||a==="CANVAS"||a==="SOURCE")return!1}return Iu(t)&&Ae(s)?!1:t in e}function ny(e,t){const s=e._def.props;if(!s)return!1;const n=Qe(t);return Array.isArray(s)?s.some(a=>Qe(a)===n):Object.keys(s).some(a=>Qe(a)===n)}const Nu={};function Fp(e,t,s){let n=Oi(e,t);Vl(n)&&(n=Pe({},n,t));class a extends ir{constructor(l){super(n,l,s)}}return a.def=n,a}const ay=((e,t)=>Fp(e,t,Jp)),iy=typeof HTMLElement<"u"?HTMLElement:class{};class ir extends iy{constructor(t,s={},n=Al){super(),this._def=t,this._props=s,this._createApp=n,this._isVueCE=!0,this._instance=null,this._app=null,this._nonce=this._def.nonce,this._connected=!1,this._resolved=!1,this._patching=!1,this._dirty=!1,this._numberProps=null,this._styleChildren=new WeakSet,this._styleAnchors=new WeakMap,this._ob=null,this.shadowRoot&&n!==Al?this._root=this.shadowRoot:t.shadowRoot!==!1?(this.attachShadow(Pe({},t.shadowRootOptions,{mode:"open"})),this._root=this.shadowRoot):this._root=this}connectedCallback(){if(!this.isConnected)return;!this.shadowRoot&&!this._resolved&&this._parseSlots(),this._connected=!0;let t=this;for(;t=t&&(t.assignedSlot||t.parentNode||t.host);)if(t instanceof ir){this._parent=t;break}this._instance||(this._resolved?this._mount(this._def):t&&t._pendingResolve?this._pendingResolve=t._pendingResolve.then(()=>{this._pendingResolve=void 0,this._resolveDef()}):this._resolveDef())}_setParent(t=this._parent){t&&(this._instance.parent=t._instance,this._inheritParentContext(t))}_inheritParentContext(t=this._parent){t&&this._app&&Object.setPrototypeOf(this._app._context.provides,t._instance.provides)}disconnectedCallback(){this._connected=!1,St(()=>{this._connected||(this._ob&&(this._ob.disconnect(),this._ob=null),this._app&&this._app.unmount(),this._instance&&(this._instance.ce=void 0),this._app=this._instance=null,this._teleportTargets&&(this._teleportTargets.clear(),this._teleportTargets=void 0))})}_processMutations(t){for(const s of t)this._setAttr(s.attributeName)}_resolveDef(){if(this._pendingResolve)return;for(let n=0;n<this.attributes.length;n++)this._setAttr(this.attributes[n].name);this._ob=new MutationObserver(this._processMutations.bind(this)),this._ob.observe(this,{attributes:!0});const t=(n,a=!1)=>{this._resolved=!0,this._pendingResolve=void 0;const{props:i,styles:l}=n;let r;if(i&&!he(i))for(const o in i){const c=i[o];(c===Number||c&&c.type===Number)&&(o in this._props&&(this._props[o]=dl(this._props[o])),(r||(r=Object.create(null)))[Qe(o)]=!0)}this._numberProps=r,this._resolveProps(n),this.shadowRoot&&this._applyStyles(l),this._mount(n)},s=this._def.__asyncLoader;s?this._pendingResolve=s().then(n=>{n.configureApp=this._def.configureApp,t(this._def=n,!0)}):t(this._def)}_mount(t){this._app=this._createApp(t),this._inheritParentContext(),t.configureApp&&t.configureApp(this._app),this._app._ceVNode=this._createVNode(),this._app.mount(this._root);const s=this._instance&&this._instance.exposed;if(s)for(const n in s)ze(this,n)||Object.defineProperty(this,n,{get:()=>Ds(s[n])})}_resolveProps(t){const{props:s}=t,n=he(s)?s:Object.keys(s||{});for(const a of Object.keys(this))a[0]!=="_"&&n.includes(a)&&this._setProp(a,this[a]);for(const a of n.map(Qe))Object.defineProperty(this,a,{get(){return this._getProp(a)},set(i){this._setProp(a,i,!0,!this._patching)}})}_setAttr(t){if(t.startsWith("data-v-"))return;const s=this.hasAttribute(t);let n=s?this.getAttribute(t):Nu;const a=Qe(t);s&&this._numberProps&&this._numberProps[a]&&(n=dl(n)),this._setProp(a,n,!1,!0)}_getProp(t){return this._props[t]}_setProp(t,s,n=!0,a=!1){if(s!==this._props[t]&&(this._dirty=!0,s===Nu?delete this._props[t]:(this._props[t]=s,t==="key"&&this._app&&(this._app._ceVNode.key=s)),a&&this._instance&&this._update(),n)){const i=this._ob;i&&(this._processMutations(i.takeRecords()),i.disconnect()),s===!0?this.setAttribute(es(t),""):typeof s=="string"||typeof s=="number"?this.setAttribute(es(t),s+""):s||this.removeAttribute(es(t)),i&&i.observe(this,{attributes:!0})}}_update(){const t=this._createVNode();this._app&&(t.appContext=this._app._context),Zp(t,this._root)}_createVNode(){const t={};this.shadowRoot||(t.onVnodeMounted=t.onVnodeUpdated=this._renderSlots.bind(this));const s=lt(this._def,Pe(t,this._props));return this._instance||(s.ce=n=>{this._instance=n,n.ce=this,n.isCE=!0;const a=(i,l)=>{this.dispatchEvent(new CustomEvent(i,Vl(l[0])?Pe({detail:l},l[0]):{detail:l}))};n.emit=(i,...l)=>{a(i,l),es(i)!==i&&a(es(i),l)},this._setParent()}),s}_applyStyles(t,s,n){if(!t)return;if(s){if(s===this._def||this._styleChildren.has(s))return;this._styleChildren.add(s)}const a=this._nonce,i=this.shadowRoot,l=n?this._getStyleAnchor(n)||this._getStyleAnchor(this._def):this._getRootStyleInsertionAnchor(i);let r=null;for(let o=t.length-1;o>=0;o--){const c=document.createElement("style");a&&c.setAttribute("nonce",a),c.textContent=t[o],i.insertBefore(c,r||l),r=c,o===0&&(n||this._styleAnchors.set(this._def,c),s&&this._styleAnchors.set(s,c))}}_getStyleAnchor(t){if(!t)return null;const s=this._styleAnchors.get(t);return s&&s.parentNode===this.shadowRoot?s:(s&&this._styleAnchors.delete(t),null)}_getRootStyleInsertionAnchor(t){for(let s=0;s<t.childNodes.length;s++){const n=t.childNodes[s];if(!(n instanceof HTMLStyleElement))return n}return null}_parseSlots(){const t=this._slots={};let s;for(;s=this.firstChild;){const n=s.nodeType===1&&s.getAttribute("slot")||"default";(t[n]||(t[n]=[])).push(s),this.removeChild(s)}}_renderSlots(){const t=this._getSlots(),s=this._instance.type.__scopeId;for(let n=0;n<t.length;n++){const a=t[n],i=a.getAttribute("name")||"default",l=this._slots[i],r=a.parentNode;if(l)for(const o of l){if(s&&o.nodeType===1){const c=s+"-s",u=document.createTreeWalker(o,1);o.setAttribute(c,"");let d;for(;d=u.nextNode();)d.setAttribute(c,"")}r.insertBefore(o,a)}else for(;a.firstChild;)r.insertBefore(a.firstChild,a);r.removeChild(a)}}_getSlots(){const t=[this];this._teleportTargets&&t.push(...this._teleportTargets);const s=new Set;for(const n of t){const a=n.querySelectorAll("slot");for(let i=0;i<a.length;i++)s.add(a[i])}return Array.from(s)}_injectChildStyle(t,s){this._applyStyles(t.styles,t,s)}_beginPatch(){this._patching=!0,this._dirty=!1}_endPatch(){this._patching=!1,this._dirty&&this._instance&&this._update()}_hasShadowRoot(){return this._def.shadowRoot!==!1}_removeChildStyle(t){}}function $p(e){const t=Kt(),s=t&&t.ce;return s||null}function ly(){const e=$p();return e&&e.shadowRoot}function ry(e="$style"){{const t=Kt();if(!t)return Fe;const s=t.type.__cssModules;if(!s)return Fe;const n=s[e];return n||Fe}}const Up=new WeakMap,Bp=new WeakMap,Cl=Symbol("_moveCb"),Lu=Symbol("_enterCb"),oy=e=>(delete e.props.mode,e),cy=oy({name:"TransitionGroup",props:Pe({},Np,{tag:String,moveClass:String}),setup(e,{slots:t}){const s=Kt(),n=Ho();let a,i;return tr(()=>{if(!a.length)return;const l=e.moveClass||`${e.name||"v"}-move`;if(!hy(a[0].el,s.vnode.el,l)){a=[];return}a.forEach(dy),a.forEach(fy);const r=a.filter(py);co(s.vnode.el),r.forEach(o=>{const c=o.el,u=c.style;As(c,l),u.transform=u.webkitTransform=u.transitionDuration="";const d=c[Cl]=f=>{f&&f.target!==c||(!f||f.propertyName.endsWith("transform"))&&(c.removeEventListener("transitionend",d),c[Cl]=null,mn(c,l))};c.addEventListener("transitionend",d)}),a=[]}),()=>{const l=Be(e),r=Lp(l);let o=l.tag||At;if(a=[],i)for(let c=0;c<i.length;c++){const u=i[c];u.el&&u.el instanceof Element&&!u.el[nc]&&(a.push(u),ln(u,wa(u,r,n,s)),Up.set(u,Hp(u.el)))}i=t.default?Xl(t.default()):[];for(let c=0;c<i.length;c++){const u=i[c];u.key!=null&&ln(u,wa(u,r,n,s))}return lt(o,null,i)}}}),uy=cy;function dy(e){const t=e.el;t[Cl]&&t[Cl](),t[Lu]&&t[Lu]()}function fy(e){Bp.set(e,Hp(e.el))}function py(e){const t=Up.get(e),s=Bp.get(e),n=t.left-s.left,a=t.top-s.top;if(n||a){const i=e.el,l=i.style,r=i.getBoundingClientRect();let o=1,c=1;return i.offsetWidth&&(o=r.width/i.offsetWidth),i.offsetHeight&&(c=r.height/i.offsetHeight),(!Number.isFinite(o)||o===0)&&(o=1),(!Number.isFinite(c)||c===0)&&(c=1),Math.abs(o-1)<.01&&(o=1),Math.abs(c-1)<.01&&(c=1),l.transform=l.webkitTransform=`translate(${n/o}px,${a/c}px)`,l.transitionDuration="0s",e}}function Hp(e){const t=e.getBoundingClientRect();return{left:t.left,top:t.top}}function hy(e,t,s){const n=e.cloneNode(),a=e[Ta];a&&a.forEach(r=>{r.split(/\s+/).forEach(o=>o&&n.classList.remove(o))}),s.split(/\s+/).forEach(r=>r&&n.classList.add(r)),n.style.display="none";const i=t.nodeType===1?t:t.parentNode;i.appendChild(n);const{hasTransform:l}=Op(n);return i.removeChild(n),l}const wn=e=>{const t=e.props["onUpdate:modelValue"]||!1;return he(t)?s=>ma(t,s):t};function gy(e){e.target.composing=!0}function Ou(e){const t=e.target;t.composing&&(t.composing=!1,t.dispatchEvent(new Event("input")))}const ms=Symbol("_assign");function Du(e,t,s){return t&&(e=e.trim()),s&&(e=ql(e)),e}const El={created(e,{modifiers:{lazy:t,trim:s,number:n}},a){e[ms]=wn(a);const i=n||a.props&&a.props.type==="number";Zs(e,t?"change":"input",l=>{l.target.composing||e[ms](Du(e.value,s,i))}),(s||i)&&Zs(e,"change",()=>{e.value=Du(e.value,s,i)}),t||(Zs(e,"compositionstart",gy),Zs(e,"compositionend",Ou),Zs(e,"change",Ou))},mounted(e,{value:t}){e.value=t??""},beforeUpdate(e,{value:t,oldValue:s,modifiers:{lazy:n,trim:a,number:i}},l){if(e[ms]=wn(l),e.composing)return;const r=(i||e.type==="number")&&!/^0\d/.test(e.value)?ql(e.value):e.value,o=t??"";if(r===o)return;const c=e.getRootNode();(c instanceof Document||c instanceof ShadowRoot)&&c.activeElement===e&&e.type!=="range"&&(n&&t===s||a&&e.value.trim()===o)||(e.value=o)}},ac={deep:!0,created(e,t,s){e[ms]=wn(s),Zs(e,"change",()=>{const n=e._modelValue,a=Ca(e),i=e.checked,l=e[ms];if(he(n)){const r=Gl(n,a),o=r!==-1;if(i&&!o)l(n.concat(a));else if(!i&&o){const c=[...n];c.splice(r,1),l(c)}}else if(Wn(n)){const r=new Set(n);i?r.add(a):r.delete(a),l(r)}else l(jp(e,i))})},mounted:Mu,beforeUpdate(e,t,s){e[ms]=wn(s),Mu(e,t,s)}};function Mu(e,{value:t,oldValue:s},n){e._modelValue=t;let a;if(he(t))a=Gl(t,n.props.value)>-1;else if(Wn(t))a=t.has(n.props.value);else{if(t===s)return;a=sn(t,jp(e,!0))}e.checked!==a&&(e.checked=a)}const ic={created(e,{value:t},s){e.checked=sn(t,s.props.value),e[ms]=wn(s),Zs(e,"change",()=>{e[ms](Ca(e))})},beforeUpdate(e,{value:t,oldValue:s},n){e[ms]=wn(n),t!==s&&(e.checked=sn(t,n.props.value))}},Vp={deep:!0,created(e,{value:t,modifiers:{number:s}},n){const a=Wn(t);Zs(e,"change",()=>{const i=Array.prototype.filter.call(e.options,l=>l.selected).map(l=>s?ql(Ca(l)):Ca(l));e[ms](e.multiple?a?new Set(i):i:i[0]),e._assigning=!0,St(()=>{e._assigning=!1})}),e[ms]=wn(n)},mounted(e,{value:t}){Pu(e,t)},beforeUpdate(e,t,s){e[ms]=wn(s)},updated(e,{value:t}){e._assigning||Pu(e,t)}};function Pu(e,t){const s=e.multiple,n=he(t);if(!(s&&!n&&!Wn(t))){for(let a=0,i=e.options.length;a<i;a++){const l=e.options[a],r=Ca(l);if(s)if(n){const o=typeof r;o==="string"||o==="number"?l.selected=t.some(c=>String(c)===String(r)):l.selected=Gl(t,r)>-1}else l.selected=t.has(r);else if(sn(Ca(l),t)){e.selectedIndex!==a&&(e.selectedIndex=a);return}}!s&&e.selectedIndex!==-1&&(e.selectedIndex=-1)}}function Ca(e){return"_value"in e?e._value:e.value}function jp(e,t){const s=t?"_trueValue":"_falseValue";return s in e?e[s]:t}const zp={created(e,t,s){Yi(e,t,s,null,"created")},mounted(e,t,s){Yi(e,t,s,null,"mounted")},beforeUpdate(e,t,s,n){Yi(e,t,s,n,"beforeUpdate")},updated(e,t,s,n){Yi(e,t,s,n,"updated")}};function qp(e,t){switch(e){case"SELECT":return Vp;case"TEXTAREA":return El;default:switch(t){case"checkbox":return ac;case"radio":return ic;default:return El}}}function Yi(e,t,s,n,a){const l=qp(e.tagName,s.props&&s.props.type)[a];l&&l(e,t,s,n)}function my(){El.getSSRProps=({value:e})=>({value:e}),ic.getSSRProps=({value:e},t)=>{if(t.props&&sn(t.props.value,e))return{checked:!0}},ac.getSSRProps=({value:e},t)=>{if(he(e)){if(t.props&&Gl(e,t.props.value)>-1)return{checked:!0}}else if(Wn(e)){if(t.props&&e.has(t.props.value))return{checked:!0}}else if(e)return{checked:!0}},zp.getSSRProps=(e,t)=>{if(typeof t.type!="string")return;const s=qp(t.type.toUpperCase(),t.props&&t.props.type);if(s.getSSRProps)return s.getSSRProps(e,t)}}const vy=["ctrl","shift","alt","meta"],by={stop:e=>e.stopPropagation(),prevent:e=>e.preventDefault(),self:e=>e.target!==e.currentTarget,ctrl:e=>!e.ctrlKey,shift:e=>!e.shiftKey,alt:e=>!e.altKey,meta:e=>!e.metaKey,left:e=>"button"in e&&e.button!==0,middle:e=>"button"in e&&e.button!==1,right:e=>"button"in e&&e.button!==2,exact:(e,t)=>vy.some(s=>e[`${s}Key`]&&!t.includes(s))},yy=(e,t)=>{if(!e)return e;const s=e._withMods||(e._withMods={}),n=t.join(".");return s[n]||(s[n]=((a,...i)=>{for(let l=0;l<t.length;l++){const r=by[t[l]];if(r&&r(a,t))return}return e(a,...i)}))},xy={esc:"escape",space:" ",up:"arrow-up",left:"arrow-left",right:"arrow-right",down:"arrow-down",delete:"backspace"},_y=(e,t)=>{const s=e._withKeys||(e._withKeys={}),n=t.join(".");return s[n]||(s[n]=(a=>{if(!("key"in a))return;const i=es(a.key);if(t.some(l=>l===i||xy[l]===i))return e(a)}))},Kp=Pe({patchProp:Pp},Rp);let li,Fu=!1;function Gp(){return li||(li=lp(Kp))}function Wp(){return li=Fu?li:rp(Kp),Fu=!0,li}const Zp=((...e)=>{Gp().render(...e)}),ky=((...e)=>{Wp().hydrate(...e)}),Al=((...e)=>{const t=Gp().createApp(...e),{mount:s}=t;return t.mount=n=>{const a=Qp(n);if(!a)return;const i=t._component;!we(i)&&!i.render&&!i.template&&(i.template=a.innerHTML),a.nodeType===1&&(a.textContent="");const l=s(a,!1,Yp(a));return a instanceof Element&&(a.removeAttribute("v-cloak"),a.setAttribute("data-v-app","")),l},t}),Jp=((...e)=>{const t=Wp().createApp(...e),{mount:s}=t;return t.mount=n=>{const a=Qp(n);if(a)return s(a,!0,Yp(a))},t});function Yp(e){if(e instanceof SVGElement)return"svg";if(typeof MathMLElement=="function"&&e instanceof MathMLElement)return"mathml"}function Qp(e){return Ae(e)?document.querySelector(e):e}let $u=!1;const wy=()=>{$u||($u=!0,my(),zb())},Sy=Object.freeze(Object.defineProperty({__proto__:null,BaseTransition:Ff,BaseTransitionPropsValidators:Vo,Comment:ht,DeprecationTypes:Pb,EffectScope:Lo,ErrorCodes:Um,ErrorTypeStrings:Rb,Fragment:At,KeepAlive:bv,ReactiveEffect:di,Static:Un,Suspense:ub,Teleport:ev,Text:_n,TrackOpTypes:Lm,Transition:Bb,TransitionGroup:uy,TriggerOpTypes:Om,VueElement:ir,assertNumber:$m,callWithAsyncErrorHandling:ls,callWithErrorHandling:Ma,camelize:Qe,capitalize:Zn,cloneVNode:Ps,compatUtils:Mb,computed:J,createApp:Al,createBlock:_l,createCommentVNode:bp,createElementBlock:mb,createElementVNode:ec,createHydrationRenderer:rp,createPropsRestProxy:Bv,createRenderer:lp,createSSRApp:Jp,createSlots:Tv,createStaticVNode:yb,createTextVNode:tc,createVNode:lt,customRef:_f,defineAsyncComponent:mv,defineComponent:Oi,defineCustomElement:Fp,defineEmits:Iv,defineExpose:Nv,defineModel:Dv,defineOptions:Lv,defineProps:Rv,defineSSRCustomElement:ay,defineSlots:Ov,devtools:Ib,effect:tm,effectScope:Qg,getCurrentInstance:Kt,getCurrentScope:sf,getCurrentWatcher:Dm,getTransitionRawChildren:Xl,guardReactiveProps:vp,h:Sa,handleError:Jn,hasInjectionContext:Gm,hydrate:ky,hydrateOnIdle:uv,hydrateOnInteraction:hv,hydrateOnMediaQuery:pv,hydrateOnVisible:fv,initCustomFormatter:Cb,initDirectivesForSSR:wy,inject:hs,isMemoSame:Cp,isProxy:Ni,isReactive:Xs,isReadonly:Ms,isRef:yt,isRuntimeOnly:wb,isShallow:ss,isVNode:rn,markRaw:yf,mergeDefaults:$v,mergeModels:Uv,mergeProps:yp,nextTick:St,nodeOps:Rp,normalizeClass:Ii,normalizeProps:Ug,normalizeStyle:Ri,onActivated:zo,onBeforeMount:Bf,onBeforeUnmount:sr,onBeforeUpdate:Ko,onDeactivated:qo,onErrorCaptured:zf,onMounted:He,onRenderTracked:jf,onRenderTriggered:Vf,onScopeDispose:Xg,onServerPrefetch:Hf,onUnmounted:gt,onUpdated:tr,onWatcherCleanup:wf,openBlock:yi,patchProp:Pp,popScopeId:zm,provide:ni,proxyRefs:$o,pushScopeId:jm,queuePostFlushCb:hi,reactive:Sn,readonly:pl,ref:h,registerRuntimeCompiler:wp,render:Zp,renderList:Sv,renderSlot:Cv,resolveComponent:_v,resolveDirective:wv,resolveDynamicComponent:kv,resolveFilter:Db,resolveTransitionHooks:wa,setBlockTracking:xi,setDevtoolsHook:Nb,setTransitionHooks:ln,shallowReactive:Po,shallowReadonly:xm,shallowRef:Fo,ssrContextKey:Rf,ssrUtils:Ob,stop:sm,toDisplayString:ef,toHandlerKey:ga,toHandlers:Ev,toRaw:Be,toRef:Rm,toRefs:Cm,toValue:wm,transformVNodeArgs:vb,triggerRef:km,unref:Ds,useAttrs:Fv,useCssModule:ry,useCssVars:qb,useHost:$p,useId:sv,useModel:Zv,useSSRContext:If,useShadowRoot:ly,useSlots:Pv,useTemplateRef:nv,useTransitionState:Ho,vModelCheckbox:ac,vModelDynamic:zp,vModelRadio:ic,vModelSelect:Vp,vModelText:El,vShow:Dp,version:Ep,warn:Ab,watch:gs,watchEffect:Wm,watchPostEffect:Zm,watchSyncEffect:Nf,withAsyncContext:Hv,withCtx:Bo,withDefaults:Mv,withDirectives:Km,withKeys:_y,withMemo:Eb,withModifiers:yy,withScopeId:qm},Symbol.toStringTag,{value:"Module"}));/**
* @vue/compiler-core v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const ki=Symbol(""),ri=Symbol(""),lc=Symbol(""),Rl=Symbol(""),Xp=Symbol(""),qn=Symbol(""),eh=Symbol(""),th=Symbol(""),rc=Symbol(""),oc=Symbol(""),Pi=Symbol(""),cc=Symbol(""),sh=Symbol(""),uc=Symbol(""),dc=Symbol(""),fc=Symbol(""),pc=Symbol(""),hc=Symbol(""),gc=Symbol(""),nh=Symbol(""),ah=Symbol(""),lr=Symbol(""),Il=Symbol(""),mc=Symbol(""),vc=Symbol(""),wi=Symbol(""),Fi=Symbol(""),bc=Symbol(""),fo=Symbol(""),Ty=Symbol(""),po=Symbol(""),Nl=Symbol(""),Cy=Symbol(""),Ey=Symbol(""),yc=Symbol(""),Ay=Symbol(""),Ry=Symbol(""),xc=Symbol(""),ih=Symbol(""),Ea={[ki]:"Fragment",[ri]:"Teleport",[lc]:"Suspense",[Rl]:"KeepAlive",[Xp]:"BaseTransition",[qn]:"openBlock",[eh]:"createBlock",[th]:"createElementBlock",[rc]:"createVNode",[oc]:"createElementVNode",[Pi]:"createCommentVNode",[cc]:"createTextVNode",[sh]:"createStaticVNode",[uc]:"resolveComponent",[dc]:"resolveDynamicComponent",[fc]:"resolveDirective",[pc]:"resolveFilter",[hc]:"withDirectives",[gc]:"renderList",[nh]:"renderSlot",[ah]:"createSlots",[lr]:"toDisplayString",[Il]:"mergeProps",[mc]:"normalizeClass",[vc]:"normalizeStyle",[wi]:"normalizeProps",[Fi]:"guardReactiveProps",[bc]:"toHandlers",[fo]:"camelize",[Ty]:"capitalize",[po]:"toHandlerKey",[Nl]:"setBlockTracking",[Cy]:"pushScopeId",[Ey]:"popScopeId",[yc]:"withCtx",[Ay]:"unref",[Ry]:"isRef",[xc]:"withMemo",[ih]:"isMemoSame"};function Iy(e){Object.getOwnPropertySymbols(e).forEach(t=>{Ea[t]=e[t]})}const cs={start:{line:1,column:1,offset:0},end:{line:1,column:1,offset:0},source:""};function Ny(e,t=""){return{type:0,source:t,children:e,helpers:new Set,components:[],directives:[],hoists:[],imports:[],cached:[],temps:0,codegenNode:void 0,loc:cs}}function Si(e,t,s,n,a,i,l,r=!1,o=!1,c=!1,u=cs){return e&&(r?(e.helper(qn),e.helper(Ia(e.inSSR,c))):e.helper(Ra(e.inSSR,c)),l&&e.helper(hc)),{type:13,tag:t,props:s,children:n,patchFlag:a,dynamicProps:i,directives:l,isBlock:r,disableTracking:o,isComponent:c,loc:u}}function Bn(e,t=cs){return{type:17,loc:t,elements:e}}function ps(e,t=cs){return{type:15,loc:t,properties:e}}function bt(e,t){return{type:16,loc:cs,key:Ae(e)?Ne(e,!0):e,value:t}}function Ne(e,t=!1,s=cs,n=0){return{type:4,loc:s,content:e,isStatic:t,constType:t?3:n}}function ks(e,t=cs){return{type:8,loc:t,children:e}}function Tt(e,t=[],s=cs){return{type:14,loc:s,callee:e,arguments:t}}function Aa(e,t=void 0,s=!1,n=!1,a=cs){return{type:18,params:e,returns:t,newline:s,isSlot:n,loc:a}}function ho(e,t,s,n=!0){return{type:19,test:e,consequent:t,alternate:s,newline:n,loc:cs}}function Ly(e,t,s=!1,n=!1){return{type:20,index:e,value:t,needPauseTracking:s,inVOnce:n,needArraySpread:!1,loc:cs}}function Oy(e){return{type:21,body:e,loc:cs}}function Ra(e,t){return e||t?rc:oc}function Ia(e,t){return e||t?eh:th}function _c(e,{helper:t,removeHelper:s,inSSR:n}){e.isBlock||(e.isBlock=!0,s(Ra(n,e.isComponent)),t(qn),t(Ia(n,e.isComponent)))}const Uu=new Uint8Array([123,123]),Bu=new Uint8Array([125,125]);function Hu(e){return e>=97&&e<=122||e>=65&&e<=90}function as(e){return e===32||e===10||e===9||e===12||e===13}function pn(e){return e===47||e===62||as(e)}function Ll(e){const t=new Uint8Array(e.length);for(let s=0;s<e.length;s++)t[s]=e.charCodeAt(s);return t}const Dt={Cdata:new Uint8Array([67,68,65,84,65,91]),CdataEnd:new Uint8Array([93,93,62]),CommentEnd:new Uint8Array([45,45,62]),ScriptEnd:new Uint8Array([60,47,115,99,114,105,112,116]),StyleEnd:new Uint8Array([60,47,115,116,121,108,101]),TitleEnd:new Uint8Array([60,47,116,105,116,108,101]),TextareaEnd:new Uint8Array([60,47,116,101,120,116,97,114,101,97])};class Dy{constructor(t,s){this.stack=t,this.cbs=s,this.state=1,this.buffer="",this.sectionStart=0,this.index=0,this.entityStart=0,this.baseState=1,this.inRCDATA=!1,this.inXML=!1,this.inVPre=!1,this.newlines=[],this.mode=0,this.delimiterOpen=Uu,this.delimiterClose=Bu,this.delimiterIndex=-1,this.currentSequence=void 0,this.sequenceIndex=0}get inSFCRoot(){return this.mode===2&&this.stack.length===0}reset(){this.state=1,this.mode=0,this.buffer="",this.sectionStart=0,this.index=0,this.baseState=1,this.inRCDATA=!1,this.currentSequence=void 0,this.newlines.length=0,this.delimiterOpen=Uu,this.delimiterClose=Bu}getPos(t){let s=1,n=t+1;const a=this.newlines.length;let i=-1;if(a>100){let l=-1,r=a;for(;l+1<r;){const o=l+r>>>1;this.newlines[o]<t?l=o:r=o}i=l}else for(let l=a-1;l>=0;l--)if(t>this.newlines[l]){i=l;break}return i>=0&&(s=i+2,n=t-this.newlines[i]),{column:n,line:s,offset:t}}peek(){return this.buffer.charCodeAt(this.index+1)}stateText(t){t===60?(this.index>this.sectionStart&&this.cbs.ontext(this.sectionStart,this.index),this.state=5,this.sectionStart=this.index):!this.inVPre&&t===this.delimiterOpen[0]&&(this.state=2,this.delimiterIndex=0,this.stateInterpolationOpen(t))}stateInterpolationOpen(t){if(t===this.delimiterOpen[this.delimiterIndex])if(this.delimiterIndex===this.delimiterOpen.length-1){const s=this.index+1-this.delimiterOpen.length;s>this.sectionStart&&this.cbs.ontext(this.sectionStart,s),this.state=3,this.sectionStart=s}else this.delimiterIndex++;else this.inRCDATA?(this.state=32,this.stateInRCDATA(t)):(this.state=1,this.stateText(t))}stateInterpolation(t){t===this.delimiterClose[0]&&(this.state=4,this.delimiterIndex=0,this.stateInterpolationClose(t))}stateInterpolationClose(t){t===this.delimiterClose[this.delimiterIndex]?this.delimiterIndex===this.delimiterClose.length-1?(this.cbs.oninterpolation(this.sectionStart,this.index+1),this.inRCDATA?this.state=32:this.state=1,this.sectionStart=this.index+1):this.delimiterIndex++:(this.state=3,this.stateInterpolation(t))}stateSpecialStartSequence(t){const s=this.sequenceIndex===this.currentSequence.length;if(!(s?pn(t):(t|32)===this.currentSequence[this.sequenceIndex]))this.inRCDATA=!1;else if(!s){this.sequenceIndex++;return}this.sequenceIndex=0,this.state=6,this.stateInTagName(t)}stateInRCDATA(t){if(this.sequenceIndex===this.currentSequence.length){if(t===62||as(t)){const s=this.index-this.currentSequence.length;if(this.sectionStart<s){const n=this.index;this.index=s,this.cbs.ontext(this.sectionStart,s),this.index=n}this.sectionStart=s+2,this.stateInClosingTagName(t),this.inRCDATA=!1;return}this.sequenceIndex=0}(t|32)===this.currentSequence[this.sequenceIndex]?this.sequenceIndex+=1:this.sequenceIndex===0?this.currentSequence===Dt.TitleEnd||this.currentSequence===Dt.TextareaEnd&&!this.inSFCRoot?!this.inVPre&&t===this.delimiterOpen[0]&&(this.state=2,this.delimiterIndex=0,this.stateInterpolationOpen(t)):this.fastForwardTo(60)&&(this.sequenceIndex=1):this.sequenceIndex=+(t===60)}stateCDATASequence(t){t===Dt.Cdata[this.sequenceIndex]?++this.sequenceIndex===Dt.Cdata.length&&(this.state=28,this.currentSequence=Dt.CdataEnd,this.sequenceIndex=0,this.sectionStart=this.index+1):(this.sequenceIndex=0,this.state=23,this.stateInDeclaration(t))}fastForwardTo(t){for(;++this.index<this.buffer.length;){const s=this.buffer.charCodeAt(this.index);if(s===10&&this.newlines.push(this.index),s===t)return!0}return this.index=this.buffer.length-1,!1}stateInCommentLike(t){t===this.currentSequence[this.sequenceIndex]?++this.sequenceIndex===this.currentSequence.length&&(this.currentSequence===Dt.CdataEnd?this.cbs.oncdata(this.sectionStart,this.index-2):this.cbs.oncomment(this.sectionStart,this.index-2),this.sequenceIndex=0,this.sectionStart=this.index+1,this.state=1):this.sequenceIndex===0?this.fastForwardTo(this.currentSequence[0])&&(this.sequenceIndex=1):t!==this.currentSequence[this.sequenceIndex-1]&&(this.sequenceIndex=0)}startSpecial(t,s){this.enterRCDATA(t,s),this.state=31}enterRCDATA(t,s){this.inRCDATA=!0,this.currentSequence=t,this.sequenceIndex=s}stateBeforeTagName(t){t===33?(this.state=22,this.sectionStart=this.index+1):t===63?(this.state=24,this.sectionStart=this.index+1):Hu(t)?(this.sectionStart=this.index,this.mode===0?this.state=6:this.inSFCRoot?this.state=34:this.inXML?this.state=6:t===116?this.state=30:this.state=t===115?29:6):t===47?this.state=8:(this.state=1,this.stateText(t))}stateInTagName(t){pn(t)&&this.handleTagName(t)}stateInSFCRootTagName(t){if(pn(t)){const s=this.buffer.slice(this.sectionStart,this.index);s!=="template"&&this.enterRCDATA(Ll("</"+s),0),this.handleTagName(t)}}handleTagName(t){this.cbs.onopentagname(this.sectionStart,this.index),this.sectionStart=-1,this.state=11,this.stateBeforeAttrName(t)}stateBeforeClosingTagName(t){as(t)||(t===62?(this.state=1,this.sectionStart=this.index+1):(this.state=Hu(t)?9:27,this.sectionStart=this.index))}stateInClosingTagName(t){(t===62||as(t))&&(this.cbs.onclosetag(this.sectionStart,this.index),this.sectionStart=-1,this.state=10,this.stateAfterClosingTagName(t))}stateAfterClosingTagName(t){t===62&&(this.state=1,this.sectionStart=this.index+1)}stateBeforeAttrName(t){t===62?(this.cbs.onopentagend(this.index),this.inRCDATA?this.state=32:this.state=1,this.sectionStart=this.index+1):t===47?this.state=7:t===60&&this.peek()===47?(this.cbs.onopentagend(this.index),this.state=5,this.sectionStart=this.index):as(t)||this.handleAttrStart(t)}handleAttrStart(t){t===118&&this.peek()===45?(this.state=13,this.sectionStart=this.index):t===46||t===58||t===64||t===35?(this.cbs.ondirname(this.index,this.index+1),this.state=14,this.sectionStart=this.index+1):(this.state=12,this.sectionStart=this.index)}stateInSelfClosingTag(t){t===62?(this.cbs.onselfclosingtag(this.index),this.state=1,this.sectionStart=this.index+1,this.inRCDATA=!1):as(t)||(this.state=11,this.stateBeforeAttrName(t))}stateInAttrName(t){(t===61||pn(t))&&(this.cbs.onattribname(this.sectionStart,this.index),this.handleAttrNameEnd(t))}stateInDirName(t){t===61||pn(t)?(this.cbs.ondirname(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===58?(this.cbs.ondirname(this.sectionStart,this.index),this.state=14,this.sectionStart=this.index+1):t===46&&(this.cbs.ondirname(this.sectionStart,this.index),this.state=16,this.sectionStart=this.index+1)}stateInDirArg(t){t===61||pn(t)?(this.cbs.ondirarg(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===91?this.state=15:t===46&&(this.cbs.ondirarg(this.sectionStart,this.index),this.state=16,this.sectionStart=this.index+1)}stateInDynamicDirArg(t){t===93?this.state=14:(t===61||pn(t))&&(this.cbs.ondirarg(this.sectionStart,this.index+1),this.handleAttrNameEnd(t))}stateInDirModifier(t){t===61||pn(t)?(this.cbs.ondirmodifier(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===46&&(this.cbs.ondirmodifier(this.sectionStart,this.index),this.sectionStart=this.index+1)}handleAttrNameEnd(t){this.sectionStart=this.index,this.state=17,this.cbs.onattribnameend(this.index),this.stateAfterAttrName(t)}stateAfterAttrName(t){t===61?this.state=18:t===47||t===62?(this.cbs.onattribend(0,this.sectionStart),this.sectionStart=-1,this.state=11,this.stateBeforeAttrName(t)):as(t)||(this.cbs.onattribend(0,this.sectionStart),this.handleAttrStart(t))}stateBeforeAttrValue(t){t===34?(this.state=19,this.sectionStart=this.index+1):t===39?(this.state=20,this.sectionStart=this.index+1):as(t)||(this.sectionStart=this.index,this.state=21,this.stateInAttrValueNoQuotes(t))}handleInAttrValue(t,s){(t===s||this.fastForwardTo(s))&&(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=-1,this.cbs.onattribend(s===34?3:2,this.index+1),this.state=11)}stateInAttrValueDoubleQuotes(t){this.handleInAttrValue(t,34)}stateInAttrValueSingleQuotes(t){this.handleInAttrValue(t,39)}stateInAttrValueNoQuotes(t){as(t)||t===62?(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=-1,this.cbs.onattribend(1,this.index),this.state=11,this.stateBeforeAttrName(t)):(t===39||t===60||t===61||t===96)&&this.cbs.onerr(18,this.index)}stateBeforeDeclaration(t){t===91?(this.state=26,this.sequenceIndex=0):this.state=t===45?25:23}stateInDeclaration(t){(t===62||this.fastForwardTo(62))&&(this.state=1,this.sectionStart=this.index+1)}stateInProcessingInstruction(t){(t===62||this.fastForwardTo(62))&&(this.cbs.onprocessinginstruction(this.sectionStart,this.index),this.state=1,this.sectionStart=this.index+1)}stateBeforeComment(t){t===45?(this.state=28,this.currentSequence=Dt.CommentEnd,this.sequenceIndex=2,this.sectionStart=this.index+1):this.state=23}stateInSpecialComment(t){(t===62||this.fastForwardTo(62))&&(this.cbs.oncomment(this.sectionStart,this.index),this.state=1,this.sectionStart=this.index+1)}stateBeforeSpecialS(t){t===Dt.ScriptEnd[3]?this.startSpecial(Dt.ScriptEnd,4):t===Dt.StyleEnd[3]?this.startSpecial(Dt.StyleEnd,4):(this.state=6,this.stateInTagName(t))}stateBeforeSpecialT(t){t===Dt.TitleEnd[3]?this.startSpecial(Dt.TitleEnd,4):t===Dt.TextareaEnd[3]?this.startSpecial(Dt.TextareaEnd,4):(this.state=6,this.stateInTagName(t))}startEntity(){}stateInEntity(){}parse(t){for(this.buffer=t;this.index<this.buffer.length;){const s=this.buffer.charCodeAt(this.index);switch(s===10&&this.state!==33&&this.newlines.push(this.index),this.state){case 1:{this.stateText(s);break}case 2:{this.stateInterpolationOpen(s);break}case 3:{this.stateInterpolation(s);break}case 4:{this.stateInterpolationClose(s);break}case 31:{this.stateSpecialStartSequence(s);break}case 32:{this.stateInRCDATA(s);break}case 26:{this.stateCDATASequence(s);break}case 19:{this.stateInAttrValueDoubleQuotes(s);break}case 12:{this.stateInAttrName(s);break}case 13:{this.stateInDirName(s);break}case 14:{this.stateInDirArg(s);break}case 15:{this.stateInDynamicDirArg(s);break}case 16:{this.stateInDirModifier(s);break}case 28:{this.stateInCommentLike(s);break}case 27:{this.stateInSpecialComment(s);break}case 11:{this.stateBeforeAttrName(s);break}case 6:{this.stateInTagName(s);break}case 34:{this.stateInSFCRootTagName(s);break}case 9:{this.stateInClosingTagName(s);break}case 5:{this.stateBeforeTagName(s);break}case 17:{this.stateAfterAttrName(s);break}case 20:{this.stateInAttrValueSingleQuotes(s);break}case 18:{this.stateBeforeAttrValue(s);break}case 8:{this.stateBeforeClosingTagName(s);break}case 10:{this.stateAfterClosingTagName(s);break}case 29:{this.stateBeforeSpecialS(s);break}case 30:{this.stateBeforeSpecialT(s);break}case 21:{this.stateInAttrValueNoQuotes(s);break}case 7:{this.stateInSelfClosingTag(s);break}case 23:{this.stateInDeclaration(s);break}case 22:{this.stateBeforeDeclaration(s);break}case 25:{this.stateBeforeComment(s);break}case 24:{this.stateInProcessingInstruction(s);break}case 33:{this.stateInEntity();break}}this.index++}this.cleanup(),this.finish()}cleanup(){this.sectionStart!==this.index&&(this.state===1||this.state===32&&this.sequenceIndex===0?(this.cbs.ontext(this.sectionStart,this.index),this.sectionStart=this.index):(this.state===19||this.state===20||this.state===21)&&(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=this.index))}finish(){this.handleTrailingData(),this.cbs.onend()}handleTrailingData(){const t=this.buffer.length;this.sectionStart>=t||(this.state===28?this.currentSequence===Dt.CdataEnd?this.cbs.oncdata(this.sectionStart,t):this.cbs.oncomment(this.sectionStart,t):this.state===6||this.state===11||this.state===18||this.state===17||this.state===12||this.state===13||this.state===14||this.state===15||this.state===16||this.state===20||this.state===19||this.state===21||this.state===9||this.cbs.ontext(this.sectionStart,t))}emitCodePoint(t,s){}}function Vu(e,{compatConfig:t}){const s=t&&t[e];return e==="MODE"?s||3:s}function Hn(e,t){const s=Vu("MODE",t),n=Vu(e,t);return s===3?n===!0:n!==!1}function Ti(e,t,s,...n){return Hn(e,t)}function kc(e){throw e}function lh(e){}function at(e,t,s,n){const a=`https://vuejs.org/error-reference/#compiler-${e}`,i=new SyntaxError(String(a));return i.code=e,i.loc=t,i}const ts=e=>e.type===4&&e.isStatic;function rh(e){switch(e){case"Teleport":case"teleport":return ri;case"Suspense":case"suspense":return lc;case"KeepAlive":case"keep-alive":return Rl;case"BaseTransition":case"base-transition":return Xp}}const My=/^$|^\d|[^\$\w\xA0-\uFFFF]/,wc=e=>!My.test(e),oh=/[A-Za-z_$\xA0-\uFFFF]/,Py=/[\.\?\w$\xA0-\uFFFF]/,Fy=/\s+[.[]\s*|\s*[.[]\s+/g,ch=e=>e.type===4?e.content:e.loc.source,$y=e=>{const t=ch(e).trim().replace(Fy,r=>r.trim());let s=0,n=[],a=0,i=0,l=null;for(let r=0;r<t.length;r++){const o=t.charAt(r);switch(s){case 0:if(o==="[")n.push(s),s=1,a++;else if(o==="(")n.push(s),s=2,i++;else if(!(r===0?oh:Py).test(o))return!1;break;case 1:o==="'"||o==='"'||o==="`"?(n.push(s),s=3,l=o):o==="["?a++:o==="]"&&(--a||(s=n.pop()));break;case 2:if(o==="'"||o==='"'||o==="`")n.push(s),s=3,l=o;else if(o==="(")i++;else if(o===")"){if(r===t.length-1)return!1;--i||(s=n.pop())}break;case 3:o===l&&(s=n.pop(),l=null);break}}return!a&&!i},uh=$y,Uy=/^\s*(?:async\s*)?(?:\([^)]*?\)|[\w$_]+)\s*(?::[^=]+)?=>|^\s*(?:async\s+)?function(?:\s+[\w$]+)?\s*\(/,By=e=>Uy.test(ch(e)),Hy=By;function fs(e,t,s=!1){for(let n=0;n<e.props.length;n++){const a=e.props[n];if(a.type===7&&(s||a.exp)&&(Ae(t)?a.name===t:t.test(a.name)))return a}}function rr(e,t,s=!1,n=!1){for(let a=0;a<e.props.length;a++){const i=e.props[a];if(i.type===6){if(s)continue;if(i.name===t&&(i.value||n))return i}else if(i.name==="bind"&&(i.exp||n)&&Dn(i.arg,t))return i}}function Dn(e,t){return!!(e&&ts(e)&&e.content===t)}function Vy(e){return e.props.some(t=>t.type===7&&t.name==="bind"&&(!t.arg||t.arg.type!==4||!t.arg.isStatic))}function Lr(e){return e.type===5||e.type===2}function ju(e){return e.type===7&&e.name==="pre"}function jy(e){return e.type===7&&e.name==="slot"}function Ol(e){return e.type===1&&e.tagType===3}function Dl(e){return e.type===1&&e.tagType===2}const zy=new Set([wi,Fi]);function dh(e,t=[]){if(e&&!Ae(e)&&e.type===14){const s=e.callee;if(!Ae(s)&&zy.has(s))return dh(e.arguments[0],t.concat(e))}return[e,t]}function Ml(e,t,s){let n,a=e.type===13?e.props:e.arguments[2],i=[],l;if(a&&!Ae(a)&&a.type===14){const r=dh(a);a=r[0],i=r[1],l=i[i.length-1]}if(a==null||Ae(a))n=ps([t]);else if(a.type===14){const r=a.arguments[0];!Ae(r)&&r.type===15?zu(t,r)||r.properties.unshift(t):a.callee===bc?n=Tt(s.helper(Il),[ps([t]),a]):a.arguments.unshift(ps([t])),!n&&(n=a)}else a.type===15?(zu(t,a)||a.properties.unshift(t),n=a):(n=Tt(s.helper(Il),[ps([t]),a]),l&&l.callee===Fi&&(l=i[i.length-2]));e.type===13?l?l.arguments[0]=n:e.props=n:l?l.arguments[0]=n:e.arguments[2]=n}function zu(e,t){let s=!1;if(e.key.type===4){const n=e.key.content;s=t.properties.some(a=>a.key.type===4&&a.key.content===n)}return s}function Ci(e,t){return`_${t}_${e.replace(/[^\w]/g,(s,n)=>s==="-"?"_":e.charCodeAt(n).toString())}`}function qy(e){return e.type===14&&e.callee===xc?e.arguments[1].returns:e}const Ky=/([\s\S]*?)\s+(?:in|of)\s+(\S[\s\S]*)/;function fh(e){for(let t=0;t<e.length;t++)if(!as(e.charCodeAt(t)))return!1;return!0}function Sc(e){return e.type===2&&fh(e.content)||e.type===12&&Sc(e.content)}function ph(e){return e.type===3||Sc(e)}const hh={parseMode:"base",ns:0,delimiters:["{{","}}"],getNamespace:()=>0,isVoidTag:da,isPreTag:da,isIgnoreNewlineTag:da,isCustomElement:da,onError:kc,onWarn:lh,comments:!1,prefixIdentifiers:!1};let Ve=hh,Ei=null,tn="",Pt=null,$e=null,Jt="",js=-1,Ln=-1,Tc=0,yn=!1,go=null;const nt=[],ut=new Dy(nt,{onerr:Bs,ontext(e,t){Qi(It(e,t),e,t)},ontextentity(e,t,s){Qi(e,t,s)},oninterpolation(e,t){if(yn)return Qi(It(e,t),e,t);let s=e+ut.delimiterOpen.length,n=t-ut.delimiterClose.length;for(;as(tn.charCodeAt(s));)s++;for(;as(tn.charCodeAt(n-1));)n--;let a=It(s,n);a.includes("&")&&(a=Ve.decodeEntities(a,!1)),mo({type:5,content:rl(a,!1,pt(s,n)),loc:pt(e,t)})},onopentagname(e,t){const s=It(e,t);Pt={type:1,tag:s,ns:Ve.getNamespace(s,nt[0],Ve.ns),tagType:0,props:[],children:[],loc:pt(e-1,t),codegenNode:void 0}},onopentagend(e){Ku(e)},onclosetag(e,t){const s=It(e,t);if(!Ve.isVoidTag(s)){let n=!1;for(let a=0;a<nt.length;a++)if(nt[a].tag.toLowerCase()===s.toLowerCase()){n=!0,a>0&&Bs(24,nt[0].loc.start.offset);for(let l=0;l<=a;l++){const r=nt.shift();ll(r,t,l<a)}break}n||Bs(23,gh(e,60))}},onselfclosingtag(e){const t=Pt.tag;Pt.isSelfClosing=!0,Ku(e),nt[0]&&nt[0].tag===t&&ll(nt.shift(),e)},onattribname(e,t){$e={type:6,name:It(e,t),nameLoc:pt(e,t),value:void 0,loc:pt(e)}},ondirname(e,t){const s=It(e,t),n=s==="."||s===":"?"bind":s==="@"?"on":s==="#"?"slot":s.slice(2);if(!yn&&n===""&&Bs(26,e),yn||n==="")$e={type:6,name:s,nameLoc:pt(e,t),value:void 0,loc:pt(e)};else if($e={type:7,name:n,rawName:s,exp:void 0,arg:void 0,modifiers:s==="."?[Ne("prop")]:[],loc:pt(e)},n==="pre"){yn=ut.inVPre=!0,go=Pt;const a=Pt.props;for(let i=0;i<a.length;i++)a[i].type===7&&(a[i]=s0(a[i]))}},ondirarg(e,t){if(e===t)return;const s=It(e,t);if(yn&&!ju($e))$e.name+=s,Mn($e.nameLoc,t);else{const n=s[0]!=="[";$e.arg=rl(n?s:s.slice(1,-1),n,pt(e,t),n?3:0)}},ondirmodifier(e,t){const s=It(e,t);if(yn&&!ju($e))$e.name+="."+s,Mn($e.nameLoc,t);else if($e.name==="slot"){const n=$e.arg;n&&(n.content+="."+s,Mn(n.loc,t))}else{const n=Ne(s,!0,pt(e,t));$e.modifiers.push(n)}},onattribdata(e,t){Jt+=It(e,t),js<0&&(js=e),Ln=t},onattribentity(e,t,s){Jt+=e,js<0&&(js=t),Ln=s},onattribnameend(e){const t=$e.loc.start.offset,s=It(t,e);$e.type===7&&($e.rawName=s),Pt.props.some(n=>(n.type===7?n.rawName:n.name)===s)&&Bs(2,t)},onattribend(e,t){if(Pt&&$e){if(Mn($e.loc,t),e!==0)if(Jt.includes("&")&&(Jt=Ve.decodeEntities(Jt,!0)),$e.type===6)$e.name==="class"&&(Jt=vh(Jt).trim()),e===1&&!Jt&&Bs(13,t),$e.value={type:2,content:Jt,loc:e===1?pt(js,Ln):pt(js-1,Ln+1)},ut.inSFCRoot&&Pt.tag==="template"&&$e.name==="lang"&&Jt&&Jt!=="html"&&ut.enterRCDATA(Ll("</template"),0);else{let s=0;$e.exp=rl(Jt,!1,pt(js,Ln),0,s),$e.name==="for"&&($e.forParseResult=Wy($e.exp));let n=-1;$e.name==="bind"&&(n=$e.modifiers.findIndex(a=>a.content==="sync"))>-1&&Ti("COMPILER_V_BIND_SYNC",Ve,$e.loc,$e.arg.loc.source)&&($e.name="model",$e.modifiers.splice(n,1))}($e.type!==7||$e.name!=="pre")&&Pt.props.push($e)}Jt="",js=Ln=-1},oncomment(e,t){Ve.comments&&mo({type:3,content:It(e,t),loc:pt(e-4,t+3)})},onend(){const e=tn.length;for(let t=0;t<nt.length;t++)ll(nt[t],e-1),Bs(24,nt[t].loc.start.offset)},oncdata(e,t){(nt[0]?nt[0].ns:Ve.ns)!==0?Qi(It(e,t),e,t):Bs(1,e-9)},onprocessinginstruction(e){(nt[0]?nt[0].ns:Ve.ns)===0&&Bs(21,e-1)}}),qu=/,([^,\}\]]*)(?:,([^,\}\]]*))?$/,Gy=/^\(|\)$/g;function Wy(e){const t=e.loc,s=e.content,n=s.match(Ky);if(!n)return;const[,a,i]=n,l=(d,f,p=!1)=>{const b=t.start.offset+f,g=b+d.length;return rl(d,!1,pt(b,g),0,p?1:0)},r={source:l(i.trim(),s.indexOf(i,a.length)),value:void 0,key:void 0,index:void 0,finalized:!1};let o=a.trim().replace(Gy,"").trim();const c=a.indexOf(o),u=o.match(qu);if(u){o=o.replace(qu,"").trim();const d=u[1].trim();let f;if(d&&(f=s.indexOf(d,c+o.length),r.key=l(d,f,!0)),u[2]){const p=u[2].trim();p&&(r.index=l(p,s.indexOf(p,r.key?f+d.length:c+o.length),!0))}}return o&&(r.value=l(o,c,!0)),r}function It(e,t){return tn.slice(e,t)}function Ku(e){ut.inSFCRoot&&(Pt.innerLoc=pt(e+1,e+1)),mo(Pt);const{tag:t,ns:s}=Pt;s===0&&Ve.isPreTag(t)&&Tc++,Ve.isVoidTag(t)?ll(Pt,e):(nt.unshift(Pt),(s===1||s===2)&&(ut.inXML=!0)),Pt=null}function Qi(e,t,s){{const i=nt[0]&&nt[0].tag;i!=="script"&&i!=="style"&&e.includes("&")&&(e=Ve.decodeEntities(e,!1))}const n=nt[0]||Ei,a=n.children[n.children.length-1];a&&a.type===2?(a.content+=e,Mn(a.loc,s)):n.children.push({type:2,content:e,loc:pt(t,s)})}function ll(e,t,s=!1){s?Mn(e.loc,gh(t,60)):Mn(e.loc,Zy(t,62)+1),ut.inSFCRoot&&(e.children.length?e.innerLoc.end=Pe({},e.children[e.children.length-1].loc.end):e.innerLoc.end=Pe({},e.innerLoc.start),e.innerLoc.source=It(e.innerLoc.start.offset,e.innerLoc.end.offset));const{tag:n,ns:a,children:i}=e;if(yn||(n==="slot"?e.tagType=2:Gu(e)?e.tagType=3:Yy(e)&&(e.tagType=1)),ut.inRCDATA||(e.children=mh(i)),a===0&&Ve.isIgnoreNewlineTag(n)){const l=i[0];l&&l.type===2&&(l.content=l.content.replace(/^\r?\n/,""))}a===0&&Ve.isPreTag(n)&&Tc--,go===e&&(yn=ut.inVPre=!1,go=null),ut.inXML&&(nt[0]?nt[0].ns:Ve.ns)===0&&(ut.inXML=!1);{const l=e.props;if(!ut.inSFCRoot&&Hn("COMPILER_NATIVE_TEMPLATE",Ve)&&e.tag==="template"&&!Gu(e)){const o=nt[0]||Ei,c=o.children.indexOf(e);o.children.splice(c,1,...e.children)}const r=l.find(o=>o.type===6&&o.name==="inline-template");r&&Ti("COMPILER_INLINE_TEMPLATE",Ve,r.loc)&&e.children.length&&(r.value={type:2,content:It(e.children[0].loc.start.offset,e.children[e.children.length-1].loc.end.offset),loc:r.loc})}}function Zy(e,t){let s=e;for(;tn.charCodeAt(s)!==t&&s<tn.length-1;)s++;return s}function gh(e,t){let s=e;for(;tn.charCodeAt(s)!==t&&s>=0;)s--;return s}const Jy=new Set(["if","else","else-if","for","slot"]);function Gu({tag:e,props:t}){if(e==="template"){for(let s=0;s<t.length;s++)if(t[s].type===7&&Jy.has(t[s].name))return!0}return!1}function Yy({tag:e,props:t}){if(Ve.isCustomElement(e))return!1;if(e==="component"||Qy(e.charCodeAt(0))||rh(e)||Ve.isBuiltInComponent&&Ve.isBuiltInComponent(e)||Ve.isNativeTag&&!Ve.isNativeTag(e))return!0;for(let s=0;s<t.length;s++){const n=t[s];if(n.type===6){if(n.name==="is"&&n.value){if(n.value.content.startsWith("vue:"))return!0;if(Ti("COMPILER_IS_ON_ELEMENT",Ve,n.loc))return!0}}else if(n.name==="bind"&&Dn(n.arg,"is")&&Ti("COMPILER_IS_ON_ELEMENT",Ve,n.loc))return!0}return!1}function Qy(e){return e>64&&e<91}const Xy=/\r\n/g;function mh(e){const t=Ve.whitespace!=="preserve";let s=!1;for(let n=0;n<e.length;n++){const a=e[n];if(a.type===2)if(Tc)a.content=a.content.replace(Xy,`
`);else if(fh(a.content)){const i=e[n-1]&&e[n-1].type,l=e[n+1]&&e[n+1].type;!i||!l||t&&(i===3&&(l===3||l===1)||i===1&&(l===3||l===1&&e0(a.content)))?(s=!0,e[n]=null):a.content=" "}else t&&(a.content=vh(a.content))}return s?e.filter(Boolean):e}function e0(e){for(let t=0;t<e.length;t++){const s=e.charCodeAt(t);if(s===10||s===13)return!0}return!1}function vh(e){let t="",s=!1;for(let n=0;n<e.length;n++)as(e.charCodeAt(n))?s||(t+=" ",s=!0):(t+=e[n],s=!1);return t}function mo(e){(nt[0]||Ei).children.push(e)}function pt(e,t){return{start:ut.getPos(e),end:t==null?t:ut.getPos(t),source:t==null?t:It(e,t)}}function t0(e){return pt(e.start.offset,e.end.offset)}function Mn(e,t){e.end=ut.getPos(t),e.source=It(e.start.offset,t)}function s0(e){const t={type:6,name:e.rawName,nameLoc:pt(e.loc.start.offset,e.loc.start.offset+e.rawName.length),value:void 0,loc:e.loc};if(e.exp){const s=e.exp.loc;s.end.offset<e.loc.end.offset&&(s.start.offset--,s.start.column--,s.end.offset++,s.end.column++),t.value={type:2,content:e.exp.content,loc:s}}return t}function rl(e,t=!1,s,n=0,a=0){return Ne(e,t,s,n)}function Bs(e,t,s){Ve.onError(at(e,pt(t,t)))}function n0(){ut.reset(),Pt=null,$e=null,Jt="",js=-1,Ln=-1,nt.length=0}function a0(e,t){if(n0(),tn=e,Ve=Pe({},hh),t){let a;for(a in t)t[a]!=null&&(Ve[a]=t[a])}ut.mode=Ve.parseMode==="html"?1:Ve.parseMode==="sfc"?2:0,ut.inXML=Ve.ns===1||Ve.ns===2;const s=t&&t.delimiters;s&&(ut.delimiterOpen=Ll(s[0]),ut.delimiterClose=Ll(s[1]));const n=Ei=Ny([],e);return ut.parse(tn),n.loc=pt(0,e.length),n.children=mh(n.children),Ei=null,n}function i0(e,t){ol(e,void 0,t,!!bh(e))}function bh(e){const t=e.children.filter(s=>s.type!==3);return t.length===1&&t[0].type===1&&!Dl(t[0])?t[0]:null}function ol(e,t,s,n=!1,a=!1){const{children:i}=e,l=[];for(let u=0;u<i.length;u++){const d=i[u];if(d.type===1&&d.tagType===0){const f=n?0:is(d,s);if(f>0){if(f>=2){d.codegenNode.patchFlag=-1,l.push(d);continue}}else{const p=d.codegenNode;if(p.type===13){const b=p.patchFlag;if((b===void 0||b===512||b===1)&&xh(d,s)>=2){const g=_h(d);g&&(p.props=s.hoist(g))}p.dynamicProps&&(p.dynamicProps=s.hoist(p.dynamicProps))}}}else if(d.type===12&&(n?0:is(d,s))>=2){d.codegenNode.type===14&&d.codegenNode.arguments.length>0&&d.codegenNode.arguments.push("-1"),l.push(d);continue}if(d.type===1){const f=d.tagType===1;f&&s.scopes.vSlot++,ol(d,e,s,!1,a),f&&s.scopes.vSlot--}else if(d.type===11)ol(d,e,s,d.children.length===1,!0);else if(d.type===9)for(let f=0;f<d.branches.length;f++)ol(d.branches[f],e,s,d.branches[f].children.length===1,a)}let r=!1;if(l.length===i.length&&e.type===1){if(e.tagType===0&&e.codegenNode&&e.codegenNode.type===13&&he(e.codegenNode.children))e.codegenNode.children=o(Bn(e.codegenNode.children)),r=!0;else if(e.tagType===1&&e.codegenNode&&e.codegenNode.type===13&&e.codegenNode.children&&!he(e.codegenNode.children)&&e.codegenNode.children.type===15){const u=c(e.codegenNode,"default");u&&(u.returns=o(Bn(u.returns)),r=!0)}else if(e.tagType===3&&t&&t.type===1&&t.tagType===1&&t.codegenNode&&t.codegenNode.type===13&&t.codegenNode.children&&!he(t.codegenNode.children)&&t.codegenNode.children.type===15){const u=fs(e,"slot",!0),d=u&&u.arg&&c(t.codegenNode,u.arg);d&&(d.returns=o(Bn(d.returns)),r=!0)}}if(!r)for(const u of l)u.codegenNode=s.cache(u.codegenNode);function o(u){const d=s.cache(u);return d.needArraySpread=!0,d}function c(u,d){if(u.children&&!he(u.children)&&u.children.type===15){const f=u.children.properties.find(p=>p.key===d||p.key.content===d);return f&&f.value}}l.length&&s.transformHoist&&s.transformHoist(i,s,e)}function is(e,t){const{constantCache:s}=t;switch(e.type){case 1:if(e.tagType!==0)return 0;const n=s.get(e);if(n!==void 0)return n;const a=e.codegenNode;if(a.type!==13||a.isBlock&&e.tag!=="svg"&&e.tag!=="foreignObject"&&e.tag!=="math")return 0;if(a.patchFlag===void 0){let l=3;const r=xh(e,t);if(r===0)return s.set(e,0),0;r<l&&(l=r);for(let o=0;o<e.children.length;o++){const c=is(e.children[o],t);if(c===0)return s.set(e,0),0;c<l&&(l=c)}if(l>1)for(let o=0;o<e.props.length;o++){const c=e.props[o];if(c.type===7&&c.name==="bind"&&c.exp){const u=is(c.exp,t);if(u===0)return s.set(e,0),0;u<l&&(l=u)}}if(a.isBlock){for(let o=0;o<e.props.length;o++)if(e.props[o].type===7)return s.set(e,0),0;t.removeHelper(qn),t.removeHelper(Ia(t.inSSR,a.isComponent)),a.isBlock=!1,t.helper(Ra(t.inSSR,a.isComponent))}return s.set(e,l),l}else return s.set(e,0),0;case 2:case 3:return 3;case 9:case 11:case 10:return 0;case 5:case 12:return is(e.content,t);case 4:return e.constType;case 8:let i=3;for(let l=0;l<e.children.length;l++){const r=e.children[l];if(Ae(r)||Bt(r))continue;const o=is(r,t);if(o===0)return 0;o<i&&(i=o)}return i;case 20:return 2;default:return 0}}const l0=new Set([mc,vc,wi,Fi]);function yh(e,t){if(e.type===14&&!Ae(e.callee)&&l0.has(e.callee)){const s=e.arguments[0];if(s.type===4)return is(s,t);if(s.type===14)return yh(s,t)}return 0}function xh(e,t){let s=3;const n=_h(e);if(n&&n.type===15){const{properties:a}=n;for(let i=0;i<a.length;i++){const{key:l,value:r}=a[i],o=is(l,t);if(o===0)return o;o<s&&(s=o);let c;if(r.type===4?c=is(r,t):r.type===14?c=yh(r,t):c=0,c===0)return c;c<s&&(s=c)}}return s}function _h(e){const t=e.codegenNode;if(t.type===13)return t.props}function r0(e,{filename:t="",prefixIdentifiers:s=!1,hoistStatic:n=!1,hmr:a=!1,cacheHandlers:i=!1,nodeTransforms:l=[],directiveTransforms:r={},transformHoist:o=null,isBuiltInComponent:c=Ot,isCustomElement:u=Ot,expressionPlugins:d=[],scopeId:f=null,slotted:p=!0,ssr:b=!1,inSSR:g=!1,ssrCssVars:T="",bindingMetadata:O=Fe,inline:y=!1,isTS:v=!1,onError:x=kc,onWarn:w=lh,compatConfig:I}){const L=t.replace(/\?.*$/,"").match(/([^/\\]+)\.\w+$/),k={filename:t,selfName:L&&Zn(Qe(L[1])),prefixIdentifiers:s,hoistStatic:n,hmr:a,cacheHandlers:i,nodeTransforms:l,directiveTransforms:r,transformHoist:o,isBuiltInComponent:c,isCustomElement:u,expressionPlugins:d,scopeId:f,slotted:p,ssr:b,inSSR:g,ssrCssVars:T,bindingMetadata:O,inline:y,isTS:v,onError:x,onWarn:w,compatConfig:I,root:e,helpers:new Map,components:new Set,directives:new Set,hoists:[],imports:[],cached:[],constantCache:new WeakMap,vForMemoKeyedNodes:new WeakSet,temps:0,identifiers:Object.create(null),scopes:{vFor:0,vSlot:0,vPre:0,vOnce:0},parent:null,grandParent:null,currentNode:e,childIndex:0,inVOnce:!1,helper(A){const D=k.helpers.get(A)||0;return k.helpers.set(A,D+1),A},removeHelper(A){const D=k.helpers.get(A);if(D){const B=D-1;B?k.helpers.set(A,B):k.helpers.delete(A)}},helperString(A){return`_${Ea[k.helper(A)]}`},replaceNode(A){k.parent.children[k.childIndex]=k.currentNode=A},removeNode(A){const D=k.parent.children,B=A?D.indexOf(A):k.currentNode?k.childIndex:-1;!A||A===k.currentNode?(k.currentNode=null,k.onNodeRemoved()):k.childIndex>B&&(k.childIndex--,k.onNodeRemoved()),k.parent.children.splice(B,1)},onNodeRemoved:Ot,addIdentifiers(A){},removeIdentifiers(A){},hoist(A){Ae(A)&&(A=Ne(A)),k.hoists.push(A);const D=Ne(`_hoisted_${k.hoists.length}`,!1,A.loc,2);return D.hoisted=A,D},cache(A,D=!1,B=!1){const M=Ly(k.cached.length,A,D,B);return k.cached.push(M),M}};return k.filters=new Set,k}function o0(e,t){const s=r0(e,t);or(e,s),t.hoistStatic&&i0(e,s),t.ssr||c0(e,s),e.helpers=new Set([...s.helpers.keys()]),e.components=[...s.components],e.directives=[...s.directives],e.imports=s.imports,e.hoists=s.hoists,e.temps=s.temps,e.cached=s.cached,e.transformed=!0,e.filters=[...s.filters]}function c0(e,t){const{helper:s}=t,{children:n}=e;if(n.length===1){const a=bh(e);if(a&&a.codegenNode){const i=a.codegenNode;i.type===13&&_c(i,t),e.codegenNode=i}else e.codegenNode=n[0]}else if(n.length>1){let a=64;e.codegenNode=Si(t,s(ki),void 0,e.children,a,void 0,void 0,!0,void 0,!1)}}function u0(e,t){let s=0;const n=()=>{s--};for(;s<e.children.length;s++){const a=e.children[s];Ae(a)||(t.grandParent=t.parent,t.parent=e,t.childIndex=s,t.onNodeRemoved=n,or(a,t))}}function or(e,t){t.currentNode=e;const{nodeTransforms:s}=t,n=[];for(let i=0;i<s.length;i++){const l=s[i](e,t);if(l&&(he(l)?n.push(...l):n.push(l)),t.currentNode)e=t.currentNode;else return}switch(e.type){case 3:t.ssr||t.helper(Pi);break;case 5:t.ssr||t.helper(lr);break;case 9:for(let i=0;i<e.branches.length;i++)or(e.branches[i],t);break;case 10:case 11:case 1:case 0:u0(e,t);break}t.currentNode=e;let a=n.length;for(;a--;)n[a]()}function kh(e,t){const s=Ae(e)?n=>n===e:n=>e.test(n);return(n,a)=>{if(n.type===1){const{props:i}=n;if(n.tagType===3&&i.some(jy))return;const l=[];for(let r=0;r<i.length;r++){const o=i[r];if(o.type===7&&s(o.name)){i.splice(r,1),r--;const c=t(n,o,a);c&&l.push(c)}}return l}}}const cr="/*@__PURE__*/",wh=e=>`${Ea[e]}: _${Ea[e]}`;function d0(e,{mode:t="function",prefixIdentifiers:s=t==="module",sourceMap:n=!1,filename:a="template.vue.html",scopeId:i=null,optimizeImports:l=!1,runtimeGlobalName:r="Vue",runtimeModuleName:o="vue",ssrRuntimeModuleName:c="vue/server-renderer",ssr:u=!1,isTS:d=!1,inSSR:f=!1}){const p={mode:t,prefixIdentifiers:s,sourceMap:n,filename:a,scopeId:i,optimizeImports:l,runtimeGlobalName:r,runtimeModuleName:o,ssrRuntimeModuleName:c,ssr:u,isTS:d,inSSR:f,source:e.source,code:"",column:1,line:1,offset:0,indentLevel:0,pure:!1,map:void 0,helper(g){return`_${Ea[g]}`},push(g,T=-2,O){p.code+=g},indent(){b(++p.indentLevel)},deindent(g=!1){g?--p.indentLevel:b(--p.indentLevel)},newline(){b(p.indentLevel)}};function b(g){p.push(`
`+"  ".repeat(g),0)}return p}function f0(e,t={}){const s=d0(e,t);t.onContextCreated&&t.onContextCreated(s);const{mode:n,push:a,prefixIdentifiers:i,indent:l,deindent:r,newline:o,scopeId:c,ssr:u}=s,d=Array.from(e.helpers),f=d.length>0,p=!i&&n!=="module";p0(e,s);const g=u?"ssrRender":"render",O=(u?["_ctx","_push","_parent","_attrs"]:["_ctx","_cache"]).join(", ");if(a(`function ${g}(${O}) {`),l(),p&&(a("with (_ctx) {"),l(),f&&(a(`const { ${d.map(wh).join(", ")} } = _Vue
`,-1),o())),e.components.length&&(Or(e.components,"component",s),(e.directives.length||e.temps>0)&&o()),e.directives.length&&(Or(e.directives,"directive",s),e.temps>0&&o()),e.filters&&e.filters.length&&(o(),Or(e.filters,"filter",s),o()),e.temps>0){a("let ");for(let y=0;y<e.temps;y++)a(`${y>0?", ":""}_temp${y}`)}return(e.components.length||e.directives.length||e.temps)&&(a(`
...
`,-1)}h0(e.hoists,t),i(),a("return ")}function Or(e,t,{helper:s,push:n,newline:a,isTS:i}){const l=s(t==="filter"?pc:t==="component"?uc:fc);for(let r=0;r<e.length;r++){let o=e[r];const c=o.endsWith("__self");c&&(o=o.slice(0,-6)),n(`const ${Ci(o,t)} = ${l}(${JSON.stringify(o)}${c?", true":""})${i?"!":""}`),r<e.length-1&&a()}}function h0(e,t){if(!e.length)return;t.pure=!0;const{push:s,newline:n}=t;n();for(let a=0;a<e.length;a++){const i=e[a];i&&(s(`const _hoisted_${a+1} = `),Ut(i,t),n())}t.pure=!1}function Cc(e,t){const s=e.length>3||!1;t.push("["),s&&t.indent(),$i(e,t,s),s&&t.deindent(),t.push("]")}function $i(e,t,s=!1,n=!0){const{push:a,newline:i}=t;for(let l=0;l<e.length;l++){const r=e[l];Ae(r)?a(r,-3):he(r)?Cc(r,t):Ut(r,t),l<e.length-1&&(s?(n&&a(","),i()):n&&a(", "))}}function Ut(e,t){if(Ae(e)){t.push(e,-3);return}if(Bt(e)){t.push(t.helper(e));return}switch(e.type){case 1:case 9:case 11:Ut(e.codegenNode,t);break;case 2:g0(e,t);break;case 4:Sh(e,t);break;case 5:m0(e,t);break;case 12:Ut(e.codegenNode,t);break;case 8:Th(e,t);break;case 3:b0(e,t);break;case 13:y0(e,t);break;case 14:_0(e,t);break;case 15:k0(e,t);break;case 17:w0(e,t);break;case 18:S0(e,t);break;case 19:T0(e,t);break;case 20:C0(e,t);break;case 21:$i(e.body,t,!0,!1);break}}function g0(e,t){t.push(JSON.stringify(e.content),-3,e)}function Sh(e,t){const{content:s,isStatic:n}=e;t.push(n?JSON.stringify(s):s,-3,e)}function m0(e,t){const{push:s,helper:n,pure:a}=t;a&&s(cr),s(`${n(lr)}(`),Ut(e.content,t),s(")")}function Th(e,t){for(let s=0;s<e.children.length;s++){const n=e.children[s];Ae(n)?t.push(n,-3):Ut(n,t)}}function v0(e,t){const{push:s}=t;if(e.type===8)s("["),Th(e,t),s("]");else if(e.isStatic){const n=wc(e.content)?e.content:JSON.stringify(e.content);s(n,-2,e)}else s(`[${e.content}]`,-3,e)}function b0(e,t){const{push:s,helper:n,pure:a}=t;a&&s(cr),s(`${n(Pi)}(${JSON.stringify(e.content)})`,-3,e)}function y0(e,t){const{push:s,helper:n,pure:a}=t,{tag:i,props:l,children:r,patchFlag:o,dynamicProps:c,directives:u,isBlock:d,disableTracking:f,isComponent:p}=e;let b;o&&(b=String(o)),u&&s(n(hc)+"("),d&&s(`(${n(qn)}(${f?"true":""}), `),a&&s(cr);const g=d?Ia(t.inSSR,p):Ra(t.inSSR,p);s(n(g)+"(",-2,e),$i(x0([i,l,r,b,c]),t),s(")"),d&&s(")"),u&&(s(", "),Ut(u,t),s(")"))}function x0(e){let t=e.length;for(;t--&&e[t]==null;);return e.slice(0,t+1).map(s=>s||"null")}function _0(e,t){const{push:s,helper:n,pure:a}=t,i=Ae(e.callee)?e.callee:n(e.callee);a&&s(cr),s(i+"(",-2,e),$i(e.arguments,t),s(")")}function k0(e,t){const{push:s,indent:n,deindent:a,newline:i}=t,{properties:l}=e;if(!l.length){s("{}",-2,e);return}const r=l.length>1||!1;s(r?"{":"{ "),r&&n();for(let o=0;o<l.length;o++){const{key:c,value:u}=l[o];v0(c,t),s(": "),Ut(u,t),o<l.length-1&&(s(","),i())}r&&a(),s(r?"}":" }")}function w0(e,t){Cc(e.elements,t)}function S0(e,t){const{push:s,indent:n,deindent:a}=t,{params:i,returns:l,body:r,newline:o,isSlot:c}=e;c&&s(`_${Ea[yc]}(`),s("(",-2,e),he(i)?$i(i,t):i&&Ut(i,t),s(") => "),(o||r)&&(s("{"),n()),l?(o&&s("return "),he(l)?Cc(l,t):Ut(l,t)):r&&Ut(r,t),(o||r)&&(a(),s("}")),c&&(e.isNonScopedSlot&&s(", undefined, true"),s(")"))}function T0(e,t){const{test:s,consequent:n,alternate:a,newline:i}=e,{push:l,indent:r,deindent:o,newline:c}=t;if(s.type===4){const d=!wc(s.content);d&&l("("),Sh(s,t),d&&l(")")}else l("("),Ut(s,t),l(")");i&&r(),t.indentLevel++,i||l(" "),l("? "),Ut(n,t),t.indentLevel--,i&&c(),i||l(" "),l(": ");const u=a.type===19;u||t.indentLevel++,Ut(a,t),u||t.indentLevel--,i&&o(!0)}function C0(e,t){const{push:s,helper:n,indent:a,deindent:i,newline:l}=t,{needPauseTracking:r,needArraySpread:o}=e;o&&s("[...("),s(`_cache[${e.index}] || (`),r&&(a(),s(`${n(Nl)}(-1`),e.inVOnce&&s(", true"),s("),"),l(),s("(")),s(`_cache[${e.index}] = `),Ut(e.value,t),r&&(s(`).cacheIndex = ${e.index},`),l(),s(`${n(Nl)}(1),`),l(),s(`_cache[${e.index}]`),i()),s(")"),o&&s(")]")}new RegExp("\\b"+"arguments,await,break,case,catch,class,const,continue,debugger,default,delete,do,else,export,extends,finally,for,function,if,import,let,new,return,super,switch,throw,try,var,void,while,with,yield".split(",").join("\\b|\\b")+"\\b");const E0=kh(/^(?:if|else|else-if)$/,(e,t,s)=>A0(e,t,s,(n,a,i)=>{const l=s.parent.children;let r=l.indexOf(n),o=0;for(;r-->=0;){const c=l[r];c&&c.type===9&&(o+=c.branches.length)}return()=>{if(i)n.codegenNode=Zu(a,o,s);else{const c=R0(n.codegenNode);c.alternate=Zu(a,o+n.branches.length-1,s)}}}));function A0(e,t,s,n){if(t.name!=="else"&&(!t.exp||!t.exp.content.trim())){const a=t.exp?t.exp.loc:e.loc;s.onError(at(28,t.loc)),t.exp=Ne("true",!1,a)}if(t.name==="if"){const a=Wu(e,t),i={type:9,loc:t0(e.loc),branches:[a]};if(s.replaceNode(i),n)return n(i,a,!0)}else{const a=s.parent.children;let i=a.indexOf(e);for(;i-->=-1;){const l=a[i];if(l&&ph(l)){s.removeNode(l);continue}if(l&&l.type===9){(t.name==="else-if"||t.name==="else")&&l.branches[l.branches.length-1].condition===void 0&&s.onError(at(30,e.loc)),s.removeNode();const r=Wu(e,t);l.branches.push(r);const o=n&&n(l,r,!1);or(r,s),o&&o(),s.currentNode=null}else s.onError(at(30,e.loc));break}}}function Wu(e,t){const s=e.tagType===3;return{type:10,loc:e.loc,condition:t.name==="else"?void 0:t.exp,children:s&&!fs(e,"for")?e.children:[e],userKey:rr(e,"key"),isTemplateIf:s}}function Zu(e,t,s){return e.condition?ho(e.condition,Ju(e,t,s),Tt(s.helper(Pi),['""',"true"])):Ju(e,t,s)}function Ju(e,t,s){const{helper:n}=s,a=bt("key",Ne(`${t}`,!1,cs,2)),{children:i}=e,l=i[0];if(i.length!==1||l.type!==1)if(i.length===1&&l.type===11){const o=l.codegenNode;return Ml(o,a,s),o}else return Si(s,n(ki),ps([a]),i,64,void 0,void 0,!0,!1,!1,e.loc);else{const o=l.codegenNode,c=qy(o);return c.type===13&&_c(c,s),Ml(c,a,s),o}}function R0(e){for(;;)if(e.type===19)if(e.alternate.type===19)e=e.alternate;else return e;else e.type===20&&(e=e.value)}const I0=kh("for",(e,t,s)=>{const{helper:n,removeHelper:a}=s;return N0(e,t,s,i=>{const l=Tt(n(gc),[i.source]),r=Ol(e),o=fs(e,"memo"),c=rr(e,"key",!1,!0);c&&c.type;let u=c&&(c.type===6?c.value?Ne(c.value.content,!0):void 0:c.exp);const d=u?bt("key",u):null,f=i.source.type===4&&i.source.constType>0,p=f?64:c?128:256;return i.codegenNode=Si(s,n(ki),void 0,l,p,void 0,void 0,!0,!f,!1,e.loc),()=>{let b;const{children:g}=i,T=g.length!==1||g[0].type!==1,O=Dl(e)?e:r&&e.children.length===1&&Dl(e.children[0])?e.children[0]:null;if(O?(b=O.codegenNode,r&&d&&Ml(b,d,s)):T?b=Si(s,n(ki),d?ps([d]):void 0,e.children,64,void 0,void 0,!0,void 0,!1):(b=g[0].codegenNode,r&&d&&Ml(b,d,s),b.isBlock!==!f&&(b.isBlock?(a(qn),a(Ia(s.inSSR,b.isComponent))):a(Ra(s.inSSR,b.isComponent))),b.isBlock=!f,b.isBlock?(n(qn),n(Ia(s.inSSR,b.isComponent))):n(Ra(s.inSSR,b.isComponent))),o){const y=Aa(vo(i.parseResult,[Ne("_cached")]));y.body=Oy([ks(["const _memo = (",o.exp,")"]),ks(["if (_cached && _cached.el",...u?[" && _cached.key === ",u]:[],` && ${s.helperString(ih)}(_cached, _memo)) return _cached`]),ks(["const _item = ",b]),Ne("_item.memo = _memo"),Ne("return _item")]),l.arguments.push(y,Ne("_cache"),Ne(String(s.cached.length))),s.cached.push(null)}else l.arguments.push(Aa(vo(i.parseResult),b,!0))}})});function N0(e,t,s,n){if(!t.exp){s.onError(at(31,t.loc));return}const a=t.forParseResult;if(!a){s.onError(at(32,t.loc));return}Ch(a);const{addIdentifiers:i,removeIdentifiers:l,scopes:r}=s,{source:o,value:c,key:u,index:d}=a,f={type:11,loc:t.loc,source:o,valueAlias:c,keyAlias:u,objectIndexAlias:d,parseResult:a,children:Ol(e)?e.children:[e]};s.replaceNode(f),r.vFor++;const p=n&&n(f);return()=>{r.vFor--,p&&p()}}function Ch(e,t){e.finalized||(e.finalized=!0)}function vo({value:e,key:t,index:s},n=[]){return L0([e,t,s,...n])}function L0(e){let t=e.length;for(;t--&&!e[t];);return e.slice(0,t+1).map((s,n)=>s||Ne("_".repeat(n+1),!1))}const Yu=Ne("undefined",!1),O0=(e,t)=>{if(e.type===1&&(e.tagType===1||e.tagType===3)){const s=fs(e,"slot");if(s)return s.exp,t.scopes.vSlot++,()=>{t.scopes.vSlot--}}},D0=(e,t,s,n)=>Aa(e,s,!1,!0,s.length?s[0].loc:n);function M0(e,t,s=D0){t.helper(yc);const{children:n,loc:a}=e,i=[],l=[];let r=t.scopes.vSlot>0||t.scopes.vFor>0;const o=fs(e,"slot",!0);if(o){const{arg:T,exp:O}=o;T&&!ts(T)&&(r=!0),i.push(bt(T||Ne("default",!0),s(O,void 0,n,a)))}let c=!1,u=!1;const d=[],f=new Set;let p=0;for(let T=0;T<n.length;T++){const O=n[T];let y;if(!Ol(O)||!(y=fs(O,"slot",!0))){O.type!==3&&d.push(O);continue}if(o){t.onError(at(37,y.loc));break}c=!0;const{children:v,loc:x}=O,{arg:w=Ne("default",!0),exp:I,loc:L}=y;let k;ts(w)?k=w?w.content:"default":r=!0;const A=fs(O,"for"),D=s(I,A,v,x);let B,M;if(B=fs(O,"if"))r=!0,l.push(ho(B.exp,Xi(w,D,p++),Yu));else if(M=fs(O,/^else(?:-if)?$/,!0)){let S=T,U;for(;S--&&(U=n[S],!!ph(U)););if(U&&Ol(U)&&fs(U,/^(?:else-)?if$/)){let W=l[l.length-1];for(;W.alternate.type===19;)W=W.alternate;W.alternate=M.exp?ho(M.exp,Xi(w,D,p++),Yu):Xi(w,D,p++)}else t.onError(at(30,M.loc))}else if(A){r=!0;const S=A.forParseResult;S?(Ch(S),l.push(Tt(t.helper(gc),[S.source,Aa(vo(S),Xi(w,D),!0)]))):t.onError(at(32,A.loc))}else{if(k){if(f.has(k)){t.onError(at(38,L));continue}f.add(k),k==="default"&&(u=!0)}i.push(bt(w,D))}}if(!o){const T=(O,y)=>{const v=s(O,void 0,y,a);return t.compatConfig&&(v.isNonScopedSlot=!0),bt("default",v)};c?d.length&&!d.every(Sc)&&(u?t.onError(at(39,d[0].loc)):i.push(T(void 0,d))):i.push(T(void 0,n))}const b=r?2:cl(e.children)?3:1;let g=ps(i.concat(bt("_",Ne(b+"",!1))),a);return l.length&&(g=Tt(t.helper(ah),[g,Bn(l)])),{slots:g,hasDynamicSlots:r}}function Xi(e,t,s){const n=[bt("name",e),bt("fn",t)];return s!=null&&n.push(bt("key",Ne(String(s),!0))),ps(n)}function cl(e){for(let t=0;t<e.length;t++){const s=e[t];switch(s.type){case 1:if(s.tagType===2||cl(s.children))return!0;break;case 9:if(cl(s.branches))return!0;break;case 10:case 11:if(cl(s.children))return!0;break}}return!1}const Eh=new WeakMap,P0=(e,t)=>function(){if(e=t.currentNode,!(e.type===1&&(e.tagType===0||e.tagType===1)))return;const{tag:n,props:a}=e,i=e.tagType===1;let l=i?F0(e,t):`"${n}"`;const r=je(l)&&l.callee===dc;let o,c,u=0,d,f,p,b=r||l===ri||l===lc||!i&&(n==="svg"||n==="foreignObject"||n==="math");if(a.length>0){const g=Ah(e,t,void 0,i,r);o=g.props,u=g.patchFlag,f=g.dynamicPropNames;const T=g.directives;p=T&&T.length?Bn(T.map(O=>U0(O,t))):void 0,g.shouldUseBlock&&(b=!0)}if(e.children.length>0)if(l===Rl&&(b=!0,u|=1024),i&&l!==ri&&l!==Rl){const{slots:T,hasDynamicSlots:O}=M0(e,t);c=T,O&&(u|=1024)}else if(e.children.length===1&&l!==ri){const T=e.children[0],O=T.type,y=O===5||O===8;y&&is(T,t)===0&&(u|=1),y||O===2?c=T:c=e.children}else c=e.children;f&&f.length&&(d=B0(f)),e.codegenNode=Si(t,l,o,c,u===0?void 0:u,d,p,!!b,!1,i,e.loc)};function F0(e,t,s=!1){let{tag:n}=e;const a=bo(n),i=rr(e,"is",!1,!0);if(i)if(a||Hn("COMPILER_IS_ON_ELEMENT",t)){let r;if(i.type===6?r=i.value&&Ne(i.value.content,!0):(r=i.exp,r||(r=Ne("is",!1,i.arg.loc))),r)return Tt(t.helper(dc),[r])}else i.type===6&&i.value.content.startsWith("vue:")&&(n=i.value.content.slice(4));const l=rh(n)||t.isBuiltInComponent(n);return l?(s||t.helper(l),l):(t.helper(uc),t.components.add(n),Ci(n,"component"))}function Ah(e,t,s=e.props,n,a,i=!1){const{tag:l,loc:r,children:o}=e;let c=[];const u=[],d=[],f=o.length>0;let p=!1,b=0,g=!1,T=!1,O=!1,y=!1,v=!1,x=!1;const w=[],I=D=>{c.length&&(u.push(ps(Qu(c),r)),c=[]),D&&u.push(D)},L=()=>{t.scopes.vFor>0&&c.push(bt(Ne("ref_for",!0),Ne("true")))},k=({key:D,value:B})=>{if(ts(D)){const M=D.content,S=Gn(M);if(S&&(!n||a)&&M.toLowerCase()!=="onclick"&&M!=="onUpdate:modelValue"&&!Qs(M)&&(y=!0),S&&Qs(M)&&(x=!0),S&&B.type===14&&(B=B.arguments[0]),B.type===20||(B.type===4||B.type===8)&&is(B,t)>0)return;M==="ref"?g=!0:M==="class"?T=!0:M==="style"?O=!0:M!=="key"&&!w.includes(M)&&w.push(M),n&&(M==="class"||M==="style")&&!w.includes(M)&&w.push(M)}else v=!0};for(let D=0;D<s.length;D++){const B=s[D];if(B.type===6){const{loc:M,name:S,nameLoc:U,value:W}=B;let F=!0;if(S==="ref"&&(g=!0,L()),S==="is"&&(bo(l)||W&&W.content.startsWith("vue:")||Hn("COMPILER_IS_ON_ELEMENT",t)))continue;c.push(bt(Ne(S,!0,U),Ne(W?W.content:"",F,W?W.loc:M)))}else{const{name:M,arg:S,exp:U,loc:W,modifiers:F}=B,N=M==="bind",C=M==="on";if(M==="slot"){n||t.onError(at(40,W));continue}if(M==="once"||M==="memo"||M==="is"||N&&Dn(S,"is")&&(bo(l)||Hn("COMPILER_IS_ON_ELEMENT",t))||C&&i)continue;if((N&&Dn(S,"key")||C&&f&&Dn(S,"vue:before-update"))&&(p=!0),N&&Dn(S,"ref")&&L(),!S&&(N||C)){if(v=!0,U)if(N){if(I(),Hn("COMPILER_V_BIND_OBJECT_ORDER",t)){u.unshift(U);continue}L(),I(),u.push(U)}else I({type:14,loc:W,callee:t.helper(bc),arguments:n?[U]:[U,"true"]});else t.onError(at(N?34:35,W));continue}N&&F.some(ce=>ce.content==="prop")&&(b|=32);const j=t.directiveTransforms[M];if(j){const{props:ce,needRuntime:de}=j(B,e,t);!i&&ce.forEach(k),C&&S&&!ts(S)?I(ps(ce,r)):c.push(...ce),de&&(d.push(B),Bt(de)&&Eh.set(B,de))}else Ig(M)||(d.push(B),f&&(p=!0))}}let A;if(u.length?(I(),u.length>1?A=Tt(t.helper(Il),u,r):A=u[0]):c.length&&(A=ps(Qu(c),r)),v?b|=16:(T&&!n&&(b|=2),O&&!n&&(b|=4),w.length&&(b|=8),y&&(b|=32)),!p&&(b===0||b===32)&&(g||x||d.length>0)&&(b|=512),!t.inSSR&&A)switch(A.type){case 15:let D=-1,B=-1,M=!1;for(let W=0;W<A.properties.length;W++){const F=A.properties[W].key;ts(F)?F.content==="class"?D=W:F.content==="style"&&(B=W):F.isHandlerKey||(M=!0)}const S=A.properties[D],U=A.properties[B];M?A=Tt(t.helper(wi),[A]):(S&&!ts(S.value)&&(S.value=Tt(t.helper(mc),[S.value])),U&&(O||U.value.type===4&&U.value.content.trim()[0]==="["||U.value.type===17)&&(U.value=Tt(t.helper(vc),[U.value])));break;case 14:break;default:A=Tt(t.helper(wi),[Tt(t.helper(Fi),[A])]);break}return{props:A,directives:d,patchFlag:b,dynamicPropNames:w,shouldUseBlock:p}}function Qu(e){const t=new Map,s=[];for(let n=0;n<e.length;n++){const a=e[n];if(a.key.type===8||!a.key.isStatic){s.push(a);continue}const i=a.key.content,l=t.get(i);l?(i==="style"||i==="class"||Gn(i))&&$0(l,a):(t.set(i,a),s.push(a))}return s}function $0(e,t){e.value.type===17?e.value.elements.push(t.value):e.value=Bn([e.value,t.value],e.loc)}function U0(e,t){const s=[],n=Eh.get(e);n?s.push(t.helperString(n)):(t.helper(fc),t.directives.add(e.name),s.push(Ci(e.name,"directive")));const{loc:a}=e;if(e.exp&&s.push(e.exp),e.arg&&(e.exp||s.push("void 0"),s.push(e.arg)),Object.keys(e.modifiers).length){e.arg||(e.exp||s.push("void 0"),s.push("void 0"));const i=Ne("true",!1,a);s.push(ps(e.modifiers.map(l=>bt(l,i)),a))}return Bn(s,e.loc)}function B0(e){let t="[";for(let s=0,n=e.length;s<n;s++)t+=JSON.stringify(e[s]),s<n-1&&(t+=", ");return t+"]"}function bo(e){return e==="component"||e==="Component"}const H0=(e,t)=>{if(Dl(e)){const{children:s,loc:n}=e,{slotName:a,slotProps:i}=V0(e,t),l=[t.prefixIdentifiers?"_ctx.$slots":"$slots",a,"{}","undefined","true"];let r=2;i&&(l[2]=i,r=3),s.length&&(l[3]=Aa([],s,!1,!1,n),r=4),t.scopeId&&!t.slotted&&(r=5),l.splice(r),e.codegenNode=Tt(t.helper(nh),l,n)}};function V0(e,t){let s='"default"',n;const a=[];for(let i=0;i<e.props.length;i++){const l=e.props[i];if(l.type===6)l.value&&(l.name==="name"?s=JSON.stringify(l.value.content):(l.name=Qe(l.name),a.push(l)));else if(l.name==="bind"&&Dn(l.arg,"name")){if(l.exp)s=l.exp;else if(l.arg&&l.arg.type===4){const r=Qe(l.arg.content);s=l.exp=Ne(r,!1,l.arg.loc)}}else l.name==="bind"&&l.arg&&ts(l.arg)&&(l.arg.content=Qe(l.arg.content)),a.push(l)}if(a.length>0){const{props:i,directives:l}=Ah(e,t,a,!1,!1);n=i,l.length&&t.onError(at(36,l[0].loc))}return{slotName:s,slotProps:n}}const Rh=(e,t,s,n)=>{const{loc:a,modifiers:i,arg:l}=e;!e.exp&&!i.length&&s.onError(at(35,a));let r;if(l.type===4)if(l.isStatic){let d=l.content;d.startsWith("vue:")&&(d=`vnode-${d.slice(4)}`);const f=t.tagType!==0||d.startsWith("vnode")||!/[A-Z]/.test(d)?ga(Qe(d)):`on:${d}`;r=Ne(f,!0,l.loc)}else r=ks([`${s.helperString(po)}(`,l,")"]);else r=l,r.children.unshift(`${s.helperString(po)}(`),r.children.push(")");let o=e.exp;o&&!o.content.trim()&&(o=void 0);let c=s.cacheHandlers&&!o&&!s.inVOnce;if(o){const d=uh(o),f=!(d||Hy(o)),p=o.content.includes(";");(f||c&&d)&&(o=ks([`${f?"$event":"(...args)"} => ${p?"{":"("}`,o,p?"}":")"]))}let u={props:[bt(r,o||Ne("() => {}",!1,a))]};return n&&(u=n(u)),c&&(u.props[0].value=s.cache(u.props[0].value)),u.props.forEach(d=>d.key.isHandlerKey=!0),u},j0=(e,t,s)=>{const{modifiers:n,loc:a}=e,i=e.arg;let{exp:l}=e;return l&&l.type===4&&!l.content.trim()&&(l=void 0),i.type!==4?(i.children.unshift("("),i.children.push(') || ""')):i.isStatic||(i.content=i.content?`${i.content} || ""`:'""'),n.some(r=>r.content==="camel")&&(i.type===4?i.isStatic?i.content=Qe(i.content):i.content=`${s.helperString(fo)}(${i.content})`:(i.children.unshift(`${s.helperString(fo)}(`),i.children.push(")"))),s.inSSR||(n.some(r=>r.content==="prop")&&Xu(i,"."),n.some(r=>r.content==="attr")&&Xu(i,"^")),{props:[bt(i,l)]}},Xu=(e,t)=>{e.type===4?e.isStatic?e.content=t+e.content:e.content=`\`${t}\${${e.content}}\``:(e.children.unshift(`'${t}' + (`),e.children.push(")"))},z0=(e,t)=>{if(e.type===0||e.type===1||e.type===11||e.type===10)return()=>{const s=e.children;let n,a=!1;for(let i=0;i<s.length;i++){const l=s[i];if(Lr(l)){a=!0;for(let r=i+1;r<s.length;r++){const o=s[r];if(Lr(o))n||(n=s[i]=ks([l],l.loc)),n.children.push(" + ",o),s.splice(r,1),r--;else{n=void 0;break}}}}if(!(!a||s.length===1&&(e.type===0||e.type===1&&e.tagType===0&&!e.props.find(i=>i.type===7&&!t.directiveTransforms[i.name])&&e.tag!=="template")))for(let i=0;i<s.length;i++){const l=s[i];if(Lr(l)||l.type===8){const r=[];(l.type!==2||l.content!==" ")&&r.push(l),!t.ssr&&is(l,t)===0&&r.push("1"),s[i]={type:12,content:l,loc:l.loc,codegenNode:Tt(t.helper(cc),r)}}}}},ed=new WeakSet,q0=(e,t)=>{if(e.type===1&&fs(e,"once",!0))return ed.has(e)||t.inVOnce||t.inSSR?void 0:(ed.add(e),t.inVOnce=!0,t.helper(Nl),()=>{t.inVOnce=!1;const s=t.currentNode;s.codegenNode&&(s.codegenNode=t.cache(s.codegenNode,!0,!0))})},Ih=(e,t,s)=>{const{exp:n,arg:a}=e;if(!n)return s.onError(at(41,e.loc)),qa();const i=n.loc.source.trim(),l=n.type===4?n.content:i,r=s.bindingMetadata[i];if(r==="props"||r==="props-aliased")return s.onError(at(44,n.loc)),qa();if(r==="literal-const"||r==="setup-const")return s.onError(at(45,n.loc)),qa();if(!l.trim()||!uh(n))return s.onError(at(42,n.loc)),qa();const o=a||Ne("modelValue",!0),c=a?ts(a)?`onUpdate:${Qe(a.content)}`:ks(['"onUpdate:" + ',a]):"onUpdate:modelValue";let u;const d=s.isTS?"($event: any)":"$event";u=ks([`${d} => ((`,n,") = $event)"]);const f=[bt(o,e.exp),bt(c,u)];if(e.modifiers.length&&t.tagType===1){const p=e.modifiers.map(g=>g.content).map(g=>(wc(g)?g:JSON.stringify(g))+": true").join(", "),b=a?ts(a)?`${a.content}Modifiers`:ks([a,' + "Modifiers"']):"modelModifiers";f.push(bt(b,Ne(`{ ${p} }`,!1,e.loc,2)))}return qa(f)};function qa(e=[]){return{props:e}}const K0=/[\w).+\-_$\]]/,G0=(e,t)=>{Hn("COMPILER_FILTERS",t)&&(e.type===5?Pl(e.content,t):e.type===1&&e.props.forEach(s=>{s.type===7&&s.name!=="for"&&s.exp&&Pl(s.exp,t)}))};function Pl(e,t){if(e.type===4)td(e,t);else for(let s=0;s<e.children.length;s++){const n=e.children[s];typeof n=="object"&&(n.type===4?td(n,t):n.type===8?Pl(e,t):n.type===5&&Pl(n.content,t))}}function td(e,t){const s=e.content;let n=!1,a=!1,i=!1,l=!1,r=0,o=0,c=0,u=0,d,f,p,b,g=[];for(p=0;p<s.length;p++)if(f=d,d=s.charCodeAt(p),n)d===39&&f!==92&&(n=!1);else if(a)d===34&&f!==92&&(a=!1);else if(i)d===96&&f!==92&&(i=!1);else if(l)d===47&&f!==92&&(l=!1);else if(d===124&&s.charCodeAt(p+1)!==124&&s.charCodeAt(p-1)!==124&&!r&&!o&&!c)b===void 0?(u=p+1,b=s.slice(0,p).trim()):T();else{switch(d){case 34:a=!0;break;case 39:n=!0;break;case 96:i=!0;break;case 40:c++;break;case 41:c--;break;case 91:o++;break;case 93:o--;break;case 123:r++;break;case 125:r--;break}if(d===47){let O=p-1,y;for(;O>=0&&(y=s.charAt(O),y===" ");O--);(!y||!K0.test(y))&&(l=!0)}}b===void 0?b=s.slice(0,p).trim():u!==0&&T();function T(){g.push(s.slice(u,p).trim()),u=p+1}if(g.length){for(p=0;p<g.length;p++)b=W0(b,g[p],t);e.content=b,e.ast=void 0}}function W0(e,t,s){s.helper(pc);const n=t.indexOf("(");if(n<0)return s.filters.add(t),`${Ci(t,"filter")}(${e})`;{const a=t.slice(0,n),i=t.slice(n+1);return s.filters.add(a),`${Ci(a,"filter")}(${e}${i!==")"?","+i:i}`}}const sd=new WeakSet,Z0=(e,t)=>{if(e.type===1){const s=fs(e,"memo");return!s||sd.has(e)||t.inSSR?void 0:(sd.add(e),()=>{const n=e.codegenNode||t.currentNode.codegenNode;n&&n.type===13&&(e.tagType!==1&&_c(n,t),e.codegenNode=Tt(t.helper(xc),[s.exp,Aa(void 0,n),"_cache",String(t.cached.length)]),t.cached.push(null))})}},J0=(e,t)=>{if(e.type===1){for(const s of e.props)if(s.type===7&&s.name==="bind"&&(!s.exp||s.exp.type===4&&!s.exp.content.trim())&&s.arg){const n=s.arg;if(n.type!==4||!n.isStatic)t.onError(at(53,n.loc)),s.exp=Ne("",!0,n.loc);else{const a=Qe(n.content);(oh.test(a[0])||a[0]==="-")&&(s.exp=Ne(a,!1,n.loc))}}}};function Y0(e){return[[J0,q0,E0,Z0,I0,G0,H0,P0,O0,z0],{on:Rh,bind:j0,model:Ih}]}function Q0(e,t={}){const s=t.onError||kc,n=t.mode==="module";t.prefixIdentifiers===!0?s(at(48)):n&&s(at(49));const a=!1;t.cacheHandlers&&s(at(50)),t.scopeId&&!n&&s(at(51));const i=Pe({},t,{prefixIdentifiers:a}),l=Ae(e)?a0(e,i):e,[r,o]=Y0();return o0(l,Pe({},i,{nodeTransforms:[...r,...t.nodeTransforms||[]],directiveTransforms:Pe({},o,t.directiveTransforms||{})})),f0(l,i)}const X0=()=>({props:[]});/**
* @vue/compiler-dom v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const Nh=Symbol(""),Lh=Symbol(""),Oh=Symbol(""),Dh=Symbol(""),yo=Symbol(""),Mh=Symbol(""),Ph=Symbol(""),Fh=Symbol(""),$h=Symbol(""),Uh=Symbol("");Iy({[Nh]:"vModelRadio",[Lh]:"vModelCheckbox",[Oh]:"vModelText",[Dh]:"vModelSelect",[yo]:"vModelDynamic",[Mh]:"withModifiers",[Ph]:"withKeys",[Fh]:"vShow",[$h]:"Transition",[Uh]:"TransitionGroup"});let aa;function ex(e,t=!1){return aa||(aa=document.createElement("div")),t?(aa.innerHTML=`<div foo="${e.replace(/"/g,"&quot;")}">`,aa.children[0].getAttribute("foo")):(aa.innerHTML=e,aa.textContent)}const tx={parseMode:"html",isVoidTag:Gg,isNativeTag:e=>zg(e)||qg(e)||Kg(e),isPreTag:e=>e==="pre",isIgnoreNewlineTag:e=>e==="pre"||e==="textarea",decodeEntities:ex,isBuiltInComponent:e=>{if(e==="Transition"||e==="transition")return $h;if(e==="TransitionGroup"||e==="transition-group")return Uh},getNamespace(e,t,s){let n=t?t.ns:s;if(t&&n===2)if(t.tag==="annotation-xml"){if(e==="svg")return 1;t.props.some(a=>a.type===6&&a.name==="encoding"&&a.value!=null&&(a.value.content==="text/html"||a.value.content==="application/xhtml+xml"))&&(n=0)}else/^m(?:[ions]|text)$/.test(t.tag)&&e!=="mglyph"&&e!=="malignmark"&&(n=0);else t&&n===1&&(t.tag==="foreignObject"||t.tag==="desc"||t.tag==="title")&&(n=0);if(n===0){if(e==="svg")return 1;if(e==="math")return 2}return n}},sx=e=>{e.type===1&&e.props.forEach((t,s)=>{t.type===6&&t.name==="style"&&t.value&&(e.props[s]={type:7,name:"bind",arg:Ne("style",!0,t.loc),exp:nx(t.value.content,t.loc),modifiers:[],loc:t.loc})})},nx=(e,t)=>{const s=Yd(e);return Ne(JSON.stringify(s),!1,t,3)};function kn(e,t){return at(e,t)}const ax=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(kn(54,a)),t.children.length&&(s.onError(kn(55,a)),t.children.length=0),{props:[bt(Ne("innerHTML",!0,a),n||Ne("",!0))]}},ix=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(kn(56,a)),t.children.length&&(s.onError(kn(57,a)),t.children.length=0),{props:[bt(Ne("textContent",!0),n?is(n,s)>0?n:Tt(s.helperString(lr),[n],a):Ne("",!0))]}},lx=(e,t,s)=>{const n=Ih(e,t,s);if(!n.props.length||t.tagType===1)return n;e.arg&&s.onError(kn(59,e.arg.loc));const{tag:a}=t,i=s.isCustomElement(a);if(a==="input"||a==="textarea"||a==="select"||i){let l=Oh,r=!1;if(a==="input"||i){const o=rr(t,"type");if(o){if(o.type===7)l=yo;else if(o.value)switch(o.value.content){case"radio":l=Nh;break;case"checkbox":l=Lh;break;case"file":r=!0,s.onError(kn(60,e.loc));break}}else Vy(t)&&(l=yo)}else a==="select"&&(l=Dh);r||(n.needRuntime=s.helper(l))}else s.onError(kn(58,e.loc));return n.props=n.props.filter(l=>!(l.key.type===4&&l.key.content==="modelValue")),n},rx=os("passive,once,capture"),ox=os("stop,prevent,self,ctrl,shift,alt,meta,exact,middle"),cx=os("left,right"),Bh=os("onkeyup,onkeydown,onkeypress"),ux=(e,t,s,n)=>{const a=[],i=[],l=[];for(let r=0;r<t.length;r++){const o=t[r].content;o==="native"&&Ti("COMPILER_V_ON_NATIVE",s)||rx(o)?l.push(o):cx(o)?ts(e)?Bh(e.content.toLowerCase())?a.push(o):i.push(o):(a.push(o),i.push(o)):ox(o)?i.push(o):a.push(o)}return{keyModifiers:a,nonKeyModifiers:i,eventOptionModifiers:l}},nd=(e,t)=>ts(e)&&e.content.toLowerCase()==="onclick"?Ne(t,!0):e.type!==4?ks(["(",e,`) === "onClick" ? "${t}" : (`,e,")"]):e,dx=(e,t,s)=>Rh(e,t,s,n=>{const{modifiers:a}=e;if(!a.length)return n;let{key:i,value:l}=n.props[0];const{keyModifiers:r,nonKeyModifiers:o,eventOptionModifiers:c}=ux(i,a,s,e.loc);if(o.includes("right")&&(i=nd(i,"onContextmenu")),o.includes("middle")&&(i=nd(i,"onMouseup")),o.length&&(l=Tt(s.helper(Mh),[l,JSON.stringify(o)])),r.length&&(!ts(i)||Bh(i.content.toLowerCase()))&&(l=Tt(s.helper(Ph),[l,JSON.stringify(r)])),c.length){const u=c.map(Zn).join("");i=ts(i)?Ne(`${i.content}${u}`,!0):ks(["(",i,`) + "${u}"`])}return{props:[bt(i,l)]}}),fx=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(kn(62,a)),{props:[],needRuntime:s.helper(Fh)}},px=(e,t)=>{e.type===1&&e.tagType===0&&(e.tag==="script"||e.tag==="style")&&t.removeNode()},hx=[sx],gx={cloak:X0,html:ax,text:ix,model:lx,on:dx,show:fx};function mx(e,t={}){return Q0(e,Pe({},tx,t,{nodeTransforms:[px,...hx,...t.nodeTransforms||[]],directiveTransforms:Pe({},gx,t.directiveTransforms||{}),transformHoist:null}))}/**
* vue v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const ad=Object.create(null);function vx(e,t){if(!Ae(e))if(e.nodeType)e=e.innerHTML;else return Ot;const s=Og(e,t),n=ad[s];if(n)return n;if(e[0]==="#"){const r=document.querySelector(e);e=r?r.innerHTML:""}const a=Pe({hoistStatic:!0,onError:void 0,onWarn:Ot},t);!a.isCustomElement&&typeof customElements<"u"&&(a.isCustomElement=r=>!!customElements.get(r));const{code:i}=mx(e,a),l=new Function("Vue",i)(Sy);return l._rc=!0,ad[s]=l}wp(vx);const Fl=Sn({items:[]});let bx=1;function ur(e,t="info",s=3e3){const n=bx++;return Fl.items.push({id:n,message:String(e),type:t}),s>0&&setTimeout(()=>Ec(n),s),n}function Ec(e){const t=Fl.items.findIndex(s=>s.id===e);t>=0&&Fl.items.splice(t,1)}function _e(e,t="info",s=3e3){return ur(e,t,s)}_e.success=(e,t=3e3)=>ur(e,"success",t);_e.error=(e,t=5e3)=>ur(e,"error",t);_e.info=(e,t=3e3)=>ur(e,"info",t);_e.dismiss=Ec;const yx={setup(){return{state:Fl,dismiss:Ec}},template:`
  ...
  `},Ks=Sn({open:!1,title:"Confirm",message:"",confirmLabel:"Confirm",cancelLabel:"Cancel",danger:!1});let _a=null;function rs({title:e="Confirm",message:t="",confirmLabel:s="Confirm",cancelLabel:n="Cancel",danger:a=!1}={}){return _a&&_a(!1),Ks.title=e,Ks.message=t,Ks.confirmLabel=s,Ks.cancelLabel=n,Ks.danger=a,Ks.open=!0,new Promise(i=>{_a=i})}function id(e){Ks.open=!1,_a&&(_a(e),_a=null)}const xx={setup(){function e(t){Ks.open&&t.key==="Escape"&&(t.stopPropagation(),id(!1))}return He(()=>document.addEventListener("keydown",e,!0)),gt(()=>document.removeEventListener("keydown",e,!0)),{state:Ks,settle:id}},template:`
 ...
 * @license MIT
 */const ca=typeof document<"u";function Hh(e){return typeof e=="object"||"displayName"in e||"props"in e||"__vccOpts"in e}function _x(e){return e.__esModule||e[Symbol.toStringTag]==="Module"||e.default&&Hh(e.default)}const Ge=Object.assign;function Dr(e,t){const s={};for(const n in t){const a=t[n];s[n]=Ss(a)?a.map(e):e(a)}return s}const oi=()=>{},Ss=Array.isArray;function ld(e,t){const s={};for(const n in e)s[n]=n in t?t[n]:e[n];return s}const Vh=/#/g,kx=/&/g,wx=/\//g,Sx=/=/g,Tx=/\?/g,jh=/\+/g,Cx=/%5B/g,Ex=/%5D/g,zh=/%5E/g,Ax=/%60/g,qh=/%7B/g,Rx=/%7C/g,Kh=/%7D/g,Ix=/%20/g;function Ac(e){return e==null?"":encodeURI(""+e).replace(Rx,"|").replace(Cx,"[").replace(Ex,"]")}function Nx(e){return Ac(e).replace(qh,"{").replace(Kh,"}").replace(zh,"^")}function xo(e){return Ac(e).replace(jh,"%2B").replace(Ix,"+").replace(Vh,"%23").replace(kx,"%26").replace(Ax,"`").replace(qh,"{").replace(Kh,"}").replace(zh,"^")}function Lx(e){return xo(e).replace(Sx,"%3D")}function Ox(e){return Ac(e).replace(Vh,"%23").replace(Tx,"%3F")}function Dx(e){return Ox(e).replace(wx,"%2F")}function Ai(e){if(e==null)return null;try{return decodeURIComponent(""+e)}catch{}return""+e}const Mx=/\/$/,Px=e=>e.replace(Mx,"");function Mr(e,t,s="/"){let n,a={},i="",l="";const r=t.indexOf("#");let o=t.indexOf("?");return o=r>=0&&o>r?-1:o,o>=0&&(n=t.slice(0,o),i=t.slice(o,r>0?r:t.length),a=e(i.slice(1))),r>=0&&(n=n||t.slice(0,r),l=t.slice(r,t.length)),n=Bx(n??t,s),{fullPath:n+i+l,path:n,query:a,hash:Ai(l)}}function Fx(e,t){const s=t.query?e(t.query):"";return t.path+(s&&"?")+s+(t.hash||"")}function rd(e,t){return!t||!e.toLowerCase().startsWith(t.toLowerCase())?e:e.slice(t.length)||"/"}function $x(e,t,s){const n=t.matched.length-1,a=s.matched.length-1;return n>-1&&n===a&&Na(t.matched[n],s.matched[a])&&Gh(t.params,s.params)&&e(t.query)===e(s.query)&&t.hash===s.hash}function Na(e,t){return(e.aliasOf||e)===(t.aliasOf||t)}function Gh(e,t){if(Object.keys(e).length!==Object.keys(t).length)return!1;for(var s in e)if(!Ux(e[s],t[s]))return!1;return!0}function Ux(e,t){return Ss(e)?od(e,t):Ss(t)?od(t,e):(e==null?void 0:e.valueOf())===(t==null?void 0:t.valueOf())}function od(e,t){return Ss(t)?e.length===t.length&&e.every((s,n)=>s===t[n]):e.length===1&&e[0]===t}function Bx(e,t){if(e.startsWith("/"))return e;if(!e)return t;const s=t.split("/"),n=e.split("/"),a=n[n.length-1];(a===".."||a===".")&&n.push("");let i=s.length-1,l,r;for(l=0;l<n.length;l++)if(r=n[l],r!==".")if(r==="..")i>1&&i--;else break;return s.slice(0,i).join("/")+"/"+n.slice(l).join("/")}const hn={path:"/",name:void 0,params:{},query:{},hash:"",fullPath:"/",matched:[],meta:{},redirectedFrom:void 0};let _o=(function(e){return e.pop="pop",e.push="push",e})({}),Pr=(function(e){return e.back="back",e.forward="forward",e.unknown="",e})({});function Hx(e){if(!e)if(ca){const t=document.querySelector("base");e=t&&t.getAttribute("href")||"/",e=e.replace(/^\w+:\/\/[^\/]+/,"")}else e="/";return e[0]!=="/"&&e[0]!=="#"&&(e="/"+e),Px(e)}const Vx=/^[^#]+#/;function jx(e,t){return e.replace(Vx,"#")+t}function zx(e,t){const s=document.documentElement.getBoundingClientRect(),n=e.getBoundingClientRect();return{behavior:t.behavior,left:n.left-s.left-(t.left||0),top:n.top-s.top-(t.top||0)}}const dr=()=>({left:window.scrollX,top:window.scrollY});function qx(e){let t;if("el"in e){const s=e.el,n=typeof s=="string"&&s.startsWith("#"),a=typeof s=="string"?n?document.getElementById(s.slice(1)):document.querySelector(s):s;if(!a)return;t=zx(a,e)}else t=e;"scrollBehavior"in document.documentElement.style?window.scrollTo(t):window.scrollTo(t.left!=null?t.left:window.scrollX,t.top!=null?t.top:window.scrollY)}function cd(e,t){return(history.state?history.state.position-t:-1)+e}const ko=new Map;function Kx(e,t){ko.set(e,t)}function Gx(e){const t=ko.get(e);return ko.delete(e),t}function Wx(e){return typeof e=="string"||e&&typeof e=="object"}function Wh(e){return typeof e=="string"||typeof e=="symbol"}let ct=(function(e){return e[e.MATCHER_NOT_FOUND=1]="MATCHER_NOT_FOUND",e[e.NAVIGATION_GUARD_REDIRECT=2]="NAVIGATION_GUARD_REDIRECT",e[e.NAVIGATION_ABORTED=4]="NAVIGATION_ABORTED",e[e.NAVIGATION_CANCELLED=8]="NAVIGATION_CANCELLED",e[e.NAVIGATION_DUPLICATED=16]="NAVIGATION_DUPLICATED",e})({});const Zh=Symbol("");ct.MATCHER_NOT_FOUND+"",ct.NAVIGATION_GUARD_REDIRECT+"",ct.NAVIGATION_ABORTED+"",ct.NAVIGATION_CANCELLED+"",ct.NAVIGATION_DUPLICATED+"";function La(e,t){return Ge(new Error,{type:e,[Zh]:!0},t)}function Hs(e,t){return e instanceof Error&&Zh in e&&(t==null||!!(e.type&t))}const Zx=["params","query","hash"];function Jx(e){if(typeof e=="string")return e;if(e.path!=null)return e.path;const t={};for(const s of Zx)s in e&&(t[s]=e[s]);return JSON.stringify(t,null,2)}function Yx(e){const t={};if(e===""||e==="?")return t;const s=(e[0]==="?"?e.slice(1):e).split("&");for(let n=0;n<s.length;++n){const a=s[n].replace(jh," "),i=a.indexOf("="),l=Ai(i<0?a:a.slice(0,i)),r=i<0?null:Ai(a.slice(i+1));if(l in t){let o=t[l];Ss(o)||(o=t[l]=[o]),o.push(r)}else t[l]=r}return t}function ud(e){let t="";for(let s in e){const n=e[s];if(s=Lx(s),n==null){n!==void 0&&(t+=(t.length?"&":"")+s);continue}(Ss(n)?n.map(a=>a&&xo(a)):[n&&xo(n)]).forEach(a=>{a!==void 0&&(t+=(t.length?"&":"")+s,a!=null&&(t+="="+a))})}return t}function Qx(e){const t={};for(const s in e){const n=e[s];n!==void 0&&(t[s]=Ss(n)?n.map(a=>a==null?null:""+a):n==null?n:""+n)}return t}const Xx=Symbol(""),dd=Symbol(""),fr=Symbol(""),Rc=Symbol(""),wo=Symbol("");function Ka(){let e=[];function t(n){return e.push(n),()=>{const a=e.indexOf(n);a>-1&&e.splice(a,1)}}function s(){e=[]}return{add:t,list:()=>e.slice(),reset:s}}function xn(e,t,s,n,a,i=l=>l()){const l=n&&(n.enterCallbacks[a]=n.enterCallbacks[a]||[]);return()=>new Promise((r,o)=>{const c=f=>{f===!1?o(La(ct.NAVIGATION_ABORTED,{from:s,to:t})):f instanceof Error?o(f):Wx(f)?o(La(ct.NAVIGATION_GUARD_REDIRECT,{from:t,to:f})):(l&&n.enterCallbacks[a]===l&&typeof f=="function"&&l.push(f),r())},u=i(()=>e.call(n&&n.instances[a],t,s,c));let d=Promise.resolve(u);e.length<3&&(d=d.then(c)),d.catch(f=>o(f))})}function Fr(e,t,s,n,a=i=>i()){const i=[];for(const l of e)for(const r in l.components){let o=l.components[r];if(!(t!=="beforeRouteEnter"&&!l.instances[r]))if(Hh(o)){const c=(o.__vccOpts||o)[t];c&&i.push(xn(c,s,n,l,r,a))}else{let c=o();i.push(()=>c.then(u=>{if(!u)throw new Error(`Couldn't resolve component "${r}" at "${l.path}"`);const d=_x(u)?u.default:u;l.mods[r]=u,l.components[r]=d;const f=(d.__vccOpts||d)[t];return f&&xn(f,s,n,l,r,a)()}))}}return i}function e_(e,t){const s=[],n=[],a=[],i=Math.max(t.matched.length,e.matched.length);for(let l=0;l<i;l++){const r=t.matched[l];r&&(e.matched.find(c=>Na(c,r))?n.push(r):s.push(r));const o=e.matched[l];o&&(t.matched.find(c=>Na(c,o))||a.push(o))}return[s,n,a]}/*!
 * vue-router v4.6.4
 * (c) 2025 Eduardo San Martin Morote
 * @license MIT
 */let t_=()=>location.protocol+"//"+location.host;function Jh(e,t){const{pathname:s,search:n,hash:a}=t,i=e.indexOf("#");if(i>-1){let l=a.includes(e.slice(i))?e.slice(i).length:1,r=a.slice(l);return r[0]!=="/"&&(r="/"+r),rd(r,"")}return rd(s,e)+n+a}function s_(e,t,s,n){let a=[],i=[],l=null;const r=({state:f})=>{const p=Jh(e,location),b=s.value,g=t.value;let T=0;if(f){if(s.value=p,t.value=f,l&&l===b){l=null;return}T=g?f.position-g.position:0}else n(p);a.forEach(O=>{O(s.value,b,{delta:T,type:_o.pop,direction:T?T>0?Pr.forward:Pr.back:Pr.unknown})})};function o(){l=s.value}function c(f){a.push(f);const p=()=>{const b=a.indexOf(f);b>-1&&a.splice(b,1)};return i.push(p),p}function u(){if(document.visibilityState==="hidden"){const{history:f}=window;if(!f.state)return;f.replaceState(Ge({},f.state,{scroll:dr()}),"")}}function d(){for(const f of i)f();i=[],window.removeEventListener("popstate",r),window.removeEventListener("pagehide",u),document.removeEventListener("visibilitychange",u)}return window.addEventListener("popstate",r),window.addEventListener("pagehide",u),document.addEventListener("visibilitychange",u),{pauseListeners:o,listen:c,destroy:d}}function fd(e,t,s,n=!1,a=!1){return{back:e,current:t,forward:s,replaced:n,position:window.history.length,scroll:a?dr():null}}function n_(e){const{history:t,location:s}=window,n={value:Jh(e,s)},a={value:t.state};a.value||i(n.value,{back:null,current:n.value,forward:null,position:t.length-1,replaced:!0,scroll:null},!0);function i(o,c,u){const d=e.indexOf("#"),f=d>-1?(s.host&&document.querySelector("base")?e:e.slice(d))+o:t_()+e+o;try{t[u?"replaceState":"pushState"](c,"",f),a.value=c}catch(p){console.error(p),s[u?"replace":"assign"](f)}}function l(o,c){i(o,Ge({},t.state,fd(a.value.back,o,a.value.forward,!0),c,{position:a.value.position}),!0),n.value=o}function r(o,c){const u=Ge({},a.value,t.state,{forward:o,scroll:dr()});i(u.current,u,!0),i(o,Ge({},fd(n.value,o,null),{position:u.position+1},c),!1),n.value=o}return{location:n,state:a,push:r,replace:l}}function a_(e){e=Hx(e);const t=n_(e),s=s_(e,t.state,t.location,t.replace);function n(i,l=!0){l||s.pauseListeners(),history.go(i)}const a=Ge({location:"",base:e,go:n,createHref:jx.bind(null,e)},t,s);return Object.defineProperty(a,"location",{enumerable:!0,get:()=>t.location.value}),Object.defineProperty(a,"state",{enumerable:!0,get:()=>t.state.value}),a}function i_(e){return e=location.host?e||location.pathname+location.search:"",e.includes("#")||(e+="#"),a_(e)}let Pn=(function(e){return e[e.Static=0]="Static",e[e.Param=1]="Param",e[e.Group=2]="Group",e})({});var kt=(function(e){return e[e.Static=0]="Static",e[e.Param=1]="Param",e[e.ParamRegExp=2]="ParamRegExp",e[e.ParamRegExpEnd=3]="ParamRegExpEnd",e[e.EscapeNext=4]="EscapeNext",e})(kt||{});const l_={type:Pn.Static,value:""},r_=/[a-zA-Z0-9_]/;function o_(e){if(!e)return[[]];if(e==="/")return[[l_]];if(!e.startsWith("/"))throw new Error(`Invalid path "${e}"`);function t(p){throw new Error(`ERR (${s})/"${c}": ${p}`)}let s=kt.Static,n=s;const a=[];let i;function l(){i&&a.push(i),i=[]}let r=0,o,c="",u="";function d(){c&&(s===kt.Static?i.push({type:Pn.Static,value:c}):s===kt.Param||s===kt.ParamRegExp||s===kt.ParamRegExpEnd?(i.length>1&&(o==="*"||o==="+")&&t(`A repeatable param (${c}) must be alone in its segment. eg: '/:ids+.`),i.push({type:Pn.Param,value:c,regexp:u,repeatable:o==="*"||o==="+",optional:o==="*"||o==="?"})):t("Invalid state to consume buffer"),c="")}function f(){c+=o}for(;r<e.length;){if(o=e[r++],o==="\\"&&s!==kt.ParamRegExp){n=s,s=kt.EscapeNext;continue}switch(s){case kt.Static:o==="/"?(c&&d(),l()):o===":"?(d(),s=kt.Param):f();break;case kt.EscapeNext:f(),s=n;break;case kt.Param:o==="("?s=kt.ParamRegExp:r_.test(o)?f():(d(),s=kt.Static,o!=="*"&&o!=="?"&&o!=="+"&&r--);break;case kt.ParamRegExp:o===")"?u[u.length-1]=="\\"?u=u.slice(0,-1)+o:s=kt.ParamRegExpEnd:u+=o;break;case kt.ParamRegExpEnd:d(),s=kt.Static,o!=="*"&&o!=="?"&&o!=="+"&&r--,u="";break;default:t("Unknown state");break}}return s===kt.ParamRegExp&&t(`Unfinished custom RegExp for param "${c}"`),d(),l(),a}const pd="[^/]+?",c_={sensitive:!1,strict:!1,start:!0,end:!0};var jt=(function(e){return e[e._multiplier=10]="_multiplier",e[e.Root=90]="Root",e[e.Segment=40]="Segment",e[e.SubSegment=30]="SubSegment",e[e.Static=40]="Static",e[e.Dynamic=20]="Dynamic",e[e.BonusCustomRegExp=10]="BonusCustomRegExp",e[e.BonusWildcard=-50]="BonusWildcard",e[e.BonusRepeatable=-20]="BonusRepeatable",e[e.BonusOptional=-8]="BonusOptional",e[e.BonusStrict=.7000000000000001]="BonusStrict",e[e.BonusCaseSensitive=.25]="BonusCaseSensitive",e})(jt||{});const u_=/[.+*?^${}()[\]/\\]/g;function d_(e,t){const s=Ge({},c_,t),n=[];let a=s.start?"^":"";const i=[];for(const c of e){const u=c.length?[]:[jt.Root];s.strict&&!c.length&&(a+="/");for(let d=0;d<c.length;d++){const f=c[d];let p=jt.Segment+(s.sensitive?jt.BonusCaseSensitive:0);if(f.type===Pn.Static)d||(a+="/"),a+=f.value.replace(u_,"\\$&"),p+=jt.Static;else if(f.type===Pn.Param){const{value:b,repeatable:g,optional:T,regexp:O}=f;i.push({name:b,repeatable:g,optional:T});const y=O||pd;if(y!==pd){p+=jt.BonusCustomRegExp;try{`${y}`}catch(x){throw new Error(`Invalid custom RegExp for param "${b}" (${y}): `+x.message)}}let v=g?`((?:${y})(?:/(?:${y}))*)`:`(${y})`;d||(v=T&&c.length<2?`(?:/${v})`:"/"+v),T&&(v+="?"),a+=v,p+=jt.Dynamic,T&&(p+=jt.BonusOptional),g&&(p+=jt.BonusRepeatable),y===".*"&&(p+=jt.BonusWildcard)}u.push(p)}n.push(u)}if(s.strict&&s.end){const c=n.length-1;n[c][n[c].length-1]+=jt.BonusStrict}s.strict||(a+="/?"),s.end?a+="$":s.strict&&!a.endsWith("/")&&(a+="(?:/|$)");const l=new RegExp(a,s.sensitive?"":"i");function r(c){const u=c.match(l),d={};if(!u)return null;for(let f=1;f<u.length;f++){const p=u[f]||"",b=i[f-1];d[b.name]=p&&b.repeatable?p.split("/"):p}return d}function o(c){let u="",d=!1;for(const f of e){(!d||!u.endsWith("/"))&&(u+="/"),d=!1;for(const p of f)if(p.type===Pn.Static)u+=p.value;else if(p.type===Pn.Param){const{value:b,repeatable:g,optional:T}=p,O=b in c?c[b]:"";if(Ss(O)&&!g)throw new Error(`Provided param "${b}" is an array but it is not repeatable (* or + modifiers)`);const y=Ss(O)?O.join("/"):O;if(!y)if(T)f.length<2&&(u.endsWith("/")?u=u.slice(0,-1):d=!0);else throw new Error(`Missing required param "${b}"`);u+=y}}return u||"/"}return{re:l,score:n,keys:i,parse:r,stringify:o}}function f_(e,t){let s=0;for(;s<e.length&&s<t.length;){const n=t[s]-e[s];if(n)return n;s++}return e.length<t.length?e.length===1&&e[0]===jt.Static+jt.Segment?-1:1:e.length>t.length?t.length===1&&t[0]===jt.Static+jt.Segment?1:-1:0}function Yh(e,t){let s=0;const n=e.score,a=t.score;for(;s<n.length&&s<a.length;){const i=f_(n[s],a[s]);if(i)return i;s++}if(Math.abs(a.length-n.length)===1){if(hd(n))return 1;if(hd(a))return-1}return a.length-n.length}function hd(e){const t=e[e.length-1];return e.length>0&&t[t.length-1]<0}const p_={strict:!1,end:!0,sensitive:!1};function h_(e,t,s){const n=d_(o_(e.path),s),a=Ge(n,{record:e,parent:t,children:[],alias:[]});return t&&!a.record.aliasOf==!t.record.aliasOf&&t.children.push(a),a}function g_(e,t){const s=[],n=new Map;t=ld(p_,t);function a(d){return n.get(d)}function i(d,f,p){const b=!p,g=md(d);g.aliasOf=p&&p.record;const T=ld(t,d),O=[g];if("alias"in d){const x=typeof d.alias=="string"?[d.alias]:d.alias;for(const w of x)O.push(md(Ge({},g,{components:p?p.record.components:g.components,path:w,aliasOf:p?p.record:g})))}let y,v;for(const x of O){const{path:w}=x;if(f&&w[0]!=="/"){const I=f.record.path,L=I[I.length-1]==="/"?"":"/";x.path=f.record.path+(w&&L+w)}if(y=h_(x,f,T),p?p.alias.push(y):(v=v||y,v!==y&&v.alias.push(y),b&&d.name&&!vd(y)&&l(d.name)),Qh(y)&&o(y),g.children){const I=g.children;for(let L=0;L<I.length;L++)i(I[L],y,p&&p.children[L])}p=p||y}return v?()=>{l(v)}:oi}function l(d){if(Wh(d)){const f=n.get(d);f&&(n.delete(d),s.splice(s.indexOf(f),1),f.children.forEach(l),f.alias.forEach(l))}else{const f=s.indexOf(d);f>-1&&(s.splice(f,1),d.record.name&&n.delete(d.record.name),d.children.forEach(l),d.alias.forEach(l))}}function r(){return s}function o(d){const f=b_(d,s);s.splice(f,0,d),d.record.name&&!vd(d)&&n.set(d.record.name,d)}function c(d,f){let p,b={},g,T;if("name"in d&&d.name){if(p=n.get(d.name),!p)throw La(ct.MATCHER_NOT_FOUND,{location:d});T=p.record.name,b=Ge(gd(f.params,p.keys.filter(v=>!v.optional).concat(p.parent?p.parent.keys.filter(v=>v.optional):[]).map(v=>v.name)),d.params&&gd(d.params,p.keys.map(v=>v.name))),g=p.stringify(b)}else if(d.path!=null)g=d.path,p=s.find(v=>v.re.test(g)),p&&(b=p.parse(g),T=p.record.name);else{if(p=f.name?n.get(f.name):s.find(v=>v.re.test(f.path)),!p)throw La(ct.MATCHER_NOT_FOUND,{location:d,currentLocation:f});T=p.record.name,b=Ge({},f.params,d.params),g=p.stringify(b)}const O=[];let y=p;for(;y;)O.unshift(y.record),y=y.parent;return{name:T,path:g,params:b,matched:O,meta:v_(O)}}e.forEach(d=>i(d));function u(){s.length=0,n.clear()}return{addRoute:i,resolve:c,removeRoute:l,clearRoutes:u,getRoutes:r,getRecordMatcher:a}}function gd(e,t){const s={};for(const n of t)n in e&&(s[n]=e[n]);return s}function md(e){const t={path:e.path,redirect:e.redirect,name:e.name,meta:e.meta||{},aliasOf:e.aliasOf,beforeEnter:e.beforeEnter,props:m_(e),children:e.children||[],instances:{},leaveGuards:new Set,updateGuards:new Set,enterCallbacks:{},components:"components"in e?e.components||null:e.component&&{default:e.component}};return Object.defineProperty(t,"mods",{value:{}}),t}function m_(e){const t={},s=e.props||!1;if("component"in e)t.default=s;else for(const n in e.components)t[n]=typeof s=="object"?s[n]:s;return t}function vd(e){for(;e;){if(e.record.aliasOf)return!0;e=e.parent}return!1}function v_(e){return e.reduce((t,s)=>Ge(t,s.meta),{})}function b_(e,t){let s=0,n=t.length;for(;s!==n;){const i=s+n>>1;Yh(e,t[i])<0?n=i:s=i+1}const a=y_(e);return a&&(n=t.lastIndexOf(a,n-1)),n}function y_(e){let t=e;for(;t=t.parent;)if(Qh(t)&&Yh(e,t)===0)return t}function Qh({record:e}){return!!(e.name||e.components&&Object.keys(e.components).length||e.redirect)}function bd(e){const t=hs(fr),s=hs(Rc),n=J(()=>{const o=Ds(e.to);return t.resolve(o)}),a=J(()=>{const{matched:o}=n.value,{length:c}=o,u=o[c-1],d=s.matched;if(!u||!d.length)return-1;const f=d.findIndex(Na.bind(null,u));if(f>-1)return f;const p=yd(o[c-2]);return c>1&&yd(u)===p&&d[d.length-1].path!==p?d.findIndex(Na.bind(null,o[c-2])):f}),i=J(()=>a.value>-1&&S_(s.params,n.value.params)),l=J(()=>a.value>-1&&a.value===s.matched.length-1&&Gh(s.params,n.value.params));function r(o={}){if(w_(o)){const c=t[Ds(e.replace)?"replace":"push"](Ds(e.to)).catch(oi);return e.viewTransition&&typeof document<"u"&&"startViewTransition"in document&&document.startViewTransition(()=>c),c}return Promise.resolve()}return{route:n,href:J(()=>n.value.href),isActive:i,isExactActive:l,navigate:r}}function x_(e){return e.length===1?e[0]:e}const __=Oi({name:"RouterLink",compatConfig:{MODE:3},props:{to:{type:[String,Object],required:!0},replace:Boolean,activeClass:String,exactActiveClass:String,custom:Boolean,ariaCurrentValue:{type:String,default:"page"},viewTransition:Boolean},useLink:bd,setup(e,{slots:t}){const s=Sn(bd(e)),{options:n}=hs(fr),a=J(()=>({[xd(e.activeClass,n.linkActiveClass,"router-link-active")]:s.isActive,[xd(e.exactActiveClass,n.linkExactActiveClass,"router-link-exact-active")]:s.isExactActive}));return()=>{const i=t.default&&x_(t.default(s));return e.custom?i:Sa("a",{"aria-current":s.isExactActive?e.ariaCurrentValue:null,href:s.href,onClick:s.navigate,class:a.value},i)}}}),k_=__;function w_(e){if(!(e.metaKey||e.altKey||e.ctrlKey||e.shiftKey)&&!e.defaultPrevented&&!(e.button!==void 0&&e.button!==0)){if(e.currentTarget&&e.currentTarget.getAttribute){const t=e.currentTarget.getAttribute("target");if(/\b_blank\b/i.test(t))return}return e.preventDefault&&e.preventDefault(),!0}}function S_(e,t){for(const s in t){const n=t[s],a=e[s];if(typeof n=="string"){if(n!==a)return!1}else if(!Ss(a)||a.length!==n.length||n.some((i,l)=>i.valueOf()!==a[l].valueOf()))return!1}return!0}function yd(e){return e?e.aliasOf?e.aliasOf.path:e.path:""}const xd=(e,t,s)=>e??t??s,T_=Oi({name:"RouterView",inheritAttrs:!1,props:{name:{type:String,default:"default"},route:Object},compatConfig:{MODE:3},setup(e,{attrs:t,slots:s}){const n=hs(wo),a=J(()=>e.route||n.value),i=hs(dd,0),l=J(()=>{let c=Ds(i);const{matched:u}=a.value;let d;for(;(d=u[c])&&!d.components;)c++;return c}),r=J(()=>a.value.matched[l.value]);ni(dd,J(()=>l.value+1)),ni(Xx,r),ni(wo,a);const o=h();return gs(()=>[o.value,r.value,e.name],([c,u,d],[f,p,b])=>{u&&(u.instances[d]=c,p&&p!==u&&c&&c===f&&(u.leaveGuards.size||(u.leaveGuards=p.leaveGuards),u.updateGuards.size||(u.updateGuards=p.updateGuards))),c&&u&&(!p||!Na(u,p)||!f)&&(u.enterCallbacks[d]||[]).forEach(g=>g(c))},{flush:"post"}),()=>{const c=a.value,u=e.name,d=r.value,f=d&&d.components[u];if(!f)return _d(s.default,{Component:f,route:c});const p=d.props[u],b=p?p===!0?c.params:typeof p=="function"?p(c):p:null,T=Sa(f,Ge({},b,t,{onVnodeUnmounted:O=>{O.component.isUnmounted&&(d.instances[u]=null)},ref:o}));return _d(s.default,{Component:T,route:c})||T}}});function _d(e,t){if(!e)return null;const s=e(t);return s.length===1?s[0]:s}const C_=T_;function E_(e){const t=g_(e.routes,e),s=e.parseQuery||Yx,n=e.stringifyQuery||ud,a=e.history,i=Ka(),l=Ka(),r=Ka(),o=Fo(hn);let c=hn;ca&&e.scrollBehavior&&"scrollRestoration"in history&&(history.scrollRestoration="manual");const u=Dr.bind(null,z=>""+z),d=Dr.bind(null,Dx),f=Dr.bind(null,Ai);function p(z,oe){let ie,me;return Wh(z)?(ie=t.getRecordMatcher(z),me=oe):me=z,t.addRoute(me,ie)}function b(z){const oe=t.getRecordMatcher(z);oe&&t.removeRoute(oe)}function g(){return t.getRoutes().map(z=>z.record)}function T(z){return!!t.getRecordMatcher(z)}function O(z,oe){if(oe=Ge({},oe||o.value),typeof z=="string"){const E=Mr(s,z,oe.path),P=t.resolve({path:E.path},oe),Z=a.createHref(E.fullPath);return Ge(E,P,{params:f(P.params),hash:Ai(E.hash),redirectedFrom:void 0,href:Z})}let ie;if(z.path!=null)ie=Ge({},z,{path:Mr(s,z.path,oe.path).path});else{const E=Ge({},z.params);for(const P in E)E[P]==null&&delete E[P];ie=Ge({},z,{params:d(E)}),oe.params=d(oe.params)}const me=t.resolve(ie,oe),pe=z.hash||"";me.params=u(f(me.params));const Le=Fx(n,Ge({},z,{hash:Nx(pe),path:me.path})),m=a.createHref(Le);return Ge({fullPath:Le,hash:pe,query:n===ud?Qx(z.query):z.query||{}},me,{redirectedFrom:void 0,href:m})}function y(z){return typeof z=="string"?Mr(s,z,o.value.path):Ge({},z)}function v(z,oe){if(c!==z)return La(ct.NAVIGATION_CANCELLED,{from:oe,to:z})}function x(z){return L(z)}function w(z){return x(Ge(y(z),{replace:!0}))}function I(z,oe){const ie=z.matched[z.matched.length-1];if(ie&&ie.redirect){const{redirect:me}=ie;let pe=typeof me=="function"?me(z,oe):me;return typeof pe=="string"&&(pe=pe.includes("?")||pe.includes("#")?pe=y(pe):{path:pe},pe.params={}),Ge({query:z.query,hash:z.hash,params:pe.path!=null?{}:z.params},pe)}}function L(z,oe){const ie=c=O(z),me=o.value,pe=z.state,Le=z.force,m=z.replace===!0,E=I(ie,me);if(E)return L(Ge(y(E),{state:typeof E=="object"?Ge({},pe,E.state):pe,force:Le,replace:m}),oe||ie);const P=ie;P.redirectedFrom=oe;let Z;return!Le&&$x(n,me,ie)&&(Z=La(ct.NAVIGATION_DUPLICATED,{to:P,from:me}),de(me,me,!0,!1)),(Z?Promise.resolve(Z):D(P,me)).catch(R=>Hs(R)?Hs(R,ct.NAVIGATION_GUARD_REDIRECT)?R:ce(R):C(R,P,me)).then(R=>{if(R){if(Hs(R,ct.NAVIGATION_GUARD_REDIRECT))return L(Ge({replace:m},y(R.to),{state:typeof R.to=="object"?Ge({},pe,R.to.state):pe,force:Le}),oe||P)}else R=M(P,me,!0,m,pe);return B(P,me,R),R})}function k(z,oe){const ie=v(z,oe);return ie?Promise.reject(ie):Promise.resolve()}function A(z){const oe=Q.values().next().value;return oe&&typeof oe.runWithContext=="function"?oe.runWithContext(z):z()}function D(z,oe){let ie;const[me,pe,Le]=e_(z,oe);ie=Fr(me.reverse(),"beforeRouteLeave",z,oe);for(const E of me)E.leaveGuards.forEach(P=>{ie.push(xn(P,z,oe))});const m=k.bind(null,z,oe);return ie.push(m),Ie(ie).then(()=>{ie=[];for(const E of i.list())ie.push(xn(E,z,oe));return ie.push(m),Ie(ie)}).then(()=>{ie=Fr(pe,"beforeRouteUpdate",z,oe);for(const E of pe)E.updateGuards.forEach(P=>{ie.push(xn(P,z,oe))});return ie.push(m),Ie(ie)}).then(()=>{ie=[];for(const E of Le)if(E.beforeEnter)if(Ss(E.beforeEnter))for(const P of E.beforeEnter)ie.push(xn(P,z,oe));else ie.push(xn(E.beforeEnter,z,oe));return ie.push(m),Ie(ie)}).then(()=>(z.matched.forEach(E=>E.enterCallbacks={}),ie=Fr(Le,"beforeRouteEnter",z,oe,A),ie.push(m),Ie(ie))).then(()=>{ie=[];for(const E of l.list())ie.push(xn(E,z,oe));return ie.push(m),Ie(ie)}).catch(E=>Hs(E,ct.NAVIGATION_CANCELLED)?E:Promise.reject(E))}function B(z,oe,ie){r.list().forEach(me=>A(()=>me(z,oe,ie)))}function M(z,oe,ie,me,pe){const Le=v(z,oe);if(Le)return Le;const m=oe===hn,E=ca?history.state:{};ie&&(me||m?a.replace(z.fullPath,Ge({scroll:m&&E&&E.scroll},pe)):a.push(z.fullPath,pe)),o.value=z,de(z,oe,ie,m),ce()}let S;function U(){S||(S=a.listen((z,oe,ie)=>{if(!ue.listening)return;const me=O(z),pe=I(me,ue.currentRoute.value);if(pe){L(Ge(pe,{replace:!0,force:!0}),me).catch(oi);return}c=me;const Le=o.value;ca&&Kx(cd(Le.fullPath,ie.delta),dr()),D(me,Le).catch(m=>Hs(m,ct.NAVIGATION_ABORTED|ct.NAVIGATION_CANCELLED)?m:Hs(m,ct.NAVIGATION_GUARD_REDIRECT)?(L(Ge(y(m.to),{force:!0}),me).then(E=>{Hs(E,ct.NAVIGATION_ABORTED|ct.NAVIGATION_DUPLICATED)&&!ie.delta&&ie.type===_o.pop&&a.go(-1,!1)}).catch(oi),Promise.reject()):(ie.delta&&a.go(-ie.delta,!1),C(m,me,Le))).then(m=>{m=m||M(me,Le,!1),m&&(ie.delta&&!Hs(m,ct.NAVIGATION_CANCELLED)?a.go(-ie.delta,!1):ie.type===_o.pop&&Hs(m,ct.NAVIGATION_ABORTED|ct.NAVIGATION_DUPLICATED)&&a.go(-1,!1)),B(me,Le,m)}).catch(oi)}))}let W=Ka(),F=Ka(),N;function C(z,oe,ie){ce(z);const me=F.list();return me.length?me.forEach(pe=>pe(z,oe,ie)):console.error(z),Promise.reject(z)}function j(){return N&&o.value!==hn?Promise.resolve():new Promise((z,oe)=>{W.add([z,oe])})}function ce(z){return N||(N=!z,U(),W.list().forEach(([oe,ie])=>z?ie(z):oe()),W.reset()),z}function de(z,oe,ie,me){const{scrollBehavior:pe}=e;if(!ca||!pe)return Promise.resolve();const Le=!ie&&Gx(cd(z.fullPath,0))||(me||!ie)&&history.state&&history.state.scroll||null;return St().then(()=>pe(z,oe,Le)).then(m=>m&&qx(m)).catch(m=>C(m,z,oe))}const se=z=>a.go(z);let fe;const Q=new Set,ue={currentRoute:o,listening:!0,addRoute:p,removeRoute:b,clearRoutes:t.clearRoutes,hasRoute:T,getRoutes:g,resolve:O,options:e,push:x,replace:w,go:se,back:()=>se(-1),forward:()=>se(1),beforeEach:i.add,beforeResolve:l.add,afterEach:r.add,onError:F.add,isReady:j,install(z){z.component("RouterLink",k_),z.component("RouterView",C_),z.config.globalProperties.$router=ue,Object.defineProperty(z.config.globalProperties,"$route",{enumerable:!0,get:()=>Ds(o)}),ca&&!fe&&o.value===hn&&(fe=!0,x(a.location).catch(me=>{}));const oe={};for(const me in hn)Object.defineProperty(oe,me,{get:()=>o.value[me],enumerable:!0});z.provide(fr,ue),z.provide(Rc,Po(oe)),z.provide(wo,o);const ie=z.unmount;Q.add(z),z.unmount=function(){Q.delete(z),Q.size<1&&(c=hn,S&&S(),S=null,o.value=hn,fe=!1,N=!1),ie()}}};function Ie(z){return z.reduce((oe,ie)=>oe.then(()=>A(ie)),Promise.resolve())}return ue}function Xh(){return hs(fr)}function A_(e){return hs(Rc)}const R_=[{group:"Workspace",label:"Dashboard",icon:"dashboard",to:{path:"/dashboard"}},{group:"Workspace",label:"Chat",icon:"chat",to:{path:"/chat"}},...["Live","Agents","Loops","Processes","Schedules"].map(e=>({group:"Operations",label:e,icon:"operations",to:{path:"/operations",query:{tab:e.toLowerCase()}}})),...["Audit","Sessions","Traces","Usage"].map(e=>({group:"History",label:e,icon:"history",to:{path:"/history",query:{tab:e.toLowerCase()}}})),...["Tools","Skills","Knowledge","Memory","Learned"].map(e=>({group:"Capabilities",label:e,icon:"capabilities",to:{path:"/capabilities",query:{tab:e.toLowerCase()}}})),{group:"Manage",label:"Personality",icon:"personality",to:{path:"/personality"}},...[["Health","health"],["Resources","resources"],["Logs","logs"],["Config","config"],["Discord","discord"],["Host Access","host-access"],["API Tokens","api-tokens"],["LLM Config","llm"],["Internals","internals"],["Update","update"]].map(([e,t])=>({group:"System",label:e,icon:"system",to:{path:"/system",query:{tab:t}}}))],Qt=Sn({open:!1,query:"",selected:0});function kd(){Qt.query="",Qt.selected=0,Qt.open=!0}function $r(){Qt.open=!1}function I_(e,t){const s=e.label.toLowerCase(),n=`${e.group} ${e.label}`.toLowerCase();return t?s.startsWith(t)?100:n.startsWith(t)?80:s.includes(t)?60:n.includes(t)?40:0:1}const N_={setup(){const e=Xh(),t=h(null),s=J(()=>{const i=Qt.query.trim().toLowerCase();return R_.map(l=>({...l,_score:I_(l,i)})).filter(l=>l._score>0).sort((l,r)=>r._score-l._score)});gs(()=>Qt.open,async i=>{var l;i&&(await St(),(l=t.value)==null||l.focus())}),gs(()=>Qt.query,()=>{Qt.selected=0});function n(i){$r(),e.push(i.to)}function a(i){if(i.key==="Escape"){i.preventDefault(),$r();return}if(i.key==="ArrowDown")i.preventDefault(),Qt.selected=Math.min(Qt.selected+1,s.value.length-1);else if(i.key==="ArrowUp")i.preventDefault(),Qt.selected=Math.max(Qt.selected-1,0);else if(i.key==="Enter"){i.preventDefault();const l=s.value[Qt.selected];l&&n(l)}}return{state:Qt,results:s,inputEl:t,go:n,onKeydown:a,closePalette:$r}},template:`
    <transition name="modal">
...
... (truncated)`:s}function Sd(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function sg(e){return e==null||!isFinite(e)?"—":Number(e).toLocaleString()}function M_(e){return e==null||!isFinite(e)?"—":e>=1e3?`${(e/1e3).toFixed(1)}k`:String(e)}const P_={template:`
    ...
    </div>`,setup(){const e=h({}),t=h(!0),s=h(null),n=h([]),a=h(!1),i=h([]),l=h(!1),r=h([]),o=h(0),c=h(null),u=h({reload:!1,clearSessions:!1,stopLoops:!1});let d=0;const f=J(()=>{const S=e.value.uptime_seconds||0,U=Math.floor(S/86400),W=Math.floor(S%86400/3600),F=Math.floor(S%3600/60),N=[];return U>0&&N.push(`${U}d`),W>0&&N.push(`${W}h`),(N.length===0||U===0&&W===0)&&N.push(`${F}m`),N.join(" ")}),p=J(()=>{const S=e.value.uptime_seconds||0;return 125.66*(1-Math.min(S/86400,1))}),b=J(()=>{const S=e.value;return[{label:"Guilds",value:S.guild_count??0,icon:"home",iconColor:"text-blue-400"},{label:"Sessions",value:S.session_count??0,icon:"message",iconColor:"text-yellow-400"},{label:"Tools",value:S.tool_count??0,icon:"wrench",iconColor:"text-purple-400",sub:`${S.skill_count??0} skills`,subColor:"text-gray-500"},{label:"Loops",value:S.loop_count??0,icon:"rotate",iconColor:"text-green-400",color:S.loop_count>0?"text-green-400":"",highlight:S.loop_count>0},{label:"Agents",value:S.agent_running??0,icon:"bot",iconColor:"text-cyan-400",sub:S.agent_count>0?`${S.agent_count} total`:"",subColor:"text-gray-500",highlight:(S.agent_running??0)>0},{label:"Processes",value:S.process_running??0,icon:"sliders",iconColor:"text-orange-400",sub:S.process_count>0?`${S.process_count} total`:"",subColor:"text-gray-500",highlight:(S.process_running??0)>0},{label:"Schedules",value:S.schedule_count??0,icon:"clock",iconColor:"text-amber-400",sub:(S.schedule_failing>0?`${S.schedule_failing} failing`:"")+(S.schedule_failing>0&&S.schedule_paused>0?", ":"")+(S.schedule_paused>0?`${S.schedule_paused} paused`:"")||void 0,subColor:S.schedule_failing>0?"text-red-400":"text-yellow-400",color:S.schedule_failing>0?"text-red-400":"",highlight:S.schedule_failing>0},{label:"Users",value:S.user_count??0,icon:"users",iconColor:"text-indigo-400"},...c.value!==null?[{label:"Knowledge",value:c.value,icon:"book",iconColor:"text-teal-400",sub:"chunks",subColor:"text-gray-500"}]:[]]}),g=J(()=>{const S=e.value,U=[];return U.push({label:"Bot",status:S.status==="online"?"ok":"warn",detail:S.status==="online"?"Online":"Starting"}),(S.schedule_failing||0)>0?U.push({label:"Schedules",status:"error",detail:`${S.schedule_failing} failing`}):(S.schedule_count||0)>0&&U.push({label:"Schedules",status:"ok",detail:`${S.schedule_count} configured`}),(S.loop_count||0)>0&&U.push({label:"Loops",status:"ok",detail:`${S.loop_count} active`}),(S.agent_running||0)>0&&U.push({label:"Agents",status:"ok",detail:`${S.agent_running} running`}),(S.process_running||0)>0&&U.push({label:"Processes",status:"ok",detail:`${S.process_running} running`}),U});async function T(){try{e.value=await G.get("/api/status"),s.value=null}catch(S){s.value=S.message}finally{t.value=!1}}async function O(){a.value=!0;try{n.value=await G.get("/api/audit?limit=10"),o.value=0}catch{}a.value=!1}async function y(){l.value=!0;try{i.value=await G.get("/api/audit?error_only=1&limit=5")}catch{}l.value=!1}async function v(){try{const S=await G.get("/api/knowledge");c.value=(Array.isArray(S)?S:[]).reduce((U,W)=>U+(W.chunks||0),0)}catch{c.value=null}}async function x(){try{const S=await G.get("/api/agents");r.value=S.filter(U=>U.status==="running")}catch{}}async function w(){u.value={...u.value,reload:!0};try{await G.post("/api/reload"),_e.success("Config reloaded")}catch(S){_e.error(S.message)}u.value={...u.value,reload:!1}}async function I(){if(!await rs({title:"Clear all sessions",message:"Clear all conversation sessions? This cannot be undone.",confirmLabel:"Clear All",danger:!0}))return;u.value={...u.value,clearSessions:!0};const U=e.value.session_count;e.value={...e.value,session_count:0};try{const W=await G.post("/api/sessions/clear-all");_e.success(`Cleared ${W.count} session${W.count!==1?"s":""}`),await T()}catch(W){e.value={...e.value,session_count:U},_e.error(W.message)}u.value={...u.value,clearSessions:!1}}async function L(){if(!await rs({title:"Stop all loops",message:"Stop all running loops?",confirmLabel:"Stop Loops",danger:!0}))return;u.value={...u.value,stopLoops:!0};const U=e.value.loop_count;e.value={...e.value,loop_count:0};try{const W=await G.post("/api/loops/stop-all");_e.success(W.result),await T()}catch(W){e.value={...e.value,loop_count:U},_e.error(W.message)}u.value={...u.value,stopLoops:!1}}function k(){t.value=!0,s.value=null,T(),O(),y(),x()}let A=null,D=null,B=null;function M(S){if(S.payload&&S.payload.tool_name){const U={...S.payload,_isNew:!0,_key:++d};n.value.unshift(U),n.value.length>10&&n.value.pop(),o.value++,U.error&&(i.value.unshift(U),i.value.length>5&&i.value.pop()),setTimeout(()=>{U._isNew=!1},1500),clearTimeout(B),B=setTimeout(()=>{o.value=0},1e4)}}return He(async()=>{await Promise.all([T(),O(),y(),x(),v()]),A=setInterval(T,15e3),D=setInterval(x,1e4),We.subscribe("events",M)}),gt(()=>{A&&clearInterval(A),D&&clearInterval(D),clearTimeout(B),We.unsubscribe("events",M)}),{status:e,loading:t,error:s,uptime:f,uptimeRingOffset:p,stats:b,healthIndicators:g,activity:n,activityLoading:a,newEventCount:o,errors:i,errorsLoading:l,agents:r,actionLoading:u,fetchActivity:O,fetchStatus:T,formatTime:Nc,formatDuration:Oa,retry:k,reloadConfig:w,clearSessions:I,stopAllLoops:L}}};/*! @license DOMPurify 3.4.9 | (c) Cure53 and other contributors | Released under the Apache license 2.0 and Mozilla Public License 2.0 | github.com/cure53/DOMPurify/blob/3.4.9/LICENSE */function Td(e,t){(t==null||t>e.length)&&(t=e.length);for(var s=0,n=Array(t);s<t;s++)n[s]=e[s];return n}function F_(e){if(Array.isArray(e))return e}function $_(e,t){var s=e==null?null:typeof Symbol<"u"&&e[Symbol.iterator]||e["@@iterator"];if(s!=null){var n,a,i,l,r=[],o=!0,c=!1;try{if(i=(s=s.call(e)).next,t!==0)for(;!(o=(n=i.call(s)).done)&&(r.push(n.value),r.length!==t);o=!0);}catch(u){c=!0,a=u}finally{try{if(!o&&s.return!=null&&(l=s.return(),Object(l)!==l))return}finally{if(c)throw a}}return r}}function U_(){throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`)}function B_(e,t){return F_(e)||$_(e,t)||H_(e,t)||U_()}function H_(e,t){if(e){if(typeof e=="string")return Td(e,t);var s={}.toString.call(e).slice(8,-1);return s==="Object"&&e.constructor&&(s=e.constructor.name),s==="Map"||s==="Set"?Array.from(e):s==="Arguments"||/^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(s)?Td(e,t):void 0}}const ng=Object.entries,Cd=Object.setPrototypeOf,V_=Object.isFrozen,j_=Object.getPrototypeOf,z_=Object.getOwnPropertyDescriptor;let Gt=Object.freeze,vs=Object.seal,ua=Object.create,ag=typeof Reflect<"u"&&Reflect,To=ag.apply,Co=ag.construct;Gt||(Gt=function(t){return t});vs||(vs=function(t){return t});To||(To=function(t,s){for(var n=arguments.length,a=new Array(n>2?n-2:0),i=2;i<n;i++)a[i-2]=arguments[i];return t.apply(s,a)});Co||(Co=function(t){for(var s=arguments.length,n=new Array(s>1?s-1:0),a=1;a<s;a++)n[a-1]=arguments[a];return new t(...n)});const Vs=xt(Array.prototype.forEach),q_=xt(Array.prototype.lastIndexOf),Ed=xt(Array.prototype.pop),ia=xt(Array.prototype.push),K_=xt(Array.prototype.splice),Vt=Array.isArray,ei=xt(String.prototype.toLowerCase),Ur=xt(String.prototype.toString),Ad=xt(String.prototype.match),la=xt(String.prototype.replace),Rd=xt(String.prototype.indexOf),G_=xt(String.prototype.trim),W_=xt(Number.prototype.toString),Z_=xt(Boolean.prototype.toString),Id=typeof BigInt>"u"?null:xt(BigInt.prototype.toString),Nd=typeof Symbol>"u"?null:xt(Symbol.prototype.toString),ot=xt(Object.prototype.hasOwnProperty),Ga=xt(Object.prototype.toString),Rt=xt(RegExp.prototype.test),Nn=J_(TypeError);function xt(e){return function(t){t instanceof RegExp&&(t.lastIndex=0);for(var s=arguments.length,n=new Array(s>1?s-1:0),a=1;a<s;a++)n[a-1]=arguments[a];return To(e,t,n)}}function J_(e){return function(){for(var t=arguments.length,s=new Array(t),n=0;n<t;n++)s[n]=arguments[n];return Co(e,s)}}function Oe(e,t){let s=arguments.length>2&&arguments[2]!==void 0?arguments[2]:ei;if(Cd&&Cd(e,null),!Vt(t))return e;let n=t.length;for(;n--;){let a=t[n];if(typeof a=="string"){const i=s(a);i!==a&&(V_(t)||(t[n]=i),a=i)}e[a]=!0}return e}function Y_(e){for(let t=0;t<e.length;t++)ot(e,t)||(e[t]=null);return e}function Mt(e){const t=ua(null);for(const n of ng(e)){var s=B_(n,2);const a=s[0],i=s[1];ot(e,a)&&(Vt(i)?t[a]=Y_(i):i&&typeof i=="object"&&i.constructor===Object?t[a]=Mt(i):t[a]=i)}return t}function Q_(e){switch(typeof e){case"string":return e;case"number":return W_(e);case"boolean":return Z_(e);case"bigint":return Id?Id(e):"0";case"symbol":return Nd?Nd(e):"Symbol()";case"undefined":return Ga(e);case"function":case"object":{if(e===null)return Ga(e);const t=e,s=Rs(t,"toString");if(typeof s=="function"){const n=s(t);return typeof n=="string"?n:Ga(n)}return Ga(e)}default:return Ga(e)}}function Rs(e,t){for(;e!==null;){const n=z_(e,t);if(n){if(n.get)return xt(n.get);if(typeof n.value=="function")return xt(n.value)}e=j_(e)}function s(){return null}return s}function X_(e){try{return Rt(e,""),!0}catch{return!1}}const Ld=Gt(["a","abbr","acronym","address","area","article","aside","audio","b","bdi","bdo","big","blink","blockquote","body","br","button","canvas","caption","center","cite","code","col","colgroup","content","data","datalist","dd","decorator","del","details","dfn","dialog","dir","div","dl","dt","element","em","fieldset","figcaption","figure","font","footer","form","h1","h2","h3","h4","h5","h6","head","header","hgroup","hr","html","i","img","input","ins","kbd","label","legend","li","main","map","mark","marquee","menu","menuitem","meter","nav","nobr","ol","optgroup","option","output","p","picture","pre","progress","q","rp","rt","ruby","s","samp","search","section","select","shadow","slot","small","source","spacer","span","strike","strong","style","sub","summary","sup","table","tbody","td","template","textarea","tfoot","th","thead","time","tr","track","tt","u","ul","var","video","wbr"]),Br=Gt(["svg","a","altglyph","altglyphdef","altglyphitem","animatecolor","animatemotion","animatetransform","circle","clippath","defs","desc","ellipse","enterkeyhint","exportparts","filter","font","g","glyph","glyphref","hkern","image","inputmode","line","lineargradient","marker","mask","metadata","mpath","part","path","pattern","polygon","polyline","radialgradient","rect","stop","style","switch","symbol","text","textpath","title","tref","tspan","view","vkern"]),Hr=Gt(["feBlend","feColorMatrix","feComponentTransfer","feComposite","feConvolveMatrix","feDiffuseLighting","feDisplacementMap","feDistantLight","feDropShadow","feFlood","feFuncA","feFuncB","feFuncG","feFuncR","feGaussianBlur","feImage","feMerge","feMergeNode","feMorphology","feOffset","fePointLight","feSpecularLighting","feSpotLight","feTile","feTurbulence"]),ek=Gt(["animate","color-profile","cursor","discard","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","foreignobject","hatch","hatchpath","mesh","meshgradient","meshpatch","meshrow","missing-glyph","script","set","solidcolor","unknown","use"]),Vr=Gt(["math","menclose","merror","mfenced","mfrac","mglyph","mi","mlabeledtr","mmultiscripts","mn","mo","mover","mpadded","mphantom","mroot","mrow","ms","mspace","msqrt","mstyle","msub","msup","msubsup","mtable","mtd","mtext","mtr","munder","munderover","mprescripts"]),tk=Gt(["maction","maligngroup","malignmark","mlongdiv","mscarries","mscarry","msgroup","mstack","msline","msrow","semantics","annotation","annotation-xml","mprescripts","none"]),Od=Gt(["#text"]),Dd=Gt(["accept","action","align","alt","autocapitalize","autocomplete","autopictureinpicture","autoplay","background","bgcolor","border","capture","cellpadding","cellspacing","checked","cite","class","clear","color","cols","colspan","command","commandfor","controls","controlslist","coords","crossorigin","datetime","decoding","default","dir","disabled","disablepictureinpicture","disableremoteplayback","download","draggable","enctype","enterkeyhint","exportparts","face","for","headers","height","hidden","high","href","hreflang","id","inert","inputmode","integrity","ismap","kind","label","lang","list","loading","loop","low","max","maxlength","media","method","min","minlength","multiple","muted","name","nonce","noshade","novalidate","nowrap","open","optimum","part","pattern","placeholder","playsinline","popover","popovertarget","popovertargetaction","poster","preload","pubdate","radiogroup","readonly","rel","required","rev","reversed","role","rows","rowspan","spellcheck","scope","selected","shape","size","sizes","slot","span","srclang","start","src","srcset","step","style","summary","tabindex","title","translate","type","usemap","valign","value","width","wrap","xmlns"]),jr=Gt(["accent-height","accumulate","additive","alignment-baseline","amplitude","ascent","attributename","attributetype","azimuth","basefrequency","baseline-shift","begin","bias","by","class","clip","clippathunits","clip-path","clip-rule","color","color-interpolation","color-interpolation-filters","color-profile","color-rendering","cx","cy","d","dx","dy","diffuseconstant","direction","display","divisor","dur","edgemode","elevation","end","exponent","fill","fill-opacity","fill-rule","filter","filterunits","flood-color","flood-opacity","font-family","font-size","font-size-adjust","font-stretch","font-style","font-variant","font-weight","fx","fy","g1","g2","glyph-name","glyphref","gradientunits","gradienttransform","height","href","id","image-rendering","in","in2","intercept","k","k1","k2","k3","k4","kerning","keypoints","keysplines","keytimes","lang","lengthadjust","letter-spacing","kernelmatrix","kernelunitlength","lighting-color","local","marker-end","marker-mid","marker-start","markerheight","markerunits","markerwidth","maskcontentunits","maskunits","max","mask","mask-type","media","method","mode","min","name","numoctaves","offset","operator","opacity","order","orient","orientation","origin","overflow","paint-order","path","pathlength","patterncontentunits","patterntransform","patternunits","points","preservealpha","preserveaspectratio","primitiveunits","r","rx","ry","radius","refx","refy","repeatcount","repeatdur","restart","result","rotate","scale","seed","shape-rendering","slope","specularconstant","specularexponent","spreadmethod","startoffset","stddeviation","stitchtiles","stop-color","stop-opacity","stroke-dasharray","stroke-dashoffset","stroke-linecap","stroke-linejoin","stroke-miterlimit","stroke-opacity","stroke","stroke-width","style","surfacescale","systemlanguage","tabindex","tablevalues","targetx","targety","transform","transform-origin","text-anchor","text-decoration","text-rendering","textlength","type","u1","u2","unicode","values","viewbox","visibility","version","vert-adv-y","vert-origin-x","vert-origin-y","width","word-spacing","wrap","writing-mode","xchannelselector","ychannelselector","x","x1","x2","xmlns","y","y1","y2","z","zoomandpan"]),Md=Gt(["accent","accentunder","align","bevelled","close","columnalign","columnlines","columnspacing","columnspan","denomalign","depth","dir","display","displaystyle","encoding","fence","frame","height","href","id","largeop","length","linethickness","lquote","lspace","mathbackground","mathcolor","mathsize","mathvariant","maxsize","minsize","movablelimits","notation","numalign","open","rowalign","rowlines","rowspacing","rowspan","rspace","rquote","scriptlevel","scriptminsize","scriptsizemultiplier","selection","separator","separators","stretchy","subscriptshift","supscriptshift","symmetric","voffset","width","xmlns"]),el=Gt(["xlink:href","xml:id","xlink:title","xml:space","xmlns:xlink"]),sk=vs(/{{[\w\W]*|^[\w\W]*}}/g),nk=vs(/<%[\w\W]*|^[\w\W]*%>/g),ak=vs(/\${[\w\W]*/g),ik=vs(/^data-[\-\w.\u00B7-\uFFFF]+$/),lk=vs(/^aria-[\-\w]+$/),Pd=vs(/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|sms|cid|xmpp|matrix):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i),rk=vs(/^(?:\w+script|data):/i),ok=vs(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g),ck=vs(/^html$/i),uk=vs(/^[a-z][.\w]*(-[.\w]+)+$/i),Es={element:1,attribute:2,text:3,cdataSection:4,entityReference:5,entityNode:6,progressingInstruction:7,comment:8,document:9,documentType:10,documentFragment:11,notation:12},dk=function(){return typeof window>"u"?null:window},fk=function(t,s){if(typeof t!="object"||typeof t.createPolicy!="function")return null;let n=null;const a="data-tt-policy-suffix";s&&s.hasAttribute(a)&&(n=s.getAttribute(a));const i="dompurify"+(n?"#"+n:"");try{return t.createPolicy(i,{createHTML(l){return l},createScriptURL(l){return l}})}catch{return console.warn("TrustedTypes policy "+i+" could not be created."),null}},Fd=function(){return{afterSanitizeAttributes:[],afterSanitizeElements:[],afterSanitizeShadowDOM:[],beforeSanitizeAttributes:[],beforeSanitizeElements:[],beforeSanitizeShadowDOM:[],uponSanitizeAttribute:[],uponSanitizeElement:[],uponSanitizeShadowNode:[]}};function ig(){let e=arguments.length>0&&arguments[0]!==void 0?arguments[0]:dk();const t=ve=>ig(ve);if(t.version="3.4.9",t.removed=[],!e||!e.document||e.document.nodeType!==Es.document||!e.Element)return t.isSupported=!1,t;let s=e.document;const n=s,a=n.currentScript;e.DocumentFragment;const i=e.HTMLTemplateElement,l=e.Node,r=e.Element,o=e.NodeFilter,c=e.NamedNodeMap;c===void 0&&(e.NamedNodeMap||e.MozNamedAttrMap),e.HTMLFormElement;const u=e.DOMParser,d=e.trustedTypes,f=r.prototype,p=Rs(f,"cloneNode"),b=Rs(f,"remove"),g=Rs(f,"nextSibling"),T=Rs(f,"childNodes"),O=Rs(f,"parentNode"),y=Rs(f,"shadowRoot"),v=Rs(f,"attributes"),x=l&&l.prototype?Rs(l.prototype,"nodeType"):null,w=l&&l.prototype?Rs(l.prototype,"nodeName"):null;if(typeof i=="function"){const ve=s.createElement("template");ve.content&&ve.content.ownerDocument&&(s=ve.content.ownerDocument)}let I,L="",k,A=!1,D=0;const B=function(){if(D>0)throw Nn('A configured TRUSTED_TYPES_POLICY callback (createHTML or createScriptURL) must not call DOMPurify.sanitize, as that causes infinite recursion. Do not pass a policy whose callbacks wrap DOMPurify as TRUSTED_TYPES_POLICY; see the "DOMPurify and Trusted Types" section of the README.')},M=function(_){B(),D++;try{return I.createHTML(_)}finally{D--}},S=function(_){B(),D++;try{return I.createScriptURL(_)}finally{D--}},U=function(){return A||(k=fk(d,a),A=!0),k},W=s,F=W.implementation,N=W.createNodeIterator,C=W.createDocumentFragment,j=W.getElementsByTagName,ce=n.importNode;let de=Fd();t.isSupported=typeof ng=="function"&&typeof O=="function"&&F&&F.createHTMLDocument!==void 0;const se=sk,fe=nk,Q=ak,ue=ik,Ie=lk,z=rk,oe=ok,ie=uk;let me=Pd,pe=null;const Le=Oe({},[...Ld,...Br,...Hr,...Vr,...Od]);let m=null;const E=Oe({},[...Dd,...jr,...Md,...el]);let P=Object.seal(ua(null,{tagNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},allowCustomizedBuiltInElements:{writable:!0,configurable:!1,enumerable:!0,value:!1}})),Z=null,R=null;const $=Object.seal(ua(null,{tagCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeCheck:{writable:!0,configurable:!1,enumerable:!0,value:null}}));let Y=!0,ee=!0,te=!1,X=!0,be=!1,le=!0,ge=!1,xe=!1,ke=!1,Ee=!1,H=!1,re=!1,ye=!0,Me=!1;const Ue="user-content-";let Je=!0,dt=!1,Ye={},Ke=null;const Wt=Oe({},["annotation-xml","audio","colgroup","desc","foreignobject","head","iframe","math","mi","mn","mo","ms","mtext","noembed","noframes","noscript","plaintext","script","selectedcontent","style","svg","template","thead","title","video","xmp"]);let bs=null;const Ts=Oe({},["audio","video","img","source","image","track"]);let Cn=null;const Qn=Oe({},["alt","class","for","id","label","name","pattern","placeholder","role","summary","title","value","style","xmlns"]),cn="http://www.w3.org/1998/Math/MathML",un="http://www.w3.org/2000/svg",q="http://www.w3.org/1999/xhtml";let Se=q,Cs=!1,En=null;const mr=Oe({},[cn,un,q],Ur);let $a=Oe({},["mi","mo","mn","ms","mtext"]),Ua=Oe({},["annotation-xml"]);const vr=Oe({},["title","style","font","a","script"]);let ys=null;const Xn=["application/xhtml+xml","text/html"],ea="text/html";let it=null,Fs=null;const V=s.createElement("form"),ne=function(_){return _ instanceof RegExp||_ instanceof Function},Te=function(){let _=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};if(Fs&&Fs===_)return;(!_||typeof _!="object")&&(_={}),_=Mt(_),ys=Xn.indexOf(_.PARSER_MEDIA_TYPE)===-1?ea:_.PARSER_MEDIA_TYPE,it=ys==="application/xhtml+xml"?Ur:ei,pe=ot(_,"ALLOWED_TAGS")&&Vt(_.ALLOWED_TAGS)?Oe({},_.ALLOWED_TAGS,it):Le,m=ot(_,"ALLOWED_ATTR")&&Vt(_.ALLOWED_ATTR)?Oe({},_.ALLOWED_ATTR,it):E,En=ot(_,"ALLOWED_NAMESPACES")&&Vt(_.ALLOWED_NAMESPACES)?Oe({},_.ALLOWED_NAMESPACES,Ur):mr,Cn=ot(_,"ADD_URI_SAFE_ATTR")&&Vt(_.ADD_URI_SAFE_ATTR)?Oe(Mt(Qn),_.ADD_URI_SAFE_ATTR,it):Qn,bs=ot(_,"ADD_DATA_URI_TAGS")&&Vt(_.ADD_DATA_URI_TAGS)?Oe(Mt(Ts),_.ADD_DATA_URI_TAGS,it):Ts,Ke=ot(_,"FORBID_CONTENTS")&&Vt(_.FORBID_CONTENTS)?Oe({},_.FORBID_CONTENTS,it):Wt,Z=ot(_,"FORBID_TAGS")&&Vt(_.FORBID_TAGS)?Oe({},_.FORBID_TAGS,it):Mt({}),R=ot(_,"FORBID_ATTR")&&Vt(_.FORBID_ATTR)?Oe({},_.FORBID_ATTR,it):Mt({}),Ye=ot(_,"USE_PROFILES")?_.USE_PROFILES&&typeof _.USE_PROFILES=="object"?Mt(_.USE_PROFILES):_.USE_PROFILES:!1,Y=_.ALLOW_ARIA_ATTR!==!1,ee=_.ALLOW_DATA_ATTR!==!1,te=_.ALLOW_UNKNOWN_PROTOCOLS||!1,X=_.ALLOW_SELF_CLOSE_IN_ATTR!==!1,be=_.SAFE_FOR_TEMPLATES||!1,le=_.SAFE_FOR_XML!==!1,ge=_.WHOLE_DOCUMENT||!1,Ee=_.RETURN_DOM||!1,H=_.RETURN_DOM_FRAGMENT||!1,re=_.RETURN_TRUSTED_TYPE||!1,ke=_.FORCE_BODY||!1,ye=_.SANITIZE_DOM!==!1,Me=_.SANITIZE_NAMED_PROPS||!1,Je=_.KEEP_CONTENT!==!1,dt=_.IN_PLACE||!1,me=X_(_.ALLOWED_URI_REGEXP)?_.ALLOWED_URI_REGEXP:Pd,Se=typeof _.NAMESPACE=="string"?_.NAMESPACE:q,$a=ot(_,"MATHML_TEXT_INTEGRATION_POINTS")&&_.MATHML_TEXT_INTEGRATION_POINTS&&typeof _.MATHML_TEXT_INTEGRATION_POINTS=="object"?Mt(_.MATHML_TEXT_INTEGRATION_POINTS):Oe({},["mi","mo","mn","ms","mtext"]),Ua=ot(_,"HTML_INTEGRATION_POINTS")&&_.HTML_INTEGRATION_POINTS&&typeof _.HTML_INTEGRATION_POINTS=="object"?Mt(_.HTML_INTEGRATION_POINTS):Oe({},["annotation-xml"]);const K=ot(_,"CUSTOM_ELEMENT_HANDLING")&&_.CUSTOM_ELEMENT_HANDLING&&typeof _.CUSTOM_ELEMENT_HANDLING=="object"?Mt(_.CUSTOM_ELEMENT_HANDLING):ua(null);if(P=ua(null),ot(K,"tagNameCheck")&&ne(K.tagNameCheck)&&(P.tagNameCheck=K.tagNameCheck),ot(K,"attributeNameCheck")&&ne(K.attributeNameCheck)&&(P.attributeNameCheck=K.attributeNameCheck),ot(K,"allowCustomizedBuiltInElements")&&typeof K.allowCustomizedBuiltInElements=="boolean"&&(P.allowCustomizedBuiltInElements=K.allowCustomizedBuiltInElements),be&&(ee=!1),H&&(Ee=!0),Ye&&(pe=Oe({},Od),m=ua(null),Ye.html===!0&&(Oe(pe,Ld),Oe(m,Dd)),Ye.svg===!0&&(Oe(pe,Br),Oe(m,jr),Oe(m,el)),Ye.svgFilters===!0&&(Oe(pe,Hr),Oe(m,jr),Oe(m,el)),Ye.mathMl===!0&&(Oe(pe,Vr),Oe(m,Md),Oe(m,el))),$.tagCheck=null,$.attributeCheck=null,ot(_,"ADD_TAGS")&&(typeof _.ADD_TAGS=="function"?$.tagCheck=_.ADD_TAGS:Vt(_.ADD_TAGS)&&(pe===Le&&(pe=Mt(pe)),Oe(pe,_.ADD_TAGS,it))),ot(_,"ADD_ATTR")&&(typeof _.ADD_ATTR=="function"?$.attributeCheck=_.ADD_ATTR:Vt(_.ADD_ATTR)&&(m===E&&(m=Mt(m)),Oe(m,_.ADD_ATTR,it))),ot(_,"ADD_URI_SAFE_ATTR")&&Vt(_.ADD_URI_SAFE_ATTR)&&Oe(Cn,_.ADD_URI_SAFE_ATTR,it),ot(_,"FORBID_CONTENTS")&&Vt(_.FORBID_CONTENTS)&&(Ke===Wt&&(Ke=Mt(Ke)),Oe(Ke,_.FORBID_CONTENTS,it)),ot(_,"ADD_FORBID_CONTENTS")&&Vt(_.ADD_FORBID_CONTENTS)&&(Ke===Wt&&(Ke=Mt(Ke)),Oe(Ke,_.ADD_FORBID_CONTENTS,it)),Je&&(pe["#text"]=!0),ge&&Oe(pe,["html","head","body"]),pe.table&&(Oe(pe,["tbody"]),delete Z.tbody),_.TRUSTED_TYPES_POLICY){if(typeof _.TRUSTED_TYPES_POLICY.createHTML!="function")throw Nn('TRUSTED_TYPES_POLICY configuration option must provide a "createHTML" hook.');if(typeof _.TRUSTED_TYPES_POLICY.createScriptURL!="function")throw Nn('TRUSTED_TYPES_POLICY configuration option must provide a "createScriptURL" hook.');const ae=I;I=_.TRUSTED_TYPES_POLICY;try{L=M("")}catch(Ce){throw I=ae,Ce}}else _.TRUSTED_TYPES_POLICY===null?(I=void 0,L=""):(I===void 0&&(I=U()),I&&typeof L=="string"&&(L=M("")));(de.uponSanitizeElement.length>0||de.uponSanitizeAttribute.length>0)&&pe===Le&&(pe=Mt(pe)),de.uponSanitizeAttribute.length>0&&m===E&&(m=Mt(m)),Gt&&Gt(_),Fs=_},Xe=Oe({},[...Br,...Hr,...ek]),rt=Oe({},[...Vr,...tk]),Zt=function(_){let K=O(_);(!K||!K.tagName)&&(K={namespaceURI:Se,tagName:"template"});const ae=ei(_.tagName),Ce=ei(K.tagName);return En[_.namespaceURI]?_.namespaceURI===un?K.namespaceURI===q?ae==="svg":K.namespaceURI===cn?ae==="svg"&&(Ce==="annotation-xml"||$a[Ce]):!!Xe[ae]:_.namespaceURI===cn?K.namespaceURI===q?ae==="math":K.namespaceURI===un?ae==="math"&&Ua[Ce]:!!rt[ae]:_.namespaceURI===q?K.namespaceURI===un&&!Ua[Ce]||K.namespaceURI===cn&&!$a[Ce]?!1:!rt[ae]&&(vr[ae]||!Xe[ae]):!!(ys==="application/xhtml+xml"&&En[_.namespaceURI]):!1},ns=function(_){ia(t.removed,{element:_});try{O(_).removeChild(_)}catch{if(b(_),!O(_))throw Nn("a node selected for removal could not be detached from its tree and cannot be safely returned; refusing to sanitize in place")}},Vc=function(_){const K=T?T(_):_.childNodes;if(K){const Ce=[];Vs(K,Re=>{ia(Ce,Re)}),Vs(Ce,Re=>{try{b(Re)}catch{}})}const ae=v?v(_):null;if(ae)for(let Ce=ae.length-1;Ce>=0;--Ce){const Re=ae[Ce],De=Re&&Re.name;if(typeof De=="string")try{_.removeAttribute(De)}catch{}}},An=function(_,K){try{ia(t.removed,{attribute:K.getAttributeNode(_),from:K})}catch{ia(t.removed,{attribute:null,from:K})}if(K.removeAttribute(_),_==="is")if(Ee||H)try{ns(K)}catch{}else try{K.setAttribute(_,"")}catch{}},yg=function(_){const K=v?v(_):_.attributes;if(K)for(let ae=K.length-1;ae>=0;--ae){const Ce=K[ae],Re=Ce&&Ce.name;if(!(typeof Re!="string"||m[it(Re)]))try{_.removeAttribute(Re)}catch{}}},xg=function(_){const K=[_];for(;K.length>0;){const ae=K.pop();(x?x(ae):ae.nodeType)===Es.element&&yg(ae);const Re=T?T(ae):ae.childNodes;if(Re)for(let De=Re.length-1;De>=0;--De)K.push(Re[De])}},jc=function(_){let K=null,ae=null;if(ke)_="<remove></remove>"+_;else{const De=Ad(_,/^[\r\n\t ]+/);ae=De&&De[0]}ys==="application/xhtml+xml"&&Se===q&&(_='<html xmlns="http://www.w3.org/1999/xhtml"><head></head><body>'+_+"</body></html>");const Ce=I?M(_):_;if(Se===q)try{K=new u().parseFromString(Ce,ys)}catch{}if(!K||!K.documentElement){K=F.createDocument(Se,"template",null);try{K.documentElement.innerHTML=Cs?L:Ce}catch{}}const Re=K.body||K.documentElement;return _&&ae&&Re.insertBefore(s.createTextNode(ae),Re.childNodes[0]||null),Se===q?j.call(K,ge?"html":"body")[0]:ge?K.documentElement:Re},zc=function(_){return N.call(_.ownerDocument||_,_,o.SHOW_ELEMENT|o.SHOW_COMMENT|o.SHOW_TEXT|o.SHOW_PROCESSING_INSTRUCTION|o.SHOW_CDATA_SECTION,null)},br=function(_){var K,ae;_.normalize();const Ce=N.call(_.ownerDocument||_,_,o.SHOW_TEXT|o.SHOW_COMMENT|o.SHOW_CDATA_SECTION|o.SHOW_PROCESSING_INSTRUCTION,null);let Re=Ce.nextNode();for(;Re;){let _t=Re.data;Vs([se,fe,Q],tt=>{_t=la(_t,tt," ")}),Re.data=_t,Re=Ce.nextNode()}const De=(K=(ae=_.querySelectorAll)===null||ae===void 0?void 0:ae.call(_,"template"))!==null&&K!==void 0?K:[];Vs(Array.from(De),_t=>{ta(_t.content)&&br(_t.content)})},Bi=function(_){const K=w?w(_):null;return typeof K!="string"||it(K)!=="form"?!1:typeof _.nodeName!="string"||typeof _.textContent!="string"||typeof _.removeChild!="function"||_.attributes!==v(_)||typeof _.removeAttribute!="function"||typeof _.setAttribute!="function"||typeof _.namespaceURI!="string"||typeof _.insertBefore!="function"||typeof _.hasChildNodes!="function"||_.nodeType!==x(_)||_.childNodes!==T(_)},ta=function(_){if(!x||typeof _!="object"||_===null)return!1;try{return x(_)===Es.documentFragment}catch{return!1}},Ba=function(_){if(!x||typeof _!="object"||_===null)return!1;try{return typeof x(_)=="number"}catch{return!1}};function $s(ve,_,K){Vs(ve,ae=>{ae.call(t,_,K,Fs)})}const qc=function(_){let K=null;if($s(de.beforeSanitizeElements,_,null),Bi(_))return ns(_),!0;const ae=it(w?w(_):_.nodeName);if($s(de.uponSanitizeElement,_,{tagName:ae,allowedTags:pe}),le&&_.hasChildNodes()&&!Ba(_.firstElementChild)&&Rt(/<[/\w!]/g,_.innerHTML)&&Rt(/<[/\w!]/g,_.textContent)||le&&_.namespaceURI===q&&ae==="style"&&Ba(_.firstElementChild)||_.nodeType===Es.progressingInstruction||le&&_.nodeType===Es.comment&&Rt(/<[/\w]/g,_.data))return ns(_),!0;if(Z[ae]||!($.tagCheck instanceof Function&&$.tagCheck(ae))&&!pe[ae]){if(!Z[ae]&&Gc(ae)&&(P.tagNameCheck instanceof RegExp&&Rt(P.tagNameCheck,ae)||P.tagNameCheck instanceof Function&&P.tagNameCheck(ae)))return!1;if(Je&&!Ke[ae]){const Re=O(_),De=T(_);if(De&&Re){const _t=De.length;for(let tt=_t-1;tt>=0;--tt){const ft=dt?De[tt]:p(De[tt],!0);Re.insertBefore(ft,g(_))}}}return ns(_),!0}return(x?x(_):_.nodeType)===Es.element&&!Zt(_)||(ae==="noscript"||ae==="noembed"||ae==="noframes")&&Rt(/<\/no(script|embed|frames)/i,_.innerHTML)?(ns(_),!0):(be&&_.nodeType===Es.text&&(K=_.textContent,Vs([se,fe,Q],Re=>{K=la(K,Re," ")}),_.textContent!==K&&(ia(t.removed,{element:_.cloneNode()}),_.textContent=K)),$s(de.afterSanitizeElements,_,null),!1)},Kc=function(_,K,ae){if(R[K]||ye&&(K==="id"||K==="name")&&(ae in s||ae in V))return!1;const Ce=m[K]||$.attributeCheck instanceof Function&&$.attributeCheck(K,_);if(!(ee&&!R[K]&&Rt(ue,K))){if(!(Y&&Rt(Ie,K))){if(!Ce||R[K]){if(!(Gc(_)&&(P.tagNameCheck instanceof RegExp&&Rt(P.tagNameCheck,_)||P.tagNameCheck instanceof Function&&P.tagNameCheck(_))&&(P.attributeNameCheck instanceof RegExp&&Rt(P.attributeNameCheck,K)||P.attributeNameCheck instanceof Function&&P.attributeNameCheck(K,_))||K==="is"&&P.allowCustomizedBuiltInElements&&(P.tagNameCheck instanceof RegExp&&Rt(P.tagNameCheck,ae)||P.tagNameCheck instanceof Function&&P.tagNameCheck(ae))))return!1}else if(!Cn[K]){if(!Rt(me,la(ae,oe,""))){if(!((K==="src"||K==="xlink:href"||K==="href")&&_!=="script"&&Rd(ae,"data:")===0&&bs[_])){if(!(te&&!Rt(z,la(ae,oe,"")))){if(ae)return!1}}}}}}return!0},_g=Oe({},["annotation-xml","color-profile","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","missing-glyph"]),Gc=function(_){return!_g[ei(_)]&&Rt(ie,_)},Wc=function(_){$s(de.beforeSanitizeAttributes,_,null);const K=_.attributes;if(!K||Bi(_))return;const ae={attrName:"",attrValue:"",keepAttr:!0,allowedAttributes:m,forceKeepAttr:void 0};let Ce=K.length;for(;Ce--;){const Re=K[Ce],De=Re.name,_t=Re.namespaceURI,tt=Re.value,ft=it(De),dn=tt;let Ct=De==="value"?dn:G_(dn);if(ae.attrName=ft,ae.attrValue=Ct,ae.keepAttr=!0,ae.forceKeepAttr=void 0,$s(de.uponSanitizeAttribute,_,ae),Ct=ae.attrValue,Me&&(ft==="id"||ft==="name")&&Rd(Ct,Ue)!==0&&(An(De,_),Ct=Ue+Ct),le&&Rt(/((--!?|])>)|<\/(style|script|title|xmp|textarea|noscript|iframe|noembed|noframes)/i,Ct)){An(De,_);continue}if(ft==="attributename"&&Ad(Ct,"href")){An(De,_);continue}if(ae.forceKeepAttr)continue;if(!ae.keepAttr){An(De,_);continue}if(!X&&Rt(/\/>/i,Ct)){An(De,_);continue}be&&Vs([se,fe,Q],Jc=>{Ct=la(Ct,Jc," ")});const Zc=it(_.nodeName);if(!Kc(Zc,ft,Ct)){An(De,_);continue}if(I&&typeof d=="object"&&typeof d.getAttributeType=="function"&&!_t)switch(d.getAttributeType(Zc,ft)){case"TrustedHTML":{Ct=M(Ct);break}case"TrustedScriptURL":{Ct=S(Ct);break}}if(Ct!==dn)try{_t?_.setAttributeNS(_t,De,Ct):_.setAttribute(De,Ct),Bi(_)?ns(_):Ed(t.removed)}catch{An(De,_)}}$s(de.afterSanitizeAttributes,_,null)},Hi=function(_){let K=null;const ae=zc(_);for($s(de.beforeSanitizeShadowDOM,_,null);K=ae.nextNode();)if($s(de.uponSanitizeShadowNode,K,null),qc(K),Wc(K),ta(K.content)&&Hi(K.content),(x?x(K):K.nodeType)===Es.element){const Re=y?y(K):K.shadowRoot;ta(Re)&&(yr(Re),Hi(Re))}$s(de.afterSanitizeShadowDOM,_,null)},yr=function(_){const K=[{node:_,shadow:null}];for(;K.length>0;){const ae=K.pop();if(ae.shadow){Hi(ae.shadow);continue}const Ce=ae.node,De=(x?x(Ce):Ce.nodeType)===Es.element,_t=T?T(Ce):Ce.childNodes;if(_t)for(let tt=_t.length-1;tt>=0;--tt)K.push({node:_t[tt],shadow:null});if(De){const tt=w?w(Ce):null;if(typeof tt=="string"&&it(tt)==="template"){const ft=Ce.content;ta(ft)&&K.push({node:ft,shadow:null})}}if(De){const tt=y?y(Ce):Ce.shadowRoot;ta(tt)&&K.push({node:null,shadow:tt},{node:tt,shadow:null})}}};return t.sanitize=function(ve){let _=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{},K=null,ae=null,Ce=null,Re=null;if(Cs=!ve,Cs&&(ve="<!-->"),typeof ve!="string"&&!Ba(ve)&&(ve=Q_(ve),typeof ve!="string"))throw Nn("dirty is not a string, aborting");if(!t.isSupported)return ve;xe||Te(_),t.removed=[];const De=dt&&typeof ve!="string"&&Ba(ve);if(De){const ft=w?w(ve):ve.nodeName;if(typeof ft=="string"){const dn=it(ft);if(!pe[dn]||Z[dn])throw Nn("root node is forbidden and cannot be sanitized in-place")}if(Bi(ve))throw Nn("root node is clobbered and cannot be sanitized in-place");try{yr(ve)}catch(dn){throw Vc(ve),dn}}else if(Ba(ve))K=jc("<!---->"),ae=K.ownerDocument.importNode(ve,!0),ae.nodeType===Es.element&&ae.nodeName==="BODY"||ae.nodeName==="HTML"?K=ae:K.appendChild(ae),yr(ae);else{if(!Ee&&!be&&!ge&&ve.indexOf("<")===-1)return I&&re?M(ve):ve;if(K=jc(ve),!K)return Ee?null:re?L:""}K&&ke&&ns(K.firstChild);const _t=zc(De?ve:K);try{for(;Ce=_t.nextNode();)qc(Ce),Wc(Ce),ta(Ce.content)&&Hi(Ce.content)}catch(ft){throw De&&Vc(ve),ft}if(De)return Vs(t.removed,ft=>{ft.element&&xg(ft.element)}),be&&br(ve),ve;if(Ee){if(be&&br(K),H)for(Re=C.call(K.ownerDocument);K.firstChild;)Re.appendChild(K.firstChild);else Re=K;return(m.shadowroot||m.shadowrootmode)&&(Re=ce.call(n,Re,!0)),Re}let tt=ge?K.outerHTML:K.innerHTML;return ge&&pe["!doctype"]&&K.ownerDocument&&K.ownerDocument.doctype&&K.ownerDocument.doctype.name&&Rt(ck,K.ownerDocument.doctype.name)&&(tt="<!DOCTYPE "+K.ownerDocument.doctype.name+`>
`+tt),be&&Vs([se,fe,Q],ft=>{tt=la(tt,ft," ")}),I&&re?M(tt):tt},t.setConfig=function(){let ve=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};Te(ve),xe=!0},t.clearConfig=function(){Fs=null,xe=!1,I=k,L=""},t.isValidAttribute=function(ve,_,K){Fs||Te({});const ae=it(ve),Ce=it(_);return Kc(ae,Ce,K)},t.addHook=function(ve,_){typeof _=="function"&&ia(de[ve],_)},t.removeHook=function(ve,_){if(_!==void 0){const K=q_(de[ve],_);return K===-1?void 0:K_(de[ve],K,1)[0]}return Ed(de[ve])},t.removeHooks=function(ve){de[ve]=[]},t.removeAllHooks=function(){de=Fd()},t}var $d=ig();function Oc(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}var Yn=Oc();function lg(e){Yn=e}var ci={exec:()=>null};function Ze(e,t=""){let s=typeof e=="string"?e:e.source;const n={replace:(a,i)=>{let l=typeof i=="string"?i:i.source;return l=l.replace(qt.caret,"$1"),s=s.replace(a,l),n},getRegex:()=>new RegExp(s,t)};return n}var qt={codeRemoveIndent:/^(?: {1,4}| {0,3}\t)/gm,outputLinkReplace:/\\([\[\]])/g,indentCodeCompensation:/^(\s+)(?:```)/,beginningSpace:/^\s+/,endingHash:/#$/,startingSpaceChar:/^ /,endingSpaceChar:/ $/,nonSpaceChar:/[^ ]/,newLineCharGlobal:/\n/g,tabCharGlobal:/\t/g,multipleSpaceGlobal:/\s+/g,blankLine:/^[ \t]*$/,doubleBlankLine:/\n[ \t]*\n[ \t]*$/,blockquoteStart:/^ {0,3}>/,blockquoteSetextReplace:/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,blockquoteSetextReplace2:/^ {0,3}>[ \t]?/gm,listReplaceTabs:/^\t+/,listReplaceNesting:/^ {1,4}(?=( {4})*[^ ])/g,listIsTask:/^\[[ xX]\] /,listReplaceTask:/^\[[ xX]\] +/,anyLine:/\n.*\n/,hrefBrackets:/^<(.*)>$/,tableDelimiter:/[:|]/,tableAlignChars:/^\||\| *$/g,tableRowBlankLine:/\n[ \t]*$/,tableAlignRight:/^ *-+: *$/,tableAlignCenter:/^ *:-+: *$/,tableAlignLeft:/^ *:-+ *$/,startATag:/^<a /i,endATag:/^<\/a>/i,startPreScriptTag:/^<(pre|code|kbd|script)(\s|>)/i,endPreScriptTag:/^<\/(pre|code|kbd|script)(\s|>)/i,startAngleBracket:/^</,endAngleBracket:/>$/,pedanticHrefTitle:/^([^'"]*[^\s])\s+(['"])(.*)\2/,unicodeAlphaNumeric:/[\p{L}\p{N}]/u,escapeTest:/[&<>"']/,escapeReplace:/[&<>"']/g,escapeTestNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,escapeReplaceNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/g,unescapeTest:/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig,caret:/(^|[^\[])\^/g,percentDecode:/%25/g,findPipe:/\|/g,splitPipe:/ \|/,slashPipe:/\\\|/g,carriageReturn:/\r\n|\r/g,spaceLine:/^ +$/gm,notSpaceStart:/^\S*/,endingNewline:/\n$/,listItemRegex:e=>new RegExp(`^( {0,3}${e})((?:[	 ][^\\n]*)?(?:\\n|$))`),nextBulletRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),hrRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),fencesBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:\`\`\`|~~~)`),headingBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}#`),htmlBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}<(?:[a-z].*>|!--)`,"i")},pk=/^(?:[ \t]*(?:\n|$))+/,hk=/^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/,gk=/^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/,Ui=/^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,mk=/^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/,Dc=/(?:[*+-]|\d{1,9}[.)])/,rg=/^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/,og=Ze(rg).replace(/bull/g,Dc).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/\|table/g,"").getRegex(),vk=Ze(rg).replace(/bull/g,Dc).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/table/g,/ {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),Mc=/^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/,bk=/^[^\n]+/,Pc=/(?!\s*\])(?:\\.|[^\[\]\\])+/,yk=Ze(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace("label",Pc).replace("title",/(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(),xk=Ze(/^( {0,3}bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g,Dc).getRegex(),pr="address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul",Fc=/<!--(?:-?>|[\s\S]*?(?:-->|$))/,_k=Ze("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))","i").replace("comment",Fc).replace("tag",pr).replace("attribute",/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),cg=Ze(Mc).replace("hr",Ui).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("|table","").replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",pr).getRegex(),kk=Ze(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph",cg).getRegex(),$c={blockquote:kk,code:hk,def:yk,fences:gk,heading:mk,hr:Ui,html:_k,lheading:og,list:xk,newline:pk,paragraph:cg,table:ci,text:bk},Ud=Ze("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr",Ui).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("blockquote"," {0,3}>").replace("code","(?: {4}| {0,3}	)[^\\n]").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",pr).getRegex(),wk={...$c,lheading:vk,table:Ud,paragraph:Ze(Mc).replace("hr",Ui).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("table",Ud).replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",pr).getRegex()},Sk={...$c,html:Ze(`^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:"[^"]*"|'[^']*'|\\s[^'"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))`).replace("comment",Fc).replace(/tag/g,"(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b").getRegex(),def:/^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +(["(][^\n]+[")]))? *(?:\n+|$)/,heading:/^(#{1,6})(.*)(?:\n+|$)/,fences:ci,lheading:/^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,paragraph:Ze(Mc).replace("hr",Ui).replace("heading",` *#{1,6} *[^
]`).replace("lheading",og).replace("|table","").replace("blockquote"," {0,3}>").replace("|fences","").replace("|list","").replace("|html","").replace("|tag","").getRegex()},Tk=/^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/,Ck=/^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/,ug=/^( {2,}|\\)\n(?!\s*$)/,Ek=/^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/,hr=/[\p{P}\p{S}]/u,Uc=/[\s\p{P}\p{S}]/u,dg=/[^\s\p{P}\p{S}]/u,Ak=Ze(/^((?![*_])punctSpace)/,"u").replace(/punctSpace/g,Uc).getRegex(),fg=/(?!~)[\p{P}\p{S}]/u,Rk=/(?!~)[\s\p{P}\p{S}]/u,Ik=/(?:[^\s\p{P}\p{S}]|~)/u,Nk=/\[[^[\]]*?\]\((?:\\.|[^\\\(\)]|\((?:\\.|[^\\\(\)])*\))*\)|`[^`]*?`|<[^<>]*?>/g,pg=/^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,Lk=Ze(pg,"u").replace(/punct/g,hr).getRegex(),Ok=Ze(pg,"u").replace(/punct/g,fg).getRegex(),hg="^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",Dk=Ze(hg,"gu").replace(/notPunctSpace/g,dg).replace(/punctSpace/g,Uc).replace(/punct/g,hr).getRegex(),Mk=Ze(hg,"gu").replace(/notPunctSpace/g,Ik).replace(/punctSpace/g,Rk).replace(/punct/g,fg).getRegex(),Pk=Ze("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)","gu").replace(/notPunctSpace/g,dg).replace(/punctSpace/g,Uc).replace(/punct/g,hr).getRegex(),Fk=Ze(/\\(punct)/,"gu").replace(/punct/g,hr).getRegex(),$k=Ze(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme",/[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email",/[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),Uk=Ze(Fc).replace("(?:-->|$)","-->").getRegex(),Bk=Ze("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment",Uk).replace("attribute",/\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),$l=/(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/,Hk=Ze(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label",$l).replace("href",/<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title",/"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),gg=Ze(/^!?\[(label)\]\[(ref)\]/).replace("label",$l).replace("ref",Pc).getRegex(),mg=Ze(/^!?\[(ref)\](?:\[\])?/).replace("ref",Pc).getRegex(),Vk=Ze("reflink|nolink(?!\\()","g").replace("reflink",gg).replace("nolink",mg).getRegex(),Bc={_backpedal:ci,anyPunctuation:Fk,autolink:$k,blockSkip:Nk,br:ug,code:Ck,del:ci,emStrongLDelim:Lk,emStrongRDelimAst:Dk,emStrongRDelimUnd:Pk,escape:Tk,link:Hk,nolink:mg,punctuation:Ak,reflink:gg,reflinkSearch:Vk,tag:Bk,text:Ek,url:ci},jk={...Bc,link:Ze(/^!?\[(label)\]\((.*?)\)/).replace("label",$l).getRegex(),reflink:Ze(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label",$l).getRegex()},Eo={...Bc,emStrongRDelimAst:Mk,emStrongLDelim:Ok,url:Ze(/^((?:ftp|https?):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/,"i").replace("email",/[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),_backpedal:/(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,del:/^(~~?)(?=[^\s~])((?:\\.|[^\\])*?(?:\\.|[^\s~\\]))\1(?=[^~]|$)/,text:/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|https?:\/\/|ftp:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/},zk={...Eo,br:Ze(ug).replace("{2,}","*").getRegex(),text:Ze(Eo.text).replace("\\b_","\\b_| {2,}\\n").replace(/\{2,\}/g,"*").getRegex()},tl={normal:$c,gfm:wk,pedantic:Sk},Wa={normal:Bc,gfm:Eo,breaks:zk,pedantic:jk},qk={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},Bd=e=>qk[e];function Is(e,t){if(t){if(qt.escapeTest.test(e))return e.replace(qt.escapeReplace,Bd)}else if(qt.escapeTestNoEncode.test(e))return e.replace(qt.escapeReplaceNoEncode,Bd);return e}function Hd(e){try{e=encodeURI(e).replace(qt.percentDecode,"%")}catch{return null}return e}function Vd(e,t){var i;const s=e.replace(qt.findPipe,(l,r,o)=>{let c=!1,u=r;for(;--u>=0&&o[u]==="\\";)c=!c;return c?"|":" |"}),n=s.split(qt.splitPipe);let a=0;if(n[0].trim()||n.shift(),n.length>0&&!((i=n.at(-1))!=null&&i.trim())&&n.pop(),t)if(n.length>t)n.splice(t);else for(;n.length<t;)n.push("");for(;a<n.length;a++)n[a]=n[a].trim().replace(qt.slashPipe,"|");return n}function Za(e,t,s){const n=e.length;if(n===0)return"";let a=0;for(;a<n&&e.charAt(n-a-1)===t;)a++;return e.slice(0,n-a)}function Kk(e,t){if(e.indexOf(t[1])===-1)return-1;let s=0;for(let n=0;n<e.length;n++)if(e[n]==="\\")n++;else if(e[n]===t[0])s++;else if(e[n]===t[1]&&(s--,s<0))return n;return s>0?-2:-1}function jd(e,t,s,n,a){const i=t.href,l=t.title||null,r=e[1].replace(a.other.outputLinkReplace,"$1");n.state.inLink=!0;const o={type:e[0].charAt(0)==="!"?"image":"link",raw:s,href:i,title:l,text:r,tokens:n.inlineTokens(r)};return n.state.inLink=!1,o}function Gk(e,t,s){const n=e.match(s.other.indentCodeCompensation);if(n===null)return t;const a=n[1];return t.split(`
...
Please report this to https://github.com/markedjs/marked.`,e){const n="<p>An error occurred:</p><pre>"+Is(s.message+"",!0)+"</pre>";return t?Promise.resolve(n):n}if(t)return Promise.reject(s);throw s}}},Kn=new Wk;function qe(e,t){return Kn.parse(e,t)}qe.options=qe.setOptions=function(e){return Kn.setOptions(e),qe.defaults=Kn.defaults,lg(qe.defaults),qe};qe.getDefaults=Oc;qe.defaults=Yn;qe.use=function(...e){return Kn.use(...e),qe.defaults=Kn.defaults,lg(qe.defaults),qe};qe.walkTokens=function(e,t){return Kn.walkTokens(e,t)};qe.parseInline=Kn.parseInline;qe.Parser=Ys;qe.parser=Ys.parse;qe.Renderer=Bl;qe.TextRenderer=Hc;qe.Lexer=Js;qe.lexer=Js.lex;qe.Tokenizer=Ul;qe.Hooks=ul;qe.parse=qe;qe.options;qe.setOptions;qe.use;qe.walkTokens;qe.parseInline;Ys.parse;Js.lex;const Zk={breaks:!0,gfm:!0};function zd(e){if(!e)return"";try{if(typeof qe<"u"&&qe.parse){const t=qe.parse(e,Zk);return typeof $d<"u"?$d.sanitize(t):t}}catch{}return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\n/g,"<br>")}function Jk(e){const t=new Date(e),s=t.getHours().toString().padStart(2,"0"),n=t.getMinutes().toString().padStart(2,"0");return`${s}:${n}`}const Yk={run_command:"terminal",ssh_command:"terminal",run_script:"terminal",read_file:"file",write_file:"edit",list_directory:"folder",search_knowledge:"search",ingest_document:"book",generate_image:"image",analyze_image:"eye",analyze_pdf:"file",browser_screenshot:"globe",manage_process:"sliders"};function Qk(e){return Yk[e]||"wrench"}const Xk=/https?:\/\/\S+\.(?:png|jpg|jpeg|gif|webp|svg)(?:\?\S*)?/gi;function qd(e){if(!e)return[];const t=e.match(Xk);return t?[...new Set(t)]:[]}const ew={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(""),s=h(!1),n=h(null),a=h(null),i=h(0),l=h("");let r=null,o=0;const c=["Check system health","List running services","Show disk usage","What can you do?"],u=J(()=>t.value.trim().length>0&&!s.value),d=J(()=>{const S=We.state;return S==="connected"?"Connected":S==="reconnecting"?"Reconnecting…":S==="connecting"?"Connecting…":"REST fallback"}),f=["Watching across all realms...","Processing...","Consulting the bifrost...","Observing..."],p=J(()=>{const S=Math.floor(i.value/4)%f.length,U=i.value;return U>3?`${f[S]} (${U}s)`:f[0]});function b(){St(()=>{n.value&&(n.value.scrollTop=n.value.scrollHeight)})}function g(){if(!a.value)return;const S=a.value;S.style.height="auto",S.style.height=Math.min(S.scrollHeight,120)+"px"}function T(S,U,W={}){const F={id:++o,role:S,content:U,timestamp:Date.now(),html:S==="bot"?zd(U):"",tools_used:W.tools_used||[],is_error:W.is_error||!1,images:S==="bot"?qd(U):[],files:W.files||[],_showTools:!1};return e.value.push(F),b(),S==="bot"&&St(()=>O()),F}function O(){if(!n.value)return;n.value.querySelectorAll(".chat-markdown pre:not([data-copy])").forEach(U=>{U.setAttribute("data-copy","true"),U.style.position="relative";const W=document.createElement("button");W.className="chat-code-copy",W.textContent="Copy",W.addEventListener("click",()=>{const F=U.querySelector("code"),N=F?F.textContent:U.textContent;navigator.clipboard.writeText(N).then(()=>{W.textContent="Copied!",setTimeout(()=>{W.textContent="Copy"},1500)}).catch(()=>{})}),U.appendChild(W)})}function y(S){if(S===0)return!0;const U=e.value[S-1],W=e.value[S],F=new Date(U.timestamp).toDateString(),N=new Date(W.timestamp).toDateString();return F!==N}function v(S){const U=new Date(S),W=new Date;if(U.toDateString()===W.toDateString())return"Today";const F=new Date(W);return F.setDate(F.getDate()-1),U.toDateString()===F.toDateString()?"Yesterday":U.toLocaleDateString(void 0,{month:"short",day:"numeric",year:"numeric"})}function x(S){t.value=S,St(()=>B())}function w(S){window.open(S,"_blank","noopener")}function I(S){S.target.style.display="none"}function L(){i.value=0,r=setInterval(()=>{i.value++},1e3)}function k(){r&&(clearInterval(r),r=null),i.value=0}function A(S){s.value&&(s.value=!1,k(),S.type==="chat_response"?T("bot",S.content,{tools_used:S.tools_used||[],is_error:S.is_error||!1,files:S.files||[]}):S.type==="chat_error"&&T("bot",S.error||"Unknown error",{is_error:!0}),St(()=>{var U;return(U=a.value)==null?void 0:U.focus()}))}async function D(S){try{const U=await G.post("/api/chat",{content:S,channel_id:l.value});T("bot",U.response,{tools_used:U.tools_used||[],is_error:U.is_error||!1,files:U.files||[]})}catch(U){T("bot",U.message||"Failed to send message",{is_error:!0})}}async function B(){const S=t.value.trim();if(!S||s.value)return;T("user",S),t.value="",s.value=!0,L(),a.value&&(a.value.style.height="auto"),We.connected&&We.sendChat(S,{channelId:l.value})||(await D(S),s.value=!1,k()),St(()=>{var W;return(W=a.value)==null?void 0:W.focus()})}async function M(){try{if(!l.value){const U=await G.get("/api/auth/session");l.value=U.channel_id||U.user_id||"web-user"}const S=await G.get("/api/sessions/"+encodeURIComponent(l.value));if(S&&S.messages&&S.messages.length>0){for(const U of S.messages){const W=U.role==="user"?"user":"bot";let F=U.content||"";if(W==="user"){const C=F.match(/^\[.*?\]:\s*/);C&&(F=F.slice(C[0].length))}if(!F.trim())continue;const N={id:++o,role:W,content:F,timestamp:U.timestamp?U.timestamp*1e3:Date.now(),html:W==="bot"?zd(F):"",tools_used:[],is_error:!1,images:W==="bot"?qd(F):[],files:[],_showTools:!1};e.value.push(N)}St(()=>{b(),O()})}}catch{}}return He(()=>{We.subscribe("chat",A),M(),St(()=>{var S;return(S=a.value)==null?void 0:S.focus()})}),gt(()=>{We.unsubscribe("chat",A),k()}),{messages:e,input:t,sending:s,messagesEl:n,inputEl:a,canSend:u,wsStatus:d,typingText:p,suggestions:c,send:B,autoResize:g,formatTime:Jk,formatDate:v,showDateSeparator:y,useSuggestion:x,openImage:w,onImageError:I,getToolIcon:Qk}}},gr={props:{tabs:{type:Array,required:!0},defaultTab:{type:String,default:""},groupLabel:{type:String,default:""}},setup(e){const t=A_(),s=Xh(),n=J({get(){var o;const r=t.query.tab;return r&&e.tabs.some(c=>c.id===r)?r:e.defaultTab||((o=e.tabs[0])==null?void 0:o.id)||""},set(r){s.replace({query:{...t.query,tab:r}})}}),a=J(()=>{var r;return((r=e.tabs.find(o=>o.id===n.value))==null?void 0:r.component)||null}),i=J(()=>{var r;return((r=e.tabs.find(o=>o.id===n.value))==null?void 0:r.label)||""});gs(i,r=>{e.groupLabel&&r&&(document.title=`Odin — ${e.groupLabel} › ${r}`)},{immediate:!0});function l(r,o){if(!["ArrowLeft","ArrowRight","Home","End"].includes(r.key))return;r.preventDefault();let c=o;r.key==="ArrowRight"&&(c=(o+1)%e.tabs.length),r.key==="ArrowLeft"&&(c=(o-1+e.tabs.length)%e.tabs.length),r.key==="Home"&&(c=0),r.key==="End"&&(c=e.tabs.length-1),n.value=e.tabs[c].id,requestAnimationFrame(()=>{var u;return(u=document.getElementById("tab-"+e.tabs[c].id))==null?void 0:u.focus()})}return{activeTab:n,activeComponent:a,activeLabel:i,onTabKeydown:l}},template:`
...
`)}return}}let i=null;function l(){const c=Date.now();e.value.forEach(u=>{u.status==="running"&&(u.elapsed=c-u.startTime)})}He(()=>{We.on("events",a),i=setInterval(l,500)}),gt(()=>{We.off("events",a),i&&clearInterval(i)});function r(c){return c<1e3?`${c}ms`:`${(c/1e3).toFixed(1)}s`}function o(c){return c==="running"?"clock":c==="success"?"success":c==="error"?"error":"info"}return{activeTasks:e,recentHistory:t,streamOutput:s,formatMs:r,statusIcon:o}},template:`
  ...
  `},sw={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(null),a=h(!0),i=h("all");let l=null;const r=J(()=>e.value.filter(F=>F.status==="running").length),o=J(()=>e.value.filter(F=>F.status==="completed").length),c=J(()=>e.value.filter(F=>["failed","timeout","killed"].includes(F.status)).length),u=J(()=>[{value:"all",label:"All",count:e.value.length},{value:"running",label:"Running",count:r.value},{value:"completed",label:"Completed",count:o.value},{value:"failed",label:"Failed",count:c.value}]),d=J(()=>i.value==="all"?e.value:i.value==="failed"?e.value.filter(F=>["failed","timeout","killed"].includes(F.status)):e.value.filter(F=>F.status===i.value));function f(F){const N=Number(F.max_iterations)||0;return N<=0?0:Math.min(100,Math.round(F.iteration_count/N*100))}function p(F){return(Number(F.max_iterations)||0)>0}function b(F,N){return F?F==="N/A"?"N/A":N==="current_inheritance"?`inherit (currently ${F})`:F:"unknown"}function g(F){return b(F.display_model,F.display_model_source||F.display_source)}function T(F){return b(F.display_reasoning_effort,F.display_reasoning_effort_source||F.display_source)}function O(F){return{last_execution:"last executed",current_inheritance:"inherited from current config — not yet executed",spawn_override_pending:"requested at spawn — not yet executed",unknown:"no execution data"}[F]||""}const y=h(null),v=h(null),x=h(!1),w=h(null),I=h("");let L=0;async function k(F){const N=++L;v.value=F.id,y.value=null,w.value=null,x.value=!0;try{const C=await G.get(`/api/agents/${encodeURIComponent(F.id)}`);if(N!==L)return;y.value=C}catch(C){if(N!==L)return;w.value=C.message||"Failed to load agent detail"}N===L&&(x.value=!1)}function A(){L++,v.value=null,y.value=null,w.value=null,x.value=!1,I.value=""}async function D(){if(!v.value)return;const F=L;try{const N=await G.get(`/api/agents/${encodeURIComponent(v.value)}`);if(F!==L)return;y.value=N}catch{}}async function B(F,N){try{await navigator.clipboard.writeText(N||""),I.value=F,setTimeout(()=>{I.value===F&&(I.value="")},1500)}catch{_e.error("Copy failed")}}async function M(F=!1){F=F===!0,F||(t.value=!0);try{const N=await G.get("/api/agents");e.value=Array.isArray(N)?N:[],s.value=null}catch(N){F||(s.value=N.message)}F||(t.value=!1)}async function S(F){const N=e.value.find(j=>j.id===F);if(await rs({title:"Kill agent",message:`Kill agent "${(N==null?void 0:N.label)||F}"? Its current work will be lost.`,confirmLabel:"Kill",danger:!0})){n.value=F;try{await G.del(`/api/agents/${encodeURIComponent(F)}`),_e.success("Agent killed"),await M()}catch(j){_e.error(j.message||"Failed to kill agent")}n.value=null}}function U(){W(),a.value&&(l=setInterval(()=>{a.value&&(M(!0),v.value&&D())},5e3))}function W(){l&&(clearInterval(l),l=null)}return He(()=>{M(),U()}),gt(()=>{W()}),{agents:e,loading:t,error:s,killing:n,autoRefresh:a,statusFilter:i,runningCount:r,completedCount:o,failedCount:c,statusFilters:u,filteredAgents:d,formatTs:Fa,formatDuration:Oa,progressPercent:f,hasProgress:p,displayModelText:g,displayEffortText:T,displaySourceLabel:O,detail:y,detailId:v,detailLoading:x,detailError:w,copied:I,openDetail:k,closeDetail:A,copyText:B,fetchAgents:M,killAgent:S,startAutoRefresh:U,stopAutoRefresh:W}}},nw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(!1),a=h({goal:"",interval_seconds:60,mode:"notify",max_iterations:50,stop_condition:"",channel_id:""}),i=h(!1),l=h(null),r=h(null),o=h(null),c=h({}),u=J(()=>e.value.reduce((I,L)=>I+(L.iteration_count||0),0)),d=J(()=>e.value.filter(I=>I.status==="running").length);function f(I){return I==="running"?"loop-status-running":I==="error"?"loop-status-error":"loop-status-stopped"}function p(I){return I==="running"?"badge-success":I==="error"?"badge-danger":I==="completed"?"badge-info":"badge-warning"}function b(I){return I==="act"?"badge-warning":I==="silent"?"badge-info":"badge-success"}function g(I){c.value={...c.value,[I]:!c.value[I]}}async function T(I=!1){I=I===!0,I||(t.value=!0);try{e.value=await G.get("/api/loops"),s.value=null}catch(L){I||(s.value=L.message)}I||(t.value=!1)}async function O(){l.value=null;const I=a.value;if(!I.goal.trim()){l.value="Goal is required";return}if(!I.channel_id.trim()){l.value="Channel ID is required";return}const L={goal:I.goal.trim(),channel_id:I.channel_id.trim(),interval_seconds:I.interval_seconds||60,mode:I.mode,max_iterations:I.max_iterations||50};I.stop_condition.trim()&&(L.stop_condition=I.stop_condition.trim()),i.value=!0;try{const k=await G.post("/api/loops",L);_e.success(`Loop started: ${k.loop_id}`),a.value={goal:"",interval_seconds:60,mode:"notify",max_iterations:50,stop_condition:"",channel_id:""},n.value=!1,await T()}catch(k){l.value=k.message}i.value=!1}async function y(I){if(await rs({title:"Stop loop",message:`Stop loop ${I}? The current iteration will finish before stopping.`,confirmLabel:"Stop Loop",danger:!0})){r.value=I;try{await G.del(`/api/loops/${encodeURIComponent(I)}`),_e.success("Loop stopped"),await T()}catch(k){_e.error(k.message||"Failed to stop loop")}r.value=null}}async function v(I){o.value=I;try{await G.post(`/api/loops/${encodeURIComponent(I)}/restart`),_e.success("Loop restarted"),await T()}catch(L){_e.error(L.message||"Failed to restart loop")}o.value=null}function x(I){I.payload&&(I.payload.loop_id||I.payload.type==="loop")&&T(!0)}let w=null;return He(()=>{T(),We.subscribe("events",x),w=setInterval(()=>{T(!0)},5e3)}),gt(()=>{We.unsubscribe("events",x),w&&clearInterval(w)}),{loops:e,loading:t,error:s,showCreate:n,form:a,creating:i,createError:l,stoppingId:r,restartingId:o,expandedHistory:c,totalIterations:u,runningCount:d,statusDotClass:f,statusBadge:p,modeBadge:b,formatDuration:Oa,formatAge:eg,toggleHistory:g,fetchLoops:T,doCreate:O,doStop:y,doRestart:v}}},aw={template:`
    <div class="p-6 page-fade-in">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(!0);let a=null;const i=h(null),l=J(()=>e.value.filter(g=>g.status==="running").length),r=J(()=>e.value.filter(g=>g.status!=="running").length);function o(g){return g==="running"?"loop-status-running":g==="failed"||g==="error"?"loop-status-error":"loop-status-stopped"}function c(g){return g==="running"?"badge-success":g==="completed"||g==="exited"?"badge-info":g==="killed"||g==="error"||g==="failed"?"badge-danger":"badge-warning"}async function u(g=!1){g=g===!0,g||(t.value=!0);try{e.value=await G.get("/api/processes"),s.value=null}catch(T){g||(s.value=T.message)}g||(t.value=!1)}function d(){f(),n.value&&(a=setInterval(()=>{t.value||u(!0)},5e3))}function f(){a&&(clearInterval(a),a=null)}gs(n,g=>{g?d():f()});async function p(g){if(await rs({title:"Kill process",message:`Kill process ${g}?`,confirmLabel:"Kill",danger:!0})){i.value=g;try{await G.del(`/api/processes/${g}`),_e.success(`Process ${g} killed`),await u()}catch(O){_e.error(O.message||"Failed to kill process")}i.value=null}}function b(g){g.payload&&(g.payload.pid||g.payload.type==="process")&&u(!0)}return He(()=>{u(),We.subscribe("events",b),d()}),gt(()=>{We.unsubscribe("events",b),f()}),{processes:e,loading:t,error:s,autoRefresh:n,killingPid:i,runningCount:l,completedCount:r,procStatusDot:o,statusBadge:c,formatDuration:Oa,fetchProcesses:u,doKill:p}}},iw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-start justify-between mb-4 flex-wrap gap-3">
        <div>
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(!1),a=h({description:"",action:"reminder",channel_id:"",cron:"",run_at:"",message:"",tool_name:"",tool_input_str:""}),i=h(!1),l=h(null),r=h(null),o=h(!1),c=[{label:"Every hour",expr:"0 * * * *"},{label:"Every 6h",expr:"0 */6 * * *"},{label:"Daily 9am",expr:"0 9 * * *"},{label:"Weekly Mon",expr:"0 9 * * 1"},{label:"Every 30m",expr:"*/30 * * * *"}],u=h(null),d=h(null),f=h(null),p=h(null),b=h(null),g=h([]),T=h(!1),O=J(()=>e.value.filter(N=>N.cron&&!N.one_time).length),y=J(()=>e.value.filter(N=>N.one_time).length),v=J(()=>e.value.filter(N=>N.trigger).length),x=J(()=>e.value.filter(N=>N.paused).length),w=J(()=>e.value.filter(N=>N.consecutive_failures>0).length);function I(N){if(!N)return"-";const C=Date.now(),ce=(new Date(N).getTime()-C)/1e3;if(ce<0)return"overdue";if(ce<60)return"in < 1 min";if(ce<3600)return`in ${Math.floor(ce/60)} min`;if(ce<86400){const se=Math.floor(ce/3600),fe=Math.floor(ce%3600/60);return fe>0?`in ${se}h ${fe}m`:`in ${se}h`}const de=Math.floor(ce/86400);return`in ${de} day${de!==1?"s":""}`}function L(N){return N==null?"-":N<1e3?`${N}ms`:N<6e4?`${(N/1e3).toFixed(1)}s`:Oa(N/1e3)}function k(){r.value=null}async function A(){const N=a.value.cron.trim();if(N){o.value=!0;try{r.value=await G.post("/api/schedules/validate-cron",{expression:N})}catch(C){r.value={valid:!1,error:C.message}}o.value=!1}}async function D(){t.value=!0,s.value=null;try{e.value=await G.get("/api/schedules")}catch(N){s.value=N.message}t.value=!1}async function B(N){if(b.value===N){b.value=null,g.value=[];return}b.value=N,T.value=!0,g.value=[];try{g.value=await G.get(`/api/schedules/${encodeURIComponent(N)}/history?limit=10`)}catch{g.value=[]}T.value=!1}async function M(){l.value=null;const N=a.value;if(!N.description.trim()){l.value="Description is required";return}if(!N.channel_id.trim()){l.value="Channel ID is required";return}if(!N.cron.trim()&&!N.run_at.trim()){l.value="Cron expression or run_at time is required";return}const C={description:N.description.trim(),action:N.action,channel_id:N.channel_id.trim()};if(N.cron.trim()&&(C.cron=N.cron.trim()),N.run_at.trim()&&(C.run_at=N.run_at.trim()),N.action==="reminder"&&N.message.trim()&&(C.message=N.message.trim()),N.action==="check"&&(N.tool_name.trim()&&(C.tool_name=N.tool_name.trim()),N.tool_input_str.trim()))try{C.tool_input=JSON.parse(N.tool_input_str.trim())}catch{l.value="Tool input must be valid JSON";return}i.value=!0;try{await G.post("/api/schedules",C),_e.success("Schedule created"),a.value={description:"",action:"reminder",channel_id:"",cron:"",run_at:"",message:"",tool_name:"",tool_input_str:""},r.value=null,n.value=!1,await D()}catch(j){l.value=j.message}i.value=!1}async function S(N){u.value=N;try{const C=await G.post(`/api/schedules/${encodeURIComponent(N)}/run`);if(C.status==="failure")_e.error(`Execution failed: ${C.error||"unknown error"}`);else{const j=C.warning?`Executed (${C.warning})`:"Executed successfully";_e.success(j)}await D()}catch(C){_e.error(C.message||"Failed to trigger")}u.value=null}async function U(N){f.value=N.id;const C=!N.paused;try{await G.put(`/api/schedules/${encodeURIComponent(N.id)}`,{paused:C}),_e.success(C?"Schedule paused":"Schedule resumed"),await D()}catch(j){_e.error(j.message||"Failed to update schedule")}f.value=null}async function W(N){p.value=N;try{await G.post(`/api/schedules/${encodeURIComponent(N)}/reset-failures`),_e.success("Failure counters reset"),await D()}catch(C){_e.error(C.message||"Failed to reset")}p.value=null}async function F(N){const C=e.value.find(ce=>ce.id===N);if(await rs({title:"Delete schedule",message:`Delete "${(C==null?void 0:C.description)||N}"? This cannot be undone.`,confirmLabel:"Delete",danger:!0})){d.value=N;try{await G.del(`/api/schedules/${encodeURIComponent(N)}`),_e.success("Schedule deleted"),await D()}catch(ce){_e.error(ce.message||"Failed to delete schedule")}d.value=null}}return He(()=>{D()}),{schedules:e,loading:t,error:s,showCreate:n,form:a,creating:i,createError:l,cronResult:r,validatingCron:o,cronPresets:c,runningId:u,deletingId:d,togglingId:f,resettingId:p,expandedId:b,history:g,historyLoading:T,cronCount:O,oneTimeCount:y,webhookCount:v,pausedCount:x,failingCount:w,formatTs:Fa,formatAge:eg,formatFuture:I,formatMs:L,formatDuration:Oa,onCronInput:k,validateCron:A,toggleExpand:B,fetchSchedules:D,doCreate:M,doRunNow:S,doTogglePause:U,doResetFailures:W,doDelete:F}}},lw={components:{TabbedPage:gr},setup(){return{tabs:[{id:"live",label:"Live",component:tw},{id:"agents",label:"Agents",component:sw},{id:"loops",label:"Loops",component:nw},{id:"processes",label:"Processes",component:aw},{id:"schedules",label:"Schedules",component:iw}]}},template:'<tabbed-page :tabs="tabs" default-tab="live" group-label="Operations" />'},rw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(null),a=h(null),i=h(!1),l=h(null),r=h(!1),o=h(new Set),c=h(!1),u=h("all"),d=h(""),f=h("last_active"),p=h(!1),b=Kd,g=ow,T=h([]),O=h(!1),y=h(""),v=h("flat"),x=h(new Set),w=h(""),I=h(""),L=h(""),k=h(null),A=h(!1);function D(){try{const H=localStorage.getItem("odin-session-presets");H&&(T.value=JSON.parse(H))}catch{}}function B(){try{localStorage.setItem("odin-session-presets",JSON.stringify(T.value))}catch{}}const M=J(()=>d.value.trim()!==""||u.value!=="all"),S=J(()=>{let H=[...e.value];const re=Kd.find(Je=>Je.id===u.value),ye=re?re.filters:{};if(ye.source&&(H=H.filter(Je=>Je.source===ye.source)),ye.minMessages&&(H=H.filter(Je=>Je.message_count>=ye.minMessages)),ye.hasCompaction&&(H=H.filter(Je=>Je.has_summary)),ye.maxAge!=null){const Je=Date.now()/1e3;H=H.filter(dt=>dt.last_active&&Je-dt.last_active<=ye.maxAge)}if(d.value.trim()){const Je=d.value.toLowerCase().trim();H=H.filter(dt=>(dt.channel_id||"").toLowerCase().includes(Je)||(dt.last_user_id||"").toLowerCase().includes(Je)||(dt.source||"").toLowerCase().includes(Je))}const Me=f.value,Ue=p.value?1:-1;return H.sort((Je,dt)=>{const Ye=Je[Me]||0,Ke=dt[Me]||0;return(Ye-Ke)*Ue}),H}),U=J(()=>{if(!a.value||!a.value.messages)return[];const H=a.value.messages;if(H.length===0)return[];const re=[];let ye=[];for(const Me of H)Me.role==="user"&&ye.length>0&&(re.push(ye),ye=[]),ye.push(Me);return ye.length>0&&re.push(ye),re}),W=J(()=>S.value.length>0&&o.value.size===S.value.length);function F(H){const re=H.find(ye=>ye.role==="user");if(re&&re.content){const ye=re.content.slice(0,120);return ye.length<re.content.length?ye+"...":ye}return"(no user message)"}function N(H){const re=new Set(x.value);re.has(H)?re.delete(H):re.add(H),x.value=re}function C(H){u.value=H}function j(H){u.value=H.id,H.filters.searchQuery!=null&&(d.value=H.filters.searchQuery),H.filters.sortBy&&(f.value=H.filters.sortBy)}function ce(){if(!y.value.trim())return;const H={id:"custom-"+Date.now(),name:y.value.trim(),filters:{searchQuery:d.value,sortBy:f.value}};T.value=[...T.value,H],B(),O.value=!1,y.value=""}function de(H){T.value=T.value.filter(re=>re.id!==H),B(),u.value===H&&(u.value="all")}function se(){u.value="all",d.value="",f.value="last_active",p.value=!1}function fe(H){if(!H)return"—";const re=Date.now()/1e3-H;if(re<60)return"just now";if(re<3600){const Me=Math.floor(re/60);return`${Me} minute${Me!==1?"s":""} ago`}if(re<86400){const Me=Math.floor(re/3600);return`${Me} hour${Me!==1?"s":""} ago`}const ye=Math.floor(re/86400);return`${ye} day${ye!==1?"s":""} ago`}function Q(H){if(!H)return"";try{return new Date(H*1e3).toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return""}}function ue(H){if(!H)return"";try{return new Date(H*1e3).toLocaleString()}catch{return""}}function Ie(H){return H==="user"?"bg-gray-900/50 border border-gray-800":H==="assistant"?"bg-indigo-950/30 border border-indigo-900/30":"bg-gray-900/30 border border-gray-800/50"}function z(H){return H==="user"?"sess-msg-user":H==="assistant"?"sess-msg-assistant":"sess-msg-system"}function oe(H){return H==="user"?"badge-info":H==="assistant"?"badge-success":"badge-warning"}function ie(H){return H==="user"?"sess-dot-user":H==="assistant"?"sess-dot-assistant":"sess-dot-system"}function me(H){return H==="user"?"text-cyan-400":H==="assistant"?"text-indigo-400":"text-gray-500"}function pe(H){return H?H.length>2e3?H.slice(0,2e3)+`
... (truncated)`:H:""}async function Le(){const H=w.value.trim();if(H){A.value=!0;try{let re=`/api/sessions/search?q=${encodeURIComponent(H)}&limit=50`;I.value.trim()&&(re+=`&channel_id=${encodeURIComponent(I.value.trim())}`),L.value.trim()&&(re+=`&user_id=${encodeURIComponent(L.value.trim())}`);const ye=await G.get(re);k.value=ye.results||[]}catch{k.value=[]}A.value=!1}}function m(){w.value="",I.value="",L.value="",k.value=null}function E(H){return H?H.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/&gt;&gt;&gt;/g,'<mark class="fts-highlight">').replace(/&lt;&lt;&lt;/g,"</mark>"):""}function P(H){return H==="user"?"fts-result-user":H==="assistant"?"fts-result-assistant":H==="summary"?"fts-result-summary":H==="fts"?"fts-result-fts":H==="channel"?"fts-result-channel":"fts-result-default"}function Z(H){return H==="user"?"badge-info":H==="assistant"?"badge-success":H==="summary"?"badge-warning":H==="fts"?"badge-success":"badge-info"}async function R(){t.value=!0,s.value=null;try{e.value=await G.get("/api/sessions")}catch(H){s.value=H.message}t.value=!1}function $(){s.value=null,R()}async function Y(H){if(n.value===H){n.value=null,a.value=null,x.value=new Set;return}n.value=H,a.value=null,i.value=!0,x.value=new Set;try{a.value=await G.get(`/api/sessions/${encodeURIComponent(H)}`)}catch{a.value={messages:[],summary:""}}i.value=!1}function ee(H){const re=new Set(o.value);re.has(H)?re.delete(H):re.add(H),o.value=re}function te(){W.value?o.value=new Set:o.value=new Set(S.value.map(H=>H.channel_id))}function X(H){l.value=H}async function be(){if(l.value){r.value=!0;try{await G.del(`/api/sessions/${encodeURIComponent(l.value)}`),n.value===l.value&&(n.value=null,a.value=null),o.value.delete(l.value),await R()}catch(H){s.value=H.message||"Failed to clear session"}r.value=!1,l.value=null}}function le(){c.value=!0}async function ge(){if(o.value.size!==0){r.value=!0;try{await G.post("/api/sessions/clear-bulk",{channel_ids:[...o.value]}),o.value.has(n.value)&&(n.value=null,a.value=null),o.value=new Set,await R()}catch(H){s.value=H.message||"Failed to clear sessions"}r.value=!1,c.value=!1}}function xe(H,re){const ye=G._token;let Me=`/api/sessions/${encodeURIComponent(H)}/export?format=${re}`;ye&&(Me+=`&token=${encodeURIComponent(ye)}`);const Ue=document.createElement("a");Ue.href=Me,Ue.download=`session-${H}.${re==="text"?"txt":"json"}`,document.body.appendChild(Ue),Ue.click(),document.body.removeChild(Ue)}let ke=null;function Ee(H){H.payload&&H.payload.channel_id&&(clearTimeout(ke),ke=setTimeout(()=>{R(),n.value&&H.payload.channel_id===n.value&&G.get(`/api/sessions/${encodeURIComponent(n.value)}`).then(re=>{a.value=re}).catch(()=>{})},2e3))}return He(()=>{D(),R(),We.subscribe("events",Ee)}),gt(()=>{We.unsubscribe("events",Ee),clearTimeout(ke)}),{sessions:e,loading:t,error:s,expandedId:n,detail:a,detailLoading:i,clearTarget:l,clearing:r,selected:o,allSelected:W,bulkClearing:c,activePreset:u,searchQuery:d,sortBy:f,sortAsc:p,filterPresets:b,sortOptions:g,filteredSessions:S,hasActiveFilters:M,customPresets:T,showSavePreset:O,newPresetName:y,threadView:v,threads:U,collapsedThreads:x,ftsQuery:w,ftsChannelId:I,ftsUserId:L,ftsResults:k,ftsSearching:A,formatAge:fe,formatTimestamp:Q,formatFullTimestamp:ue,messageClass:Ie,threadMsgClass:z,roleBadge:oe,roleDotClass:ie,roleLabelClass:me,truncateContent:pe,threadSummary:F,fetchSessions:R,retry:$,toggleSession:Y,toggleSelect:ee,toggleSelectAll:te,confirmClear:X,clearSession:be,confirmBulkClear:le,doBulkClear:ge,exportSession:xe,applyPreset:C,applyCustomPreset:j,saveCustomPreset:ce,removeCustomPreset:de,resetFilters:se,toggleThread:N,runFtsSearch:Le,clearFtsSearch:m,highlightSnippet:E,ftsResultClass:P,ftsTypeBadge:Z}}},uw={props:["trace"],template:`
  ...
  `,setup(){return{formatTokens:M_}}},dw={components:{ContextAssemblyPanel:uw},template:`
    ...
    </div>`,setup(){const e=h([]),t=h([]),s=h(!0),n=h(null),a=h(null),i=h(null),l=h(""),r=h(""),o=h(0),c=h({}),u=h({channel_id:"",user_id:"",tool_name:"",errors_only:!1,limit:50});function d(L){if(!L)return"—";try{const k=new Date(L);return isNaN(k.getTime())?L:k.toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"})}catch{return L}}function f(L){return!L&&L!==0?"—":L<1e3?L+"ms":(L/1e3).toFixed(1)+"s"}function p(L){return!L&&L!==0?"—":L>=1e3?(L/1e3).toFixed(1)+"k":String(L)}function b(L){if(!L)return"";if(typeof L=="string")return L;try{return JSON.stringify(L,null,2)}catch{return String(L)}}function g(L){a.value===L?a.value=null:(a.value=L,c.value={})}function T(L,k){const A=L+"-"+k;c.value={...c.value,[A]:!c.value[A]}}function O(L,k){return!!c.value[L+"-"+k]}function y(){u.value={channel_id:"",user_id:"",tool_name:"",errors_only:!1,limit:50},r.value="",l.value="",i.value=null,w()}async function v(){try{const L=await G.get("/api/trajectories");e.value=L.files||[],o.value=L.count||0}catch{}}let x=0;async function w(){const L=++x;s.value=!0,n.value=null,a.value=null,i.value=null,c.value={};try{if(r.value){const k=await G.get(`/api/trajectories/${encodeURIComponent(r.value)}?limit=${u.value.limit}`);if(L!==x)return;let A=k.entries||[];u.value.tool_name&&(A=A.filter(D=>(D.tools_used||[]).includes(u.value.tool_name))),u.value.errors_only&&(A=A.filter(D=>D.is_error)),u.value.channel_id&&(A=A.filter(D=>D.channel_id===u.value.channel_id)),u.value.user_id&&(A=A.filter(D=>D.user_id===u.value.user_id)),t.value=A}else{const k=new URLSearchParams;u.value.channel_id&&k.set("channel_id",u.value.channel_id),u.value.user_id&&k.set("user_id",u.value.user_id),u.value.tool_name&&k.set("tool_name",u.value.tool_name),u.value.errors_only&&k.set("errors_only","true"),k.set("limit",String(u.value.limit));const A=k.toString(),D=await G.get(`/api/trajectories/search/query?${A}`);if(L!==x)return;t.value=D.results||[]}}catch(k){if(L!==x)return;n.value=k.message}L===x&&(s.value=!1)}async function I(){if(!l.value.trim())return;const L=++x;s.value=!0,n.value=null,c.value={};try{const k=await G.get(`/api/trajectories/message/${encodeURIComponent(l.value.trim())}`);if(L!==x)return;i.value=k.entry||null,i.value||(n.value="No trace found for this message ID")}catch(k){if(L!==x)return;k.status===404?(i.value=null,n.value="No trace found for message ID: "+l.value):n.value=k.message}L===x&&(s.value=!1)}return He(async()=>{await v(),await w()}),{files:e,entries:t,loading:s,error:n,expandedIdx:a,singleTrace:i,messageIdQuery:l,selectedFile:r,totalSaved:o,filters:u,expandedIterations:c,formatTs:d,formatDuration:f,formatTokens:p,formatJSON:b,truncateBlock:tg,toggleExpand:g,toggleIteration:T,isIterationExpanded:O,clearFilters:y,fetchFiles:v,fetchTraces:w,lookupMessage:I}}},fw={template:`
  ...
  `,setup(){const e=h(!0),t=h(null),s=h({by_user:{},by_channel:{},by_tool:{},recent:[],pricing:{}}),n=h({requests:0,input_tokens:0,output_tokens:0,total_tokens:0,cost_usd:0}),a=h("user");let i=null;const l=[{key:"user",label:"By User"},{key:"channel",label:"By Channel"},{key:"tool",label:"By Tool"},{key:"recent",label:"Recent"}],r=J(()=>[...s.value.recent||[]].reverse()),o=async()=>{try{const u=await G.get("/api/usage");s.value=u,n.value=u.totals||n.value,t.value=null}catch(u){t.value=u.message}finally{e.value=!1}},c=()=>{e.value=!0,o()};return He(()=>{o(),i=setInterval(o,15e3)}),gt(()=>{i&&clearInterval(i)}),{loading:e,error:t,data:s,totals:n,activeTab:a,tabs:l,recentReversed:r,fmtNum:sg,formatTime:Nc,retry:c}}},pw={components:{TabbedPage:gr},setup(){return{tabs:[{id:"audit",label:"Audit",component:rw},{id:"sessions",label:"Sessions",component:cw},{id:"traces",label:"Traces",component:dw},{id:"usage",label:"Usage",component:fw}]}},template:'<tabbed-page :tabs="tabs" default-tab="audit" group-label="History" />'},zr=[{id:"system",label:"System & Commands",icon:"terminal",match:e=>/^(run_command|run_script|read_file|write_file|list_directory|search_files|manage_process|file_|post_file)/.test(e)},{id:"devops",label:"DevOps & Infrastructure",icon:"server",match:e=>/^(git_ops|docker_ops|kubectl|terraform_ops|http_probe)/.test(e)},{id:"agents",label:"Agents & Orchestration",icon:"bot",match:e=>/^(spawn_agent|send_to_agent|wait_for_agents|get_agent_results|kill_agent|list_agents|spawn_loop_agents|collect_loop_agents)/.test(e)},{id:"workflow",label:"Workflows & Tasks",icon:"workflow",match:e=>/^(delegate_task|cancel_task|list_tasks|schedule_|start_loop|stop_loop|list_loops|delete_schedule|list_schedules|update_schedule|parse_time)/.test(e)},{id:"network",label:"Network & Web",icon:"globe",match:e=>/^(web_|browser_|search_web|fetch_url|http_)/.test(e)},{id:"knowledge",label:"Knowledge & Search",icon:"book",match:e=>/^(search_knowledge|ingest_|knowledge_|search_history|search_audit|bulk_ingest|delete_knowledge|list_knowledge)/.test(e)},{id:"discord",label:"Discord & Admin",icon:"message",match:e=>/^(send_|add_reaction|create_poll|purge_|discord_|embed_|read_channel|set_permission)/.test(e)},{id:"skills",label:"Skills",icon:"puzzle",match:e=>/^(create_skill|edit_skill|delete_skill|enable_skill|disable_skill|install_skill|export_skill|skill_status|invoke_skill|list_skills)/.test(e)},{id:"memory",label:"Memory & State",icon:"brain",match:e=>/^(memory_manage|list_manage)/.test(e)},{id:"ai",label:"AI & Generation",icon:"sparkles",match:e=>/^(generate_|analyze_|claude_|vision_|comfyui_)/.test(e)},{id:"integrations",label:"Integrations",icon:"link",match:e=>/^(issue_tracker|slack_|grafana_|mcp_)/.test(e)},{id:"other",label:"Other Tools",icon:"wrench",match:()=>!0}],hw={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(""),a=h({}),i=h({}),l=h("cards"),r=h(null),o=J(()=>e.value.filter(y=>y.is_core).length),c=J(()=>e.value.filter(y=>!y.is_core).length),u=J(()=>Object.values(a.value).reduce((y,v)=>y+v,0));function d(y){for(const v of zr)if(v.id!=="other"&&v.match(y))return v.id;return"other"}const f=J(()=>{let y=e.value;if(n.value){const v=n.value.toLowerCase();y=y.filter(x=>x.name.toLowerCase().includes(v)||(x.description||"").toLowerCase().includes(v))}return r.value&&(y=y.filter(v=>d(v.name)===r.value)),y}),p=J(()=>{const y=new Set;for(const v of e.value)y.add(d(v.name));return zr.filter(v=>y.has(v.id))}),b=J(()=>{const y=f.value,v={};for(const w of y){const I=d(w.name);v[I]||(v[I]=[]),v[I].push(w)}const x=[];for(const w of zr)v[w.id]&&v[w.id].length>0&&x.push({label:w.label,icon:w.icon,tools:v[w.id].sort((I,L)=>I.name.localeCompare(L.name))});return x});function g(y){i.value={...i.value,[y]:!i.value[y]}}async function T(){t.value=!0,s.value=null;try{const[y,v]=await Promise.all([G.get("/api/tools"),G.get("/api/tools/stats").catch(()=>({}))]);e.value=y,a.value=v||{};const x=Object.values(v||{}).filter(w=>w>0).sort((w,I)=>w-I)}catch(y){s.value=y.message}t.value=!1}function O(){T()}return He(()=>{T()}),{tools:e,loading:t,error:s,search:n,stats:a,expanded:i,viewMode:l,activeCategory:r,coreCount:o,skillCount:c,totalUsage:u,filteredTools:f,groupedTools:b,usedCategories:p,truncate:Lc,toggleExpand:g,refresh:O}}};function gw(e){if(!e)return"";let t=e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");t=t.replace(/("""[\s\S]*?"""|'''[\s\S]*?'''|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g,'<span class="sk-str">$1</span>'),t=t.replace(/(#[^\n]*)/g,'<span class="sk-cmt">$1</span>');const s="\\b(def|class|return|if|elif|else|for|while|import|from|as|try|except|finally|raise|with|async|await|yield|pass|break|continue|and|or|not|in|is|None|True|False|self|lambda)\\b";t=t.replace(new RegExp(s,"g"),'<span class="sk-kw">$1</span>');const n="\\b(print|len|range|str|int|float|list|dict|set|tuple|type|isinstance|hasattr|getattr|setattr|super|property|staticmethod|classmethod|enumerate|zip|map|filter|sorted|reversed|any|all|min|max|sum|abs|round|open|format)\\b";return t=t.replace(new RegExp(n,"g"),'<span class="sk-builtin">$1</span>'),t=t.replace(/(@\w+)/g,'<span class="sk-dec">$1</span>'),t=t.replace(/\b(\d+\.?\d*)\b/g,'<span class="sk-num">$1</span>'),t}function mw(e){if(!e)return"1";const t=e.split(`
...
`)}const vw={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h({}),a=h({}),i=h(null),l=h(""),r=h(null),o=h(!1),c=h("create"),u=h(""),d=h(""),f=h(null),p=h(null),b=h(!1),g=h(null),T=h(null),O=h(!1),y=J(()=>e.value.length),v=J(()=>e.value.reduce((Q,ue)=>Q+(ue.execution_count||0),0)),x=J(()=>e.value.reduce((Q,ue)=>Q+D(ue.code),0)),w=J(()=>{if(!l.value)return e.value;const Q=l.value.toLowerCase();return e.value.filter(ue=>ue.name.toLowerCase().includes(Q)||(ue.description||"").toLowerCase().includes(Q))}),I=J(()=>d.value?d.value.split(`
`).length:0),L=J(()=>{const Q=Math.max(I.value,1);return Array.from({length:Q},(ue,Ie)=>Ie+1).join(`
`)}),k=J(()=>{const Q=d.value.trim();return Q?Q.includes("SKILL_DEFINITION")?Q.includes("async def execute")?{valid:!0,message:""}:{valid:!1,message:"Missing async def execute function"}:{valid:!1,message:"Missing SKILL_DEFINITION dict"}:null});function A(Q){return gw(Q)}function D(Q){return Q?Q.split(`
`).length:0}function B(Q){return mw(Q)}function M(Q){n.value={...n.value,[Q]:!n.value[Q]}}async function S(Q){try{await navigator.clipboard.writeText(Q);const ue=e.value.find(Ie=>Ie.code===Q);ue&&(r.value=ue.name,setTimeout(()=>{r.value=null},2e3))}catch{}}function U(Q){if(Q.key==="Tab"){Q.preventDefault();const ue=Q.target,Ie=ue.selectionStart,z=ue.selectionEnd;d.value=d.value.substring(0,Ie)+"    "+d.value.substring(z),St(()=>{ue.selectionStart=ue.selectionEnd=Ie+4})}}function W(Q){const ue=Q.target.previousElementSibling;ue&&(ue.scrollTop=Q.target.scrollTop)}async function F(){t.value=!0,s.value=null;try{e.value=await G.get("/api/skills")}catch(Q){s.value=Q.message}t.value=!1}async function N(Q){i.value=Q,delete a.value[Q],a.value={...a.value};try{const ue=await G.post(`/api/skills/${encodeURIComponent(Q)}/test`);a.value={...a.value,[Q]:ue}}catch(ue){a.value={...a.value,[Q]:{result:ue.message,is_error:!0}}}i.value=null}function C(){o.value=!0,c.value="create",u.value="",d.value="",f.value=null,p.value=null}function j(Q){o.value=!0,c.value="edit",u.value=Q.name,d.value=Q.code||"",f.value=null,p.value=null}function ce(){o.value=!1,f.value=null,p.value=null}async function de(){f.value=null,p.value=null;const Q=u.value.trim(),ue=d.value.trim();if(!Q){f.value="Name is required";return}if(!ue){f.value="Code is required";return}b.value=!0;try{c.value==="create"?(await G.post("/api/skills",{name:Q,code:ue}),p.value="Skill created successfully"):(await G.put(`/api/skills/${encodeURIComponent(Q)}`,{code:ue}),p.value="Skill updated successfully"),await F(),setTimeout(()=>{o.value=!1},800)}catch(Ie){f.value=Ie.message}b.value=!1}function se(Q){T.value=Q}async function fe(){if(T.value){O.value=!0;try{await G.del(`/api/skills/${encodeURIComponent(T.value)}`),await F()}catch{}O.value=!1,T.value=null}}return He(()=>{F()}),{skills:e,loading:t,error:s,showCode:n,testResults:a,testing:i,search:l,copied:r,editing:o,editMode:c,editName:u,editCode:d,editError:f,editSuccess:p,saving:b,editorRef:g,deleteTarget:T,deleting:O,enabledCount:y,totalExecutions:v,totalLines:x,displayedSkills:w,editLineCount:I,editorLineNums:L,editValidation:k,highlight:A,truncate:Lc,formatTs:Fa,countLines:D,getLineNumbers:B,toggleCode:M,copyCode:S,handleEditorKey:U,syncScroll:W,fetchSkills:F,testSkill:N,showCreate:C,editSkill:j,cancelEdit:ce,saveSkill:de,confirmDelete:se,doDelete:fe}}};function bw(e,t){if(!e||!t)return Sd(e);const s=Sd(e),n=t.trim().split(/\s+/).filter(Boolean);if(!n.length)return s;const a=n.map(i=>i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")).join("|");try{return s.replace(new RegExp(`(${a})`,"gi"),'<mark class="knowledge-highlight">$1</mark>')}catch{return s}}const yw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(""),a=h(null),i=h(!1),l=h(""),r=h(null),o=h(!1),c=h(""),u=h(""),d=h(null),f=h(null),p=h(!1),b=h(null),g=h(null);let T=null;const O=h(null),y=h(!1),v=h({}),x=h({}),w=h(null),I=h(null),L=J(()=>e.value.reduce((C,j)=>C+(j.chunks||0),0)),k=J(()=>new Set(e.value.map(j=>j.uploader).filter(Boolean)).size);function A(C,j){const ce=x.value[j];if(!ce||ce.length===0)return 0;const de=Math.max(...ce.map(se=>se.char_count||0));return de===0?0:Math.round(C.char_count/de*100)}async function D(){t.value=!0,s.value=null;try{const C=await G.get("/api/knowledge");e.value=Array.isArray(C)?C:[]}catch(C){s.value=C.message}t.value=!1}async function B(C){if(v.value[C]){v.value[C]=!1,I.value=null;return}if(v.value[C]=!0,!(x.value[C]||w.value===C)){w.value=C;try{const j=await G.get(`/api/knowledge/${encodeURIComponent(C)}/chunks`);x.value[C]=Array.isArray(j)?j:[]}catch(j){x.value[C]=[],_e.error(`Failed to load chunks: ${j.message}`)}w.value=null}}async function M(){const C=n.value.trim();if(C){i.value=!0,r.value=null,l.value=C;try{const j=await G.get(`/api/knowledge/search?q=${encodeURIComponent(C)}`);a.value=Array.isArray(j)?j:[]}catch(j){a.value=[],r.value=j.message||"Search failed"}i.value=!1}}function S(){a.value=null,n.value="",r.value=null}async function U(){d.value=null,f.value=null;const C=c.value.trim(),j=u.value.trim();if(!C){d.value="Source name is required";return}if(!j){d.value="Content is required";return}p.value=!0;try{const ce=await G.post("/api/knowledge",{source:C,content:j});f.value=`Ingested ${ce.chunks||0} chunks from "${C}"`,c.value="",u.value="",x.value={},await D(),setTimeout(()=>{o.value=!1,f.value=null},1500)}catch(ce){d.value=ce.message}p.value=!1}async function W(C){b.value=C,g.value=null,T&&(clearTimeout(T),T=null);try{const j=await G.post(`/api/knowledge/${encodeURIComponent(C)}/reingest`);g.value={source:C,error:!1,message:`Re-ingested ${j.chunks||0} chunks`},delete x.value[C],await D(),T=setTimeout(()=>{g.value=null,T=null},3e3)}catch(j){g.value={source:C,error:!0,message:j.message}}b.value=null}function F(C){O.value=C}async function N(){if(O.value){y.value=!0;try{await G.del(`/api/knowledge/${encodeURIComponent(O.value)}`),delete x.value[O.value],await D()}catch{}y.value=!1,O.value=null}}return He(()=>{D()}),{sources:e,loading:t,error:s,searchQuery:n,searchResults:a,searching:i,lastQuery:l,searchError:r,showIngest:o,ingestSource:c,ingestContent:u,ingestError:d,ingestSuccess:f,ingesting:p,reingesting:b,reingestResult:g,deleteTarget:O,deleting:y,expanded:v,sourceChunks:x,loadingChunks:w,selectedChunk:I,totalChunks:L,uploaderCount:k,truncate:Lc,formatTs:Fa,highlightTerms:bw,chunkBarWidth:A,fetchSources:D,toggleSource:B,doSearch:M,clearSearch:S,doIngest:U,doReingest:W,confirmDelete:F,doDelete:N}}},xw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h({}),s=h(!0),n=h(null),a=h({}),i=h(null),l=h(""),r=h(!1),o=h({scope:"global",key:"",value:""}),c=h(!1),u=h(null),d=h(null),f=h(null),p=h(""),b=h(!1),g=h(null),T=h(null),O=h(new Set),y=h(null),v=h(!1),x=h(!1),w=J(()=>e.value.reduce((se,fe)=>se+fe.count,0)),I=J(()=>O.value.size);function L(se){const fe=t.value[se];if(!fe)return[];if(!l.value.trim())return fe;const Q=l.value.trim().toLowerCase();return fe.filter(ue=>ue.key.toLowerCase().includes(Q)||ue.value&&ue.value.toLowerCase().includes(Q))}function k(se,fe){return O.value.has(se+"/"+fe)}function A(se,fe){const Q=se+"/"+fe,ue=new Set(O.value);ue.has(Q)?ue.delete(Q):ue.add(Q),O.value=ue}function D(se){const fe=t.value[se];return!fe||fe.length===0?!1:fe.every(Q=>O.value.has(se+"/"+Q.key))}function B(se,fe){const Q=t.value[se];if(!Q)return;const ue=new Set(O.value);for(const Ie of Q){const z=se+"/"+Ie.key;fe?ue.add(z):ue.delete(z)}O.value=ue}async function M(){s.value=!0,n.value=null;try{const se=await G.get("/api/memory");e.value=Object.entries(se).map(([fe,Q])=>({name:fe,keys:Q.keys||[],count:Q.count||0}))}catch(se){n.value=se.message}s.value=!1}async function S(se){if(a.value[se]){a.value[se]=!1;return}a.value[se]=!0;const fe=e.value.find(ue=>ue.name===se);if(!fe||t.value[se]||i.value===se)return;i.value=se;const Q=await Promise.all(fe.keys.map(async ue=>{try{const Ie=await G.get(`/api/memory/${encodeURIComponent(se)}/${encodeURIComponent(ue)}`);return{key:ue,value:Ie.value||""}}catch{return{key:ue,value:"(error loading)"}}}));t.value[se]=Q,i.value=null}function U(se,fe,Q){f.value=se+"/"+fe,p.value=Q}async function W(se,fe){b.value=!0,g.value=null;try{await G.put(`/api/memory/${encodeURIComponent(se)}/${encodeURIComponent(fe)}`,{value:p.value});const Q=t.value[se];if(Q){const ue=Q.find(Ie=>Ie.key===fe);ue&&(ue.value=p.value)}f.value=null}catch(Q){g.value=`Failed to save: ${Q.message||"unknown error"}`}b.value=!1}async function F(se,fe){try{await navigator.clipboard.writeText(fe.value),T.value=se+"/"+fe.key,setTimeout(()=>{T.value=null},1500)}catch{}}async function N(){u.value=null,d.value=null;const se=o.value.scope.trim(),fe=o.value.key.trim(),Q=o.value.value.trim();if(!se){u.value="Scope is required";return}if(!fe){u.value="Key is required";return}if(!Q){u.value="Value is required";return}c.value=!0;try{await G.put(`/api/memory/${encodeURIComponent(se)}/${encodeURIComponent(fe)}`,{value:Q}),d.value="Entry saved",o.value={scope:"global",key:"",value:""},t.value={},await M(),setTimeout(()=>{r.value=!1,d.value=null},800)}catch(ue){u.value=ue.message}c.value=!1}function C(se,fe){y.value={scope:se,key:fe}}async function j(){if(!y.value)return;v.value=!0,g.value=null;const{scope:se,key:fe}=y.value;try{await G.del(`/api/memory/${encodeURIComponent(se)}/${encodeURIComponent(fe)}`);const Q=t.value[se];Q&&(t.value[se]=Q.filter(z=>z.key!==fe));const ue=e.value.find(z=>z.name===se);ue&&(ue.count--,ue.keys=ue.keys.filter(z=>z!==fe));const Ie=new Set(O.value);Ie.delete(se+"/"+fe),O.value=Ie}catch(Q){g.value=`Failed to delete: ${Q.message||"unknown error"}`}v.value=!1,y.value=null}function ce(){x.value=!0}async function de(){v.value=!0,g.value=null;const se=[];for(const fe of O.value){const Q=fe.indexOf("/");se.push({scope:fe.slice(0,Q),key:fe.slice(Q+1)})}try{await G.post("/api/memory/bulk-delete",{entries:se}),O.value=new Set,t.value={},await M()}catch(fe){g.value=`Bulk delete failed: ${fe.message||"unknown error"}`}v.value=!1,x.value=!1}return He(()=>{M()}),{scopes:e,scopeEntries:t,loading:s,error:n,expanded:a,loadingScope:i,filterQuery:l,showAdd:r,addForm:o,adding:c,addError:u,addSuccess:d,editingKey:f,editValue:p,saving:b,actionError:g,copied:T,selected:O,selectedCount:I,totalEntries:w,deleteTarget:y,deleting:v,showBulkDelete:x,fetchMemory:M,toggleScope:S,startEdit:U,doEdit:W,copyValue:F,doAdd:N,confirmDelete:C,doDelete:j,confirmBulkDelete:ce,doBulkDelete:de,isSelected:k,toggleSelect:A,isScopeAllSelected:D,toggleSelectAll:B,filteredEntries:L}}},_w={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
        <div>
  ...
  `,setup(){const e=h([]),t=h(null),s=h(!0),n=h(null),a=h(null),i=h(null),l=h(""),r=J(()=>[...new Set(e.value.map(T=>T.category))].sort()),o=J(()=>{const g={};return e.value.forEach(T=>{g[T.category]=(g[T.category]||0)+1}),g}),c=J(()=>a.value?e.value.filter(g=>g.category===a.value):e.value);function u(g){return g==="correction"?"badge-warning":g==="operational"?"badge-info":g==="preference"?"badge-success":"badge-info"}function d(g){i.value=g.key,l.value=g.content}async function f(g){try{await G.put("/api/learned/"+encodeURIComponent(g),{content:l.value}),i.value=null,_e.success("Entry updated"),await b()}catch(T){_e.error(T.message||"Failed to save entry")}}async function p(g){if(await rs({title:"Delete learned entry",message:`Delete "${g}"? Odin will no longer apply this learned context.`,confirmLabel:"Delete",danger:!0}))try{await G.del("/api/learned/"+encodeURIComponent(g)),_e.success("Entry deleted"),await b()}catch(O){_e.error(O.message||"Failed to delete entry")}}async function b(){s.value=!0,n.value=null;try{const g=await G.get("/api/learned");e.value=g.entries||[],t.value={last_reflection:g.last_reflection,count:g.count}}catch(g){n.value=g.message}s.value=!1}return He(b),{entries:e,meta:t,loading:s,error:n,filterCat:a,editing:i,editContent:l,categories:r,catCounts:o,filtered:c,catBadge:u,formatTs:Fa,startEdit:d,saveEdit:f,deleteEntry:p,fetchEntries:b}}},kw={components:{TabbedPage:gr},setup(){return{tabs:[{id:"tools",label:"Tools",component:hw},{id:"skills",label:"Skills",component:vw},{id:"knowledge",label:"Knowledge",component:yw},{id:"memory",label:"Memory",component:xw},{id:"learned",label:"Learned",component:_w}]}},template:'<tabbed-page :tabs="tabs" default-tab="tools" group-label="Capabilities" />'},ww={setup(){const e=h("odin"),t=h(""),s=h(""),n=h(""),a=h({}),i=h([]),l=h([]),r=h(!1),o=h(!1),c=h(null),u=h(!0),d=h(""),f=h(!1),p=h(!1),b=J(()=>e.value==="custom"),g=J(()=>[...i.value,...l.value]),T=J(()=>l.value.includes(e.value)),O=J(()=>{var k;return b.value?t.value||"Odin":((k=a.value[e.value])==null?void 0:k.name)||e.value}),y=J(()=>{var k;return b.value?s.value||"(empty — will use Odin default)":((k=a.value[e.value])==null?void 0:k.identity)||""}),v=J(()=>{var k;return b.value?n.value||"(empty — will use Odin default)":((k=a.value[e.value])==null?void 0:k.voice)||""});async function x(){u.value=!0;try{const k=await G.get("/api/personality");e.value=k.preset||"odin",t.value=k.custom_name||"",s.value=k.custom_identity||"",n.value=k.custom_voice||"",a.value=k.presets||{},i.value=k.builtin_presets||[],l.value=k.user_presets||[]}catch(k){c.value=k.message}finally{u.value=!1}}async function w(){r.value=!0,c.value=null,o.value=!1;try{await G.put("/api/personality",{preset:e.value,custom_name:t.value,custom_identity:s.value,custom_voice:n.value}),o.value=!0,setTimeout(()=>o.value=!1,3e3)}catch(k){c.value=k.message}finally{r.value=!1}}async function I(){const k=d.value.trim();if(k){p.value=!0,c.value=null;try{await G.post("/api/personality/presets",{name:k,display_name:O.value,identity:y.value,voice:v.value}),f.value=!1,d.value="",await x(),e.value=k.toLowerCase().replace(/ /g,"_")}catch(A){c.value=A.message}finally{p.value=!1}}}async function L(){if(await rs({title:"Delete preset",message:`Delete preset "${e.value}"? This cannot be undone.`,confirmLabel:"Delete",danger:!0})){c.value=null;try{await G.del(`/api/personality/presets/${encodeURIComponent(e.value)}`),await x(),e.value="odin"}catch(A){c.value=A.message}}}return He(x),{preset:e,customName:t,customIdentity:s,customVoice:n,presets:a,presetNames:g,isCustom:b,isUserPreset:T,previewName:O,previewIdentity:y,previewVoice:v,saving:r,saved:o,error:c,loading:u,save:w,showSavePreset:f,newPresetName:d,savingPreset:p,saveAsPreset:I,deletePreset:L,builtinPresets:i,userPresets:l}},template:`
  ...
  `,setup(){const e=h(!0),t=h(null),s=h(!1),n=h("sessions"),a=h(null);let i=null;const l=[{key:"sessions",label:"Sessions"},{key:"knowledge",label:"Knowledge"},{key:"trajectories",label:"Trajectories"},{key:"storage",label:"Storage"}],r=J(()=>{if(!a.value||!a.value.collected_at)return"";try{return new Date(a.value.collected_at).toLocaleTimeString()}catch{return""}}),o=J(()=>{if(!a.value)return[];const f=a.value,p=f.storage_total_bytes||1;return[{label:"Session Persistence",mb:f.sessions.persist_dir.total_mb,bytes:f.sessions.persist_dir.total_bytes,files:f.sessions.persist_dir.file_count,pct:Math.min(100,Math.round(f.sessions.persist_dir.total_bytes/p*100)),color:"res-bar-blue"},{label:"Knowledge Database",mb:f.knowledge.db_file.total_mb,bytes:f.knowledge.db_file.total_bytes,files:f.knowledge.db_file.file_count,pct:Math.min(100,Math.round(f.knowledge.db_file.total_bytes/p*100)),color:"res-bar-purple"},{label:"Message Trajectories",mb:f.trajectories.message_dir.total_mb,bytes:f.trajectories.message_dir.total_bytes,files:f.trajectories.message_dir.file_count,pct:Math.min(100,Math.round(f.trajectories.message_dir.total_bytes/p*100)),color:"res-bar-emerald"},{label:"Agent Trajectories",mb:f.trajectories.agent_dir.total_mb,bytes:f.trajectories.agent_dir.total_bytes,files:f.trajectories.agent_dir.file_count,pct:Math.min(100,Math.round(f.trajectories.agent_dir.total_bytes/p*100)),color:"res-bar-amber"}]});async function c(){try{const f=await G.get("/api/resource-usage");a.value=f,t.value=null}catch(f){t.value=f.message||"Failed to load resource usage"}finally{e.value=!1,s.value=!1}}async function u(){s.value=!0,await c()}function d(){e.value=!0,t.value=null,c()}return He(()=>{c(),i=setInterval(c,3e4)}),gt(()=>{i&&clearInterval(i)}),{loading:e,error:t,refreshing:s,data:a,activeTab:n,tabs:l,collectedAt:r,storageItems:o,fmtNum:sg,refresh:u,retry:d}}},Rw=["INFO","WARNING","ERROR"],Iw=[{id:"all",name:"All Logs",icon:"list",filters:{}},{id:"errors",name:"Errors Only",icon:"error",filters:{level:"ERROR"}},{id:"warnings",name:"Warnings+",icon:"warning",filters:{levels:["WARNING","ERROR"]}},{id:"tools",name:"Tool Activity",icon:"wrench",filters:{hasToolName:!0}},{id:"recent-errors",name:"Recent Errors",icon:"flame",filters:{level:"ERROR",timeRange:"last_1h"}}],qr=[{value:"",label:"All Time"},{value:"last_5m",label:"Last 5 min",seconds:300},{value:"last_15m",label:"Last 15 min",seconds:900},{value:"last_1h",label:"Last 1 hour",seconds:3600},{value:"last_4h",label:"Last 4 hours",seconds:14400},{value:"last_24h",label:"Last 24 hours",seconds:86400}],Nw=[50,100,200,500],Lw={template:`
    ...
    </div>`,setup(){const e=h("live"),t=h([]),s=h(!1),n=h(!0),a=h(""),i=h(""),l=h(!1),r=h(!1),o=h(We.state||"disconnected"),c=J(()=>{switch(o.value){case"connected":return"Live";case"connecting":return"Connecting…";case"reconnecting":return"Reconnecting…";default:return"Disconnected"}}),u=h(null),d=h(!1),f=h(null),p=2e3,b=Rw,g=Iw,T=qr,O=h("all"),y=h(""),v=h([]),x=h(!1),w=h(""),I=h([]);function L(){try{const V=localStorage.getItem("odin-log-presets");V&&(v.value=JSON.parse(V))}catch{}}function k(){try{localStorage.setItem("odin-log-presets",JSON.stringify(v.value))}catch{}}const A=J(()=>a.value!==""||i.value.trim()!==""||y.value!==""),D=J(()=>{const V=qr.find(ne=>ne.value===y.value);return V?V.label:""}),B=J(()=>{if(!l.value||!i.value)return null;try{return new RegExp(i.value,"i"),null}catch(V){return V.message}}),M=24,S=J(()=>{if(t.value.length===0)return[];const V=[],ne=new Date,Te=3600*1e3;for(let Xe=M-1;Xe>=0;Xe--){const rt=new Date(ne.getTime()-(Xe+1)*Te),Zt=new Date(ne.getTime()-Xe*Te);V.push({start:rt,end:Zt,label:N(rt,Zt),shortLabel:Zt.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),total:0,info:0,warnings:0,errors:0})}for(const Xe of t.value){if(!Xe._time)continue;const rt=Xe._time.getTime();for(const Zt of V)if(rt>=Zt.start.getTime()&&rt<Zt.end.getTime()){Zt.total++,Xe.level==="ERROR"?Zt.errors++:Xe.level==="WARNING"?Zt.warnings++:Zt.info++;break}}return V}),U=J(()=>{let V=1;for(const ne of S.value)ne.total>V&&(V=ne.total);return V}),W=J(()=>S.value.length===0?"":"Last 24 hours"),F=J(()=>Math.ceil(M/8));function N(V,ne){const Te={hour:"2-digit",minute:"2-digit"};return V.toLocaleTimeString([],Te)+" - "+ne.toLocaleTimeString([],Te)}function C(V,ne){return!ne||!V?"0px":Math.max(2,V/ne*100)+"%"}function j(V){const ne=ce.value.findIndex(Te=>Te._time&&Te._time.getTime()>=V.start.getTime()&&Te._time.getTime()<V.end.getTime());if(ne>=0&&u.value){const Te=u.value.querySelectorAll(".log-line");Te[ne]&&(Te[ne].scrollIntoView({behavior:"smooth",block:"center"}),n.value=!1)}}const ce=J(()=>{let V=t.value;if(a.value&&(V=V.filter(ne=>(ne.level||"INFO")===a.value)),y.value){const ne=qr.find(Te=>Te.value===y.value);if(ne&&ne.seconds){const Te=new Date(Date.now()-ne.seconds*1e3);V=V.filter(Xe=>Xe._time&&Xe._time>=Te)}}if(i.value&&!B.value)if(l.value)try{const ne=new RegExp(i.value,"i");V=V.filter(Te=>{const Xe=Te.text||Te.raw||"",rt=Te.tool||"";return ne.test(Xe)||ne.test(rt)})}catch{}else{const ne=i.value.toLowerCase();V=V.filter(Te=>{const Xe=(Te.text||Te.raw||"").toLowerCase(),rt=(Te.tool||"").toLowerCase();return Xe.includes(ne)||rt.includes(ne)})}return V});function de(V){if(V.type==="log"&&V.line)try{const ne=typeof V.line=="string"?JSON.parse(V.line):V.line,Te=ne.timestamp?new Date(ne.timestamp):new Date;return{ts:Te.toLocaleTimeString(),_time:Te,level:ne.error?"ERROR":"INFO",text:ne.tool_name?`[${ne.tool_name}] ${ne.result_summary||""}`.trim():ne.message||JSON.stringify(ne),tool:ne.tool_name||"",raw:null}}catch{return{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:String(V.line),tool:"",raw:String(V.line)}}if(V.payload){const ne=V.payload,Te=ne.timestamp?new Date(ne.timestamp):new Date;return{ts:Te.toLocaleTimeString(),_time:Te,level:ne.error?"ERROR":"INFO",text:ne.tool_name?`[${ne.tool_name}] ${ne.result_summary||""}`.trim():ne.message||JSON.stringify(ne),tool:ne.tool_name||"",raw:null}}return typeof V=="string"?{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:V,tool:"",raw:V}:{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:JSON.stringify(V),tool:"",raw:null}}function se(V){const ne=de(V);if(s.value){I.value.push(ne);return}fe(ne)}function fe(V){t.value.push(V),t.value.length>p&&(t.value=t.value.slice(-p)),n.value&&St(()=>Q())}function Q(V=!1){const ne=u.value;ne&&ne.scrollTo({top:ne.scrollHeight,behavior:V?"smooth":"instant"})}function ue(){n.value=!0,d.value=!1,St(()=>Q(!0))}const Ie=new Set(["PageUp","PageDown","ArrowUp","ArrowDown","Home","End"," "]);function z(){const V=u.value;if(!V)return;const ne=V.scrollHeight-V.scrollTop-V.clientHeight<40;d.value=!n.value&&!ne&&t.value.length>0,pe.value&&oe()}function oe(){const V=u.value;!V||!n.value||V.scrollHeight-V.scrollTop-V.clientHeight>=40&&(n.value=!1,d.value=t.value.length>0)}function ie(){n.value&&requestAnimationFrame(oe)}function me(V){Ie.has(V.key)&&ie()}const pe=h(!1);function Le(){n.value&&(pe.value=!0,requestAnimationFrame(oe))}function m(){pe.value&&(pe.value=!1,oe())}function E(){n.value&&(d.value=!1,St(()=>Q()))}function P(){if(s.value=!s.value,!s.value&&I.value.length>0){for(const V of I.value)fe(V);I.value=[]}}function Z(){t.value=[],I.value=[],d.value=!1}function R(){let V;e.value==="search"?V=Wt.value.map(rt=>{const Zt=rt.error?"ERROR":"INFO",ns=rt.tool_name?`[${rt.tool_name}] `:"";return`${rt.timestamp||""} ${Zt} ${ns}${rt.result_summary||rt.message||""}`}).join(`
`):V=ce.value.map(rt=>`${rt.ts} ${rt.level} ${rt.text}`).join(`
`);const ne=new Blob([V],{type:"text/plain"}),Te=URL.createObjectURL(ne),Xe=document.createElement("a");Xe.href=Te,Xe.download=`odin-logs-${new Date().toISOString().slice(0,19).replace(/:/g,"-")}.txt`,Xe.click(),URL.revokeObjectURL(Te)}function $(V,ne){const Te=`${V.ts} ${V.level} ${V.text||V.raw||""}`;navigator.clipboard.writeText(Te).then(()=>{f.value=ne,setTimeout(()=>{f.value=null},1500)}).catch(()=>{})}function Y(V){a.value=a.value===V?"":V,O.value="all"}function ee(V){return V.level==="ERROR"?"log-line-error":V.level==="WARNING"?"log-line-warning":"text-gray-300"}function te(V){return V==="ERROR"?"text-red-500 font-semibold":V==="WARNING"?"text-yellow-500":"text-blue-500"}function X(V){return V==="ERROR"?"log-chip-error":V==="WARNING"?"log-chip-warning":"log-chip-info"}function be(V){O.value=V.id;const ne=V.filters;a.value=ne.level||"",y.value=ne.timeRange||"",i.value=ne.text||"",ne.levels&&(a.value=ne.levels[0]||""),ne.hasToolName&&(i.value="")}function le(V){O.value=V.id,a.value=V.filters.level||"",y.value=V.filters.timeRange||"",i.value=V.filters.text||""}function ge(){if(!w.value.trim())return;const V={id:"custom-"+Date.now(),name:w.value.trim(),filters:{level:a.value,timeRange:y.value,text:i.value}};v.value=[...v.value,V],k(),x.value=!1,w.value=""}function xe(V){v.value=v.value.filter(ne=>ne.id!==V),k(),O.value===V&&(O.value="all")}const ke=h("all"),Ee=h(""),H=h(""),re=h(""),ye=h(""),Me=h(""),Ue=h(100),Je=Nw,dt=h(!1),Ye=h(!1),Ke=h(""),Wt=h([]),bs=h(null),Ts=h(null);function Cn(){e.value="search",bs.value||Qn()}async function Qn(){try{bs.value=await G.get("/api/logs/stats")}catch{}}function cn(){const V=Me.value;if(!V){re.value="",ye.value="";return}const Te={last_5m:300,last_15m:900,last_1h:3600,last_4h:14400,last_24h:86400,last_7d:604800}[V];if(Te){const Xe=new Date(Date.now()-Te*1e3);re.value=un(Xe),ye.value=""}}function un(V){const ne=Te=>String(Te).padStart(2,"0");return`${V.getFullYear()}-${ne(V.getMonth()+1)}-${ne(V.getDate())}T${ne(V.getHours())}:${ne(V.getMinutes())}`}function q(V){if(!V)return"";const ne=new Date(V);return isNaN(ne.getTime())?"":ne.toISOString()}async function Se(){dt.value=!0,Ke.value="",Ye.value=!0,Ts.value=null;try{const V=new URLSearchParams;ke.value&&ke.value!=="all"&&V.set("level",ke.value),Ee.value&&V.set("tool",Ee.value),H.value&&V.set("q",H.value);const ne=q(re.value),Te=q(ye.value);ne&&V.set("start",ne),Te&&V.set("end",Te),V.set("limit",String(Ue.value));const Xe=await G.get(`/api/logs/search?${V.toString()}`);Wt.value=Xe.entries||[]}catch(V){Ke.value=V.message||"Search failed",Wt.value=[]}finally{dt.value=!1}}function Cs(){ke.value="all",Ee.value="",H.value="",re.value="",ye.value="",Me.value="",Ue.value=100,Wt.value=[],Ye.value=!1,Ke.value="",Ts.value=null}function En(V){Ts.value=Ts.value===V?null:V}function mr(V){if(!V.timestamp)return"";try{return new Date(V.timestamp).toLocaleString()}catch{return V.timestamp}}function $a(V){return V.type==="web_action"?`${V.status||""} (${V.execution_time_ms||0}ms)`:(V.result_summary||"").slice(0,200)}function Ua(V){return V.error?"log-line-error":"text-gray-300"}function vr(V){try{return JSON.stringify(V,null,2)}catch{return String(V)}}let ys=null,Xn=null,ea=!1;function it(){ea||(ea=!0,We.subscribe("logs",se),r.value=We.connected,o.value=We.state||"disconnected",ys=We.onStateChange,Xn=(V,ne)=>{o.value=V,r.value=V==="connected",ys&&ys(V,ne)},We.onStateChange=Xn)}function Fs(){ea&&(ea=!1,We.unsubscribe("logs",se),We.onStateChange===Xn&&(We.onStateChange=ys),Xn=null,ys=null)}return He(()=>{L(),window.addEventListener("pointerup",m),window.addEventListener("pointercancel",m)}),zo(it),qo(Fs),gt(()=>{Fs(),window.removeEventListener("pointerup",m),window.removeEventListener("pointercancel",m)}),{mode:e,logs:t,paused:s,autoScroll:n,levelFilter:a,textFilter:i,useRegex:l,subscribed:r,wsState:o,wsStateLabel:c,logContainer:u,filteredLogs:ce,pauseBuffer:I,showJumpBottom:d,copiedIndex:f,regexError:B,levels:b,logPresets:g,timeRanges:T,timeRange:y,activeLogPreset:O,customLogPresets:v,showSaveLogPreset:x,newLogPresetName:w,hasActiveLogFilters:A,timeRangeLabel:D,timelineBuckets:S,timelineMax:U,timelineSpanLabel:W,timelineLabelSkip:F,togglePause:P,clearLogs:Z,exportLogs:R,logLineClass:ee,levelClass:te,levelChipClass:X,toggleLevel:Y,copyLine:$,jumpToBottom:ue,onScroll:z,onUserScrollIntent:ie,onUserScrollKey:me,onAutoScrollToggle:E,onPointerDown:Le,applyLogPreset:be,applyCustomLogPreset:le,saveLogCustomPreset:ge,removeLogCustomPreset:xe,segmentHeight:C,jumpToTimelineBucket:j,searchLevel:ke,searchTool:Ee,searchKeyword:H,searchStart:re,searchEnd:ye,searchTimePreset:Me,searchLimit:Ue,searchLimits:Je,searching:dt,searchRan:Ye,searchError:Ke,searchResults:Wt,searchStats:bs,expandedSearch:Ts,switchToSearch:Cn,runSearch:Se,clearSearchFilters:Cs,toggleSearchExpand:En,formatSearchTs:mr,searchEntryText:$a,searchLogLineClass:Ua,formatJson:vr,applySearchTimePreset:cn}}},Ow=new Set(["token","api_token","secret","ssh_key_path","credentials_path","api_key","password"]),Dw={"logging.level":["DEBUG","INFO","WARNING","ERROR","CRITICAL"]},Mw={"discord.allowed_users":{type:"array",itemType:"string",message:"Must be a list of user IDs"},"discord.channels":{type:"array",itemType:"string",message:"Must be a list of channel IDs"},"openai_codex.max_tokens":{type:"number",min:1,max:128e3,message:"Must be 1–128000"},"sessions.max_history":{type:"number",min:1,max:1e4,message:"Must be 1–10000"},"sessions.max_age_hours":{type:"number",min:1,message:"Must be at least 1"},"learning.max_entries":{type:"number",min:1,message:"Must be at least 1"},"learning.consolidation_target":{type:"number",min:1,message:"Must be at least 1"},"browser.default_timeout_ms":{type:"number",min:100,message:"Must be at least 100ms"},"browser.viewport_width":{type:"number",min:100,max:7680,message:"Must be 100–7680"},"browser.viewport_height":{type:"number",min:100,max:4320,message:"Must be 100–4320"},"tools.command_timeout_seconds":{type:"number",min:10,max:3600,message:"Must be 10–3600 seconds"}},Kr=[{key:"core",label:"Core",icon:"sliders",sections:["timezone","discord","logging","permissions","personality","graceful_degradation"]},{key:"llm",label:"LLM & AI",icon:"brain",sections:["llm_provider","openai_codex","ollama","kimi","context","agents"]},{key:"data",label:"Data & Storage",icon:"database",sections:["sessions","learning","search","usage","audit","attachments"]},{key:"services",label:"Services",icon:"link",sections:["webhook","observability","email","browser","comfyui","mcp","slack"]},{key:"infra",label:"Infrastructure",icon:"server",sections:["tools"]},{key:"ui",label:"Web UI",icon:"globe",sections:["web"]},{key:"automation",label:"Automation",icon:"workflow",sections:["message_triggers","reaction_triggers","grafana_alerts","outbound_webhooks"]},{key:"integrations",label:"Integrations",icon:"puzzle",sections:["issue_tracker"]}],vg="••••••••",Pw=50;function Fw(e){return Ow.has(e)}function $w(e){return e===vg}function sl(e){return JSON.parse(JSON.stringify(e))}function Vn(e,t){return JSON.stringify(e)===JSON.stringify(t)}function Uw(e,t){const s={};for(const n of Object.keys(t)){if(!(n in e))continue;const a=e[n],i=t[n];if(!Vn(a,i))if(typeof a=="object"&&a!==null&&!Array.isArray(a)&&typeof i=="object"&&i!==null&&!Array.isArray(i)){const l={};for(const r of Object.keys(i))Vn(a[r],i[r])||(l[r]=i[r]);Object.keys(l).length>0&&(s[n]=l)}else s[n]=i}return s}function Bw(e,t,s){const n=Mw[e+"."+t];if(!n)return null;if(n.type==="number"){const a=Number(s);if(isNaN(a))return"Must be a number";if(n.min!==void 0&&a<n.min)return n.message||"Value too low";if(n.max!==void 0&&a>n.max)return n.message||"Value too high"}return n.type==="array"&&!Array.isArray(s)?n.message||"Must be an array":null}function Gd(e,t){const s=[];for(const n of Object.keys(t)){if(!(n in e))continue;const a=e[n],i=t[n];if(!Vn(a,i))if(typeof a=="object"&&a!==null&&!Array.isArray(a)&&typeof i=="object"&&i!==null&&!Array.isArray(i))for(const l of Object.keys(i))Vn(a[l],i[l])||s.push({section:n,key:l,oldVal:a[l],newVal:i[l]});else s.push({section:n,key:null,oldVal:a,newVal:i})}return s}const Hw={template:`
    <div class="p-6 page-fade-in">
      <!-- Header -->
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
    ...
    </div>`,setup(){const e=h(null),t=h(null),s=h(!0),n=h(null),a=h({}),i=h({}),l=h({}),r=h(!1),o=h(!1),c=h(null),u=h(!1),d=h([]),f=h([]),p=J(()=>d.value.length>0),b=J(()=>f.value.length>0),g=J(()=>r.value&&t.value?t.value:e.value),T=J(()=>!e.value||!t.value?!1:!Vn(e.value,t.value)),O=J(()=>!e.value||!t.value?0:Gd(e.value,t.value).length),y=J(()=>{if(!r.value||!t.value)return{};const R={};for(const $ of Object.keys(t.value)){const Y=t.value[$];if(typeof Y=="object"&&Y!==null&&!Array.isArray(Y))for(const ee of Object.keys(Y)){const te=Bw($,ee,Y[ee]);te&&(R[$+"."+ee]=te)}}return R}),v=J(()=>Object.keys(y.value).length>0),x=J(()=>e.value?Object.keys(e.value).length:0),w=J(()=>L.value.length),I=J(()=>!e.value||!t.value?[]:Gd(e.value,t.value)),L=J(()=>e.value?Kr.map(R=>({...R,sections:R.sections.filter($=>$ in e.value)})).filter(R=>R.sections.length>0):[]),k=J(()=>{if(!e.value)return[];const R=new Set(Kr.flatMap($=>$.sections));return Object.keys(e.value).filter($=>!R.has($))});function A(R){return g.value?g.value[R]:null}function D(R){return!e.value||!t.value?!1:!Vn(e.value[R],t.value[R])}function B(R){return R.sections.some($=>D($))}function M(R,$){if(!e.value||!t.value)return!1;const Y=e.value[R],ee=t.value[R];return!Y||!ee?!1:!Vn(Y[$],ee[$])}function S(R){return t.value?t.value[R]:e.value[R]}function U(R,$){const Y=t.value||e.value;return Y[R]?Y[R][$]:void 0}function W(R,$){const Y=r.value&&t.value?t.value:e.value;return Y[R]?Y[R][$]:!1}function F(R,$){return y.value[R+"."+$]||null}function N(R,$){return Dw[R+"."+$]||null}function C(R,$,Y){t.value&&($===null?t.value[R]=Y:(t.value[R]||(t.value[R]={}),t.value[R][$]=Y),t.value={...t.value})}function j(R,$,Y){if(!t.value)return;const ee=sl(t.value);C(R,$,Y),d.value.push(ee),d.value.length>Pw&&d.value.shift(),f.value=[]}function ce(R,$,Y){try{const ee=JSON.parse(Y);j(R,$,ee)}catch{}}function de(){d.value.length!==0&&(f.value.push(sl(t.value)),t.value=d.value.pop())}function se(){f.value.length!==0&&(d.value.push(sl(t.value)),t.value=f.value.pop())}function fe(R,$,Y){if(!t.value||!t.value[R])return;const ee=[...t.value[R][$]];ee.splice(Y,1),j(R,$,ee)}function Q(R,$){if(!t.value||!t.value[R])return;const Y=[...t.value[R][$]||[]],ee=prompt("Enter new value:");ee!==null&&(Y.push(ee),j(R,$,Y))}function ue(R){a.value={...a.value,[R]:!a.value[R]}}function Ie(R){l.value={...l.value,[R]:!l.value[R]}}function z(R){i.value={...i.value,[R]:!i.value[R]}}function oe(R){try{return JSON.stringify(R,null,2)}catch{return String(R)}}function ie(R){return R==null?"null":typeof R=="object"?JSON.stringify(R,null,2):String(R)}function me(R,$){c.value={type:R,message:$},setTimeout(()=>{c.value=null},3e3)}function pe(){t.value=sl(e.value),r.value=!0,d.value=[],f.value=[]}function Le(){r.value=!1,t.value=null,d.value=[],f.value=[]}function m(){u.value=!0}async function E(){if(!(!T.value||v.value)){o.value=!0;try{const R=Uw(e.value,t.value);if(Object.keys(R).length===0){me("success","No changes to save."),o.value=!1;return}const $=await G.put("/api/config",R);e.value=$,r.value=!1,t.value=null,d.value=[],f.value=[],me("success","Config saved successfully.")}catch(R){me("error",R.message||"Failed to save config")}o.value=!1}}async function P(){s.value=!0,n.value=null;try{e.value=await G.get("/api/config");for(const R of Object.keys(e.value))a.value[R]===void 0&&(a.value[R]=!0);for(const R of Kr)l.value[R.key]===void 0&&(l.value[R.key]=!0)}catch(R){n.value=R.message}s.value=!1}function Z(R){if(!r.value)return;const $=R.target;$ instanceof HTMLElement&&($.matches("input, textarea, select")||$.isContentEditable)||((R.ctrlKey||R.metaKey)&&!R.shiftKey&&R.key.toLowerCase()==="z"?(R.preventDefault(),de()):(R.ctrlKey||R.metaKey)&&(R.key==="y"||R.shiftKey&&R.key==="z"||R.shiftKey&&R.key==="Z")&&(R.preventDefault(),se()))}return He(()=>{P(),document.addEventListener("keydown",Z)}),gt(()=>{document.removeEventListener("keydown",Z)}),{config:e,displayConfig:g,editValues:t,loading:s,error:n,expanded:a,expandedNested:i,expandedGroups:l,editing:r,saving:o,toast:c,hasChanges:T,hasErrors:v,changeCount:O,REDACTED:vg,showDiffModal:u,diffEntries:I,canUndo:p,canRedo:b,sectionCount:x,groupCount:w,visibleGroups:L,ungroupedSections:k,validationErrors:y,isSensitiveKey:Fw,isRedacted:$w,sectionChanged:D,groupChanged:B,fieldChanged:M,getDisplay:A,getEdited:S,getEditedField:U,getDisplayBool:W,pushEdit:j,pushEditJson:ce,getValidationError:F,getEnumOptions:N,removeArrayItem:fe,addArrayItem:Q,toggleSection:ue,toggleGroup:Ie,toggleNested:z,formatJson:oe,formatDiffVal:ie,showToast:me,showDiff:m,fetchConfig:P,startEdit:pe,cancelEdit:Le,saveConfig:E,undo:de,redo:se}}},Vw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=h(!0),t=h(""),s=h(null),n=h([]),a=h({allowed_hosts:[],default_host:""}),i=h({}),l=h(!1),r=h(""),o=h(!1),c=h(0),u=h([]),d=h(null),f=J(()=>{const C={};for(const j of u.value)C[j.id]=j;return C});function p(C){return f.value[C]||null}const b=J(()=>/^\d{15,25}$/.test(r.value.trim())),g=J(()=>{if(o.value){if(T.value[c.value])return"host-user-option-"+c.value;if(b.value)return"host-user-option-raw"}}),T=J(()=>{const C=r.value.toLowerCase().trim();return C?u.value.filter(j=>!i.value[j.id]&&(j.display_name.toLowerCase().includes(C)||j.username.toLowerCase().includes(C)||j.id.includes(C))):u.value.filter(j=>!i.value[j.id])});function O(C,j){return C?C.allowed_hosts===null||C.allowed_hosts===void 0?{allowed_hosts:[...j],default_host:C.default_host||"",allow_all:!0}:{allowed_hosts:C.allowed_hosts,default_host:C.default_host||"",allow_all:!1}:{allowed_hosts:[...j],default_host:j[0]||"",allow_all:!0}}async function y(){e.value=!0,t.value="";try{const C=await G.get("/api/host-access");s.value=C,n.value=C.available_hosts||[],a.value=O(C.default_policy,n.value);const j=C.users||{},ce={};for(const[de,se]of Object.entries(j))ce[de]=O(se,n.value);i.value=ce}catch(C){t.value=C.message||"Failed to fetch host access data"}finally{e.value=!1}try{u.value=await G.get("/api/discord/members")||[]}catch{u.value=[]}}async function v(){try{const C=a.value.allow_all?null:a.value.allowed_hosts;await G.put("/api/host-access/default-policy",{allowed_hosts:C,default_host:a.value.default_host}),_e.success("Default policy updated")}catch(C){_e.error(C.message||"Failed to save")}}function x(C,j){a.value.allow_all=!1,j?a.value.allowed_hosts.includes(C)||a.value.allowed_hosts.push(C):(a.value.allowed_hosts=a.value.allowed_hosts.filter(ce=>ce!==C),a.value.default_host===C&&(a.value.default_host=a.value.allowed_hosts[0]||"")),v()}async function w(C){const j=i.value[C];if(j)try{const ce=j.allow_all?null:j.allowed_hosts;await G.put(`/api/host-access/user/${C}`,{allowed_hosts:ce,default_host:j.default_host});const de=p(C);_e.success(`Updated access for ${de?de.display_name:C}`)}catch(ce){_e.error(ce.message||"Failed to save")}}function I(C,j,ce){const de=i.value[C];de&&(de.allow_all=!1,ce?de.allowed_hosts.includes(j)||de.allowed_hosts.push(j):(de.allowed_hosts=de.allowed_hosts.filter(se=>se!==j),de.default_host===j&&(de.default_host=de.allowed_hosts[0]||"")),w(C))}function L(C,j){const ce=i.value[C];ce&&(ce.default_host=j,w(C))}function k(){l.value=!0,r.value="",c.value=0,St(()=>{d.value&&d.value.focus()})}function A(){o.value=!0,c.value=0}function D(){c.value<T.value.length-1&&c.value++}function B(){c.value>0&&c.value--}function M(){const C=T.value[c.value];if(C){U(C);return}b.value&&S()}function S(){const C=r.value.trim();/^\d{15,25}$/.test(C)&&(i.value[C]={allowed_hosts:[...n.value],default_host:n.value[0]||"",allow_all:!1},w(C),r.value="",o.value=!1,l.value=!1)}function U(C){i.value[C.id]={allowed_hosts:[...n.value],default_host:n.value[0]||"",allow_all:!1},w(C.id),r.value="",o.value=!1,l.value=!1}function W(){o.value=!1}function F(){setTimeout(()=>{o.value=!1},150)}async function N(C){const j=p(C);if(await rs({title:"Remove user override",message:`Remove the host access override for ${j?j.display_name:C}? They will fall back to the default policy.`,confirmLabel:"Remove",danger:!0}))try{await G.del(`/api/host-access/user/${C}`),delete i.value[C],_e.success(`Removed override for ${j?j.display_name:C}`)}catch(de){_e.error(de.message||"Failed to delete")}}return He(y),{loading:e,error:t,data:s,availableHosts:n,defaultPolicy:a,users:i,showAddUser:l,searchQuery:r,showDropdown:o,highlightIdx:c,members:u,filteredMembers:T,isRawId:b,activeOptionId:g,searchInput:d,fetchData:y,saveDefaultPolicy:v,toggleDefaultHost:x,getMember:p,toggleUserHost:I,setUserDefault:L,openAddUser:k,deleteUser:N,onSearchInput:A,highlightNext:D,highlightPrev:B,selectHighlighted:M,selectMember:U,closeDropdown:W,onBlur:F,addRawId:S}}},zw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=h(!0),t=h(""),s=h(null),n=h([]),a=h(!1),i=h(!1),l=h(null),r=h(null),o=h(!1),c=h({user_id:"",username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""}),u=h({username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""}),d=J(()=>c.value.host_mode==="select"?c.value.allowed_hosts:c.value.host_mode==="none"?[]:n.value),f=J(()=>u.value.host_mode==="select"?u.value.allowed_hosts:u.value.host_mode==="none"?[]:n.value);function p(k){return k==="admin"?"text-xs px-1.5 py-0.5 rounded bg-red-900/50 text-red-400":k==="user"?"text-xs px-1.5 py-0.5 rounded bg-blue-900/50 text-blue-400":"text-xs px-1.5 py-0.5 rounded bg-gray-700 text-gray-400"}async function b(){e.value=!0,t.value="";try{const k=await G.get("/api/tokens");s.value=k.tokens||[],n.value=k.available_hosts||[]}catch(k){t.value=k.message||"Failed to load tokens"}finally{e.value=!1}}function g(k){return!k||!k.trim()?[]:k.split(",").map(A=>A.trim()).filter(Boolean)}function T(k,A){const D=c.value.allowed_hosts;if(A&&!D.includes(k)&&D.push(k),!A){const B=D.indexOf(k);B>=0&&D.splice(B,1)}}function O(k,A){const D=u.value.allowed_hosts;if(A&&!D.includes(k)&&D.push(k),!A){const B=D.indexOf(k);B>=0&&D.splice(B,1)}}async function y(){var k;i.value=!0;try{const A=g(c.value.allowed_tools_str),D=c.value.host_mode,B=D==="none"?[]:D==="select"?c.value.allowed_hosts:null,M={user_id:c.value.user_id.trim(),username:c.value.username.trim()||"API",tier:c.value.tier,label:c.value.label.trim(),allowed_tools:A.length?A:[]};B!==null&&(M.allowed_hosts=B),M.default_host=c.value.default_host||"";const S=await G.post("/api/tokens",M);l.value=S.token,c.value={user_id:"",username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""},a.value=!1,_e.success("Token created"),await b()}catch(A){_e.error(((k=A.data)==null?void 0:k.error)||A.message||"Failed to create token")}finally{i.value=!1}}function v(k){r.value=k;const A=k.allowed_hosts;let D="default";A==null?D="default":Array.isArray(A)&&A.length===0?D="none":Array.isArray(A)&&(D="select"),u.value={username:k.username||"",tier:k.tier||"admin",label:k.label||"",host_mode:D,allowed_hosts:Array.isArray(A)?[...A]:[],default_host:k.default_host||"",allowed_tools_str:(k.allowed_tools||[]).join(", ")}}async function x(){var k;if(r.value){o.value=!0;try{const A=g(u.value.allowed_tools_str),D=u.value.host_mode,B={username:u.value.username,tier:u.value.tier,label:u.value.label,allowed_tools:A};D==="none"?B.allowed_hosts=[]:D==="select"?B.allowed_hosts=u.value.allowed_hosts:B.allowed_hosts=null,B.default_host=u.value.default_host||"",await G.put("/api/tokens/"+encodeURIComponent(r.value.user_id),B),r.value=null,_e.success("Token updated"),await b()}catch(A){_e.error(((k=A.data)==null?void 0:k.error)||A.message||"Failed to update")}finally{o.value=!1}}}async function w(k){var D;if(await rs({title:"Regenerate token",message:`Regenerate token for ${k.username||k.user_id}? The old token will stop working immediately.`,confirmLabel:"Regenerate",danger:!0}))try{const B=await G.post("/api/tokens/"+encodeURIComponent(k.user_id)+"/regenerate");l.value=B.token,_e.success("Token regenerated")}catch(B){_e.error(((D=B.data)==null?void 0:D.error)||B.message||"Failed to regenerate")}}async function I(k){var D;if(await rs({title:"Delete token",message:`Delete token for ${k.username||k.user_id}? This cannot be undone.`,confirmLabel:"Delete",danger:!0}))try{await G.del("/api/tokens/"+encodeURIComponent(k.user_id)),_e.success("Token deleted"),await b()}catch(B){_e.error(((D=B.data)==null?void 0:D.error)||B.message||"Failed to delete")}}async function L(){if(l.value)try{await navigator.clipboard.writeText(l.value),_e.success("Copied to clipboard")}catch{_e.error("Copy failed — select and copy manually")}}return He(b),{loading:e,error:t,tokens:s,availableHosts:n,showCreate:a,creating:i,newToken:l,editing:r,saving:o,createForm:c,editForm:u,createDefaultHostOptions:d,editDefaultHostOptions:f,fetchData:b,tierBadge:p,toggleCreateHost:T,toggleEditHost:O,createToken:y,startEdit:v,saveEdit:x,confirmRegenerate:w,confirmDelete:I,copyToken:L}}};function nl(e,t=500){let s=null;const n=(...a)=>{s&&clearTimeout(s),s=setTimeout(()=>{s=null,e(...a)},t)};return n.pending=()=>s!==null,n.cancel=()=>{s&&(clearTimeout(s),s=null)},n}const qw={template:`
    <div class="p-6 page-fade-in">
  ...
  `,setup(){const e=h(!0),t=h(null),s=h("codex"),n=h({enabled:!1,model:"gpt-5.5",max_tokens:4096,reasoning_effort:"medium",agent_reasoning_effort:"",agent_model:""}),a=["gpt-5.6-sol","gpt-5.6-terra","gpt-5.6-luna","gpt-5.5"],i=J(()=>{const q=n.value.model;return q&&!a.includes(q)?[q,...a]:a}),l=J(()=>{const q=n.value.agent_model;return q&&q!=="auto"&&!a.includes(q)?[q,...a]:a}),r=["gpt-5.5","gpt-5.4","gpt-5.4-mini"],o=J(()=>!r.includes(n.value.model)&&!(r.includes(n.value.agent_model)&&n.value.agent_reasoning_effort==="")),c=J(()=>{const q=n.value.agent_model;return q==="auto"?!0:!r.includes(q||n.value.model)}),u=J(()=>{const q=n.value.agent_reasoning_effort;return q==="auto"?!1:(q||n.value.reasoning_effort)==="max"}),d=q=>r.includes(q)&&(n.value.reasoning_effort==="max"||n.value.agent_model===""&&u.value),f=q=>r.includes(q)&&u.value,p=h({enabled:!1,model:"gpt-5.6-luna"}),b=h({unavailable_reason:null}),g=J(()=>{const q=p.value.model;return q&&!a.includes(q)?[q,...a]:a});function T(q){const Se=q.target.value;p.value.enabled=Se!=="",Se!==""&&(p.value.model=Se),ye()}const O=h(!1),y=h({enabled:!1,base_url:"",model:"",api_key:"",max_tokens:4096}),v=h({enabled:!1,api_key:"",model:"",max_tokens:4096}),x=h(!1),w=h(!1),I=h(!1),L=h(!1),k=h(!1),A=h(!1),D=h(!1),B=h({configured:!1}),M=h([]),S=h(""),U=h(!1),W=h(!1),F=h({configured:!1}),N=h([]),C=h(""),j=h(!1),ce=h(!1),de=h(!0),se=h(""),fe=h({configured:!1,accounts:[]}),Q=h(null),ue=h(null),Ie=h(""),z=h(null),oe=h(!1),ie=h(null),me=h(null),pe=h("");let Le=null;function m(q,Se="success"){_e(q,Se==="error"?"error":"success")}function E(q){if(!q)return"?";const Se=q/(1024*1024*1024);return Se>=1?Se.toFixed(1)+" GB":(q/(1024*1024)).toFixed(0)+" MB"}async function P(){e.value=!0,await Promise.all([Z(),R(),be(),$()]),e.value=!1}async function Z(){try{const q=await G.get("/api/llm/status");t.value=q,s.value=q.active_provider||"codex",q.codex&&!re.pending()&&(n.value.enabled=q.codex.enabled,n.value.model=q.codex.model||"gpt-5.5",n.value.reasoning_effort=q.codex.reasoning_effort||"medium",n.value.agent_reasoning_effort=q.codex.agent_reasoning_effort||"",n.value.agent_model=q.codex.agent_model||"",n.value.max_tokens=q.codex.max_tokens||4096),q.ollama&&!Me.pending()&&(y.value.enabled=q.ollama.enabled,y.value.base_url=q.ollama.base_url||"",y.value.model=q.ollama.model||"",y.value.max_tokens=q.ollama.max_tokens||4096),q.kimi&&!Ue.pending()&&(v.value.enabled=q.kimi.enabled,v.value.model=q.kimi.model||"",v.value.max_tokens=q.kimi.max_tokens||4096),q.auxiliary&&(b.value=q.auxiliary,ye.pending()||(p.value.enabled=q.auxiliary.enabled,p.value.model=q.auxiliary.model||"gpt-5.6-luna"))}catch{t.value={active_provider:"codex",codex:{configured:!1},ollama:{configured:!1},kimi:{configured:!1}}}}async function R(){try{if(B.value=await G.get("/api/ollama/status"),B.value.model&&(S.value=B.value.model),B.value.configured)try{const q=await G.get("/api/ollama/models");M.value=q.models||[]}catch{M.value=[]}else if(y.value.base_url)try{const q=await G.post("/api/ollama/probe-models",{base_url:y.value.base_url});M.value=q.models||[]}catch{M.value=[]}}catch{B.value={configured:!1}}}async function $(){de.value=!0,se.value="";try{fe.value=await G.get("/api/codex/status")}catch(q){se.value=q.message||"Failed to fetch Codex status"}finally{de.value=!1}}async function Y(){const q=t.value?t.value.active_provider:"codex";D.value=!0;try{const Se=await G.post("/api/llm/switch",{provider:s.value});Se.error?(s.value=q,m(Se.error,"error")):(m("Switched to "+s.value+" ("+Se.model+")"),await P())}catch(Se){s.value=q,m(Se.message||"Switch failed","error")}finally{D.value=!1}}async function ee(){U.value=!0;try{const q=await G.post("/api/ollama/reload");m(q.configured?"Ollama reloaded":q.reason||"Ollama not configured",q.configured?"success":"error"),await P()}catch(q){m(q.message||"Reload failed","error")}finally{U.value=!1}}async function te(){W.value=!0;try{await G.post("/api/ollama/model",{model:S.value}),m("Model set to "+S.value),await P()}catch(q){m(q.message||"Failed","error")}finally{W.value=!1}}async function X(){const q=y.value.base_url;if(!q){m("Enter a base URL first","error");return}A.value=!0;try{const Se=await G.post("/api/ollama/probe-models",{base_url:q});M.value=Se.models||[],M.value.length?(m(M.value.length+" model(s) found"),!y.value.model&&M.value.length&&(y.value.model=M.value[0].name)):m("No models found at "+q,"error")}catch(Se){m(Se.message||"Could not reach Ollama","error")}finally{A.value=!1}}async function be(){try{if(F.value=await G.get("/api/kimi/status"),F.value.model&&(C.value=F.value.model),F.value.configured)try{const q=await G.get("/api/kimi/models");N.value=q.models||[]}catch{N.value=[]}}catch{F.value={configured:!1}}}async function le(){j.value=!0;try{const q=await G.post("/api/kimi/reload");m(q.configured?"Kimi reloaded":q.reason||"Kimi not configured",q.configured?"success":"error"),await P()}catch(q){m(q.message||"Reload failed","error")}finally{j.value=!1}}async function ge(){ce.value=!0;try{await G.post("/api/kimi/model",{model:C.value}),m("Model set to "+C.value),await P()}catch(q){m(q.message||"Failed","error")}finally{ce.value=!1}}async function xe(){if(I.value){re();return}I.value=!0;try{await G.put("/api/llm/codex/config",n.value),m("Codex config saved"),await Promise.all([Z(),$()])}catch(q){m(q.message||"Failed","error"),await Promise.all([Z(),$()])}finally{I.value=!1}}async function ke(){if(L.value){Me();return}L.value=!0;try{const q={...y.value},Se=x.value?y.value.api_key:null;Se===null&&delete q.api_key,await G.put("/api/llm/ollama/config",q),m("Ollama config saved"),Se!==null&&y.value.api_key===Se&&(y.value.api_key="",x.value=!1),await Promise.all([Z(),R()])}catch(q){m(q.message||"Failed","error")}finally{L.value=!1}}async function Ee(){if(k.value){Ue();return}k.value=!0;try{const q={...v.value},Se=w.value?v.value.api_key:null;Se===null&&delete q.api_key,await G.put("/api/llm/kimi/config",q),m("Kimi config saved"),Se!==null&&v.value.api_key===Se&&(v.value.api_key="",w.value=!1),await Promise.all([Z(),be()])}catch(q){m(q.message||"Failed","error")}finally{k.value=!1}}async function H(){if(O.value){ye();return}O.value=!0;try{await G.put("/api/llm/auxiliary/config",p.value),m("Auxiliary config saved"),await Z()}catch(q){m(q.message||"Failed","error"),await Z()}finally{O.value=!1}}const re=nl(xe),ye=nl(H),Me=nl(ke),Ue=nl(Ee),Je=()=>(re.cancel(),xe()),dt=()=>(Me.cancel(),ke()),Ye=()=>(Ue.cancel(),Ee());async function Ke(q){try{await G.post("/api/codex/account/"+q+"/activate"),m("Active account switched"),await $()}catch(Se){m(Se.message||"Failed","error")}}async function Wt(q){Q.value=q;try{await G.post("/api/codex/account/"+q+"/refresh"),m("Token refreshed"),await $()}catch(Se){m(Se.message||"Refresh failed","error")}finally{Q.value=null}}function bs(q,Se){ue.value=q,Ie.value=Se||""}async function Ts(q){try{await G.put("/api/codex/account/"+q+"/label",{label:Ie.value}),m("Label updated"),ue.value=null,await $()}catch(Se){m(Se.message||"Failed","error")}}async function Cn(q,Se){if(await rs({title:"Delete Codex account",message:`Delete ${Se||"account #"+(q+1)}? The pool will reload without it.`,confirmLabel:"Delete",danger:!0}))try{await G.del("/api/codex/account/"+q),m("Deleted. Pool reloaded."),await $()}catch(En){m(En.message||"Failed","error")}}async function Qn(){oe.value=!0;try{const q=await G.post("/api/codex/device-code");ie.value=q,z.value="pending",cn(q)}catch(q){m(q.message||"Failed","error")}finally{oe.value=!1}}async function cn(q){Le={cancelled:!1};const Se=Le;try{const Cs=await G.post("/api/codex/device-poll",{device_auth_id:q.device_auth_id,user_code:q.user_code,interval:q.interval});if(Se.cancelled)return;me.value=Cs,z.value="success",await P()}catch(Cs){if(Se.cancelled)return;pe.value=Cs.message||"Device login failed",z.value="error"}}function un(){Le&&(Le.cancelled=!0),z.value=null,ie.value=null}return He(P),gt(()=>{Le&&(Le.cancelled=!0),re.cancel(),ye.cancel(),Me.cancel(),Ue.cancel()}),{loading:e,llmStatus:t,selectedProvider:s,switching:D,codexForm:n,codexModelOptions:i,codexAgentModelOptions:l,mainMaxAllowed:o,agentMaxAllowed:c,mainModelOptionDisabled:d,agentModelOptionDisabled:f,auxForm:p,auxData:b,auxModelOptions:g,onAuxModelChange:T,savingAux:O,saveAuxConfigDebounced:ye,ollamaForm:y,kimiForm:v,savingCodex:I,savingOllama:L,savingKimi:k,probingOllama:A,ollamaKeyDirty:x,kimiKeyDirty:w,ollamaStatus:B,ollamaModels:M,ollamaSelectedModel:S,reloading:U,settingModel:W,kimiStatus:F,kimiModels:N,kimiSelectedModel:C,reloadingKimi:j,settingKimiModel:ce,codexLoading:de,codexError:se,codexData:fe,refreshing:Q,editingLabel:ue,labelValue:Ie,deviceState:z,deviceLoading:oe,deviceInfo:ie,deviceResult:me,deviceError:pe,fetchAll:P,switchProvider:Y,reloadOllama:ee,setOllamaModel:te,reloadKimi:le,setKimiModel:ge,probeOllamaModels:X,saveCodexConfig:xe,saveOllamaConfig:ke,saveKimiConfig:Ee,saveCodexConfigDebounced:re,saveOllamaConfigDebounced:Me,saveKimiConfigDebounced:Ue,saveCodexConfigNow:Je,saveOllamaConfigNow:dt,saveKimiConfigNow:Ye,activateAccount:Ke,refreshAccount:Wt,startEditLabel:bs,saveLabel:Ts,deleteAccount:Cn,startDeviceLogin:Qn,cancelDeviceLogin:un,formatSize:E}}},Wd={ok:"text-green-400",pass:"text-green-400",degraded:"text-yellow-400",warn:"text-yellow-400",down:"text-red-400",fail:"text-red-400",unconfigured:"text-gray-500",skipped:"text-gray-500"};function Kw(e){return Wd[e]||Wd[(e||"").toLowerCase()]||"text-gray-400"}const Gw={template:`
  ...
  `},Zw={components:{TabbedPage:gr},setup(){return{tabs:[{id:"health",label:"Health",component:Ew},{id:"resources",label:"Resources",component:Aw},{id:"logs",label:"Logs",component:Lw},{id:"config",label:"Config",component:Hw},{id:"discord",label:"Discord",component:Vw},{id:"host-access",label:"Host Access",component:jw},{id:"api-tokens",label:"API Tokens",component:zw},{id:"llm",label:"LLM Config",component:qw},{id:"internals",label:"Internals",component:Gw},{id:"update",label:"Update",component:Ww}]}},template:'<tabbed-page :tabs="tabs" default-tab="health" group-label="System" />'},mt=(e,t)=>s=>({path:e,query:{...s.query,tab:t}}),bg=[{path:"/",redirect:"/dashboard"},{path:"/dashboard",component:P_,meta:{label:"Dashboard",icon:"dashboard",section:"Workspace",description:"System posture and recent activity"}},{path:"/chat",component:ew,meta:{label:"Chat",icon:"chat",section:"Workspace",description:"Direct operator conversation"}},{path:"/operations",component:lw,meta:{label:"Operations",icon:"operations",section:"Operate",description:"Execution, agents, loops, processes, and schedules"}},{path:"/history",component:pw,meta:{label:"History",icon:"history",section:"Observe",description:"Audit trail, sessions, traces, and usage"}},{path:"/capabilities",component:kw,meta:{label:"Capabilities",icon:"capabilities",section:"Manage",description:"Tools, skills, knowledge, and memory"}},{path:"/personality",component:ww,meta:{label:"Personality",icon:"personality",section:"Manage",description:"Behavior and response profile"}},{path:"/system",component:Zw,meta:{label:"System",icon:"system",section:"Manage",description:"Health, configuration, access, and updates"}},{path:"/execution",redirect:mt("/operations","live")},{path:"/agents",redirect:mt("/operations","agents")},{path:"/loops",redirect:mt("/operations","loops")},{path:"/processes",redirect:mt("/operations","processes")},{path:"/schedules",redirect:mt("/operations","schedules")},{path:"/audit",redirect:mt("/history","audit")},{path:"/sessions",redirect:mt("/history","sessions")},{path:"/traces",redirect:mt("/history","traces")},{path:"/usage",redirect:mt("/history","usage")},{path:"/tools",redirect:mt("/capabilities","tools")},{path:"/skills",redirect:mt("/capabilities","skills")},{path:"/knowledge",redirect:mt("/capabilities","knowledge")},{path:"/memory",redirect:mt("/capabilities","memory")},{path:"/learned",redirect:mt("/capabilities","learned")},{path:"/health",redirect:mt("/system","health")},{path:"/resources",redirect:mt("/system","resources")},{path:"/logs",redirect:mt("/system","logs")},{path:"/config",redirect:mt("/system","config")},{path:"/host-access",redirect:mt("/system","host-access")},{path:"/internals",redirect:mt("/system","internals")}],ui=E_({history:i_(),routes:bg});ui.afterEach(e=>{var s;const t=(s=e.meta)==null?void 0:s.label;document.title=t?`Odin — ${t}`:"Odin — Management"});const Jw={template:`
    ...
    <toast-container />
    <confirm-host />
    <command-palette />`,setup(){const e=h("checking"),t=h(!1),s=h(!1),n=h(!1),a=h(null),i=h(null),l=h(!1);let r=null,o=null;const c=h(!1),u=h("disconnected"),d=h(-1),f=h(null);let p=null;const b=h("starting"),g=h(""),T=bg.filter(N=>N.meta),O=J(()=>["Workspace","Operate","Observe","Manage"].map(N=>({name:N,routes:T.filter(C=>C.meta.section===N)})).filter(N=>N.routes.length)),y=J(()=>{var N;return((N=ui.currentRoute.value.meta)==null?void 0:N.label)||"Odin"}),v=J(()=>{var N;return((N=ui.currentRoute.value.meta)==null?void 0:N.section)||"Management"}),x=J(()=>{var N;return((N=ui.currentRoute.value.meta)==null?void 0:N.description)||"Management console"});G.onSessionExpired=()=>{t.value=!0,We.disconnect(),G.setToken(""),e.value="login"};function w(N){var C;if((N.ctrlKey||N.metaKey)&&N.key.toLowerCase()==="k"){e.value==="ready"&&(N.preventDefault(),kd());return}if(n.value&&N.key==="Tab"){const j=[...((C=a.value)==null?void 0:C.querySelectorAll('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])'))||[]];if(j.length){const ce=j[0],de=j[j.length-1];if(N.shiftKey&&(document.activeElement===ce||!a.value.contains(document.activeElement))){N.preventDefault(),de.focus();return}if(!N.shiftKey&&(document.activeElement===de||!a.value.contains(document.activeElement))){N.preventDefault(),ce.focus();return}}}if(N.key==="Escape"&&n.value){n.value=!1,N.preventDefault();return}if(N.key==="/"&&!["INPUT","TEXTAREA","SELECT"].includes(N.target.tagName)){N.preventDefault();const j=document.querySelector('.hm-main input[type="text"], .hm-main .hm-input:not(textarea):not(select)');j&&j.focus()}}function I(){l.value=!!(r!=null&&r.matches),l.value||(n.value=!1)}He(async()=>{document.addEventListener("keydown",w),r=window.matchMedia("(max-width: 900px)"),I(),r.addEventListener("change",I);const N=await G.check();N.ok?(e.value="ready",W()):N.needsAuth?e.value="login":(e.value="ready",W())});function L(){t.value=!1,e.value="ready",W()}async function k(){await G.logout(),We.disconnect(),e.value="login"}function A(){s.value=!s.value}function D(){n.value=!n.value}gs(n,async N=>{var C,j;if(N)o=document.activeElement,await St(),(j=(C=a.value)==null?void 0:C.querySelector(".nav-item"))==null||j.focus();else if(o!=null&&o.isConnected){const ce=o;o=null,requestAnimationFrame(()=>ce.focus())}});const B=J(()=>{switch(u.value){case"connected":return"Live";case"connecting":return"Connecting…";case"reconnecting":return"Reconnecting…";default:return"Disconnected"}});function M(N,C="info",j=3e3){f.value={text:N,level:C},clearTimeout(p),p=setTimeout(()=>{f.value=null},j)}let S=null,U=!1;function W(){We.onStatusChange=N=>{c.value=N},We.onStateChange=(N,C)=>{u.value=N,d.value=C.latency??-1,N==="connected"?(U&&M("Connection restored","success"),U=!0):N==="reconnecting"&&C.attempt===1&&M("Connection lost — reconnecting…","warn")},We.connect(),F(),S&&clearInterval(S),S=setInterval(F,15e3)}async function F(){try{const N=await G.get("/api/status");b.value=N.status==="online"?"online":"starting";const C=N.uptime_seconds||0,j=Math.floor(C/3600),ce=Math.floor(C%3600/60);g.value=`${j}h ${ce}m uptime`}catch{b.value="offline",g.value=""}}return gt(()=>{S&&clearInterval(S),We.disconnect(),document.removeEventListener("keydown",w),r==null||r.removeEventListener("change",I)}),{authState:e,sessionExpired:t,sidebarCollapsed:s,mobileOpen:n,wsConnected:c,wsState:u,wsLatency:d,wsLabel:B,wsToast:f,botStatus:b,botUptime:g,navRoutes:T,navGroups:O,currentPage:y,currentSection:v,currentDescription:x,sidebarEl:a,mobileMenuButton:i,isMobileViewport:l,onLogin:L,logout:k,toggleSidebar:A,toggleMobileNavigation:D,openPalette:kd}}},Tn=Al(Yw);Tn.component("odin-icon",L_);Tn.component("login-screen",Jw);Tn.component("toast-container",yx);Tn.component("confirm-host",xx);Tn.component("command-palette",N_);Tn.directive("modal-focus",D_);Tn.use(ui);Tn.mount("#app");
