var av=Object.defineProperty;var iv=(e,t,s)=>t in e?av(e,t,{enumerable:!0,configurable:!0,writable:!0,value:s}):e[t]=s;var pt=(e,t,s)=>iv(e,typeof t!="symbol"?t+"":t,s);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function s(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(a){if(a.ep)return;a.ep=!0;const i=s(a);fetch(a.href,i)}})();class lv{constructor(){this._persist=localStorage.getItem("odin_persist")==="1",this._token=this._persist?localStorage.getItem("odin_token")||"":sessionStorage.getItem("odin_token")||"";const t=this._persist?localStorage:sessionStorage;this._sessionTimeout=parseInt(t.getItem("odin_session_timeout")||"0",10),this._lastActivity=Date.now(),this._activityTimer=null,this.onSessionExpired=null,this._token&&this._sessionTimeout>0&&this._startActivityMonitor()}get token(){return this._token}get sessionTimeout(){return this._sessionTimeout}setToken(t,s=0){if(this._token=t,this._sessionTimeout=s,this._lastActivity=Date.now(),t){const n=this._persist?localStorage:sessionStorage;n.setItem("odin_token",t),this._persist&&localStorage.setItem("odin_persist","1"),s>0?n.setItem("odin_session_timeout",String(s)):n.removeItem("odin_session_timeout"),this._startActivityMonitor()}else sessionStorage.removeItem("odin_token"),sessionStorage.removeItem("odin_session_timeout"),localStorage.removeItem("odin_token"),localStorage.removeItem("odin_persist"),localStorage.removeItem("odin_session_timeout"),this._stopActivityMonitor()}setPersist(t){this._persist=t}_startActivityMonitor(){this._stopActivityMonitor(),!(this._sessionTimeout<=0)&&(this._activityTimer=setInterval(()=>{(Date.now()-this._lastActivity)/1e3>=this._sessionTimeout&&(this._stopActivityMonitor(),this.onSessionExpired&&this.onSessionExpired())},1e4))}_stopActivityMonitor(){this._activityTimer&&(clearInterval(this._activityTimer),this._activityTimer=null)}_headers(t={}){const s={"Content-Type":"application/json",...t};return this._token&&(s.Authorization=`Bearer ${this._token}`),s}async _request(t,s,n=null,{signal:a}={}){this._lastActivity=Date.now();const i={method:t,headers:this._headers(),signal:a};n!==null&&(i.body=JSON.stringify(n));const l=await fetch(s,i);if(l.status===401)throw new vl("Unauthorized");const o=await l.json().catch(()=>null);if(!l.ok){const r=(o==null?void 0:o.error)||`HTTP ${l.status}`;throw new gd(r,l.status,o)}return o}get(t,s={}){return this._request("GET",t,null,s)}async getBlob(t){this._lastActivity=Date.now();const s=await fetch(t,{method:"GET",headers:this._headers()});if(s.status===401)throw new vl("Unauthorized");if(!s.ok){const n=await s.json().catch(()=>null);throw new gd((n==null?void 0:n.error)||`HTTP ${s.status}`,s.status,n)}return s.blob()}post(t,s){return this._request("POST",t,s)}put(t,s){return this._request("PUT",t,s)}del(t){return this._request("DELETE",t)}async login(t){const s=await fetch("/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:t})}),n=await s.json().catch(()=>null);if(!s.ok)throw new vl((n==null?void 0:n.error)||"Login failed");return this.setToken(n.session_id,n.timeout_seconds||0),n}async logout(){const t=this.post("/api/auth/logout",{});this.setToken("");try{await t}catch{}}async check(){try{return await this.get("/api/status"),{ok:!0,needsAuth:!1}}catch(t){return t instanceof vl?{ok:!1,needsAuth:!0}:{ok:!1,needsAuth:!1,error:t.message}}}}class vl extends Error{constructor(t){super(t),this.name="AuthError"}}class gd extends Error{constructor(t,s,n){super(t),this.name="ApiError",this.status=s,this.data=n}}class ov{constructor(t){this._api=t,this._ws=null,this._handlers={logs:[],events:[],chat:[]},this._reconnectDelay=1e3,this._maxReconnectDelay=3e4,this._shouldConnect=!1,this._subscriptions=new Set,this._reconnectAttempt=0,this._reconnectTimer=null,this._lastPongTime=0,this._pingInterval=null,this._forcedRetireTimer=null,this._subscriptionAckTimer=null,this._pendingReconnect=null,this._latency=-1,this._chatPending=!1,this._state="disconnected",this._lifecycle={status:new Set,state:new Set,latency:new Set,reconnected:new Set},this._everConnected=!1,this._reconnectEpoch=0}onStatus(t){return this._addLifecycle("status",t)}onState(t){return this._addLifecycle("state",t)}onLatencyChange(t){return this._addLifecycle("latency",t)}onReconnected(t){return this._addLifecycle("reconnected",t)}_addLifecycle(t,s){return this._lifecycle[t].add(s),()=>{this._lifecycle[t].delete(s)}}_emitLifecycle(t,...s){for(const n of[...this._lifecycle[t]])try{n(...s)}catch{}}get connected(){var t;return((t=this._ws)==null?void 0:t.readyState)===WebSocket.OPEN}get state(){return this._state}get reconnectAttempt(){return this._reconnectAttempt}get latency(){return this._latency}get reconnectEpoch(){return this._reconnectEpoch}_resetLatency(){this._latency=-1,this._emitLifecycle("latency",-1)}connect(){this._shouldConnect=!0,this._setState("connecting"),this._open()}disconnect(){this._shouldConnect=!1,this._everConnected=!1,this._reconnectTimer&&(clearTimeout(this._reconnectTimer),this._reconnectTimer=null),this._forcedRetireTimer&&(clearTimeout(this._forcedRetireTimer),this._forcedRetireTimer=null),this._subscriptionAckTimer&&(clearTimeout(this._subscriptionAckTimer),this._subscriptionAckTimer=null),this._pendingReconnect=null,this._reconnectAttempt=0,this._resetLatency(),this._stopPing(),this._ws&&(this._ws.close(),this._ws=null),this._setState("disconnected")}_setState(t){this._state!==t&&(this._state=t,this._emitLifecycle("state",t,{attempt:this._reconnectAttempt,latency:this._latency}))}_startPing(t){this._stopPing(),this._lastPongTime=Date.now(),this._pingInterval=setInterval(()=>{if(!(this._ws!==t||t.readyState!==WebSocket.OPEN)){if(this._lastPongTime&&Date.now()-this._lastPongTime>47e3){this._beginForcedRetirement(t,"pong timeout");return}try{t.send(JSON.stringify({type:"ping",ts:Date.now()}))}catch{}}},15e3)}_beginForcedRetirement(t,s){if(!(this._ws!==t||this._forcedRetireTimer)){this._stopPing(),this._reconnectAttempt++,this._setState("reconnecting"),this._emitLifecycle("status",!1),this._forcedRetireTimer=setTimeout(()=>{this._forcedRetireTimer=null,this._retireSocket(t,!0,!0)},1e3);try{t.close(4e3,s)}catch{}}}_scheduleReconnect(t=!0){!this._shouldConnect||this._reconnectTimer||(t&&this._reconnectAttempt++,this._setState("reconnecting"),this._reconnectTimer=setTimeout(()=>{this._reconnectTimer=null,this._open()},this._reconnectDelay),this._reconnectDelay=Math.min(this._reconnectDelay*2,this._maxReconnectDelay))}_retireSocket(t,s=!1,n=!1){if(this._ws===t){if(this._forcedRetireTimer&&(clearTimeout(this._forcedRetireTimer),this._forcedRetireTimer=null),this._subscriptionAckTimer&&(clearTimeout(this._subscriptionAckTimer),this._subscriptionAckTimer=null),this._pendingReconnect=null,this._ws=null,this._stopPing(),this._resetLatency(),this._chatPending){this._chatPending=!1;const a={type:"chat_error",error:"Connection lost — the response may still complete; check session history."};for(const i of this._handlers.chat||[])i(a)}s||this._emitLifecycle("status",!1),this._shouldConnect?this._scheduleReconnect(!n):this._setState("disconnected")}}_beginReconnectBarrier(t,s){if(!s)return;const n=new Set(this._subscriptions);if(n.size===0){this._reconnectEpoch+=1,this._emitLifecycle("reconnected",this._reconnectEpoch);return}this._pendingReconnect={socket:t,channels:n},this._subscriptionAckTimer=setTimeout(()=>{var a;((a=this._pendingReconnect)==null?void 0:a.socket)===t&&this._beginForcedRetirement(t,"subscription acknowledgement timeout")},5e3)}_ackSubscription(t,s){const n=this._pendingReconnect;!n||n.socket!==t||!n.channels.has(s)||(n.channels.delete(s),!(n.channels.size>0)&&(this._pendingReconnect=null,this._subscriptionAckTimer&&(clearTimeout(this._subscriptionAckTimer),this._subscriptionAckTimer=null),this._reconnectEpoch+=1,this._emitLifecycle("reconnected",this._reconnectEpoch)))}_stopPing(){this._pingInterval&&(clearInterval(this._pingInterval),this._pingInterval=null)}subscribe(t,s){var n;if(this._handlers[t]||(this._handlers[t]=[]),this._handlers[t].push(s),t!=="chat"&&(this._subscriptions.add(t),this.connected)){const a=this._ws;((n=this._pendingReconnect)==null?void 0:n.socket)===a&&this._pendingReconnect.channels.add(t),a.send(JSON.stringify({subscribe:t}))}}unsubscribe(t,s){const n=this._handlers[t];if(n){const a=n.indexOf(s);if(a>=0&&n.splice(a,1),n.length===0&&t!=="chat"&&(this._subscriptions.delete(t),this.connected)){const i=this._ws;i.send(JSON.stringify({unsubscribe:t})),this._ackSubscription(i,t)}}}on(t,s){return this.subscribe(t,s)}off(t,s){return this.unsubscribe(t,s)}sendChat(t,{channelId:s,userId:n,username:a}={}){return this.connected?(this._ws.send(JSON.stringify({type:"chat",content:t,channel_id:s||"web-default",user_id:n||void 0,username:a||void 0})),this._chatPending=!0,!0):!1}_open(){if(this._ws||!this._shouldConnect)return;const s=`${location.protocol==="https:"?"wss:":"ws:"}//${location.host}/api/ws`,n=this._api.token?["odin.bearer."+btoa(this._api.token).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"")]:void 0,a=n?new WebSocket(s,n):new WebSocket(s);this._ws=a;const i=()=>this._ws===a;a.onopen=()=>{if(!i())return;const l=this._everConnected;this._everConnected=!0,this._reconnectDelay=1e3,this._reconnectAttempt=0;for(const o of this._subscriptions)a.send(JSON.stringify({subscribe:o}));this._startPing(a),this._setState("connected"),this._emitLifecycle("status",!0),this._beginReconnectBarrier(a,l)},a.onmessage=l=>{if(!i())return;let o;try{o=JSON.parse(l.data)}catch{return}const r=o.type;if(r==="pong"){o.ts&&(this._latency=Date.now()-o.ts,this._lastPongTime=Date.now(),this._emitLifecycle("latency",this._latency));return}if(r==="subscribed"){this._ackSubscription(a,o.channel);return}if(r==="log")for(const c of this._handlers.logs||[])c(o);else if(r==="event")for(const c of this._handlers.events||[])c(o);else if(r==="chat_response"||r==="chat_error"){this._chatPending=!1;for(const c of this._handlers.chat||[])c(o)}},a.onclose=()=>{const l=!!this._forcedRetireTimer;this._retireSocket(a,l,l)},a.onerror=()=>{}}}const G=new lv,Qe=new ov(G);/**
* @vue/shared v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/function Os(e){const t=Object.create(null);for(const s of e.split(","))t[s]=1;return s=>s in t}const Ke={},za=[],Wt=()=>{},Ua=()=>!1,_a=e=>e.charCodeAt(0)===111&&e.charCodeAt(1)===110&&(e.charCodeAt(2)>122||e.charCodeAt(2)<97),bo=e=>e.startsWith("onUpdate:"),qe=Object.assign,ic=(e,t)=>{const s=e.indexOf(t);s>-1&&e.splice(s,1)},rv=Object.prototype.hasOwnProperty,nt=(e,t)=>rv.call(e,t),Ce=Array.isArray,ja=e=>di(e)==="[object Map]",wa=e=>di(e)==="[object Set]",bd=e=>di(e)==="[object Date]",cv=e=>di(e)==="[object RegExp]",Me=e=>typeof e=="function",Be=e=>typeof e=="string",is=e=>typeof e=="symbol",et=e=>e!==null&&typeof e=="object",lc=e=>(et(e)||Me(e))&&Me(e.then)&&Me(e.catch),kp=Object.prototype.toString,di=e=>kp.call(e),dv=e=>di(e).slice(8,-1),yo=e=>di(e)==="[object Object]",xo=e=>Be(e)&&e!=="NaN"&&e[0]!=="-"&&""+parseInt(e,10)===e,Tn=Os(",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted"),uv=Os("bind,cloak,else-if,else,for,html,if,model,on,once,pre,show,slot,text,memo"),_o=e=>{const t=Object.create(null);return(s=>t[s]||(t[s]=e(s)))},pv=/-\w/g,ut=_o(e=>e.replace(pv,t=>t.slice(1).toUpperCase())),fv=/\B([A-Z])/g,ws=_o(e=>e.replace(fv,"-$1").toLowerCase()),ka=_o(e=>e.charAt(0).toUpperCase()+e.slice(1)),Va=_o(e=>e?`on${ka(e)}`:""),Ut=(e,t)=>!Object.is(e,t),qa=(e,...t)=>{for(let s=0;s<e.length;s++)e[s](...t)},Sp=(e,t,s,n=!1)=>{Object.defineProperty(e,t,{configurable:!0,enumerable:!1,writable:n,value:s})},wo=e=>{const t=parseFloat(e);return isNaN(t)?e:t},Hl=e=>{const t=Be(e)?Number(e):NaN;return isNaN(t)?e:t};let yd;const ko=()=>yd||(yd=typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{});function hv(e,t){return e+JSON.stringify(t,(s,n)=>typeof n=="function"?n.toString():n)}const mv="Infinity,undefined,NaN,isFinite,isNaN,parseFloat,parseInt,decodeURI,decodeURIComponent,encodeURI,encodeURIComponent,Math,Number,Date,Array,Object,Boolean,String,RegExp,Map,Set,JSON,Intl,BigInt,console,Error,Symbol",vv=Os(mv);function il(e){if(Ce(e)){const t={};for(let s=0;s<e.length;s++){const n=e[s],a=Be(n)?Tp(n):il(n);if(a)for(const i in a)t[i]=a[i]}return t}else if(Be(e)||et(e))return e}const gv=/;(?![^(]*\))/g,bv=/:([^]+)/,yv=/\/\*[^]*?\*\//g;function Tp(e){const t={};return e.replace(yv,"").split(gv).forEach(s=>{if(s){const n=s.split(bv);n.length>1&&(t[n[0].trim()]=n[1].trim())}}),t}function ll(e){let t="";if(Be(e))t=e;else if(Ce(e))for(let s=0;s<e.length;s++){const n=ll(e[s]);n&&(t+=n+" ")}else if(et(e))for(const s in e)e[s]&&(t+=s+" ");return t.trim()}function xv(e){if(!e)return null;let{class:t,style:s}=e;return t&&!Be(t)&&(e.class=ll(t)),s&&(e.style=il(s)),e}const _v="html,body,base,head,link,meta,style,title,address,article,aside,footer,header,hgroup,h1,h2,h3,h4,h5,h6,nav,section,div,dd,dl,dt,figcaption,figure,picture,hr,img,li,main,ol,p,pre,ul,a,b,abbr,bdi,bdo,br,cite,code,data,dfn,em,i,kbd,mark,q,rp,rt,ruby,s,samp,small,span,strong,sub,sup,time,u,var,wbr,area,audio,map,track,video,embed,object,param,source,canvas,script,noscript,del,ins,caption,col,colgroup,table,thead,tbody,td,th,tr,button,datalist,fieldset,form,input,label,legend,meter,optgroup,option,output,progress,select,textarea,details,dialog,menu,summary,template,blockquote,iframe,tfoot",wv="svg,animate,animateMotion,animateTransform,circle,clipPath,color-profile,defs,desc,discard,ellipse,feBlend,feColorMatrix,feComponentTransfer,feComposite,feConvolveMatrix,feDiffuseLighting,feDisplacementMap,feDistantLight,feDropShadow,feFlood,feFuncA,feFuncB,feFuncG,feFuncR,feGaussianBlur,feImage,feMerge,feMergeNode,feMorphology,feOffset,fePointLight,feSpecularLighting,feSpotLight,feTile,feTurbulence,filter,foreignObject,g,hatch,hatchpath,image,line,linearGradient,marker,mask,mesh,meshgradient,meshpatch,meshrow,metadata,mpath,path,pattern,polygon,polyline,radialGradient,rect,set,solidcolor,stop,switch,symbol,text,textPath,title,tspan,unknown,use,view",kv="annotation,annotation-xml,maction,maligngroup,malignmark,math,menclose,merror,mfenced,mfrac,mfraction,mglyph,mi,mlabeledtr,mlongdiv,mmultiscripts,mn,mo,mover,mpadded,mphantom,mprescripts,mroot,mrow,ms,mscarries,mscarry,msgroup,msline,mspace,msqrt,msrow,mstack,mstyle,msub,msubsup,msup,mtable,mtd,mtext,mtr,munder,munderover,none,semantics",Sv="area,base,br,col,embed,hr,img,input,link,meta,param,source,track,wbr",Tv=Os(_v),Cv=Os(wv),Ev=Os(kv),Av=Os(Sv),Rv="itemscope,allowfullscreen,formnovalidate,ismap,nomodule,novalidate,readonly",Iv=Os(Rv);function Cp(e){return!!e||e===""}function Ov(e,t){if(e.length!==t.length)return!1;let s=!0;for(let n=0;s&&n<e.length;n++)s=Rn(e[n],t[n]);return s}function Rn(e,t){if(e===t)return!0;let s=bd(e),n=bd(t);if(s||n)return s&&n?e.getTime()===t.getTime():!1;if(s=is(e),n=is(t),s||n)return e===t;if(s=Ce(e),n=Ce(t),s||n)return s&&n?Ov(e,t):!1;if(s=et(e),n=et(t),s||n){if(!s||!n)return!1;const a=Object.keys(e).length,i=Object.keys(t).length;if(a!==i)return!1;for(const l in e){const o=e.hasOwnProperty(l),r=t.hasOwnProperty(l);if(o&&!r||!o&&r||!Rn(e[l],t[l]))return!1}}return String(e)===String(t)}function So(e,t){return e.findIndex(s=>Rn(s,t))}const Ep=e=>!!(e&&e.__v_isRef===!0),Ap=e=>Be(e)?e:e==null?"":Ce(e)||et(e)&&(e.toString===kp||!Me(e.toString))?Ep(e)?Ap(e.value):JSON.stringify(e,Rp,2):String(e),Rp=(e,t)=>Ep(t)?Rp(e,t.value):ja(t)?{[`Map(${t.size})`]:[...t.entries()].reduce((s,[n,a],i)=>(s[Wo(n,i)+" =>"]=a,s),{})}:wa(t)?{[`Set(${t.size})`]:[...t.values()].map(s=>Wo(s))}:is(t)?Wo(t):et(t)&&!Ce(t)&&!yo(t)?String(t):t,Wo=(e,t="")=>{var s;return is(e)?`Symbol(${(s=e.description)!=null?s:t})`:e};function Lv(e){return e==null?"initial":typeof e=="string"?e===""?" ":e:String(e)}/**
* @vue/reactivity v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/let $t;class oc{constructor(t=!1){this.detached=t,this._active=!0,this._on=0,this.effects=[],this.cleanups=[],this._isPaused=!1,this._warnOnRun=!0,this.__v_skip=!0,!t&&$t&&($t.active?(this.parent=$t,this.index=($t.scopes||($t.scopes=[])).push(this)-1):(this._active=!1,this._warnOnRun=!1))}get active(){return this._active}pause(){if(this._active){this._isPaused=!0;let t,s;if(this.scopes)for(t=0,s=this.scopes.length;t<s;t++)this.scopes[t].pause();for(t=0,s=this.effects.length;t<s;t++)this.effects[t].pause()}}resume(){if(this._active&&this._isPaused){this._isPaused=!1;let t,s;if(this.scopes)for(t=0,s=this.scopes.length;t<s;t++)this.scopes[t].resume();for(t=0,s=this.effects.length;t<s;t++)this.effects[t].resume()}}run(t){if(this._active){const s=$t;try{return $t=this,t()}finally{$t=s}}}on(){++this._on===1&&(this.prevScope=$t,$t=this)}off(){if(this._on>0&&--this._on===0){if($t===this)$t=this.prevScope;else{let t=$t;for(;t;){if(t.prevScope===this){t.prevScope=this.prevScope;break}t=t.prevScope}}this.prevScope=void 0}}stop(t){if(this._active){this._active=!1;let s,n;for(s=0,n=this.effects.length;s<n;s++)this.effects[s].stop();for(this.effects.length=0,s=0,n=this.cleanups.length;s<n;s++)this.cleanups[s]();if(this.cleanups.length=0,this.scopes){for(s=0,n=this.scopes.length;s<n;s++)this.scopes[s].stop(!0);this.scopes.length=0}if(!this.detached&&this.parent&&!t){const a=this.parent.scopes.pop();a&&a!==this&&(this.parent.scopes[this.index]=a,a.index=this.index)}this.parent=void 0}}}function Nv(e){return new oc(e)}function Ip(){return $t}function Dv(e,t=!1){$t&&$t.cleanups.push(e)}let ht;const Zo=new WeakSet;class Hi{constructor(t){this.fn=t,this.deps=void 0,this.depsTail=void 0,this.flags=5,this.next=void 0,this.cleanup=void 0,this.scheduler=void 0,$t&&($t.active?$t.effects.push(this):this.flags&=-2)}pause(){this.flags|=64}resume(){this.flags&64&&(this.flags&=-65,Zo.has(this)&&(Zo.delete(this),this.trigger()))}notify(){this.flags&2&&!(this.flags&32)||this.flags&8||Lp(this)}run(){if(!(this.flags&1))return this.fn();this.flags|=2,xd(this),Np(this);const t=ht,s=qs;ht=this,qs=!0;try{return this.fn()}finally{Dp(this),ht=t,qs=s,this.flags&=-3}}stop(){if(this.flags&1){for(let t=this.deps;t;t=t.nextDep)dc(t);this.deps=this.depsTail=void 0,xd(this),this.onStop&&this.onStop(),this.flags&=-2}}trigger(){this.flags&64?Zo.add(this):this.scheduler?this.scheduler():this.runIfDirty()}runIfDirty(){Sr(this)&&this.run()}get dirty(){return Sr(this)}}let Op=0,Ii,Oi;function Lp(e,t=!1){if(e.flags|=8,t){e.next=Oi,Oi=e;return}e.next=Ii,Ii=e}function rc(){Op++}function cc(){if(--Op>0)return;if(Oi){let t=Oi;for(Oi=void 0;t;){const s=t.next;t.next=void 0,t.flags&=-9,t=s}}let e;for(;Ii;){let t=Ii;for(Ii=void 0;t;){const s=t.next;if(t.next=void 0,t.flags&=-9,t.flags&1)try{t.trigger()}catch(n){e||(e=n)}t=s}}if(e)throw e}function Np(e){for(let t=e.deps;t;t=t.nextDep)t.version=-1,t.prevActiveLink=t.dep.activeLink,t.dep.activeLink=t}function Dp(e){let t,s=e.depsTail,n=s;for(;n;){const a=n.prevDep;n.version===-1?(n===s&&(s=a),dc(n),Pv(n)):t=n,n.dep.activeLink=n.prevActiveLink,n.prevActiveLink=void 0,n=a}e.deps=t,e.depsTail=s}function Sr(e){for(let t=e.deps;t;t=t.nextDep)if(t.dep.version!==t.version||t.dep.computed&&(Pp(t.dep.computed)||t.dep.version!==t.version))return!0;return!!e._dirty}function Pp(e){if(e.flags&4&&!(e.flags&16)||(e.flags&=-17,e.globalVersion===zi)||(e.globalVersion=zi,!e.isSSR&&e.flags&128&&(!e.deps&&!e._dirty||!Sr(e))))return;e.flags|=2;const t=e.dep,s=ht,n=qs;ht=e,qs=!0;try{Np(e);const a=e.fn(e._value);(t.version===0||Ut(a,e._value))&&(e.flags|=128,e._value=a,t.version++)}catch(a){throw t.version++,a}finally{ht=s,qs=n,Dp(e),e.flags&=-3}}function dc(e,t=!1){const{dep:s,prevSub:n,nextSub:a}=e;if(n&&(n.nextSub=a,e.prevSub=void 0),a&&(a.prevSub=n,e.nextSub=void 0),s.subs===e&&(s.subs=n,!n&&s.computed)){s.computed.flags&=-5;for(let i=s.computed.deps;i;i=i.nextDep)dc(i,!0)}!t&&!--s.sc&&s.map&&s.map.delete(s.key)}function Pv(e){const{prevDep:t,nextDep:s}=e;t&&(t.nextDep=s,e.prevDep=void 0),s&&(s.prevDep=t,e.nextDep=void 0)}function Mv(e,t){e.effect instanceof Hi&&(e=e.effect.fn);const s=new Hi(e);t&&qe(s,t);try{s.run()}catch(a){throw s.stop(),a}const n=s.run.bind(s);return n.effect=s,n}function Fv(e){e.effect.stop()}let qs=!0;const Mp=[];function In(){Mp.push(qs),qs=!1}function On(){const e=Mp.pop();qs=e===void 0?!0:e}function xd(e){const{cleanup:t}=e;if(e.cleanup=void 0,t){const s=ht;ht=void 0;try{t()}finally{ht=s}}}let zi=0;class $v{constructor(t,s){this.sub=t,this.dep=s,this.version=s.version,this.nextDep=this.prevDep=this.nextSub=this.prevSub=this.prevActiveLink=void 0}}class To{constructor(t){this.computed=t,this.version=0,this.activeLink=void 0,this.subs=void 0,this.map=void 0,this.key=void 0,this.sc=0,this.__v_skip=!0}track(t){if(!ht||!qs||ht===this.computed)return;let s=this.activeLink;if(s===void 0||s.sub!==ht)s=this.activeLink=new $v(ht,this),ht.deps?(s.prevDep=ht.depsTail,ht.depsTail.nextDep=s,ht.depsTail=s):ht.deps=ht.depsTail=s,Fp(s);else if(s.version===-1&&(s.version=this.version,s.nextDep)){const n=s.nextDep;n.prevDep=s.prevDep,s.prevDep&&(s.prevDep.nextDep=n),s.prevDep=ht.depsTail,s.nextDep=void 0,ht.depsTail.nextDep=s,ht.depsTail=s,ht.deps===s&&(ht.deps=n)}return s}trigger(t){this.version++,zi++,this.notify(t)}notify(t){rc();try{for(let s=this.subs;s;s=s.prevSub)s.sub.notify()&&s.sub.dep.notify()}finally{cc()}}}function Fp(e){if(e.dep.sc++,e.sub.flags&4){const t=e.dep.computed;if(t&&!e.dep.subs){t.flags|=20;for(let n=t.deps;n;n=n.nextDep)Fp(n)}const s=e.dep.subs;s!==e&&(e.prevSub=s,s&&(s.nextSub=e)),e.dep.subs=e}}const zl=new WeakMap,pa=Symbol(""),Tr=Symbol(""),ji=Symbol("");function ts(e,t,s){if(qs&&ht){let n=zl.get(e);n||zl.set(e,n=new Map);let a=n.get(s);a||(n.set(s,a=new To),a.map=n,a.key=s),a.track()}}function xn(e,t,s,n,a,i){const l=zl.get(e);if(!l){zi++;return}const o=r=>{r&&r.trigger()};if(rc(),t==="clear")l.forEach(o);else{const r=Ce(e),c=r&&xo(s);if(r&&s==="length"){const d=Number(n);l.forEach((u,p)=>{(p==="length"||p===ji||!is(p)&&p>=d)&&o(u)})}else switch((s!==void 0||l.has(void 0))&&o(l.get(s)),c&&o(l.get(ji)),t){case"add":r?c&&o(l.get("length")):(o(l.get(pa)),ja(e)&&o(l.get(Tr)));break;case"delete":r||(o(l.get(pa)),ja(e)&&o(l.get(Tr)));break;case"set":ja(e)&&o(l.get(pa));break}}cc()}function Bv(e,t){const s=zl.get(e);return s&&s.get(t)}function Ia(e){const t=Ye(e);return t===e?t:(ts(t,"iterate",ji),Ss(e)?t:t.map(Ks))}function Co(e){return ts(e=Ye(e),"iterate",ji),e}function nn(e,t){return ln(e)?Qa(Cn(e)?Ks(t):t):Ks(t)}const Uv={__proto__:null,[Symbol.iterator](){return Jo(this,Symbol.iterator,e=>nn(this,e))},concat(...e){return Ia(this).concat(...e.map(t=>Ce(t)?Ia(t):t))},entries(){return Jo(this,"entries",e=>(e[1]=nn(this,e[1]),e))},every(e,t){return pn(this,"every",e,t,void 0,arguments)},filter(e,t){return pn(this,"filter",e,t,s=>s.map(n=>nn(this,n)),arguments)},find(e,t){return pn(this,"find",e,t,s=>nn(this,s),arguments)},findIndex(e,t){return pn(this,"findIndex",e,t,void 0,arguments)},findLast(e,t){return pn(this,"findLast",e,t,s=>nn(this,s),arguments)},findLastIndex(e,t){return pn(this,"findLastIndex",e,t,void 0,arguments)},forEach(e,t){return pn(this,"forEach",e,t,void 0,arguments)},includes(...e){return Yo(this,"includes",e)},indexOf(...e){return Yo(this,"indexOf",e)},join(e){return Ia(this).join(e)},lastIndexOf(...e){return Yo(this,"lastIndexOf",e)},map(e,t){return pn(this,"map",e,t,void 0,arguments)},pop(){return mi(this,"pop")},push(...e){return mi(this,"push",e)},reduce(e,...t){return _d(this,"reduce",e,t)},reduceRight(e,...t){return _d(this,"reduceRight",e,t)},shift(){return mi(this,"shift")},some(e,t){return pn(this,"some",e,t,void 0,arguments)},splice(...e){return mi(this,"splice",e)},toReversed(){return Ia(this).toReversed()},toSorted(e){return Ia(this).toSorted(e)},toSpliced(...e){return Ia(this).toSpliced(...e)},unshift(...e){return mi(this,"unshift",e)},values(){return Jo(this,"values",e=>nn(this,e))}};function Jo(e,t,s){const n=Co(e),a=n[t]();return n!==e&&!Ss(e)&&(a._next=a.next,a.next=()=>{const i=a._next();return i.done||(i.value=s(i.value)),i}),a}const Hv=Array.prototype;function pn(e,t,s,n,a,i){const l=Co(e),o=l!==e&&!Ss(e),r=l[t];if(r!==Hv[t]){const u=r.apply(e,i);return o?Ks(u):u}let c=s;l!==e&&(o?c=function(u,p){return s.call(this,nn(e,u),p,e)}:s.length>2&&(c=function(u,p){return s.call(this,u,p,e)}));const d=r.call(l,c,n);return o&&a?a(d):d}function _d(e,t,s,n){const a=Co(e),i=a!==e&&!Ss(e);let l=s,o=!1;a!==e&&(i?(o=n.length===0,l=function(c,d,u){return o&&(o=!1,c=nn(e,c)),s.call(this,c,nn(e,d),u,e)}):s.length>3&&(l=function(c,d,u){return s.call(this,c,d,u,e)}));const r=a[t](l,...n);return o?nn(e,r):r}function Yo(e,t,s){const n=Ye(e);ts(n,"iterate",ji);const a=n[t](...s);return(a===-1||a===!1)&&ol(s[0])?(s[0]=Ye(s[0]),n[t](...s)):a}function mi(e,t,s=[]){In(),rc();const n=Ye(e)[t].apply(e,s);return cc(),On(),n}const zv=Os("__proto__,__v_isRef,__isVue"),$p=new Set(Object.getOwnPropertyNames(Symbol).filter(e=>e!=="arguments"&&e!=="caller").map(e=>Symbol[e]).filter(is));function jv(e){is(e)||(e=String(e));const t=Ye(this);return ts(t,"has",e),t.hasOwnProperty(e)}class Bp{constructor(t=!1,s=!1){this._isReadonly=t,this._isShallow=s}get(t,s,n){if(s==="__v_skip")return t.__v_skip;const a=this._isReadonly,i=this._isShallow;if(s==="__v_isReactive")return!a;if(s==="__v_isReadonly")return a;if(s==="__v_isShallow")return i;if(s==="__v_raw")return n===(a?i?qp:Vp:i?jp:zp).get(t)||Object.getPrototypeOf(t)===Object.getPrototypeOf(n)?t:void 0;const l=Ce(t);if(!a){let r;if(l&&(r=Uv[s]))return r;if(s==="hasOwnProperty")return jv}const o=Reflect.get(t,s,Lt(t)?t:n);if((is(s)?$p.has(s):zv(s))||(a||ts(t,"get",s),i))return o;if(Lt(o)){const r=l&&xo(s)?o:o.value;return a&&et(r)?jl(r):r}return et(o)?a?jl(o):Yn(o):o}}class Up extends Bp{constructor(t=!1){super(!1,t)}set(t,s,n,a){let i=t[s];const l=Ce(t)&&xo(s);if(!this._isShallow){const c=ln(i);if(!Ss(n)&&!ln(n)&&(i=Ye(i),n=Ye(n)),!l&&Lt(i)&&!Lt(n))return c||(i.value=n),!0}const o=l?Number(s)<t.length:nt(t,s),r=Reflect.set(t,s,n,Lt(t)?t:a);return t===Ye(a)&&(o?Ut(n,i)&&xn(t,"set",s,n):xn(t,"add",s,n)),r}deleteProperty(t,s){const n=nt(t,s);t[s];const a=Reflect.deleteProperty(t,s);return a&&n&&xn(t,"delete",s,void 0),a}has(t,s){const n=Reflect.has(t,s);return(!is(s)||!$p.has(s))&&ts(t,"has",s),n}ownKeys(t){return ts(t,"iterate",Ce(t)?"length":pa),Reflect.ownKeys(t)}}class Hp extends Bp{constructor(t=!1){super(!0,t)}set(t,s){return!0}deleteProperty(t,s){return!0}}const Vv=new Up,qv=new Hp,Gv=new Up(!0),Kv=new Hp(!0),Cr=e=>e,gl=e=>Reflect.getPrototypeOf(e);function Wv(e,t,s){return function(...n){const a=this.__v_raw,i=Ye(a),l=ja(i),o=e==="entries"||e===Symbol.iterator&&l,r=e==="keys"&&l,c=a[e](...n),d=s?Cr:t?Qa:Ks;return!t&&ts(i,"iterate",r?Tr:pa),qe(Object.create(c),{next(){const{value:u,done:p}=c.next();return p?{value:u,done:p}:{value:o?[d(u[0]),d(u[1])]:d(u),done:p}}})}}function bl(e){return function(...t){return e==="delete"?!1:e==="clear"?void 0:this}}function Zv(e,t){const s={get(a){const i=this.__v_raw,l=Ye(i),o=Ye(a);e||(Ut(a,o)&&ts(l,"get",a),ts(l,"get",o));const{has:r}=gl(l),c=t?Cr:e?Qa:Ks;if(r.call(l,a))return c(i.get(a));if(r.call(l,o))return c(i.get(o));i!==l&&i.get(a)},get size(){const a=this.__v_raw;return!e&&ts(Ye(a),"iterate",pa),a.size},has(a){const i=this.__v_raw,l=Ye(i),o=Ye(a);return e||(Ut(a,o)&&ts(l,"has",a),ts(l,"has",o)),a===o?i.has(a):i.has(a)||i.has(o)},forEach(a,i){const l=this,o=l.__v_raw,r=Ye(o),c=t?Cr:e?Qa:Ks;return!e&&ts(r,"iterate",pa),o.forEach((d,u)=>a.call(i,c(d),c(u),l))}};return qe(s,e?{add:bl("add"),set:bl("set"),delete:bl("delete"),clear:bl("clear")}:{add(a){const i=Ye(this),l=gl(i),o=Ye(a),r=!t&&!Ss(a)&&!ln(a)?o:a;return l.has.call(i,r)||Ut(a,r)&&l.has.call(i,a)||Ut(o,r)&&l.has.call(i,o)||(i.add(r),xn(i,"add",r,r)),this},set(a,i){!t&&!Ss(i)&&!ln(i)&&(i=Ye(i));const l=Ye(this),{has:o,get:r}=gl(l);let c=o.call(l,a);c||(a=Ye(a),c=o.call(l,a));const d=r.call(l,a);return l.set(a,i),c?Ut(i,d)&&xn(l,"set",a,i):xn(l,"add",a,i),this},delete(a){const i=Ye(this),{has:l,get:o}=gl(i);let r=l.call(i,a);r||(a=Ye(a),r=l.call(i,a)),o&&o.call(i,a);const c=i.delete(a);return r&&xn(i,"delete",a,void 0),c},clear(){const a=Ye(this),i=a.size!==0,l=a.clear();return i&&xn(a,"clear",void 0,void 0),l}}),["keys","values","entries",Symbol.iterator].forEach(a=>{s[a]=Wv(a,e,t)}),s}function Eo(e,t){const s=Zv(e,t);return(n,a,i)=>a==="__v_isReactive"?!e:a==="__v_isReadonly"?e:a==="__v_raw"?n:Reflect.get(nt(s,a)&&a in n?s:n,a,i)}const Jv={get:Eo(!1,!1)},Yv={get:Eo(!1,!0)},Qv={get:Eo(!0,!1)},Xv={get:Eo(!0,!0)},zp=new WeakMap,jp=new WeakMap,Vp=new WeakMap,qp=new WeakMap;function eg(e){switch(e){case"Object":case"Array":return 1;case"Map":case"Set":case"WeakMap":case"WeakSet":return 2;default:return 0}}function Yn(e){return ln(e)?e:Ao(e,!1,Vv,Jv,zp)}function uc(e){return Ao(e,!1,Gv,Yv,jp)}function jl(e){return Ao(e,!0,qv,Qv,Vp)}function tg(e){return Ao(e,!0,Kv,Xv,qp)}function Ao(e,t,s,n,a){if(!et(e)||e.__v_raw&&!(t&&e.__v_isReactive)||e.__v_skip||!Object.isExtensible(e))return e;const i=a.get(e);if(i)return i;const l=eg(dv(e));if(l===0)return e;const o=new Proxy(e,l===2?n:s);return a.set(e,o),o}function Cn(e){return ln(e)?Cn(e.__v_raw):!!(e&&e.__v_isReactive)}function ln(e){return!!(e&&e.__v_isReadonly)}function Ss(e){return!!(e&&e.__v_isShallow)}function ol(e){return e?!!e.__v_raw:!1}function Ye(e){const t=e&&e.__v_raw;return t?Ye(t):e}function Gp(e){return!nt(e,"__v_skip")&&Object.isExtensible(e)&&Sp(e,"__v_skip",!0),e}const Ks=e=>et(e)?Yn(e):e,Qa=e=>et(e)?jl(e):e;function Lt(e){return e?e.__v_isRef===!0:!1}function f(e){return Kp(e,!1)}function pc(e){return Kp(e,!0)}function Kp(e,t){return Lt(e)?e:new sg(e,t)}class sg{constructor(t,s){this.dep=new To,this.__v_isRef=!0,this.__v_isShallow=!1,this._rawValue=s?t:Ye(t),this._value=s?t:Ks(t),this.__v_isShallow=s}get value(){return this.dep.track(),this._value}set value(t){const s=this._rawValue,n=this.__v_isShallow||Ss(t)||ln(t);t=n?t:Ye(t),Ut(t,s)&&(this._rawValue=t,this._value=n?t:Ks(t),this.dep.trigger())}}function ng(e){e.dep&&e.dep.trigger()}function an(e){return Lt(e)?e.value:e}function ag(e){return Me(e)?e():an(e)}const ig={get:(e,t,s)=>t==="__v_raw"?e:an(Reflect.get(e,t,s)),set:(e,t,s,n)=>{const a=e[t];return Lt(a)&&!Lt(s)?(a.value=s,!0):Reflect.set(e,t,s,n)}};function fc(e){return Cn(e)?e:new Proxy(e,ig)}class lg{constructor(t){this.__v_isRef=!0,this._value=void 0;const s=this.dep=new To,{get:n,set:a}=t(s.track.bind(s),s.trigger.bind(s));this._get=n,this._set=a}get value(){return this._value=this._get()}set value(t){this._set(t)}}function Wp(e){return new lg(e)}function og(e){const t=Ce(e)?new Array(e.length):{};for(const s in e)t[s]=Zp(e,s);return t}class rg{constructor(t,s,n){this._object=t,this._defaultValue=n,this.__v_isRef=!0,this._value=void 0,this._key=is(s)?s:String(s),this._raw=Ye(t);let a=!0,i=t;if(!Ce(t)||is(this._key)||!xo(this._key))do a=!ol(i)||Ss(i);while(a&&(i=i.__v_raw));this._shallow=a}get value(){let t=this._object[this._key];return this._shallow&&(t=an(t)),this._value=t===void 0?this._defaultValue:t}set value(t){if(this._shallow&&Lt(this._raw[this._key])){const s=this._object[this._key];if(Lt(s)){s.value=t;return}}this._object[this._key]=t}get dep(){return Bv(this._raw,this._key)}}class cg{constructor(t){this._getter=t,this.__v_isRef=!0,this.__v_isReadonly=!0,this._value=void 0}get value(){return this._value=this._getter()}}function dg(e,t,s){return Lt(e)?e:Me(e)?new cg(e):et(e)&&arguments.length>1?Zp(e,t,s):f(e)}function Zp(e,t,s){return new rg(e,t,s)}class ug{constructor(t,s,n){this.fn=t,this.setter=s,this._value=void 0,this.dep=new To(this),this.__v_isRef=!0,this.deps=void 0,this.depsTail=void 0,this.flags=16,this.globalVersion=zi-1,this.next=void 0,this.effect=this,this.__v_isReadonly=!s,this.isSSR=n}notify(){if(this.flags|=16,!(this.flags&8)&&ht!==this)return Lp(this,!0),!0}get value(){const t=this.dep.track();return Pp(this),t&&(t.version=this.dep.version),this._value}set value(t){this.setter&&this.setter(t)}}function pg(e,t,s=!1){let n,a;return Me(e)?n=e:(n=e.get,a=e.set),new ug(n,a,s)}const fg={GET:"get",HAS:"has",ITERATE:"iterate"},hg={SET:"set",ADD:"add",DELETE:"delete",CLEAR:"clear"},yl={},Vl=new WeakMap;let Vn;function mg(){return Vn}function Jp(e,t=!1,s=Vn){if(s){let n=Vl.get(s);n||Vl.set(s,n=[]),n.push(e)}}function vg(e,t,s=Ke){const{immediate:n,deep:a,once:i,scheduler:l,augmentJob:o,call:r}=s,c=b=>a?b:Ss(b)||a===!1||a===0?_n(b,1):_n(b);let d,u,p,h,m=!1,v=!1;if(Lt(e)?(u=()=>e.value,m=Ss(e)):Cn(e)?(u=()=>c(e),m=!0):Ce(e)?(v=!0,m=e.some(b=>Cn(b)||Ss(b)),u=()=>e.map(b=>{if(Lt(b))return b.value;if(Cn(b))return c(b);if(Me(b))return r?r(b,2):b()})):Me(e)?t?u=r?()=>r(e,2):e:u=()=>{if(p){In();try{p()}finally{On()}}const b=Vn;Vn=d;try{return r?r(e,3,[h]):e(h)}finally{Vn=b}}:u=Wt,t&&a){const b=u,S=a===!0?1/0:a;u=()=>_n(b(),S)}const k=Ip(),R=()=>{d.stop(),k&&k.active&&ic(k.effects,d)};if(i&&t){const b=t;t=(...S)=>{const w=b(...S);return R(),w}}let y=v?new Array(e.length).fill(yl):yl;const g=b=>{if(!(!(d.flags&1)||!d.dirty&&!b))if(t){const S=d.run();if(b||a||m||(v?S.some((w,A)=>Ut(w,y[A])):Ut(S,y))){p&&p();const w=Vn;Vn=d;try{const A=[S,y===yl?void 0:v&&y[0]===yl?[]:y,h];y=S,r?r(t,3,A):t(...A)}finally{Vn=w}}}else d.run()};return o&&o(g),d=new Hi(u),d.scheduler=l?()=>l(g,!1):g,h=b=>Jp(b,!1,d),p=d.onStop=()=>{const b=Vl.get(d);if(b){if(r)r(b,4);else for(const S of b)S();Vl.delete(d)}},t?n?g(!0):y=d.run():l?l(g.bind(null,!0),!0):d.run(),R.pause=d.pause.bind(d),R.resume=d.resume.bind(d),R.stop=R,R}function _n(e,t=1/0,s){if(t<=0||!et(e)||e.__v_skip||(s=s||new Map,(s.get(e)||0)>=t))return e;if(s.set(e,t),t--,Lt(e))_n(e.value,t,s);else if(Ce(e))for(let n=0;n<e.length;n++)_n(e[n],t,s);else if(wa(e)||ja(e))e.forEach(n=>{_n(n,t,s)});else if(yo(e)){for(const n in e)_n(e[n],t,s);for(const n of Object.getOwnPropertySymbols(e))Object.prototype.propertyIsEnumerable.call(e,n)&&_n(e[n],t,s)}return e}/**
* @vue/runtime-core v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const Yp=[];function gg(e){Yp.push(e)}function bg(){Yp.pop()}function yg(e,t){}const xg={SETUP_FUNCTION:0,0:"SETUP_FUNCTION",RENDER_FUNCTION:1,1:"RENDER_FUNCTION",NATIVE_EVENT_HANDLER:5,5:"NATIVE_EVENT_HANDLER",COMPONENT_EVENT_HANDLER:6,6:"COMPONENT_EVENT_HANDLER",VNODE_HOOK:7,7:"VNODE_HOOK",DIRECTIVE_HOOK:8,8:"DIRECTIVE_HOOK",TRANSITION_HOOK:9,9:"TRANSITION_HOOK",APP_ERROR_HANDLER:10,10:"APP_ERROR_HANDLER",APP_WARN_HANDLER:11,11:"APP_WARN_HANDLER",FUNCTION_REF:12,12:"FUNCTION_REF",ASYNC_COMPONENT_LOADER:13,13:"ASYNC_COMPONENT_LOADER",SCHEDULER:14,14:"SCHEDULER",COMPONENT_UPDATE:15,15:"COMPONENT_UPDATE",APP_UNMOUNT_CLEANUP:16,16:"APP_UNMOUNT_CLEANUP"},_g={sp:"serverPrefetch hook",bc:"beforeCreate hook",c:"created hook",bm:"beforeMount hook",m:"mounted hook",bu:"beforeUpdate hook",u:"updated",bum:"beforeUnmount hook",um:"unmounted hook",a:"activated hook",da:"deactivated hook",ec:"errorCaptured hook",rtc:"renderTracked hook",rtg:"renderTriggered hook",0:"setup function",1:"render function",2:"watcher getter",3:"watcher callback",4:"watcher cleanup function",5:"native event handler",6:"component event handler",7:"vnode hook",8:"directive hook",9:"transition hook",10:"app errorHandler",11:"app warnHandler",12:"ref function",13:"async component loader",14:"scheduler flush",15:"component update",16:"app unmount cleanup function"};function ui(e,t,s,n){try{return n?e(...n):e()}catch(a){Sa(a,t,s)}}function Is(e,t,s,n){if(Me(e)){const a=ui(e,t,s,n);return a&&lc(a)&&a.catch(i=>{Sa(i,t,s)}),a}if(Ce(e)){const a=[];for(let i=0;i<e.length;i++)a.push(Is(e[i],t,s,n));return a}}function Sa(e,t,s,n=!0){const a=t?t.vnode:null,{errorHandler:i,throwUnhandledErrorInProduction:l}=t&&t.appContext.config||Ke;if(t){let o=t.parent;const r=t.proxy,c=`https://vuejs.org/error-reference/#runtime-${s}`;for(;o;){const d=o.ec;if(d){for(let u=0;u<d.length;u++)if(d[u](e,r,c)===!1)return}o=o.parent}if(i){In(),ui(i,null,10,[e,r,c]),On();return}}wg(e,s,a,n,l)}function wg(e,t,s,n=!0,a=!1){if(a)throw e;console.error(e)}const fs=[];let tn=-1;const Ga=[];let qn=null,Ma=0;const Qp=Promise.resolve();let ql=null;function It(e){const t=ql||Qp;return e?t.then(this?e.bind(this):e):t}function kg(e){let t=tn+1,s=fs.length;for(;t<s;){const n=t+s>>>1,a=fs[n],i=qi(a);i<e||i===e&&a.flags&2?t=n+1:s=n}return t}function hc(e){if(!(e.flags&1)){const t=qi(e),s=fs[fs.length-1];!s||!(e.flags&2)&&t>=qi(s)?fs.push(e):fs.splice(kg(t),0,e),e.flags|=1,Xp()}}function Xp(){ql||(ql=Qp.then(ef))}function Vi(e){Ce(e)?Ga.push(...e):qn&&e.id===-1?qn.splice(Ma+1,0,e):e.flags&1||(Ga.push(e),e.flags|=1),Xp()}function wd(e,t,s=tn+1){for(;s<fs.length;s++){const n=fs[s];if(n&&n.flags&2){if(e&&n.id!==e.uid)continue;fs.splice(s,1),s--,n.flags&4&&(n.flags&=-2),n(),n.flags&4||(n.flags&=-2)}}}function Gl(e){if(Ga.length){const t=[...new Set(Ga)].sort((s,n)=>qi(s)-qi(n));if(Ga.length=0,qn){qn.push(...t);return}for(qn=t,Ma=0;Ma<qn.length;Ma++){const s=qn[Ma];s.flags&4&&(s.flags&=-2),s.flags&8||s(),s.flags&=-2}qn=null,Ma=0}}const qi=e=>e.id==null?e.flags&2?-1:1/0:e.id;function ef(e){try{for(tn=0;tn<fs.length;tn++){const t=fs[tn];t&&!(t.flags&8)&&(t.flags&4&&(t.flags&=-2),ui(t,t.i,t.i?15:14),t.flags&4||(t.flags&=-2))}}finally{for(;tn<fs.length;tn++){const t=fs[tn];t&&(t.flags&=-2)}tn=-1,fs.length=0,Gl(),ql=null,(fs.length||Ga.length)&&ef()}}let Fa,xl=[];function tf(e,t){var s,n;Fa=e,Fa?(Fa.enabled=!0,xl.forEach(({event:a,args:i})=>Fa.emit(a,...i)),xl=[]):typeof window<"u"&&window.HTMLElement&&!((n=(s=window.navigator)==null?void 0:s.userAgent)!=null&&n.includes("jsdom"))?((t.__VUE_DEVTOOLS_HOOK_REPLAY__=t.__VUE_DEVTOOLS_HOOK_REPLAY__||[]).push(i=>{tf(i,t)}),setTimeout(()=>{Fa||(t.__VUE_DEVTOOLS_HOOK_REPLAY__=null,xl=[])},3e3)):xl=[]}let Kt=null,Ro=null;function Gi(e){const t=Kt;return Kt=e,Ro=e&&e.type.__scopeId||null,t}function Sg(e){Ro=e}function Tg(){Ro=null}const Cg=e=>mc;function mc(e,t=Kt,s){if(!t||e._n)return e;const n=(...a)=>{n._d&&Ji(-1);const i=Gi(t);let l;try{l=e(...a)}finally{Gi(i),n._d&&Ji(1)}return l};return n._n=!0,n._c=!0,n._d=!0,n}function Eg(e,t){if(Kt===null)return e;const s=ul(Kt),n=e.dirs||(e.dirs=[]);for(let a=0;a<t.length;a++){let[i,l,o,r=Ke]=t[a];i&&(Me(i)&&(i={mounted:i,updated:i}),i.deep&&_n(l),n.push({dir:i,instance:s,value:l,oldValue:void 0,arg:o,modifiers:r}))}return e}function sn(e,t,s,n){const a=e.dirs,i=t&&t.dirs;for(let l=0;l<a.length;l++){const o=a[l];i&&(o.oldValue=i[l].value);let r=o.dir[n];r&&(In(),Is(r,s,8,[e.el,o,e,t]),On())}}function Li(e,t){if(Gt){let s=Gt.provides;const n=Gt.parent&&Gt.parent.provides;n===s&&(s=Gt.provides=Object.create(n)),s[e]=t}}function Bs(e,t,s=!1){const n=ms();if(n||fa){let a=fa?fa._context.provides:n?n.parent==null||n.ce?n.vnode.appContext&&n.vnode.appContext.provides:n.parent.provides:void 0;if(a&&e in a)return a[e];if(arguments.length>1)return s&&Me(t)?t.call(n&&n.proxy):t}}function Ag(){return!!(ms()||fa)}const sf=Symbol.for("v-scx"),nf=()=>Bs(sf);function Rg(e,t){return rl(e,null,t)}function Ig(e,t){return rl(e,null,{flush:"post"})}function af(e,t){return rl(e,null,{flush:"sync"})}function ns(e,t,s){return rl(e,t,s)}function rl(e,t,s=Ke){const{immediate:n,deep:a,flush:i,once:l}=s,o=qe({},s),r=t&&n||!t&&i!=="post";let c;if(ba){if(i==="sync"){const h=nf();c=h.__watcherHandles||(h.__watcherHandles=[])}else if(!r){const h=()=>{};return h.stop=Wt,h.resume=Wt,h.pause=Wt,h}}const d=Gt;o.call=(h,m,v)=>Is(h,d,m,v);let u=!1;i==="post"?o.scheduler=h=>{Rt(h,d&&d.suspense)}:i!=="sync"&&(u=!0,o.scheduler=(h,m)=>{m?h():hc(h)}),o.augmentJob=h=>{t&&(h.flags|=4),u&&(h.flags|=2,d&&(h.id=d.uid,h.i=d))};const p=vg(e,t,o);return ba&&(c?c.push(p):r&&p()),p}function Og(e,t,s){const n=this.proxy,a=Be(e)?e.includes(".")?lf(n,e):()=>n[e]:e.bind(n,n);let i;Me(t)?i=t:(i=t.handler,s=t);const l=pi(this),o=rl(a,i.bind(n),s);return l(),o}function lf(e,t){const s=t.split(".");return()=>{let n=e;for(let a=0;a<s.length&&n;a++)n=n[s[a]];return n}}const zn=new WeakMap,of=Symbol("_vte"),rf=e=>e.__isTeleport,ra=e=>e&&(e.disabled||e.disabled===""),Lg=e=>e&&(e.defer||e.defer===""),kd=e=>typeof SVGElement<"u"&&e instanceof SVGElement,Sd=e=>typeof MathMLElement=="function"&&e instanceof MathMLElement,Er=(e,t)=>{const s=e&&e.to;return Be(s)?t?t(s):null:s},Ng={name:"Teleport",__isTeleport:!0,process(e,t,s,n,a,i,l,o,r,c){const{mc:d,pc:u,pbc:p,o:{insert:h,querySelector:m,createText:v,createComment:k,parentNode:R}}=c,y=ra(t.props);let{dynamicChildren:g}=t;const b=(A,E,x)=>{A.shapeFlag&16&&d(A.children,E,x,a,i,l,o,r)},S=(A=t)=>{const E=ra(A.props),x=A.target=Er(A.props,m),N=Ar(x,A,v,h);x&&(l!=="svg"&&kd(x)?l="svg":l!=="mathml"&&Sd(x)&&(l="mathml"),a&&a.isCE&&(a.ce._teleportTargets||(a.ce._teleportTargets=new Set)).add(x),E||(b(A,x,N),Ti(A,!1)))},w=A=>{const E=()=>{if(zn.get(A)===E){if(zn.delete(A),ra(A.props)){const x=R(A.el)||s;b(A,x,A.anchor),Ti(A,!0)}S(A)}};zn.set(A,E),Rt(E,i)};if(e==null){const A=t.el=v(""),E=t.anchor=v("");if(h(A,s,n),h(E,s,n),Lg(t.props)||i&&i.pendingBranch){w(t);return}y&&(b(t,s,E),Ti(t,!0)),S()}else{t.el=e.el;const A=t.anchor=e.anchor,E=zn.get(e);if(E){E.flags|=8,zn.delete(e),w(t);return}t.targetStart=e.targetStart;const x=t.target=e.target,N=t.targetAnchor=e.targetAnchor,$=ra(e.props),C=$?s:x,M=$?A:N;if(l==="svg"||kd(x)?l="svg":(l==="mathml"||Sd(x))&&(l="mathml"),g?(p(e.dynamicChildren,g,C,a,i,l,o),Cc(e,t,!0)):r||u(e,t,C,M,a,i,l,o,!1),y)$?t.props&&e.props&&t.props.to!==e.props.to&&(t.props.to=e.props.to):_l(t,s,A,c,1);else if((t.props&&t.props.to)!==(e.props&&e.props.to)){const V=t.target=Er(t.props,m);V&&_l(t,V,null,c,0)}else $&&_l(t,x,N,c,1);Ti(t,y)}},remove(e,t,s,{um:n,o:{remove:a}},i){const{shapeFlag:l,children:o,anchor:r,targetStart:c,targetAnchor:d,target:u,props:p}=e,h=i||!ra(p),m=zn.get(e);if(m&&(m.flags|=8,zn.delete(e)),u&&(a(c),a(d)),i&&a(r),!m&&l&16)for(let v=0;v<o.length;v++){const k=o[v];n(k,t,s,h,!!k.dynamicChildren)}},move:_l,hydrate:Dg};function _l(e,t,s,{o:{insert:n},m:a},i=2){i===0&&n(e.targetAnchor,t,s);const{el:l,anchor:o,shapeFlag:r,children:c,props:d}=e,u=i===2;if(u&&n(l,t,s),!zn.has(e)&&(!u||ra(d))&&r&16)for(let p=0;p<c.length;p++)a(c[p],t,s,2);u&&n(o,t,s)}function Dg(e,t,s,n,a,i,{o:{nextSibling:l,parentNode:o,querySelector:r,insert:c,createText:d}},u){function p(k,R){let y=R;for(;y;){if(y&&y.nodeType===8){if(y.data==="teleport start anchor")t.targetStart=y;else if(y.data==="teleport anchor"){t.targetAnchor=y,k._lpa=t.targetAnchor&&l(t.targetAnchor);break}}y=l(y)}}function h(k,R){R.anchor=u(l(k),R,o(k),s,n,a,i)}const m=t.target=Er(t.props,r),v=ra(t.props);if(m){const k=m._lpa||m.firstChild;t.shapeFlag&16&&(v?(h(e,t),p(m,k),t.targetAnchor||Ar(m,t,d,c,o(e)===m?e:null)):(t.anchor=l(e),p(m,k),t.targetAnchor||Ar(m,t,d,c),u(k&&l(k),t,m,s,n,a,i))),Ti(t,v)}else v&&t.shapeFlag&16&&(h(e,t),t.targetStart=e,t.targetAnchor=l(e));return t.anchor&&l(t.anchor)}const Pg=Ng;function Ti(e,t){const s=e.ctx;if(s&&s.ut){let n,a;for(t?(n=e.el,a=e.anchor):(n=e.targetStart,a=e.targetAnchor);n&&n!==a;)n.nodeType===1&&n.setAttribute("data-v-owner",s.uid),n=n.nextSibling;s.ut()}}function Ar(e,t,s,n,a=null){const i=t.targetStart=s(""),l=t.targetAnchor=s("");return i[of]=l,e&&(n(i,e,a),n(l,e,a)),l}const Ps=Symbol("_leaveCb"),vi=Symbol("_enterCb");function vc(){const e={isMounted:!1,isLeaving:!1,isUnmounting:!1,leavingVNodes:new Map};return Ge(()=>{e.isMounted=!0}),No(()=>{e.isUnmounting=!0}),e}const Ds=[Function,Array],gc={mode:String,appear:Boolean,persisted:Boolean,onBeforeEnter:Ds,onEnter:Ds,onAfterEnter:Ds,onEnterCancelled:Ds,onBeforeLeave:Ds,onLeave:Ds,onAfterLeave:Ds,onLeaveCancelled:Ds,onBeforeAppear:Ds,onAppear:Ds,onAfterAppear:Ds,onAppearCancelled:Ds},cf=e=>{const t=e.subTree;return t.component?cf(t.component):t},Mg={name:"BaseTransition",props:gc,setup(e,{slots:t}){const s=ms(),n=vc();return()=>{const a=t.default&&Io(t.default(),!0),i=a&&a.length?df(a):s.subTree?Gf():void 0;if(!i)return;const l=Ye(e),{mode:o}=l;if(n.isLeaving)return Qo(i);const r=Td(i);if(!r)return Qo(i);let c=Xa(r,l,n,s,u=>c=u);r.type!==Ct&&Ln(r,c);let d=s.subTree&&Td(s.subTree);if(d&&d.type!==Ct&&!Vs(d,r)&&cf(s).type!==Ct){let u=Xa(d,l,n,s);if(Ln(d,u),o==="out-in"&&r.type!==Ct)return n.isLeaving=!0,u.afterLeave=()=>{n.isLeaving=!1,s.job.flags&8||s.update(),delete u.afterLeave,d=void 0},Qo(i);o==="in-out"&&r.type!==Ct?u.delayLeave=(p,h,m)=>{const v=pf(n,d);v[String(d.key)]=d,p[Ps]=()=>{h(),p[Ps]=void 0,delete c.delayedLeave,d=void 0},c.delayedLeave=()=>{m(),delete c.delayedLeave,d=void 0}}:d=void 0}else d&&(d=void 0);return i}}};function df(e){let t=e[0];if(e.length>1){for(const s of e)if(s.type!==Ct){t=s;break}}return t}const uf=Mg;function pf(e,t){const{leavingVNodes:s}=e;let n=s.get(t.type);return n||(n=Object.create(null),s.set(t.type,n)),n}function Xa(e,t,s,n,a){const{appear:i,mode:l,persisted:o=!1,onBeforeEnter:r,onEnter:c,onAfterEnter:d,onEnterCancelled:u,onBeforeLeave:p,onLeave:h,onAfterLeave:m,onLeaveCancelled:v,onBeforeAppear:k,onAppear:R,onAfterAppear:y,onAppearCancelled:g}=t,b=String(e.key),S=pf(s,e),w=(x,N)=>{x&&Is(x,n,9,N)},A=(x,N)=>{const $=N[1];w(x,N),Ce(x)?x.every(C=>C.length<=1)&&$():x.length<=1&&$()},E={mode:l,persisted:o,beforeEnter(x){let N=r;if(!s.isMounted)if(i)N=k||r;else return;x[Ps]&&x[Ps](!0);const $=S[b];$&&Vs(e,$)&&$.el[Ps]&&$.el[Ps](),w(N,[x])},enter(x){if(S[b]===e)return;let N=c,$=d,C=u;if(!s.isMounted)if(i)N=R||c,$=y||d,C=g||u;else return;let M=!1;x[vi]=q=>{M||(M=!0,q?w(C,[x]):w($,[x]),E.delayedLeave&&E.delayedLeave(),x[vi]=void 0)};const V=x[vi].bind(null,!1);N?A(N,[x,V]):V()},leave(x,N){const $=String(e.key);if(x[vi]&&x[vi](!0),s.isUnmounting)return N();w(p,[x]);let C=!1;x[Ps]=V=>{C||(C=!0,N(),V?w(v,[x]):w(m,[x]),x[Ps]=void 0,S[$]===e&&delete S[$])};const M=x[Ps].bind(null,!1);S[$]=e,h?A(h,[x,M]):M()},clone(x){const N=Xa(x,t,s,n,a);return a&&a(N),N}};return E}function Qo(e){if(dl(e))return e=on(e),e.children=null,e}function Td(e){if(!dl(e))return rf(e.type)&&e.children?df(e.children):e;if(e.component)return e.component.subTree;const{shapeFlag:t,children:s}=e;if(s){if(t&16)return s[0];if(t&32&&Me(s.default))return s.default()}}function Ln(e,t){e.shapeFlag&6&&e.component?(e.transition=t,Ln(e.component.subTree,t)):e.shapeFlag&128?(e.ssContent.transition=t.clone(e.ssContent),e.ssFallback.transition=t.clone(e.ssFallback)):e.transition=t}function Io(e,t=!1,s){let n=[],a=0;for(let i=0;i<e.length;i++){let l=e[i];const o=s==null?l.key:String(s)+String(l.key!=null?l.key:i);l.type===Ht?(l.patchFlag&128&&a++,n=n.concat(Io(l.children,t,o))):(t||l.type!==Ct)&&n.push(o!=null?on(l,{key:o}):l)}if(a>1)for(let i=0;i<n.length;i++)n[i].patchFlag=-2;return n}function cl(e,t){return Me(e)?qe({name:e.name},t,{setup:e}):e}function Fg(){const e=ms();return e?(e.appContext.config.idPrefix||"v")+"-"+e.ids[0]+e.ids[1]++:""}function bc(e){e.ids=[e.ids[0]+e.ids[2]+++"-",0,0]}function $g(e){const t=ms(),s=pc(null);if(t){const a=t.refs===Ke?t.refs={}:t.refs;Object.defineProperty(a,e,{enumerable:!0,get:()=>s.value,set:i=>s.value=i})}return s}function Cd(e,t){let s;return!!((s=Object.getOwnPropertyDescriptor(e,t))&&!s.configurable)}const Kl=new WeakMap;function Ka(e,t,s,n,a=!1){if(Ce(e)){e.forEach((v,k)=>Ka(v,t&&(Ce(t)?t[k]:t),s,n,a));return}if(En(n)&&!a){n.shapeFlag&512&&n.type.__asyncResolved&&n.component.subTree.component&&Ka(e,t,s,n.component.subTree);return}const i=n.shapeFlag&4?ul(n.component):n.el,l=a?null:i,{i:o,r}=e,c=t&&t.r,d=o.refs===Ke?o.refs={}:o.refs,u=o.setupState,p=Ye(u),h=u===Ke?Ua:v=>Cd(d,v)?!1:nt(p,v),m=(v,k)=>!(k&&Cd(d,k));if(c!=null&&c!==r){if(Ed(t),Be(c))d[c]=null,h(c)&&(u[c]=null);else if(Lt(c)){const v=t;m(c,v.k)&&(c.value=null),v.k&&(d[v.k]=null)}}if(Me(r))ui(r,o,12,[l,d]);else{const v=Be(r),k=Lt(r);if(v||k){const R=()=>{if(e.f){const y=v?h(r)?u[r]:d[r]:m()||!e.k?r.value:d[e.k];if(a)Ce(y)&&ic(y,i);else if(Ce(y))y.includes(i)||y.push(i);else if(v)d[r]=[i],h(r)&&(u[r]=d[r]);else{const g=[i];m(r,e.k)&&(r.value=g),e.k&&(d[e.k]=g)}}else v?(d[r]=l,h(r)&&(u[r]=l)):k&&(m(r,e.k)&&(r.value=l),e.k&&(d[e.k]=l))};if(l){const y=()=>{R(),Kl.delete(e)};y.id=-1,Kl.set(e,y),Rt(y,s)}else Ed(e),R()}}}function Ed(e){const t=Kl.get(e);t&&(t.flags|=8,Kl.delete(e))}let Ad=!1;const Oa=()=>{Ad||(console.error("Hydration completed but contains mismatches."),Ad=!0)},Bg=e=>e.namespaceURI.includes("svg")&&e.tagName!=="foreignObject",Ug=e=>e.namespaceURI.includes("MathML"),wl=e=>{if(e.nodeType===1){if(Bg(e))return"svg";if(Ug(e))return"mathml"}},Ha=e=>e.nodeType===8;function Hg(e){const{mt:t,p:s,o:{patchProp:n,createText:a,nextSibling:i,parentNode:l,remove:o,insert:r,createComment:c}}=e,d=(g,b)=>{if(!b.hasChildNodes()){s(null,g,b),Gl(),b._vnode=g;return}u(b.firstChild,g,null,null,null),Gl(),b._vnode=g},u=(g,b,S,w,A,E=!1)=>{E=E||!!b.dynamicChildren;const x=Ha(g)&&g.data==="[",N=()=>v(g,b,S,w,A,x),{type:$,ref:C,shapeFlag:M,patchFlag:V}=b;let q=g.nodeType;b.el=g,V===-2&&(E=!1,b.dynamicChildren=null);let D=null;switch($){case Wn:q!==3?b.children===""?(r(b.el=a(""),l(g),g),D=g):D=N():(g.data!==b.children&&(Oa(),g.data=b.children),D=i(g));break;case Ct:y(g)?(D=i(g),R(b.el=g.content.firstChild,g,S)):q!==8||x?D=N():D=i(g);break;case ha:if(x&&(g=i(g),q=g.nodeType),q===1||q===3){D=g;const I=!b.children.length;for(let O=0;O<b.staticCount;O++)I&&(b.children+=D.nodeType===1?D.outerHTML:D.data),O===b.staticCount-1&&(b.anchor=D),D=i(D);return x?i(D):D}else N();break;case Ht:x?D=m(g,b,S,w,A,E):D=N();break;default:if(M&1)(q!==1||b.type.toLowerCase()!==g.tagName.toLowerCase())&&!y(g)?D=N():D=p(g,b,S,w,A,E);else if(M&6){b.slotScopeIds=A;const I=l(g);if(x?D=k(g):Ha(g)&&g.data==="teleport start"?D=k(g,g.data,"teleport end"):D=i(g),t(b,I,null,S,w,wl(I),E),En(b)&&!b.type.__asyncResolved){let O;x?(O=xt(Ht),O.anchor=D?D.previousSibling:I.lastChild):O=g.nodeType===3?Ac(""):xt("div"),O.el=g,b.component.subTree=O}}else M&64?q!==8?D=N():D=b.type.hydrate(g,b,S,w,A,E,e,h):M&128&&(D=b.type.hydrate(g,b,S,w,wl(l(g)),A,E,e,u))}return C!=null&&Ka(C,null,w,b),D},p=(g,b,S,w,A,E)=>{E=E||!!b.dynamicChildren;const{type:x,props:N,patchFlag:$,shapeFlag:C,dirs:M,transition:V}=b,q=x==="input"||x==="option";if(q||$!==-1){M&&sn(b,null,S,"created");let D=!1;if(y(g)){D=Ff(null,V)&&S&&S.vnode.props&&S.vnode.props.appear;const O=g.content.firstChild;if(D){const U=O.getAttribute("class");U&&(O.$cls=U),V.beforeEnter(O)}R(O,g,S),b.el=g=O}if(C&16&&!(N&&(N.innerHTML||N.textContent))){let O=h(g.firstChild,b,g,S,w,A,E);for(O&&!kl(g,1)&&Oa();O;){const U=O;O=O.nextSibling,o(U)}}else if(C&8){let O=b.children;O[0]===`
`&&(g.tagName==="PRE"||g.tagName==="TEXTAREA")&&(O=O.slice(1));const{textContent:U}=g;U!==O&&U!==O.replace(/\r\n|\r/g,`
`)&&(kl(g,0)||Oa(),g.textContent=b.children)}if(N){if(q||!E||$&48){const O=g.tagName.includes("-");for(const U in N)(q&&(U.endsWith("value")||U==="indeterminate")||_a(U)&&!Tn(U)||U[0]==="."||O&&!Tn(U))&&n(g,U,null,N[U],void 0,S)}else if(N.onClick)n(g,"onClick",null,N.onClick,void 0,S);else if($&4&&Cn(N.style))for(const O in N.style)N.style[O]}let I;(I=N&&N.onVnodeBeforeMount)&&ys(I,S,b),M&&sn(b,null,S,"beforeMount"),((I=N&&N.onVnodeMounted)||M||D)&&Hf(()=>{I&&ys(I,S,b),D&&V.enter(g),M&&sn(b,null,S,"mounted")},w)}return g.nextSibling},h=(g,b,S,w,A,E,x)=>{x=x||!!b.dynamicChildren;const N=b.children,$=N.length;let C=!1;for(let M=0;M<$;M++){const V=x?N[M]:N[M]=_s(N[M]),q=V.type===Wn;g?(q&&!x&&M+1<$&&_s(N[M+1]).type===Wn&&(r(a(g.data.slice(V.children.length)),S,i(g)),g.data=V.children),g=u(g,V,w,A,E,x)):q&&!V.children?r(V.el=a(""),S):(C||(C=!0,kl(S,1)||Oa()),s(null,V,S,null,w,A,wl(S),E))}return g},m=(g,b,S,w,A,E)=>{const{slotScopeIds:x}=b;x&&(A=A?A.concat(x):x);const N=l(g),$=h(i(g),b,N,S,w,A,E);return $&&Ha($)&&$.data==="]"?i(b.anchor=$):(Oa(),r(b.anchor=c("]"),N,$),$)},v=(g,b,S,w,A,E)=>{if(kl(g.parentElement,1)||Oa(),b.el=null,E){const $=k(g);for(;;){const C=i(g);if(C&&C!==$)o(C);else break}}const x=i(g),N=l(g);return o(g),s(null,b,N,x,S,w,wl(N),A),S&&(S.vnode.el=b.el,Po(S,b.el)),x},k=(g,b="[",S="]")=>{let w=0;for(;g;)if(g=i(g),g&&Ha(g)&&(g.data===b&&w++,g.data===S)){if(w===0)return i(g);w--}return g},R=(g,b,S)=>{const w=b.parentNode;w&&w.replaceChild(g,b);let A=S;for(;A;)A.vnode.el===b&&(A.vnode.el=A.subTree.el=g),A=A.parent},y=g=>g.nodeType===1&&g.tagName==="TEMPLATE";return[d,u]}const Rd="data-allow-mismatch",zg={0:"text",1:"children",2:"class",3:"style",4:"attribute"};function kl(e,t){if(t===0||t===1)for(;e&&!e.hasAttribute(Rd);)e=e.parentElement;const s=e&&e.getAttribute(Rd);if(s==null)return!1;if(s==="")return!0;{const n=s.split(",");return t===0&&n.includes("children")?!0:n.includes(zg[t])}}const jg=ko().requestIdleCallback||(e=>setTimeout(e,1)),Vg=ko().cancelIdleCallback||(e=>clearTimeout(e)),qg=(e=1e4)=>t=>{const s=jg(t,{timeout:e});return()=>Vg(s)};function Gg(e){const{top:t,left:s,bottom:n,right:a}=e.getBoundingClientRect(),{innerHeight:i,innerWidth:l}=window;return(t>0&&t<i||n>0&&n<i)&&(s>0&&s<l||a>0&&a<l)}const Kg=e=>(t,s)=>{const n=new IntersectionObserver(a=>{for(const i of a)if(i.isIntersecting){n.disconnect(),t();break}},e);return s(a=>{if(a instanceof Element){if(Gg(a))return t(),n.disconnect(),!1;n.observe(a)}}),()=>n.disconnect()},Wg=e=>t=>{if(e){const s=matchMedia(e);if(s.matches)t();else return s.addEventListener("change",t,{once:!0}),()=>s.removeEventListener("change",t)}},Zg=(e=[])=>(t,s)=>{Be(e)&&(e=[e]);let n=!1;const a=l=>{n||(n=!0,i(),t(),l.target.dispatchEvent(new l.constructor(l.type,l)))},i=()=>{s(l=>{for(const o of e)l.removeEventListener(o,a)})};return s(l=>{for(const o of e)l.addEventListener(o,a,{once:!0})}),i};function Jg(e,t){if(Ha(e)&&e.data==="["){let s=1,n=e.nextSibling;for(;n;){if(n.nodeType===1){if(t(n)===!1)break}else if(Ha(n))if(n.data==="]"){if(--s===0)break}else n.data==="["&&s++;n=n.nextSibling}}else t(e)}const En=e=>!!e.type.__asyncLoader;function Yg(e){Me(e)&&(e={loader:e});const{loader:t,loadingComponent:s,errorComponent:n,delay:a=200,hydrate:i,timeout:l,suspensible:o=!0,onError:r}=e;let c=null,d,u=0;const p=()=>(u++,c=null,h()),h=()=>{let m;return c||(m=c=t().catch(v=>{if(v=v instanceof Error?v:new Error(String(v)),r)return new Promise((k,R)=>{r(v,()=>k(p()),()=>R(v),u+1)});throw v}).then(v=>m!==c&&c?c:(v&&(v.__esModule||v[Symbol.toStringTag]==="Module")&&(v=v.default),d=v,v)))};return cl({name:"AsyncComponentWrapper",__asyncLoader:h,__asyncHydrate(m,v,k){let R=!1;(v.bu||(v.bu=[])).push(()=>R=!0);const y=()=>{R||k()},g=i?()=>{const b=i(y,S=>Jg(m,S));b&&(v.bum||(v.bum=[])).push(b)}:y;d?g():h().then(()=>!v.isUnmounted&&g())},get __asyncResolved(){return d},setup(){const m=Gt;if(bc(m),d)return()=>Sl(d,m);const v=S=>{c=null,Sa(S,m,13,!n)};if(o&&m.suspense||ba)return h().then(S=>()=>Sl(S,m)).catch(S=>(v(S),()=>n?xt(n,{error:S}):null));const k=f(!1),R=f(),y=f(!!a);let g,b;return bt(()=>{g!=null&&clearTimeout(g),b!=null&&clearTimeout(b)}),a&&(b=setTimeout(()=>{m.isUnmounted||(y.value=!1)},a)),l!=null&&(g=setTimeout(()=>{if(!m.isUnmounted&&!k.value&&!R.value){const S=new Error(`Async component timed out after ${l}ms.`);v(S),R.value=S}},l)),h().then(()=>{m.isUnmounted||(k.value=!0,m.parent&&dl(m.parent.vnode)&&m.parent.update())}).catch(S=>{if(m.isUnmounted){c=null;return}v(S),R.value=S}),()=>{if(k.value&&d)return Sl(d,m);if(R.value&&n)return xt(n,{error:R.value});if(s&&!y.value)return Sl(s,m)}}})}function Sl(e,t){const{ref:s,props:n,children:a,ce:i}=t.vnode,l=xt(e,n,a);return l.ref=s,l.ce=i,delete t.vnode.ce,l}const dl=e=>e.type.__isKeepAlive,Qg={name:"KeepAlive",__isKeepAlive:!0,props:{include:[String,RegExp,Array],exclude:[String,RegExp,Array],max:[String,Number]},setup(e,{slots:t}){const s=ms(),n=s.ctx;if(!n.renderer)return()=>{const y=t.default&&t.default();return y&&y.length===1?y[0]:y};const a=new Map,i=new Set;let l=null;const o=s.suspense,{renderer:{p:r,m:c,um:d,o:{createElement:u}}}=n,p=u("div");n.activate=(y,g,b,S,w)=>{const A=y.component;c(y,g,b,0,o),r(A.vnode,y,g,b,A,o,S,y.slotScopeIds,w),Rt(()=>{A.isDeactivated=!1,A.a&&qa(A.a);const E=y.props&&y.props.onVnodeMounted;E&&ys(E,A.parent,y)},o)},n.deactivate=y=>{const g=y.component;Zl(g.m),Zl(g.a),c(y,p,null,1,o),Rt(()=>{g.da&&qa(g.da);const b=y.props&&y.props.onVnodeUnmounted;b&&ys(b,g.parent,y),g.isDeactivated=!0},o)};function h(y){Xo(y),d(y,s,o,!0)}function m(y){a.forEach((g,b)=>{const S=Fr(En(g)?g.type.__asyncResolved||{}:g.type);S&&!y(S)&&v(b)})}function v(y){const g=a.get(y);g&&(!l||!Vs(g,l))?h(g):l&&Xo(l),a.delete(y),i.delete(y)}ns(()=>[e.include,e.exclude],([y,g])=>{y&&m(b=>Ci(y,b)),g&&m(b=>!Ci(g,b))},{flush:"post",deep:!0});let k=null;const R=()=>{k!=null&&(Jl(s.subTree.type)?Rt(()=>{a.set(k,Tl(s.subTree))},s.subTree.suspense):a.set(k,Tl(s.subTree)))};return Ge(R),Lo(R),No(()=>{a.forEach(y=>{const{subTree:g,suspense:b}=s,S=Tl(g);if(y.type===S.type&&y.key===S.key){Xo(S);const w=S.component.da;w&&Rt(w,b);return}h(y)})}),()=>{if(k=null,!t.default)return l=null;const y=t.default(),g=y[0];if(y.length>1)return l=null,y;if(!Nn(g)||!(g.shapeFlag&4)&&!(g.shapeFlag&128))return l=null,g;let b=Tl(g);if(b.type===Ct)return l=null,b;const S=b.type,w=Fr(En(b)?b.type.__asyncResolved||{}:S),{include:A,exclude:E,max:x}=e;if(A&&(!w||!Ci(A,w))||E&&w&&Ci(E,w))return b.shapeFlag&=-257,l=b,g;const N=b.key==null?S:b.key,$=a.get(N);return b.el&&(b=on(b),g.shapeFlag&128&&(g.ssContent=b)),k=N,$?(b.el=$.el,b.component=$.component,b.transition&&Ln(b,b.transition),b.shapeFlag|=512,i.delete(N),i.add(N)):(i.add(N),x&&i.size>parseInt(x,10)&&v(i.values().next().value)),b.shapeFlag|=256,l=b,Jl(g.type)?g:b}}},Xg=Qg;function Ci(e,t){return Ce(e)?e.some(s=>Ci(s,t)):Be(e)?e.split(",").includes(t):cv(e)?(e.lastIndex=0,e.test(t)):!1}function gs(e,t){ff(e,"a",t)}function ls(e,t){ff(e,"da",t)}function ff(e,t,s=Gt){const n=e.__wdc||(e.__wdc=()=>{let a=s;for(;a;){if(a.isDeactivated)return;a=a.parent}return e()});if(Oo(t,n,s),s){let a=s.parent;for(;a&&a.parent;)dl(a.parent.vnode)&&eb(n,t,s,a),a=a.parent}}function eb(e,t,s,n){const a=Oo(t,e,n,!0);bt(()=>{ic(n[t],a)},s)}function Xo(e){e.shapeFlag&=-257,e.shapeFlag&=-513}function Tl(e){return e.shapeFlag&128?e.ssContent:e}function Oo(e,t,s=Gt,n=!1){if(s){const a=s[e]||(s[e]=[]),i=t.__weh||(t.__weh=(...l)=>{In();const o=pi(s),r=Is(t,s,e,l);return o(),On(),r});return n?a.unshift(i):a.push(i),i}}const Dn=e=>(t,s=Gt)=>{(!ba||e==="sp")&&Oo(e,(...n)=>t(...n),s)},hf=Dn("bm"),Ge=Dn("m"),yc=Dn("bu"),Lo=Dn("u"),No=Dn("bum"),bt=Dn("um"),mf=Dn("sp"),vf=Dn("rtg"),gf=Dn("rtc");function bf(e,t=Gt){Oo("ec",e,t)}const xc="components",tb="directives";function sb(e,t){return _c(xc,e,!0,t)||e}const yf=Symbol.for("v-ndc");function nb(e){return Be(e)?_c(xc,e,!1)||e:e||yf}function ab(e){return _c(tb,e)}function _c(e,t,s=!0,n=!1){const a=Kt||Gt;if(a){const i=a.type;if(e===xc){const o=Fr(i,!1);if(o&&(o===t||o===ut(t)||o===ka(ut(t))))return i}const l=Id(a[e]||i[e],t)||Id(a.appContext[e],t);return!l&&n?i:l}}function Id(e,t){return e&&(e[t]||e[ut(t)]||e[ka(ut(t))])}function ib(e,t,s,n){let a;const i=s&&s[n],l=Ce(e);if(l||Be(e)){const o=l&&Cn(e);let r=!1,c=!1;o&&(r=!Ss(e),c=ln(e),e=Co(e)),a=new Array(e.length);for(let d=0,u=e.length;d<u;d++)a[d]=t(r?c?Qa(Ks(e[d])):Ks(e[d]):e[d],d,void 0,i&&i[d])}else if(typeof e=="number"){a=new Array(e);for(let o=0;o<e;o++)a[o]=t(o+1,o,void 0,i&&i[o])}else if(et(e))if(e[Symbol.iterator])a=Array.from(e,(o,r)=>t(o,r,void 0,i&&i[r]));else{const o=Object.keys(e);a=new Array(o.length);for(let r=0,c=o.length;r<c;r++){const d=o[r];a[r]=t(e[d],d,r,i&&i[r])}}else a=[];return s&&(s[n]=a),a}function lb(e,t){for(let s=0;s<t.length;s++){const n=t[s];if(Ce(n))for(let a=0;a<n.length;a++)e[n[a].name]=n[a].fn;else n&&(e[n.name]=n.key?(...a)=>{const i=n.fn(...a);return i&&(i.key=n.key),i}:n.fn)}return e}function ob(e,t,s={},n,a){if(Kt.ce||Kt.parent&&En(Kt.parent)&&Kt.parent.ce){const c=Object.keys(s).length>0;return t!=="default"&&(s.name=t),Zi(),Yl(Ht,null,[xt("slot",s,n&&n())],c?-2:64)}let i=e[t];i&&i._c&&(i._d=!1),Zi();const l=i&&wc(i(s)),o=s.key||l&&l.key,r=Yl(Ht,{key:(o&&!is(o)?o:`_${t}`)+(!l&&n?"_fb":"")},l||(n?n():[]),l&&e._===1?64:-2);return!a&&r.scopeId&&(r.slotScopeIds=[r.scopeId+"-s"]),i&&i._c&&(i._d=!0),r}function wc(e){return e.some(t=>Nn(t)?!(t.type===Ct||t.type===Ht&&!wc(t.children)):!0)?e:null}function rb(e,t){const s={};for(const n in e)s[t&&/[A-Z]/.test(n)?`on:${n}`:Va(n)]=e[n];return s}const Rr=e=>e?Zf(e)?ul(e):Rr(e.parent):null,Ni=qe(Object.create(null),{$:e=>e,$el:e=>e.vnode.el,$data:e=>e.data,$props:e=>e.props,$attrs:e=>e.attrs,$slots:e=>e.slots,$refs:e=>e.refs,$parent:e=>Rr(e.parent),$root:e=>Rr(e.root),$host:e=>e.ce,$emit:e=>e.emit,$options:e=>kc(e),$forceUpdate:e=>e.f||(e.f=()=>{hc(e.update)}),$nextTick:e=>e.n||(e.n=It.bind(e.proxy)),$watch:e=>Og.bind(e)}),er=(e,t)=>e!==Ke&&!e.__isScriptSetup&&nt(e,t),Ir={get({_:e},t){if(t==="__v_skip")return!0;const{ctx:s,setupState:n,data:a,props:i,accessCache:l,type:o,appContext:r}=e;if(t[0]!=="$"){const p=l[t];if(p!==void 0)switch(p){case 1:return n[t];case 2:return a[t];case 4:return s[t];case 3:return i[t]}else{if(er(n,t))return l[t]=1,n[t];if(a!==Ke&&nt(a,t))return l[t]=2,a[t];if(nt(i,t))return l[t]=3,i[t];if(s!==Ke&&nt(s,t))return l[t]=4,s[t];Or&&(l[t]=0)}}const c=Ni[t];let d,u;if(c)return t==="$attrs"&&ts(e.attrs,"get",""),c(e);if((d=o.__cssModules)&&(d=d[t]))return d;if(s!==Ke&&nt(s,t))return l[t]=4,s[t];if(u=r.config.globalProperties,nt(u,t))return u[t]},set({_:e},t,s){const{data:n,setupState:a,ctx:i}=e;return er(a,t)?(a[t]=s,!0):n!==Ke&&nt(n,t)?(n[t]=s,!0):nt(e.props,t)||t[0]==="$"&&t.slice(1)in e?!1:(i[t]=s,!0)},has({_:{data:e,setupState:t,accessCache:s,ctx:n,appContext:a,props:i,type:l}},o){let r;return!!(s[o]||e!==Ke&&o[0]!=="$"&&nt(e,o)||er(t,o)||nt(i,o)||nt(n,o)||nt(Ni,o)||nt(a.config.globalProperties,o)||(r=l.__cssModules)&&r[o])},defineProperty(e,t,s){return s.get!=null?e._.accessCache[t]=0:nt(s,"value")&&this.set(e,t,s.value,null),Reflect.defineProperty(e,t,s)}},cb=qe({},Ir,{get(e,t){if(t!==Symbol.unscopables)return Ir.get(e,t,e)},has(e,t){return t[0]!=="_"&&!vv(t)}});function db(){return null}function ub(){return null}function pb(e){}function fb(e){}function hb(){return null}function mb(){}function vb(e,t){return null}function gb(){return xf().slots}function bb(){return xf().attrs}function xf(e){const t=ms();return t.setupContext||(t.setupContext=Xf(t))}function Ki(e){return Ce(e)?e.reduce((t,s)=>(t[s]=null,t),{}):e}function yb(e,t){const s=Ki(e);for(const n in t){if(n.startsWith("__skip"))continue;let a=s[n];a?Ce(a)||Me(a)?a=s[n]={type:a,default:t[n]}:a.default=t[n]:a===null&&(a=s[n]={default:t[n]}),a&&t[`__skip_${n}`]&&(a.skipFactory=!0)}return s}function xb(e,t){return!e||!t?e||t:Ce(e)&&Ce(t)?e.concat(t):qe({},Ki(e),Ki(t))}function _b(e,t){const s={};for(const n in e)t.includes(n)||Object.defineProperty(s,n,{enumerable:!0,get:()=>e[n]});return s}function wb(e){const t=ms(),s=ba;let n=e();Yi(),s&&Za(!1);const a=()=>{pi(t),s&&Za(!0)},i=()=>{ms()!==t&&t.scope.off(),Yi(),s&&Za(!1)};return lc(n)&&(n=n.catch(l=>{throw a(),Promise.resolve().then(()=>Promise.resolve().then(i)),l})),[n,()=>{a(),Promise.resolve().then(i)}]}let Or=!0;function kb(e){const t=kc(e),s=e.proxy,n=e.ctx;Or=!1,t.beforeCreate&&Od(t.beforeCreate,e,"bc");const{data:a,computed:i,methods:l,watch:o,provide:r,inject:c,created:d,beforeMount:u,mounted:p,beforeUpdate:h,updated:m,activated:v,deactivated:k,beforeDestroy:R,beforeUnmount:y,destroyed:g,unmounted:b,render:S,renderTracked:w,renderTriggered:A,errorCaptured:E,serverPrefetch:x,expose:N,inheritAttrs:$,components:C,directives:M,filters:V}=t;if(c&&Sb(c,n,null),l)for(const I in l){const O=l[I];Me(O)&&(n[I]=O.bind(s))}if(a){const I=a.call(s,s);et(I)&&(e.data=Yn(I))}if(Or=!0,i)for(const I in i){const O=i[I],U=Me(O)?O.bind(s,s):Me(O.get)?O.get.bind(s,s):Wt,W=!Me(O)&&Me(O.set)?O.set.bind(s):Wt,K=J({get:U,set:W});Object.defineProperty(n,I,{enumerable:!0,configurable:!0,get:()=>K.value,set:X=>K.value=X})}if(o)for(const I in o)_f(o[I],n,s,I);if(r){const I=Me(r)?r.call(s):r;Reflect.ownKeys(I).forEach(O=>{Li(O,I[O])})}d&&Od(d,e,"c");function D(I,O){Ce(O)?O.forEach(U=>I(U.bind(s))):O&&I(O.bind(s))}if(D(hf,u),D(Ge,p),D(yc,h),D(Lo,m),D(gs,v),D(ls,k),D(bf,E),D(gf,w),D(vf,A),D(No,y),D(bt,b),D(mf,x),Ce(N))if(N.length){const I=e.exposed||(e.exposed={});N.forEach(O=>{Object.defineProperty(I,O,{get:()=>s[O],set:U=>s[O]=U,enumerable:!0})})}else e.exposed||(e.exposed={});S&&e.render===Wt&&(e.render=S),$!=null&&(e.inheritAttrs=$),C&&(e.components=C),M&&(e.directives=M),x&&bc(e)}function Sb(e,t,s=Wt){Ce(e)&&(e=Lr(e));for(const n in e){const a=e[n];let i;et(a)?"default"in a?i=Bs(a.from||n,a.default,!0):i=Bs(a.from||n):i=Bs(a),Lt(i)?Object.defineProperty(t,n,{enumerable:!0,configurable:!0,get:()=>i.value,set:l=>i.value=l}):t[n]=i}}function Od(e,t,s){Is(Ce(e)?e.map(n=>n.bind(t.proxy)):e.bind(t.proxy),t,s)}function _f(e,t,s,n){let a=n.includes(".")?lf(s,n):()=>s[n];if(Be(e)){const i=t[e];Me(i)&&ns(a,i)}else if(Me(e))ns(a,e.bind(s));else if(et(e))if(Ce(e))e.forEach(i=>_f(i,t,s,n));else{const i=Me(e.handler)?e.handler.bind(s):t[e.handler];Me(i)&&ns(a,i,e)}}function kc(e){const t=e.type,{mixins:s,extends:n}=t,{mixins:a,optionsCache:i,config:{optionMergeStrategies:l}}=e.appContext,o=i.get(t);let r;return o?r=o:!a.length&&!s&&!n?r=t:(r={},a.length&&a.forEach(c=>Wl(r,c,l,!0)),Wl(r,t,l)),et(t)&&i.set(t,r),r}function Wl(e,t,s,n=!1){const{mixins:a,extends:i}=t;i&&Wl(e,i,s,!0),a&&a.forEach(l=>Wl(e,l,s,!0));for(const l in t)if(!(n&&l==="expose")){const o=Tb[l]||s&&s[l];e[l]=o?o(e[l],t[l]):t[l]}return e}const Tb={data:Ld,props:Nd,emits:Nd,methods:Ei,computed:Ei,beforeCreate:ds,created:ds,beforeMount:ds,mounted:ds,beforeUpdate:ds,updated:ds,beforeDestroy:ds,beforeUnmount:ds,destroyed:ds,unmounted:ds,activated:ds,deactivated:ds,errorCaptured:ds,serverPrefetch:ds,components:Ei,directives:Ei,watch:Eb,provide:Ld,inject:Cb};function Ld(e,t){return t?e?function(){return qe(Me(e)?e.call(this,this):e,Me(t)?t.call(this,this):t)}:t:e}function Cb(e,t){return Ei(Lr(e),Lr(t))}function Lr(e){if(Ce(e)){const t={};for(let s=0;s<e.length;s++)t[e[s]]=e[s];return t}return e}function ds(e,t){return e?[...new Set([].concat(e,t))]:t}function Ei(e,t){return e?qe(Object.create(null),e,t):t}function Nd(e,t){return e?Ce(e)&&Ce(t)?[...new Set([...e,...t])]:qe(Object.create(null),Ki(e),Ki(t??{})):t}function Eb(e,t){if(!e)return t;if(!t)return e;const s=qe(Object.create(null),e);for(const n in t)s[n]=ds(e[n],t[n]);return s}function wf(){return{app:null,config:{isNativeTag:Ua,performance:!1,globalProperties:{},optionMergeStrategies:{},errorHandler:void 0,warnHandler:void 0,compilerOptions:{}},mixins:[],components:{},directives:{},provides:Object.create(null),optionsCache:new WeakMap,propsCache:new WeakMap,emitsCache:new WeakMap}}let Ab=0;function Rb(e,t){return function(n,a=null){Me(n)||(n=qe({},n)),a!=null&&!et(a)&&(a=null);const i=wf(),l=new WeakSet,o=[];let r=!1;const c=i.app={_uid:Ab++,_component:n,_props:a,_container:null,_context:i,_instance:null,version:th,get config(){return i.config},set config(d){},use(d,...u){return l.has(d)||(d&&Me(d.install)?(l.add(d),d.install(c,...u)):Me(d)&&(l.add(d),d(c,...u))),c},mixin(d){return i.mixins.includes(d)||i.mixins.push(d),c},component(d,u){return u?(i.components[d]=u,c):i.components[d]},directive(d,u){return u?(i.directives[d]=u,c):i.directives[d]},mount(d,u,p){if(!r){const h=c._ceVNode||xt(n,a);return h.appContext=i,p===!0?p="svg":p===!1&&(p=void 0),u&&t?t(h,d):e(h,d,p),r=!0,c._container=d,d.__vue_app__=c,ul(h.component)}},onUnmount(d){o.push(d)},unmount(){r&&(Is(o,c._instance,16),e(null,c._container),delete c._container.__vue_app__)},provide(d,u){return i.provides[d]=u,c},runWithContext(d){const u=fa;fa=c;try{return d()}finally{fa=u}}};return c}}let fa=null;function Ib(e,t,s=Ke){const n=ms(),a=ut(t),i=ws(t),l=kf(e,a),o=Wp((r,c)=>{let d,u=Ke,p;return af(()=>{const h=e[a];Ut(d,h)&&(d=h,c())}),{get(){return r(),s.get?s.get(d):d},set(h){const m=s.set?s.set(h):h;if(!Ut(m,d)&&!(u!==Ke&&Ut(h,u)))return;const v=n.vnode.props,k=!!(v&&(t in v||a in v||i in v)&&(`onUpdate:${t}`in v||`onUpdate:${a}`in v||`onUpdate:${i}`in v));k||(d=h,c()),n.emit(`update:${t}`,m),Ut(h,u)&&(Ut(h,m)&&!Ut(m,p)||k&&u!==Ke&&!Ut(m,d))&&c(),u=h,p=m}}});return o[Symbol.iterator]=()=>{let r=0;return{next(){return r<2?{value:r++?l||Ke:o,done:!1}:{done:!0}}}},o}const kf=(e,t)=>t==="modelValue"||t==="model-value"?e.modelModifiers:e[`${t}Modifiers`]||e[`${ut(t)}Modifiers`]||e[`${ws(t)}Modifiers`];function Ob(e,t,...s){if(e.isUnmounted)return;const n=e.vnode.props||Ke;let a=s;const i=t.startsWith("update:"),l=i&&kf(n,t.slice(7));l&&(l.trim&&(a=s.map(d=>Be(d)?d.trim():d)),l.number&&(a=s.map(wo)));let o,r=n[o=Va(t)]||n[o=Va(ut(t))];!r&&i&&(r=n[o=Va(ws(t))]),r&&Is(r,e,6,a);const c=n[o+"Once"];if(c){if(!e.emitted)e.emitted={};else if(e.emitted[o])return;e.emitted[o]=!0,Is(c,e,6,a)}}const Lb=new WeakMap;function Sf(e,t,s=!1){const n=s?Lb:t.emitsCache,a=n.get(e);if(a!==void 0)return a;const i=e.emits;let l={},o=!1;if(!Me(e)){const r=c=>{const d=Sf(c,t,!0);d&&(o=!0,qe(l,d))};!s&&t.mixins.length&&t.mixins.forEach(r),e.extends&&r(e.extends),e.mixins&&e.mixins.forEach(r)}return!i&&!o?(et(e)&&n.set(e,null),null):(Ce(i)?i.forEach(r=>l[r]=null):qe(l,i),et(e)&&n.set(e,l),l)}function Do(e,t){return!e||!_a(t)?!1:(t=t.slice(2).replace(/Once$/,""),nt(e,t[0].toLowerCase()+t.slice(1))||nt(e,ws(t))||nt(e,t))}function Dl(e){const{type:t,vnode:s,proxy:n,withProxy:a,propsOptions:[i],slots:l,attrs:o,emit:r,render:c,renderCache:d,props:u,data:p,setupState:h,ctx:m,inheritAttrs:v}=e,k=Gi(e);let R,y;try{if(s.shapeFlag&4){const b=a||n,S=b;R=_s(c.call(S,b,d,u,h,p,m)),y=o}else{const b=t;R=_s(b.length>1?b(u,{attrs:o,slots:l,emit:r}):b(u,null)),y=t.props?o:Db(o)}}catch(b){Di.length=0,Sa(b,e,1),R=xt(Ct)}let g=R;if(y&&v!==!1){const b=Object.keys(y),{shapeFlag:S}=g;b.length&&S&7&&(i&&b.some(bo)&&(y=Pb(y,i)),g=on(g,y,!1,!0))}return s.dirs&&(g=on(g,null,!1,!0),g.dirs=g.dirs?g.dirs.concat(s.dirs):s.dirs),s.transition&&Ln(g,s.transition),R=g,Gi(k),R}function Nb(e,t=!0){let s;for(let n=0;n<e.length;n++){const a=e[n];if(Nn(a)){if(a.type!==Ct||a.children==="v-if"){if(s)return;s=a}}else return}return s}const Db=e=>{let t;for(const s in e)(s==="class"||s==="style"||_a(s))&&((t||(t={}))[s]=e[s]);return t},Pb=(e,t)=>{const s={};for(const n in e)(!bo(n)||!(n.slice(9)in t))&&(s[n]=e[n]);return s};function Mb(e,t,s){const{props:n,children:a,component:i}=e,{props:l,children:o,patchFlag:r}=t,c=i.emitsOptions;if(t.dirs||t.transition)return!0;if(s&&r>=0){if(r&1024)return!0;if(r&16)return n?Dd(n,l,c):!!l;if(r&8){const d=t.dynamicProps;for(let u=0;u<d.length;u++){const p=d[u];if(Tf(l,n,p)&&!Do(c,p))return!0}}}else return(a||o)&&(!o||!o.$stable)?!0:n===l?!1:n?l?Dd(n,l,c):!0:!!l;return!1}function Dd(e,t,s){const n=Object.keys(t);if(n.length!==Object.keys(e).length)return!0;for(let a=0;a<n.length;a++){const i=n[a];if(Tf(t,e,i)&&!Do(s,i))return!0}return!1}function Tf(e,t,s){const n=e[s],a=t[s];return s==="style"&&et(n)&&et(a)?!Rn(n,a):n!==a}function Po({vnode:e,parent:t,suspense:s},n){for(;t;){const a=t.subTree;if(a.suspense&&a.suspense.activeBranch===e&&(a.suspense.vnode.el=a.el=n,e=a),a===e)(e=t.vnode).el=n,t=t.parent;else break}s&&s.activeBranch===e&&(s.vnode.el=n)}const Cf={},Ef=()=>Object.create(Cf),Af=e=>Object.getPrototypeOf(e)===Cf;function Fb(e,t,s,n=!1){const a={},i=Ef();e.propsDefaults=Object.create(null),Rf(e,t,a,i);for(const l in e.propsOptions[0])l in a||(a[l]=void 0);s?e.props=n?a:uc(a):e.type.props?e.props=a:e.props=i,e.attrs=i}function $b(e,t,s,n){const{props:a,attrs:i,vnode:{patchFlag:l}}=e,o=Ye(a),[r]=e.propsOptions;let c=!1;if((n||l>0)&&!(l&16)){if(l&8){const d=e.vnode.dynamicProps;for(let u=0;u<d.length;u++){let p=d[u];if(Do(e.emitsOptions,p))continue;const h=t[p];if(r)if(nt(i,p))h!==i[p]&&(i[p]=h,c=!0);else{const m=ut(p);a[m]=Nr(r,o,m,h,e,!1)}else h!==i[p]&&(i[p]=h,c=!0)}}}else{Rf(e,t,a,i)&&(c=!0);let d;for(const u in o)(!t||!nt(t,u)&&((d=ws(u))===u||!nt(t,d)))&&(r?s&&(s[u]!==void 0||s[d]!==void 0)&&(a[u]=Nr(r,o,u,void 0,e,!0)):delete a[u]);if(i!==o)for(const u in i)(!t||!nt(t,u))&&(delete i[u],c=!0)}c&&xn(e.attrs,"set","")}function Rf(e,t,s,n){const[a,i]=e.propsOptions;let l=!1,o;if(t)for(let r in t){if(Tn(r))continue;const c=t[r];let d;a&&nt(a,d=ut(r))?!i||!i.includes(d)?s[d]=c:(o||(o={}))[d]=c:Do(e.emitsOptions,r)||(!(r in n)||c!==n[r])&&(n[r]=c,l=!0)}if(i){const r=Ye(s),c=o||Ke;for(let d=0;d<i.length;d++){const u=i[d];s[u]=Nr(a,r,u,c[u],e,!nt(c,u))}}return l}function Nr(e,t,s,n,a,i){const l=e[s];if(l!=null){const o=nt(l,"default");if(o&&n===void 0){const r=l.default;if(l.type!==Function&&!l.skipFactory&&Me(r)){const{propsDefaults:c}=a;if(s in c)n=c[s];else{const d=pi(a);n=c[s]=r.call(null,t),d()}}else n=r;a.ce&&a.ce._setProp(s,n)}l[0]&&(i&&!o?n=!1:l[1]&&(n===""||n===ws(s))&&(n=!0))}return n}const Bb=new WeakMap;function If(e,t,s=!1){const n=s?Bb:t.propsCache,a=n.get(e);if(a)return a;const i=e.props,l={},o=[];let r=!1;if(!Me(e)){const d=u=>{r=!0;const[p,h]=If(u,t,!0);qe(l,p),h&&o.push(...h)};!s&&t.mixins.length&&t.mixins.forEach(d),e.extends&&d(e.extends),e.mixins&&e.mixins.forEach(d)}if(!i&&!r)return et(e)&&n.set(e,za),za;if(Ce(i))for(let d=0;d<i.length;d++){const u=ut(i[d]);Pd(u)&&(l[u]=Ke)}else if(i)for(const d in i){const u=ut(d);if(Pd(u)){const p=i[d],h=l[u]=Ce(p)||Me(p)?{type:p}:qe({},p),m=h.type;let v=!1,k=!0;if(Ce(m))for(let R=0;R<m.length;++R){const y=m[R],g=Me(y)&&y.name;if(g==="Boolean"){v=!0;break}else g==="String"&&(k=!1)}else v=Me(m)&&m.name==="Boolean";h[0]=v,h[1]=k,(v||nt(h,"default"))&&o.push(u)}}const c=[l,o];return et(e)&&n.set(e,c),c}function Pd(e){return e[0]!=="$"&&!Tn(e)}const Sc=e=>e==="_"||e==="_ctx"||e==="$stable",Tc=e=>Ce(e)?e.map(_s):[_s(e)],Ub=(e,t,s)=>{if(t._n)return t;const n=mc((...a)=>Tc(t(...a)),s);return n._c=!1,n},Of=(e,t,s)=>{const n=e._ctx;for(const a in e){if(Sc(a))continue;const i=e[a];if(Me(i))t[a]=Ub(a,i,n);else if(i!=null){const l=Tc(i);t[a]=()=>l}}},Lf=(e,t)=>{const s=Tc(t);e.slots.default=()=>s},Nf=(e,t,s)=>{for(const n in t)(s||!Sc(n))&&(e[n]=t[n])},Hb=(e,t,s)=>{const n=e.slots=Ef();if(e.vnode.shapeFlag&32){const a=t._;a?(Nf(n,t,s),s&&Sp(n,"_",a,!0)):Of(t,n)}else t&&Lf(e,t)},zb=(e,t,s)=>{const{vnode:n,slots:a}=e;let i=!0,l=Ke;if(n.shapeFlag&32){const o=t._;o?s&&o===1?i=!1:Nf(a,t,s):(i=!t.$stable,Of(t,a)),l=t}else t&&(Lf(e,t),l={default:1});if(i)for(const o in a)!Sc(o)&&l[o]==null&&delete a[o]},Rt=Hf;function Df(e){return Mf(e)}function Pf(e){return Mf(e,Hg)}function Mf(e,t){const s=ko();s.__VUE__=!0;const{insert:n,remove:a,patchProp:i,createElement:l,createText:o,createComment:r,setText:c,setElementText:d,parentNode:u,nextSibling:p,setScopeId:h=Wt,insertStaticContent:m}=e,v=(_,P,H,ie=null,se=null,ae=null,me=void 0,pe=null,ce=!!P.dynamicChildren)=>{if(_===P)return;_&&!Vs(_,P)&&(ie=Q(_),X(_,se,ae,!0),_=null),P.patchFlag===-2&&(ce=!1,P.dynamicChildren=null);const{type:le,ref:_e,shapeFlag:ge}=P;switch(le){case Wn:k(_,P,H,ie);break;case Ct:R(_,P,H,ie);break;case ha:_==null&&y(P,H,ie,me);break;case Ht:C(_,P,H,ie,se,ae,me,pe,ce);break;default:ge&1?S(_,P,H,ie,se,ae,me,pe,ce):ge&6?M(_,P,H,ie,se,ae,me,pe,ce):(ge&64||ge&128)&&le.process(_,P,H,ie,se,ae,me,pe,ce,re)}_e!=null&&se?Ka(_e,_&&_.ref,ae,P||_,!P):_e==null&&_&&_.ref!=null&&Ka(_.ref,null,ae,_,!0)},k=(_,P,H,ie)=>{if(_==null)n(P.el=o(P.children),H,ie);else{const se=P.el=_.el;P.children!==_.children&&c(se,P.children)}},R=(_,P,H,ie)=>{_==null?n(P.el=r(P.children||""),H,ie):P.el=_.el},y=(_,P,H,ie)=>{[_.el,_.anchor]=m(_.children,P,H,ie,_.el,_.anchor)},g=({el:_,anchor:P},H,ie)=>{let se;for(;_&&_!==P;)se=p(_),n(_,H,ie),_=se;n(P,H,ie)},b=({el:_,anchor:P})=>{let H;for(;_&&_!==P;)H=p(_),a(_),_=H;a(P)},S=(_,P,H,ie,se,ae,me,pe,ce)=>{if(P.type==="svg"?me="svg":P.type==="math"&&(me="mathml"),_==null)w(P,H,ie,se,ae,me,pe,ce);else{const le=_.el&&_.el._isVueCE?_.el:null;try{le&&le._beginPatch(),x(_,P,se,ae,me,pe,ce)}finally{le&&le._endPatch()}}},w=(_,P,H,ie,se,ae,me,pe)=>{let ce,le;const{props:_e,shapeFlag:ge,transition:we,dirs:Re}=_;if(ce=_.el=l(_.type,ae,_e&&_e.is,_e),ge&8?d(ce,_.children):ge&16&&E(_.children,ce,null,ie,se,tr(_,ae),me,pe),Re&&sn(_,null,ie,"created"),A(ce,_,_.scopeId,me,ie),_e){for(const he in _e)he!=="value"&&!Tn(he)&&i(ce,he,null,_e[he],ae,ie);"value"in _e&&i(ce,"value",null,_e.value,ae),(le=_e.onVnodeBeforeMount)&&ys(le,ie,_)}Re&&sn(_,null,ie,"beforeMount");const F=Ff(se,we);F&&we.beforeEnter(ce),n(ce,P,H),((le=_e&&_e.onVnodeMounted)||F||Re)&&Rt(()=>{try{le&&ys(le,ie,_),F&&we.enter(ce),Re&&sn(_,null,ie,"mounted")}finally{}},se)},A=(_,P,H,ie,se)=>{if(H&&h(_,H),ie)for(let ae=0;ae<ie.length;ae++)h(_,ie[ae]);if(se){let ae=se.subTree;if(P===ae||Jl(ae.type)&&(ae.ssContent===P||ae.ssFallback===P)){const me=se.vnode;A(_,me,me.scopeId,me.slotScopeIds,se.parent)}}},E=(_,P,H,ie,se,ae,me,pe,ce=0)=>{for(let le=ce;le<_.length;le++){const _e=_[le]=pe?bn(_[le]):_s(_[le]);v(null,_e,P,H,ie,se,ae,me,pe)}},x=(_,P,H,ie,se,ae,me)=>{const pe=P.el=_.el;let{patchFlag:ce,dynamicChildren:le,dirs:_e}=P;ce|=_.patchFlag&16;const ge=_.props||Ke,we=P.props||Ke;let Re;if(H&&na(H,!1),(Re=we.onVnodeBeforeUpdate)&&ys(Re,H,P,_),_e&&sn(P,_,H,"beforeUpdate"),H&&na(H,!0),(ge.innerHTML&&we.innerHTML==null||ge.textContent&&we.textContent==null)&&d(pe,""),le?N(_.dynamicChildren,le,pe,H,ie,tr(P,se),ae):me||O(_,P,pe,null,H,ie,tr(P,se),ae,!1),ce>0){if(ce&16)$(pe,ge,we,H,se);else if(ce&2&&ge.class!==we.class&&i(pe,"class",null,we.class,se),ce&4&&i(pe,"style",ge.style,we.style,se),ce&8){const F=P.dynamicProps;for(let he=0;he<F.length;he++){const Se=F[he],Oe=ge[Se],Fe=we[Se];(Fe!==Oe||Se==="value")&&i(pe,Se,Oe,Fe,se,H)}}ce&1&&_.children!==P.children&&d(pe,P.children)}else!me&&le==null&&$(pe,ge,we,H,se);((Re=we.onVnodeUpdated)||_e)&&Rt(()=>{Re&&ys(Re,H,P,_),_e&&sn(P,_,H,"updated")},ie)},N=(_,P,H,ie,se,ae,me)=>{for(let pe=0;pe<P.length;pe++){const ce=_[pe],le=P[pe],_e=ce.el&&(ce.type===Ht||!Vs(ce,le)||ce.shapeFlag&198)?u(ce.el):H;v(ce,le,_e,null,ie,se,ae,me,!0)}},$=(_,P,H,ie,se)=>{if(P!==H){if(P!==Ke)for(const ae in P)!Tn(ae)&&!(ae in H)&&i(_,ae,P[ae],null,se,ie);for(const ae in H){if(Tn(ae))continue;const me=H[ae],pe=P[ae];me!==pe&&ae!=="value"&&i(_,ae,pe,me,se,ie)}"value"in H&&i(_,"value",P.value,H.value,se)}},C=(_,P,H,ie,se,ae,me,pe,ce)=>{const le=P.el=_?_.el:o(""),_e=P.anchor=_?_.anchor:o("");let{patchFlag:ge,dynamicChildren:we,slotScopeIds:Re}=P;Re&&(pe=pe?pe.concat(Re):Re),_==null?(n(le,H,ie),n(_e,H,ie),E(P.children||[],H,_e,se,ae,me,pe,ce)):ge>0&&ge&64&&we&&_.dynamicChildren&&_.dynamicChildren.length===we.length?(N(_.dynamicChildren,we,H,se,ae,me,pe),(P.key!=null||se&&P===se.subTree)&&Cc(_,P,!0)):O(_,P,H,_e,se,ae,me,pe,ce)},M=(_,P,H,ie,se,ae,me,pe,ce)=>{P.slotScopeIds=pe,_==null?P.shapeFlag&512?se.ctx.activate(P,H,ie,me,ce):V(P,H,ie,se,ae,me,ce):q(_,P,ce)},V=(_,P,H,ie,se,ae,me)=>{const pe=_.component=Wf(_,ie,se);if(dl(_)&&(pe.ctx.renderer=re),Jf(pe,!1,me),pe.asyncDep){if(se&&se.registerDep(pe,D,me),!_.el){const ce=pe.subTree=xt(Ct);R(null,ce,P,H),_.placeholder=ce.el}}else D(pe,_,P,H,se,ae,me)},q=(_,P,H)=>{const ie=P.component=_.component;if(Mb(_,P,H))if(ie.asyncDep&&!ie.asyncResolved){I(ie,P,H);return}else ie.next=P,ie.update();else P.el=_.el,ie.vnode=P},D=(_,P,H,ie,se,ae,me)=>{const pe=()=>{if(_.isMounted){let{next:ge,bu:we,u:Re,parent:F,vnode:he}=_;{const ct=$f(_);if(ct){ge&&(ge.el=he.el,I(_,ge,me)),ct.asyncDep.then(()=>{Rt(()=>{_.isUnmounted||le()},se)});return}}let Se=ge,Oe;na(_,!1),ge?(ge.el=he.el,I(_,ge,me)):ge=he,we&&qa(we),(Oe=ge.props&&ge.props.onVnodeBeforeUpdate)&&ys(Oe,F,ge,he),na(_,!0);const Fe=Dl(_),ft=_.subTree;_.subTree=Fe,v(ft,Fe,u(ft.el),Q(ft),_,se,ae),ge.el=Fe.el,Se===null&&Po(_,Fe.el),Re&&Rt(Re,se),(Oe=ge.props&&ge.props.onVnodeUpdated)&&Rt(()=>ys(Oe,F,ge,he),se)}else{let ge;const{el:we,props:Re}=P,{bm:F,m:he,parent:Se,root:Oe,type:Fe}=_,ft=En(P);if(na(_,!1),F&&qa(F),!ft&&(ge=Re&&Re.onVnodeBeforeMount)&&ys(ge,Se,P),na(_,!0),we&&Ie){const ct=()=>{_.subTree=Dl(_),Ie(we,_.subTree,_,se,null)};ft&&Fe.__asyncHydrate?Fe.__asyncHydrate(we,_,ct):ct()}else{Oe.ce&&Oe.ce._hasShadowRoot()&&Oe.ce._injectChildStyle(Fe,_.parent?_.parent.type:void 0);const ct=_.subTree=Dl(_);v(null,ct,H,ie,_,se,ae),P.el=ct.el}if(he&&Rt(he,se),!ft&&(ge=Re&&Re.onVnodeMounted)){const ct=P;Rt(()=>ys(ge,Se,ct),se)}(P.shapeFlag&256||Se&&En(Se.vnode)&&Se.vnode.shapeFlag&256)&&_.a&&Rt(_.a,se),_.isMounted=!0,P=H=ie=null}};_.scope.on();const ce=_.effect=new Hi(pe);_.scope.off();const le=_.update=ce.run.bind(ce),_e=_.job=ce.runIfDirty.bind(ce);_e.i=_,_e.id=_.uid,ce.scheduler=()=>hc(_e),na(_,!0),le()},I=(_,P,H)=>{P.component=_;const ie=_.vnode.props;_.vnode=P,_.next=null,$b(_,P.props,ie,H),zb(_,P.children,H),In(),wd(_),On()},O=(_,P,H,ie,se,ae,me,pe,ce=!1)=>{const le=_&&_.children,_e=_?_.shapeFlag:0,ge=P.children,{patchFlag:we,shapeFlag:Re}=P;if(we>0){if(we&128){W(le,ge,H,ie,se,ae,me,pe,ce);return}else if(we&256){U(le,ge,H,ie,se,ae,me,pe,ce);return}}Re&8?(_e&16&&De(le,se,ae),ge!==le&&d(H,ge)):_e&16?Re&16?W(le,ge,H,ie,se,ae,me,pe,ce):De(le,se,ae,!0):(_e&8&&d(H,""),Re&16&&E(ge,H,ie,se,ae,me,pe,ce))},U=(_,P,H,ie,se,ae,me,pe,ce)=>{_=_||za,P=P||za;const le=_.length,_e=P.length,ge=Math.min(le,_e);let we;for(we=0;we<ge;we++){const Re=P[we]=ce?bn(P[we]):_s(P[we]);v(_[we],Re,H,null,se,ae,me,pe,ce)}le>_e?De(_,se,ae,!0,!1,ge):E(P,H,ie,se,ae,me,pe,ce,ge)},W=(_,P,H,ie,se,ae,me,pe,ce)=>{let le=0;const _e=P.length;let ge=_.length-1,we=_e-1;for(;le<=ge&&le<=we;){const Re=_[le],F=P[le]=ce?bn(P[le]):_s(P[le]);if(Vs(Re,F))v(Re,F,H,null,se,ae,me,pe,ce);else break;le++}for(;le<=ge&&le<=we;){const Re=_[ge],F=P[we]=ce?bn(P[we]):_s(P[we]);if(Vs(Re,F))v(Re,F,H,null,se,ae,me,pe,ce);else break;ge--,we--}if(le>ge){if(le<=we){const Re=we+1,F=Re<_e?P[Re].el:ie;for(;le<=we;)v(null,P[le]=ce?bn(P[le]):_s(P[le]),H,F,se,ae,me,pe,ce),le++}}else if(le>we)for(;le<=ge;)X(_[le],se,ae,!0),le++;else{const Re=le,F=le,he=new Map;for(le=F;le<=we;le++){const tt=P[le]=ce?bn(P[le]):_s(P[le]);tt.key!=null&&he.set(tt.key,le)}let Se,Oe=0;const Fe=we-F+1;let ft=!1,ct=0;const Et=new Array(Fe);for(le=0;le<Fe;le++)Et[le]=0;for(le=Re;le<=ge;le++){const tt=_[le];if(Oe>=Fe){X(tt,se,ae,!0);continue}let Ze;if(tt.key!=null)Ze=he.get(tt.key);else for(Se=F;Se<=we;Se++)if(Et[Se-F]===0&&Vs(tt,P[Se])){Ze=Se;break}Ze===void 0?X(tt,se,ae,!0):(Et[Ze-F]=le+1,Ze>=ct?ct=Ze:ft=!0,v(tt,P[Ze],H,null,se,ae,me,pe,ce),Oe++)}const At=ft?jb(Et):za;for(Se=At.length-1,le=Fe-1;le>=0;le--){const tt=F+le,Ze=P[tt],ne=P[tt+1],Te=tt+1<_e?ne.el||Bf(ne):ie;Et[le]===0?v(null,Ze,H,Te,se,ae,me,pe,ce):ft&&(Se<0||le!==At[Se]?K(Ze,H,Te,2):Se--)}}},K=(_,P,H,ie,se=null)=>{const{el:ae,type:me,transition:pe,children:ce,shapeFlag:le}=_;if(le&6){K(_.component.subTree,P,H,ie);return}if(le&128){_.suspense.move(P,H,ie);return}if(le&64){me.move(_,P,H,re);return}if(me===Ht){n(ae,P,H);for(let ge=0;ge<ce.length;ge++)K(ce[ge],P,H,ie);n(_.anchor,P,H);return}if(me===ha){g(_,P,H);return}if(ie!==2&&le&1&&pe)if(ie===0)pe.persisted&&!ae[Ps]?n(ae,P,H):(pe.beforeEnter(ae),n(ae,P,H),Rt(()=>pe.enter(ae),se));else{const{leave:ge,delayLeave:we,afterLeave:Re}=pe,F=()=>{_.ctx.isUnmounted?a(ae):n(ae,P,H)},he=()=>{const Se=ae._isLeaving||!!ae[Ps];ae._isLeaving&&ae[Ps](!0),pe.persisted&&!Se?F():ge(ae,()=>{F(),Re&&Re()})};we?we(ae,F,he):he()}else n(ae,P,H)},X=(_,P,H,ie=!1,se=!1)=>{const{type:ae,props:me,ref:pe,children:ce,dynamicChildren:le,shapeFlag:_e,patchFlag:ge,dirs:we,cacheIndex:Re,memo:F}=_;if(ge===-2&&(se=!1),pe!=null&&(In(),Ka(pe,null,H,_,!0),On()),Re!=null&&(P.renderCache[Re]=void 0),_e&256){P.ctx.deactivate(_);return}const he=_e&1&&we,Se=!En(_);let Oe;if(Se&&(Oe=me&&me.onVnodeBeforeUnmount)&&ys(Oe,P,_),_e&6)de(_.component,H,ie);else{if(_e&128){_.suspense.unmount(H,ie);return}he&&sn(_,null,P,"beforeUnmount"),_e&64?_.type.remove(_,P,H,re,ie):le&&!le.hasOnce&&(ae!==Ht||ge>0&&ge&64)?De(le,P,H,!1,!0):(ae===Ht&&ge&384||!se&&_e&16)&&De(ce,P,H),ie&&oe(_)}const Fe=F!=null&&Re==null;(Se&&(Oe=me&&me.onVnodeUnmounted)||he||Fe)&&Rt(()=>{Oe&&ys(Oe,P,_),he&&sn(_,null,P,"unmounted"),Fe&&(_.el=null)},H)},oe=_=>{const{type:P,el:H,anchor:ie,transition:se}=_;if(P===Ht){ee(H,ie);return}if(P===ha){b(_);return}const ae=()=>{a(H),se&&!se.persisted&&se.afterLeave&&se.afterLeave()};if(_.shapeFlag&1&&se&&!se.persisted){const{leave:me,delayLeave:pe}=se,ce=()=>me(H,ae);pe?pe(_.el,ae,ce):ce()}else ae()},ee=(_,P)=>{let H;for(;_!==P;)H=p(_),a(_),_=H;a(P)},de=(_,P,H)=>{const{bum:ie,scope:se,job:ae,subTree:me,um:pe,m:ce,a:le}=_;Zl(ce),Zl(le),ie&&qa(ie),se.stop(),ae&&(ae.flags|=8,X(me,_,P,H)),pe&&Rt(pe,P),Rt(()=>{_.isUnmounted=!0},P)},De=(_,P,H,ie=!1,se=!1,ae=0)=>{for(let me=ae;me<_.length;me++)X(_[me],P,H,ie,se)},Q=_=>{if(_.shapeFlag&6)return Q(_.component.subTree);if(_.shapeFlag&128)return _.suspense.next();const P=p(_.anchor||_.el),H=P&&P[of];return H?p(H):P};let be=!1;const z=(_,P,H)=>{let ie;_==null?P._vnode&&(X(P._vnode,null,null,!0),ie=P._vnode.component):v(P._vnode||null,_,P,null,null,null,H),P._vnode=_,be||(be=!0,wd(ie),Gl(),be=!1)},re={p:v,um:X,m:K,r:oe,mt:V,mc:E,pc:O,pbc:N,n:Q,o:e};let ue,Ie;return t&&([ue,Ie]=t(re)),{render:z,hydrate:ue,createApp:Rb(z,ue)}}function tr({type:e,props:t},s){return s==="svg"&&e==="foreignObject"||s==="mathml"&&e==="annotation-xml"&&t&&t.encoding&&t.encoding.includes("html")?void 0:s}function na({effect:e,job:t},s){s?(e.flags|=32,t.flags|=4):(e.flags&=-33,t.flags&=-5)}function Ff(e,t){return(!e||e&&!e.pendingBranch)&&t&&!t.persisted}function Cc(e,t,s=!1){const n=e.children,a=t.children;if(Ce(n)&&Ce(a))for(let i=0;i<n.length;i++){const l=n[i];let o=a[i];o.shapeFlag&1&&!o.dynamicChildren&&((o.patchFlag<=0||o.patchFlag===32)&&(o=a[i]=bn(a[i]),o.el=l.el),!s&&o.patchFlag!==-2&&Cc(l,o)),o.type===Wn&&(o.patchFlag===-1&&(o=a[i]=bn(o)),o.el=l.el),o.type===Ct&&!o.el&&(o.el=l.el)}}function jb(e){const t=e.slice(),s=[0];let n,a,i,l,o;const r=e.length;for(n=0;n<r;n++){const c=e[n];if(c!==0){if(a=s[s.length-1],e[a]<c){t[n]=a,s.push(n);continue}for(i=0,l=s.length-1;i<l;)o=i+l>>1,e[s[o]]<c?i=o+1:l=o;c<e[s[i]]&&(i>0&&(t[n]=s[i-1]),s[i]=n)}}for(i=s.length,l=s[i-1];i-- >0;)s[i]=l,l=t[l];return s}function $f(e){const t=e.subTree.component;if(t)return t.asyncDep&&!t.asyncResolved?t:$f(t)}function Zl(e){if(e)for(let t=0;t<e.length;t++)e[t].flags|=8}function Bf(e){if(e.placeholder)return e.placeholder;const t=e.component;return t?Bf(t.subTree):null}const Jl=e=>e.__isSuspense;let Dr=0;const Vb={name:"Suspense",__isSuspense:!0,process(e,t,s,n,a,i,l,o,r,c){if(e==null)Gb(t,s,n,a,i,l,o,r,c);else{if(i&&i.deps>0&&!e.suspense.isInFallback){t.suspense=e.suspense,t.suspense.vnode=t,t.el=e.el;return}Kb(e,t,s,n,a,l,o,r,c)}},hydrate:Wb,normalize:Zb},qb=Vb;function Wi(e,t){const s=e.props&&e.props[t];Me(s)&&s()}function Gb(e,t,s,n,a,i,l,o,r){const{p:c,o:{createElement:d}}=r,u=d("div"),p=e.suspense=Uf(e,a,n,t,u,s,i,l,o,r);c(null,p.pendingBranch=e.ssContent,u,null,n,p,i,l),p.deps>0?(Wi(e,"onPending"),Wi(e,"onFallback"),c(null,e.ssFallback,t,s,n,null,i,l),Wa(p,e.ssFallback)):p.resolve(!1,!0)}function Kb(e,t,s,n,a,i,l,o,{p:r,um:c,o:{createElement:d}}){const u=t.suspense=e.suspense;u.vnode=t,t.el=e.el;const p=t.ssContent,h=t.ssFallback,{activeBranch:m,pendingBranch:v,isInFallback:k,isHydrating:R}=u;if(v)u.pendingBranch=p,Vs(v,p)?(r(v,p,u.hiddenContainer,null,a,u,i,l,o),u.deps<=0?u.resolve():k&&(R||(r(m,h,s,n,a,null,i,l,o),Wa(u,h)))):(u.pendingId=Dr++,R?(u.isHydrating=!1,u.activeBranch=v):c(v,a,u),u.deps=0,u.effects.length=0,u.hiddenContainer=d("div"),k?(r(null,p,u.hiddenContainer,null,a,u,i,l,o),u.deps<=0?u.resolve():(r(m,h,s,n,a,null,i,l,o),Wa(u,h))):m&&Vs(m,p)?(r(m,p,s,n,a,u,i,l,o),u.resolve(!0)):(r(null,p,u.hiddenContainer,null,a,u,i,l,o),u.deps<=0&&u.resolve()));else if(m&&Vs(m,p))r(m,p,s,n,a,u,i,l,o),Wa(u,p);else if(Wi(t,"onPending"),u.pendingBranch=p,p.shapeFlag&512?u.pendingId=p.component.suspenseId:u.pendingId=Dr++,r(null,p,u.hiddenContainer,null,a,u,i,l,o),u.deps<=0)u.resolve();else{const{timeout:y,pendingId:g}=u;y>0?setTimeout(()=>{u.pendingId===g&&u.fallback(h)},y):y===0&&u.fallback(h)}}function Uf(e,t,s,n,a,i,l,o,r,c,d=!1){const{p:u,m:p,um:h,n:m,o:{parentNode:v,remove:k}}=c;let R;const y=Jb(e);y&&t&&t.pendingBranch&&(R=t.pendingId,t.deps++);const g=e.props?Hl(e.props.timeout):void 0,b=i,S={vnode:e,parent:t,parentComponent:s,namespace:l,container:n,hiddenContainer:a,deps:0,pendingId:Dr++,timeout:typeof g=="number"?g:-1,activeBranch:null,isFallbackMountPending:!1,pendingBranch:null,isInFallback:!d,isHydrating:d,isUnmounted:!1,effects:[],resolve(w=!1,A=!1){const{vnode:E,activeBranch:x,pendingBranch:N,pendingId:$,effects:C,parentComponent:M,container:V,isInFallback:q}=S;let D=!1;if(S.isHydrating)S.isHydrating=!1;else if(!w){D=x&&N.transition&&N.transition.mode==="out-in";let U=!1;D&&(x.transition.afterLeave=()=>{$===S.pendingId&&(p(N,V,i===b&&!U?m(x):i,0),Vi(C),q&&E.ssFallback&&(E.ssFallback.el=null))}),x&&!S.isFallbackMountPending&&(v(x.el)===V&&(i=m(x),U=!0),h(x,M,S,!0),!D&&q&&E.ssFallback&&Rt(()=>E.ssFallback.el=null,S)),D||p(N,V,i,0)}S.isFallbackMountPending=!1,Wa(S,N),S.pendingBranch=null,S.isInFallback=!1;let I=S.parent,O=!1;for(;I;){if(I.pendingBranch){I.effects.push(...C),O=!0;break}I=I.parent}!O&&!D&&Vi(C),S.effects=[],y&&t&&t.pendingBranch&&R===t.pendingId&&(t.deps--,t.deps===0&&!A&&t.resolve()),Wi(E,"onResolve")},fallback(w){if(!S.pendingBranch)return;const{vnode:A,activeBranch:E,parentComponent:x,container:N,namespace:$}=S;Wi(A,"onFallback");const C=m(E),M=()=>{S.isFallbackMountPending=!1,S.isInFallback&&(u(null,w,N,C,x,null,$,o,r),Wa(S,w))},V=w.transition&&w.transition.mode==="out-in";V&&(S.isFallbackMountPending=!0,E.transition.afterLeave=M),S.isInFallback=!0,h(E,x,null,!0),V||M()},move(w,A,E){S.activeBranch&&p(S.activeBranch,w,A,E),S.container=w},next(){return S.activeBranch&&m(S.activeBranch)},registerDep(w,A,E){const x=!!S.pendingBranch;x&&S.deps++;const N=w.vnode.el;w.asyncDep.catch($=>{Sa($,w,0)}).then($=>{if(w.isUnmounted||S.isUnmounted||S.pendingId!==w.suspenseId)return;Yi(),w.asyncResolved=!0;const{vnode:C}=w;Pr(w,$,!1),N&&(C.el=N);const M=!N&&w.subTree.el;A(w,C,v(N||w.subTree.el),N?null:m(w.subTree),S,l,E),M&&(C.placeholder=null,k(M)),Po(w,C.el),x&&--S.deps===0&&S.resolve()})},unmount(w,A){S.isUnmounted=!0,S.activeBranch&&h(S.activeBranch,s,w,A),S.pendingBranch&&h(S.pendingBranch,s,w,A)}};return S}function Wb(e,t,s,n,a,i,l,o,r){const c=t.suspense=Uf(t,n,s,e.parentNode,document.createElement("div"),null,a,i,l,o,!0),d=r(e,c.pendingBranch=t.ssContent,s,c,i,l);return c.deps===0&&c.resolve(!1,!0),d}function Zb(e){const{shapeFlag:t,children:s}=e,n=t&32;e.ssContent=Md(n?s.default:s),e.ssFallback=n?Md(s.fallback):xt(Ct)}function Md(e){let t;if(Me(e)){const s=ga&&e._c;s&&(e._d=!1,Zi()),e=e(),s&&(e._d=!0,t=ss,zf())}return Ce(e)&&(e=Nb(e)),e=_s(e),t&&!e.dynamicChildren&&(e.dynamicChildren=t.filter(s=>s!==e)),e}function Hf(e,t){t&&t.pendingBranch?Ce(e)?t.effects.push(...e):t.effects.push(e):Vi(e)}function Wa(e,t){e.activeBranch=t;const{vnode:s,parentComponent:n}=e;let a=t.el;for(;!a&&t.component;)t=t.component.subTree,a=t.el;s.el=a,n&&n.subTree===s&&(n.vnode.el=a,Po(n,a))}function Jb(e){const t=e.props&&e.props.suspensible;return t!=null&&t!==!1}const Ht=Symbol.for("v-fgt"),Wn=Symbol.for("v-txt"),Ct=Symbol.for("v-cmt"),ha=Symbol.for("v-stc"),Di=[];let ss=null;function Zi(e=!1){Di.push(ss=e?null:[])}function zf(){Di.pop(),ss=Di[Di.length-1]||null}let ga=1;function Ji(e,t=!1){ga+=e,e<0&&ss&&t&&(ss.hasOnce=!0)}function jf(e){return e.dynamicChildren=ga>0?ss||za:null,zf(),ga>0&&ss&&ss.push(e),e}function Yb(e,t,s,n,a,i){return jf(Ec(e,t,s,n,a,i,!0))}function Yl(e,t,s,n,a){return jf(xt(e,t,s,n,a,!0))}function Nn(e){return e?e.__v_isVNode===!0:!1}function Vs(e,t){return e.type===t.type&&e.key===t.key}function Qb(e){}const Vf=({key:e})=>e??null,Pl=({ref:e,ref_key:t,ref_for:s})=>(typeof e=="number"&&(e=""+e),e!=null?Be(e)||Lt(e)||Me(e)?{i:Kt,r:e,k:t,f:!!s}:e:null);function Ec(e,t=null,s=null,n=0,a=null,i=e===Ht?0:1,l=!1,o=!1){const r={__v_isVNode:!0,__v_skip:!0,type:e,props:t,key:t&&Vf(t),ref:t&&Pl(t),scopeId:Ro,slotScopeIds:null,children:s,component:null,suspense:null,ssContent:null,ssFallback:null,dirs:null,transition:null,el:null,anchor:null,target:null,targetStart:null,targetAnchor:null,staticCount:0,shapeFlag:i,patchFlag:n,dynamicProps:a,dynamicChildren:null,appContext:null,ctx:Kt};return o?(Rc(r,s),i&128&&e.normalize(r)):s&&(r.shapeFlag|=Be(s)?8:16),ga>0&&!l&&ss&&(r.patchFlag>0||i&6)&&r.patchFlag!==32&&ss.push(r),r}const xt=Xb;function Xb(e,t=null,s=null,n=0,a=null,i=!1){if((!e||e===yf)&&(e=Ct),Nn(e)){const o=on(e,t,!0);return s&&Rc(o,s),ga>0&&!i&&ss&&(o.shapeFlag&6?ss[ss.indexOf(e)]=o:ss.push(o)),o.patchFlag=-2,o}if(ly(e)&&(e=e.__vccOpts),t){t=qf(t);let{class:o,style:r}=t;o&&!Be(o)&&(t.class=ll(o)),et(r)&&(ol(r)&&!Ce(r)&&(r=qe({},r)),t.style=il(r))}const l=Be(e)?1:Jl(e)?128:rf(e)?64:et(e)?4:Me(e)?2:0;return Ec(e,t,s,n,a,l,i,!0)}function qf(e){return e?ol(e)||Af(e)?qe({},e):e:null}function on(e,t,s=!1,n=!1){const{props:a,ref:i,patchFlag:l,children:o,transition:r}=e,c=t?Kf(a||{},t):a,d={__v_isVNode:!0,__v_skip:!0,type:e.type,props:c,key:c&&Vf(c),ref:t&&t.ref?s&&i?Ce(i)?i.concat(Pl(t)):[i,Pl(t)]:Pl(t):i,scopeId:e.scopeId,slotScopeIds:e.slotScopeIds,children:o,target:e.target,targetStart:e.targetStart,targetAnchor:e.targetAnchor,staticCount:e.staticCount,shapeFlag:e.shapeFlag,patchFlag:t&&e.type!==Ht?l===-1?16:l|16:l,dynamicProps:e.dynamicProps,dynamicChildren:e.dynamicChildren,appContext:e.appContext,dirs:e.dirs,transition:r,component:e.component,suspense:e.suspense,ssContent:e.ssContent&&on(e.ssContent),ssFallback:e.ssFallback&&on(e.ssFallback),placeholder:e.placeholder,el:e.el,anchor:e.anchor,ctx:e.ctx,ce:e.ce};return r&&n&&Ln(d,r.clone(d)),d}function Ac(e=" ",t=0){return xt(Wn,null,e,t)}function ey(e,t){const s=xt(ha,null,e);return s.staticCount=t,s}function Gf(e="",t=!1){return t?(Zi(),Yl(Ct,null,e)):xt(Ct,null,e)}function _s(e){return e==null||typeof e=="boolean"?xt(Ct):Ce(e)?xt(Ht,null,e.slice()):Nn(e)?bn(e):xt(Wn,null,String(e))}function bn(e){return e.el===null&&e.patchFlag!==-1||e.memo?e:on(e)}function Rc(e,t){let s=0;const{shapeFlag:n}=e;if(t==null)t=null;else if(Ce(t))s=16;else if(typeof t=="object")if(n&65){const a=t.default;a&&(a._c&&(a._d=!1),Rc(e,a()),a._c&&(a._d=!0));return}else{s=32;const a=t._;!a&&!Af(t)?t._ctx=Kt:a===3&&Kt&&(Kt.slots._===1?t._=1:(t._=2,e.patchFlag|=1024))}else Me(t)?(t={default:t,_ctx:Kt},s=32):(t=String(t),n&64?(s=16,t=[Ac(t)]):s=8);e.children=t,e.shapeFlag|=s}function Kf(...e){const t={};for(let s=0;s<e.length;s++){const n=e[s];for(const a in n)if(a==="class")t.class!==n.class&&(t.class=ll([t.class,n.class]));else if(a==="style")t.style=il([t.style,n.style]);else if(_a(a)){const i=t[a],l=n[a];l&&i!==l&&!(Ce(i)&&i.includes(l))?t[a]=i?[].concat(i,l):l:l==null&&i==null&&!bo(a)&&(t[a]=l)}else a!==""&&(t[a]=n[a])}return t}function ys(e,t,s,n=null){Is(e,t,7,[s,n])}const ty=wf();let sy=0;function Wf(e,t,s){const n=e.type,a=(t?t.appContext:e.appContext)||ty,i={uid:sy++,vnode:e,type:n,parent:t,appContext:a,root:null,next:null,subTree:null,effect:null,update:null,job:null,scope:new oc(!0),render:null,proxy:null,exposed:null,exposeProxy:null,withProxy:null,provides:t?t.provides:Object.create(a.provides),ids:t?t.ids:["",0,0],accessCache:null,renderCache:[],components:null,directives:null,propsOptions:If(n,a),emitsOptions:Sf(n,a),emit:null,emitted:null,propsDefaults:Ke,inheritAttrs:n.inheritAttrs,ctx:Ke,data:Ke,props:Ke,attrs:Ke,slots:Ke,refs:Ke,setupState:Ke,setupContext:null,suspense:s,suspenseId:s?s.pendingId:0,asyncDep:null,asyncResolved:!1,isMounted:!1,isUnmounted:!1,isDeactivated:!1,bc:null,c:null,bm:null,m:null,bu:null,u:null,um:null,bum:null,da:null,a:null,rtg:null,rtc:null,ec:null,sp:null};return i.ctx={_:i},i.root=t?t.root:i,i.emit=Ob.bind(null,i),e.ce&&e.ce(i),i}let Gt=null;const ms=()=>Gt||Kt;let Ql,Za;{const e=ko(),t=(s,n)=>{let a;return(a=e[s])||(a=e[s]=[]),a.push(n),i=>{a.length>1?a.forEach(l=>l(i)):a[0](i)}};Ql=t("__VUE_INSTANCE_SETTERS__",s=>Gt=s),Za=t("__VUE_SSR_SETTERS__",s=>ba=s)}const pi=e=>{const t=Gt;return Ql(e),e.scope.on(),()=>{e.scope.off(),Ql(t)}},Yi=()=>{Gt&&Gt.scope.off(),Ql(null)};function Zf(e){return e.vnode.shapeFlag&4}let ba=!1;function Jf(e,t=!1,s=!1){t&&Za(t);const{props:n,children:a}=e.vnode,i=Zf(e);Fb(e,n,i,t),Hb(e,a,s||t);const l=i?ny(e,t):void 0;return t&&Za(!1),l}function ny(e,t){const s=e.type;e.accessCache=Object.create(null),e.proxy=new Proxy(e.ctx,Ir);const{setup:n}=s;if(n){In();const a=e.setupContext=n.length>1?Xf(e):null,i=pi(e),l=ui(n,e,0,[e.props,a]),o=lc(l);if(On(),i(),(o||e.sp)&&!En(e)&&bc(e),o){if(l.then(Yi,Yi),t)return l.then(r=>{Pr(e,r,t)}).catch(r=>{Sa(r,e,0)});e.asyncDep=l}else Pr(e,l,t)}else Qf(e,t)}function Pr(e,t,s){Me(t)?e.type.__ssrInlineRender?e.ssrRender=t:e.render=t:et(t)&&(e.setupState=fc(t)),Qf(e,s)}let Xl,Mr;function Yf(e){Xl=e,Mr=t=>{t.render._rc&&(t.withProxy=new Proxy(t.ctx,cb))}}const ay=()=>!Xl;function Qf(e,t,s){const n=e.type;if(!e.render){if(!t&&Xl&&!n.render){const a=n.template||kc(e).template;if(a){const{isCustomElement:i,compilerOptions:l}=e.appContext.config,{delimiters:o,compilerOptions:r}=n,c=qe(qe({isCustomElement:i,delimiters:o},l),r);n.render=Xl(a,c)}}e.render=n.render||Wt,Mr&&Mr(e)}{const a=pi(e);In();try{kb(e)}finally{On(),a()}}}const iy={get(e,t){return ts(e,"get",""),e[t]}};function Xf(e){const t=s=>{e.exposed=s||{}};return{attrs:new Proxy(e.attrs,iy),slots:e.slots,emit:e.emit,expose:t}}function ul(e){return e.exposed?e.exposeProxy||(e.exposeProxy=new Proxy(fc(Gp(e.exposed)),{get(t,s){if(s in t)return t[s];if(s in Ni)return Ni[s](e)},has(t,s){return s in t||s in Ni}})):e.proxy}function Fr(e,t=!0){return Me(e)?e.displayName||e.name:e.name||t&&e.__name}function ly(e){return Me(e)&&"__vccOpts"in e}const J=(e,t)=>pg(e,t,ba);function ei(e,t,s){try{Ji(-1);const n=arguments.length;return n===2?et(t)&&!Ce(t)?Nn(t)?xt(e,null,[t]):xt(e,t):xt(e,null,t):(n>3?s=Array.prototype.slice.call(arguments,2):n===3&&Nn(s)&&(s=[s]),xt(e,t,s))}finally{Ji(1)}}function oy(){}function ry(e,t,s,n){const a=s[n];if(a&&eh(a,e))return a;const i=t();return i.memo=e.slice(),i.cacheIndex=n,s[n]=i}function eh(e,t){const s=e.memo;if(s.length!=t.length)return!1;for(let n=0;n<s.length;n++)if(Ut(s[n],t[n]))return!1;return ga>0&&ss&&ss.push(e),!0}const th="3.5.38",cy=Wt,dy=_g,uy=Fa,py=tf,fy={createComponentInstance:Wf,setupComponent:Jf,renderComponentRoot:Dl,setCurrentRenderingInstance:Gi,isVNode:Nn,normalizeVNode:_s,getComponentPublicInstance:ul,ensureValidVNode:wc,pushWarningContext:gg,popWarningContext:bg},hy=fy,my=null,vy=null,gy=null;/**
* @vue/runtime-dom v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/let $r;const Fd=typeof window<"u"&&window.trustedTypes;if(Fd)try{$r=Fd.createPolicy("vue",{createHTML:e=>e})}catch{}const sh=$r?e=>$r.createHTML(e):e=>e,by="http://www.w3.org/2000/svg",yy="http://www.w3.org/1998/Math/MathML",gn=typeof document<"u"?document:null,$d=gn&&gn.createElement("template"),nh={insert:(e,t,s)=>{t.insertBefore(e,s||null)},remove:e=>{const t=e.parentNode;t&&t.removeChild(e)},createElement:(e,t,s,n)=>{const a=t==="svg"?gn.createElementNS(by,e):t==="mathml"?gn.createElementNS(yy,e):s?gn.createElement(e,{is:s}):gn.createElement(e);return e==="select"&&n&&n.multiple!=null&&a.setAttribute("multiple",n.multiple),a},createText:e=>gn.createTextNode(e),createComment:e=>gn.createComment(e),setText:(e,t)=>{e.nodeValue=t},setElementText:(e,t)=>{e.textContent=t},parentNode:e=>e.parentNode,nextSibling:e=>e.nextSibling,querySelector:e=>gn.querySelector(e),setScopeId(e,t){e.setAttribute(t,"")},insertStaticContent(e,t,s,n,a,i){const l=s?s.previousSibling:t.lastChild;if(a&&(a===i||a.nextSibling))for(;t.insertBefore(a.cloneNode(!0),s),!(a===i||!(a=a.nextSibling)););else{$d.innerHTML=sh(n==="svg"?`<svg>${e}</svg>`:n==="mathml"?`<math>${e}</math>`:e);const o=$d.content;if(n==="svg"||n==="mathml"){const r=o.firstChild;for(;r.firstChild;)o.appendChild(r.firstChild);o.removeChild(r)}t.insertBefore(o,s)}return[l?l.nextSibling:t.firstChild,s?s.previousSibling:t.lastChild]}},Bn="transition",gi="animation",ti=Symbol("_vtc"),ah={name:String,type:String,css:{type:Boolean,default:!0},duration:[String,Number,Object],enterFromClass:String,enterActiveClass:String,enterToClass:String,appearFromClass:String,appearActiveClass:String,appearToClass:String,leaveFromClass:String,leaveActiveClass:String,leaveToClass:String},ih=qe({},gc,ah),xy=e=>(e.displayName="Transition",e.props=ih,e),_y=xy((e,{slots:t})=>ei(uf,lh(e),t)),aa=(e,t=[])=>{Ce(e)?e.forEach(s=>s(...t)):e&&e(...t)},Bd=e=>e?Ce(e)?e.some(t=>t.length>1):e.length>1:!1;function lh(e){const t={};for(const C in e)C in ah||(t[C]=e[C]);if(e.css===!1)return t;const{name:s="v",type:n,duration:a,enterFromClass:i=`${s}-enter-from`,enterActiveClass:l=`${s}-enter-active`,enterToClass:o=`${s}-enter-to`,appearFromClass:r=i,appearActiveClass:c=l,appearToClass:d=o,leaveFromClass:u=`${s}-leave-from`,leaveActiveClass:p=`${s}-leave-active`,leaveToClass:h=`${s}-leave-to`}=e,m=wy(a),v=m&&m[0],k=m&&m[1],{onBeforeEnter:R,onEnter:y,onEnterCancelled:g,onLeave:b,onLeaveCancelled:S,onBeforeAppear:w=R,onAppear:A=y,onAppearCancelled:E=g}=t,x=(C,M,V,q)=>{C._enterCancelled=q,jn(C,M?d:o),jn(C,M?c:l),V&&V()},N=(C,M)=>{C._isLeaving=!1,jn(C,u),jn(C,h),jn(C,p),M&&M()},$=C=>(M,V)=>{const q=C?A:y,D=()=>x(M,C,V);aa(q,[M,D]),Ud(()=>{jn(M,C?r:i),Qs(M,C?d:o),Bd(q)||Hd(M,n,v,D)})};return qe(t,{onBeforeEnter(C){aa(R,[C]),Qs(C,i),Qs(C,l)},onBeforeAppear(C){aa(w,[C]),Qs(C,r),Qs(C,c)},onEnter:$(!1),onAppear:$(!0),onLeave(C,M){C._isLeaving=!0;const V=()=>N(C,M);Qs(C,u),C._enterCancelled?(Qs(C,p),Br(C)):(Br(C),Qs(C,p)),Ud(()=>{C._isLeaving&&(jn(C,u),Qs(C,h),Bd(b)||Hd(C,n,k,V))}),aa(b,[C,V])},onEnterCancelled(C){x(C,!1,void 0,!0),aa(g,[C])},onAppearCancelled(C){x(C,!0,void 0,!0),aa(E,[C])},onLeaveCancelled(C){N(C),aa(S,[C])}})}function wy(e){if(e==null)return null;if(et(e))return[sr(e.enter),sr(e.leave)];{const t=sr(e);return[t,t]}}function sr(e){return Hl(e)}function Qs(e,t){t.split(/\s+/).forEach(s=>s&&e.classList.add(s)),(e[ti]||(e[ti]=new Set)).add(t)}function jn(e,t){t.split(/\s+/).forEach(n=>n&&e.classList.remove(n));const s=e[ti];s&&(s.delete(t),s.size||(e[ti]=void 0))}function Ud(e){requestAnimationFrame(()=>{requestAnimationFrame(e)})}let ky=0;function Hd(e,t,s,n){const a=e._endId=++ky,i=()=>{a===e._endId&&n()};if(s!=null)return setTimeout(i,s);const{type:l,timeout:o,propCount:r}=oh(e,t);if(!l)return n();const c=l+"end";let d=0;const u=()=>{e.removeEventListener(c,p),i()},p=h=>{h.target===e&&++d>=r&&u()};setTimeout(()=>{d<r&&u()},o+1),e.addEventListener(c,p)}function oh(e,t){const s=window.getComputedStyle(e),n=m=>(s[m]||"").split(", "),a=n(`${Bn}Delay`),i=n(`${Bn}Duration`),l=zd(a,i),o=n(`${gi}Delay`),r=n(`${gi}Duration`),c=zd(o,r);let d=null,u=0,p=0;t===Bn?l>0&&(d=Bn,u=l,p=i.length):t===gi?c>0&&(d=gi,u=c,p=r.length):(u=Math.max(l,c),d=u>0?l>c?Bn:gi:null,p=d?d===Bn?i.length:r.length:0);const h=d===Bn&&/\b(?:transform|all)(?:,|$)/.test(n(`${Bn}Property`).toString());return{type:d,timeout:u,propCount:p,hasTransform:h}}function zd(e,t){for(;e.length<t.length;)e=e.concat(e);return Math.max(...t.map((s,n)=>jd(s)+jd(e[n])))}function jd(e){return e==="auto"?0:Number(e.slice(0,-1).replace(",","."))*1e3}function Br(e){return(e?e.ownerDocument:document).body.offsetHeight}function Sy(e,t,s){const n=e[ti];n&&(t=(t?[t,...n]:[...n]).join(" ")),t==null?e.removeAttribute("class"):s?e.setAttribute("class",t):e.className=t}const eo=Symbol("_vod"),Ic=Symbol("_vsh"),rh={name:"show",beforeMount(e,{value:t},{transition:s}){e[eo]=e.style.display==="none"?"":e.style.display,s&&t?s.beforeEnter(e):bi(e,t)},mounted(e,{value:t},{transition:s}){s&&t&&s.enter(e)},updated(e,{value:t,oldValue:s},{transition:n}){!t!=!s&&(n?t?(n.beforeEnter(e),bi(e,!0),n.enter(e)):n.leave(e,()=>{bi(e,!1)}):bi(e,t))},beforeUnmount(e,{value:t}){bi(e,t)}};function bi(e,t){e.style.display=t?e[eo]:"none",e[Ic]=!t}function Ty(){rh.getSSRProps=({value:e})=>{if(!e)return{style:{display:"none"}}}}const ch=Symbol("");function Cy(e){const t=ms();if(!t)return;const s=t.ut=(a=e(t.proxy))=>{Array.from(document.querySelectorAll(`[data-v-owner="${t.uid}"]`)).forEach(i=>to(i,a))},n=()=>{const a=e(t.proxy);t.ce?to(t.ce,a):Ur(t.subTree,a),s(a)};yc(()=>{Vi(n)}),Ge(()=>{ns(n,Wt,{flush:"post"});const a=new MutationObserver(n);a.observe(t.subTree.el.parentNode,{childList:!0}),bt(()=>a.disconnect())})}function Ur(e,t){if(e.shapeFlag&128){const s=e.suspense;e=s.activeBranch,s.pendingBranch&&!s.isHydrating&&s.effects.push(()=>{Ur(s.activeBranch,t)})}for(;e.component;)e=e.component.subTree;if(e.shapeFlag&1&&e.el)to(e.el,t);else if(e.type===Ht)e.children.forEach(s=>Ur(s,t));else if(e.type===ha){let{el:s,anchor:n}=e;for(;s&&(to(s,t),s!==n);)s=s.nextSibling}}function to(e,t){if(e.nodeType===1){const s=e.style;let n="";for(const a in t){const i=Lv(t[a]);s.setProperty(`--${a}`,i),n+=`--${a}: ${i};`}s[ch]=n}}const Ey=/(?:^|;)\s*display\s*:/;function Ay(e,t,s){const n=e.style,a=Be(s);let i=!1;if(s&&!a){if(t)if(Be(t))for(const l of t.split(";")){const o=l.slice(0,l.indexOf(":")).trim();s[o]==null&&Ai(n,o,"")}else for(const l in t)s[l]==null&&Ai(n,l,"");for(const l in s){l==="display"&&(i=!0);const o=s[l];o!=null?Iy(e,l,!Be(t)&&t?t[l]:void 0,o)||Ai(n,l,o):Ai(n,l,"")}}else if(a){if(t!==s){const l=n[ch];l&&(s+=";"+l),n.cssText=s,i=Ey.test(s)}}else t&&e.removeAttribute("style");eo in e&&(e[eo]=i?n.display:"",e[Ic]&&(n.display="none"))}const Vd=/\s*!important$/;function Ai(e,t,s){if(Ce(s))s.forEach(n=>Ai(e,t,n));else if(s==null&&(s=""),t.startsWith("--"))e.setProperty(t,s);else{const n=Ry(e,t);Vd.test(s)?e.setProperty(ws(n),s.replace(Vd,""),"important"):e[n]=s}}const qd=["Webkit","Moz","ms"],nr={};function Ry(e,t){const s=nr[t];if(s)return s;let n=ut(t);if(n!=="filter"&&n in e)return nr[t]=n;n=ka(n);for(let a=0;a<qd.length;a++){const i=qd[a]+n;if(i in e)return nr[t]=i}return t}function Iy(e,t,s,n){return e.tagName==="TEXTAREA"&&(t==="width"||t==="height")&&Be(n)&&s===n}const Gd="http://www.w3.org/1999/xlink";function Kd(e,t,s,n,a,i=Iv(t)){n&&t.startsWith("xlink:")?s==null?e.removeAttributeNS(Gd,t.slice(6,t.length)):e.setAttributeNS(Gd,t,s):s==null||i&&!Cp(s)?e.removeAttribute(t):e.setAttribute(t,i?"":is(s)?String(s):s)}function Wd(e,t,s,n,a){if(t==="innerHTML"||t==="textContent"){s!=null&&(e[t]=t==="innerHTML"?sh(s):s);return}const i=e.tagName;if(t==="value"&&i!=="PROGRESS"&&!i.includes("-")){const o=i==="OPTION"?e.getAttribute("value")||"":e.value,r=s==null?e.type==="checkbox"?"on":"":String(s);(o!==r||!("_value"in e))&&(e.value=r),s==null&&e.removeAttribute(t),e._value=s;return}let l=!1;if(s===""||s==null){const o=typeof e[t];o==="boolean"?s=Cp(s):s==null&&o==="string"?(s="",l=!0):o==="number"&&(s=0,l=!0)}try{e[t]=s}catch{}l&&e.removeAttribute(a||t)}function wn(e,t,s,n){e.addEventListener(t,s,n)}function Oy(e,t,s,n){e.removeEventListener(t,s,n)}const Zd=Symbol("_vei");function Ly(e,t,s,n,a=null){const i=e[Zd]||(e[Zd]={}),l=i[t];if(n&&l)l.value=n;else{const[o,r]=Ny(t);if(n){const c=i[t]=My(n,a);wn(e,o,c,r)}else l&&(Oy(e,o,l,r),i[t]=void 0)}}const Jd=/(?:Once|Passive|Capture)$/;function Ny(e){let t;if(Jd.test(e)){t={};let n;for(;n=e.match(Jd);)e=e.slice(0,e.length-n[0].length),t[n[0].toLowerCase()]=!0}return[e[2]===":"?e.slice(3):ws(e.slice(2)),t]}let ar=0;const Dy=Promise.resolve(),Py=()=>ar||(Dy.then(()=>ar=0),ar=Date.now());function My(e,t){const s=n=>{if(!n._vts)n._vts=Date.now();else if(n._vts<=s.attached)return;const a=s.value;if(Ce(a)){const i=n.stopImmediatePropagation;n.stopImmediatePropagation=()=>{i.call(n),n._stopped=!0};const l=a.slice(),o=[n];for(let r=0;r<l.length&&!n._stopped;r++){const c=l[r];c&&Is(c,t,5,o)}}else Is(a,t,5,[n])};return s.value=e,s.attached=Py(),s}const Yd=e=>e.charCodeAt(0)===111&&e.charCodeAt(1)===110&&e.charCodeAt(2)>96&&e.charCodeAt(2)<123,dh=(e,t,s,n,a,i)=>{const l=a==="svg";t==="class"?Sy(e,n,l):t==="style"?Ay(e,s,n):_a(t)?bo(t)||Ly(e,t,s,n,i):(t[0]==="."?(t=t.slice(1),!0):t[0]==="^"?(t=t.slice(1),!1):Fy(e,t,n,l))?(Wd(e,t,n),!e.tagName.includes("-")&&(t==="value"||t==="checked"||t==="selected")&&Kd(e,t,n,l,i,t!=="value")):e._isVueCE&&($y(e,t)||e._def.__asyncLoader&&(/[A-Z]/.test(t)||!Be(n)))?Wd(e,ut(t),n,i,t):(t==="true-value"?e._trueValue=n:t==="false-value"&&(e._falseValue=n),Kd(e,t,n,l))};function Fy(e,t,s,n){if(n)return!!(t==="innerHTML"||t==="textContent"||t in e&&Yd(t)&&Me(s));if(t==="spellcheck"||t==="draggable"||t==="translate"||t==="autocorrect"||t==="sandbox"&&e.tagName==="IFRAME"||t==="form"||t==="list"&&e.tagName==="INPUT"||t==="type"&&e.tagName==="TEXTAREA")return!1;if(t==="width"||t==="height"){const a=e.tagName;if(a==="IMG"||a==="VIDEO"||a==="CANVAS"||a==="SOURCE")return!1}return Yd(t)&&Be(s)?!1:t in e}function $y(e,t){const s=e._def.props;if(!s)return!1;const n=ut(t);return Array.isArray(s)?s.some(a=>ut(a)===n):Object.keys(s).some(a=>ut(a)===n)}const Qd={};function uh(e,t,s){let n=cl(e,t);yo(n)&&(n=qe({},n,t));class a extends Mo{constructor(l){super(n,l,s)}}return a.def=n,a}const By=((e,t)=>uh(e,t,Sh)),Uy=typeof HTMLElement<"u"?HTMLElement:class{};class Mo extends Uy{constructor(t,s={},n=ao){super(),this._def=t,this._props=s,this._createApp=n,this._isVueCE=!0,this._instance=null,this._app=null,this._nonce=this._def.nonce,this._connected=!1,this._resolved=!1,this._patching=!1,this._dirty=!1,this._numberProps=null,this._styleChildren=new WeakSet,this._styleAnchors=new WeakMap,this._ob=null,this.shadowRoot&&n!==ao?this._root=this.shadowRoot:t.shadowRoot!==!1?(this.attachShadow(qe({},t.shadowRootOptions,{mode:"open"})),this._root=this.shadowRoot):this._root=this}connectedCallback(){if(!this.isConnected)return;!this.shadowRoot&&!this._resolved&&this._parseSlots(),this._connected=!0;let t=this;for(;t=t&&(t.assignedSlot||t.parentNode||t.host);)if(t instanceof Mo){this._parent=t;break}this._instance||(this._resolved?this._mount(this._def):t&&t._pendingResolve?this._pendingResolve=t._pendingResolve.then(()=>{this._pendingResolve=void 0,this._resolveDef()}):this._resolveDef())}_setParent(t=this._parent){t&&(this._instance.parent=t._instance,this._inheritParentContext(t))}_inheritParentContext(t=this._parent){t&&this._app&&Object.setPrototypeOf(this._app._context.provides,t._instance.provides)}disconnectedCallback(){this._connected=!1,It(()=>{this._connected||(this._ob&&(this._ob.disconnect(),this._ob=null),this._app&&this._app.unmount(),this._instance&&(this._instance.ce=void 0),this._app=this._instance=null,this._teleportTargets&&(this._teleportTargets.clear(),this._teleportTargets=void 0))})}_processMutations(t){for(const s of t)this._setAttr(s.attributeName)}_resolveDef(){if(this._pendingResolve)return;for(let n=0;n<this.attributes.length;n++)this._setAttr(this.attributes[n].name);this._ob=new MutationObserver(this._processMutations.bind(this)),this._ob.observe(this,{attributes:!0});const t=(n,a=!1)=>{this._resolved=!0,this._pendingResolve=void 0;const{props:i,styles:l}=n;let o;if(i&&!Ce(i))for(const r in i){const c=i[r];(c===Number||c&&c.type===Number)&&(r in this._props&&(this._props[r]=Hl(this._props[r])),(o||(o=Object.create(null)))[ut(r)]=!0)}this._numberProps=o,this._resolveProps(n),this.shadowRoot&&this._applyStyles(l),this._mount(n)},s=this._def.__asyncLoader;s?this._pendingResolve=s().then(n=>{n.configureApp=this._def.configureApp,t(this._def=n,!0)}):t(this._def)}_mount(t){this._app=this._createApp(t),this._inheritParentContext(),t.configureApp&&t.configureApp(this._app),this._app._ceVNode=this._createVNode(),this._app.mount(this._root);const s=this._instance&&this._instance.exposed;if(s)for(const n in s)nt(this,n)||Object.defineProperty(this,n,{get:()=>an(s[n])})}_resolveProps(t){const{props:s}=t,n=Ce(s)?s:Object.keys(s||{});for(const a of Object.keys(this))a[0]!=="_"&&n.includes(a)&&this._setProp(a,this[a]);for(const a of n.map(ut))Object.defineProperty(this,a,{get(){return this._getProp(a)},set(i){this._setProp(a,i,!0,!this._patching)}})}_setAttr(t){if(t.startsWith("data-v-"))return;const s=this.hasAttribute(t);let n=s?this.getAttribute(t):Qd;const a=ut(t);s&&this._numberProps&&this._numberProps[a]&&(n=Hl(n)),this._setProp(a,n,!1,!0)}_getProp(t){return this._props[t]}_setProp(t,s,n=!0,a=!1){if(s!==this._props[t]&&(this._dirty=!0,s===Qd?delete this._props[t]:(this._props[t]=s,t==="key"&&this._app&&(this._app._ceVNode.key=s)),a&&this._instance&&this._update(),n)){const i=this._ob;i&&(this._processMutations(i.takeRecords()),i.disconnect()),s===!0?this.setAttribute(ws(t),""):typeof s=="string"||typeof s=="number"?this.setAttribute(ws(t),s+""):s||this.removeAttribute(ws(t)),i&&i.observe(this,{attributes:!0})}}_update(){const t=this._createVNode();this._app&&(t.appContext=this._app._context),kh(t,this._root)}_createVNode(){const t={};this.shadowRoot||(t.onVnodeMounted=t.onVnodeUpdated=this._renderSlots.bind(this));const s=xt(this._def,qe(t,this._props));return this._instance||(s.ce=n=>{this._instance=n,n.ce=this,n.isCE=!0;const a=(i,l)=>{this.dispatchEvent(new CustomEvent(i,yo(l[0])?qe({detail:l},l[0]):{detail:l}))};n.emit=(i,...l)=>{a(i,l),ws(i)!==i&&a(ws(i),l)},this._setParent()}),s}_applyStyles(t,s,n){if(!t)return;if(s){if(s===this._def||this._styleChildren.has(s))return;this._styleChildren.add(s)}const a=this._nonce,i=this.shadowRoot,l=n?this._getStyleAnchor(n)||this._getStyleAnchor(this._def):this._getRootStyleInsertionAnchor(i);let o=null;for(let r=t.length-1;r>=0;r--){const c=document.createElement("style");a&&c.setAttribute("nonce",a),c.textContent=t[r],i.insertBefore(c,o||l),o=c,r===0&&(n||this._styleAnchors.set(this._def,c),s&&this._styleAnchors.set(s,c))}}_getStyleAnchor(t){if(!t)return null;const s=this._styleAnchors.get(t);return s&&s.parentNode===this.shadowRoot?s:(s&&this._styleAnchors.delete(t),null)}_getRootStyleInsertionAnchor(t){for(let s=0;s<t.childNodes.length;s++){const n=t.childNodes[s];if(!(n instanceof HTMLStyleElement))return n}return null}_parseSlots(){const t=this._slots={};let s;for(;s=this.firstChild;){const n=s.nodeType===1&&s.getAttribute("slot")||"default";(t[n]||(t[n]=[])).push(s),this.removeChild(s)}}_renderSlots(){const t=this._getSlots(),s=this._instance.type.__scopeId;for(let n=0;n<t.length;n++){const a=t[n],i=a.getAttribute("name")||"default",l=this._slots[i],o=a.parentNode;if(l)for(const r of l){if(s&&r.nodeType===1){const c=s+"-s",d=document.createTreeWalker(r,1);r.setAttribute(c,"");let u;for(;u=d.nextNode();)u.setAttribute(c,"")}o.insertBefore(r,a)}else for(;a.firstChild;)o.insertBefore(a.firstChild,a);o.removeChild(a)}}_getSlots(){const t=[this];this._teleportTargets&&t.push(...this._teleportTargets);const s=new Set;for(const n of t){const a=n.querySelectorAll("slot");for(let i=0;i<a.length;i++)s.add(a[i])}return Array.from(s)}_injectChildStyle(t,s){this._applyStyles(t.styles,t,s)}_beginPatch(){this._patching=!0,this._dirty=!1}_endPatch(){this._patching=!1,this._dirty&&this._instance&&this._update()}_hasShadowRoot(){return this._def.shadowRoot!==!1}_removeChildStyle(t){}}function ph(e){const t=ms(),s=t&&t.ce;return s||null}function Hy(){const e=ph();return e&&e.shadowRoot}function zy(e="$style"){{const t=ms();if(!t)return Ke;const s=t.type.__cssModules;if(!s)return Ke;const n=s[e];return n||Ke}}const fh=new WeakMap,hh=new WeakMap,so=Symbol("_moveCb"),Xd=Symbol("_enterCb"),jy=e=>(delete e.props.mode,e),Vy=jy({name:"TransitionGroup",props:qe({},ih,{tag:String,moveClass:String}),setup(e,{slots:t}){const s=ms(),n=vc();let a,i;return Lo(()=>{if(!a.length)return;const l=e.moveClass||`${e.name||"v"}-move`;if(!Zy(a[0].el,s.vnode.el,l)){a=[];return}a.forEach(Gy),a.forEach(Ky);const o=a.filter(Wy);Br(s.vnode.el),o.forEach(r=>{const c=r.el,d=c.style;Qs(c,l),d.transform=d.webkitTransform=d.transitionDuration="";const u=c[so]=p=>{p&&p.target!==c||(!p||p.propertyName.endsWith("transform"))&&(c.removeEventListener("transitionend",u),c[so]=null,jn(c,l))};c.addEventListener("transitionend",u)}),a=[]}),()=>{const l=Ye(e),o=lh(l);let r=l.tag||Ht;if(a=[],i)for(let c=0;c<i.length;c++){const d=i[c];d.el&&d.el instanceof Element&&!d.el[Ic]&&(a.push(d),Ln(d,Xa(d,o,n,s)),fh.set(d,mh(d.el)))}i=t.default?Io(t.default()):[];for(let c=0;c<i.length;c++){const d=i[c];d.key!=null&&Ln(d,Xa(d,o,n,s))}return xt(r,null,i)}}}),qy=Vy;function Gy(e){const t=e.el;t[so]&&t[so](),t[Xd]&&t[Xd]()}function Ky(e){hh.set(e,mh(e.el))}function Wy(e){const t=fh.get(e),s=hh.get(e),n=t.left-s.left,a=t.top-s.top;if(n||a){const i=e.el,l=i.style,o=i.getBoundingClientRect();let r=1,c=1;return i.offsetWidth&&(r=o.width/i.offsetWidth),i.offsetHeight&&(c=o.height/i.offsetHeight),(!Number.isFinite(r)||r===0)&&(r=1),(!Number.isFinite(c)||c===0)&&(c=1),Math.abs(r-1)<.01&&(r=1),Math.abs(c-1)<.01&&(c=1),l.transform=l.webkitTransform=`translate(${n/r}px,${a/c}px)`,l.transitionDuration="0s",e}}function mh(e){const t=e.getBoundingClientRect();return{left:t.left,top:t.top}}function Zy(e,t,s){const n=e.cloneNode(),a=e[ti];a&&a.forEach(o=>{o.split(/\s+/).forEach(r=>r&&n.classList.remove(r))}),s.split(/\s+/).forEach(o=>o&&n.classList.add(o)),n.style.display="none";const i=t.nodeType===1?t:t.parentNode;i.appendChild(n);const{hasTransform:l}=oh(n);return i.removeChild(n),l}const Jn=e=>{const t=e.props["onUpdate:modelValue"]||!1;return Ce(t)?s=>qa(t,s):t};function Jy(e){e.target.composing=!0}function eu(e){const t=e.target;t.composing&&(t.composing=!1,t.dispatchEvent(new Event("input")))}const Us=Symbol("_assign");function tu(e,t,s){return t&&(e=e.trim()),s&&(e=wo(e)),e}const no={created(e,{modifiers:{lazy:t,trim:s,number:n}},a){e[Us]=Jn(a);const i=n||a.props&&a.props.type==="number";wn(e,t?"change":"input",l=>{l.target.composing||e[Us](tu(e.value,s,i))}),(s||i)&&wn(e,"change",()=>{e.value=tu(e.value,s,i)}),t||(wn(e,"compositionstart",Jy),wn(e,"compositionend",eu),wn(e,"change",eu))},mounted(e,{value:t}){e.value=t??""},beforeUpdate(e,{value:t,oldValue:s,modifiers:{lazy:n,trim:a,number:i}},l){if(e[Us]=Jn(l),e.composing)return;const o=(i||e.type==="number")&&!/^0\d/.test(e.value)?wo(e.value):e.value,r=t??"";if(o===r)return;const c=e.getRootNode();(c instanceof Document||c instanceof ShadowRoot)&&c.activeElement===e&&e.type!=="range"&&(n&&t===s||a&&e.value.trim()===r)||(e.value=r)}},Oc={deep:!0,created(e,t,s){e[Us]=Jn(s),wn(e,"change",()=>{const n=e._modelValue,a=si(e),i=e.checked,l=e[Us];if(Ce(n)){const o=So(n,a),r=o!==-1;if(i&&!r)l(n.concat(a));else if(!i&&r){const c=[...n];c.splice(o,1),l(c)}}else if(wa(n)){const o=new Set(n);i?o.add(a):o.delete(a),l(o)}else l(gh(e,i))})},mounted:su,beforeUpdate(e,t,s){e[Us]=Jn(s),su(e,t,s)}};function su(e,{value:t,oldValue:s},n){e._modelValue=t;let a;if(Ce(t))a=So(t,n.props.value)>-1;else if(wa(t))a=t.has(n.props.value);else{if(t===s)return;a=Rn(t,gh(e,!0))}e.checked!==a&&(e.checked=a)}const Lc={created(e,{value:t},s){e.checked=Rn(t,s.props.value),e[Us]=Jn(s),wn(e,"change",()=>{e[Us](si(e))})},beforeUpdate(e,{value:t,oldValue:s},n){e[Us]=Jn(n),t!==s&&(e.checked=Rn(t,n.props.value))}},vh={deep:!0,created(e,{value:t,modifiers:{number:s}},n){const a=wa(t);wn(e,"change",()=>{const i=Array.prototype.filter.call(e.options,l=>l.selected).map(l=>s?wo(si(l)):si(l));e[Us](e.multiple?a?new Set(i):i:i[0]),e._assigning=!0,It(()=>{e._assigning=!1})}),e[Us]=Jn(n)},mounted(e,{value:t}){nu(e,t)},beforeUpdate(e,t,s){e[Us]=Jn(s)},updated(e,{value:t}){e._assigning||nu(e,t)}};function nu(e,t){const s=e.multiple,n=Ce(t);if(!(s&&!n&&!wa(t))){for(let a=0,i=e.options.length;a<i;a++){const l=e.options[a],o=si(l);if(s)if(n){const r=typeof o;r==="string"||r==="number"?l.selected=t.some(c=>String(c)===String(o)):l.selected=So(t,o)>-1}else l.selected=t.has(o);else if(Rn(si(l),t)){e.selectedIndex!==a&&(e.selectedIndex=a);return}}!s&&e.selectedIndex!==-1&&(e.selectedIndex=-1)}}function si(e){return"_value"in e?e._value:e.value}function gh(e,t){const s=t?"_trueValue":"_falseValue";return s in e?e[s]:t}const bh={created(e,t,s){Cl(e,t,s,null,"created")},mounted(e,t,s){Cl(e,t,s,null,"mounted")},beforeUpdate(e,t,s,n){Cl(e,t,s,n,"beforeUpdate")},updated(e,t,s,n){Cl(e,t,s,n,"updated")}};function yh(e,t){switch(e){case"SELECT":return vh;case"TEXTAREA":return no;default:switch(t){case"checkbox":return Oc;case"radio":return Lc;default:return no}}}function Cl(e,t,s,n,a){const l=yh(e.tagName,s.props&&s.props.type)[a];l&&l(e,t,s,n)}function Yy(){no.getSSRProps=({value:e})=>({value:e}),Lc.getSSRProps=({value:e},t)=>{if(t.props&&Rn(t.props.value,e))return{checked:!0}},Oc.getSSRProps=({value:e},t)=>{if(Ce(e)){if(t.props&&So(e,t.props.value)>-1)return{checked:!0}}else if(wa(e)){if(t.props&&e.has(t.props.value))return{checked:!0}}else if(e)return{checked:!0}},bh.getSSRProps=(e,t)=>{if(typeof t.type!="string")return;const s=yh(t.type.toUpperCase(),t.props&&t.props.type);if(s.getSSRProps)return s.getSSRProps(e,t)}}const Qy=["ctrl","shift","alt","meta"],Xy={stop:e=>e.stopPropagation(),prevent:e=>e.preventDefault(),self:e=>e.target!==e.currentTarget,ctrl:e=>!e.ctrlKey,shift:e=>!e.shiftKey,alt:e=>!e.altKey,meta:e=>!e.metaKey,left:e=>"button"in e&&e.button!==0,middle:e=>"button"in e&&e.button!==1,right:e=>"button"in e&&e.button!==2,exact:(e,t)=>Qy.some(s=>e[`${s}Key`]&&!t.includes(s))},ex=(e,t)=>{if(!e)return e;const s=e._withMods||(e._withMods={}),n=t.join(".");return s[n]||(s[n]=((a,...i)=>{for(let l=0;l<t.length;l++){const o=Xy[t[l]];if(o&&o(a,t))return}return e(a,...i)}))},tx={esc:"escape",space:" ",up:"arrow-up",left:"arrow-left",right:"arrow-right",down:"arrow-down",delete:"backspace"},sx=(e,t)=>{const s=e._withKeys||(e._withKeys={}),n=t.join(".");return s[n]||(s[n]=(a=>{if(!("key"in a))return;const i=ws(a.key);if(t.some(l=>l===i||tx[l]===i))return e(a)}))},xh=qe({patchProp:dh},nh);let Pi,au=!1;function _h(){return Pi||(Pi=Df(xh))}function wh(){return Pi=au?Pi:Pf(xh),au=!0,Pi}const kh=((...e)=>{_h().render(...e)}),nx=((...e)=>{wh().hydrate(...e)}),ao=((...e)=>{const t=_h().createApp(...e),{mount:s}=t;return t.mount=n=>{const a=Ch(n);if(!a)return;const i=t._component;!Me(i)&&!i.render&&!i.template&&(i.template=a.innerHTML),a.nodeType===1&&(a.textContent="");const l=s(a,!1,Th(a));return a instanceof Element&&(a.removeAttribute("v-cloak"),a.setAttribute("data-v-app","")),l},t}),Sh=((...e)=>{const t=wh().createApp(...e),{mount:s}=t;return t.mount=n=>{const a=Ch(n);if(a)return s(a,!0,Th(a))},t});function Th(e){if(e instanceof SVGElement)return"svg";if(typeof MathMLElement=="function"&&e instanceof MathMLElement)return"mathml"}function Ch(e){return Be(e)?document.querySelector(e):e}let iu=!1;const ax=()=>{iu||(iu=!0,Yy(),Ty())},ix=Object.freeze(Object.defineProperty({__proto__:null,BaseTransition:uf,BaseTransitionPropsValidators:gc,Comment:Ct,DeprecationTypes:gy,EffectScope:oc,ErrorCodes:xg,ErrorTypeStrings:dy,Fragment:Ht,KeepAlive:Xg,ReactiveEffect:Hi,Static:ha,Suspense:qb,Teleport:Pg,Text:Wn,TrackOpTypes:fg,Transition:_y,TransitionGroup:qy,TriggerOpTypes:hg,VueElement:Mo,assertNumber:yg,callWithAsyncErrorHandling:Is,callWithErrorHandling:ui,camelize:ut,capitalize:ka,cloneVNode:on,compatUtils:vy,computed:J,createApp:ao,createBlock:Yl,createCommentVNode:Gf,createElementBlock:Yb,createElementVNode:Ec,createHydrationRenderer:Pf,createPropsRestProxy:_b,createRenderer:Df,createSSRApp:Sh,createSlots:lb,createStaticVNode:ey,createTextVNode:Ac,createVNode:xt,customRef:Wp,defineAsyncComponent:Yg,defineComponent:cl,defineCustomElement:uh,defineEmits:ub,defineExpose:pb,defineModel:mb,defineOptions:fb,defineProps:db,defineSSRCustomElement:By,defineSlots:hb,devtools:uy,effect:Mv,effectScope:Nv,getCurrentInstance:ms,getCurrentScope:Ip,getCurrentWatcher:mg,getTransitionRawChildren:Io,guardReactiveProps:qf,h:ei,handleError:Sa,hasInjectionContext:Ag,hydrate:nx,hydrateOnIdle:qg,hydrateOnInteraction:Zg,hydrateOnMediaQuery:Wg,hydrateOnVisible:Kg,initCustomFormatter:oy,initDirectivesForSSR:ax,inject:Bs,isMemoSame:eh,isProxy:ol,isReactive:Cn,isReadonly:ln,isRef:Lt,isRuntimeOnly:ay,isShallow:Ss,isVNode:Nn,markRaw:Gp,mergeDefaults:yb,mergeModels:xb,mergeProps:Kf,nextTick:It,nodeOps:nh,normalizeClass:ll,normalizeProps:xv,normalizeStyle:il,onActivated:gs,onBeforeMount:hf,onBeforeUnmount:No,onBeforeUpdate:yc,onDeactivated:ls,onErrorCaptured:bf,onMounted:Ge,onRenderTracked:gf,onRenderTriggered:vf,onScopeDispose:Dv,onServerPrefetch:mf,onUnmounted:bt,onUpdated:Lo,onWatcherCleanup:Jp,openBlock:Zi,patchProp:dh,popScopeId:Tg,provide:Li,proxyRefs:fc,pushScopeId:Sg,queuePostFlushCb:Vi,reactive:Yn,readonly:jl,ref:f,registerRuntimeCompiler:Yf,render:kh,renderList:ib,renderSlot:ob,resolveComponent:sb,resolveDirective:ab,resolveDynamicComponent:nb,resolveFilter:my,resolveTransitionHooks:Xa,setBlockTracking:Ji,setDevtoolsHook:py,setTransitionHooks:Ln,shallowReactive:uc,shallowReadonly:tg,shallowRef:pc,ssrContextKey:sf,ssrUtils:hy,stop:Fv,toDisplayString:Ap,toHandlerKey:Va,toHandlers:rb,toRaw:Ye,toRef:dg,toRefs:og,toValue:ag,transformVNodeArgs:Qb,triggerRef:ng,unref:an,useAttrs:bb,useCssModule:zy,useCssVars:Cy,useHost:ph,useId:Fg,useModel:Ib,useSSRContext:nf,useShadowRoot:Hy,useSlots:gb,useTemplateRef:$g,useTransitionState:vc,vModelCheckbox:Oc,vModelDynamic:bh,vModelRadio:Lc,vModelSelect:vh,vModelText:no,vShow:rh,version:th,warn:cy,watch:ns,watchEffect:Rg,watchPostEffect:Ig,watchSyncEffect:af,withAsyncContext:wb,withCtx:mc,withDefaults:vb,withDirectives:Eg,withKeys:sx,withMemo:ry,withModifiers:ex,withScopeId:Cg},Symbol.toStringTag,{value:"Module"}));/**
* @vue/compiler-core v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const Qi=Symbol(""),Mi=Symbol(""),Nc=Symbol(""),io=Symbol(""),Eh=Symbol(""),ya=Symbol(""),Ah=Symbol(""),Rh=Symbol(""),Dc=Symbol(""),Pc=Symbol(""),pl=Symbol(""),Mc=Symbol(""),Ih=Symbol(""),Fc=Symbol(""),$c=Symbol(""),Bc=Symbol(""),Uc=Symbol(""),Hc=Symbol(""),zc=Symbol(""),Oh=Symbol(""),Lh=Symbol(""),Fo=Symbol(""),lo=Symbol(""),jc=Symbol(""),Vc=Symbol(""),Xi=Symbol(""),fl=Symbol(""),qc=Symbol(""),Hr=Symbol(""),lx=Symbol(""),zr=Symbol(""),oo=Symbol(""),ox=Symbol(""),rx=Symbol(""),Gc=Symbol(""),cx=Symbol(""),dx=Symbol(""),Kc=Symbol(""),Nh=Symbol(""),ni={[Qi]:"Fragment",[Mi]:"Teleport",[Nc]:"Suspense",[io]:"KeepAlive",[Eh]:"BaseTransition",[ya]:"openBlock",[Ah]:"createBlock",[Rh]:"createElementBlock",[Dc]:"createVNode",[Pc]:"createElementVNode",[pl]:"createCommentVNode",[Mc]:"createTextVNode",[Ih]:"createStaticVNode",[Fc]:"resolveComponent",[$c]:"resolveDynamicComponent",[Bc]:"resolveDirective",[Uc]:"resolveFilter",[Hc]:"withDirectives",[zc]:"renderList",[Oh]:"renderSlot",[Lh]:"createSlots",[Fo]:"toDisplayString",[lo]:"mergeProps",[jc]:"normalizeClass",[Vc]:"normalizeStyle",[Xi]:"normalizeProps",[fl]:"guardReactiveProps",[qc]:"toHandlers",[Hr]:"camelize",[lx]:"capitalize",[zr]:"toHandlerKey",[oo]:"setBlockTracking",[ox]:"pushScopeId",[rx]:"popScopeId",[Gc]:"withCtx",[cx]:"unref",[dx]:"isRef",[Kc]:"withMemo",[Nh]:"isMemoSame"};function ux(e){Object.getOwnPropertySymbols(e).forEach(t=>{ni[t]=e[t]})}const Ls={start:{line:1,column:1,offset:0},end:{line:1,column:1,offset:0},source:""};function px(e,t=""){return{type:0,source:t,children:e,helpers:new Set,components:[],directives:[],hoists:[],imports:[],cached:[],temps:0,codegenNode:void 0,loc:Ls}}function el(e,t,s,n,a,i,l,o=!1,r=!1,c=!1,d=Ls){return e&&(o?(e.helper(ya),e.helper(li(e.inSSR,c))):e.helper(ii(e.inSSR,c)),l&&e.helper(Hc)),{type:13,tag:t,props:s,children:n,patchFlag:a,dynamicProps:i,directives:l,isBlock:o,disableTracking:r,isComponent:c,loc:d}}function ma(e,t=Ls){return{type:17,loc:t,elements:e}}function $s(e,t=Ls){return{type:15,loc:t,properties:e}}function Ot(e,t){return{type:16,loc:Ls,key:Be(e)?He(e,!0):e,value:t}}function He(e,t=!1,s=Ls,n=0){return{type:4,loc:s,content:e,isStatic:t,constType:t?3:n}}function Gs(e,t=Ls){return{type:8,loc:t,children:e}}function Bt(e,t=[],s=Ls){return{type:14,loc:s,callee:e,arguments:t}}function ai(e,t=void 0,s=!1,n=!1,a=Ls){return{type:18,params:e,returns:t,newline:s,isSlot:n,loc:a}}function jr(e,t,s,n=!0){return{type:19,test:e,consequent:t,alternate:s,newline:n,loc:Ls}}function fx(e,t,s=!1,n=!1){return{type:20,index:e,value:t,needPauseTracking:s,inVOnce:n,needArraySpread:!1,loc:Ls}}function hx(e){return{type:21,body:e,loc:Ls}}function ii(e,t){return e||t?Dc:Pc}function li(e,t){return e||t?Ah:Rh}function Wc(e,{helper:t,removeHelper:s,inSSR:n}){e.isBlock||(e.isBlock=!0,s(ii(n,e.isComponent)),t(ya),t(li(n,e.isComponent)))}const lu=new Uint8Array([123,123]),ou=new Uint8Array([125,125]);function ru(e){return e>=97&&e<=122||e>=65&&e<=90}function As(e){return e===32||e===10||e===9||e===12||e===13}function Un(e){return e===47||e===62||As(e)}function ro(e){const t=new Uint8Array(e.length);for(let s=0;s<e.length;s++)t[s]=e.charCodeAt(s);return t}const Qt={Cdata:new Uint8Array([67,68,65,84,65,91]),CdataEnd:new Uint8Array([93,93,62]),CommentEnd:new Uint8Array([45,45,62]),ScriptEnd:new Uint8Array([60,47,115,99,114,105,112,116]),StyleEnd:new Uint8Array([60,47,115,116,121,108,101]),TitleEnd:new Uint8Array([60,47,116,105,116,108,101]),TextareaEnd:new Uint8Array([60,47,116,101,120,116,97,114,101,97])};class mx{constructor(t,s){this.stack=t,this.cbs=s,this.state=1,this.buffer="",this.sectionStart=0,this.index=0,this.entityStart=0,this.baseState=1,this.inRCDATA=!1,this.inXML=!1,this.inVPre=!1,this.newlines=[],this.mode=0,this.delimiterOpen=lu,this.delimiterClose=ou,this.delimiterIndex=-1,this.currentSequence=void 0,this.sequenceIndex=0}get inSFCRoot(){return this.mode===2&&this.stack.length===0}reset(){this.state=1,this.mode=0,this.buffer="",this.sectionStart=0,this.index=0,this.baseState=1,this.inRCDATA=!1,this.currentSequence=void 0,this.newlines.length=0,this.delimiterOpen=lu,this.delimiterClose=ou}getPos(t){let s=1,n=t+1;const a=this.newlines.length;let i=-1;if(a>100){let l=-1,o=a;for(;l+1<o;){const r=l+o>>>1;this.newlines[r]<t?l=r:o=r}i=l}else for(let l=a-1;l>=0;l--)if(t>this.newlines[l]){i=l;break}return i>=0&&(s=i+2,n=t-this.newlines[i]),{column:n,line:s,offset:t}}peek(){return this.buffer.charCodeAt(this.index+1)}stateText(t){t===60?(this.index>this.sectionStart&&this.cbs.ontext(this.sectionStart,this.index),this.state=5,this.sectionStart=this.index):!this.inVPre&&t===this.delimiterOpen[0]&&(this.state=2,this.delimiterIndex=0,this.stateInterpolationOpen(t))}stateInterpolationOpen(t){if(t===this.delimiterOpen[this.delimiterIndex])if(this.delimiterIndex===this.delimiterOpen.length-1){const s=this.index+1-this.delimiterOpen.length;s>this.sectionStart&&this.cbs.ontext(this.sectionStart,s),this.state=3,this.sectionStart=s}else this.delimiterIndex++;else this.inRCDATA?(this.state=32,this.stateInRCDATA(t)):(this.state=1,this.stateText(t))}stateInterpolation(t){t===this.delimiterClose[0]&&(this.state=4,this.delimiterIndex=0,this.stateInterpolationClose(t))}stateInterpolationClose(t){t===this.delimiterClose[this.delimiterIndex]?this.delimiterIndex===this.delimiterClose.length-1?(this.cbs.oninterpolation(this.sectionStart,this.index+1),this.inRCDATA?this.state=32:this.state=1,this.sectionStart=this.index+1):this.delimiterIndex++:(this.state=3,this.stateInterpolation(t))}stateSpecialStartSequence(t){const s=this.sequenceIndex===this.currentSequence.length;if(!(s?Un(t):(t|32)===this.currentSequence[this.sequenceIndex]))this.inRCDATA=!1;else if(!s){this.sequenceIndex++;return}this.sequenceIndex=0,this.state=6,this.stateInTagName(t)}stateInRCDATA(t){if(this.sequenceIndex===this.currentSequence.length){if(t===62||As(t)){const s=this.index-this.currentSequence.length;if(this.sectionStart<s){const n=this.index;this.index=s,this.cbs.ontext(this.sectionStart,s),this.index=n}this.sectionStart=s+2,this.stateInClosingTagName(t),this.inRCDATA=!1;return}this.sequenceIndex=0}(t|32)===this.currentSequence[this.sequenceIndex]?this.sequenceIndex+=1:this.sequenceIndex===0?this.currentSequence===Qt.TitleEnd||this.currentSequence===Qt.TextareaEnd&&!this.inSFCRoot?!this.inVPre&&t===this.delimiterOpen[0]&&(this.state=2,this.delimiterIndex=0,this.stateInterpolationOpen(t)):this.fastForwardTo(60)&&(this.sequenceIndex=1):this.sequenceIndex=+(t===60)}stateCDATASequence(t){t===Qt.Cdata[this.sequenceIndex]?++this.sequenceIndex===Qt.Cdata.length&&(this.state=28,this.currentSequence=Qt.CdataEnd,this.sequenceIndex=0,this.sectionStart=this.index+1):(this.sequenceIndex=0,this.state=23,this.stateInDeclaration(t))}fastForwardTo(t){for(;++this.index<this.buffer.length;){const s=this.buffer.charCodeAt(this.index);if(s===10&&this.newlines.push(this.index),s===t)return!0}return this.index=this.buffer.length-1,!1}stateInCommentLike(t){t===this.currentSequence[this.sequenceIndex]?++this.sequenceIndex===this.currentSequence.length&&(this.currentSequence===Qt.CdataEnd?this.cbs.oncdata(this.sectionStart,this.index-2):this.cbs.oncomment(this.sectionStart,this.index-2),this.sequenceIndex=0,this.sectionStart=this.index+1,this.state=1):this.sequenceIndex===0?this.fastForwardTo(this.currentSequence[0])&&(this.sequenceIndex=1):t!==this.currentSequence[this.sequenceIndex-1]&&(this.sequenceIndex=0)}startSpecial(t,s){this.enterRCDATA(t,s),this.state=31}enterRCDATA(t,s){this.inRCDATA=!0,this.currentSequence=t,this.sequenceIndex=s}stateBeforeTagName(t){t===33?(this.state=22,this.sectionStart=this.index+1):t===63?(this.state=24,this.sectionStart=this.index+1):ru(t)?(this.sectionStart=this.index,this.mode===0?this.state=6:this.inSFCRoot?this.state=34:this.inXML?this.state=6:t===116?this.state=30:this.state=t===115?29:6):t===47?this.state=8:(this.state=1,this.stateText(t))}stateInTagName(t){Un(t)&&this.handleTagName(t)}stateInSFCRootTagName(t){if(Un(t)){const s=this.buffer.slice(this.sectionStart,this.index);s!=="template"&&this.enterRCDATA(ro("</"+s),0),this.handleTagName(t)}}handleTagName(t){this.cbs.onopentagname(this.sectionStart,this.index),this.sectionStart=-1,this.state=11,this.stateBeforeAttrName(t)}stateBeforeClosingTagName(t){As(t)||(t===62?(this.state=1,this.sectionStart=this.index+1):(this.state=ru(t)?9:27,this.sectionStart=this.index))}stateInClosingTagName(t){(t===62||As(t))&&(this.cbs.onclosetag(this.sectionStart,this.index),this.sectionStart=-1,this.state=10,this.stateAfterClosingTagName(t))}stateAfterClosingTagName(t){t===62&&(this.state=1,this.sectionStart=this.index+1)}stateBeforeAttrName(t){t===62?(this.cbs.onopentagend(this.index),this.inRCDATA?this.state=32:this.state=1,this.sectionStart=this.index+1):t===47?this.state=7:t===60&&this.peek()===47?(this.cbs.onopentagend(this.index),this.state=5,this.sectionStart=this.index):As(t)||this.handleAttrStart(t)}handleAttrStart(t){t===118&&this.peek()===45?(this.state=13,this.sectionStart=this.index):t===46||t===58||t===64||t===35?(this.cbs.ondirname(this.index,this.index+1),this.state=14,this.sectionStart=this.index+1):(this.state=12,this.sectionStart=this.index)}stateInSelfClosingTag(t){t===62?(this.cbs.onselfclosingtag(this.index),this.state=1,this.sectionStart=this.index+1,this.inRCDATA=!1):As(t)||(this.state=11,this.stateBeforeAttrName(t))}stateInAttrName(t){(t===61||Un(t))&&(this.cbs.onattribname(this.sectionStart,this.index),this.handleAttrNameEnd(t))}stateInDirName(t){t===61||Un(t)?(this.cbs.ondirname(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===58?(this.cbs.ondirname(this.sectionStart,this.index),this.state=14,this.sectionStart=this.index+1):t===46&&(this.cbs.ondirname(this.sectionStart,this.index),this.state=16,this.sectionStart=this.index+1)}stateInDirArg(t){t===61||Un(t)?(this.cbs.ondirarg(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===91?this.state=15:t===46&&(this.cbs.ondirarg(this.sectionStart,this.index),this.state=16,this.sectionStart=this.index+1)}stateInDynamicDirArg(t){t===93?this.state=14:(t===61||Un(t))&&(this.cbs.ondirarg(this.sectionStart,this.index+1),this.handleAttrNameEnd(t))}stateInDirModifier(t){t===61||Un(t)?(this.cbs.ondirmodifier(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===46&&(this.cbs.ondirmodifier(this.sectionStart,this.index),this.sectionStart=this.index+1)}handleAttrNameEnd(t){this.sectionStart=this.index,this.state=17,this.cbs.onattribnameend(this.index),this.stateAfterAttrName(t)}stateAfterAttrName(t){t===61?this.state=18:t===47||t===62?(this.cbs.onattribend(0,this.sectionStart),this.sectionStart=-1,this.state=11,this.stateBeforeAttrName(t)):As(t)||(this.cbs.onattribend(0,this.sectionStart),this.handleAttrStart(t))}stateBeforeAttrValue(t){t===34?(this.state=19,this.sectionStart=this.index+1):t===39?(this.state=20,this.sectionStart=this.index+1):As(t)||(this.sectionStart=this.index,this.state=21,this.stateInAttrValueNoQuotes(t))}handleInAttrValue(t,s){(t===s||this.fastForwardTo(s))&&(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=-1,this.cbs.onattribend(s===34?3:2,this.index+1),this.state=11)}stateInAttrValueDoubleQuotes(t){this.handleInAttrValue(t,34)}stateInAttrValueSingleQuotes(t){this.handleInAttrValue(t,39)}stateInAttrValueNoQuotes(t){As(t)||t===62?(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=-1,this.cbs.onattribend(1,this.index),this.state=11,this.stateBeforeAttrName(t)):(t===39||t===60||t===61||t===96)&&this.cbs.onerr(18,this.index)}stateBeforeDeclaration(t){t===91?(this.state=26,this.sequenceIndex=0):this.state=t===45?25:23}stateInDeclaration(t){(t===62||this.fastForwardTo(62))&&(this.state=1,this.sectionStart=this.index+1)}stateInProcessingInstruction(t){(t===62||this.fastForwardTo(62))&&(this.cbs.onprocessinginstruction(this.sectionStart,this.index),this.state=1,this.sectionStart=this.index+1)}stateBeforeComment(t){t===45?(this.state=28,this.currentSequence=Qt.CommentEnd,this.sequenceIndex=2,this.sectionStart=this.index+1):this.state=23}stateInSpecialComment(t){(t===62||this.fastForwardTo(62))&&(this.cbs.oncomment(this.sectionStart,this.index),this.state=1,this.sectionStart=this.index+1)}stateBeforeSpecialS(t){t===Qt.ScriptEnd[3]?this.startSpecial(Qt.ScriptEnd,4):t===Qt.StyleEnd[3]?this.startSpecial(Qt.StyleEnd,4):(this.state=6,this.stateInTagName(t))}stateBeforeSpecialT(t){t===Qt.TitleEnd[3]?this.startSpecial(Qt.TitleEnd,4):t===Qt.TextareaEnd[3]?this.startSpecial(Qt.TextareaEnd,4):(this.state=6,this.stateInTagName(t))}startEntity(){}stateInEntity(){}parse(t){for(this.buffer=t;this.index<this.buffer.length;){const s=this.buffer.charCodeAt(this.index);switch(s===10&&this.state!==33&&this.newlines.push(this.index),this.state){case 1:{this.stateText(s);break}case 2:{this.stateInterpolationOpen(s);break}case 3:{this.stateInterpolation(s);break}case 4:{this.stateInterpolationClose(s);break}case 31:{this.stateSpecialStartSequence(s);break}case 32:{this.stateInRCDATA(s);break}case 26:{this.stateCDATASequence(s);break}case 19:{this.stateInAttrValueDoubleQuotes(s);break}case 12:{this.stateInAttrName(s);break}case 13:{this.stateInDirName(s);break}case 14:{this.stateInDirArg(s);break}case 15:{this.stateInDynamicDirArg(s);break}case 16:{this.stateInDirModifier(s);break}case 28:{this.stateInCommentLike(s);break}case 27:{this.stateInSpecialComment(s);break}case 11:{this.stateBeforeAttrName(s);break}case 6:{this.stateInTagName(s);break}case 34:{this.stateInSFCRootTagName(s);break}case 9:{this.stateInClosingTagName(s);break}case 5:{this.stateBeforeTagName(s);break}case 17:{this.stateAfterAttrName(s);break}case 20:{this.stateInAttrValueSingleQuotes(s);break}case 18:{this.stateBeforeAttrValue(s);break}case 8:{this.stateBeforeClosingTagName(s);break}case 10:{this.stateAfterClosingTagName(s);break}case 29:{this.stateBeforeSpecialS(s);break}case 30:{this.stateBeforeSpecialT(s);break}case 21:{this.stateInAttrValueNoQuotes(s);break}case 7:{this.stateInSelfClosingTag(s);break}case 23:{this.stateInDeclaration(s);break}case 22:{this.stateBeforeDeclaration(s);break}case 25:{this.stateBeforeComment(s);break}case 24:{this.stateInProcessingInstruction(s);break}case 33:{this.stateInEntity();break}}this.index++}this.cleanup(),this.finish()}cleanup(){this.sectionStart!==this.index&&(this.state===1||this.state===32&&this.sequenceIndex===0?(this.cbs.ontext(this.sectionStart,this.index),this.sectionStart=this.index):(this.state===19||this.state===20||this.state===21)&&(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=this.index))}finish(){this.handleTrailingData(),this.cbs.onend()}handleTrailingData(){const t=this.buffer.length;this.sectionStart>=t||(this.state===28?this.currentSequence===Qt.CdataEnd?this.cbs.oncdata(this.sectionStart,t):this.cbs.oncomment(this.sectionStart,t):this.state===6||this.state===11||this.state===18||this.state===17||this.state===12||this.state===13||this.state===14||this.state===15||this.state===16||this.state===20||this.state===19||this.state===21||this.state===9||this.cbs.ontext(this.sectionStart,t))}emitCodePoint(t,s){}}function cu(e,{compatConfig:t}){const s=t&&t[e];return e==="MODE"?s||3:s}function va(e,t){const s=cu("MODE",t),n=cu(e,t);return s===3?n===!0:n!==!1}function tl(e,t,s,...n){return va(e,t)}function Zc(e){throw e}function Dh(e){}function gt(e,t,s,n){const a=`https://vuejs.org/error-reference/#compiler-${e}`,i=new SyntaxError(String(a));return i.code=e,i.loc=t,i}const ks=e=>e.type===4&&e.isStatic;function Ph(e){switch(e){case"Teleport":case"teleport":return Mi;case"Suspense":case"suspense":return Nc;case"KeepAlive":case"keep-alive":return io;case"BaseTransition":case"base-transition":return Eh}}const vx=/^$|^\d|[^\$\w\xA0-\uFFFF]/,Jc=e=>!vx.test(e),Mh=/[A-Za-z_$\xA0-\uFFFF]/,gx=/[\.\?\w$\xA0-\uFFFF]/,bx=/\s+[.[]\s*|\s*[.[]\s+/g,Fh=e=>e.type===4?e.content:e.loc.source,yx=e=>{const t=Fh(e).trim().replace(bx,o=>o.trim());let s=0,n=[],a=0,i=0,l=null;for(let o=0;o<t.length;o++){const r=t.charAt(o);switch(s){case 0:if(r==="[")n.push(s),s=1,a++;else if(r==="(")n.push(s),s=2,i++;else if(!(o===0?Mh:gx).test(r))return!1;break;case 1:r==="'"||r==='"'||r==="`"?(n.push(s),s=3,l=r):r==="["?a++:r==="]"&&(--a||(s=n.pop()));break;case 2:if(r==="'"||r==='"'||r==="`")n.push(s),s=3,l=r;else if(r==="(")i++;else if(r===")"){if(o===t.length-1)return!1;--i||(s=n.pop())}break;case 3:r===l&&(s=n.pop(),l=null);break}}return!a&&!i},$h=yx,xx=/^\s*(?:async\s*)?(?:\([^)]*?\)|[\w$_]+)\s*(?::[^=]+)?=>|^\s*(?:async\s+)?function(?:\s+[\w$]+)?\s*\(/,_x=e=>xx.test(Fh(e)),wx=_x;function Fs(e,t,s=!1){for(let n=0;n<e.props.length;n++){const a=e.props[n];if(a.type===7&&(s||a.exp)&&(Be(t)?a.name===t:t.test(a.name)))return a}}function $o(e,t,s=!1,n=!1){for(let a=0;a<e.props.length;a++){const i=e.props[a];if(i.type===6){if(s)continue;if(i.name===t&&(i.value||n))return i}else if(i.name==="bind"&&(i.exp||n)&&ca(i.arg,t))return i}}function ca(e,t){return!!(e&&ks(e)&&e.content===t)}function kx(e){return e.props.some(t=>t.type===7&&t.name==="bind"&&(!t.arg||t.arg.type!==4||!t.arg.isStatic))}function ir(e){return e.type===5||e.type===2}function du(e){return e.type===7&&e.name==="pre"}function Sx(e){return e.type===7&&e.name==="slot"}function co(e){return e.type===1&&e.tagType===3}function uo(e){return e.type===1&&e.tagType===2}const Tx=new Set([Xi,fl]);function Bh(e,t=[]){if(e&&!Be(e)&&e.type===14){const s=e.callee;if(!Be(s)&&Tx.has(s))return Bh(e.arguments[0],t.concat(e))}return[e,t]}function po(e,t,s){let n,a=e.type===13?e.props:e.arguments[2],i=[],l;if(a&&!Be(a)&&a.type===14){const o=Bh(a);a=o[0],i=o[1],l=i[i.length-1]}if(a==null||Be(a))n=$s([t]);else if(a.type===14){const o=a.arguments[0];!Be(o)&&o.type===15?uu(t,o)||o.properties.unshift(t):a.callee===qc?n=Bt(s.helper(lo),[$s([t]),a]):a.arguments.unshift($s([t])),!n&&(n=a)}else a.type===15?(uu(t,a)||a.properties.unshift(t),n=a):(n=Bt(s.helper(lo),[$s([t]),a]),l&&l.callee===fl&&(l=i[i.length-2]));e.type===13?l?l.arguments[0]=n:e.props=n:l?l.arguments[0]=n:e.arguments[2]=n}function uu(e,t){let s=!1;if(e.key.type===4){const n=e.key.content;s=t.properties.some(a=>a.key.type===4&&a.key.content===n)}return s}function sl(e,t){return`_${t}_${e.replace(/[^\w]/g,(s,n)=>s==="-"?"_":e.charCodeAt(n).toString())}`}function Cx(e){return e.type===14&&e.callee===Kc?e.arguments[1].returns:e}const Ex=/([\s\S]*?)\s+(?:in|of)\s+(\S[\s\S]*)/;function Uh(e){for(let t=0;t<e.length;t++)if(!As(e.charCodeAt(t)))return!1;return!0}function Yc(e){return e.type===2&&Uh(e.content)||e.type===12&&Yc(e.content)}function Hh(e){return e.type===3||Yc(e)}const zh={parseMode:"base",ns:0,delimiters:["{{","}}"],getNamespace:()=>0,isVoidTag:Ua,isPreTag:Ua,isIgnoreNewlineTag:Ua,isCustomElement:Ua,onError:Zc,onWarn:Dh,comments:!1,prefixIdentifiers:!1};let Xe=zh,nl=null,An="",es=null,We=null,bs="",vn=-1,la=-1,Qc=0,Gn=!1,Vr=null;const vt=[],St=new mx(vt,{onerr:fn,ontext(e,t){El(qt(e,t),e,t)},ontextentity(e,t,s){El(e,t,s)},oninterpolation(e,t){if(Gn)return El(qt(e,t),e,t);let s=e+St.delimiterOpen.length,n=t-St.delimiterClose.length;for(;As(An.charCodeAt(s));)s++;for(;As(An.charCodeAt(n-1));)n--;let a=qt(s,n);a.includes("&")&&(a=Xe.decodeEntities(a,!1)),qr({type:5,content:Fl(a,!1,Tt(s,n)),loc:Tt(e,t)})},onopentagname(e,t){const s=qt(e,t);es={type:1,tag:s,ns:Xe.getNamespace(s,vt[0],Xe.ns),tagType:0,props:[],children:[],loc:Tt(e-1,t),codegenNode:void 0}},onopentagend(e){fu(e)},onclosetag(e,t){const s=qt(e,t);if(!Xe.isVoidTag(s)){let n=!1;for(let a=0;a<vt.length;a++)if(vt[a].tag.toLowerCase()===s.toLowerCase()){n=!0,a>0&&fn(24,vt[0].loc.start.offset);for(let l=0;l<=a;l++){const o=vt.shift();Ml(o,t,l<a)}break}n||fn(23,jh(e,60))}},onselfclosingtag(e){const t=es.tag;es.isSelfClosing=!0,fu(e),vt[0]&&vt[0].tag===t&&Ml(vt.shift(),e)},onattribname(e,t){We={type:6,name:qt(e,t),nameLoc:Tt(e,t),value:void 0,loc:Tt(e)}},ondirname(e,t){const s=qt(e,t),n=s==="."||s===":"?"bind":s==="@"?"on":s==="#"?"slot":s.slice(2);if(!Gn&&n===""&&fn(26,e),Gn||n==="")We={type:6,name:s,nameLoc:Tt(e,t),value:void 0,loc:Tt(e)};else if(We={type:7,name:n,rawName:s,exp:void 0,arg:void 0,modifiers:s==="."?[He("prop")]:[],loc:Tt(e)},n==="pre"){Gn=St.inVPre=!0,Vr=es;const a=es.props;for(let i=0;i<a.length;i++)a[i].type===7&&(a[i]=Fx(a[i]))}},ondirarg(e,t){if(e===t)return;const s=qt(e,t);if(Gn&&!du(We))We.name+=s,da(We.nameLoc,t);else{const n=s[0]!=="[";We.arg=Fl(n?s:s.slice(1,-1),n,Tt(e,t),n?3:0)}},ondirmodifier(e,t){const s=qt(e,t);if(Gn&&!du(We))We.name+="."+s,da(We.nameLoc,t);else if(We.name==="slot"){const n=We.arg;n&&(n.content+="."+s,da(n.loc,t))}else{const n=He(s,!0,Tt(e,t));We.modifiers.push(n)}},onattribdata(e,t){bs+=qt(e,t),vn<0&&(vn=e),la=t},onattribentity(e,t,s){bs+=e,vn<0&&(vn=t),la=s},onattribnameend(e){const t=We.loc.start.offset,s=qt(t,e);We.type===7&&(We.rawName=s),es.props.some(n=>(n.type===7?n.rawName:n.name)===s)&&fn(2,t)},onattribend(e,t){if(es&&We){if(da(We.loc,t),e!==0)if(bs.includes("&")&&(bs=Xe.decodeEntities(bs,!0)),We.type===6)We.name==="class"&&(bs=qh(bs).trim()),e===1&&!bs&&fn(13,t),We.value={type:2,content:bs,loc:e===1?Tt(vn,la):Tt(vn-1,la+1)},St.inSFCRoot&&es.tag==="template"&&We.name==="lang"&&bs&&bs!=="html"&&St.enterRCDATA(ro("</template"),0);else{let s=0;We.exp=Fl(bs,!1,Tt(vn,la),0,s),We.name==="for"&&(We.forParseResult=Rx(We.exp));let n=-1;We.name==="bind"&&(n=We.modifiers.findIndex(a=>a.content==="sync"))>-1&&tl("COMPILER_V_BIND_SYNC",Xe,We.loc,We.arg.loc.source)&&(We.name="model",We.modifiers.splice(n,1))}(We.type!==7||We.name!=="pre")&&es.props.push(We)}bs="",vn=la=-1},oncomment(e,t){Xe.comments&&qr({type:3,content:qt(e,t),loc:Tt(e-4,t+3)})},onend(){const e=An.length;for(let t=0;t<vt.length;t++)Ml(vt[t],e-1),fn(24,vt[t].loc.start.offset)},oncdata(e,t){(vt[0]?vt[0].ns:Xe.ns)!==0?El(qt(e,t),e,t):fn(1,e-9)},onprocessinginstruction(e){(vt[0]?vt[0].ns:Xe.ns)===0&&fn(21,e-1)}}),pu=/,([^,\}\]]*)(?:,([^,\}\]]*))?$/,Ax=/^\(|\)$/g;function Rx(e){const t=e.loc,s=e.content,n=s.match(Ex);if(!n)return;const[,a,i]=n,l=(u,p,h=!1)=>{const m=t.start.offset+p,v=m+u.length;return Fl(u,!1,Tt(m,v),0,h?1:0)},o={source:l(i.trim(),s.indexOf(i,a.length)),value:void 0,key:void 0,index:void 0,finalized:!1};let r=a.trim().replace(Ax,"").trim();const c=a.indexOf(r),d=r.match(pu);if(d){r=r.replace(pu,"").trim();const u=d[1].trim();let p;if(u&&(p=s.indexOf(u,c+r.length),o.key=l(u,p,!0)),d[2]){const h=d[2].trim();h&&(o.index=l(h,s.indexOf(h,o.key?p+u.length:c+r.length),!0))}}return r&&(o.value=l(r,c,!0)),o}function qt(e,t){return An.slice(e,t)}function fu(e){St.inSFCRoot&&(es.innerLoc=Tt(e+1,e+1)),qr(es);const{tag:t,ns:s}=es;s===0&&Xe.isPreTag(t)&&Qc++,Xe.isVoidTag(t)?Ml(es,e):(vt.unshift(es),(s===1||s===2)&&(St.inXML=!0)),es=null}function El(e,t,s){{const i=vt[0]&&vt[0].tag;i!=="script"&&i!=="style"&&e.includes("&")&&(e=Xe.decodeEntities(e,!1))}const n=vt[0]||nl,a=n.children[n.children.length-1];a&&a.type===2?(a.content+=e,da(a.loc,s)):n.children.push({type:2,content:e,loc:Tt(t,s)})}function Ml(e,t,s=!1){s?da(e.loc,jh(t,60)):da(e.loc,Ix(t,62)+1),St.inSFCRoot&&(e.children.length?e.innerLoc.end=qe({},e.children[e.children.length-1].loc.end):e.innerLoc.end=qe({},e.innerLoc.start),e.innerLoc.source=qt(e.innerLoc.start.offset,e.innerLoc.end.offset));const{tag:n,ns:a,children:i}=e;if(Gn||(n==="slot"?e.tagType=2:hu(e)?e.tagType=3:Lx(e)&&(e.tagType=1)),St.inRCDATA||(e.children=Vh(i)),a===0&&Xe.isIgnoreNewlineTag(n)){const l=i[0];l&&l.type===2&&(l.content=l.content.replace(/^\r?\n/,""))}a===0&&Xe.isPreTag(n)&&Qc--,Vr===e&&(Gn=St.inVPre=!1,Vr=null),St.inXML&&(vt[0]?vt[0].ns:Xe.ns)===0&&(St.inXML=!1);{const l=e.props;if(!St.inSFCRoot&&va("COMPILER_NATIVE_TEMPLATE",Xe)&&e.tag==="template"&&!hu(e)){const r=vt[0]||nl,c=r.children.indexOf(e);r.children.splice(c,1,...e.children)}const o=l.find(r=>r.type===6&&r.name==="inline-template");o&&tl("COMPILER_INLINE_TEMPLATE",Xe,o.loc)&&e.children.length&&(o.value={type:2,content:qt(e.children[0].loc.start.offset,e.children[e.children.length-1].loc.end.offset),loc:o.loc})}}function Ix(e,t){let s=e;for(;An.charCodeAt(s)!==t&&s<An.length-1;)s++;return s}function jh(e,t){let s=e;for(;An.charCodeAt(s)!==t&&s>=0;)s--;return s}const Ox=new Set(["if","else","else-if","for","slot"]);function hu({tag:e,props:t}){if(e==="template"){for(let s=0;s<t.length;s++)if(t[s].type===7&&Ox.has(t[s].name))return!0}return!1}function Lx({tag:e,props:t}){if(Xe.isCustomElement(e))return!1;if(e==="component"||Nx(e.charCodeAt(0))||Ph(e)||Xe.isBuiltInComponent&&Xe.isBuiltInComponent(e)||Xe.isNativeTag&&!Xe.isNativeTag(e))return!0;for(let s=0;s<t.length;s++){const n=t[s];if(n.type===6){if(n.name==="is"&&n.value){if(n.value.content.startsWith("vue:"))return!0;if(tl("COMPILER_IS_ON_ELEMENT",Xe,n.loc))return!0}}else if(n.name==="bind"&&ca(n.arg,"is")&&tl("COMPILER_IS_ON_ELEMENT",Xe,n.loc))return!0}return!1}function Nx(e){return e>64&&e<91}const Dx=/\r\n/g;function Vh(e){const t=Xe.whitespace!=="preserve";let s=!1;for(let n=0;n<e.length;n++){const a=e[n];if(a.type===2)if(Qc)a.content=a.content.replace(Dx,`
`);else if(Uh(a.content)){const i=e[n-1]&&e[n-1].type,l=e[n+1]&&e[n+1].type;!i||!l||t&&(i===3&&(l===3||l===1)||i===1&&(l===3||l===1&&Px(a.content)))?(s=!0,e[n]=null):a.content=" "}else t&&(a.content=qh(a.content))}return s?e.filter(Boolean):e}function Px(e){for(let t=0;t<e.length;t++){const s=e.charCodeAt(t);if(s===10||s===13)return!0}return!1}function qh(e){let t="",s=!1;for(let n=0;n<e.length;n++)As(e.charCodeAt(n))?s||(t+=" ",s=!0):(t+=e[n],s=!1);return t}function qr(e){(vt[0]||nl).children.push(e)}function Tt(e,t){return{start:St.getPos(e),end:t==null?t:St.getPos(t),source:t==null?t:qt(e,t)}}function Mx(e){return Tt(e.start.offset,e.end.offset)}function da(e,t){e.end=St.getPos(t),e.source=qt(e.start.offset,t)}function Fx(e){const t={type:6,name:e.rawName,nameLoc:Tt(e.loc.start.offset,e.loc.start.offset+e.rawName.length),value:void 0,loc:e.loc};if(e.exp){const s=e.exp.loc;s.end.offset<e.loc.end.offset&&(s.start.offset--,s.start.column--,s.end.offset++,s.end.column++),t.value={type:2,content:e.exp.content,loc:s}}return t}function Fl(e,t=!1,s,n=0,a=0){return He(e,t,s,n)}function fn(e,t,s){Xe.onError(gt(e,Tt(t,t)))}function $x(){St.reset(),es=null,We=null,bs="",vn=-1,la=-1,vt.length=0}function Bx(e,t){if($x(),An=e,Xe=qe({},zh),t){let a;for(a in t)t[a]!=null&&(Xe[a]=t[a])}St.mode=Xe.parseMode==="html"?1:Xe.parseMode==="sfc"?2:0,St.inXML=Xe.ns===1||Xe.ns===2;const s=t&&t.delimiters;s&&(St.delimiterOpen=ro(s[0]),St.delimiterClose=ro(s[1]));const n=nl=px([],e);return St.parse(An),n.loc=Tt(0,e.length),n.children=Vh(n.children),nl=null,n}function Ux(e,t){$l(e,void 0,t,!!Gh(e))}function Gh(e){const t=e.children.filter(s=>s.type!==3);return t.length===1&&t[0].type===1&&!uo(t[0])?t[0]:null}function $l(e,t,s,n=!1,a=!1){const{children:i}=e,l=[];for(let d=0;d<i.length;d++){const u=i[d];if(u.type===1&&u.tagType===0){const p=n?0:Rs(u,s);if(p>0){if(p>=2){u.codegenNode.patchFlag=-1,l.push(u);continue}}else{const h=u.codegenNode;if(h.type===13){const m=h.patchFlag;if((m===void 0||m===512||m===1)&&Wh(u,s)>=2){const v=Zh(u);v&&(h.props=s.hoist(v))}h.dynamicProps&&(h.dynamicProps=s.hoist(h.dynamicProps))}}}else if(u.type===12&&(n?0:Rs(u,s))>=2){u.codegenNode.type===14&&u.codegenNode.arguments.length>0&&u.codegenNode.arguments.push("-1"),l.push(u);continue}if(u.type===1){const p=u.tagType===1;p&&s.scopes.vSlot++,$l(u,e,s,!1,a),p&&s.scopes.vSlot--}else if(u.type===11)$l(u,e,s,u.children.length===1,!0);else if(u.type===9)for(let p=0;p<u.branches.length;p++)$l(u.branches[p],e,s,u.branches[p].children.length===1,a)}let o=!1;if(l.length===i.length&&e.type===1){if(e.tagType===0&&e.codegenNode&&e.codegenNode.type===13&&Ce(e.codegenNode.children))e.codegenNode.children=r(ma(e.codegenNode.children)),o=!0;else if(e.tagType===1&&e.codegenNode&&e.codegenNode.type===13&&e.codegenNode.children&&!Ce(e.codegenNode.children)&&e.codegenNode.children.type===15){const d=c(e.codegenNode,"default");d&&(d.returns=r(ma(d.returns)),o=!0)}else if(e.tagType===3&&t&&t.type===1&&t.tagType===1&&t.codegenNode&&t.codegenNode.type===13&&t.codegenNode.children&&!Ce(t.codegenNode.children)&&t.codegenNode.children.type===15){const d=Fs(e,"slot",!0),u=d&&d.arg&&c(t.codegenNode,d.arg);u&&(u.returns=r(ma(u.returns)),o=!0)}}if(!o)for(const d of l)d.codegenNode=s.cache(d.codegenNode);function r(d){const u=s.cache(d);return u.needArraySpread=!0,u}function c(d,u){if(d.children&&!Ce(d.children)&&d.children.type===15){const p=d.children.properties.find(h=>h.key===u||h.key.content===u);return p&&p.value}}l.length&&s.transformHoist&&s.transformHoist(i,s,e)}function Rs(e,t){const{constantCache:s}=t;switch(e.type){case 1:if(e.tagType!==0)return 0;const n=s.get(e);if(n!==void 0)return n;const a=e.codegenNode;if(a.type!==13||a.isBlock&&e.tag!=="svg"&&e.tag!=="foreignObject"&&e.tag!=="math")return 0;if(a.patchFlag===void 0){let l=3;const o=Wh(e,t);if(o===0)return s.set(e,0),0;o<l&&(l=o);for(let r=0;r<e.children.length;r++){const c=Rs(e.children[r],t);if(c===0)return s.set(e,0),0;c<l&&(l=c)}if(l>1)for(let r=0;r<e.props.length;r++){const c=e.props[r];if(c.type===7&&c.name==="bind"&&c.exp){const d=Rs(c.exp,t);if(d===0)return s.set(e,0),0;d<l&&(l=d)}}if(a.isBlock){for(let r=0;r<e.props.length;r++)if(e.props[r].type===7)return s.set(e,0),0;t.removeHelper(ya),t.removeHelper(li(t.inSSR,a.isComponent)),a.isBlock=!1,t.helper(ii(t.inSSR,a.isComponent))}return s.set(e,l),l}else return s.set(e,0),0;case 2:case 3:return 3;case 9:case 11:case 10:return 0;case 5:case 12:return Rs(e.content,t);case 4:return e.constType;case 8:let i=3;for(let l=0;l<e.children.length;l++){const o=e.children[l];if(Be(o)||is(o))continue;const r=Rs(o,t);if(r===0)return 0;r<i&&(i=r)}return i;case 20:return 2;default:return 0}}const Hx=new Set([jc,Vc,Xi,fl]);function Kh(e,t){if(e.type===14&&!Be(e.callee)&&Hx.has(e.callee)){const s=e.arguments[0];if(s.type===4)return Rs(s,t);if(s.type===14)return Kh(s,t)}return 0}function Wh(e,t){let s=3;const n=Zh(e);if(n&&n.type===15){const{properties:a}=n;for(let i=0;i<a.length;i++){const{key:l,value:o}=a[i],r=Rs(l,t);if(r===0)return r;r<s&&(s=r);let c;if(o.type===4?c=Rs(o,t):o.type===14?c=Kh(o,t):c=0,c===0)return c;c<s&&(s=c)}}return s}function Zh(e){const t=e.codegenNode;if(t.type===13)return t.props}function zx(e,{filename:t="",prefixIdentifiers:s=!1,hoistStatic:n=!1,hmr:a=!1,cacheHandlers:i=!1,nodeTransforms:l=[],directiveTransforms:o={},transformHoist:r=null,isBuiltInComponent:c=Wt,isCustomElement:d=Wt,expressionPlugins:u=[],scopeId:p=null,slotted:h=!0,ssr:m=!1,inSSR:v=!1,ssrCssVars:k="",bindingMetadata:R=Ke,inline:y=!1,isTS:g=!1,onError:b=Zc,onWarn:S=Dh,compatConfig:w}){const A=t.replace(/\?.*$/,"").match(/([^/\\]+)\.\w+$/),E={filename:t,selfName:A&&ka(ut(A[1])),prefixIdentifiers:s,hoistStatic:n,hmr:a,cacheHandlers:i,nodeTransforms:l,directiveTransforms:o,transformHoist:r,isBuiltInComponent:c,isCustomElement:d,expressionPlugins:u,scopeId:p,slotted:h,ssr:m,inSSR:v,ssrCssVars:k,bindingMetadata:R,inline:y,isTS:g,onError:b,onWarn:S,compatConfig:w,root:e,helpers:new Map,components:new Set,directives:new Set,hoists:[],imports:[],cached:[],constantCache:new WeakMap,vForMemoKeyedNodes:new WeakSet,temps:0,identifiers:Object.create(null),scopes:{vFor:0,vSlot:0,vPre:0,vOnce:0},parent:null,grandParent:null,currentNode:e,childIndex:0,inVOnce:!1,helper(x){const N=E.helpers.get(x)||0;return E.helpers.set(x,N+1),x},removeHelper(x){const N=E.helpers.get(x);if(N){const $=N-1;$?E.helpers.set(x,$):E.helpers.delete(x)}},helperString(x){return`_${ni[E.helper(x)]}`},replaceNode(x){E.parent.children[E.childIndex]=E.currentNode=x},removeNode(x){const N=E.parent.children,$=x?N.indexOf(x):E.currentNode?E.childIndex:-1;!x||x===E.currentNode?(E.currentNode=null,E.onNodeRemoved()):E.childIndex>$&&(E.childIndex--,E.onNodeRemoved()),E.parent.children.splice($,1)},onNodeRemoved:Wt,addIdentifiers(x){},removeIdentifiers(x){},hoist(x){Be(x)&&(x=He(x)),E.hoists.push(x);const N=He(`_hoisted_${E.hoists.length}`,!1,x.loc,2);return N.hoisted=x,N},cache(x,N=!1,$=!1){const C=fx(E.cached.length,x,N,$);return E.cached.push(C),C}};return E.filters=new Set,E}function jx(e,t){const s=zx(e,t);Bo(e,s),t.hoistStatic&&Ux(e,s),t.ssr||Vx(e,s),e.helpers=new Set([...s.helpers.keys()]),e.components=[...s.components],e.directives=[...s.directives],e.imports=s.imports,e.hoists=s.hoists,e.temps=s.temps,e.cached=s.cached,e.transformed=!0,e.filters=[...s.filters]}function Vx(e,t){const{helper:s}=t,{children:n}=e;if(n.length===1){const a=Gh(e);if(a&&a.codegenNode){const i=a.codegenNode;i.type===13&&Wc(i,t),e.codegenNode=i}else e.codegenNode=n[0]}else if(n.length>1){let a=64;e.codegenNode=el(t,s(Qi),void 0,e.children,a,void 0,void 0,!0,void 0,!1)}}function qx(e,t){let s=0;const n=()=>{s--};for(;s<e.children.length;s++){const a=e.children[s];Be(a)||(t.grandParent=t.parent,t.parent=e,t.childIndex=s,t.onNodeRemoved=n,Bo(a,t))}}function Bo(e,t){t.currentNode=e;const{nodeTransforms:s}=t,n=[];for(let i=0;i<s.length;i++){const l=s[i](e,t);if(l&&(Ce(l)?n.push(...l):n.push(l)),t.currentNode)e=t.currentNode;else return}switch(e.type){case 3:t.ssr||t.helper(pl);break;case 5:t.ssr||t.helper(Fo);break;case 9:for(let i=0;i<e.branches.length;i++)Bo(e.branches[i],t);break;case 10:case 11:case 1:case 0:qx(e,t);break}t.currentNode=e;let a=n.length;for(;a--;)n[a]()}function Jh(e,t){const s=Be(e)?n=>n===e:n=>e.test(n);return(n,a)=>{if(n.type===1){const{props:i}=n;if(n.tagType===3&&i.some(Sx))return;const l=[];for(let o=0;o<i.length;o++){const r=i[o];if(r.type===7&&s(r.name)){i.splice(o,1),o--;const c=t(n,r,a);c&&l.push(c)}}return l}}}const Uo="/*@__PURE__*/",Yh=e=>`${ni[e]}: _${ni[e]}`;function Gx(e,{mode:t="function",prefixIdentifiers:s=t==="module",sourceMap:n=!1,filename:a="template.vue.html",scopeId:i=null,optimizeImports:l=!1,runtimeGlobalName:o="Vue",runtimeModuleName:r="vue",ssrRuntimeModuleName:c="vue/server-renderer",ssr:d=!1,isTS:u=!1,inSSR:p=!1}){const h={mode:t,prefixIdentifiers:s,sourceMap:n,filename:a,scopeId:i,optimizeImports:l,runtimeGlobalName:o,runtimeModuleName:r,ssrRuntimeModuleName:c,ssr:d,isTS:u,inSSR:p,source:e.source,code:"",column:1,line:1,offset:0,indentLevel:0,pure:!1,map:void 0,helper(v){return`_${ni[v]}`},push(v,k=-2,R){h.code+=v},indent(){m(++h.indentLevel)},deindent(v=!1){v?--h.indentLevel:m(--h.indentLevel)},newline(){m(h.indentLevel)}};function m(v){h.push(`
...
`,0),r()),d||a("return "),e.codegenNode?as(e.codegenNode,s):a("null"),h&&(o(),a("}")),o(),a("}"),{ast:e,code:s.code,preamble:"",map:s.map?s.map.toJSON():void 0}}function Wx(e,t){const{ssr:s,prefixIdentifiers:n,push:a,newline:i,runtimeModuleName:l,runtimeGlobalName:o,ssrRuntimeModuleName:r}=t,c=o,d=Array.from(e.helpers);if(d.length>0&&(a(`const _Vue = ${c}
`,-1),e.hoists.length)){const u=[Dc,Pc,pl,Mc,Ih].filter(p=>d.includes(p)).map(Yh).join(", ");a(`const { ${u} } = _Vue
`,-1)}Zx(e.hoists,t),i(),a("return ")}function lr(e,t,{helper:s,push:n,newline:a,isTS:i}){const l=s(t==="filter"?Uc:t==="component"?Fc:Bc);for(let o=0;o<e.length;o++){let r=e[o];const c=r.endsWith("__self");c&&(r=r.slice(0,-6)),n(`const ${sl(r,t)} = ${l}(${JSON.stringify(r)}${c?", true":""})${i?"!":""}`),o<e.length-1&&a()}}function Zx(e,t){if(!e.length)return;t.pure=!0;const{push:s,newline:n}=t;n();for(let a=0;a<e.length;a++){const i=e[a];i&&(s(`const _hoisted_${a+1} = `),as(i,t),n())}t.pure=!1}function Xc(e,t){const s=e.length>3||!1;t.push("["),s&&t.indent(),hl(e,t,s),s&&t.deindent(),t.push("]")}function hl(e,t,s=!1,n=!0){const{push:a,newline:i}=t;for(let l=0;l<e.length;l++){const o=e[l];Be(o)?a(o,-3):Ce(o)?Xc(o,t):as(o,t),l<e.length-1&&(s?(n&&a(","),i()):n&&a(", "))}}function as(e,t){if(Be(e)){t.push(e,-3);return}if(is(e)){t.push(t.helper(e));return}switch(e.type){case 1:case 9:case 11:as(e.codegenNode,t);break;case 2:Jx(e,t);break;case 4:Qh(e,t);break;case 5:Yx(e,t);break;case 12:as(e.codegenNode,t);break;case 8:Xh(e,t);break;case 3:Xx(e,t);break;case 13:e0(e,t);break;case 14:s0(e,t);break;case 15:n0(e,t);break;case 17:a0(e,t);break;case 18:i0(e,t);break;case 19:l0(e,t);break;case 20:o0(e,t);break;case 21:hl(e.body,t,!0,!1);break}}function Jx(e,t){t.push(JSON.stringify(e.content),-3,e)}function Qh(e,t){const{content:s,isStatic:n}=e;t.push(n?JSON.stringify(s):s,-3,e)}function Yx(e,t){const{push:s,helper:n,pure:a}=t;a&&s(Uo),s(`${n(Fo)}(`),as(e.content,t),s(")")}function Xh(e,t){for(let s=0;s<e.children.length;s++){const n=e.children[s];Be(n)?t.push(n,-3):as(n,t)}}function Qx(e,t){const{push:s}=t;if(e.type===8)s("["),Xh(e,t),s("]");else if(e.isStatic){const n=Jc(e.content)?e.content:JSON.stringify(e.content);s(n,-2,e)}else s(`[${e.content}]`,-3,e)}function Xx(e,t){const{push:s,helper:n,pure:a}=t;a&&s(Uo),s(`${n(pl)}(${JSON.stringify(e.content)})`,-3,e)}function e0(e,t){const{push:s,helper:n,pure:a}=t,{tag:i,props:l,children:o,patchFlag:r,dynamicProps:c,directives:d,isBlock:u,disableTracking:p,isComponent:h}=e;let m;r&&(m=String(r)),d&&s(n(Hc)+"("),u&&s(`(${n(ya)}(${p?"true":""}), `),a&&s(Uo);const v=u?li(t.inSSR,h):ii(t.inSSR,h);s(n(v)+"(",-2,e),hl(t0([i,l,o,m,c]),t),s(")"),u&&s(")"),d&&(s(", "),as(d,t),s(")"))}function t0(e){let t=e.length;for(;t--&&e[t]==null;);return e.slice(0,t+1).map(s=>s||"null")}function s0(e,t){const{push:s,helper:n,pure:a}=t,i=Be(e.callee)?e.callee:n(e.callee);a&&s(Uo),s(i+"(",-2,e),hl(e.arguments,t),s(")")}function n0(e,t){const{push:s,indent:n,deindent:a,newline:i}=t,{properties:l}=e;if(!l.length){s("{}",-2,e);return}const o=l.length>1||!1;s(o?"{":"{ "),o&&n();for(let r=0;r<l.length;r++){const{key:c,value:d}=l[r];Qx(c,t),s(": "),as(d,t),r<l.length-1&&(s(","),i())}o&&a(),s(o?"}":" }")}function a0(e,t){Xc(e.elements,t)}function i0(e,t){const{push:s,indent:n,deindent:a}=t,{params:i,returns:l,body:o,newline:r,isSlot:c}=e;c&&s(`_${ni[Gc]}(`),s("(",-2,e),Ce(i)?hl(i,t):i&&as(i,t),s(") => "),(r||o)&&(s("{"),n()),l?(r&&s("return "),Ce(l)?Xc(l,t):as(l,t)):o&&as(o,t),(r||o)&&(a(),s("}")),c&&(e.isNonScopedSlot&&s(", undefined, true"),s(")"))}function l0(e,t){const{test:s,consequent:n,alternate:a,newline:i}=e,{push:l,indent:o,deindent:r,newline:c}=t;if(s.type===4){const u=!Jc(s.content);u&&l("("),Qh(s,t),u&&l(")")}else l("("),as(s,t),l(")");i&&o(),t.indentLevel++,i||l(" "),l("? "),as(n,t),t.indentLevel--,i&&c(),i||l(" "),l(": ");const d=a.type===19;d||t.indentLevel++,as(a,t),d||t.indentLevel--,i&&r(!0)}function o0(e,t){const{push:s,helper:n,indent:a,deindent:i,newline:l}=t,{needPauseTracking:o,needArraySpread:r}=e;r&&s("[...("),s(`_cache[${e.index}] || (`),o&&(a(),s(`${n(oo)}(-1`),e.inVOnce&&s(", true"),s("),"),l(),s("(")),s(`_cache[${e.index}] = `),as(e.value,t),o&&(s(`).cacheIndex = ${e.index},`),l(),s(`${n(oo)}(1),`),l(),s(`_cache[${e.index}]`),i()),s(")"),r&&s(")]")}new RegExp("\\b"+"arguments,await,break,case,catch,class,const,continue,debugger,default,delete,do,else,export,extends,finally,for,function,if,import,let,new,return,super,switch,throw,try,var,void,while,with,yield".split(",").join("\\b|\\b")+"\\b");const r0=Jh(/^(?:if|else|else-if)$/,(e,t,s)=>c0(e,t,s,(n,a,i)=>{const l=s.parent.children;let o=l.indexOf(n),r=0;for(;o-->=0;){const c=l[o];c&&c.type===9&&(r+=c.branches.length)}return()=>{if(i)n.codegenNode=vu(a,r,s);else{const c=d0(n.codegenNode);c.alternate=vu(a,r+n.branches.length-1,s)}}}));function c0(e,t,s,n){if(t.name!=="else"&&(!t.exp||!t.exp.content.trim())){const a=t.exp?t.exp.loc:e.loc;s.onError(gt(28,t.loc)),t.exp=He("true",!1,a)}if(t.name==="if"){const a=mu(e,t),i={type:9,loc:Mx(e.loc),branches:[a]};if(s.replaceNode(i),n)return n(i,a,!0)}else{const a=s.parent.children;let i=a.indexOf(e);for(;i-->=-1;){const l=a[i];if(l&&Hh(l)){s.removeNode(l);continue}if(l&&l.type===9){(t.name==="else-if"||t.name==="else")&&l.branches[l.branches.length-1].condition===void 0&&s.onError(gt(30,e.loc)),s.removeNode();const o=mu(e,t);l.branches.push(o);const r=n&&n(l,o,!1);Bo(o,s),r&&r(),s.currentNode=null}else s.onError(gt(30,e.loc));break}}}function mu(e,t){const s=e.tagType===3;return{type:10,loc:e.loc,condition:t.name==="else"?void 0:t.exp,children:s&&!Fs(e,"for")?e.children:[e],userKey:$o(e,"key"),isTemplateIf:s}}function vu(e,t,s){return e.condition?jr(e.condition,gu(e,t,s),Bt(s.helper(pl),['""',"true"])):gu(e,t,s)}function gu(e,t,s){const{helper:n}=s,a=Ot("key",He(`${t}`,!1,Ls,2)),{children:i}=e,l=i[0];if(i.length!==1||l.type!==1)if(i.length===1&&l.type===11){const r=l.codegenNode;return po(r,a,s),r}else return el(s,n(Qi),$s([a]),i,64,void 0,void 0,!0,!1,!1,e.loc);else{const r=l.codegenNode,c=Cx(r);return c.type===13&&Wc(c,s),po(c,a,s),r}}function d0(e){for(;;)if(e.type===19)if(e.alternate.type===19)e=e.alternate;else return e;else e.type===20&&(e=e.value)}const u0=Jh("for",(e,t,s)=>{const{helper:n,removeHelper:a}=s;return p0(e,t,s,i=>{const l=Bt(n(zc),[i.source]),o=co(e),r=Fs(e,"memo"),c=$o(e,"key",!1,!0);c&&c.type;let d=c&&(c.type===6?c.value?He(c.value.content,!0):void 0:c.exp);const u=d?Ot("key",d):null,p=i.source.type===4&&i.source.constType>0,h=p?64:c?128:256;return i.codegenNode=el(s,n(Qi),void 0,l,h,void 0,void 0,!0,!p,!1,e.loc),()=>{let m;const{children:v}=i,k=v.length!==1||v[0].type!==1,R=uo(e)?e:o&&e.children.length===1&&uo(e.children[0])?e.children[0]:null;if(R?(m=R.codegenNode,o&&u&&po(m,u,s)):k?m=el(s,n(Qi),u?$s([u]):void 0,e.children,64,void 0,void 0,!0,void 0,!1):(m=v[0].codegenNode,o&&u&&po(m,u,s),m.isBlock!==!p&&(m.isBlock?(a(ya),a(li(s.inSSR,m.isComponent))):a(ii(s.inSSR,m.isComponent))),m.isBlock=!p,m.isBlock?(n(ya),n(li(s.inSSR,m.isComponent))):n(ii(s.inSSR,m.isComponent))),r){const y=ai(Gr(i.parseResult,[He("_cached")]));y.body=hx([Gs(["const _memo = (",r.exp,")"]),Gs(["if (_cached && _cached.el",...d?[" && _cached.key === ",d]:[],` && ${s.helperString(Nh)}(_cached, _memo)) return _cached`]),Gs(["const _item = ",m]),He("_item.memo = _memo"),He("return _item")]),l.arguments.push(y,He("_cache"),He(String(s.cached.length))),s.cached.push(null)}else l.arguments.push(ai(Gr(i.parseResult),m,!0))}})});function p0(e,t,s,n){if(!t.exp){s.onError(gt(31,t.loc));return}const a=t.forParseResult;if(!a){s.onError(gt(32,t.loc));return}em(a);const{addIdentifiers:i,removeIdentifiers:l,scopes:o}=s,{source:r,value:c,key:d,index:u}=a,p={type:11,loc:t.loc,source:r,valueAlias:c,keyAlias:d,objectIndexAlias:u,parseResult:a,children:co(e)?e.children:[e]};s.replaceNode(p),o.vFor++;const h=n&&n(p);return()=>{o.vFor--,h&&h()}}function em(e,t){e.finalized||(e.finalized=!0)}function Gr({value:e,key:t,index:s},n=[]){return f0([e,t,s,...n])}function f0(e){let t=e.length;for(;t--&&!e[t];);return e.slice(0,t+1).map((s,n)=>s||He("_".repeat(n+1),!1))}const bu=He("undefined",!1),h0=(e,t)=>{if(e.type===1&&(e.tagType===1||e.tagType===3)){const s=Fs(e,"slot");if(s)return s.exp,t.scopes.vSlot++,()=>{t.scopes.vSlot--}}},m0=(e,t,s,n)=>ai(e,s,!1,!0,s.length?s[0].loc:n);function v0(e,t,s=m0){t.helper(Gc);const{children:n,loc:a}=e,i=[],l=[];let o=t.scopes.vSlot>0||t.scopes.vFor>0;const r=Fs(e,"slot",!0);if(r){const{arg:k,exp:R}=r;k&&!ks(k)&&(o=!0),i.push(Ot(k||He("default",!0),s(R,void 0,n,a)))}let c=!1,d=!1;const u=[],p=new Set;let h=0;for(let k=0;k<n.length;k++){const R=n[k];let y;if(!co(R)||!(y=Fs(R,"slot",!0))){R.type!==3&&u.push(R);continue}if(r){t.onError(gt(37,y.loc));break}c=!0;const{children:g,loc:b}=R,{arg:S=He("default",!0),exp:w,loc:A}=y;let E;ks(S)?E=S?S.content:"default":o=!0;const x=Fs(R,"for"),N=s(w,x,g,b);let $,C;if($=Fs(R,"if"))o=!0,l.push(jr($.exp,Al(S,N,h++),bu));else if(C=Fs(R,/^else(?:-if)?$/,!0)){let M=k,V;for(;M--&&(V=n[M],!!Hh(V)););if(V&&co(V)&&Fs(V,/^(?:else-)?if$/)){let q=l[l.length-1];for(;q.alternate.type===19;)q=q.alternate;q.alternate=C.exp?jr(C.exp,Al(S,N,h++),bu):Al(S,N,h++)}else t.onError(gt(30,C.loc))}else if(x){o=!0;const M=x.forParseResult;M?(em(M),l.push(Bt(t.helper(zc),[M.source,ai(Gr(M),Al(S,N),!0)]))):t.onError(gt(32,x.loc))}else{if(E){if(p.has(E)){t.onError(gt(38,A));continue}p.add(E),E==="default"&&(d=!0)}i.push(Ot(S,N))}}if(!r){const k=(R,y)=>{const g=s(R,void 0,y,a);return t.compatConfig&&(g.isNonScopedSlot=!0),Ot("default",g)};c?u.length&&!u.every(Yc)&&(d?t.onError(gt(39,u[0].loc)):i.push(k(void 0,u))):i.push(k(void 0,n))}const m=o?2:Bl(e.children)?3:1;let v=$s(i.concat(Ot("_",He(m+"",!1))),a);return l.length&&(v=Bt(t.helper(Lh),[v,ma(l)])),{slots:v,hasDynamicSlots:o}}function Al(e,t,s){const n=[Ot("name",e),Ot("fn",t)];return s!=null&&n.push(Ot("key",He(String(s),!0))),$s(n)}function Bl(e){for(let t=0;t<e.length;t++){const s=e[t];switch(s.type){case 1:if(s.tagType===2||Bl(s.children))return!0;break;case 9:if(Bl(s.branches))return!0;break;case 10:case 11:if(Bl(s.children))return!0;break}}return!1}const tm=new WeakMap,g0=(e,t)=>function(){if(e=t.currentNode,!(e.type===1&&(e.tagType===0||e.tagType===1)))return;const{tag:n,props:a}=e,i=e.tagType===1;let l=i?b0(e,t):`"${n}"`;const o=et(l)&&l.callee===$c;let r,c,d=0,u,p,h,m=o||l===Mi||l===Nc||!i&&(n==="svg"||n==="foreignObject"||n==="math");if(a.length>0){const v=sm(e,t,void 0,i,o);r=v.props,d=v.patchFlag,p=v.dynamicPropNames;const k=v.directives;h=k&&k.length?ma(k.map(R=>x0(R,t))):void 0,v.shouldUseBlock&&(m=!0)}if(e.children.length>0)if(l===io&&(m=!0,d|=1024),i&&l!==Mi&&l!==io){const{slots:k,hasDynamicSlots:R}=v0(e,t);c=k,R&&(d|=1024)}else if(e.children.length===1&&l!==Mi){const k=e.children[0],R=k.type,y=R===5||R===8;y&&Rs(k,t)===0&&(d|=1),y||R===2?c=k:c=e.children}else c=e.children;p&&p.length&&(u=_0(p)),e.codegenNode=el(t,l,r,c,d===0?void 0:d,u,h,!!m,!1,i,e.loc)};function b0(e,t,s=!1){let{tag:n}=e;const a=Kr(n),i=$o(e,"is",!1,!0);if(i)if(a||va("COMPILER_IS_ON_ELEMENT",t)){let o;if(i.type===6?o=i.value&&He(i.value.content,!0):(o=i.exp,o||(o=He("is",!1,i.arg.loc))),o)return Bt(t.helper($c),[o])}else i.type===6&&i.value.content.startsWith("vue:")&&(n=i.value.content.slice(4));const l=Ph(n)||t.isBuiltInComponent(n);return l?(s||t.helper(l),l):(t.helper(Fc),t.components.add(n),sl(n,"component"))}function sm(e,t,s=e.props,n,a,i=!1){const{tag:l,loc:o,children:r}=e;let c=[];const d=[],u=[],p=r.length>0;let h=!1,m=0,v=!1,k=!1,R=!1,y=!1,g=!1,b=!1;const S=[],w=N=>{c.length&&(d.push($s(yu(c),o)),c=[]),N&&d.push(N)},A=()=>{t.scopes.vFor>0&&c.push(Ot(He("ref_for",!0),He("true")))},E=({key:N,value:$})=>{if(ks(N)){const C=N.content,M=_a(C);if(M&&(!n||a)&&C.toLowerCase()!=="onclick"&&C!=="onUpdate:modelValue"&&!Tn(C)&&(y=!0),M&&Tn(C)&&(b=!0),M&&$.type===14&&($=$.arguments[0]),$.type===20||($.type===4||$.type===8)&&Rs($,t)>0)return;C==="ref"?v=!0:C==="class"?k=!0:C==="style"?R=!0:C!=="key"&&!S.includes(C)&&S.push(C),n&&(C==="class"||C==="style")&&!S.includes(C)&&S.push(C)}else g=!0};for(let N=0;N<s.length;N++){const $=s[N];if($.type===6){const{loc:C,name:M,nameLoc:V,value:q}=$;let D=!0;if(M==="ref"&&(v=!0,A()),M==="is"&&(Kr(l)||q&&q.content.startsWith("vue:")||va("COMPILER_IS_ON_ELEMENT",t)))continue;c.push(Ot(He(M,!0,V),He(q?q.content:"",D,q?q.loc:C)))}else{const{name:C,arg:M,exp:V,loc:q,modifiers:D}=$,I=C==="bind",O=C==="on";if(C==="slot"){n||t.onError(gt(40,q));continue}if(C==="once"||C==="memo"||C==="is"||I&&ca(M,"is")&&(Kr(l)||va("COMPILER_IS_ON_ELEMENT",t))||O&&i)continue;if((I&&ca(M,"key")||O&&p&&ca(M,"vue:before-update"))&&(h=!0),I&&ca(M,"ref")&&A(),!M&&(I||O)){if(g=!0,V)if(I){if(w(),va("COMPILER_V_BIND_OBJECT_ORDER",t)){d.unshift(V);continue}A(),w(),d.push(V)}else w({type:14,loc:q,callee:t.helper(qc),arguments:n?[V]:[V,"true"]});else t.onError(gt(I?34:35,q));continue}I&&D.some(W=>W.content==="prop")&&(m|=32);const U=t.directiveTransforms[C];if(U){const{props:W,needRuntime:K}=U($,e,t);!i&&W.forEach(E),O&&M&&!ks(M)?w($s(W,o)):c.push(...W),K&&(u.push($),is(K)&&tm.set($,K))}else uv(C)||(u.push($),p&&(h=!0))}}let x;if(d.length?(w(),d.length>1?x=Bt(t.helper(lo),d,o):x=d[0]):c.length&&(x=$s(yu(c),o)),g?m|=16:(k&&!n&&(m|=2),R&&!n&&(m|=4),S.length&&(m|=8),y&&(m|=32)),!h&&(m===0||m===32)&&(v||b||u.length>0)&&(m|=512),!t.inSSR&&x)switch(x.type){case 15:let N=-1,$=-1,C=!1;for(let q=0;q<x.properties.length;q++){const D=x.properties[q].key;ks(D)?D.content==="class"?N=q:D.content==="style"&&($=q):D.isHandlerKey||(C=!0)}const M=x.properties[N],V=x.properties[$];C?x=Bt(t.helper(Xi),[x]):(M&&!ks(M.value)&&(M.value=Bt(t.helper(jc),[M.value])),V&&(R||V.value.type===4&&V.value.content.trim()[0]==="["||V.value.type===17)&&(V.value=Bt(t.helper(Vc),[V.value])));break;case 14:break;default:x=Bt(t.helper(Xi),[Bt(t.helper(fl),[x])]);break}return{props:x,directives:u,patchFlag:m,dynamicPropNames:S,shouldUseBlock:h}}function yu(e){const t=new Map,s=[];for(let n=0;n<e.length;n++){const a=e[n];if(a.key.type===8||!a.key.isStatic){s.push(a);continue}const i=a.key.content,l=t.get(i);l?(i==="style"||i==="class"||_a(i))&&y0(l,a):(t.set(i,a),s.push(a))}return s}function y0(e,t){e.value.type===17?e.value.elements.push(t.value):e.value=ma([e.value,t.value],e.loc)}function x0(e,t){const s=[],n=tm.get(e);n?s.push(t.helperString(n)):(t.helper(Bc),t.directives.add(e.name),s.push(sl(e.name,"directive")));const{loc:a}=e;if(e.exp&&s.push(e.exp),e.arg&&(e.exp||s.push("void 0"),s.push(e.arg)),Object.keys(e.modifiers).length){e.arg||(e.exp||s.push("void 0"),s.push("void 0"));const i=He("true",!1,a);s.push($s(e.modifiers.map(l=>Ot(l,i)),a))}return ma(s,e.loc)}function _0(e){let t="[";for(let s=0,n=e.length;s<n;s++)t+=JSON.stringify(e[s]),s<n-1&&(t+=", ");return t+"]"}function Kr(e){return e==="component"||e==="Component"}const w0=(e,t)=>{if(uo(e)){const{children:s,loc:n}=e,{slotName:a,slotProps:i}=k0(e,t),l=[t.prefixIdentifiers?"_ctx.$slots":"$slots",a,"{}","undefined","true"];let o=2;i&&(l[2]=i,o=3),s.length&&(l[3]=ai([],s,!1,!1,n),o=4),t.scopeId&&!t.slotted&&(o=5),l.splice(o),e.codegenNode=Bt(t.helper(Oh),l,n)}};function k0(e,t){let s='"default"',n;const a=[];for(let i=0;i<e.props.length;i++){const l=e.props[i];if(l.type===6)l.value&&(l.name==="name"?s=JSON.stringify(l.value.content):(l.name=ut(l.name),a.push(l)));else if(l.name==="bind"&&ca(l.arg,"name")){if(l.exp)s=l.exp;else if(l.arg&&l.arg.type===4){const o=ut(l.arg.content);s=l.exp=He(o,!1,l.arg.loc)}}else l.name==="bind"&&l.arg&&ks(l.arg)&&(l.arg.content=ut(l.arg.content)),a.push(l)}if(a.length>0){const{props:i,directives:l}=sm(e,t,a,!1,!1);n=i,l.length&&t.onError(gt(36,l[0].loc))}return{slotName:s,slotProps:n}}const nm=(e,t,s,n)=>{const{loc:a,modifiers:i,arg:l}=e;!e.exp&&!i.length&&s.onError(gt(35,a));let o;if(l.type===4)if(l.isStatic){let u=l.content;u.startsWith("vue:")&&(u=`vnode-${u.slice(4)}`);const p=t.tagType!==0||u.startsWith("vnode")||!/[A-Z]/.test(u)?Va(ut(u)):`on:${u}`;o=He(p,!0,l.loc)}else o=Gs([`${s.helperString(zr)}(`,l,")"]);else o=l,o.children.unshift(`${s.helperString(zr)}(`),o.children.push(")");let r=e.exp;r&&!r.content.trim()&&(r=void 0);let c=s.cacheHandlers&&!r&&!s.inVOnce;if(r){const u=$h(r),p=!(u||wx(r)),h=r.content.includes(";");(p||c&&u)&&(r=Gs([`${p?"$event":"(...args)"} => ${h?"{":"("}`,r,h?"}":")"]))}let d={props:[Ot(o,r||He("() => {}",!1,a))]};return n&&(d=n(d)),c&&(d.props[0].value=s.cache(d.props[0].value)),d.props.forEach(u=>u.key.isHandlerKey=!0),d},S0=(e,t,s)=>{const{modifiers:n,loc:a}=e,i=e.arg;let{exp:l}=e;return l&&l.type===4&&!l.content.trim()&&(l=void 0),i.type!==4?(i.children.unshift("("),i.children.push(') || ""')):i.isStatic||(i.content=i.content?`${i.content} || ""`:'""'),n.some(o=>o.content==="camel")&&(i.type===4?i.isStatic?i.content=ut(i.content):i.content=`${s.helperString(Hr)}(${i.content})`:(i.children.unshift(`${s.helperString(Hr)}(`),i.children.push(")"))),s.inSSR||(n.some(o=>o.content==="prop")&&xu(i,"."),n.some(o=>o.content==="attr")&&xu(i,"^")),{props:[Ot(i,l)]}},xu=(e,t)=>{e.type===4?e.isStatic?e.content=t+e.content:e.content=`\`${t}\${${e.content}}\``:(e.children.unshift(`'${t}' + (`),e.children.push(")"))},T0=(e,t)=>{if(e.type===0||e.type===1||e.type===11||e.type===10)return()=>{const s=e.children;let n,a=!1;for(let i=0;i<s.length;i++){const l=s[i];if(ir(l)){a=!0;for(let o=i+1;o<s.length;o++){const r=s[o];if(ir(r))n||(n=s[i]=Gs([l],l.loc)),n.children.push(" + ",r),s.splice(o,1),o--;else{n=void 0;break}}}}if(!(!a||s.length===1&&(e.type===0||e.type===1&&e.tagType===0&&!e.props.find(i=>i.type===7&&!t.directiveTransforms[i.name])&&e.tag!=="template")))for(let i=0;i<s.length;i++){const l=s[i];if(ir(l)||l.type===8){const o=[];(l.type!==2||l.content!==" ")&&o.push(l),!t.ssr&&Rs(l,t)===0&&o.push("1"),s[i]={type:12,content:l,loc:l.loc,codegenNode:Bt(t.helper(Mc),o)}}}}},_u=new WeakSet,C0=(e,t)=>{if(e.type===1&&Fs(e,"once",!0))return _u.has(e)||t.inVOnce||t.inSSR?void 0:(_u.add(e),t.inVOnce=!0,t.helper(oo),()=>{t.inVOnce=!1;const s=t.currentNode;s.codegenNode&&(s.codegenNode=t.cache(s.codegenNode,!0,!0))})},am=(e,t,s)=>{const{exp:n,arg:a}=e;if(!n)return s.onError(gt(41,e.loc)),yi();const i=n.loc.source.trim(),l=n.type===4?n.content:i,o=s.bindingMetadata[i];if(o==="props"||o==="props-aliased")return s.onError(gt(44,n.loc)),yi();if(o==="literal-const"||o==="setup-const")return s.onError(gt(45,n.loc)),yi();if(!l.trim()||!$h(n))return s.onError(gt(42,n.loc)),yi();const r=a||He("modelValue",!0),c=a?ks(a)?`onUpdate:${ut(a.content)}`:Gs(['"onUpdate:" + ',a]):"onUpdate:modelValue";let d;const u=s.isTS?"($event: any)":"$event";d=Gs([`${u} => ((`,n,") = $event)"]);const p=[Ot(r,e.exp),Ot(c,d)];if(e.modifiers.length&&t.tagType===1){const h=e.modifiers.map(v=>v.content).map(v=>(Jc(v)?v:JSON.stringify(v))+": true").join(", "),m=a?ks(a)?`${a.content}Modifiers`:Gs([a,' + "Modifiers"']):"modelModifiers";p.push(Ot(m,He(`{ ${h} }`,!1,e.loc,2)))}return yi(p)};function yi(e=[]){return{props:e}}const E0=/[\w).+\-_$\]]/,A0=(e,t)=>{va("COMPILER_FILTERS",t)&&(e.type===5?fo(e.content,t):e.type===1&&e.props.forEach(s=>{s.type===7&&s.name!=="for"&&s.exp&&fo(s.exp,t)}))};function fo(e,t){if(e.type===4)wu(e,t);else for(let s=0;s<e.children.length;s++){const n=e.children[s];typeof n=="object"&&(n.type===4?wu(n,t):n.type===8?fo(e,t):n.type===5&&fo(n.content,t))}}function wu(e,t){const s=e.content;let n=!1,a=!1,i=!1,l=!1,o=0,r=0,c=0,d=0,u,p,h,m,v=[];for(h=0;h<s.length;h++)if(p=u,u=s.charCodeAt(h),n)u===39&&p!==92&&(n=!1);else if(a)u===34&&p!==92&&(a=!1);else if(i)u===96&&p!==92&&(i=!1);else if(l)u===47&&p!==92&&(l=!1);else if(u===124&&s.charCodeAt(h+1)!==124&&s.charCodeAt(h-1)!==124&&!o&&!r&&!c)m===void 0?(d=h+1,m=s.slice(0,h).trim()):k();else{switch(u){case 34:a=!0;break;case 39:n=!0;break;case 96:i=!0;break;case 40:c++;break;case 41:c--;break;case 91:r++;break;case 93:r--;break;case 123:o++;break;case 125:o--;break}if(u===47){let R=h-1,y;for(;R>=0&&(y=s.charAt(R),y===" ");R--);(!y||!E0.test(y))&&(l=!0)}}m===void 0?m=s.slice(0,h).trim():d!==0&&k();function k(){v.push(s.slice(d,h).trim()),d=h+1}if(v.length){for(h=0;h<v.length;h++)m=R0(m,v[h],t);e.content=m,e.ast=void 0}}function R0(e,t,s){s.helper(Uc);const n=t.indexOf("(");if(n<0)return s.filters.add(t),`${sl(t,"filter")}(${e})`;{const a=t.slice(0,n),i=t.slice(n+1);return s.filters.add(a),`${sl(a,"filter")}(${e}${i!==")"?","+i:i}`}}const ku=new WeakSet,I0=(e,t)=>{if(e.type===1){const s=Fs(e,"memo");return!s||ku.has(e)||t.inSSR?void 0:(ku.add(e),()=>{const n=e.codegenNode||t.currentNode.codegenNode;n&&n.type===13&&(e.tagType!==1&&Wc(n,t),e.codegenNode=Bt(t.helper(Kc),[s.exp,ai(void 0,n),"_cache",String(t.cached.length)]),t.cached.push(null))})}},O0=(e,t)=>{if(e.type===1){for(const s of e.props)if(s.type===7&&s.name==="bind"&&(!s.exp||s.exp.type===4&&!s.exp.content.trim())&&s.arg){const n=s.arg;if(n.type!==4||!n.isStatic)t.onError(gt(53,n.loc)),s.exp=He("",!0,n.loc);else{const a=ut(n.content);(Mh.test(a[0])||a[0]==="-")&&(s.exp=He(a,!1,n.loc))}}}};function L0(e){return[[O0,C0,r0,I0,u0,A0,w0,g0,h0,T0],{on:nm,bind:S0,model:am}]}function N0(e,t={}){const s=t.onError||Zc,n=t.mode==="module";t.prefixIdentifiers===!0?s(gt(48)):n&&s(gt(49));const a=!1;t.cacheHandlers&&s(gt(50)),t.scopeId&&!n&&s(gt(51));const i=qe({},t,{prefixIdentifiers:a}),l=Be(e)?Bx(e,i):e,[o,r]=L0();return jx(l,qe({},i,{nodeTransforms:[...o,...t.nodeTransforms||[]],directiveTransforms:qe({},r,t.directiveTransforms||{})})),Kx(l,i)}const D0=()=>({props:[]});/**
* @vue/compiler-dom v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const im=Symbol(""),lm=Symbol(""),om=Symbol(""),rm=Symbol(""),Wr=Symbol(""),cm=Symbol(""),dm=Symbol(""),um=Symbol(""),pm=Symbol(""),fm=Symbol("");ux({[im]:"vModelRadio",[lm]:"vModelCheckbox",[om]:"vModelText",[rm]:"vModelSelect",[Wr]:"vModelDynamic",[cm]:"withModifiers",[dm]:"withKeys",[um]:"vShow",[pm]:"Transition",[fm]:"TransitionGroup"});let La;function P0(e,t=!1){return La||(La=document.createElement("div")),t?(La.innerHTML=`<div foo="${e.replace(/"/g,"&quot;")}">`,La.children[0].getAttribute("foo")):(La.innerHTML=e,La.textContent)}const M0={parseMode:"html",isVoidTag:Av,isNativeTag:e=>Tv(e)||Cv(e)||Ev(e),isPreTag:e=>e==="pre",isIgnoreNewlineTag:e=>e==="pre"||e==="textarea",decodeEntities:P0,isBuiltInComponent:e=>{if(e==="Transition"||e==="transition")return pm;if(e==="TransitionGroup"||e==="transition-group")return fm},getNamespace(e,t,s){let n=t?t.ns:s;if(t&&n===2)if(t.tag==="annotation-xml"){if(e==="svg")return 1;t.props.some(a=>a.type===6&&a.name==="encoding"&&a.value!=null&&(a.value.content==="text/html"||a.value.content==="application/xhtml+xml"))&&(n=0)}else/^m(?:[ions]|text)$/.test(t.tag)&&e!=="mglyph"&&e!=="malignmark"&&(n=0);else t&&n===1&&(t.tag==="foreignObject"||t.tag==="desc"||t.tag==="title")&&(n=0);if(n===0){if(e==="svg")return 1;if(e==="math")return 2}return n}},F0=e=>{e.type===1&&e.props.forEach((t,s)=>{t.type===6&&t.name==="style"&&t.value&&(e.props[s]={type:7,name:"bind",arg:He("style",!0,t.loc),exp:$0(t.value.content,t.loc),modifiers:[],loc:t.loc})})},$0=(e,t)=>{const s=Tp(e);return He(JSON.stringify(s),!1,t,3)};function Zn(e,t){return gt(e,t)}const B0=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(Zn(54,a)),t.children.length&&(s.onError(Zn(55,a)),t.children.length=0),{props:[Ot(He("innerHTML",!0,a),n||He("",!0))]}},U0=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(Zn(56,a)),t.children.length&&(s.onError(Zn(57,a)),t.children.length=0),{props:[Ot(He("textContent",!0),n?Rs(n,s)>0?n:Bt(s.helperString(Fo),[n],a):He("",!0))]}},H0=(e,t,s)=>{const n=am(e,t,s);if(!n.props.length||t.tagType===1)return n;e.arg&&s.onError(Zn(59,e.arg.loc));const{tag:a}=t,i=s.isCustomElement(a);if(a==="input"||a==="textarea"||a==="select"||i){let l=om,o=!1;if(a==="input"||i){const r=$o(t,"type");if(r){if(r.type===7)l=Wr;else if(r.value)switch(r.value.content){case"radio":l=im;break;case"checkbox":l=lm;break;case"file":o=!0,s.onError(Zn(60,e.loc));break}}else kx(t)&&(l=Wr)}else a==="select"&&(l=rm);o||(n.needRuntime=s.helper(l))}else s.onError(Zn(58,e.loc));return n.props=n.props.filter(l=>!(l.key.type===4&&l.key.content==="modelValue")),n},z0=Os("passive,once,capture"),j0=Os("stop,prevent,self,ctrl,shift,alt,meta,exact,middle"),V0=Os("left,right"),hm=Os("onkeyup,onkeydown,onkeypress"),q0=(e,t,s,n)=>{const a=[],i=[],l=[];for(let o=0;o<t.length;o++){const r=t[o].content;r==="native"&&tl("COMPILER_V_ON_NATIVE",s)||z0(r)?l.push(r):V0(r)?ks(e)?hm(e.content.toLowerCase())?a.push(r):i.push(r):(a.push(r),i.push(r)):j0(r)?i.push(r):a.push(r)}return{keyModifiers:a,nonKeyModifiers:i,eventOptionModifiers:l}},Su=(e,t)=>ks(e)&&e.content.toLowerCase()==="onclick"?He(t,!0):e.type!==4?Gs(["(",e,`) === "onClick" ? "${t}" : (`,e,")"]):e,G0=(e,t,s)=>nm(e,t,s,n=>{const{modifiers:a}=e;if(!a.length)return n;let{key:i,value:l}=n.props[0];const{keyModifiers:o,nonKeyModifiers:r,eventOptionModifiers:c}=q0(i,a,s,e.loc);if(r.includes("right")&&(i=Su(i,"onContextmenu")),r.includes("middle")&&(i=Su(i,"onMouseup")),r.length&&(l=Bt(s.helper(cm),[l,JSON.stringify(r)])),o.length&&(!ks(i)||hm(i.content.toLowerCase()))&&(l=Bt(s.helper(dm),[l,JSON.stringify(o)])),c.length){const d=c.map(ka).join("");i=ks(i)?He(`${i.content}${d}`,!0):Gs(["(",i,`) + "${d}"`])}return{props:[Ot(i,l)]}}),K0=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(Zn(62,a)),{props:[],needRuntime:s.helper(um)}},W0=(e,t)=>{e.type===1&&e.tagType===0&&(e.tag==="script"||e.tag==="style")&&t.removeNode()},Z0=[F0],J0={cloak:D0,html:B0,text:U0,model:H0,on:G0,show:K0};function Y0(e,t={}){return N0(e,qe({},M0,t,{nodeTransforms:[W0,...Z0,...t.nodeTransforms||[]],directiveTransforms:qe({},J0,t.directiveTransforms||{}),transformHoist:null}))}/**
* vue v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const Tu=Object.create(null);function Q0(e,t){if(!Be(e))if(e.nodeType)e=e.innerHTML;else return Wt;const s=hv(e,t),n=Tu[s];if(n)return n;if(e[0]==="#"){const o=document.querySelector(e);e=o?o.innerHTML:""}const a=qe({hoistStatic:!0,onError:void 0,onWarn:Wt},t);!a.isCustomElement&&typeof customElements<"u"&&(a.isCustomElement=o=>!!customElements.get(o));const{code:i}=Y0(e,a),l=new Function("Vue",i)(ix);return l._rc=!0,Tu[s]=l}Yf(Q0);const ho=Yn({items:[]});let X0=1;function Ho(e,t="info",s=3e3){const n=X0++;return ho.items.push({id:n,message:String(e),type:t}),s>0&&setTimeout(()=>ed(n),s),n}function ed(e){const t=ho.items.findIndex(s=>s.id===e);t>=0&&ho.items.splice(t,1)}function xe(e,t="info",s=3e3){return Ho(e,t,s)}xe.success=(e,t=3e3)=>Ho(e,"success",t);xe.error=(e,t=5e3)=>Ho(e,"error",t);xe.info=(e,t=3e3)=>Ho(e,"info",t);xe.dismiss=ed;const e_={setup(){return{state:ho,dismiss:ed}},template:`
  ...
  `},yn=Yn({open:!1,title:"Confirm",message:"",confirmLabel:"Confirm",cancelLabel:"Cancel",danger:!1});let Ja=null;function zt({title:e="Confirm",message:t="",confirmLabel:s="Confirm",cancelLabel:n="Cancel",danger:a=!1}={}){return Ja&&Ja(!1),yn.title=e,yn.message=t,yn.confirmLabel=s,yn.cancelLabel=n,yn.danger=a,yn.open=!0,new Promise(i=>{Ja=i})}function Cu(e){yn.open=!1,Ja&&(Ja(e),Ja=null)}const t_={setup(){function e(t){yn.open&&t.key==="Escape"&&(t.stopPropagation(),Cu(!1))}return Ge(()=>document.addEventListener("keydown",e,!0)),bt(()=>document.removeEventListener("keydown",e,!0)),{state:yn,settle:Cu}},template:`
 ...
 * @license MIT
 */const $a=typeof document<"u";function mm(e){return typeof e=="object"||"displayName"in e||"props"in e||"__vccOpts"in e}function s_(e){return e.__esModule||e[Symbol.toStringTag]==="Module"||e.default&&mm(e.default)}const ot=Object.assign;function or(e,t){const s={};for(const n in t){const a=t[n];s[n]=Ws(a)?a.map(e):e(a)}return s}const Fi=()=>{},Ws=Array.isArray;function Eu(e,t){const s={};for(const n in e)s[n]=n in t?t[n]:e[n];return s}const vm=/#/g,n_=/&/g,a_=/\//g,i_=/=/g,l_=/\?/g,gm=/\+/g,o_=/%5B/g,r_=/%5D/g,bm=/%5E/g,c_=/%60/g,ym=/%7B/g,d_=/%7C/g,xm=/%7D/g,u_=/%20/g;function td(e){return e==null?"":encodeURI(""+e).replace(d_,"|").replace(o_,"[").replace(r_,"]")}function p_(e){return td(e).replace(ym,"{").replace(xm,"}").replace(bm,"^")}function Zr(e){return td(e).replace(gm,"%2B").replace(u_,"+").replace(vm,"%23").replace(n_,"%26").replace(c_,"`").replace(ym,"{").replace(xm,"}").replace(bm,"^")}function f_(e){return Zr(e).replace(i_,"%3D")}function h_(e){return td(e).replace(vm,"%23").replace(l_,"%3F")}function m_(e){return h_(e).replace(a_,"%2F")}function al(e){if(e==null)return null;try{return decodeURIComponent(""+e)}catch{}return""+e}const v_=/\/$/,g_=e=>e.replace(v_,"");function rr(e,t,s="/"){let n,a={},i="",l="";const o=t.indexOf("#");let r=t.indexOf("?");return r=o>=0&&r>o?-1:r,r>=0&&(n=t.slice(0,r),i=t.slice(r,o>0?o:t.length),a=e(i.slice(1))),o>=0&&(n=n||t.slice(0,o),l=t.slice(o,t.length)),n=__(n??t,s),{fullPath:n+i+l,path:n,query:a,hash:al(l)}}function b_(e,t){const s=t.query?e(t.query):"";return t.path+(s&&"?")+s+(t.hash||"")}function Au(e,t){return!t||!e.toLowerCase().startsWith(t.toLowerCase())?e:e.slice(t.length)||"/"}function y_(e,t,s){const n=t.matched.length-1,a=s.matched.length-1;return n>-1&&n===a&&oi(t.matched[n],s.matched[a])&&_m(t.params,s.params)&&e(t.query)===e(s.query)&&t.hash===s.hash}function oi(e,t){return(e.aliasOf||e)===(t.aliasOf||t)}function _m(e,t){if(Object.keys(e).length!==Object.keys(t).length)return!1;for(var s in e)if(!x_(e[s],t[s]))return!1;return!0}function x_(e,t){return Ws(e)?Ru(e,t):Ws(t)?Ru(t,e):(e==null?void 0:e.valueOf())===(t==null?void 0:t.valueOf())}function Ru(e,t){return Ws(t)?e.length===t.length&&e.every((s,n)=>s===t[n]):e.length===1&&e[0]===t}function __(e,t){if(e.startsWith("/"))return e;if(!e)return t;const s=t.split("/"),n=e.split("/"),a=n[n.length-1];(a===".."||a===".")&&n.push("");let i=s.length-1,l,o;for(l=0;l<n.length;l++)if(o=n[l],o!==".")if(o==="..")i>1&&i--;else break;return s.slice(0,i).join("/")+"/"+n.slice(l).join("/")}const Hn={path:"/",name:void 0,params:{},query:{},hash:"",fullPath:"/",matched:[],meta:{},redirectedFrom:void 0};let Jr=(function(e){return e.pop="pop",e.push="push",e})({}),cr=(function(e){return e.back="back",e.forward="forward",e.unknown="",e})({});function w_(e){if(!e)if($a){const t=document.querySelector("base");e=t&&t.getAttribute("href")||"/",e=e.replace(/^\w+:\/\/[^\/]+/,"")}else e="/";return e[0]!=="/"&&e[0]!=="#"&&(e="/"+e),g_(e)}const k_=/^[^#]+#/;function S_(e,t){return e.replace(k_,"#")+t}function T_(e,t){const s=document.documentElement.getBoundingClientRect(),n=e.getBoundingClientRect();return{behavior:t.behavior,left:n.left-s.left-(t.left||0),top:n.top-s.top-(t.top||0)}}const zo=()=>({left:window.scrollX,top:window.scrollY});function C_(e){let t;if("el"in e){const s=e.el,n=typeof s=="string"&&s.startsWith("#"),a=typeof s=="string"?n?document.getElementById(s.slice(1)):document.querySelector(s):s;if(!a)return;t=T_(a,e)}else t=e;"scrollBehavior"in document.documentElement.style?window.scrollTo(t):window.scrollTo(t.left!=null?t.left:window.scrollX,t.top!=null?t.top:window.scrollY)}function Iu(e,t){return(history.state?history.state.position-t:-1)+e}const Yr=new Map;function E_(e,t){Yr.set(e,t)}function A_(e){const t=Yr.get(e);return Yr.delete(e),t}function R_(e){return typeof e=="string"||e&&typeof e=="object"}function wm(e){return typeof e=="string"||typeof e=="symbol"}let kt=(function(e){return e[e.MATCHER_NOT_FOUND=1]="MATCHER_NOT_FOUND",e[e.NAVIGATION_GUARD_REDIRECT=2]="NAVIGATION_GUARD_REDIRECT",e[e.NAVIGATION_ABORTED=4]="NAVIGATION_ABORTED",e[e.NAVIGATION_CANCELLED=8]="NAVIGATION_CANCELLED",e[e.NAVIGATION_DUPLICATED=16]="NAVIGATION_DUPLICATED",e})({});const km=Symbol("");kt.MATCHER_NOT_FOUND+"",kt.NAVIGATION_GUARD_REDIRECT+"",kt.NAVIGATION_ABORTED+"",kt.NAVIGATION_CANCELLED+"",kt.NAVIGATION_DUPLICATED+"";function ri(e,t){return ot(new Error,{type:e,[km]:!0},t)}function hn(e,t){return e instanceof Error&&km in e&&(t==null||!!(e.type&t))}const I_=["params","query","hash"];function O_(e){if(typeof e=="string")return e;if(e.path!=null)return e.path;const t={};for(const s of I_)s in e&&(t[s]=e[s]);return JSON.stringify(t,null,2)}function L_(e){const t={};if(e===""||e==="?")return t;const s=(e[0]==="?"?e.slice(1):e).split("&");for(let n=0;n<s.length;++n){const a=s[n].replace(gm," "),i=a.indexOf("="),l=al(i<0?a:a.slice(0,i)),o=i<0?null:al(a.slice(i+1));if(l in t){let r=t[l];Ws(r)||(r=t[l]=[r]),r.push(o)}else t[l]=o}return t}function Ou(e){let t="";for(let s in e){const n=e[s];if(s=f_(s),n==null){n!==void 0&&(t+=(t.length?"&":"")+s);continue}(Ws(n)?n.map(a=>a&&Zr(a)):[n&&Zr(n)]).forEach(a=>{a!==void 0&&(t+=(t.length?"&":"")+s,a!=null&&(t+="="+a))})}return t}function N_(e){const t={};for(const s in e){const n=e[s];n!==void 0&&(t[s]=Ws(n)?n.map(a=>a==null?null:""+a):n==null?n:""+n)}return t}const D_=Symbol(""),Lu=Symbol(""),jo=Symbol(""),sd=Symbol(""),Qr=Symbol("");function xi(){let e=[];function t(n){return e.push(n),()=>{const a=e.indexOf(n);a>-1&&e.splice(a,1)}}function s(){e=[]}return{add:t,list:()=>e.slice(),reset:s}}function Kn(e,t,s,n,a,i=l=>l()){const l=n&&(n.enterCallbacks[a]=n.enterCallbacks[a]||[]);return()=>new Promise((o,r)=>{const c=p=>{p===!1?r(ri(kt.NAVIGATION_ABORTED,{from:s,to:t})):p instanceof Error?r(p):R_(p)?r(ri(kt.NAVIGATION_GUARD_REDIRECT,{from:t,to:p})):(l&&n.enterCallbacks[a]===l&&typeof p=="function"&&l.push(p),o())},d=i(()=>e.call(n&&n.instances[a],t,s,c));let u=Promise.resolve(d);e.length<3&&(u=u.then(c)),u.catch(p=>r(p))})}function dr(e,t,s,n,a=i=>i()){const i=[];for(const l of e)for(const o in l.components){let r=l.components[o];if(!(t!=="beforeRouteEnter"&&!l.instances[o]))if(mm(r)){const c=(r.__vccOpts||r)[t];c&&i.push(Kn(c,s,n,l,o,a))}else{let c=r();i.push(()=>c.then(d=>{if(!d)throw new Error(`Couldn't resolve component "${o}" at "${l.path}"`);const u=s_(d)?d.default:d;l.mods[o]=d,l.components[o]=u;const p=(u.__vccOpts||u)[t];return p&&Kn(p,s,n,l,o,a)()}))}}return i}function P_(e,t){const s=[],n=[],a=[],i=Math.max(t.matched.length,e.matched.length);for(let l=0;l<i;l++){const o=t.matched[l];o&&(e.matched.find(c=>oi(c,o))?n.push(o):s.push(o));const r=e.matched[l];r&&(t.matched.find(c=>oi(c,r))||a.push(r))}return[s,n,a]}/*!
 * vue-router v4.6.4
 * (c) 2025 Eduardo San Martin Morote
 * @license MIT
 */let M_=()=>location.protocol+"//"+location.host;function Sm(e,t){const{pathname:s,search:n,hash:a}=t,i=e.indexOf("#");if(i>-1){let l=a.includes(e.slice(i))?e.slice(i).length:1,o=a.slice(l);return o[0]!=="/"&&(o="/"+o),Au(o,"")}return Au(s,e)+n+a}function F_(e,t,s,n){let a=[],i=[],l=null;const o=({state:p})=>{const h=Sm(e,location),m=s.value,v=t.value;let k=0;if(p){if(s.value=h,t.value=p,l&&l===m){l=null;return}k=v?p.position-v.position:0}else n(h);a.forEach(R=>{R(s.value,m,{delta:k,type:Jr.pop,direction:k?k>0?cr.forward:cr.back:cr.unknown})})};function r(){l=s.value}function c(p){a.push(p);const h=()=>{const m=a.indexOf(p);m>-1&&a.splice(m,1)};return i.push(h),h}function d(){if(document.visibilityState==="hidden"){const{history:p}=window;if(!p.state)return;p.replaceState(ot({},p.state,{scroll:zo()}),"")}}function u(){for(const p of i)p();i=[],window.removeEventListener("popstate",o),window.removeEventListener("pagehide",d),document.removeEventListener("visibilitychange",d)}return window.addEventListener("popstate",o),window.addEventListener("pagehide",d),document.addEventListener("visibilitychange",d),{pauseListeners:r,listen:c,destroy:u}}function Nu(e,t,s,n=!1,a=!1){return{back:e,current:t,forward:s,replaced:n,position:window.history.length,scroll:a?zo():null}}function $_(e){const{history:t,location:s}=window,n={value:Sm(e,s)},a={value:t.state};a.value||i(n.value,{back:null,current:n.value,forward:null,position:t.length-1,replaced:!0,scroll:null},!0);function i(r,c,d){const u=e.indexOf("#"),p=u>-1?(s.host&&document.querySelector("base")?e:e.slice(u))+r:M_()+e+r;try{t[d?"replaceState":"pushState"](c,"",p),a.value=c}catch(h){console.error(h),s[d?"replace":"assign"](p)}}function l(r,c){i(r,ot({},t.state,Nu(a.value.back,r,a.value.forward,!0),c,{position:a.value.position}),!0),n.value=r}function o(r,c){const d=ot({},a.value,t.state,{forward:r,scroll:zo()});i(d.current,d,!0),i(r,ot({},Nu(n.value,r,null),{position:d.position+1},c),!1),n.value=r}return{location:n,state:a,push:o,replace:l}}function B_(e){e=w_(e);const t=$_(e),s=F_(e,t.state,t.location,t.replace);function n(i,l=!0){l||s.pauseListeners(),history.go(i)}const a=ot({location:"",base:e,go:n,createHref:S_.bind(null,e)},t,s);return Object.defineProperty(a,"location",{enumerable:!0,get:()=>t.location.value}),Object.defineProperty(a,"state",{enumerable:!0,get:()=>t.state.value}),a}function U_(e){return e=location.host?e||location.pathname+location.search:"",e.includes("#")||(e+="#"),B_(e)}let ua=(function(e){return e[e.Static=0]="Static",e[e.Param=1]="Param",e[e.Group=2]="Group",e})({});var Ft=(function(e){return e[e.Static=0]="Static",e[e.Param=1]="Param",e[e.ParamRegExp=2]="ParamRegExp",e[e.ParamRegExpEnd=3]="ParamRegExpEnd",e[e.EscapeNext=4]="EscapeNext",e})(Ft||{});const H_={type:ua.Static,value:""},z_=/[a-zA-Z0-9_]/;function j_(e){if(!e)return[[]];if(e==="/")return[[H_]];if(!e.startsWith("/"))throw new Error(`Invalid path "${e}"`);function t(h){throw new Error(`ERR (${s})/"${c}": ${h}`)}let s=Ft.Static,n=s;const a=[];let i;function l(){i&&a.push(i),i=[]}let o=0,r,c="",d="";function u(){c&&(s===Ft.Static?i.push({type:ua.Static,value:c}):s===Ft.Param||s===Ft.ParamRegExp||s===Ft.ParamRegExpEnd?(i.length>1&&(r==="*"||r==="+")&&t(`A repeatable param (${c}) must be alone in its segment. eg: '/:ids+.`),i.push({type:ua.Param,value:c,regexp:d,repeatable:r==="*"||r==="+",optional:r==="*"||r==="?"})):t("Invalid state to consume buffer"),c="")}function p(){c+=r}for(;o<e.length;){if(r=e[o++],r==="\\"&&s!==Ft.ParamRegExp){n=s,s=Ft.EscapeNext;continue}switch(s){case Ft.Static:r==="/"?(c&&u(),l()):r===":"?(u(),s=Ft.Param):p();break;case Ft.EscapeNext:p(),s=n;break;case Ft.Param:r==="("?s=Ft.ParamRegExp:z_.test(r)?p():(u(),s=Ft.Static,r!=="*"&&r!=="?"&&r!=="+"&&o--);break;case Ft.ParamRegExp:r===")"?d[d.length-1]=="\\"?d=d.slice(0,-1)+r:s=Ft.ParamRegExpEnd:d+=r;break;case Ft.ParamRegExpEnd:u(),s=Ft.Static,r!=="*"&&r!=="?"&&r!=="+"&&o--,d="";break;default:t("Unknown state");break}}return s===Ft.ParamRegExp&&t(`Unfinished custom RegExp for param "${c}"`),u(),l(),a}const Du="[^/]+?",V_={sensitive:!1,strict:!1,start:!0,end:!0};var ps=(function(e){return e[e._multiplier=10]="_multiplier",e[e.Root=90]="Root",e[e.Segment=40]="Segment",e[e.SubSegment=30]="SubSegment",e[e.Static=40]="Static",e[e.Dynamic=20]="Dynamic",e[e.BonusCustomRegExp=10]="BonusCustomRegExp",e[e.BonusWildcard=-50]="BonusWildcard",e[e.BonusRepeatable=-20]="BonusRepeatable",e[e.BonusOptional=-8]="BonusOptional",e[e.BonusStrict=.7000000000000001]="BonusStrict",e[e.BonusCaseSensitive=.25]="BonusCaseSensitive",e})(ps||{});const q_=/[.+*?^${}()[\]/\\]/g;function G_(e,t){const s=ot({},V_,t),n=[];let a=s.start?"^":"";const i=[];for(const c of e){const d=c.length?[]:[ps.Root];s.strict&&!c.length&&(a+="/");for(let u=0;u<c.length;u++){const p=c[u];let h=ps.Segment+(s.sensitive?ps.BonusCaseSensitive:0);if(p.type===ua.Static)u||(a+="/"),a+=p.value.replace(q_,"\\$&"),h+=ps.Static;else if(p.type===ua.Param){const{value:m,repeatable:v,optional:k,regexp:R}=p;i.push({name:m,repeatable:v,optional:k});const y=R||Du;if(y!==Du){h+=ps.BonusCustomRegExp;try{`${y}`}catch(b){throw new Error(`Invalid custom RegExp for param "${m}" (${y}): `+b.message)}}let g=v?`((?:${y})(?:/(?:${y}))*)`:`(${y})`;u||(g=k&&c.length<2?`(?:/${g})`:"/"+g),k&&(g+="?"),a+=g,h+=ps.Dynamic,k&&(h+=ps.BonusOptional),v&&(h+=ps.BonusRepeatable),y===".*"&&(h+=ps.BonusWildcard)}d.push(h)}n.push(d)}if(s.strict&&s.end){const c=n.length-1;n[c][n[c].length-1]+=ps.BonusStrict}s.strict||(a+="/?"),s.end?a+="$":s.strict&&!a.endsWith("/")&&(a+="(?:/|$)");const l=new RegExp(a,s.sensitive?"":"i");function o(c){const d=c.match(l),u={};if(!d)return null;for(let p=1;p<d.length;p++){const h=d[p]||"",m=i[p-1];u[m.name]=h&&m.repeatable?h.split("/"):h}return u}function r(c){let d="",u=!1;for(const p of e){(!u||!d.endsWith("/"))&&(d+="/"),u=!1;for(const h of p)if(h.type===ua.Static)d+=h.value;else if(h.type===ua.Param){const{value:m,repeatable:v,optional:k}=h,R=m in c?c[m]:"";if(Ws(R)&&!v)throw new Error(`Provided param "${m}" is an array but it is not repeatable (* or + modifiers)`);const y=Ws(R)?R.join("/"):R;if(!y)if(k)p.length<2&&(d.endsWith("/")?d=d.slice(0,-1):u=!0);else throw new Error(`Missing required param "${m}"`);d+=y}}return d||"/"}return{re:l,score:n,keys:i,parse:o,stringify:r}}function K_(e,t){let s=0;for(;s<e.length&&s<t.length;){const n=t[s]-e[s];if(n)return n;s++}return e.length<t.length?e.length===1&&e[0]===ps.Static+ps.Segment?-1:1:e.length>t.length?t.length===1&&t[0]===ps.Static+ps.Segment?1:-1:0}function Tm(e,t){let s=0;const n=e.score,a=t.score;for(;s<n.length&&s<a.length;){const i=K_(n[s],a[s]);if(i)return i;s++}if(Math.abs(a.length-n.length)===1){if(Pu(n))return 1;if(Pu(a))return-1}return a.length-n.length}function Pu(e){const t=e[e.length-1];return e.length>0&&t[t.length-1]<0}const W_={strict:!1,end:!0,sensitive:!1};function Z_(e,t,s){const n=G_(j_(e.path),s),a=ot(n,{record:e,parent:t,children:[],alias:[]});return t&&!a.record.aliasOf==!t.record.aliasOf&&t.children.push(a),a}function J_(e,t){const s=[],n=new Map;t=Eu(W_,t);function a(u){return n.get(u)}function i(u,p,h){const m=!h,v=Fu(u);v.aliasOf=h&&h.record;const k=Eu(t,u),R=[v];if("alias"in u){const b=typeof u.alias=="string"?[u.alias]:u.alias;for(const S of b)R.push(Fu(ot({},v,{components:h?h.record.components:v.components,path:S,aliasOf:h?h.record:v})))}let y,g;for(const b of R){const{path:S}=b;if(p&&S[0]!=="/"){const w=p.record.path,A=w[w.length-1]==="/"?"":"/";b.path=p.record.path+(S&&A+S)}if(y=Z_(b,p,k),h?h.alias.push(y):(g=g||y,g!==y&&g.alias.push(y),m&&u.name&&!$u(y)&&l(u.name)),Cm(y)&&r(y),v.children){const w=v.children;for(let A=0;A<w.length;A++)i(w[A],y,h&&h.children[A])}h=h||y}return g?()=>{l(g)}:Fi}function l(u){if(wm(u)){const p=n.get(u);p&&(n.delete(u),s.splice(s.indexOf(p),1),p.children.forEach(l),p.alias.forEach(l))}else{const p=s.indexOf(u);p>-1&&(s.splice(p,1),u.record.name&&n.delete(u.record.name),u.children.forEach(l),u.alias.forEach(l))}}function o(){return s}function r(u){const p=X_(u,s);s.splice(p,0,u),u.record.name&&!$u(u)&&n.set(u.record.name,u)}function c(u,p){let h,m={},v,k;if("name"in u&&u.name){if(h=n.get(u.name),!h)throw ri(kt.MATCHER_NOT_FOUND,{location:u});k=h.record.name,m=ot(Mu(p.params,h.keys.filter(g=>!g.optional).concat(h.parent?h.parent.keys.filter(g=>g.optional):[]).map(g=>g.name)),u.params&&Mu(u.params,h.keys.map(g=>g.name))),v=h.stringify(m)}else if(u.path!=null)v=u.path,h=s.find(g=>g.re.test(v)),h&&(m=h.parse(v),k=h.record.name);else{if(h=p.name?n.get(p.name):s.find(g=>g.re.test(p.path)),!h)throw ri(kt.MATCHER_NOT_FOUND,{location:u,currentLocation:p});k=h.record.name,m=ot({},p.params,u.params),v=h.stringify(m)}const R=[];let y=h;for(;y;)R.unshift(y.record),y=y.parent;return{name:k,path:v,params:m,matched:R,meta:Q_(R)}}e.forEach(u=>i(u));function d(){s.length=0,n.clear()}return{addRoute:i,resolve:c,removeRoute:l,clearRoutes:d,getRoutes:o,getRecordMatcher:a}}function Mu(e,t){const s={};for(const n of t)n in e&&(s[n]=e[n]);return s}function Fu(e){const t={path:e.path,redirect:e.redirect,name:e.name,meta:e.meta||{},aliasOf:e.aliasOf,beforeEnter:e.beforeEnter,props:Y_(e),children:e.children||[],instances:{},leaveGuards:new Set,updateGuards:new Set,enterCallbacks:{},components:"components"in e?e.components||null:e.component&&{default:e.component}};return Object.defineProperty(t,"mods",{value:{}}),t}function Y_(e){const t={},s=e.props||!1;if("component"in e)t.default=s;else for(const n in e.components)t[n]=typeof s=="object"?s[n]:s;return t}function $u(e){for(;e;){if(e.record.aliasOf)return!0;e=e.parent}return!1}function Q_(e){return e.reduce((t,s)=>ot(t,s.meta),{})}function X_(e,t){let s=0,n=t.length;for(;s!==n;){const i=s+n>>1;Tm(e,t[i])<0?n=i:s=i+1}const a=ew(e);return a&&(n=t.lastIndexOf(a,n-1)),n}function ew(e){let t=e;for(;t=t.parent;)if(Cm(t)&&Tm(e,t)===0)return t}function Cm({record:e}){return!!(e.name||e.components&&Object.keys(e.components).length||e.redirect)}function Bu(e){const t=Bs(jo),s=Bs(sd),n=J(()=>{const r=an(e.to);return t.resolve(r)}),a=J(()=>{const{matched:r}=n.value,{length:c}=r,d=r[c-1],u=s.matched;if(!d||!u.length)return-1;const p=u.findIndex(oi.bind(null,d));if(p>-1)return p;const h=Uu(r[c-2]);return c>1&&Uu(d)===h&&u[u.length-1].path!==h?u.findIndex(oi.bind(null,r[c-2])):p}),i=J(()=>a.value>-1&&iw(s.params,n.value.params)),l=J(()=>a.value>-1&&a.value===s.matched.length-1&&_m(s.params,n.value.params));function o(r={}){if(aw(r)){const c=t[an(e.replace)?"replace":"push"](an(e.to)).catch(Fi);return e.viewTransition&&typeof document<"u"&&"startViewTransition"in document&&document.startViewTransition(()=>c),c}return Promise.resolve()}return{route:n,href:J(()=>n.value.href),isActive:i,isExactActive:l,navigate:o}}function tw(e){return e.length===1?e[0]:e}const sw=cl({name:"RouterLink",compatConfig:{MODE:3},props:{to:{type:[String,Object],required:!0},replace:Boolean,activeClass:String,exactActiveClass:String,custom:Boolean,ariaCurrentValue:{type:String,default:"page"},viewTransition:Boolean},useLink:Bu,setup(e,{slots:t}){const s=Yn(Bu(e)),{options:n}=Bs(jo),a=J(()=>({[Hu(e.activeClass,n.linkActiveClass,"router-link-active")]:s.isActive,[Hu(e.exactActiveClass,n.linkExactActiveClass,"router-link-exact-active")]:s.isExactActive}));return()=>{const i=t.default&&tw(t.default(s));return e.custom?i:ei("a",{"aria-current":s.isExactActive?e.ariaCurrentValue:null,href:s.href,onClick:s.navigate,class:a.value},i)}}}),nw=sw;function aw(e){if(!(e.metaKey||e.altKey||e.ctrlKey||e.shiftKey)&&!e.defaultPrevented&&!(e.button!==void 0&&e.button!==0)){if(e.currentTarget&&e.currentTarget.getAttribute){const t=e.currentTarget.getAttribute("target");if(/\b_blank\b/i.test(t))return}return e.preventDefault&&e.preventDefault(),!0}}function iw(e,t){for(const s in t){const n=t[s],a=e[s];if(typeof n=="string"){if(n!==a)return!1}else if(!Ws(a)||a.length!==n.length||n.some((i,l)=>i.valueOf()!==a[l].valueOf()))return!1}return!0}function Uu(e){return e?e.aliasOf?e.aliasOf.path:e.path:""}const Hu=(e,t,s)=>e??t??s,lw=cl({name:"RouterView",inheritAttrs:!1,props:{name:{type:String,default:"default"},route:Object},compatConfig:{MODE:3},setup(e,{attrs:t,slots:s}){const n=Bs(Qr),a=J(()=>e.route||n.value),i=Bs(Lu,0),l=J(()=>{let c=an(i);const{matched:d}=a.value;let u;for(;(u=d[c])&&!u.components;)c++;return c}),o=J(()=>a.value.matched[l.value]);Li(Lu,J(()=>l.value+1)),Li(D_,o),Li(Qr,a);const r=f();return ns(()=>[r.value,o.value,e.name],([c,d,u],[p,h,m])=>{d&&(d.instances[u]=c,h&&h!==d&&c&&c===p&&(d.leaveGuards.size||(d.leaveGuards=h.leaveGuards),d.updateGuards.size||(d.updateGuards=h.updateGuards))),c&&d&&(!h||!oi(d,h)||!p)&&(d.enterCallbacks[u]||[]).forEach(v=>v(c))},{flush:"post"}),()=>{const c=a.value,d=e.name,u=o.value,p=u&&u.components[d];if(!p)return zu(s.default,{Component:p,route:c});const h=u.props[d],m=h?h===!0?c.params:typeof h=="function"?h(c):h:null,k=ei(p,ot({},m,t,{onVnodeUnmounted:R=>{R.component.isUnmounted&&(u.instances[d]=null)},ref:r}));return zu(s.default,{Component:k,route:c})||k}}});function zu(e,t){if(!e)return null;const s=e(t);return s.length===1?s[0]:s}const ow=lw;function rw(e){const t=J_(e.routes,e),s=e.parseQuery||L_,n=e.stringifyQuery||Ou,a=e.history,i=xi(),l=xi(),o=xi(),r=pc(Hn);let c=Hn;$a&&e.scrollBehavior&&"scrollRestoration"in history&&(history.scrollRestoration="manual");const d=or.bind(null,Q=>""+Q),u=or.bind(null,m_),p=or.bind(null,al);function h(Q,be){let z,re;return wm(Q)?(z=t.getRecordMatcher(Q),re=be):re=Q,t.addRoute(re,z)}function m(Q){const be=t.getRecordMatcher(Q);be&&t.removeRoute(be)}function v(){return t.getRoutes().map(Q=>Q.record)}function k(Q){return!!t.getRecordMatcher(Q)}function R(Q,be){if(be=ot({},be||r.value),typeof Q=="string"){const P=rr(s,Q,be.path),H=t.resolve({path:P.path},be),ie=a.createHref(P.fullPath);return ot(P,H,{params:p(H.params),hash:al(P.hash),redirectedFrom:void 0,href:ie})}let z;if(Q.path!=null)z=ot({},Q,{path:rr(s,Q.path,be.path).path});else{const P=ot({},Q.params);for(const H in P)P[H]==null&&delete P[H];z=ot({},Q,{params:u(P)}),be.params=u(be.params)}const re=t.resolve(z,be),ue=Q.hash||"";re.params=d(p(re.params));const Ie=b_(n,ot({},Q,{hash:p_(ue),path:re.path})),_=a.createHref(Ie);return ot({fullPath:Ie,hash:ue,query:n===Ou?N_(Q.query):Q.query||{}},re,{redirectedFrom:void 0,href:_})}function y(Q){return typeof Q=="string"?rr(s,Q,r.value.path):ot({},Q)}function g(Q,be){if(c!==Q)return ri(kt.NAVIGATION_CANCELLED,{from:be,to:Q})}function b(Q){return A(Q)}function S(Q){return b(ot(y(Q),{replace:!0}))}function w(Q,be){const z=Q.matched[Q.matched.length-1];if(z&&z.redirect){const{redirect:re}=z;let ue=typeof re=="function"?re(Q,be):re;return typeof ue=="string"&&(ue=ue.includes("?")||ue.includes("#")?ue=y(ue):{path:ue},ue.params={}),ot({query:Q.query,hash:Q.hash,params:ue.path!=null?{}:Q.params},ue)}}function A(Q,be){const z=c=R(Q),re=r.value,ue=Q.state,Ie=Q.force,_=Q.replace===!0,P=w(z,re);if(P)return A(ot(y(P),{state:typeof P=="object"?ot({},ue,P.state):ue,force:Ie,replace:_}),be||z);const H=z;H.redirectedFrom=be;let ie;return!Ie&&y_(n,re,z)&&(ie=ri(kt.NAVIGATION_DUPLICATED,{to:H,from:re}),K(re,re,!0,!1)),(ie?Promise.resolve(ie):N(H,re)).catch(se=>hn(se)?hn(se,kt.NAVIGATION_GUARD_REDIRECT)?se:W(se):O(se,H,re)).then(se=>{if(se){if(hn(se,kt.NAVIGATION_GUARD_REDIRECT))return A(ot({replace:_},y(se.to),{state:typeof se.to=="object"?ot({},ue,se.to.state):ue,force:Ie}),be||H)}else se=C(H,re,!0,_,ue);return $(H,re,se),se})}function E(Q,be){const z=g(Q,be);return z?Promise.reject(z):Promise.resolve()}function x(Q){const be=ee.values().next().value;return be&&typeof be.runWithContext=="function"?be.runWithContext(Q):Q()}function N(Q,be){let z;const[re,ue,Ie]=P_(Q,be);z=dr(re.reverse(),"beforeRouteLeave",Q,be);for(const P of re)P.leaveGuards.forEach(H=>{z.push(Kn(H,Q,be))});const _=E.bind(null,Q,be);return z.push(_),De(z).then(()=>{z=[];for(const P of i.list())z.push(Kn(P,Q,be));return z.push(_),De(z)}).then(()=>{z=dr(ue,"beforeRouteUpdate",Q,be);for(const P of ue)P.updateGuards.forEach(H=>{z.push(Kn(H,Q,be))});return z.push(_),De(z)}).then(()=>{z=[];for(const P of Ie)if(P.beforeEnter)if(Ws(P.beforeEnter))for(const H of P.beforeEnter)z.push(Kn(H,Q,be));else z.push(Kn(P.beforeEnter,Q,be));return z.push(_),De(z)}).then(()=>(Q.matched.forEach(P=>P.enterCallbacks={}),z=dr(Ie,"beforeRouteEnter",Q,be,x),z.push(_),De(z))).then(()=>{z=[];for(const P of l.list())z.push(Kn(P,Q,be));return z.push(_),De(z)}).catch(P=>hn(P,kt.NAVIGATION_CANCELLED)?P:Promise.reject(P))}function $(Q,be,z){o.list().forEach(re=>x(()=>re(Q,be,z)))}function C(Q,be,z,re,ue){const Ie=g(Q,be);if(Ie)return Ie;const _=be===Hn,P=$a?history.state:{};z&&(re||_?a.replace(Q.fullPath,ot({scroll:_&&P&&P.scroll},ue)):a.push(Q.fullPath,ue)),r.value=Q,K(Q,be,z,_),W()}let M;function V(){M||(M=a.listen((Q,be,z)=>{if(!de.listening)return;const re=R(Q),ue=w(re,de.currentRoute.value);if(ue){A(ot(ue,{replace:!0,force:!0}),re).catch(Fi);return}c=re;const Ie=r.value;$a&&E_(Iu(Ie.fullPath,z.delta),zo()),N(re,Ie).catch(_=>hn(_,kt.NAVIGATION_ABORTED|kt.NAVIGATION_CANCELLED)?_:hn(_,kt.NAVIGATION_GUARD_REDIRECT)?(A(ot(y(_.to),{force:!0}),re).then(P=>{hn(P,kt.NAVIGATION_ABORTED|kt.NAVIGATION_DUPLICATED)&&!z.delta&&z.type===Jr.pop&&a.go(-1,!1)}).catch(Fi),Promise.reject()):(z.delta&&a.go(-z.delta,!1),O(_,re,Ie))).then(_=>{_=_||C(re,Ie,!1),_&&(z.delta&&!hn(_,kt.NAVIGATION_CANCELLED)?a.go(-z.delta,!1):z.type===Jr.pop&&hn(_,kt.NAVIGATION_ABORTED|kt.NAVIGATION_DUPLICATED)&&a.go(-1,!1)),$(re,Ie,_)}).catch(Fi)}))}let q=xi(),D=xi(),I;function O(Q,be,z){W(Q);const re=D.list();return re.length?re.forEach(ue=>ue(Q,be,z)):console.error(Q),Promise.reject(Q)}function U(){return I&&r.value!==Hn?Promise.resolve():new Promise((Q,be)=>{q.add([Q,be])})}function W(Q){return I||(I=!Q,V(),q.list().forEach(([be,z])=>Q?z(Q):be()),q.reset()),Q}function K(Q,be,z,re){const{scrollBehavior:ue}=e;if(!$a||!ue)return Promise.resolve();const Ie=!z&&A_(Iu(Q.fullPath,0))||(re||!z)&&history.state&&history.state.scroll||null;return It().then(()=>ue(Q,be,Ie)).then(_=>_&&C_(_)).catch(_=>O(_,Q,be))}const X=Q=>a.go(Q);let oe;const ee=new Set,de={currentRoute:r,listening:!0,addRoute:h,removeRoute:m,clearRoutes:t.clearRoutes,hasRoute:k,getRoutes:v,resolve:R,options:e,push:b,replace:S,go:X,back:()=>X(-1),forward:()=>X(1),beforeEach:i.add,beforeResolve:l.add,afterEach:o.add,onError:D.add,isReady:U,install(Q){Q.component("RouterLink",nw),Q.component("RouterView",ow),Q.config.globalProperties.$router=de,Object.defineProperty(Q.config.globalProperties,"$route",{enumerable:!0,get:()=>an(r)}),$a&&!oe&&r.value===Hn&&(oe=!0,b(a.location).catch(re=>{}));const be={};for(const re in Hn)Object.defineProperty(be,re,{get:()=>r.value[re],enumerable:!0});Q.provide(jo,de),Q.provide(sd,uc(be)),Q.provide(Qr,r);const z=Q.unmount;ee.add(Q),Q.unmount=function(){ee.delete(Q),ee.size<1&&(c=Hn,M&&M(),M=null,r.value=Hn,oe=!1,I=!1),z()}}};function De(Q){return Q.reduce((be,z)=>be.then(()=>x(z)),Promise.resolve())}return de}function Em(){return Bs(jo)}function cw(e){return Bs(sd)}const Vo={props:{tabs:{type:Array,required:!0},defaultTab:{type:String,default:""},groupLabel:{type:String,default:""}},setup(e){const t=cw(),s=Em(),n=J({get(){var r;const o=t.query.tab;return o&&e.tabs.some(c=>c.id===o)?o:e.defaultTab||((r=e.tabs[0])==null?void 0:r.id)||""},set(o){s.replace({query:{...t.query,tab:o}})}}),a=J(()=>{var o;return((o=e.tabs.find(r=>r.id===n.value))==null?void 0:o.component)||null}),i=J(()=>{var o;return((o=e.tabs.find(r=>r.id===n.value))==null?void 0:o.label)||""});ns(i,o=>{e.groupLabel&&o&&(document.title=`Odin — ${e.groupLabel} › ${o}`)},{immediate:!0});function l(o,r){if(!["ArrowLeft","ArrowRight","Home","End"].includes(o.key))return;o.preventDefault();let c=r;o.key==="ArrowRight"&&(c=(r+1)%e.tabs.length),o.key==="ArrowLeft"&&(c=(r-1+e.tabs.length)%e.tabs.length),o.key==="Home"&&(c=0),o.key==="End"&&(c=e.tabs.length-1),n.value=e.tabs[c].id,requestAnimationFrame(()=>{var d;return(d=document.getElementById("tab-"+e.tabs[c].id))==null?void 0:d.focus()})}return{activeTab:n,activeComponent:a,activeLabel:i,onTabKeydown:l}},template:`
...
`)}}return}}let i=null;function l(){const p=Date.now();e.value.forEach(h=>{h.status==="running"&&(h.elapsed=p-h.startTime)})}let o=!1;function r(){o||(o=!0,Qe.on("events",a),i||(i=setInterval(l,500)))}function c(){o&&(o=!1,Qe.off("events",a),i&&(clearInterval(i),i=null))}Ge(r),gs(r),ls(c),bt(c);function d(p){return p<1e3?`${p}ms`:`${(p/1e3).toFixed(1)}s`}function u(p){return p==="running"?"clock":p==="success"?"success":p==="error"?"error":"info"}return{activeTasks:e,recentHistory:t,streamOutput:s,formatMs:d,statusIcon:u}},template:`
...
... (truncated)`:s}function ju(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function id(e){return e==null||!isFinite(e)?"—":Number(e).toLocaleString()}function Im(e){return e==null||!isFinite(e)?"—":e>=1e3?`${(e/1e3).toFixed(1)}k`:String(e)}const Om=Symbol("agent-detail-cancelled"),fw=15e3;function hw(e,{timeoutMs:t,timeoutLabel:s,scheduleTimeout:n,cancelTimeout:a}){const i=typeof AbortController=="function"?new AbortController:null;let l=null,o=!1,r,c;const d=new Promise((h,m)=>{r=h,c=m});function u(h,m){o||(o=!0,l!==null&&a(l),l=null,(h?r:c)(m))}let p;try{p=e(i==null?void 0:i.signal)}catch(h){u(!1,h)}return o||Promise.resolve(p).then(h=>u(!0,h),h=>u(!1,h)),!o&&Number.isFinite(t)&&t>0&&(l=n(()=>{const h=Math.max(1,Math.round(t/1e3));u(!1,new Error(`${s} request timed out after ${h}s`)),i==null||i.abort()},t)),{promise:d,cancel(){u(!0,Om),i==null||i.abort()}}}function Lm({state:e,requestDetail:t,timeoutMs:s=fw,detailLabel:n="Agent detail",scheduleTimeout:a=globalThis.setTimeout.bind(globalThis),cancelTimeout:i=globalThis.clearTimeout.bind(globalThis)}){if(!e||typeof e!="object")throw new TypeError("agent detail state is required");if(typeof t!="function")throw new TypeError("requestDetail must be a function");let l=null;function o(){const p=l;l=null,p==null||p.cancel()}function r(p,{initial:h,coalesce:m}){if(!p)return Promise.resolve();if(m&&l&&l.agentId===p&&e.detailId===p)return l.promise;o();const v={agentId:p,cancel:null,promise:null};l=v,h?(e.detail=null,e.detailError=null,e.detailLoading=!0):e.detail===null&&e.detailError===null&&(e.detailLoading=!0);const k=hw(R=>t(p,{signal:R}),{timeoutMs:s,timeoutLabel:n,scheduleTimeout:a,cancelTimeout:i});return v.cancel=k.cancel,v.promise=(async()=>{let R=null,y=null;try{R=await k.promise}catch(g){y=g}R!==Om&&(l!==v||e.detailId!==p||(l=null,!y&&(R===null||typeof R!="object")&&(y=new Error(`${n} response was empty or invalid`)),y?e.detail===null&&(e.detailError=(y==null?void 0:y.message)||`Failed to load ${n.toLowerCase()}`):(e.detail=R,e.detailError=null),e.detailLoading=!1))})(),v.promise}function c(p){return e.detailId=p,r(p,{initial:!0,coalesce:!1})}function d(){const p=e.detailId;return p?r(p,{initial:!1,coalesce:!0}):Promise.resolve()}function u(){o(),e.detailId=null,e.detail=null,e.detailError=null,e.detailLoading=!1}return{open:c,refresh:d,close:u,hasInFlight:()=>l!==null}}function mw({isEnabled:e,refreshList:t,hasOpenDetail:s,refreshDetail:n,intervalMs:a=5e3,scheduleInterval:i=globalThis.setInterval.bind(globalThis),cancelInterval:l=globalThis.clearInterval.bind(globalThis)}){let o=null;function r(){e()&&(t(),s()&&n())}function c(){o!==null&&(l(o),o=null)}function d(){c(),e()&&(o=i(r,a))}function u(){e()?d():c()}return{start:d,stop:c,sync:u,isRunning:()=>o!==null}}const vw={template:`
    ...
    </div>`,setup(){const e=f([]),t=f(!0),s=f(null),n=f(null),a=f(!0),i=f("all");let l=!1;const o=J(()=>e.value.filter(O=>O.status==="running").length),r=J(()=>e.value.filter(O=>O.status==="completed").length),c=J(()=>e.value.filter(O=>["failed","timeout","killed"].includes(O.status)).length),d=J(()=>[{value:"all",label:"All",count:e.value.length},{value:"running",label:"Running",count:o.value},{value:"completed",label:"Completed",count:r.value},{value:"failed",label:"Failed",count:c.value}]),u=J(()=>i.value==="all"?e.value:i.value==="failed"?e.value.filter(O=>["failed","timeout","killed"].includes(O.status)):e.value.filter(O=>O.status===i.value));function p(O){const U=Number(O.max_iterations)||0;return U<=0?0:Math.min(100,Math.round(O.iteration_count/U*100))}function h(O){return(Number(O.max_iterations)||0)>0}function m(O,U){return O?O==="N/A"?"N/A":U==="current_inheritance"?`inherit (currently ${O})`:O:"unknown"}function v(O){return m(O.display_model,O.display_model_source||O.display_source)}function k(O){return m(O.display_reasoning_effort,O.display_reasoning_effort_source||O.display_source)}function R(O){return{last_execution:"last executed",current_inheritance:"inherited from current config — not yet executed",spawn_override_pending:"requested at spawn — not yet executed",unknown:"no execution data"}[O]||""}const y=f(null),g=f(null),b=f(!1),S=f(null),w=f(""),E=Lm({state:{get detail(){return y.value},set detail(O){y.value=O},get detailId(){return g.value},set detailId(O){g.value=O},get detailLoading(){return b.value},set detailLoading(O){b.value=O},get detailError(){return S.value},set detailError(O){S.value=O}},requestDetail:(O,{signal:U})=>G.get(`/api/agents/${encodeURIComponent(O)}`,{signal:U})});async function x(O){w.value="",await E.open(O.id)}function N(){E.close(),w.value=""}async function $(){await E.refresh()}async function C(O,U){try{await navigator.clipboard.writeText(U||""),w.value=O,setTimeout(()=>{w.value===O&&(w.value="")},1500)}catch{xe.error("Copy failed")}}async function M(O=!1){O=O===!0,O||(t.value=!0);try{const U=await G.get("/api/agents");e.value=Array.isArray(U)?U:[],s.value=null}catch(U){O||(s.value=U.message)}O||(t.value=!1)}async function V(O){const U=e.value.find(K=>K.id===O);if(await zt({title:"Kill agent",message:`Kill agent "${(U==null?void 0:U.label)||O}"? Its current work will be lost.`,confirmLabel:"Kill",danger:!0})){n.value=O;try{await G.del(`/api/agents/${encodeURIComponent(O)}`),xe.success("Agent killed"),await M()}catch(K){xe.error(K.message||"Failed to kill agent")}n.value=null}}const q=mw({isEnabled:()=>a.value&&l,refreshList:()=>M(!0),hasOpenDetail:()=>!!g.value,refreshDetail:$});function D(){q.start()}function I(){q.stop()}return ns(a,()=>q.sync()),Ge(()=>{l=!0,M(),D()}),gs(()=>{l=!0,M(!0),D()}),ls(()=>{l=!1,I()}),bt(()=>{l=!1,I(),E.close()}),{agents:e,loading:t,error:s,killing:n,autoRefresh:a,statusFilter:i,runningCount:o,completedCount:r,failedCount:c,statusFilters:d,filteredAgents:u,formatTs:Ta,formatDuration:ci,progressPercent:p,hasProgress:h,displayModelText:v,displayEffortText:k,displaySourceLabel:R,detail:y,detailId:g,detailLoading:b,detailError:S,copied:w,openDetail:x,closeDetail:N,copyText:C,fetchAgents:M,killAgent:V,startAutoRefresh:D,stopAutoRefresh:I}}},gw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=f([]),t=f(!0),s=f(null),n=f(!1),a=f({goal:"",interval_seconds:60,mode:"notify",max_iterations:50,stop_condition:"",channel_id:""}),i=f(!1),l=f(null),o=f(null),r=f(null),c=f(null),d=f(null),u=f(!1),p=f(null),h=f("");let m=!1;const k=Lm({state:{get detail(){return c.value},set detail(I){c.value=I},get detailId(){return d.value},set detailId(I){d.value=I},get detailLoading(){return u.value},set detailLoading(I){u.value=I},get detailError(){return p.value},set detailError(I){p.value=I}},detailLabel:"Loop detail",requestDetail:(I,{signal:O})=>G.get(`/api/loops/${encodeURIComponent(I)}?limit=100`,{signal:O})});async function R(I){h.value="",await k.open(I.id)}function y(){k.close(),h.value=""}async function g(I,O){try{await navigator.clipboard.writeText(O||""),h.value=I,setTimeout(()=>{h.value===I&&(h.value="")},1500)}catch{xe.error("Copy failed")}}const b=J(()=>e.value.reduce((I,O)=>I+(O.iteration_count||0),0)),S=J(()=>e.value.filter(I=>I.status==="running").length);function w(I){return I==="running"?"loop-status-running":I==="error"?"loop-status-error":"loop-status-stopped"}function A(I){return I==="running"?"badge-success":I==="error"?"badge-danger":I==="completed"?"badge-info":"badge-warning"}function E(I){return I==="act"?"badge-warning":I==="silent"?"badge-info":"badge-success"}async function x(I=!1){I=I===!0,I||(t.value=!0);try{const O=await G.get("/api/loops");e.value=Array.isArray(O)?O:[],s.value=null}catch(O){I||(s.value=O.message)}I||(t.value=!1)}async function N(){l.value=null;const I=a.value;if(!I.goal.trim()){l.value="Goal is required";return}if(!I.channel_id.trim()){l.value="Channel ID is required";return}const O={goal:I.goal.trim(),channel_id:I.channel_id.trim(),interval_seconds:I.interval_seconds||60,mode:I.mode,max_iterations:I.max_iterations||50};I.stop_condition.trim()&&(O.stop_condition=I.stop_condition.trim()),i.value=!0;try{const U=await G.post("/api/loops",O);xe.success(`Loop started: ${U.loop_id}`),a.value={goal:"",interval_seconds:60,mode:"notify",max_iterations:50,stop_condition:"",channel_id:""},n.value=!1,await x()}catch(U){l.value=U.message}i.value=!1}async function $(I){if(await zt({title:"Stop loop",message:`Stop loop ${I}? The current iteration will finish before stopping.`,confirmLabel:"Stop Loop",danger:!0})){o.value=I;try{await G.del(`/api/loops/${encodeURIComponent(I)}`),xe.success("Loop stopped"),await x()}catch(U){xe.error(U.message||"Failed to stop loop")}o.value=null}}async function C(I){r.value=I;try{await G.post(`/api/loops/${encodeURIComponent(I)}/restart`),xe.success("Loop restarted"),await x()}catch(O){xe.error(O.message||"Failed to restart loop")}r.value=null}function M(I){m&&I.payload&&(I.payload.loop_id||I.payload.type==="loop")&&(x(!0),d.value&&k.refresh())}let V=null;function q(){V!==null&&clearInterval(V),V=null}function D(){q(),m&&(V=setInterval(()=>{x(!0),d.value&&k.refresh()},5e3))}return Ge(()=>{m=!0,x(),Qe.subscribe("events",M),D()}),gs(()=>{m=!0,x(!0),D()}),ls(()=>{m=!1,q()}),bt(()=>{m=!1,Qe.unsubscribe("events",M),q(),k.close()}),{loops:e,loading:t,error:s,showCreate:n,form:a,creating:i,createError:l,stoppingId:o,restartingId:r,detail:c,detailId:d,detailLoading:u,detailError:p,copied:h,totalIterations:b,runningCount:S,statusDotClass:w,statusBadge:A,modeBadge:E,formatAge:Am,formatDuration:ci,formatTs:Ta,formatTokens:Im,openDetail:R,closeDetail:y,copyText:g,fetchLoops:x,doCreate:N,doStop:$,doRestart:C}}},bw={template:`
    <div class="p-6 page-fade-in">
    ...
    </div>`,setup(){const e=f([]),t=f(!0),s=f(null),n=f(!1),a=f({description:"",action:"reminder",channel_id:"",cron:"",run_at:"",message:"",tool_name:"",tool_input_str:"",report_format:""}),i=f(!1),l=f(null),o=f(null),r=J(()=>ww(a.value.run_at));ns(()=>a.value.run_at,()=>{o.value=null});const c=J(()=>{var re;const z=r.value;return z.state==="ok"?z.instant:z.state==="ambiguous"&&o.value!==null&&((re=z.options[o.value])==null?void 0:re.instant)||null}),d=J(()=>{const z=c.value;return z?`${z.toLocaleString()} local — ${z.toISOString()} UTC`:""}),u=f(null),p=f(!1),h=[{label:"Every hour",expr:"0 * * * *"},{label:"Every 6h",expr:"0 */6 * * *"},{label:"Daily 9am",expr:"0 9 * * *"},{label:"Weekly Mon",expr:"0 9 * * 1"},{label:"Every 30m",expr:"*/30 * * * *"}],m=f(null),v=f(null),k=f(null),R=f(null),y=f(null),g=f(null),b=f([]),S=f(!1),w=f("");let A=0;const E=J(()=>e.value.filter(z=>z.cron&&!z.one_time).length),x=J(()=>e.value.filter(z=>z.one_time).length),N=J(()=>e.value.filter(z=>z.trigger).length),$=J(()=>e.value.filter(z=>z.paused).length),C=J(()=>e.value.filter(z=>z.consecutive_failures>0).length);function M(z){if(!z)return"-";const re=Date.now(),Ie=(new Date(z).getTime()-re)/1e3;if(Ie<0)return"overdue";if(Ie<60)return"in < 1 min";if(Ie<3600)return`in ${Math.floor(Ie/60)} min`;if(Ie<86400){const P=Math.floor(Ie/3600),H=Math.floor(Ie%3600/60);return H>0?`in ${P}h ${H}m`:`in ${P}h`}const _=Math.floor(Ie/86400);return`in ${_} day${_!==1?"s":""}`}function V(z){return z==null?"-":z<1e3?`${z}ms`:z<6e4?`${(z/1e3).toFixed(1)}s`:ci(z/1e3)}function q(z=a.value.cron){a.value.cron=z,Vu(a.value,"cron"),u.value=null}function D(z=a.value.run_at){a.value.run_at=z,Vu(a.value,"run_at"),u.value=null}async function I(){const z=a.value.cron.trim();if(z){p.value=!0;try{u.value=await G.post("/api/schedules/validate-cron",{expression:z})}catch(re){u.value={valid:!1,error:re.message}}p.value=!1}}async function O(){t.value=!0,s.value=null;try{e.value=await G.get("/api/schedules")}catch(z){s.value=z.message}t.value=!1}async function U(z){if(g.value===z){g.value=null,b.value=[];return}g.value=z,S.value=!0,b.value=[];const re=++A;try{const ue=await G.get(`/api/schedules/${encodeURIComponent(z)}/history?limit=10`);if(re!==A||g.value!==z)return;b.value=ue,w.value=""}catch(ue){if(re!==A||g.value!==z)return;b.value=[],w.value=ue.message||"Failed to load execution history"}re===A&&(S.value=!1)}async function W(){l.value=null;const z=a.value;if(!z.description.trim()){l.value="Description is required";return}if(!z.channel_id.trim()){l.value="Channel ID is required";return}if(!z.cron.trim()&&!z.run_at.trim()){l.value="Cron expression or run_at time is required";return}if(z.cron.trim()&&z.run_at.trim()){l.value="Choose either Cron or One-Time, not both";return}const re={description:z.description.trim(),action:z.action,channel_id:z.channel_id.trim()};if(z.cron.trim()&&(re.cron=z.cron.trim()),z.run_at.trim()){const ue=r.value;if(ue.state==="nonexistent"){l.value="That local time does not exist (daylight saving gap)";return}if(ue.state==="invalid"){l.value="One-time run time is not a valid date";return}const Ie=c.value;if(ue.state==="ambiguous"&&o.value===null){l.value="That local time happens twice — choose which occurrence to use";return}if(!Ie){l.value="One-time run time could not be resolved";return}re.run_at=Ie.toISOString()}if(z.action==="reminder"&&z.message.trim()&&(re.message=z.message.trim()),z.action==="check"&&(z.tool_name.trim()&&(re.tool_name=z.tool_name.trim()),z.report_format&&(re.report_format=z.report_format),z.tool_input_str.trim()))try{re.tool_input=JSON.parse(z.tool_input_str.trim())}catch{l.value="Tool input must be valid JSON";return}i.value=!0;try{await G.post("/api/schedules",re),xe.success("Schedule created"),a.value={description:"",action:"reminder",channel_id:"",cron:"",run_at:"",message:"",tool_name:"",tool_input_str:"",report_format:""},u.value=null,n.value=!1,await O()}catch(ue){l.value=ue.message}i.value=!1}async function K(z){m.value=z;try{const re=await G.post(`/api/schedules/${encodeURIComponent(z)}/run`);if(re.status==="failure")xe.error(`Execution failed: ${re.error||"unknown error"}`);else{const ue=re.warning?`Executed (${re.warning})`:"Executed successfully";xe.success(ue)}await O()}catch(re){xe.error(re.message||"Failed to trigger")}m.value=null}async function X(z){k.value=z.id;const re=!z.paused;try{await G.put(`/api/schedules/${encodeURIComponent(z.id)}`,{paused:re}),xe.success(re?"Schedule paused":"Schedule resumed"),await O()}catch(ue){xe.error(ue.message||"Failed to update schedule")}k.value=null}const oe=new Map;function ee(z,re){const ue=oe.get(z.id);ue&&clearTimeout(ue.timer);const Ie={run:()=>de(z,re),timer:null};Ie.timer=setTimeout(()=>{oe.delete(z.id),Ie.run()},500),oe.set(z.id,Ie)}async function de(z,re){y.value=z.id;try{await G.put(`/api/schedules/${encodeURIComponent(z.id)}`,{report_format:re}),xe.success(re?"Structured report enabled":"Plain-text report enabled")}catch(ue){xe.error(`Update failed: ${ue.message}`)}finally{await O(),y.value=null}}function De(){for(const[z,re]of[...oe])clearTimeout(re.timer),oe.delete(z),re.run()}async function Q(z){R.value=z;try{await G.post(`/api/schedules/${encodeURIComponent(z)}/reset-failures`),xe.success("Failure counters reset"),await O()}catch(re){xe.error(re.message||"Failed to reset")}R.value=null}async function be(z){const re=e.value.find(Ie=>Ie.id===z);if(await zt({title:"Delete schedule",message:`Delete "${(re==null?void 0:re.description)||z}"? This cannot be undone.`,confirmLabel:"Delete",danger:!0})){v.value=z;try{await G.del(`/api/schedules/${encodeURIComponent(z)}`),xe.success("Schedule deleted"),await O()}catch(Ie){xe.error(Ie.message||"Failed to delete schedule")}v.value=null}}return Ge(()=>{O()}),bt(De),{schedules:e,loading:t,error:s,showCreate:n,form:a,creating:i,createError:l,runAtUtcPreview:d,runAtAnalysis:r,runAtOccurrence:o,cronResult:u,validatingCron:p,cronPresets:h,runningId:m,deletingId:v,togglingId:k,resettingId:R,reportUpdatingId:y,flushReportFormatTimers:De,expandedId:g,history:b,historyLoading:S,historyError:w,cronCount:E,oneTimeCount:x,webhookCount:N,pausedCount:$,failingCount:C,formatTs:Ta,formatAge:Am,formatFuture:M,formatMs:V,formatDuration:ci,onCronInput:q,onRunAtInput:D,validateCron:I,toggleExpand:U,fetchSchedules:O,doCreate:W,doRunNow:K,doTogglePause:X,doUpdateReportFormat:ee,doResetFailures:Q,doDelete:be}}},Nm=[{id:"live",label:"Live",component:dw},{id:"agents",label:"Agents",component:vw},{id:"loops",label:"Loops",component:gw},{id:"processes",label:"Processes",component:bw},{id:"schedules",label:"Schedules",component:kw}],Sw={components:{TabbedPage:Vo},setup(){return{tabs:Nm}},template:'<tabbed-page :tabs="tabs" default-tab="live" group-label="Operations" />'},Tw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=f([]),t=f(!0),s=f(null),n=f(null),a=f(null),i=f(!1);let l=0;const o=f(null),r=f(!1),c=f(new Set),d=f(!1),u=f("all"),p=f(""),h=f("last_active"),m=f(!1),v=qu,k=Cw,R=f([]),y=f(!1),g=f(""),b=f("flat"),S=f(new Set),w=f(""),A=f(""),E=f(""),x=f(null),N=f(!1),$=f(""),C=f(!1);let M=0;ns([w,A,E],()=>{M++,N.value=!1,$.value="",C.value=x.value!==null},{flush:"sync"});function V(){try{const ne=localStorage.getItem("odin-session-presets");ne&&(R.value=JSON.parse(ne))}catch{}}function q(){try{localStorage.setItem("odin-session-presets",JSON.stringify(R.value))}catch{}}const D=J(()=>p.value.trim()!==""||u.value!=="all"),I=J(()=>{let ne=[...e.value];const Te=qu.find(je=>je.id===u.value),Le=Te?Te.filters:{};if(Le.source&&(ne=ne.filter(je=>je.source===Le.source)),Le.minMessages&&(ne=ne.filter(je=>je.message_count>=Le.minMessages)),Le.hasCompaction&&(ne=ne.filter(je=>je.has_summary)),Le.maxAge!=null){const je=Date.now()/1e3;ne=ne.filter(Dt=>Dt.last_active&&je-Dt.last_active<=Le.maxAge)}if(p.value.trim()){const je=p.value.toLowerCase().trim();ne=ne.filter(Dt=>(Dt.channel_id||"").toLowerCase().includes(je)||(Dt.last_user_id||"").toLowerCase().includes(je)||(Dt.source||"").toLowerCase().includes(je))}const Je=h.value,jt=m.value?1:-1;return ne.sort((je,Dt)=>{const Pt=je[Je]||0,os=Dt[Je]||0;return(Pt-os)*jt}),ne}),O=J(()=>{if(!a.value||!a.value.messages)return[];const ne=a.value.messages;if(ne.length===0)return[];const Te=[];let Le=[];for(const Je of ne)Je.role==="user"&&Le.length>0&&(Te.push(Le),Le=[]),Le.push(Je);return Le.length>0&&Te.push(Le),Te}),U=J(()=>I.value.length>0&&c.value.size===I.value.length);function W(ne){const Te=ne.find(Le=>Le.role==="user");if(Te&&Te.content){const Le=Te.content.slice(0,120);return Le.length<Te.content.length?Le+"...":Le}return"(no user message)"}function K(ne){const Te=new Set(S.value);Te.has(ne)?Te.delete(ne):Te.add(ne),S.value=Te}function X(ne){u.value=ne}function oe(ne){u.value=ne.id,ne.filters.searchQuery!=null&&(p.value=ne.filters.searchQuery),ne.filters.sortBy&&(h.value=ne.filters.sortBy)}function ee(){if(!g.value.trim())return;const ne={id:"custom-"+Date.now(),name:g.value.trim(),filters:{searchQuery:p.value,sortBy:h.value}};R.value=[...R.value,ne],q(),y.value=!1,g.value=""}function de(ne){R.value=R.value.filter(Te=>Te.id!==ne),q(),u.value===ne&&(u.value="all")}function De(){u.value="all",p.value="",h.value="last_active",m.value=!1}function Q(ne){if(!ne)return"—";const Te=Date.now()/1e3-ne;if(Te<60)return"just now";if(Te<3600){const Je=Math.floor(Te/60);return`${Je} minute${Je!==1?"s":""} ago`}if(Te<86400){const Je=Math.floor(Te/3600);return`${Je} hour${Je!==1?"s":""} ago`}const Le=Math.floor(Te/86400);return`${Le} day${Le!==1?"s":""} ago`}function be(ne){if(!ne)return"";try{return new Date(ne*1e3).toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return""}}function z(ne){if(!ne)return"";try{return new Date(ne*1e3).toLocaleString()}catch{return""}}function re(ne){return ne==="user"?"bg-gray-900/50 border border-gray-800":ne==="assistant"?"bg-indigo-950/30 border border-indigo-900/30":"bg-gray-900/30 border border-gray-800/50"}function ue(ne){return ne==="user"?"sess-msg-user":ne==="assistant"?"sess-msg-assistant":"sess-msg-system"}function Ie(ne){return ne==="user"?"badge-info":ne==="assistant"?"badge-success":"badge-warning"}function _(ne){return ne==="user"?"sess-dot-user":ne==="assistant"?"sess-dot-assistant":"sess-dot-system"}function P(ne){return ne==="user"?"text-cyan-400":ne==="assistant"?"text-indigo-400":"text-gray-500"}function H(ne){return ne?ne.length>2e3?ne.slice(0,2e3)+`
... (truncated)`:ne:""}async function ie(){const ne=w.value.trim();if(!ne)return;const Te=++M;N.value=!0,$.value="",C.value=x.value!==null;try{let Le=`/api/sessions/search?q=${encodeURIComponent(ne)}&limit=50`;A.value.trim()&&(Le+=`&channel_id=${encodeURIComponent(A.value.trim())}`),E.value.trim()&&(Le+=`&user_id=${encodeURIComponent(E.value.trim())}`);const Je=await G.get(Le);if(Te!==M)return;x.value=Je.results||[],C.value=!1}catch(Le){if(Te!==M)return;$.value=Le.message||"Search failed. Please retry."}finally{Te===M&&(N.value=!1)}}function se(){M++,w.value="",A.value="",E.value="",x.value=null,$.value="",C.value=!1,N.value=!1}function ae(ne){return ne?ne.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/&gt;&gt;&gt;/g,'<mark class="fts-highlight">').replace(/&lt;&lt;&lt;/g,"</mark>"):""}function me(ne){return ne==="user"?"fts-result-user":ne==="assistant"?"fts-result-assistant":ne==="summary"?"fts-result-summary":ne==="fts"?"fts-result-fts":ne==="channel"?"fts-result-channel":"fts-result-default"}function pe(ne){return ne==="user"?"badge-info":ne==="assistant"?"badge-success":ne==="summary"?"badge-warning":ne==="fts"?"badge-success":"badge-info"}let ce=0;async function le(){const ne=++ce;t.value=!0,s.value=null;try{const Te=await G.get("/api/sessions");if(ne!==ce)return;e.value=Te}catch(Te){if(ne!==ce)return;s.value=Te.message}ne===ce&&(t.value=!1)}function _e(){s.value=null,le()}async function ge(ne){if(n.value===ne){n.value=null,a.value=null,S.value=new Set;return}n.value=ne,a.value=null,i.value=!0,S.value=new Set;const Te=++l;try{const Le=await G.get(`/api/sessions/${encodeURIComponent(ne)}`);Te===l&&n.value===ne&&(a.value=Le)}catch(Le){Te===l&&n.value===ne&&(a.value={messages:[],summary:"",error:Le.message||"Failed to load session"})}finally{Te===l&&(i.value=!1)}}function we(ne){const Te=new Set(c.value);Te.has(ne)?Te.delete(ne):Te.add(ne),c.value=Te}function Re(){U.value?c.value=new Set:c.value=new Set(I.value.map(ne=>ne.channel_id))}function F(ne){o.value=ne}async function he(){if(o.value){r.value=!0;try{await G.del(`/api/sessions/${encodeURIComponent(o.value)}`),n.value===o.value&&(n.value=null,a.value=null),c.value.delete(o.value),await le()}catch(ne){s.value=ne.message||"Failed to clear session"}r.value=!1,o.value=null}}function Se(){d.value=!0}async function Oe(){if(c.value.size!==0){r.value=!0;try{await G.post("/api/sessions/clear-bulk",{channel_ids:[...c.value]}),c.value.has(n.value)&&(n.value=null,a.value=null),c.value=new Set,await le()}catch(ne){s.value=ne.message||"Failed to clear sessions"}r.value=!1,d.value=!1}}async function Fe(ne,Te){const Le=`/api/sessions/${encodeURIComponent(ne)}/export?format=${Te}`;try{const Je=await G.getBlob(Le),jt=URL.createObjectURL(Je),je=document.createElement("a");je.href=jt,je.download=`session-${ne}.${Te==="text"?"txt":"json"}`,je.click(),URL.revokeObjectURL(jt)}catch(Je){s.value=Je.message||"Failed to export session"}}let ft=null;function ct(ne){ne.payload&&ne.payload.channel_id&&(clearTimeout(ft),ft=setTimeout(()=>{if(le(),n.value&&ne.payload.channel_id===n.value){const Te=n.value,Le=l;G.get(`/api/sessions/${encodeURIComponent(Te)}`).then(Je=>{Le!==l||n.value!==Te||(a.value=Je)}).catch(()=>{})}},2e3))}let Et=!1,At=null;function tt(){Et||(Et=!0,le(),Qe.subscribe("events",ct),At=Qe.onReconnected(()=>le()))}Ge(()=>{V(),tt()}),gs(()=>{tt()});function Ze(){Et&&(Et=!1,Qe.unsubscribe("events",ct),At&&(At(),At=null),clearTimeout(ft))}return ls(Ze),bt(Ze),{sessions:e,loading:t,error:s,expandedId:n,detail:a,detailLoading:i,clearTarget:o,clearing:r,selected:c,allSelected:U,bulkClearing:d,activePreset:u,searchQuery:p,sortBy:h,sortAsc:m,filterPresets:v,sortOptions:k,filteredSessions:I,hasActiveFilters:D,customPresets:R,showSavePreset:y,newPresetName:g,threadView:b,threads:O,collapsedThreads:S,ftsQuery:w,ftsChannelId:A,ftsUserId:E,ftsResults:x,ftsSearching:N,ftsError:$,ftsStale:C,formatAge:Q,formatTimestamp:be,formatFullTimestamp:z,messageClass:re,threadMsgClass:ue,roleBadge:Ie,roleDotClass:_,roleLabelClass:P,truncateContent:H,threadSummary:W,fetchSessions:le,retry:_e,toggleSession:ge,toggleSelect:we,toggleSelectAll:Re,confirmClear:F,clearSession:he,confirmBulkClear:Se,doBulkClear:Oe,exportSession:Fe,applyPreset:X,applyCustomPreset:oe,saveCustomPreset:ee,removeCustomPreset:de,resetFilters:De,toggleThread:K,runFtsSearch:ie,clearFtsSearch:se,highlightSnippet:ae,ftsResultClass:me,ftsTypeBadge:pe}}},Aw={props:["trace"],template:`
  ...
  `,setup(){return{formatTokens:Im}}},Rw={components:{ContextAssemblyPanel:Aw},template:`
    ...
    </div>`,setup(){const e=f([]),t=f([]),s=f(!0),n=f(null),a=f(null),i=f(null),l=f(""),o=f(""),r=f(0),c=f({}),d=f({channel_id:"",user_id:"",tool_name:"",errors_only:!1,limit:50});function u(A){if(!A)return"—";try{const E=new Date(A);return isNaN(E.getTime())?A:E.toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"})}catch{return A}}function p(A){return!A&&A!==0?"—":A<1e3?A+"ms":(A/1e3).toFixed(1)+"s"}function h(A){return!A&&A!==0?"—":A>=1e3?(A/1e3).toFixed(1)+"k":String(A)}function m(A){if(!A)return"";if(typeof A=="string")return A;try{return JSON.stringify(A,null,2)}catch{return String(A)}}function v(A){a.value===A?a.value=null:(a.value=A,c.value={})}function k(A,E){const x=A+"-"+E;c.value={...c.value,[x]:!c.value[x]}}function R(A,E){return!!c.value[A+"-"+E]}function y(){d.value={channel_id:"",user_id:"",tool_name:"",errors_only:!1,limit:50},o.value="",l.value="",i.value=null,S()}async function g(){try{const A=await G.get("/api/trajectories");e.value=A.files||[],r.value=A.count||0}catch{}}let b=0;async function S(){const A=++b;s.value=!0,n.value=null,a.value=null,i.value=null,c.value={};try{if(o.value){const E=await G.get(`/api/trajectories/${encodeURIComponent(o.value)}?limit=${d.value.limit}`);if(A!==b)return;let x=E.entries||[];d.value.tool_name&&(x=x.filter(N=>(N.tools_used||[]).includes(d.value.tool_name))),d.value.errors_only&&(x=x.filter(N=>N.is_error)),d.value.channel_id&&(x=x.filter(N=>N.channel_id===d.value.channel_id)),d.value.user_id&&(x=x.filter(N=>N.user_id===d.value.user_id)),t.value=x}else{const E=new URLSearchParams;d.value.channel_id&&E.set("channel_id",d.value.channel_id),d.value.user_id&&E.set("user_id",d.value.user_id),d.value.tool_name&&E.set("tool_name",d.value.tool_name),d.value.errors_only&&E.set("errors_only","true"),E.set("limit",String(d.value.limit));const x=E.toString(),N=await G.get(`/api/trajectories/search/query?${x}`);if(A!==b)return;t.value=N.results||[]}}catch(E){if(A!==b)return;n.value=E.message}A===b&&(s.value=!1)}async function w(){if(!l.value.trim())return;const A=++b;s.value=!0,n.value=null,c.value={};try{const E=await G.get(`/api/trajectories/message/${encodeURIComponent(l.value.trim())}`);if(A!==b)return;i.value=E.entry||null,i.value||(n.value="No trace found for this message ID")}catch(E){if(A!==b)return;E.status===404?(i.value=null,n.value="No trace found for message ID: "+l.value):n.value=E.message}A===b&&(s.value=!1)}return Ge(async()=>{await g(),await S()}),{files:e,entries:t,loading:s,error:n,expandedIdx:a,singleTrace:i,messageIdQuery:l,selectedFile:o,totalSaved:r,filters:d,expandedIterations:c,formatTs:u,formatDuration:p,formatTokens:h,formatJSON:m,truncateBlock:Rm,toggleExpand:v,toggleIteration:k,isIterationExpanded:R,clearFilters:y,fetchFiles:g,fetchTraces:S,lookupMessage:w}}};function Iw(e){const t=Number(e);return!Number.isFinite(t)||t<=0?"—":t<1e3?`${Math.round(t)} ms`:t<6e4?`${(t/1e3).toFixed(1)} s`:t<36e5?`${(t/6e4).toFixed(1)} min`:`${(t/36e5).toFixed(1)} h`}function Ow(e){return e?`${e.approximate?"~":""}${id(e.total||0)}`:"0"}const Lw={template:`
  ...
  `,setup(){const e=f(!0),t=f(null),s=f(!1),n=f({available:!0,coverage:{},work:{},activity:[],serving:[],tools:[],automation:[]}),a=f("7d"),i=f(0),l=f(Date.now());let o=null,r=null,c=!1,d=0;const u=[{key:"24h",label:"24 hours"},{key:"7d",label:"7 days"},{key:"30d",label:"30 days"},{key:"all",label:"All time"}],p=J(()=>n.value.work||{}),h=J(()=>Math.max(1,...(n.value.activity_over_time||[]).map(w=>Number(w.count||0)))),m=J(()=>({minWidth:`max(100%, ${(n.value.activity_over_time||[]).length*5}px)`})),v=w=>({height:`${Math.max(4,Math.round(Number(w||0)/h.value*100))}%`}),k=J(()=>s.value&&l.value-i.value>3e4);async function R(){const w=++d,A=a.value;try{const E=await G.get(`/api/usage?range=${encodeURIComponent(A)}`);if(w!==d||A!==a.value)return;n.value=E,i.value=Date.now(),l.value=i.value,s.value=!0,t.value=null}catch(E){w===d&&(t.value=E.message)}finally{w===d&&(e.value=!1)}}function y(w){a.value=w,e.value=!s.value,R()}function g(){e.value=!0,R()}function b(){c||(c=!0,R(),o=setInterval(R,15e3),r=setInterval(()=>{l.value=Date.now()},1e3))}function S(){c&&(c=!1,d+=1,o&&clearInterval(o),r&&clearInterval(r),o=null,r=null)}return Ge(b),gs(b),ls(S),bt(S),{data:n,work:p,loading:e,error:t,hasData:s,range:a,ranges:u,isStale:k,fmtNum:id,fmtDuration:Iw,tokenLabel:Ow,activityTrackStyle:m,activityBar:v,selectRange:y,retry:g}}},Dm=[{id:"audit",label:"Audit",component:Tw},{id:"sessions",label:"Sessions",component:Ew},{id:"traces",label:"Traces",component:Rw},{id:"usage",label:"Usage & Activity",component:Lw}],Nw={components:{TabbedPage:Vo},setup(){return{tabs:Dm}},template:'<tabbed-page :tabs="tabs" default-tab="audit" group-label="History" />'},ur=[{id:"system",label:"System & Commands",icon:"terminal",match:e=>/^(run_command|run_script|read_file|apply_patch|list_directory|search_files|manage_process|file_|post_file)/.test(e)},{id:"devops",label:"DevOps & Infrastructure",icon:"server",match:e=>/^(git_ops|docker_ops|kubectl|terraform_ops|http_probe)/.test(e)},{id:"agents",label:"Agents & Orchestration",icon:"bot",match:e=>/^(spawn_agent|send_to_agent|wait_for_agents|get_agent_results|kill_agent|list_agents|spawn_loop_agents|collect_loop_agents)/.test(e)},{id:"workflow",label:"Workflows & Tasks",icon:"workflow",match:e=>/^(delegate_task|cancel_task|list_tasks|schedule_|start_loop|stop_loop|list_loops|delete_schedule|list_schedules|update_schedule|parse_time)/.test(e)},{id:"network",label:"Network & Web",icon:"globe",match:e=>/^(web_|browser_|search_web|fetch_url|http_)/.test(e)},{id:"knowledge",label:"Knowledge & Search",icon:"book",match:e=>/^(search_knowledge|ingest_|knowledge_|search_history|search_audit|bulk_ingest|delete_knowledge|list_knowledge)/.test(e)},{id:"discord",label:"Discord & Admin",icon:"message",match:e=>/^(send_|add_reaction|create_poll|purge_|discord_|embed_|read_channel|set_permission)/.test(e)},{id:"skills",label:"Skills",icon:"puzzle",match:e=>/^(create_skill|edit_skill|delete_skill|enable_skill|disable_skill|install_skill|export_skill|skill_status|invoke_skill|list_skills)/.test(e)},{id:"memory",label:"Memory & State",icon:"brain",match:e=>/^(memory_manage|list_manage)/.test(e)},{id:"ai",label:"AI & Generation",icon:"sparkles",match:e=>/^(generate_|analyze_|vision_|comfyui_)/.test(e)},{id:"integrations",label:"Integrations",icon:"link",match:e=>/^(issue_tracker|slack_|grafana_|mcp_)/.test(e)},{id:"other",label:"Other Tools",icon:"wrench",match:()=>!0}],Dw={template:`
    ...
    </div>`,setup(){const e=f([]),t=f(!0),s=f(null),n=f(""),a=f({}),i=f({}),l=f("cards"),o=f(null),r=f(null),c=f(!1),d=f(new Set),u={disabled:"Disabled by operator",unavailable:"Unavailable — required backend is not configured",global_disabled:"Global tools disabled"};function p(x){return x.source!=="builtin"?"":u[x.state]||""}function h(x,N){const $=x&&Array.isArray(x.tools)?x.tools:null;if(c.value=!!$,r.value=$?!!x.global_enabled:null,!$){e.value=N.map(V=>({...V,source:"unknown",enabled:void 0,state:null}));return}const C=new Set($.map(V=>V.name)),M=N.filter(V=>!C.has(V.name)).map(V=>({...V,source:V.name.startsWith("mcp_")?"mcp":"skill",enabled:!0,state:null}));e.value=[...$.map(V=>({...V,source:"builtin"})),...M]}async function m(x,N){if(d.value.has(x.name))return;const $=!!N.target.checked,C=new Set(d.value);C.add(x.name),d.value=C;try{const M=await G.post(`/api/tools/builtins/${encodeURIComponent(x.name)}/enabled`,{enabled:$});h(M,e.value),s.value=null;try{const V=await G.get("/api/tools");h(M,V)}catch(V){console.warn("Built-in toggle committed; visible catalog refresh failed",V)}}catch(M){N.target.checked=!!x.enabled,s.value=M.message||`Failed to toggle ${x.name}`}finally{const M=new Set(d.value);M.delete(x.name),d.value=M}}const v=J(()=>e.value.filter(x=>x.source==="builtin"&&x.is_core).length),k=J(()=>e.value.filter(x=>x.source==="skill").length),R=J(()=>Object.values(a.value).reduce((x,N)=>x+N,0));function y(x){for(const N of ur)if(N.id!=="other"&&N.match(x))return N.id;return"other"}const g=J(()=>{let x=e.value;if(n.value){const N=n.value.toLowerCase();x=x.filter($=>$.name.toLowerCase().includes(N)||($.description||"").toLowerCase().includes(N))}return o.value&&(x=x.filter(N=>y(N.name)===o.value)),x}),b=J(()=>{const x=new Set;for(const N of e.value)x.add(y(N.name));return ur.filter(N=>x.has(N.id))}),S=J(()=>{const x=g.value,N={};for(const C of x){const M=y(C.name);N[M]||(N[M]=[]),N[M].push(C)}const $=[];for(const C of ur)N[C.id]&&N[C.id].length>0&&$.push({label:C.label,icon:C.icon,tools:N[C.id].sort((M,V)=>M.name.localeCompare(V.name))});return $});function w(x){i.value={...i.value,[x]:!i.value[x]}}async function A(){t.value=!0,s.value=null;try{const[x,N,$]=await Promise.all([G.get("/api/tools"),G.get("/api/tools/stats").catch(()=>({})),G.get("/api/tools/builtins").catch(()=>null)]);h($,x),a.value=N||{}}catch(x){s.value=x.message}t.value=!1}function E(){A()}return Ge(()=>{A()}),{tools:e,loading:t,error:s,search:n,stats:a,expanded:i,viewMode:l,activeCategory:o,globalEnabled:r,inventoryAvailable:c,togglePending:d,coreCount:v,skillCount:k,totalUsage:R,filteredTools:g,groupedTools:S,usedCategories:b,stateBadge:p,applyInventory:h,toggleBuiltinTool:m,truncate:ad,toggleExpand:w,refresh:E}}};function Pw(e){if(!e)return"";let t=e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");t=t.replace(/("""[\s\S]*?"""|'''[\s\S]*?'''|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g,'<span class="sk-str">$1</span>'),t=t.replace(/(#[^\n]*)/g,'<span class="sk-cmt">$1</span>');const s="\\b(def|class|return|if|elif|else|for|while|import|from|as|try|except|finally|raise|with|async|await|yield|pass|break|continue|and|or|not|in|is|None|True|False|self|lambda)\\b";t=t.replace(new RegExp(s,"g"),'<span class="sk-kw">$1</span>');const n="\\b(print|len|range|str|int|float|list|dict|set|tuple|type|isinstance|hasattr|getattr|setattr|super|property|staticmethod|classmethod|enumerate|zip|map|filter|sorted|reversed|any|all|min|max|sum|abs|round|open|format)\\b";return t=t.replace(new RegExp(n,"g"),'<span class="sk-builtin">$1</span>'),t=t.replace(/(@\w+)/g,'<span class="sk-dec">$1</span>'),t=t.replace(/\b(\d+\.?\d*)\b/g,'<span class="sk-num">$1</span>'),t}function Mw(e){if(!e)return"1";const t=e.split(`
...
`)}const Fw={template:`
    ...
    </div>`,setup(){const e=f([]),t=f(!0),s=f(null),n=f({}),a=f({}),i=f(null),l=f(""),o=f(null),r=f(!1),c=f("create"),d=f(""),u=f(""),p=f(null),h=f(null),m=f(!1),v=f(null),k=f(null),R=f(!1),y=J(()=>e.value.length),g=J(()=>e.value.reduce((ee,de)=>ee+(de.execution_count||0),0)),b=J(()=>e.value.reduce((ee,de)=>ee+N(de.code),0)),S=J(()=>{if(!l.value)return e.value;const ee=l.value.toLowerCase();return e.value.filter(de=>de.name.toLowerCase().includes(ee)||(de.description||"").toLowerCase().includes(ee))}),w=J(()=>u.value?u.value.split(`
`).length:0),A=J(()=>{const ee=Math.max(w.value,1);return Array.from({length:ee},(de,De)=>De+1).join(`
`)}),E=J(()=>{const ee=u.value.trim();return ee?ee.includes("SKILL_DEFINITION")?ee.includes("async def execute")?{valid:!0,message:""}:{valid:!1,message:"Missing async def execute function"}:{valid:!1,message:"Missing SKILL_DEFINITION dict"}:null});function x(ee){return Pw(ee)}function N(ee){return ee?ee.split(`
`).length:0}function $(ee){return Mw(ee)}function C(ee){n.value={...n.value,[ee]:!n.value[ee]}}async function M(ee){try{await navigator.clipboard.writeText(ee);const de=e.value.find(De=>De.code===ee);de&&(o.value=de.name,setTimeout(()=>{o.value=null},2e3))}catch{}}function V(ee){if(ee.key==="Tab"){ee.preventDefault();const de=ee.target,De=de.selectionStart,Q=de.selectionEnd;u.value=u.value.substring(0,De)+"    "+u.value.substring(Q),It(()=>{de.selectionStart=de.selectionEnd=De+4})}}function q(ee){const de=ee.target.previousElementSibling;de&&(de.scrollTop=ee.target.scrollTop)}async function D(){t.value=!0,s.value=null;try{e.value=await G.get("/api/skills")}catch(ee){s.value=ee.message}t.value=!1}async function I(ee){i.value=ee,delete a.value[ee],a.value={...a.value};try{const de=await G.post(`/api/skills/${encodeURIComponent(ee)}/test`);a.value={...a.value,[ee]:de}}catch(de){a.value={...a.value,[ee]:{result:de.message,is_error:!0}}}i.value=null}function O(){r.value=!0,c.value="create",d.value="",u.value="",p.value=null,h.value=null}function U(ee){r.value=!0,c.value="edit",d.value=ee.name,u.value=ee.code||"",p.value=null,h.value=null}function W(){r.value=!1,p.value=null,h.value=null}async function K(){p.value=null,h.value=null;const ee=d.value.trim(),de=u.value.trim();if(!ee){p.value="Name is required";return}if(!de){p.value="Code is required";return}m.value=!0;try{c.value==="create"?(await G.post("/api/skills",{name:ee,code:de}),h.value="Skill created successfully"):(await G.put(`/api/skills/${encodeURIComponent(ee)}`,{code:de}),h.value="Skill updated successfully"),await D(),setTimeout(()=>{r.value=!1},800)}catch(De){p.value=De.message}m.value=!1}function X(ee){k.value=ee}async function oe(){if(k.value){R.value=!0;try{await G.del(`/api/skills/${encodeURIComponent(k.value)}`),await D()}catch(ee){xe.error(`Failed to delete skill: ${ee.message||"unknown error"}`)}R.value=!1,k.value=null}}return Ge(()=>{D()}),{skills:e,loading:t,error:s,showCode:n,testResults:a,testing:i,search:l,copied:o,editing:r,editMode:c,editName:d,editCode:u,editError:p,editSuccess:h,saving:m,editorRef:v,deleteTarget:k,deleting:R,enabledCount:y,totalExecutions:g,totalLines:b,displayedSkills:S,editLineCount:w,editorLineNums:A,editValidation:E,highlight:x,truncate:ad,formatTs:Ta,countLines:N,getLineNumbers:$,toggleCode:C,copyCode:M,handleEditorKey:V,syncScroll:q,fetchSkills:D,testSkill:I,showCreate:O,editSkill:U,cancelEdit:W,saveSkill:K,confirmDelete:X,doDelete:oe}}};class Ms extends Error{constructor(t,s=""){super(t),this.name="MCPFormError",this.field=s}}const $w=/^[A-Za-z_][A-Za-z0-9_]*$/;function Gu(e){return String(e||"").split(/\r?\n/).map(t=>t.trim()).filter(Boolean)}function Ku(e,t,s){const n={},a=[...new Set((t||[]).map(l=>String(l)))],i=new Set(a);for(const l of e||[]){const o=String((l==null?void 0:l.key)||"").trim(),r=String((l==null?void 0:l.value)??"");if(!(!o&&!r)){if(!o)throw new Ms(`${s} key is required when a value is entered.`,"authentication");if(/[\r\n\0]/.test(o))throw new Ms(`${s} keys cannot contain line breaks or NUL bytes.`,"authentication");if(Object.hasOwn(n,o))throw new Ms(`${s} key “${o}” appears more than once.`,"authentication");if(i.has(o))throw new Ms(`${s} key “${o}” cannot be replaced and removed in the same save.`,"authentication");n[o]=r}}return{set:n,remove:a}}function Bw(e){try{const t=new URL(e);return(t.protocol==="http:"||t.protocol==="https:")&&!!t.hostname}catch{return!1}}function Uw(e,{mode:t="add",originalTransport:s=""}={}){const n=t==="add",a=String(e.name||"").trim();if(!a)throw new Ms("Server name is required.","name");if(a.length>128||!$w.test(a))throw new Ms("Use at most 128 letters, digits, or underscores, with no leading digit.","name");const i=e.transport==="http"?"http":"stdio",l=!n&&!!s&&i!==s,o={enabled:!!e.enabled,transport:i};if(n&&(o.name=a),i==="stdio"){const d=String(e.command||"").trim();if((n||l)&&!d)throw new Ms("An executable path is required for a new stdio connection.","command");if(d&&(o.command=d),(n||e.replaceArgs)&&(o.args=Gu(e.argsText)),n||e.replaceCwd){const u=String(e.cwd||"").trim();if(u&&(!u.startsWith("/")||u.includes("\0")))throw new Ms("Working directory must be an absolute path.","cwd");o.cwd=u}}else{const d=String(e.url||"").trim();if((n||l)&&!d)throw new Ms("An HTTP endpoint is required for this connection.","url");if(d&&!Bw(d))throw new Ms("Endpoint must be a valid http:// or https:// URL.","url");d&&(o.url=d)}if(n||e.replaceTimeout){const d=Number(e.timeoutSeconds);if(!Number.isInteger(d)||d<1||d>3600)throw new Ms("Timeout must be a whole number from 1 to 3600 seconds.","timeout");o.timeout_seconds=d}(n||e.replaceAllowlist)&&(o.tool_allowlist=Gu(e.allowlistText));const r=Ku(e.headerRows,e.headersRemove,"Header"),c=Ku(e.envRows,e.envRemove,"Environment variable");return Object.keys(r.set).length&&(o.headers_set=r.set),r.remove.length&&(o.headers_remove=r.remove),Object.keys(c.set).length&&(o.env_set=c.set),c.remove.length&&(o.env_remove=c.remove),o}function Hw(e,t){return t?e.transport!==t.transport||!!e.enabled!=!!t.enabled?!0:Object.keys(e).some(s=>!["enabled","transport"].includes(s)):!1}function zw(e){const t=String(e||"").toLowerCase();return["disabled","connecting","connected","stale","error","blocked"].includes(t)?t:"error"}function jw(e,t){const s=String(t||"").trim().toLowerCase();return s?[e==null?void 0:e.original_name,e==null?void 0:e.published_name,e==null?void 0:e.description,e==null?void 0:e.exclusion_reason].filter(Boolean).some(n=>String(n).toLowerCase().includes(s)):!0}const Vw=Object.freeze([{id:"identity",label:"Identity"},{id:"transport",label:"Transport"},{id:"authentication",label:"Authentication"},{id:"limits",label:"Limits"}]);function qw(e,{root:t=document,reducedMotion:s=typeof window<"u"&&(n=>(n=window.matchMedia)==null?void 0:n.call(window,"(prefers-reduced-motion: reduce)").matches)()}={}){var l;const a=t.querySelector(".mcp-editor-groups"),i=a==null?void 0:a.querySelector(`#mcp-form-${e}`);return i?(i.scrollIntoView({behavior:s?"auto":"smooth",block:"start",inline:"nearest"}),(l=i.querySelector("[data-mcp-form-heading]"))==null||l.focus({preventScroll:!0}),!0):!1}const Gw=1e4,Kw=Object.freeze({disabled:"Disabled",connecting:"Connecting",connected:"Connected",stale:"Stale",error:"Error",blocked:"Blocked"});function pr(){return{name:"",enabled:!0,transport:"stdio",command:"",argsText:"",cwd:"",url:"",timeoutSeconds:120,allowlistText:"",replaceArgs:!1,replaceCwd:!1,replaceTimeout:!1,replaceAllowlist:!1,headerRows:[],envRows:[],headersRemove:[],envRemove:[]}}function Ww(e){if(e==null)return"Never";const t=Math.max(0,Number(e)||0);return t<60?`${Math.round(t)}s ago`:t<3600?`${Math.round(t/60)}m ago`:t<86400?`${Math.round(t/3600)}h ago`:`${Math.round(t/86400)}d ago`}const Zw={template:`
  ...
  `,setup(){const e=f(null),t=f(!1),s=f(!1),n=f(""),a=f(new Set),i=f(new Set),l=f({}),o=f({}),r=f({}),c=f(new Set),d=f(!1),u=f("add"),p=f(""),h=f(null),m=f(pr()),v=f(""),k=f(!1);let R=null,y=0,g=!1,b=!1;const S=Vw,w=J(()=>{var F;return((F=e.value)==null?void 0:F.servers)||[]}),A=J(()=>{var F;return!!((F=e.value)!=null&&F.enabled)}),E=J(()=>{var F,he,Se,Oe;return{serverCount:((F=e.value)==null?void 0:F.server_count)||0,enabledCount:((he=e.value)==null?void 0:he.enabled_server_count)||0,connectedCount:((Se=e.value)==null?void 0:Se.connected_count)||0,toolCount:((Oe=e.value)==null?void 0:Oe.published_tool_count)||0}}),x=J(()=>{var F;return((F=h.value)==null?void 0:F.header_keys)||[]}),N=J(()=>{var F;return((F=h.value)==null?void 0:F.env_keys)||[]}),$=J(()=>{var F;return u.value==="edit"&&((F=h.value)==null?void 0:F.transport)==="http"}),C=J(()=>u.value==="add"||!$.value),M=J(()=>$.value?"Replace endpoint URL":"Endpoint URL"),V=J(()=>$.value?"Leave blank to keep the saved endpoint":"https://mcp.example.com/mcp");function q(){D(),R=window.setInterval(()=>I({quiet:!0}),Gw)}function D(){R&&window.clearInterval(R),R=null}async function I({quiet:F=!1}={}){const he=++y;F||(t.value=!0);try{const Se=await G.get("/api/mcp/status");if(he!==y||!g)return;e.value=Se,n.value="";const Oe=new Set((Se.servers||[]).map(Fe=>Fe.name));i.value=new Set([...i.value].filter(Fe=>Oe.has(Fe)))}catch(Se){he===y&&g&&(n.value=Se.message||"Failed to load MCP status")}finally{he===y&&(t.value=!1)}}function O(F){return s.value||a.value.has(F)}function U(F,he){const Se=new Set(a.value);he?Se.add(F):Se.delete(F),a.value=Se}function W(F){return zw(F.state)}function K(F){if(W(F)==="disabled"){if(!F.enabled)return"Disabled — server switch off";if(!A.value)return"Disabled — global MCP is off"}return Kw[W(F)]}function X(F){return F.transport==="http"?"Streamable HTTP":"stdio"}function oe(F){return F.negotiated_version?`${F.era?`${String(F.era).charAt(0).toUpperCase()}${String(F.era).slice(1)}`:"Protocol"} · ${F.negotiated_version}`:"Not negotiated"}function ee(F){return F.discovered_count?`${F.published_count||0} published · ${F.excluded_count||0} excluded`:"No tools discovered"}const de=f(new Set);async function De(F,he){if(de.value.has(F.name))return;const Se=!!he.target.checked,Oe=new Set(de.value);Oe.add(F.name),de.value=Oe;try{const Fe=await G.post(`/api/mcp/servers/${encodeURIComponent(F.name)}/enabled`,{enabled:Se});Fe&&Array.isArray(Fe.servers)?e.value=Fe:await I({quiet:!0})}catch(Fe){he.target.checked=!!F.enabled,xe.error(Fe.message||`Failed to toggle ${F.name}`)}finally{const Fe=new Set(de.value);Fe.delete(F.name),de.value=Fe}}async function Q(F){if(F!==A.value&&!(!F&&!await zt({title:"Disable MCP tool publication",message:"Disable MCP globally? All MCP tools will be unpublished immediately and active transports will be stopped. Saved server configuration remains.",confirmLabel:"Disable MCP",danger:!0}))){s.value=!0;try{await G.post("/api/mcp/enabled",{enabled:F}),xe.success(F?"MCP enabled":"MCP disabled"),await I({quiet:!0})}catch(he){xe.error(he.message||"Failed to update MCP state"),await I({quiet:!0})}finally{s.value=!1}}}async function be(F){U(F.name,!0);try{await G.post(`/api/mcp/servers/${encodeURIComponent(F.name)}/reconnect`,{}),xe.success(`Reconnected ${F.name}`)}catch(he){xe.error(he.message||`Failed to reconnect ${F.name}`)}finally{U(F.name,!1),await I({quiet:!0})}}async function z(F){U(F.name,!0);try{await G.post(`/api/mcp/servers/${encodeURIComponent(F.name)}/refresh-tools`,{}),xe.success(`Refreshed tools from ${F.name}`),await Ie(F.name,!0)}catch(he){xe.error(he.message||`Failed to refresh ${F.name}`)}finally{U(F.name,!1),await I({quiet:!0})}}async function re(F){if(await zt({title:`Remove ${F.name}`,message:`Remove this saved MCP server? Its ${F.published_count||0} published tool${F.published_count===1?"":"s"} will disappear immediately and configured authentication keys will be deleted. This cannot be undone.`,confirmLabel:"Remove server",danger:!0})){U(F.name,!0);try{await G.del(`/api/mcp/servers/${encodeURIComponent(F.name)}`),xe.success(`Removed ${F.name}`),delete o.value[F.name]}catch(Se){xe.error(Se.message||`Failed to remove ${F.name}`)}finally{U(F.name,!1),await I({quiet:!0})}}}async function ue(F){const he=new Set(i.value);if(he.has(F.name)){he.delete(F.name),i.value=he;return}he.add(F.name),i.value=he,Object.hasOwn(o.value,F.name)||await Ie(F.name)}async function Ie(F,he=!1){if(!he&&Object.hasOwn(o.value,F))return;const Se=new Set(c.value);Se.add(F),c.value=Se,r.value={...r.value,[F]:""};try{const Oe=await G.get(`/api/mcp/servers/${encodeURIComponent(F)}/tools`);o.value={...o.value,[F]:Oe.tools||[]}}catch(Oe){r.value={...r.value,[F]:Oe.message||"Failed to load tools"}}finally{const Oe=new Set(c.value);Oe.delete(F),c.value=Oe}}function _(F){return(o.value[F]||[]).filter(he=>jw(he,l.value[F]))}function P(F,he){l.value={...l.value,[F]:he}}function H(){u.value="add",p.value="",h.value=null,m.value=pr(),v.value="",d.value=!0}function ie(F){u.value="edit",p.value=F.name,h.value=F,m.value={...pr(),name:F.name,enabled:!!F.enabled,transport:F.transport||"stdio"},v.value="",d.value=!0}function se(){k.value||(d.value=!1)}function ae(F){d.value&&qw(F)}function me(F){const he=F==="headers"?"headerRows":"envRows";m.value[he].push({key:"",value:""})}function pe(F,he){const Se=F==="headers"?"headerRows":"envRows";m.value[Se].splice(he,1)}function ce(F,he){const Se=F==="headers"?"headersRemove":"envRemove",Oe=m.value[Se];m.value[Se]=Oe.includes(he)?Oe.filter(Fe=>Fe!==he):[...Oe,he]}async function le(){var he,Se;v.value="";let F;try{F=Uw(m.value,{mode:u.value,originalTransport:((he=h.value)==null?void 0:he.transport)||""})}catch(Oe){v.value=Oe instanceof Ms?Oe.message:"Invalid MCP server configuration",await It(),(Se=document.querySelector(".mcp-editor"))==null||Se.scrollTo({top:0,behavior:"smooth"});return}if(!(u.value==="edit"&&Hw(F,h.value)&&!await zt({title:`Change ${p.value} connection`,message:"Saving this configuration replaces the server runtime. Any current connection will be retired and its tools unpublished; enabled servers reconnect after the change.",confirmLabel:"Save and reconnect",danger:!0}))){k.value=!0;try{u.value==="add"?await G.post("/api/mcp/servers",F):await G.put(`/api/mcp/servers/${encodeURIComponent(p.value)}`,F),xe.success(u.value==="add"?`Saved ${F.name}`:`Updated ${p.value}`),d.value=!1,await I({quiet:!0})}catch(Oe){v.value=Oe.message||"Failed to save MCP server"}finally{k.value=!1}}}let _e=null;function ge(F){`${(F==null?void 0:F.event)||""} ${(F==null?void 0:F.type)||""} ${(F==null?void 0:F.tool)||""} ${(F==null?void 0:F.message)||""}`.toLowerCase().includes("mcp")&&(_e&&window.clearTimeout(_e),_e=window.setTimeout(()=>I({quiet:!0}),200))}function we(){g||(g=!0,b||(Qe.subscribe("events",ge),b=!0),I(),q())}function Re(){g=!1,D(),_e&&window.clearTimeout(_e),_e=null,b&&(Qe.unsubscribe("events",ge),b=!1)}return Ge(we),gs(we),ls(Re),bt(Re),{status:e,loading:t,mutating:s,pageError:n,servers:w,masterEnabled:A,aggregate:E,expandedServers:i,toolQueries:l,toolErrors:r,toolsLoading:c,editorOpen:d,editorMode:u,editingName:p,editingServer:h,form:m,formError:v,saving:k,editorGroups:S,configuredHeaderKeys:x,configuredEnvKeys:N,savedHttpEndpoint:$,endpointRequired:C,endpointFieldLabel:M,endpointPlaceholder:V,refreshAll:I,busy:O,serverState:W,stateLabel:K,transportLabel:X,protocolLabel:oe,toolSummary:ee,formatAge:Ww,setMasterEnabled:Q,togglePending:de,toggleServerEnabled:De,reconnect:be,refreshTools:z,removeServer:re,toggleTools:ue,filteredTools:_,setToolQuery:P,openAdd:H,openEdit:ie,closeEditor:se,jumpToEditorGroup:ae,addSecretRow:me,removeSecretRow:pe,toggleSecretRemoval:ce,saveServer:le}}};function Jw(e,t){if(!e||!t)return ju(e);const s=ju(e),n=t.trim().split(/\s+/).filter(Boolean);if(!n.length)return s;const a=n.map(i=>i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")).join("|");try{return s.replace(new RegExp(`(${a})`,"gi"),'<mark class="knowledge-highlight">$1</mark>')}catch{return s}}const Yw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=f([]),t=f(!0),s=f(null),n=f(""),a=f(null),i=f(!1),l=f(""),o=f(null),r=f(!1),c=f(""),d=f(""),u=f(null),p=f(null),h=f(!1),m=f(null),v=f(null);let k=null;const R=f(null),y=f(!1),g=f({}),b=f({}),S=f({}),w=f({}),A=new Map,E=f(null),x=J(()=>e.value.reduce((K,X)=>K+(X.chunks||0),0)),N=J(()=>new Set(e.value.map(X=>X.uploader).filter(Boolean)).size);function $(K,X){const oe=b.value[X];if(!oe||oe.length===0)return 0;const ee=Math.max(...oe.map(de=>de.char_count||0));return ee===0?0:Math.round(K.char_count/ee*100)}async function C(){t.value=!0,s.value=null;try{const K=await G.get("/api/knowledge");e.value=Array.isArray(K)?K:[]}catch(K){s.value=K.message}t.value=!1}async function M(K){if(g.value[K]){g.value[K]=!1,E.value=null;return}if(g.value[K]=!0,Object.prototype.hasOwnProperty.call(b.value,K))return;if(A.has(K))return A.get(K);const X={...w.value,[K]:!0};w.value=X;const oe={...S.value};delete oe[K],S.value=oe;const ee=G.get(`/api/knowledge/${encodeURIComponent(K)}/chunks`).then(de=>{b.value={...b.value,[K]:Array.isArray(de)?de:[]}}).catch(de=>{S.value={...S.value,[K]:de.message||"load failed"}}).finally(()=>{if(A.get(K)!==ee)return;A.delete(K);const de={...w.value};delete de[K],w.value=de});return A.set(K,ee),ee}let V=0;async function q(){const K=n.value.trim();if(!K)return;const X=++V;i.value=!0,o.value=null,l.value=K;try{const oe=await G.get(`/api/knowledge/search?q=${encodeURIComponent(K)}`);if(X!==V)return;a.value=Array.isArray(oe)?oe:[]}catch(oe){if(X!==V)return;a.value=[],o.value=oe.message||"Search failed"}X===V&&(i.value=!1)}function D(){V+=1,i.value=!1,a.value=null,n.value="",o.value=null}async function I(){u.value=null,p.value=null;const K=c.value.trim(),X=d.value.trim();if(!K){u.value="Source name is required";return}if(!X){u.value="Content is required";return}h.value=!0;try{const oe=await G.post("/api/knowledge",{source:K,content:X});p.value=`Ingested ${oe.chunks||0} chunks from "${K}"`,c.value="",d.value="",b.value={},await C(),setTimeout(()=>{r.value=!1,p.value=null},1500)}catch(oe){u.value=oe.message}h.value=!1}async function O(K){m.value=K,v.value=null,k&&(clearTimeout(k),k=null);try{const X=await G.post(`/api/knowledge/${encodeURIComponent(K)}/reingest`);v.value={source:K,error:!1,message:`Re-ingested ${X.chunks||0} chunks`},delete b.value[K],await C(),k=setTimeout(()=>{v.value=null,k=null},3e3)}catch(X){v.value={source:K,error:!0,message:X.message}}m.value=null}function U(K){R.value=K}async function W(){if(R.value){y.value=!0;try{await G.del(`/api/knowledge/${encodeURIComponent(R.value)}`),delete b.value[R.value],await C()}catch(K){xe.error(`Failed to delete source: ${K.message||"unknown error"}`)}y.value=!1,R.value=null}}return Ge(()=>{C()}),{sources:e,loading:t,error:s,searchQuery:n,searchResults:a,searching:i,lastQuery:l,searchError:o,showIngest:r,ingestSource:c,ingestContent:d,ingestError:u,ingestSuccess:p,ingesting:h,reingesting:m,reingestResult:v,deleteTarget:R,deleting:y,expanded:g,sourceChunks:b,chunkErrors:S,loadingChunks:w,selectedChunk:E,totalChunks:x,uploaderCount:N,truncate:ad,formatTs:Ta,highlightTerms:Jw,chunkBarWidth:$,fetchSources:C,toggleSource:M,doSearch:q,clearSearch:D,doIngest:I,doReingest:O,confirmDelete:U,doDelete:W}}},Qw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=f([]),t=f({}),s=f(!0),n=f(null),a=f({}),i=f(null),l=f(""),o=f(!1),r=f({scope:"global",key:"",value:""}),c=f(!1),d=f(null),u=f(null),p=f(null),h=f(""),m=f(!1),v=f(null),k=f(null),R=f(new Set),y=f(null),g=f(!1),b=f(!1),S=J(()=>e.value.reduce((X,oe)=>X+oe.count,0)),w=J(()=>R.value.size);function A(X){const oe=t.value[X];if(!oe)return[];if(!l.value.trim())return oe;const ee=l.value.trim().toLowerCase();return oe.filter(de=>de.key.toLowerCase().includes(ee)||de.value&&de.value.toLowerCase().includes(ee))}function E(X,oe){return R.value.has(X+"/"+oe)}function x(X,oe){const ee=X+"/"+oe,de=new Set(R.value);de.has(ee)?de.delete(ee):de.add(ee),R.value=de}function N(X){const oe=t.value[X];return!oe||oe.length===0?!1:oe.every(ee=>R.value.has(X+"/"+ee.key))}function $(X,oe){const ee=t.value[X];if(!ee)return;const de=new Set(R.value);for(const De of ee){const Q=X+"/"+De.key;oe?de.add(Q):de.delete(Q)}R.value=de}async function C(){s.value=!0,n.value=null;try{const X=await G.get("/api/memory");e.value=Object.entries(X).map(([oe,ee])=>({name:oe,keys:ee.keys||[],count:ee.count||0}))}catch(X){n.value=X.message}s.value=!1}async function M(X){if(a.value[X]){a.value[X]=!1;return}a.value[X]=!0;const oe=e.value.find(de=>de.name===X);if(!oe||t.value[X]||i.value===X)return;i.value=X;let ee;try{const De=(await G.get(`/api/memory/${encodeURIComponent(X)}`)).entries||{};ee=oe.keys.map(Q=>Object.prototype.hasOwnProperty.call(De,Q)?{key:Q,value:De[Q]||"",failed:!1}:{key:Q,value:"",failed:!0,error:"Not found in scope"})}catch(de){ee=oe.keys.map(De=>({key:De,value:"",failed:!0,error:de.message||"Failed to load"}))}t.value[X]=ee,i.value=null}function V(X,oe,ee){p.value=X+"/"+oe,h.value=ee}async function q(X,oe){m.value=!0,v.value=null;try{await G.put(`/api/memory/${encodeURIComponent(X)}/${encodeURIComponent(oe)}`,{value:h.value});const ee=t.value[X];if(ee){const de=ee.find(De=>De.key===oe);de&&(de.value=h.value)}p.value=null}catch(ee){v.value=`Failed to save: ${ee.message||"unknown error"}`}m.value=!1}async function D(X,oe){try{await navigator.clipboard.writeText(oe.value),k.value=X+"/"+oe.key,setTimeout(()=>{k.value=null},1500)}catch{}}async function I(){d.value=null,u.value=null;const X=r.value.scope.trim(),oe=r.value.key.trim(),ee=r.value.value.trim();if(!X){d.value="Scope is required";return}if(!oe){d.value="Key is required";return}if(!ee){d.value="Value is required";return}c.value=!0;try{await G.put(`/api/memory/${encodeURIComponent(X)}/${encodeURIComponent(oe)}`,{value:ee}),u.value="Entry saved",r.value={scope:"global",key:"",value:""},t.value={},await C(),setTimeout(()=>{o.value=!1,u.value=null},800)}catch(de){d.value=de.message}c.value=!1}function O(X,oe){y.value={scope:X,key:oe}}async function U(){if(!y.value)return;g.value=!0,v.value=null;const{scope:X,key:oe}=y.value;try{await G.del(`/api/memory/${encodeURIComponent(X)}/${encodeURIComponent(oe)}`);const ee=t.value[X];ee&&(t.value[X]=ee.filter(Q=>Q.key!==oe));const de=e.value.find(Q=>Q.name===X);de&&(de.count--,de.keys=de.keys.filter(Q=>Q!==oe));const De=new Set(R.value);De.delete(X+"/"+oe),R.value=De}catch(ee){v.value=`Failed to delete: ${ee.message||"unknown error"}`}g.value=!1,y.value=null}function W(){b.value=!0}async function K(){g.value=!0,v.value=null;const X=[];for(const oe of R.value){const ee=oe.indexOf("/");X.push({scope:oe.slice(0,ee),key:oe.slice(ee+1)})}try{await G.post("/api/memory/bulk-delete",{entries:X}),R.value=new Set,t.value={},await C()}catch(oe){v.value=`Bulk delete failed: ${oe.message||"unknown error"}`}g.value=!1,b.value=!1}return Ge(()=>{C()}),{scopes:e,scopeEntries:t,loading:s,error:n,expanded:a,loadingScope:i,filterQuery:l,showAdd:o,addForm:r,adding:c,addError:d,addSuccess:u,editingKey:p,editValue:h,saving:m,actionError:v,copied:k,selected:R,selectedCount:w,totalEntries:S,deleteTarget:y,deleting:g,showBulkDelete:b,fetchMemory:C,toggleScope:M,startEdit:V,doEdit:q,copyValue:D,doAdd:I,confirmDelete:O,doDelete:U,confirmBulkDelete:W,doBulkDelete:K,isSelected:E,toggleSelect:x,isScopeAllSelected:N,toggleSelectAll:$,filteredEntries:A}}},Xw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
        <div>
  ...
  `,setup(){const e=f(!0),t=f(null),s=f(!1),n=f(!1),a=f("sessions"),i=f(null);let l=null;const o=[{key:"sessions",label:"Sessions"},{key:"knowledge",label:"Knowledge"},{key:"trajectories",label:"Trajectories"},{key:"storage",label:"Storage"}],r=J(()=>{if(!i.value||!i.value.collected_at)return"";try{return new Date(i.value.collected_at).toLocaleTimeString()}catch{return""}}),c=J(()=>{if(!i.value)return[];const k=i.value,R=k.storage_total_bytes||1;return[{label:"Session Persistence",mb:k.sessions.persist_dir.total_mb,bytes:k.sessions.persist_dir.total_bytes,files:k.sessions.persist_dir.file_count,pct:Math.min(100,Math.round(k.sessions.persist_dir.total_bytes/R*100)),color:"res-bar-blue"},{label:"Knowledge Database",mb:k.knowledge.db_file.total_mb,bytes:k.knowledge.db_file.total_bytes,files:k.knowledge.db_file.file_count,pct:Math.min(100,Math.round(k.knowledge.db_file.total_bytes/R*100)),color:"res-bar-purple"},{label:"Message Trajectories",mb:k.trajectories.message_dir.total_mb,bytes:k.trajectories.message_dir.total_bytes,files:k.trajectories.message_dir.file_count,pct:Math.min(100,Math.round(k.trajectories.message_dir.total_bytes/R*100)),color:"res-bar-emerald"},{label:"Agent Trajectories",mb:k.trajectories.agent_dir.total_mb,bytes:k.trajectories.agent_dir.total_bytes,files:k.trajectories.agent_dir.file_count,pct:Math.min(100,Math.round(k.trajectories.agent_dir.total_bytes/R*100)),color:"res-bar-amber"}]});async function d(){try{const k=await G.get("/api/resource-usage");i.value=k,t.value=null,s.value=!0}catch(k){t.value=k.message||"Failed to load resource usage"}finally{e.value=!1,n.value=!1}}async function u(){n.value=!0,await d()}function p(){e.value=!0,t.value=null,d()}let h=!1;function m(){h||(h=!0,d(),l||(l=setInterval(d,3e4)))}function v(){h&&(h=!1,l&&(clearInterval(l),l=null))}return Ge(m),gs(m),ls(v),bt(v),{hasData:s,loading:e,error:t,refreshing:n,data:i,activeTab:a,tabs:o,collectedAt:r,storageItems:c,fmtNum:id,refresh:u,retry:p}}},lk=["INFO","WARNING","ERROR"],ok=[{id:"all",name:"All Logs",icon:"list",filters:{}},{id:"errors",name:"Errors Only",icon:"error",filters:{level:"ERROR"}},{id:"warnings",name:"Warnings+",icon:"warning",filters:{levels:["WARNING","ERROR"]}},{id:"tools",name:"Tool Activity",icon:"wrench",filters:{hasToolName:!0}},{id:"recent-errors",name:"Recent Errors",icon:"flame",filters:{level:"ERROR",timeRange:"last_1h"}}],fr=[{value:"",label:"All Time"},{value:"last_5m",label:"Last 5 min",seconds:300},{value:"last_15m",label:"Last 15 min",seconds:900},{value:"last_1h",label:"Last 1 hour",seconds:3600},{value:"last_4h",label:"Last 4 hours",seconds:14400},{value:"last_24h",label:"Last 24 hours",seconds:86400}],rk=[50,100,200,500],ck={template:`
    ...
    </div>`,setup(){const e=f("live"),t=f([]),s=f(!1),n=f(!0),a=f(""),i=f(""),l=f(!1),o=f(!1),r=f(Qe.state||"disconnected"),c=J(()=>{switch(r.value){case"connected":return"Live";case"connecting":return"Connecting…";case"reconnecting":return"Reconnecting…";default:return"Disconnected"}}),d=f(null),u=f(!1),p=f(null),h=2e3,m=lk,v=ok,k=fr,R=f("all"),y=f(""),g=f([]),b=f(!1),S=f(""),w=f([]);function A(){try{const Z=localStorage.getItem("odin-log-presets");Z&&(g.value=JSON.parse(Z))}catch{}}function E(){try{localStorage.setItem("odin-log-presets",JSON.stringify(g.value))}catch{}}const x=J(()=>a.value!==""||i.value.trim()!==""||y.value!==""),N=J(()=>{const Z=fr.find(fe=>fe.value===y.value);return Z?Z.label:""}),$=J(()=>{if(!l.value||!i.value)return null;try{return new RegExp(i.value,"i"),null}catch(Z){return Z.message}}),C=24,M=J(()=>{if(W.value.length===0)return[];const Z=[],fe=new Date,Ne=3600*1e3;for(let Ve=C-1;Ve>=0;Ve--){const st=new Date(fe.getTime()-(Ve+1)*Ne),Mt=new Date(fe.getTime()-Ve*Ne);Z.push({start:st,end:Mt,label:I(st,Mt),shortLabel:Mt.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),total:0,info:0,warnings:0,errors:0})}for(const Ve of W.value){if(!Ve._time)continue;const st=Ve._time.getTime();for(const Mt of Z)if(st>=Mt.start.getTime()&&st<Mt.end.getTime()){Mt.total++,Ve.level==="ERROR"?Mt.errors++:Ve.level==="WARNING"?Mt.warnings++:Mt.info++;break}}return Z}),V=J(()=>{let Z=1;for(const fe of M.value)fe.total>Z&&(Z=fe.total);return Z}),q=J(()=>{if(M.value.length===0)return"";const Z=W.value.map(Ve=>Ve._time&&Ve._time.getTime()).filter(Boolean);if(Z.length===0)return"";const fe=new Date(Math.min(...Z));return`${W.value.length} shown, oldest ${fe.toLocaleTimeString()}`}),D=J(()=>Math.ceil(C/8));function I(Z,fe){const Ne={hour:"2-digit",minute:"2-digit"};return Z.toLocaleTimeString([],Ne)+" - "+fe.toLocaleTimeString([],Ne)}function O(Z,fe){return!fe||!Z?"0px":Math.max(2,Z/fe*100)+"%"}function U(Z){const fe=W.value.findIndex(Ne=>Ne._time&&Ne._time.getTime()>=Z.start.getTime()&&Ne._time.getTime()<Z.end.getTime());if(fe>=0&&d.value){const Ne=d.value.querySelectorAll(".log-line");Ne[fe]&&(Ne[fe].scrollIntoView({behavior:"smooth",block:"center"}),n.value=!1)}}const W=J(()=>{let Z=t.value;if(a.value&&(Z=Z.filter(fe=>(fe.level||"INFO")===a.value)),y.value){const fe=fr.find(Ne=>Ne.value===y.value);if(fe&&fe.seconds){const Ne=new Date(Date.now()-fe.seconds*1e3);Z=Z.filter(Ve=>Ve._time&&Ve._time>=Ne)}}if(i.value&&!$.value)if(l.value)try{const fe=new RegExp(i.value,"i");Z=Z.filter(Ne=>{const Ve=Ne.text||Ne.raw||"",st=Ne.tool||"";return fe.test(Ve)||fe.test(st)})}catch{}else{const fe=i.value.toLowerCase();Z=Z.filter(Ne=>{const Ve=(Ne.text||Ne.raw||"").toLowerCase(),st=(Ne.tool||"").toLowerCase();return Ve.includes(fe)||st.includes(fe)})}return Z});function K(Z){if(Z.type==="log"&&Z.line)try{const fe=typeof Z.line=="string"?JSON.parse(Z.line):Z.line,Ne=fe.timestamp?new Date(fe.timestamp):new Date;return{ts:Ne.toLocaleTimeString(),_time:Ne,level:fe.error?"ERROR":"INFO",text:fe.tool_name?`[${fe.tool_name}] ${fe.result_summary||""}`.trim():fe.message||JSON.stringify(fe),tool:fe.tool_name||"",raw:null}}catch{return{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:String(Z.line),tool:"",raw:String(Z.line)}}if(Z.payload){const fe=Z.payload,Ne=fe.timestamp?new Date(fe.timestamp):new Date;return{ts:Ne.toLocaleTimeString(),_time:Ne,level:fe.error?"ERROR":"INFO",text:fe.tool_name?`[${fe.tool_name}] ${fe.result_summary||""}`.trim():fe.message||JSON.stringify(fe),tool:fe.tool_name||"",raw:null}}return typeof Z=="string"?{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:Z,tool:"",raw:Z}:{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:JSON.stringify(Z),tool:"",raw:null}}function X(Z){const fe=K(Z);if(s.value){w.value.push(fe);return}oe(fe)}function oe(Z){t.value.push(Z),t.value.length>h&&(t.value=t.value.slice(-h)),n.value&&It(()=>ee())}function ee(Z=!1){const fe=d.value;fe&&fe.scrollTo({top:fe.scrollHeight,behavior:Z?"smooth":"instant"})}function de(){n.value=!0,u.value=!1,It(()=>ee(!0))}const De=new Set(["PageUp","PageDown","ArrowUp","ArrowDown","Home","End"," "]);function Q(){const Z=d.value;if(!Z)return;const fe=Z.scrollHeight-Z.scrollTop-Z.clientHeight<40;u.value=!n.value&&!fe&&t.value.length>0,ue.value&&be()}function be(){const Z=d.value;!Z||!n.value||Z.scrollHeight-Z.scrollTop-Z.clientHeight>=40&&(n.value=!1,u.value=t.value.length>0)}function z(){n.value&&requestAnimationFrame(be)}function re(Z){De.has(Z.key)&&z()}const ue=f(!1);function Ie(){n.value&&(ue.value=!0,requestAnimationFrame(be))}function _(){ue.value&&(ue.value=!1,be())}function P(){n.value&&(u.value=!1,It(()=>ee()))}function H(){if(s.value=!s.value,!s.value&&w.value.length>0){for(const Z of w.value)oe(Z);w.value=[]}}function ie(){t.value=[],w.value=[],u.value=!1}function se(){let Z;e.value==="search"?Z=ne.value.map(st=>{const Mt=st.error?"ERROR":"INFO",ea=st.tool_name?`[${st.tool_name}] `:"";return`${st.timestamp||""} ${Mt} ${ea}${st.result_summary||st.message||""}`}).join(`
`):Z=W.value.map(st=>`${st.ts} ${st.level} ${st.text}`).join(`
`);const fe=new Blob([Z],{type:"text/plain"}),Ne=URL.createObjectURL(fe),Ve=document.createElement("a");Ve.href=Ne,Ve.download=`odin-logs-${new Date().toISOString().slice(0,19).replace(/:/g,"-")}.txt`,Ve.click(),URL.revokeObjectURL(Ne)}function ae(Z,fe){const Ne=`${Z.ts} ${Z.level} ${Z.text||Z.raw||""}`;navigator.clipboard.writeText(Ne).then(()=>{p.value=fe,setTimeout(()=>{p.value=null},1500)}).catch(()=>{})}function me(Z){a.value=a.value===Z?"":Z,R.value="all"}function pe(Z){return Z.level==="ERROR"?"log-line-error":Z.level==="WARNING"?"log-line-warning":"text-gray-300"}function ce(Z){return Z==="ERROR"?"text-red-500 font-semibold":Z==="WARNING"?"text-yellow-500":"text-blue-500"}function le(Z){return Z==="ERROR"?"log-chip-error":Z==="WARNING"?"log-chip-warning":"log-chip-info"}function _e(Z){R.value=Z.id;const fe=Z.filters;a.value=fe.level||"",y.value=fe.timeRange||"",i.value=fe.text||"",fe.levels&&(a.value=fe.levels[0]||""),fe.hasToolName&&(i.value="")}function ge(Z){R.value=Z.id,a.value=Z.filters.level||"",y.value=Z.filters.timeRange||"",i.value=Z.filters.text||""}function we(){if(!S.value.trim())return;const Z={id:"custom-"+Date.now(),name:S.value.trim(),filters:{level:a.value,timeRange:y.value,text:i.value}};g.value=[...g.value,Z],E(),b.value=!1,S.value=""}function Re(Z){g.value=g.value.filter(fe=>fe.id!==Z),E(),R.value===Z&&(R.value="all")}const F=f("all"),he=f(""),Se=f(""),Oe=f(""),Fe=f(""),ft=f(""),ct=f(100),Et=rk,At=f(!1),tt=f(!1),Ze=f(""),ne=f([]),Te=f(null),Le=f(null);function Je(){e.value="search",Te.value||jt()}async function jt(){try{Te.value=await G.get("/api/logs/stats")}catch{}}function je(){const Z=ft.value;if(!Z){Oe.value="",Fe.value="";return}const Ne={last_5m:300,last_15m:900,last_1h:3600,last_4h:14400,last_24h:86400,last_7d:604800}[Z];if(Ne){const Ve=new Date(Date.now()-Ne*1e3);Oe.value=Dt(Ve),Fe.value=""}}function Dt(Z){const fe=Ne=>String(Ne).padStart(2,"0");return`${Z.getFullYear()}-${fe(Z.getMonth()+1)}-${fe(Z.getDate())}T${fe(Z.getHours())}:${fe(Z.getMinutes())}`}function Pt(Z){if(!Z)return"";const fe=new Date(Z);return isNaN(fe.getTime())?"":fe.toISOString()}async function os(){At.value=!0,Ze.value="",tt.value=!0,Le.value=null;try{const Z=new URLSearchParams;F.value&&F.value!=="all"&&Z.set("level",F.value),he.value&&Z.set("tool",he.value),Se.value&&Z.set("q",Se.value);const fe=Pt(Oe.value),Ne=Pt(Fe.value);fe&&Z.set("start",fe),Ne&&Z.set("end",Ne),Z.set("limit",String(ct.value));const Ve=await G.get(`/api/logs/search?${Z.toString()}`);ne.value=Ve.entries||[]}catch(Z){Ze.value=Z.message||"Search failed",ne.value=[]}finally{At.value=!1}}function rn(){F.value="all",he.value="",Se.value="",Oe.value="",Fe.value="",ft.value="",ct.value=100,ne.value=[],tt.value=!1,Ze.value="",Le.value=null}function Ts(Z){Le.value=Le.value===Z?null:Z}function Xn(Z){if(!Z.timestamp)return"";try{return new Date(Z.timestamp).toLocaleString()}catch{return Z.timestamp}}function Zs(Z){return Z.type==="web_action"?`${Z.status||""} (${Z.execution_time_ms||0}ms)`:(Z.result_summary||"").slice(0,200)}function Ns(Z){return Z.error?"log-line-error":"text-gray-300"}function Pn(Z){try{return JSON.stringify(Z,null,2)}catch{return String(Z)}}let Zt=null,zs=!1;function rs(){zs||(zs=!0,Qe.subscribe("logs",X),o.value=Qe.connected,r.value=Qe.state||"disconnected",Zt=Qe.onState(Z=>{r.value=Z,o.value=Z==="connected"}))}function it(){zs&&(zs=!1,Qe.unsubscribe("logs",X),Zt&&(Zt(),Zt=null))}return Ge(()=>{A(),window.addEventListener("pointerup",_),window.addEventListener("pointercancel",_)}),gs(rs),ls(it),bt(()=>{it(),window.removeEventListener("pointerup",_),window.removeEventListener("pointercancel",_)}),{mode:e,logs:t,paused:s,autoScroll:n,levelFilter:a,textFilter:i,useRegex:l,subscribed:o,wsState:r,wsStateLabel:c,logContainer:d,filteredLogs:W,pauseBuffer:w,showJumpBottom:u,copiedIndex:p,regexError:$,levels:m,logPresets:v,timeRanges:k,timeRange:y,activeLogPreset:R,customLogPresets:g,showSaveLogPreset:b,newLogPresetName:S,hasActiveLogFilters:x,timeRangeLabel:N,timelineBuckets:M,timelineMax:V,timelineSpanLabel:q,timelineLabelSkip:D,togglePause:H,clearLogs:ie,exportLogs:se,logLineClass:pe,levelClass:ce,levelChipClass:le,toggleLevel:me,copyLine:ae,jumpToBottom:de,onScroll:Q,onUserScrollIntent:z,onUserScrollKey:re,onAutoScrollToggle:P,onPointerDown:Ie,applyLogPreset:_e,applyCustomLogPreset:ge,saveLogCustomPreset:we,removeLogCustomPreset:Re,segmentHeight:O,jumpToTimelineBucket:U,searchLevel:F,searchTool:he,searchKeyword:Se,searchStart:Oe,searchEnd:Fe,searchTimePreset:ft,searchLimit:ct,searchLimits:Et,searching:At,searchRan:tt,searchError:Ze,searchResults:ne,searchStats:Te,expandedSearch:Le,switchToSearch:Je,runSearch:os,clearSearchFilters:rn,toggleSearchExpand:Ts,formatSearchTs:Xn,searchEntryText:Zs,searchLogLineClass:Ns,formatJson:Pn,applySearchTimePreset:je}}};function Rl(e=[]){const t=[],s=new Set;function n(a){const i=[a.kind,a.label,a.apply_mode||"",a.code||"",a.text||""].join("\0");s.has(i)||(s.add(i),t.push({...a,key:i}))}for(const a of e)for(const i of(a==null?void 0:a.consumers)||[])n({kind:"consumer",label:i.name,apply_mode:i.apply_mode,text:i.detail});for(const a of e)a!=null&&a.apply_handler&&n({kind:"handler",label:"Apply handler",code:a.apply_handler});for(const a of e)a!=null&&a.restart_reason&&n({kind:"restart",label:"Why a restart is required",text:a.restart_reason});for(const a of e)a!=null&&a.activation_policy&&n({kind:"activation",label:"Activation policy",text:a.activation_policy});return t}const dk=Object.freeze([{key:"all",label:"All fields",short:"All",icon:"grid"},{key:"applied",label:"Applied",short:"Applied",icon:"success"},{key:"pending_restart",label:"Pending restart",short:"Restart",icon:"refresh"},{key:"dormant",label:"Saved, not active",short:"Saved only",icon:"pause"},{key:"invalid",label:"Invalid",short:"Invalid",icon:"error"},{key:"drift",label:"Drift",short:"Drift",icon:"warning"},{key:"unknown",label:"Effective state unknown",short:"Unknown",icon:"info"}]);function uk(e,t={}){var a,i;const s=t.getStyle||(l=>globalThis.getComputedStyle(l)),n=Object.hasOwn(t,"fallback")?t.fallback:(a=globalThis.document)==null?void 0:a.scrollingElement;for(let l=e;l;l=l.parentElement){const o=((i=s(l))==null?void 0:i.overflowY)||"";if(/^(auto|scroll|overlay)$/.test(o)&&l.scrollHeight>l.clientHeight)return l}return n&&n.scrollHeight>n.clientHeight?n:e||n||null}const Ya=[{key:"core",label:"Core",icon:"sliders",sections:["timezone","logging","permissions","graceful_degradation"]},{key:"models",label:"Models & AI",icon:"brain",sections:["image","llm_recovery"]},{key:"runtime",label:"Runtime",icon:"activity",sections:["context","sessions","agents","turn_state"]},{key:"data",label:"Data & Storage",icon:"database",sections:["learning","search","usage","audit","attachments"]},{key:"services",label:"Services",icon:"link",sections:["webhook","observability","email","browser","comfyui","slack","mcp"]},{key:"automation",label:"Automation",icon:"workflow",sections:["message_triggers","reaction_triggers","grafana_alerts","outbound_webhooks","issue_tracker"]},{key:"infrastructure",label:"Infrastructure",icon:"server",sections:["tools","web"]}],pk={live_read:"Applies immediately",live_apply:"Dedicated live apply",live_for_new_work:"Applies to new work",restart:"Restart required",activation_required:"Saved only — see activation note",legacy_control:"Controlled elsewhere",dormant:"Saved for future support"},hr=new Set(["llm_provider","openai_codex","ollama","kimi","personality","discord"]),fk=Object.freeze(["web.api_tokens","outbound_webhooks.targets"]);function Wu(e){return fk.some(t=>e===t||e.startsWith(`${t}.`))}const Mm="odin_config_center_expanded_v1",Fm="odin_config_center_category_v1",hk=50,mk=650,mr=()=>G.get("/api/config/meta");function oa(e){return e===void 0?void 0:JSON.parse(JSON.stringify(e))}function $i(e,t){return JSON.stringify(e)===JSON.stringify(t)}function Na(e){return String(e).replace(/[_-]+/g," ").replace(/\b\w/g,t=>t.toUpperCase())}function vk(e){return e===void 0?"unset":e===null?"null":typeof e=="boolean"?e?"Enabled":"Disabled":Array.isArray(e)?e.length?`${e.length} item${e.length===1?"":"s"}`:"Empty list":typeof e=="object"?Object.keys(e).length?`${Object.keys(e).length} field${Object.keys(e).length===1?"":"s"}`:"Empty object":e===""?"Empty":String(e)}function gk(e){if(e===void 0)return"unset";if(e===null)return"null";if(typeof e=="object")try{return JSON.stringify(e,null,2)}catch{return String(e)}return String(e)}function $m(e,t){if($i(e,t))return;if(!(e&&t&&typeof e=="object"&&typeof t=="object"&&!Array.isArray(e)&&!Array.isArray(t)))return oa(t);const n={};for(const[a,i]of Object.entries(t)){const l=$m(e[a],i);l!==void 0&&(n[a]=l)}return Object.keys(n).length?n:void 0}function bk(e,t){const s={};for(const[n,a]of Object.entries(t||{})){const i=$m(e==null?void 0:e[n],a);i!==void 0&&(s[n]=i)}return s}function Bm(e,t,s,n){if($i(e,t))return;if(e&&t&&typeof e=="object"&&typeof t=="object"&&!Array.isArray(e)&&!Array.isArray(t)){const i=new Set([...Object.keys(e),...Object.keys(t)]);for(const l of i)Bm(e[l],t[l],s?`${s}.${l}`:l,n);return}n.push({path:s,oldVal:e,newVal:t})}function yk(){try{const e=JSON.parse(localStorage.getItem(Mm)||"{}");return e&&typeof e=="object"&&!Array.isArray(e)?e:{}}catch{return{}}}function xk(){try{const e=localStorage.getItem(Fm);return Ya.some(t=>t.key===e)?e:Ya[0].key}catch{return Ya[0].key}}const _k={template:`
  ...
  `,setup(){const e=f(null),t=f(null),s=f(!0),n=f(null),a=f(!1),i=f(null),l=f(null),o=f(null),r=f(!1),c=f(!1),d=f(null),u=f(""),p=f("all"),h=f(xk()),m=f(yk()),v=f({}),k=f({}),R=f(""),y=f({}),g=f({}),b=f([]),S=f([]),w=f(!1),A=f(!1),E=f(!1);let x=null,N=null,$={path:null,at:0},C=0;const M=J(()=>{var T;return(((T=t.value)==null?void 0:T.fields)||[]).filter(B=>!hr.has(B.path.split(".")[0])&&!Wu(B.path))}),V=J(()=>new Map(M.value.map(T=>[T.path,T]))),q=J(()=>W.value.reduce((T,B)=>T+B.sections.length,0)),D=J(()=>M.value.length),I=J(()=>dk),O=J(()=>b.value.length>0),U=J(()=>S.value.length>0),W=J(()=>{if(!e.value)return[];const T=new Set(Ya.flatMap(Ee=>Ee.sections)),B=Ya.map(Ee=>({...Ee,sections:Ee.sections.filter(dt=>Object.hasOwn(e.value,dt)&&!hr.has(dt))})).filter(Ee=>Ee.sections.length),Y=Object.keys(e.value).filter(Ee=>!T.has(Ee)&&!hr.has(Ee));return Y.length&&B.push({key:"other",label:"Other",icon:"folder",sections:Y}),B}),K=J(()=>e.value?{...e.value,...v.value}:null),X=J(()=>{if(!e.value)return[];const T=[];for(const[B,Y]of Object.entries(v.value))Bm(e.value[B],Y,B,T);return T.filter(B=>!$i(B.oldVal,B.newVal)).map(B=>{const Y=P(B.path);return{...B,label:(Y==null?void 0:Y.label)||Na(B.path.split(".").at(-1)),apply_mode:(Y==null?void 0:Y.apply_mode)||ce(B.path.split(".")[0])}})}),oe=J(()=>X.value.length>0),ee=J(()=>X.value.length),de=J(()=>new Set(X.value.map(T=>T.path.split(".")[0])).size),De=J(()=>!!u.value||p.value!=="all"),Q=J(()=>{const T={...g.value};for(const B of X.value){const Y=P(B.path),Ee=fi(Y,B.newVal);Ee&&(T[B.path]=Ee)}return T}),be=J(()=>Object.keys(Q.value).length>0),z=J(()=>e.value?(De.value?W.value:W.value.filter(B=>B.key===h.value)).map(B=>({...B,sections:B.sections.filter(Y=>Ze(Y))})).filter(B=>B.sections.length):[]),re=J(()=>{const T=["live_read","live_apply","live_for_new_work","restart","activation_required","legacy_control","dormant"],B=new Map(T.map(Y=>[Y,[]]));for(const Y of X.value){const Ee=B.has(Y.apply_mode)?Y.apply_mode:"restart";B.get(Ee).push(Y)}return T.filter(Y=>B.get(Y).length).map(Y=>({key:Y,label:Jt(Y),entries:B.get(Y)}))}),ue=J(()=>X.value.filter(T=>T.apply_mode==="restart").length),Ie=J(()=>M.value.filter(T=>T.pending_restart)),_=J(()=>Ie.value.length);function P(T){const B=V.value.get(T);return B?{...B,apply_details:Rl([B])}:null}function H(T){const B=`${T}.`;return M.value.filter(Y=>Y.path===T||Y.path.startsWith(B))}function ie(){return M.value.some(T=>T.path==="tools.hosts"||T.path.startsWith("tools.hosts."))}function se(){var Y,Ee;const T=((Ee=(Y=e.value)==null?void 0:Y.tools)==null?void 0:Ee.hosts)||{},B=Object.keys(T).length;return`${B} host${B===1?"":"s"} configured.`}function ae(T){return H(T).length}function me(T){return Na(T)}function pe(T){const B=H(T);if(!B.length)return`${Na(T)} configuration.`;const Y=B.find(Cs=>Cs.sensitivity==="public"&&Cs.description)||B.find(Cs=>Cs.description),Ee=(Y==null?void 0:Y.description)||"";return Ee.match(/setting for (.+)\.$/i)?`${Na(T)} settings and runtime behaviour.`:Ee}function ce(T){const B=[...new Set(H(T).map(Y=>Y.apply_mode))];return B.length===1?B[0]:B.includes("restart")?"restart":B.includes("activation_required")?"activation_required":B[0]||"restart"}function le(T){const B=[...new Set(H(T).map(Y=>Jt(Y.apply_mode)))];return B.length?B.length===1?B[0]:`Mixed apply behaviour: ${B.join(" · ")}`:""}function _e(T){return Rl(H(T))}function ge(T){var B;return Object.hasOwn(v.value,T)?v.value[T]:(B=e.value)==null?void 0:B[T]}function we(){const T=ge("mcp")||{},B=Object.keys(T.servers||{}).length;return`${T.enabled?"Globally enabled":"Globally disabled"} · ${B} configured server${B===1?"":"s"}.`}function Re(T,B){return B.split(".").reduce((Y,Ee)=>Y==null?void 0:Y[Ee],T)}function F(T){const B=K.value;return H(T).filter(Y=>Wu(Y.path)?!1:Y.path.split(".").length<=2?!0:!Y.path.includes(".*")).map(Y=>({...Y,key:Y.path.split(".").at(-1),value:Re(B,Y.path),apply_details:Rl([Y]),editor:Y.path==="agents.final_warning_iterations"?"warning-chips":null}))}function he(T){const B=T.path.split(".");return B.length>2?B.slice(0,2).join("."):null}function Se(T){const B=new Map;for(const Y of F(T)){const Ee=he(Y),dt=Ee||`${T}.__root`;B.has(dt)||B.set(dt,{key:dt,path:Ee,entries:[]}),B.get(dt).entries.push(Y)}return[...B.values()].map(Y=>{const Ee=Y.entries.find(dt=>dt.group_description);return{...Y,label:Y.path?Na(Y.path.split(".").at(-1)):null,description:(Ee==null?void 0:Ee.group_description)||null,apply_details:Rl(Y.entries),runtime_summaries:Fe(Y.entries)}})}function Oe(T){return{save:T.save_effect||(T.apply_mode==="dormant"?"Saving records this value in config.yml.":"Saving records this value and validates the section."),runtime:T.runtime_effect||{live_read:"Odin reads the saved value during current work.",live_apply:"Odin reloads this setting without a restart.",live_for_new_work:"New work uses the saved value; existing work keeps its snapshot.",restart:"Odin keeps using its startup value until a clean restart.",activation_required:"Odin keeps the current behavior until you enable this feature separately.",legacy_control:"Odin keeps the existing compatibility behavior until you apply this choice.",dormant:"This version of Odin does not use the saved value. Restarting will not activate it."}[T.apply_mode]||"Effective runtime state is not currently observable."}}function Fe(T){const B=new Map;for(const Y of T){const Ee=Oe(Y),dt=`${Y.apply_mode}|${Ee.save}|${Ee.runtime}`;B.has(dt)||B.set(dt,{key:dt,label:Jt(Y.apply_mode),save:Ee.save,runtime:Ee.runtime})}return[...B.values()]}function ft(T){if(ct(T))return T.runtime_effect||T.activation_policy||"";if(T.apply_mode==="activation_required"){const B=T.activation_policy||T.runtime_effect;return B?`Not active after saving. No activation control exists in this release. ${B}`:"Not active after saving; no activation control exists in this release."}return""}function ct(T){return T.action_available===!0&&!!(T.action_label&&T.action_endpoint)}async function Et(T){if(ct(T))try{if(jt(T.path))throw new Error("Save this setting before applying its action.");const B=String(T.action_method||"POST").toLowerCase(),Y={post:G.post.bind(G),put:G.put.bind(G),delete:G.del.bind(G)}[B];if(!Y)throw new Error("Unsupported configuration action");await Y(T.action_endpoint,T.action_body||void 0),await Pe(),Yt("success",`${T.action_label} completed.`)}catch(B){Yt("error",B.message||`${T.action_label} failed`)}}function At(T,B){return[T.label,T.path,T.description,...T.aliases||[]].filter(Boolean).join(" ").toLowerCase().includes(B)}function tt(T){const B=u.value.trim().toLowerCase();return B?H(T).filter(Y=>At(Y,B)):[]}function Ze(T){const B=H(T);if(p.value!=="all"&&!B.some(Ee=>Ee.apply_state===p.value))return!1;const Y=u.value.trim().toLowerCase();return!Y||`${me(T)} ${T}`.toLowerCase().includes(Y)?!0:B.some(Ee=>At(Ee,Y))}function ne(T,B){return H(T).filter(Y=>Y.apply_state===B).length}function Te(T){return T==="all"?D.value:M.value.filter(B=>B.apply_state===T).length}function Le(T){const B=T.sections.flatMap(Y=>H(Y));return{fields:B.length,modified:X.value.filter(Y=>T.sections.includes(Y.path.split(".")[0])).length,pending_restart:B.filter(Y=>Y.apply_state==="pending_restart").length,invalid:B.filter(Y=>Y.apply_state==="invalid").length,dormant:B.filter(Y=>Y.apply_state==="dormant").length}}function Je(T){var B;return Object.hasOwn(v.value,T)&&!$i((B=e.value)==null?void 0:B[T],v.value[T])}function jt(T){return X.value.some(B=>B.path===T||B.path.startsWith(`${T}.`))}function je(T){h.value=T,u.value="",p.value="all";try{localStorage.setItem(Fm,T)}catch{}}function Dt(T){p.value=T}function Pt(){u.value="",p.value="all"}function os(T){var B;return((B=W.value.find(Y=>Y.sections.includes(T)))==null?void 0:B.sections)||[]}function rn(T){const B=os(T),Y=B.find(Ee=>m.value[Ee]===!0);return Y||B.find(Ee=>m.value[Ee]!==!1)||null}function Ts(T){return u.value&&!E.value&&Ze(T)?!0:E.value?rn(T)===T:Object.hasOwn(m.value,T)?m.value[T]===!0:!0}function Xn(T){const B=!Ts(T);if(E.value){const Y={...m.value};for(const Ee of os(T))Y[Ee]===!0&&(Y[Ee]=!1);Y[T]=B,m.value=Y;return}m.value={...m.value,[T]:B}}function Zs(){b.value.push(oa(v.value)),b.value.length>hk&&b.value.shift(),S.value=[]}function Ns(){oe.value&&(Zs(),v.value={},g.value={},w.value=!1)}function Pn(T,B=!1){const Y=Date.now();if(B&&$.path===T&&Y-$.at<mk){$.at=Y;return}Zs(),$={path:T,at:Y}}function Zt(T,B,Y){if(!B.length)return Y;const Ee=oa(T??{});let dt=Ee;for(let Cs=0;Cs<B.length-1;Cs+=1){const un=B[Cs];dt[un]=oa(dt[un]??{}),dt=dt[un]}return dt[B.at(-1)]=Y,Ee}function zs(T){var B;return Object.hasOwn(v.value,T)?v.value[T]:oa((B=e.value)==null?void 0:B[T])}function rs(T,B,Y={}){var md;const[Ee,...dt]=T.path.split(".");Pn(T.path,!!Y.coalesce);const Cs=zs(Ee),un=dt.length?Zt(Cs,dt,B):B,sa={...v.value};if($i(un,(md=e.value)==null?void 0:md[Ee])?delete sa[Ee]:sa[Ee]=un,v.value=sa,g.value[T.path]){const vd={...g.value};delete vd[T.path],g.value=vd}}function it(T){$={path:null,at:0},k.value={...k.value,[T]:String(Re(K.value,T)??"")}}function Z(T){if($={path:null,at:0},!Object.hasOwn(k.value,T))return;const B={...k.value};delete B[T],k.value=B}function fe(T){const B=k.value[T.path];if($={path:null,at:0},B===""){g.value={...g.value,[T.path]:"Enter a number."};return}const Y=Number(B);if(Number.isNaN(Y)||T.type==="integer"&&!Number.isInteger(Y)){g.value={...g.value,[T.path]:T.type==="integer"?"Enter a whole number.":"Enter a number."};return}const Ee={...k.value};delete Ee[T.path],k.value=Ee,rs(T,Y,{coalesce:!0})}function Ne(T){return Object.hasOwn(k.value,T.path)?k.value[T.path]:T.value??""}function Ve(T,B){if(k.value={...k.value,[T.path]:B},B===""){g.value={...g.value,[T.path]:"Enter a number."};return}const Y=Number(B);if(!Number.isFinite(Y)||T.type==="integer"&&!Number.isInteger(Y)){g.value={...g.value,[T.path]:T.type==="integer"?"Enter a whole number.":"Enter a valid number."};return}if(g.value[T.path]){const Ee={...g.value};delete Ee[T.path],g.value=Ee}rs(T,Y,{coalesce:!0})}function st(T){const B=Number.parseInt(R.value,10);if(!Number.isInteger(B)||B<1){g.value={...g.value,[T.path]:"Warning thresholds must be positive whole numbers."};return}const Y=[...new Set([...T.value||[],B])].sort((Ee,dt)=>dt-Ee);R.value="",rs(T,Y)}function Mt(T,B){rs(T,(T.value||[]).filter(Y=>Y!==B))}function ea(T){return T.apply_mode==="live_read"?"Odin reads the saved file value on next use.":T.apply_mode==="live_for_new_work"?"New work uses the saved file value.":T.apply_mode==="live_apply"?T.apply_handler?`Apply the saved value through ${T.apply_handler}.`:"Apply it through its dedicated owner page or endpoint.":T.apply_mode==="restart"?"Restart Odin for the saved collection to take effect.":T.apply_mode==="activation_required"?"Saving does not enable it. No activation control exists in this release.":T.apply_mode==="dormant"?"This release does not use the saved collection.":"Follow the runtime details shown for this setting."}function cs(T){return T.type==="array"&&Array.isArray(T.value)&&!T.structured_container&&!T.structured_container_child&&T.sensitivity==="public"&&T.value.every(B=>["string","number","boolean"].includes(typeof B))}function Ea(T){const B=String(y.value[T.path]??"").trim();if(!B)return;const Y=[...new Set([...T.value||[],B])];y.value={...y.value,[T.path]:""},rs(T,Y)}function js(T,B){rs(T,(T.value||[]).filter(Y=>Y!==B))}function fi(T,B){var Ee;if(!T)return null;if((Ee=T.enum)!=null&&Ee.length&&!T.enum.includes(B))return`Choose one of: ${T.enum.join(", ")}`;if(T.path==="agents.final_warning_iterations"&&(!Array.isArray(B)||!B.length))return"Add at least one warning threshold.";const Y=T.constraints||{};if((T.type==="integer"||T.type==="number")&&typeof B=="number"){if(Y.minimum!==void 0&&B<Y.minimum)return`Must be at least ${Y.minimum}${T.unit?` ${T.unit}`:""}`;if(Y.maximum!==void 0&&B>Y.maximum)return`Must be at most ${Y.maximum}${T.unit?` ${T.unit}`:""}`}return null}function hi(T){return Q.value[T.path]||null}function Aa(T){const B=`${T}.`;return Object.keys(Q.value).some(Y=>Y===T||Y.startsWith(B))}function ta(){b.value.length&&(S.value.push(oa(v.value)),v.value=b.value.pop(),g.value={},k.value={},$={path:null,at:0})}function Mn(){S.value.length&&(b.value.push(oa(v.value)),v.value=S.value.pop(),g.value={},k.value={},$={path:null,at:0})}function Fn(){!oe.value||be.value||(w.value=!0,A.value=!1)}function Js(){w.value=!1}function cn(){Ns()}function Jt(T){return pk[T]||Na(T||"unknown")}function Ra(T){return`apply-${String(T||"unknown").replaceAll("_","-")}`}function j(T){return`cfgc-field-${T.replace(/[^a-zA-Z0-9_-]/g,"-")}`}function ye(T){return`${j(T)}-input`}function Ae(T){const B=document.getElementById(j(T))||document.getElementById(j(T.split(".").slice(0,2).join(".")));B==null||B.scrollIntoView({behavior:"smooth",block:"center"})}function Yt(T,B){l.value={type:T,message:B},window.setTimeout(()=>{var Y;((Y=l.value)==null?void 0:Y.message)===B&&(l.value=null)},3500)}function dn(){r.value=!1,p.value="pending_restart",u.value="";const T=uk(n.value);T&&(T.scrollTop=0)}function $n(){r.value=!1}function ke(T=1800){N&&window.clearTimeout(N),N=window.setTimeout(L,T)}async function L(){if(c.value){if(C+=1,C>45){c.value=!1,d.value="Odin did not return with the new startup settings within 90 seconds.";return}try{if(t.value=await mr(),_.value===0){c.value=!1,d.value=null,Yt("success","Odin restarted and the saved startup settings are active.");return}}catch{}ke(2e3)}}async function te(){if(!c.value){d.value=null;try{await G.post("/api/restart",{}),c.value=!0,C=0,r.value=!1,ke()}catch(T){d.value=T.message||"Odin could not schedule a restart."}}}async function ve(){if(!(!oe.value||be.value||a.value)){a.value=!0;try{const T=bk(e.value,v.value),B=await G.put("/api/config",T);e.value=B,v.value={},b.value=[],S.value=[],g.value={},w.value=!1;try{t.value=await mr(),o.value=null,r.value=_.value>0,Yt("success",_.value?`Configuration saved. ${_.value} setting${_.value===1?"":"s"} still use startup values.`:"Configuration saved. Apply status has been refreshed.")}catch(Y){o.value=Y.message||"Unknown metadata error.",Yt("error",`Configuration saved, but apply status could not be refreshed: ${o.value}`)}}catch(T){Yt("error",T.message||"Configuration could not be saved")}finally{a.value=!1}}}async function Pe(){var T,B;if(!oe.value){s.value=!0,i.value=null;try{const Y=await G.get("/api/config"),Ee=await mr();e.value=Y,t.value=Ee,o.value=null;const dt=W.value;if(dt.some(Cs=>Cs.key===h.value)||(h.value=((T=dt[0])==null?void 0:T.key)||Ya[0].key),E.value){const un=(((B=dt.find(sa=>sa.key===h.value))==null?void 0:B.sections)||[]).find(sa=>m.value[sa]===!0);m.value=un?{...m.value,[un]:!0}:{}}}catch(Y){i.value=Y.message||"Unknown configuration error"}finally{s.value=!1}}}function $e(T){if(w.value||!(T.ctrlKey||T.metaKey))return;const B=T.target;B instanceof HTMLElement&&(B.matches("input, textarea, select")||B.isContentEditable)||(!T.shiftKey&&T.key.toLowerCase()==="z"?(T.preventDefault(),ta()):(T.key.toLowerCase()==="y"||T.shiftKey&&T.key.toLowerCase()==="z")&&(T.preventDefault(),Mn()))}function Ue(T){E.value=T.matches}ns(m,T=>{try{localStorage.setItem(Mm,JSON.stringify(T))}catch{}},{deep:!0});let yt=!1;function lt(){yt||(yt=!0,document.addEventListener("keydown",$e))}function mt(){yt&&(yt=!1,document.removeEventListener("keydown",$e))}return Ge(()=>{var T;Pe(),lt(),x=window.matchMedia("(max-width: 760px)"),Ue(x),(T=x.addEventListener)==null||T.call(x,"change",Ue)}),gs(lt),ls(mt),bt(()=>{var T;mt(),(T=x==null?void 0:x.removeEventListener)==null||T.call(x,"change",Ue),N&&window.clearTimeout(N)}),{armKeydown:lt,disarmKeydown:mt,handleKeydown:$e,config:e,meta:t,loading:s,saving:a,error:i,toast:l,metaRefreshError:o,restartPromptOpen:r,restartScheduled:c,restartError:d,configMain:n,searchQuery:u,healthFilter:p,activeCategory:h,reviewOpen:w,mobileOverflowOpen:A,warningThresholdInput:R,arrayInputs:y,healthFilters:I,visibleCategories:W,displayGroups:z,reviewGroups:re,sectionCount:q,fieldCount:D,hasChanges:oe,changeCount:ee,changedSectionCount:de,hasDraftErrors:be,canUndo:O,canRedo:U,globalFilterActive:De,reviewRestartCount:ue,pendingRestartCount:_,pendingRestartFields:Ie,healthCount:Te,categoryStats:Le,selectCategory:je,selectHealthFilter:Dt,clearFilters:Pt,sectionLabel:me,sectionDescription:pe,sectionFieldCount:ae,sectionHealthCount:ne,sectionApplySummary:le,sectionApplyDetails:_e,sectionEntries:F,fieldGroups:Se,sectionSearchHits:tt,mcpConfigSummary:we,fieldRuntimeCopy:Oe,fieldSpecificRuntimeNote:ft,hasHonestAction:ct,runFieldAction:Et,hasHostsCollection:ie,hostsConfigSummary:se,sectionChanged:Je,fieldChanged:jt,isSectionExpanded:Ts,toggleSection:Xn,discardAllDrafts:Ns,setFieldValue:rs,setNumberFieldValue:Ve,numberInputValue:Ne,beginInputEdit:it,endTextInputEdit:Z,endInputEdit:fe,addWarningThreshold:st,removeWarningThreshold:Mt,isScalarArray:cs,addScalarArrayItem:Ea,removeScalarArrayItem:js,fieldError:hi,sectionHasErrors:Aa,undo:ta,redo:Mn,openReview:Fn,closeReview:Js,mobileCancel:cn,applyModeLabel:Jt,applyClass:Ra,compactValue:vk,formatValue:gk,structuredApplyCopy:ea,fieldId:j,fieldInputId:ye,focusField:Ae,fetchConfig:Pe,saveConfig:ve,restartOdin:te,restartLater:$n,reviewPendingRestart:dn}}},wk=/^\d{15,25}$/;function Um(e){return String((e==null?void 0:e.display_name)||(e==null?void 0:e.username)||(e==null?void 0:e.id)||"Unknown user")}const Hm={props:{members:{type:Array,default:()=>[]},excludedIds:{type:Array,default:()=>[]},placeholder:{type:String,default:"Search Discord users…"},ariaLabel:{type:String,default:"Search Discord users"},optionsId:{type:String,required:!0},autofocus:{type:Boolean,default:!1}},emits:["select"],template:`
  ...
  `,setup(e,{emit:t}){const s=f(""),n=f(!1),a=f(0),i=f(null),l=J(()=>new Set((e.excludedIds||[]).map(String))),o=J(()=>{const S=s.value.toLowerCase().trim();return(e.members||[]).filter(w=>l.value.has(String(w.id))?!1:S?u(w).toLowerCase().includes(S)||String(w.username||"").toLowerCase().includes(S)||String(w.id).includes(S):!0)}),r=J(()=>{const S=s.value.trim();return o.value.length===0&&wk.test(S)&&!l.value.has(S)?S:""}),c=J(()=>o.value.length+(r.value?1:0)),d=J(()=>{if(n.value){if(o.value[a.value])return`${e.optionsId}-${a.value}`;if(r.value&&a.value===o.value.length)return`${e.optionsId}-raw`}});function u(S){return Um(S)}function p(){n.value=!0,a.value=0}function h(){p()}function m(){const S=Math.max(c.value-1,0);a.value=Math.min(a.value+1,S)}function v(){a.value=Math.max(a.value-1,0)}function k(){const S=o.value[a.value];S?R(S):r.value&&a.value===o.value.length&&y(r.value)}function R(S){y(String(S.id))}function y(S){t("select",S),s.value="",n.value=!1,a.value=0}function g(){n.value=!1}function b(){setTimeout(g,150)}return Ge(()=>{e.autofocus&&It(()=>{var S;return(S=i.value)==null?void 0:S.focus()})}),{query:s,open:n,highlightedIndex:a,input:i,filteredMembers:o,rawId:r,activeOptionId:d,memberName:u,openOptions:p,onInput:h,highlightNext:m,highlightPrevious:v,selectHighlighted:k,selectMember:R,selectId:y,closeOptions:g,onBlur:b}}};function Zu(e,t,s){var n;return((n=e==null?void 0:e.config)==null?void 0:n[t])!=null?e.config[t]:s==null?void 0:s[t]}const kk={components:{DiscordUserCombobox:Hm},template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=f([]),t=f(!0),s=f(null),n=f({}),a=f(null),i=f(null),l=f(!1),o=f(null),r=f({}),c=f([]);let d=0;const u=Object.freeze([{key:"allowed_users",label:"Allowed users",description:"Absolute gate for ordinary conversational intake. Guild/channel settings cannot readmit blocked users; prefix commands use separate authorization and allowed test webhooks bypass this gate.",placeholder:"Search Discord users…",userAutocomplete:!0,fullWidth:!0},{key:"channels",label:"Allowed channels",description:"Absolute gate for ordinary conversational intake. Guild/channel settings cannot readmit blocked channels; prefix commands use separate authorization.",placeholder:"Discord channel ID",fullWidth:!0},{key:"ignore_bot_ids",label:"Ignored bot IDs",description:"Ignored unless the bot explicitly mentions Odin; the effective respond-to-bots policy still applies.",placeholder:"Search Discord users or bots…",userAutocomplete:!0,fullWidth:!0}]),p=J(()=>JSON.stringify(a.value)!==JSON.stringify(i.value)),h=J(()=>new Map(c.value.map(D=>[String(D.id),D])));function m(D){return D.config&&D.config.enabled!==void 0?D.config.enabled:!0}function v(D){return Zu(D,"require_mention",a.value)}function k(D){return Zu(D,"respond_to_bots",a.value)}function R(D){return D.config&&Object.keys(D.config).length>0}function y(D){n.value[D]=!n.value[D]}function g(D){const I=D.discord||{};return{allowed_users:[...I.allowed_users||[]],channels:[...I.channels||[]],respond_to_bots:!!I.respond_to_bots,require_mention:!!I.require_mention,ignore_bot_ids:[...I.ignore_bot_ids||[]]}}async function b({showLoading:D=!0}={}){const I=++d;D&&(t.value=!0),s.value=null;try{const O=await G.get("/api/discord/guilds");I===d&&(e.value=O)}catch(O){I===d&&(s.value=O.message)}finally{D&&I===d&&(t.value=!1)}}async function S(){t.value=!0,s.value=null;try{const[D,I,O]=await Promise.all([G.get("/api/discord/guilds"),G.get("/api/discord/members").catch(()=>[]),G.get("/api/config")]),U=g(O),W=p.value;a.value=U,W||(i.value=JSON.parse(JSON.stringify(U))),c.value=I,e.value=D,o.value=null}catch(D){s.value=D.message}finally{t.value=!1}}let w=Promise.resolve();const A=f(new Set);function E(D,I){const O=new Set(A.value);O.add(D),A.value=O;const U=w.then(I);return w=U.catch(()=>{}),U.finally(()=>{const W=new Set(A.value);W.delete(D),A.value=W})}function x(D,I,O,U){const W=(U==null?void 0:U.target)??null;return E(`guild:${D}:${I}`,async()=>{try{await G.put("/api/discord/guild/"+D+"/config",{[I]:O}),await b({showLoading:!1})}catch(K){s.value=K.message,W&&typeof O=="boolean"&&(W.checked=!O)}})}function N(D,I,O,U,W){const K=(W==null?void 0:W.target)??null;return E(`channel:${D}:${O}`,async()=>{try{await G.put("/api/discord/channel/"+D+"/config",{[O]:U}),await b({showLoading:!1})}catch(X){s.value=X.message,K&&typeof U=="boolean"&&(K.checked=!U)}})}function $(D,I){return E(`channel:${D}:clear`,async()=>{try{await G.put("/api/discord/channel/"+D+"/config",{clear:!0}),await b({showLoading:!1})}catch(O){s.value=O.message}})}function C(D,I){const O=String(I);if(!D.userAutocomplete)return O;const U=h.value.get(O);return U?Um(U):O}function M(D,I=null){const O=String(I??r.value[D]??"").trim();!O||i.value[D].includes(O)||(i.value[D]=[...i.value[D],O],r.value={...r.value,[D]:""})}function V(D,I){i.value[D]=i.value[D].filter(O=>O!==I)}async function q(){if(!(!p.value||l.value)){l.value=!0,o.value=null;try{const I=(await G.put("/api/config",{discord:i.value})).discord||i.value;a.value={allowed_users:[...I.allowed_users||[]],channels:[...I.channels||[]],respond_to_bots:!!I.respond_to_bots,require_mention:!!I.require_mention,ignore_bot_ids:[...I.ignore_bot_ids||[]]},i.value=JSON.parse(JSON.stringify(a.value))}catch(D){o.value=D.message||"Global defaults could not be saved."}finally{l.value=!1}}}return Ge(S),{guilds:e,loading:t,error:s,expanded:n,globalDraft:i,globalSaving:l,globalError:o,globalArrayInputs:r,globalMembers:c,globalListEditors:u,globalChanged:p,guildEnabled:m,guildMention:v,guildBots:k,hasOverride:R,toggleGuild:y,fetchAll:S,fetchGuilds:b,setGuildConfig:x,setChannelConfig:N,clearOverride:$,mutationPending:A,globalItemLabel:C,addGlobalItem:M,removeGlobalItem:V,saveGlobalDefaults:q}}},Es=e=>e==null?e:JSON.parse(JSON.stringify(e));function Sk({applyDefault:e,applyUser:t,applyDelete:s,onDefaultConfirmed:n=()=>{},onDefaultRollback:a=()=>{},onUserConfirmed:i=()=>{},onUserRollback:l=()=>{},onUserDeleted:o=()=>{},onError:r=()=>{}}){let c=Promise.resolve(),d=0,u=0;const p=new Map;let h=null;const m=new Map;function v(w){d+=1;const A=c.then(w,w);return c=A.catch(()=>{}),A}function k(w,A){h=Es(w),m.clear();for(const[E,x]of Object.entries(A||{}))m.set(E,Es(x))}function R(w){const A=Es(w),E=++u;return v(async()=>{try{await e(Es(A)),h=Es(A),E===u&&n(Es(A))}catch(x){E===u&&(a(Es(h)),r(x,{kind:"default"}))}})}function y(w,A){const E=Es(A),x=(p.get(w)||0)+1;return p.set(w,x),v(async()=>{try{await t(w,Es(E)),m.set(w,Es(E)),x===p.get(w)&&i(w,Es(E))}catch(N){x===p.get(w)&&(l(w,Es(m.get(w)??null)),r(N,{kind:"user",uid:w}))}})}function g(w){const A=(p.get(w)||0)+1;return p.set(w,A),v(async()=>{try{await s(w),m.delete(w),A===p.get(w)&&o(w)}catch(E){A===p.get(w)&&(l(w,Es(m.get(w)??null)),r(E,{kind:"delete",uid:w}))}})}async function b(){for(;;){const w=c;if(await w,w===c)return d}}async function S(w){for(;;){const A=await b(),E=await w();if(A===d)return E}}return{seed:k,saveDefault:R,saveUser:y,deleteUser:g,whenIdle:b,readSnapshot:S,get revision(){return d}}}const Tk={components:{DiscordUserCombobox:Hm},template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=f(!0),t=f(""),s=f(null),n=f([]),a=f({}),i=f({allowed_hosts:[],default_host:""}),l=f({}),o=f(!1),r=f([]),c=J(()=>{const C={};for(const M of r.value)C[M.id]=M;return C});function d(C){return c.value[C]||null}function u(C,M){return C?C.allowed_hosts===null||C.allowed_hosts===void 0?{allowed_hosts:[...M],default_host:C.default_host||"",allow_all:!0}:{allowed_hosts:C.allowed_hosts,default_host:C.default_host||"",allow_all:!1}:{allowed_hosts:[...M],default_host:M[0]||"",allow_all:!0}}const p=Sk({applyDefault:async C=>{const M=C.allow_all?null:C.allowed_hosts;await G.put("/api/host-access/default-policy",{allowed_hosts:M,default_host:C.default_host})},applyUser:async(C,M)=>{const V=M.allow_all?null:M.allowed_hosts;await G.put(`/api/host-access/user/${C}`,{allowed_hosts:V,default_host:M.default_host})},applyDelete:C=>G.del(`/api/host-access/user/${C}`),onDefaultConfirmed:()=>xe.success("Default policy updated"),onDefaultRollback:C=>{C&&(i.value=C)},onUserConfirmed:C=>{const M=d(C);xe.success(`Updated access for ${M?M.display_name:C}`)},onUserRollback:(C,M)=>{const V={...l.value};M?V[C]=M:delete V[C],l.value=V},onUserDeleted:C=>{const M={...l.value};delete M[C],l.value=M},onError:(C,M)=>{var q;const V=M.uid?` ${((q=d(M.uid))==null?void 0:q.display_name)||M.uid}`:"";xe.error(`${C.message||"Failed to save"} — reverted${V}`)}});let h=0;async function m(){const C=++h;e.value=!0,t.value="";try{const M=await p.readSnapshot(()=>G.get("/api/host-access"));if(C!==h)return;s.value=M,n.value=M.available_hosts||[],a.value=M.host_descriptions||{},i.value=u(M.default_policy,n.value);const V=M.users||{},q={};for(const[D,I]of Object.entries(V))q[D]=u(I,n.value);l.value=q,p.seed(i.value,q)}catch(M){C===h&&(t.value=M.message||"Failed to fetch host access data")}finally{C===h&&(e.value=!1)}try{const M=await G.get("/api/discord/members")||[];C===h&&(r.value=M)}catch{C===h&&(r.value=[])}}const v=500,k=new Map;function R(C,M){const V=k.get(C);V&&clearTimeout(V.timer);const q={run:M,timer:null};q.timer=setTimeout(()=>{k.delete(C),M()},v),k.set(C,q)}function y(C){const M=k.get(C);M&&(clearTimeout(M.timer),k.delete(C))}function g(){for(const[C,M]of[...k])clearTimeout(M.timer),k.delete(C),M.run()}function b(){R("default",()=>p.saveDefault(i.value))}function S(C,M){i.value.allow_all=!1,M?i.value.allowed_hosts.includes(C)||i.value.allowed_hosts.push(C):(i.value.allowed_hosts=i.value.allowed_hosts.filter(V=>V!==C),i.value.default_host===C&&(i.value.default_host=i.value.allowed_hosts[0]||"")),b()}function w(C){R(`user:${C}`,()=>{const M=l.value[C];M&&p.saveUser(C,M)})}function A(C,M,V){const q=l.value[C];q&&(q.allow_all=!1,V?q.allowed_hosts.includes(M)||q.allowed_hosts.push(M):(q.allowed_hosts=q.allowed_hosts.filter(D=>D!==M),q.default_host===M&&(q.default_host=q.allowed_hosts[0]||"")),w(C))}function E(C,M){const V=l.value[C];V&&(V.default_host=M,w(C))}function x(){o.value=!0}function N(C){!/^\d{15,25}$/.test(C)||l.value[C]||(l.value[C]={allowed_hosts:[...n.value],default_host:n.value[0]||"",allow_all:!1},p.saveUser(C,l.value[C]),o.value=!1)}async function $(C){const M=d(C);await zt({title:"Remove user override",message:`Remove the host access override for ${M?M.display_name:C}? They will fall back to the default policy.`,confirmLabel:"Remove",danger:!0})&&(y(`user:${C}`),await p.deleteUser(C),l.value[C]||xe.success(`Removed override for ${M?M.display_name:C}`))}return Ge(m),ls(g),bt(g),{loading:e,error:t,data:s,availableHosts:n,hostDescriptions:a,defaultPolicy:i,users:l,showAddUser:o,members:r,fetchData:m,saveDefaultPolicy:b,toggleDefaultHost:S,getMember:d,toggleUserHost:A,setUserDefault:E,openAddUser:x,addUserById:N,deleteUser:$,flushPendingSaves:g}}},Ck={template:`
    <div class="p-6 page-fade-in space-y-6">
      <div class="flex items-center justify-between">
    ...
    </div>`,setup(){const e=f([]),t=f(!1),s=f(""),n=f([]),a=f(!1),i=f(!1),l=f(1),o=f(""),r=f(!1),c=f(null),d=f(""),u=f([]),p=f(!1),h=f(null),m=f(""),v=()=>({alias:"",address:"",port:22,ssh_user:"root",os:"linux",description:"",trust_mode:"pinned",enabled:!0,confirm_local:!1,confirm_tofu:!1}),k=f(v()),R=J(()=>["127.0.0.1","localhost","::1"].includes(k.value.address));async function y(){t.value=!0,s.value="";try{const q=await G.get("/api/hosts");e.value=q.hosts||[],o.value=q.default_host||"",r.value=!!q.tofu_enabled}catch(q){s.value=q.message}finally{t.value=!1}}async function g(){try{await G.post("/api/hosts/settings",{default_host:o.value,allow_host_tofu:r.value}),xe.success("Host settings saved and published live"),await y()}catch(q){xe.error(q.message)}}function b(){d.value="",u.value=[],p.value=!1,h.value=null,c.value=null,m.value="",l.value=1,a.value=!0}function S(){i.value=!1,k.value=v(),b()}function w(q){i.value=!0,k.value={...v(),...q},b()}async function A(){try{c.value=await G.get("/api/hosts/public-key")}catch(q){xe.error(q.message)}}async function E(q){try{const D=await G.post("/api/hosts/"+encodeURIComponent(q.alias)+"/import-legacy",{});i.value=!0,k.value={...v(),...q,trust_mode:"pinned"},b(),d.value=D.candidate_token,u.value=D.fingerprints||[],m.value=u.value.join(`
`),l.value=4,xe.info("Imported existing known_hosts trust. Test before activation.")}catch(D){xe.error(D.message)}}async function x(){try{const q=m.value.split(/\s+/).filter(Boolean),D={...k.value,expected_fingerprints:q,candidate_fingerprints:u.value},I=await G.post("/api/hosts/candidates",D);if(d.value=I.candidate_token,u.value=I.fingerprints||[],k.value.trust_mode==="tofu"&&D.candidate_fingerprints.length===0){k.value.confirm_tofu=!1,xe.info("Fingerprint scanned. Review it, tick confirmation, then scan again.");return}l.value=4}catch(q){xe.error(q.message)}}async function N(){try{const q=await G.post("/api/hosts/candidates/"+d.value+"/test",{});p.value=!!q.tested,h.value=q.last_test,p.value&&(l.value=5)}catch(q){xe.error(q.message)}}async function $(){try{await G.post("/api/hosts/candidates/"+d.value+"/commit",{}),xe.success("Host saved and published live"),a.value=!1,await y()}catch(q){xe.error(q.message)}}async function C(q){try{await G.post("/api/hosts/"+encodeURIComponent(q.alias)+"/enabled",{enabled:!q.enabled}),await y()}catch(D){xe.error(D.message)}}async function M(q){var D;if(await zt("Delete host "+q.alias+"? Dependencies will block deletion.")){n.value=[];try{await G.del("/api/hosts/"+encodeURIComponent(q.alias)),await y()}catch(I){n.value=Array.isArray((D=I.data)==null?void 0:D.pending_references)?I.data.pending_references:[],xe.error(I.message)}}}async function V(q){if(await zt("Force revoke "+q.alias+"? Remote outcomes may be unknown."))try{await G.post("/api/hosts/"+encodeURIComponent(q.alias)+"/force-revoke",{}),await y()}catch(D){xe.error(D.message)}}return Ge(y),{hosts:e,loading:t,error:s,pendingReferences:n,wizard:a,editing:i,step:l,defaultHost:o,tofuEnabled:r,form:k,isLocal:R,keyInfo:c,candidate:d,observed:u,tested:p,testResult:h,fingerprintsText:m,load:y,saveSettings:g,beginAdd:S,beginEdit:w,loadKey:A,importLegacy:E,prepare:x,testConnection:N,commit:$,toggle:C,remove:M,forceRevoke:V}}},Ek={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=f(!0),t=f(""),s=f(null),n=f([]),a=f(!1),i=f(!1),l=f(null),o=f(null),r=f(!1),c=f({user_id:"",username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""}),d=f({username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""}),u=J(()=>c.value.host_mode==="select"?c.value.allowed_hosts:c.value.host_mode==="none"?[]:n.value),p=J(()=>d.value.host_mode==="select"?d.value.allowed_hosts:d.value.host_mode==="none"?[]:n.value);function h(E){return E==="admin"?"text-xs px-1.5 py-0.5 rounded bg-red-900/50 text-red-400":E==="user"?"text-xs px-1.5 py-0.5 rounded bg-blue-900/50 text-blue-400":"text-xs px-1.5 py-0.5 rounded bg-gray-700 text-gray-400"}async function m(){e.value=!0,t.value="";try{const E=await G.get("/api/tokens");s.value=E.tokens||[],n.value=E.available_hosts||[]}catch(E){t.value=E.message||"Failed to load tokens"}finally{e.value=!1}}function v(E){return!E||!E.trim()?[]:E.split(",").map(x=>x.trim()).filter(Boolean)}function k(E,x){const N=c.value.allowed_hosts;if(x&&!N.includes(E)&&N.push(E),!x){const $=N.indexOf(E);$>=0&&N.splice($,1)}}function R(E,x){const N=d.value.allowed_hosts;if(x&&!N.includes(E)&&N.push(E),!x){const $=N.indexOf(E);$>=0&&N.splice($,1)}}async function y(){var E;i.value=!0;try{const x=v(c.value.allowed_tools_str),N=c.value.host_mode,$=N==="none"?[]:N==="select"?c.value.allowed_hosts:null,C={user_id:c.value.user_id.trim(),username:c.value.username.trim()||"API",tier:c.value.tier,label:c.value.label.trim(),allowed_tools:x.length?x:[]};$!==null&&(C.allowed_hosts=$),C.default_host=c.value.default_host||"";const M=await G.post("/api/tokens",C);l.value=M.token,c.value={user_id:"",username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""},a.value=!1,xe.success("Token created"),await m()}catch(x){xe.error(((E=x.data)==null?void 0:E.error)||x.message||"Failed to create token")}finally{i.value=!1}}function g(E){o.value=E;const x=E.allowed_hosts;let N="default";x==null?N="default":Array.isArray(x)&&x.length===0?N="none":Array.isArray(x)&&(N="select"),d.value={username:E.username||"",tier:E.tier||"admin",label:E.label||"",host_mode:N,allowed_hosts:Array.isArray(x)?[...x]:[],default_host:E.default_host||"",allowed_tools_str:(E.allowed_tools||[]).join(", ")}}async function b(){var E;if(o.value){r.value=!0;try{const x=v(d.value.allowed_tools_str),N=d.value.host_mode,$={username:d.value.username,tier:d.value.tier,label:d.value.label,allowed_tools:x};N==="none"?$.allowed_hosts=[]:N==="select"?$.allowed_hosts=d.value.allowed_hosts:$.allowed_hosts=null,$.default_host=d.value.default_host||"",await G.put("/api/tokens/"+encodeURIComponent(o.value.user_id),$),o.value=null,xe.success("Token updated"),await m()}catch(x){xe.error(((E=x.data)==null?void 0:E.error)||x.message||"Failed to update")}finally{r.value=!1}}}async function S(E){var N;if(await zt({title:"Regenerate token",message:`Regenerate token for ${E.username||E.user_id}? The old token will stop working immediately.`,confirmLabel:"Regenerate",danger:!0}))try{const $=await G.post("/api/tokens/"+encodeURIComponent(E.user_id)+"/regenerate");l.value=$.token,xe.success("Token regenerated")}catch($){xe.error(((N=$.data)==null?void 0:N.error)||$.message||"Failed to regenerate")}}async function w(E){var N;if(await zt({title:"Delete token",message:`Delete token for ${E.username||E.user_id}? This cannot be undone.`,confirmLabel:"Delete",danger:!0}))try{await G.del("/api/tokens/"+encodeURIComponent(E.user_id)),xe.success("Token deleted"),await m()}catch($){xe.error(((N=$.data)==null?void 0:N.error)||$.message||"Failed to delete")}}async function A(){if(l.value)try{await navigator.clipboard.writeText(l.value),xe.success("Copied to clipboard")}catch{xe.error("Copy failed — select and copy manually")}}return Ge(m),{loading:e,error:t,tokens:s,availableHosts:n,showCreate:a,creating:i,newToken:l,editing:o,saving:r,createForm:c,editForm:d,createDefaultHostOptions:u,editDefaultHostOptions:p,fetchData:m,tierBadge:h,toggleCreateHost:k,toggleEditHost:R,createToken:y,startEdit:g,saveEdit:b,confirmRegenerate:S,confirmDelete:w,copyToken:A}}},Ak=Object.freeze(["enabled","model","reasoning_effort","agent_reasoning_effort","agent_model"]),Rk=Object.freeze(["request_timeout_seconds","stream_stall_timeout_seconds","retry","connection_pool","context_compression","context_budget_overrides","context_utilization"]),Ik=Object.freeze(["enabled","base_url","model","max_tokens"]),Ok=Object.freeze(["enabled","model","max_tokens"]);function qo(e,t){return Object.fromEntries(t.map(s=>[s,e[s]]))}function Ju(e){return qo(e,Ak)}function Yu(e){return qo(e,Rk)}function Lk(e,{includeApiKey:t=!1}={}){const s=qo(e,Ik);return t&&(s.api_key=e.api_key),s}function Nk(e){return{timeout:e.timeout}}function Dk(e,{includeApiKey:t=!1}={}){const s=qo(e,Ok);return t&&(s.api_key=e.api_key),s}function Pk(e){return{timeout:e.timeout}}function Il(e,t=500){let s=null;const n=(...a)=>{s&&clearTimeout(s),s=setTimeout(()=>{s=null,e(...a)},t)};return n.pending=()=>s!==null,n.cancel=()=>{s&&(clearTimeout(s),s=null)},n}const Mk={template:`
    <div class="p-6 page-fade-in">
  ...
  `,setup(){const e=f(!0),t=f(null),s=f(!1),n=f("codex"),a=f({enabled:!1,model:"gpt-5.6-sol",reasoning_effort:"xhigh",agent_reasoning_effort:"auto",agent_model:"auto",request_timeout_seconds:3600,stream_stall_timeout_seconds:180,retry:{max_retries:3,base_delay:1,max_delay:30},connection_pool:{max_connections:10,keepalive_timeout:30},context_compression:{enabled:!0,max_context_chars:null,keep_recent_iterations:30},context_budget_overrides:{},context_utilization:60}),i=["gpt-6-astra","gpt-5.6-sol","gpt-5.6-terra","gpt-5.6-luna","gpt-5.5"],l=J(()=>{const j=a.value.model;return j&&!i.includes(j)?[j,...i]:i}),o=J(()=>{const j=a.value.agent_model;return j&&j!=="auto"&&!i.includes(j)?[j,...i]:i}),r={"gpt-5.5":["max"],"gpt-5.4":["max"],"gpt-5.4-mini":["max"],"gpt-6-astra":["none"]},c=(j,ye)=>!!j&&!!ye&&(r[j]||[]).includes(ye),d=j=>!c(a.value.model,j)&&!(a.value.agent_reasoning_effort===""&&c(a.value.agent_model,j)),u=j=>{const ye=a.value.agent_model;return ye==="auto"?!0:!c(ye||a.value.model,j)},p=J(()=>{const j=a.value.agent_reasoning_effort;return j==="auto"?null:j||a.value.reasoning_effort}),h=j=>c(j,a.value.reasoning_effort)||a.value.agent_model===""&&c(j,p.value),m=j=>c(j,p.value),v=f({enabled:!1,model:"gpt-5.6-luna"}),k=f({unavailable_reason:null}),R=J(()=>{const j=v.value.model;return j&&!i.includes(j)?[j,...i]:i});function y(j){const ye=j.target.value;v.value.enabled=ye!=="",ye!==""&&(v.value.model=ye),Ne()}const g=f(!1),b=f({codex:!1,ollama:!1,kimi:!1}),S=f(null),w=f(!1),A=f(""),E=f(null),x=f(!1);let N=0;const $=J(()=>{var j;return Object.entries(((j=S.value)==null?void 0:j.models)||{}).map(([ye,Ae])=>{var Yt,dn,$n;return{model:ye,floor:Ae.floor,override:Ae.override,effectiveBudget:(Yt=Ae.effective)==null?void 0:Yt.effective_budget,configuredPrimaryChars:(dn=Ae.configured)==null?void 0:dn.primary_chars,primaryChars:($n=Ae.effective)==null?void 0:$n.primary_chars,provenance:Ae.provenance,clampExpiresAt:Ae.clamp_expires_at,densityPriorMilli:Ae.density_prior_milli,densityScope:Ae.density_scope,workloadCalibration:Ae.workload_calibration}})}),C=J(()=>{var j;return((j=S.value)==null?void 0:j.clamps)||[]}),M=J(()=>{var j,ye;return((ye=(j=S.value)==null?void 0:j.models)==null?void 0:ye[a.value.model])||null}),V=f({enabled:!1,base_url:"",model:"",api_key:"",max_tokens:4096,timeout:300}),q=f({enabled:!1,api_key:"",model:"",max_tokens:4096,timeout:300}),D=f(!1),I=f(!1),O=f(!1),U=f(!1),W=f(!1),K=f(!1),X=f(!1),oe=f({configured:null}),ee=f(!1),de=f([]),De=f(""),Q=f(!1),be=f(!1),z=f({configured:null}),re=f(!1),ue=f([]),Ie=f(""),_=f(!1),P=f(!1),H=f(!0),ie=f(""),se=f({configured:null,accounts:[]}),ae=f(null),me=f(null),pe=f(""),ce=f(null),le=f(!1),_e=f(null),ge=f(null),we=f("");let Re=null;function F(j,ye="success"){xe(j,ye==="error"?"error":"success")}function he(j){if(!j)return"?";const ye=j/(1024*1024*1024);return ye>=1?ye.toFixed(1)+" GB":(j/(1024*1024)).toFixed(0)+" MB"}function Se(j){return Number.isFinite(Number(j))?Number(j).toLocaleString():"—"}function Oe(j){return j==null?"automatic (model-derived)":Number(j).toLocaleString()+" characters"}function Fe(j){const ye=new Date(j);return Number.isNaN(ye.getTime())?"unknown":ye.toLocaleString([],{dateStyle:"medium",timeStyle:"short"})}function ft(j){return typeof j=="string"&&j.length>12?j.slice(0,8)+"…"+j.slice(-4):j}function ct(j){return typeof j!="number"||!Number.isFinite(j)?"—":(j/1e3).toFixed(2)}function Et(j){return j==="temporary learned clamp"?"is-clamp":j==="override"?"is-override":"is-built-in"}function At(j){const ye=a.value.context_budget_overrides[j.model];return j.floor!=null&&Number.isFinite(Number(ye))&&Number(ye)>j.floor}function tt(j,ye){const Ae={...a.value.context_budget_overrides};ye.target.value===""?delete Ae[j]:Ae[j]=Number(ye.target.value),a.value.context_budget_overrides=Ae,x.value=!0}function Ze(j){a.value.context_utilization=j.target.value===""?"":Number(j.target.value),x.value=!0}function ne(j){const ye={...a.value.context_budget_overrides};delete ye[j],a.value.context_budget_overrides=ye,x.value=!0}async function Te(){e.value=!0,await Promise.all([Le(),jt(),Ts(),je(),Je()]),e.value=!1}async function Le({preserveBasic:j=!1,preserveAdvanced:ye=!1}={}){try{const Ae=await G.get("/api/llm/status");t.value=Ae,s.value=!1,n.value=Ae.active_provider||"codex",Ae.codex&&!fe.pending()&&(j||(a.value.enabled=Ae.codex.enabled,a.value.model=Ae.codex.model||"gpt-5.6-sol",a.value.reasoning_effort=Ae.codex.reasoning_effort||"medium",a.value.agent_reasoning_effort=Ae.codex.agent_reasoning_effort||"",a.value.agent_model=Ae.codex.agent_model||""),ye||(a.value.request_timeout_seconds=Ae.codex.request_timeout_seconds??a.value.request_timeout_seconds,a.value.stream_stall_timeout_seconds=Ae.codex.stream_stall_timeout_seconds??a.value.stream_stall_timeout_seconds,a.value.retry={...a.value.retry,...Ae.codex.retry||{}},a.value.connection_pool={...a.value.connection_pool,...Ae.codex.connection_pool||{}},a.value.context_compression={...a.value.context_compression,...Ae.codex.context_compression||{}},!x.value&&!O.value&&(a.value.context_budget_overrides={...Ae.codex.context_budget_overrides||{}},a.value.context_utilization=Ae.codex.context_utilization??a.value.context_utilization))),Ae.ollama&&!Ve.pending()&&(j||(V.value.enabled=Ae.ollama.enabled,V.value.base_url=Ae.ollama.base_url||"",V.value.model=Ae.ollama.model||"",V.value.max_tokens=Ae.ollama.max_tokens||4096),ye||(V.value.timeout=Ae.ollama.timeout??V.value.timeout)),Ae.kimi&&!st.pending()&&(j||(q.value.enabled=Ae.kimi.enabled,q.value.model=Ae.kimi.model||"",q.value.max_tokens=Ae.kimi.max_tokens||4096),ye||(q.value.timeout=Ae.kimi.timeout??q.value.timeout)),Ae.auxiliary&&(k.value=Ae.auxiliary,Ne.pending()||(v.value.enabled=Ae.auxiliary.enabled,v.value.model=Ae.auxiliary.model||"gpt-5.6-luna"))}catch{t.value||(t.value={active_provider:"",codex:{configured:null},ollama:{configured:null},kimi:{configured:null}}),s.value=!0}}async function Je(){const j=++N;w.value=!0,A.value="";try{const ye=await G.get("/api/context/windows");if(j!==N)return;S.value=ye,!O.value&&!x.value&&(a.value.context_budget_overrides=Object.fromEntries(Object.entries(ye.models||{}).filter(([,Ae])=>Ae.override!=null).map(([Ae,Yt])=>[Ae,Yt.override])),a.value.context_utilization=ye.utilization??a.value.context_utilization)}catch(ye){j===N&&(A.value=ye.message||"Failed to load context budgets")}finally{j===N&&(w.value=!1)}}async function jt(){try{if(oe.value=await G.get("/api/ollama/status"),ee.value=!1,oe.value.model&&(De.value=oe.value.model),oe.value.configured)try{const j=await G.get("/api/ollama/models");de.value=j.models||[]}catch{de.value=[]}else if(V.value.base_url)try{const j=await G.post("/api/ollama/probe-models",{base_url:V.value.base_url});de.value=j.models||[]}catch{de.value=[]}}catch{ee.value=!0}}async function je(){H.value=!0,ie.value="";try{se.value=await G.get("/api/codex/status")}catch(j){ie.value=j.message||"Failed to fetch Codex status"}finally{H.value=!1}}async function Dt(){const j=t.value?t.value.active_provider:"codex";X.value=!0;try{const ye=await G.post("/api/llm/switch",{provider:n.value});ye.error?(n.value=j,F(ye.error,"error")):(F("Switched to "+n.value+" ("+ye.model+")"),await Te())}catch(ye){n.value=j,F(ye.message||"Switch failed","error")}finally{X.value=!1}}async function Pt(){Q.value=!0;try{const j=await G.post("/api/ollama/reload");F(j.configured?"Ollama reloaded":j.reason||"Ollama not configured",j.configured?"success":"error"),await Te()}catch(j){F(j.message||"Reload failed","error")}finally{Q.value=!1}}async function os(){be.value=!0;try{await G.post("/api/ollama/model",{model:De.value}),F("Model set to "+De.value),await Te()}catch(j){F(j.message||"Failed","error")}finally{be.value=!1}}async function rn(){const j=V.value.base_url;if(!j){F("Enter a base URL first","error");return}K.value=!0;try{const ye=await G.post("/api/ollama/probe-models",{base_url:j});de.value=ye.models||[],de.value.length?(F(de.value.length+" model(s) found"),!V.value.model&&de.value.length&&(V.value.model=de.value[0].name)):F("No models found at "+j,"error")}catch(ye){F(ye.message||"Could not reach Ollama","error")}finally{K.value=!1}}async function Ts(){try{if(z.value=await G.get("/api/kimi/status"),re.value=!1,z.value.model&&(Ie.value=z.value.model),z.value.configured)try{const j=await G.get("/api/kimi/models");ue.value=j.models||[]}catch{ue.value=[]}}catch{re.value=!0}}async function Xn(){_.value=!0;try{const j=await G.post("/api/kimi/reload");F(j.configured?"Kimi reloaded":j.reason||"Kimi not configured",j.configured?"success":"error"),await Te()}catch(j){F(j.message||"Reload failed","error")}finally{_.value=!1}}async function Zs(){P.value=!0;try{await G.post("/api/kimi/model",{model:Ie.value}),F("Model set to "+Ie.value),await Te()}catch(j){F(j.message||"Failed","error")}finally{P.value=!1}}async function Ns(){if(O.value){fe();return}O.value=!0;const j=Ju(a.value);try{await G.put("/api/llm/codex/config",j),F("Codex config saved"),await Promise.all([Le({preserveBasic:!0,preserveAdvanced:!0}),je()])}catch(ye){F(ye.message||"Failed","error");const Ae=JSON.stringify(Ju(a.value))!==JSON.stringify(j);await Promise.all([Le({preserveBasic:Ae,preserveAdvanced:!0}),je()])}finally{O.value=!1}}async function Pn(){if(O.value)return;O.value=!0;const j=Yu(a.value);try{await G.put("/api/llm/codex/config",j),JSON.stringify({context_budget_overrides:a.value.context_budget_overrides,context_utilization:a.value.context_utilization})===JSON.stringify({context_budget_overrides:j.context_budget_overrides,context_utilization:j.context_utilization})&&(x.value=!1),F("Codex advanced settings saved"),await Promise.all([Le({preserveBasic:!0,preserveAdvanced:!0}),je(),Je()])}catch(ye){F(ye.message||"Failed","error");const Ae=JSON.stringify(Yu(a.value))!==JSON.stringify(j);await Promise.all([Le({preserveBasic:!0,preserveAdvanced:Ae}),je(),Je()])}finally{O.value=!1}}async function Zt(){if(U.value){Ve();return}U.value=!0;try{const j=D.value?V.value.api_key:null,ye=Lk(V.value,{includeApiKey:j!==null});await G.put("/api/llm/ollama/config",ye),F("Ollama config saved"),j!==null&&V.value.api_key===j&&(V.value.api_key="",D.value=!1),await Promise.all([Le({preserveBasic:!0,preserveAdvanced:!0}),jt()])}catch(j){F(j.message||"Failed","error")}finally{U.value=!1}}async function zs(){if(!U.value){U.value=!0;try{await G.put("/api/llm/ollama/config",Nk(V.value)),F("Ollama timeout saved"),await Promise.all([Le({preserveBasic:!0,preserveAdvanced:!0}),jt()])}catch(j){F(j.message||"Failed","error")}finally{U.value=!1}}}async function rs(){if(W.value){st();return}W.value=!0;try{const j=I.value?q.value.api_key:null,ye=Dk(q.value,{includeApiKey:j!==null});await G.put("/api/llm/kimi/config",ye),F("Kimi config saved"),j!==null&&q.value.api_key===j&&(q.value.api_key="",I.value=!1),await Promise.all([Le({preserveBasic:!0,preserveAdvanced:!0}),Ts()])}catch(j){F(j.message||"Failed","error")}finally{W.value=!1}}async function it(){if(!W.value){W.value=!0;try{await G.put("/api/llm/kimi/config",Pk(q.value)),F("Kimi timeout saved"),await Promise.all([Le({preserveBasic:!0,preserveAdvanced:!0}),Ts()])}catch(j){F(j.message||"Failed","error")}finally{W.value=!1}}}async function Z(){if(g.value){Ne();return}g.value=!0;try{await G.put("/api/llm/auxiliary/config",v.value),F("Auxiliary config saved"),await Le()}catch(j){F(j.message||"Failed","error"),await Le()}finally{g.value=!1}}const fe=Il(Ns),Ne=Il(Z),Ve=Il(Zt),st=Il(rs),Mt=()=>(fe.cancel(),Ns()),ea=()=>(Ve.cancel(),Zt()),cs=()=>(st.cancel(),rs()),Ea=()=>Pn(),js=()=>zs(),fi=()=>it();async function hi(j){const ye=j.account_key+":"+j.model;E.value=ye;try{const Ae=await G.post("/api/context/windows/clear",{account_key:j.account_key,model:j.model});F(Ae.cleared?"Temporary clamp cleared":"Clamp was already inactive"),await Je()}catch(Ae){F(Ae.message||"Failed to clear clamp","error"),await Je()}finally{E.value=null}}async function Aa(j){try{await G.post("/api/codex/account/"+j+"/activate"),F("Active account switched"),await je()}catch(ye){F(ye.message||"Failed","error")}}async function ta(j){ae.value=j;try{await G.post("/api/codex/account/"+j+"/refresh"),F("Token refreshed"),await je()}catch(ye){F(ye.message||"Refresh failed","error")}finally{ae.value=null}}function Mn(j,ye){me.value=j,pe.value=ye||""}async function Fn(j){try{await G.put("/api/codex/account/"+j+"/label",{label:pe.value}),F("Label updated"),me.value=null,await je()}catch(ye){F(ye.message||"Failed","error")}}async function Js(j,ye){if(await zt({title:"Delete Codex account",message:`Delete ${ye||"account #"+(j+1)}? The pool will reload without it.`,confirmLabel:"Delete",danger:!0}))try{await G.del("/api/codex/account/"+j),F("Deleted. Pool reloaded."),await je()}catch(Yt){F(Yt.message||"Failed","error")}}async function cn(){le.value=!0;try{const j=await G.post("/api/codex/device-code");_e.value=j,ce.value="pending",Jt(j)}catch(j){F(j.message||"Failed","error")}finally{le.value=!1}}async function Jt(j){Re={cancelled:!1};const ye=Re;try{const Ae=await G.post("/api/codex/device-poll",{device_auth_id:j.device_auth_id,user_code:j.user_code,interval:j.interval});if(ye.cancelled)return;ge.value=Ae,ce.value="success",await Te()}catch(Ae){if(ye.cancelled)return;we.value=Ae.message||"Device login failed",ce.value="error"}}function Ra(){Re&&(Re.cancelled=!0),ce.value=null,_e.value=null}return Ge(Te),bt(()=>{Re&&(Re.cancelled=!0),fe.cancel(),Ne.cancel(),Ve.cancel(),st.cancel()}),{loading:e,llmStatus:t,llmStatusLoadFailed:s,selectedProvider:n,switching:X,advancedOpen:b,codexForm:a,codexModelOptions:l,codexAgentModelOptions:o,mainEffortAllowed:d,agentEffortAllowed:u,mainModelOptionDisabled:h,agentModelOptionDisabled:m,auxForm:v,auxData:k,auxModelOptions:R,onAuxModelChange:y,savingAux:g,saveAuxConfigDebounced:Ne,ollamaForm:V,kimiForm:q,savingCodex:O,savingOllama:U,savingKimi:W,probingOllama:K,ollamaKeyDirty:D,kimiKeyDirty:I,fetchCodexStatus:je,ollamaStatus:oe,ollamaStatusLoadFailed:ee,ollamaModels:de,ollamaSelectedModel:De,reloading:Q,settingModel:be,kimiStatus:z,kimiStatusLoadFailed:re,kimiModels:ue,kimiSelectedModel:Ie,reloadingKimi:_,settingKimiModel:P,codexLoading:H,codexError:ie,codexData:se,refreshing:ae,editingLabel:me,labelValue:pe,contextWindows:S,contextWindowsLoading:w,contextWindowsError:A,contextBudgetRows:$,activeClampRows:C,activeContextBudget:M,clearingClamp:E,contextPolicyDirty:x,deviceState:ce,deviceLoading:le,deviceInfo:_e,deviceResult:ge,deviceError:we,fetchAll:Te,fetchLLMStatus:Le,fetchOllamaStatus:jt,fetchKimiStatus:Ts,switchProvider:Dt,reloadOllama:Pt,setOllamaModel:os,reloadKimi:Xn,setKimiModel:Zs,probeOllamaModels:rn,saveCodexConfig:Ns,saveOllamaConfig:Zt,saveKimiConfig:rs,saveCodexAdvancedConfig:Pn,saveOllamaAdvancedConfig:zs,saveKimiAdvancedConfig:it,saveCodexConfigDebounced:fe,saveOllamaConfigDebounced:Ve,saveKimiConfigDebounced:st,saveCodexConfigNow:Mt,saveOllamaConfigNow:ea,saveKimiConfigNow:cs,saveCodexAdvancedConfigNow:Ea,saveOllamaAdvancedConfigNow:js,saveKimiAdvancedConfigNow:fi,activateAccount:Aa,refreshAccount:ta,startEditLabel:Mn,saveLabel:Fn,deleteAccount:Js,startDeviceLogin:cn,cancelDeviceLogin:Ra,formatSize:he,fetchContextWindows:Je,clearContextClamp:hi,setContextOverride:tt,setContextUtilization:Ze,resetContextOverride:ne,overrideAboveFloor:At,formatCount:Se,formatContextCeiling:Oe,formatExpiry:Fe,shortAccountKey:ft,provenanceClass:Et,formatDensity:ct}}},Qu={ok:"text-green-400",pass:"text-green-400",degraded:"text-yellow-400",warn:"text-yellow-400",down:"text-red-400",fail:"text-red-400",unconfigured:"text-gray-500",skipped:"text-gray-500"};function Fk(e){return Qu[e]||Qu[(e||"").toLowerCase()]||"text-gray-400"}const $k={template:`
  ...
  `,setup(){const e=f(null),t=f(""),s=f(null),n=f(!1),a=f(0),i=f(null),l=f(""),o=f(null),r=f(!1),c=f(0),d=f(Date.now());let u=null,p=0,h=0;async function m(){const I=++p;n.value=!0;try{const O=await G.get("/api/turn-state/turns?limit=100");if(I!==p)return;t.value=O.availability,e.value=O.availability==="available"?O.data:null,s.value=null,a.value=Date.now()}catch(O){if(I!==p)return;s.value=O.message||"Turn-state read failed",O.status===503&&(t.value="unavailable")}I===p&&(n.value=!1)}async function v(){const I=++h;r.value=!0;try{const O=await G.get("/api/turn-state/capacity-breakers");if(I!==h)return;l.value=O.availability,i.value=O.availability==="available"?O.data:null,o.value=null,c.value=Date.now()}catch(O){if(I!==h)return;o.value=O.message||"Breaker read failed",O.status===503&&(l.value="unavailable")}I===h&&(r.value=!1)}function k(){m(),v()}const R=J(()=>e.value!==null&&_i(d.value,a.value)>Xu),y=J(()=>i.value!==null&&_i(d.value,c.value)>Xu),g=J(()=>R.value||y.value),b=J(()=>Math.round(_i(d.value,a.value)/1e3)),S=J(()=>Math.round(_i(d.value,c.value)/1e3));function w(I){return vr(I,d.value/1e3)}function A(I){return Uk[w(I)]}const E=J(()=>{var U;const I=[...((U=e.value)==null?void 0:U.turns)||[]],O=d.value/1e3;return I.sort((W,K)=>vr(W,O)-vr(K,O)||(K.last_progress_at||0)-(W.last_progress_at||0))});function x(I){return I.state==="closed"?"badge-success":I.state==="probing"?"badge-warning":"badge-danger"}function N(I){if(I.state==="closed")return"—";const O=_i(d.value,c.value)/1e3,U=Math.max(0,(I.cooldown_remaining_seconds||0)-O);return U>0?`${Math.ceil(U)}s`:I.state==="probing"?"probe in flight":"probe eligible"}function $(I){if(!I)return"";const O=Math.max(0,Math.round(d.value/1e3-I));if(O<90)return`${O}s ago`;const U=Math.round(O/60);return U<90?`${U}m ago`:`${Math.round(U/60)}h ago`}let C=null,M=null,V=!1;function q(){V||(V=!0,k(),C=setInterval(k,Bk),u=setInterval(()=>{d.value=Date.now()},1e3),M=Qe.onReconnected(k))}function D(){V&&(V=!1,C&&(clearInterval(C),C=null),u&&(clearInterval(u),u=null),M&&(M(),M=null))}return Ge(q),gs(q),ls(D),bt(D),{turnsData:e,turnsAvailability:t,turnsError:s,turnsLoading:n,breakersData:i,breakersAvailability:l,breakersError:o,breakersLoading:r,turnsStale:R,breakersStale:y,anyStale:g,turnsAgeSeconds:b,breakersAgeSeconds:S,sortedTurns:E,priorityOf:w,priorityBadge:A,breakerBadge:x,cooldownLabel:N,ageLabel:$,fetchTurns:m,fetchBreakers:v,refreshAll:k,arm:q,disarm:D}}},zk={setup(){const e=f(""),t=f(""),s=f(!1),n=f(""),a=f(!1),i=f(!1),l=f(!1),o=f(null),r=f(!1);async function c(){a.value=!0,o.value=null,r.value=!1;try{const u=await G.get("/api/update/check");e.value=u.current||"",t.value=u.latest||"",s.value=u.update_available||!1,n.value=u.changelog||"",u.error&&(o.value=u.error),r.value=!0}catch(u){o.value=u.message}finally{a.value=!1}}async function d(){if(await zt({title:"Update & restart",message:"Update Odin and restart? Active tasks will be interrupted.",confirmLabel:"Update & Restart",danger:!0})){i.value=!0,o.value=null;try{await G.post("/api/update/apply",{version:"latest"}),l.value=!0,setTimeout(()=>location.reload(),8e3)}catch(p){o.value=p.message}finally{i.value=!1}}}return Ge(c),{current:e,latest:t,updateAvailable:s,changelog:n,checking:a,applying:i,applied:l,error:o,checkDone:r,checkUpdate:c,applyUpdate:d}},template:`
  ...
  `},Xr={brand:"M12 3 4.5 8v8L12 21l7.5-5V8L12 3Zm0 4.2 4.6 3.1L12 16.8l-4.6-6.5L12 7.2Zm0 3.3v3.7",dashboard:"M4 13h6V4H4v9Zm0 7h6v-4H4v4Zm10 0h6v-9h-6v9Zm0-16v4h6V4h-6Z",chat:"M20 15a3 3 0 0 1-3 3H9l-5 3v-6a3 3 0 0 1-1-2.2V7a3 3 0 0 1 3-3h11a3 3 0 0 1 3 3v8Z",operations:"M5 12h3l2-6 4 12 2-6h3M4 4v16h16",history:"M4 12a8 8 0 1 0 2.3-5.7L4 8.5M4 4v4.5h4.5M12 7v5l3 2",home:"M3 11.5 12 4l9 7.5M5.5 10v10h13V10M9 20v-6h6v6",users:"M16 20v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2m7-10a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm13 10v-2a4 4 0 0 0-3-3.9m-2-11.8a4 4 0 0 1 0 7.7",capabilities:"M14.7 6.3a4 4 0 0 0-5.6 5.6L4 17v3h3v-2h2v-2h2l1.1-1.1a4 4 0 0 0 5.6-5.6l-3 3-3-3 3-3Z",personality:"M12 3a8 8 0 0 0-8 8c0 4 3 7 7 7v3h3v-3c3 0 6-3 6-7a8 8 0 0 0-8-8ZM8.5 10h.01M15.5 10h.01M9 14c1.7 1.2 4.3 1.2 6 0",system:"M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8Zm0-5v2m0 14v2M3 12h2m14 0h2M5.6 5.6 7 7m10 10 1.4 1.4M18.4 5.6 17 7M7 17l-1.4 1.4",menu:"M4 7h16M4 12h16M4 17h16",panelLeft:"M9 4H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h4V4Zm0 0h10a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H9M6 8h.01M6 12h.01",chevronLeft:"m15 18-6-6 6-6",chevronRight:"m9 18 6-6-6-6",chevronDown:"m6 9 6 6 6-6",chevronUp:"m18 15-6-6-6 6",search:"m21 21-4.3-4.3M19 11a8 8 0 1 1-16 0 8 8 0 0 1 16 0Z",logout:"M10 17l5-5-5-5m5 5H3m10-8h5a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-5",success:"m5 12 4 4L19 6",warning:"M12 3 2.8 20h18.4L12 3Zm0 6v4m0 3h.01",info:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0-8v4m0-8h.01",error:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm-3-12 6 6m0-6-6 6",edit:"M4 20h4l11-11-4-4L4 16v4Zm9-13 4 4",trash:"M4 7h16m-10 4v5m4-5v5M9 4h6l1 3H8l1-3Zm-3 3 1 13h10l1-13",brain:"M9 5a3 3 0 0 0-5 2.2A3.5 3.5 0 0 0 4 14a3 3 0 0 0 5 2.2V5Zm6 0a3 3 0 0 1 5 2.2 3.5 3.5 0 0 1 0 6.8 3 3 0 0 1-5 2.2V5ZM9 9H7m2 4H6m9-4h2m-2 4h3M12 4v16",refresh:"M20 6v5h-5M4 18v-5h5M18.5 10A7 7 0 0 0 6 7.5L4 11m16 2-2 3.5A7 7 0 0 1 5.5 14",close:"M6 6l12 12M18 6 6 18",command:"M7 8a3 3 0 1 1-3-3h3v14a3 3 0 1 1-3-3h13a3 3 0 1 1-3 3V5a3 3 0 1 1 3 3H7Z",external:"M14 4h6v6m0-6-9 9M19 13v6a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1h6",activity:"M4 12h4l2-5 4 10 2-5h4",shield:"M12 3 5 6v5c0 4.5 2.8 7.7 7 10 4.2-2.3 7-5.5 7-10V6l-7-3Z",database:"M20 6c0 1.7-3.6 3-8 3S4 7.7 4 6s3.6-3 8-3 8 1.3 8 3Zm0 0v6c0 1.7-3.6 3-8 3s-8-1.3-8-3V6m16 6v6c0 1.7-3.6 3-8 3s-8-1.3-8-3v-6",server:"M4 4h16v6H4V4Zm0 10h16v6H4v-6Zm3-7h.01M7 17h.01",terminal:"M5 7l4 4-4 4m6 1h8M3 4h18v16H3V4Z",wrench:"M14.7 6.3a4 4 0 0 0-5.6 5.6L4 17v3h3v-2h2v-2h2l1.1-1.1a4 4 0 0 0 5.6-5.6l-3 3-3-3 3-3Z",bot:"M8 4h8m-4-2v2M5 8h14a2 2 0 0 1 2 2v8H3v-8a2 2 0 0 1 2-2Zm3 4h.01M16 12h.01M8 16h8M3 13H1m22 0h-2",workflow:"M5 5h5v5H5V5Zm9 9h5v5h-5v-5ZM10 7.5h4a3 3 0 0 1 3 3V14M7.5 10v4a3 3 0 0 0 3 3H14",globe:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0-18c2.2 2.5 3.3 5.5 3.3 9S14.2 18.5 12 21m0-18C9.8 5.5 8.7 8.5 8.7 12s1.1 6.5 3.3 9M3 12h18",book:"M4 5a3 3 0 0 1 3-2h5v17H7a3 3 0 0 0-3 1V5Zm16 0a3 3 0 0 0-3-2h-5v17h5a3 3 0 0 1 3 1V5Z",message:"M4 4h16v13H8l-4 4V4Zm4 5h8m-8 4h5",puzzle:"M9 4h3a2 2 0 1 1 4 0h4v5a2 2 0 1 0 0 4v7h-7a2 2 0 1 1-4 0H4v-7a2 2 0 1 0 0-4V4h5",sparkles:"m12 3 1.2 3.8L17 8l-3.8 1.2L12 13l-1.2-3.8L7 8l3.8-1.2L12 3Zm6 10 .8 2.2L21 16l-2.2.8L18 19l-.8-2.2L15 16l2.2-.8L18 13ZM5 14l1 2.8L9 18l-3 1.2L5 22l-1-2.8L1 18l3-1.2L5 14Z",link:"M9.5 14.5 14.5 9m-7 8H6a4 4 0 0 1 0-8h3m6 0h3a4 4 0 0 1 0 8h-3",file:"M6 3h8l4 4v14H6V3Zm8 0v5h5M9 13h6m-6 4h6",folder:"M3 6h7l2 2h9v11H3V6Z",image:"M4 4h16v16H4V4Zm3 12 4-4 3 3 2-2 4 4M9 9h.01",attachment:"m8 12 5-5a3 3 0 1 1 4 4l-7 7a5 5 0 0 1-7-7l7-7",clock:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0-13v5l3 2",calendar:"M5 5h14v15H5V5Zm3-2v4m8-4v4M5 10h14",chart:"M4 20V10m5 10V4m5 16v-7m5 7V7M2 20h20",sliders:"M4 7h10m4 0h2M4 17h2m4 0h10M16 4v6M8 14v6",code:"m9 6-6 6 6 6m6-12 6 6-6 6",copy:"M8 8h11v12H8V8Zm-3 8H4V4h11v1",play:"m8 5 11 7-11 7V5Z",grid:"M4 4h6v6H4V4Zm10 0h6v6h-6V4ZM4 14h6v6H4v-6Zm10 0h6v6h-6v-6Z",list:"M9 6h11M9 12h11M9 18h11M4 6h.01M4 12h.01M4 18h.01",target:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0-5a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm0-4h.01",rotate:"M20 6v5h-5M4 18v-5h5M18.5 10A7 7 0 0 0 6 7.5L4 11m16 2-2 3.5A7 7 0 0 1 5.5 14",archive:"M4 8h16v12H4V8Zm-1-4h18v4H3V4Zm6 8h6",flame:"M12 22c4 0 7-3 7-7 0-5-4-7-4-11-3 2-5 5-5 8-1-1-2-3-1-5-3 2-5 5-5 8 0 4 3 7 8 7Z",eye:"M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Zm10 3a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z",upload:"M12 16V4m-5 5 5-5 5 5M5 20h14",download:"M12 4v12m-5-5 5 5 5-5M5 20h14",undo:"M9 7 4 12l5 5m-5-5h10a6 6 0 0 1 6 6",redo:"m15 7 5 5-5 5m5-5H10a6 6 0 0 0-6 6",minus:"M5 12h14",plus:"M12 5v14M5 12h14",network:"M12 3v4m0 10v4M3 12h4m10 0h4M7.8 7.8l2.1 2.1m4.2 4.2 2.1 2.1m0-8.4-2.1 2.1m-4.2 4.2-2.1 2.1M12 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z",more:"M6 12h.01M12 12h.01M18 12h.01",pause:"M9 5v14m6-14v14",sort:"M8 5v14m0 0-3-3m3 3 3-3M16 19V5m0 0-3 3m3-3 3 3"};Object.freeze(Object.keys(Xr));const Kk={name:"OdinIcon",props:{name:{type:String,required:!0},size:{type:[Number,String],default:18},strokeWidth:{type:[Number,String],default:1.8}},setup(e,{attrs:t}){return()=>ei("svg",{...t,class:["odin-icon",t.class],width:e.size,height:e.size,viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":e.strokeWidth,"stroke-linecap":"round","stroke-linejoin":"round","aria-hidden":t["aria-label"]?void 0:"true",focusable:"false"},[ei("path",{d:Xr[e.name]||Xr.info})])}},Wk=["a[href]","button:not([disabled])",'input:not([disabled]):not([type="hidden"])',"select:not([disabled])","textarea:not([disabled])",'[tabindex]:not([tabindex="-1"])'].join(",");function tp(e){return[...e.querySelectorAll(Wk)].filter(t=>!t.hasAttribute("hidden")&&t.getAttribute("aria-hidden")!=="true")}const Zk={mounted(e){const t=document.activeElement,s=n=>{if(n.key!=="Tab")return;const a=tp(e);if(!a.length){n.preventDefault(),e.focus();return}const i=a[0],l=a[a.length-1];n.shiftKey&&document.activeElement===i?(n.preventDefault(),l.focus()):!n.shiftKey&&document.activeElement===l&&(n.preventDefault(),i.focus())};e.__odinModalFocus={previous:t,onKeydown:s},e.addEventListener("keydown",s),requestAnimationFrame(()=>{(e.querySelector("[autofocus]")||tp(e)[0]||e).focus()})},unmounted(e){var s;const t=e.__odinModalFocus;t&&(e.removeEventListener("keydown",t.onKeydown),(s=t.previous)!=null&&s.isConnected&&typeof t.previous.focus=="function"&&requestAnimationFrame(()=>t.previous.focus()),delete e.__odinModalFocus)}},Jk={template:`
    ...
    </div>`,setup(){const e=f({}),t=f(!0),s=f(null),n=f([]),a=f(!1),i=f([]),l=f(!1),o=f(!1),r=f([]),c=f(0),d=f(null),u=f({reload:!1,clearSessions:!1,stopLoops:!1});let p=0;const h=J(()=>{const W=e.value.uptime_seconds||0,K=Math.floor(W/86400),X=Math.floor(W%86400/3600),oe=Math.floor(W%3600/60),ee=[];return K>0&&ee.push(`${K}d`),X>0&&ee.push(`${X}h`),(ee.length===0||K===0&&X===0)&&ee.push(`${oe}m`),ee.join(" ")}),m=J(()=>{const W=e.value.uptime_seconds||0;return 125.66*(1-Math.min(W/86400,1))}),v=J(()=>{const W=e.value;return[{label:"Guilds",value:W.guild_count??0,icon:"home",iconColor:"text-blue-400"},{label:"Sessions",value:W.session_count??0,icon:"message",iconColor:"text-yellow-400"},{label:"Tools",value:W.tool_count??0,icon:"wrench",iconColor:"text-purple-400",sub:`${W.skill_count??0} skills`,subColor:"text-gray-500"},{label:"Loops",value:W.loop_count??0,icon:"rotate",iconColor:"text-green-400",color:W.loop_count>0?"text-green-400":"",highlight:W.loop_count>0},{label:"Agents",value:W.agent_running??0,icon:"bot",iconColor:"text-cyan-400",sub:W.agent_count>0?`${W.agent_count} total`:"",subColor:"text-gray-500",highlight:(W.agent_running??0)>0},{label:"Processes",value:W.process_running??0,icon:"sliders",iconColor:"text-orange-400",sub:W.process_count>0?`${W.process_count} total`:"",subColor:"text-gray-500",highlight:(W.process_running??0)>0},{label:"Schedules",value:W.schedule_count??0,icon:"clock",iconColor:"text-amber-400",sub:(W.schedule_failing>0?`${W.schedule_failing} failing`:"")+(W.schedule_failing>0&&W.schedule_paused>0?", ":"")+(W.schedule_paused>0?`${W.schedule_paused} paused`:"")||void 0,subColor:W.schedule_failing>0?"text-red-400":"text-yellow-400",color:W.schedule_failing>0?"text-red-400":"",highlight:W.schedule_failing>0},{label:"Users",value:W.user_count??0,icon:"users",iconColor:"text-indigo-400"},...d.value!==null?[{label:"Knowledge",value:d.value,icon:"book",iconColor:"text-teal-400",sub:"chunks",subColor:"text-gray-500"}]:[]]}),k=J(()=>{const W=e.value,K=[];return K.push({label:"Bot",status:W.status==="online"?"ok":"warn",detail:W.status==="online"?"Online":"Starting"}),(W.schedule_failing||0)>0?K.push({label:"Schedules",status:"error",detail:`${W.schedule_failing} failing`}):(W.schedule_count||0)>0&&K.push({label:"Schedules",status:"ok",detail:`${W.schedule_count} configured`}),(W.loop_count||0)>0&&K.push({label:"Loops",status:"ok",detail:`${W.loop_count} active`}),(W.agent_running||0)>0&&K.push({label:"Agents",status:"ok",detail:`${W.agent_running} running`}),(W.process_running||0)>0&&K.push({label:"Processes",status:"ok",detail:`${W.process_running} running`}),K});async function R(){try{e.value=await G.get("/api/status"),s.value=null}catch(W){s.value=W.message}finally{t.value=!1}}let y=0,g=0,b=0,S=0;function w(W,K){const X=new Set;return[...K,...W].filter(oe=>{const ee=oe._hmac||JSON.stringify([oe.timestamp,oe.tool_name,oe.user_id,oe.result_summary,oe.error]);return X.has(ee)?!1:(X.add(ee),!0)})}async function A(){const W=++y,K=b;a.value=!0;try{const X=await G.get("/api/audit?limit=10");if(W!==y)return;const oe=K===b?[]:n.value.filter(ee=>(ee._liveEpoch||0)>K);n.value=w(X,oe).slice(0,10),c.value=oe.length}catch{}W===y&&(a.value=!1)}async function E(){const W=++g,K=S;l.value=!0;try{const X=await G.get("/api/audit?error_only=1&limit=5");if(W!==g)return;const oe=K===S?[]:i.value.filter(ee=>(ee._liveErrorEpoch||0)>K);i.value=w(X,oe).slice(0,5),o.value=!1}catch{if(W!==g)return;o.value=K===S||i.value.length===0}W===g&&(l.value=!1)}async function x(){try{const W=await G.get("/api/knowledge");d.value=(Array.isArray(W)?W:[]).reduce((K,X)=>K+(X.chunks||0),0)}catch{d.value=null}}async function N(){try{const W=await G.get("/api/agents");r.value=W.filter(K=>K.status==="running")}catch{}}async function $(){u.value={...u.value,reload:!0};try{await G.post("/api/reload"),xe.success("Config reloaded")}catch(W){xe.error(W.message)}u.value={...u.value,reload:!1}}async function C(){if(!await zt({title:"Clear all sessions",message:"Clear all conversation sessions? This cannot be undone.",confirmLabel:"Clear All",danger:!0}))return;u.value={...u.value,clearSessions:!0};const K=e.value.session_count;e.value={...e.value,session_count:0};try{const X=await G.post("/api/sessions/clear-all");xe.success(`Cleared ${X.count} session${X.count!==1?"s":""}`),await R()}catch(X){e.value={...e.value,session_count:K},xe.error(X.message)}u.value={...u.value,clearSessions:!1}}async function M(){if(!await zt({title:"Stop all loops",message:"Stop all running loops?",confirmLabel:"Stop Loops",danger:!0}))return;u.value={...u.value,stopLoops:!0};const K=e.value.loop_count;e.value={...e.value,loop_count:0};try{const X=await G.post("/api/loops/stop-all");xe.success(X.result),await R()}catch(X){e.value={...e.value,loop_count:K},xe.error(X.message)}u.value={...u.value,stopLoops:!1}}function V(){t.value=!0,s.value=null,R(),A(),E(),N()}let q=null,D=null,I=null;function O(W){if(W.payload&&W.payload.tool_name){b+=1;const K={...W.payload,_isNew:!0,_key:++p,_liveEpoch:b};n.value.unshift(K),n.value.length>10&&n.value.pop(),c.value++,K.error&&(S+=1,K._liveErrorEpoch=S,o.value=!1,i.value.unshift(K),i.value.length>5&&i.value.pop()),setTimeout(()=>{K._isNew=!1},1500),clearTimeout(I),I=setTimeout(()=>{c.value=0},1e4)}}let U=null;return Ge(async()=>{await Promise.all([R(),A(),E(),N(),x()]),q=setInterval(R,15e3),D=setInterval(N,1e4),Qe.subscribe("events",O),U=Qe.onReconnected(()=>{A(),E()})}),bt(()=>{q&&clearInterval(q),D&&clearInterval(D),clearTimeout(I),Qe.unsubscribe("events",O),U&&(U(),U=null)}),{status:e,loading:t,error:s,uptime:h,uptimeRingOffset:m,stats:v,healthIndicators:k,activity:n,activityLoading:a,newEventCount:c,errors:i,errorsLoading:l,errorsError:o,agents:r,actionLoading:u,fetchActivity:A,fetchErrors:E,fetchStatus:R,onEvent:O,formatTime:uw,formatDuration:ci,retry:V,reloadConfig:$,clearSessions:C,stopAllLoops:M}}};/*! @license DOMPurify 3.4.9 | (c) Cure53 and other contributors | Released under the Apache license 2.0 and Mozilla Public License 2.0 | github.com/cure53/DOMPurify/blob/3.4.9/LICENSE */function sp(e,t){(t==null||t>e.length)&&(t=e.length);for(var s=0,n=Array(t);s<t;s++)n[s]=e[s];return n}function Yk(e){if(Array.isArray(e))return e}function Qk(e,t){var s=e==null?null:typeof Symbol<"u"&&e[Symbol.iterator]||e["@@iterator"];if(s!=null){var n,a,i,l,o=[],r=!0,c=!1;try{if(i=(s=s.call(e)).next,t!==0)for(;!(r=(n=i.call(s)).done)&&(o.push(n.value),o.length!==t);r=!0);}catch(d){c=!0,a=d}finally{try{if(!r&&s.return!=null&&(l=s.return(),Object(l)!==l))return}finally{if(c)throw a}}return o}}function Xk(){throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`)}function eS(e,t){return Yk(e)||Qk(e,t)||tS(e,t)||Xk()}function tS(e,t){if(e){if(typeof e=="string")return sp(e,t);var s={}.toString.call(e).slice(8,-1);return s==="Object"&&e.constructor&&(s=e.constructor.name),s==="Map"||s==="Set"?Array.from(e):s==="Arguments"||/^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(s)?sp(e,t):void 0}}const jm=Object.entries,np=Object.setPrototypeOf,sS=Object.isFrozen,nS=Object.getPrototypeOf,aS=Object.getOwnPropertyDescriptor;let vs=Object.freeze,Hs=Object.seal,Ba=Object.create,Vm=typeof Reflect<"u"&&Reflect,ec=Vm.apply,tc=Vm.construct;vs||(vs=function(t){return t});Hs||(Hs=function(t){return t});ec||(ec=function(t,s){for(var n=arguments.length,a=new Array(n>2?n-2:0),i=2;i<n;i++)a[i-2]=arguments[i];return t.apply(s,a)});tc||(tc=function(t){for(var s=arguments.length,n=new Array(s>1?s-1:0),a=1;a<s;a++)n[a-1]=arguments[a];return new t(...n)});const mn=Nt(Array.prototype.forEach),iS=Nt(Array.prototype.lastIndexOf),ap=Nt(Array.prototype.pop),Da=Nt(Array.prototype.push),lS=Nt(Array.prototype.splice),us=Array.isArray,Ri=Nt(String.prototype.toLowerCase),br=Nt(String.prototype.toString),ip=Nt(String.prototype.match),Pa=Nt(String.prototype.replace),lp=Nt(String.prototype.indexOf),oS=Nt(String.prototype.trim),rS=Nt(Number.prototype.toString),cS=Nt(Boolean.prototype.toString),op=typeof BigInt>"u"?null:Nt(BigInt.prototype.toString),rp=typeof Symbol>"u"?null:Nt(Symbol.prototype.toString),wt=Nt(Object.prototype.hasOwnProperty),wi=Nt(Object.prototype.toString),Vt=Nt(RegExp.prototype.test),ia=dS(TypeError);function Nt(e){return function(t){t instanceof RegExp&&(t.lastIndex=0);for(var s=arguments.length,n=new Array(s>1?s-1:0),a=1;a<s;a++)n[a-1]=arguments[a];return ec(e,t,n)}}function dS(e){return function(){for(var t=arguments.length,s=new Array(t),n=0;n<t;n++)s[n]=arguments[n];return tc(e,s)}}function ze(e,t){let s=arguments.length>2&&arguments[2]!==void 0?arguments[2]:Ri;if(np&&np(e,null),!us(t))return e;let n=t.length;for(;n--;){let a=t[n];if(typeof a=="string"){const i=s(a);i!==a&&(sS(t)||(t[n]=i),a=i)}e[a]=!0}return e}function uS(e){for(let t=0;t<e.length;t++)wt(e,t)||(e[t]=null);return e}function Xt(e){const t=Ba(null);for(const n of jm(e)){var s=eS(n,2);const a=s[0],i=s[1];wt(e,a)&&(us(i)?t[a]=uS(i):i&&typeof i=="object"&&i.constructor===Object?t[a]=Xt(i):t[a]=i)}return t}function pS(e){switch(typeof e){case"string":return e;case"number":return rS(e);case"boolean":return cS(e);case"bigint":return op?op(e):"0";case"symbol":return rp?rp(e):"Symbol()";case"undefined":return wi(e);case"function":case"object":{if(e===null)return wi(e);const t=e,s=Xs(t,"toString");if(typeof s=="function"){const n=s(t);return typeof n=="string"?n:wi(n)}return wi(e)}default:return wi(e)}}function Xs(e,t){for(;e!==null;){const n=aS(e,t);if(n){if(n.get)return Nt(n.get);if(typeof n.value=="function")return Nt(n.value)}e=nS(e)}function s(){return null}return s}function fS(e){try{return Vt(e,""),!0}catch{return!1}}const cp=vs(["a","abbr","acronym","address","area","article","aside","audio","b","bdi","bdo","big","blink","blockquote","body","br","button","canvas","caption","center","cite","code","col","colgroup","content","data","datalist","dd","decorator","del","details","dfn","dialog","dir","div","dl","dt","element","em","fieldset","figcaption","figure","font","footer","form","h1","h2","h3","h4","h5","h6","head","header","hgroup","hr","html","i","img","input","ins","kbd","label","legend","li","main","map","mark","marquee","menu","menuitem","meter","nav","nobr","ol","optgroup","option","output","p","picture","pre","progress","q","rp","rt","ruby","s","samp","search","section","select","shadow","slot","small","source","spacer","span","strike","strong","style","sub","summary","sup","table","tbody","td","template","textarea","tfoot","th","thead","time","tr","track","tt","u","ul","var","video","wbr"]),yr=vs(["svg","a","altglyph","altglyphdef","altglyphitem","animatecolor","animatemotion","animatetransform","circle","clippath","defs","desc","ellipse","enterkeyhint","exportparts","filter","font","g","glyph","glyphref","hkern","image","inputmode","line","lineargradient","marker","mask","metadata","mpath","part","path","pattern","polygon","polyline","radialgradient","rect","stop","style","switch","symbol","text","textpath","title","tref","tspan","view","vkern"]),xr=vs(["feBlend","feColorMatrix","feComponentTransfer","feComposite","feConvolveMatrix","feDiffuseLighting","feDisplacementMap","feDistantLight","feDropShadow","feFlood","feFuncA","feFuncB","feFuncG","feFuncR","feGaussianBlur","feImage","feMerge","feMergeNode","feMorphology","feOffset","fePointLight","feSpecularLighting","feSpotLight","feTile","feTurbulence"]),hS=vs(["animate","color-profile","cursor","discard","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","foreignobject","hatch","hatchpath","mesh","meshgradient","meshpatch","meshrow","missing-glyph","script","set","solidcolor","unknown","use"]),_r=vs(["math","menclose","merror","mfenced","mfrac","mglyph","mi","mlabeledtr","mmultiscripts","mn","mo","mover","mpadded","mphantom","mroot","mrow","ms","mspace","msqrt","mstyle","msub","msup","msubsup","mtable","mtd","mtext","mtr","munder","munderover","mprescripts"]),mS=vs(["maction","maligngroup","malignmark","mlongdiv","mscarries","mscarry","msgroup","mstack","msline","msrow","semantics","annotation","annotation-xml","mprescripts","none"]),dp=vs(["#text"]),up=vs(["accept","action","align","alt","autocapitalize","autocomplete","autopictureinpicture","autoplay","background","bgcolor","border","capture","cellpadding","cellspacing","checked","cite","class","clear","color","cols","colspan","command","commandfor","controls","controlslist","coords","crossorigin","datetime","decoding","default","dir","disabled","disablepictureinpicture","disableremoteplayback","download","draggable","enctype","enterkeyhint","exportparts","face","for","headers","height","hidden","high","href","hreflang","id","inert","inputmode","integrity","ismap","kind","label","lang","list","loading","loop","low","max","maxlength","media","method","min","minlength","multiple","muted","name","nonce","noshade","novalidate","nowrap","open","optimum","part","pattern","placeholder","playsinline","popover","popovertarget","popovertargetaction","poster","preload","pubdate","radiogroup","readonly","rel","required","rev","reversed","role","rows","rowspan","spellcheck","scope","selected","shape","size","sizes","slot","span","srclang","start","src","srcset","step","style","summary","tabindex","title","translate","type","usemap","valign","value","width","wrap","xmlns"]),wr=vs(["accent-height","accumulate","additive","alignment-baseline","amplitude","ascent","attributename","attributetype","azimuth","basefrequency","baseline-shift","begin","bias","by","class","clip","clippathunits","clip-path","clip-rule","color","color-interpolation","color-interpolation-filters","color-profile","color-rendering","cx","cy","d","dx","dy","diffuseconstant","direction","display","divisor","dur","edgemode","elevation","end","exponent","fill","fill-opacity","fill-rule","filter","filterunits","flood-color","flood-opacity","font-family","font-size","font-size-adjust","font-stretch","font-style","font-variant","font-weight","fx","fy","g1","g2","glyph-name","glyphref","gradientunits","gradienttransform","height","href","id","image-rendering","in","in2","intercept","k","k1","k2","k3","k4","kerning","keypoints","keysplines","keytimes","lang","lengthadjust","letter-spacing","kernelmatrix","kernelunitlength","lighting-color","local","marker-end","marker-mid","marker-start","markerheight","markerunits","markerwidth","maskcontentunits","maskunits","max","mask","mask-type","media","method","mode","min","name","numoctaves","offset","operator","opacity","order","orient","orientation","origin","overflow","paint-order","path","pathlength","patterncontentunits","patterntransform","patternunits","points","preservealpha","preserveaspectratio","primitiveunits","r","rx","ry","radius","refx","refy","repeatcount","repeatdur","restart","result","rotate","scale","seed","shape-rendering","slope","specularconstant","specularexponent","spreadmethod","startoffset","stddeviation","stitchtiles","stop-color","stop-opacity","stroke-dasharray","stroke-dashoffset","stroke-linecap","stroke-linejoin","stroke-miterlimit","stroke-opacity","stroke","stroke-width","style","surfacescale","systemlanguage","tabindex","tablevalues","targetx","targety","transform","transform-origin","text-anchor","text-decoration","text-rendering","textlength","type","u1","u2","unicode","values","viewbox","visibility","version","vert-adv-y","vert-origin-x","vert-origin-y","width","word-spacing","wrap","writing-mode","xchannelselector","ychannelselector","x","x1","x2","xmlns","y","y1","y2","z","zoomandpan"]),pp=vs(["accent","accentunder","align","bevelled","close","columnalign","columnlines","columnspacing","columnspan","denomalign","depth","dir","display","displaystyle","encoding","fence","frame","height","href","id","largeop","length","linethickness","lquote","lspace","mathbackground","mathcolor","mathsize","mathvariant","maxsize","minsize","movablelimits","notation","numalign","open","rowalign","rowlines","rowspacing","rowspan","rspace","rquote","scriptlevel","scriptminsize","scriptsizemultiplier","selection","separator","separators","stretchy","subscriptshift","supscriptshift","symmetric","voffset","width","xmlns"]),Ll=vs(["xlink:href","xml:id","xlink:title","xml:space","xmlns:xlink"]),vS=Hs(/{{[\w\W]*|^[\w\W]*}}/g),gS=Hs(/<%[\w\W]*|^[\w\W]*%>/g),bS=Hs(/\${[\w\W]*/g),yS=Hs(/^data-[\-\w.\u00B7-\uFFFF]+$/),xS=Hs(/^aria-[\-\w]+$/),fp=Hs(/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|sms|cid|xmpp|matrix):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i),_S=Hs(/^(?:\w+script|data):/i),wS=Hs(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g),kS=Hs(/^html$/i),SS=Hs(/^[a-z][.\w]*(-[.\w]+)+$/i),Ys={element:1,attribute:2,text:3,cdataSection:4,entityReference:5,entityNode:6,progressingInstruction:7,comment:8,document:9,documentType:10,documentFragment:11,notation:12},TS=function(){return typeof window>"u"?null:window},CS=function(t,s){if(typeof t!="object"||typeof t.createPolicy!="function")return null;let n=null;const a="data-tt-policy-suffix";s&&s.hasAttribute(a)&&(n=s.getAttribute(a));const i="dompurify"+(n?"#"+n:"");try{return t.createPolicy(i,{createHTML(l){return l},createScriptURL(l){return l}})}catch{return console.warn("TrustedTypes policy "+i+" could not be created."),null}},hp=function(){return{afterSanitizeAttributes:[],afterSanitizeElements:[],afterSanitizeShadowDOM:[],beforeSanitizeAttributes:[],beforeSanitizeElements:[],beforeSanitizeShadowDOM:[],uponSanitizeAttribute:[],uponSanitizeElement:[],uponSanitizeShadowNode:[]}};function qm(){let e=arguments.length>0&&arguments[0]!==void 0?arguments[0]:TS();const t=ke=>qm(ke);if(t.version="3.4.9",t.removed=[],!e||!e.document||e.document.nodeType!==Ys.document||!e.Element)return t.isSupported=!1,t;let s=e.document;const n=s,a=n.currentScript;e.DocumentFragment;const i=e.HTMLTemplateElement,l=e.Node,o=e.Element,r=e.NodeFilter,c=e.NamedNodeMap;c===void 0&&(e.NamedNodeMap||e.MozNamedAttrMap),e.HTMLFormElement;const d=e.DOMParser,u=e.trustedTypes,p=o.prototype,h=Xs(p,"cloneNode"),m=Xs(p,"remove"),v=Xs(p,"nextSibling"),k=Xs(p,"childNodes"),R=Xs(p,"parentNode"),y=Xs(p,"shadowRoot"),g=Xs(p,"attributes"),b=l&&l.prototype?Xs(l.prototype,"nodeType"):null,S=l&&l.prototype?Xs(l.prototype,"nodeName"):null;if(typeof i=="function"){const ke=s.createElement("template");ke.content&&ke.content.ownerDocument&&(s=ke.content.ownerDocument)}let w,A="",E,x=!1,N=0;const $=function(){if(N>0)throw ia('A configured TRUSTED_TYPES_POLICY callback (createHTML or createScriptURL) must not call DOMPurify.sanitize, as that causes infinite recursion. Do not pass a policy whose callbacks wrap DOMPurify as TRUSTED_TYPES_POLICY; see the "DOMPurify and Trusted Types" section of the README.')},C=function(L){$(),N++;try{return w.createHTML(L)}finally{N--}},M=function(L){$(),N++;try{return w.createScriptURL(L)}finally{N--}},V=function(){return x||(E=CS(u,a),x=!0),E},q=s,D=q.implementation,I=q.createNodeIterator,O=q.createDocumentFragment,U=q.getElementsByTagName,W=n.importNode;let K=hp();t.isSupported=typeof jm=="function"&&typeof R=="function"&&D&&D.createHTMLDocument!==void 0;const X=vS,oe=gS,ee=bS,de=yS,De=xS,Q=_S,be=wS,z=SS;let re=fp,ue=null;const Ie=ze({},[...cp,...yr,...xr,..._r,...dp]);let _=null;const P=ze({},[...up,...wr,...pp,...Ll]);let H=Object.seal(Ba(null,{tagNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},allowCustomizedBuiltInElements:{writable:!0,configurable:!1,enumerable:!0,value:!1}})),ie=null,se=null;const ae=Object.seal(Ba(null,{tagCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeCheck:{writable:!0,configurable:!1,enumerable:!0,value:null}}));let me=!0,pe=!0,ce=!1,le=!0,_e=!1,ge=!0,we=!1,Re=!1,F=!1,he=!1,Se=!1,Oe=!1,Fe=!0,ft=!1;const ct="user-content-";let Et=!0,At=!1,tt={},Ze=null;const ne=ze({},["annotation-xml","audio","colgroup","desc","foreignobject","head","iframe","math","mi","mn","mo","ms","mtext","noembed","noframes","noscript","plaintext","script","selectedcontent","style","svg","template","thead","title","video","xmp"]);let Te=null;const Le=ze({},["audio","video","img","source","image","track"]);let Je=null;const jt=ze({},["alt","class","for","id","label","name","pattern","placeholder","role","summary","title","value","style","xmlns"]),je="http://www.w3.org/1998/Math/MathML",Dt="http://www.w3.org/2000/svg",Pt="http://www.w3.org/1999/xhtml";let os=Pt,rn=!1,Ts=null;const Xn=ze({},[je,Dt,Pt],br);let Zs=ze({},["mi","mo","mn","ms","mtext"]),Ns=ze({},["annotation-xml"]);const Pn=ze({},["title","style","font","a","script"]);let Zt=null;const zs=["application/xhtml+xml","text/html"],rs="text/html";let it=null,Z=null;const fe=s.createElement("form"),Ne=function(L){return L instanceof RegExp||L instanceof Function},Ve=function(){let L=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};if(Z&&Z===L)return;(!L||typeof L!="object")&&(L={}),L=Xt(L),Zt=zs.indexOf(L.PARSER_MEDIA_TYPE)===-1?rs:L.PARSER_MEDIA_TYPE,it=Zt==="application/xhtml+xml"?br:Ri,ue=wt(L,"ALLOWED_TAGS")&&us(L.ALLOWED_TAGS)?ze({},L.ALLOWED_TAGS,it):Ie,_=wt(L,"ALLOWED_ATTR")&&us(L.ALLOWED_ATTR)?ze({},L.ALLOWED_ATTR,it):P,Ts=wt(L,"ALLOWED_NAMESPACES")&&us(L.ALLOWED_NAMESPACES)?ze({},L.ALLOWED_NAMESPACES,br):Xn,Je=wt(L,"ADD_URI_SAFE_ATTR")&&us(L.ADD_URI_SAFE_ATTR)?ze(Xt(jt),L.ADD_URI_SAFE_ATTR,it):jt,Te=wt(L,"ADD_DATA_URI_TAGS")&&us(L.ADD_DATA_URI_TAGS)?ze(Xt(Le),L.ADD_DATA_URI_TAGS,it):Le,Ze=wt(L,"FORBID_CONTENTS")&&us(L.FORBID_CONTENTS)?ze({},L.FORBID_CONTENTS,it):ne,ie=wt(L,"FORBID_TAGS")&&us(L.FORBID_TAGS)?ze({},L.FORBID_TAGS,it):Xt({}),se=wt(L,"FORBID_ATTR")&&us(L.FORBID_ATTR)?ze({},L.FORBID_ATTR,it):Xt({}),tt=wt(L,"USE_PROFILES")?L.USE_PROFILES&&typeof L.USE_PROFILES=="object"?Xt(L.USE_PROFILES):L.USE_PROFILES:!1,me=L.ALLOW_ARIA_ATTR!==!1,pe=L.ALLOW_DATA_ATTR!==!1,ce=L.ALLOW_UNKNOWN_PROTOCOLS||!1,le=L.ALLOW_SELF_CLOSE_IN_ATTR!==!1,_e=L.SAFE_FOR_TEMPLATES||!1,ge=L.SAFE_FOR_XML!==!1,we=L.WHOLE_DOCUMENT||!1,he=L.RETURN_DOM||!1,Se=L.RETURN_DOM_FRAGMENT||!1,Oe=L.RETURN_TRUSTED_TYPE||!1,F=L.FORCE_BODY||!1,Fe=L.SANITIZE_DOM!==!1,ft=L.SANITIZE_NAMED_PROPS||!1,Et=L.KEEP_CONTENT!==!1,At=L.IN_PLACE||!1,re=fS(L.ALLOWED_URI_REGEXP)?L.ALLOWED_URI_REGEXP:fp,os=typeof L.NAMESPACE=="string"?L.NAMESPACE:Pt,Zs=wt(L,"MATHML_TEXT_INTEGRATION_POINTS")&&L.MATHML_TEXT_INTEGRATION_POINTS&&typeof L.MATHML_TEXT_INTEGRATION_POINTS=="object"?Xt(L.MATHML_TEXT_INTEGRATION_POINTS):ze({},["mi","mo","mn","ms","mtext"]),Ns=wt(L,"HTML_INTEGRATION_POINTS")&&L.HTML_INTEGRATION_POINTS&&typeof L.HTML_INTEGRATION_POINTS=="object"?Xt(L.HTML_INTEGRATION_POINTS):ze({},["annotation-xml"]);const te=wt(L,"CUSTOM_ELEMENT_HANDLING")&&L.CUSTOM_ELEMENT_HANDLING&&typeof L.CUSTOM_ELEMENT_HANDLING=="object"?Xt(L.CUSTOM_ELEMENT_HANDLING):Ba(null);if(H=Ba(null),wt(te,"tagNameCheck")&&Ne(te.tagNameCheck)&&(H.tagNameCheck=te.tagNameCheck),wt(te,"attributeNameCheck")&&Ne(te.attributeNameCheck)&&(H.attributeNameCheck=te.attributeNameCheck),wt(te,"allowCustomizedBuiltInElements")&&typeof te.allowCustomizedBuiltInElements=="boolean"&&(H.allowCustomizedBuiltInElements=te.allowCustomizedBuiltInElements),_e&&(pe=!1),Se&&(he=!0),tt&&(ue=ze({},dp),_=Ba(null),tt.html===!0&&(ze(ue,cp),ze(_,up)),tt.svg===!0&&(ze(ue,yr),ze(_,wr),ze(_,Ll)),tt.svgFilters===!0&&(ze(ue,xr),ze(_,wr),ze(_,Ll)),tt.mathMl===!0&&(ze(ue,_r),ze(_,pp),ze(_,Ll))),ae.tagCheck=null,ae.attributeCheck=null,wt(L,"ADD_TAGS")&&(typeof L.ADD_TAGS=="function"?ae.tagCheck=L.ADD_TAGS:us(L.ADD_TAGS)&&(ue===Ie&&(ue=Xt(ue)),ze(ue,L.ADD_TAGS,it))),wt(L,"ADD_ATTR")&&(typeof L.ADD_ATTR=="function"?ae.attributeCheck=L.ADD_ATTR:us(L.ADD_ATTR)&&(_===P&&(_=Xt(_)),ze(_,L.ADD_ATTR,it))),wt(L,"ADD_URI_SAFE_ATTR")&&us(L.ADD_URI_SAFE_ATTR)&&ze(Je,L.ADD_URI_SAFE_ATTR,it),wt(L,"FORBID_CONTENTS")&&us(L.FORBID_CONTENTS)&&(Ze===ne&&(Ze=Xt(Ze)),ze(Ze,L.FORBID_CONTENTS,it)),wt(L,"ADD_FORBID_CONTENTS")&&us(L.ADD_FORBID_CONTENTS)&&(Ze===ne&&(Ze=Xt(Ze)),ze(Ze,L.ADD_FORBID_CONTENTS,it)),Et&&(ue["#text"]=!0),we&&ze(ue,["html","head","body"]),ue.table&&(ze(ue,["tbody"]),delete ie.tbody),L.TRUSTED_TYPES_POLICY){if(typeof L.TRUSTED_TYPES_POLICY.createHTML!="function")throw ia('TRUSTED_TYPES_POLICY configuration option must provide a "createHTML" hook.');if(typeof L.TRUSTED_TYPES_POLICY.createScriptURL!="function")throw ia('TRUSTED_TYPES_POLICY configuration option must provide a "createScriptURL" hook.');const ve=w;w=L.TRUSTED_TYPES_POLICY;try{A=C("")}catch(Pe){throw w=ve,Pe}}else L.TRUSTED_TYPES_POLICY===null?(w=void 0,A=""):(w===void 0&&(w=V()),w&&typeof A=="string"&&(A=C("")));(K.uponSanitizeElement.length>0||K.uponSanitizeAttribute.length>0)&&ue===Ie&&(ue=Xt(ue)),K.uponSanitizeAttribute.length>0&&_===P&&(_=Xt(_)),vs&&vs(L),Z=L},st=ze({},[...yr,...xr,...hS]),Mt=ze({},[..._r,...mS]),ea=function(L){let te=R(L);(!te||!te.tagName)&&(te={namespaceURI:os,tagName:"template"});const ve=Ri(L.tagName),Pe=Ri(te.tagName);return Ts[L.namespaceURI]?L.namespaceURI===Dt?te.namespaceURI===Pt?ve==="svg":te.namespaceURI===je?ve==="svg"&&(Pe==="annotation-xml"||Zs[Pe]):!!st[ve]:L.namespaceURI===je?te.namespaceURI===Pt?ve==="math":te.namespaceURI===Dt?ve==="math"&&Ns[Pe]:!!Mt[ve]:L.namespaceURI===Pt?te.namespaceURI===Dt&&!Ns[Pe]||te.namespaceURI===je&&!Zs[Pe]?!1:!Mt[ve]&&(Pn[ve]||!st[ve]):!!(Zt==="application/xhtml+xml"&&Ts[L.namespaceURI]):!1},cs=function(L){Da(t.removed,{element:L});try{R(L).removeChild(L)}catch{if(m(L),!R(L))throw ia("a node selected for removal could not be detached from its tree and cannot be safely returned; refusing to sanitize in place")}},Ea=function(L){const te=k?k(L):L.childNodes;if(te){const Pe=[];mn(te,$e=>{Da(Pe,$e)}),mn(Pe,$e=>{try{m($e)}catch{}})}const ve=g?g(L):null;if(ve)for(let Pe=ve.length-1;Pe>=0;--Pe){const $e=ve[Pe],Ue=$e&&$e.name;if(typeof Ue=="string")try{L.removeAttribute(Ue)}catch{}}},js=function(L,te){try{Da(t.removed,{attribute:te.getAttributeNode(L),from:te})}catch{Da(t.removed,{attribute:null,from:te})}if(te.removeAttribute(L),L==="is")if(he||Se)try{cs(te)}catch{}else try{te.setAttribute(L,"")}catch{}},fi=function(L){const te=g?g(L):L.attributes;if(te)for(let ve=te.length-1;ve>=0;--ve){const Pe=te[ve],$e=Pe&&Pe.name;if(!(typeof $e!="string"||_[it($e)]))try{L.removeAttribute($e)}catch{}}},hi=function(L){const te=[L];for(;te.length>0;){const ve=te.pop();(b?b(ve):ve.nodeType)===Ys.element&&fi(ve);const $e=k?k(ve):ve.childNodes;if($e)for(let Ue=$e.length-1;Ue>=0;--Ue)te.push($e[Ue])}},Aa=function(L){let te=null,ve=null;if(F)L="<remove></remove>"+L;else{const Ue=ip(L,/^[\r\n\t ]+/);ve=Ue&&Ue[0]}Zt==="application/xhtml+xml"&&os===Pt&&(L='<html xmlns="http://www.w3.org/1999/xhtml"><head></head><body>'+L+"</body></html>");const Pe=w?C(L):L;if(os===Pt)try{te=new d().parseFromString(Pe,Zt)}catch{}if(!te||!te.documentElement){te=D.createDocument(os,"template",null);try{te.documentElement.innerHTML=rn?A:Pe}catch{}}const $e=te.body||te.documentElement;return L&&ve&&$e.insertBefore(s.createTextNode(ve),$e.childNodes[0]||null),os===Pt?U.call(te,we?"html":"body")[0]:we?te.documentElement:$e},ta=function(L){return I.call(L.ownerDocument||L,L,r.SHOW_ELEMENT|r.SHOW_COMMENT|r.SHOW_TEXT|r.SHOW_PROCESSING_INSTRUCTION|r.SHOW_CDATA_SECTION,null)},Mn=function(L){var te,ve;L.normalize();const Pe=I.call(L.ownerDocument||L,L,r.SHOW_TEXT|r.SHOW_COMMENT|r.SHOW_CDATA_SECTION|r.SHOW_PROCESSING_INSTRUCTION,null);let $e=Pe.nextNode();for(;$e;){let yt=$e.data;mn([X,oe,ee],lt=>{yt=Pa(yt,lt," ")}),$e.data=yt,$e=Pe.nextNode()}const Ue=(te=(ve=L.querySelectorAll)===null||ve===void 0?void 0:ve.call(L,"template"))!==null&&te!==void 0?te:[];mn(Array.from(Ue),yt=>{Js(yt.content)&&Mn(yt.content)})},Fn=function(L){const te=S?S(L):null;return typeof te!="string"||it(te)!=="form"?!1:typeof L.nodeName!="string"||typeof L.textContent!="string"||typeof L.removeChild!="function"||L.attributes!==g(L)||typeof L.removeAttribute!="function"||typeof L.setAttribute!="function"||typeof L.namespaceURI!="string"||typeof L.insertBefore!="function"||typeof L.hasChildNodes!="function"||L.nodeType!==b(L)||L.childNodes!==k(L)},Js=function(L){if(!b||typeof L!="object"||L===null)return!1;try{return b(L)===Ys.documentFragment}catch{return!1}},cn=function(L){if(!b||typeof L!="object"||L===null)return!1;try{return typeof b(L)=="number"}catch{return!1}};function Jt(ke,L,te){mn(ke,ve=>{ve.call(t,L,te,Z)})}const Ra=function(L){let te=null;if(Jt(K.beforeSanitizeElements,L,null),Fn(L))return cs(L),!0;const ve=it(S?S(L):L.nodeName);if(Jt(K.uponSanitizeElement,L,{tagName:ve,allowedTags:ue}),ge&&L.hasChildNodes()&&!cn(L.firstElementChild)&&Vt(/<[/\w!]/g,L.innerHTML)&&Vt(/<[/\w!]/g,L.textContent)||ge&&L.namespaceURI===Pt&&ve==="style"&&cn(L.firstElementChild)||L.nodeType===Ys.progressingInstruction||ge&&L.nodeType===Ys.comment&&Vt(/<[/\w]/g,L.data))return cs(L),!0;if(ie[ve]||!(ae.tagCheck instanceof Function&&ae.tagCheck(ve))&&!ue[ve]){if(!ie[ve]&&Ae(ve)&&(H.tagNameCheck instanceof RegExp&&Vt(H.tagNameCheck,ve)||H.tagNameCheck instanceof Function&&H.tagNameCheck(ve)))return!1;if(Et&&!Ze[ve]){const $e=R(L),Ue=k(L);if(Ue&&$e){const yt=Ue.length;for(let lt=yt-1;lt>=0;--lt){const mt=At?Ue[lt]:h(Ue[lt],!0);$e.insertBefore(mt,v(L))}}}return cs(L),!0}return(b?b(L):L.nodeType)===Ys.element&&!ea(L)||(ve==="noscript"||ve==="noembed"||ve==="noframes")&&Vt(/<\/no(script|embed|frames)/i,L.innerHTML)?(cs(L),!0):(_e&&L.nodeType===Ys.text&&(te=L.textContent,mn([X,oe,ee],$e=>{te=Pa(te,$e," ")}),L.textContent!==te&&(Da(t.removed,{element:L.cloneNode()}),L.textContent=te)),Jt(K.afterSanitizeElements,L,null),!1)},j=function(L,te,ve){if(se[te]||Fe&&(te==="id"||te==="name")&&(ve in s||ve in fe))return!1;const Pe=_[te]||ae.attributeCheck instanceof Function&&ae.attributeCheck(te,L);if(!(pe&&!se[te]&&Vt(de,te))){if(!(me&&Vt(De,te))){if(!Pe||se[te]){if(!(Ae(L)&&(H.tagNameCheck instanceof RegExp&&Vt(H.tagNameCheck,L)||H.tagNameCheck instanceof Function&&H.tagNameCheck(L))&&(H.attributeNameCheck instanceof RegExp&&Vt(H.attributeNameCheck,te)||H.attributeNameCheck instanceof Function&&H.attributeNameCheck(te,L))||te==="is"&&H.allowCustomizedBuiltInElements&&(H.tagNameCheck instanceof RegExp&&Vt(H.tagNameCheck,ve)||H.tagNameCheck instanceof Function&&H.tagNameCheck(ve))))return!1}else if(!Je[te]){if(!Vt(re,Pa(ve,be,""))){if(!((te==="src"||te==="xlink:href"||te==="href")&&L!=="script"&&lp(ve,"data:")===0&&Te[L])){if(!(ce&&!Vt(Q,Pa(ve,be,"")))){if(ve)return!1}}}}}}return!0},ye=ze({},["annotation-xml","color-profile","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","missing-glyph"]),Ae=function(L){return!ye[Ri(L)]&&Vt(z,L)},Yt=function(L){Jt(K.beforeSanitizeAttributes,L,null);const te=L.attributes;if(!te||Fn(L))return;const ve={attrName:"",attrValue:"",keepAttr:!0,allowedAttributes:_,forceKeepAttr:void 0};let Pe=te.length;for(;Pe--;){const $e=te[Pe],Ue=$e.name,yt=$e.namespaceURI,lt=$e.value,mt=it(Ue),T=lt;let B=Ue==="value"?T:oS(T);if(ve.attrName=mt,ve.attrValue=B,ve.keepAttr=!0,ve.forceKeepAttr=void 0,Jt(K.uponSanitizeAttribute,L,ve),B=ve.attrValue,ft&&(mt==="id"||mt==="name")&&lp(B,ct)!==0&&(js(Ue,L),B=ct+B),ge&&Vt(/((--!?|])>)|<\/(style|script|title|xmp|textarea|noscript|iframe|noembed|noframes)/i,B)){js(Ue,L);continue}if(mt==="attributename"&&ip(B,"href")){js(Ue,L);continue}if(ve.forceKeepAttr)continue;if(!ve.keepAttr){js(Ue,L);continue}if(!le&&Vt(/\/>/i,B)){js(Ue,L);continue}_e&&mn([X,oe,ee],Ee=>{B=Pa(B,Ee," ")});const Y=it(L.nodeName);if(!j(Y,mt,B)){js(Ue,L);continue}if(w&&typeof u=="object"&&typeof u.getAttributeType=="function"&&!yt)switch(u.getAttributeType(Y,mt)){case"TrustedHTML":{B=C(B);break}case"TrustedScriptURL":{B=M(B);break}}if(B!==T)try{yt?L.setAttributeNS(yt,Ue,B):L.setAttribute(Ue,B),Fn(L)?cs(L):ap(t.removed)}catch{js(Ue,L)}}Jt(K.afterSanitizeAttributes,L,null)},dn=function(L){let te=null;const ve=ta(L);for(Jt(K.beforeSanitizeShadowDOM,L,null);te=ve.nextNode();)if(Jt(K.uponSanitizeShadowNode,te,null),Ra(te),Yt(te),Js(te.content)&&dn(te.content),(b?b(te):te.nodeType)===Ys.element){const $e=y?y(te):te.shadowRoot;Js($e)&&($n($e),dn($e))}Jt(K.afterSanitizeShadowDOM,L,null)},$n=function(L){const te=[{node:L,shadow:null}];for(;te.length>0;){const ve=te.pop();if(ve.shadow){dn(ve.shadow);continue}const Pe=ve.node,Ue=(b?b(Pe):Pe.nodeType)===Ys.element,yt=k?k(Pe):Pe.childNodes;if(yt)for(let lt=yt.length-1;lt>=0;--lt)te.push({node:yt[lt],shadow:null});if(Ue){const lt=S?S(Pe):null;if(typeof lt=="string"&&it(lt)==="template"){const mt=Pe.content;Js(mt)&&te.push({node:mt,shadow:null})}}if(Ue){const lt=y?y(Pe):Pe.shadowRoot;Js(lt)&&te.push({node:null,shadow:lt},{node:lt,shadow:null})}}};return t.sanitize=function(ke){let L=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{},te=null,ve=null,Pe=null,$e=null;if(rn=!ke,rn&&(ke="<!-->"),typeof ke!="string"&&!cn(ke)&&(ke=pS(ke),typeof ke!="string"))throw ia("dirty is not a string, aborting");if(!t.isSupported)return ke;Re||Ve(L),t.removed=[];const Ue=At&&typeof ke!="string"&&cn(ke);if(Ue){const mt=S?S(ke):ke.nodeName;if(typeof mt=="string"){const T=it(mt);if(!ue[T]||ie[T])throw ia("root node is forbidden and cannot be sanitized in-place")}if(Fn(ke))throw ia("root node is clobbered and cannot be sanitized in-place");try{$n(ke)}catch(T){throw Ea(ke),T}}else if(cn(ke))te=Aa("<!---->"),ve=te.ownerDocument.importNode(ke,!0),ve.nodeType===Ys.element&&ve.nodeName==="BODY"||ve.nodeName==="HTML"?te=ve:te.appendChild(ve),$n(ve);else{if(!he&&!_e&&!we&&ke.indexOf("<")===-1)return w&&Oe?C(ke):ke;if(te=Aa(ke),!te)return he?null:Oe?A:""}te&&F&&cs(te.firstChild);const yt=ta(Ue?ke:te);try{for(;Pe=yt.nextNode();)Ra(Pe),Yt(Pe),Js(Pe.content)&&dn(Pe.content)}catch(mt){throw Ue&&Ea(ke),mt}if(Ue)return mn(t.removed,mt=>{mt.element&&hi(mt.element)}),_e&&Mn(ke),ke;if(he){if(_e&&Mn(te),Se)for($e=O.call(te.ownerDocument);te.firstChild;)$e.appendChild(te.firstChild);else $e=te;return(_.shadowroot||_.shadowrootmode)&&($e=W.call(n,$e,!0)),$e}let lt=we?te.outerHTML:te.innerHTML;return we&&ue["!doctype"]&&te.ownerDocument&&te.ownerDocument.doctype&&te.ownerDocument.doctype.name&&Vt(kS,te.ownerDocument.doctype.name)&&(lt="<!DOCTYPE "+te.ownerDocument.doctype.name+`>
`+lt),_e&&mn([X,oe,ee],mt=>{lt=Pa(lt,mt," ")}),w&&Oe?C(lt):lt},t.setConfig=function(){let ke=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};Ve(ke),Re=!0},t.clearConfig=function(){Z=null,Re=!1,w=E,A=""},t.isValidAttribute=function(ke,L,te){Z||Ve({});const ve=it(ke),Pe=it(L);return j(ve,Pe,te)},t.addHook=function(ke,L){typeof L=="function"&&Da(K[ke],L)},t.removeHook=function(ke,L){if(L!==void 0){const te=iS(K[ke],L);return te===-1?void 0:lS(K[ke],te,1)[0]}return ap(K[ke])},t.removeHooks=function(ke){K[ke]=[]},t.removeAllHooks=function(){K=hp()},t}var mp=qm();function ld(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}var Ca=ld();function Gm(e){Ca=e}var Bi={exec:()=>null};function rt(e,t=""){let s=typeof e=="string"?e:e.source;const n={replace:(a,i)=>{let l=typeof i=="string"?i:i.source;return l=l.replace(hs.caret,"$1"),s=s.replace(a,l),n},getRegex:()=>new RegExp(s,t)};return n}var hs={codeRemoveIndent:/^(?: {1,4}| {0,3}\t)/gm,outputLinkReplace:/\\([\[\]])/g,indentCodeCompensation:/^(\s+)(?:```)/,beginningSpace:/^\s+/,endingHash:/#$/,startingSpaceChar:/^ /,endingSpaceChar:/ $/,nonSpaceChar:/[^ ]/,newLineCharGlobal:/\n/g,tabCharGlobal:/\t/g,multipleSpaceGlobal:/\s+/g,blankLine:/^[ \t]*$/,doubleBlankLine:/\n[ \t]*\n[ \t]*$/,blockquoteStart:/^ {0,3}>/,blockquoteSetextReplace:/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,blockquoteSetextReplace2:/^ {0,3}>[ \t]?/gm,listReplaceTabs:/^\t+/,listReplaceNesting:/^ {1,4}(?=( {4})*[^ ])/g,listIsTask:/^\[[ xX]\] /,listReplaceTask:/^\[[ xX]\] +/,anyLine:/\n.*\n/,hrefBrackets:/^<(.*)>$/,tableDelimiter:/[:|]/,tableAlignChars:/^\||\| *$/g,tableRowBlankLine:/\n[ \t]*$/,tableAlignRight:/^ *-+: *$/,tableAlignCenter:/^ *:-+: *$/,tableAlignLeft:/^ *:-+ *$/,startATag:/^<a /i,endATag:/^<\/a>/i,startPreScriptTag:/^<(pre|code|kbd|script)(\s|>)/i,endPreScriptTag:/^<\/(pre|code|kbd|script)(\s|>)/i,startAngleBracket:/^</,endAngleBracket:/>$/,pedanticHrefTitle:/^([^'"]*[^\s])\s+(['"])(.*)\2/,unicodeAlphaNumeric:/[\p{L}\p{N}]/u,escapeTest:/[&<>"']/,escapeReplace:/[&<>"']/g,escapeTestNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,escapeReplaceNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/g,unescapeTest:/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig,caret:/(^|[^\[])\^/g,percentDecode:/%25/g,findPipe:/\|/g,splitPipe:/ \|/,slashPipe:/\\\|/g,carriageReturn:/\r\n|\r/g,spaceLine:/^ +$/gm,notSpaceStart:/^\S*/,endingNewline:/\n$/,listItemRegex:e=>new RegExp(`^( {0,3}${e})((?:[	 ][^\\n]*)?(?:\\n|$))`),nextBulletRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),hrRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),fencesBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:\`\`\`|~~~)`),headingBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}#`),htmlBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}<(?:[a-z].*>|!--)`,"i")},ES=/^(?:[ \t]*(?:\n|$))+/,AS=/^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/,RS=/^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/,ml=/^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,IS=/^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/,od=/(?:[*+-]|\d{1,9}[.)])/,Km=/^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/,Wm=rt(Km).replace(/bull/g,od).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/\|table/g,"").getRegex(),OS=rt(Km).replace(/bull/g,od).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/table/g,/ {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),rd=/^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/,LS=/^[^\n]+/,cd=/(?!\s*\])(?:\\.|[^\[\]\\])+/,NS=rt(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace("label",cd).replace("title",/(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(),DS=rt(/^( {0,3}bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g,od).getRegex(),Go="address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul",dd=/<!--(?:-?>|[\s\S]*?(?:-->|$))/,PS=rt("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))","i").replace("comment",dd).replace("tag",Go).replace("attribute",/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),Zm=rt(rd).replace("hr",ml).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("|table","").replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Go).getRegex(),MS=rt(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph",Zm).getRegex(),ud={blockquote:MS,code:AS,def:NS,fences:RS,heading:IS,hr:ml,html:PS,lheading:Wm,list:DS,newline:ES,paragraph:Zm,table:Bi,text:LS},vp=rt("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr",ml).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("blockquote"," {0,3}>").replace("code","(?: {4}| {0,3}	)[^\\n]").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Go).getRegex(),FS={...ud,lheading:OS,table:vp,paragraph:rt(rd).replace("hr",ml).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("table",vp).replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Go).getRegex()},$S={...ud,html:rt(`^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:"[^"]*"|'[^']*'|\\s[^'"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))`).replace("comment",dd).replace(/tag/g,"(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b").getRegex(),def:/^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +(["(][^\n]+[")]))? *(?:\n+|$)/,heading:/^(#{1,6})(.*)(?:\n+|$)/,fences:Bi,lheading:/^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,paragraph:rt(rd).replace("hr",ml).replace("heading",` *#{1,6} *[^
]`).replace("lheading",Wm).replace("|table","").replace("blockquote"," {0,3}>").replace("|fences","").replace("|list","").replace("|html","").replace("|tag","").getRegex()},BS=/^\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/,US=/^(`+)([^`]|[^`][\s\S]*?[^`])\1(?!`)/,Jm=/^( {2,}|\\)\n(?!\s*$)/,HS=/^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/,Ko=/[\p{P}\p{S}]/u,pd=/[\s\p{P}\p{S}]/u,Ym=/[^\s\p{P}\p{S}]/u,zS=rt(/^((?![*_])punctSpace)/,"u").replace(/punctSpace/g,pd).getRegex(),Qm=/(?!~)[\p{P}\p{S}]/u,jS=/(?!~)[\s\p{P}\p{S}]/u,VS=/(?:[^\s\p{P}\p{S}]|~)/u,qS=/\[[^[\]]*?\]\((?:\\.|[^\\\(\)]|\((?:\\.|[^\\\(\)])*\))*\)|`[^`]*?`|<[^<>]*?>/g,Xm=/^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,GS=rt(Xm,"u").replace(/punct/g,Ko).getRegex(),KS=rt(Xm,"u").replace(/punct/g,Qm).getRegex(),ev="^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)",WS=rt(ev,"gu").replace(/notPunctSpace/g,Ym).replace(/punctSpace/g,pd).replace(/punct/g,Ko).getRegex(),ZS=rt(ev,"gu").replace(/notPunctSpace/g,VS).replace(/punctSpace/g,jS).replace(/punct/g,Qm).getRegex(),JS=rt("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)punct(_+)(?=[\\s]|$)|notPunctSpace(_+)(?!_)(?=punctSpace|$)|(?!_)punctSpace(_+)(?=notPunctSpace)|[\\s](_+)(?!_)(?=punct)|(?!_)punct(_+)(?!_)(?=punct)","gu").replace(/notPunctSpace/g,Ym).replace(/punctSpace/g,pd).replace(/punct/g,Ko).getRegex(),YS=rt(/\\(punct)/,"gu").replace(/punct/g,Ko).getRegex(),QS=rt(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme",/[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email",/[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex(),XS=rt(dd).replace("(?:-->|$)","-->").getRegex(),e1=rt("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment",XS).replace("attribute",/\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex(),mo=/(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/,t1=rt(/^!?\[(label)\]\(\s*(href)(?:(?:[ \t]*(?:\n[ \t]*)?)(title))?\s*\)/).replace("label",mo).replace("href",/<(?:\\.|[^\n<>\\])+>|[^ \t\n\x00-\x1f]*/).replace("title",/"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex(),tv=rt(/^!?\[(label)\]\[(ref)\]/).replace("label",mo).replace("ref",cd).getRegex(),sv=rt(/^!?\[(ref)\](?:\[\])?/).replace("ref",cd).getRegex(),s1=rt("reflink|nolink(?!\\()","g").replace("reflink",tv).replace("nolink",sv).getRegex(),fd={_backpedal:Bi,anyPunctuation:YS,autolink:QS,blockSkip:qS,br:Jm,code:US,del:Bi,emStrongLDelim:GS,emStrongRDelimAst:WS,emStrongRDelimUnd:JS,escape:BS,link:t1,nolink:sv,punctuation:zS,reflink:tv,reflinkSearch:s1,tag:e1,text:HS,url:Bi},n1={...fd,link:rt(/^!?\[(label)\]\((.*?)\)/).replace("label",mo).getRegex(),reflink:rt(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label",mo).getRegex()},sc={...fd,emStrongRDelimAst:ZS,emStrongLDelim:KS,url:rt(/^((?:ftp|https?):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/,"i").replace("email",/[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(),_backpedal:/(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/,del:/^(~~?)(?=[^\s~])((?:\\.|[^\\])*?(?:\\.|[^\s~\\]))\1(?=[^~]|$)/,text:/^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|https?:\/\/|ftp:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/},a1={...sc,br:rt(Jm).replace("{2,}","*").getRegex(),text:rt(sc.text).replace("\\b_","\\b_| {2,}\\n").replace(/\{2,\}/g,"*").getRegex()},Nl={normal:ud,gfm:FS,pedantic:$S},ki={normal:fd,gfm:sc,breaks:a1,pedantic:n1},i1={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"},gp=e=>i1[e];function en(e,t){if(t){if(hs.escapeTest.test(e))return e.replace(hs.escapeReplace,gp)}else if(hs.escapeTestNoEncode.test(e))return e.replace(hs.escapeReplaceNoEncode,gp);return e}function bp(e){try{e=encodeURI(e).replace(hs.percentDecode,"%")}catch{return null}return e}function yp(e,t){var i;const s=e.replace(hs.findPipe,(l,o,r)=>{let c=!1,d=o;for(;--d>=0&&r[d]==="\\";)c=!c;return c?"|":" |"}),n=s.split(hs.splitPipe);let a=0;if(n[0].trim()||n.shift(),n.length>0&&!((i=n.at(-1))!=null&&i.trim())&&n.pop(),t)if(n.length>t)n.splice(t);else for(;n.length<t;)n.push("");for(;a<n.length;a++)n[a]=n[a].trim().replace(hs.slashPipe,"|");return n}function Si(e,t,s){const n=e.length;if(n===0)return"";let a=0;for(;a<n&&e.charAt(n-a-1)===t;)a++;return e.slice(0,n-a)}function l1(e,t){if(e.indexOf(t[1])===-1)return-1;let s=0;for(let n=0;n<e.length;n++)if(e[n]==="\\")n++;else if(e[n]===t[0])s++;else if(e[n]===t[1]&&(s--,s<0))return n;return s>0?-2:-1}function xp(e,t,s,n,a){const i=t.href,l=t.title||null,o=e[1].replace(a.other.outputLinkReplace,"$1");n.state.inLink=!0;const r={type:e[0].charAt(0)==="!"?"image":"link",raw:s,href:i,title:l,text:o,tokens:n.inlineTokens(o)};return n.state.inLink=!1,r}function o1(e,t,s){const n=e.match(s.other.indentCodeCompensation);if(n===null)return t;const a=n[1];return t.split(`
...
Please report this to https://github.com/markedjs/marked.`,e){const n="<p>An error occurred:</p><pre>"+en(s.message+"",!0)+"</pre>";return t?Promise.resolve(n):n}if(t)return Promise.reject(s);throw s}}},xa=new r1;function at(e,t){return xa.parse(e,t)}at.options=at.setOptions=function(e){return xa.setOptions(e),at.defaults=xa.defaults,Gm(at.defaults),at};at.getDefaults=ld;at.defaults=Ca;at.use=function(...e){return xa.use(...e),at.defaults=xa.defaults,Gm(at.defaults),at};at.walkTokens=function(e,t){return xa.walkTokens(e,t)};at.parseInline=xa.parseInline;at.Parser=Sn;at.parser=Sn.parse;at.Renderer=go;at.TextRenderer=hd;at.Lexer=kn;at.lexer=kn.lex;at.Tokenizer=vo;at.Hooks=Ul;at.parse=at;at.options;at.setOptions;at.use;at.walkTokens;at.parseInline;Sn.parse;kn.lex;const c1={breaks:!0,gfm:!0};function _p(e){if(!e)return"";try{if(typeof at<"u"&&at.parse){const t=at.parse(e,c1);return typeof mp<"u"?mp.sanitize(t):t}}catch{}return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\n/g,"<br>")}function d1(e){const t=new Date(e),s=t.getHours().toString().padStart(2,"0"),n=t.getMinutes().toString().padStart(2,"0");return`${s}:${n}`}const u1={run_command:"terminal",ssh_command:"terminal",run_script:"terminal",read_file:"file",apply_patch:"edit",list_directory:"folder",search_knowledge:"search",ingest_document:"book",generate_image:"image",analyze_image:"eye",analyze_pdf:"file",browser_screenshot:"globe",manage_process:"sliders"};function p1(e){return u1[e]||"wrench"}const f1=/https?:\/\/\S+\.(?:png|jpg|jpeg|gif|webp|svg)(?:\?\S*)?/gi;function wp(e){if(!e)return[];const t=e.match(f1);return t?[...new Set(t)]:[]}const h1={template:`
    ...
    </div>`,setup(){const e=f([]),t=f(""),s=f(!1),n=f(""),a=f(null),i=f(null),l=f(0),o=f("");let r=null,c=0;const d=["Check system health","List running services","Show disk usage","What can you do?"],u=J(()=>t.value.trim().length>0&&!s.value),p=f(Qe.state||"disconnected");let h=null;const m=J(()=>{const D=p.value;return D==="connected"?"Connected":D==="reconnecting"?"Reconnecting…":D==="connecting"?"Connecting…":"REST fallback"}),v=["Watching across all realms...","Processing...","Consulting the bifrost...","Observing..."],k=J(()=>{const D=Math.floor(l.value/4)%v.length,I=l.value;return I>3?`${v[D]} (${I}s)`:v[0]});function R(){It(()=>{a.value&&(a.value.scrollTop=a.value.scrollHeight)})}function y(){if(!i.value)return;const D=i.value;D.style.height="auto",D.style.height=Math.min(D.scrollHeight,120)+"px"}function g(D,I,O={}){const U={id:++c,role:D,content:I,timestamp:Date.now(),html:D==="bot"?_p(I):"",tools_used:O.tools_used||[],is_error:O.is_error||!1,images:D==="bot"?wp(I):[],files:O.files||[],_showTools:!1};return e.value.push(U),R(),D==="bot"&&It(()=>b()),U}function b(){if(!a.value)return;a.value.querySelectorAll(".chat-markdown pre:not([data-copy])").forEach(I=>{I.setAttribute("data-copy","true"),I.style.position="relative";const O=document.createElement("button");O.className="chat-code-copy",O.textContent="Copy",O.addEventListener("click",()=>{const U=I.querySelector("code"),W=U?U.textContent:I.textContent;navigator.clipboard.writeText(W).then(()=>{O.textContent="Copied!",setTimeout(()=>{O.textContent="Copy"},1500)}).catch(()=>{})}),I.appendChild(O)})}function S(D){if(D===0)return!0;const I=e.value[D-1],O=e.value[D],U=new Date(I.timestamp).toDateString(),W=new Date(O.timestamp).toDateString();return U!==W}function w(D){const I=new Date(D),O=new Date;if(I.toDateString()===O.toDateString())return"Today";const U=new Date(O);return U.setDate(U.getDate()-1),I.toDateString()===U.toDateString()?"Yesterday":I.toLocaleDateString(void 0,{month:"short",day:"numeric",year:"numeric"})}function A(D){t.value=D,It(()=>V())}function E(D){window.open(D,"_blank","noopener")}function x(D){D.target.style.display="none"}function N(){l.value=0,r=setInterval(()=>{l.value++},1e3)}function $(){r&&(clearInterval(r),r=null),l.value=0}function C(D){s.value&&(s.value=!1,$(),D.type==="chat_response"?g("bot",D.content,{tools_used:D.tools_used||[],is_error:D.is_error||!1,files:D.files||[]}):D.type==="chat_error"&&g("bot",D.error||"Unknown error",{is_error:!0}),It(()=>{var I;return(I=i.value)==null?void 0:I.focus()}))}async function M(D){try{const I=await G.post("/api/chat",{content:D,channel_id:o.value});g("bot",I.response,{tools_used:I.tools_used||[],is_error:I.is_error||!1,files:I.files||[]})}catch(I){g("bot",I.message||"Failed to send message",{is_error:!0})}}async function V(){const D=t.value.trim();if(!D||s.value)return;g("user",D),t.value="",s.value=!0,N(),i.value&&(i.value.style.height="auto"),Qe.connected&&Qe.sendChat(D,{channelId:o.value})||(await M(D),s.value=!1,$()),It(()=>{var O;return(O=i.value)==null?void 0:O.focus()})}async function q(){n.value="";try{if(!o.value){const I=await G.get("/api/auth/session");o.value=I.channel_id||I.user_id||"web-user"}const D=await G.get("/api/sessions/"+encodeURIComponent(o.value));if(D&&D.messages&&D.messages.length>0){for(const I of D.messages){const O=I.role==="user"?"user":"bot";let U=I.content||"";if(O==="user"){const K=U.match(/^\[.*?\]:\s*/);K&&(U=U.slice(K[0].length))}if(!U.trim())continue;const W={id:++c,role:O,content:U,timestamp:I.timestamp?I.timestamp*1e3:Date.now(),html:O==="bot"?_p(U):"",tools_used:[],is_error:!1,images:O==="bot"?wp(U):[],files:[],_showTools:!1};e.value.push(W)}It(()=>{R(),b()})}}catch(D){D&&D.status!==404&&(n.value="Couldn't load chat history — earlier messages may be missing. Refresh to retry.",xe.error(n.value))}}return Ge(()=>{Qe.subscribe("chat",C),p.value=Qe.state||"disconnected",h=Qe.onState(D=>{p.value=D}),q(),It(()=>{var D;return(D=i.value)==null?void 0:D.focus()})}),bt(()=>{Qe.unsubscribe("chat",C),h&&(h(),h=null),$()}),{messages:e,input:t,sending:s,historyError:n,messagesEl:a,inputEl:i,canSend:u,wsStatus:m,typingText:k,suggestions:d,send:V,autoResize:y,formatTime:d1,formatDate:w,showDateSeparator:S,useSuggestion:A,openImage:E,onImageError:x,getToolIcon:p1,loadHistory:q}}},m1={setup(){const e=f("odin"),t=f(""),s=f(""),n=f(""),a=f({}),i=f([]),l=f([]),o=f(!1),r=f(!1),c=f(null),d=f(!0),u=f(""),p=f(!1),h=f(!1),m=J(()=>e.value==="custom"),v=J(()=>[...i.value,...l.value]),k=J(()=>l.value.includes(e.value)),R=J(()=>{var E;return m.value?t.value||"Odin":((E=a.value[e.value])==null?void 0:E.name)||e.value}),y=J(()=>{var E;return m.value?s.value||"(empty — will use Odin default)":((E=a.value[e.value])==null?void 0:E.identity)||""}),g=J(()=>{var E;return m.value?n.value||"(empty — will use Odin default)":((E=a.value[e.value])==null?void 0:E.voice)||""});async function b(){d.value=!0;try{const E=await G.get("/api/personality");e.value=E.preset||"odin",t.value=E.custom_name||"",s.value=E.custom_identity||"",n.value=E.custom_voice||"",a.value=E.presets||{},i.value=E.builtin_presets||[],l.value=E.user_presets||[]}catch(E){c.value=E.message}finally{d.value=!1}}async function S(){o.value=!0,c.value=null,r.value=!1;try{await G.put("/api/personality",{preset:e.value,custom_name:t.value,custom_identity:s.value,custom_voice:n.value}),r.value=!0,setTimeout(()=>r.value=!1,3e3)}catch(E){c.value=E.message}finally{o.value=!1}}async function w(){const E=u.value.trim();if(E){h.value=!0,c.value=null;try{await G.post("/api/personality/presets",{name:E,display_name:R.value,identity:y.value,voice:g.value}),p.value=!1,u.value="",await b(),e.value=E.toLowerCase().replace(/ /g,"_")}catch(x){c.value=x.message}finally{h.value=!1}}}async function A(){if(await zt({title:"Delete preset",message:`Delete preset "${e.value}"? This cannot be undone.`,confirmLabel:"Delete",danger:!0})){c.value=null;try{await G.del(`/api/personality/presets/${encodeURIComponent(e.value)}`),await b(),e.value="odin"}catch(x){c.value=x.message}}}return Ge(b),{preset:e,customName:t,customIdentity:s,customVoice:n,presets:a,presetNames:v,isCustom:m,isUserPreset:k,previewName:R,previewIdentity:y,previewVoice:g,saving:o,saved:r,error:c,loading:d,save:S,showSavePreset:p,newPresetName:u,savingPreset:h,saveAsPreset:w,deletePreset:A,builtinPresets:i,userPresets:l}},template:`
  ...
  `},_t=(e,t)=>s=>({path:e,query:{...s.query,tab:t}}),nv=[{path:"/",redirect:"/dashboard"},{path:"/dashboard",component:Jk,meta:{label:"Dashboard",icon:"dashboard",section:"Workspace",description:"System posture and recent activity"}},{path:"/chat",component:h1,meta:{label:"Chat",icon:"chat",section:"Workspace",description:"Direct operator conversation"}},{path:"/operations",component:Sw,meta:{label:"Operations",icon:"operations",section:"Operate",description:"Execution, agents, loops, processes, and schedules"}},{path:"/history",component:Nw,meta:{label:"History",icon:"history",section:"Observe",description:"Audit trail, sessions, traces, and usage"}},{path:"/capabilities",component:ek,meta:{label:"Capabilities",icon:"capabilities",section:"Manage",description:"Tools, skills, knowledge, and memory"}},{path:"/personality",component:m1,meta:{label:"Personality",icon:"personality",section:"Manage",description:"Behavior and response profile"}},{path:"/system",component:jk,meta:{label:"System",icon:"system",section:"Manage",description:"Health, configuration, access, and updates"}},{path:"/execution",redirect:_t("/operations","live")},{path:"/agents",redirect:_t("/operations","agents")},{path:"/loops",redirect:_t("/operations","loops")},{path:"/processes",redirect:_t("/operations","processes")},{path:"/schedules",redirect:_t("/operations","schedules")},{path:"/audit",redirect:_t("/history","audit")},{path:"/sessions",redirect:_t("/history","sessions")},{path:"/traces",redirect:_t("/history","traces")},{path:"/usage",redirect:_t("/history","usage")},{path:"/tools",redirect:_t("/capabilities","tools")},{path:"/skills",redirect:_t("/capabilities","skills")},{path:"/mcp",redirect:_t("/capabilities","mcp-servers")},{path:"/knowledge",redirect:_t("/capabilities","knowledge")},{path:"/memory",redirect:_t("/capabilities","memory")},{path:"/learned",redirect:_t("/capabilities","learned")},{path:"/health",redirect:_t("/system","health")},{path:"/resources",redirect:_t("/system","resources")},{path:"/logs",redirect:_t("/system","logs")},{path:"/config",redirect:_t("/system","config")},{path:"/host-access",redirect:_t("/system","host-access")},{path:"/hosts",redirect:_t("/system","hosts")},{path:"/internals",redirect:_t("/system","internals")}],Ui=rw({history:U_(),routes:nv});Ui.afterEach(e=>{var s;const t=(s=e.meta)==null?void 0:s.label;document.title=t?`Odin — ${t}`:"Odin — Management"});const v1={template:`
    ...
    <toast-container />
    <confirm-host />
    <command-palette />`,setup(){const e=f("checking"),t=f(!1),s=f(!1),n=f(!1),a=f(null),i=f(null),l=f(!1);let o=null,r=null;const c=f(!1),d=f("disconnected"),u=f(-1),p=f(null);let h=null;const m=f("starting"),v=f(""),k=nv.filter(U=>U.meta),R=J(()=>["Workspace","Operate","Observe","Manage"].map(U=>({name:U,routes:k.filter(W=>W.meta.section===U)})).filter(U=>U.routes.length)),y=J(()=>{var U;return((U=Ui.currentRoute.value.meta)==null?void 0:U.label)||"Odin"}),g=J(()=>{var U;return((U=Ui.currentRoute.value.meta)==null?void 0:U.section)||"Management"}),b=J(()=>{var U;return((U=Ui.currentRoute.value.meta)==null?void 0:U.description)||"Management console"});function S(){Qe.disconnect(),V&&(clearInterval(V),V=null)}G.onSessionExpired=()=>{t.value=!0,S(),G.setToken(""),e.value="login"};function w(U){var W;if((U.ctrlKey||U.metaKey)&&U.key.toLowerCase()==="k"){e.value==="ready"&&(U.preventDefault(),ep());return}if(n.value&&U.key==="Tab"){const K=[...((W=a.value)==null?void 0:W.querySelectorAll('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])'))||[]];if(K.length){const X=K[0],oe=K[K.length-1];if(U.shiftKey&&(document.activeElement===X||!a.value.contains(document.activeElement))){U.preventDefault(),oe.focus();return}if(!U.shiftKey&&(document.activeElement===oe||!a.value.contains(document.activeElement))){U.preventDefault(),X.focus();return}}}if(U.key==="Escape"&&n.value){n.value=!1,U.preventDefault();return}if(U.key==="/"&&!["INPUT","TEXTAREA","SELECT"].includes(U.target.tagName)){U.preventDefault();const K=document.querySelector('.hm-main input[type="text"], .hm-main .hm-input:not(textarea):not(select)');K&&K.focus()}}function A(){l.value=!!(o!=null&&o.matches),l.value||(n.value=!1)}Ge(async()=>{document.addEventListener("keydown",w),o=window.matchMedia("(max-width: 900px)"),A(),o.addEventListener("change",A);const U=await G.check();U.ok?(e.value="ready",I()):U.needsAuth?e.value="login":(e.value="ready",I())});function E(){t.value=!1,e.value="ready",I()}async function x(){S(),e.value="login",await G.logout()}function N(){s.value=!s.value}function $(){n.value=!n.value}ns(n,async U=>{var W,K;if(U)r=document.activeElement,await It(),(K=(W=a.value)==null?void 0:W.querySelector(".nav-item"))==null||K.focus();else if(r!=null&&r.isConnected){const X=r;r=null,requestAnimationFrame(()=>X.focus())}});const C=J(()=>{switch(d.value){case"connected":return"Live";case"connecting":return"Connecting…";case"reconnecting":return"Reconnecting…";default:return"Disconnected"}});function M(U,W="info",K=3e3){p.value={text:U,level:W},clearTimeout(h),h=setTimeout(()=>{p.value=null},K)}let V=null,q=!1,D=[];function I(){for(const U of D)U();D=[Qe.onStatus(U=>{c.value=U}),Qe.onLatencyChange(U=>{u.value=U}),Qe.onState((U,W)=>{d.value=U,U==="connected"?(q&&M("Connection restored","success"),q=!0):U==="reconnecting"&&W.attempt===1&&M("Connection lost — reconnecting…","warn")})],Qe.connect(),O(),V&&clearInterval(V),V=setInterval(O,15e3)}async function O(){try{const U=await G.get("/api/status");m.value=U.status==="online"?"online":"starting";const W=U.uptime_seconds||0,K=Math.floor(W/3600),X=Math.floor(W%3600/60);v.value=`${K}h ${X}m uptime`}catch{m.value="offline",v.value=""}}return bt(()=>{V&&clearInterval(V);for(const U of D)U();D=[],Qe.disconnect(),document.removeEventListener("keydown",w),o==null||o.removeEventListener("change",A)}),{authState:e,sessionExpired:t,sidebarCollapsed:s,mobileOpen:n,wsConnected:c,wsState:d,wsLatency:u,wsLabel:C,wsToast:p,botStatus:m,botUptime:v,navRoutes:k,navGroups:R,currentPage:y,currentSection:g,currentDescription:b,sidebarEl:a,mobileMenuButton:i,isMobileViewport:l,onLogin:E,logout:x,toggleSidebar:N,toggleMobileNavigation:$,openPalette:ep}}},Qn=ao(g1);Qn.component("odin-icon",Kk);Qn.component("login-screen",v1);Qn.component("toast-container",e_);Qn.component("confirm-host",t_);Qn.component("command-palette",Gk);Qn.directive("modal-focus",Zk);Qn.use(Ui);Qn.mount("#app");
