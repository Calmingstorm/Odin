var Ym=Object.defineProperty;var Qm=(e,t,s)=>t in e?Ym(e,t,{enumerable:!0,configurable:!0,writable:!0,value:s}):e[t]=s;var ct=(e,t,s)=>Qm(e,typeof t!="symbol"?t+"":t,s);(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const i of a)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function s(a){const i={};return a.integrity&&(i.integrity=a.integrity),a.referrerPolicy&&(i.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?i.credentials="include":a.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(a){if(a.ep)return;a.ep=!0;const i=s(a);fetch(a.href,i)}})();class Xm{constructor(){this._persist=localStorage.getItem("odin_persist")==="1",this._token=this._persist?localStorage.getItem("odin_token")||"":sessionStorage.getItem("odin_token")||"";const t=this._persist?localStorage:sessionStorage;this._sessionTimeout=parseInt(t.getItem("odin_session_timeout")||"0",10),this._lastActivity=Date.now(),this._activityTimer=null,this.onSessionExpired=null,this._token&&this._sessionTimeout>0&&this._startActivityMonitor()}get token(){return this._token}get sessionTimeout(){return this._sessionTimeout}setToken(t,s=0){if(this._token=t,this._sessionTimeout=s,this._lastActivity=Date.now(),t){const n=this._persist?localStorage:sessionStorage;n.setItem("odin_token",t),this._persist&&localStorage.setItem("odin_persist","1"),s>0?n.setItem("odin_session_timeout",String(s)):n.removeItem("odin_session_timeout"),this._startActivityMonitor()}else sessionStorage.removeItem("odin_token"),sessionStorage.removeItem("odin_session_timeout"),localStorage.removeItem("odin_token"),localStorage.removeItem("odin_persist"),localStorage.removeItem("odin_session_timeout"),this._stopActivityMonitor()}setPersist(t){this._persist=t}_startActivityMonitor(){this._stopActivityMonitor(),!(this._sessionTimeout<=0)&&(this._activityTimer=setInterval(()=>{(Date.now()-this._lastActivity)/1e3>=this._sessionTimeout&&(this._stopActivityMonitor(),this.onSessionExpired&&this.onSessionExpired())},1e4))}_stopActivityMonitor(){this._activityTimer&&(clearInterval(this._activityTimer),this._activityTimer=null)}_headers(t={}){const s={"Content-Type":"application/json",...t};return this._token&&(s.Authorization=`Bearer ${this._token}`),s}async _request(t,s,n=null,{signal:a}={}){this._lastActivity=Date.now();const i={method:t,headers:this._headers(),signal:a};n!==null&&(i.body=JSON.stringify(n));const l=await fetch(s,i);if(l.status===401)throw new ul("Unauthorized");const r=await l.json().catch(()=>null);if(!l.ok){const o=(r==null?void 0:r.error)||`HTTP ${l.status}`;throw new cd(o,l.status,r)}return r}get(t,s={}){return this._request("GET",t,null,s)}async getBlob(t){this._lastActivity=Date.now();const s=await fetch(t,{method:"GET",headers:this._headers()});if(s.status===401)throw new ul("Unauthorized");if(!s.ok){const n=await s.json().catch(()=>null);throw new cd((n==null?void 0:n.error)||`HTTP ${s.status}`,s.status,n)}return s.blob()}post(t,s){return this._request("POST",t,s)}put(t,s){return this._request("PUT",t,s)}del(t){return this._request("DELETE",t)}async login(t){const s=await fetch("/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:t})}),n=await s.json().catch(()=>null);if(!s.ok)throw new ul((n==null?void 0:n.error)||"Login failed");return this.setToken(n.session_id,n.timeout_seconds||0),n}async logout(){try{await this.post("/api/auth/logout",{})}catch{}this.setToken("")}async check(){try{return await this.get("/api/status"),{ok:!0,needsAuth:!1}}catch(t){return t instanceof ul?{ok:!1,needsAuth:!0}:{ok:!1,needsAuth:!1,error:t.message}}}}class ul extends Error{constructor(t){super(t),this.name="AuthError"}}class cd extends Error{constructor(t,s,n){super(t),this.name="ApiError",this.status=s,this.data=n}}class ev{constructor(t){this._api=t,this._ws=null,this._handlers={logs:[],events:[],chat:[]},this._reconnectDelay=1e3,this._maxReconnectDelay=3e4,this._shouldConnect=!1,this._subscriptions=new Set,this._reconnectAttempt=0,this._lastPongTime=0,this._pingInterval=null,this._latency=-1,this._chatPending=!1,this._state="disconnected",this._lifecycle={status:new Set,state:new Set,latency:new Set,reconnected:new Set},this._everConnected=!1,this._reconnectEpoch=0}onStatus(t){return this._addLifecycle("status",t)}onState(t){return this._addLifecycle("state",t)}onLatencyChange(t){return this._addLifecycle("latency",t)}onReconnected(t){return this._addLifecycle("reconnected",t)}_addLifecycle(t,s){return this._lifecycle[t].add(s),()=>{this._lifecycle[t].delete(s)}}_emitLifecycle(t,...s){for(const n of[...this._lifecycle[t]])try{n(...s)}catch{}}get connected(){var t;return((t=this._ws)==null?void 0:t.readyState)===WebSocket.OPEN}get state(){return this._state}get reconnectAttempt(){return this._reconnectAttempt}get latency(){return this._latency}get reconnectEpoch(){return this._reconnectEpoch}_resetLatency(){this._latency=-1,this._emitLifecycle("latency",-1)}connect(){this._shouldConnect=!0,this._setState("connecting"),this._open()}disconnect(){this._shouldConnect=!1,this._everConnected=!1,this._reconnectTimer&&(clearTimeout(this._reconnectTimer),this._reconnectTimer=null),this._reconnectAttempt=0,this._resetLatency(),this._stopPing(),this._ws&&(this._ws.close(),this._ws=null),this._setState("disconnected")}_setState(t){this._state!==t&&(this._state=t,this._emitLifecycle("state",t,{attempt:this._reconnectAttempt,latency:this._latency}))}_startPing(){this._stopPing(),this._lastPongTime=Date.now(),this._pingInterval=setInterval(()=>{if(this.connected){if(this._lastPongTime&&Date.now()-this._lastPongTime>47e3){try{this._ws.close(4e3,"pong timeout")}catch{}return}try{this._ws.send(JSON.stringify({type:"ping",ts:Date.now()}))}catch{}}},15e3)}_stopPing(){this._pingInterval&&(clearInterval(this._pingInterval),this._pingInterval=null)}subscribe(t,s){this._handlers[t]||(this._handlers[t]=[]),this._handlers[t].push(s),t!=="chat"&&(this._subscriptions.add(t),this.connected&&this._ws.send(JSON.stringify({subscribe:t})))}unsubscribe(t,s){const n=this._handlers[t];if(n){const a=n.indexOf(s);a>=0&&n.splice(a,1),n.length===0&&t!=="chat"&&(this._subscriptions.delete(t),this.connected&&this._ws.send(JSON.stringify({unsubscribe:t})))}}on(t,s){return this.subscribe(t,s)}off(t,s){return this.unsubscribe(t,s)}sendChat(t,{channelId:s,userId:n,username:a}={}){return this.connected?(this._ws.send(JSON.stringify({type:"chat",content:t,channel_id:s||"web-default",user_id:n||void 0,username:a||void 0})),this._chatPending=!0,!0):!1}_open(){if(this._ws||!this._shouldConnect)return;const s=`${location.protocol==="https:"?"wss:":"ws:"}//${location.host}/api/ws`,n=this._api.token?["odin.bearer."+btoa(this._api.token).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"")]:void 0,a=n?new WebSocket(s,n):new WebSocket(s);this._ws=a;const i=()=>this._ws===a;a.onopen=()=>{if(!i())return;const l=this._everConnected;this._everConnected=!0,this._reconnectDelay=1e3,this._reconnectAttempt=0;for(const r of this._subscriptions)a.send(JSON.stringify({subscribe:r}));this._startPing(),this._setState("connected"),this._emitLifecycle("status",!0),l&&(this._reconnectEpoch+=1,this._emitLifecycle("reconnected",this._reconnectEpoch))},a.onmessage=l=>{if(!i())return;let r;try{r=JSON.parse(l.data)}catch{return}const o=r.type;if(o==="pong"){r.ts&&(this._latency=Date.now()-r.ts,this._lastPongTime=Date.now(),this._emitLifecycle("latency",this._latency));return}if(o==="log")for(const c of this._handlers.logs||[])c(r);else if(o==="event")for(const c of this._handlers.events||[])c(r);else if(o==="chat_response"||o==="chat_error"){this._chatPending=!1;for(const c of this._handlers.chat||[])c(r)}},a.onclose=()=>{if(i()){if(this._ws=null,this._stopPing(),this._resetLatency(),this._chatPending){this._chatPending=!1;const l={type:"chat_error",error:"Connection lost — the response may still complete; check session history."};for(const r of this._handlers.chat||[])r(l)}this._emitLifecycle("status",!1),this._shouldConnect?(this._reconnectAttempt++,this._setState("reconnecting"),this._reconnectTimer=setTimeout(()=>this._open(),this._reconnectDelay),this._reconnectDelay=Math.min(this._reconnectDelay*2,this._maxReconnectDelay)):this._setState("disconnected")}},a.onerror=()=>{}}}const K=new Xm,Je=new ev(K);/**
* @vue/shared v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/function xs(e){const t=Object.create(null);for(const s of e.split(","))t[s]=1;return s=>s in t}const qe={},Fa=[],zt=()=>{},Da=()=>!1,va=e=>e.charCodeAt(0)===111&&e.charCodeAt(1)===110&&(e.charCodeAt(2)>122||e.charCodeAt(2)<97),pr=e=>e.startsWith("onUpdate:"),Ve=Object.assign,Xo=(e,t)=>{const s=e.indexOf(t);s>-1&&e.splice(s,1)},tv=Object.prototype.hasOwnProperty,tt=(e,t)=>tv.call(e,t),Ee=Array.isArray,$a=e=>ii(e)==="[object Map]",ga=e=>ii(e)==="[object Set]",dd=e=>ii(e)==="[object Date]",sv=e=>ii(e)==="[object RegExp]",$e=e=>typeof e=="function",Be=e=>typeof e=="string",Yt=e=>typeof e=="symbol",et=e=>e!==null&&typeof e=="object",ec=e=>(et(e)||$e(e))&&$e(e.then)&&$e(e.catch),hp=Object.prototype.toString,ii=e=>hp.call(e),nv=e=>ii(e).slice(8,-1),fr=e=>ii(e)==="[object Object]",hr=e=>Be(e)&&e!=="NaN"&&e[0]!=="-"&&""+parseInt(e,10)===e,yn=xs(",key,ref,ref_for,ref_key,onVnodeBeforeMount,onVnodeMounted,onVnodeBeforeUpdate,onVnodeUpdated,onVnodeBeforeUnmount,onVnodeUnmounted"),av=xs("bind,cloak,else-if,else,for,html,if,model,on,once,pre,show,slot,text,memo"),mr=e=>{const t=Object.create(null);return(s=>t[s]||(t[s]=e(s)))},iv=/-\w/g,rt=mr(e=>e.replace(iv,t=>t.slice(1).toUpperCase())),lv=/\B([A-Z])/g,ps=mr(e=>e.replace(lv,"-$1").toLowerCase()),ba=mr(e=>e.charAt(0).toUpperCase()+e.slice(1)),Ba=mr(e=>e?`on${ba(e)}`:""),Dt=(e,t)=>!Object.is(e,t),Ua=(e,...t)=>{for(let s=0;s<e.length;s++)e[s](...t)},mp=(e,t,s,n=!1)=>{Object.defineProperty(e,t,{configurable:!0,enumerable:!1,writable:n,value:s})},vr=e=>{const t=parseFloat(e);return isNaN(t)?e:t},Pl=e=>{const t=Be(e)?Number(e):NaN;return isNaN(t)?e:t};let ud;const gr=()=>ud||(ud=typeof globalThis<"u"?globalThis:typeof self<"u"?self:typeof window<"u"?window:typeof global<"u"?global:{});function rv(e,t){return e+JSON.stringify(t,(s,n)=>typeof n=="function"?n.toString():n)}const ov="Infinity,undefined,NaN,isFinite,isNaN,parseFloat,parseInt,decodeURI,decodeURIComponent,encodeURI,encodeURIComponent,Math,Number,Date,Array,Object,Boolean,String,RegExp,Map,Set,JSON,Intl,BigInt,console,Error,Symbol",cv=xs(ov);function el(e){if(Ee(e)){const t={};for(let s=0;s<e.length;s++){const n=e[s],a=Be(n)?vp(n):el(n);if(a)for(const i in a)t[i]=a[i]}return t}else if(Be(e)||et(e))return e}const dv=/;(?![^(]*\))/g,uv=/:([^]+)/,pv=/\/\*[^]*?\*\//g;function vp(e){const t={};return e.replace(pv,"").split(dv).forEach(s=>{if(s){const n=s.split(uv);n.length>1&&(t[n[0].trim()]=n[1].trim())}}),t}function tl(e){let t="";if(Be(e))t=e;else if(Ee(e))for(let s=0;s<e.length;s++){const n=tl(e[s]);n&&(t+=n+" ")}else if(et(e))for(const s in e)e[s]&&(t+=s+" ");return t.trim()}function fv(e){if(!e)return null;let{class:t,style:s}=e;return t&&!Be(t)&&(e.class=tl(t)),s&&(e.style=el(s)),e}const hv="html,body,base,head,link,meta,style,title,address,article,aside,footer,header,hgroup,h1,h2,h3,h4,h5,h6,nav,section,div,dd,dl,dt,figcaption,figure,picture,hr,img,li,main,ol,p,pre,ul,a,b,abbr,bdi,bdo,br,cite,code,data,dfn,em,i,kbd,mark,q,rp,rt,ruby,s,samp,small,span,strong,sub,sup,time,u,var,wbr,area,audio,map,track,video,embed,object,param,source,canvas,script,noscript,del,ins,caption,col,colgroup,table,thead,tbody,td,th,tr,button,datalist,fieldset,form,input,label,legend,meter,optgroup,option,output,progress,select,textarea,details,dialog,menu,summary,template,blockquote,iframe,tfoot",mv="svg,animate,animateMotion,animateTransform,circle,clipPath,color-profile,defs,desc,discard,ellipse,feBlend,feColorMatrix,feComponentTransfer,feComposite,feConvolveMatrix,feDiffuseLighting,feDisplacementMap,feDistantLight,feDropShadow,feFlood,feFuncA,feFuncB,feFuncG,feFuncR,feGaussianBlur,feImage,feMerge,feMergeNode,feMorphology,feOffset,fePointLight,feSpecularLighting,feSpotLight,feTile,feTurbulence,filter,foreignObject,g,hatch,hatchpath,image,line,linearGradient,marker,mask,mesh,meshgradient,meshpatch,meshrow,metadata,mpath,path,pattern,polygon,polyline,radialGradient,rect,set,solidcolor,stop,switch,symbol,text,textPath,title,tspan,unknown,use,view",vv="annotation,annotation-xml,maction,maligngroup,malignmark,math,menclose,merror,mfenced,mfrac,mfraction,mglyph,mi,mlabeledtr,mlongdiv,mmultiscripts,mn,mo,mover,mpadded,mphantom,mprescripts,mroot,mrow,ms,mscarries,mscarry,msgroup,msline,mspace,msqrt,msrow,mstack,mstyle,msub,msubsup,msup,mtable,mtd,mtext,mtr,munder,munderover,none,semantics",gv="area,base,br,col,embed,hr,img,input,link,meta,param,source,track,wbr",bv=xs(hv),yv=xs(mv),xv=xs(vv),_v=xs(gv),wv="itemscope,allowfullscreen,formnovalidate,ismap,nomodule,novalidate,readonly",kv=xs(wv);function gp(e){return!!e||e===""}function Sv(e,t){if(e.length!==t.length)return!1;let s=!0;for(let n=0;s&&n<e.length;n++)s=kn(e[n],t[n]);return s}function kn(e,t){if(e===t)return!0;let s=dd(e),n=dd(t);if(s||n)return s&&n?e.getTime()===t.getTime():!1;if(s=Yt(e),n=Yt(t),s||n)return e===t;if(s=Ee(e),n=Ee(t),s||n)return s&&n?Sv(e,t):!1;if(s=et(e),n=et(t),s||n){if(!s||!n)return!1;const a=Object.keys(e).length,i=Object.keys(t).length;if(a!==i)return!1;for(const l in e){const r=e.hasOwnProperty(l),o=t.hasOwnProperty(l);if(r&&!o||!r&&o||!kn(e[l],t[l]))return!1}}return String(e)===String(t)}function br(e,t){return e.findIndex(s=>kn(s,t))}const bp=e=>!!(e&&e.__v_isRef===!0),yp=e=>Be(e)?e:e==null?"":Ee(e)||et(e)&&(e.toString===hp||!$e(e.toString))?bp(e)?yp(e.value):JSON.stringify(e,xp,2):String(e),xp=(e,t)=>bp(t)?xp(e,t.value):$a(t)?{[`Map(${t.size})`]:[...t.entries()].reduce((s,[n,a],i)=>(s[zr(n,i)+" =>"]=a,s),{})}:ga(t)?{[`Set(${t.size})`]:[...t.values()].map(s=>zr(s))}:Yt(t)?zr(t):et(t)&&!Ee(t)&&!fr(t)?String(t):t,zr=(e,t="")=>{var s;return Yt(e)?`Symbol(${(s=e.description)!=null?s:t})`:e};function Tv(e){return e==null?"initial":typeof e=="string"?e===""?" ":e:String(e)}/**
* @vue/reactivity v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/let Nt;class tc{constructor(t=!1){this.detached=t,this._active=!0,this._on=0,this.effects=[],this.cleanups=[],this._isPaused=!1,this._warnOnRun=!0,this.__v_skip=!0,!t&&Nt&&(Nt.active?(this.parent=Nt,this.index=(Nt.scopes||(Nt.scopes=[])).push(this)-1):(this._active=!1,this._warnOnRun=!1))}get active(){return this._active}pause(){if(this._active){this._isPaused=!0;let t,s;if(this.scopes)for(t=0,s=this.scopes.length;t<s;t++)this.scopes[t].pause();for(t=0,s=this.effects.length;t<s;t++)this.effects[t].pause()}}resume(){if(this._active&&this._isPaused){this._isPaused=!1;let t,s;if(this.scopes)for(t=0,s=this.scopes.length;t<s;t++)this.scopes[t].resume();for(t=0,s=this.effects.length;t<s;t++)this.effects[t].resume()}}run(t){if(this._active){const s=Nt;try{return Nt=this,t()}finally{Nt=s}}}on(){++this._on===1&&(this.prevScope=Nt,Nt=this)}off(){if(this._on>0&&--this._on===0){if(Nt===this)Nt=this.prevScope;else{let t=Nt;for(;t;){if(t.prevScope===this){t.prevScope=this.prevScope;break}t=t.prevScope}}this.prevScope=void 0}}stop(t){if(this._active){this._active=!1;let s,n;for(s=0,n=this.effects.length;s<n;s++)this.effects[s].stop();for(this.effects.length=0,s=0,n=this.cleanups.length;s<n;s++)this.cleanups[s]();if(this.cleanups.length=0,this.scopes){for(s=0,n=this.scopes.length;s<n;s++)this.scopes[s].stop(!0);this.scopes.length=0}if(!this.detached&&this.parent&&!t){const a=this.parent.scopes.pop();a&&a!==this&&(this.parent.scopes[this.index]=a,a.index=this.index)}this.parent=void 0}}}function Cv(e){return new tc(e)}function _p(){return Nt}function Ev(e,t=!1){Nt&&Nt.cleanups.push(e)}let dt;const jr=new WeakSet;class Pi{constructor(t){this.fn=t,this.deps=void 0,this.depsTail=void 0,this.flags=5,this.next=void 0,this.cleanup=void 0,this.scheduler=void 0,Nt&&(Nt.active?Nt.effects.push(this):this.flags&=-2)}pause(){this.flags|=64}resume(){this.flags&64&&(this.flags&=-65,jr.has(this)&&(jr.delete(this),this.trigger()))}notify(){this.flags&2&&!(this.flags&32)||this.flags&8||kp(this)}run(){if(!(this.flags&1))return this.fn();this.flags|=2,pd(this),Sp(this);const t=dt,s=Fs;dt=this,Fs=!0;try{return this.fn()}finally{Tp(this),dt=t,Fs=s,this.flags&=-3}}stop(){if(this.flags&1){for(let t=this.deps;t;t=t.nextDep)ac(t);this.deps=this.depsTail=void 0,pd(this),this.onStop&&this.onStop(),this.flags&=-2}}trigger(){this.flags&64?jr.add(this):this.scheduler?this.scheduler():this.runIfDirty()}runIfDirty(){bo(this)&&this.run()}get dirty(){return bo(this)}}let wp=0,Ti,Ci;function kp(e,t=!1){if(e.flags|=8,t){e.next=Ci,Ci=e;return}e.next=Ti,Ti=e}function sc(){wp++}function nc(){if(--wp>0)return;if(Ci){let t=Ci;for(Ci=void 0;t;){const s=t.next;t.next=void 0,t.flags&=-9,t=s}}let e;for(;Ti;){let t=Ti;for(Ti=void 0;t;){const s=t.next;if(t.next=void 0,t.flags&=-9,t.flags&1)try{t.trigger()}catch(n){e||(e=n)}t=s}}if(e)throw e}function Sp(e){for(let t=e.deps;t;t=t.nextDep)t.version=-1,t.prevActiveLink=t.dep.activeLink,t.dep.activeLink=t}function Tp(e){let t,s=e.depsTail,n=s;for(;n;){const a=n.prevDep;n.version===-1?(n===s&&(s=a),ac(n),Av(n)):t=n,n.dep.activeLink=n.prevActiveLink,n.prevActiveLink=void 0,n=a}e.deps=t,e.depsTail=s}function bo(e){for(let t=e.deps;t;t=t.nextDep)if(t.dep.version!==t.version||t.dep.computed&&(Cp(t.dep.computed)||t.dep.version!==t.version))return!0;return!!e._dirty}function Cp(e){if(e.flags&4&&!(e.flags&16)||(e.flags&=-17,e.globalVersion===Fi)||(e.globalVersion=Fi,!e.isSSR&&e.flags&128&&(!e.deps&&!e._dirty||!bo(e))))return;e.flags|=2;const t=e.dep,s=dt,n=Fs;dt=e,Fs=!0;try{Sp(e);const a=e.fn(e._value);(t.version===0||Dt(a,e._value))&&(e.flags|=128,e._value=a,t.version++)}catch(a){throw t.version++,a}finally{dt=s,Fs=n,Tp(e),e.flags&=-3}}function ac(e,t=!1){const{dep:s,prevSub:n,nextSub:a}=e;if(n&&(n.nextSub=a,e.prevSub=void 0),a&&(a.prevSub=n,e.nextSub=void 0),s.subs===e&&(s.subs=n,!n&&s.computed)){s.computed.flags&=-5;for(let i=s.computed.deps;i;i=i.nextDep)ac(i,!0)}!t&&!--s.sc&&s.map&&s.map.delete(s.key)}function Av(e){const{prevDep:t,nextDep:s}=e;t&&(t.nextDep=s,e.prevDep=void 0),s&&(s.prevDep=t,e.nextDep=void 0)}function Rv(e,t){e.effect instanceof Pi&&(e=e.effect.fn);const s=new Pi(e);t&&Ve(s,t);try{s.run()}catch(a){throw s.stop(),a}const n=s.run.bind(s);return n.effect=s,n}function Iv(e){e.effect.stop()}let Fs=!0;const Ep=[];function Sn(){Ep.push(Fs),Fs=!1}function Tn(){const e=Ep.pop();Fs=e===void 0?!0:e}function pd(e){const{cleanup:t}=e;if(e.cleanup=void 0,t){const s=dt;dt=void 0;try{t()}finally{dt=s}}}let Fi=0;class Ov{constructor(t,s){this.sub=t,this.dep=s,this.version=s.version,this.nextDep=this.prevDep=this.nextSub=this.prevSub=this.prevActiveLink=void 0}}class yr{constructor(t){this.computed=t,this.version=0,this.activeLink=void 0,this.subs=void 0,this.map=void 0,this.key=void 0,this.sc=0,this.__v_skip=!0}track(t){if(!dt||!Fs||dt===this.computed)return;let s=this.activeLink;if(s===void 0||s.sub!==dt)s=this.activeLink=new Ov(dt,this),dt.deps?(s.prevDep=dt.depsTail,dt.depsTail.nextDep=s,dt.depsTail=s):dt.deps=dt.depsTail=s,Ap(s);else if(s.version===-1&&(s.version=this.version,s.nextDep)){const n=s.nextDep;n.prevDep=s.prevDep,s.prevDep&&(s.prevDep.nextDep=n),s.prevDep=dt.depsTail,s.nextDep=void 0,dt.depsTail.nextDep=s,dt.depsTail=s,dt.deps===s&&(dt.deps=n)}return s}trigger(t){this.version++,Fi++,this.notify(t)}notify(t){sc();try{for(let s=this.subs;s;s=s.prevSub)s.sub.notify()&&s.sub.dep.notify()}finally{nc()}}}function Ap(e){if(e.dep.sc++,e.sub.flags&4){const t=e.dep.computed;if(t&&!e.dep.subs){t.flags|=20;for(let n=t.deps;n;n=n.nextDep)Ap(n)}const s=e.dep.subs;s!==e&&(e.prevSub=s,s&&(s.nextSub=e)),e.dep.subs=e}}const Fl=new WeakMap,ra=Symbol(""),yo=Symbol(""),$i=Symbol("");function Kt(e,t,s){if(Fs&&dt){let n=Fl.get(e);n||Fl.set(e,n=new Map);let a=n.get(s);a||(n.set(s,a=new yr),a.map=n,a.key=s),a.track()}}function hn(e,t,s,n,a,i){const l=Fl.get(e);if(!l){Fi++;return}const r=o=>{o&&o.trigger()};if(sc(),t==="clear")l.forEach(r);else{const o=Ee(e),c=o&&hr(s);if(o&&s==="length"){const d=Number(n);l.forEach((u,p)=>{(p==="length"||p===$i||!Yt(p)&&p>=d)&&r(u)})}else switch((s!==void 0||l.has(void 0))&&r(l.get(s)),c&&r(l.get($i)),t){case"add":o?c&&r(l.get("length")):(r(l.get(ra)),$a(e)&&r(l.get(yo)));break;case"delete":o||(r(l.get(ra)),$a(e)&&r(l.get(yo)));break;case"set":$a(e)&&r(l.get(ra));break}}nc()}function Lv(e,t){const s=Fl.get(e);return s&&s.get(t)}function Ta(e){const t=Ze(e);return t===e?t:(Kt(t,"iterate",$i),hs(e)?t:t.map(Bs))}function xr(e){return Kt(e=Ze(e),"iterate",$i),e}function Xs(e,t){return tn(e)?Ka(xn(e)?Bs(t):t):Bs(t)}const Nv={__proto__:null,[Symbol.iterator](){return Vr(this,Symbol.iterator,e=>Xs(this,e))},concat(...e){return Ta(this).concat(...e.map(t=>Ee(t)?Ta(t):t))},entries(){return Vr(this,"entries",e=>(e[1]=Xs(this,e[1]),e))},every(e,t){return ln(this,"every",e,t,void 0,arguments)},filter(e,t){return ln(this,"filter",e,t,s=>s.map(n=>Xs(this,n)),arguments)},find(e,t){return ln(this,"find",e,t,s=>Xs(this,s),arguments)},findIndex(e,t){return ln(this,"findIndex",e,t,void 0,arguments)},findLast(e,t){return ln(this,"findLast",e,t,s=>Xs(this,s),arguments)},findLastIndex(e,t){return ln(this,"findLastIndex",e,t,void 0,arguments)},forEach(e,t){return ln(this,"forEach",e,t,void 0,arguments)},includes(...e){return qr(this,"includes",e)},indexOf(...e){return qr(this,"indexOf",e)},join(e){return Ta(this).join(e)},lastIndexOf(...e){return qr(this,"lastIndexOf",e)},map(e,t){return ln(this,"map",e,t,void 0,arguments)},pop(){return ui(this,"pop")},push(...e){return ui(this,"push",e)},reduce(e,...t){return fd(this,"reduce",e,t)},reduceRight(e,...t){return fd(this,"reduceRight",e,t)},shift(){return ui(this,"shift")},some(e,t){return ln(this,"some",e,t,void 0,arguments)},splice(...e){return ui(this,"splice",e)},toReversed(){return Ta(this).toReversed()},toSorted(e){return Ta(this).toSorted(e)},toSpliced(...e){return Ta(this).toSpliced(...e)},unshift(...e){return ui(this,"unshift",e)},values(){return Vr(this,"values",e=>Xs(this,e))}};function Vr(e,t,s){const n=xr(e),a=n[t]();return n!==e&&!hs(e)&&(a._next=a.next,a.next=()=>{const i=a._next();return i.done||(i.value=s(i.value)),i}),a}const Mv=Array.prototype;function ln(e,t,s,n,a,i){const l=xr(e),r=l!==e&&!hs(e),o=l[t];if(o!==Mv[t]){const u=o.apply(e,i);return r?Bs(u):u}let c=s;l!==e&&(r?c=function(u,p){return s.call(this,Xs(e,u),p,e)}:s.length>2&&(c=function(u,p){return s.call(this,u,p,e)}));const d=o.call(l,c,n);return r&&a?a(d):d}function fd(e,t,s,n){const a=xr(e),i=a!==e&&!hs(e);let l=s,r=!1;a!==e&&(i?(r=n.length===0,l=function(c,d,u){return r&&(r=!1,c=Xs(e,c)),s.call(this,c,Xs(e,d),u,e)}):s.length>3&&(l=function(c,d,u){return s.call(this,c,d,u,e)}));const o=a[t](l,...n);return r?Xs(e,o):o}function qr(e,t,s){const n=Ze(e);Kt(n,"iterate",$i);const a=n[t](...s);return(a===-1||a===!1)&&sl(s[0])?(s[0]=Ze(s[0]),n[t](...s)):a}function ui(e,t,s=[]){Sn(),sc();const n=Ze(e)[t].apply(e,s);return nc(),Tn(),n}const Dv=xs("__proto__,__v_isRef,__isVue"),Rp=new Set(Object.getOwnPropertyNames(Symbol).filter(e=>e!=="arguments"&&e!=="caller").map(e=>Symbol[e]).filter(Yt));function Pv(e){Yt(e)||(e=String(e));const t=Ze(this);return Kt(t,"has",e),t.hasOwnProperty(e)}class Ip{constructor(t=!1,s=!1){this._isReadonly=t,this._isShallow=s}get(t,s,n){if(s==="__v_skip")return t.__v_skip;const a=this._isReadonly,i=this._isShallow;if(s==="__v_isReactive")return!a;if(s==="__v_isReadonly")return a;if(s==="__v_isShallow")return i;if(s==="__v_raw")return n===(a?i?Pp:Dp:i?Mp:Np).get(t)||Object.getPrototypeOf(t)===Object.getPrototypeOf(n)?t:void 0;const l=Ee(t);if(!a){let o;if(l&&(o=Nv[s]))return o;if(s==="hasOwnProperty")return Pv}const r=Reflect.get(t,s,Rt(t)?t:n);if((Yt(s)?Rp.has(s):Dv(s))||(a||Kt(t,"get",s),i))return r;if(Rt(r)){const o=l&&hr(s)?r:r.value;return a&&et(o)?$l(o):o}return et(r)?a?$l(r):jn(r):r}}class Op extends Ip{constructor(t=!1){super(!1,t)}set(t,s,n,a){let i=t[s];const l=Ee(t)&&hr(s);if(!this._isShallow){const c=tn(i);if(!hs(n)&&!tn(n)&&(i=Ze(i),n=Ze(n)),!l&&Rt(i)&&!Rt(n))return c||(i.value=n),!0}const r=l?Number(s)<t.length:tt(t,s),o=Reflect.set(t,s,n,Rt(t)?t:a);return t===Ze(a)&&(r?Dt(n,i)&&hn(t,"set",s,n):hn(t,"add",s,n)),o}deleteProperty(t,s){const n=tt(t,s);t[s];const a=Reflect.deleteProperty(t,s);return a&&n&&hn(t,"delete",s,void 0),a}has(t,s){const n=Reflect.has(t,s);return(!Yt(s)||!Rp.has(s))&&Kt(t,"has",s),n}ownKeys(t){return Kt(t,"iterate",Ee(t)?"length":ra),Reflect.ownKeys(t)}}class Lp extends Ip{constructor(t=!1){super(!0,t)}set(t,s){return!0}deleteProperty(t,s){return!0}}const Fv=new Op,$v=new Lp,Bv=new Op(!0),Uv=new Lp(!0),xo=e=>e,pl=e=>Reflect.getPrototypeOf(e);function Hv(e,t,s){return function(...n){const a=this.__v_raw,i=Ze(a),l=$a(i),r=e==="entries"||e===Symbol.iterator&&l,o=e==="keys"&&l,c=a[e](...n),d=s?xo:t?Ka:Bs;return!t&&Kt(i,"iterate",o?yo:ra),Ve(Object.create(c),{next(){const{value:u,done:p}=c.next();return p?{value:u,done:p}:{value:r?[d(u[0]),d(u[1])]:d(u),done:p}}})}}function fl(e){return function(...t){return e==="delete"?!1:e==="clear"?void 0:this}}function zv(e,t){const s={get(a){const i=this.__v_raw,l=Ze(i),r=Ze(a);e||(Dt(a,r)&&Kt(l,"get",a),Kt(l,"get",r));const{has:o}=pl(l),c=t?xo:e?Ka:Bs;if(o.call(l,a))return c(i.get(a));if(o.call(l,r))return c(i.get(r));i!==l&&i.get(a)},get size(){const a=this.__v_raw;return!e&&Kt(Ze(a),"iterate",ra),a.size},has(a){const i=this.__v_raw,l=Ze(i),r=Ze(a);return e||(Dt(a,r)&&Kt(l,"has",a),Kt(l,"has",r)),a===r?i.has(a):i.has(a)||i.has(r)},forEach(a,i){const l=this,r=l.__v_raw,o=Ze(r),c=t?xo:e?Ka:Bs;return!e&&Kt(o,"iterate",ra),r.forEach((d,u)=>a.call(i,c(d),c(u),l))}};return Ve(s,e?{add:fl("add"),set:fl("set"),delete:fl("delete"),clear:fl("clear")}:{add(a){const i=Ze(this),l=pl(i),r=Ze(a),o=!t&&!hs(a)&&!tn(a)?r:a;return l.has.call(i,o)||Dt(a,o)&&l.has.call(i,a)||Dt(r,o)&&l.has.call(i,r)||(i.add(o),hn(i,"add",o,o)),this},set(a,i){!t&&!hs(i)&&!tn(i)&&(i=Ze(i));const l=Ze(this),{has:r,get:o}=pl(l);let c=r.call(l,a);c||(a=Ze(a),c=r.call(l,a));const d=o.call(l,a);return l.set(a,i),c?Dt(i,d)&&hn(l,"set",a,i):hn(l,"add",a,i),this},delete(a){const i=Ze(this),{has:l,get:r}=pl(i);let o=l.call(i,a);o||(a=Ze(a),o=l.call(i,a)),r&&r.call(i,a);const c=i.delete(a);return o&&hn(i,"delete",a,void 0),c},clear(){const a=Ze(this),i=a.size!==0,l=a.clear();return i&&hn(a,"clear",void 0,void 0),l}}),["keys","values","entries",Symbol.iterator].forEach(a=>{s[a]=Hv(a,e,t)}),s}function _r(e,t){const s=zv(e,t);return(n,a,i)=>a==="__v_isReactive"?!e:a==="__v_isReadonly"?e:a==="__v_raw"?n:Reflect.get(tt(s,a)&&a in n?s:n,a,i)}const jv={get:_r(!1,!1)},Vv={get:_r(!1,!0)},qv={get:_r(!0,!1)},Gv={get:_r(!0,!0)},Np=new WeakMap,Mp=new WeakMap,Dp=new WeakMap,Pp=new WeakMap;function Kv(e){switch(e){case"Object":case"Array":return 1;case"Map":case"Set":case"WeakMap":case"WeakSet":return 2;default:return 0}}function jn(e){return tn(e)?e:wr(e,!1,Fv,jv,Np)}function ic(e){return wr(e,!1,Bv,Vv,Mp)}function $l(e){return wr(e,!0,$v,qv,Dp)}function Wv(e){return wr(e,!0,Uv,Gv,Pp)}function wr(e,t,s,n,a){if(!et(e)||e.__v_raw&&!(t&&e.__v_isReactive)||e.__v_skip||!Object.isExtensible(e))return e;const i=a.get(e);if(i)return i;const l=Kv(nv(e));if(l===0)return e;const r=new Proxy(e,l===2?n:s);return a.set(e,r),r}function xn(e){return tn(e)?xn(e.__v_raw):!!(e&&e.__v_isReactive)}function tn(e){return!!(e&&e.__v_isReadonly)}function hs(e){return!!(e&&e.__v_isShallow)}function sl(e){return e?!!e.__v_raw:!1}function Ze(e){const t=e&&e.__v_raw;return t?Ze(t):e}function Fp(e){return!tt(e,"__v_skip")&&Object.isExtensible(e)&&mp(e,"__v_skip",!0),e}const Bs=e=>et(e)?jn(e):e,Ka=e=>et(e)?$l(e):e;function Rt(e){return e?e.__v_isRef===!0:!1}function h(e){return $p(e,!1)}function lc(e){return $p(e,!0)}function $p(e,t){return Rt(e)?e:new Zv(e,t)}class Zv{constructor(t,s){this.dep=new yr,this.__v_isRef=!0,this.__v_isShallow=!1,this._rawValue=s?t:Ze(t),this._value=s?t:Bs(t),this.__v_isShallow=s}get value(){return this.dep.track(),this._value}set value(t){const s=this._rawValue,n=this.__v_isShallow||hs(t)||tn(t);t=n?t:Ze(t),Dt(t,s)&&(this._rawValue=t,this._value=n?t:Bs(t),this.dep.trigger())}}function Jv(e){e.dep&&e.dep.trigger()}function en(e){return Rt(e)?e.value:e}function Yv(e){return $e(e)?e():en(e)}const Qv={get:(e,t,s)=>t==="__v_raw"?e:en(Reflect.get(e,t,s)),set:(e,t,s,n)=>{const a=e[t];return Rt(a)&&!Rt(s)?(a.value=s,!0):Reflect.set(e,t,s,n)}};function rc(e){return xn(e)?e:new Proxy(e,Qv)}class Xv{constructor(t){this.__v_isRef=!0,this._value=void 0;const s=this.dep=new yr,{get:n,set:a}=t(s.track.bind(s),s.trigger.bind(s));this._get=n,this._set=a}get value(){return this._value=this._get()}set value(t){this._set(t)}}function Bp(e){return new Xv(e)}function eg(e){const t=Ee(e)?new Array(e.length):{};for(const s in e)t[s]=Up(e,s);return t}class tg{constructor(t,s,n){this._object=t,this._defaultValue=n,this.__v_isRef=!0,this._value=void 0,this._key=Yt(s)?s:String(s),this._raw=Ze(t);let a=!0,i=t;if(!Ee(t)||Yt(this._key)||!hr(this._key))do a=!sl(i)||hs(i);while(a&&(i=i.__v_raw));this._shallow=a}get value(){let t=this._object[this._key];return this._shallow&&(t=en(t)),this._value=t===void 0?this._defaultValue:t}set value(t){if(this._shallow&&Rt(this._raw[this._key])){const s=this._object[this._key];if(Rt(s)){s.value=t;return}}this._object[this._key]=t}get dep(){return Lv(this._raw,this._key)}}class sg{constructor(t){this._getter=t,this.__v_isRef=!0,this.__v_isReadonly=!0,this._value=void 0}get value(){return this._value=this._getter()}}function ng(e,t,s){return Rt(e)?e:$e(e)?new sg(e):et(e)&&arguments.length>1?Up(e,t,s):h(e)}function Up(e,t,s){return new tg(e,t,s)}class ag{constructor(t,s,n){this.fn=t,this.setter=s,this._value=void 0,this.dep=new yr(this),this.__v_isRef=!0,this.deps=void 0,this.depsTail=void 0,this.flags=16,this.globalVersion=Fi-1,this.next=void 0,this.effect=this,this.__v_isReadonly=!s,this.isSSR=n}notify(){if(this.flags|=16,!(this.flags&8)&&dt!==this)return kp(this,!0),!0}get value(){const t=this.dep.track();return Cp(this),t&&(t.version=this.dep.version),this._value}set value(t){this.setter&&this.setter(t)}}function ig(e,t,s=!1){let n,a;return $e(e)?n=e:(n=e.get,a=e.set),new ag(n,a,s)}const lg={GET:"get",HAS:"has",ITERATE:"iterate"},rg={SET:"set",ADD:"add",DELETE:"delete",CLEAR:"clear"},hl={},Bl=new WeakMap;let Pn;function og(){return Pn}function Hp(e,t=!1,s=Pn){if(s){let n=Bl.get(s);n||Bl.set(s,n=[]),n.push(e)}}function cg(e,t,s=qe){const{immediate:n,deep:a,once:i,scheduler:l,augmentJob:r,call:o}=s,c=_=>a?_:hs(_)||a===!1||a===0?mn(_,1):mn(_);let d,u,p,f,m=!1,b=!1;if(Rt(e)?(u=()=>e.value,m=hs(e)):xn(e)?(u=()=>c(e),m=!0):Ee(e)?(b=!0,m=e.some(_=>xn(_)||hs(_)),u=()=>e.map(_=>{if(Rt(_))return _.value;if(xn(_))return c(_);if($e(_))return o?o(_,2):_()})):$e(e)?t?u=o?()=>o(e,2):e:u=()=>{if(p){Sn();try{p()}finally{Tn()}}const _=Pn;Pn=d;try{return o?o(e,3,[f]):e(f)}finally{Pn=_}}:u=zt,t&&a){const _=u,C=a===!0?1/0:a;u=()=>mn(_(),C)}const E=_p(),I=()=>{d.stop(),E&&E.active&&Xo(E.effects,d)};if(i&&t){const _=t;t=(...C)=>{const v=_(...C);return I(),v}}let w=b?new Array(e.length).fill(hl):hl;const g=_=>{if(!(!(d.flags&1)||!d.dirty&&!_))if(t){const C=d.run();if(_||a||m||(b?C.some((v,S)=>Dt(v,w[S])):Dt(C,w))){p&&p();const v=Pn;Pn=d;try{const S=[C,w===hl?void 0:b&&w[0]===hl?[]:w,f];w=C,o?o(t,3,S):t(...S)}finally{Pn=v}}}else d.run()};return r&&r(g),d=new Pi(u),d.scheduler=l?()=>l(g,!1):g,f=_=>Hp(_,!1,d),p=d.onStop=()=>{const _=Bl.get(d);if(_){if(o)o(_,4);else for(const C of _)C();Bl.delete(d)}},t?n?g(!0):w=d.run():l?l(g.bind(null,!0),!0):d.run(),I.pause=d.pause.bind(d),I.resume=d.resume.bind(d),I.stop=I,I}function mn(e,t=1/0,s){if(t<=0||!et(e)||e.__v_skip||(s=s||new Map,(s.get(e)||0)>=t))return e;if(s.set(e,t),t--,Rt(e))mn(e.value,t,s);else if(Ee(e))for(let n=0;n<e.length;n++)mn(e[n],t,s);else if(ga(e)||$a(e))e.forEach(n=>{mn(n,t,s)});else if(fr(e)){for(const n in e)mn(e[n],t,s);for(const n of Object.getOwnPropertySymbols(e))Object.prototype.propertyIsEnumerable.call(e,n)&&mn(e[n],t,s)}return e}/**
* @vue/runtime-core v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const zp=[];function dg(e){zp.push(e)}function ug(){zp.pop()}function pg(e,t){}const fg={SETUP_FUNCTION:0,0:"SETUP_FUNCTION",RENDER_FUNCTION:1,1:"RENDER_FUNCTION",NATIVE_EVENT_HANDLER:5,5:"NATIVE_EVENT_HANDLER",COMPONENT_EVENT_HANDLER:6,6:"COMPONENT_EVENT_HANDLER",VNODE_HOOK:7,7:"VNODE_HOOK",DIRECTIVE_HOOK:8,8:"DIRECTIVE_HOOK",TRANSITION_HOOK:9,9:"TRANSITION_HOOK",APP_ERROR_HANDLER:10,10:"APP_ERROR_HANDLER",APP_WARN_HANDLER:11,11:"APP_WARN_HANDLER",FUNCTION_REF:12,12:"FUNCTION_REF",ASYNC_COMPONENT_LOADER:13,13:"ASYNC_COMPONENT_LOADER",SCHEDULER:14,14:"SCHEDULER",COMPONENT_UPDATE:15,15:"COMPONENT_UPDATE",APP_UNMOUNT_CLEANUP:16,16:"APP_UNMOUNT_CLEANUP"},hg={sp:"serverPrefetch hook",bc:"beforeCreate hook",c:"created hook",bm:"beforeMount hook",m:"mounted hook",bu:"beforeUpdate hook",u:"updated",bum:"beforeUnmount hook",um:"unmounted hook",a:"activated hook",da:"deactivated hook",ec:"errorCaptured hook",rtc:"renderTracked hook",rtg:"renderTriggered hook",0:"setup function",1:"render function",2:"watcher getter",3:"watcher callback",4:"watcher cleanup function",5:"native event handler",6:"component event handler",7:"vnode hook",8:"directive hook",9:"transition hook",10:"app errorHandler",11:"app warnHandler",12:"ref function",13:"async component loader",14:"scheduler flush",15:"component update",16:"app unmount cleanup function"};function li(e,t,s,n){try{return n?e(...n):e()}catch(a){ya(a,t,s)}}function ys(e,t,s,n){if($e(e)){const a=li(e,t,s,n);return a&&ec(a)&&a.catch(i=>{ya(i,t,s)}),a}if(Ee(e)){const a=[];for(let i=0;i<e.length;i++)a.push(ys(e[i],t,s,n));return a}}function ya(e,t,s,n=!0){const a=t?t.vnode:null,{errorHandler:i,throwUnhandledErrorInProduction:l}=t&&t.appContext.config||qe;if(t){let r=t.parent;const o=t.proxy,c=`https://vuejs.org/error-reference/#runtime-${s}`;for(;r;){const d=r.ec;if(d){for(let u=0;u<d.length;u++)if(d[u](e,o,c)===!1)return}r=r.parent}if(i){Sn(),li(i,null,10,[e,o,c]),Tn();return}}mg(e,s,a,n,l)}function mg(e,t,s,n=!0,a=!1){if(a)throw e;console.error(e)}const ns=[];let Ys=-1;const Ha=[];let Fn=null,Oa=0;const jp=Promise.resolve();let Ul=null;function Et(e){const t=Ul||jp;return e?t.then(this?e.bind(this):e):t}function vg(e){let t=Ys+1,s=ns.length;for(;t<s;){const n=t+s>>>1,a=ns[n],i=Ui(a);i<e||i===e&&a.flags&2?t=n+1:s=n}return t}function oc(e){if(!(e.flags&1)){const t=Ui(e),s=ns[ns.length-1];!s||!(e.flags&2)&&t>=Ui(s)?ns.push(e):ns.splice(vg(t),0,e),e.flags|=1,Vp()}}function Vp(){Ul||(Ul=jp.then(qp))}function Bi(e){Ee(e)?Ha.push(...e):Fn&&e.id===-1?Fn.splice(Oa+1,0,e):e.flags&1||(Ha.push(e),e.flags|=1),Vp()}function hd(e,t,s=Ys+1){for(;s<ns.length;s++){const n=ns[s];if(n&&n.flags&2){if(e&&n.id!==e.uid)continue;ns.splice(s,1),s--,n.flags&4&&(n.flags&=-2),n(),n.flags&4||(n.flags&=-2)}}}function Hl(e){if(Ha.length){const t=[...new Set(Ha)].sort((s,n)=>Ui(s)-Ui(n));if(Ha.length=0,Fn){Fn.push(...t);return}for(Fn=t,Oa=0;Oa<Fn.length;Oa++){const s=Fn[Oa];s.flags&4&&(s.flags&=-2),s.flags&8||s(),s.flags&=-2}Fn=null,Oa=0}}const Ui=e=>e.id==null?e.flags&2?-1:1/0:e.id;function qp(e){try{for(Ys=0;Ys<ns.length;Ys++){const t=ns[Ys];t&&!(t.flags&8)&&(t.flags&4&&(t.flags&=-2),li(t,t.i,t.i?15:14),t.flags&4||(t.flags&=-2))}}finally{for(;Ys<ns.length;Ys++){const t=ns[Ys];t&&(t.flags&=-2)}Ys=-1,ns.length=0,Hl(),Ul=null,(ns.length||Ha.length)&&qp()}}let La,ml=[];function Gp(e,t){var s,n;La=e,La?(La.enabled=!0,ml.forEach(({event:a,args:i})=>La.emit(a,...i)),ml=[]):typeof window<"u"&&window.HTMLElement&&!((n=(s=window.navigator)==null?void 0:s.userAgent)!=null&&n.includes("jsdom"))?((t.__VUE_DEVTOOLS_HOOK_REPLAY__=t.__VUE_DEVTOOLS_HOOK_REPLAY__||[]).push(i=>{Gp(i,t)}),setTimeout(()=>{La||(t.__VUE_DEVTOOLS_HOOK_REPLAY__=null,ml=[])},3e3)):ml=[]}let Ht=null,kr=null;function Hi(e){const t=Ht;return Ht=e,kr=e&&e.type.__scopeId||null,t}function gg(e){kr=e}function bg(){kr=null}const yg=e=>cc;function cc(e,t=Ht,s){if(!t||e._n)return e;const n=(...a)=>{n._d&&qi(-1);const i=Hi(t);let l;try{l=e(...a)}finally{Hi(i),n._d&&qi(1)}return l};return n._n=!0,n._c=!0,n._d=!0,n}function xg(e,t){if(Ht===null)return e;const s=ll(Ht),n=e.dirs||(e.dirs=[]);for(let a=0;a<t.length;a++){let[i,l,r,o=qe]=t[a];i&&($e(i)&&(i={mounted:i,updated:i}),i.deep&&mn(l),n.push({dir:i,instance:s,value:l,oldValue:void 0,arg:r,modifiers:o}))}return e}function Qs(e,t,s,n){const a=e.dirs,i=t&&t.dirs;for(let l=0;l<a.length;l++){const r=a[l];i&&(r.oldValue=i[l].value);let o=r.dir[n];o&&(Sn(),ys(o,s,8,[e.el,r,e,t]),Tn())}}function Ei(e,t){if(Ut){let s=Ut.provides;const n=Ut.parent&&Ut.parent.provides;n===s&&(s=Ut.provides=Object.create(n)),s[e]=t}}function Rs(e,t,s=!1){const n=ls();if(n||oa){let a=oa?oa._context.provides:n?n.parent==null||n.ce?n.vnode.appContext&&n.vnode.appContext.provides:n.parent.provides:void 0;if(a&&e in a)return a[e];if(arguments.length>1)return s&&$e(t)?t.call(n&&n.proxy):t}}function _g(){return!!(ls()||oa)}const Kp=Symbol.for("v-scx"),Wp=()=>Rs(Kp);function wg(e,t){return nl(e,null,t)}function kg(e,t){return nl(e,null,{flush:"post"})}function Zp(e,t){return nl(e,null,{flush:"sync"})}function is(e,t,s){return nl(e,t,s)}function nl(e,t,s=qe){const{immediate:n,deep:a,flush:i,once:l}=s,r=Ve({},s),o=t&&n||!t&&i!=="post";let c;if(fa){if(i==="sync"){const f=Wp();c=f.__watcherHandles||(f.__watcherHandles=[])}else if(!o){const f=()=>{};return f.stop=zt,f.resume=zt,f.pause=zt,f}}const d=Ut;r.call=(f,m,b)=>ys(f,d,m,b);let u=!1;i==="post"?r.scheduler=f=>{Ct(f,d&&d.suspense)}:i!=="sync"&&(u=!0,r.scheduler=(f,m)=>{m?f():oc(f)}),r.augmentJob=f=>{t&&(f.flags|=4),u&&(f.flags|=2,d&&(f.id=d.uid,f.i=d))};const p=cg(e,t,r);return fa&&(c?c.push(p):o&&p()),p}function Sg(e,t,s){const n=this.proxy,a=Be(e)?e.includes(".")?Jp(n,e):()=>n[e]:e.bind(n,n);let i;$e(t)?i=t:(i=t.handler,s=t);const l=ri(this),r=nl(a,i.bind(n),s);return l(),r}function Jp(e,t){const s=t.split(".");return()=>{let n=e;for(let a=0;a<s.length&&n;a++)n=n[s[a]];return n}}const Mn=new WeakMap,Yp=Symbol("_vte"),Qp=e=>e.__isTeleport,na=e=>e&&(e.disabled||e.disabled===""),Tg=e=>e&&(e.defer||e.defer===""),md=e=>typeof SVGElement<"u"&&e instanceof SVGElement,vd=e=>typeof MathMLElement=="function"&&e instanceof MathMLElement,_o=(e,t)=>{const s=e&&e.to;return Be(s)?t?t(s):null:s},Cg={name:"Teleport",__isTeleport:!0,process(e,t,s,n,a,i,l,r,o,c){const{mc:d,pc:u,pbc:p,o:{insert:f,querySelector:m,createText:b,createComment:E,parentNode:I}}=c,w=na(t.props);let{dynamicChildren:g}=t;const _=(S,T,y)=>{S.shapeFlag&16&&d(S.children,T,y,a,i,l,r,o)},C=(S=t)=>{const T=na(S.props),y=S.target=_o(S.props,m),O=wo(y,S,b,f);y&&(l!=="svg"&&md(y)?l="svg":l!=="mathml"&&vd(y)&&(l="mathml"),a&&a.isCE&&(a.ce._teleportTargets||(a.ce._teleportTargets=new Set)).add(y),T||(_(S,y,O),xi(S,!1)))},v=S=>{const T=()=>{if(Mn.get(S)===T){if(Mn.delete(S),na(S.props)){const y=I(S.el)||s;_(S,y,S.anchor),xi(S,!0)}C(S)}};Mn.set(S,T),Ct(T,i)};if(e==null){const S=t.el=b(""),T=t.anchor=b("");if(f(S,s,n),f(T,s,n),Tg(t.props)||i&&i.pendingBranch){v(t);return}w&&(_(t,s,T),xi(t,!0)),C()}else{t.el=e.el;const S=t.anchor=e.anchor,T=Mn.get(e);if(T){T.flags|=8,Mn.delete(e),v(t);return}t.targetStart=e.targetStart;const y=t.target=e.target,O=t.targetAnchor=e.targetAnchor,$=na(e.props),F=$?s:y,N=$?S:O;if(l==="svg"||md(y)?l="svg":(l==="mathml"||vd(y))&&(l="mathml"),g?(p(e.dynamicChildren,g,F,a,i,l,r),xc(e,t,!0)):o||u(e,t,F,N,a,i,l,r,!1),w)$?t.props&&e.props&&t.props.to!==e.props.to&&(t.props.to=e.props.to):vl(t,s,S,c,1);else if((t.props&&t.props.to)!==(e.props&&e.props.to)){const G=t.target=_o(t.props,m);G&&vl(t,G,null,c,0)}else $&&vl(t,y,O,c,1);xi(t,w)}},remove(e,t,s,{um:n,o:{remove:a}},i){const{shapeFlag:l,children:r,anchor:o,targetStart:c,targetAnchor:d,target:u,props:p}=e,f=i||!na(p),m=Mn.get(e);if(m&&(m.flags|=8,Mn.delete(e)),u&&(a(c),a(d)),i&&a(o),!m&&l&16)for(let b=0;b<r.length;b++){const E=r[b];n(E,t,s,f,!!E.dynamicChildren)}},move:vl,hydrate:Eg};function vl(e,t,s,{o:{insert:n},m:a},i=2){i===0&&n(e.targetAnchor,t,s);const{el:l,anchor:r,shapeFlag:o,children:c,props:d}=e,u=i===2;if(u&&n(l,t,s),!Mn.has(e)&&(!u||na(d))&&o&16)for(let p=0;p<c.length;p++)a(c[p],t,s,2);u&&n(r,t,s)}function Eg(e,t,s,n,a,i,{o:{nextSibling:l,parentNode:r,querySelector:o,insert:c,createText:d}},u){function p(E,I){let w=I;for(;w;){if(w&&w.nodeType===8){if(w.data==="teleport start anchor")t.targetStart=w;else if(w.data==="teleport anchor"){t.targetAnchor=w,E._lpa=t.targetAnchor&&l(t.targetAnchor);break}}w=l(w)}}function f(E,I){I.anchor=u(l(E),I,r(E),s,n,a,i)}const m=t.target=_o(t.props,o),b=na(t.props);if(m){const E=m._lpa||m.firstChild;t.shapeFlag&16&&(b?(f(e,t),p(m,E),t.targetAnchor||wo(m,t,d,c,r(e)===m?e:null)):(t.anchor=l(e),p(m,E),t.targetAnchor||wo(m,t,d,c),u(E&&l(E),t,m,s,n,a,i))),xi(t,b)}else b&&t.shapeFlag&16&&(f(e,t),t.targetStart=e,t.targetAnchor=l(e));return t.anchor&&l(t.anchor)}const Ag=Cg;function xi(e,t){const s=e.ctx;if(s&&s.ut){let n,a;for(t?(n=e.el,a=e.anchor):(n=e.targetStart,a=e.targetAnchor);n&&n!==a;)n.nodeType===1&&n.setAttribute("data-v-owner",s.uid),n=n.nextSibling;s.ut()}}function wo(e,t,s,n,a=null){const i=t.targetStart=s(""),l=t.targetAnchor=s("");return i[Yp]=l,e&&(n(i,e,a),n(l,e,a)),l}const Ts=Symbol("_leaveCb"),pi=Symbol("_enterCb");function dc(){const e={isMounted:!1,isLeaving:!1,isUnmounting:!1,leavingVNodes:new Map};return Ke(()=>{e.isMounted=!0}),Er(()=>{e.isUnmounting=!0}),e}const Ss=[Function,Array],uc={mode:String,appear:Boolean,persisted:Boolean,onBeforeEnter:Ss,onEnter:Ss,onAfterEnter:Ss,onEnterCancelled:Ss,onBeforeLeave:Ss,onLeave:Ss,onAfterLeave:Ss,onLeaveCancelled:Ss,onBeforeAppear:Ss,onAppear:Ss,onAfterAppear:Ss,onAppearCancelled:Ss},Xp=e=>{const t=e.subTree;return t.component?Xp(t.component):t},Rg={name:"BaseTransition",props:uc,setup(e,{slots:t}){const s=ls(),n=dc();return()=>{const a=t.default&&Sr(t.default(),!0),i=a&&a.length?ef(a):s.subTree?Ff():void 0;if(!i)return;const l=Ze(e),{mode:r}=l;if(n.isLeaving)return Gr(i);const o=gd(i);if(!o)return Gr(i);let c=Wa(o,l,n,s,u=>c=u);o.type!==St&&Cn(o,c);let d=s.subTree&&gd(s.subTree);if(d&&d.type!==St&&!Ps(d,o)&&Xp(s).type!==St){let u=Wa(d,l,n,s);if(Cn(d,u),r==="out-in"&&o.type!==St)return n.isLeaving=!0,u.afterLeave=()=>{n.isLeaving=!1,s.job.flags&8||s.update(),delete u.afterLeave,d=void 0},Gr(i);r==="in-out"&&o.type!==St?u.delayLeave=(p,f,m)=>{const b=sf(n,d);b[String(d.key)]=d,p[Ts]=()=>{f(),p[Ts]=void 0,delete c.delayedLeave,d=void 0},c.delayedLeave=()=>{m(),delete c.delayedLeave,d=void 0}}:d=void 0}else d&&(d=void 0);return i}}};function ef(e){let t=e[0];if(e.length>1){for(const s of e)if(s.type!==St){t=s;break}}return t}const tf=Rg;function sf(e,t){const{leavingVNodes:s}=e;let n=s.get(t.type);return n||(n=Object.create(null),s.set(t.type,n)),n}function Wa(e,t,s,n,a){const{appear:i,mode:l,persisted:r=!1,onBeforeEnter:o,onEnter:c,onAfterEnter:d,onEnterCancelled:u,onBeforeLeave:p,onLeave:f,onAfterLeave:m,onLeaveCancelled:b,onBeforeAppear:E,onAppear:I,onAfterAppear:w,onAppearCancelled:g}=t,_=String(e.key),C=sf(s,e),v=(y,O)=>{y&&ys(y,n,9,O)},S=(y,O)=>{const $=O[1];v(y,O),Ee(y)?y.every(F=>F.length<=1)&&$():y.length<=1&&$()},T={mode:l,persisted:r,beforeEnter(y){let O=o;if(!s.isMounted)if(i)O=E||o;else return;y[Ts]&&y[Ts](!0);const $=C[_];$&&Ps(e,$)&&$.el[Ts]&&$.el[Ts](),v(O,[y])},enter(y){if(C[_]===e)return;let O=c,$=d,F=u;if(!s.isMounted)if(i)O=I||c,$=w||d,F=g||u;else return;let N=!1;y[pi]=z=>{N||(N=!0,z?v(F,[y]):v($,[y]),T.delayedLeave&&T.delayedLeave(),y[pi]=void 0)};const G=y[pi].bind(null,!1);O?S(O,[y,G]):G()},leave(y,O){const $=String(e.key);if(y[pi]&&y[pi](!0),s.isUnmounting)return O();v(p,[y]);let F=!1;y[Ts]=G=>{F||(F=!0,O(),G?v(b,[y]):v(m,[y]),y[Ts]=void 0,C[$]===e&&delete C[$])};const N=y[Ts].bind(null,!1);C[$]=e,f?S(f,[y,N]):N()},clone(y){const O=Wa(y,t,s,n,a);return a&&a(O),O}};return T}function Gr(e){if(il(e))return e=sn(e),e.children=null,e}function gd(e){if(!il(e))return Qp(e.type)&&e.children?ef(e.children):e;if(e.component)return e.component.subTree;const{shapeFlag:t,children:s}=e;if(s){if(t&16)return s[0];if(t&32&&$e(s.default))return s.default()}}function Cn(e,t){e.shapeFlag&6&&e.component?(e.transition=t,Cn(e.component.subTree,t)):e.shapeFlag&128?(e.ssContent.transition=t.clone(e.ssContent),e.ssFallback.transition=t.clone(e.ssFallback)):e.transition=t}function Sr(e,t=!1,s){let n=[],a=0;for(let i=0;i<e.length;i++){let l=e[i];const r=s==null?l.key:String(s)+String(l.key!=null?l.key:i);l.type===Pt?(l.patchFlag&128&&a++,n=n.concat(Sr(l.children,t,r))):(t||l.type!==St)&&n.push(r!=null?sn(l,{key:r}):l)}if(a>1)for(let i=0;i<n.length;i++)n[i].patchFlag=-2;return n}function al(e,t){return $e(e)?Ve({name:e.name},t,{setup:e}):e}function Ig(){const e=ls();return e?(e.appContext.config.idPrefix||"v")+"-"+e.ids[0]+e.ids[1]++:""}function pc(e){e.ids=[e.ids[0]+e.ids[2]+++"-",0,0]}function Og(e){const t=ls(),s=lc(null);if(t){const a=t.refs===qe?t.refs={}:t.refs;Object.defineProperty(a,e,{enumerable:!0,get:()=>s.value,set:i=>s.value=i})}return s}function bd(e,t){let s;return!!((s=Object.getOwnPropertyDescriptor(e,t))&&!s.configurable)}const zl=new WeakMap;function za(e,t,s,n,a=!1){if(Ee(e)){e.forEach((b,E)=>za(b,t&&(Ee(t)?t[E]:t),s,n,a));return}if(_n(n)&&!a){n.shapeFlag&512&&n.type.__asyncResolved&&n.component.subTree.component&&za(e,t,s,n.component.subTree);return}const i=n.shapeFlag&4?ll(n.component):n.el,l=a?null:i,{i:r,r:o}=e,c=t&&t.r,d=r.refs===qe?r.refs={}:r.refs,u=r.setupState,p=Ze(u),f=u===qe?Da:b=>bd(d,b)?!1:tt(p,b),m=(b,E)=>!(E&&bd(d,E));if(c!=null&&c!==o){if(yd(t),Be(c))d[c]=null,f(c)&&(u[c]=null);else if(Rt(c)){const b=t;m(c,b.k)&&(c.value=null),b.k&&(d[b.k]=null)}}if($e(o))li(o,r,12,[l,d]);else{const b=Be(o),E=Rt(o);if(b||E){const I=()=>{if(e.f){const w=b?f(o)?u[o]:d[o]:m()||!e.k?o.value:d[e.k];if(a)Ee(w)&&Xo(w,i);else if(Ee(w))w.includes(i)||w.push(i);else if(b)d[o]=[i],f(o)&&(u[o]=d[o]);else{const g=[i];m(o,e.k)&&(o.value=g),e.k&&(d[e.k]=g)}}else b?(d[o]=l,f(o)&&(u[o]=l)):E&&(m(o,e.k)&&(o.value=l),e.k&&(d[e.k]=l))};if(l){const w=()=>{I(),zl.delete(e)};w.id=-1,zl.set(e,w),Ct(w,s)}else yd(e),I()}}}function yd(e){const t=zl.get(e);t&&(t.flags|=8,zl.delete(e))}let xd=!1;const Ca=()=>{xd||(console.error("Hydration completed but contains mismatches."),xd=!0)},Lg=e=>e.namespaceURI.includes("svg")&&e.tagName!=="foreignObject",Ng=e=>e.namespaceURI.includes("MathML"),gl=e=>{if(e.nodeType===1){if(Lg(e))return"svg";if(Ng(e))return"mathml"}},Pa=e=>e.nodeType===8;function Mg(e){const{mt:t,p:s,o:{patchProp:n,createText:a,nextSibling:i,parentNode:l,remove:r,insert:o,createComment:c}}=e,d=(g,_)=>{if(!_.hasChildNodes()){s(null,g,_),Hl(),_._vnode=g;return}u(_.firstChild,g,null,null,null),Hl(),_._vnode=g},u=(g,_,C,v,S,T=!1)=>{T=T||!!_.dynamicChildren;const y=Pa(g)&&g.data==="[",O=()=>b(g,_,C,v,S,y),{type:$,ref:F,shapeFlag:N,patchFlag:G}=_;let z=g.nodeType;_.el=g,G===-2&&(T=!1,_.dynamicChildren=null);let P=null;switch($){case Un:z!==3?_.children===""?(o(_.el=a(""),l(g),g),P=g):P=O():(g.data!==_.children&&(Ca(),g.data=_.children),P=i(g));break;case St:w(g)?(P=i(g),I(_.el=g.content.firstChild,g,C)):z!==8||y?P=O():P=i(g);break;case ca:if(y&&(g=i(g),z=g.nodeType),z===1||z===3){P=g;const D=!_.children.length;for(let R=0;R<_.staticCount;R++)D&&(_.children+=P.nodeType===1?P.outerHTML:P.data),R===_.staticCount-1&&(_.anchor=P),P=i(P);return y?i(P):P}else O();break;case Pt:y?P=m(g,_,C,v,S,T):P=O();break;default:if(N&1)(z!==1||_.type.toLowerCase()!==g.tagName.toLowerCase())&&!w(g)?P=O():P=p(g,_,C,v,S,T);else if(N&6){_.slotScopeIds=S;const D=l(g);if(y?P=E(g):Pa(g)&&g.data==="teleport start"?P=E(g,g.data,"teleport end"):P=i(g),t(_,D,null,C,v,gl(D),T),_n(_)&&!_.type.__asyncResolved){let R;y?(R=mt(Pt),R.anchor=P?P.previousSibling:D.lastChild):R=g.nodeType===3?wc(""):mt("div"),R.el=g,_.component.subTree=R}}else N&64?z!==8?P=O():P=_.type.hydrate(g,_,C,v,S,T,e,f):N&128&&(P=_.type.hydrate(g,_,C,v,gl(l(g)),S,T,e,u))}return F!=null&&za(F,null,v,_),P},p=(g,_,C,v,S,T)=>{T=T||!!_.dynamicChildren;const{type:y,props:O,patchFlag:$,shapeFlag:F,dirs:N,transition:G}=_,z=y==="input"||y==="option";if(z||$!==-1){N&&Qs(_,null,C,"created");let P=!1;if(w(g)){P=Af(null,G)&&C&&C.vnode.props&&C.vnode.props.appear;const R=g.content.firstChild;if(P){const ee=R.getAttribute("class");ee&&(R.$cls=ee),G.beforeEnter(R)}I(R,g,C),_.el=g=R}if(F&16&&!(O&&(O.innerHTML||O.textContent))){let R=f(g.firstChild,_,g,C,v,S,T);for(R&&!bl(g,1)&&Ca();R;){const ee=R;R=R.nextSibling,r(ee)}}else if(F&8){let R=_.children;R[0]===`
`&&(g.tagName==="PRE"||g.tagName==="TEXTAREA")&&(R=R.slice(1));const{textContent:ee}=g;ee!==R&&ee!==R.replace(/\r\n|\r/g,`
`)&&(bl(g,0)||Ca(),g.textContent=_.children)}if(O){if(z||!T||$&48){const R=g.tagName.includes("-");for(const ee in O)(z&&(ee.endsWith("value")||ee==="indeterminate")||va(ee)&&!yn(ee)||ee[0]==="."||R&&!yn(ee))&&n(g,ee,null,O[ee],void 0,C)}else if(O.onClick)n(g,"onClick",null,O.onClick,void 0,C);else if($&4&&xn(O.style))for(const R in O.style)O.style[R]}let D;(D=O&&O.onVnodeBeforeMount)&&cs(D,C,_),N&&Qs(_,null,C,"beforeMount"),((D=O&&O.onVnodeMounted)||N||P)&&Lf(()=>{D&&cs(D,C,_),P&&G.enter(g),N&&Qs(_,null,C,"mounted")},v)}return g.nextSibling},f=(g,_,C,v,S,T,y)=>{y=y||!!_.dynamicChildren;const O=_.children,$=O.length;let F=!1;for(let N=0;N<$;N++){const G=y?O[N]:O[N]=us(O[N]),z=G.type===Un;g?(z&&!y&&N+1<$&&us(O[N+1]).type===Un&&(o(a(g.data.slice(G.children.length)),C,i(g)),g.data=G.children),g=u(g,G,v,S,T,y)):z&&!G.children?o(G.el=a(""),C):(F||(F=!0,bl(C,1)||Ca()),s(null,G,C,null,v,S,gl(C),T))}return g},m=(g,_,C,v,S,T)=>{const{slotScopeIds:y}=_;y&&(S=S?S.concat(y):y);const O=l(g),$=f(i(g),_,O,C,v,S,T);return $&&Pa($)&&$.data==="]"?i(_.anchor=$):(Ca(),o(_.anchor=c("]"),O,$),$)},b=(g,_,C,v,S,T)=>{if(bl(g.parentElement,1)||Ca(),_.el=null,T){const $=E(g);for(;;){const F=i(g);if(F&&F!==$)r(F);else break}}const y=i(g),O=l(g);return r(g),s(null,_,O,y,C,v,gl(O),S),C&&(C.vnode.el=_.el,Rr(C,_.el)),y},E=(g,_="[",C="]")=>{let v=0;for(;g;)if(g=i(g),g&&Pa(g)&&(g.data===_&&v++,g.data===C)){if(v===0)return i(g);v--}return g},I=(g,_,C)=>{const v=_.parentNode;v&&v.replaceChild(g,_);let S=C;for(;S;)S.vnode.el===_&&(S.vnode.el=S.subTree.el=g),S=S.parent},w=g=>g.nodeType===1&&g.tagName==="TEMPLATE";return[d,u]}const _d="data-allow-mismatch",Dg={0:"text",1:"children",2:"class",3:"style",4:"attribute"};function bl(e,t){if(t===0||t===1)for(;e&&!e.hasAttribute(_d);)e=e.parentElement;const s=e&&e.getAttribute(_d);if(s==null)return!1;if(s==="")return!0;{const n=s.split(",");return t===0&&n.includes("children")?!0:n.includes(Dg[t])}}const Pg=gr().requestIdleCallback||(e=>setTimeout(e,1)),Fg=gr().cancelIdleCallback||(e=>clearTimeout(e)),$g=(e=1e4)=>t=>{const s=Pg(t,{timeout:e});return()=>Fg(s)};function Bg(e){const{top:t,left:s,bottom:n,right:a}=e.getBoundingClientRect(),{innerHeight:i,innerWidth:l}=window;return(t>0&&t<i||n>0&&n<i)&&(s>0&&s<l||a>0&&a<l)}const Ug=e=>(t,s)=>{const n=new IntersectionObserver(a=>{for(const i of a)if(i.isIntersecting){n.disconnect(),t();break}},e);return s(a=>{if(a instanceof Element){if(Bg(a))return t(),n.disconnect(),!1;n.observe(a)}}),()=>n.disconnect()},Hg=e=>t=>{if(e){const s=matchMedia(e);if(s.matches)t();else return s.addEventListener("change",t,{once:!0}),()=>s.removeEventListener("change",t)}},zg=(e=[])=>(t,s)=>{Be(e)&&(e=[e]);let n=!1;const a=l=>{n||(n=!0,i(),t(),l.target.dispatchEvent(new l.constructor(l.type,l)))},i=()=>{s(l=>{for(const r of e)l.removeEventListener(r,a)})};return s(l=>{for(const r of e)l.addEventListener(r,a,{once:!0})}),i};function jg(e,t){if(Pa(e)&&e.data==="["){let s=1,n=e.nextSibling;for(;n;){if(n.nodeType===1){if(t(n)===!1)break}else if(Pa(n))if(n.data==="]"){if(--s===0)break}else n.data==="["&&s++;n=n.nextSibling}}else t(e)}const _n=e=>!!e.type.__asyncLoader;function Vg(e){$e(e)&&(e={loader:e});const{loader:t,loadingComponent:s,errorComponent:n,delay:a=200,hydrate:i,timeout:l,suspensible:r=!0,onError:o}=e;let c=null,d,u=0;const p=()=>(u++,c=null,f()),f=()=>{let m;return c||(m=c=t().catch(b=>{if(b=b instanceof Error?b:new Error(String(b)),o)return new Promise((E,I)=>{o(b,()=>E(p()),()=>I(b),u+1)});throw b}).then(b=>m!==c&&c?c:(b&&(b.__esModule||b[Symbol.toStringTag]==="Module")&&(b=b.default),d=b,b)))};return al({name:"AsyncComponentWrapper",__asyncLoader:f,__asyncHydrate(m,b,E){let I=!1;(b.bu||(b.bu=[])).push(()=>I=!0);const w=()=>{I||E()},g=i?()=>{const _=i(w,C=>jg(m,C));_&&(b.bum||(b.bum=[])).push(_)}:w;d?g():f().then(()=>!b.isUnmounted&&g())},get __asyncResolved(){return d},setup(){const m=Ut;if(pc(m),d)return()=>yl(d,m);const b=C=>{c=null,ya(C,m,13,!n)};if(r&&m.suspense||fa)return f().then(C=>()=>yl(C,m)).catch(C=>(b(C),()=>n?mt(n,{error:C}):null));const E=h(!1),I=h(),w=h(!!a);let g,_;return _t(()=>{g!=null&&clearTimeout(g),_!=null&&clearTimeout(_)}),a&&(_=setTimeout(()=>{m.isUnmounted||(w.value=!1)},a)),l!=null&&(g=setTimeout(()=>{if(!m.isUnmounted&&!E.value&&!I.value){const C=new Error(`Async component timed out after ${l}ms.`);b(C),I.value=C}},l)),f().then(()=>{m.isUnmounted||(E.value=!0,m.parent&&il(m.parent.vnode)&&m.parent.update())}).catch(C=>{if(m.isUnmounted){c=null;return}b(C),I.value=C}),()=>{if(E.value&&d)return yl(d,m);if(I.value&&n)return mt(n,{error:I.value});if(s&&!w.value)return yl(s,m)}}})}function yl(e,t){const{ref:s,props:n,children:a,ce:i}=t.vnode,l=mt(e,n,a);return l.ref=s,l.ce=i,delete t.vnode.ce,l}const il=e=>e.type.__isKeepAlive,qg={name:"KeepAlive",__isKeepAlive:!0,props:{include:[String,RegExp,Array],exclude:[String,RegExp,Array],max:[String,Number]},setup(e,{slots:t}){const s=ls(),n=s.ctx;if(!n.renderer)return()=>{const w=t.default&&t.default();return w&&w.length===1?w[0]:w};const a=new Map,i=new Set;let l=null;const r=s.suspense,{renderer:{p:o,m:c,um:d,o:{createElement:u}}}=n,p=u("div");n.activate=(w,g,_,C,v)=>{const S=w.component;c(w,g,_,0,r),o(S.vnode,w,g,_,S,r,C,w.slotScopeIds,v),Ct(()=>{S.isDeactivated=!1,S.a&&Ua(S.a);const T=w.props&&w.props.onVnodeMounted;T&&cs(T,S.parent,w)},r)},n.deactivate=w=>{const g=w.component;Vl(g.m),Vl(g.a),c(w,p,null,1,r),Ct(()=>{g.da&&Ua(g.da);const _=w.props&&w.props.onVnodeUnmounted;_&&cs(_,g.parent,w),g.isDeactivated=!0},r)};function f(w){Kr(w),d(w,s,r,!0)}function m(w){a.forEach((g,_)=>{const C=Oo(_n(g)?g.type.__asyncResolved||{}:g.type);C&&!w(C)&&b(_)})}function b(w){const g=a.get(w);g&&(!l||!Ps(g,l))?f(g):l&&Kr(l),a.delete(w),i.delete(w)}is(()=>[e.include,e.exclude],([w,g])=>{w&&m(_=>_i(w,_)),g&&m(_=>!_i(g,_))},{flush:"post",deep:!0});let E=null;const I=()=>{E!=null&&(ql(s.subTree.type)?Ct(()=>{a.set(E,xl(s.subTree))},s.subTree.suspense):a.set(E,xl(s.subTree)))};return Ke(I),Cr(I),Er(()=>{a.forEach(w=>{const{subTree:g,suspense:_}=s,C=xl(g);if(w.type===C.type&&w.key===C.key){Kr(C);const v=C.component.da;v&&Ct(v,_);return}f(w)})}),()=>{if(E=null,!t.default)return l=null;const w=t.default(),g=w[0];if(w.length>1)return l=null,w;if(!En(g)||!(g.shapeFlag&4)&&!(g.shapeFlag&128))return l=null,g;let _=xl(g);if(_.type===St)return l=null,_;const C=_.type,v=Oo(_n(_)?_.type.__asyncResolved||{}:C),{include:S,exclude:T,max:y}=e;if(S&&(!v||!_i(S,v))||T&&v&&_i(T,v))return _.shapeFlag&=-257,l=_,g;const O=_.key==null?C:_.key,$=a.get(O);return _.el&&(_=sn(_),g.shapeFlag&128&&(g.ssContent=_)),E=O,$?(_.el=$.el,_.component=$.component,_.transition&&Cn(_,_.transition),_.shapeFlag|=512,i.delete(O),i.add(O)):(i.add(O),y&&i.size>parseInt(y,10)&&b(i.values().next().value)),_.shapeFlag|=256,l=_,ql(g.type)?g:_}}},Gg=qg;function _i(e,t){return Ee(e)?e.some(s=>_i(s,t)):Be(e)?e.split(",").includes(t):sv(e)?(e.lastIndex=0,e.test(t)):!1}function _s(e,t){nf(e,"a",t)}function ws(e,t){nf(e,"da",t)}function nf(e,t,s=Ut){const n=e.__wdc||(e.__wdc=()=>{let a=s;for(;a;){if(a.isDeactivated)return;a=a.parent}return e()});if(Tr(t,n,s),s){let a=s.parent;for(;a&&a.parent;)il(a.parent.vnode)&&Kg(n,t,s,a),a=a.parent}}function Kg(e,t,s,n){const a=Tr(t,e,n,!0);_t(()=>{Xo(n[t],a)},s)}function Kr(e){e.shapeFlag&=-257,e.shapeFlag&=-513}function xl(e){return e.shapeFlag&128?e.ssContent:e}function Tr(e,t,s=Ut,n=!1){if(s){const a=s[e]||(s[e]=[]),i=t.__weh||(t.__weh=(...l)=>{Sn();const r=ri(s),o=ys(t,s,e,l);return r(),Tn(),o});return n?a.unshift(i):a.push(i),i}}const An=e=>(t,s=Ut)=>{(!fa||e==="sp")&&Tr(e,(...n)=>t(...n),s)},af=An("bm"),Ke=An("m"),fc=An("bu"),Cr=An("u"),Er=An("bum"),_t=An("um"),lf=An("sp"),rf=An("rtg"),of=An("rtc");function cf(e,t=Ut){Tr("ec",e,t)}const hc="components",Wg="directives";function Zg(e,t){return mc(hc,e,!0,t)||e}const df=Symbol.for("v-ndc");function Jg(e){return Be(e)?mc(hc,e,!1)||e:e||df}function Yg(e){return mc(Wg,e)}function mc(e,t,s=!0,n=!1){const a=Ht||Ut;if(a){const i=a.type;if(e===hc){const r=Oo(i,!1);if(r&&(r===t||r===rt(t)||r===ba(rt(t))))return i}const l=wd(a[e]||i[e],t)||wd(a.appContext[e],t);return!l&&n?i:l}}function wd(e,t){return e&&(e[t]||e[rt(t)]||e[ba(rt(t))])}function Qg(e,t,s,n){let a;const i=s&&s[n],l=Ee(e);if(l||Be(e)){const r=l&&xn(e);let o=!1,c=!1;r&&(o=!hs(e),c=tn(e),e=xr(e)),a=new Array(e.length);for(let d=0,u=e.length;d<u;d++)a[d]=t(o?c?Ka(Bs(e[d])):Bs(e[d]):e[d],d,void 0,i&&i[d])}else if(typeof e=="number"){a=new Array(e);for(let r=0;r<e;r++)a[r]=t(r+1,r,void 0,i&&i[r])}else if(et(e))if(e[Symbol.iterator])a=Array.from(e,(r,o)=>t(r,o,void 0,i&&i[o]));else{const r=Object.keys(e);a=new Array(r.length);for(let o=0,c=r.length;o<c;o++){const d=r[o];a[o]=t(e[d],d,o,i&&i[o])}}else a=[];return s&&(s[n]=a),a}function Xg(e,t){for(let s=0;s<t.length;s++){const n=t[s];if(Ee(n))for(let a=0;a<n.length;a++)e[n[a].name]=n[a].fn;else n&&(e[n.name]=n.key?(...a)=>{const i=n.fn(...a);return i&&(i.key=n.key),i}:n.fn)}return e}function eb(e,t,s={},n,a){if(Ht.ce||Ht.parent&&_n(Ht.parent)&&Ht.parent.ce){const c=Object.keys(s).length>0;return t!=="default"&&(s.name=t),Vi(),Gl(Pt,null,[mt("slot",s,n&&n())],c?-2:64)}let i=e[t];i&&i._c&&(i._d=!1),Vi();const l=i&&vc(i(s)),r=s.key||l&&l.key,o=Gl(Pt,{key:(r&&!Yt(r)?r:`_${t}`)+(!l&&n?"_fb":"")},l||(n?n():[]),l&&e._===1?64:-2);return!a&&o.scopeId&&(o.slotScopeIds=[o.scopeId+"-s"]),i&&i._c&&(i._d=!0),o}function vc(e){return e.some(t=>En(t)?!(t.type===St||t.type===Pt&&!vc(t.children)):!0)?e:null}function tb(e,t){const s={};for(const n in e)s[t&&/[A-Z]/.test(n)?`on:${n}`:Ba(n)]=e[n];return s}const ko=e=>e?Uf(e)?ll(e):ko(e.parent):null,Ai=Ve(Object.create(null),{$:e=>e,$el:e=>e.vnode.el,$data:e=>e.data,$props:e=>e.props,$attrs:e=>e.attrs,$slots:e=>e.slots,$refs:e=>e.refs,$parent:e=>ko(e.parent),$root:e=>ko(e.root),$host:e=>e.ce,$emit:e=>e.emit,$options:e=>gc(e),$forceUpdate:e=>e.f||(e.f=()=>{oc(e.update)}),$nextTick:e=>e.n||(e.n=Et.bind(e.proxy)),$watch:e=>Sg.bind(e)}),Wr=(e,t)=>e!==qe&&!e.__isScriptSetup&&tt(e,t),So={get({_:e},t){if(t==="__v_skip")return!0;const{ctx:s,setupState:n,data:a,props:i,accessCache:l,type:r,appContext:o}=e;if(t[0]!=="$"){const p=l[t];if(p!==void 0)switch(p){case 1:return n[t];case 2:return a[t];case 4:return s[t];case 3:return i[t]}else{if(Wr(n,t))return l[t]=1,n[t];if(a!==qe&&tt(a,t))return l[t]=2,a[t];if(tt(i,t))return l[t]=3,i[t];if(s!==qe&&tt(s,t))return l[t]=4,s[t];To&&(l[t]=0)}}const c=Ai[t];let d,u;if(c)return t==="$attrs"&&Kt(e.attrs,"get",""),c(e);if((d=r.__cssModules)&&(d=d[t]))return d;if(s!==qe&&tt(s,t))return l[t]=4,s[t];if(u=o.config.globalProperties,tt(u,t))return u[t]},set({_:e},t,s){const{data:n,setupState:a,ctx:i}=e;return Wr(a,t)?(a[t]=s,!0):n!==qe&&tt(n,t)?(n[t]=s,!0):tt(e.props,t)||t[0]==="$"&&t.slice(1)in e?!1:(i[t]=s,!0)},has({_:{data:e,setupState:t,accessCache:s,ctx:n,appContext:a,props:i,type:l}},r){let o;return!!(s[r]||e!==qe&&r[0]!=="$"&&tt(e,r)||Wr(t,r)||tt(i,r)||tt(n,r)||tt(Ai,r)||tt(a.config.globalProperties,r)||(o=l.__cssModules)&&o[r])},defineProperty(e,t,s){return s.get!=null?e._.accessCache[t]=0:tt(s,"value")&&this.set(e,t,s.value,null),Reflect.defineProperty(e,t,s)}},sb=Ve({},So,{get(e,t){if(t!==Symbol.unscopables)return So.get(e,t,e)},has(e,t){return t[0]!=="_"&&!cv(t)}});function nb(){return null}function ab(){return null}function ib(e){}function lb(e){}function rb(){return null}function ob(){}function cb(e,t){return null}function db(){return uf().slots}function ub(){return uf().attrs}function uf(e){const t=ls();return t.setupContext||(t.setupContext=Vf(t))}function zi(e){return Ee(e)?e.reduce((t,s)=>(t[s]=null,t),{}):e}function pb(e,t){const s=zi(e);for(const n in t){if(n.startsWith("__skip"))continue;let a=s[n];a?Ee(a)||$e(a)?a=s[n]={type:a,default:t[n]}:a.default=t[n]:a===null&&(a=s[n]={default:t[n]}),a&&t[`__skip_${n}`]&&(a.skipFactory=!0)}return s}function fb(e,t){return!e||!t?e||t:Ee(e)&&Ee(t)?e.concat(t):Ve({},zi(e),zi(t))}function hb(e,t){const s={};for(const n in e)t.includes(n)||Object.defineProperty(s,n,{enumerable:!0,get:()=>e[n]});return s}function mb(e){const t=ls(),s=fa;let n=e();Gi(),s&&Va(!1);const a=()=>{ri(t),s&&Va(!0)},i=()=>{ls()!==t&&t.scope.off(),Gi(),s&&Va(!1)};return ec(n)&&(n=n.catch(l=>{throw a(),Promise.resolve().then(()=>Promise.resolve().then(i)),l})),[n,()=>{a(),Promise.resolve().then(i)}]}let To=!0;function vb(e){const t=gc(e),s=e.proxy,n=e.ctx;To=!1,t.beforeCreate&&kd(t.beforeCreate,e,"bc");const{data:a,computed:i,methods:l,watch:r,provide:o,inject:c,created:d,beforeMount:u,mounted:p,beforeUpdate:f,updated:m,activated:b,deactivated:E,beforeDestroy:I,beforeUnmount:w,destroyed:g,unmounted:_,render:C,renderTracked:v,renderTriggered:S,errorCaptured:T,serverPrefetch:y,expose:O,inheritAttrs:$,components:F,directives:N,filters:G}=t;if(c&&gb(c,n,null),l)for(const D in l){const R=l[D];$e(R)&&(n[D]=R.bind(s))}if(a){const D=a.call(s,s);et(D)&&(e.data=jn(D))}if(To=!0,i)for(const D in i){const R=i[D],ee=$e(R)?R.bind(s,s):$e(R.get)?R.get.bind(s,s):zt,ne=!$e(R)&&$e(R.set)?R.set.bind(s):zt,ge=W({get:ee,set:ne});Object.defineProperty(n,D,{enumerable:!0,configurable:!0,get:()=>ge.value,set:ie=>ge.value=ie})}if(r)for(const D in r)pf(r[D],n,s,D);if(o){const D=$e(o)?o.call(s):o;Reflect.ownKeys(D).forEach(R=>{Ei(R,D[R])})}d&&kd(d,e,"c");function P(D,R){Ee(R)?R.forEach(ee=>D(ee.bind(s))):R&&D(R.bind(s))}if(P(af,u),P(Ke,p),P(fc,f),P(Cr,m),P(_s,b),P(ws,E),P(cf,T),P(of,v),P(rf,S),P(Er,w),P(_t,_),P(lf,y),Ee(O))if(O.length){const D=e.exposed||(e.exposed={});O.forEach(R=>{Object.defineProperty(D,R,{get:()=>s[R],set:ee=>s[R]=ee,enumerable:!0})})}else e.exposed||(e.exposed={});C&&e.render===zt&&(e.render=C),$!=null&&(e.inheritAttrs=$),F&&(e.components=F),N&&(e.directives=N),y&&pc(e)}function gb(e,t,s=zt){Ee(e)&&(e=Co(e));for(const n in e){const a=e[n];let i;et(a)?"default"in a?i=Rs(a.from||n,a.default,!0):i=Rs(a.from||n):i=Rs(a),Rt(i)?Object.defineProperty(t,n,{enumerable:!0,configurable:!0,get:()=>i.value,set:l=>i.value=l}):t[n]=i}}function kd(e,t,s){ys(Ee(e)?e.map(n=>n.bind(t.proxy)):e.bind(t.proxy),t,s)}function pf(e,t,s,n){let a=n.includes(".")?Jp(s,n):()=>s[n];if(Be(e)){const i=t[e];$e(i)&&is(a,i)}else if($e(e))is(a,e.bind(s));else if(et(e))if(Ee(e))e.forEach(i=>pf(i,t,s,n));else{const i=$e(e.handler)?e.handler.bind(s):t[e.handler];$e(i)&&is(a,i,e)}}function gc(e){const t=e.type,{mixins:s,extends:n}=t,{mixins:a,optionsCache:i,config:{optionMergeStrategies:l}}=e.appContext,r=i.get(t);let o;return r?o=r:!a.length&&!s&&!n?o=t:(o={},a.length&&a.forEach(c=>jl(o,c,l,!0)),jl(o,t,l)),et(t)&&i.set(t,o),o}function jl(e,t,s,n=!1){const{mixins:a,extends:i}=t;i&&jl(e,i,s,!0),a&&a.forEach(l=>jl(e,l,s,!0));for(const l in t)if(!(n&&l==="expose")){const r=bb[l]||s&&s[l];e[l]=r?r(e[l],t[l]):t[l]}return e}const bb={data:Sd,props:Td,emits:Td,methods:wi,computed:wi,beforeCreate:es,created:es,beforeMount:es,mounted:es,beforeUpdate:es,updated:es,beforeDestroy:es,beforeUnmount:es,destroyed:es,unmounted:es,activated:es,deactivated:es,errorCaptured:es,serverPrefetch:es,components:wi,directives:wi,watch:xb,provide:Sd,inject:yb};function Sd(e,t){return t?e?function(){return Ve($e(e)?e.call(this,this):e,$e(t)?t.call(this,this):t)}:t:e}function yb(e,t){return wi(Co(e),Co(t))}function Co(e){if(Ee(e)){const t={};for(let s=0;s<e.length;s++)t[e[s]]=e[s];return t}return e}function es(e,t){return e?[...new Set([].concat(e,t))]:t}function wi(e,t){return e?Ve(Object.create(null),e,t):t}function Td(e,t){return e?Ee(e)&&Ee(t)?[...new Set([...e,...t])]:Ve(Object.create(null),zi(e),zi(t??{})):t}function xb(e,t){if(!e)return t;if(!t)return e;const s=Ve(Object.create(null),e);for(const n in t)s[n]=es(e[n],t[n]);return s}function ff(){return{app:null,config:{isNativeTag:Da,performance:!1,globalProperties:{},optionMergeStrategies:{},errorHandler:void 0,warnHandler:void 0,compilerOptions:{}},mixins:[],components:{},directives:{},provides:Object.create(null),optionsCache:new WeakMap,propsCache:new WeakMap,emitsCache:new WeakMap}}let _b=0;function wb(e,t){return function(n,a=null){$e(n)||(n=Ve({},n)),a!=null&&!et(a)&&(a=null);const i=ff(),l=new WeakSet,r=[];let o=!1;const c=i.app={_uid:_b++,_component:n,_props:a,_container:null,_context:i,_instance:null,version:Gf,get config(){return i.config},set config(d){},use(d,...u){return l.has(d)||(d&&$e(d.install)?(l.add(d),d.install(c,...u)):$e(d)&&(l.add(d),d(c,...u))),c},mixin(d){return i.mixins.includes(d)||i.mixins.push(d),c},component(d,u){return u?(i.components[d]=u,c):i.components[d]},directive(d,u){return u?(i.directives[d]=u,c):i.directives[d]},mount(d,u,p){if(!o){const f=c._ceVNode||mt(n,a);return f.appContext=i,p===!0?p="svg":p===!1&&(p=void 0),u&&t?t(f,d):e(f,d,p),o=!0,c._container=d,d.__vue_app__=c,ll(f.component)}},onUnmount(d){r.push(d)},unmount(){o&&(ys(r,c._instance,16),e(null,c._container),delete c._container.__vue_app__)},provide(d,u){return i.provides[d]=u,c},runWithContext(d){const u=oa;oa=c;try{return d()}finally{oa=u}}};return c}}let oa=null;function kb(e,t,s=qe){const n=ls(),a=rt(t),i=ps(t),l=hf(e,a),r=Bp((o,c)=>{let d,u=qe,p;return Zp(()=>{const f=e[a];Dt(d,f)&&(d=f,c())}),{get(){return o(),s.get?s.get(d):d},set(f){const m=s.set?s.set(f):f;if(!Dt(m,d)&&!(u!==qe&&Dt(f,u)))return;const b=n.vnode.props,E=!!(b&&(t in b||a in b||i in b)&&(`onUpdate:${t}`in b||`onUpdate:${a}`in b||`onUpdate:${i}`in b));E||(d=f,c()),n.emit(`update:${t}`,m),Dt(f,u)&&(Dt(f,m)&&!Dt(m,p)||E&&u!==qe&&!Dt(m,d))&&c(),u=f,p=m}}});return r[Symbol.iterator]=()=>{let o=0;return{next(){return o<2?{value:o++?l||qe:r,done:!1}:{done:!0}}}},r}const hf=(e,t)=>t==="modelValue"||t==="model-value"?e.modelModifiers:e[`${t}Modifiers`]||e[`${rt(t)}Modifiers`]||e[`${ps(t)}Modifiers`];function Sb(e,t,...s){if(e.isUnmounted)return;const n=e.vnode.props||qe;let a=s;const i=t.startsWith("update:"),l=i&&hf(n,t.slice(7));l&&(l.trim&&(a=s.map(d=>Be(d)?d.trim():d)),l.number&&(a=s.map(vr)));let r,o=n[r=Ba(t)]||n[r=Ba(rt(t))];!o&&i&&(o=n[r=Ba(ps(t))]),o&&ys(o,e,6,a);const c=n[r+"Once"];if(c){if(!e.emitted)e.emitted={};else if(e.emitted[r])return;e.emitted[r]=!0,ys(c,e,6,a)}}const Tb=new WeakMap;function mf(e,t,s=!1){const n=s?Tb:t.emitsCache,a=n.get(e);if(a!==void 0)return a;const i=e.emits;let l={},r=!1;if(!$e(e)){const o=c=>{const d=mf(c,t,!0);d&&(r=!0,Ve(l,d))};!s&&t.mixins.length&&t.mixins.forEach(o),e.extends&&o(e.extends),e.mixins&&e.mixins.forEach(o)}return!i&&!r?(et(e)&&n.set(e,null),null):(Ee(i)?i.forEach(o=>l[o]=null):Ve(l,i),et(e)&&n.set(e,l),l)}function Ar(e,t){return!e||!va(t)?!1:(t=t.slice(2).replace(/Once$/,""),tt(e,t[0].toLowerCase()+t.slice(1))||tt(e,ps(t))||tt(e,t))}function Rl(e){const{type:t,vnode:s,proxy:n,withProxy:a,propsOptions:[i],slots:l,attrs:r,emit:o,render:c,renderCache:d,props:u,data:p,setupState:f,ctx:m,inheritAttrs:b}=e,E=Hi(e);let I,w;try{if(s.shapeFlag&4){const _=a||n,C=_;I=us(c.call(C,_,d,u,f,p,m)),w=r}else{const _=t;I=us(_.length>1?_(u,{attrs:r,slots:l,emit:o}):_(u,null)),w=t.props?r:Eb(r)}}catch(_){Ri.length=0,ya(_,e,1),I=mt(St)}let g=I;if(w&&b!==!1){const _=Object.keys(w),{shapeFlag:C}=g;_.length&&C&7&&(i&&_.some(pr)&&(w=Ab(w,i)),g=sn(g,w,!1,!0))}return s.dirs&&(g=sn(g,null,!1,!0),g.dirs=g.dirs?g.dirs.concat(s.dirs):s.dirs),s.transition&&Cn(g,s.transition),I=g,Hi(E),I}function Cb(e,t=!0){let s;for(let n=0;n<e.length;n++){const a=e[n];if(En(a)){if(a.type!==St||a.children==="v-if"){if(s)return;s=a}}else return}return s}const Eb=e=>{let t;for(const s in e)(s==="class"||s==="style"||va(s))&&((t||(t={}))[s]=e[s]);return t},Ab=(e,t)=>{const s={};for(const n in e)(!pr(n)||!(n.slice(9)in t))&&(s[n]=e[n]);return s};function Rb(e,t,s){const{props:n,children:a,component:i}=e,{props:l,children:r,patchFlag:o}=t,c=i.emitsOptions;if(t.dirs||t.transition)return!0;if(s&&o>=0){if(o&1024)return!0;if(o&16)return n?Cd(n,l,c):!!l;if(o&8){const d=t.dynamicProps;for(let u=0;u<d.length;u++){const p=d[u];if(vf(l,n,p)&&!Ar(c,p))return!0}}}else return(a||r)&&(!r||!r.$stable)?!0:n===l?!1:n?l?Cd(n,l,c):!0:!!l;return!1}function Cd(e,t,s){const n=Object.keys(t);if(n.length!==Object.keys(e).length)return!0;for(let a=0;a<n.length;a++){const i=n[a];if(vf(t,e,i)&&!Ar(s,i))return!0}return!1}function vf(e,t,s){const n=e[s],a=t[s];return s==="style"&&et(n)&&et(a)?!kn(n,a):n!==a}function Rr({vnode:e,parent:t,suspense:s},n){for(;t;){const a=t.subTree;if(a.suspense&&a.suspense.activeBranch===e&&(a.suspense.vnode.el=a.el=n,e=a),a===e)(e=t.vnode).el=n,t=t.parent;else break}s&&s.activeBranch===e&&(s.vnode.el=n)}const gf={},bf=()=>Object.create(gf),yf=e=>Object.getPrototypeOf(e)===gf;function Ib(e,t,s,n=!1){const a={},i=bf();e.propsDefaults=Object.create(null),xf(e,t,a,i);for(const l in e.propsOptions[0])l in a||(a[l]=void 0);s?e.props=n?a:ic(a):e.type.props?e.props=a:e.props=i,e.attrs=i}function Ob(e,t,s,n){const{props:a,attrs:i,vnode:{patchFlag:l}}=e,r=Ze(a),[o]=e.propsOptions;let c=!1;if((n||l>0)&&!(l&16)){if(l&8){const d=e.vnode.dynamicProps;for(let u=0;u<d.length;u++){let p=d[u];if(Ar(e.emitsOptions,p))continue;const f=t[p];if(o)if(tt(i,p))f!==i[p]&&(i[p]=f,c=!0);else{const m=rt(p);a[m]=Eo(o,r,m,f,e,!1)}else f!==i[p]&&(i[p]=f,c=!0)}}}else{xf(e,t,a,i)&&(c=!0);let d;for(const u in r)(!t||!tt(t,u)&&((d=ps(u))===u||!tt(t,d)))&&(o?s&&(s[u]!==void 0||s[d]!==void 0)&&(a[u]=Eo(o,r,u,void 0,e,!0)):delete a[u]);if(i!==r)for(const u in i)(!t||!tt(t,u))&&(delete i[u],c=!0)}c&&hn(e.attrs,"set","")}function xf(e,t,s,n){const[a,i]=e.propsOptions;let l=!1,r;if(t)for(let o in t){if(yn(o))continue;const c=t[o];let d;a&&tt(a,d=rt(o))?!i||!i.includes(d)?s[d]=c:(r||(r={}))[d]=c:Ar(e.emitsOptions,o)||(!(o in n)||c!==n[o])&&(n[o]=c,l=!0)}if(i){const o=Ze(s),c=r||qe;for(let d=0;d<i.length;d++){const u=i[d];s[u]=Eo(a,o,u,c[u],e,!tt(c,u))}}return l}function Eo(e,t,s,n,a,i){const l=e[s];if(l!=null){const r=tt(l,"default");if(r&&n===void 0){const o=l.default;if(l.type!==Function&&!l.skipFactory&&$e(o)){const{propsDefaults:c}=a;if(s in c)n=c[s];else{const d=ri(a);n=c[s]=o.call(null,t),d()}}else n=o;a.ce&&a.ce._setProp(s,n)}l[0]&&(i&&!r?n=!1:l[1]&&(n===""||n===ps(s))&&(n=!0))}return n}const Lb=new WeakMap;function _f(e,t,s=!1){const n=s?Lb:t.propsCache,a=n.get(e);if(a)return a;const i=e.props,l={},r=[];let o=!1;if(!$e(e)){const d=u=>{o=!0;const[p,f]=_f(u,t,!0);Ve(l,p),f&&r.push(...f)};!s&&t.mixins.length&&t.mixins.forEach(d),e.extends&&d(e.extends),e.mixins&&e.mixins.forEach(d)}if(!i&&!o)return et(e)&&n.set(e,Fa),Fa;if(Ee(i))for(let d=0;d<i.length;d++){const u=rt(i[d]);Ed(u)&&(l[u]=qe)}else if(i)for(const d in i){const u=rt(d);if(Ed(u)){const p=i[d],f=l[u]=Ee(p)||$e(p)?{type:p}:Ve({},p),m=f.type;let b=!1,E=!0;if(Ee(m))for(let I=0;I<m.length;++I){const w=m[I],g=$e(w)&&w.name;if(g==="Boolean"){b=!0;break}else g==="String"&&(E=!1)}else b=$e(m)&&m.name==="Boolean";f[0]=b,f[1]=E,(b||tt(f,"default"))&&r.push(u)}}const c=[l,r];return et(e)&&n.set(e,c),c}function Ed(e){return e[0]!=="$"&&!yn(e)}const bc=e=>e==="_"||e==="_ctx"||e==="$stable",yc=e=>Ee(e)?e.map(us):[us(e)],Nb=(e,t,s)=>{if(t._n)return t;const n=cc((...a)=>yc(t(...a)),s);return n._c=!1,n},wf=(e,t,s)=>{const n=e._ctx;for(const a in e){if(bc(a))continue;const i=e[a];if($e(i))t[a]=Nb(a,i,n);else if(i!=null){const l=yc(i);t[a]=()=>l}}},kf=(e,t)=>{const s=yc(t);e.slots.default=()=>s},Sf=(e,t,s)=>{for(const n in t)(s||!bc(n))&&(e[n]=t[n])},Mb=(e,t,s)=>{const n=e.slots=bf();if(e.vnode.shapeFlag&32){const a=t._;a?(Sf(n,t,s),s&&mp(n,"_",a,!0)):wf(t,n)}else t&&kf(e,t)},Db=(e,t,s)=>{const{vnode:n,slots:a}=e;let i=!0,l=qe;if(n.shapeFlag&32){const r=t._;r?s&&r===1?i=!1:Sf(a,t,s):(i=!t.$stable,wf(t,a)),l=t}else t&&(kf(e,t),l={default:1});if(i)for(const r in a)!bc(r)&&l[r]==null&&delete a[r]},Ct=Lf;function Tf(e){return Ef(e)}function Cf(e){return Ef(e,Mg)}function Ef(e,t){const s=gr();s.__VUE__=!0;const{insert:n,remove:a,patchProp:i,createElement:l,createText:r,createComment:o,setText:c,setElementText:d,parentNode:u,nextSibling:p,setScopeId:f=zt,insertStaticContent:m}=e,b=(k,L,B,se=null,Y=null,X=null,me=void 0,ce=null,de=!!L.dynamicChildren)=>{if(k===L)return;k&&!Ps(k,L)&&(se=U(k),ie(k,Y,X,!0),k=null),L.patchFlag===-2&&(de=!1,L.dynamicChildren=null);const{type:ae,ref:we,shapeFlag:ve}=L;switch(ae){case Un:E(k,L,B,se);break;case St:I(k,L,B,se);break;case ca:k==null&&w(L,B,se,me);break;case Pt:F(k,L,B,se,Y,X,me,ce,de);break;default:ve&1?C(k,L,B,se,Y,X,me,ce,de):ve&6?N(k,L,B,se,Y,X,me,ce,de):(ve&64||ve&128)&&ae.process(k,L,B,se,Y,X,me,ce,de,Se)}we!=null&&Y?za(we,k&&k.ref,X,L||k,!L):we==null&&k&&k.ref!=null&&za(k.ref,null,X,k,!0)},E=(k,L,B,se)=>{if(k==null)n(L.el=r(L.children),B,se);else{const Y=L.el=k.el;L.children!==k.children&&c(Y,L.children)}},I=(k,L,B,se)=>{k==null?n(L.el=o(L.children||""),B,se):L.el=k.el},w=(k,L,B,se)=>{[k.el,k.anchor]=m(k.children,L,B,se,k.el,k.anchor)},g=({el:k,anchor:L},B,se)=>{let Y;for(;k&&k!==L;)Y=p(k),n(k,B,se),k=Y;n(L,B,se)},_=({el:k,anchor:L})=>{let B;for(;k&&k!==L;)B=p(k),a(k),k=B;a(L)},C=(k,L,B,se,Y,X,me,ce,de)=>{if(L.type==="svg"?me="svg":L.type==="math"&&(me="mathml"),k==null)v(L,B,se,Y,X,me,ce,de);else{const ae=k.el&&k.el._isVueCE?k.el:null;try{ae&&ae._beginPatch(),y(k,L,Y,X,me,ce,de)}finally{ae&&ae._endPatch()}}},v=(k,L,B,se,Y,X,me,ce)=>{let de,ae;const{props:we,shapeFlag:ve,transition:xe,dirs:oe}=k;if(de=k.el=l(k.type,X,we&&we.is,we),ve&8?d(de,k.children):ve&16&&T(k.children,de,null,se,Y,Zr(k,X),me,ce),oe&&Qs(k,null,se,"created"),S(de,k,k.scopeId,me,se),we){for(const pe in we)pe!=="value"&&!yn(pe)&&i(de,pe,null,we[pe],X,se);"value"in we&&i(de,"value",null,we.value,X),(ae=we.onVnodeBeforeMount)&&cs(ae,se,k)}oe&&Qs(k,null,se,"beforeMount");const H=Af(Y,xe);H&&xe.beforeEnter(de),n(de,L,B),((ae=we&&we.onVnodeMounted)||H||oe)&&Ct(()=>{try{ae&&cs(ae,se,k),H&&xe.enter(de),oe&&Qs(k,null,se,"mounted")}finally{}},Y)},S=(k,L,B,se,Y)=>{if(B&&f(k,B),se)for(let X=0;X<se.length;X++)f(k,se[X]);if(Y){let X=Y.subTree;if(L===X||ql(X.type)&&(X.ssContent===L||X.ssFallback===L)){const me=Y.vnode;S(k,me,me.scopeId,me.slotScopeIds,Y.parent)}}},T=(k,L,B,se,Y,X,me,ce,de=0)=>{for(let ae=de;ae<k.length;ae++){const we=k[ae]=ce?pn(k[ae]):us(k[ae]);b(null,we,L,B,se,Y,X,me,ce)}},y=(k,L,B,se,Y,X,me)=>{const ce=L.el=k.el;let{patchFlag:de,dynamicChildren:ae,dirs:we}=L;de|=k.patchFlag&16;const ve=k.props||qe,xe=L.props||qe;let oe;if(B&&Qn(B,!1),(oe=xe.onVnodeBeforeUpdate)&&cs(oe,B,L,k),we&&Qs(L,k,B,"beforeUpdate"),B&&Qn(B,!0),(ve.innerHTML&&xe.innerHTML==null||ve.textContent&&xe.textContent==null)&&d(ce,""),ae?O(k.dynamicChildren,ae,ce,B,se,Zr(L,Y),X):me||R(k,L,ce,null,B,se,Zr(L,Y),X,!1),de>0){if(de&16)$(ce,ve,xe,B,Y);else if(de&2&&ve.class!==xe.class&&i(ce,"class",null,xe.class,Y),de&4&&i(ce,"style",ve.style,xe.style,Y),de&8){const H=L.dynamicProps;for(let pe=0;pe<H.length;pe++){const Te=H[pe],Ae=ve[Te],De=xe[Te];(De!==Ae||Te==="value")&&i(ce,Te,Ae,De,Y,B)}}de&1&&k.children!==L.children&&d(ce,L.children)}else!me&&ae==null&&$(ce,ve,xe,B,Y);((oe=xe.onVnodeUpdated)||we)&&Ct(()=>{oe&&cs(oe,B,L,k),we&&Qs(L,k,B,"updated")},se)},O=(k,L,B,se,Y,X,me)=>{for(let ce=0;ce<L.length;ce++){const de=k[ce],ae=L[ce],we=de.el&&(de.type===Pt||!Ps(de,ae)||de.shapeFlag&198)?u(de.el):B;b(de,ae,we,null,se,Y,X,me,!0)}},$=(k,L,B,se,Y)=>{if(L!==B){if(L!==qe)for(const X in L)!yn(X)&&!(X in B)&&i(k,X,L[X],null,Y,se);for(const X in B){if(yn(X))continue;const me=B[X],ce=L[X];me!==ce&&X!=="value"&&i(k,X,ce,me,Y,se)}"value"in B&&i(k,"value",L.value,B.value,Y)}},F=(k,L,B,se,Y,X,me,ce,de)=>{const ae=L.el=k?k.el:r(""),we=L.anchor=k?k.anchor:r("");let{patchFlag:ve,dynamicChildren:xe,slotScopeIds:oe}=L;oe&&(ce=ce?ce.concat(oe):oe),k==null?(n(ae,B,se),n(we,B,se),T(L.children||[],B,we,Y,X,me,ce,de)):ve>0&&ve&64&&xe&&k.dynamicChildren&&k.dynamicChildren.length===xe.length?(O(k.dynamicChildren,xe,B,Y,X,me,ce),(L.key!=null||Y&&L===Y.subTree)&&xc(k,L,!0)):R(k,L,B,we,Y,X,me,ce,de)},N=(k,L,B,se,Y,X,me,ce,de)=>{L.slotScopeIds=ce,k==null?L.shapeFlag&512?Y.ctx.activate(L,B,se,me,de):G(L,B,se,Y,X,me,de):z(k,L,de)},G=(k,L,B,se,Y,X,me)=>{const ce=k.component=Bf(k,se,Y);if(il(k)&&(ce.ctx.renderer=Se),Hf(ce,!1,me),ce.asyncDep){if(Y&&Y.registerDep(ce,P,me),!k.el){const de=ce.subTree=mt(St);I(null,de,L,B),k.placeholder=de.el}}else P(ce,k,L,B,Y,X,me)},z=(k,L,B)=>{const se=L.component=k.component;if(Rb(k,L,B))if(se.asyncDep&&!se.asyncResolved){D(se,L,B);return}else se.next=L,se.update();else L.el=k.el,se.vnode=L},P=(k,L,B,se,Y,X,me)=>{const ce=()=>{if(k.isMounted){let{next:ve,bu:xe,u:oe,parent:H,vnode:pe}=k;{const it=Rf(k);if(it){ve&&(ve.el=pe.el,D(k,ve,me)),it.asyncDep.then(()=>{Ct(()=>{k.isUnmounted||ae()},Y)});return}}let Te=ve,Ae;Qn(k,!1),ve?(ve.el=pe.el,D(k,ve,me)):ve=pe,xe&&Ua(xe),(Ae=ve.props&&ve.props.onVnodeBeforeUpdate)&&cs(Ae,H,ve,pe),Qn(k,!0);const De=Rl(k),ut=k.subTree;k.subTree=De,b(ut,De,u(ut.el),U(ut),k,Y,X),ve.el=De.el,Te===null&&Rr(k,De.el),oe&&Ct(oe,Y),(Ae=ve.props&&ve.props.onVnodeUpdated)&&Ct(()=>cs(Ae,H,ve,pe),Y)}else{let ve;const{el:xe,props:oe}=L,{bm:H,m:pe,parent:Te,root:Ae,type:De}=k,ut=_n(L);if(Qn(k,!1),H&&Ua(H),!ut&&(ve=oe&&oe.onVnodeBeforeMount)&&cs(ve,Te,L),Qn(k,!0),xe&&Ue){const it=()=>{k.subTree=Rl(k),Ue(xe,k.subTree,k,Y,null)};ut&&De.__asyncHydrate?De.__asyncHydrate(xe,k,it):it()}else{Ae.ce&&Ae.ce._hasShadowRoot()&&Ae.ce._injectChildStyle(De,k.parent?k.parent.type:void 0);const it=k.subTree=Rl(k);b(null,it,B,se,k,Y,X),L.el=it.el}if(pe&&Ct(pe,Y),!ut&&(ve=oe&&oe.onVnodeMounted)){const it=L;Ct(()=>cs(ve,Te,it),Y)}(L.shapeFlag&256||Te&&_n(Te.vnode)&&Te.vnode.shapeFlag&256)&&k.a&&Ct(k.a,Y),k.isMounted=!0,L=B=se=null}};k.scope.on();const de=k.effect=new Pi(ce);k.scope.off();const ae=k.update=de.run.bind(de),we=k.job=de.runIfDirty.bind(de);we.i=k,we.id=k.uid,de.scheduler=()=>oc(we),Qn(k,!0),ae()},D=(k,L,B)=>{L.component=k;const se=k.vnode.props;k.vnode=L,k.next=null,Ob(k,L.props,se,B),Db(k,L.children,B),Sn(),hd(k),Tn()},R=(k,L,B,se,Y,X,me,ce,de=!1)=>{const ae=k&&k.children,we=k?k.shapeFlag:0,ve=L.children,{patchFlag:xe,shapeFlag:oe}=L;if(xe>0){if(xe&128){ne(ae,ve,B,se,Y,X,me,ce,de);return}else if(xe&256){ee(ae,ve,B,se,Y,X,me,ce,de);return}}oe&8?(we&16&&J(ae,Y,X),ve!==ae&&d(B,ve)):we&16?oe&16?ne(ae,ve,B,se,Y,X,me,ce,de):J(ae,Y,X,!0):(we&8&&d(B,""),oe&16&&T(ve,B,se,Y,X,me,ce,de))},ee=(k,L,B,se,Y,X,me,ce,de)=>{k=k||Fa,L=L||Fa;const ae=k.length,we=L.length,ve=Math.min(ae,we);let xe;for(xe=0;xe<ve;xe++){const oe=L[xe]=de?pn(L[xe]):us(L[xe]);b(k[xe],oe,B,null,Y,X,me,ce,de)}ae>we?J(k,Y,X,!0,!1,ve):T(L,B,se,Y,X,me,ce,de,ve)},ne=(k,L,B,se,Y,X,me,ce,de)=>{let ae=0;const we=L.length;let ve=k.length-1,xe=we-1;for(;ae<=ve&&ae<=xe;){const oe=k[ae],H=L[ae]=de?pn(L[ae]):us(L[ae]);if(Ps(oe,H))b(oe,H,B,null,Y,X,me,ce,de);else break;ae++}for(;ae<=ve&&ae<=xe;){const oe=k[ve],H=L[xe]=de?pn(L[xe]):us(L[xe]);if(Ps(oe,H))b(oe,H,B,null,Y,X,me,ce,de);else break;ve--,xe--}if(ae>ve){if(ae<=xe){const oe=xe+1,H=oe<we?L[oe].el:se;for(;ae<=xe;)b(null,L[ae]=de?pn(L[ae]):us(L[ae]),B,H,Y,X,me,ce,de),ae++}}else if(ae>xe)for(;ae<=ve;)ie(k[ae],Y,X,!0),ae++;else{const oe=ae,H=ae,pe=new Map;for(ae=H;ae<=xe;ae++){const Le=L[ae]=de?pn(L[ae]):us(L[ae]);Le.key!=null&&pe.set(Le.key,ae)}let Te,Ae=0;const De=xe-H+1;let ut=!1,it=0;const te=new Array(De);for(ae=0;ae<De;ae++)te[ae]=0;for(ae=oe;ae<=ve;ae++){const Le=k[ae];if(Ae>=De){ie(Le,Y,X,!0);continue}let Me;if(Le.key!=null)Me=pe.get(Le.key);else for(Te=H;Te<=xe;Te++)if(te[Te-H]===0&&Ps(Le,L[Te])){Me=Te;break}Me===void 0?ie(Le,Y,X,!0):(te[Me-H]=ae+1,Me>=it?it=Me:ut=!0,b(Le,L[Me],B,null,Y,X,me,ce,de),Ae++)}const Oe=ut?Pb(te):Fa;for(Te=Oe.length-1,ae=De-1;ae>=0;ae--){const Le=H+ae,Me=L[Le],Qe=L[Le+1],je=Le+1<we?Qe.el||If(Qe):se;te[ae]===0?b(null,Me,B,je,Y,X,me,ce,de):ut&&(Te<0||ae!==Oe[Te]?ge(Me,B,je,2):Te--)}}},ge=(k,L,B,se,Y=null)=>{const{el:X,type:me,transition:ce,children:de,shapeFlag:ae}=k;if(ae&6){ge(k.component.subTree,L,B,se);return}if(ae&128){k.suspense.move(L,B,se);return}if(ae&64){me.move(k,L,B,Se);return}if(me===Pt){n(X,L,B);for(let ve=0;ve<de.length;ve++)ge(de[ve],L,B,se);n(k.anchor,L,B);return}if(me===ca){g(k,L,B);return}if(se!==2&&ae&1&&ce)if(se===0)ce.persisted&&!X[Ts]?n(X,L,B):(ce.beforeEnter(X),n(X,L,B),Ct(()=>ce.enter(X),Y));else{const{leave:ve,delayLeave:xe,afterLeave:oe}=ce,H=()=>{k.ctx.isUnmounted?a(X):n(X,L,B)},pe=()=>{const Te=X._isLeaving||!!X[Ts];X._isLeaving&&X[Ts](!0),ce.persisted&&!Te?H():ve(X,()=>{H(),oe&&oe()})};xe?xe(X,H,pe):pe()}else n(X,L,B)},ie=(k,L,B,se=!1,Y=!1)=>{const{type:X,props:me,ref:ce,children:de,dynamicChildren:ae,shapeFlag:we,patchFlag:ve,dirs:xe,cacheIndex:oe,memo:H}=k;if(ve===-2&&(Y=!1),ce!=null&&(Sn(),za(ce,null,B,k,!0),Tn()),oe!=null&&(L.renderCache[oe]=void 0),we&256){L.ctx.deactivate(k);return}const pe=we&1&&xe,Te=!_n(k);let Ae;if(Te&&(Ae=me&&me.onVnodeBeforeUnmount)&&cs(Ae,L,k),we&6)ye(k.component,B,se);else{if(we&128){k.suspense.unmount(B,se);return}pe&&Qs(k,null,L,"beforeUnmount"),we&64?k.type.remove(k,L,B,Se,se):ae&&!ae.hasOnce&&(X!==Pt||ve>0&&ve&64)?J(ae,L,B,!1,!0):(X===Pt&&ve&384||!Y&&we&16)&&J(de,L,B),se&&be(k)}const De=H!=null&&oe==null;(Te&&(Ae=me&&me.onVnodeUnmounted)||pe||De)&&Ct(()=>{Ae&&cs(Ae,L,k),pe&&Qs(k,null,L,"unmounted"),De&&(k.el=null)},B)},be=k=>{const{type:L,el:B,anchor:se,transition:Y}=k;if(L===Pt){Q(B,se);return}if(L===ca){_(k);return}const X=()=>{a(B),Y&&!Y.persisted&&Y.afterLeave&&Y.afterLeave()};if(k.shapeFlag&1&&Y&&!Y.persisted){const{leave:me,delayLeave:ce}=Y,de=()=>me(B,X);ce?ce(k.el,X,de):de()}else X()},Q=(k,L)=>{let B;for(;k!==L;)B=p(k),a(k),k=B;a(L)},ye=(k,L,B)=>{const{bum:se,scope:Y,job:X,subTree:me,um:ce,m:de,a:ae}=k;Vl(de),Vl(ae),se&&Ua(se),Y.stop(),X&&(X.flags|=8,ie(me,k,L,B)),ce&&Ct(ce,L),Ct(()=>{k.isUnmounted=!0},L)},J=(k,L,B,se=!1,Y=!1,X=0)=>{for(let me=X;me<k.length;me++)ie(k[me],L,B,se,Y)},U=k=>{if(k.shapeFlag&6)return U(k.component.subTree);if(k.shapeFlag&128)return k.suspense.next();const L=p(k.anchor||k.el),B=L&&L[Yp];return B?p(B):L};let le=!1;const re=(k,L,B)=>{let se;k==null?L._vnode&&(ie(L._vnode,null,null,!0),se=L._vnode.component):b(L._vnode||null,k,L,null,null,null,B),L._vnode=k,le||(le=!0,hd(se),Hl(),le=!1)},Se={p:b,um:ie,m:ge,r:be,mt:G,mc:T,pc:R,pbc:O,n:U,o:e};let _e,Ue;return t&&([_e,Ue]=t(Se)),{render:re,hydrate:_e,createApp:wb(re,_e)}}function Zr({type:e,props:t},s){return s==="svg"&&e==="foreignObject"||s==="mathml"&&e==="annotation-xml"&&t&&t.encoding&&t.encoding.includes("html")?void 0:s}function Qn({effect:e,job:t},s){s?(e.flags|=32,t.flags|=4):(e.flags&=-33,t.flags&=-5)}function Af(e,t){return(!e||e&&!e.pendingBranch)&&t&&!t.persisted}function xc(e,t,s=!1){const n=e.children,a=t.children;if(Ee(n)&&Ee(a))for(let i=0;i<n.length;i++){const l=n[i];let r=a[i];r.shapeFlag&1&&!r.dynamicChildren&&((r.patchFlag<=0||r.patchFlag===32)&&(r=a[i]=pn(a[i]),r.el=l.el),!s&&r.patchFlag!==-2&&xc(l,r)),r.type===Un&&(r.patchFlag===-1&&(r=a[i]=pn(r)),r.el=l.el),r.type===St&&!r.el&&(r.el=l.el)}}function Pb(e){const t=e.slice(),s=[0];let n,a,i,l,r;const o=e.length;for(n=0;n<o;n++){const c=e[n];if(c!==0){if(a=s[s.length-1],e[a]<c){t[n]=a,s.push(n);continue}for(i=0,l=s.length-1;i<l;)r=i+l>>1,e[s[r]]<c?i=r+1:l=r;c<e[s[i]]&&(i>0&&(t[n]=s[i-1]),s[i]=n)}}for(i=s.length,l=s[i-1];i-- >0;)s[i]=l,l=t[l];return s}function Rf(e){const t=e.subTree.component;if(t)return t.asyncDep&&!t.asyncResolved?t:Rf(t)}function Vl(e){if(e)for(let t=0;t<e.length;t++)e[t].flags|=8}function If(e){if(e.placeholder)return e.placeholder;const t=e.component;return t?If(t.subTree):null}const ql=e=>e.__isSuspense;let Ao=0;const Fb={name:"Suspense",__isSuspense:!0,process(e,t,s,n,a,i,l,r,o,c){if(e==null)Bb(t,s,n,a,i,l,r,o,c);else{if(i&&i.deps>0&&!e.suspense.isInFallback){t.suspense=e.suspense,t.suspense.vnode=t,t.el=e.el;return}Ub(e,t,s,n,a,l,r,o,c)}},hydrate:Hb,normalize:zb},$b=Fb;function ji(e,t){const s=e.props&&e.props[t];$e(s)&&s()}function Bb(e,t,s,n,a,i,l,r,o){const{p:c,o:{createElement:d}}=o,u=d("div"),p=e.suspense=Of(e,a,n,t,u,s,i,l,r,o);c(null,p.pendingBranch=e.ssContent,u,null,n,p,i,l),p.deps>0?(ji(e,"onPending"),ji(e,"onFallback"),c(null,e.ssFallback,t,s,n,null,i,l),ja(p,e.ssFallback)):p.resolve(!1,!0)}function Ub(e,t,s,n,a,i,l,r,{p:o,um:c,o:{createElement:d}}){const u=t.suspense=e.suspense;u.vnode=t,t.el=e.el;const p=t.ssContent,f=t.ssFallback,{activeBranch:m,pendingBranch:b,isInFallback:E,isHydrating:I}=u;if(b)u.pendingBranch=p,Ps(b,p)?(o(b,p,u.hiddenContainer,null,a,u,i,l,r),u.deps<=0?u.resolve():E&&(I||(o(m,f,s,n,a,null,i,l,r),ja(u,f)))):(u.pendingId=Ao++,I?(u.isHydrating=!1,u.activeBranch=b):c(b,a,u),u.deps=0,u.effects.length=0,u.hiddenContainer=d("div"),E?(o(null,p,u.hiddenContainer,null,a,u,i,l,r),u.deps<=0?u.resolve():(o(m,f,s,n,a,null,i,l,r),ja(u,f))):m&&Ps(m,p)?(o(m,p,s,n,a,u,i,l,r),u.resolve(!0)):(o(null,p,u.hiddenContainer,null,a,u,i,l,r),u.deps<=0&&u.resolve()));else if(m&&Ps(m,p))o(m,p,s,n,a,u,i,l,r),ja(u,p);else if(ji(t,"onPending"),u.pendingBranch=p,p.shapeFlag&512?u.pendingId=p.component.suspenseId:u.pendingId=Ao++,o(null,p,u.hiddenContainer,null,a,u,i,l,r),u.deps<=0)u.resolve();else{const{timeout:w,pendingId:g}=u;w>0?setTimeout(()=>{u.pendingId===g&&u.fallback(f)},w):w===0&&u.fallback(f)}}function Of(e,t,s,n,a,i,l,r,o,c,d=!1){const{p:u,m:p,um:f,n:m,o:{parentNode:b,remove:E}}=c;let I;const w=jb(e);w&&t&&t.pendingBranch&&(I=t.pendingId,t.deps++);const g=e.props?Pl(e.props.timeout):void 0,_=i,C={vnode:e,parent:t,parentComponent:s,namespace:l,container:n,hiddenContainer:a,deps:0,pendingId:Ao++,timeout:typeof g=="number"?g:-1,activeBranch:null,isFallbackMountPending:!1,pendingBranch:null,isInFallback:!d,isHydrating:d,isUnmounted:!1,effects:[],resolve(v=!1,S=!1){const{vnode:T,activeBranch:y,pendingBranch:O,pendingId:$,effects:F,parentComponent:N,container:G,isInFallback:z}=C;let P=!1;if(C.isHydrating)C.isHydrating=!1;else if(!v){P=y&&O.transition&&O.transition.mode==="out-in";let ee=!1;P&&(y.transition.afterLeave=()=>{$===C.pendingId&&(p(O,G,i===_&&!ee?m(y):i,0),Bi(F),z&&T.ssFallback&&(T.ssFallback.el=null))}),y&&!C.isFallbackMountPending&&(b(y.el)===G&&(i=m(y),ee=!0),f(y,N,C,!0),!P&&z&&T.ssFallback&&Ct(()=>T.ssFallback.el=null,C)),P||p(O,G,i,0)}C.isFallbackMountPending=!1,ja(C,O),C.pendingBranch=null,C.isInFallback=!1;let D=C.parent,R=!1;for(;D;){if(D.pendingBranch){D.effects.push(...F),R=!0;break}D=D.parent}!R&&!P&&Bi(F),C.effects=[],w&&t&&t.pendingBranch&&I===t.pendingId&&(t.deps--,t.deps===0&&!S&&t.resolve()),ji(T,"onResolve")},fallback(v){if(!C.pendingBranch)return;const{vnode:S,activeBranch:T,parentComponent:y,container:O,namespace:$}=C;ji(S,"onFallback");const F=m(T),N=()=>{C.isFallbackMountPending=!1,C.isInFallback&&(u(null,v,O,F,y,null,$,r,o),ja(C,v))},G=v.transition&&v.transition.mode==="out-in";G&&(C.isFallbackMountPending=!0,T.transition.afterLeave=N),C.isInFallback=!0,f(T,y,null,!0),G||N()},move(v,S,T){C.activeBranch&&p(C.activeBranch,v,S,T),C.container=v},next(){return C.activeBranch&&m(C.activeBranch)},registerDep(v,S,T){const y=!!C.pendingBranch;y&&C.deps++;const O=v.vnode.el;v.asyncDep.catch($=>{ya($,v,0)}).then($=>{if(v.isUnmounted||C.isUnmounted||C.pendingId!==v.suspenseId)return;Gi(),v.asyncResolved=!0;const{vnode:F}=v;Ro(v,$,!1),O&&(F.el=O);const N=!O&&v.subTree.el;S(v,F,b(O||v.subTree.el),O?null:m(v.subTree),C,l,T),N&&(F.placeholder=null,E(N)),Rr(v,F.el),y&&--C.deps===0&&C.resolve()})},unmount(v,S){C.isUnmounted=!0,C.activeBranch&&f(C.activeBranch,s,v,S),C.pendingBranch&&f(C.pendingBranch,s,v,S)}};return C}function Hb(e,t,s,n,a,i,l,r,o){const c=t.suspense=Of(t,n,s,e.parentNode,document.createElement("div"),null,a,i,l,r,!0),d=o(e,c.pendingBranch=t.ssContent,s,c,i,l);return c.deps===0&&c.resolve(!1,!0),d}function zb(e){const{shapeFlag:t,children:s}=e,n=t&32;e.ssContent=Ad(n?s.default:s),e.ssFallback=n?Ad(s.fallback):mt(St)}function Ad(e){let t;if($e(e)){const s=pa&&e._c;s&&(e._d=!1,Vi()),e=e(),s&&(e._d=!0,t=Wt,Nf())}return Ee(e)&&(e=Cb(e)),e=us(e),t&&!e.dynamicChildren&&(e.dynamicChildren=t.filter(s=>s!==e)),e}function Lf(e,t){t&&t.pendingBranch?Ee(e)?t.effects.push(...e):t.effects.push(e):Bi(e)}function ja(e,t){e.activeBranch=t;const{vnode:s,parentComponent:n}=e;let a=t.el;for(;!a&&t.component;)t=t.component.subTree,a=t.el;s.el=a,n&&n.subTree===s&&(n.vnode.el=a,Rr(n,a))}function jb(e){const t=e.props&&e.props.suspensible;return t!=null&&t!==!1}const Pt=Symbol.for("v-fgt"),Un=Symbol.for("v-txt"),St=Symbol.for("v-cmt"),ca=Symbol.for("v-stc"),Ri=[];let Wt=null;function Vi(e=!1){Ri.push(Wt=e?null:[])}function Nf(){Ri.pop(),Wt=Ri[Ri.length-1]||null}let pa=1;function qi(e,t=!1){pa+=e,e<0&&Wt&&t&&(Wt.hasOnce=!0)}function Mf(e){return e.dynamicChildren=pa>0?Wt||Fa:null,Nf(),pa>0&&Wt&&Wt.push(e),e}function Vb(e,t,s,n,a,i){return Mf(_c(e,t,s,n,a,i,!0))}function Gl(e,t,s,n,a){return Mf(mt(e,t,s,n,a,!0))}function En(e){return e?e.__v_isVNode===!0:!1}function Ps(e,t){return e.type===t.type&&e.key===t.key}function qb(e){}const Df=({key:e})=>e??null,Il=({ref:e,ref_key:t,ref_for:s})=>(typeof e=="number"&&(e=""+e),e!=null?Be(e)||Rt(e)||$e(e)?{i:Ht,r:e,k:t,f:!!s}:e:null);function _c(e,t=null,s=null,n=0,a=null,i=e===Pt?0:1,l=!1,r=!1){const o={__v_isVNode:!0,__v_skip:!0,type:e,props:t,key:t&&Df(t),ref:t&&Il(t),scopeId:kr,slotScopeIds:null,children:s,component:null,suspense:null,ssContent:null,ssFallback:null,dirs:null,transition:null,el:null,anchor:null,target:null,targetStart:null,targetAnchor:null,staticCount:0,shapeFlag:i,patchFlag:n,dynamicProps:a,dynamicChildren:null,appContext:null,ctx:Ht};return r?(kc(o,s),i&128&&e.normalize(o)):s&&(o.shapeFlag|=Be(s)?8:16),pa>0&&!l&&Wt&&(o.patchFlag>0||i&6)&&o.patchFlag!==32&&Wt.push(o),o}const mt=Gb;function Gb(e,t=null,s=null,n=0,a=null,i=!1){if((!e||e===df)&&(e=St),En(e)){const r=sn(e,t,!0);return s&&kc(r,s),pa>0&&!i&&Wt&&(r.shapeFlag&6?Wt[Wt.indexOf(e)]=r:Wt.push(r)),r.patchFlag=-2,r}if(Xb(e)&&(e=e.__vccOpts),t){t=Pf(t);let{class:r,style:o}=t;r&&!Be(r)&&(t.class=tl(r)),et(o)&&(sl(o)&&!Ee(o)&&(o=Ve({},o)),t.style=el(o))}const l=Be(e)?1:ql(e)?128:Qp(e)?64:et(e)?4:$e(e)?2:0;return _c(e,t,s,n,a,l,i,!0)}function Pf(e){return e?sl(e)||yf(e)?Ve({},e):e:null}function sn(e,t,s=!1,n=!1){const{props:a,ref:i,patchFlag:l,children:r,transition:o}=e,c=t?$f(a||{},t):a,d={__v_isVNode:!0,__v_skip:!0,type:e.type,props:c,key:c&&Df(c),ref:t&&t.ref?s&&i?Ee(i)?i.concat(Il(t)):[i,Il(t)]:Il(t):i,scopeId:e.scopeId,slotScopeIds:e.slotScopeIds,children:r,target:e.target,targetStart:e.targetStart,targetAnchor:e.targetAnchor,staticCount:e.staticCount,shapeFlag:e.shapeFlag,patchFlag:t&&e.type!==Pt?l===-1?16:l|16:l,dynamicProps:e.dynamicProps,dynamicChildren:e.dynamicChildren,appContext:e.appContext,dirs:e.dirs,transition:o,component:e.component,suspense:e.suspense,ssContent:e.ssContent&&sn(e.ssContent),ssFallback:e.ssFallback&&sn(e.ssFallback),placeholder:e.placeholder,el:e.el,anchor:e.anchor,ctx:e.ctx,ce:e.ce};return o&&n&&Cn(d,o.clone(d)),d}function wc(e=" ",t=0){return mt(Un,null,e,t)}function Kb(e,t){const s=mt(ca,null,e);return s.staticCount=t,s}function Ff(e="",t=!1){return t?(Vi(),Gl(St,null,e)):mt(St,null,e)}function us(e){return e==null||typeof e=="boolean"?mt(St):Ee(e)?mt(Pt,null,e.slice()):En(e)?pn(e):mt(Un,null,String(e))}function pn(e){return e.el===null&&e.patchFlag!==-1||e.memo?e:sn(e)}function kc(e,t){let s=0;const{shapeFlag:n}=e;if(t==null)t=null;else if(Ee(t))s=16;else if(typeof t=="object")if(n&65){const a=t.default;a&&(a._c&&(a._d=!1),kc(e,a()),a._c&&(a._d=!0));return}else{s=32;const a=t._;!a&&!yf(t)?t._ctx=Ht:a===3&&Ht&&(Ht.slots._===1?t._=1:(t._=2,e.patchFlag|=1024))}else $e(t)?(t={default:t,_ctx:Ht},s=32):(t=String(t),n&64?(s=16,t=[wc(t)]):s=8);e.children=t,e.shapeFlag|=s}function $f(...e){const t={};for(let s=0;s<e.length;s++){const n=e[s];for(const a in n)if(a==="class")t.class!==n.class&&(t.class=tl([t.class,n.class]));else if(a==="style")t.style=el([t.style,n.style]);else if(va(a)){const i=t[a],l=n[a];l&&i!==l&&!(Ee(i)&&i.includes(l))?t[a]=i?[].concat(i,l):l:l==null&&i==null&&!pr(a)&&(t[a]=l)}else a!==""&&(t[a]=n[a])}return t}function cs(e,t,s,n=null){ys(e,t,7,[s,n])}const Wb=ff();let Zb=0;function Bf(e,t,s){const n=e.type,a=(t?t.appContext:e.appContext)||Wb,i={uid:Zb++,vnode:e,type:n,parent:t,appContext:a,root:null,next:null,subTree:null,effect:null,update:null,job:null,scope:new tc(!0),render:null,proxy:null,exposed:null,exposeProxy:null,withProxy:null,provides:t?t.provides:Object.create(a.provides),ids:t?t.ids:["",0,0],accessCache:null,renderCache:[],components:null,directives:null,propsOptions:_f(n,a),emitsOptions:mf(n,a),emit:null,emitted:null,propsDefaults:qe,inheritAttrs:n.inheritAttrs,ctx:qe,data:qe,props:qe,attrs:qe,slots:qe,refs:qe,setupState:qe,setupContext:null,suspense:s,suspenseId:s?s.pendingId:0,asyncDep:null,asyncResolved:!1,isMounted:!1,isUnmounted:!1,isDeactivated:!1,bc:null,c:null,bm:null,m:null,bu:null,u:null,um:null,bum:null,da:null,a:null,rtg:null,rtc:null,ec:null,sp:null};return i.ctx={_:i},i.root=t?t.root:i,i.emit=Sb.bind(null,i),e.ce&&e.ce(i),i}let Ut=null;const ls=()=>Ut||Ht;let Kl,Va;{const e=gr(),t=(s,n)=>{let a;return(a=e[s])||(a=e[s]=[]),a.push(n),i=>{a.length>1?a.forEach(l=>l(i)):a[0](i)}};Kl=t("__VUE_INSTANCE_SETTERS__",s=>Ut=s),Va=t("__VUE_SSR_SETTERS__",s=>fa=s)}const ri=e=>{const t=Ut;return Kl(e),e.scope.on(),()=>{e.scope.off(),Kl(t)}},Gi=()=>{Ut&&Ut.scope.off(),Kl(null)};function Uf(e){return e.vnode.shapeFlag&4}let fa=!1;function Hf(e,t=!1,s=!1){t&&Va(t);const{props:n,children:a}=e.vnode,i=Uf(e);Ib(e,n,i,t),Mb(e,a,s||t);const l=i?Jb(e,t):void 0;return t&&Va(!1),l}function Jb(e,t){const s=e.type;e.accessCache=Object.create(null),e.proxy=new Proxy(e.ctx,So);const{setup:n}=s;if(n){Sn();const a=e.setupContext=n.length>1?Vf(e):null,i=ri(e),l=li(n,e,0,[e.props,a]),r=ec(l);if(Tn(),i(),(r||e.sp)&&!_n(e)&&pc(e),r){if(l.then(Gi,Gi),t)return l.then(o=>{Ro(e,o,t)}).catch(o=>{ya(o,e,0)});e.asyncDep=l}else Ro(e,l,t)}else jf(e,t)}function Ro(e,t,s){$e(t)?e.type.__ssrInlineRender?e.ssrRender=t:e.render=t:et(t)&&(e.setupState=rc(t)),jf(e,s)}let Wl,Io;function zf(e){Wl=e,Io=t=>{t.render._rc&&(t.withProxy=new Proxy(t.ctx,sb))}}const Yb=()=>!Wl;function jf(e,t,s){const n=e.type;if(!e.render){if(!t&&Wl&&!n.render){const a=n.template||gc(e).template;if(a){const{isCustomElement:i,compilerOptions:l}=e.appContext.config,{delimiters:r,compilerOptions:o}=n,c=Ve(Ve({isCustomElement:i,delimiters:r},l),o);n.render=Wl(a,c)}}e.render=n.render||zt,Io&&Io(e)}{const a=ri(e);Sn();try{vb(e)}finally{Tn(),a()}}}const Qb={get(e,t){return Kt(e,"get",""),e[t]}};function Vf(e){const t=s=>{e.exposed=s||{}};return{attrs:new Proxy(e.attrs,Qb),slots:e.slots,emit:e.emit,expose:t}}function ll(e){return e.exposed?e.exposeProxy||(e.exposeProxy=new Proxy(rc(Fp(e.exposed)),{get(t,s){if(s in t)return t[s];if(s in Ai)return Ai[s](e)},has(t,s){return s in t||s in Ai}})):e.proxy}function Oo(e,t=!0){return $e(e)?e.displayName||e.name:e.name||t&&e.__name}function Xb(e){return $e(e)&&"__vccOpts"in e}const W=(e,t)=>ig(e,t,fa);function Za(e,t,s){try{qi(-1);const n=arguments.length;return n===2?et(t)&&!Ee(t)?En(t)?mt(e,null,[t]):mt(e,t):mt(e,null,t):(n>3?s=Array.prototype.slice.call(arguments,2):n===3&&En(s)&&(s=[s]),mt(e,t,s))}finally{qi(1)}}function ey(){}function ty(e,t,s,n){const a=s[n];if(a&&qf(a,e))return a;const i=t();return i.memo=e.slice(),i.cacheIndex=n,s[n]=i}function qf(e,t){const s=e.memo;if(s.length!=t.length)return!1;for(let n=0;n<s.length;n++)if(Dt(s[n],t[n]))return!1;return pa>0&&Wt&&Wt.push(e),!0}const Gf="3.5.38",sy=zt,ny=hg,ay=La,iy=Gp,ly={createComponentInstance:Bf,setupComponent:Hf,renderComponentRoot:Rl,setCurrentRenderingInstance:Hi,isVNode:En,normalizeVNode:us,getComponentPublicInstance:ll,ensureValidVNode:vc,pushWarningContext:dg,popWarningContext:ug},ry=ly,oy=null,cy=null,dy=null;/**
* @vue/runtime-dom v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/let Lo;const Rd=typeof window<"u"&&window.trustedTypes;if(Rd)try{Lo=Rd.createPolicy("vue",{createHTML:e=>e})}catch{}const Kf=Lo?e=>Lo.createHTML(e):e=>e,uy="http://www.w3.org/2000/svg",py="http://www.w3.org/1998/Math/MathML",un=typeof document<"u"?document:null,Id=un&&un.createElement("template"),Wf={insert:(e,t,s)=>{t.insertBefore(e,s||null)},remove:e=>{const t=e.parentNode;t&&t.removeChild(e)},createElement:(e,t,s,n)=>{const a=t==="svg"?un.createElementNS(uy,e):t==="mathml"?un.createElementNS(py,e):s?un.createElement(e,{is:s}):un.createElement(e);return e==="select"&&n&&n.multiple!=null&&a.setAttribute("multiple",n.multiple),a},createText:e=>un.createTextNode(e),createComment:e=>un.createComment(e),setText:(e,t)=>{e.nodeValue=t},setElementText:(e,t)=>{e.textContent=t},parentNode:e=>e.parentNode,nextSibling:e=>e.nextSibling,querySelector:e=>un.querySelector(e),setScopeId(e,t){e.setAttribute(t,"")},insertStaticContent(e,t,s,n,a,i){const l=s?s.previousSibling:t.lastChild;if(a&&(a===i||a.nextSibling))for(;t.insertBefore(a.cloneNode(!0),s),!(a===i||!(a=a.nextSibling)););else{Id.innerHTML=Kf(n==="svg"?`<svg>${e}</svg>`:n==="mathml"?`<math>${e}</math>`:e);const r=Id.content;if(n==="svg"||n==="mathml"){const o=r.firstChild;for(;o.firstChild;)r.appendChild(o.firstChild);r.removeChild(o)}t.insertBefore(r,s)}return[l?l.nextSibling:t.firstChild,s?s.previousSibling:t.lastChild]}},On="transition",fi="animation",Ja=Symbol("_vtc"),Zf={name:String,type:String,css:{type:Boolean,default:!0},duration:[String,Number,Object],enterFromClass:String,enterActiveClass:String,enterToClass:String,appearFromClass:String,appearActiveClass:String,appearToClass:String,leaveFromClass:String,leaveActiveClass:String,leaveToClass:String},Jf=Ve({},uc,Zf),fy=e=>(e.displayName="Transition",e.props=Jf,e),hy=fy((e,{slots:t})=>Za(tf,Yf(e),t)),Xn=(e,t=[])=>{Ee(e)?e.forEach(s=>s(...t)):e&&e(...t)},Od=e=>e?Ee(e)?e.some(t=>t.length>1):e.length>1:!1;function Yf(e){const t={};for(const F in e)F in Zf||(t[F]=e[F]);if(e.css===!1)return t;const{name:s="v",type:n,duration:a,enterFromClass:i=`${s}-enter-from`,enterActiveClass:l=`${s}-enter-active`,enterToClass:r=`${s}-enter-to`,appearFromClass:o=i,appearActiveClass:c=l,appearToClass:d=r,leaveFromClass:u=`${s}-leave-from`,leaveActiveClass:p=`${s}-leave-active`,leaveToClass:f=`${s}-leave-to`}=e,m=my(a),b=m&&m[0],E=m&&m[1],{onBeforeEnter:I,onEnter:w,onEnterCancelled:g,onLeave:_,onLeaveCancelled:C,onBeforeAppear:v=I,onAppear:S=w,onAppearCancelled:T=g}=t,y=(F,N,G,z)=>{F._enterCancelled=z,Dn(F,N?d:r),Dn(F,N?c:l),G&&G()},O=(F,N)=>{F._isLeaving=!1,Dn(F,u),Dn(F,f),Dn(F,p),N&&N()},$=F=>(N,G)=>{const z=F?S:w,P=()=>y(N,F,G);Xn(z,[N,P]),Ld(()=>{Dn(N,F?o:i),Ws(N,F?d:r),Od(z)||Nd(N,n,b,P)})};return Ve(t,{onBeforeEnter(F){Xn(I,[F]),Ws(F,i),Ws(F,l)},onBeforeAppear(F){Xn(v,[F]),Ws(F,o),Ws(F,c)},onEnter:$(!1),onAppear:$(!0),onLeave(F,N){F._isLeaving=!0;const G=()=>O(F,N);Ws(F,u),F._enterCancelled?(Ws(F,p),No(F)):(No(F),Ws(F,p)),Ld(()=>{F._isLeaving&&(Dn(F,u),Ws(F,f),Od(_)||Nd(F,n,E,G))}),Xn(_,[F,G])},onEnterCancelled(F){y(F,!1,void 0,!0),Xn(g,[F])},onAppearCancelled(F){y(F,!0,void 0,!0),Xn(T,[F])},onLeaveCancelled(F){O(F),Xn(C,[F])}})}function my(e){if(e==null)return null;if(et(e))return[Jr(e.enter),Jr(e.leave)];{const t=Jr(e);return[t,t]}}function Jr(e){return Pl(e)}function Ws(e,t){t.split(/\s+/).forEach(s=>s&&e.classList.add(s)),(e[Ja]||(e[Ja]=new Set)).add(t)}function Dn(e,t){t.split(/\s+/).forEach(n=>n&&e.classList.remove(n));const s=e[Ja];s&&(s.delete(t),s.size||(e[Ja]=void 0))}function Ld(e){requestAnimationFrame(()=>{requestAnimationFrame(e)})}let vy=0;function Nd(e,t,s,n){const a=e._endId=++vy,i=()=>{a===e._endId&&n()};if(s!=null)return setTimeout(i,s);const{type:l,timeout:r,propCount:o}=Qf(e,t);if(!l)return n();const c=l+"end";let d=0;const u=()=>{e.removeEventListener(c,p),i()},p=f=>{f.target===e&&++d>=o&&u()};setTimeout(()=>{d<o&&u()},r+1),e.addEventListener(c,p)}function Qf(e,t){const s=window.getComputedStyle(e),n=m=>(s[m]||"").split(", "),a=n(`${On}Delay`),i=n(`${On}Duration`),l=Md(a,i),r=n(`${fi}Delay`),o=n(`${fi}Duration`),c=Md(r,o);let d=null,u=0,p=0;t===On?l>0&&(d=On,u=l,p=i.length):t===fi?c>0&&(d=fi,u=c,p=o.length):(u=Math.max(l,c),d=u>0?l>c?On:fi:null,p=d?d===On?i.length:o.length:0);const f=d===On&&/\b(?:transform|all)(?:,|$)/.test(n(`${On}Property`).toString());return{type:d,timeout:u,propCount:p,hasTransform:f}}function Md(e,t){for(;e.length<t.length;)e=e.concat(e);return Math.max(...t.map((s,n)=>Dd(s)+Dd(e[n])))}function Dd(e){return e==="auto"?0:Number(e.slice(0,-1).replace(",","."))*1e3}function No(e){return(e?e.ownerDocument:document).body.offsetHeight}function gy(e,t,s){const n=e[Ja];n&&(t=(t?[t,...n]:[...n]).join(" ")),t==null?e.removeAttribute("class"):s?e.setAttribute("class",t):e.className=t}const Zl=Symbol("_vod"),Sc=Symbol("_vsh"),Xf={name:"show",beforeMount(e,{value:t},{transition:s}){e[Zl]=e.style.display==="none"?"":e.style.display,s&&t?s.beforeEnter(e):hi(e,t)},mounted(e,{value:t},{transition:s}){s&&t&&s.enter(e)},updated(e,{value:t,oldValue:s},{transition:n}){!t!=!s&&(n?t?(n.beforeEnter(e),hi(e,!0),n.enter(e)):n.leave(e,()=>{hi(e,!1)}):hi(e,t))},beforeUnmount(e,{value:t}){hi(e,t)}};function hi(e,t){e.style.display=t?e[Zl]:"none",e[Sc]=!t}function by(){Xf.getSSRProps=({value:e})=>{if(!e)return{style:{display:"none"}}}}const eh=Symbol("");function yy(e){const t=ls();if(!t)return;const s=t.ut=(a=e(t.proxy))=>{Array.from(document.querySelectorAll(`[data-v-owner="${t.uid}"]`)).forEach(i=>Jl(i,a))},n=()=>{const a=e(t.proxy);t.ce?Jl(t.ce,a):Mo(t.subTree,a),s(a)};fc(()=>{Bi(n)}),Ke(()=>{is(n,zt,{flush:"post"});const a=new MutationObserver(n);a.observe(t.subTree.el.parentNode,{childList:!0}),_t(()=>a.disconnect())})}function Mo(e,t){if(e.shapeFlag&128){const s=e.suspense;e=s.activeBranch,s.pendingBranch&&!s.isHydrating&&s.effects.push(()=>{Mo(s.activeBranch,t)})}for(;e.component;)e=e.component.subTree;if(e.shapeFlag&1&&e.el)Jl(e.el,t);else if(e.type===Pt)e.children.forEach(s=>Mo(s,t));else if(e.type===ca){let{el:s,anchor:n}=e;for(;s&&(Jl(s,t),s!==n);)s=s.nextSibling}}function Jl(e,t){if(e.nodeType===1){const s=e.style;let n="";for(const a in t){const i=Tv(t[a]);s.setProperty(`--${a}`,i),n+=`--${a}: ${i};`}s[eh]=n}}const xy=/(?:^|;)\s*display\s*:/;function _y(e,t,s){const n=e.style,a=Be(s);let i=!1;if(s&&!a){if(t)if(Be(t))for(const l of t.split(";")){const r=l.slice(0,l.indexOf(":")).trim();s[r]==null&&ki(n,r,"")}else for(const l in t)s[l]==null&&ki(n,l,"");for(const l in s){l==="display"&&(i=!0);const r=s[l];r!=null?ky(e,l,!Be(t)&&t?t[l]:void 0,r)||ki(n,l,r):ki(n,l,"")}}else if(a){if(t!==s){const l=n[eh];l&&(s+=";"+l),n.cssText=s,i=xy.test(s)}}else t&&e.removeAttribute("style");Zl in e&&(e[Zl]=i?n.display:"",e[Sc]&&(n.display="none"))}const Pd=/\s*!important$/;function ki(e,t,s){if(Ee(s))s.forEach(n=>ki(e,t,n));else if(s==null&&(s=""),t.startsWith("--"))e.setProperty(t,s);else{const n=wy(e,t);Pd.test(s)?e.setProperty(ps(n),s.replace(Pd,""),"important"):e[n]=s}}const Fd=["Webkit","Moz","ms"],Yr={};function wy(e,t){const s=Yr[t];if(s)return s;let n=rt(t);if(n!=="filter"&&n in e)return Yr[t]=n;n=ba(n);for(let a=0;a<Fd.length;a++){const i=Fd[a]+n;if(i in e)return Yr[t]=i}return t}function ky(e,t,s,n){return e.tagName==="TEXTAREA"&&(t==="width"||t==="height")&&Be(n)&&s===n}const $d="http://www.w3.org/1999/xlink";function Bd(e,t,s,n,a,i=kv(t)){n&&t.startsWith("xlink:")?s==null?e.removeAttributeNS($d,t.slice(6,t.length)):e.setAttributeNS($d,t,s):s==null||i&&!gp(s)?e.removeAttribute(t):e.setAttribute(t,i?"":Yt(s)?String(s):s)}function Ud(e,t,s,n,a){if(t==="innerHTML"||t==="textContent"){s!=null&&(e[t]=t==="innerHTML"?Kf(s):s);return}const i=e.tagName;if(t==="value"&&i!=="PROGRESS"&&!i.includes("-")){const r=i==="OPTION"?e.getAttribute("value")||"":e.value,o=s==null?e.type==="checkbox"?"on":"":String(s);(r!==o||!("_value"in e))&&(e.value=o),s==null&&e.removeAttribute(t),e._value=s;return}let l=!1;if(s===""||s==null){const r=typeof e[t];r==="boolean"?s=gp(s):s==null&&r==="string"?(s="",l=!0):r==="number"&&(s=0,l=!0)}try{e[t]=s}catch{}l&&e.removeAttribute(a||t)}function vn(e,t,s,n){e.addEventListener(t,s,n)}function Sy(e,t,s,n){e.removeEventListener(t,s,n)}const Hd=Symbol("_vei");function Ty(e,t,s,n,a=null){const i=e[Hd]||(e[Hd]={}),l=i[t];if(n&&l)l.value=n;else{const[r,o]=Cy(t);if(n){const c=i[t]=Ry(n,a);vn(e,r,c,o)}else l&&(Sy(e,r,l,o),i[t]=void 0)}}const zd=/(?:Once|Passive|Capture)$/;function Cy(e){let t;if(zd.test(e)){t={};let n;for(;n=e.match(zd);)e=e.slice(0,e.length-n[0].length),t[n[0].toLowerCase()]=!0}return[e[2]===":"?e.slice(3):ps(e.slice(2)),t]}let Qr=0;const Ey=Promise.resolve(),Ay=()=>Qr||(Ey.then(()=>Qr=0),Qr=Date.now());function Ry(e,t){const s=n=>{if(!n._vts)n._vts=Date.now();else if(n._vts<=s.attached)return;const a=s.value;if(Ee(a)){const i=n.stopImmediatePropagation;n.stopImmediatePropagation=()=>{i.call(n),n._stopped=!0};const l=a.slice(),r=[n];for(let o=0;o<l.length&&!n._stopped;o++){const c=l[o];c&&ys(c,t,5,r)}}else ys(a,t,5,[n])};return s.value=e,s.attached=Ay(),s}const jd=e=>e.charCodeAt(0)===111&&e.charCodeAt(1)===110&&e.charCodeAt(2)>96&&e.charCodeAt(2)<123,th=(e,t,s,n,a,i)=>{const l=a==="svg";t==="class"?gy(e,n,l):t==="style"?_y(e,s,n):va(t)?pr(t)||Ty(e,t,s,n,i):(t[0]==="."?(t=t.slice(1),!0):t[0]==="^"?(t=t.slice(1),!1):Iy(e,t,n,l))?(Ud(e,t,n),!e.tagName.includes("-")&&(t==="value"||t==="checked"||t==="selected")&&Bd(e,t,n,l,i,t!=="value")):e._isVueCE&&(Oy(e,t)||e._def.__asyncLoader&&(/[A-Z]/.test(t)||!Be(n)))?Ud(e,rt(t),n,i,t):(t==="true-value"?e._trueValue=n:t==="false-value"&&(e._falseValue=n),Bd(e,t,n,l))};function Iy(e,t,s,n){if(n)return!!(t==="innerHTML"||t==="textContent"||t in e&&jd(t)&&$e(s));if(t==="spellcheck"||t==="draggable"||t==="translate"||t==="autocorrect"||t==="sandbox"&&e.tagName==="IFRAME"||t==="form"||t==="list"&&e.tagName==="INPUT"||t==="type"&&e.tagName==="TEXTAREA")return!1;if(t==="width"||t==="height"){const a=e.tagName;if(a==="IMG"||a==="VIDEO"||a==="CANVAS"||a==="SOURCE")return!1}return jd(t)&&Be(s)?!1:t in e}function Oy(e,t){const s=e._def.props;if(!s)return!1;const n=rt(t);return Array.isArray(s)?s.some(a=>rt(a)===n):Object.keys(s).some(a=>rt(a)===n)}const Vd={};function sh(e,t,s){let n=al(e,t);fr(n)&&(n=Ve({},n,t));class a extends Ir{constructor(l){super(n,l,s)}}return a.def=n,a}const Ly=((e,t)=>sh(e,t,mh)),Ny=typeof HTMLElement<"u"?HTMLElement:class{};class Ir extends Ny{constructor(t,s={},n=Xl){super(),this._def=t,this._props=s,this._createApp=n,this._isVueCE=!0,this._instance=null,this._app=null,this._nonce=this._def.nonce,this._connected=!1,this._resolved=!1,this._patching=!1,this._dirty=!1,this._numberProps=null,this._styleChildren=new WeakSet,this._styleAnchors=new WeakMap,this._ob=null,this.shadowRoot&&n!==Xl?this._root=this.shadowRoot:t.shadowRoot!==!1?(this.attachShadow(Ve({},t.shadowRootOptions,{mode:"open"})),this._root=this.shadowRoot):this._root=this}connectedCallback(){if(!this.isConnected)return;!this.shadowRoot&&!this._resolved&&this._parseSlots(),this._connected=!0;let t=this;for(;t=t&&(t.assignedSlot||t.parentNode||t.host);)if(t instanceof Ir){this._parent=t;break}this._instance||(this._resolved?this._mount(this._def):t&&t._pendingResolve?this._pendingResolve=t._pendingResolve.then(()=>{this._pendingResolve=void 0,this._resolveDef()}):this._resolveDef())}_setParent(t=this._parent){t&&(this._instance.parent=t._instance,this._inheritParentContext(t))}_inheritParentContext(t=this._parent){t&&this._app&&Object.setPrototypeOf(this._app._context.provides,t._instance.provides)}disconnectedCallback(){this._connected=!1,Et(()=>{this._connected||(this._ob&&(this._ob.disconnect(),this._ob=null),this._app&&this._app.unmount(),this._instance&&(this._instance.ce=void 0),this._app=this._instance=null,this._teleportTargets&&(this._teleportTargets.clear(),this._teleportTargets=void 0))})}_processMutations(t){for(const s of t)this._setAttr(s.attributeName)}_resolveDef(){if(this._pendingResolve)return;for(let n=0;n<this.attributes.length;n++)this._setAttr(this.attributes[n].name);this._ob=new MutationObserver(this._processMutations.bind(this)),this._ob.observe(this,{attributes:!0});const t=(n,a=!1)=>{this._resolved=!0,this._pendingResolve=void 0;const{props:i,styles:l}=n;let r;if(i&&!Ee(i))for(const o in i){const c=i[o];(c===Number||c&&c.type===Number)&&(o in this._props&&(this._props[o]=Pl(this._props[o])),(r||(r=Object.create(null)))[rt(o)]=!0)}this._numberProps=r,this._resolveProps(n),this.shadowRoot&&this._applyStyles(l),this._mount(n)},s=this._def.__asyncLoader;s?this._pendingResolve=s().then(n=>{n.configureApp=this._def.configureApp,t(this._def=n,!0)}):t(this._def)}_mount(t){this._app=this._createApp(t),this._inheritParentContext(),t.configureApp&&t.configureApp(this._app),this._app._ceVNode=this._createVNode(),this._app.mount(this._root);const s=this._instance&&this._instance.exposed;if(s)for(const n in s)tt(this,n)||Object.defineProperty(this,n,{get:()=>en(s[n])})}_resolveProps(t){const{props:s}=t,n=Ee(s)?s:Object.keys(s||{});for(const a of Object.keys(this))a[0]!=="_"&&n.includes(a)&&this._setProp(a,this[a]);for(const a of n.map(rt))Object.defineProperty(this,a,{get(){return this._getProp(a)},set(i){this._setProp(a,i,!0,!this._patching)}})}_setAttr(t){if(t.startsWith("data-v-"))return;const s=this.hasAttribute(t);let n=s?this.getAttribute(t):Vd;const a=rt(t);s&&this._numberProps&&this._numberProps[a]&&(n=Pl(n)),this._setProp(a,n,!1,!0)}_getProp(t){return this._props[t]}_setProp(t,s,n=!0,a=!1){if(s!==this._props[t]&&(this._dirty=!0,s===Vd?delete this._props[t]:(this._props[t]=s,t==="key"&&this._app&&(this._app._ceVNode.key=s)),a&&this._instance&&this._update(),n)){const i=this._ob;i&&(this._processMutations(i.takeRecords()),i.disconnect()),s===!0?this.setAttribute(ps(t),""):typeof s=="string"||typeof s=="number"?this.setAttribute(ps(t),s+""):s||this.removeAttribute(ps(t)),i&&i.observe(this,{attributes:!0})}}_update(){const t=this._createVNode();this._app&&(t.appContext=this._app._context),hh(t,this._root)}_createVNode(){const t={};this.shadowRoot||(t.onVnodeMounted=t.onVnodeUpdated=this._renderSlots.bind(this));const s=mt(this._def,Ve(t,this._props));return this._instance||(s.ce=n=>{this._instance=n,n.ce=this,n.isCE=!0;const a=(i,l)=>{this.dispatchEvent(new CustomEvent(i,fr(l[0])?Ve({detail:l},l[0]):{detail:l}))};n.emit=(i,...l)=>{a(i,l),ps(i)!==i&&a(ps(i),l)},this._setParent()}),s}_applyStyles(t,s,n){if(!t)return;if(s){if(s===this._def||this._styleChildren.has(s))return;this._styleChildren.add(s)}const a=this._nonce,i=this.shadowRoot,l=n?this._getStyleAnchor(n)||this._getStyleAnchor(this._def):this._getRootStyleInsertionAnchor(i);let r=null;for(let o=t.length-1;o>=0;o--){const c=document.createElement("style");a&&c.setAttribute("nonce",a),c.textContent=t[o],i.insertBefore(c,r||l),r=c,o===0&&(n||this._styleAnchors.set(this._def,c),s&&this._styleAnchors.set(s,c))}}_getStyleAnchor(t){if(!t)return null;const s=this._styleAnchors.get(t);return s&&s.parentNode===this.shadowRoot?s:(s&&this._styleAnchors.delete(t),null)}_getRootStyleInsertionAnchor(t){for(let s=0;s<t.childNodes.length;s++){const n=t.childNodes[s];if(!(n instanceof HTMLStyleElement))return n}return null}_parseSlots(){const t=this._slots={};let s;for(;s=this.firstChild;){const n=s.nodeType===1&&s.getAttribute("slot")||"default";(t[n]||(t[n]=[])).push(s),this.removeChild(s)}}_renderSlots(){const t=this._getSlots(),s=this._instance.type.__scopeId;for(let n=0;n<t.length;n++){const a=t[n],i=a.getAttribute("name")||"default",l=this._slots[i],r=a.parentNode;if(l)for(const o of l){if(s&&o.nodeType===1){const c=s+"-s",d=document.createTreeWalker(o,1);o.setAttribute(c,"");let u;for(;u=d.nextNode();)u.setAttribute(c,"")}r.insertBefore(o,a)}else for(;a.firstChild;)r.insertBefore(a.firstChild,a);r.removeChild(a)}}_getSlots(){const t=[this];this._teleportTargets&&t.push(...this._teleportTargets);const s=new Set;for(const n of t){const a=n.querySelectorAll("slot");for(let i=0;i<a.length;i++)s.add(a[i])}return Array.from(s)}_injectChildStyle(t,s){this._applyStyles(t.styles,t,s)}_beginPatch(){this._patching=!0,this._dirty=!1}_endPatch(){this._patching=!1,this._dirty&&this._instance&&this._update()}_hasShadowRoot(){return this._def.shadowRoot!==!1}_removeChildStyle(t){}}function nh(e){const t=ls(),s=t&&t.ce;return s||null}function My(){const e=nh();return e&&e.shadowRoot}function Dy(e="$style"){{const t=ls();if(!t)return qe;const s=t.type.__cssModules;if(!s)return qe;const n=s[e];return n||qe}}const ah=new WeakMap,ih=new WeakMap,Yl=Symbol("_moveCb"),qd=Symbol("_enterCb"),Py=e=>(delete e.props.mode,e),Fy=Py({name:"TransitionGroup",props:Ve({},Jf,{tag:String,moveClass:String}),setup(e,{slots:t}){const s=ls(),n=dc();let a,i;return Cr(()=>{if(!a.length)return;const l=e.moveClass||`${e.name||"v"}-move`;if(!zy(a[0].el,s.vnode.el,l)){a=[];return}a.forEach(By),a.forEach(Uy);const r=a.filter(Hy);No(s.vnode.el),r.forEach(o=>{const c=o.el,d=c.style;Ws(c,l),d.transform=d.webkitTransform=d.transitionDuration="";const u=c[Yl]=p=>{p&&p.target!==c||(!p||p.propertyName.endsWith("transform"))&&(c.removeEventListener("transitionend",u),c[Yl]=null,Dn(c,l))};c.addEventListener("transitionend",u)}),a=[]}),()=>{const l=Ze(e),r=Yf(l);let o=l.tag||Pt;if(a=[],i)for(let c=0;c<i.length;c++){const d=i[c];d.el&&d.el instanceof Element&&!d.el[Sc]&&(a.push(d),Cn(d,Wa(d,r,n,s)),ah.set(d,lh(d.el)))}i=t.default?Sr(t.default()):[];for(let c=0;c<i.length;c++){const d=i[c];d.key!=null&&Cn(d,Wa(d,r,n,s))}return mt(o,null,i)}}}),$y=Fy;function By(e){const t=e.el;t[Yl]&&t[Yl](),t[qd]&&t[qd]()}function Uy(e){ih.set(e,lh(e.el))}function Hy(e){const t=ah.get(e),s=ih.get(e),n=t.left-s.left,a=t.top-s.top;if(n||a){const i=e.el,l=i.style,r=i.getBoundingClientRect();let o=1,c=1;return i.offsetWidth&&(o=r.width/i.offsetWidth),i.offsetHeight&&(c=r.height/i.offsetHeight),(!Number.isFinite(o)||o===0)&&(o=1),(!Number.isFinite(c)||c===0)&&(c=1),Math.abs(o-1)<.01&&(o=1),Math.abs(c-1)<.01&&(c=1),l.transform=l.webkitTransform=`translate(${n/o}px,${a/c}px)`,l.transitionDuration="0s",e}}function lh(e){const t=e.getBoundingClientRect();return{left:t.left,top:t.top}}function zy(e,t,s){const n=e.cloneNode(),a=e[Ja];a&&a.forEach(r=>{r.split(/\s+/).forEach(o=>o&&n.classList.remove(o))}),s.split(/\s+/).forEach(r=>r&&n.classList.add(r)),n.style.display="none";const i=t.nodeType===1?t:t.parentNode;i.appendChild(n);const{hasTransform:l}=Qf(n);return i.removeChild(n),l}const zn=e=>{const t=e.props["onUpdate:modelValue"]||!1;return Ee(t)?s=>Ua(t,s):t};function jy(e){e.target.composing=!0}function Gd(e){const t=e.target;t.composing&&(t.composing=!1,t.dispatchEvent(new Event("input")))}const Is=Symbol("_assign");function Kd(e,t,s){return t&&(e=e.trim()),s&&(e=vr(e)),e}const Ql={created(e,{modifiers:{lazy:t,trim:s,number:n}},a){e[Is]=zn(a);const i=n||a.props&&a.props.type==="number";vn(e,t?"change":"input",l=>{l.target.composing||e[Is](Kd(e.value,s,i))}),(s||i)&&vn(e,"change",()=>{e.value=Kd(e.value,s,i)}),t||(vn(e,"compositionstart",jy),vn(e,"compositionend",Gd),vn(e,"change",Gd))},mounted(e,{value:t}){e.value=t??""},beforeUpdate(e,{value:t,oldValue:s,modifiers:{lazy:n,trim:a,number:i}},l){if(e[Is]=zn(l),e.composing)return;const r=(i||e.type==="number")&&!/^0\d/.test(e.value)?vr(e.value):e.value,o=t??"";if(r===o)return;const c=e.getRootNode();(c instanceof Document||c instanceof ShadowRoot)&&c.activeElement===e&&e.type!=="range"&&(n&&t===s||a&&e.value.trim()===o)||(e.value=o)}},Tc={deep:!0,created(e,t,s){e[Is]=zn(s),vn(e,"change",()=>{const n=e._modelValue,a=Ya(e),i=e.checked,l=e[Is];if(Ee(n)){const r=br(n,a),o=r!==-1;if(i&&!o)l(n.concat(a));else if(!i&&o){const c=[...n];c.splice(r,1),l(c)}}else if(ga(n)){const r=new Set(n);i?r.add(a):r.delete(a),l(r)}else l(oh(e,i))})},mounted:Wd,beforeUpdate(e,t,s){e[Is]=zn(s),Wd(e,t,s)}};function Wd(e,{value:t,oldValue:s},n){e._modelValue=t;let a;if(Ee(t))a=br(t,n.props.value)>-1;else if(ga(t))a=t.has(n.props.value);else{if(t===s)return;a=kn(t,oh(e,!0))}e.checked!==a&&(e.checked=a)}const Cc={created(e,{value:t},s){e.checked=kn(t,s.props.value),e[Is]=zn(s),vn(e,"change",()=>{e[Is](Ya(e))})},beforeUpdate(e,{value:t,oldValue:s},n){e[Is]=zn(n),t!==s&&(e.checked=kn(t,n.props.value))}},rh={deep:!0,created(e,{value:t,modifiers:{number:s}},n){const a=ga(t);vn(e,"change",()=>{const i=Array.prototype.filter.call(e.options,l=>l.selected).map(l=>s?vr(Ya(l)):Ya(l));e[Is](e.multiple?a?new Set(i):i:i[0]),e._assigning=!0,Et(()=>{e._assigning=!1})}),e[Is]=zn(n)},mounted(e,{value:t}){Zd(e,t)},beforeUpdate(e,t,s){e[Is]=zn(s)},updated(e,{value:t}){e._assigning||Zd(e,t)}};function Zd(e,t){const s=e.multiple,n=Ee(t);if(!(s&&!n&&!ga(t))){for(let a=0,i=e.options.length;a<i;a++){const l=e.options[a],r=Ya(l);if(s)if(n){const o=typeof r;o==="string"||o==="number"?l.selected=t.some(c=>String(c)===String(r)):l.selected=br(t,r)>-1}else l.selected=t.has(r);else if(kn(Ya(l),t)){e.selectedIndex!==a&&(e.selectedIndex=a);return}}!s&&e.selectedIndex!==-1&&(e.selectedIndex=-1)}}function Ya(e){return"_value"in e?e._value:e.value}function oh(e,t){const s=t?"_trueValue":"_falseValue";return s in e?e[s]:t}const ch={created(e,t,s){_l(e,t,s,null,"created")},mounted(e,t,s){_l(e,t,s,null,"mounted")},beforeUpdate(e,t,s,n){_l(e,t,s,n,"beforeUpdate")},updated(e,t,s,n){_l(e,t,s,n,"updated")}};function dh(e,t){switch(e){case"SELECT":return rh;case"TEXTAREA":return Ql;default:switch(t){case"checkbox":return Tc;case"radio":return Cc;default:return Ql}}}function _l(e,t,s,n,a){const l=dh(e.tagName,s.props&&s.props.type)[a];l&&l(e,t,s,n)}function Vy(){Ql.getSSRProps=({value:e})=>({value:e}),Cc.getSSRProps=({value:e},t)=>{if(t.props&&kn(t.props.value,e))return{checked:!0}},Tc.getSSRProps=({value:e},t)=>{if(Ee(e)){if(t.props&&br(e,t.props.value)>-1)return{checked:!0}}else if(ga(e)){if(t.props&&e.has(t.props.value))return{checked:!0}}else if(e)return{checked:!0}},ch.getSSRProps=(e,t)=>{if(typeof t.type!="string")return;const s=dh(t.type.toUpperCase(),t.props&&t.props.type);if(s.getSSRProps)return s.getSSRProps(e,t)}}const qy=["ctrl","shift","alt","meta"],Gy={stop:e=>e.stopPropagation(),prevent:e=>e.preventDefault(),self:e=>e.target!==e.currentTarget,ctrl:e=>!e.ctrlKey,shift:e=>!e.shiftKey,alt:e=>!e.altKey,meta:e=>!e.metaKey,left:e=>"button"in e&&e.button!==0,middle:e=>"button"in e&&e.button!==1,right:e=>"button"in e&&e.button!==2,exact:(e,t)=>qy.some(s=>e[`${s}Key`]&&!t.includes(s))},Ky=(e,t)=>{if(!e)return e;const s=e._withMods||(e._withMods={}),n=t.join(".");return s[n]||(s[n]=((a,...i)=>{for(let l=0;l<t.length;l++){const r=Gy[t[l]];if(r&&r(a,t))return}return e(a,...i)}))},Wy={esc:"escape",space:" ",up:"arrow-up",left:"arrow-left",right:"arrow-right",down:"arrow-down",delete:"backspace"},Zy=(e,t)=>{const s=e._withKeys||(e._withKeys={}),n=t.join(".");return s[n]||(s[n]=(a=>{if(!("key"in a))return;const i=ps(a.key);if(t.some(l=>l===i||Wy[l]===i))return e(a)}))},uh=Ve({patchProp:th},Wf);let Ii,Jd=!1;function ph(){return Ii||(Ii=Tf(uh))}function fh(){return Ii=Jd?Ii:Cf(uh),Jd=!0,Ii}const hh=((...e)=>{ph().render(...e)}),Jy=((...e)=>{fh().hydrate(...e)}),Xl=((...e)=>{const t=ph().createApp(...e),{mount:s}=t;return t.mount=n=>{const a=gh(n);if(!a)return;const i=t._component;!$e(i)&&!i.render&&!i.template&&(i.template=a.innerHTML),a.nodeType===1&&(a.textContent="");const l=s(a,!1,vh(a));return a instanceof Element&&(a.removeAttribute("v-cloak"),a.setAttribute("data-v-app","")),l},t}),mh=((...e)=>{const t=fh().createApp(...e),{mount:s}=t;return t.mount=n=>{const a=gh(n);if(a)return s(a,!0,vh(a))},t});function vh(e){if(e instanceof SVGElement)return"svg";if(typeof MathMLElement=="function"&&e instanceof MathMLElement)return"mathml"}function gh(e){return Be(e)?document.querySelector(e):e}let Yd=!1;const Yy=()=>{Yd||(Yd=!0,Vy(),by())},Qy=Object.freeze(Object.defineProperty({__proto__:null,BaseTransition:tf,BaseTransitionPropsValidators:uc,Comment:St,DeprecationTypes:dy,EffectScope:tc,ErrorCodes:fg,ErrorTypeStrings:ny,Fragment:Pt,KeepAlive:Gg,ReactiveEffect:Pi,Static:ca,Suspense:$b,Teleport:Ag,Text:Un,TrackOpTypes:lg,Transition:hy,TransitionGroup:$y,TriggerOpTypes:rg,VueElement:Ir,assertNumber:pg,callWithAsyncErrorHandling:ys,callWithErrorHandling:li,camelize:rt,capitalize:ba,cloneVNode:sn,compatUtils:cy,computed:W,createApp:Xl,createBlock:Gl,createCommentVNode:Ff,createElementBlock:Vb,createElementVNode:_c,createHydrationRenderer:Cf,createPropsRestProxy:hb,createRenderer:Tf,createSSRApp:mh,createSlots:Xg,createStaticVNode:Kb,createTextVNode:wc,createVNode:mt,customRef:Bp,defineAsyncComponent:Vg,defineComponent:al,defineCustomElement:sh,defineEmits:ab,defineExpose:ib,defineModel:ob,defineOptions:lb,defineProps:nb,defineSSRCustomElement:Ly,defineSlots:rb,devtools:ay,effect:Rv,effectScope:Cv,getCurrentInstance:ls,getCurrentScope:_p,getCurrentWatcher:og,getTransitionRawChildren:Sr,guardReactiveProps:Pf,h:Za,handleError:ya,hasInjectionContext:_g,hydrate:Jy,hydrateOnIdle:$g,hydrateOnInteraction:zg,hydrateOnMediaQuery:Hg,hydrateOnVisible:Ug,initCustomFormatter:ey,initDirectivesForSSR:Yy,inject:Rs,isMemoSame:qf,isProxy:sl,isReactive:xn,isReadonly:tn,isRef:Rt,isRuntimeOnly:Yb,isShallow:hs,isVNode:En,markRaw:Fp,mergeDefaults:pb,mergeModels:fb,mergeProps:$f,nextTick:Et,nodeOps:Wf,normalizeClass:tl,normalizeProps:fv,normalizeStyle:el,onActivated:_s,onBeforeMount:af,onBeforeUnmount:Er,onBeforeUpdate:fc,onDeactivated:ws,onErrorCaptured:cf,onMounted:Ke,onRenderTracked:of,onRenderTriggered:rf,onScopeDispose:Ev,onServerPrefetch:lf,onUnmounted:_t,onUpdated:Cr,onWatcherCleanup:Hp,openBlock:Vi,patchProp:th,popScopeId:bg,provide:Ei,proxyRefs:rc,pushScopeId:gg,queuePostFlushCb:Bi,reactive:jn,readonly:$l,ref:h,registerRuntimeCompiler:zf,render:hh,renderList:Qg,renderSlot:eb,resolveComponent:Zg,resolveDirective:Yg,resolveDynamicComponent:Jg,resolveFilter:oy,resolveTransitionHooks:Wa,setBlockTracking:qi,setDevtoolsHook:iy,setTransitionHooks:Cn,shallowReactive:ic,shallowReadonly:Wv,shallowRef:lc,ssrContextKey:Kp,ssrUtils:ry,stop:Iv,toDisplayString:yp,toHandlerKey:Ba,toHandlers:tb,toRaw:Ze,toRef:ng,toRefs:eg,toValue:Yv,transformVNodeArgs:qb,triggerRef:Jv,unref:en,useAttrs:ub,useCssModule:Dy,useCssVars:yy,useHost:nh,useId:Ig,useModel:kb,useSSRContext:Wp,useShadowRoot:My,useSlots:db,useTemplateRef:Og,useTransitionState:dc,vModelCheckbox:Tc,vModelDynamic:ch,vModelRadio:Cc,vModelSelect:rh,vModelText:Ql,vShow:Xf,version:Gf,warn:sy,watch:is,watchEffect:wg,watchPostEffect:kg,watchSyncEffect:Zp,withAsyncContext:mb,withCtx:cc,withDefaults:cb,withDirectives:xg,withKeys:Zy,withMemo:ty,withModifiers:Ky,withScopeId:yg},Symbol.toStringTag,{value:"Module"}));/**
* @vue/compiler-core v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const Ki=Symbol(""),Oi=Symbol(""),Ec=Symbol(""),er=Symbol(""),bh=Symbol(""),ha=Symbol(""),yh=Symbol(""),xh=Symbol(""),Ac=Symbol(""),Rc=Symbol(""),rl=Symbol(""),Ic=Symbol(""),_h=Symbol(""),Oc=Symbol(""),Lc=Symbol(""),Nc=Symbol(""),Mc=Symbol(""),Dc=Symbol(""),Pc=Symbol(""),wh=Symbol(""),kh=Symbol(""),Or=Symbol(""),tr=Symbol(""),Fc=Symbol(""),$c=Symbol(""),Wi=Symbol(""),ol=Symbol(""),Bc=Symbol(""),Do=Symbol(""),Xy=Symbol(""),Po=Symbol(""),sr=Symbol(""),ex=Symbol(""),tx=Symbol(""),Uc=Symbol(""),sx=Symbol(""),nx=Symbol(""),Hc=Symbol(""),Sh=Symbol(""),Qa={[Ki]:"Fragment",[Oi]:"Teleport",[Ec]:"Suspense",[er]:"KeepAlive",[bh]:"BaseTransition",[ha]:"openBlock",[yh]:"createBlock",[xh]:"createElementBlock",[Ac]:"createVNode",[Rc]:"createElementVNode",[rl]:"createCommentVNode",[Ic]:"createTextVNode",[_h]:"createStaticVNode",[Oc]:"resolveComponent",[Lc]:"resolveDynamicComponent",[Nc]:"resolveDirective",[Mc]:"resolveFilter",[Dc]:"withDirectives",[Pc]:"renderList",[wh]:"renderSlot",[kh]:"createSlots",[Or]:"toDisplayString",[tr]:"mergeProps",[Fc]:"normalizeClass",[$c]:"normalizeStyle",[Wi]:"normalizeProps",[ol]:"guardReactiveProps",[Bc]:"toHandlers",[Do]:"camelize",[Xy]:"capitalize",[Po]:"toHandlerKey",[sr]:"setBlockTracking",[ex]:"pushScopeId",[tx]:"popScopeId",[Uc]:"withCtx",[sx]:"unref",[nx]:"isRef",[Hc]:"withMemo",[Sh]:"isMemoSame"};function ax(e){Object.getOwnPropertySymbols(e).forEach(t=>{Qa[t]=e[t]})}const ks={start:{line:1,column:1,offset:0},end:{line:1,column:1,offset:0},source:""};function ix(e,t=""){return{type:0,source:t,children:e,helpers:new Set,components:[],directives:[],hoists:[],imports:[],cached:[],temps:0,codegenNode:void 0,loc:ks}}function Zi(e,t,s,n,a,i,l,r=!1,o=!1,c=!1,d=ks){return e&&(r?(e.helper(ha),e.helper(ti(e.inSSR,c))):e.helper(ei(e.inSSR,c)),l&&e.helper(Dc)),{type:13,tag:t,props:s,children:n,patchFlag:a,dynamicProps:i,directives:l,isBlock:r,disableTracking:o,isComponent:c,loc:d}}function da(e,t=ks){return{type:17,loc:t,elements:e}}function As(e,t=ks){return{type:15,loc:t,properties:e}}function At(e,t){return{type:16,loc:ks,key:Be(e)?He(e,!0):e,value:t}}function He(e,t=!1,s=ks,n=0){return{type:4,loc:s,content:e,isStatic:t,constType:t?3:n}}function $s(e,t=ks){return{type:8,loc:t,children:e}}function Mt(e,t=[],s=ks){return{type:14,loc:s,callee:e,arguments:t}}function Xa(e,t=void 0,s=!1,n=!1,a=ks){return{type:18,params:e,returns:t,newline:s,isSlot:n,loc:a}}function Fo(e,t,s,n=!0){return{type:19,test:e,consequent:t,alternate:s,newline:n,loc:ks}}function lx(e,t,s=!1,n=!1){return{type:20,index:e,value:t,needPauseTracking:s,inVOnce:n,needArraySpread:!1,loc:ks}}function rx(e){return{type:21,body:e,loc:ks}}function ei(e,t){return e||t?Ac:Rc}function ti(e,t){return e||t?yh:xh}function zc(e,{helper:t,removeHelper:s,inSSR:n}){e.isBlock||(e.isBlock=!0,s(ei(n,e.isComponent)),t(ha),t(ti(n,e.isComponent)))}const Qd=new Uint8Array([123,123]),Xd=new Uint8Array([125,125]);function eu(e){return e>=97&&e<=122||e>=65&&e<=90}function gs(e){return e===32||e===10||e===9||e===12||e===13}function Ln(e){return e===47||e===62||gs(e)}function nr(e){const t=new Uint8Array(e.length);for(let s=0;s<e.length;s++)t[s]=e.charCodeAt(s);return t}const Vt={Cdata:new Uint8Array([67,68,65,84,65,91]),CdataEnd:new Uint8Array([93,93,62]),CommentEnd:new Uint8Array([45,45,62]),ScriptEnd:new Uint8Array([60,47,115,99,114,105,112,116]),StyleEnd:new Uint8Array([60,47,115,116,121,108,101]),TitleEnd:new Uint8Array([60,47,116,105,116,108,101]),TextareaEnd:new Uint8Array([60,47,116,101,120,116,97,114,101,97])};class ox{constructor(t,s){this.stack=t,this.cbs=s,this.state=1,this.buffer="",this.sectionStart=0,this.index=0,this.entityStart=0,this.baseState=1,this.inRCDATA=!1,this.inXML=!1,this.inVPre=!1,this.newlines=[],this.mode=0,this.delimiterOpen=Qd,this.delimiterClose=Xd,this.delimiterIndex=-1,this.currentSequence=void 0,this.sequenceIndex=0}get inSFCRoot(){return this.mode===2&&this.stack.length===0}reset(){this.state=1,this.mode=0,this.buffer="",this.sectionStart=0,this.index=0,this.baseState=1,this.inRCDATA=!1,this.currentSequence=void 0,this.newlines.length=0,this.delimiterOpen=Qd,this.delimiterClose=Xd}getPos(t){let s=1,n=t+1;const a=this.newlines.length;let i=-1;if(a>100){let l=-1,r=a;for(;l+1<r;){const o=l+r>>>1;this.newlines[o]<t?l=o:r=o}i=l}else for(let l=a-1;l>=0;l--)if(t>this.newlines[l]){i=l;break}return i>=0&&(s=i+2,n=t-this.newlines[i]),{column:n,line:s,offset:t}}peek(){return this.buffer.charCodeAt(this.index+1)}stateText(t){t===60?(this.index>this.sectionStart&&this.cbs.ontext(this.sectionStart,this.index),this.state=5,this.sectionStart=this.index):!this.inVPre&&t===this.delimiterOpen[0]&&(this.state=2,this.delimiterIndex=0,this.stateInterpolationOpen(t))}stateInterpolationOpen(t){if(t===this.delimiterOpen[this.delimiterIndex])if(this.delimiterIndex===this.delimiterOpen.length-1){const s=this.index+1-this.delimiterOpen.length;s>this.sectionStart&&this.cbs.ontext(this.sectionStart,s),this.state=3,this.sectionStart=s}else this.delimiterIndex++;else this.inRCDATA?(this.state=32,this.stateInRCDATA(t)):(this.state=1,this.stateText(t))}stateInterpolation(t){t===this.delimiterClose[0]&&(this.state=4,this.delimiterIndex=0,this.stateInterpolationClose(t))}stateInterpolationClose(t){t===this.delimiterClose[this.delimiterIndex]?this.delimiterIndex===this.delimiterClose.length-1?(this.cbs.oninterpolation(this.sectionStart,this.index+1),this.inRCDATA?this.state=32:this.state=1,this.sectionStart=this.index+1):this.delimiterIndex++:(this.state=3,this.stateInterpolation(t))}stateSpecialStartSequence(t){const s=this.sequenceIndex===this.currentSequence.length;if(!(s?Ln(t):(t|32)===this.currentSequence[this.sequenceIndex]))this.inRCDATA=!1;else if(!s){this.sequenceIndex++;return}this.sequenceIndex=0,this.state=6,this.stateInTagName(t)}stateInRCDATA(t){if(this.sequenceIndex===this.currentSequence.length){if(t===62||gs(t)){const s=this.index-this.currentSequence.length;if(this.sectionStart<s){const n=this.index;this.index=s,this.cbs.ontext(this.sectionStart,s),this.index=n}this.sectionStart=s+2,this.stateInClosingTagName(t),this.inRCDATA=!1;return}this.sequenceIndex=0}(t|32)===this.currentSequence[this.sequenceIndex]?this.sequenceIndex+=1:this.sequenceIndex===0?this.currentSequence===Vt.TitleEnd||this.currentSequence===Vt.TextareaEnd&&!this.inSFCRoot?!this.inVPre&&t===this.delimiterOpen[0]&&(this.state=2,this.delimiterIndex=0,this.stateInterpolationOpen(t)):this.fastForwardTo(60)&&(this.sequenceIndex=1):this.sequenceIndex=+(t===60)}stateCDATASequence(t){t===Vt.Cdata[this.sequenceIndex]?++this.sequenceIndex===Vt.Cdata.length&&(this.state=28,this.currentSequence=Vt.CdataEnd,this.sequenceIndex=0,this.sectionStart=this.index+1):(this.sequenceIndex=0,this.state=23,this.stateInDeclaration(t))}fastForwardTo(t){for(;++this.index<this.buffer.length;){const s=this.buffer.charCodeAt(this.index);if(s===10&&this.newlines.push(this.index),s===t)return!0}return this.index=this.buffer.length-1,!1}stateInCommentLike(t){t===this.currentSequence[this.sequenceIndex]?++this.sequenceIndex===this.currentSequence.length&&(this.currentSequence===Vt.CdataEnd?this.cbs.oncdata(this.sectionStart,this.index-2):this.cbs.oncomment(this.sectionStart,this.index-2),this.sequenceIndex=0,this.sectionStart=this.index+1,this.state=1):this.sequenceIndex===0?this.fastForwardTo(this.currentSequence[0])&&(this.sequenceIndex=1):t!==this.currentSequence[this.sequenceIndex-1]&&(this.sequenceIndex=0)}startSpecial(t,s){this.enterRCDATA(t,s),this.state=31}enterRCDATA(t,s){this.inRCDATA=!0,this.currentSequence=t,this.sequenceIndex=s}stateBeforeTagName(t){t===33?(this.state=22,this.sectionStart=this.index+1):t===63?(this.state=24,this.sectionStart=this.index+1):eu(t)?(this.sectionStart=this.index,this.mode===0?this.state=6:this.inSFCRoot?this.state=34:this.inXML?this.state=6:t===116?this.state=30:this.state=t===115?29:6):t===47?this.state=8:(this.state=1,this.stateText(t))}stateInTagName(t){Ln(t)&&this.handleTagName(t)}stateInSFCRootTagName(t){if(Ln(t)){const s=this.buffer.slice(this.sectionStart,this.index);s!=="template"&&this.enterRCDATA(nr("</"+s),0),this.handleTagName(t)}}handleTagName(t){this.cbs.onopentagname(this.sectionStart,this.index),this.sectionStart=-1,this.state=11,this.stateBeforeAttrName(t)}stateBeforeClosingTagName(t){gs(t)||(t===62?(this.state=1,this.sectionStart=this.index+1):(this.state=eu(t)?9:27,this.sectionStart=this.index))}stateInClosingTagName(t){(t===62||gs(t))&&(this.cbs.onclosetag(this.sectionStart,this.index),this.sectionStart=-1,this.state=10,this.stateAfterClosingTagName(t))}stateAfterClosingTagName(t){t===62&&(this.state=1,this.sectionStart=this.index+1)}stateBeforeAttrName(t){t===62?(this.cbs.onopentagend(this.index),this.inRCDATA?this.state=32:this.state=1,this.sectionStart=this.index+1):t===47?this.state=7:t===60&&this.peek()===47?(this.cbs.onopentagend(this.index),this.state=5,this.sectionStart=this.index):gs(t)||this.handleAttrStart(t)}handleAttrStart(t){t===118&&this.peek()===45?(this.state=13,this.sectionStart=this.index):t===46||t===58||t===64||t===35?(this.cbs.ondirname(this.index,this.index+1),this.state=14,this.sectionStart=this.index+1):(this.state=12,this.sectionStart=this.index)}stateInSelfClosingTag(t){t===62?(this.cbs.onselfclosingtag(this.index),this.state=1,this.sectionStart=this.index+1,this.inRCDATA=!1):gs(t)||(this.state=11,this.stateBeforeAttrName(t))}stateInAttrName(t){(t===61||Ln(t))&&(this.cbs.onattribname(this.sectionStart,this.index),this.handleAttrNameEnd(t))}stateInDirName(t){t===61||Ln(t)?(this.cbs.ondirname(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===58?(this.cbs.ondirname(this.sectionStart,this.index),this.state=14,this.sectionStart=this.index+1):t===46&&(this.cbs.ondirname(this.sectionStart,this.index),this.state=16,this.sectionStart=this.index+1)}stateInDirArg(t){t===61||Ln(t)?(this.cbs.ondirarg(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===91?this.state=15:t===46&&(this.cbs.ondirarg(this.sectionStart,this.index),this.state=16,this.sectionStart=this.index+1)}stateInDynamicDirArg(t){t===93?this.state=14:(t===61||Ln(t))&&(this.cbs.ondirarg(this.sectionStart,this.index+1),this.handleAttrNameEnd(t))}stateInDirModifier(t){t===61||Ln(t)?(this.cbs.ondirmodifier(this.sectionStart,this.index),this.handleAttrNameEnd(t)):t===46&&(this.cbs.ondirmodifier(this.sectionStart,this.index),this.sectionStart=this.index+1)}handleAttrNameEnd(t){this.sectionStart=this.index,this.state=17,this.cbs.onattribnameend(this.index),this.stateAfterAttrName(t)}stateAfterAttrName(t){t===61?this.state=18:t===47||t===62?(this.cbs.onattribend(0,this.sectionStart),this.sectionStart=-1,this.state=11,this.stateBeforeAttrName(t)):gs(t)||(this.cbs.onattribend(0,this.sectionStart),this.handleAttrStart(t))}stateBeforeAttrValue(t){t===34?(this.state=19,this.sectionStart=this.index+1):t===39?(this.state=20,this.sectionStart=this.index+1):gs(t)||(this.sectionStart=this.index,this.state=21,this.stateInAttrValueNoQuotes(t))}handleInAttrValue(t,s){(t===s||this.fastForwardTo(s))&&(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=-1,this.cbs.onattribend(s===34?3:2,this.index+1),this.state=11)}stateInAttrValueDoubleQuotes(t){this.handleInAttrValue(t,34)}stateInAttrValueSingleQuotes(t){this.handleInAttrValue(t,39)}stateInAttrValueNoQuotes(t){gs(t)||t===62?(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=-1,this.cbs.onattribend(1,this.index),this.state=11,this.stateBeforeAttrName(t)):(t===39||t===60||t===61||t===96)&&this.cbs.onerr(18,this.index)}stateBeforeDeclaration(t){t===91?(this.state=26,this.sequenceIndex=0):this.state=t===45?25:23}stateInDeclaration(t){(t===62||this.fastForwardTo(62))&&(this.state=1,this.sectionStart=this.index+1)}stateInProcessingInstruction(t){(t===62||this.fastForwardTo(62))&&(this.cbs.onprocessinginstruction(this.sectionStart,this.index),this.state=1,this.sectionStart=this.index+1)}stateBeforeComment(t){t===45?(this.state=28,this.currentSequence=Vt.CommentEnd,this.sequenceIndex=2,this.sectionStart=this.index+1):this.state=23}stateInSpecialComment(t){(t===62||this.fastForwardTo(62))&&(this.cbs.oncomment(this.sectionStart,this.index),this.state=1,this.sectionStart=this.index+1)}stateBeforeSpecialS(t){t===Vt.ScriptEnd[3]?this.startSpecial(Vt.ScriptEnd,4):t===Vt.StyleEnd[3]?this.startSpecial(Vt.StyleEnd,4):(this.state=6,this.stateInTagName(t))}stateBeforeSpecialT(t){t===Vt.TitleEnd[3]?this.startSpecial(Vt.TitleEnd,4):t===Vt.TextareaEnd[3]?this.startSpecial(Vt.TextareaEnd,4):(this.state=6,this.stateInTagName(t))}startEntity(){}stateInEntity(){}parse(t){for(this.buffer=t;this.index<this.buffer.length;){const s=this.buffer.charCodeAt(this.index);switch(s===10&&this.state!==33&&this.newlines.push(this.index),this.state){case 1:{this.stateText(s);break}case 2:{this.stateInterpolationOpen(s);break}case 3:{this.stateInterpolation(s);break}case 4:{this.stateInterpolationClose(s);break}case 31:{this.stateSpecialStartSequence(s);break}case 32:{this.stateInRCDATA(s);break}case 26:{this.stateCDATASequence(s);break}case 19:{this.stateInAttrValueDoubleQuotes(s);break}case 12:{this.stateInAttrName(s);break}case 13:{this.stateInDirName(s);break}case 14:{this.stateInDirArg(s);break}case 15:{this.stateInDynamicDirArg(s);break}case 16:{this.stateInDirModifier(s);break}case 28:{this.stateInCommentLike(s);break}case 27:{this.stateInSpecialComment(s);break}case 11:{this.stateBeforeAttrName(s);break}case 6:{this.stateInTagName(s);break}case 34:{this.stateInSFCRootTagName(s);break}case 9:{this.stateInClosingTagName(s);break}case 5:{this.stateBeforeTagName(s);break}case 17:{this.stateAfterAttrName(s);break}case 20:{this.stateInAttrValueSingleQuotes(s);break}case 18:{this.stateBeforeAttrValue(s);break}case 8:{this.stateBeforeClosingTagName(s);break}case 10:{this.stateAfterClosingTagName(s);break}case 29:{this.stateBeforeSpecialS(s);break}case 30:{this.stateBeforeSpecialT(s);break}case 21:{this.stateInAttrValueNoQuotes(s);break}case 7:{this.stateInSelfClosingTag(s);break}case 23:{this.stateInDeclaration(s);break}case 22:{this.stateBeforeDeclaration(s);break}case 25:{this.stateBeforeComment(s);break}case 24:{this.stateInProcessingInstruction(s);break}case 33:{this.stateInEntity();break}}this.index++}this.cleanup(),this.finish()}cleanup(){this.sectionStart!==this.index&&(this.state===1||this.state===32&&this.sequenceIndex===0?(this.cbs.ontext(this.sectionStart,this.index),this.sectionStart=this.index):(this.state===19||this.state===20||this.state===21)&&(this.cbs.onattribdata(this.sectionStart,this.index),this.sectionStart=this.index))}finish(){this.handleTrailingData(),this.cbs.onend()}handleTrailingData(){const t=this.buffer.length;this.sectionStart>=t||(this.state===28?this.currentSequence===Vt.CdataEnd?this.cbs.oncdata(this.sectionStart,t):this.cbs.oncomment(this.sectionStart,t):this.state===6||this.state===11||this.state===18||this.state===17||this.state===12||this.state===13||this.state===14||this.state===15||this.state===16||this.state===20||this.state===19||this.state===21||this.state===9||this.cbs.ontext(this.sectionStart,t))}emitCodePoint(t,s){}}function tu(e,{compatConfig:t}){const s=t&&t[e];return e==="MODE"?s||3:s}function ua(e,t){const s=tu("MODE",t),n=tu(e,t);return s===3?n===!0:n!==!1}function Ji(e,t,s,...n){return ua(e,t)}function jc(e){throw e}function Th(e){}function ht(e,t,s,n){const a=`https://vuejs.org/error-reference/#compiler-${e}`,i=new SyntaxError(String(a));return i.code=e,i.loc=t,i}const fs=e=>e.type===4&&e.isStatic;function Ch(e){switch(e){case"Teleport":case"teleport":return Oi;case"Suspense":case"suspense":return Ec;case"KeepAlive":case"keep-alive":return er;case"BaseTransition":case"base-transition":return bh}}const cx=/^$|^\d|[^\$\w\xA0-\uFFFF]/,Vc=e=>!cx.test(e),Eh=/[A-Za-z_$\xA0-\uFFFF]/,dx=/[\.\?\w$\xA0-\uFFFF]/,ux=/\s+[.[]\s*|\s*[.[]\s+/g,Ah=e=>e.type===4?e.content:e.loc.source,px=e=>{const t=Ah(e).trim().replace(ux,r=>r.trim());let s=0,n=[],a=0,i=0,l=null;for(let r=0;r<t.length;r++){const o=t.charAt(r);switch(s){case 0:if(o==="[")n.push(s),s=1,a++;else if(o==="(")n.push(s),s=2,i++;else if(!(r===0?Eh:dx).test(o))return!1;break;case 1:o==="'"||o==='"'||o==="`"?(n.push(s),s=3,l=o):o==="["?a++:o==="]"&&(--a||(s=n.pop()));break;case 2:if(o==="'"||o==='"'||o==="`")n.push(s),s=3,l=o;else if(o==="(")i++;else if(o===")"){if(r===t.length-1)return!1;--i||(s=n.pop())}break;case 3:o===l&&(s=n.pop(),l=null);break}}return!a&&!i},Rh=px,fx=/^\s*(?:async\s*)?(?:\([^)]*?\)|[\w$_]+)\s*(?::[^=]+)?=>|^\s*(?:async\s+)?function(?:\s+[\w$]+)?\s*\(/,hx=e=>fx.test(Ah(e)),mx=hx;function Es(e,t,s=!1){for(let n=0;n<e.props.length;n++){const a=e.props[n];if(a.type===7&&(s||a.exp)&&(Be(t)?a.name===t:t.test(a.name)))return a}}function Lr(e,t,s=!1,n=!1){for(let a=0;a<e.props.length;a++){const i=e.props[a];if(i.type===6){if(s)continue;if(i.name===t&&(i.value||n))return i}else if(i.name==="bind"&&(i.exp||n)&&aa(i.arg,t))return i}}function aa(e,t){return!!(e&&fs(e)&&e.content===t)}function vx(e){return e.props.some(t=>t.type===7&&t.name==="bind"&&(!t.arg||t.arg.type!==4||!t.arg.isStatic))}function Xr(e){return e.type===5||e.type===2}function su(e){return e.type===7&&e.name==="pre"}function gx(e){return e.type===7&&e.name==="slot"}function ar(e){return e.type===1&&e.tagType===3}function ir(e){return e.type===1&&e.tagType===2}const bx=new Set([Wi,ol]);function Ih(e,t=[]){if(e&&!Be(e)&&e.type===14){const s=e.callee;if(!Be(s)&&bx.has(s))return Ih(e.arguments[0],t.concat(e))}return[e,t]}function lr(e,t,s){let n,a=e.type===13?e.props:e.arguments[2],i=[],l;if(a&&!Be(a)&&a.type===14){const r=Ih(a);a=r[0],i=r[1],l=i[i.length-1]}if(a==null||Be(a))n=As([t]);else if(a.type===14){const r=a.arguments[0];!Be(r)&&r.type===15?nu(t,r)||r.properties.unshift(t):a.callee===Bc?n=Mt(s.helper(tr),[As([t]),a]):a.arguments.unshift(As([t])),!n&&(n=a)}else a.type===15?(nu(t,a)||a.properties.unshift(t),n=a):(n=Mt(s.helper(tr),[As([t]),a]),l&&l.callee===ol&&(l=i[i.length-2]));e.type===13?l?l.arguments[0]=n:e.props=n:l?l.arguments[0]=n:e.arguments[2]=n}function nu(e,t){let s=!1;if(e.key.type===4){const n=e.key.content;s=t.properties.some(a=>a.key.type===4&&a.key.content===n)}return s}function Yi(e,t){return`_${t}_${e.replace(/[^\w]/g,(s,n)=>s==="-"?"_":e.charCodeAt(n).toString())}`}function yx(e){return e.type===14&&e.callee===Hc?e.arguments[1].returns:e}const xx=/([\s\S]*?)\s+(?:in|of)\s+(\S[\s\S]*)/;function Oh(e){for(let t=0;t<e.length;t++)if(!gs(e.charCodeAt(t)))return!1;return!0}function qc(e){return e.type===2&&Oh(e.content)||e.type===12&&qc(e.content)}function Lh(e){return e.type===3||qc(e)}const Nh={parseMode:"base",ns:0,delimiters:["{{","}}"],getNamespace:()=>0,isVoidTag:Da,isPreTag:Da,isIgnoreNewlineTag:Da,isCustomElement:Da,onError:jc,onWarn:Th,comments:!1,prefixIdentifiers:!1};let Xe=Nh,Qi=null,wn="",Gt=null,Ge=null,os="",dn=-1,ta=-1,Gc=0,$n=!1,$o=null;const ft=[],xt=new ox(ft,{onerr:rn,ontext(e,t){wl(Bt(e,t),e,t)},ontextentity(e,t,s){wl(e,t,s)},oninterpolation(e,t){if($n)return wl(Bt(e,t),e,t);let s=e+xt.delimiterOpen.length,n=t-xt.delimiterClose.length;for(;gs(wn.charCodeAt(s));)s++;for(;gs(wn.charCodeAt(n-1));)n--;let a=Bt(s,n);a.includes("&")&&(a=Xe.decodeEntities(a,!1)),Bo({type:5,content:Ll(a,!1,kt(s,n)),loc:kt(e,t)})},onopentagname(e,t){const s=Bt(e,t);Gt={type:1,tag:s,ns:Xe.getNamespace(s,ft[0],Xe.ns),tagType:0,props:[],children:[],loc:kt(e-1,t),codegenNode:void 0}},onopentagend(e){iu(e)},onclosetag(e,t){const s=Bt(e,t);if(!Xe.isVoidTag(s)){let n=!1;for(let a=0;a<ft.length;a++)if(ft[a].tag.toLowerCase()===s.toLowerCase()){n=!0,a>0&&rn(24,ft[0].loc.start.offset);for(let l=0;l<=a;l++){const r=ft.shift();Ol(r,t,l<a)}break}n||rn(23,Mh(e,60))}},onselfclosingtag(e){const t=Gt.tag;Gt.isSelfClosing=!0,iu(e),ft[0]&&ft[0].tag===t&&Ol(ft.shift(),e)},onattribname(e,t){Ge={type:6,name:Bt(e,t),nameLoc:kt(e,t),value:void 0,loc:kt(e)}},ondirname(e,t){const s=Bt(e,t),n=s==="."||s===":"?"bind":s==="@"?"on":s==="#"?"slot":s.slice(2);if(!$n&&n===""&&rn(26,e),$n||n==="")Ge={type:6,name:s,nameLoc:kt(e,t),value:void 0,loc:kt(e)};else if(Ge={type:7,name:n,rawName:s,exp:void 0,arg:void 0,modifiers:s==="."?[He("prop")]:[],loc:kt(e)},n==="pre"){$n=xt.inVPre=!0,$o=Gt;const a=Gt.props;for(let i=0;i<a.length;i++)a[i].type===7&&(a[i]=Ix(a[i]))}},ondirarg(e,t){if(e===t)return;const s=Bt(e,t);if($n&&!su(Ge))Ge.name+=s,ia(Ge.nameLoc,t);else{const n=s[0]!=="[";Ge.arg=Ll(n?s:s.slice(1,-1),n,kt(e,t),n?3:0)}},ondirmodifier(e,t){const s=Bt(e,t);if($n&&!su(Ge))Ge.name+="."+s,ia(Ge.nameLoc,t);else if(Ge.name==="slot"){const n=Ge.arg;n&&(n.content+="."+s,ia(n.loc,t))}else{const n=He(s,!0,kt(e,t));Ge.modifiers.push(n)}},onattribdata(e,t){os+=Bt(e,t),dn<0&&(dn=e),ta=t},onattribentity(e,t,s){os+=e,dn<0&&(dn=t),ta=s},onattribnameend(e){const t=Ge.loc.start.offset,s=Bt(t,e);Ge.type===7&&(Ge.rawName=s),Gt.props.some(n=>(n.type===7?n.rawName:n.name)===s)&&rn(2,t)},onattribend(e,t){if(Gt&&Ge){if(ia(Ge.loc,t),e!==0)if(os.includes("&")&&(os=Xe.decodeEntities(os,!0)),Ge.type===6)Ge.name==="class"&&(os=Ph(os).trim()),e===1&&!os&&rn(13,t),Ge.value={type:2,content:os,loc:e===1?kt(dn,ta):kt(dn-1,ta+1)},xt.inSFCRoot&&Gt.tag==="template"&&Ge.name==="lang"&&os&&os!=="html"&&xt.enterRCDATA(nr("</template"),0);else{let s=0;Ge.exp=Ll(os,!1,kt(dn,ta),0,s),Ge.name==="for"&&(Ge.forParseResult=wx(Ge.exp));let n=-1;Ge.name==="bind"&&(n=Ge.modifiers.findIndex(a=>a.content==="sync"))>-1&&Ji("COMPILER_V_BIND_SYNC",Xe,Ge.loc,Ge.arg.loc.source)&&(Ge.name="model",Ge.modifiers.splice(n,1))}(Ge.type!==7||Ge.name!=="pre")&&Gt.props.push(Ge)}os="",dn=ta=-1},oncomment(e,t){Xe.comments&&Bo({type:3,content:Bt(e,t),loc:kt(e-4,t+3)})},onend(){const e=wn.length;for(let t=0;t<ft.length;t++)Ol(ft[t],e-1),rn(24,ft[t].loc.start.offset)},oncdata(e,t){(ft[0]?ft[0].ns:Xe.ns)!==0?wl(Bt(e,t),e,t):rn(1,e-9)},onprocessinginstruction(e){(ft[0]?ft[0].ns:Xe.ns)===0&&rn(21,e-1)}}),au=/,([^,\}\]]*)(?:,([^,\}\]]*))?$/,_x=/^\(|\)$/g;function wx(e){const t=e.loc,s=e.content,n=s.match(xx);if(!n)return;const[,a,i]=n,l=(u,p,f=!1)=>{const m=t.start.offset+p,b=m+u.length;return Ll(u,!1,kt(m,b),0,f?1:0)},r={source:l(i.trim(),s.indexOf(i,a.length)),value:void 0,key:void 0,index:void 0,finalized:!1};let o=a.trim().replace(_x,"").trim();const c=a.indexOf(o),d=o.match(au);if(d){o=o.replace(au,"").trim();const u=d[1].trim();let p;if(u&&(p=s.indexOf(u,c+o.length),r.key=l(u,p,!0)),d[2]){const f=d[2].trim();f&&(r.index=l(f,s.indexOf(f,r.key?p+u.length:c+o.length),!0))}}return o&&(r.value=l(o,c,!0)),r}function Bt(e,t){return wn.slice(e,t)}function iu(e){xt.inSFCRoot&&(Gt.innerLoc=kt(e+1,e+1)),Bo(Gt);const{tag:t,ns:s}=Gt;s===0&&Xe.isPreTag(t)&&Gc++,Xe.isVoidTag(t)?Ol(Gt,e):(ft.unshift(Gt),(s===1||s===2)&&(xt.inXML=!0)),Gt=null}function wl(e,t,s){{const i=ft[0]&&ft[0].tag;i!=="script"&&i!=="style"&&e.includes("&")&&(e=Xe.decodeEntities(e,!1))}const n=ft[0]||Qi,a=n.children[n.children.length-1];a&&a.type===2?(a.content+=e,ia(a.loc,s)):n.children.push({type:2,content:e,loc:kt(t,s)})}function Ol(e,t,s=!1){s?ia(e.loc,Mh(t,60)):ia(e.loc,kx(t,62)+1),xt.inSFCRoot&&(e.children.length?e.innerLoc.end=Ve({},e.children[e.children.length-1].loc.end):e.innerLoc.end=Ve({},e.innerLoc.start),e.innerLoc.source=Bt(e.innerLoc.start.offset,e.innerLoc.end.offset));const{tag:n,ns:a,children:i}=e;if($n||(n==="slot"?e.tagType=2:lu(e)?e.tagType=3:Tx(e)&&(e.tagType=1)),xt.inRCDATA||(e.children=Dh(i)),a===0&&Xe.isIgnoreNewlineTag(n)){const l=i[0];l&&l.type===2&&(l.content=l.content.replace(/^\r?\n/,""))}a===0&&Xe.isPreTag(n)&&Gc--,$o===e&&($n=xt.inVPre=!1,$o=null),xt.inXML&&(ft[0]?ft[0].ns:Xe.ns)===0&&(xt.inXML=!1);{const l=e.props;if(!xt.inSFCRoot&&ua("COMPILER_NATIVE_TEMPLATE",Xe)&&e.tag==="template"&&!lu(e)){const o=ft[0]||Qi,c=o.children.indexOf(e);o.children.splice(c,1,...e.children)}const r=l.find(o=>o.type===6&&o.name==="inline-template");r&&Ji("COMPILER_INLINE_TEMPLATE",Xe,r.loc)&&e.children.length&&(r.value={type:2,content:Bt(e.children[0].loc.start.offset,e.children[e.children.length-1].loc.end.offset),loc:r.loc})}}function kx(e,t){let s=e;for(;wn.charCodeAt(s)!==t&&s<wn.length-1;)s++;return s}function Mh(e,t){let s=e;for(;wn.charCodeAt(s)!==t&&s>=0;)s--;return s}const Sx=new Set(["if","else","else-if","for","slot"]);function lu({tag:e,props:t}){if(e==="template"){for(let s=0;s<t.length;s++)if(t[s].type===7&&Sx.has(t[s].name))return!0}return!1}function Tx({tag:e,props:t}){if(Xe.isCustomElement(e))return!1;if(e==="component"||Cx(e.charCodeAt(0))||Ch(e)||Xe.isBuiltInComponent&&Xe.isBuiltInComponent(e)||Xe.isNativeTag&&!Xe.isNativeTag(e))return!0;for(let s=0;s<t.length;s++){const n=t[s];if(n.type===6){if(n.name==="is"&&n.value){if(n.value.content.startsWith("vue:"))return!0;if(Ji("COMPILER_IS_ON_ELEMENT",Xe,n.loc))return!0}}else if(n.name==="bind"&&aa(n.arg,"is")&&Ji("COMPILER_IS_ON_ELEMENT",Xe,n.loc))return!0}return!1}function Cx(e){return e>64&&e<91}const Ex=/\r\n/g;function Dh(e){const t=Xe.whitespace!=="preserve";let s=!1;for(let n=0;n<e.length;n++){const a=e[n];if(a.type===2)if(Gc)a.content=a.content.replace(Ex,`
`);else if(Oh(a.content)){const i=e[n-1]&&e[n-1].type,l=e[n+1]&&e[n+1].type;!i||!l||t&&(i===3&&(l===3||l===1)||i===1&&(l===3||l===1&&Ax(a.content)))?(s=!0,e[n]=null):a.content=" "}else t&&(a.content=Ph(a.content))}return s?e.filter(Boolean):e}function Ax(e){for(let t=0;t<e.length;t++){const s=e.charCodeAt(t);if(s===10||s===13)return!0}return!1}function Ph(e){let t="",s=!1;for(let n=0;n<e.length;n++)gs(e.charCodeAt(n))?s||(t+=" ",s=!0):(t+=e[n],s=!1);return t}function Bo(e){(ft[0]||Qi).children.push(e)}function kt(e,t){return{start:xt.getPos(e),end:t==null?t:xt.getPos(t),source:t==null?t:Bt(e,t)}}function Rx(e){return kt(e.start.offset,e.end.offset)}function ia(e,t){e.end=xt.getPos(t),e.source=Bt(e.start.offset,t)}function Ix(e){const t={type:6,name:e.rawName,nameLoc:kt(e.loc.start.offset,e.loc.start.offset+e.rawName.length),value:void 0,loc:e.loc};if(e.exp){const s=e.exp.loc;s.end.offset<e.loc.end.offset&&(s.start.offset--,s.start.column--,s.end.offset++,s.end.column++),t.value={type:2,content:e.exp.content,loc:s}}return t}function Ll(e,t=!1,s,n=0,a=0){return He(e,t,s,n)}function rn(e,t,s){Xe.onError(ht(e,kt(t,t)))}function Ox(){xt.reset(),Gt=null,Ge=null,os="",dn=-1,ta=-1,ft.length=0}function Lx(e,t){if(Ox(),wn=e,Xe=Ve({},Nh),t){let a;for(a in t)t[a]!=null&&(Xe[a]=t[a])}xt.mode=Xe.parseMode==="html"?1:Xe.parseMode==="sfc"?2:0,xt.inXML=Xe.ns===1||Xe.ns===2;const s=t&&t.delimiters;s&&(xt.delimiterOpen=nr(s[0]),xt.delimiterClose=nr(s[1]));const n=Qi=ix([],e);return xt.parse(wn),n.loc=kt(0,e.length),n.children=Dh(n.children),Qi=null,n}function Nx(e,t){Nl(e,void 0,t,!!Fh(e))}function Fh(e){const t=e.children.filter(s=>s.type!==3);return t.length===1&&t[0].type===1&&!ir(t[0])?t[0]:null}function Nl(e,t,s,n=!1,a=!1){const{children:i}=e,l=[];for(let d=0;d<i.length;d++){const u=i[d];if(u.type===1&&u.tagType===0){const p=n?0:bs(u,s);if(p>0){if(p>=2){u.codegenNode.patchFlag=-1,l.push(u);continue}}else{const f=u.codegenNode;if(f.type===13){const m=f.patchFlag;if((m===void 0||m===512||m===1)&&Bh(u,s)>=2){const b=Uh(u);b&&(f.props=s.hoist(b))}f.dynamicProps&&(f.dynamicProps=s.hoist(f.dynamicProps))}}}else if(u.type===12&&(n?0:bs(u,s))>=2){u.codegenNode.type===14&&u.codegenNode.arguments.length>0&&u.codegenNode.arguments.push("-1"),l.push(u);continue}if(u.type===1){const p=u.tagType===1;p&&s.scopes.vSlot++,Nl(u,e,s,!1,a),p&&s.scopes.vSlot--}else if(u.type===11)Nl(u,e,s,u.children.length===1,!0);else if(u.type===9)for(let p=0;p<u.branches.length;p++)Nl(u.branches[p],e,s,u.branches[p].children.length===1,a)}let r=!1;if(l.length===i.length&&e.type===1){if(e.tagType===0&&e.codegenNode&&e.codegenNode.type===13&&Ee(e.codegenNode.children))e.codegenNode.children=o(da(e.codegenNode.children)),r=!0;else if(e.tagType===1&&e.codegenNode&&e.codegenNode.type===13&&e.codegenNode.children&&!Ee(e.codegenNode.children)&&e.codegenNode.children.type===15){const d=c(e.codegenNode,"default");d&&(d.returns=o(da(d.returns)),r=!0)}else if(e.tagType===3&&t&&t.type===1&&t.tagType===1&&t.codegenNode&&t.codegenNode.type===13&&t.codegenNode.children&&!Ee(t.codegenNode.children)&&t.codegenNode.children.type===15){const d=Es(e,"slot",!0),u=d&&d.arg&&c(t.codegenNode,d.arg);u&&(u.returns=o(da(u.returns)),r=!0)}}if(!r)for(const d of l)d.codegenNode=s.cache(d.codegenNode);function o(d){const u=s.cache(d);return u.needArraySpread=!0,u}function c(d,u){if(d.children&&!Ee(d.children)&&d.children.type===15){const p=d.children.properties.find(f=>f.key===u||f.key.content===u);return p&&p.value}}l.length&&s.transformHoist&&s.transformHoist(i,s,e)}function bs(e,t){const{constantCache:s}=t;switch(e.type){case 1:if(e.tagType!==0)return 0;const n=s.get(e);if(n!==void 0)return n;const a=e.codegenNode;if(a.type!==13||a.isBlock&&e.tag!=="svg"&&e.tag!=="foreignObject"&&e.tag!=="math")return 0;if(a.patchFlag===void 0){let l=3;const r=Bh(e,t);if(r===0)return s.set(e,0),0;r<l&&(l=r);for(let o=0;o<e.children.length;o++){const c=bs(e.children[o],t);if(c===0)return s.set(e,0),0;c<l&&(l=c)}if(l>1)for(let o=0;o<e.props.length;o++){const c=e.props[o];if(c.type===7&&c.name==="bind"&&c.exp){const d=bs(c.exp,t);if(d===0)return s.set(e,0),0;d<l&&(l=d)}}if(a.isBlock){for(let o=0;o<e.props.length;o++)if(e.props[o].type===7)return s.set(e,0),0;t.removeHelper(ha),t.removeHelper(ti(t.inSSR,a.isComponent)),a.isBlock=!1,t.helper(ei(t.inSSR,a.isComponent))}return s.set(e,l),l}else return s.set(e,0),0;case 2:case 3:return 3;case 9:case 11:case 10:return 0;case 5:case 12:return bs(e.content,t);case 4:return e.constType;case 8:let i=3;for(let l=0;l<e.children.length;l++){const r=e.children[l];if(Be(r)||Yt(r))continue;const o=bs(r,t);if(o===0)return 0;o<i&&(i=o)}return i;case 20:return 2;default:return 0}}const Mx=new Set([Fc,$c,Wi,ol]);function $h(e,t){if(e.type===14&&!Be(e.callee)&&Mx.has(e.callee)){const s=e.arguments[0];if(s.type===4)return bs(s,t);if(s.type===14)return $h(s,t)}return 0}function Bh(e,t){let s=3;const n=Uh(e);if(n&&n.type===15){const{properties:a}=n;for(let i=0;i<a.length;i++){const{key:l,value:r}=a[i],o=bs(l,t);if(o===0)return o;o<s&&(s=o);let c;if(r.type===4?c=bs(r,t):r.type===14?c=$h(r,t):c=0,c===0)return c;c<s&&(s=c)}}return s}function Uh(e){const t=e.codegenNode;if(t.type===13)return t.props}function Dx(e,{filename:t="",prefixIdentifiers:s=!1,hoistStatic:n=!1,hmr:a=!1,cacheHandlers:i=!1,nodeTransforms:l=[],directiveTransforms:r={},transformHoist:o=null,isBuiltInComponent:c=zt,isCustomElement:d=zt,expressionPlugins:u=[],scopeId:p=null,slotted:f=!0,ssr:m=!1,inSSR:b=!1,ssrCssVars:E="",bindingMetadata:I=qe,inline:w=!1,isTS:g=!1,onError:_=jc,onWarn:C=Th,compatConfig:v}){const S=t.replace(/\?.*$/,"").match(/([^/\\]+)\.\w+$/),T={filename:t,selfName:S&&ba(rt(S[1])),prefixIdentifiers:s,hoistStatic:n,hmr:a,cacheHandlers:i,nodeTransforms:l,directiveTransforms:r,transformHoist:o,isBuiltInComponent:c,isCustomElement:d,expressionPlugins:u,scopeId:p,slotted:f,ssr:m,inSSR:b,ssrCssVars:E,bindingMetadata:I,inline:w,isTS:g,onError:_,onWarn:C,compatConfig:v,root:e,helpers:new Map,components:new Set,directives:new Set,hoists:[],imports:[],cached:[],constantCache:new WeakMap,vForMemoKeyedNodes:new WeakSet,temps:0,identifiers:Object.create(null),scopes:{vFor:0,vSlot:0,vPre:0,vOnce:0},parent:null,grandParent:null,currentNode:e,childIndex:0,inVOnce:!1,helper(y){const O=T.helpers.get(y)||0;return T.helpers.set(y,O+1),y},removeHelper(y){const O=T.helpers.get(y);if(O){const $=O-1;$?T.helpers.set(y,$):T.helpers.delete(y)}},helperString(y){return`_${Qa[T.helper(y)]}`},replaceNode(y){T.parent.children[T.childIndex]=T.currentNode=y},removeNode(y){const O=T.parent.children,$=y?O.indexOf(y):T.currentNode?T.childIndex:-1;!y||y===T.currentNode?(T.currentNode=null,T.onNodeRemoved()):T.childIndex>$&&(T.childIndex--,T.onNodeRemoved()),T.parent.children.splice($,1)},onNodeRemoved:zt,addIdentifiers(y){},removeIdentifiers(y){},hoist(y){Be(y)&&(y=He(y)),T.hoists.push(y);const O=He(`_hoisted_${T.hoists.length}`,!1,y.loc,2);return O.hoisted=y,O},cache(y,O=!1,$=!1){const F=lx(T.cached.length,y,O,$);return T.cached.push(F),F}};return T.filters=new Set,T}function Px(e,t){const s=Dx(e,t);Nr(e,s),t.hoistStatic&&Nx(e,s),t.ssr||Fx(e,s),e.helpers=new Set([...s.helpers.keys()]),e.components=[...s.components],e.directives=[...s.directives],e.imports=s.imports,e.hoists=s.hoists,e.temps=s.temps,e.cached=s.cached,e.transformed=!0,e.filters=[...s.filters]}function Fx(e,t){const{helper:s}=t,{children:n}=e;if(n.length===1){const a=Fh(e);if(a&&a.codegenNode){const i=a.codegenNode;i.type===13&&zc(i,t),e.codegenNode=i}else e.codegenNode=n[0]}else if(n.length>1){let a=64;e.codegenNode=Zi(t,s(Ki),void 0,e.children,a,void 0,void 0,!0,void 0,!1)}}function $x(e,t){let s=0;const n=()=>{s--};for(;s<e.children.length;s++){const a=e.children[s];Be(a)||(t.grandParent=t.parent,t.parent=e,t.childIndex=s,t.onNodeRemoved=n,Nr(a,t))}}function Nr(e,t){t.currentNode=e;const{nodeTransforms:s}=t,n=[];for(let i=0;i<s.length;i++){const l=s[i](e,t);if(l&&(Ee(l)?n.push(...l):n.push(l)),t.currentNode)e=t.currentNode;else return}switch(e.type){case 3:t.ssr||t.helper(rl);break;case 5:t.ssr||t.helper(Or);break;case 9:for(let i=0;i<e.branches.length;i++)Nr(e.branches[i],t);break;case 10:case 11:case 1:case 0:$x(e,t);break}t.currentNode=e;let a=n.length;for(;a--;)n[a]()}function Hh(e,t){const s=Be(e)?n=>n===e:n=>e.test(n);return(n,a)=>{if(n.type===1){const{props:i}=n;if(n.tagType===3&&i.some(gx))return;const l=[];for(let r=0;r<i.length;r++){const o=i[r];if(o.type===7&&s(o.name)){i.splice(r,1),r--;const c=t(n,o,a);c&&l.push(c)}}return l}}}const Mr="/*@__PURE__*/",zh=e=>`${Qa[e]}: _${Qa[e]}`;function Bx(e,{mode:t="function",prefixIdentifiers:s=t==="module",sourceMap:n=!1,filename:a="template.vue.html",scopeId:i=null,optimizeImports:l=!1,runtimeGlobalName:r="Vue",runtimeModuleName:o="vue",ssrRuntimeModuleName:c="vue/server-renderer",ssr:d=!1,isTS:u=!1,inSSR:p=!1}){const f={mode:t,prefixIdentifiers:s,sourceMap:n,filename:a,scopeId:i,optimizeImports:l,runtimeGlobalName:r,runtimeModuleName:o,ssrRuntimeModuleName:c,ssr:d,isTS:u,inSSR:p,source:e.source,code:"",column:1,line:1,offset:0,indentLevel:0,pure:!1,map:void 0,helper(b){return`_${Qa[b]}`},push(b,E=-2,I){f.code+=b},indent(){m(++f.indentLevel)},deindent(b=!1){b?--f.indentLevel:m(--f.indentLevel)},newline(){m(f.indentLevel)}};function m(b){f.push(`
`+"  ".repeat(b),0)}return f}function Ux(e,t={}){const s=Bx(e,t);t.onContextCreated&&t.onContextCreated(s);const{mode:n,push:a,prefixIdentifiers:i,indent:l,deindent:r,newline:o,scopeId:c,ssr:d}=s,u=Array.from(e.helpers),p=u.length>0,f=!i&&n!=="module";Hx(e,s);const b=d?"ssrRender":"render",I=(d?["_ctx","_push","_parent","_attrs"]:["_ctx","_cache"]).join(", ");if(a(`function ${b}(${I}) {`),l(),f&&(a("with (_ctx) {"),l(),p&&(a(`const { ${u.map(zh).join(", ")} } = _Vue
...
`,-1)}zx(e.hoists,t),i(),a("return ")}function eo(e,t,{helper:s,push:n,newline:a,isTS:i}){const l=s(t==="filter"?Mc:t==="component"?Oc:Nc);for(let r=0;r<e.length;r++){let o=e[r];const c=o.endsWith("__self");c&&(o=o.slice(0,-6)),n(`const ${Yi(o,t)} = ${l}(${JSON.stringify(o)}${c?", true":""})${i?"!":""}`),r<e.length-1&&a()}}function zx(e,t){if(!e.length)return;t.pure=!0;const{push:s,newline:n}=t;n();for(let a=0;a<e.length;a++){const i=e[a];i&&(s(`const _hoisted_${a+1} = `),Zt(i,t),n())}t.pure=!1}function Kc(e,t){const s=e.length>3||!1;t.push("["),s&&t.indent(),cl(e,t,s),s&&t.deindent(),t.push("]")}function cl(e,t,s=!1,n=!0){const{push:a,newline:i}=t;for(let l=0;l<e.length;l++){const r=e[l];Be(r)?a(r,-3):Ee(r)?Kc(r,t):Zt(r,t),l<e.length-1&&(s?(n&&a(","),i()):n&&a(", "))}}function Zt(e,t){if(Be(e)){t.push(e,-3);return}if(Yt(e)){t.push(t.helper(e));return}switch(e.type){case 1:case 9:case 11:Zt(e.codegenNode,t);break;case 2:jx(e,t);break;case 4:jh(e,t);break;case 5:Vx(e,t);break;case 12:Zt(e.codegenNode,t);break;case 8:Vh(e,t);break;case 3:Gx(e,t);break;case 13:Kx(e,t);break;case 14:Zx(e,t);break;case 15:Jx(e,t);break;case 17:Yx(e,t);break;case 18:Qx(e,t);break;case 19:Xx(e,t);break;case 20:e0(e,t);break;case 21:cl(e.body,t,!0,!1);break}}function jx(e,t){t.push(JSON.stringify(e.content),-3,e)}function jh(e,t){const{content:s,isStatic:n}=e;t.push(n?JSON.stringify(s):s,-3,e)}function Vx(e,t){const{push:s,helper:n,pure:a}=t;a&&s(Mr),s(`${n(Or)}(`),Zt(e.content,t),s(")")}function Vh(e,t){for(let s=0;s<e.children.length;s++){const n=e.children[s];Be(n)?t.push(n,-3):Zt(n,t)}}function qx(e,t){const{push:s}=t;if(e.type===8)s("["),Vh(e,t),s("]");else if(e.isStatic){const n=Vc(e.content)?e.content:JSON.stringify(e.content);s(n,-2,e)}else s(`[${e.content}]`,-3,e)}function Gx(e,t){const{push:s,helper:n,pure:a}=t;a&&s(Mr),s(`${n(rl)}(${JSON.stringify(e.content)})`,-3,e)}function Kx(e,t){const{push:s,helper:n,pure:a}=t,{tag:i,props:l,children:r,patchFlag:o,dynamicProps:c,directives:d,isBlock:u,disableTracking:p,isComponent:f}=e;let m;o&&(m=String(o)),d&&s(n(Dc)+"("),u&&s(`(${n(ha)}(${p?"true":""}), `),a&&s(Mr);const b=u?ti(t.inSSR,f):ei(t.inSSR,f);s(n(b)+"(",-2,e),cl(Wx([i,l,r,m,c]),t),s(")"),u&&s(")"),d&&(s(", "),Zt(d,t),s(")"))}function Wx(e){let t=e.length;for(;t--&&e[t]==null;);return e.slice(0,t+1).map(s=>s||"null")}function Zx(e,t){const{push:s,helper:n,pure:a}=t,i=Be(e.callee)?e.callee:n(e.callee);a&&s(Mr),s(i+"(",-2,e),cl(e.arguments,t),s(")")}function Jx(e,t){const{push:s,indent:n,deindent:a,newline:i}=t,{properties:l}=e;if(!l.length){s("{}",-2,e);return}const r=l.length>1||!1;s(r?"{":"{ "),r&&n();for(let o=0;o<l.length;o++){const{key:c,value:d}=l[o];qx(c,t),s(": "),Zt(d,t),o<l.length-1&&(s(","),i())}r&&a(),s(r?"}":" }")}function Yx(e,t){Kc(e.elements,t)}function Qx(e,t){const{push:s,indent:n,deindent:a}=t,{params:i,returns:l,body:r,newline:o,isSlot:c}=e;c&&s(`_${Qa[Uc]}(`),s("(",-2,e),Ee(i)?cl(i,t):i&&Zt(i,t),s(") => "),(o||r)&&(s("{"),n()),l?(o&&s("return "),Ee(l)?Kc(l,t):Zt(l,t)):r&&Zt(r,t),(o||r)&&(a(),s("}")),c&&(e.isNonScopedSlot&&s(", undefined, true"),s(")"))}function Xx(e,t){const{test:s,consequent:n,alternate:a,newline:i}=e,{push:l,indent:r,deindent:o,newline:c}=t;if(s.type===4){const u=!Vc(s.content);u&&l("("),jh(s,t),u&&l(")")}else l("("),Zt(s,t),l(")");i&&r(),t.indentLevel++,i||l(" "),l("? "),Zt(n,t),t.indentLevel--,i&&c(),i||l(" "),l(": ");const d=a.type===19;d||t.indentLevel++,Zt(a,t),d||t.indentLevel--,i&&o(!0)}function e0(e,t){const{push:s,helper:n,indent:a,deindent:i,newline:l}=t,{needPauseTracking:r,needArraySpread:o}=e;o&&s("[...("),s(`_cache[${e.index}] || (`),r&&(a(),s(`${n(sr)}(-1`),e.inVOnce&&s(", true"),s("),"),l(),s("(")),s(`_cache[${e.index}] = `),Zt(e.value,t),r&&(s(`).cacheIndex = ${e.index},`),l(),s(`${n(sr)}(1),`),l(),s(`_cache[${e.index}]`),i()),s(")"),o&&s(")]")}new RegExp("\\b"+"arguments,await,break,case,catch,class,const,continue,debugger,default,delete,do,else,export,extends,finally,for,function,if,import,let,new,return,super,switch,throw,try,var,void,while,with,yield".split(",").join("\\b|\\b")+"\\b");const t0=Hh(/^(?:if|else|else-if)$/,(e,t,s)=>s0(e,t,s,(n,a,i)=>{const l=s.parent.children;let r=l.indexOf(n),o=0;for(;r-->=0;){const c=l[r];c&&c.type===9&&(o+=c.branches.length)}return()=>{if(i)n.codegenNode=ou(a,o,s);else{const c=n0(n.codegenNode);c.alternate=ou(a,o+n.branches.length-1,s)}}}));function s0(e,t,s,n){if(t.name!=="else"&&(!t.exp||!t.exp.content.trim())){const a=t.exp?t.exp.loc:e.loc;s.onError(ht(28,t.loc)),t.exp=He("true",!1,a)}if(t.name==="if"){const a=ru(e,t),i={type:9,loc:Rx(e.loc),branches:[a]};if(s.replaceNode(i),n)return n(i,a,!0)}else{const a=s.parent.children;let i=a.indexOf(e);for(;i-->=-1;){const l=a[i];if(l&&Lh(l)){s.removeNode(l);continue}if(l&&l.type===9){(t.name==="else-if"||t.name==="else")&&l.branches[l.branches.length-1].condition===void 0&&s.onError(ht(30,e.loc)),s.removeNode();const r=ru(e,t);l.branches.push(r);const o=n&&n(l,r,!1);Nr(r,s),o&&o(),s.currentNode=null}else s.onError(ht(30,e.loc));break}}}function ru(e,t){const s=e.tagType===3;return{type:10,loc:e.loc,condition:t.name==="else"?void 0:t.exp,children:s&&!Es(e,"for")?e.children:[e],userKey:Lr(e,"key"),isTemplateIf:s}}function ou(e,t,s){return e.condition?Fo(e.condition,cu(e,t,s),Mt(s.helper(rl),['""',"true"])):cu(e,t,s)}function cu(e,t,s){const{helper:n}=s,a=At("key",He(`${t}`,!1,ks,2)),{children:i}=e,l=i[0];if(i.length!==1||l.type!==1)if(i.length===1&&l.type===11){const o=l.codegenNode;return lr(o,a,s),o}else return Zi(s,n(Ki),As([a]),i,64,void 0,void 0,!0,!1,!1,e.loc);else{const o=l.codegenNode,c=yx(o);return c.type===13&&zc(c,s),lr(c,a,s),o}}function n0(e){for(;;)if(e.type===19)if(e.alternate.type===19)e=e.alternate;else return e;else e.type===20&&(e=e.value)}const a0=Hh("for",(e,t,s)=>{const{helper:n,removeHelper:a}=s;return i0(e,t,s,i=>{const l=Mt(n(Pc),[i.source]),r=ar(e),o=Es(e,"memo"),c=Lr(e,"key",!1,!0);c&&c.type;let d=c&&(c.type===6?c.value?He(c.value.content,!0):void 0:c.exp);const u=d?At("key",d):null,p=i.source.type===4&&i.source.constType>0,f=p?64:c?128:256;return i.codegenNode=Zi(s,n(Ki),void 0,l,f,void 0,void 0,!0,!p,!1,e.loc),()=>{let m;const{children:b}=i,E=b.length!==1||b[0].type!==1,I=ir(e)?e:r&&e.children.length===1&&ir(e.children[0])?e.children[0]:null;if(I?(m=I.codegenNode,r&&u&&lr(m,u,s)):E?m=Zi(s,n(Ki),u?As([u]):void 0,e.children,64,void 0,void 0,!0,void 0,!1):(m=b[0].codegenNode,r&&u&&lr(m,u,s),m.isBlock!==!p&&(m.isBlock?(a(ha),a(ti(s.inSSR,m.isComponent))):a(ei(s.inSSR,m.isComponent))),m.isBlock=!p,m.isBlock?(n(ha),n(ti(s.inSSR,m.isComponent))):n(ei(s.inSSR,m.isComponent))),o){const w=Xa(Uo(i.parseResult,[He("_cached")]));w.body=rx([$s(["const _memo = (",o.exp,")"]),$s(["if (_cached && _cached.el",...d?[" && _cached.key === ",d]:[],` && ${s.helperString(Sh)}(_cached, _memo)) return _cached`]),$s(["const _item = ",m]),He("_item.memo = _memo"),He("return _item")]),l.arguments.push(w,He("_cache"),He(String(s.cached.length))),s.cached.push(null)}else l.arguments.push(Xa(Uo(i.parseResult),m,!0))}})});function i0(e,t,s,n){if(!t.exp){s.onError(ht(31,t.loc));return}const a=t.forParseResult;if(!a){s.onError(ht(32,t.loc));return}qh(a);const{addIdentifiers:i,removeIdentifiers:l,scopes:r}=s,{source:o,value:c,key:d,index:u}=a,p={type:11,loc:t.loc,source:o,valueAlias:c,keyAlias:d,objectIndexAlias:u,parseResult:a,children:ar(e)?e.children:[e]};s.replaceNode(p),r.vFor++;const f=n&&n(p);return()=>{r.vFor--,f&&f()}}function qh(e,t){e.finalized||(e.finalized=!0)}function Uo({value:e,key:t,index:s},n=[]){return l0([e,t,s,...n])}function l0(e){let t=e.length;for(;t--&&!e[t];);return e.slice(0,t+1).map((s,n)=>s||He("_".repeat(n+1),!1))}const du=He("undefined",!1),r0=(e,t)=>{if(e.type===1&&(e.tagType===1||e.tagType===3)){const s=Es(e,"slot");if(s)return s.exp,t.scopes.vSlot++,()=>{t.scopes.vSlot--}}},o0=(e,t,s,n)=>Xa(e,s,!1,!0,s.length?s[0].loc:n);function c0(e,t,s=o0){t.helper(Uc);const{children:n,loc:a}=e,i=[],l=[];let r=t.scopes.vSlot>0||t.scopes.vFor>0;const o=Es(e,"slot",!0);if(o){const{arg:E,exp:I}=o;E&&!fs(E)&&(r=!0),i.push(At(E||He("default",!0),s(I,void 0,n,a)))}let c=!1,d=!1;const u=[],p=new Set;let f=0;for(let E=0;E<n.length;E++){const I=n[E];let w;if(!ar(I)||!(w=Es(I,"slot",!0))){I.type!==3&&u.push(I);continue}if(o){t.onError(ht(37,w.loc));break}c=!0;const{children:g,loc:_}=I,{arg:C=He("default",!0),exp:v,loc:S}=w;let T;fs(C)?T=C?C.content:"default":r=!0;const y=Es(I,"for"),O=s(v,y,g,_);let $,F;if($=Es(I,"if"))r=!0,l.push(Fo($.exp,kl(C,O,f++),du));else if(F=Es(I,/^else(?:-if)?$/,!0)){let N=E,G;for(;N--&&(G=n[N],!!Lh(G)););if(G&&ar(G)&&Es(G,/^(?:else-)?if$/)){let z=l[l.length-1];for(;z.alternate.type===19;)z=z.alternate;z.alternate=F.exp?Fo(F.exp,kl(C,O,f++),du):kl(C,O,f++)}else t.onError(ht(30,F.loc))}else if(y){r=!0;const N=y.forParseResult;N?(qh(N),l.push(Mt(t.helper(Pc),[N.source,Xa(Uo(N),kl(C,O),!0)]))):t.onError(ht(32,y.loc))}else{if(T){if(p.has(T)){t.onError(ht(38,S));continue}p.add(T),T==="default"&&(d=!0)}i.push(At(C,O))}}if(!o){const E=(I,w)=>{const g=s(I,void 0,w,a);return t.compatConfig&&(g.isNonScopedSlot=!0),At("default",g)};c?u.length&&!u.every(qc)&&(d?t.onError(ht(39,u[0].loc)):i.push(E(void 0,u))):i.push(E(void 0,n))}const m=r?2:Ml(e.children)?3:1;let b=As(i.concat(At("_",He(m+"",!1))),a);return l.length&&(b=Mt(t.helper(kh),[b,da(l)])),{slots:b,hasDynamicSlots:r}}function kl(e,t,s){const n=[At("name",e),At("fn",t)];return s!=null&&n.push(At("key",He(String(s),!0))),As(n)}function Ml(e){for(let t=0;t<e.length;t++){const s=e[t];switch(s.type){case 1:if(s.tagType===2||Ml(s.children))return!0;break;case 9:if(Ml(s.branches))return!0;break;case 10:case 11:if(Ml(s.children))return!0;break}}return!1}const Gh=new WeakMap,d0=(e,t)=>function(){if(e=t.currentNode,!(e.type===1&&(e.tagType===0||e.tagType===1)))return;const{tag:n,props:a}=e,i=e.tagType===1;let l=i?u0(e,t):`"${n}"`;const r=et(l)&&l.callee===Lc;let o,c,d=0,u,p,f,m=r||l===Oi||l===Ec||!i&&(n==="svg"||n==="foreignObject"||n==="math");if(a.length>0){const b=Kh(e,t,void 0,i,r);o=b.props,d=b.patchFlag,p=b.dynamicPropNames;const E=b.directives;f=E&&E.length?da(E.map(I=>f0(I,t))):void 0,b.shouldUseBlock&&(m=!0)}if(e.children.length>0)if(l===er&&(m=!0,d|=1024),i&&l!==Oi&&l!==er){const{slots:E,hasDynamicSlots:I}=c0(e,t);c=E,I&&(d|=1024)}else if(e.children.length===1&&l!==Oi){const E=e.children[0],I=E.type,w=I===5||I===8;w&&bs(E,t)===0&&(d|=1),w||I===2?c=E:c=e.children}else c=e.children;p&&p.length&&(u=h0(p)),e.codegenNode=Zi(t,l,o,c,d===0?void 0:d,u,f,!!m,!1,i,e.loc)};function u0(e,t,s=!1){let{tag:n}=e;const a=Ho(n),i=Lr(e,"is",!1,!0);if(i)if(a||ua("COMPILER_IS_ON_ELEMENT",t)){let r;if(i.type===6?r=i.value&&He(i.value.content,!0):(r=i.exp,r||(r=He("is",!1,i.arg.loc))),r)return Mt(t.helper(Lc),[r])}else i.type===6&&i.value.content.startsWith("vue:")&&(n=i.value.content.slice(4));const l=Ch(n)||t.isBuiltInComponent(n);return l?(s||t.helper(l),l):(t.helper(Oc),t.components.add(n),Yi(n,"component"))}function Kh(e,t,s=e.props,n,a,i=!1){const{tag:l,loc:r,children:o}=e;let c=[];const d=[],u=[],p=o.length>0;let f=!1,m=0,b=!1,E=!1,I=!1,w=!1,g=!1,_=!1;const C=[],v=O=>{c.length&&(d.push(As(uu(c),r)),c=[]),O&&d.push(O)},S=()=>{t.scopes.vFor>0&&c.push(At(He("ref_for",!0),He("true")))},T=({key:O,value:$})=>{if(fs(O)){const F=O.content,N=va(F);if(N&&(!n||a)&&F.toLowerCase()!=="onclick"&&F!=="onUpdate:modelValue"&&!yn(F)&&(w=!0),N&&yn(F)&&(_=!0),N&&$.type===14&&($=$.arguments[0]),$.type===20||($.type===4||$.type===8)&&bs($,t)>0)return;F==="ref"?b=!0:F==="class"?E=!0:F==="style"?I=!0:F!=="key"&&!C.includes(F)&&C.push(F),n&&(F==="class"||F==="style")&&!C.includes(F)&&C.push(F)}else g=!0};for(let O=0;O<s.length;O++){const $=s[O];if($.type===6){const{loc:F,name:N,nameLoc:G,value:z}=$;let P=!0;if(N==="ref"&&(b=!0,S()),N==="is"&&(Ho(l)||z&&z.content.startsWith("vue:")||ua("COMPILER_IS_ON_ELEMENT",t)))continue;c.push(At(He(N,!0,G),He(z?z.content:"",P,z?z.loc:F)))}else{const{name:F,arg:N,exp:G,loc:z,modifiers:P}=$,D=F==="bind",R=F==="on";if(F==="slot"){n||t.onError(ht(40,z));continue}if(F==="once"||F==="memo"||F==="is"||D&&aa(N,"is")&&(Ho(l)||ua("COMPILER_IS_ON_ELEMENT",t))||R&&i)continue;if((D&&aa(N,"key")||R&&p&&aa(N,"vue:before-update"))&&(f=!0),D&&aa(N,"ref")&&S(),!N&&(D||R)){if(g=!0,G)if(D){if(v(),ua("COMPILER_V_BIND_OBJECT_ORDER",t)){d.unshift(G);continue}S(),v(),d.push(G)}else v({type:14,loc:z,callee:t.helper(Bc),arguments:n?[G]:[G,"true"]});else t.onError(ht(D?34:35,z));continue}D&&P.some(ne=>ne.content==="prop")&&(m|=32);const ee=t.directiveTransforms[F];if(ee){const{props:ne,needRuntime:ge}=ee($,e,t);!i&&ne.forEach(T),R&&N&&!fs(N)?v(As(ne,r)):c.push(...ne),ge&&(u.push($),Yt(ge)&&Gh.set($,ge))}else av(F)||(u.push($),p&&(f=!0))}}let y;if(d.length?(v(),d.length>1?y=Mt(t.helper(tr),d,r):y=d[0]):c.length&&(y=As(uu(c),r)),g?m|=16:(E&&!n&&(m|=2),I&&!n&&(m|=4),C.length&&(m|=8),w&&(m|=32)),!f&&(m===0||m===32)&&(b||_||u.length>0)&&(m|=512),!t.inSSR&&y)switch(y.type){case 15:let O=-1,$=-1,F=!1;for(let z=0;z<y.properties.length;z++){const P=y.properties[z].key;fs(P)?P.content==="class"?O=z:P.content==="style"&&($=z):P.isHandlerKey||(F=!0)}const N=y.properties[O],G=y.properties[$];F?y=Mt(t.helper(Wi),[y]):(N&&!fs(N.value)&&(N.value=Mt(t.helper(Fc),[N.value])),G&&(I||G.value.type===4&&G.value.content.trim()[0]==="["||G.value.type===17)&&(G.value=Mt(t.helper($c),[G.value])));break;case 14:break;default:y=Mt(t.helper(Wi),[Mt(t.helper(ol),[y])]);break}return{props:y,directives:u,patchFlag:m,dynamicPropNames:C,shouldUseBlock:f}}function uu(e){const t=new Map,s=[];for(let n=0;n<e.length;n++){const a=e[n];if(a.key.type===8||!a.key.isStatic){s.push(a);continue}const i=a.key.content,l=t.get(i);l?(i==="style"||i==="class"||va(i))&&p0(l,a):(t.set(i,a),s.push(a))}return s}function p0(e,t){e.value.type===17?e.value.elements.push(t.value):e.value=da([e.value,t.value],e.loc)}function f0(e,t){const s=[],n=Gh.get(e);n?s.push(t.helperString(n)):(t.helper(Nc),t.directives.add(e.name),s.push(Yi(e.name,"directive")));const{loc:a}=e;if(e.exp&&s.push(e.exp),e.arg&&(e.exp||s.push("void 0"),s.push(e.arg)),Object.keys(e.modifiers).length){e.arg||(e.exp||s.push("void 0"),s.push("void 0"));const i=He("true",!1,a);s.push(As(e.modifiers.map(l=>At(l,i)),a))}return da(s,e.loc)}function h0(e){let t="[";for(let s=0,n=e.length;s<n;s++)t+=JSON.stringify(e[s]),s<n-1&&(t+=", ");return t+"]"}function Ho(e){return e==="component"||e==="Component"}const m0=(e,t)=>{if(ir(e)){const{children:s,loc:n}=e,{slotName:a,slotProps:i}=v0(e,t),l=[t.prefixIdentifiers?"_ctx.$slots":"$slots",a,"{}","undefined","true"];let r=2;i&&(l[2]=i,r=3),s.length&&(l[3]=Xa([],s,!1,!1,n),r=4),t.scopeId&&!t.slotted&&(r=5),l.splice(r),e.codegenNode=Mt(t.helper(wh),l,n)}};function v0(e,t){let s='"default"',n;const a=[];for(let i=0;i<e.props.length;i++){const l=e.props[i];if(l.type===6)l.value&&(l.name==="name"?s=JSON.stringify(l.value.content):(l.name=rt(l.name),a.push(l)));else if(l.name==="bind"&&aa(l.arg,"name")){if(l.exp)s=l.exp;else if(l.arg&&l.arg.type===4){const r=rt(l.arg.content);s=l.exp=He(r,!1,l.arg.loc)}}else l.name==="bind"&&l.arg&&fs(l.arg)&&(l.arg.content=rt(l.arg.content)),a.push(l)}if(a.length>0){const{props:i,directives:l}=Kh(e,t,a,!1,!1);n=i,l.length&&t.onError(ht(36,l[0].loc))}return{slotName:s,slotProps:n}}const Wh=(e,t,s,n)=>{const{loc:a,modifiers:i,arg:l}=e;!e.exp&&!i.length&&s.onError(ht(35,a));let r;if(l.type===4)if(l.isStatic){let u=l.content;u.startsWith("vue:")&&(u=`vnode-${u.slice(4)}`);const p=t.tagType!==0||u.startsWith("vnode")||!/[A-Z]/.test(u)?Ba(rt(u)):`on:${u}`;r=He(p,!0,l.loc)}else r=$s([`${s.helperString(Po)}(`,l,")"]);else r=l,r.children.unshift(`${s.helperString(Po)}(`),r.children.push(")");let o=e.exp;o&&!o.content.trim()&&(o=void 0);let c=s.cacheHandlers&&!o&&!s.inVOnce;if(o){const u=Rh(o),p=!(u||mx(o)),f=o.content.includes(";");(p||c&&u)&&(o=$s([`${p?"$event":"(...args)"} => ${f?"{":"("}`,o,f?"}":")"]))}let d={props:[At(r,o||He("() => {}",!1,a))]};return n&&(d=n(d)),c&&(d.props[0].value=s.cache(d.props[0].value)),d.props.forEach(u=>u.key.isHandlerKey=!0),d},g0=(e,t,s)=>{const{modifiers:n,loc:a}=e,i=e.arg;let{exp:l}=e;return l&&l.type===4&&!l.content.trim()&&(l=void 0),i.type!==4?(i.children.unshift("("),i.children.push(') || ""')):i.isStatic||(i.content=i.content?`${i.content} || ""`:'""'),n.some(r=>r.content==="camel")&&(i.type===4?i.isStatic?i.content=rt(i.content):i.content=`${s.helperString(Do)}(${i.content})`:(i.children.unshift(`${s.helperString(Do)}(`),i.children.push(")"))),s.inSSR||(n.some(r=>r.content==="prop")&&pu(i,"."),n.some(r=>r.content==="attr")&&pu(i,"^")),{props:[At(i,l)]}},pu=(e,t)=>{e.type===4?e.isStatic?e.content=t+e.content:e.content=`\`${t}\${${e.content}}\``:(e.children.unshift(`'${t}' + (`),e.children.push(")"))},b0=(e,t)=>{if(e.type===0||e.type===1||e.type===11||e.type===10)return()=>{const s=e.children;let n,a=!1;for(let i=0;i<s.length;i++){const l=s[i];if(Xr(l)){a=!0;for(let r=i+1;r<s.length;r++){const o=s[r];if(Xr(o))n||(n=s[i]=$s([l],l.loc)),n.children.push(" + ",o),s.splice(r,1),r--;else{n=void 0;break}}}}if(!(!a||s.length===1&&(e.type===0||e.type===1&&e.tagType===0&&!e.props.find(i=>i.type===7&&!t.directiveTransforms[i.name])&&e.tag!=="template")))for(let i=0;i<s.length;i++){const l=s[i];if(Xr(l)||l.type===8){const r=[];(l.type!==2||l.content!==" ")&&r.push(l),!t.ssr&&bs(l,t)===0&&r.push("1"),s[i]={type:12,content:l,loc:l.loc,codegenNode:Mt(t.helper(Ic),r)}}}}},fu=new WeakSet,y0=(e,t)=>{if(e.type===1&&Es(e,"once",!0))return fu.has(e)||t.inVOnce||t.inSSR?void 0:(fu.add(e),t.inVOnce=!0,t.helper(sr),()=>{t.inVOnce=!1;const s=t.currentNode;s.codegenNode&&(s.codegenNode=t.cache(s.codegenNode,!0,!0))})},Zh=(e,t,s)=>{const{exp:n,arg:a}=e;if(!n)return s.onError(ht(41,e.loc)),mi();const i=n.loc.source.trim(),l=n.type===4?n.content:i,r=s.bindingMetadata[i];if(r==="props"||r==="props-aliased")return s.onError(ht(44,n.loc)),mi();if(r==="literal-const"||r==="setup-const")return s.onError(ht(45,n.loc)),mi();if(!l.trim()||!Rh(n))return s.onError(ht(42,n.loc)),mi();const o=a||He("modelValue",!0),c=a?fs(a)?`onUpdate:${rt(a.content)}`:$s(['"onUpdate:" + ',a]):"onUpdate:modelValue";let d;const u=s.isTS?"($event: any)":"$event";d=$s([`${u} => ((`,n,") = $event)"]);const p=[At(o,e.exp),At(c,d)];if(e.modifiers.length&&t.tagType===1){const f=e.modifiers.map(b=>b.content).map(b=>(Vc(b)?b:JSON.stringify(b))+": true").join(", "),m=a?fs(a)?`${a.content}Modifiers`:$s([a,' + "Modifiers"']):"modelModifiers";p.push(At(m,He(`{ ${f} }`,!1,e.loc,2)))}return mi(p)};function mi(e=[]){return{props:e}}const x0=/[\w).+\-_$\]]/,_0=(e,t)=>{ua("COMPILER_FILTERS",t)&&(e.type===5?rr(e.content,t):e.type===1&&e.props.forEach(s=>{s.type===7&&s.name!=="for"&&s.exp&&rr(s.exp,t)}))};function rr(e,t){if(e.type===4)hu(e,t);else for(let s=0;s<e.children.length;s++){const n=e.children[s];typeof n=="object"&&(n.type===4?hu(n,t):n.type===8?rr(e,t):n.type===5&&rr(n.content,t))}}function hu(e,t){const s=e.content;let n=!1,a=!1,i=!1,l=!1,r=0,o=0,c=0,d=0,u,p,f,m,b=[];for(f=0;f<s.length;f++)if(p=u,u=s.charCodeAt(f),n)u===39&&p!==92&&(n=!1);else if(a)u===34&&p!==92&&(a=!1);else if(i)u===96&&p!==92&&(i=!1);else if(l)u===47&&p!==92&&(l=!1);else if(u===124&&s.charCodeAt(f+1)!==124&&s.charCodeAt(f-1)!==124&&!r&&!o&&!c)m===void 0?(d=f+1,m=s.slice(0,f).trim()):E();else{switch(u){case 34:a=!0;break;case 39:n=!0;break;case 96:i=!0;break;case 40:c++;break;case 41:c--;break;case 91:o++;break;case 93:o--;break;case 123:r++;break;case 125:r--;break}if(u===47){let I=f-1,w;for(;I>=0&&(w=s.charAt(I),w===" ");I--);(!w||!x0.test(w))&&(l=!0)}}m===void 0?m=s.slice(0,f).trim():d!==0&&E();function E(){b.push(s.slice(d,f).trim()),d=f+1}if(b.length){for(f=0;f<b.length;f++)m=w0(m,b[f],t);e.content=m,e.ast=void 0}}function w0(e,t,s){s.helper(Mc);const n=t.indexOf("(");if(n<0)return s.filters.add(t),`${Yi(t,"filter")}(${e})`;{const a=t.slice(0,n),i=t.slice(n+1);return s.filters.add(a),`${Yi(a,"filter")}(${e}${i!==")"?","+i:i}`}}const mu=new WeakSet,k0=(e,t)=>{if(e.type===1){const s=Es(e,"memo");return!s||mu.has(e)||t.inSSR?void 0:(mu.add(e),()=>{const n=e.codegenNode||t.currentNode.codegenNode;n&&n.type===13&&(e.tagType!==1&&zc(n,t),e.codegenNode=Mt(t.helper(Hc),[s.exp,Xa(void 0,n),"_cache",String(t.cached.length)]),t.cached.push(null))})}},S0=(e,t)=>{if(e.type===1){for(const s of e.props)if(s.type===7&&s.name==="bind"&&(!s.exp||s.exp.type===4&&!s.exp.content.trim())&&s.arg){const n=s.arg;if(n.type!==4||!n.isStatic)t.onError(ht(53,n.loc)),s.exp=He("",!0,n.loc);else{const a=rt(n.content);(Eh.test(a[0])||a[0]==="-")&&(s.exp=He(a,!1,n.loc))}}}};function T0(e){return[[S0,y0,t0,k0,a0,_0,m0,d0,r0,b0],{on:Wh,bind:g0,model:Zh}]}function C0(e,t={}){const s=t.onError||jc,n=t.mode==="module";t.prefixIdentifiers===!0?s(ht(48)):n&&s(ht(49));const a=!1;t.cacheHandlers&&s(ht(50)),t.scopeId&&!n&&s(ht(51));const i=Ve({},t,{prefixIdentifiers:a}),l=Be(e)?Lx(e,i):e,[r,o]=T0();return Px(l,Ve({},i,{nodeTransforms:[...r,...t.nodeTransforms||[]],directiveTransforms:Ve({},o,t.directiveTransforms||{})})),Ux(l,i)}const E0=()=>({props:[]});/**
* @vue/compiler-dom v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const Jh=Symbol(""),Yh=Symbol(""),Qh=Symbol(""),Xh=Symbol(""),zo=Symbol(""),em=Symbol(""),tm=Symbol(""),sm=Symbol(""),nm=Symbol(""),am=Symbol("");ax({[Jh]:"vModelRadio",[Yh]:"vModelCheckbox",[Qh]:"vModelText",[Xh]:"vModelSelect",[zo]:"vModelDynamic",[em]:"withModifiers",[tm]:"withKeys",[sm]:"vShow",[nm]:"Transition",[am]:"TransitionGroup"});let Ea;function A0(e,t=!1){return Ea||(Ea=document.createElement("div")),t?(Ea.innerHTML=`<div foo="${e.replace(/"/g,"&quot;")}">`,Ea.children[0].getAttribute("foo")):(Ea.innerHTML=e,Ea.textContent)}const R0={parseMode:"html",isVoidTag:_v,isNativeTag:e=>bv(e)||yv(e)||xv(e),isPreTag:e=>e==="pre",isIgnoreNewlineTag:e=>e==="pre"||e==="textarea",decodeEntities:A0,isBuiltInComponent:e=>{if(e==="Transition"||e==="transition")return nm;if(e==="TransitionGroup"||e==="transition-group")return am},getNamespace(e,t,s){let n=t?t.ns:s;if(t&&n===2)if(t.tag==="annotation-xml"){if(e==="svg")return 1;t.props.some(a=>a.type===6&&a.name==="encoding"&&a.value!=null&&(a.value.content==="text/html"||a.value.content==="application/xhtml+xml"))&&(n=0)}else/^m(?:[ions]|text)$/.test(t.tag)&&e!=="mglyph"&&e!=="malignmark"&&(n=0);else t&&n===1&&(t.tag==="foreignObject"||t.tag==="desc"||t.tag==="title")&&(n=0);if(n===0){if(e==="svg")return 1;if(e==="math")return 2}return n}},I0=e=>{e.type===1&&e.props.forEach((t,s)=>{t.type===6&&t.name==="style"&&t.value&&(e.props[s]={type:7,name:"bind",arg:He("style",!0,t.loc),exp:O0(t.value.content,t.loc),modifiers:[],loc:t.loc})})},O0=(e,t)=>{const s=vp(e);return He(JSON.stringify(s),!1,t,3)};function Hn(e,t){return ht(e,t)}const L0=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(Hn(54,a)),t.children.length&&(s.onError(Hn(55,a)),t.children.length=0),{props:[At(He("innerHTML",!0,a),n||He("",!0))]}},N0=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(Hn(56,a)),t.children.length&&(s.onError(Hn(57,a)),t.children.length=0),{props:[At(He("textContent",!0),n?bs(n,s)>0?n:Mt(s.helperString(Or),[n],a):He("",!0))]}},M0=(e,t,s)=>{const n=Zh(e,t,s);if(!n.props.length||t.tagType===1)return n;e.arg&&s.onError(Hn(59,e.arg.loc));const{tag:a}=t,i=s.isCustomElement(a);if(a==="input"||a==="textarea"||a==="select"||i){let l=Qh,r=!1;if(a==="input"||i){const o=Lr(t,"type");if(o){if(o.type===7)l=zo;else if(o.value)switch(o.value.content){case"radio":l=Jh;break;case"checkbox":l=Yh;break;case"file":r=!0,s.onError(Hn(60,e.loc));break}}else vx(t)&&(l=zo)}else a==="select"&&(l=Xh);r||(n.needRuntime=s.helper(l))}else s.onError(Hn(58,e.loc));return n.props=n.props.filter(l=>!(l.key.type===4&&l.key.content==="modelValue")),n},D0=xs("passive,once,capture"),P0=xs("stop,prevent,self,ctrl,shift,alt,meta,exact,middle"),F0=xs("left,right"),im=xs("onkeyup,onkeydown,onkeypress"),$0=(e,t,s,n)=>{const a=[],i=[],l=[];for(let r=0;r<t.length;r++){const o=t[r].content;o==="native"&&Ji("COMPILER_V_ON_NATIVE",s)||D0(o)?l.push(o):F0(o)?fs(e)?im(e.content.toLowerCase())?a.push(o):i.push(o):(a.push(o),i.push(o)):P0(o)?i.push(o):a.push(o)}return{keyModifiers:a,nonKeyModifiers:i,eventOptionModifiers:l}},vu=(e,t)=>fs(e)&&e.content.toLowerCase()==="onclick"?He(t,!0):e.type!==4?$s(["(",e,`) === "onClick" ? "${t}" : (`,e,")"]):e,B0=(e,t,s)=>Wh(e,t,s,n=>{const{modifiers:a}=e;if(!a.length)return n;let{key:i,value:l}=n.props[0];const{keyModifiers:r,nonKeyModifiers:o,eventOptionModifiers:c}=$0(i,a,s,e.loc);if(o.includes("right")&&(i=vu(i,"onContextmenu")),o.includes("middle")&&(i=vu(i,"onMouseup")),o.length&&(l=Mt(s.helper(em),[l,JSON.stringify(o)])),r.length&&(!fs(i)||im(i.content.toLowerCase()))&&(l=Mt(s.helper(tm),[l,JSON.stringify(r)])),c.length){const d=c.map(ba).join("");i=fs(i)?He(`${i.content}${d}`,!0):$s(["(",i,`) + "${d}"`])}return{props:[At(i,l)]}}),U0=(e,t,s)=>{const{exp:n,loc:a}=e;return n||s.onError(Hn(62,a)),{props:[],needRuntime:s.helper(sm)}},H0=(e,t)=>{e.type===1&&e.tagType===0&&(e.tag==="script"||e.tag==="style")&&t.removeNode()},z0=[I0],j0={cloak:E0,html:L0,text:N0,model:M0,on:B0,show:U0};function V0(e,t={}){return C0(e,Ve({},R0,t,{nodeTransforms:[H0,...z0,...t.nodeTransforms||[]],directiveTransforms:Ve({},j0,t.directiveTransforms||{}),transformHoist:null}))}/**
* vue v3.5.38
* (c) 2018-present Yuxi (Evan) You and Vue contributors
* @license MIT
**/const gu=Object.create(null);function q0(e,t){if(!Be(e))if(e.nodeType)e=e.innerHTML;else return zt;const s=rv(e,t),n=gu[s];if(n)return n;if(e[0]==="#"){const r=document.querySelector(e);e=r?r.innerHTML:""}const a=Ve({hoistStatic:!0,onError:void 0,onWarn:zt},t);!a.isCustomElement&&typeof customElements<"u"&&(a.isCustomElement=r=>!!customElements.get(r));const{code:i}=V0(e,a),l=new Function("Vue",i)(Qy);return l._rc=!0,gu[s]=l}zf(q0);const or=jn({items:[]});let G0=1;function Dr(e,t="info",s=3e3){const n=G0++;return or.items.push({id:n,message:String(e),type:t}),s>0&&setTimeout(()=>Wc(n),s),n}function Wc(e){const t=or.items.findIndex(s=>s.id===e);t>=0&&or.items.splice(t,1)}function Ie(e,t="info",s=3e3){return Dr(e,t,s)}Ie.success=(e,t=3e3)=>Dr(e,"success",t);Ie.error=(e,t=5e3)=>Dr(e,"error",t);Ie.info=(e,t=3e3)=>Dr(e,"info",t);Ie.dismiss=Wc;const K0={setup(){return{state:or,dismiss:Wc}},template:`
 ...
 * @license MIT
 */const Na=typeof document<"u";function lm(e){return typeof e=="object"||"displayName"in e||"props"in e||"__vccOpts"in e}function Z0(e){return e.__esModule||e[Symbol.toStringTag]==="Module"||e.default&&lm(e.default)}const nt=Object.assign;function to(e,t){const s={};for(const n in t){const a=t[n];s[n]=Us(a)?a.map(e):e(a)}return s}const Li=()=>{},Us=Array.isArray;function yu(e,t){const s={};for(const n in e)s[n]=n in t?t[n]:e[n];return s}const rm=/#/g,J0=/&/g,Y0=/\//g,Q0=/=/g,X0=/\?/g,om=/\+/g,e_=/%5B/g,t_=/%5D/g,cm=/%5E/g,s_=/%60/g,dm=/%7B/g,n_=/%7C/g,um=/%7D/g,a_=/%20/g;function Zc(e){return e==null?"":encodeURI(""+e).replace(n_,"|").replace(e_,"[").replace(t_,"]")}function i_(e){return Zc(e).replace(dm,"{").replace(um,"}").replace(cm,"^")}function jo(e){return Zc(e).replace(om,"%2B").replace(a_,"+").replace(rm,"%23").replace(J0,"%26").replace(s_,"`").replace(dm,"{").replace(um,"}").replace(cm,"^")}function l_(e){return jo(e).replace(Q0,"%3D")}function r_(e){return Zc(e).replace(rm,"%23").replace(X0,"%3F")}function o_(e){return r_(e).replace(Y0,"%2F")}function Xi(e){if(e==null)return null;try{return decodeURIComponent(""+e)}catch{}return""+e}const c_=/\/$/,d_=e=>e.replace(c_,"");function so(e,t,s="/"){let n,a={},i="",l="";const r=t.indexOf("#");let o=t.indexOf("?");return o=r>=0&&o>r?-1:o,o>=0&&(n=t.slice(0,o),i=t.slice(o,r>0?r:t.length),a=e(i.slice(1))),r>=0&&(n=n||t.slice(0,r),l=t.slice(r,t.length)),n=h_(n??t,s),{fullPath:n+i+l,path:n,query:a,hash:Xi(l)}}function u_(e,t){const s=t.query?e(t.query):"";return t.path+(s&&"?")+s+(t.hash||"")}function xu(e,t){return!t||!e.toLowerCase().startsWith(t.toLowerCase())?e:e.slice(t.length)||"/"}function p_(e,t,s){const n=t.matched.length-1,a=s.matched.length-1;return n>-1&&n===a&&si(t.matched[n],s.matched[a])&&pm(t.params,s.params)&&e(t.query)===e(s.query)&&t.hash===s.hash}function si(e,t){return(e.aliasOf||e)===(t.aliasOf||t)}function pm(e,t){if(Object.keys(e).length!==Object.keys(t).length)return!1;for(var s in e)if(!f_(e[s],t[s]))return!1;return!0}function f_(e,t){return Us(e)?_u(e,t):Us(t)?_u(t,e):(e==null?void 0:e.valueOf())===(t==null?void 0:t.valueOf())}function _u(e,t){return Us(t)?e.length===t.length&&e.every((s,n)=>s===t[n]):e.length===1&&e[0]===t}function h_(e,t){if(e.startsWith("/"))return e;if(!e)return t;const s=t.split("/"),n=e.split("/"),a=n[n.length-1];(a===".."||a===".")&&n.push("");let i=s.length-1,l,r;for(l=0;l<n.length;l++)if(r=n[l],r!==".")if(r==="..")i>1&&i--;else break;return s.slice(0,i).join("/")+"/"+n.slice(l).join("/")}const Nn={path:"/",name:void 0,params:{},query:{},hash:"",fullPath:"/",matched:[],meta:{},redirectedFrom:void 0};let Vo=(function(e){return e.pop="pop",e.push="push",e})({}),no=(function(e){return e.back="back",e.forward="forward",e.unknown="",e})({});function m_(e){if(!e)if(Na){const t=document.querySelector("base");e=t&&t.getAttribute("href")||"/",e=e.replace(/^\w+:\/\/[^\/]+/,"")}else e="/";return e[0]!=="/"&&e[0]!=="#"&&(e="/"+e),d_(e)}const v_=/^[^#]+#/;function g_(e,t){return e.replace(v_,"#")+t}function b_(e,t){const s=document.documentElement.getBoundingClientRect(),n=e.getBoundingClientRect();return{behavior:t.behavior,left:n.left-s.left-(t.left||0),top:n.top-s.top-(t.top||0)}}const Pr=()=>({left:window.scrollX,top:window.scrollY});function y_(e){let t;if("el"in e){const s=e.el,n=typeof s=="string"&&s.startsWith("#"),a=typeof s=="string"?n?document.getElementById(s.slice(1)):document.querySelector(s):s;if(!a)return;t=b_(a,e)}else t=e;"scrollBehavior"in document.documentElement.style?window.scrollTo(t):window.scrollTo(t.left!=null?t.left:window.scrollX,t.top!=null?t.top:window.scrollY)}function wu(e,t){return(history.state?history.state.position-t:-1)+e}const qo=new Map;function x_(e,t){qo.set(e,t)}function __(e){const t=qo.get(e);return qo.delete(e),t}function w_(e){return typeof e=="string"||e&&typeof e=="object"}function fm(e){return typeof e=="string"||typeof e=="symbol"}let yt=(function(e){return e[e.MATCHER_NOT_FOUND=1]="MATCHER_NOT_FOUND",e[e.NAVIGATION_GUARD_REDIRECT=2]="NAVIGATION_GUARD_REDIRECT",e[e.NAVIGATION_ABORTED=4]="NAVIGATION_ABORTED",e[e.NAVIGATION_CANCELLED=8]="NAVIGATION_CANCELLED",e[e.NAVIGATION_DUPLICATED=16]="NAVIGATION_DUPLICATED",e})({});const hm=Symbol("");yt.MATCHER_NOT_FOUND+"",yt.NAVIGATION_GUARD_REDIRECT+"",yt.NAVIGATION_ABORTED+"",yt.NAVIGATION_CANCELLED+"",yt.NAVIGATION_DUPLICATED+"";function ni(e,t){return nt(new Error,{type:e,[hm]:!0},t)}function on(e,t){return e instanceof Error&&hm in e&&(t==null||!!(e.type&t))}const k_=["params","query","hash"];function S_(e){if(typeof e=="string")return e;if(e.path!=null)return e.path;const t={};for(const s of k_)s in e&&(t[s]=e[s]);return JSON.stringify(t,null,2)}function T_(e){const t={};if(e===""||e==="?")return t;const s=(e[0]==="?"?e.slice(1):e).split("&");for(let n=0;n<s.length;++n){const a=s[n].replace(om," "),i=a.indexOf("="),l=Xi(i<0?a:a.slice(0,i)),r=i<0?null:Xi(a.slice(i+1));if(l in t){let o=t[l];Us(o)||(o=t[l]=[o]),o.push(r)}else t[l]=r}return t}function ku(e){let t="";for(let s in e){const n=e[s];if(s=l_(s),n==null){n!==void 0&&(t+=(t.length?"&":"")+s);continue}(Us(n)?n.map(a=>a&&jo(a)):[n&&jo(n)]).forEach(a=>{a!==void 0&&(t+=(t.length?"&":"")+s,a!=null&&(t+="="+a))})}return t}function C_(e){const t={};for(const s in e){const n=e[s];n!==void 0&&(t[s]=Us(n)?n.map(a=>a==null?null:""+a):n==null?n:""+n)}return t}const E_=Symbol(""),Su=Symbol(""),Fr=Symbol(""),Jc=Symbol(""),Go=Symbol("");function vi(){let e=[];function t(n){return e.push(n),()=>{const a=e.indexOf(n);a>-1&&e.splice(a,1)}}function s(){e=[]}return{add:t,list:()=>e.slice(),reset:s}}function Bn(e,t,s,n,a,i=l=>l()){const l=n&&(n.enterCallbacks[a]=n.enterCallbacks[a]||[]);return()=>new Promise((r,o)=>{const c=p=>{p===!1?o(ni(yt.NAVIGATION_ABORTED,{from:s,to:t})):p instanceof Error?o(p):w_(p)?o(ni(yt.NAVIGATION_GUARD_REDIRECT,{from:t,to:p})):(l&&n.enterCallbacks[a]===l&&typeof p=="function"&&l.push(p),r())},d=i(()=>e.call(n&&n.instances[a],t,s,c));let u=Promise.resolve(d);e.length<3&&(u=u.then(c)),u.catch(p=>o(p))})}function ao(e,t,s,n,a=i=>i()){const i=[];for(const l of e)for(const r in l.components){let o=l.components[r];if(!(t!=="beforeRouteEnter"&&!l.instances[r]))if(lm(o)){const c=(o.__vccOpts||o)[t];c&&i.push(Bn(c,s,n,l,r,a))}else{let c=o();i.push(()=>c.then(d=>{if(!d)throw new Error(`Couldn't resolve component "${r}" at "${l.path}"`);const u=Z0(d)?d.default:d;l.mods[r]=d,l.components[r]=u;const p=(u.__vccOpts||u)[t];return p&&Bn(p,s,n,l,r,a)()}))}}return i}function A_(e,t){const s=[],n=[],a=[],i=Math.max(t.matched.length,e.matched.length);for(let l=0;l<i;l++){const r=t.matched[l];r&&(e.matched.find(c=>si(c,r))?n.push(r):s.push(r));const o=e.matched[l];o&&(t.matched.find(c=>si(c,o))||a.push(o))}return[s,n,a]}/*!
 * vue-router v4.6.4
 * (c) 2025 Eduardo San Martin Morote
 * @license MIT
 */let R_=()=>location.protocol+"//"+location.host;function mm(e,t){const{pathname:s,search:n,hash:a}=t,i=e.indexOf("#");if(i>-1){let l=a.includes(e.slice(i))?e.slice(i).length:1,r=a.slice(l);return r[0]!=="/"&&(r="/"+r),xu(r,"")}return xu(s,e)+n+a}function I_(e,t,s,n){let a=[],i=[],l=null;const r=({state:p})=>{const f=mm(e,location),m=s.value,b=t.value;let E=0;if(p){if(s.value=f,t.value=p,l&&l===m){l=null;return}E=b?p.position-b.position:0}else n(f);a.forEach(I=>{I(s.value,m,{delta:E,type:Vo.pop,direction:E?E>0?no.forward:no.back:no.unknown})})};function o(){l=s.value}function c(p){a.push(p);const f=()=>{const m=a.indexOf(p);m>-1&&a.splice(m,1)};return i.push(f),f}function d(){if(document.visibilityState==="hidden"){const{history:p}=window;if(!p.state)return;p.replaceState(nt({},p.state,{scroll:Pr()}),"")}}function u(){for(const p of i)p();i=[],window.removeEventListener("popstate",r),window.removeEventListener("pagehide",d),document.removeEventListener("visibilitychange",d)}return window.addEventListener("popstate",r),window.addEventListener("pagehide",d),document.addEventListener("visibilitychange",d),{pauseListeners:o,listen:c,destroy:u}}function Tu(e,t,s,n=!1,a=!1){return{back:e,current:t,forward:s,replaced:n,position:window.history.length,scroll:a?Pr():null}}function O_(e){const{history:t,location:s}=window,n={value:mm(e,s)},a={value:t.state};a.value||i(n.value,{back:null,current:n.value,forward:null,position:t.length-1,replaced:!0,scroll:null},!0);function i(o,c,d){const u=e.indexOf("#"),p=u>-1?(s.host&&document.querySelector("base")?e:e.slice(u))+o:R_()+e+o;try{t[d?"replaceState":"pushState"](c,"",p),a.value=c}catch(f){console.error(f),s[d?"replace":"assign"](p)}}function l(o,c){i(o,nt({},t.state,Tu(a.value.back,o,a.value.forward,!0),c,{position:a.value.position}),!0),n.value=o}function r(o,c){const d=nt({},a.value,t.state,{forward:o,scroll:Pr()});i(d.current,d,!0),i(o,nt({},Tu(n.value,o,null),{position:d.position+1},c),!1),n.value=o}return{location:n,state:a,push:r,replace:l}}function L_(e){e=m_(e);const t=O_(e),s=I_(e,t.state,t.location,t.replace);function n(i,l=!0){l||s.pauseListeners(),history.go(i)}const a=nt({location:"",base:e,go:n,createHref:g_.bind(null,e)},t,s);return Object.defineProperty(a,"location",{enumerable:!0,get:()=>t.location.value}),Object.defineProperty(a,"state",{enumerable:!0,get:()=>t.state.value}),a}function N_(e){return e=location.host?e||location.pathname+location.search:"",e.includes("#")||(e+="#"),L_(e)}let la=(function(e){return e[e.Static=0]="Static",e[e.Param=1]="Param",e[e.Group=2]="Group",e})({});var Lt=(function(e){return e[e.Static=0]="Static",e[e.Param=1]="Param",e[e.ParamRegExp=2]="ParamRegExp",e[e.ParamRegExpEnd=3]="ParamRegExpEnd",e[e.EscapeNext=4]="EscapeNext",e})(Lt||{});const M_={type:la.Static,value:""},D_=/[a-zA-Z0-9_]/;function P_(e){if(!e)return[[]];if(e==="/")return[[M_]];if(!e.startsWith("/"))throw new Error(`Invalid path "${e}"`);function t(f){throw new Error(`ERR (${s})/"${c}": ${f}`)}let s=Lt.Static,n=s;const a=[];let i;function l(){i&&a.push(i),i=[]}let r=0,o,c="",d="";function u(){c&&(s===Lt.Static?i.push({type:la.Static,value:c}):s===Lt.Param||s===Lt.ParamRegExp||s===Lt.ParamRegExpEnd?(i.length>1&&(o==="*"||o==="+")&&t(`A repeatable param (${c}) must be alone in its segment. eg: '/:ids+.`),i.push({type:la.Param,value:c,regexp:d,repeatable:o==="*"||o==="+",optional:o==="*"||o==="?"})):t("Invalid state to consume buffer"),c="")}function p(){c+=o}for(;r<e.length;){if(o=e[r++],o==="\\"&&s!==Lt.ParamRegExp){n=s,s=Lt.EscapeNext;continue}switch(s){case Lt.Static:o==="/"?(c&&u(),l()):o===":"?(u(),s=Lt.Param):p();break;case Lt.EscapeNext:p(),s=n;break;case Lt.Param:o==="("?s=Lt.ParamRegExp:D_.test(o)?p():(u(),s=Lt.Static,o!=="*"&&o!=="?"&&o!=="+"&&r--);break;case Lt.ParamRegExp:o===")"?d[d.length-1]=="\\"?d=d.slice(0,-1)+o:s=Lt.ParamRegExpEnd:d+=o;break;case Lt.ParamRegExpEnd:u(),s=Lt.Static,o!=="*"&&o!=="?"&&o!=="+"&&r--,d="";break;default:t("Unknown state");break}}return s===Lt.ParamRegExp&&t(`Unfinished custom RegExp for param "${c}"`),u(),l(),a}const Cu="[^/]+?",F_={sensitive:!1,strict:!1,start:!0,end:!0};var ss=(function(e){return e[e._multiplier=10]="_multiplier",e[e.Root=90]="Root",e[e.Segment=40]="Segment",e[e.SubSegment=30]="SubSegment",e[e.Static=40]="Static",e[e.Dynamic=20]="Dynamic",e[e.BonusCustomRegExp=10]="BonusCustomRegExp",e[e.BonusWildcard=-50]="BonusWildcard",e[e.BonusRepeatable=-20]="BonusRepeatable",e[e.BonusOptional=-8]="BonusOptional",e[e.BonusStrict=.7000000000000001]="BonusStrict",e[e.BonusCaseSensitive=.25]="BonusCaseSensitive",e})(ss||{});const $_=/[.+*?^${}()[\]/\\]/g;function B_(e,t){const s=nt({},F_,t),n=[];let a=s.start?"^":"";const i=[];for(const c of e){const d=c.length?[]:[ss.Root];s.strict&&!c.length&&(a+="/");for(let u=0;u<c.length;u++){const p=c[u];let f=ss.Segment+(s.sensitive?ss.BonusCaseSensitive:0);if(p.type===la.Static)u||(a+="/"),a+=p.value.replace($_,"\\$&"),f+=ss.Static;else if(p.type===la.Param){const{value:m,repeatable:b,optional:E,regexp:I}=p;i.push({name:m,repeatable:b,optional:E});const w=I||Cu;if(w!==Cu){f+=ss.BonusCustomRegExp;try{`${w}`}catch(_){throw new Error(`Invalid custom RegExp for param "${m}" (${w}): `+_.message)}}let g=b?`((?:${w})(?:/(?:${w}))*)`:`(${w})`;u||(g=E&&c.length<2?`(?:/${g})`:"/"+g),E&&(g+="?"),a+=g,f+=ss.Dynamic,E&&(f+=ss.BonusOptional),b&&(f+=ss.BonusRepeatable),w===".*"&&(f+=ss.BonusWildcard)}d.push(f)}n.push(d)}if(s.strict&&s.end){const c=n.length-1;n[c][n[c].length-1]+=ss.BonusStrict}s.strict||(a+="/?"),s.end?a+="$":s.strict&&!a.endsWith("/")&&(a+="(?:/|$)");const l=new RegExp(a,s.sensitive?"":"i");function r(c){const d=c.match(l),u={};if(!d)return null;for(let p=1;p<d.length;p++){const f=d[p]||"",m=i[p-1];u[m.name]=f&&m.repeatable?f.split("/"):f}return u}function o(c){let d="",u=!1;for(const p of e){(!u||!d.endsWith("/"))&&(d+="/"),u=!1;for(const f of p)if(f.type===la.Static)d+=f.value;else if(f.type===la.Param){const{value:m,repeatable:b,optional:E}=f,I=m in c?c[m]:"";if(Us(I)&&!b)throw new Error(`Provided param "${m}" is an array but it is not repeatable (* or + modifiers)`);const w=Us(I)?I.join("/"):I;if(!w)if(E)p.length<2&&(d.endsWith("/")?d=d.slice(0,-1):u=!0);else throw new Error(`Missing required param "${m}"`);d+=w}}return d||"/"}return{re:l,score:n,keys:i,parse:r,stringify:o}}function U_(e,t){let s=0;for(;s<e.length&&s<t.length;){const n=t[s]-e[s];if(n)return n;s++}return e.length<t.length?e.length===1&&e[0]===ss.Static+ss.Segment?-1:1:e.length>t.length?t.length===1&&t[0]===ss.Static+ss.Segment?1:-1:0}function vm(e,t){let s=0;const n=e.score,a=t.score;for(;s<n.length&&s<a.length;){const i=U_(n[s],a[s]);if(i)return i;s++}if(Math.abs(a.length-n.length)===1){if(Eu(n))return 1;if(Eu(a))return-1}return a.length-n.length}function Eu(e){const t=e[e.length-1];return e.length>0&&t[t.length-1]<0}const H_={strict:!1,end:!0,sensitive:!1};function z_(e,t,s){const n=B_(P_(e.path),s),a=nt(n,{record:e,parent:t,children:[],alias:[]});return t&&!a.record.aliasOf==!t.record.aliasOf&&t.children.push(a),a}function j_(e,t){const s=[],n=new Map;t=yu(H_,t);function a(u){return n.get(u)}function i(u,p,f){const m=!f,b=Ru(u);b.aliasOf=f&&f.record;const E=yu(t,u),I=[b];if("alias"in u){const _=typeof u.alias=="string"?[u.alias]:u.alias;for(const C of _)I.push(Ru(nt({},b,{components:f?f.record.components:b.components,path:C,aliasOf:f?f.record:b})))}let w,g;for(const _ of I){const{path:C}=_;if(p&&C[0]!=="/"){const v=p.record.path,S=v[v.length-1]==="/"?"":"/";_.path=p.record.path+(C&&S+C)}if(w=z_(_,p,E),f?f.alias.push(w):(g=g||w,g!==w&&g.alias.push(w),m&&u.name&&!Iu(w)&&l(u.name)),gm(w)&&o(w),b.children){const v=b.children;for(let S=0;S<v.length;S++)i(v[S],w,f&&f.children[S])}f=f||w}return g?()=>{l(g)}:Li}function l(u){if(fm(u)){const p=n.get(u);p&&(n.delete(u),s.splice(s.indexOf(p),1),p.children.forEach(l),p.alias.forEach(l))}else{const p=s.indexOf(u);p>-1&&(s.splice(p,1),u.record.name&&n.delete(u.record.name),u.children.forEach(l),u.alias.forEach(l))}}function r(){return s}function o(u){const p=G_(u,s);s.splice(p,0,u),u.record.name&&!Iu(u)&&n.set(u.record.name,u)}function c(u,p){let f,m={},b,E;if("name"in u&&u.name){if(f=n.get(u.name),!f)throw ni(yt.MATCHER_NOT_FOUND,{location:u});E=f.record.name,m=nt(Au(p.params,f.keys.filter(g=>!g.optional).concat(f.parent?f.parent.keys.filter(g=>g.optional):[]).map(g=>g.name)),u.params&&Au(u.params,f.keys.map(g=>g.name))),b=f.stringify(m)}else if(u.path!=null)b=u.path,f=s.find(g=>g.re.test(b)),f&&(m=f.parse(b),E=f.record.name);else{if(f=p.name?n.get(p.name):s.find(g=>g.re.test(p.path)),!f)throw ni(yt.MATCHER_NOT_FOUND,{location:u,currentLocation:p});E=f.record.name,m=nt({},p.params,u.params),b=f.stringify(m)}const I=[];let w=f;for(;w;)I.unshift(w.record),w=w.parent;return{name:E,path:b,params:m,matched:I,meta:q_(I)}}e.forEach(u=>i(u));function d(){s.length=0,n.clear()}return{addRoute:i,resolve:c,removeRoute:l,clearRoutes:d,getRoutes:r,getRecordMatcher:a}}function Au(e,t){const s={};for(const n of t)n in e&&(s[n]=e[n]);return s}function Ru(e){const t={path:e.path,redirect:e.redirect,name:e.name,meta:e.meta||{},aliasOf:e.aliasOf,beforeEnter:e.beforeEnter,props:V_(e),children:e.children||[],instances:{},leaveGuards:new Set,updateGuards:new Set,enterCallbacks:{},components:"components"in e?e.components||null:e.component&&{default:e.component}};return Object.defineProperty(t,"mods",{value:{}}),t}function V_(e){const t={},s=e.props||!1;if("component"in e)t.default=s;else for(const n in e.components)t[n]=typeof s=="object"?s[n]:s;return t}function Iu(e){for(;e;){if(e.record.aliasOf)return!0;e=e.parent}return!1}function q_(e){return e.reduce((t,s)=>nt(t,s.meta),{})}function G_(e,t){let s=0,n=t.length;for(;s!==n;){const i=s+n>>1;vm(e,t[i])<0?n=i:s=i+1}const a=K_(e);return a&&(n=t.lastIndexOf(a,n-1)),n}function K_(e){let t=e;for(;t=t.parent;)if(gm(t)&&vm(e,t)===0)return t}function gm({record:e}){return!!(e.name||e.components&&Object.keys(e.components).length||e.redirect)}function Ou(e){const t=Rs(Fr),s=Rs(Jc),n=W(()=>{const o=en(e.to);return t.resolve(o)}),a=W(()=>{const{matched:o}=n.value,{length:c}=o,d=o[c-1],u=s.matched;if(!d||!u.length)return-1;const p=u.findIndex(si.bind(null,d));if(p>-1)return p;const f=Lu(o[c-2]);return c>1&&Lu(d)===f&&u[u.length-1].path!==f?u.findIndex(si.bind(null,o[c-2])):p}),i=W(()=>a.value>-1&&Q_(s.params,n.value.params)),l=W(()=>a.value>-1&&a.value===s.matched.length-1&&pm(s.params,n.value.params));function r(o={}){if(Y_(o)){const c=t[en(e.replace)?"replace":"push"](en(e.to)).catch(Li);return e.viewTransition&&typeof document<"u"&&"startViewTransition"in document&&document.startViewTransition(()=>c),c}return Promise.resolve()}return{route:n,href:W(()=>n.value.href),isActive:i,isExactActive:l,navigate:r}}function W_(e){return e.length===1?e[0]:e}const Z_=al({name:"RouterLink",compatConfig:{MODE:3},props:{to:{type:[String,Object],required:!0},replace:Boolean,activeClass:String,exactActiveClass:String,custom:Boolean,ariaCurrentValue:{type:String,default:"page"},viewTransition:Boolean},useLink:Ou,setup(e,{slots:t}){const s=jn(Ou(e)),{options:n}=Rs(Fr),a=W(()=>({[Nu(e.activeClass,n.linkActiveClass,"router-link-active")]:s.isActive,[Nu(e.exactActiveClass,n.linkExactActiveClass,"router-link-exact-active")]:s.isExactActive}));return()=>{const i=t.default&&W_(t.default(s));return e.custom?i:Za("a",{"aria-current":s.isExactActive?e.ariaCurrentValue:null,href:s.href,onClick:s.navigate,class:a.value},i)}}}),J_=Z_;function Y_(e){if(!(e.metaKey||e.altKey||e.ctrlKey||e.shiftKey)&&!e.defaultPrevented&&!(e.button!==void 0&&e.button!==0)){if(e.currentTarget&&e.currentTarget.getAttribute){const t=e.currentTarget.getAttribute("target");if(/\b_blank\b/i.test(t))return}return e.preventDefault&&e.preventDefault(),!0}}function Q_(e,t){for(const s in t){const n=t[s],a=e[s];if(typeof n=="string"){if(n!==a)return!1}else if(!Us(a)||a.length!==n.length||n.some((i,l)=>i.valueOf()!==a[l].valueOf()))return!1}return!0}function Lu(e){return e?e.aliasOf?e.aliasOf.path:e.path:""}const Nu=(e,t,s)=>e??t??s,X_=al({name:"RouterView",inheritAttrs:!1,props:{name:{type:String,default:"default"},route:Object},compatConfig:{MODE:3},setup(e,{attrs:t,slots:s}){const n=Rs(Go),a=W(()=>e.route||n.value),i=Rs(Su,0),l=W(()=>{let c=en(i);const{matched:d}=a.value;let u;for(;(u=d[c])&&!u.components;)c++;return c}),r=W(()=>a.value.matched[l.value]);Ei(Su,W(()=>l.value+1)),Ei(E_,r),Ei(Go,a);const o=h();return is(()=>[o.value,r.value,e.name],([c,d,u],[p,f,m])=>{d&&(d.instances[u]=c,f&&f!==d&&c&&c===p&&(d.leaveGuards.size||(d.leaveGuards=f.leaveGuards),d.updateGuards.size||(d.updateGuards=f.updateGuards))),c&&d&&(!f||!si(d,f)||!p)&&(d.enterCallbacks[u]||[]).forEach(b=>b(c))},{flush:"post"}),()=>{const c=a.value,d=e.name,u=r.value,p=u&&u.components[d];if(!p)return Mu(s.default,{Component:p,route:c});const f=u.props[d],m=f?f===!0?c.params:typeof f=="function"?f(c):f:null,E=Za(p,nt({},m,t,{onVnodeUnmounted:I=>{I.component.isUnmounted&&(u.instances[d]=null)},ref:o}));return Mu(s.default,{Component:E,route:c})||E}}});function Mu(e,t){if(!e)return null;const s=e(t);return s.length===1?s[0]:s}const ew=X_;function tw(e){const t=j_(e.routes,e),s=e.parseQuery||T_,n=e.stringifyQuery||ku,a=e.history,i=vi(),l=vi(),r=vi(),o=lc(Nn);let c=Nn;Na&&e.scrollBehavior&&"scrollRestoration"in history&&(history.scrollRestoration="manual");const d=to.bind(null,U=>""+U),u=to.bind(null,o_),p=to.bind(null,Xi);function f(U,le){let re,Se;return fm(U)?(re=t.getRecordMatcher(U),Se=le):Se=U,t.addRoute(Se,re)}function m(U){const le=t.getRecordMatcher(U);le&&t.removeRoute(le)}function b(){return t.getRoutes().map(U=>U.record)}function E(U){return!!t.getRecordMatcher(U)}function I(U,le){if(le=nt({},le||o.value),typeof U=="string"){const L=so(s,U,le.path),B=t.resolve({path:L.path},le),se=a.createHref(L.fullPath);return nt(L,B,{params:p(B.params),hash:Xi(L.hash),redirectedFrom:void 0,href:se})}let re;if(U.path!=null)re=nt({},U,{path:so(s,U.path,le.path).path});else{const L=nt({},U.params);for(const B in L)L[B]==null&&delete L[B];re=nt({},U,{params:u(L)}),le.params=u(le.params)}const Se=t.resolve(re,le),_e=U.hash||"";Se.params=d(p(Se.params));const Ue=u_(n,nt({},U,{hash:i_(_e),path:Se.path})),k=a.createHref(Ue);return nt({fullPath:Ue,hash:_e,query:n===ku?C_(U.query):U.query||{}},Se,{redirectedFrom:void 0,href:k})}function w(U){return typeof U=="string"?so(s,U,o.value.path):nt({},U)}function g(U,le){if(c!==U)return ni(yt.NAVIGATION_CANCELLED,{from:le,to:U})}function _(U){return S(U)}function C(U){return _(nt(w(U),{replace:!0}))}function v(U,le){const re=U.matched[U.matched.length-1];if(re&&re.redirect){const{redirect:Se}=re;let _e=typeof Se=="function"?Se(U,le):Se;return typeof _e=="string"&&(_e=_e.includes("?")||_e.includes("#")?_e=w(_e):{path:_e},_e.params={}),nt({query:U.query,hash:U.hash,params:_e.path!=null?{}:U.params},_e)}}function S(U,le){const re=c=I(U),Se=o.value,_e=U.state,Ue=U.force,k=U.replace===!0,L=v(re,Se);if(L)return S(nt(w(L),{state:typeof L=="object"?nt({},_e,L.state):_e,force:Ue,replace:k}),le||re);const B=re;B.redirectedFrom=le;let se;return!Ue&&p_(n,Se,re)&&(se=ni(yt.NAVIGATION_DUPLICATED,{to:B,from:Se}),ge(Se,Se,!0,!1)),(se?Promise.resolve(se):O(B,Se)).catch(Y=>on(Y)?on(Y,yt.NAVIGATION_GUARD_REDIRECT)?Y:ne(Y):R(Y,B,Se)).then(Y=>{if(Y){if(on(Y,yt.NAVIGATION_GUARD_REDIRECT))return S(nt({replace:k},w(Y.to),{state:typeof Y.to=="object"?nt({},_e,Y.to.state):_e,force:Ue}),le||B)}else Y=F(B,Se,!0,k,_e);return $(B,Se,Y),Y})}function T(U,le){const re=g(U,le);return re?Promise.reject(re):Promise.resolve()}function y(U){const le=Q.values().next().value;return le&&typeof le.runWithContext=="function"?le.runWithContext(U):U()}function O(U,le){let re;const[Se,_e,Ue]=A_(U,le);re=ao(Se.reverse(),"beforeRouteLeave",U,le);for(const L of Se)L.leaveGuards.forEach(B=>{re.push(Bn(B,U,le))});const k=T.bind(null,U,le);return re.push(k),J(re).then(()=>{re=[];for(const L of i.list())re.push(Bn(L,U,le));return re.push(k),J(re)}).then(()=>{re=ao(_e,"beforeRouteUpdate",U,le);for(const L of _e)L.updateGuards.forEach(B=>{re.push(Bn(B,U,le))});return re.push(k),J(re)}).then(()=>{re=[];for(const L of Ue)if(L.beforeEnter)if(Us(L.beforeEnter))for(const B of L.beforeEnter)re.push(Bn(B,U,le));else re.push(Bn(L.beforeEnter,U,le));return re.push(k),J(re)}).then(()=>(U.matched.forEach(L=>L.enterCallbacks={}),re=ao(Ue,"beforeRouteEnter",U,le,y),re.push(k),J(re))).then(()=>{re=[];for(const L of l.list())re.push(Bn(L,U,le));return re.push(k),J(re)}).catch(L=>on(L,yt.NAVIGATION_CANCELLED)?L:Promise.reject(L))}function $(U,le,re){r.list().forEach(Se=>y(()=>Se(U,le,re)))}function F(U,le,re,Se,_e){const Ue=g(U,le);if(Ue)return Ue;const k=le===Nn,L=Na?history.state:{};re&&(Se||k?a.replace(U.fullPath,nt({scroll:k&&L&&L.scroll},_e)):a.push(U.fullPath,_e)),o.value=U,ge(U,le,re,k),ne()}let N;function G(){N||(N=a.listen((U,le,re)=>{if(!ye.listening)return;const Se=I(U),_e=v(Se,ye.currentRoute.value);if(_e){S(nt(_e,{replace:!0,force:!0}),Se).catch(Li);return}c=Se;const Ue=o.value;Na&&x_(wu(Ue.fullPath,re.delta),Pr()),O(Se,Ue).catch(k=>on(k,yt.NAVIGATION_ABORTED|yt.NAVIGATION_CANCELLED)?k:on(k,yt.NAVIGATION_GUARD_REDIRECT)?(S(nt(w(k.to),{force:!0}),Se).then(L=>{on(L,yt.NAVIGATION_ABORTED|yt.NAVIGATION_DUPLICATED)&&!re.delta&&re.type===Vo.pop&&a.go(-1,!1)}).catch(Li),Promise.reject()):(re.delta&&a.go(-re.delta,!1),R(k,Se,Ue))).then(k=>{k=k||F(Se,Ue,!1),k&&(re.delta&&!on(k,yt.NAVIGATION_CANCELLED)?a.go(-re.delta,!1):re.type===Vo.pop&&on(k,yt.NAVIGATION_ABORTED|yt.NAVIGATION_DUPLICATED)&&a.go(-1,!1)),$(Se,Ue,k)}).catch(Li)}))}let z=vi(),P=vi(),D;function R(U,le,re){ne(U);const Se=P.list();return Se.length?Se.forEach(_e=>_e(U,le,re)):console.error(U),Promise.reject(U)}function ee(){return D&&o.value!==Nn?Promise.resolve():new Promise((U,le)=>{z.add([U,le])})}function ne(U){return D||(D=!U,G(),z.list().forEach(([le,re])=>U?re(U):le()),z.reset()),U}function ge(U,le,re,Se){const{scrollBehavior:_e}=e;if(!Na||!_e)return Promise.resolve();const Ue=!re&&__(wu(U.fullPath,0))||(Se||!re)&&history.state&&history.state.scroll||null;return Et().then(()=>_e(U,le,Ue)).then(k=>k&&y_(k)).catch(k=>R(k,U,le))}const ie=U=>a.go(U);let be;const Q=new Set,ye={currentRoute:o,listening:!0,addRoute:f,removeRoute:m,clearRoutes:t.clearRoutes,hasRoute:E,getRoutes:b,resolve:I,options:e,push:_,replace:C,go:ie,back:()=>ie(-1),forward:()=>ie(1),beforeEach:i.add,beforeResolve:l.add,afterEach:r.add,onError:P.add,isReady:ee,install(U){U.component("RouterLink",J_),U.component("RouterView",ew),U.config.globalProperties.$router=ye,Object.defineProperty(U.config.globalProperties,"$route",{enumerable:!0,get:()=>en(o)}),Na&&!be&&o.value===Nn&&(be=!0,_(a.location).catch(Se=>{}));const le={};for(const Se in Nn)Object.defineProperty(le,Se,{get:()=>o.value[Se],enumerable:!0});U.provide(Fr,ye),U.provide(Jc,ic(le)),U.provide(Go,o);const re=U.unmount;Q.add(U),U.unmount=function(){Q.delete(U),Q.size<1&&(c=Nn,N&&N(),N=null,o.value=Nn,be=!1,D=!1),re()}}};function J(U){return U.reduce((le,re)=>le.then(()=>y(re)),Promise.resolve())}return ye}function bm(){return Rs(Fr)}function sw(e){return Rs(Jc)}const $r={props:{tabs:{type:Array,required:!0},defaultTab:{type:String,default:""},groupLabel:{type:String,default:""}},setup(e){const t=sw(),s=bm(),n=W({get(){var o;const r=t.query.tab;return r&&e.tabs.some(c=>c.id===r)?r:e.defaultTab||((o=e.tabs[0])==null?void 0:o.id)||""},set(r){s.replace({query:{...t.query,tab:r}})}}),a=W(()=>{var r;return((r=e.tabs.find(o=>o.id===n.value))==null?void 0:r.component)||null}),i=W(()=>{var r;return((r=e.tabs.find(o=>o.id===n.value))==null?void 0:r.label)||""});is(i,r=>{e.groupLabel&&r&&(document.title=`Odin — ${e.groupLabel} › ${r}`)},{immediate:!0});function l(r,o){if(!["ArrowLeft","ArrowRight","Home","End"].includes(r.key))return;r.preventDefault();let c=o;r.key==="ArrowRight"&&(c=(o+1)%e.tabs.length),r.key==="ArrowLeft"&&(c=(o-1+e.tabs.length)%e.tabs.length),r.key==="Home"&&(c=0),r.key==="End"&&(c=e.tabs.length-1),n.value=e.tabs[c].id,requestAnimationFrame(()=>{var d;return(d=document.getElementById("tab-"+e.tabs[c].id))==null?void 0:d.focus()})}return{activeTab:n,activeComponent:a,activeLabel:i,onTabKeydown:l}},template:`
...
... (truncated)`:s}function Du(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function wm(e){return e==null||!isFinite(e)?"—":Number(e).toLocaleString()}function km(e){return e==null||!isFinite(e)?"—":e>=1e3?`${(e/1e3).toFixed(1)}k`:String(e)}const Sm=Symbol("agent-detail-cancelled"),iw=15e3;function lw(e,{timeoutMs:t,timeoutLabel:s,scheduleTimeout:n,cancelTimeout:a}){const i=typeof AbortController=="function"?new AbortController:null;let l=null,r=!1,o,c;const d=new Promise((f,m)=>{o=f,c=m});function u(f,m){r||(r=!0,l!==null&&a(l),l=null,(f?o:c)(m))}let p;try{p=e(i==null?void 0:i.signal)}catch(f){u(!1,f)}return r||Promise.resolve(p).then(f=>u(!0,f),f=>u(!1,f)),!r&&Number.isFinite(t)&&t>0&&(l=n(()=>{const f=Math.max(1,Math.round(t/1e3));u(!1,new Error(`${s} request timed out after ${f}s`)),i==null||i.abort()},t)),{promise:d,cancel(){u(!0,Sm),i==null||i.abort()}}}function Tm({state:e,requestDetail:t,timeoutMs:s=iw,detailLabel:n="Agent detail",scheduleTimeout:a=globalThis.setTimeout.bind(globalThis),cancelTimeout:i=globalThis.clearTimeout.bind(globalThis)}){if(!e||typeof e!="object")throw new TypeError("agent detail state is required");if(typeof t!="function")throw new TypeError("requestDetail must be a function");let l=null;function r(){const p=l;l=null,p==null||p.cancel()}function o(p,{initial:f,coalesce:m}){if(!p)return Promise.resolve();if(m&&l&&l.agentId===p&&e.detailId===p)return l.promise;r();const b={agentId:p,cancel:null,promise:null};l=b,f?(e.detail=null,e.detailError=null,e.detailLoading=!0):e.detail===null&&e.detailError===null&&(e.detailLoading=!0);const E=lw(I=>t(p,{signal:I}),{timeoutMs:s,timeoutLabel:n,scheduleTimeout:a,cancelTimeout:i});return b.cancel=E.cancel,b.promise=(async()=>{let I=null,w=null;try{I=await E.promise}catch(g){w=g}I!==Sm&&(l!==b||e.detailId!==p||(l=null,!w&&(I===null||typeof I!="object")&&(w=new Error(`${n} response was empty or invalid`)),w?e.detail===null&&(e.detailError=(w==null?void 0:w.message)||`Failed to load ${n.toLowerCase()}`):(e.detail=I,e.detailError=null),e.detailLoading=!1))})(),b.promise}function c(p){return e.detailId=p,o(p,{initial:!0,coalesce:!1})}function d(){const p=e.detailId;return p?o(p,{initial:!1,coalesce:!0}):Promise.resolve()}function u(){r(),e.detailId=null,e.detail=null,e.detailError=null,e.detailLoading=!1}return{open:c,refresh:d,close:u,hasInFlight:()=>l!==null}}function rw({isEnabled:e,refreshList:t,hasOpenDetail:s,refreshDetail:n,intervalMs:a=5e3,scheduleInterval:i=globalThis.setInterval.bind(globalThis),cancelInterval:l=globalThis.clearInterval.bind(globalThis)}){let r=null;function o(){e()&&(t(),s()&&n())}function c(){r!==null&&(l(r),r=null)}function d(){c(),e()&&(r=i(o,a))}function u(){e()?d():c()}return{start:d,stop:c,sync:u,isRunning:()=>r!==null}}const ow={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(null),a=h(!0),i=h("all");let l=!1;const r=W(()=>e.value.filter(R=>R.status==="running").length),o=W(()=>e.value.filter(R=>R.status==="completed").length),c=W(()=>e.value.filter(R=>["failed","timeout","killed"].includes(R.status)).length),d=W(()=>[{value:"all",label:"All",count:e.value.length},{value:"running",label:"Running",count:r.value},{value:"completed",label:"Completed",count:o.value},{value:"failed",label:"Failed",count:c.value}]),u=W(()=>i.value==="all"?e.value:i.value==="failed"?e.value.filter(R=>["failed","timeout","killed"].includes(R.status)):e.value.filter(R=>R.status===i.value));function p(R){const ee=Number(R.max_iterations)||0;return ee<=0?0:Math.min(100,Math.round(R.iteration_count/ee*100))}function f(R){return(Number(R.max_iterations)||0)>0}function m(R,ee){return R?R==="N/A"?"N/A":ee==="current_inheritance"?`inherit (currently ${R})`:R:"unknown"}function b(R){return m(R.display_model,R.display_model_source||R.display_source)}function E(R){return m(R.display_reasoning_effort,R.display_reasoning_effort_source||R.display_source)}function I(R){return{last_execution:"last executed",current_inheritance:"inherited from current config — not yet executed",spawn_override_pending:"requested at spawn — not yet executed",unknown:"no execution data"}[R]||""}const w=h(null),g=h(null),_=h(!1),C=h(null),v=h(""),T=Tm({state:{get detail(){return w.value},set detail(R){w.value=R},get detailId(){return g.value},set detailId(R){g.value=R},get detailLoading(){return _.value},set detailLoading(R){_.value=R},get detailError(){return C.value},set detailError(R){C.value=R}},requestDetail:(R,{signal:ee})=>K.get(`/api/agents/${encodeURIComponent(R)}`,{signal:ee})});async function y(R){v.value="",await T.open(R.id)}function O(){T.close(),v.value=""}async function $(){await T.refresh()}async function F(R,ee){try{await navigator.clipboard.writeText(ee||""),v.value=R,setTimeout(()=>{v.value===R&&(v.value="")},1500)}catch{Ie.error("Copy failed")}}async function N(R=!1){R=R===!0,R||(t.value=!0);try{const ee=await K.get("/api/agents");e.value=Array.isArray(ee)?ee:[],s.value=null}catch(ee){R||(s.value=ee.message)}R||(t.value=!1)}async function G(R){const ee=e.value.find(ge=>ge.id===R);if(await Jt({title:"Kill agent",message:`Kill agent "${(ee==null?void 0:ee.label)||R}"? Its current work will be lost.`,confirmLabel:"Kill",danger:!0})){n.value=R;try{await K.del(`/api/agents/${encodeURIComponent(R)}`),Ie.success("Agent killed"),await N()}catch(ge){Ie.error(ge.message||"Failed to kill agent")}n.value=null}}const z=rw({isEnabled:()=>a.value&&l,refreshList:()=>N(!0),hasOpenDetail:()=>!!g.value,refreshDetail:$});function P(){z.start()}function D(){z.stop()}return is(a,()=>z.sync()),Ke(()=>{l=!0,N(),P()}),_s(()=>{l=!0,N(!0),P()}),ws(()=>{l=!1,D()}),_t(()=>{l=!1,D(),T.close()}),{agents:e,loading:t,error:s,killing:n,autoRefresh:a,statusFilter:i,runningCount:r,completedCount:o,failedCount:c,statusFilters:d,filteredAgents:u,formatTs:xa,formatDuration:ai,progressPercent:p,hasProgress:f,displayModelText:b,displayEffortText:E,displaySourceLabel:I,detail:w,detailId:g,detailLoading:_,detailError:C,copied:v,openDetail:y,closeDetail:O,copyText:F,fetchAgents:N,killAgent:G,startAutoRefresh:P,stopAutoRefresh:D}}},cw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(!1),a=h({goal:"",interval_seconds:60,mode:"notify",max_iterations:50,stop_condition:"",channel_id:""}),i=h(!1),l=h(null),r=h(null),o=h(null),c=h(null),d=h(null),u=h(!1),p=h(null),f=h("");let m=!1;const E=Tm({state:{get detail(){return c.value},set detail(D){c.value=D},get detailId(){return d.value},set detailId(D){d.value=D},get detailLoading(){return u.value},set detailLoading(D){u.value=D},get detailError(){return p.value},set detailError(D){p.value=D}},detailLabel:"Loop detail",requestDetail:(D,{signal:R})=>K.get(`/api/loops/${encodeURIComponent(D)}?limit=100`,{signal:R})});async function I(D){f.value="",await E.open(D.id)}function w(){E.close(),f.value=""}async function g(D,R){try{await navigator.clipboard.writeText(R||""),f.value=D,setTimeout(()=>{f.value===D&&(f.value="")},1500)}catch{Ie.error("Copy failed")}}const _=W(()=>e.value.reduce((D,R)=>D+(R.iteration_count||0),0)),C=W(()=>e.value.filter(D=>D.status==="running").length);function v(D){return D==="running"?"loop-status-running":D==="error"?"loop-status-error":"loop-status-stopped"}function S(D){return D==="running"?"badge-success":D==="error"?"badge-danger":D==="completed"?"badge-info":"badge-warning"}function T(D){return D==="act"?"badge-warning":D==="silent"?"badge-info":"badge-success"}async function y(D=!1){D=D===!0,D||(t.value=!0);try{const R=await K.get("/api/loops");e.value=Array.isArray(R)?R:[],s.value=null}catch(R){D||(s.value=R.message)}D||(t.value=!1)}async function O(){l.value=null;const D=a.value;if(!D.goal.trim()){l.value="Goal is required";return}if(!D.channel_id.trim()){l.value="Channel ID is required";return}const R={goal:D.goal.trim(),channel_id:D.channel_id.trim(),interval_seconds:D.interval_seconds||60,mode:D.mode,max_iterations:D.max_iterations||50};D.stop_condition.trim()&&(R.stop_condition=D.stop_condition.trim()),i.value=!0;try{const ee=await K.post("/api/loops",R);Ie.success(`Loop started: ${ee.loop_id}`),a.value={goal:"",interval_seconds:60,mode:"notify",max_iterations:50,stop_condition:"",channel_id:""},n.value=!1,await y()}catch(ee){l.value=ee.message}i.value=!1}async function $(D){if(await Jt({title:"Stop loop",message:`Stop loop ${D}? The current iteration will finish before stopping.`,confirmLabel:"Stop Loop",danger:!0})){r.value=D;try{await K.del(`/api/loops/${encodeURIComponent(D)}`),Ie.success("Loop stopped"),await y()}catch(ee){Ie.error(ee.message||"Failed to stop loop")}r.value=null}}async function F(D){o.value=D;try{await K.post(`/api/loops/${encodeURIComponent(D)}/restart`),Ie.success("Loop restarted"),await y()}catch(R){Ie.error(R.message||"Failed to restart loop")}o.value=null}function N(D){m&&D.payload&&(D.payload.loop_id||D.payload.type==="loop")&&(y(!0),d.value&&E.refresh())}let G=null;function z(){G!==null&&clearInterval(G),G=null}function P(){z(),m&&(G=setInterval(()=>{y(!0),d.value&&E.refresh()},5e3))}return Ke(()=>{m=!0,y(),Je.subscribe("events",N),P()}),_s(()=>{m=!0,y(!0),P()}),ws(()=>{m=!1,z()}),_t(()=>{m=!1,Je.unsubscribe("events",N),z(),E.close()}),{loops:e,loading:t,error:s,showCreate:n,form:a,creating:i,createError:l,stoppingId:r,restartingId:o,detail:c,detailId:d,detailLoading:u,detailError:p,copied:f,totalIterations:_,runningCount:C,statusDotClass:v,statusBadge:S,modeBadge:T,formatAge:xm,formatDuration:ai,formatTs:xa,formatTokens:km,openDetail:I,closeDetail:w,copyText:g,fetchLoops:y,doCreate:O,doStop:$,doRestart:F}}},dw={template:`
    <div class="p-6 page-fade-in">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(!1),a=h({description:"",action:"reminder",channel_id:"",cron:"",run_at:"",message:"",tool_name:"",tool_input_str:"",report_format:""}),i=h(!1),l=h(null),r=h(null),o=W(()=>hw(a.value.run_at));is(()=>a.value.run_at,()=>{r.value=null});const c=W(()=>{var U;const J=o.value;return J.state==="ok"?J.instant:J.state==="ambiguous"&&r.value!==null&&((U=J.options[r.value])==null?void 0:U.instant)||null}),d=W(()=>{const J=c.value;return J?`${J.toLocaleString()} local — ${J.toISOString()} UTC`:""}),u=h(null),p=h(!1),f=[{label:"Every hour",expr:"0 * * * *"},{label:"Every 6h",expr:"0 */6 * * *"},{label:"Daily 9am",expr:"0 9 * * *"},{label:"Weekly Mon",expr:"0 9 * * 1"},{label:"Every 30m",expr:"*/30 * * * *"}],m=h(null),b=h(null),E=h(null),I=h(null),w=h(null),g=h(null),_=h([]),C=h(!1),v=h("");let S=0;const T=W(()=>e.value.filter(J=>J.cron&&!J.one_time).length),y=W(()=>e.value.filter(J=>J.one_time).length),O=W(()=>e.value.filter(J=>J.trigger).length),$=W(()=>e.value.filter(J=>J.paused).length),F=W(()=>e.value.filter(J=>J.consecutive_failures>0).length);function N(J){if(!J)return"-";const U=Date.now(),re=(new Date(J).getTime()-U)/1e3;if(re<0)return"overdue";if(re<60)return"in < 1 min";if(re<3600)return`in ${Math.floor(re/60)} min`;if(re<86400){const _e=Math.floor(re/3600),Ue=Math.floor(re%3600/60);return Ue>0?`in ${_e}h ${Ue}m`:`in ${_e}h`}const Se=Math.floor(re/86400);return`in ${Se} day${Se!==1?"s":""}`}function G(J){return J==null?"-":J<1e3?`${J}ms`:J<6e4?`${(J/1e3).toFixed(1)}s`:ai(J/1e3)}function z(J=a.value.cron){a.value.cron=J,Pu(a.value,"cron"),u.value=null}function P(J=a.value.run_at){a.value.run_at=J,Pu(a.value,"run_at"),u.value=null}async function D(){const J=a.value.cron.trim();if(J){p.value=!0;try{u.value=await K.post("/api/schedules/validate-cron",{expression:J})}catch(U){u.value={valid:!1,error:U.message}}p.value=!1}}async function R(){t.value=!0,s.value=null;try{e.value=await K.get("/api/schedules")}catch(J){s.value=J.message}t.value=!1}async function ee(J){if(g.value===J){g.value=null,_.value=[];return}g.value=J,C.value=!0,_.value=[];const U=++S;try{const le=await K.get(`/api/schedules/${encodeURIComponent(J)}/history?limit=10`);if(U!==S||g.value!==J)return;_.value=le,v.value=""}catch(le){if(U!==S||g.value!==J)return;_.value=[],v.value=le.message||"Failed to load execution history"}U===S&&(C.value=!1)}async function ne(){l.value=null;const J=a.value;if(!J.description.trim()){l.value="Description is required";return}if(!J.channel_id.trim()){l.value="Channel ID is required";return}if(!J.cron.trim()&&!J.run_at.trim()){l.value="Cron expression or run_at time is required";return}if(J.cron.trim()&&J.run_at.trim()){l.value="Choose either Cron or One-Time, not both";return}const U={description:J.description.trim(),action:J.action,channel_id:J.channel_id.trim()};if(J.cron.trim()&&(U.cron=J.cron.trim()),J.run_at.trim()){const le=o.value;if(le.state==="nonexistent"){l.value="That local time does not exist (daylight saving gap)";return}if(le.state==="invalid"){l.value="One-time run time is not a valid date";return}const re=c.value;if(le.state==="ambiguous"&&r.value===null){l.value="That local time happens twice — choose which occurrence to use";return}if(!re){l.value="One-time run time could not be resolved";return}U.run_at=re.toISOString()}if(J.action==="reminder"&&J.message.trim()&&(U.message=J.message.trim()),J.action==="check"&&(J.tool_name.trim()&&(U.tool_name=J.tool_name.trim()),J.report_format&&(U.report_format=J.report_format),J.tool_input_str.trim()))try{U.tool_input=JSON.parse(J.tool_input_str.trim())}catch{l.value="Tool input must be valid JSON";return}i.value=!0;try{await K.post("/api/schedules",U),Ie.success("Schedule created"),a.value={description:"",action:"reminder",channel_id:"",cron:"",run_at:"",message:"",tool_name:"",tool_input_str:"",report_format:""},u.value=null,n.value=!1,await R()}catch(le){l.value=le.message}i.value=!1}async function ge(J){m.value=J;try{const U=await K.post(`/api/schedules/${encodeURIComponent(J)}/run`);if(U.status==="failure")Ie.error(`Execution failed: ${U.error||"unknown error"}`);else{const le=U.warning?`Executed (${U.warning})`:"Executed successfully";Ie.success(le)}await R()}catch(U){Ie.error(U.message||"Failed to trigger")}m.value=null}async function ie(J){E.value=J.id;const U=!J.paused;try{await K.put(`/api/schedules/${encodeURIComponent(J.id)}`,{paused:U}),Ie.success(U?"Schedule paused":"Schedule resumed"),await R()}catch(le){Ie.error(le.message||"Failed to update schedule")}E.value=null}async function be(J,U){w.value=J.id;try{await K.put(`/api/schedules/${encodeURIComponent(J.id)}`,{report_format:U}),Ie.success(U?"Structured report enabled":"Plain-text report enabled")}catch(le){Ie.error(`Update failed: ${le.message}`)}finally{await R(),w.value=null}}async function Q(J){I.value=J;try{await K.post(`/api/schedules/${encodeURIComponent(J)}/reset-failures`),Ie.success("Failure counters reset"),await R()}catch(U){Ie.error(U.message||"Failed to reset")}I.value=null}async function ye(J){const U=e.value.find(re=>re.id===J);if(await Jt({title:"Delete schedule",message:`Delete "${(U==null?void 0:U.description)||J}"? This cannot be undone.`,confirmLabel:"Delete",danger:!0})){b.value=J;try{await K.del(`/api/schedules/${encodeURIComponent(J)}`),Ie.success("Schedule deleted"),await R()}catch(re){Ie.error(re.message||"Failed to delete schedule")}b.value=null}}return Ke(()=>{R()}),{schedules:e,loading:t,error:s,showCreate:n,form:a,creating:i,createError:l,runAtUtcPreview:d,runAtAnalysis:o,runAtOccurrence:r,cronResult:u,validatingCron:p,cronPresets:f,runningId:m,deletingId:b,togglingId:E,resettingId:I,reportUpdatingId:w,expandedId:g,history:_,historyLoading:C,historyError:v,cronCount:T,oneTimeCount:y,webhookCount:O,pausedCount:$,failingCount:F,formatTs:xa,formatAge:xm,formatFuture:N,formatMs:G,formatDuration:ai,onCronInput:z,onRunAtInput:P,validateCron:D,toggleExpand:ee,fetchSchedules:R,doCreate:ne,doRunNow:ge,doTogglePause:ie,doUpdateReportFormat:be,doResetFailures:Q,doDelete:ye}}},Cm=[{id:"live",label:"Live",component:nw},{id:"agents",label:"Agents",component:ow},{id:"loops",label:"Loops",component:cw},{id:"processes",label:"Processes",component:dw},{id:"schedules",label:"Schedules",component:mw}],vw={components:{TabbedPage:$r},setup(){return{tabs:Cm}},template:'<tabbed-page :tabs="tabs" default-tab="live" group-label="Operations" />'},gw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(null),a=h(null),i=h(!1);let l=0;const r=h(null),o=h(!1),c=h(new Set),d=h(!1),u=h("all"),p=h(""),f=h("last_active"),m=h(!1),b=Fu,E=bw,I=h([]),w=h(!1),g=h(""),_=h("flat"),C=h(new Set),v=h(""),S=h(""),T=h(""),y=h(null),O=h(!1);function $(){try{const te=localStorage.getItem("odin-session-presets");te&&(I.value=JSON.parse(te))}catch{}}function F(){try{localStorage.setItem("odin-session-presets",JSON.stringify(I.value))}catch{}}const N=W(()=>p.value.trim()!==""||u.value!=="all"),G=W(()=>{let te=[...e.value];const Oe=Fu.find(je=>je.id===u.value),Le=Oe?Oe.filters:{};if(Le.source&&(te=te.filter(je=>je.source===Le.source)),Le.minMessages&&(te=te.filter(je=>je.message_count>=Le.minMessages)),Le.hasCompaction&&(te=te.filter(je=>je.has_summary)),Le.maxAge!=null){const je=Date.now()/1e3;te=te.filter(pt=>pt.last_active&&je-pt.last_active<=Le.maxAge)}if(p.value.trim()){const je=p.value.toLowerCase().trim();te=te.filter(pt=>(pt.channel_id||"").toLowerCase().includes(je)||(pt.last_user_id||"").toLowerCase().includes(je)||(pt.source||"").toLowerCase().includes(je))}const Me=f.value,Qe=m.value?1:-1;return te.sort((je,pt)=>{const Tt=je[Me]||0,Hs=pt[Me]||0;return(Tt-Hs)*Qe}),te}),z=W(()=>{if(!a.value||!a.value.messages)return[];const te=a.value.messages;if(te.length===0)return[];const Oe=[];let Le=[];for(const Me of te)Me.role==="user"&&Le.length>0&&(Oe.push(Le),Le=[]),Le.push(Me);return Le.length>0&&Oe.push(Le),Oe}),P=W(()=>G.value.length>0&&c.value.size===G.value.length);function D(te){const Oe=te.find(Le=>Le.role==="user");if(Oe&&Oe.content){const Le=Oe.content.slice(0,120);return Le.length<Oe.content.length?Le+"...":Le}return"(no user message)"}function R(te){const Oe=new Set(C.value);Oe.has(te)?Oe.delete(te):Oe.add(te),C.value=Oe}function ee(te){u.value=te}function ne(te){u.value=te.id,te.filters.searchQuery!=null&&(p.value=te.filters.searchQuery),te.filters.sortBy&&(f.value=te.filters.sortBy)}function ge(){if(!g.value.trim())return;const te={id:"custom-"+Date.now(),name:g.value.trim(),filters:{searchQuery:p.value,sortBy:f.value}};I.value=[...I.value,te],F(),w.value=!1,g.value=""}function ie(te){I.value=I.value.filter(Oe=>Oe.id!==te),F(),u.value===te&&(u.value="all")}function be(){u.value="all",p.value="",f.value="last_active",m.value=!1}function Q(te){if(!te)return"—";const Oe=Date.now()/1e3-te;if(Oe<60)return"just now";if(Oe<3600){const Me=Math.floor(Oe/60);return`${Me} minute${Me!==1?"s":""} ago`}if(Oe<86400){const Me=Math.floor(Oe/3600);return`${Me} hour${Me!==1?"s":""} ago`}const Le=Math.floor(Oe/86400);return`${Le} day${Le!==1?"s":""} ago`}function ye(te){if(!te)return"";try{return new Date(te*1e3).toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"})}catch{return""}}function J(te){if(!te)return"";try{return new Date(te*1e3).toLocaleString()}catch{return""}}function U(te){return te==="user"?"bg-gray-900/50 border border-gray-800":te==="assistant"?"bg-indigo-950/30 border border-indigo-900/30":"bg-gray-900/30 border border-gray-800/50"}function le(te){return te==="user"?"sess-msg-user":te==="assistant"?"sess-msg-assistant":"sess-msg-system"}function re(te){return te==="user"?"badge-info":te==="assistant"?"badge-success":"badge-warning"}function Se(te){return te==="user"?"sess-dot-user":te==="assistant"?"sess-dot-assistant":"sess-dot-system"}function _e(te){return te==="user"?"text-cyan-400":te==="assistant"?"text-indigo-400":"text-gray-500"}function Ue(te){return te?te.length>2e3?te.slice(0,2e3)+`
... (truncated)`:te:""}async function k(){const te=v.value.trim();if(te){O.value=!0;try{let Oe=`/api/sessions/search?q=${encodeURIComponent(te)}&limit=50`;S.value.trim()&&(Oe+=`&channel_id=${encodeURIComponent(S.value.trim())}`),T.value.trim()&&(Oe+=`&user_id=${encodeURIComponent(T.value.trim())}`);const Le=await K.get(Oe);y.value=Le.results||[]}catch{y.value=[]}O.value=!1}}function L(){v.value="",S.value="",T.value="",y.value=null}function B(te){return te?te.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/&gt;&gt;&gt;/g,'<mark class="fts-highlight">').replace(/&lt;&lt;&lt;/g,"</mark>"):""}function se(te){return te==="user"?"fts-result-user":te==="assistant"?"fts-result-assistant":te==="summary"?"fts-result-summary":te==="fts"?"fts-result-fts":te==="channel"?"fts-result-channel":"fts-result-default"}function Y(te){return te==="user"?"badge-info":te==="assistant"?"badge-success":te==="summary"?"badge-warning":te==="fts"?"badge-success":"badge-info"}async function X(){t.value=!0,s.value=null;try{e.value=await K.get("/api/sessions")}catch(te){s.value=te.message}t.value=!1}function me(){s.value=null,X()}async function ce(te){if(n.value===te){n.value=null,a.value=null,C.value=new Set;return}n.value=te,a.value=null,i.value=!0,C.value=new Set;const Oe=++l;try{const Le=await K.get(`/api/sessions/${encodeURIComponent(te)}`);Oe===l&&n.value===te&&(a.value=Le)}catch(Le){Oe===l&&n.value===te&&(a.value={messages:[],summary:"",error:Le.message||"Failed to load session"})}finally{Oe===l&&(i.value=!1)}}function de(te){const Oe=new Set(c.value);Oe.has(te)?Oe.delete(te):Oe.add(te),c.value=Oe}function ae(){P.value?c.value=new Set:c.value=new Set(G.value.map(te=>te.channel_id))}function we(te){r.value=te}async function ve(){if(r.value){o.value=!0;try{await K.del(`/api/sessions/${encodeURIComponent(r.value)}`),n.value===r.value&&(n.value=null,a.value=null),c.value.delete(r.value),await X()}catch(te){s.value=te.message||"Failed to clear session"}o.value=!1,r.value=null}}function xe(){d.value=!0}async function oe(){if(c.value.size!==0){o.value=!0;try{await K.post("/api/sessions/clear-bulk",{channel_ids:[...c.value]}),c.value.has(n.value)&&(n.value=null,a.value=null),c.value=new Set,await X()}catch(te){s.value=te.message||"Failed to clear sessions"}o.value=!1,d.value=!1}}async function H(te,Oe){const Le=`/api/sessions/${encodeURIComponent(te)}/export?format=${Oe}`;try{const Me=await K.getBlob(Le),Qe=URL.createObjectURL(Me),je=document.createElement("a");je.href=Qe,je.download=`session-${te}.${Oe==="text"?"txt":"json"}`,je.click(),URL.revokeObjectURL(Qe)}catch(Me){s.value=Me.message||"Failed to export session"}}let pe=null;function Te(te){te.payload&&te.payload.channel_id&&(clearTimeout(pe),pe=setTimeout(()=>{if(X(),n.value&&te.payload.channel_id===n.value){const Oe=n.value,Le=l;K.get(`/api/sessions/${encodeURIComponent(Oe)}`).then(Me=>{Le!==l||n.value!==Oe||(a.value=Me)}).catch(()=>{})}},2e3))}let Ae=!1,De=null;function ut(){Ae||(Ae=!0,X(),Je.subscribe("events",Te),De=Je.onReconnected(()=>X()))}Ke(()=>{$(),ut()}),_s(()=>{ut()});function it(){Ae&&(Ae=!1,Je.unsubscribe("events",Te),De&&(De(),De=null),clearTimeout(pe))}return ws(it),_t(it),{sessions:e,loading:t,error:s,expandedId:n,detail:a,detailLoading:i,clearTarget:r,clearing:o,selected:c,allSelected:P,bulkClearing:d,activePreset:u,searchQuery:p,sortBy:f,sortAsc:m,filterPresets:b,sortOptions:E,filteredSessions:G,hasActiveFilters:N,customPresets:I,showSavePreset:w,newPresetName:g,threadView:_,threads:z,collapsedThreads:C,ftsQuery:v,ftsChannelId:S,ftsUserId:T,ftsResults:y,ftsSearching:O,formatAge:Q,formatTimestamp:ye,formatFullTimestamp:J,messageClass:U,threadMsgClass:le,roleBadge:re,roleDotClass:Se,roleLabelClass:_e,truncateContent:Ue,threadSummary:D,fetchSessions:X,retry:me,toggleSession:ce,toggleSelect:de,toggleSelectAll:ae,confirmClear:we,clearSession:ve,confirmBulkClear:xe,doBulkClear:oe,exportSession:H,applyPreset:ee,applyCustomPreset:ne,saveCustomPreset:ge,removeCustomPreset:ie,resetFilters:be,toggleThread:R,runFtsSearch:k,clearFtsSearch:L,highlightSnippet:B,ftsResultClass:se,ftsTypeBadge:Y}}},xw={props:["trace"],template:`
  ...
  `,setup(){return{formatTokens:km}}},_w={components:{ContextAssemblyPanel:xw},template:`
    ...
    </div>`,setup(){const e=h([]),t=h([]),s=h(!0),n=h(null),a=h(null),i=h(null),l=h(""),r=h(""),o=h(0),c=h({}),d=h({channel_id:"",user_id:"",tool_name:"",errors_only:!1,limit:50});function u(S){if(!S)return"—";try{const T=new Date(S);return isNaN(T.getTime())?S:T.toLocaleString([],{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"})}catch{return S}}function p(S){return!S&&S!==0?"—":S<1e3?S+"ms":(S/1e3).toFixed(1)+"s"}function f(S){return!S&&S!==0?"—":S>=1e3?(S/1e3).toFixed(1)+"k":String(S)}function m(S){if(!S)return"";if(typeof S=="string")return S;try{return JSON.stringify(S,null,2)}catch{return String(S)}}function b(S){a.value===S?a.value=null:(a.value=S,c.value={})}function E(S,T){const y=S+"-"+T;c.value={...c.value,[y]:!c.value[y]}}function I(S,T){return!!c.value[S+"-"+T]}function w(){d.value={channel_id:"",user_id:"",tool_name:"",errors_only:!1,limit:50},r.value="",l.value="",i.value=null,C()}async function g(){try{const S=await K.get("/api/trajectories");e.value=S.files||[],o.value=S.count||0}catch{}}let _=0;async function C(){const S=++_;s.value=!0,n.value=null,a.value=null,i.value=null,c.value={};try{if(r.value){const T=await K.get(`/api/trajectories/${encodeURIComponent(r.value)}?limit=${d.value.limit}`);if(S!==_)return;let y=T.entries||[];d.value.tool_name&&(y=y.filter(O=>(O.tools_used||[]).includes(d.value.tool_name))),d.value.errors_only&&(y=y.filter(O=>O.is_error)),d.value.channel_id&&(y=y.filter(O=>O.channel_id===d.value.channel_id)),d.value.user_id&&(y=y.filter(O=>O.user_id===d.value.user_id)),t.value=y}else{const T=new URLSearchParams;d.value.channel_id&&T.set("channel_id",d.value.channel_id),d.value.user_id&&T.set("user_id",d.value.user_id),d.value.tool_name&&T.set("tool_name",d.value.tool_name),d.value.errors_only&&T.set("errors_only","true"),T.set("limit",String(d.value.limit));const y=T.toString(),O=await K.get(`/api/trajectories/search/query?${y}`);if(S!==_)return;t.value=O.results||[]}}catch(T){if(S!==_)return;n.value=T.message}S===_&&(s.value=!1)}async function v(){if(!l.value.trim())return;const S=++_;s.value=!0,n.value=null,c.value={};try{const T=await K.get(`/api/trajectories/message/${encodeURIComponent(l.value.trim())}`);if(S!==_)return;i.value=T.entry||null,i.value||(n.value="No trace found for this message ID")}catch(T){if(S!==_)return;T.status===404?(i.value=null,n.value="No trace found for message ID: "+l.value):n.value=T.message}S===_&&(s.value=!1)}return Ke(async()=>{await g(),await C()}),{files:e,entries:t,loading:s,error:n,expandedIdx:a,singleTrace:i,messageIdQuery:l,selectedFile:r,totalSaved:o,filters:d,expandedIterations:c,formatTs:u,formatDuration:p,formatTokens:f,formatJSON:m,truncateBlock:_m,toggleExpand:b,toggleIteration:E,isIterationExpanded:I,clearFilters:w,fetchFiles:g,fetchTraces:C,lookupMessage:v}}},ww={template:`
  ...
  `,setup(){const e=h(!0),t=h(null),s=h(!1),n=h({by_user:{},by_channel:{},by_tool:{},recent:[],pricing:{}}),a=h({requests:0,input_tokens:0,output_tokens:0,total_tokens:0,cost_usd:0}),i=h("user");let l=null;const r=[{key:"user",label:"By User"},{key:"channel",label:"By Channel"},{key:"tool",label:"By Tool"},{key:"recent",label:"Recent"}],o=W(()=>[...n.value.recent||[]].reverse()),c=async()=>{try{const m=await K.get("/api/usage");n.value=m,a.value=m.totals||a.value,t.value=null,s.value=!0}catch(m){t.value=m.message}finally{e.value=!1}},d=()=>{e.value=!0,c()};let u=!1;function p(){u||(u=!0,c(),l||(l=setInterval(c,15e3)))}function f(){u&&(u=!1,l&&(clearInterval(l),l=null))}return Ke(p),_s(p),ws(f),_t(f),{hasData:s,loading:e,error:t,data:n,totals:a,activeTab:i,tabs:r,recentReversed:o,fmtNum:wm,formatTime:ym,retry:d}}},Em=[{id:"audit",label:"Audit",component:gw},{id:"sessions",label:"Sessions",component:yw},{id:"traces",label:"Traces",component:_w},{id:"usage",label:"Usage",component:ww}],kw={components:{TabbedPage:$r},setup(){return{tabs:Em}},template:'<tabbed-page :tabs="tabs" default-tab="audit" group-label="History" />'},io=[{id:"system",label:"System & Commands",icon:"terminal",match:e=>/^(run_command|run_script|read_file|write_file|list_directory|search_files|manage_process|file_|post_file)/.test(e)},{id:"devops",label:"DevOps & Infrastructure",icon:"server",match:e=>/^(git_ops|docker_ops|kubectl|terraform_ops|http_probe)/.test(e)},{id:"agents",label:"Agents & Orchestration",icon:"bot",match:e=>/^(spawn_agent|send_to_agent|wait_for_agents|get_agent_results|kill_agent|list_agents|spawn_loop_agents|collect_loop_agents)/.test(e)},{id:"workflow",label:"Workflows & Tasks",icon:"workflow",match:e=>/^(delegate_task|cancel_task|list_tasks|schedule_|start_loop|stop_loop|list_loops|delete_schedule|list_schedules|update_schedule|parse_time)/.test(e)},{id:"network",label:"Network & Web",icon:"globe",match:e=>/^(web_|browser_|search_web|fetch_url|http_)/.test(e)},{id:"knowledge",label:"Knowledge & Search",icon:"book",match:e=>/^(search_knowledge|ingest_|knowledge_|search_history|search_audit|bulk_ingest|delete_knowledge|list_knowledge)/.test(e)},{id:"discord",label:"Discord & Admin",icon:"message",match:e=>/^(send_|add_reaction|create_poll|purge_|discord_|embed_|read_channel|set_permission)/.test(e)},{id:"skills",label:"Skills",icon:"puzzle",match:e=>/^(create_skill|edit_skill|delete_skill|enable_skill|disable_skill|install_skill|export_skill|skill_status|invoke_skill|list_skills)/.test(e)},{id:"memory",label:"Memory & State",icon:"brain",match:e=>/^(memory_manage|list_manage)/.test(e)},{id:"ai",label:"AI & Generation",icon:"sparkles",match:e=>/^(generate_|analyze_|claude_|vision_|comfyui_)/.test(e)},{id:"integrations",label:"Integrations",icon:"link",match:e=>/^(issue_tracker|slack_|grafana_|mcp_)/.test(e)},{id:"other",label:"Other Tools",icon:"wrench",match:()=>!0}],Sw={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(""),a=h({}),i=h({}),l=h("cards"),r=h(null),o=h(null),c=h(!1),d=h(new Set),u={disabled:"Disabled by operator",unavailable:"Unavailable — required backend is not configured",global_disabled:"Global tools disabled"};function p(y){return y.source!=="builtin"?"":u[y.state]||""}function f(y,O){const $=y&&Array.isArray(y.tools)?y.tools:null;if(c.value=!!$,o.value=$?!!y.global_enabled:null,!$){e.value=O.map(G=>({...G,source:"unknown",enabled:void 0,state:null}));return}const F=new Set($.map(G=>G.name)),N=O.filter(G=>!F.has(G.name)).map(G=>({...G,source:G.name.startsWith("mcp_")?"mcp":"skill",enabled:!0,state:null}));e.value=[...$.map(G=>({...G,source:"builtin"})),...N]}async function m(y,O){if(d.value.has(y.name))return;const $=!!O.target.checked,F=new Set(d.value);F.add(y.name),d.value=F;try{const N=await K.post(`/api/tools/builtins/${encodeURIComponent(y.name)}/enabled`,{enabled:$});f(N,e.value),s.value=null;try{const G=await K.get("/api/tools");f(N,G)}catch(G){console.warn("Built-in toggle committed; visible catalog refresh failed",G)}}catch(N){O.target.checked=!!y.enabled,s.value=N.message||`Failed to toggle ${y.name}`}finally{const N=new Set(d.value);N.delete(y.name),d.value=N}}const b=W(()=>e.value.filter(y=>y.source==="builtin"&&y.is_core).length),E=W(()=>e.value.filter(y=>y.source==="skill").length),I=W(()=>Object.values(a.value).reduce((y,O)=>y+O,0));function w(y){for(const O of io)if(O.id!=="other"&&O.match(y))return O.id;return"other"}const g=W(()=>{let y=e.value;if(n.value){const O=n.value.toLowerCase();y=y.filter($=>$.name.toLowerCase().includes(O)||($.description||"").toLowerCase().includes(O))}return r.value&&(y=y.filter(O=>w(O.name)===r.value)),y}),_=W(()=>{const y=new Set;for(const O of e.value)y.add(w(O.name));return io.filter(O=>y.has(O.id))}),C=W(()=>{const y=g.value,O={};for(const F of y){const N=w(F.name);O[N]||(O[N]=[]),O[N].push(F)}const $=[];for(const F of io)O[F.id]&&O[F.id].length>0&&$.push({label:F.label,icon:F.icon,tools:O[F.id].sort((N,G)=>N.name.localeCompare(G.name))});return $});function v(y){i.value={...i.value,[y]:!i.value[y]}}async function S(){t.value=!0,s.value=null;try{const[y,O,$]=await Promise.all([K.get("/api/tools"),K.get("/api/tools/stats").catch(()=>({})),K.get("/api/tools/builtins").catch(()=>null)]);f($,y),a.value=O||{}}catch(y){s.value=y.message}t.value=!1}function T(){S()}return Ke(()=>{S()}),{tools:e,loading:t,error:s,search:n,stats:a,expanded:i,viewMode:l,activeCategory:r,globalEnabled:o,inventoryAvailable:c,togglePending:d,coreCount:b,skillCount:E,totalUsage:I,filteredTools:g,groupedTools:C,usedCategories:_,stateBadge:p,applyInventory:f,toggleBuiltinTool:m,truncate:Qc,toggleExpand:v,refresh:T}}};function Tw(e){if(!e)return"";let t=e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");t=t.replace(/("""[\s\S]*?"""|'''[\s\S]*?'''|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g,'<span class="sk-str">$1</span>'),t=t.replace(/(#[^\n]*)/g,'<span class="sk-cmt">$1</span>');const s="\\b(def|class|return|if|elif|else|for|while|import|from|as|try|except|finally|raise|with|async|await|yield|pass|break|continue|and|or|not|in|is|None|True|False|self|lambda)\\b";t=t.replace(new RegExp(s,"g"),'<span class="sk-kw">$1</span>');const n="\\b(print|len|range|str|int|float|list|dict|set|tuple|type|isinstance|hasattr|getattr|setattr|super|property|staticmethod|classmethod|enumerate|zip|map|filter|sorted|reversed|any|all|min|max|sum|abs|round|open|format)\\b";return t=t.replace(new RegExp(n,"g"),'<span class="sk-builtin">$1</span>'),t=t.replace(/(@\w+)/g,'<span class="sk-dec">$1</span>'),t=t.replace(/\b(\d+\.?\d*)\b/g,'<span class="sk-num">$1</span>'),t}function Cw(e){if(!e)return"1";const t=e.split(`
...
`)}const Ew={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h({}),a=h({}),i=h(null),l=h(""),r=h(null),o=h(!1),c=h("create"),d=h(""),u=h(""),p=h(null),f=h(null),m=h(!1),b=h(null),E=h(null),I=h(!1),w=W(()=>e.value.length),g=W(()=>e.value.reduce((Q,ye)=>Q+(ye.execution_count||0),0)),_=W(()=>e.value.reduce((Q,ye)=>Q+O(ye.code),0)),C=W(()=>{if(!l.value)return e.value;const Q=l.value.toLowerCase();return e.value.filter(ye=>ye.name.toLowerCase().includes(Q)||(ye.description||"").toLowerCase().includes(Q))}),v=W(()=>u.value?u.value.split(`
`).length:0),S=W(()=>{const Q=Math.max(v.value,1);return Array.from({length:Q},(ye,J)=>J+1).join(`
`)}),T=W(()=>{const Q=u.value.trim();return Q?Q.includes("SKILL_DEFINITION")?Q.includes("async def execute")?{valid:!0,message:""}:{valid:!1,message:"Missing async def execute function"}:{valid:!1,message:"Missing SKILL_DEFINITION dict"}:null});function y(Q){return Tw(Q)}function O(Q){return Q?Q.split(`
`).length:0}function $(Q){return Cw(Q)}function F(Q){n.value={...n.value,[Q]:!n.value[Q]}}async function N(Q){try{await navigator.clipboard.writeText(Q);const ye=e.value.find(J=>J.code===Q);ye&&(r.value=ye.name,setTimeout(()=>{r.value=null},2e3))}catch{}}function G(Q){if(Q.key==="Tab"){Q.preventDefault();const ye=Q.target,J=ye.selectionStart,U=ye.selectionEnd;u.value=u.value.substring(0,J)+"    "+u.value.substring(U),Et(()=>{ye.selectionStart=ye.selectionEnd=J+4})}}function z(Q){const ye=Q.target.previousElementSibling;ye&&(ye.scrollTop=Q.target.scrollTop)}async function P(){t.value=!0,s.value=null;try{e.value=await K.get("/api/skills")}catch(Q){s.value=Q.message}t.value=!1}async function D(Q){i.value=Q,delete a.value[Q],a.value={...a.value};try{const ye=await K.post(`/api/skills/${encodeURIComponent(Q)}/test`);a.value={...a.value,[Q]:ye}}catch(ye){a.value={...a.value,[Q]:{result:ye.message,is_error:!0}}}i.value=null}function R(){o.value=!0,c.value="create",d.value="",u.value="",p.value=null,f.value=null}function ee(Q){o.value=!0,c.value="edit",d.value=Q.name,u.value=Q.code||"",p.value=null,f.value=null}function ne(){o.value=!1,p.value=null,f.value=null}async function ge(){p.value=null,f.value=null;const Q=d.value.trim(),ye=u.value.trim();if(!Q){p.value="Name is required";return}if(!ye){p.value="Code is required";return}m.value=!0;try{c.value==="create"?(await K.post("/api/skills",{name:Q,code:ye}),f.value="Skill created successfully"):(await K.put(`/api/skills/${encodeURIComponent(Q)}`,{code:ye}),f.value="Skill updated successfully"),await P(),setTimeout(()=>{o.value=!1},800)}catch(J){p.value=J.message}m.value=!1}function ie(Q){E.value=Q}async function be(){if(E.value){I.value=!0;try{await K.del(`/api/skills/${encodeURIComponent(E.value)}`),await P()}catch(Q){Ie.error(`Failed to delete skill: ${Q.message||"unknown error"}`)}I.value=!1,E.value=null}}return Ke(()=>{P()}),{skills:e,loading:t,error:s,showCode:n,testResults:a,testing:i,search:l,copied:r,editing:o,editMode:c,editName:d,editCode:u,editError:p,editSuccess:f,saving:m,editorRef:b,deleteTarget:E,deleting:I,enabledCount:w,totalExecutions:g,totalLines:_,displayedSkills:C,editLineCount:v,editorLineNums:S,editValidation:T,highlight:y,truncate:Qc,formatTs:xa,countLines:O,getLineNumbers:$,toggleCode:F,copyCode:N,handleEditorKey:G,syncScroll:z,fetchSkills:P,testSkill:D,showCreate:R,editSkill:ee,cancelEdit:ne,saveSkill:ge,confirmDelete:ie,doDelete:be}}};class Cs extends Error{constructor(t,s=""){super(t),this.name="MCPFormError",this.field=s}}const Aw=/^[A-Za-z_][A-Za-z0-9_]*$/;function $u(e){return String(e||"").split(/\r?\n/).map(t=>t.trim()).filter(Boolean)}function Bu(e,t,s){const n={},a=[...new Set((t||[]).map(l=>String(l)))],i=new Set(a);for(const l of e||[]){const r=String((l==null?void 0:l.key)||"").trim(),o=String((l==null?void 0:l.value)??"");if(!(!r&&!o)){if(!r)throw new Cs(`${s} key is required when a value is entered.`,"authentication");if(/[\r\n\0]/.test(r))throw new Cs(`${s} keys cannot contain line breaks or NUL bytes.`,"authentication");if(Object.hasOwn(n,r))throw new Cs(`${s} key “${r}” appears more than once.`,"authentication");if(i.has(r))throw new Cs(`${s} key “${r}” cannot be replaced and removed in the same save.`,"authentication");n[r]=o}}return{set:n,remove:a}}function Rw(e){try{const t=new URL(e);return(t.protocol==="http:"||t.protocol==="https:")&&!!t.hostname}catch{return!1}}function Iw(e,{mode:t="add",originalTransport:s=""}={}){const n=t==="add",a=String(e.name||"").trim();if(!a)throw new Cs("Server name is required.","name");if(a.length>128||!Aw.test(a))throw new Cs("Use at most 128 letters, digits, or underscores, with no leading digit.","name");const i=e.transport==="http"?"http":"stdio",l=!n&&!!s&&i!==s,r={enabled:!!e.enabled,transport:i};if(n&&(r.name=a),i==="stdio"){const d=String(e.command||"").trim();if((n||l)&&!d)throw new Cs("An executable path is required for a new stdio connection.","command");if(d&&(r.command=d),(n||e.replaceArgs)&&(r.args=$u(e.argsText)),n||e.replaceCwd){const u=String(e.cwd||"").trim();if(u&&(!u.startsWith("/")||u.includes("\0")))throw new Cs("Working directory must be an absolute path.","cwd");r.cwd=u}}else{const d=String(e.url||"").trim();if((n||l)&&!d)throw new Cs("An HTTP endpoint is required for this connection.","url");if(d&&!Rw(d))throw new Cs("Endpoint must be a valid http:// or https:// URL.","url");d&&(r.url=d)}if(n||e.replaceTimeout){const d=Number(e.timeoutSeconds);if(!Number.isInteger(d)||d<1||d>3600)throw new Cs("Timeout must be a whole number from 1 to 3600 seconds.","timeout");r.timeout_seconds=d}(n||e.replaceAllowlist)&&(r.tool_allowlist=$u(e.allowlistText));const o=Bu(e.headerRows,e.headersRemove,"Header"),c=Bu(e.envRows,e.envRemove,"Environment variable");return Object.keys(o.set).length&&(r.headers_set=o.set),o.remove.length&&(r.headers_remove=o.remove),Object.keys(c.set).length&&(r.env_set=c.set),c.remove.length&&(r.env_remove=c.remove),r}function Ow(e,t){return t?e.transport!==t.transport||!!e.enabled!=!!t.enabled?!0:Object.keys(e).some(s=>!["enabled","transport"].includes(s)):!1}function Lw(e){const t=String(e||"").toLowerCase();return["disabled","connecting","connected","stale","error","blocked"].includes(t)?t:"error"}function Nw(e,t){const s=String(t||"").trim().toLowerCase();return s?[e==null?void 0:e.original_name,e==null?void 0:e.published_name,e==null?void 0:e.description,e==null?void 0:e.exclusion_reason].filter(Boolean).some(n=>String(n).toLowerCase().includes(s)):!0}const Mw=Object.freeze([{id:"identity",label:"Identity"},{id:"transport",label:"Transport"},{id:"authentication",label:"Authentication"},{id:"limits",label:"Limits"}]);function Dw(e,{root:t=document,reducedMotion:s=typeof window<"u"&&(n=>(n=window.matchMedia)==null?void 0:n.call(window,"(prefers-reduced-motion: reduce)").matches)()}={}){var l;const a=t.querySelector(".mcp-editor-groups"),i=a==null?void 0:a.querySelector(`#mcp-form-${e}`);return i?(i.scrollIntoView({behavior:s?"auto":"smooth",block:"start",inline:"nearest"}),(l=i.querySelector("[data-mcp-form-heading]"))==null||l.focus({preventScroll:!0}),!0):!1}const Pw=1e4,Fw=Object.freeze({disabled:"Disabled",connecting:"Connecting",connected:"Connected",stale:"Stale",error:"Error",blocked:"Blocked"});function lo(){return{name:"",enabled:!0,transport:"stdio",command:"",argsText:"",cwd:"",url:"",timeoutSeconds:120,allowlistText:"",replaceArgs:!1,replaceCwd:!1,replaceTimeout:!1,replaceAllowlist:!1,headerRows:[],envRows:[],headersRemove:[],envRemove:[]}}function $w(e){if(e==null)return"Never";const t=Math.max(0,Number(e)||0);return t<60?`${Math.round(t)}s ago`:t<3600?`${Math.round(t/60)}m ago`:t<86400?`${Math.round(t/3600)}h ago`:`${Math.round(t/86400)}d ago`}const Bw={template:`
  ...
  `,setup(){const e=h(null),t=h(!1),s=h(!1),n=h(""),a=h(new Set),i=h(new Set),l=h({}),r=h({}),o=h({}),c=h(new Set),d=h(!1),u=h("add"),p=h(""),f=h(null),m=h(lo()),b=h(""),E=h(!1);let I=null,w=0,g=!1,_=!1;const C=Mw,v=W(()=>{var H;return((H=e.value)==null?void 0:H.servers)||[]}),S=W(()=>{var H;return!!((H=e.value)!=null&&H.enabled)}),T=W(()=>{var H,pe,Te,Ae;return{serverCount:((H=e.value)==null?void 0:H.server_count)||0,enabledCount:((pe=e.value)==null?void 0:pe.enabled_server_count)||0,connectedCount:((Te=e.value)==null?void 0:Te.connected_count)||0,toolCount:((Ae=e.value)==null?void 0:Ae.published_tool_count)||0}}),y=W(()=>{var H;return((H=f.value)==null?void 0:H.header_keys)||[]}),O=W(()=>{var H;return((H=f.value)==null?void 0:H.env_keys)||[]}),$=W(()=>{var H;return u.value==="edit"&&((H=f.value)==null?void 0:H.transport)==="http"}),F=W(()=>u.value==="add"||!$.value),N=W(()=>$.value?"Replace endpoint URL":"Endpoint URL"),G=W(()=>$.value?"Leave blank to keep the saved endpoint":"https://mcp.example.com/mcp");function z(){P(),I=window.setInterval(()=>D({quiet:!0}),Pw)}function P(){I&&window.clearInterval(I),I=null}async function D({quiet:H=!1}={}){const pe=++w;H||(t.value=!0);try{const Te=await K.get("/api/mcp/status");if(pe!==w||!g)return;e.value=Te,n.value="";const Ae=new Set((Te.servers||[]).map(De=>De.name));i.value=new Set([...i.value].filter(De=>Ae.has(De)))}catch(Te){pe===w&&g&&(n.value=Te.message||"Failed to load MCP status")}finally{pe===w&&(t.value=!1)}}function R(H){return s.value||a.value.has(H)}function ee(H,pe){const Te=new Set(a.value);pe?Te.add(H):Te.delete(H),a.value=Te}function ne(H){return Lw(H.state)}function ge(H){if(ne(H)==="disabled"){if(!H.enabled)return"Disabled — server switch off";if(!S.value)return"Disabled — global MCP is off"}return Fw[ne(H)]}function ie(H){return H.transport==="http"?"Streamable HTTP":"stdio"}function be(H){return H.negotiated_version?`${H.era?`${String(H.era).charAt(0).toUpperCase()}${String(H.era).slice(1)}`:"Protocol"} · ${H.negotiated_version}`:"Not negotiated"}function Q(H){return H.discovered_count?`${H.published_count||0} published · ${H.excluded_count||0} excluded`:"No tools discovered"}const ye=h(new Set);async function J(H,pe){if(ye.value.has(H.name))return;const Te=!!pe.target.checked,Ae=new Set(ye.value);Ae.add(H.name),ye.value=Ae;try{const De=await K.post(`/api/mcp/servers/${encodeURIComponent(H.name)}/enabled`,{enabled:Te});De&&Array.isArray(De.servers)?e.value=De:await D({quiet:!0})}catch(De){pe.target.checked=!!H.enabled,Ie.error(De.message||`Failed to toggle ${H.name}`)}finally{const De=new Set(ye.value);De.delete(H.name),ye.value=De}}async function U(H){if(H!==S.value&&!(!H&&!await Jt({title:"Disable MCP tool publication",message:"Disable MCP globally? All MCP tools will be unpublished immediately and active transports will be stopped. Saved server configuration remains.",confirmLabel:"Disable MCP",danger:!0}))){s.value=!0;try{await K.post("/api/mcp/enabled",{enabled:H}),Ie.success(H?"MCP enabled":"MCP disabled"),await D({quiet:!0})}catch(pe){Ie.error(pe.message||"Failed to update MCP state"),await D({quiet:!0})}finally{s.value=!1}}}async function le(H){ee(H.name,!0);try{await K.post(`/api/mcp/servers/${encodeURIComponent(H.name)}/reconnect`,{}),Ie.success(`Reconnected ${H.name}`)}catch(pe){Ie.error(pe.message||`Failed to reconnect ${H.name}`)}finally{ee(H.name,!1),await D({quiet:!0})}}async function re(H){ee(H.name,!0);try{await K.post(`/api/mcp/servers/${encodeURIComponent(H.name)}/refresh-tools`,{}),Ie.success(`Refreshed tools from ${H.name}`),await Ue(H.name,!0)}catch(pe){Ie.error(pe.message||`Failed to refresh ${H.name}`)}finally{ee(H.name,!1),await D({quiet:!0})}}async function Se(H){if(await Jt({title:`Remove ${H.name}`,message:`Remove this saved MCP server? Its ${H.published_count||0} published tool${H.published_count===1?"":"s"} will disappear immediately and configured authentication keys will be deleted. This cannot be undone.`,confirmLabel:"Remove server",danger:!0})){ee(H.name,!0);try{await K.del(`/api/mcp/servers/${encodeURIComponent(H.name)}`),Ie.success(`Removed ${H.name}`),delete r.value[H.name]}catch(Te){Ie.error(Te.message||`Failed to remove ${H.name}`)}finally{ee(H.name,!1),await D({quiet:!0})}}}async function _e(H){const pe=new Set(i.value);if(pe.has(H.name)){pe.delete(H.name),i.value=pe;return}pe.add(H.name),i.value=pe,Object.hasOwn(r.value,H.name)||await Ue(H.name)}async function Ue(H,pe=!1){if(!pe&&Object.hasOwn(r.value,H))return;const Te=new Set(c.value);Te.add(H),c.value=Te,o.value={...o.value,[H]:""};try{const Ae=await K.get(`/api/mcp/servers/${encodeURIComponent(H)}/tools`);r.value={...r.value,[H]:Ae.tools||[]}}catch(Ae){o.value={...o.value,[H]:Ae.message||"Failed to load tools"}}finally{const Ae=new Set(c.value);Ae.delete(H),c.value=Ae}}function k(H){return(r.value[H]||[]).filter(pe=>Nw(pe,l.value[H]))}function L(H,pe){l.value={...l.value,[H]:pe}}function B(){u.value="add",p.value="",f.value=null,m.value=lo(),b.value="",d.value=!0}function se(H){u.value="edit",p.value=H.name,f.value=H,m.value={...lo(),name:H.name,enabled:!!H.enabled,transport:H.transport||"stdio"},b.value="",d.value=!0}function Y(){E.value||(d.value=!1)}function X(H){d.value&&Dw(H)}function me(H){const pe=H==="headers"?"headerRows":"envRows";m.value[pe].push({key:"",value:""})}function ce(H,pe){const Te=H==="headers"?"headerRows":"envRows";m.value[Te].splice(pe,1)}function de(H,pe){const Te=H==="headers"?"headersRemove":"envRemove",Ae=m.value[Te];m.value[Te]=Ae.includes(pe)?Ae.filter(De=>De!==pe):[...Ae,pe]}async function ae(){var pe,Te;b.value="";let H;try{H=Iw(m.value,{mode:u.value,originalTransport:((pe=f.value)==null?void 0:pe.transport)||""})}catch(Ae){b.value=Ae instanceof Cs?Ae.message:"Invalid MCP server configuration",await Et(),(Te=document.querySelector(".mcp-editor"))==null||Te.scrollTo({top:0,behavior:"smooth"});return}if(!(u.value==="edit"&&Ow(H,f.value)&&!await Jt({title:`Change ${p.value} connection`,message:"Saving this configuration replaces the server runtime. Any current connection will be retired and its tools unpublished; enabled servers reconnect after the change.",confirmLabel:"Save and reconnect",danger:!0}))){E.value=!0;try{u.value==="add"?await K.post("/api/mcp/servers",H):await K.put(`/api/mcp/servers/${encodeURIComponent(p.value)}`,H),Ie.success(u.value==="add"?`Saved ${H.name}`:`Updated ${p.value}`),d.value=!1,await D({quiet:!0})}catch(Ae){b.value=Ae.message||"Failed to save MCP server"}finally{E.value=!1}}}let we=null;function ve(H){`${(H==null?void 0:H.event)||""} ${(H==null?void 0:H.type)||""} ${(H==null?void 0:H.tool)||""} ${(H==null?void 0:H.message)||""}`.toLowerCase().includes("mcp")&&(we&&window.clearTimeout(we),we=window.setTimeout(()=>D({quiet:!0}),200))}function xe(){g||(g=!0,_||(Je.subscribe("events",ve),_=!0),D(),z())}function oe(){g=!1,P(),we&&window.clearTimeout(we),we=null,_&&(Je.unsubscribe("events",ve),_=!1)}return Ke(xe),_s(xe),ws(oe),_t(oe),{status:e,loading:t,mutating:s,pageError:n,servers:v,masterEnabled:S,aggregate:T,expandedServers:i,toolQueries:l,toolErrors:o,toolsLoading:c,editorOpen:d,editorMode:u,editingName:p,editingServer:f,form:m,formError:b,saving:E,editorGroups:C,configuredHeaderKeys:y,configuredEnvKeys:O,savedHttpEndpoint:$,endpointRequired:F,endpointFieldLabel:N,endpointPlaceholder:G,refreshAll:D,busy:R,serverState:ne,stateLabel:ge,transportLabel:ie,protocolLabel:be,toolSummary:Q,formatAge:$w,setMasterEnabled:U,togglePending:ye,toggleServerEnabled:J,reconnect:le,refreshTools:re,removeServer:Se,toggleTools:_e,filteredTools:k,setToolQuery:L,openAdd:B,openEdit:se,closeEditor:Y,jumpToEditorGroup:X,addSecretRow:me,removeSecretRow:ce,toggleSecretRemoval:de,saveServer:ae}}};function Uw(e,t){if(!e||!t)return Du(e);const s=Du(e),n=t.trim().split(/\s+/).filter(Boolean);if(!n.length)return s;const a=n.map(i=>i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")).join("|");try{return s.replace(new RegExp(`(${a})`,"gi"),'<mark class="knowledge-highlight">$1</mark>')}catch{return s}}const Hw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h(""),a=h(null),i=h(!1),l=h(""),r=h(null),o=h(!1),c=h(""),d=h(""),u=h(null),p=h(null),f=h(!1),m=h(null),b=h(null);let E=null;const I=h(null),w=h(!1),g=h({}),_=h({}),C=h({}),v=h({}),S=new Map,T=h(null),y=W(()=>e.value.reduce((ne,ge)=>ne+(ge.chunks||0),0)),O=W(()=>new Set(e.value.map(ge=>ge.uploader).filter(Boolean)).size);function $(ne,ge){const ie=_.value[ge];if(!ie||ie.length===0)return 0;const be=Math.max(...ie.map(Q=>Q.char_count||0));return be===0?0:Math.round(ne.char_count/be*100)}async function F(){t.value=!0,s.value=null;try{const ne=await K.get("/api/knowledge");e.value=Array.isArray(ne)?ne:[]}catch(ne){s.value=ne.message}t.value=!1}async function N(ne){if(g.value[ne]){g.value[ne]=!1,T.value=null;return}if(g.value[ne]=!0,Object.prototype.hasOwnProperty.call(_.value,ne))return;if(S.has(ne))return S.get(ne);const ge={...v.value,[ne]:!0};v.value=ge;const ie={...C.value};delete ie[ne],C.value=ie;const be=K.get(`/api/knowledge/${encodeURIComponent(ne)}/chunks`).then(Q=>{_.value={..._.value,[ne]:Array.isArray(Q)?Q:[]}}).catch(Q=>{C.value={...C.value,[ne]:Q.message||"load failed"}}).finally(()=>{if(S.get(ne)!==be)return;S.delete(ne);const Q={...v.value};delete Q[ne],v.value=Q});return S.set(ne,be),be}async function G(){const ne=n.value.trim();if(ne){i.value=!0,r.value=null,l.value=ne;try{const ge=await K.get(`/api/knowledge/search?q=${encodeURIComponent(ne)}`);a.value=Array.isArray(ge)?ge:[]}catch(ge){a.value=[],r.value=ge.message||"Search failed"}i.value=!1}}function z(){a.value=null,n.value="",r.value=null}async function P(){u.value=null,p.value=null;const ne=c.value.trim(),ge=d.value.trim();if(!ne){u.value="Source name is required";return}if(!ge){u.value="Content is required";return}f.value=!0;try{const ie=await K.post("/api/knowledge",{source:ne,content:ge});p.value=`Ingested ${ie.chunks||0} chunks from "${ne}"`,c.value="",d.value="",_.value={},await F(),setTimeout(()=>{o.value=!1,p.value=null},1500)}catch(ie){u.value=ie.message}f.value=!1}async function D(ne){m.value=ne,b.value=null,E&&(clearTimeout(E),E=null);try{const ge=await K.post(`/api/knowledge/${encodeURIComponent(ne)}/reingest`);b.value={source:ne,error:!1,message:`Re-ingested ${ge.chunks||0} chunks`},delete _.value[ne],await F(),E=setTimeout(()=>{b.value=null,E=null},3e3)}catch(ge){b.value={source:ne,error:!0,message:ge.message}}m.value=null}function R(ne){I.value=ne}async function ee(){if(I.value){w.value=!0;try{await K.del(`/api/knowledge/${encodeURIComponent(I.value)}`),delete _.value[I.value],await F()}catch(ne){Ie.error(`Failed to delete source: ${ne.message||"unknown error"}`)}w.value=!1,I.value=null}}return Ke(()=>{F()}),{sources:e,loading:t,error:s,searchQuery:n,searchResults:a,searching:i,lastQuery:l,searchError:r,showIngest:o,ingestSource:c,ingestContent:d,ingestError:u,ingestSuccess:p,ingesting:f,reingesting:m,reingestResult:b,deleteTarget:I,deleting:w,expanded:g,sourceChunks:_,chunkErrors:C,loadingChunks:v,selectedChunk:T,totalChunks:y,uploaderCount:O,truncate:Qc,formatTs:xa,highlightTerms:Uw,chunkBarWidth:$,fetchSources:F,toggleSource:N,doSearch:G,clearSearch:z,doIngest:P,doReingest:D,confirmDelete:R,doDelete:ee}}},zw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    </div>`,setup(){const e=h([]),t=h({}),s=h(!0),n=h(null),a=h({}),i=h(null),l=h(""),r=h(!1),o=h({scope:"global",key:"",value:""}),c=h(!1),d=h(null),u=h(null),p=h(null),f=h(""),m=h(!1),b=h(null),E=h(null),I=h(new Set),w=h(null),g=h(!1),_=h(!1),C=W(()=>e.value.reduce((ie,be)=>ie+be.count,0)),v=W(()=>I.value.size);function S(ie){const be=t.value[ie];if(!be)return[];if(!l.value.trim())return be;const Q=l.value.trim().toLowerCase();return be.filter(ye=>ye.key.toLowerCase().includes(Q)||ye.value&&ye.value.toLowerCase().includes(Q))}function T(ie,be){return I.value.has(ie+"/"+be)}function y(ie,be){const Q=ie+"/"+be,ye=new Set(I.value);ye.has(Q)?ye.delete(Q):ye.add(Q),I.value=ye}function O(ie){const be=t.value[ie];return!be||be.length===0?!1:be.every(Q=>I.value.has(ie+"/"+Q.key))}function $(ie,be){const Q=t.value[ie];if(!Q)return;const ye=new Set(I.value);for(const J of Q){const U=ie+"/"+J.key;be?ye.add(U):ye.delete(U)}I.value=ye}async function F(){s.value=!0,n.value=null;try{const ie=await K.get("/api/memory");e.value=Object.entries(ie).map(([be,Q])=>({name:be,keys:Q.keys||[],count:Q.count||0}))}catch(ie){n.value=ie.message}s.value=!1}async function N(ie){if(a.value[ie]){a.value[ie]=!1;return}a.value[ie]=!0;const be=e.value.find(ye=>ye.name===ie);if(!be||t.value[ie]||i.value===ie)return;i.value=ie;let Q;try{const J=(await K.get(`/api/memory/${encodeURIComponent(ie)}`)).entries||{};Q=be.keys.map(U=>Object.prototype.hasOwnProperty.call(J,U)?{key:U,value:J[U]||"",failed:!1}:{key:U,value:"",failed:!0,error:"Not found in scope"})}catch(ye){Q=be.keys.map(J=>({key:J,value:"",failed:!0,error:ye.message||"Failed to load"}))}t.value[ie]=Q,i.value=null}function G(ie,be,Q){p.value=ie+"/"+be,f.value=Q}async function z(ie,be){m.value=!0,b.value=null;try{await K.put(`/api/memory/${encodeURIComponent(ie)}/${encodeURIComponent(be)}`,{value:f.value});const Q=t.value[ie];if(Q){const ye=Q.find(J=>J.key===be);ye&&(ye.value=f.value)}p.value=null}catch(Q){b.value=`Failed to save: ${Q.message||"unknown error"}`}m.value=!1}async function P(ie,be){try{await navigator.clipboard.writeText(be.value),E.value=ie+"/"+be.key,setTimeout(()=>{E.value=null},1500)}catch{}}async function D(){d.value=null,u.value=null;const ie=o.value.scope.trim(),be=o.value.key.trim(),Q=o.value.value.trim();if(!ie){d.value="Scope is required";return}if(!be){d.value="Key is required";return}if(!Q){d.value="Value is required";return}c.value=!0;try{await K.put(`/api/memory/${encodeURIComponent(ie)}/${encodeURIComponent(be)}`,{value:Q}),u.value="Entry saved",o.value={scope:"global",key:"",value:""},t.value={},await F(),setTimeout(()=>{r.value=!1,u.value=null},800)}catch(ye){d.value=ye.message}c.value=!1}function R(ie,be){w.value={scope:ie,key:be}}async function ee(){if(!w.value)return;g.value=!0,b.value=null;const{scope:ie,key:be}=w.value;try{await K.del(`/api/memory/${encodeURIComponent(ie)}/${encodeURIComponent(be)}`);const Q=t.value[ie];Q&&(t.value[ie]=Q.filter(U=>U.key!==be));const ye=e.value.find(U=>U.name===ie);ye&&(ye.count--,ye.keys=ye.keys.filter(U=>U!==be));const J=new Set(I.value);J.delete(ie+"/"+be),I.value=J}catch(Q){b.value=`Failed to delete: ${Q.message||"unknown error"}`}g.value=!1,w.value=null}function ne(){_.value=!0}async function ge(){g.value=!0,b.value=null;const ie=[];for(const be of I.value){const Q=be.indexOf("/");ie.push({scope:be.slice(0,Q),key:be.slice(Q+1)})}try{await K.post("/api/memory/bulk-delete",{entries:ie}),I.value=new Set,t.value={},await F()}catch(be){b.value=`Bulk delete failed: ${be.message||"unknown error"}`}g.value=!1,_.value=!1}return Ke(()=>{F()}),{scopes:e,scopeEntries:t,loading:s,error:n,expanded:a,loadingScope:i,filterQuery:l,showAdd:r,addForm:o,adding:c,addError:d,addSuccess:u,editingKey:p,editValue:f,saving:m,actionError:b,copied:E,selected:I,selectedCount:v,totalEntries:C,deleteTarget:w,deleting:g,showBulkDelete:_,fetchMemory:F,toggleScope:N,startEdit:G,doEdit:z,copyValue:P,doAdd:D,confirmDelete:R,doDelete:ee,confirmBulkDelete:ne,doBulkDelete:ge,isSelected:T,toggleSelect:y,isScopeAllSelected:O,toggleSelectAll:$,filteredEntries:S}}},jw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
        <div>
  ...
  `,setup(){const e=h([]),t=h(null),s=h(!0),n=h(null),a=h(null),i=h(null),l=h(""),r=W(()=>[...new Set(e.value.map(E=>E.category))].sort()),o=W(()=>{const b={};return e.value.forEach(E=>{b[E.category]=(b[E.category]||0)+1}),b}),c=W(()=>a.value?e.value.filter(b=>b.category===a.value):e.value);function d(b){return b==="correction"?"badge-warning":b==="operational"?"badge-info":b==="preference"?"badge-success":"badge-info"}function u(b){i.value=b.key,l.value=b.content}async function p(b){try{await K.put("/api/learned/"+encodeURIComponent(b),{content:l.value}),i.value=null,Ie.success("Entry updated"),await m()}catch(E){Ie.error(E.message||"Failed to save entry")}}async function f(b){if(await Jt({title:"Delete learned entry",message:`Delete "${b}"? Odin will no longer apply this learned context.`,confirmLabel:"Delete",danger:!0}))try{await K.del("/api/learned/"+encodeURIComponent(b)),Ie.success("Entry deleted"),await m()}catch(I){Ie.error(I.message||"Failed to delete entry")}}async function m(){s.value=!0,n.value=null;try{const b=await K.get("/api/learned");e.value=b.entries||[],t.value={last_reflection:b.last_reflection,count:b.count}}catch(b){n.value=b.message}s.value=!1}return Ke(m),{entries:e,meta:t,loading:s,error:n,filterCat:a,editing:i,editContent:l,categories:r,catCounts:o,filtered:c,catBadge:d,formatTs:xa,startEdit:u,saveEdit:p,deleteEntry:f,fetchEntries:m}}},Am=[{id:"tools",label:"Tools",component:Sw},{id:"skills",label:"Skills",component:Ew},{id:"mcp-servers",label:"MCP Servers",component:Bw},{id:"knowledge",label:"Knowledge",component:Hw},{id:"memory",label:"Memory",component:zw},{id:"learned",label:"Learned",component:jw}],Vw={components:{TabbedPage:$r},setup(){return{tabs:Am}},template:'<tabbed-page :tabs="tabs" default-tab="tools" group-label="Capabilities" />'},qw={ok:"text-green-400",degraded:"text-yellow-400",down:"text-red-400",unconfigured:"text-gray-500"},Gw={ok:"success",degraded:"warning",down:"error",unconfigured:"minus"},Kw={healthy:"text-green-400",degraded:"text-yellow-400",unhealthy:"text-red-400"},Ww={template:`
    ...
    </div>`,setup(){const e=h({}),t=h(!0),s=h(null),n=h(!1),a=h(!1),i=W(()=>e.value.components||[]),l=W(()=>Kw[e.value.overall]||"text-gray-400"),r=W(()=>e.value.overall==="healthy"?"success":e.value.overall==="degraded"?"warning":e.value.overall==="unhealthy"?"error":"minus"),o=W(()=>{const v=e.value.overall;return v==="healthy"?"All Systems Healthy":v==="degraded"?"Some Systems Degraded":v==="unhealthy"?"System Issues Detected":"Unknown"});function c(v){return qw[v]||"text-gray-400"}function d(v){return Gw[v]||"info"}function u(v){return v==="ok"?"badge-success":v==="degraded"?"badge-warning":v==="down"?"badge-danger":"badge-info"}function p(v){return v==="closed"?"text-green-400":v==="half_open"?"text-yellow-400":v==="open"?"text-red-400":"text-gray-400"}function f(v){return v.replace(/_/g," ").replace(/\b\w/g,S=>S.toUpperCase())}function m(v){if(!v)return"—";try{return new Date(v).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}catch{return v}}function b(v){return v>=1e6?(v/1e6).toFixed(1)+"M":v>=1e3?(v/1e3).toFixed(1)+"K":String(v)}async function E(){a.value=!0;try{e.value=await K.get("/api/health/components"),s.value=null,n.value=!0}catch(v){s.value=v.message}finally{t.value=!1,a.value=!1}}function I(){t.value=!0,s.value=null,E()}let w=null,g=!1;function _(){g||(g=!0,E(),w||(w=setInterval(E,3e4)))}function C(){g&&(g=!1,w&&(clearInterval(w),w=null))}return Ke(_),_s(_),ws(C),_t(C),{data:e,hasData:n,loading:t,error:s,refreshing:a,components:i,overallColor:l,overallIcon:r,overallLabel:o,statusColor:c,statusIcon:d,badgeClass:u,circuitColor:p,formatName:f,formatTime:m,formatNumber:b,fetchHealth:E,retry:I}}},Zw={template:`
  ...
  `,setup(){const e=h(!0),t=h(null),s=h(!1),n=h(!1),a=h("sessions"),i=h(null);let l=null;const r=[{key:"sessions",label:"Sessions"},{key:"knowledge",label:"Knowledge"},{key:"trajectories",label:"Trajectories"},{key:"storage",label:"Storage"}],o=W(()=>{if(!i.value||!i.value.collected_at)return"";try{return new Date(i.value.collected_at).toLocaleTimeString()}catch{return""}}),c=W(()=>{if(!i.value)return[];const E=i.value,I=E.storage_total_bytes||1;return[{label:"Session Persistence",mb:E.sessions.persist_dir.total_mb,bytes:E.sessions.persist_dir.total_bytes,files:E.sessions.persist_dir.file_count,pct:Math.min(100,Math.round(E.sessions.persist_dir.total_bytes/I*100)),color:"res-bar-blue"},{label:"Knowledge Database",mb:E.knowledge.db_file.total_mb,bytes:E.knowledge.db_file.total_bytes,files:E.knowledge.db_file.file_count,pct:Math.min(100,Math.round(E.knowledge.db_file.total_bytes/I*100)),color:"res-bar-purple"},{label:"Message Trajectories",mb:E.trajectories.message_dir.total_mb,bytes:E.trajectories.message_dir.total_bytes,files:E.trajectories.message_dir.file_count,pct:Math.min(100,Math.round(E.trajectories.message_dir.total_bytes/I*100)),color:"res-bar-emerald"},{label:"Agent Trajectories",mb:E.trajectories.agent_dir.total_mb,bytes:E.trajectories.agent_dir.total_bytes,files:E.trajectories.agent_dir.file_count,pct:Math.min(100,Math.round(E.trajectories.agent_dir.total_bytes/I*100)),color:"res-bar-amber"}]});async function d(){try{const E=await K.get("/api/resource-usage");i.value=E,t.value=null,s.value=!0}catch(E){t.value=E.message||"Failed to load resource usage"}finally{e.value=!1,n.value=!1}}async function u(){n.value=!0,await d()}function p(){e.value=!0,t.value=null,d()}let f=!1;function m(){f||(f=!0,d(),l||(l=setInterval(d,3e4)))}function b(){f&&(f=!1,l&&(clearInterval(l),l=null))}return Ke(m),_s(m),ws(b),_t(b),{hasData:s,loading:e,error:t,refreshing:n,data:i,activeTab:a,tabs:r,collectedAt:o,storageItems:c,fmtNum:wm,refresh:u,retry:p}}},Jw=["INFO","WARNING","ERROR"],Yw=[{id:"all",name:"All Logs",icon:"list",filters:{}},{id:"errors",name:"Errors Only",icon:"error",filters:{level:"ERROR"}},{id:"warnings",name:"Warnings+",icon:"warning",filters:{levels:["WARNING","ERROR"]}},{id:"tools",name:"Tool Activity",icon:"wrench",filters:{hasToolName:!0}},{id:"recent-errors",name:"Recent Errors",icon:"flame",filters:{level:"ERROR",timeRange:"last_1h"}}],ro=[{value:"",label:"All Time"},{value:"last_5m",label:"Last 5 min",seconds:300},{value:"last_15m",label:"Last 15 min",seconds:900},{value:"last_1h",label:"Last 1 hour",seconds:3600},{value:"last_4h",label:"Last 4 hours",seconds:14400},{value:"last_24h",label:"Last 24 hours",seconds:86400}],Qw=[50,100,200,500],Xw={template:`
    ...
    </div>`,setup(){const e=h("live"),t=h([]),s=h(!1),n=h(!0),a=h(""),i=h(""),l=h(!1),r=h(!1),o=h(Je.state||"disconnected"),c=W(()=>{switch(o.value){case"connected":return"Live";case"connecting":return"Connecting…";case"reconnecting":return"Reconnecting…";default:return"Disconnected"}}),d=h(null),u=h(!1),p=h(null),f=2e3,m=Jw,b=Yw,E=ro,I=h("all"),w=h(""),g=h([]),_=h(!1),C=h(""),v=h([]);function S(){try{const q=localStorage.getItem("odin-log-presets");q&&(g.value=JSON.parse(q))}catch{}}function T(){try{localStorage.setItem("odin-log-presets",JSON.stringify(g.value))}catch{}}const y=W(()=>a.value!==""||i.value.trim()!==""||w.value!==""),O=W(()=>{const q=ro.find(ue=>ue.value===w.value);return q?q.label:""}),$=W(()=>{if(!l.value||!i.value)return null;try{return new RegExp(i.value,"i"),null}catch(q){return q.message}}),F=24,N=W(()=>{if(ne.value.length===0)return[];const q=[],ue=new Date,Ne=3600*1e3;for(let We=F-1;We>=0;We--){const ot=new Date(ue.getTime()-(We+1)*Ne),Ot=new Date(ue.getTime()-We*Ne);q.push({start:ot,end:Ot,label:D(ot,Ot),shortLabel:Ot.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),total:0,info:0,warnings:0,errors:0})}for(const We of ne.value){if(!We._time)continue;const ot=We._time.getTime();for(const Ot of q)if(ot>=Ot.start.getTime()&&ot<Ot.end.getTime()){Ot.total++,We.level==="ERROR"?Ot.errors++:We.level==="WARNING"?Ot.warnings++:Ot.info++;break}}return q}),G=W(()=>{let q=1;for(const ue of N.value)ue.total>q&&(q=ue.total);return q}),z=W(()=>{if(N.value.length===0)return"";const q=ne.value.map(We=>We._time&&We._time.getTime()).filter(Boolean);if(q.length===0)return"";const ue=new Date(Math.min(...q));return`${ne.value.length} shown, oldest ${ue.toLocaleTimeString()}`}),P=W(()=>Math.ceil(F/8));function D(q,ue){const Ne={hour:"2-digit",minute:"2-digit"};return q.toLocaleTimeString([],Ne)+" - "+ue.toLocaleTimeString([],Ne)}function R(q,ue){return!ue||!q?"0px":Math.max(2,q/ue*100)+"%"}function ee(q){const ue=ne.value.findIndex(Ne=>Ne._time&&Ne._time.getTime()>=q.start.getTime()&&Ne._time.getTime()<q.end.getTime());if(ue>=0&&d.value){const Ne=d.value.querySelectorAll(".log-line");Ne[ue]&&(Ne[ue].scrollIntoView({behavior:"smooth",block:"center"}),n.value=!1)}}const ne=W(()=>{let q=t.value;if(a.value&&(q=q.filter(ue=>(ue.level||"INFO")===a.value)),w.value){const ue=ro.find(Ne=>Ne.value===w.value);if(ue&&ue.seconds){const Ne=new Date(Date.now()-ue.seconds*1e3);q=q.filter(We=>We._time&&We._time>=Ne)}}if(i.value&&!$.value)if(l.value)try{const ue=new RegExp(i.value,"i");q=q.filter(Ne=>{const We=Ne.text||Ne.raw||"",ot=Ne.tool||"";return ue.test(We)||ue.test(ot)})}catch{}else{const ue=i.value.toLowerCase();q=q.filter(Ne=>{const We=(Ne.text||Ne.raw||"").toLowerCase(),ot=(Ne.tool||"").toLowerCase();return We.includes(ue)||ot.includes(ue)})}return q});function ge(q){if(q.type==="log"&&q.line)try{const ue=typeof q.line=="string"?JSON.parse(q.line):q.line,Ne=ue.timestamp?new Date(ue.timestamp):new Date;return{ts:Ne.toLocaleTimeString(),_time:Ne,level:ue.error?"ERROR":"INFO",text:ue.tool_name?`[${ue.tool_name}] ${ue.result_summary||""}`.trim():ue.message||JSON.stringify(ue),tool:ue.tool_name||"",raw:null}}catch{return{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:String(q.line),tool:"",raw:String(q.line)}}if(q.payload){const ue=q.payload,Ne=ue.timestamp?new Date(ue.timestamp):new Date;return{ts:Ne.toLocaleTimeString(),_time:Ne,level:ue.error?"ERROR":"INFO",text:ue.tool_name?`[${ue.tool_name}] ${ue.result_summary||""}`.trim():ue.message||JSON.stringify(ue),tool:ue.tool_name||"",raw:null}}return typeof q=="string"?{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:q,tool:"",raw:q}:{ts:new Date().toLocaleTimeString(),_time:new Date,level:"INFO",text:JSON.stringify(q),tool:"",raw:null}}function ie(q){const ue=ge(q);if(s.value){v.value.push(ue);return}be(ue)}function be(q){t.value.push(q),t.value.length>f&&(t.value=t.value.slice(-f)),n.value&&Et(()=>Q())}function Q(q=!1){const ue=d.value;ue&&ue.scrollTo({top:ue.scrollHeight,behavior:q?"smooth":"instant"})}function ye(){n.value=!0,u.value=!1,Et(()=>Q(!0))}const J=new Set(["PageUp","PageDown","ArrowUp","ArrowDown","Home","End"," "]);function U(){const q=d.value;if(!q)return;const ue=q.scrollHeight-q.scrollTop-q.clientHeight<40;u.value=!n.value&&!ue&&t.value.length>0,_e.value&&le()}function le(){const q=d.value;!q||!n.value||q.scrollHeight-q.scrollTop-q.clientHeight>=40&&(n.value=!1,u.value=t.value.length>0)}function re(){n.value&&requestAnimationFrame(le)}function Se(q){J.has(q.key)&&re()}const _e=h(!1);function Ue(){n.value&&(_e.value=!0,requestAnimationFrame(le))}function k(){_e.value&&(_e.value=!1,le())}function L(){n.value&&(u.value=!1,Et(()=>Q()))}function B(){if(s.value=!s.value,!s.value&&v.value.length>0){for(const q of v.value)be(q);v.value=[]}}function se(){t.value=[],v.value=[],u.value=!1}function Y(){let q;e.value==="search"?q=Qe.value.map(ot=>{const Ot=ot.error?"ERROR":"INFO",Gn=ot.tool_name?`[${ot.tool_name}] `:"";return`${ot.timestamp||""} ${Ot} ${Gn}${ot.result_summary||ot.message||""}`}).join(`
`):q=ne.value.map(ot=>`${ot.ts} ${ot.level} ${ot.text}`).join(`
`);const ue=new Blob([q],{type:"text/plain"}),Ne=URL.createObjectURL(ue),We=document.createElement("a");We.href=Ne,We.download=`odin-logs-${new Date().toISOString().slice(0,19).replace(/:/g,"-")}.txt`,We.click(),URL.revokeObjectURL(Ne)}function X(q,ue){const Ne=`${q.ts} ${q.level} ${q.text||q.raw||""}`;navigator.clipboard.writeText(Ne).then(()=>{p.value=ue,setTimeout(()=>{p.value=null},1500)}).catch(()=>{})}function me(q){a.value=a.value===q?"":q,I.value="all"}function ce(q){return q.level==="ERROR"?"log-line-error":q.level==="WARNING"?"log-line-warning":"text-gray-300"}function de(q){return q==="ERROR"?"text-red-500 font-semibold":q==="WARNING"?"text-yellow-500":"text-blue-500"}function ae(q){return q==="ERROR"?"log-chip-error":q==="WARNING"?"log-chip-warning":"log-chip-info"}function we(q){I.value=q.id;const ue=q.filters;a.value=ue.level||"",w.value=ue.timeRange||"",i.value=ue.text||"",ue.levels&&(a.value=ue.levels[0]||""),ue.hasToolName&&(i.value="")}function ve(q){I.value=q.id,a.value=q.filters.level||"",w.value=q.filters.timeRange||"",i.value=q.filters.text||""}function xe(){if(!C.value.trim())return;const q={id:"custom-"+Date.now(),name:C.value.trim(),filters:{level:a.value,timeRange:w.value,text:i.value}};g.value=[...g.value,q],T(),_.value=!1,C.value=""}function oe(q){g.value=g.value.filter(ue=>ue.id!==q),T(),I.value===q&&(I.value="all")}const H=h("all"),pe=h(""),Te=h(""),Ae=h(""),De=h(""),ut=h(""),it=h(100),te=Qw,Oe=h(!1),Le=h(!1),Me=h(""),Qe=h([]),je=h(null),pt=h(null);function Tt(){e.value="search",je.value||Hs()}async function Hs(){try{je.value=await K.get("/api/logs/stats")}catch{}}function zs(){const q=ut.value;if(!q){Ae.value="",De.value="";return}const Ne={last_5m:300,last_15m:900,last_1h:3600,last_4h:14400,last_24h:86400,last_7d:604800}[q];if(Ne){const We=new Date(Date.now()-Ne*1e3);Ae.value=Ls(We),De.value=""}}function Ls(q){const ue=Ne=>String(Ne).padStart(2,"0");return`${q.getFullYear()}-${ue(q.getMonth()+1)}-${ue(q.getDate())}T${ue(q.getHours())}:${ue(q.getMinutes())}`}function Ft(q){if(!q)return"";const ue=new Date(q);return isNaN(ue.getTime())?"":ue.toISOString()}async function jt(){Oe.value=!0,Me.value="",Le.value=!0,pt.value=null;try{const q=new URLSearchParams;H.value&&H.value!=="all"&&q.set("level",H.value),pe.value&&q.set("tool",pe.value),Te.value&&q.set("q",Te.value);const ue=Ft(Ae.value),Ne=Ft(De.value);ue&&q.set("start",ue),Ne&&q.set("end",Ne),q.set("limit",String(it.value));const We=await K.get(`/api/logs/search?${q.toString()}`);Qe.value=We.entries||[]}catch(q){Me.value=q.message||"Search failed",Qe.value=[]}finally{Oe.value=!1}}function nn(){H.value="all",pe.value="",Te.value="",Ae.value="",De.value="",ut.value="",it.value=100,Qe.value=[],Le.value=!1,Me.value="",pt.value=null}function js(q){pt.value=pt.value===q?null:q}function Vs(q){if(!q.timestamp)return"";try{return new Date(q.timestamp).toLocaleString()}catch{return q.timestamp}}function qs(q){return q.type==="web_action"?`${q.status||""} (${q.execution_time_ms||0}ms)`:(q.result_summary||"").slice(0,200)}function Ns(q){return q.error?"log-line-error":"text-gray-300"}function Rn(q){try{return JSON.stringify(q,null,2)}catch{return String(q)}}let vt=null,Ms=!1;function qn(){Ms||(Ms=!0,Je.subscribe("logs",ie),r.value=Je.connected,o.value=Je.state||"disconnected",vt=Je.onState(q=>{o.value=q,r.value=q==="connected"}))}function Ye(){Ms&&(Ms=!1,Je.unsubscribe("logs",ie),vt&&(vt(),vt=null))}return Ke(()=>{S(),window.addEventListener("pointerup",k),window.addEventListener("pointercancel",k)}),_s(qn),ws(Ye),_t(()=>{Ye(),window.removeEventListener("pointerup",k),window.removeEventListener("pointercancel",k)}),{mode:e,logs:t,paused:s,autoScroll:n,levelFilter:a,textFilter:i,useRegex:l,subscribed:r,wsState:o,wsStateLabel:c,logContainer:d,filteredLogs:ne,pauseBuffer:v,showJumpBottom:u,copiedIndex:p,regexError:$,levels:m,logPresets:b,timeRanges:E,timeRange:w,activeLogPreset:I,customLogPresets:g,showSaveLogPreset:_,newLogPresetName:C,hasActiveLogFilters:y,timeRangeLabel:O,timelineBuckets:N,timelineMax:G,timelineSpanLabel:z,timelineLabelSkip:P,togglePause:B,clearLogs:se,exportLogs:Y,logLineClass:ce,levelClass:de,levelChipClass:ae,toggleLevel:me,copyLine:X,jumpToBottom:ye,onScroll:U,onUserScrollIntent:re,onUserScrollKey:Se,onAutoScrollToggle:L,onPointerDown:Ue,applyLogPreset:we,applyCustomLogPreset:ve,saveLogCustomPreset:xe,removeLogCustomPreset:oe,segmentHeight:R,jumpToTimelineBucket:ee,searchLevel:H,searchTool:pe,searchKeyword:Te,searchStart:Ae,searchEnd:De,searchTimePreset:ut,searchLimit:it,searchLimits:te,searching:Oe,searchRan:Le,searchError:Me,searchResults:Qe,searchStats:je,expandedSearch:pt,switchToSearch:Tt,runSearch:jt,clearSearchFilters:nn,toggleSearchExpand:js,formatSearchTs:Vs,searchEntryText:qs,searchLogLineClass:Ns,formatJson:Rn,applySearchTimePreset:zs}}};function Sl(e=[]){const t=[],s=new Set;function n(a){const i=[a.kind,a.label,a.apply_mode||"",a.code||"",a.text||""].join("\0");s.has(i)||(s.add(i),t.push({...a,key:i}))}for(const a of e)for(const i of(a==null?void 0:a.consumers)||[])n({kind:"consumer",label:i.name,apply_mode:i.apply_mode,text:i.detail});for(const a of e)a!=null&&a.apply_handler&&n({kind:"handler",label:"Apply handler",code:a.apply_handler});for(const a of e)a!=null&&a.restart_reason&&n({kind:"restart",label:"Why a restart is required",text:a.restart_reason});for(const a of e)a!=null&&a.activation_policy&&n({kind:"activation",label:"Activation policy",text:a.activation_policy});return t}const ek=Object.freeze([{key:"all",label:"All fields",short:"All",icon:"grid"},{key:"applied",label:"Applied",short:"Applied",icon:"success"},{key:"pending_restart",label:"Pending restart",short:"Restart",icon:"refresh"},{key:"dormant",label:"Saved, not active",short:"Saved only",icon:"pause"},{key:"invalid",label:"Invalid",short:"Invalid",icon:"error"},{key:"drift",label:"Drift",short:"Drift",icon:"warning"},{key:"unknown",label:"Effective state unknown",short:"Unknown",icon:"info"}]);function tk(e,t={}){var a,i;const s=t.getStyle||(l=>globalThis.getComputedStyle(l)),n=Object.hasOwn(t,"fallback")?t.fallback:(a=globalThis.document)==null?void 0:a.scrollingElement;for(let l=e;l;l=l.parentElement){const r=((i=s(l))==null?void 0:i.overflowY)||"";if(/^(auto|scroll|overlay)$/.test(r)&&l.scrollHeight>l.clientHeight)return l}return n&&n.scrollHeight>n.clientHeight?n:e||n||null}const Ga=[{key:"core",label:"Core",icon:"sliders",sections:["timezone","logging","permissions","graceful_degradation"]},{key:"models",label:"Models & AI",icon:"brain",sections:["image","llm_recovery"]},{key:"runtime",label:"Runtime",icon:"activity",sections:["context","sessions","agents","turn_state"]},{key:"data",label:"Data & Storage",icon:"database",sections:["learning","search","usage","audit","attachments"]},{key:"services",label:"Services",icon:"link",sections:["webhook","observability","email","browser","comfyui","slack","mcp"]},{key:"automation",label:"Automation",icon:"workflow",sections:["message_triggers","reaction_triggers","grafana_alerts","outbound_webhooks","issue_tracker"]},{key:"infrastructure",label:"Infrastructure",icon:"server",sections:["tools","web"]}],sk={live_read:"Applies immediately",live_apply:"Dedicated live apply",live_for_new_work:"Applies to new work",restart:"Restart required",activation_required:"Saved only — see activation note",legacy_control:"Controlled elsewhere",dormant:"Saved for future support"},oo=new Set(["llm_provider","openai_codex","ollama","kimi","personality","discord"]),nk=Object.freeze(["web.api_tokens","outbound_webhooks.targets"]);function Uu(e){return nk.some(t=>e===t||e.startsWith(`${t}.`))}const Rm="odin_config_center_expanded_v1",Im="odin_config_center_category_v1",ak=50,ik=650,co=()=>K.get("/api/config/meta");function sa(e){return e===void 0?void 0:JSON.parse(JSON.stringify(e))}function Ni(e,t){return JSON.stringify(e)===JSON.stringify(t)}function Aa(e){return String(e).replace(/[_-]+/g," ").replace(/\b\w/g,t=>t.toUpperCase())}function lk(e){return e===void 0?"unset":e===null?"null":typeof e=="boolean"?e?"Enabled":"Disabled":Array.isArray(e)?e.length?`${e.length} item${e.length===1?"":"s"}`:"Empty list":typeof e=="object"?Object.keys(e).length?`${Object.keys(e).length} field${Object.keys(e).length===1?"":"s"}`:"Empty object":e===""?"Empty":String(e)}function rk(e){if(e===void 0)return"unset";if(e===null)return"null";if(typeof e=="object")try{return JSON.stringify(e,null,2)}catch{return String(e)}return String(e)}function Om(e,t){if(Ni(e,t))return;if(!(e&&t&&typeof e=="object"&&typeof t=="object"&&!Array.isArray(e)&&!Array.isArray(t)))return sa(t);const n={};for(const[a,i]of Object.entries(t)){const l=Om(e[a],i);l!==void 0&&(n[a]=l)}return Object.keys(n).length?n:void 0}function ok(e,t){const s={};for(const[n,a]of Object.entries(t||{})){const i=Om(e==null?void 0:e[n],a);i!==void 0&&(s[n]=i)}return s}function Lm(e,t,s,n){if(Ni(e,t))return;if(e&&t&&typeof e=="object"&&typeof t=="object"&&!Array.isArray(e)&&!Array.isArray(t)){const i=new Set([...Object.keys(e),...Object.keys(t)]);for(const l of i)Lm(e[l],t[l],s?`${s}.${l}`:l,n);return}n.push({path:s,oldVal:e,newVal:t})}function ck(){try{const e=JSON.parse(localStorage.getItem(Rm)||"{}");return e&&typeof e=="object"&&!Array.isArray(e)?e:{}}catch{return{}}}function dk(){try{const e=localStorage.getItem(Im);return Ga.some(t=>t.key===e)?e:Ga[0].key}catch{return Ga[0].key}}const uk={template:`
  ...
  `,setup(){const e=h(null),t=h(null),s=h(!0),n=h(null),a=h(!1),i=h(null),l=h(null),r=h(null),o=h(!1),c=h(!1),d=h(null),u=h(""),p=h("all"),f=h(dk()),m=h(ck()),b=h({}),E=h({}),I=h(""),w=h({}),g=h({}),_=h([]),C=h([]),v=h(!1),S=h(!1),T=h(!1);let y=null,O=null,$={path:null,at:0},F=0;const N=W(()=>{var x;return(((x=t.value)==null?void 0:x.fields)||[]).filter(M=>!oo.has(M.path.split(".")[0])&&!Uu(M.path))}),G=W(()=>new Map(N.value.map(x=>[x.path,x]))),z=W(()=>ne.value.reduce((x,M)=>x+M.sections.length,0)),P=W(()=>N.value.length),D=W(()=>ek),R=W(()=>_.value.length>0),ee=W(()=>C.value.length>0),ne=W(()=>{if(!e.value)return[];const x=new Set(Ga.flatMap(fe=>fe.sections)),M=Ga.map(fe=>({...fe,sections:fe.sections.filter(Fe=>Object.hasOwn(e.value,Fe)&&!oo.has(Fe))})).filter(fe=>fe.sections.length),V=Object.keys(e.value).filter(fe=>!x.has(fe)&&!oo.has(fe));return V.length&&M.push({key:"other",label:"Other",icon:"folder",sections:V}),M}),ge=W(()=>e.value?{...e.value,...b.value}:null),ie=W(()=>{if(!e.value)return[];const x=[];for(const[M,V]of Object.entries(b.value))Lm(e.value[M],V,M,x);return x.filter(M=>!Ni(M.oldVal,M.newVal)).map(M=>{const V=L(M.path);return{...M,label:(V==null?void 0:V.label)||Aa(M.path.split(".").at(-1)),apply_mode:(V==null?void 0:V.apply_mode)||me(M.path.split(".")[0])}})}),be=W(()=>ie.value.length>0),Q=W(()=>ie.value.length),ye=W(()=>new Set(ie.value.map(x=>x.path.split(".")[0])).size),J=W(()=>!!u.value||p.value!=="all"),U=W(()=>{const x={...g.value};for(const M of ie.value){const V=L(M.path),fe=wa(V,M.newVal);fe&&(x[M.path]=fe)}return x}),le=W(()=>Object.keys(U.value).length>0),re=W(()=>e.value?(J.value?ne.value:ne.value.filter(M=>M.key===f.value)).map(M=>({...M,sections:M.sections.filter(V=>Oe(V))})).filter(M=>M.sections.length):[]),Se=W(()=>{const x=["live_read","live_apply","live_for_new_work","restart","activation_required","legacy_control","dormant"],M=new Map(x.map(V=>[V,[]]));for(const V of ie.value){const fe=M.has(V.apply_mode)?V.apply_mode:"restart";M.get(fe).push(V)}return x.filter(V=>M.get(V).length).map(V=>({key:V,label:ms(V),entries:M.get(V)}))}),_e=W(()=>ie.value.filter(x=>x.apply_mode==="restart").length),Ue=W(()=>N.value.filter(x=>x.pending_restart)),k=W(()=>Ue.value.length);function L(x){const M=G.value.get(x);return M?{...M,apply_details:Sl([M])}:null}function B(x){const M=`${x}.`;return N.value.filter(V=>V.path===x||V.path.startsWith(M))}function se(x){return B(x).length}function Y(x){return Aa(x)}function X(x){const M=B(x);if(!M.length)return`${Aa(x)} configuration.`;const V=M.find(gt=>gt.sensitivity==="public"&&gt.description)||M.find(gt=>gt.description),fe=(V==null?void 0:V.description)||"";return fe.match(/setting for (.+)\.$/i)?`${Aa(x)} settings and runtime behaviour.`:fe}function me(x){const M=[...new Set(B(x).map(V=>V.apply_mode))];return M.length===1?M[0]:M.includes("restart")?"restart":M.includes("activation_required")?"activation_required":M[0]||"restart"}function ce(x){const M=[...new Set(B(x).map(V=>ms(V.apply_mode)))];return M.length?M.length===1?M[0]:`Mixed apply behaviour: ${M.join(" · ")}`:""}function de(x){return Sl(B(x))}function ae(x){var M;return Object.hasOwn(b.value,x)?b.value[x]:(M=e.value)==null?void 0:M[x]}function we(){const x=ae("mcp")||{},M=Object.keys(x.servers||{}).length;return`${x.enabled?"Globally enabled":"Globally disabled"} · ${M} configured server${M===1?"":"s"}.`}function ve(x,M){return M.split(".").reduce((V,fe)=>V==null?void 0:V[fe],x)}function xe(x){const M=ge.value;return B(x).filter(V=>Uu(V.path)?!1:V.path.split(".").length<=2?!0:!V.path.includes(".*")).map(V=>({...V,key:V.path.split(".").at(-1),value:ve(M,V.path),apply_details:Sl([V]),editor:V.path==="agents.final_warning_iterations"?"warning-chips":null}))}function oe(x){const M=x.path.split(".");return M.length>2?M.slice(0,2).join("."):null}function H(x){const M=new Map;for(const V of xe(x)){const fe=oe(V),Fe=fe||`${x}.__root`;M.has(Fe)||M.set(Fe,{key:Fe,path:fe,entries:[]}),M.get(Fe).entries.push(V)}return[...M.values()].map(V=>{const fe=V.entries.find(Fe=>Fe.group_description);return{...V,label:V.path?Aa(V.path.split(".").at(-1)):null,description:(fe==null?void 0:fe.group_description)||null,apply_details:Sl(V.entries),runtime_summaries:Te(V.entries)}})}function pe(x){return{save:x.save_effect||(x.apply_mode==="dormant"?"Saving records this value in config.yml.":"Saving records this value and validates the section."),runtime:x.runtime_effect||{live_read:"Odin reads the saved value during current work.",live_apply:"Odin reloads this setting without a restart.",live_for_new_work:"New work uses the saved value; existing work keeps its snapshot.",restart:"Odin keeps using its startup value until a clean restart.",activation_required:"Odin keeps the current behavior until you enable this feature separately.",legacy_control:"Odin keeps the existing compatibility behavior until you apply this choice.",dormant:"This version of Odin does not use the saved value. Restarting will not activate it."}[x.apply_mode]||"Effective runtime state is not currently observable."}}function Te(x){const M=new Map;for(const V of x){const fe=pe(V),Fe=`${V.apply_mode}|${fe.save}|${fe.runtime}`;M.has(Fe)||M.set(Fe,{key:Fe,label:ms(V.apply_mode),save:fe.save,runtime:fe.runtime})}return[...M.values()]}function Ae(x){if(De(x))return x.runtime_effect||x.activation_policy||"";if(x.apply_mode==="activation_required"){const M=x.activation_policy||x.runtime_effect;return M?`Not active after saving. No activation control exists in this release. ${M}`:"Not active after saving; no activation control exists in this release."}return""}function De(x){return x.action_available===!0&&!!(x.action_label&&x.action_endpoint)}async function ut(x){if(De(x))try{if(pt(x.path))throw new Error("Save this setting before applying its action.");const M=String(x.action_method||"POST").toLowerCase(),V={post:K.post.bind(K),put:K.put.bind(K),delete:K.del.bind(K)}[M];if(!V)throw new Error("Unsupported configuration action");await V(x.action_endpoint,x.action_body||void 0),await Z(),Xt("success",`${x.action_label} completed.`)}catch(M){Xt("error",M.message||`${x.action_label} failed`)}}function it(x,M){return[x.label,x.path,x.description,...x.aliases||[]].filter(Boolean).join(" ").toLowerCase().includes(M)}function te(x){const M=u.value.trim().toLowerCase();return M?B(x).filter(V=>it(V,M)):[]}function Oe(x){const M=B(x);if(p.value!=="all"&&!M.some(fe=>fe.apply_state===p.value))return!1;const V=u.value.trim().toLowerCase();return!V||`${Y(x)} ${x}`.toLowerCase().includes(V)?!0:M.some(fe=>it(fe,V))}function Le(x,M){return B(x).filter(V=>V.apply_state===M).length}function Me(x){return x==="all"?P.value:N.value.filter(M=>M.apply_state===x).length}function Qe(x){const M=x.sections.flatMap(V=>B(V));return{fields:M.length,modified:ie.value.filter(V=>x.sections.includes(V.path.split(".")[0])).length,pending_restart:M.filter(V=>V.apply_state==="pending_restart").length,invalid:M.filter(V=>V.apply_state==="invalid").length,dormant:M.filter(V=>V.apply_state==="dormant").length}}function je(x){var M;return Object.hasOwn(b.value,x)&&!Ni((M=e.value)==null?void 0:M[x],b.value[x])}function pt(x){return ie.value.some(M=>M.path===x||M.path.startsWith(`${x}.`))}function Tt(x){f.value=x,u.value="",p.value="all";try{localStorage.setItem(Im,x)}catch{}}function Hs(x){p.value=x}function zs(){u.value="",p.value="all"}function Ls(x){var M;return((M=ne.value.find(V=>V.sections.includes(x)))==null?void 0:M.sections)||[]}function Ft(x){const M=Ls(x),V=M.find(fe=>m.value[fe]===!0);return V||M.find(fe=>m.value[fe]!==!1)||null}function jt(x){return u.value&&!T.value&&Oe(x)?!0:T.value?Ft(x)===x:Object.hasOwn(m.value,x)?m.value[x]===!0:!0}function nn(x){const M=!jt(x);if(T.value){const V={...m.value};for(const fe of Ls(x))V[fe]===!0&&(V[fe]=!1);V[x]=M,m.value=V;return}m.value={...m.value,[x]:M}}function js(){_.value.push(sa(b.value)),_.value.length>ak&&_.value.shift(),C.value=[]}function Vs(){be.value&&(js(),b.value={},g.value={},v.value=!1)}function qs(x,M=!1){const V=Date.now();if(M&&$.path===x&&V-$.at<ik){$.at=V;return}js(),$={path:x,at:V}}function Ns(x,M,V){if(!M.length)return V;const fe=sa(x??{});let Fe=fe;for(let gt=0;gt<M.length-1;gt+=1){const lt=M[gt];Fe[lt]=sa(Fe[lt]??{}),Fe=Fe[lt]}return Fe[M.at(-1)]=V,fe}function Rn(x){var M;return Object.hasOwn(b.value,x)?b.value[x]:sa((M=e.value)==null?void 0:M[x])}function vt(x,M,V={}){var di;const[fe,...Fe]=x.path.split(".");qs(x.path,!!V.coalesce);const gt=Rn(fe),lt=Fe.length?Ns(gt,Fe,M):M,Gs={...b.value};if(Ni(lt,(di=e.value)==null?void 0:di[fe])?delete Gs[fe]:Gs[fe]=lt,b.value=Gs,g.value[x.path]){const od={...g.value};delete od[x.path],g.value=od}}function Ms(x){$={path:null,at:0},E.value={...E.value,[x]:String(ve(ge.value,x)??"")}}function qn(x){if($={path:null,at:0},!Object.hasOwn(E.value,x))return;const M={...E.value};delete M[x],E.value=M}function Ye(x){const M=E.value[x.path];if($={path:null,at:0},M===""){g.value={...g.value,[x.path]:"Enter a number."};return}const V=Number(M);if(Number.isNaN(V)||x.type==="integer"&&!Number.isInteger(V)){g.value={...g.value,[x.path]:x.type==="integer"?"Enter a whole number.":"Enter a number."};return}const fe={...E.value};delete fe[x.path],E.value=fe,vt(x,V,{coalesce:!0})}function q(x){return Object.hasOwn(E.value,x.path)?E.value[x.path]:x.value??""}function ue(x,M){if(E.value={...E.value,[x.path]:M},M===""){g.value={...g.value,[x.path]:"Enter a number."};return}const V=Number(M);if(!Number.isFinite(V)||x.type==="integer"&&!Number.isInteger(V)){g.value={...g.value,[x.path]:x.type==="integer"?"Enter a whole number.":"Enter a valid number."};return}if(g.value[x.path]){const fe={...g.value};delete fe[x.path],g.value=fe}vt(x,V,{coalesce:!0})}function Ne(x){const M=Number.parseInt(I.value,10);if(!Number.isInteger(M)||M<1){g.value={...g.value,[x.path]:"Warning thresholds must be positive whole numbers."};return}const V=[...new Set([...x.value||[],M])].sort((fe,Fe)=>Fe-fe);I.value="",vt(x,V)}function We(x,M){vt(x,(x.value||[]).filter(V=>V!==M))}function ot(x){return x.apply_mode==="live_read"?"Odin reads the saved file value on next use.":x.apply_mode==="live_for_new_work"?"New work uses the saved file value.":x.apply_mode==="live_apply"?x.apply_handler?`Apply the saved value through ${x.apply_handler}.`:"Apply it through its dedicated owner page or endpoint.":x.apply_mode==="restart"?"Restart Odin for the saved collection to take effect.":x.apply_mode==="activation_required"?"Saving does not enable it. No activation control exists in this release.":x.apply_mode==="dormant"?"This release does not use the saved collection.":"Follow the runtime details shown for this setting."}function Ot(x){return x.type==="array"&&Array.isArray(x.value)&&!x.structured_container&&!x.structured_container_child&&x.sensitivity==="public"&&x.value.every(M=>["string","number","boolean"].includes(typeof M))}function Gn(x){const M=String(w.value[x.path]??"").trim();if(!M)return;const V=[...new Set([...x.value||[],M])];w.value={...w.value,[x.path]:""},vt(x,V)}function Qt(x,M){vt(x,(x.value||[]).filter(V=>V!==M))}function wa(x,M){var fe;if(!x)return null;if((fe=x.enum)!=null&&fe.length&&!x.enum.includes(M))return`Choose one of: ${x.enum.join(", ")}`;if(x.path==="agents.final_warning_iterations"&&(!Array.isArray(M)||!M.length))return"Add at least one warning threshold.";const V=x.constraints||{};if((x.type==="integer"||x.type==="number")&&typeof M=="number"){if(V.minimum!==void 0&&M<V.minimum)return`Must be at least ${V.minimum}${x.unit?` ${x.unit}`:""}`;if(V.maximum!==void 0&&M>V.maximum)return`Must be at most ${V.maximum}${x.unit?` ${x.unit}`:""}`}return null}function Ds(x){return U.value[x.path]||null}function oi(x){const M=`${x}.`;return Object.keys(U.value).some(V=>V===x||V.startsWith(M))}function ka(){_.value.length&&(C.value.push(sa(b.value)),b.value=_.value.pop(),g.value={},E.value={},$={path:null,at:0})}function Kn(){C.value.length&&(_.value.push(sa(b.value)),b.value=C.value.pop(),g.value={},E.value={},$={path:null,at:0})}function Sa(){!be.value||le.value||(v.value=!0,S.value=!1)}function Wn(){v.value=!1}function In(){Vs()}function ms(x){return sk[x]||Aa(x||"unknown")}function an(x){return`apply-${String(x||"unknown").replaceAll("_","-")}`}function j(x){return`cfgc-field-${x.replace(/[^a-zA-Z0-9_-]/g,"-")}`}function ke(x){return`${j(x)}-input`}function Re(x){const M=document.getElementById(j(x))||document.getElementById(j(x.split(".").slice(0,2).join(".")));M==null||M.scrollIntoView({behavior:"smooth",block:"center"})}function Xt(x,M){l.value={type:x,message:M},window.setTimeout(()=>{var V;((V=l.value)==null?void 0:V.message)===M&&(l.value=null)},3500)}function Zn(){o.value=!1,p.value="pending_restart",u.value="";const x=tk(n.value);x&&(x.scrollTop=0)}function Jn(){o.value=!1}function Yn(x=1800){O&&window.clearTimeout(O),O=window.setTimeout(ci,x)}async function ci(){if(c.value){if(F+=1,F>45){c.value=!1,d.value="Odin did not return with the new startup settings within 90 seconds.";return}try{if(t.value=await co(),k.value===0){c.value=!1,d.value=null,Xt("success","Odin restarted and the saved startup settings are active.");return}}catch{}Yn(2e3)}}async function Ce(){if(!c.value){d.value=null;try{await K.post("/api/restart",{}),c.value=!0,F=0,o.value=!1,Yn()}catch(x){d.value=x.message||"Odin could not schedule a restart."}}}async function A(){if(!(!be.value||le.value||a.value)){a.value=!0;try{const x=ok(e.value,b.value),M=await K.put("/api/config",x);e.value=M,b.value={},_.value=[],C.value=[],g.value={},v.value=!1;try{t.value=await co(),r.value=null,o.value=k.value>0,Xt("success",k.value?`Configuration saved. ${k.value} setting${k.value===1?"":"s"} still use startup values.`:"Configuration saved. Apply status has been refreshed.")}catch(V){r.value=V.message||"Unknown metadata error.",Xt("error",`Configuration saved, but apply status could not be refreshed: ${r.value}`)}}catch(x){Xt("error",x.message||"Configuration could not be saved")}finally{a.value=!1}}}async function Z(){var x,M;if(!be.value){s.value=!0,i.value=null;try{const V=await K.get("/api/config"),fe=await co();e.value=V,t.value=fe,r.value=null;const Fe=ne.value;if(Fe.some(gt=>gt.key===f.value)||(f.value=((x=Fe[0])==null?void 0:x.key)||Ga[0].key),T.value){const lt=(((M=Fe.find(Gs=>Gs.key===f.value))==null?void 0:M.sections)||[]).find(Gs=>m.value[Gs]===!0);m.value=lt?{...m.value,[lt]:!0}:{}}}catch(V){i.value=V.message||"Unknown configuration error"}finally{s.value=!1}}}function he(x){if(v.value||!(x.ctrlKey||x.metaKey))return;const M=x.target;M instanceof HTMLElement&&(M.matches("input, textarea, select")||M.isContentEditable)||(!x.shiftKey&&x.key.toLowerCase()==="z"?(x.preventDefault(),ka()):(x.key.toLowerCase()==="y"||x.shiftKey&&x.key.toLowerCase()==="z")&&(x.preventDefault(),Kn()))}function Pe(x){T.value=x.matches}return is(m,x=>{try{localStorage.setItem(Rm,JSON.stringify(x))}catch{}},{deep:!0}),Ke(()=>{var x;Z(),document.addEventListener("keydown",he),y=window.matchMedia("(max-width: 760px)"),Pe(y),(x=y.addEventListener)==null||x.call(y,"change",Pe)}),_t(()=>{var x;document.removeEventListener("keydown",he),(x=y==null?void 0:y.removeEventListener)==null||x.call(y,"change",Pe),O&&window.clearTimeout(O)}),{config:e,meta:t,loading:s,saving:a,error:i,toast:l,metaRefreshError:r,restartPromptOpen:o,restartScheduled:c,restartError:d,configMain:n,searchQuery:u,healthFilter:p,activeCategory:f,reviewOpen:v,mobileOverflowOpen:S,warningThresholdInput:I,arrayInputs:w,healthFilters:D,visibleCategories:ne,displayGroups:re,reviewGroups:Se,sectionCount:z,fieldCount:P,hasChanges:be,changeCount:Q,changedSectionCount:ye,hasDraftErrors:le,canUndo:R,canRedo:ee,globalFilterActive:J,reviewRestartCount:_e,pendingRestartCount:k,pendingRestartFields:Ue,healthCount:Me,categoryStats:Qe,selectCategory:Tt,selectHealthFilter:Hs,clearFilters:zs,sectionLabel:Y,sectionDescription:X,sectionFieldCount:se,sectionHealthCount:Le,sectionApplySummary:ce,sectionApplyDetails:de,sectionEntries:xe,fieldGroups:H,sectionSearchHits:te,mcpConfigSummary:we,fieldRuntimeCopy:pe,fieldSpecificRuntimeNote:Ae,hasHonestAction:De,runFieldAction:ut,sectionChanged:je,fieldChanged:pt,isSectionExpanded:jt,toggleSection:nn,discardAllDrafts:Vs,setFieldValue:vt,setNumberFieldValue:ue,numberInputValue:q,beginInputEdit:Ms,endTextInputEdit:qn,endInputEdit:Ye,addWarningThreshold:Ne,removeWarningThreshold:We,isScalarArray:Ot,addScalarArrayItem:Gn,removeScalarArrayItem:Qt,fieldError:Ds,sectionHasErrors:oi,undo:ka,redo:Kn,openReview:Sa,closeReview:Wn,mobileCancel:In,applyModeLabel:ms,applyClass:an,compactValue:lk,formatValue:rk,structuredApplyCopy:ot,fieldId:j,fieldInputId:ke,focusField:Re,fetchConfig:Z,saveConfig:A,restartOdin:Ce,restartLater:Jn,reviewPendingRestart:Zn}}},pk=/^\d{15,25}$/;function Nm(e){return String((e==null?void 0:e.display_name)||(e==null?void 0:e.username)||(e==null?void 0:e.id)||"Unknown user")}const Mm={props:{members:{type:Array,default:()=>[]},excludedIds:{type:Array,default:()=>[]},placeholder:{type:String,default:"Search Discord users…"},ariaLabel:{type:String,default:"Search Discord users"},optionsId:{type:String,required:!0},autofocus:{type:Boolean,default:!1}},emits:["select"],template:`
  ...
  `,setup(e,{emit:t}){const s=h(""),n=h(!1),a=h(0),i=h(null),l=W(()=>new Set((e.excludedIds||[]).map(String))),r=W(()=>{const C=s.value.toLowerCase().trim();return(e.members||[]).filter(v=>l.value.has(String(v.id))?!1:C?u(v).toLowerCase().includes(C)||String(v.username||"").toLowerCase().includes(C)||String(v.id).includes(C):!0)}),o=W(()=>{const C=s.value.trim();return r.value.length===0&&pk.test(C)&&!l.value.has(C)?C:""}),c=W(()=>r.value.length+(o.value?1:0)),d=W(()=>{if(n.value){if(r.value[a.value])return`${e.optionsId}-${a.value}`;if(o.value&&a.value===r.value.length)return`${e.optionsId}-raw`}});function u(C){return Nm(C)}function p(){n.value=!0,a.value=0}function f(){p()}function m(){const C=Math.max(c.value-1,0);a.value=Math.min(a.value+1,C)}function b(){a.value=Math.max(a.value-1,0)}function E(){const C=r.value[a.value];C?I(C):o.value&&a.value===r.value.length&&w(o.value)}function I(C){w(String(C.id))}function w(C){t("select",C),s.value="",n.value=!1,a.value=0}function g(){n.value=!1}function _(){setTimeout(g,150)}return Ke(()=>{e.autofocus&&Et(()=>{var C;return(C=i.value)==null?void 0:C.focus()})}),{query:s,open:n,highlightedIndex:a,input:i,filteredMembers:r,rawId:o,activeOptionId:d,memberName:u,openOptions:p,onInput:f,highlightNext:m,highlightPrevious:b,selectHighlighted:E,selectMember:I,selectId:w,closeOptions:g,onBlur:_}}};function Hu(e,t,s){var n;return((n=e==null?void 0:e.config)==null?void 0:n[t])!=null?e.config[t]:s==null?void 0:s[t]}const fk={components:{DiscordUserCombobox:Mm},template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=h([]),t=h(!0),s=h(null),n=h({}),a=h(null),i=h(null),l=h(!1),r=h(null),o=h({}),c=h([]);let d=0;const u=Object.freeze([{key:"allowed_users",label:"Allowed users",description:"Absolute gate for ordinary conversational intake. Guild/channel settings cannot readmit blocked users; prefix commands use separate authorization and allowed test webhooks bypass this gate.",placeholder:"Search Discord users…",userAutocomplete:!0,fullWidth:!0},{key:"channels",label:"Allowed channels",description:"Absolute gate for ordinary conversational intake. Guild/channel settings cannot readmit blocked channels; prefix commands use separate authorization.",placeholder:"Discord channel ID",fullWidth:!0},{key:"ignore_bot_ids",label:"Ignored bot IDs",description:"Ignored unless the bot explicitly mentions Odin; the effective respond-to-bots policy still applies.",placeholder:"Search Discord users or bots…",userAutocomplete:!0,fullWidth:!0}]),p=W(()=>JSON.stringify(a.value)!==JSON.stringify(i.value)),f=W(()=>new Map(c.value.map(N=>[String(N.id),N])));function m(N){return N.config&&N.config.enabled!==void 0?N.config.enabled:!0}function b(N){return Hu(N,"require_mention",a.value)}function E(N){return Hu(N,"respond_to_bots",a.value)}function I(N){return N.config&&Object.keys(N.config).length>0}function w(N){n.value[N]=!n.value[N]}function g(N){const G=N.discord||{};return{allowed_users:[...G.allowed_users||[]],channels:[...G.channels||[]],respond_to_bots:!!G.respond_to_bots,require_mention:!!G.require_mention,ignore_bot_ids:[...G.ignore_bot_ids||[]]}}async function _({showLoading:N=!0}={}){const G=++d;N&&(t.value=!0),s.value=null;try{const z=await K.get("/api/discord/guilds");G===d&&(e.value=z)}catch(z){G===d&&(s.value=z.message)}finally{N&&G===d&&(t.value=!1)}}async function C(){t.value=!0,s.value=null;try{const[N,G,z]=await Promise.all([K.get("/api/discord/guilds"),K.get("/api/discord/members").catch(()=>[]),K.get("/api/config")]),P=g(z),D=p.value;a.value=P,D||(i.value=JSON.parse(JSON.stringify(P))),c.value=G,e.value=N,r.value=null}catch(N){s.value=N.message}finally{t.value=!1}}async function v(N,G,z){try{await K.put("/api/discord/guild/"+N+"/config",{[G]:z}),await _({showLoading:!1})}catch(P){s.value=P.message}}async function S(N,G,z,P){try{await K.put("/api/discord/channel/"+N+"/config",{[z]:P}),await _({showLoading:!1})}catch(D){s.value=D.message}}async function T(N,G){try{await K.put("/api/discord/channel/"+N+"/config",{clear:!0}),await _({showLoading:!1})}catch(z){s.value=z.message}}function y(N,G){const z=String(G);if(!N.userAutocomplete)return z;const P=f.value.get(z);return P?Nm(P):z}function O(N,G=null){const z=String(G??o.value[N]??"").trim();!z||i.value[N].includes(z)||(i.value[N]=[...i.value[N],z],o.value={...o.value,[N]:""})}function $(N,G){i.value[N]=i.value[N].filter(z=>z!==G)}async function F(){if(!(!p.value||l.value)){l.value=!0,r.value=null;try{const G=(await K.put("/api/config",{discord:i.value})).discord||i.value;a.value={allowed_users:[...G.allowed_users||[]],channels:[...G.channels||[]],respond_to_bots:!!G.respond_to_bots,require_mention:!!G.require_mention,ignore_bot_ids:[...G.ignore_bot_ids||[]]},i.value=JSON.parse(JSON.stringify(a.value))}catch(N){r.value=N.message||"Global defaults could not be saved."}finally{l.value=!1}}}return Ke(C),{guilds:e,loading:t,error:s,expanded:n,globalDraft:i,globalSaving:l,globalError:r,globalArrayInputs:o,globalMembers:c,globalListEditors:u,globalChanged:p,guildEnabled:m,guildMention:b,guildBots:E,hasOverride:I,toggleGuild:w,fetchAll:C,fetchGuilds:_,setGuildConfig:v,setChannelConfig:S,clearOverride:T,globalItemLabel:y,addGlobalItem:O,removeGlobalItem:$,saveGlobalDefaults:F}}},vs=e=>e==null?e:JSON.parse(JSON.stringify(e));function hk({applyDefault:e,applyUser:t,applyDelete:s,onDefaultConfirmed:n=()=>{},onDefaultRollback:a=()=>{},onUserConfirmed:i=()=>{},onUserRollback:l=()=>{},onUserDeleted:r=()=>{},onError:o=()=>{}}){let c=Promise.resolve(),d=0,u=0;const p=new Map;let f=null;const m=new Map;function b(v){d+=1;const S=c.then(v,v);return c=S.catch(()=>{}),S}function E(v,S){f=vs(v),m.clear();for(const[T,y]of Object.entries(S||{}))m.set(T,vs(y))}function I(v){const S=vs(v),T=++u;return b(async()=>{try{await e(vs(S)),f=vs(S),T===u&&n(vs(S))}catch(y){T===u&&(a(vs(f)),o(y,{kind:"default"}))}})}function w(v,S){const T=vs(S),y=(p.get(v)||0)+1;return p.set(v,y),b(async()=>{try{await t(v,vs(T)),m.set(v,vs(T)),y===p.get(v)&&i(v,vs(T))}catch(O){y===p.get(v)&&(l(v,vs(m.get(v)??null)),o(O,{kind:"user",uid:v}))}})}function g(v){const S=(p.get(v)||0)+1;return p.set(v,S),b(async()=>{try{await s(v),m.delete(v),S===p.get(v)&&r(v)}catch(T){S===p.get(v)&&(l(v,vs(m.get(v)??null)),o(T,{kind:"delete",uid:v}))}})}async function _(){for(;;){const v=c;if(await v,v===c)return d}}async function C(v){for(;;){const S=await _(),T=await v();if(S===d)return T}}return{seed:E,saveDefault:I,saveUser:w,deleteUser:g,whenIdle:_,readSnapshot:C,get revision(){return d}}}const mk={components:{DiscordUserCombobox:Mm},template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=h(!0),t=h(""),s=h(null),n=h([]),a=h({allowed_hosts:[],default_host:""}),i=h({}),l=h(!1),r=h([]),o=W(()=>{const v={};for(const S of r.value)v[S.id]=S;return v});function c(v){return o.value[v]||null}function d(v,S){return v?v.allowed_hosts===null||v.allowed_hosts===void 0?{allowed_hosts:[...S],default_host:v.default_host||"",allow_all:!0}:{allowed_hosts:v.allowed_hosts,default_host:v.default_host||"",allow_all:!1}:{allowed_hosts:[...S],default_host:S[0]||"",allow_all:!0}}const u=hk({applyDefault:async v=>{const S=v.allow_all?null:v.allowed_hosts;await K.put("/api/host-access/default-policy",{allowed_hosts:S,default_host:v.default_host})},applyUser:async(v,S)=>{const T=S.allow_all?null:S.allowed_hosts;await K.put(`/api/host-access/user/${v}`,{allowed_hosts:T,default_host:S.default_host})},applyDelete:v=>K.del(`/api/host-access/user/${v}`),onDefaultConfirmed:()=>Ie.success("Default policy updated"),onDefaultRollback:v=>{v&&(a.value=v)},onUserConfirmed:v=>{const S=c(v);Ie.success(`Updated access for ${S?S.display_name:v}`)},onUserRollback:(v,S)=>{const T={...i.value};S?T[v]=S:delete T[v],i.value=T},onUserDeleted:v=>{const S={...i.value};delete S[v],i.value=S},onError:(v,S)=>{var y;const T=S.uid?` ${((y=c(S.uid))==null?void 0:y.display_name)||S.uid}`:"";Ie.error(`${v.message||"Failed to save"} — reverted${T}`)}});let p=0;async function f(){const v=++p;e.value=!0,t.value="";try{const S=await u.readSnapshot(()=>K.get("/api/host-access"));if(v!==p)return;s.value=S,n.value=S.available_hosts||[],a.value=d(S.default_policy,n.value);const T=S.users||{},y={};for(const[O,$]of Object.entries(T))y[O]=d($,n.value);i.value=y,u.seed(a.value,y)}catch(S){v===p&&(t.value=S.message||"Failed to fetch host access data")}finally{v===p&&(e.value=!1)}try{const S=await K.get("/api/discord/members")||[];v===p&&(r.value=S)}catch{v===p&&(r.value=[])}}function m(){u.saveDefault(a.value)}function b(v,S){a.value.allow_all=!1,S?a.value.allowed_hosts.includes(v)||a.value.allowed_hosts.push(v):(a.value.allowed_hosts=a.value.allowed_hosts.filter(T=>T!==v),a.value.default_host===v&&(a.value.default_host=a.value.allowed_hosts[0]||"")),m()}function E(v){const S=i.value[v];S&&u.saveUser(v,S)}function I(v,S,T){const y=i.value[v];y&&(y.allow_all=!1,T?y.allowed_hosts.includes(S)||y.allowed_hosts.push(S):(y.allowed_hosts=y.allowed_hosts.filter(O=>O!==S),y.default_host===S&&(y.default_host=y.allowed_hosts[0]||"")),E(v))}function w(v,S){const T=i.value[v];T&&(T.default_host=S,E(v))}function g(){l.value=!0}function _(v){!/^\d{15,25}$/.test(v)||i.value[v]||(i.value[v]={allowed_hosts:[...n.value],default_host:n.value[0]||"",allow_all:!1},E(v),l.value=!1)}async function C(v){const S=c(v);await Jt({title:"Remove user override",message:`Remove the host access override for ${S?S.display_name:v}? They will fall back to the default policy.`,confirmLabel:"Remove",danger:!0})&&(await u.deleteUser(v),i.value[v]||Ie.success(`Removed override for ${S?S.display_name:v}`))}return Ke(f),{loading:e,error:t,data:s,availableHosts:n,defaultPolicy:a,users:i,showAddUser:l,members:r,fetchData:f,saveDefaultPolicy:m,toggleDefaultHost:b,getMember:c,toggleUserHost:I,setUserDefault:w,openAddUser:g,addUserById:_,deleteUser:C}}},vk={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4">
  ...
  `,setup(){const e=h(!0),t=h(""),s=h(null),n=h([]),a=h(!1),i=h(!1),l=h(null),r=h(null),o=h(!1),c=h({user_id:"",username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""}),d=h({username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""}),u=W(()=>c.value.host_mode==="select"?c.value.allowed_hosts:c.value.host_mode==="none"?[]:n.value),p=W(()=>d.value.host_mode==="select"?d.value.allowed_hosts:d.value.host_mode==="none"?[]:n.value);function f(T){return T==="admin"?"text-xs px-1.5 py-0.5 rounded bg-red-900/50 text-red-400":T==="user"?"text-xs px-1.5 py-0.5 rounded bg-blue-900/50 text-blue-400":"text-xs px-1.5 py-0.5 rounded bg-gray-700 text-gray-400"}async function m(){e.value=!0,t.value="";try{const T=await K.get("/api/tokens");s.value=T.tokens||[],n.value=T.available_hosts||[]}catch(T){t.value=T.message||"Failed to load tokens"}finally{e.value=!1}}function b(T){return!T||!T.trim()?[]:T.split(",").map(y=>y.trim()).filter(Boolean)}function E(T,y){const O=c.value.allowed_hosts;if(y&&!O.includes(T)&&O.push(T),!y){const $=O.indexOf(T);$>=0&&O.splice($,1)}}function I(T,y){const O=d.value.allowed_hosts;if(y&&!O.includes(T)&&O.push(T),!y){const $=O.indexOf(T);$>=0&&O.splice($,1)}}async function w(){var T;i.value=!0;try{const y=b(c.value.allowed_tools_str),O=c.value.host_mode,$=O==="none"?[]:O==="select"?c.value.allowed_hosts:null,F={user_id:c.value.user_id.trim(),username:c.value.username.trim()||"API",tier:c.value.tier,label:c.value.label.trim(),allowed_tools:y.length?y:[]};$!==null&&(F.allowed_hosts=$),F.default_host=c.value.default_host||"";const N=await K.post("/api/tokens",F);l.value=N.token,c.value={user_id:"",username:"",tier:"admin",label:"",host_mode:"default",allowed_hosts:[],default_host:"",allowed_tools_str:""},a.value=!1,Ie.success("Token created"),await m()}catch(y){Ie.error(((T=y.data)==null?void 0:T.error)||y.message||"Failed to create token")}finally{i.value=!1}}function g(T){r.value=T;const y=T.allowed_hosts;let O="default";y==null?O="default":Array.isArray(y)&&y.length===0?O="none":Array.isArray(y)&&(O="select"),d.value={username:T.username||"",tier:T.tier||"admin",label:T.label||"",host_mode:O,allowed_hosts:Array.isArray(y)?[...y]:[],default_host:T.default_host||"",allowed_tools_str:(T.allowed_tools||[]).join(", ")}}async function _(){var T;if(r.value){o.value=!0;try{const y=b(d.value.allowed_tools_str),O=d.value.host_mode,$={username:d.value.username,tier:d.value.tier,label:d.value.label,allowed_tools:y};O==="none"?$.allowed_hosts=[]:O==="select"?$.allowed_hosts=d.value.allowed_hosts:$.allowed_hosts=null,$.default_host=d.value.default_host||"",await K.put("/api/tokens/"+encodeURIComponent(r.value.user_id),$),r.value=null,Ie.success("Token updated"),await m()}catch(y){Ie.error(((T=y.data)==null?void 0:T.error)||y.message||"Failed to update")}finally{o.value=!1}}}async function C(T){var O;if(await Jt({title:"Regenerate token",message:`Regenerate token for ${T.username||T.user_id}? The old token will stop working immediately.`,confirmLabel:"Regenerate",danger:!0}))try{const $=await K.post("/api/tokens/"+encodeURIComponent(T.user_id)+"/regenerate");l.value=$.token,Ie.success("Token regenerated")}catch($){Ie.error(((O=$.data)==null?void 0:O.error)||$.message||"Failed to regenerate")}}async function v(T){var O;if(await Jt({title:"Delete token",message:`Delete token for ${T.username||T.user_id}? This cannot be undone.`,confirmLabel:"Delete",danger:!0}))try{await K.del("/api/tokens/"+encodeURIComponent(T.user_id)),Ie.success("Token deleted"),await m()}catch($){Ie.error(((O=$.data)==null?void 0:O.error)||$.message||"Failed to delete")}}async function S(){if(l.value)try{await navigator.clipboard.writeText(l.value),Ie.success("Copied to clipboard")}catch{Ie.error("Copy failed — select and copy manually")}}return Ke(m),{loading:e,error:t,tokens:s,availableHosts:n,showCreate:a,creating:i,newToken:l,editing:r,saving:o,createForm:c,editForm:d,createDefaultHostOptions:u,editDefaultHostOptions:p,fetchData:m,tierBadge:f,toggleCreateHost:E,toggleEditHost:I,createToken:w,startEdit:g,saveEdit:_,confirmRegenerate:C,confirmDelete:v,copyToken:S}}},gk=Object.freeze(["enabled","model","reasoning_effort","agent_reasoning_effort","agent_model"]),bk=Object.freeze(["request_timeout_seconds","stream_stall_timeout_seconds","retry","connection_pool","context_compression","context_budget_overrides","context_utilization"]),yk=Object.freeze(["enabled","base_url","model","max_tokens"]),xk=Object.freeze(["enabled","model","max_tokens"]);function Br(e,t){return Object.fromEntries(t.map(s=>[s,e[s]]))}function zu(e){return Br(e,gk)}function ju(e){return Br(e,bk)}function _k(e,{includeApiKey:t=!1}={}){const s=Br(e,yk);return t&&(s.api_key=e.api_key),s}function wk(e){return{timeout:e.timeout}}function kk(e,{includeApiKey:t=!1}={}){const s=Br(e,xk);return t&&(s.api_key=e.api_key),s}function Sk(e){return{timeout:e.timeout}}function Tl(e,t=500){let s=null;const n=(...a)=>{s&&clearTimeout(s),s=setTimeout(()=>{s=null,e(...a)},t)};return n.pending=()=>s!==null,n.cancel=()=>{s&&(clearTimeout(s),s=null)},n}const Tk={template:`
    <div class="p-6 page-fade-in">
  ...
  `,setup(){const e=h(!0),t=h(null),s=h(!1),n=h("codex"),a=h({enabled:!1,model:"gpt-5.6-sol",reasoning_effort:"xhigh",agent_reasoning_effort:"auto",agent_model:"auto",request_timeout_seconds:3600,stream_stall_timeout_seconds:180,retry:{max_retries:3,base_delay:1,max_delay:30},connection_pool:{max_connections:10,keepalive_timeout:30},context_compression:{enabled:!0,max_context_chars:null,keep_recent_iterations:30},context_budget_overrides:{},context_utilization:60}),i=["gpt-5.6-sol","gpt-5.6-terra","gpt-5.6-luna","gpt-5.5"],l=W(()=>{const j=a.value.model;return j&&!i.includes(j)?[j,...i]:i}),r=W(()=>{const j=a.value.agent_model;return j&&j!=="auto"&&!i.includes(j)?[j,...i]:i}),o=["gpt-5.5","gpt-5.4","gpt-5.4-mini"],c=W(()=>!o.includes(a.value.model)&&!(o.includes(a.value.agent_model)&&a.value.agent_reasoning_effort==="")),d=W(()=>{const j=a.value.agent_model;return j==="auto"?!0:!o.includes(j||a.value.model)}),u=W(()=>{const j=a.value.agent_reasoning_effort;return j==="auto"?!1:(j||a.value.reasoning_effort)==="max"}),p=j=>o.includes(j)&&(a.value.reasoning_effort==="max"||a.value.agent_model===""&&u.value),f=j=>o.includes(j)&&u.value,m=h({enabled:!1,model:"gpt-5.6-luna"}),b=h({unavailable_reason:null}),E=W(()=>{const j=m.value.model;return j&&!i.includes(j)?[j,...i]:i});function I(j){const ke=j.target.value;m.value.enabled=ke!=="",ke!==""&&(m.value.model=ke),q()}const w=h(!1),g=h({codex:!1,ollama:!1,kimi:!1}),_=h(null),C=h(!1),v=h(""),S=h(null),T=h(!1);let y=0;const O=W(()=>{var j;return Object.entries(((j=_.value)==null?void 0:j.models)||{}).map(([ke,Re])=>{var Xt,Zn,Jn;return{model:ke,floor:Re.floor,override:Re.override,effectiveBudget:(Xt=Re.effective)==null?void 0:Xt.effective_budget,configuredPrimaryChars:(Zn=Re.configured)==null?void 0:Zn.primary_chars,primaryChars:(Jn=Re.effective)==null?void 0:Jn.primary_chars,provenance:Re.provenance,clampExpiresAt:Re.clamp_expires_at}})}),$=W(()=>{var j;return((j=_.value)==null?void 0:j.clamps)||[]}),F=W(()=>{var j,ke;return((ke=(j=_.value)==null?void 0:j.models)==null?void 0:ke[a.value.model])||null}),N=h({enabled:!1,base_url:"",model:"",api_key:"",max_tokens:4096,timeout:300}),G=h({enabled:!1,api_key:"",model:"",max_tokens:4096,timeout:300}),z=h(!1),P=h(!1),D=h(!1),R=h(!1),ee=h(!1),ne=h(!1),ge=h(!1),ie=h({configured:null}),be=h(!1),Q=h([]),ye=h(""),J=h(!1),U=h(!1),le=h({configured:null}),re=h(!1),Se=h([]),_e=h(""),Ue=h(!1),k=h(!1),L=h(!0),B=h(""),se=h({configured:null,accounts:[]}),Y=h(null),X=h(null),me=h(""),ce=h(null),de=h(!1),ae=h(null),we=h(null),ve=h("");let xe=null;function oe(j,ke="success"){Ie(j,ke==="error"?"error":"success")}function H(j){if(!j)return"?";const ke=j/(1024*1024*1024);return ke>=1?ke.toFixed(1)+" GB":(j/(1024*1024)).toFixed(0)+" MB"}function pe(j){return Number.isFinite(Number(j))?Number(j).toLocaleString():"—"}function Te(j){return j==null?"automatic (model-derived)":Number(j).toLocaleString()+" characters"}function Ae(j){const ke=new Date(j);return Number.isNaN(ke.getTime())?"unknown":ke.toLocaleString([],{dateStyle:"medium",timeStyle:"short"})}function De(j){return typeof j=="string"&&j.length>12?j.slice(0,8)+"…"+j.slice(-4):j}function ut(j){return j==="temporary learned clamp"?"is-clamp":j==="override"?"is-override":"is-built-in"}function it(j){const ke=a.value.context_budget_overrides[j.model];return j.floor!=null&&Number.isFinite(Number(ke))&&Number(ke)>j.floor}function te(j,ke){const Re={...a.value.context_budget_overrides};ke.target.value===""?delete Re[j]:Re[j]=Number(ke.target.value),a.value.context_budget_overrides=Re,T.value=!0}function Oe(j){a.value.context_utilization=j.target.value===""?"":Number(j.target.value),T.value=!0}function Le(j){const ke={...a.value.context_budget_overrides};delete ke[j],a.value.context_budget_overrides=ke,T.value=!0}async function Me(){e.value=!0,await Promise.all([Qe(),pt(),jt(),Tt(),je()]),e.value=!1}async function Qe({preserveBasic:j=!1,preserveAdvanced:ke=!1}={}){try{const Re=await K.get("/api/llm/status");t.value=Re,s.value=!1,n.value=Re.active_provider||"codex",Re.codex&&!Ye.pending()&&(j||(a.value.enabled=Re.codex.enabled,a.value.model=Re.codex.model||"gpt-5.6-sol",a.value.reasoning_effort=Re.codex.reasoning_effort||"medium",a.value.agent_reasoning_effort=Re.codex.agent_reasoning_effort||"",a.value.agent_model=Re.codex.agent_model||""),ke||(a.value.request_timeout_seconds=Re.codex.request_timeout_seconds??a.value.request_timeout_seconds,a.value.stream_stall_timeout_seconds=Re.codex.stream_stall_timeout_seconds??a.value.stream_stall_timeout_seconds,a.value.retry={...a.value.retry,...Re.codex.retry||{}},a.value.connection_pool={...a.value.connection_pool,...Re.codex.connection_pool||{}},a.value.context_compression={...a.value.context_compression,...Re.codex.context_compression||{}},!T.value&&!D.value&&(a.value.context_budget_overrides={...Re.codex.context_budget_overrides||{}},a.value.context_utilization=Re.codex.context_utilization??a.value.context_utilization))),Re.ollama&&!ue.pending()&&(j||(N.value.enabled=Re.ollama.enabled,N.value.base_url=Re.ollama.base_url||"",N.value.model=Re.ollama.model||"",N.value.max_tokens=Re.ollama.max_tokens||4096),ke||(N.value.timeout=Re.ollama.timeout??N.value.timeout)),Re.kimi&&!Ne.pending()&&(j||(G.value.enabled=Re.kimi.enabled,G.value.model=Re.kimi.model||"",G.value.max_tokens=Re.kimi.max_tokens||4096),ke||(G.value.timeout=Re.kimi.timeout??G.value.timeout)),Re.auxiliary&&(b.value=Re.auxiliary,q.pending()||(m.value.enabled=Re.auxiliary.enabled,m.value.model=Re.auxiliary.model||"gpt-5.6-luna"))}catch{t.value||(t.value={active_provider:"",codex:{configured:null},ollama:{configured:null},kimi:{configured:null}}),s.value=!0}}async function je(){const j=++y;C.value=!0,v.value="";try{const ke=await K.get("/api/context/windows");if(j!==y)return;_.value=ke,!D.value&&!T.value&&(a.value.context_budget_overrides=Object.fromEntries(Object.entries(ke.models||{}).filter(([,Re])=>Re.override!=null).map(([Re,Xt])=>[Re,Xt.override])),a.value.context_utilization=ke.utilization??a.value.context_utilization)}catch(ke){j===y&&(v.value=ke.message||"Failed to load context budgets")}finally{j===y&&(C.value=!1)}}async function pt(){try{if(ie.value=await K.get("/api/ollama/status"),be.value=!1,ie.value.model&&(ye.value=ie.value.model),ie.value.configured)try{const j=await K.get("/api/ollama/models");Q.value=j.models||[]}catch{Q.value=[]}else if(N.value.base_url)try{const j=await K.post("/api/ollama/probe-models",{base_url:N.value.base_url});Q.value=j.models||[]}catch{Q.value=[]}}catch{be.value=!0}}async function Tt(){L.value=!0,B.value="";try{se.value=await K.get("/api/codex/status")}catch(j){B.value=j.message||"Failed to fetch Codex status"}finally{L.value=!1}}async function Hs(){const j=t.value?t.value.active_provider:"codex";ge.value=!0;try{const ke=await K.post("/api/llm/switch",{provider:n.value});ke.error?(n.value=j,oe(ke.error,"error")):(oe("Switched to "+n.value+" ("+ke.model+")"),await Me())}catch(ke){n.value=j,oe(ke.message||"Switch failed","error")}finally{ge.value=!1}}async function zs(){J.value=!0;try{const j=await K.post("/api/ollama/reload");oe(j.configured?"Ollama reloaded":j.reason||"Ollama not configured",j.configured?"success":"error"),await Me()}catch(j){oe(j.message||"Reload failed","error")}finally{J.value=!1}}async function Ls(){U.value=!0;try{await K.post("/api/ollama/model",{model:ye.value}),oe("Model set to "+ye.value),await Me()}catch(j){oe(j.message||"Failed","error")}finally{U.value=!1}}async function Ft(){const j=N.value.base_url;if(!j){oe("Enter a base URL first","error");return}ne.value=!0;try{const ke=await K.post("/api/ollama/probe-models",{base_url:j});Q.value=ke.models||[],Q.value.length?(oe(Q.value.length+" model(s) found"),!N.value.model&&Q.value.length&&(N.value.model=Q.value[0].name)):oe("No models found at "+j,"error")}catch(ke){oe(ke.message||"Could not reach Ollama","error")}finally{ne.value=!1}}async function jt(){try{if(le.value=await K.get("/api/kimi/status"),re.value=!1,le.value.model&&(_e.value=le.value.model),le.value.configured)try{const j=await K.get("/api/kimi/models");Se.value=j.models||[]}catch{Se.value=[]}}catch{re.value=!0}}async function nn(){Ue.value=!0;try{const j=await K.post("/api/kimi/reload");oe(j.configured?"Kimi reloaded":j.reason||"Kimi not configured",j.configured?"success":"error"),await Me()}catch(j){oe(j.message||"Reload failed","error")}finally{Ue.value=!1}}async function js(){k.value=!0;try{await K.post("/api/kimi/model",{model:_e.value}),oe("Model set to "+_e.value),await Me()}catch(j){oe(j.message||"Failed","error")}finally{k.value=!1}}async function Vs(){if(D.value){Ye();return}D.value=!0;const j=zu(a.value);try{await K.put("/api/llm/codex/config",j),oe("Codex config saved"),await Promise.all([Qe({preserveBasic:!0,preserveAdvanced:!0}),Tt()])}catch(ke){oe(ke.message||"Failed","error");const Re=JSON.stringify(zu(a.value))!==JSON.stringify(j);await Promise.all([Qe({preserveBasic:Re,preserveAdvanced:!0}),Tt()])}finally{D.value=!1}}async function qs(){if(D.value)return;D.value=!0;const j=ju(a.value);try{await K.put("/api/llm/codex/config",j),JSON.stringify({context_budget_overrides:a.value.context_budget_overrides,context_utilization:a.value.context_utilization})===JSON.stringify({context_budget_overrides:j.context_budget_overrides,context_utilization:j.context_utilization})&&(T.value=!1),oe("Codex advanced settings saved"),await Promise.all([Qe({preserveBasic:!0,preserveAdvanced:!0}),Tt(),je()])}catch(ke){oe(ke.message||"Failed","error");const Re=JSON.stringify(ju(a.value))!==JSON.stringify(j);await Promise.all([Qe({preserveBasic:!0,preserveAdvanced:Re}),Tt(),je()])}finally{D.value=!1}}async function Ns(){if(R.value){ue();return}R.value=!0;try{const j=z.value?N.value.api_key:null,ke=_k(N.value,{includeApiKey:j!==null});await K.put("/api/llm/ollama/config",ke),oe("Ollama config saved"),j!==null&&N.value.api_key===j&&(N.value.api_key="",z.value=!1),await Promise.all([Qe({preserveBasic:!0,preserveAdvanced:!0}),pt()])}catch(j){oe(j.message||"Failed","error")}finally{R.value=!1}}async function Rn(){if(!R.value){R.value=!0;try{await K.put("/api/llm/ollama/config",wk(N.value)),oe("Ollama timeout saved"),await Promise.all([Qe({preserveBasic:!0,preserveAdvanced:!0}),pt()])}catch(j){oe(j.message||"Failed","error")}finally{R.value=!1}}}async function vt(){if(ee.value){Ne();return}ee.value=!0;try{const j=P.value?G.value.api_key:null,ke=kk(G.value,{includeApiKey:j!==null});await K.put("/api/llm/kimi/config",ke),oe("Kimi config saved"),j!==null&&G.value.api_key===j&&(G.value.api_key="",P.value=!1),await Promise.all([Qe({preserveBasic:!0,preserveAdvanced:!0}),jt()])}catch(j){oe(j.message||"Failed","error")}finally{ee.value=!1}}async function Ms(){if(!ee.value){ee.value=!0;try{await K.put("/api/llm/kimi/config",Sk(G.value)),oe("Kimi timeout saved"),await Promise.all([Qe({preserveBasic:!0,preserveAdvanced:!0}),jt()])}catch(j){oe(j.message||"Failed","error")}finally{ee.value=!1}}}async function qn(){if(w.value){q();return}w.value=!0;try{await K.put("/api/llm/auxiliary/config",m.value),oe("Auxiliary config saved"),await Qe()}catch(j){oe(j.message||"Failed","error"),await Qe()}finally{w.value=!1}}const Ye=Tl(Vs),q=Tl(qn),ue=Tl(Ns),Ne=Tl(vt),We=()=>(Ye.cancel(),Vs()),ot=()=>(ue.cancel(),Ns()),Ot=()=>(Ne.cancel(),vt()),Gn=()=>qs(),Qt=()=>Rn(),wa=()=>Ms();async function Ds(j){const ke=j.account_key+":"+j.model;S.value=ke;try{const Re=await K.post("/api/context/windows/clear",{account_key:j.account_key,model:j.model});oe(Re.cleared?"Temporary clamp cleared":"Clamp was already inactive"),await je()}catch(Re){oe(Re.message||"Failed to clear clamp","error"),await je()}finally{S.value=null}}async function oi(j){try{await K.post("/api/codex/account/"+j+"/activate"),oe("Active account switched"),await Tt()}catch(ke){oe(ke.message||"Failed","error")}}async function ka(j){Y.value=j;try{await K.post("/api/codex/account/"+j+"/refresh"),oe("Token refreshed"),await Tt()}catch(ke){oe(ke.message||"Refresh failed","error")}finally{Y.value=null}}function Kn(j,ke){X.value=j,me.value=ke||""}async function Sa(j){try{await K.put("/api/codex/account/"+j+"/label",{label:me.value}),oe("Label updated"),X.value=null,await Tt()}catch(ke){oe(ke.message||"Failed","error")}}async function Wn(j,ke){if(await Jt({title:"Delete Codex account",message:`Delete ${ke||"account #"+(j+1)}? The pool will reload without it.`,confirmLabel:"Delete",danger:!0}))try{await K.del("/api/codex/account/"+j),oe("Deleted. Pool reloaded."),await Tt()}catch(Xt){oe(Xt.message||"Failed","error")}}async function In(){de.value=!0;try{const j=await K.post("/api/codex/device-code");ae.value=j,ce.value="pending",ms(j)}catch(j){oe(j.message||"Failed","error")}finally{de.value=!1}}async function ms(j){xe={cancelled:!1};const ke=xe;try{const Re=await K.post("/api/codex/device-poll",{device_auth_id:j.device_auth_id,user_code:j.user_code,interval:j.interval});if(ke.cancelled)return;we.value=Re,ce.value="success",await Me()}catch(Re){if(ke.cancelled)return;ve.value=Re.message||"Device login failed",ce.value="error"}}function an(){xe&&(xe.cancelled=!0),ce.value=null,ae.value=null}return Ke(Me),_t(()=>{xe&&(xe.cancelled=!0),Ye.cancel(),q.cancel(),ue.cancel(),Ne.cancel()}),{loading:e,llmStatus:t,llmStatusLoadFailed:s,selectedProvider:n,switching:ge,advancedOpen:g,codexForm:a,codexModelOptions:l,codexAgentModelOptions:r,mainMaxAllowed:c,agentMaxAllowed:d,mainModelOptionDisabled:p,agentModelOptionDisabled:f,auxForm:m,auxData:b,auxModelOptions:E,onAuxModelChange:I,savingAux:w,saveAuxConfigDebounced:q,ollamaForm:N,kimiForm:G,savingCodex:D,savingOllama:R,savingKimi:ee,probingOllama:ne,ollamaKeyDirty:z,kimiKeyDirty:P,fetchCodexStatus:Tt,ollamaStatus:ie,ollamaStatusLoadFailed:be,ollamaModels:Q,ollamaSelectedModel:ye,reloading:J,settingModel:U,kimiStatus:le,kimiStatusLoadFailed:re,kimiModels:Se,kimiSelectedModel:_e,reloadingKimi:Ue,settingKimiModel:k,codexLoading:L,codexError:B,codexData:se,refreshing:Y,editingLabel:X,labelValue:me,contextWindows:_,contextWindowsLoading:C,contextWindowsError:v,contextBudgetRows:O,activeClampRows:$,activeContextBudget:F,clearingClamp:S,contextPolicyDirty:T,deviceState:ce,deviceLoading:de,deviceInfo:ae,deviceResult:we,deviceError:ve,fetchAll:Me,fetchLLMStatus:Qe,fetchOllamaStatus:pt,fetchKimiStatus:jt,switchProvider:Hs,reloadOllama:zs,setOllamaModel:Ls,reloadKimi:nn,setKimiModel:js,probeOllamaModels:Ft,saveCodexConfig:Vs,saveOllamaConfig:Ns,saveKimiConfig:vt,saveCodexAdvancedConfig:qs,saveOllamaAdvancedConfig:Rn,saveKimiAdvancedConfig:Ms,saveCodexConfigDebounced:Ye,saveOllamaConfigDebounced:ue,saveKimiConfigDebounced:Ne,saveCodexConfigNow:We,saveOllamaConfigNow:ot,saveKimiConfigNow:Ot,saveCodexAdvancedConfigNow:Gn,saveOllamaAdvancedConfigNow:Qt,saveKimiAdvancedConfigNow:wa,activateAccount:oi,refreshAccount:ka,startEditLabel:Kn,saveLabel:Sa,deleteAccount:Wn,startDeviceLogin:In,cancelDeviceLogin:an,formatSize:H,fetchContextWindows:je,clearContextClamp:Ds,setContextOverride:te,setContextUtilization:Oe,resetContextOverride:Le,overrideAboveFloor:it,formatCount:pe,formatContextCeiling:Te,formatExpiry:Ae,shortAccountKey:De,provenanceClass:ut}}},Vu={ok:"text-green-400",pass:"text-green-400",degraded:"text-yellow-400",warn:"text-yellow-400",down:"text-red-400",fail:"text-red-400",unconfigured:"text-gray-500",skipped:"text-gray-500"};function Ck(e){return Vu[e]||Vu[(e||"").toLowerCase()]||"text-gray-400"}const Ek={template:`
  ...
  `,setup(){const e=h(!0),t=h({}),s=h([]),n=h({}),a=h({}),i=h(null),l=h(null),r=h(null),o=h(null),c=h(null),d=W(()=>{var v;return Object.values(((v=i.value)==null?void 0:v.totals)||{}).reduce((S,T)=>S+Number(T||0),0)}),u=h(""),p=h(0),f=h([]),m=W(()=>f.value.map(v=>`${v.label} (${v.path}${v.reason?`: ${v.reason}`:""})`).join("; ")),b=Object.freeze([{key:"startup",label:"Startup diagnostics",path:"/api/startup/diagnostics"},{key:"subsystems",label:"Subsystem status",path:"/api/subsystems/status"},{key:"sshPool",label:"SSH pool",path:"/api/pools/ssh"},{key:"httpPool",label:"HTTP pool",path:"/api/pools/http"},{key:"riskStats",label:"Risk stats",path:"/api/risk/stats"},{key:"recoveryStats",label:"Recovery stats",path:"/api/recovery/stats"},{key:"compressionStats",label:"Compression stats",path:"/api/compression/stats"},{key:"freshnessStats",label:"Freshness stats",path:"/api/freshness/stats"},{key:"governorStats",label:"Governor stats",path:"/api/governor/stats"}]);let E=null;async function I(){var O;const v=await Promise.allSettled(b.map($=>K.get($.path))),S=$=>v[$].status==="fulfilled"?v[$].value:null;t.value=S(0)||{};const T=S(1);s.value=Array.isArray(T)?T:T&&T.subsystems||[],n.value=S(2)||{},a.value=S(3)||{},i.value=S(4),l.value=S(5),r.value=S(6),o.value=S(7),c.value=S(8);const y=v.filter($=>$.status==="rejected");if(f.value=v.flatMap(($,F)=>{var N;return $.status==="rejected"?[{...b[F],reason:((N=$.reason)==null?void 0:N.message)||"request failed"}]:[]}),p.value=f.value.length,y.length===v.length){const $=(O=y[0])==null?void 0:O.reason;u.value=($==null?void 0:$.message)||"Failed to load internals"}else u.value="";e.value=!1}function w(){e.value=!0,u.value="",I()}let g=!1;function _(){g||(g=!0,I(),E||(E=setInterval(I,3e4)))}function C(){g&&(g=!1,E&&(clearInterval(E),E=null))}return Ke(_),_s(_),ws(C),_t(C),{loading:e,error:u,failedCount:p,failedEndpoints:f,failedEndpointSummary:m,endpoints:b,retry:w,startup:t,subsystems:s,sshPool:n,httpPool:a,riskStats:i,riskTotal:d,recoveryStats:l,compressionStats:r,freshnessStats:o,governorStats:c,statusColor:Ck,formatAgeSeconds:aw}}},Ak={setup(){const e=h(""),t=h(""),s=h(!1),n=h(""),a=h(!1),i=h(!1),l=h(!1),r=h(null),o=h(!1);async function c(){a.value=!0,r.value=null,o.value=!1;try{const u=await K.get("/api/update/check");e.value=u.current||"",t.value=u.latest||"",s.value=u.update_available||!1,n.value=u.changelog||"",u.error&&(r.value=u.error),o.value=!0}catch(u){r.value=u.message}finally{a.value=!1}}async function d(){if(await Jt({title:"Update & restart",message:"Update Odin and restart? Active tasks will be interrupted.",confirmLabel:"Update & Restart",danger:!0})){i.value=!0,r.value=null;try{await K.post("/api/update/apply",{version:"latest"}),l.value=!0,setTimeout(()=>location.reload(),8e3)}catch(p){r.value=p.message}finally{i.value=!1}}}return Ke(c),{current:e,latest:t,updateAvailable:s,changelog:n,checking:a,applying:i,applied:l,error:r,checkDone:o,checkUpdate:c,applyUpdate:d}},template:`
  ...
  `},Ko={brand:"M12 3 4.5 8v8L12 21l7.5-5V8L12 3Zm0 4.2 4.6 3.1L12 16.8l-4.6-6.5L12 7.2Zm0 3.3v3.7",dashboard:"M4 13h6V4H4v9Zm0 7h6v-4H4v4Zm10 0h6v-9h-6v9Zm0-16v4h6V4h-6Z",chat:"M20 15a3 3 0 0 1-3 3H9l-5 3v-6a3 3 0 0 1-1-2.2V7a3 3 0 0 1 3-3h11a3 3 0 0 1 3 3v8Z",operations:"M5 12h3l2-6 4 12 2-6h3M4 4v16h16",history:"M4 12a8 8 0 1 0 2.3-5.7L4 8.5M4 4v4.5h4.5M12 7v5l3 2",home:"M3 11.5 12 4l9 7.5M5.5 10v10h13V10M9 20v-6h6v6",users:"M16 20v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2m7-10a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm13 10v-2a4 4 0 0 0-3-3.9m-2-11.8a4 4 0 0 1 0 7.7",capabilities:"M14.7 6.3a4 4 0 0 0-5.6 5.6L4 17v3h3v-2h2v-2h2l1.1-1.1a4 4 0 0 0 5.6-5.6l-3 3-3-3 3-3Z",personality:"M12 3a8 8 0 0 0-8 8c0 4 3 7 7 7v3h3v-3c3 0 6-3 6-7a8 8 0 0 0-8-8ZM8.5 10h.01M15.5 10h.01M9 14c1.7 1.2 4.3 1.2 6 0",system:"M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8Zm0-5v2m0 14v2M3 12h2m14 0h2M5.6 5.6 7 7m10 10 1.4 1.4M18.4 5.6 17 7M7 17l-1.4 1.4",menu:"M4 7h16M4 12h16M4 17h16",panelLeft:"M9 4H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h4V4Zm0 0h10a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H9M6 8h.01M6 12h.01",chevronLeft:"m15 18-6-6 6-6",chevronRight:"m9 18 6-6-6-6",chevronDown:"m6 9 6 6 6-6",chevronUp:"m18 15-6-6-6 6",search:"m21 21-4.3-4.3M19 11a8 8 0 1 1-16 0 8 8 0 0 1 16 0Z",logout:"M10 17l5-5-5-5m5 5H3m10-8h5a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-5",success:"m5 12 4 4L19 6",warning:"M12 3 2.8 20h18.4L12 3Zm0 6v4m0 3h.01",info:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0-8v4m0-8h.01",error:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm-3-12 6 6m0-6-6 6",edit:"M4 20h4l11-11-4-4L4 16v4Zm9-13 4 4",trash:"M4 7h16m-10 4v5m4-5v5M9 4h6l1 3H8l1-3Zm-3 3 1 13h10l1-13",brain:"M9 5a3 3 0 0 0-5 2.2A3.5 3.5 0 0 0 4 14a3 3 0 0 0 5 2.2V5Zm6 0a3 3 0 0 1 5 2.2 3.5 3.5 0 0 1 0 6.8 3 3 0 0 1-5 2.2V5ZM9 9H7m2 4H6m9-4h2m-2 4h3M12 4v16",refresh:"M20 6v5h-5M4 18v-5h5M18.5 10A7 7 0 0 0 6 7.5L4 11m16 2-2 3.5A7 7 0 0 1 5.5 14",close:"M6 6l12 12M18 6 6 18",command:"M7 8a3 3 0 1 1-3-3h3v14a3 3 0 1 1-3-3h13a3 3 0 1 1-3 3V5a3 3 0 1 1 3 3H7Z",external:"M14 4h6v6m0-6-9 9M19 13v6a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1h6",activity:"M4 12h4l2-5 4 10 2-5h4",shield:"M12 3 5 6v5c0 4.5 2.8 7.7 7 10 4.2-2.3 7-5.5 7-10V6l-7-3Z",database:"M20 6c0 1.7-3.6 3-8 3S4 7.7 4 6s3.6-3 8-3 8 1.3 8 3Zm0 0v6c0 1.7-3.6 3-8 3s-8-1.3-8-3V6m16 6v6c0 1.7-3.6 3-8 3s-8-1.3-8-3v-6",server:"M4 4h16v6H4V4Zm0 10h16v6H4v-6Zm3-7h.01M7 17h.01",terminal:"M5 7l4 4-4 4m6 1h8M3 4h18v16H3V4Z",wrench:"M14.7 6.3a4 4 0 0 0-5.6 5.6L4 17v3h3v-2h2v-2h2l1.1-1.1a4 4 0 0 0 5.6-5.6l-3 3-3-3 3-3Z",bot:"M8 4h8m-4-2v2M5 8h14a2 2 0 0 1 2 2v8H3v-8a2 2 0 0 1 2-2Zm3 4h.01M16 12h.01M8 16h8M3 13H1m22 0h-2",workflow:"M5 5h5v5H5V5Zm9 9h5v5h-5v-5ZM10 7.5h4a3 3 0 0 1 3 3V14M7.5 10v4a3 3 0 0 0 3 3H14",globe:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0-18c2.2 2.5 3.3 5.5 3.3 9S14.2 18.5 12 21m0-18C9.8 5.5 8.7 8.5 8.7 12s1.1 6.5 3.3 9M3 12h18",book:"M4 5a3 3 0 0 1 3-2h5v17H7a3 3 0 0 0-3 1V5Zm16 0a3 3 0 0 0-3-2h-5v17h5a3 3 0 0 1 3 1V5Z",message:"M4 4h16v13H8l-4 4V4Zm4 5h8m-8 4h5",puzzle:"M9 4h3a2 2 0 1 1 4 0h4v5a2 2 0 1 0 0 4v7h-7a2 2 0 1 1-4 0H4v-7a2 2 0 1 0 0-4V4h5",sparkles:"m12 3 1.2 3.8L17 8l-3.8 1.2L12 13l-1.2-3.8L7 8l3.8-1.2L12 3Zm6 10 .8 2.2L21 16l-2.2.8L18 19l-.8-2.2L15 16l2.2-.8L18 13ZM5 14l1 2.8L9 18l-3 1.2L5 22l-1-2.8L1 18l3-1.2L5 14Z",link:"M9.5 14.5 14.5 9m-7 8H6a4 4 0 0 1 0-8h3m6 0h3a4 4 0 0 1 0 8h-3",file:"M6 3h8l4 4v14H6V3Zm8 0v5h5M9 13h6m-6 4h6",folder:"M3 6h7l2 2h9v11H3V6Z",image:"M4 4h16v16H4V4Zm3 12 4-4 3 3 2-2 4 4M9 9h.01",attachment:"m8 12 5-5a3 3 0 1 1 4 4l-7 7a5 5 0 0 1-7-7l7-7",clock:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0-13v5l3 2",calendar:"M5 5h14v15H5V5Zm3-2v4m8-4v4M5 10h14",chart:"M4 20V10m5 10V4m5 16v-7m5 7V7M2 20h20",sliders:"M4 7h10m4 0h2M4 17h2m4 0h10M16 4v6M8 14v6",code:"m9 6-6 6 6 6m6-12 6 6-6 6",copy:"M8 8h11v12H8V8Zm-3 8H4V4h11v1",play:"m8 5 11 7-11 7V5Z",grid:"M4 4h6v6H4V4Zm10 0h6v6h-6V4ZM4 14h6v6H4v-6Zm10 0h6v6h-6v-6Z",list:"M9 6h11M9 12h11M9 18h11M4 6h.01M4 12h.01M4 18h.01",target:"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0-5a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm0-4h.01",rotate:"M20 6v5h-5M4 18v-5h5M18.5 10A7 7 0 0 0 6 7.5L4 11m16 2-2 3.5A7 7 0 0 1 5.5 14",archive:"M4 8h16v12H4V8Zm-1-4h18v4H3V4Zm6 8h6",flame:"M12 22c4 0 7-3 7-7 0-5-4-7-4-11-3 2-5 5-5 8-1-1-2-3-1-5-3 2-5 5-5 8 0 4 3 7 8 7Z",eye:"M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Zm10 3a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z",upload:"M12 16V4m-5 5 5-5 5 5M5 20h14",download:"M12 4v12m-5-5 5 5 5-5M5 20h14",undo:"M9 7 4 12l5 5m-5-5h10a6 6 0 0 1 6 6",redo:"m15 7 5 5-5 5m5-5H10a6 6 0 0 0-6 6",minus:"M5 12h14",plus:"M12 5v14M5 12h14",network:"M12 3v4m0 10v4M3 12h4m10 0h4M7.8 7.8l2.1 2.1m4.2 4.2 2.1 2.1m0-8.4-2.1 2.1m-4.2 4.2-2.1 2.1M12 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z",more:"M6 12h.01M12 12h.01M18 12h.01",pause:"M9 5v14m6-14v14",sort:"M8 5v14m0 0-3-3m3 3 3-3M16 19V5m0 0-3 3m3-3 3 3"};Object.freeze(Object.keys(Ko));const Nk={name:"OdinIcon",props:{name:{type:String,required:!0},size:{type:[Number,String],default:18},strokeWidth:{type:[Number,String],default:1.8}},setup(e,{attrs:t}){return()=>Za("svg",{...t,class:["odin-icon",t.class],width:e.size,height:e.size,viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":e.strokeWidth,"stroke-linecap":"round","stroke-linejoin":"round","aria-hidden":t["aria-label"]?void 0:"true",focusable:"false"},[Za("path",{d:Ko[e.name]||Ko.info})])}},Mk=["a[href]","button:not([disabled])",'input:not([disabled]):not([type="hidden"])',"select:not([disabled])","textarea:not([disabled])",'[tabindex]:not([tabindex="-1"])'].join(",");function Gu(e){return[...e.querySelectorAll(Mk)].filter(t=>!t.hasAttribute("hidden")&&t.getAttribute("aria-hidden")!=="true")}const Dk={mounted(e){const t=document.activeElement,s=n=>{if(n.key!=="Tab")return;const a=Gu(e);if(!a.length){n.preventDefault(),e.focus();return}const i=a[0],l=a[a.length-1];n.shiftKey&&document.activeElement===i?(n.preventDefault(),l.focus()):!n.shiftKey&&document.activeElement===l&&(n.preventDefault(),i.focus())};e.__odinModalFocus={previous:t,onKeydown:s},e.addEventListener("keydown",s),requestAnimationFrame(()=>{(e.querySelector("[autofocus]")||Gu(e)[0]||e).focus()})},unmounted(e){var s;const t=e.__odinModalFocus;t&&(e.removeEventListener("keydown",t.onKeydown),(s=t.previous)!=null&&s.isConnected&&typeof t.previous.focus=="function"&&requestAnimationFrame(()=>t.previous.focus()),delete e.__odinModalFocus)}},Pk={template:`
    ...
    </div>`,setup(){const e=h({}),t=h(!0),s=h(null),n=h([]),a=h(!1),i=h([]),l=h(!1),r=h(!1),o=h([]),c=h(0),d=h(null),u=h({reload:!1,clearSessions:!1,stopLoops:!1});let p=0;const f=W(()=>{const z=e.value.uptime_seconds||0,P=Math.floor(z/86400),D=Math.floor(z%86400/3600),R=Math.floor(z%3600/60),ee=[];return P>0&&ee.push(`${P}d`),D>0&&ee.push(`${D}h`),(ee.length===0||P===0&&D===0)&&ee.push(`${R}m`),ee.join(" ")}),m=W(()=>{const z=e.value.uptime_seconds||0;return 125.66*(1-Math.min(z/86400,1))}),b=W(()=>{const z=e.value;return[{label:"Guilds",value:z.guild_count??0,icon:"home",iconColor:"text-blue-400"},{label:"Sessions",value:z.session_count??0,icon:"message",iconColor:"text-yellow-400"},{label:"Tools",value:z.tool_count??0,icon:"wrench",iconColor:"text-purple-400",sub:`${z.skill_count??0} skills`,subColor:"text-gray-500"},{label:"Loops",value:z.loop_count??0,icon:"rotate",iconColor:"text-green-400",color:z.loop_count>0?"text-green-400":"",highlight:z.loop_count>0},{label:"Agents",value:z.agent_running??0,icon:"bot",iconColor:"text-cyan-400",sub:z.agent_count>0?`${z.agent_count} total`:"",subColor:"text-gray-500",highlight:(z.agent_running??0)>0},{label:"Processes",value:z.process_running??0,icon:"sliders",iconColor:"text-orange-400",sub:z.process_count>0?`${z.process_count} total`:"",subColor:"text-gray-500",highlight:(z.process_running??0)>0},{label:"Schedules",value:z.schedule_count??0,icon:"clock",iconColor:"text-amber-400",sub:(z.schedule_failing>0?`${z.schedule_failing} failing`:"")+(z.schedule_failing>0&&z.schedule_paused>0?", ":"")+(z.schedule_paused>0?`${z.schedule_paused} paused`:"")||void 0,subColor:z.schedule_failing>0?"text-red-400":"text-yellow-400",color:z.schedule_failing>0?"text-red-400":"",highlight:z.schedule_failing>0},{label:"Users",value:z.user_count??0,icon:"users",iconColor:"text-indigo-400"},...d.value!==null?[{label:"Knowledge",value:d.value,icon:"book",iconColor:"text-teal-400",sub:"chunks",subColor:"text-gray-500"}]:[]]}),E=W(()=>{const z=e.value,P=[];return P.push({label:"Bot",status:z.status==="online"?"ok":"warn",detail:z.status==="online"?"Online":"Starting"}),(z.schedule_failing||0)>0?P.push({label:"Schedules",status:"error",detail:`${z.schedule_failing} failing`}):(z.schedule_count||0)>0&&P.push({label:"Schedules",status:"ok",detail:`${z.schedule_count} configured`}),(z.loop_count||0)>0&&P.push({label:"Loops",status:"ok",detail:`${z.loop_count} active`}),(z.agent_running||0)>0&&P.push({label:"Agents",status:"ok",detail:`${z.agent_running} running`}),(z.process_running||0)>0&&P.push({label:"Processes",status:"ok",detail:`${z.process_running} running`}),P});async function I(){try{e.value=await K.get("/api/status"),s.value=null}catch(z){s.value=z.message}finally{t.value=!1}}async function w(){a.value=!0;try{n.value=await K.get("/api/audit?limit=10"),c.value=0}catch{}a.value=!1}async function g(){l.value=!0;try{i.value=await K.get("/api/audit?error_only=1&limit=5"),r.value=!1}catch{r.value=!0}l.value=!1}async function _(){try{const z=await K.get("/api/knowledge");d.value=(Array.isArray(z)?z:[]).reduce((P,D)=>P+(D.chunks||0),0)}catch{d.value=null}}async function C(){try{const z=await K.get("/api/agents");o.value=z.filter(P=>P.status==="running")}catch{}}async function v(){u.value={...u.value,reload:!0};try{await K.post("/api/reload"),Ie.success("Config reloaded")}catch(z){Ie.error(z.message)}u.value={...u.value,reload:!1}}async function S(){if(!await Jt({title:"Clear all sessions",message:"Clear all conversation sessions? This cannot be undone.",confirmLabel:"Clear All",danger:!0}))return;u.value={...u.value,clearSessions:!0};const P=e.value.session_count;e.value={...e.value,session_count:0};try{const D=await K.post("/api/sessions/clear-all");Ie.success(`Cleared ${D.count} session${D.count!==1?"s":""}`),await I()}catch(D){e.value={...e.value,session_count:P},Ie.error(D.message)}u.value={...u.value,clearSessions:!1}}async function T(){if(!await Jt({title:"Stop all loops",message:"Stop all running loops?",confirmLabel:"Stop Loops",danger:!0}))return;u.value={...u.value,stopLoops:!0};const P=e.value.loop_count;e.value={...e.value,loop_count:0};try{const D=await K.post("/api/loops/stop-all");Ie.success(D.result),await I()}catch(D){e.value={...e.value,loop_count:P},Ie.error(D.message)}u.value={...u.value,stopLoops:!1}}function y(){t.value=!0,s.value=null,I(),w(),g(),C()}let O=null,$=null,F=null;function N(z){if(z.payload&&z.payload.tool_name){const P={...z.payload,_isNew:!0,_key:++p};n.value.unshift(P),n.value.length>10&&n.value.pop(),c.value++,P.error&&(r.value=!1,i.value.unshift(P),i.value.length>5&&i.value.pop()),setTimeout(()=>{P._isNew=!1},1500),clearTimeout(F),F=setTimeout(()=>{c.value=0},1e4)}}let G=null;return Ke(async()=>{await Promise.all([I(),w(),g(),C(),_()]),O=setInterval(I,15e3),$=setInterval(C,1e4),Je.subscribe("events",N),G=Je.onReconnected(()=>{w(),g()})}),_t(()=>{O&&clearInterval(O),$&&clearInterval($),clearTimeout(F),Je.unsubscribe("events",N),G&&(G(),G=null)}),{status:e,loading:t,error:s,uptime:f,uptimeRingOffset:m,stats:b,healthIndicators:E,activity:n,activityLoading:a,newEventCount:c,errors:i,errorsLoading:l,errorsError:r,agents:o,actionLoading:u,fetchActivity:w,fetchErrors:g,fetchStatus:I,onEvent:N,formatTime:ym,formatDuration:ai,retry:y,reloadConfig:v,clearSessions:S,stopAllLoops:T}}};/*! @license DOMPurify 3.4.9 | (c) Cure53 and other contributors | Released under the Apache license 2.0 and Mozilla Public License 2.0 | github.com/cure53/DOMPurify/blob/3.4.9/LICENSE */function Ku(e,t){(t==null||t>e.length)&&(t=e.length);for(var s=0,n=Array(t);s<t;s++)n[s]=e[s];return n}function Fk(e){if(Array.isArray(e))return e}function $k(e,t){var s=e==null?null:typeof Symbol<"u"&&e[Symbol.iterator]||e["@@iterator"];if(s!=null){var n,a,i,l,r=[],o=!0,c=!1;try{if(i=(s=s.call(e)).next,t!==0)for(;!(o=(n=i.call(s)).done)&&(r.push(n.value),r.length!==t);o=!0);}catch(d){c=!0,a=d}finally{try{if(!o&&s.return!=null&&(l=s.return(),Object(l)!==l))return}finally{if(c)throw a}}return r}}function Bk(){throw new TypeError(`Invalid attempt to destructure non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`)}function Uk(e,t){return Fk(e)||$k(e,t)||Hk(e,t)||Bk()}function Hk(e,t){if(e){if(typeof e=="string")return Ku(e,t);var s={}.toString.call(e).slice(8,-1);return s==="Object"&&e.constructor&&(s=e.constructor.name),s==="Map"||s==="Set"?Array.from(e):s==="Arguments"||/^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(s)?Ku(e,t):void 0}}const Pm=Object.entries,Wu=Object.setPrototypeOf,zk=Object.isFrozen,jk=Object.getPrototypeOf,Vk=Object.getOwnPropertyDescriptor;let rs=Object.freeze,Os=Object.seal,Ma=Object.create,Fm=typeof Reflect<"u"&&Reflect,Wo=Fm.apply,Zo=Fm.construct;rs||(rs=function(t){return t});Os||(Os=function(t){return t});Wo||(Wo=function(t,s){for(var n=arguments.length,a=new Array(n>2?n-2:0),i=2;i<n;i++)a[i-2]=arguments[i];return t.apply(s,a)});Zo||(Zo=function(t){for(var s=arguments.length,n=new Array(s>1?s-1:0),a=1;a<s;a++)n[a-1]=arguments[a];return new t(...n)});const cn=It(Array.prototype.forEach),qk=It(Array.prototype.lastIndexOf),Zu=It(Array.prototype.pop),Ra=It(Array.prototype.push),Gk=It(Array.prototype.splice),ts=Array.isArray,Si=It(String.prototype.toLowerCase),po=It(String.prototype.toString),Ju=It(String.prototype.match),Ia=It(String.prototype.replace),Yu=It(String.prototype.indexOf),Kk=It(String.prototype.trim),Wk=It(Number.prototype.toString),Zk=It(Boolean.prototype.toString),Qu=typeof BigInt>"u"?null:It(BigInt.prototype.toString),Xu=typeof Symbol>"u"?null:It(Symbol.prototype.toString),bt=It(Object.prototype.hasOwnProperty),gi=It(Object.prototype.toString),$t=It(RegExp.prototype.test),ea=Jk(TypeError);function It(e){return function(t){t instanceof RegExp&&(t.lastIndex=0);for(var s=arguments.length,n=new Array(s>1?s-1:0),a=1;a<s;a++)n[a-1]=arguments[a];return Wo(e,t,n)}}function Jk(e){return function(){for(var t=arguments.length,s=new Array(t),n=0;n<t;n++)s[n]=arguments[n];return Zo(e,s)}}function ze(e,t){let s=arguments.length>2&&arguments[2]!==void 0?arguments[2]:Si;if(Wu&&Wu(e,null),!ts(t))return e;let n=t.length;for(;n--;){let a=t[n];if(typeof a=="string"){const i=s(a);i!==a&&(zk(t)||(t[n]=i),a=i)}e[a]=!0}return e}function Yk(e){for(let t=0;t<e.length;t++)bt(e,t)||(e[t]=null);return e}function qt(e){const t=Ma(null);for(const n of Pm(e)){var s=Uk(n,2);const a=s[0],i=s[1];bt(e,a)&&(ts(i)?t[a]=Yk(i):i&&typeof i=="object"&&i.constructor===Object?t[a]=qt(i):t[a]=i)}return t}function Qk(e){switch(typeof e){case"string":return e;case"number":return Wk(e);case"boolean":return Zk(e);case"bigint":return Qu?Qu(e):"0";case"symbol":return Xu?Xu(e):"Symbol()";case"undefined":return gi(e);case"function":case"object":{if(e===null)return gi(e);const t=e,s=Zs(t,"toString");if(typeof s=="function"){const n=s(t);return typeof n=="string"?n:gi(n)}return gi(e)}default:return gi(e)}}function Zs(e,t){for(;e!==null;){const n=Vk(e,t);if(n){if(n.get)return It(n.get);if(typeof n.value=="function")return It(n.value)}e=jk(e)}function s(){return null}return s}function Xk(e){try{return $t(e,""),!0}catch{return!1}}const ep=rs(["a","abbr","acronym","address","area","article","aside","audio","b","bdi","bdo","big","blink","blockquote","body","br","button","canvas","caption","center","cite","code","col","colgroup","content","data","datalist","dd","decorator","del","details","dfn","dialog","dir","div","dl","dt","element","em","fieldset","figcaption","figure","font","footer","form","h1","h2","h3","h4","h5","h6","head","header","hgroup","hr","html","i","img","input","ins","kbd","label","legend","li","main","map","mark","marquee","menu","menuitem","meter","nav","nobr","ol","optgroup","option","output","p","picture","pre","progress","q","rp","rt","ruby","s","samp","search","section","select","shadow","slot","small","source","spacer","span","strike","strong","style","sub","summary","sup","table","tbody","td","template","textarea","tfoot","th","thead","time","tr","track","tt","u","ul","var","video","wbr"]),fo=rs(["svg","a","altglyph","altglyphdef","altglyphitem","animatecolor","animatemotion","animatetransform","circle","clippath","defs","desc","ellipse","enterkeyhint","exportparts","filter","font","g","glyph","glyphref","hkern","image","inputmode","line","lineargradient","marker","mask","metadata","mpath","part","path","pattern","polygon","polyline","radialgradient","rect","stop","style","switch","symbol","text","textpath","title","tref","tspan","view","vkern"]),ho=rs(["feBlend","feColorMatrix","feComponentTransfer","feComposite","feConvolveMatrix","feDiffuseLighting","feDisplacementMap","feDistantLight","feDropShadow","feFlood","feFuncA","feFuncB","feFuncG","feFuncR","feGaussianBlur","feImage","feMerge","feMergeNode","feMorphology","feOffset","fePointLight","feSpecularLighting","feSpotLight","feTile","feTurbulence"]),eS=rs(["animate","color-profile","cursor","discard","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","foreignobject","hatch","hatchpath","mesh","meshgradient","meshpatch","meshrow","missing-glyph","script","set","solidcolor","unknown","use"]),mo=rs(["math","menclose","merror","mfenced","mfrac","mglyph","mi","mlabeledtr","mmultiscripts","mn","mo","mover","mpadded","mphantom","mroot","mrow","ms","mspace","msqrt","mstyle","msub","msup","msubsup","mtable","mtd","mtext","mtr","munder","munderover","mprescripts"]),tS=rs(["maction","maligngroup","malignmark","mlongdiv","mscarries","mscarry","msgroup","mstack","msline","msrow","semantics","annotation","annotation-xml","mprescripts","none"]),tp=rs(["#text"]),sp=rs(["accept","action","align","alt","autocapitalize","autocomplete","autopictureinpicture","autoplay","background","bgcolor","border","capture","cellpadding","cellspacing","checked","cite","class","clear","color","cols","colspan","command","commandfor","controls","controlslist","coords","crossorigin","datetime","decoding","default","dir","disabled","disablepictureinpicture","disableremoteplayback","download","draggable","enctype","enterkeyhint","exportparts","face","for","headers","height","hidden","high","href","hreflang","id","inert","inputmode","integrity","ismap","kind","label","lang","list","loading","loop","low","max","maxlength","media","method","min","minlength","multiple","muted","name","nonce","noshade","novalidate","nowrap","open","optimum","part","pattern","placeholder","playsinline","popover","popovertarget","popovertargetaction","poster","preload","pubdate","radiogroup","readonly","rel","required","rev","reversed","role","rows","rowspan","spellcheck","scope","selected","shape","size","sizes","slot","span","srclang","start","src","srcset","step","style","summary","tabindex","title","translate","type","usemap","valign","value","width","wrap","xmlns"]),vo=rs(["accent-height","accumulate","additive","alignment-baseline","amplitude","ascent","attributename","attributetype","azimuth","basefrequency","baseline-shift","begin","bias","by","class","clip","clippathunits","clip-path","clip-rule","color","color-interpolation","color-interpolation-filters","color-profile","color-rendering","cx","cy","d","dx","dy","diffuseconstant","direction","display","divisor","dur","edgemode","elevation","end","exponent","fill","fill-opacity","fill-rule","filter","filterunits","flood-color","flood-opacity","font-family","font-size","font-size-adjust","font-stretch","font-style","font-variant","font-weight","fx","fy","g1","g2","glyph-name","glyphref","gradientunits","gradienttransform","height","href","id","image-rendering","in","in2","intercept","k","k1","k2","k3","k4","kerning","keypoints","keysplines","keytimes","lang","lengthadjust","letter-spacing","kernelmatrix","kernelunitlength","lighting-color","local","marker-end","marker-mid","marker-start","markerheight","markerunits","markerwidth","maskcontentunits","maskunits","max","mask","mask-type","media","method","mode","min","name","numoctaves","offset","operator","opacity","order","orient","orientation","origin","overflow","paint-order","path","pathlength","patterncontentunits","patterntransform","patternunits","points","preservealpha","preserveaspectratio","primitiveunits","r","rx","ry","radius","refx","refy","repeatcount","repeatdur","restart","result","rotate","scale","seed","shape-rendering","slope","specularconstant","specularexponent","spreadmethod","startoffset","stddeviation","stitchtiles","stop-color","stop-opacity","stroke-dasharray","stroke-dashoffset","stroke-linecap","stroke-linejoin","stroke-miterlimit","stroke-opacity","stroke","stroke-width","style","surfacescale","systemlanguage","tabindex","tablevalues","targetx","targety","transform","transform-origin","text-anchor","text-decoration","text-rendering","textlength","type","u1","u2","unicode","values","viewbox","visibility","version","vert-adv-y","vert-origin-x","vert-origin-y","width","word-spacing","wrap","writing-mode","xchannelselector","ychannelselector","x","x1","x2","xmlns","y","y1","y2","z","zoomandpan"]),np=rs(["accent","accentunder","align","bevelled","close","columnalign","columnlines","columnspacing","columnspan","denomalign","depth","dir","display","displaystyle","encoding","fence","frame","height","href","id","largeop","length","linethickness","lquote","lspace","mathbackground","mathcolor","mathsize","mathvariant","maxsize","minsize","movablelimits","notation","numalign","open","rowalign","rowlines","rowspacing","rowspan","rspace","rquote","scriptlevel","scriptminsize","scriptsizemultiplier","selection","separator","separators","stretchy","subscriptshift","supscriptshift","symmetric","voffset","width","xmlns"]),El=rs(["xlink:href","xml:id","xlink:title","xml:space","xmlns:xlink"]),sS=Os(/{{[\w\W]*|^[\w\W]*}}/g),nS=Os(/<%[\w\W]*|^[\w\W]*%>/g),aS=Os(/\${[\w\W]*/g),iS=Os(/^data-[\-\w.\u00B7-\uFFFF]+$/),lS=Os(/^aria-[\-\w]+$/),ap=Os(/^(?:(?:(?:f|ht)tps?|mailto|tel|callto|sms|cid|xmpp|matrix):|[^a-z]|[a-z+.\-]+(?:[^a-z+.\-:]|$))/i),rS=Os(/^(?:\w+script|data):/i),oS=Os(/[\u0000-\u0020\u00A0\u1680\u180E\u2000-\u2029\u205F\u3000]/g),cS=Os(/^html$/i),dS=Os(/^[a-z][.\w]*(-[.\w]+)+$/i),Ks={element:1,attribute:2,text:3,cdataSection:4,entityReference:5,entityNode:6,progressingInstruction:7,comment:8,document:9,documentType:10,documentFragment:11,notation:12},uS=function(){return typeof window>"u"?null:window},pS=function(t,s){if(typeof t!="object"||typeof t.createPolicy!="function")return null;let n=null;const a="data-tt-policy-suffix";s&&s.hasAttribute(a)&&(n=s.getAttribute(a));const i="dompurify"+(n?"#"+n:"");try{return t.createPolicy(i,{createHTML(l){return l},createScriptURL(l){return l}})}catch{return console.warn("TrustedTypes policy "+i+" could not be created."),null}},ip=function(){return{afterSanitizeAttributes:[],afterSanitizeElements:[],afterSanitizeShadowDOM:[],beforeSanitizeAttributes:[],beforeSanitizeElements:[],beforeSanitizeShadowDOM:[],uponSanitizeAttribute:[],uponSanitizeElement:[],uponSanitizeShadowNode:[]}};function $m(){let e=arguments.length>0&&arguments[0]!==void 0?arguments[0]:uS();const t=Ce=>$m(Ce);if(t.version="3.4.9",t.removed=[],!e||!e.document||e.document.nodeType!==Ks.document||!e.Element)return t.isSupported=!1,t;let s=e.document;const n=s,a=n.currentScript;e.DocumentFragment;const i=e.HTMLTemplateElement,l=e.Node,r=e.Element,o=e.NodeFilter,c=e.NamedNodeMap;c===void 0&&(e.NamedNodeMap||e.MozNamedAttrMap),e.HTMLFormElement;const d=e.DOMParser,u=e.trustedTypes,p=r.prototype,f=Zs(p,"cloneNode"),m=Zs(p,"remove"),b=Zs(p,"nextSibling"),E=Zs(p,"childNodes"),I=Zs(p,"parentNode"),w=Zs(p,"shadowRoot"),g=Zs(p,"attributes"),_=l&&l.prototype?Zs(l.prototype,"nodeType"):null,C=l&&l.prototype?Zs(l.prototype,"nodeName"):null;if(typeof i=="function"){const Ce=s.createElement("template");Ce.content&&Ce.content.ownerDocument&&(s=Ce.content.ownerDocument)}let v,S="",T,y=!1,O=0;const $=function(){if(O>0)throw ea('A configured TRUSTED_TYPES_POLICY callback (createHTML or createScriptURL) must not call DOMPurify.sanitize, as that causes infinite recursion. Do not pass a policy whose callbacks wrap DOMPurify as TRUSTED_TYPES_POLICY; see the "DOMPurify and Trusted Types" section of the README.')},F=function(A){$(),O++;try{return v.createHTML(A)}finally{O--}},N=function(A){$(),O++;try{return v.createScriptURL(A)}finally{O--}},G=function(){return y||(T=pS(u,a),y=!0),T},z=s,P=z.implementation,D=z.createNodeIterator,R=z.createDocumentFragment,ee=z.getElementsByTagName,ne=n.importNode;let ge=ip();t.isSupported=typeof Pm=="function"&&typeof I=="function"&&P&&P.createHTMLDocument!==void 0;const ie=sS,be=nS,Q=aS,ye=iS,J=lS,U=rS,le=oS,re=dS;let Se=ap,_e=null;const Ue=ze({},[...ep,...fo,...ho,...mo,...tp]);let k=null;const L=ze({},[...sp,...vo,...np,...El]);let B=Object.seal(Ma(null,{tagNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeNameCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},allowCustomizedBuiltInElements:{writable:!0,configurable:!1,enumerable:!0,value:!1}})),se=null,Y=null;const X=Object.seal(Ma(null,{tagCheck:{writable:!0,configurable:!1,enumerable:!0,value:null},attributeCheck:{writable:!0,configurable:!1,enumerable:!0,value:null}}));let me=!0,ce=!0,de=!1,ae=!0,we=!1,ve=!0,xe=!1,oe=!1,H=!1,pe=!1,Te=!1,Ae=!1,De=!0,ut=!1;const it="user-content-";let te=!0,Oe=!1,Le={},Me=null;const Qe=ze({},["annotation-xml","audio","colgroup","desc","foreignobject","head","iframe","math","mi","mn","mo","ms","mtext","noembed","noframes","noscript","plaintext","script","selectedcontent","style","svg","template","thead","title","video","xmp"]);let je=null;const pt=ze({},["audio","video","img","source","image","track"]);let Tt=null;const Hs=ze({},["alt","class","for","id","label","name","pattern","placeholder","role","summary","title","value","style","xmlns"]),zs="http://www.w3.org/1998/Math/MathML",Ls="http://www.w3.org/2000/svg",Ft="http://www.w3.org/1999/xhtml";let jt=Ft,nn=!1,js=null;const Vs=ze({},[zs,Ls,Ft],po);let qs=ze({},["mi","mo","mn","ms","mtext"]),Ns=ze({},["annotation-xml"]);const Rn=ze({},["title","style","font","a","script"]);let vt=null;const Ms=["application/xhtml+xml","text/html"],qn="text/html";let Ye=null,q=null;const ue=s.createElement("form"),Ne=function(A){return A instanceof RegExp||A instanceof Function},We=function(){let A=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};if(q&&q===A)return;(!A||typeof A!="object")&&(A={}),A=qt(A),vt=Ms.indexOf(A.PARSER_MEDIA_TYPE)===-1?qn:A.PARSER_MEDIA_TYPE,Ye=vt==="application/xhtml+xml"?po:Si,_e=bt(A,"ALLOWED_TAGS")&&ts(A.ALLOWED_TAGS)?ze({},A.ALLOWED_TAGS,Ye):Ue,k=bt(A,"ALLOWED_ATTR")&&ts(A.ALLOWED_ATTR)?ze({},A.ALLOWED_ATTR,Ye):L,js=bt(A,"ALLOWED_NAMESPACES")&&ts(A.ALLOWED_NAMESPACES)?ze({},A.ALLOWED_NAMESPACES,po):Vs,Tt=bt(A,"ADD_URI_SAFE_ATTR")&&ts(A.ADD_URI_SAFE_ATTR)?ze(qt(Hs),A.ADD_URI_SAFE_ATTR,Ye):Hs,je=bt(A,"ADD_DATA_URI_TAGS")&&ts(A.ADD_DATA_URI_TAGS)?ze(qt(pt),A.ADD_DATA_URI_TAGS,Ye):pt,Me=bt(A,"FORBID_CONTENTS")&&ts(A.FORBID_CONTENTS)?ze({},A.FORBID_CONTENTS,Ye):Qe,se=bt(A,"FORBID_TAGS")&&ts(A.FORBID_TAGS)?ze({},A.FORBID_TAGS,Ye):qt({}),Y=bt(A,"FORBID_ATTR")&&ts(A.FORBID_ATTR)?ze({},A.FORBID_ATTR,Ye):qt({}),Le=bt(A,"USE_PROFILES")?A.USE_PROFILES&&typeof A.USE_PROFILES=="object"?qt(A.USE_PROFILES):A.USE_PROFILES:!1,me=A.ALLOW_ARIA_ATTR!==!1,ce=A.ALLOW_DATA_ATTR!==!1,de=A.ALLOW_UNKNOWN_PROTOCOLS||!1,ae=A.ALLOW_SELF_CLOSE_IN_ATTR!==!1,we=A.SAFE_FOR_TEMPLATES||!1,ve=A.SAFE_FOR_XML!==!1,xe=A.WHOLE_DOCUMENT||!1,pe=A.RETURN_DOM||!1,Te=A.RETURN_DOM_FRAGMENT||!1,Ae=A.RETURN_TRUSTED_TYPE||!1,H=A.FORCE_BODY||!1,De=A.SANITIZE_DOM!==!1,ut=A.SANITIZE_NAMED_PROPS||!1,te=A.KEEP_CONTENT!==!1,Oe=A.IN_PLACE||!1,Se=Xk(A.ALLOWED_URI_REGEXP)?A.ALLOWED_URI_REGEXP:ap,jt=typeof A.NAMESPACE=="string"?A.NAMESPACE:Ft,qs=bt(A,"MATHML_TEXT_INTEGRATION_POINTS")&&A.MATHML_TEXT_INTEGRATION_POINTS&&typeof A.MATHML_TEXT_INTEGRATION_POINTS=="object"?qt(A.MATHML_TEXT_INTEGRATION_POINTS):ze({},["mi","mo","mn","ms","mtext"]),Ns=bt(A,"HTML_INTEGRATION_POINTS")&&A.HTML_INTEGRATION_POINTS&&typeof A.HTML_INTEGRATION_POINTS=="object"?qt(A.HTML_INTEGRATION_POINTS):ze({},["annotation-xml"]);const Z=bt(A,"CUSTOM_ELEMENT_HANDLING")&&A.CUSTOM_ELEMENT_HANDLING&&typeof A.CUSTOM_ELEMENT_HANDLING=="object"?qt(A.CUSTOM_ELEMENT_HANDLING):Ma(null);if(B=Ma(null),bt(Z,"tagNameCheck")&&Ne(Z.tagNameCheck)&&(B.tagNameCheck=Z.tagNameCheck),bt(Z,"attributeNameCheck")&&Ne(Z.attributeNameCheck)&&(B.attributeNameCheck=Z.attributeNameCheck),bt(Z,"allowCustomizedBuiltInElements")&&typeof Z.allowCustomizedBuiltInElements=="boolean"&&(B.allowCustomizedBuiltInElements=Z.allowCustomizedBuiltInElements),we&&(ce=!1),Te&&(pe=!0),Le&&(_e=ze({},tp),k=Ma(null),Le.html===!0&&(ze(_e,ep),ze(k,sp)),Le.svg===!0&&(ze(_e,fo),ze(k,vo),ze(k,El)),Le.svgFilters===!0&&(ze(_e,ho),ze(k,vo),ze(k,El)),Le.mathMl===!0&&(ze(_e,mo),ze(k,np),ze(k,El))),X.tagCheck=null,X.attributeCheck=null,bt(A,"ADD_TAGS")&&(typeof A.ADD_TAGS=="function"?X.tagCheck=A.ADD_TAGS:ts(A.ADD_TAGS)&&(_e===Ue&&(_e=qt(_e)),ze(_e,A.ADD_TAGS,Ye))),bt(A,"ADD_ATTR")&&(typeof A.ADD_ATTR=="function"?X.attributeCheck=A.ADD_ATTR:ts(A.ADD_ATTR)&&(k===L&&(k=qt(k)),ze(k,A.ADD_ATTR,Ye))),bt(A,"ADD_URI_SAFE_ATTR")&&ts(A.ADD_URI_SAFE_ATTR)&&ze(Tt,A.ADD_URI_SAFE_ATTR,Ye),bt(A,"FORBID_CONTENTS")&&ts(A.FORBID_CONTENTS)&&(Me===Qe&&(Me=qt(Me)),ze(Me,A.FORBID_CONTENTS,Ye)),bt(A,"ADD_FORBID_CONTENTS")&&ts(A.ADD_FORBID_CONTENTS)&&(Me===Qe&&(Me=qt(Me)),ze(Me,A.ADD_FORBID_CONTENTS,Ye)),te&&(_e["#text"]=!0),xe&&ze(_e,["html","head","body"]),_e.table&&(ze(_e,["tbody"]),delete se.tbody),A.TRUSTED_TYPES_POLICY){if(typeof A.TRUSTED_TYPES_POLICY.createHTML!="function")throw ea('TRUSTED_TYPES_POLICY configuration option must provide a "createHTML" hook.');if(typeof A.TRUSTED_TYPES_POLICY.createScriptURL!="function")throw ea('TRUSTED_TYPES_POLICY configuration option must provide a "createScriptURL" hook.');const he=v;v=A.TRUSTED_TYPES_POLICY;try{S=F("")}catch(Pe){throw v=he,Pe}}else A.TRUSTED_TYPES_POLICY===null?(v=void 0,S=""):(v===void 0&&(v=G()),v&&typeof S=="string"&&(S=F("")));(ge.uponSanitizeElement.length>0||ge.uponSanitizeAttribute.length>0)&&_e===Ue&&(_e=qt(_e)),ge.uponSanitizeAttribute.length>0&&k===L&&(k=qt(k)),rs&&rs(A),q=A},ot=ze({},[...fo,...ho,...eS]),Ot=ze({},[...mo,...tS]),Gn=function(A){let Z=I(A);(!Z||!Z.tagName)&&(Z={namespaceURI:jt,tagName:"template"});const he=Si(A.tagName),Pe=Si(Z.tagName);return js[A.namespaceURI]?A.namespaceURI===Ls?Z.namespaceURI===Ft?he==="svg":Z.namespaceURI===zs?he==="svg"&&(Pe==="annotation-xml"||qs[Pe]):!!ot[he]:A.namespaceURI===zs?Z.namespaceURI===Ft?he==="math":Z.namespaceURI===Ls?he==="math"&&Ns[Pe]:!!Ot[he]:A.namespaceURI===Ft?Z.namespaceURI===Ls&&!Ns[Pe]||Z.namespaceURI===zs&&!qs[Pe]?!1:!Ot[he]&&(Rn[he]||!ot[he]):!!(vt==="application/xhtml+xml"&&js[A.namespaceURI]):!1},Qt=function(A){Ra(t.removed,{element:A});try{I(A).removeChild(A)}catch{if(m(A),!I(A))throw ea("a node selected for removal could not be detached from its tree and cannot be safely returned; refusing to sanitize in place")}},wa=function(A){const Z=E?E(A):A.childNodes;if(Z){const Pe=[];cn(Z,x=>{Ra(Pe,x)}),cn(Pe,x=>{try{m(x)}catch{}})}const he=g?g(A):null;if(he)for(let Pe=he.length-1;Pe>=0;--Pe){const x=he[Pe],M=x&&x.name;if(typeof M=="string")try{A.removeAttribute(M)}catch{}}},Ds=function(A,Z){try{Ra(t.removed,{attribute:Z.getAttributeNode(A),from:Z})}catch{Ra(t.removed,{attribute:null,from:Z})}if(Z.removeAttribute(A),A==="is")if(pe||Te)try{Qt(Z)}catch{}else try{Z.setAttribute(A,"")}catch{}},oi=function(A){const Z=g?g(A):A.attributes;if(Z)for(let he=Z.length-1;he>=0;--he){const Pe=Z[he],x=Pe&&Pe.name;if(!(typeof x!="string"||k[Ye(x)]))try{A.removeAttribute(x)}catch{}}},ka=function(A){const Z=[A];for(;Z.length>0;){const he=Z.pop();(_?_(he):he.nodeType)===Ks.element&&oi(he);const x=E?E(he):he.childNodes;if(x)for(let M=x.length-1;M>=0;--M)Z.push(x[M])}},Kn=function(A){let Z=null,he=null;if(H)A="<remove></remove>"+A;else{const M=Ju(A,/^[\r\n\t ]+/);he=M&&M[0]}vt==="application/xhtml+xml"&&jt===Ft&&(A='<html xmlns="http://www.w3.org/1999/xhtml"><head></head><body>'+A+"</body></html>");const Pe=v?F(A):A;if(jt===Ft)try{Z=new d().parseFromString(Pe,vt)}catch{}if(!Z||!Z.documentElement){Z=P.createDocument(jt,"template",null);try{Z.documentElement.innerHTML=nn?S:Pe}catch{}}const x=Z.body||Z.documentElement;return A&&he&&x.insertBefore(s.createTextNode(he),x.childNodes[0]||null),jt===Ft?ee.call(Z,xe?"html":"body")[0]:xe?Z.documentElement:x},Sa=function(A){return D.call(A.ownerDocument||A,A,o.SHOW_ELEMENT|o.SHOW_COMMENT|o.SHOW_TEXT|o.SHOW_PROCESSING_INSTRUCTION|o.SHOW_CDATA_SECTION,null)},Wn=function(A){var Z,he;A.normalize();const Pe=D.call(A.ownerDocument||A,A,o.SHOW_TEXT|o.SHOW_COMMENT|o.SHOW_CDATA_SECTION|o.SHOW_PROCESSING_INSTRUCTION,null);let x=Pe.nextNode();for(;x;){let V=x.data;cn([ie,be,Q],fe=>{V=Ia(V,fe," ")}),x.data=V,x=Pe.nextNode()}const M=(Z=(he=A.querySelectorAll)===null||he===void 0?void 0:he.call(A,"template"))!==null&&Z!==void 0?Z:[];cn(Array.from(M),V=>{ms(V.content)&&Wn(V.content)})},In=function(A){const Z=C?C(A):null;return typeof Z!="string"||Ye(Z)!=="form"?!1:typeof A.nodeName!="string"||typeof A.textContent!="string"||typeof A.removeChild!="function"||A.attributes!==g(A)||typeof A.removeAttribute!="function"||typeof A.setAttribute!="function"||typeof A.namespaceURI!="string"||typeof A.insertBefore!="function"||typeof A.hasChildNodes!="function"||A.nodeType!==_(A)||A.childNodes!==E(A)},ms=function(A){if(!_||typeof A!="object"||A===null)return!1;try{return _(A)===Ks.documentFragment}catch{return!1}},an=function(A){if(!_||typeof A!="object"||A===null)return!1;try{return typeof _(A)=="number"}catch{return!1}};function j(Ce,A,Z){cn(Ce,he=>{he.call(t,A,Z,q)})}const ke=function(A){let Z=null;if(j(ge.beforeSanitizeElements,A,null),In(A))return Qt(A),!0;const he=Ye(C?C(A):A.nodeName);if(j(ge.uponSanitizeElement,A,{tagName:he,allowedTags:_e}),ve&&A.hasChildNodes()&&!an(A.firstElementChild)&&$t(/<[/\w!]/g,A.innerHTML)&&$t(/<[/\w!]/g,A.textContent)||ve&&A.namespaceURI===Ft&&he==="style"&&an(A.firstElementChild)||A.nodeType===Ks.progressingInstruction||ve&&A.nodeType===Ks.comment&&$t(/<[/\w]/g,A.data))return Qt(A),!0;if(se[he]||!(X.tagCheck instanceof Function&&X.tagCheck(he))&&!_e[he]){if(!se[he]&&Zn(he)&&(B.tagNameCheck instanceof RegExp&&$t(B.tagNameCheck,he)||B.tagNameCheck instanceof Function&&B.tagNameCheck(he)))return!1;if(te&&!Me[he]){const x=I(A),M=E(A);if(M&&x){const V=M.length;for(let fe=V-1;fe>=0;--fe){const Fe=Oe?M[fe]:f(M[fe],!0);x.insertBefore(Fe,b(A))}}}return Qt(A),!0}return(_?_(A):A.nodeType)===Ks.element&&!Gn(A)||(he==="noscript"||he==="noembed"||he==="noframes")&&$t(/<\/no(script|embed|frames)/i,A.innerHTML)?(Qt(A),!0):(we&&A.nodeType===Ks.text&&(Z=A.textContent,cn([ie,be,Q],x=>{Z=Ia(Z,x," ")}),A.textContent!==Z&&(Ra(t.removed,{element:A.cloneNode()}),A.textContent=Z)),j(ge.afterSanitizeElements,A,null),!1)},Re=function(A,Z,he){if(Y[Z]||De&&(Z==="id"||Z==="name")&&(he in s||he in ue))return!1;const Pe=k[Z]||X.attributeCheck instanceof Function&&X.attributeCheck(Z,A);if(!(ce&&!Y[Z]&&$t(ye,Z))){if(!(me&&$t(J,Z))){if(!Pe||Y[Z]){if(!(Zn(A)&&(B.tagNameCheck instanceof RegExp&&$t(B.tagNameCheck,A)||B.tagNameCheck instanceof Function&&B.tagNameCheck(A))&&(B.attributeNameCheck instanceof RegExp&&$t(B.attributeNameCheck,Z)||B.attributeNameCheck instanceof Function&&B.attributeNameCheck(Z,A))||Z==="is"&&B.allowCustomizedBuiltInElements&&(B.tagNameCheck instanceof RegExp&&$t(B.tagNameCheck,he)||B.tagNameCheck instanceof Function&&B.tagNameCheck(he))))return!1}else if(!Tt[Z]){if(!$t(Se,Ia(he,le,""))){if(!((Z==="src"||Z==="xlink:href"||Z==="href")&&A!=="script"&&Yu(he,"data:")===0&&je[A])){if(!(de&&!$t(U,Ia(he,le,"")))){if(he)return!1}}}}}}return!0},Xt=ze({},["annotation-xml","color-profile","font-face","font-face-format","font-face-name","font-face-src","font-face-uri","missing-glyph"]),Zn=function(A){return!Xt[Si(A)]&&$t(re,A)},Jn=function(A){j(ge.beforeSanitizeAttributes,A,null);const Z=A.attributes;if(!Z||In(A))return;const he={attrName:"",attrValue:"",keepAttr:!0,allowedAttributes:k,forceKeepAttr:void 0};let Pe=Z.length;for(;Pe--;){const x=Z[Pe],M=x.name,V=x.namespaceURI,fe=x.value,Fe=Ye(M),gt=fe;let lt=M==="value"?gt:Kk(gt);if(he.attrName=Fe,he.attrValue=lt,he.keepAttr=!0,he.forceKeepAttr=void 0,j(ge.uponSanitizeAttribute,A,he),lt=he.attrValue,ut&&(Fe==="id"||Fe==="name")&&Yu(lt,it)!==0&&(Ds(M,A),lt=it+lt),ve&&$t(/((--!?|])>)|<\/(style|script|title|xmp|textarea|noscript|iframe|noembed|noframes)/i,lt)){Ds(M,A);continue}if(Fe==="attributename"&&Ju(lt,"href")){Ds(M,A);continue}if(he.forceKeepAttr)continue;if(!he.keepAttr){Ds(M,A);continue}if(!ae&&$t(/\/>/i,lt)){Ds(M,A);continue}we&&cn([ie,be,Q],di=>{lt=Ia(lt,di," ")});const Gs=Ye(A.nodeName);if(!Re(Gs,Fe,lt)){Ds(M,A);continue}if(v&&typeof u=="object"&&typeof u.getAttributeType=="function"&&!V)switch(u.getAttributeType(Gs,Fe)){case"TrustedHTML":{lt=F(lt);break}case"TrustedScriptURL":{lt=N(lt);break}}if(lt!==gt)try{V?A.setAttributeNS(V,M,lt):A.setAttribute(M,lt),In(A)?Qt(A):Zu(t.removed)}catch{Ds(M,A)}}j(ge.afterSanitizeAttributes,A,null)},Yn=function(A){let Z=null;const he=Sa(A);for(j(ge.beforeSanitizeShadowDOM,A,null);Z=he.nextNode();)if(j(ge.uponSanitizeShadowNode,Z,null),ke(Z),Jn(Z),ms(Z.content)&&Yn(Z.content),(_?_(Z):Z.nodeType)===Ks.element){const x=w?w(Z):Z.shadowRoot;ms(x)&&(ci(x),Yn(x))}j(ge.afterSanitizeShadowDOM,A,null)},ci=function(A){const Z=[{node:A,shadow:null}];for(;Z.length>0;){const he=Z.pop();if(he.shadow){Yn(he.shadow);continue}const Pe=he.node,M=(_?_(Pe):Pe.nodeType)===Ks.element,V=E?E(Pe):Pe.childNodes;if(V)for(let fe=V.length-1;fe>=0;--fe)Z.push({node:V[fe],shadow:null});if(M){const fe=C?C(Pe):null;if(typeof fe=="string"&&Ye(fe)==="template"){const Fe=Pe.content;ms(Fe)&&Z.push({node:Fe,shadow:null})}}if(M){const fe=w?w(Pe):Pe.shadowRoot;ms(fe)&&Z.push({node:null,shadow:fe},{node:fe,shadow:null})}}};return t.sanitize=function(Ce){let A=arguments.length>1&&arguments[1]!==void 0?arguments[1]:{},Z=null,he=null,Pe=null,x=null;if(nn=!Ce,nn&&(Ce="<!-->"),typeof Ce!="string"&&!an(Ce)&&(Ce=Qk(Ce),typeof Ce!="string"))throw ea("dirty is not a string, aborting");if(!t.isSupported)return Ce;oe||We(A),t.removed=[];const M=Oe&&typeof Ce!="string"&&an(Ce);if(M){const Fe=C?C(Ce):Ce.nodeName;if(typeof Fe=="string"){const gt=Ye(Fe);if(!_e[gt]||se[gt])throw ea("root node is forbidden and cannot be sanitized in-place")}if(In(Ce))throw ea("root node is clobbered and cannot be sanitized in-place");try{ci(Ce)}catch(gt){throw wa(Ce),gt}}else if(an(Ce))Z=Kn("<!---->"),he=Z.ownerDocument.importNode(Ce,!0),he.nodeType===Ks.element&&he.nodeName==="BODY"||he.nodeName==="HTML"?Z=he:Z.appendChild(he),ci(he);else{if(!pe&&!we&&!xe&&Ce.indexOf("<")===-1)return v&&Ae?F(Ce):Ce;if(Z=Kn(Ce),!Z)return pe?null:Ae?S:""}Z&&H&&Qt(Z.firstChild);const V=Sa(M?Ce:Z);try{for(;Pe=V.nextNode();)ke(Pe),Jn(Pe),ms(Pe.content)&&Yn(Pe.content)}catch(Fe){throw M&&wa(Ce),Fe}if(M)return cn(t.removed,Fe=>{Fe.element&&ka(Fe.element)}),we&&Wn(Ce),Ce;if(pe){if(we&&Wn(Z),Te)for(x=R.call(Z.ownerDocument);Z.firstChild;)x.appendChild(Z.firstChild);else x=Z;return(k.shadowroot||k.shadowrootmode)&&(x=ne.call(n,x,!0)),x}let fe=xe?Z.outerHTML:Z.innerHTML;return xe&&_e["!doctype"]&&Z.ownerDocument&&Z.ownerDocument.doctype&&Z.ownerDocument.doctype.name&&$t(cS,Z.ownerDocument.doctype.name)&&(fe="<!DOCTYPE "+Z.ownerDocument.doctype.name+`>
`+fe),we&&cn([ie,be,Q],Fe=>{fe=Ia(fe,Fe," ")}),v&&Ae?F(fe):fe},t.setConfig=function(){let Ce=arguments.length>0&&arguments[0]!==void 0?arguments[0]:{};We(Ce),oe=!0},t.clearConfig=function(){q=null,oe=!1,v=T,S=""},t.isValidAttribute=function(Ce,A,Z){q||We({});const he=Ye(Ce),Pe=Ye(A);return Re(he,Pe,Z)},t.addHook=function(Ce,A){typeof A=="function"&&Ra(ge[Ce],A)},t.removeHook=function(Ce,A){if(A!==void 0){const Z=qk(ge[Ce],A);return Z===-1?void 0:Gk(ge[Ce],Z,1)[0]}return Zu(ge[Ce])},t.removeHooks=function(Ce){ge[Ce]=[]},t.removeAllHooks=function(){ge=ip()},t}var lp=$m();function Xc(){return{async:!1,breaks:!1,extensions:null,gfm:!0,hooks:null,pedantic:!1,renderer:null,silent:!1,tokenizer:null,walkTokens:null}}var _a=Xc();function Bm(e){_a=e}var Mi={exec:()=>null};function at(e,t=""){let s=typeof e=="string"?e:e.source;const n={replace:(a,i)=>{let l=typeof i=="string"?i:i.source;return l=l.replace(as.caret,"$1"),s=s.replace(a,l),n},getRegex:()=>new RegExp(s,t)};return n}var as={codeRemoveIndent:/^(?: {1,4}| {0,3}\t)/gm,outputLinkReplace:/\\([\[\]])/g,indentCodeCompensation:/^(\s+)(?:```)/,beginningSpace:/^\s+/,endingHash:/#$/,startingSpaceChar:/^ /,endingSpaceChar:/ $/,nonSpaceChar:/[^ ]/,newLineCharGlobal:/\n/g,tabCharGlobal:/\t/g,multipleSpaceGlobal:/\s+/g,blankLine:/^[ \t]*$/,doubleBlankLine:/\n[ \t]*\n[ \t]*$/,blockquoteStart:/^ {0,3}>/,blockquoteSetextReplace:/\n {0,3}((?:=+|-+) *)(?=\n|$)/g,blockquoteSetextReplace2:/^ {0,3}>[ \t]?/gm,listReplaceTabs:/^\t+/,listReplaceNesting:/^ {1,4}(?=( {4})*[^ ])/g,listIsTask:/^\[[ xX]\] /,listReplaceTask:/^\[[ xX]\] +/,anyLine:/\n.*\n/,hrefBrackets:/^<(.*)>$/,tableDelimiter:/[:|]/,tableAlignChars:/^\||\| *$/g,tableRowBlankLine:/\n[ \t]*$/,tableAlignRight:/^ *-+: *$/,tableAlignCenter:/^ *:-+: *$/,tableAlignLeft:/^ *:-+ *$/,startATag:/^<a /i,endATag:/^<\/a>/i,startPreScriptTag:/^<(pre|code|kbd|script)(\s|>)/i,endPreScriptTag:/^<\/(pre|code|kbd|script)(\s|>)/i,startAngleBracket:/^</,endAngleBracket:/>$/,pedanticHrefTitle:/^([^'"]*[^\s])\s+(['"])(.*)\2/,unicodeAlphaNumeric:/[\p{L}\p{N}]/u,escapeTest:/[&<>"']/,escapeReplace:/[&<>"']/g,escapeTestNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/,escapeReplaceNoEncode:/[<>"']|&(?!(#\d{1,7}|#[Xx][a-fA-F0-9]{1,6}|\w+);)/g,unescapeTest:/&(#(?:\d+)|(?:#x[0-9A-Fa-f]+)|(?:\w+));?/ig,caret:/(^|[^\[])\^/g,percentDecode:/%25/g,findPipe:/\|/g,splitPipe:/ \|/,slashPipe:/\\\|/g,carriageReturn:/\r\n|\r/g,spaceLine:/^ +$/gm,notSpaceStart:/^\S*/,endingNewline:/\n$/,listItemRegex:e=>new RegExp(`^( {0,3}${e})((?:[	 ][^\\n]*)?(?:\\n|$))`),nextBulletRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),hrRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),fencesBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}(?:\`\`\`|~~~)`),headingBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}#`),htmlBeginRegex:e=>new RegExp(`^ {0,${Math.min(3,e-1)}}<(?:[a-z].*>|!--)`,"i")},fS=/^(?:[ \t]*(?:\n|$))+/,hS=/^((?: {4}| {0,3}\t)[^\n]+(?:\n(?:[ \t]*(?:\n|$))*)?)+/,mS=/^ {0,3}(`{3,}(?=[^`\n]*(?:\n|$))|~{3,})([^\n]*)(?:\n|$)(?:|([\s\S]*?)(?:\n|$))(?: {0,3}\1[~`]* *(?=\n|$)|$)/,dl=/^ {0,3}((?:-[\t ]*){3,}|(?:_[ \t]*){3,}|(?:\*[ \t]*){3,})(?:\n+|$)/,vS=/^ {0,3}(#{1,6})(?=\s|$)(.*)(?:\n+|$)/,ed=/(?:[*+-]|\d{1,9}[.)])/,Um=/^(?!bull |blockCode|fences|blockquote|heading|html|table)((?:.|\n(?!\s*?\n|bull |blockCode|fences|blockquote|heading|html|table))+?)\n {0,3}(=+|-+) *(?:\n+|$)/,Hm=at(Um).replace(/bull/g,ed).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/\|table/g,"").getRegex(),gS=at(Um).replace(/bull/g,ed).replace(/blockCode/g,/(?: {4}| {0,3}\t)/).replace(/fences/g,/ {0,3}(?:`{3,}|~{3,})/).replace(/blockquote/g,/ {0,3}>/).replace(/heading/g,/ {0,3}#{1,6}/).replace(/html/g,/ {0,3}<[^\n>]+>\n/).replace(/table/g,/ {0,3}\|?(?:[:\- ]*\|)+[\:\- ]*\n/).getRegex(),td=/^([^\n]+(?:\n(?!hr|heading|lheading|blockquote|fences|list|html|table| +\n)[^\n]+)*)/,bS=/^[^\n]+/,sd=/(?!\s*\])(?:\\.|[^\[\]\\])+/,yS=at(/^ {0,3}\[(label)\]: *(?:\n[ \t]*)?([^<\s][^\s]*|<.*?>)(?:(?: +(?:\n[ \t]*)?| *\n[ \t]*)(title))? *(?:\n+|$)/).replace("label",sd).replace("title",/(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/).getRegex(),xS=at(/^( {0,3}bull)([ \t][^\n]+?)?(?:\n|$)/).replace(/bull/g,ed).getRegex(),Ur="address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|meta|nav|noframes|ol|optgroup|option|p|param|search|section|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul",nd=/<!--(?:-?>|[\s\S]*?(?:-->|$))/,_S=at("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n[ 	]*)+\\n|$))","i").replace("comment",nd).replace("tag",Ur).replace("attribute",/ +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex(),zm=at(td).replace("hr",dl).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("|table","").replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Ur).getRegex(),wS=at(/^( {0,3}> ?(paragraph|[^\n]*)(?:\n|$))+/).replace("paragraph",zm).getRegex(),ad={blockquote:wS,code:hS,def:yS,fences:mS,heading:vS,hr:dl,html:_S,lheading:Hm,list:xS,newline:fS,paragraph:zm,table:Mi,text:bS},rp=at("^ *([^\\n ].*)\\n {0,3}((?:\\| *)?:?-+:? *(?:\\| *:?-+:? *)*(?:\\| *)?)(?:\\n((?:(?! *\\n|hr|heading|blockquote|code|fences|list|html).*(?:\\n|$))*)\\n*|$)").replace("hr",dl).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("blockquote"," {0,3}>").replace("code","(?: {4}| {0,3}	)[^\\n]").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Ur).getRegex(),kS={...ad,lheading:gS,table:rp,paragraph:at(td).replace("hr",dl).replace("heading"," {0,3}#{1,6}(?:\\s|$)").replace("|lheading","").replace("table",rp).replace("blockquote"," {0,3}>").replace("fences"," {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list"," {0,3}(?:[*+-]|1[.)]) ").replace("html","</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag",Ur).getRegex()},SS={...ad,html:at(`^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:"[^"]*"|'[^']*'|\\s[^'"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))`).replace("comment",nd).replace(/tag/g,"(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:|[^\\w\\s@]*@)\\b").getRegex(),def:/^ *\[([^\]]+)\]: *<?([^\s>]+)>?(?: +(["(][^\n]+[")]))? *(?:\n+|$)/,heading:/^(#{1,6})(.*)(?:\n+|$)/,fences:Mi,lheading:/^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,paragraph:at(td).replace("hr",dl).replace("heading",` *#{1,6} *[^
...
`,e=e.substring(p.length+1),o=!0),!o){const I=this.rules.other.nextBulletRegex(m),w=this.rules.other.hrRegex(m),g=this.rules.other.fencesBeginRegex(m),_=this.rules.other.headingBeginRegex(m),C=this.rules.other.htmlBeginRegex(m);for(;e;){const v=e.split(`
`,1)[0];let S;if(p=v,this.options.pedantic?(p=p.replace(this.rules.other.listReplaceNesting,"  "),S=p):S=p.replace(this.rules.other.tabCharGlobal,"    "),g.test(p)||_.test(p)||C.test(p)||I.test(p)||w.test(p))break;if(S.search(this.rules.other.nonSpaceChar)>=m||!p.trim())d+=`
`+S.slice(m);else{if(f||u.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||g.test(u)||_.test(u)||w.test(u))break;d+=`
`+p}!f&&!p.trim()&&(f=!0),c+=v+`
...
Please report this to https://github.com/markedjs/marked.`,e){const n="<p>An error occurred:</p><pre>"+Js(s.message+"",!0)+"</pre>";return t?Promise.resolve(n):n}if(t)return Promise.reject(s);throw s}}},ma=new WS;function st(e,t){return ma.parse(e,t)}st.options=st.setOptions=function(e){return ma.setOptions(e),st.defaults=ma.defaults,Bm(st.defaults),st};st.getDefaults=Xc;st.defaults=_a;st.use=function(...e){return ma.use(...e),st.defaults=ma.defaults,Bm(st.defaults),st};st.walkTokens=function(e,t){return ma.walkTokens(e,t)};st.parseInline=ma.parseInline;st.Parser=bn;st.parser=bn.parse;st.Renderer=ur;st.TextRenderer=rd;st.Lexer=gn;st.lexer=gn.lex;st.Tokenizer=dr;st.Hooks=Dl;st.parse=st;st.options;st.setOptions;st.use;st.walkTokens;st.parseInline;bn.parse;gn.lex;const ZS={breaks:!0,gfm:!0};function pp(e){if(!e)return"";try{if(typeof st<"u"&&st.parse){const t=st.parse(e,ZS);return typeof lp<"u"?lp.sanitize(t):t}}catch{}return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\n/g,"<br>")}function JS(e){const t=new Date(e),s=t.getHours().toString().padStart(2,"0"),n=t.getMinutes().toString().padStart(2,"0");return`${s}:${n}`}const YS={run_command:"terminal",ssh_command:"terminal",run_script:"terminal",read_file:"file",write_file:"edit",list_directory:"folder",search_knowledge:"search",ingest_document:"book",generate_image:"image",analyze_image:"eye",analyze_pdf:"file",browser_screenshot:"globe",manage_process:"sliders"};function QS(e){return YS[e]||"wrench"}const XS=/https?:\/\/\S+\.(?:png|jpg|jpeg|gif|webp|svg)(?:\?\S*)?/gi;function fp(e){if(!e)return[];const t=e.match(XS);return t?[...new Set(t)]:[]}const e1={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(""),s=h(!1),n=h(""),a=h(null),i=h(null),l=h(0),r=h("");let o=null,c=0;const d=["Check system health","List running services","Show disk usage","What can you do?"],u=W(()=>t.value.trim().length>0&&!s.value),p=h(Je.state||"disconnected");let f=null;const m=W(()=>{const P=p.value;return P==="connected"?"Connected":P==="reconnecting"?"Reconnecting…":P==="connecting"?"Connecting…":"REST fallback"}),b=["Watching across all realms...","Processing...","Consulting the bifrost...","Observing..."],E=W(()=>{const P=Math.floor(l.value/4)%b.length,D=l.value;return D>3?`${b[P]} (${D}s)`:b[0]});function I(){Et(()=>{a.value&&(a.value.scrollTop=a.value.scrollHeight)})}function w(){if(!i.value)return;const P=i.value;P.style.height="auto",P.style.height=Math.min(P.scrollHeight,120)+"px"}function g(P,D,R={}){const ee={id:++c,role:P,content:D,timestamp:Date.now(),html:P==="bot"?pp(D):"",tools_used:R.tools_used||[],is_error:R.is_error||!1,images:P==="bot"?fp(D):[],files:R.files||[],_showTools:!1};return e.value.push(ee),I(),P==="bot"&&Et(()=>_()),ee}function _(){if(!a.value)return;a.value.querySelectorAll(".chat-markdown pre:not([data-copy])").forEach(D=>{D.setAttribute("data-copy","true"),D.style.position="relative";const R=document.createElement("button");R.className="chat-code-copy",R.textContent="Copy",R.addEventListener("click",()=>{const ee=D.querySelector("code"),ne=ee?ee.textContent:D.textContent;navigator.clipboard.writeText(ne).then(()=>{R.textContent="Copied!",setTimeout(()=>{R.textContent="Copy"},1500)}).catch(()=>{})}),D.appendChild(R)})}function C(P){if(P===0)return!0;const D=e.value[P-1],R=e.value[P],ee=new Date(D.timestamp).toDateString(),ne=new Date(R.timestamp).toDateString();return ee!==ne}function v(P){const D=new Date(P),R=new Date;if(D.toDateString()===R.toDateString())return"Today";const ee=new Date(R);return ee.setDate(ee.getDate()-1),D.toDateString()===ee.toDateString()?"Yesterday":D.toLocaleDateString(void 0,{month:"short",day:"numeric",year:"numeric"})}function S(P){t.value=P,Et(()=>G())}function T(P){window.open(P,"_blank","noopener")}function y(P){P.target.style.display="none"}function O(){l.value=0,o=setInterval(()=>{l.value++},1e3)}function $(){o&&(clearInterval(o),o=null),l.value=0}function F(P){s.value&&(s.value=!1,$(),P.type==="chat_response"?g("bot",P.content,{tools_used:P.tools_used||[],is_error:P.is_error||!1,files:P.files||[]}):P.type==="chat_error"&&g("bot",P.error||"Unknown error",{is_error:!0}),Et(()=>{var D;return(D=i.value)==null?void 0:D.focus()}))}async function N(P){try{const D=await K.post("/api/chat",{content:P,channel_id:r.value});g("bot",D.response,{tools_used:D.tools_used||[],is_error:D.is_error||!1,files:D.files||[]})}catch(D){g("bot",D.message||"Failed to send message",{is_error:!0})}}async function G(){const P=t.value.trim();if(!P||s.value)return;g("user",P),t.value="",s.value=!0,O(),i.value&&(i.value.style.height="auto"),Je.connected&&Je.sendChat(P,{channelId:r.value})||(await N(P),s.value=!1,$()),Et(()=>{var R;return(R=i.value)==null?void 0:R.focus()})}async function z(){n.value="";try{if(!r.value){const D=await K.get("/api/auth/session");r.value=D.channel_id||D.user_id||"web-user"}const P=await K.get("/api/sessions/"+encodeURIComponent(r.value));if(P&&P.messages&&P.messages.length>0){for(const D of P.messages){const R=D.role==="user"?"user":"bot";let ee=D.content||"";if(R==="user"){const ge=ee.match(/^\[.*?\]:\s*/);ge&&(ee=ee.slice(ge[0].length))}if(!ee.trim())continue;const ne={id:++c,role:R,content:ee,timestamp:D.timestamp?D.timestamp*1e3:Date.now(),html:R==="bot"?pp(ee):"",tools_used:[],is_error:!1,images:R==="bot"?fp(ee):[],files:[],_showTools:!1};e.value.push(ne)}Et(()=>{I(),_()})}}catch(P){P&&P.status!==404&&(n.value="Couldn't load chat history — earlier messages may be missing. Refresh to retry.",Ie.error(n.value))}}return Ke(()=>{Je.subscribe("chat",F),p.value=Je.state||"disconnected",f=Je.onState(P=>{p.value=P}),z(),Et(()=>{var P;return(P=i.value)==null?void 0:P.focus()})}),_t(()=>{Je.unsubscribe("chat",F),f&&(f(),f=null),$()}),{messages:e,input:t,sending:s,historyError:n,messagesEl:a,inputEl:i,canSend:u,wsStatus:m,typingText:E,suggestions:d,send:G,autoResize:w,formatTime:JS,formatDate:v,showDateSeparator:C,useSuggestion:S,openImage:T,onImageError:y,getToolIcon:QS,loadHistory:z}}},t1={setup(){const e=h("odin"),t=h(""),s=h(""),n=h(""),a=h({}),i=h([]),l=h([]),r=h(!1),o=h(!1),c=h(null),d=h(!0),u=h(""),p=h(!1),f=h(!1),m=W(()=>e.value==="custom"),b=W(()=>[...i.value,...l.value]),E=W(()=>l.value.includes(e.value)),I=W(()=>{var T;return m.value?t.value||"Odin":((T=a.value[e.value])==null?void 0:T.name)||e.value}),w=W(()=>{var T;return m.value?s.value||"(empty — will use Odin default)":((T=a.value[e.value])==null?void 0:T.identity)||""}),g=W(()=>{var T;return m.value?n.value||"(empty — will use Odin default)":((T=a.value[e.value])==null?void 0:T.voice)||""});async function _(){d.value=!0;try{const T=await K.get("/api/personality");e.value=T.preset||"odin",t.value=T.custom_name||"",s.value=T.custom_identity||"",n.value=T.custom_voice||"",a.value=T.presets||{},i.value=T.builtin_presets||[],l.value=T.user_presets||[]}catch(T){c.value=T.message}finally{d.value=!1}}async function C(){r.value=!0,c.value=null,o.value=!1;try{await K.put("/api/personality",{preset:e.value,custom_name:t.value,custom_identity:s.value,custom_voice:n.value}),o.value=!0,setTimeout(()=>o.value=!1,3e3)}catch(T){c.value=T.message}finally{r.value=!1}}async function v(){const T=u.value.trim();if(T){f.value=!0,c.value=null;try{await K.post("/api/personality/presets",{name:T,display_name:I.value,identity:w.value,voice:g.value}),p.value=!1,u.value="",await _(),e.value=T.toLowerCase().replace(/ /g,"_")}catch(y){c.value=y.message}finally{f.value=!1}}}async function S(){if(await Jt({title:"Delete preset",message:`Delete preset "${e.value}"? This cannot be undone.`,confirmLabel:"Delete",danger:!0})){c.value=null;try{await K.del(`/api/personality/presets/${encodeURIComponent(e.value)}`),await _(),e.value="odin"}catch(y){c.value=y.message}}}return Ke(_),{preset:e,customName:t,customIdentity:s,customVoice:n,presets:a,presetNames:b,isCustom:m,isUserPreset:E,previewName:I,previewIdentity:w,previewVoice:g,saving:r,saved:o,error:c,loading:d,save:C,showSavePreset:p,newPresetName:u,savingPreset:f,saveAsPreset:v,deletePreset:S,builtinPresets:i,userPresets:l}},template:`
  ...
  `},wt=(e,t)=>s=>({path:e,query:{...s.query,tab:t}}),Jm=[{path:"/",redirect:"/dashboard"},{path:"/dashboard",component:Pk,meta:{label:"Dashboard",icon:"dashboard",section:"Workspace",description:"System posture and recent activity"}},{path:"/chat",component:e1,meta:{label:"Chat",icon:"chat",section:"Workspace",description:"Direct operator conversation"}},{path:"/operations",component:vw,meta:{label:"Operations",icon:"operations",section:"Operate",description:"Execution, agents, loops, processes, and schedules"}},{path:"/history",component:kw,meta:{label:"History",icon:"history",section:"Observe",description:"Audit trail, sessions, traces, and usage"}},{path:"/capabilities",component:Vw,meta:{label:"Capabilities",icon:"capabilities",section:"Manage",description:"Tools, skills, knowledge, and memory"}},{path:"/personality",component:t1,meta:{label:"Personality",icon:"personality",section:"Manage",description:"Behavior and response profile"}},{path:"/system",component:Rk,meta:{label:"System",icon:"system",section:"Manage",description:"Health, configuration, access, and updates"}},{path:"/execution",redirect:wt("/operations","live")},{path:"/agents",redirect:wt("/operations","agents")},{path:"/loops",redirect:wt("/operations","loops")},{path:"/processes",redirect:wt("/operations","processes")},{path:"/schedules",redirect:wt("/operations","schedules")},{path:"/audit",redirect:wt("/history","audit")},{path:"/sessions",redirect:wt("/history","sessions")},{path:"/traces",redirect:wt("/history","traces")},{path:"/usage",redirect:wt("/history","usage")},{path:"/tools",redirect:wt("/capabilities","tools")},{path:"/skills",redirect:wt("/capabilities","skills")},{path:"/mcp",redirect:wt("/capabilities","mcp-servers")},{path:"/knowledge",redirect:wt("/capabilities","knowledge")},{path:"/memory",redirect:wt("/capabilities","memory")},{path:"/learned",redirect:wt("/capabilities","learned")},{path:"/health",redirect:wt("/system","health")},{path:"/resources",redirect:wt("/system","resources")},{path:"/logs",redirect:wt("/system","logs")},{path:"/config",redirect:wt("/system","config")},{path:"/host-access",redirect:wt("/system","host-access")},{path:"/internals",redirect:wt("/system","internals")}],Di=tw({history:N_(),routes:Jm});Di.afterEach(e=>{var s;const t=(s=e.meta)==null?void 0:s.label;document.title=t?`Odin — ${t}`:"Odin — Management"});const s1={template:`
    ...
    <toast-container />
    <confirm-host />
    <command-palette />`,setup(){const e=h("checking"),t=h(!1),s=h(!1),n=h(!1),a=h(null),i=h(null),l=h(!1);let r=null,o=null;const c=h(!1),d=h("disconnected"),u=h(-1),p=h(null);let f=null;const m=h("starting"),b=h(""),E=Jm.filter(R=>R.meta),I=W(()=>["Workspace","Operate","Observe","Manage"].map(R=>({name:R,routes:E.filter(ee=>ee.meta.section===R)})).filter(R=>R.routes.length)),w=W(()=>{var R;return((R=Di.currentRoute.value.meta)==null?void 0:R.label)||"Odin"}),g=W(()=>{var R;return((R=Di.currentRoute.value.meta)==null?void 0:R.section)||"Management"}),_=W(()=>{var R;return((R=Di.currentRoute.value.meta)==null?void 0:R.description)||"Management console"});K.onSessionExpired=()=>{t.value=!0,Je.disconnect(),K.setToken(""),e.value="login"};function C(R){var ee;if((R.ctrlKey||R.metaKey)&&R.key.toLowerCase()==="k"){e.value==="ready"&&(R.preventDefault(),qu());return}if(n.value&&R.key==="Tab"){const ne=[...((ee=a.value)==null?void 0:ee.querySelectorAll('a[href], button:not([disabled]), [tabindex]:not([tabindex="-1"])'))||[]];if(ne.length){const ge=ne[0],ie=ne[ne.length-1];if(R.shiftKey&&(document.activeElement===ge||!a.value.contains(document.activeElement))){R.preventDefault(),ie.focus();return}if(!R.shiftKey&&(document.activeElement===ie||!a.value.contains(document.activeElement))){R.preventDefault(),ge.focus();return}}}if(R.key==="Escape"&&n.value){n.value=!1,R.preventDefault();return}if(R.key==="/"&&!["INPUT","TEXTAREA","SELECT"].includes(R.target.tagName)){R.preventDefault();const ne=document.querySelector('.hm-main input[type="text"], .hm-main .hm-input:not(textarea):not(select)');ne&&ne.focus()}}function v(){l.value=!!(r!=null&&r.matches),l.value||(n.value=!1)}Ke(async()=>{document.addEventListener("keydown",C),r=window.matchMedia("(max-width: 900px)"),v(),r.addEventListener("change",v);const R=await K.check();R.ok?(e.value="ready",P()):R.needsAuth?e.value="login":(e.value="ready",P())});function S(){t.value=!1,e.value="ready",P()}async function T(){await K.logout(),Je.disconnect(),N&&(clearInterval(N),N=null),e.value="login"}function y(){s.value=!s.value}function O(){n.value=!n.value}is(n,async R=>{var ee,ne;if(R)o=document.activeElement,await Et(),(ne=(ee=a.value)==null?void 0:ee.querySelector(".nav-item"))==null||ne.focus();else if(o!=null&&o.isConnected){const ge=o;o=null,requestAnimationFrame(()=>ge.focus())}});const $=W(()=>{switch(d.value){case"connected":return"Live";case"connecting":return"Connecting…";case"reconnecting":return"Reconnecting…";default:return"Disconnected"}});function F(R,ee="info",ne=3e3){p.value={text:R,level:ee},clearTimeout(f),f=setTimeout(()=>{p.value=null},ne)}let N=null,G=!1,z=[];function P(){for(const R of z)R();z=[Je.onStatus(R=>{c.value=R}),Je.onLatencyChange(R=>{u.value=R}),Je.onState((R,ee)=>{d.value=R,R==="connected"?(G&&F("Connection restored","success"),G=!0):R==="reconnecting"&&ee.attempt===1&&F("Connection lost — reconnecting…","warn")})],Je.connect(),D(),N&&clearInterval(N),N=setInterval(D,15e3)}async function D(){try{const R=await K.get("/api/status");m.value=R.status==="online"?"online":"starting";const ee=R.uptime_seconds||0,ne=Math.floor(ee/3600),ge=Math.floor(ee%3600/60);b.value=`${ne}h ${ge}m uptime`}catch{m.value="offline",b.value=""}}return _t(()=>{N&&clearInterval(N);for(const R of z)R();z=[],Je.disconnect(),document.removeEventListener("keydown",C),r==null||r.removeEventListener("change",v)}),{authState:e,sessionExpired:t,sidebarCollapsed:s,mobileOpen:n,wsConnected:c,wsState:d,wsLatency:u,wsLabel:$,wsToast:p,botStatus:m,botUptime:b,navRoutes:E,navGroups:I,currentPage:w,currentSection:g,currentDescription:_,sidebarEl:a,mobileMenuButton:i,isMobileViewport:l,onLogin:S,logout:T,toggleSidebar:y,toggleMobileNavigation:O,openPalette:qu}}},Vn=Xl(n1);Vn.component("odin-icon",Nk);Vn.component("login-screen",s1);Vn.component("toast-container",K0);Vn.component("confirm-host",W0);Vn.component("command-palette",Lk);Vn.directive("modal-focus",Dk);Vn.use(Di);Vn.mount("#app");
