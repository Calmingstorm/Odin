...
`,e=e.substring(f.length+1),o=!0),!o){const R=this.rules.other.nextBulletRegex(g),v=this.rules.other.hrRegex(g),m=this.rules.other.fencesBeginRegex(g),x=this.rules.other.headingBeginRegex(g),k=this.rules.other.htmlBeginRegex(g);for(;e;){const _=e.split(`
`,1)[0];let A;if(f=_,this.options.pedantic?(f=f.replace(this.rules.other.listReplaceNesting,"  "),A=f):A=f.replace(this.rules.other.tabCharGlobal,"    "),m.test(f)||x.test(f)||k.test(f)||R.test(f)||v.test(f))break;if(A.search(this.rules.other.nonSpaceChar)>=g||!f.trim())d+=`
...
Please report this to https://github.com/markedjs/marked.`,e){const n="<p>An error occurred:</p><pre>"+Vs(s.message+"",!0)+"</pre>";return t?Promise.resolve(n):n}if(t)return Promise.reject(s);throw s}}},sa=new ow;function tt(e,t){return sa.parse(e,t)}tt.options=tt.setOptions=function(e){return sa.setOptions(e),tt.defaults=sa.defaults,pg(tt.defaults),tt};tt.getDefaults=Pc;tt.defaults=oa;tt.use=function(...e){return sa.use(...e),tt.defaults=sa.defaults,pg(tt.defaults),tt};tt.walkTokens=function(e,t){return sa.walkTokens(e,t)};tt.parseInline=sa.parseInline;tt.Parser=un;tt.parser=un.parse;tt.Renderer=Wl;tt.TextRenderer=zc;tt.Lexer=dn;tt.lexer=dn.lex;tt.Tokenizer=Kl;tt.Hooks=bl;tt.parse=tt;tt.options;tt.setOptions;tt.use;tt.walkTokens;tt.parseInline;un.parse;dn.lex;const cw={breaks:!0,gfm:!0};function Ku(e){if(!e)return"";try{if(typeof tt<"u"&&tt.parse){const t=tt.parse(e,cw);return typeof Hu<"u"?Hu.sanitize(t):t}}catch{}return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\n/g,"<br>")}function dw(e){const t=new Date(e),s=t.getHours().toString().padStart(2,"0"),n=t.getMinutes().toString().padStart(2,"0");return`${s}:${n}`}const uw={run_command:"terminal",ssh_command:"terminal",run_script:"terminal",read_file:"file",write_file:"edit",list_directory:"folder",search_knowledge:"search",ingest_document:"book",generate_image:"image",analyze_image:"eye",analyze_pdf:"file",browser_screenshot:"globe",manage_process:"sliders"};function fw(e){return uw[e]||"wrench"}const pw=/https?:\/\/\S+\.(?:png|jpg|jpeg|gif|webp|svg)(?:\?\S*)?/gi;function Wu(e){if(!e)return[];const t=e.match(pw);return t?[...new Set(t)]:[]}const hw={template:`
  ...
  `},gw={setup(){const e=h([]),t=h([]),s=h({}),n=50;function a(f){var y,S,R,v,m;const p=f.payload||f,g=p.type||f.type;if(g==="tool_start"){const x=((y=p.metadata)==null?void 0:y.call_id)||null,k={callId:x,id:x||`${p.action}-${Date.now()}`,tool:p.action,actor:p.actor||"",channel:p.channel_id||"",iteration:((S=p.metadata)==null?void 0:S.iteration)??0,startTime:Date.now(),elapsed:0,status:"running",output:"",result:""};e.value.unshift(k);return}if(g==="tool_end"){const x=((R=p.metadata)==null?void 0:R.call_id)||null;let k=-1;if(x&&(k=e.value.findIndex(_=>_.callId===x&&_.status==="running")),k<0&&!x)for(let _=e.value.length-1;_>=0;_--){const A=e.value[_];if(A.tool===p.action&&A.status==="running"){k=_;break}}if(k>=0){const _=e.value[k];_.status=(v=p.metadata)!=null&&v.error?"error":"success",_.elapsed=((m=p.metadata)==null?void 0:m.elapsed_ms)||Date.now()-_.startTime,_.result=p.detail||"",_.fadingOut=!0,setTimeout(()=>{const A=e.value.indexOf(_);A>=0&&e.value.splice(A,1),t.value.unshift(_),t.value.length>n&&t.value.pop()},5e3)}return}if(g==="tool_stream"){const x=p.call_id||p.tool_name||"unknown";if(p.finished){const k={...s.value};delete k[x],s.value=k}else{const _=((s.value[x]||"")+(p.chunk||"")).split(`
...
`)}const Dw={template:`
    ...
    </div>`,setup(){const e=h([]),t=h(!0),s=h(null),n=h({}),a=h({}),i=h(null),l=h(""),r=h(null),o=h(!1),c=h("create"),d=h(""),u=h(""),f=h(null),p=h(null),g=h(!1),y=h(null),S=h(null),R=h(!1),v=ee(()=>e.value.length),m=ee(()=>e.value.reduce((Q,he)=>Q+(he.execution_count||0),0)),x=ee(()=>e.value.reduce((Q,he)=>Q+L(he.code),0)),k=ee(()=>{if(!l.value)return e.value;const Q=l.value.toLowerCase();return e.value.filter(he=>he.name.toLowerCase().includes(Q)||(he.description||"").toLowerCase().includes(Q))}),_=ee(()=>u.value?u.value.split(`
`).length:0),A=ee(()=>{const Q=Math.max(_.value,1);return Array.from({length:Q},(he,Fe)=>Fe+1).join(`
`)}),T=ee(()=>{const Q=u.value.trim();return Q?Q.includes("SKILL_DEFINITION")?Q.includes("async def execute")?{valid:!0,message:""}:{valid:!1,message:"Missing async def execute function"}:{valid:!1,message:"Missing SKILL_DEFINITION dict"}:null});function C(Q){return Lw(Q)}function L(Q){return Q?Q.split(`
`).length:0}function H(Q){return Nw(Q)}function M(Q){n.value={...n.value,[Q]:!n.value[Q]}}async function D(Q){try{await navigator.clipboard.writeText(Q);const he=e.value.find(Fe=>Fe.code===Q);he&&(r.value=he.name,setTimeout(()=>{r.value=null},2e3))}catch{}}function K(Q){if(Q.key==="Tab"){Q.preventDefault();const he=Q.target,Fe=he.selectionStart,z=he.selectionEnd;u.value=u.value.substring(0,Fe)+"    "+u.value.substring(z),Ot(()=>{he.selectionStart=he.selectionEnd=Fe+4})}}function ne(Q){const he=Q.target.previousElementSibling;he&&(he.scrollTop=Q.target.scrollTop)}async function $(){t.value=!0,s.value=null;try{e.value=await Y.get("/api/skills")}catch(Q){s.value=Q.message}t.value=!1}async function O(Q){i.value=Q,delete a.value[Q],a.value={...a.value};try{const he=await Y.post(`/api/skills/${encodeURIComponent(Q)}/test`);a.value={...a.value,[Q]:he}}catch(he){a.value={...a.value,[Q]:{result:he.message,is_error:!0}}}i.value=null}function E(){o.value=!0,c.value="create",d.value="",u.value="",f.value=null,p.value=null}function N(Q){o.value=!0,c.value="edit",d.value=Q.name,u.value=Q.code||"",f.value=null,p.value=null}function B(){o.value=!1,f.value=null,p.value=null}async function W(){f.value=null,p.value=null;const Q=d.value.trim(),he=u.value.trim();if(!Q){f.value="Name is required";return}if(!he){f.value="Code is required";return}g.value=!0;try{c.value==="create"?(await Y.post("/api/skills",{name:Q,code:he}),p.value="Skill created successfully"):(await Y.put(`/api/skills/${encodeURIComponent(Q)}`,{code:he}),p.value="Skill updated successfully"),await $(),setTimeout(()=>{o.value=!1},800)}catch(Fe){f.value=Fe.message}g.value=!1}function te(Q){S.value=Q}async function oe(){if(S.value){R.value=!0;try{await Y.del(`/api/skills/${encodeURIComponent(S.value)}`),await $()}catch(Q){Se.error(`Failed to delete skill: ${Q.message||"unknown error"}`)}R.value=!1,S.value=null}}return Je(()=>{$()}),{skills:e,loading:t,error:s,showCode:n,testResults:a,testing:i,search:l,copied:r,editing:o,editMode:c,editName:d,editCode:u,editError:f,editSuccess:p,saving:g,editorRef:y,deleteTarget:S,deleting:R,enabledCount:v,totalExecutions:m,totalLines:x,displayedSkills:k,editLineCount:_,editorLineNums:A,editValidation:T,highlight:C,truncate:Mc,formatTs:ra,countLines:L,getLineNumbers:H,toggleCode:M,copyCode:D,handleEditorKey:K,syncScroll:ne,fetchSkills:$,testSkill:O,showCreate:E,editSkill:N,cancelEdit:B,saveSkill:W,confirmDelete:te,doDelete:oe}}};function Mw(e,t){if(!e||!t)return Eu(e);const s=Eu(e),n=t.trim().split(/\s+/).filter(Boolean);if(!n.length)return s;const a=n.map(i=>i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")).join("|");try{return s.replace(new RegExp(`(${a})`,"gi"),'<mark class="knowledge-highlight">$1</mark>')}catch{return s}}const Pw={template:`
    <div class="p-6 page-fade-in">
      <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
  ...
  `},Hw={ok:"text-green-400",degraded:"text-yellow-400",down:"text-red-400",unconfigured:"text-gray-500"},Vw={ok:"success",degraded:"warning",down:"error",unconfigured:"minus"},jw={healthy:"text-green-400",degraded:"text-yellow-400",unhealthy:"text-red-400"},zw={template:`
    ...
    </div>`,setup(){const e=h({}),t=h(!0),s=h(null),n=h(!1),a=h(!1),i=ee(()=>e.value.components||[]),l=ee(()=>jw[e.value.overall]||"text-gray-400"),r=ee(()=>e.value.overall==="healthy"?"success":e.value.overall==="degraded"?"warning":e.value.overall==="unhealthy"?"error":"minus"),o=ee(()=>{const _=e.value.overall;return _==="healthy"?"All Systems Healthy":_==="degraded"?"Some Systems Degraded":_==="unhealthy"?"System Issues Detected":"Unknown"});function c(_){return Hw[_]||"text-gray-400"}function d(_){return Vw[_]||"info"}function u(_){return _==="ok"?"badge-success":_==="degraded"?"badge-warning":_==="down"?"badge-danger":"badge-info"}function f(_){return _==="closed"?"text-green-400":_==="half_open"?"text-yellow-400":_==="open"?"text-red-400":"text-gray-400"}function p(_){return _.replace(/_/g," ").replace(/\b\w/g,A=>A.toUpperCase())}function g(_){if(!_)return"—";try{return new Date(_).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}catch{return _}}function y(_){return _>=1e6?(_/1e6).toFixed(1)+"M":_>=1e3?(_/1e3).toFixed(1)+"K":String(_)}async function S(){a.value=!0;try{e.value=await Y.get("/api/health/components"),s.value=null,n.value=!0}catch(_){s.value=_.message}finally{t.value=!1,a.value=!1}}function R(){t.value=!0,s.value=null,S()}let v=null,m=!1;function x(){m||(m=!0,S(),v||(v=setInterval(S,3e4)))}function k(){m&&(m=!1,v&&(clearInterval(v),v=null))}return Je(x),As(x),Rs(k),_t(k),{data:e,hasData:n,loading:t,error:s,refreshing:a,components:i,overallColor:l,overallIcon:r,overallLabel:o,statusColor:c,statusIcon:d,badgeClass:u,circuitColor:f,formatName:p,formatTime:g,formatNumber:y,fetchHealth:S,retry:R}}},qw={template:`
