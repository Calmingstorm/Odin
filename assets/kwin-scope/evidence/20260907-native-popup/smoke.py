...
from wayland_probe_sender import Sender, bind_library, ABSOLUTE, BUTTON, KEYBOARD
...
def call(dest, path, interface, method, args=None):
    value = bus.call_sync(dest, path, interface, method, args, None, Gio.DBusCallFlags.NONE, 2500, None).unpack()
    return value[0] if len(value) == 1 else value
def dbus(method, name):
    return call('org.freedesktop.DBus', '/org/freedesktop/DBus', 'org.freedesktop.DBus', method, GLib.Variant('(s)', (name,)))
try:
    bus = Gio.bus_get_sync(Gio.BusType.SESSION, None)
    result['stage'] = 'start_kwin'
    log = open(out / 'kwin.log', 'w')
    kwin = subprocess.Popen(['/usr/bin/kwin_wayland', '--virtual', '--socket', 'wayland-smoke', '--width', '800', '--height', '600'], stdout=log, stderr=subprocess.STDOUT)
    ...
    (out / 'plugins-introspection.xml').write_text(call(owner, '/Plugins', 'org.freedesktop.DBus.Introspectable', 'Introspect'))
    result['load_return'] = call(owner, '/Plugins', 'org.kde.KWin.Plugins', 'LoadPlugin', GLib.Variant('(s)', ('odin-scope',)))
    ...
    identity = json.loads(call(owner, '/org/kde/KWin/OdinScope', 'org.kde.KWin.OdinScope', 'Identity', GLib.Variant('(s)', (challenge,))))
    ...
    receiver = subprocess.Popen(['/usr/bin/python3', '/evidence/receiver.py'], env=env, stdout=receiver_log, stderr=subprocess.STDOUT)
    children.append(receiver)
    result['receiver_pid'] = receiver.pid
    request = {'protocol': 1, 'challenge': challenge, 'source_digest': 'a' * 64, 'source': {'node_id': 1, 'source_type': 1, 'session_handle': '/readonly/smoke', 'position': [0, 0], 'size': [800, 600]}}
    result['source_note'] = 'Synthetic protocol source binding for scope-only smoke, not a portal stream or input admission.'
    result['stage'] = 'snapshot'
    for attempt in range(30):
        time.sleep(.2)
        try:
            snapshot = json.loads(call(owner, '/org/kde/KWin/OdinScope', 'org.kde.KWin.OdinScope', 'Snapshot', GLib.Variant('(s)', (json.dumps(request),))))
            break
    ...
    assert dbus('GetConnectionUnixProcessID', owner) == kwin.pid
    result['loaded_plugin_mapping'] = [line for line in Path(f'/proc/{kwin.pid}/maps').read_text().splitlines() if 'odin-scope.so' in line]
    assert result['loaded_plugin_mapping']
    result['stage'] = 'scratch_menu_scope'
    result['input_attempted'] = True
    result['input_note'] = 'Explicit scratch EI helper only in isolated lab; NOT production admission or EOF qualification.'
    returned, fds = bus.call_with_unix_fd_list_sync(owner, '/org/kde/KWin/EIS/RemoteDesktop',
        'org.kde.KWin.EIS.RemoteDesktop', 'connectToEIS', GLib.Variant('(i)', (3,)), None,
        Gio.DBusCallFlags.NONE, 2500, None, None)
    ...
    try:
        ...
        button(273, False)
        pause()
        menu_snapshot = json.loads(call(owner, '/org/kde/KWin/OdinScope', 'org.kde.KWin.OdinScope', 'Snapshot', GLib.Variant('(s)', (json.dumps(request),))))
        ...
        button(272, False)
        pause()
        result['menu_events'] = [json.loads(line) for line in (out / 'menu-events.jsonl').read_text().splitlines()]
        assert any(e['event'] == 'item_selected' for e in result['menu_events'])
        restored = json.loads(call(owner, '/org/kde/KWin/OdinScope', 'org.kde.KWin.OdinScope', 'Snapshot', GLib.Variant('(s)', (json.dumps(request),))))
        ...
        pause()
        sender.close()
        call(owner, '/org/kde/KWin/EIS/RemoteDesktop', 'org.kde.KWin.EIS.RemoteDesktop', 'disconnect', GLib.Variant('(i)', (cookie,)))
    result['unload_return'] = call(owner, '/Plugins', 'org.kde.KWin.Plugins', 'UnloadPlugin', GLib.Variant('(s)', ('odin-scope',)))
